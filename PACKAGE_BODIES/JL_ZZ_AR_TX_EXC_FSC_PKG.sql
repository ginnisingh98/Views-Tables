--------------------------------------------------------
--  DDL for Package Body JL_ZZ_AR_TX_EXC_FSC_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JL_ZZ_AR_TX_EXC_FSC_PKG" as
/* $Header: jlzztefb.pls 120.3 2003/09/02 21:59:04 opedrega ship $ */
  ...
  PROCEDURE Insert_Row
    ...
    CURSOR C IS
      SELECT rowid
      FROM jl_zz_ar_tx_exc_fsc
      WHERE ship_from_code = X_ship_from_code
      AND ship_to_segment_id = X_ship_to_segment_id
      AND fiscal_classification_code = X_fiscal_classification_code
    ...
    BEGIN
      ...
      --
      current_calling_sequence := 'JL_ZZ_AR_TX_EXC_FSC_PKG.INSERT_ROW<-' ||
                                   X_calling_sequence;

      debug_info := 'Insert into JL_ZZ_AR_TX_EXC_FSC  ';
      INSERT INTO jl_zz_ar_tx_exc_fsc(exc_fsc_id,
                                      ship_from_code,
                                      ship_to_segment_id,
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_EXC_FSC_PKG.LOCK_ROW<-' ||
                                 X_calling_sequence;
    ...
    IF ((Recinfo.exc_fsc_id = X_exc_fsc_id) AND
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_EXC_FSC_PKG.CHECK_UNIQUE<-' ||
                                 X_calling_sequence;
    SELECT COUNT(1)
    INTO l_dummy
    FROM jl_zz_ar_tx_exc_fsc
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_EXC_FSC_PKG.<-CHECK_OVERLAPPED_DATES' ||
                                 X_calling_sequence;
    SELECT COUNT(1)
    INTO l_dummy
    FROM jl_zz_ar_tx_exc_fsc a
    WHERE a.ship_from_code = X_ship_from_code
    AND a.ship_to_segment_id = X_ship_to_segment_id
    AND a.fiscal_classification_code = X_fiscal_classification_code
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_EXC_FSC_PKG.CHECK_GAPS<-' ||
                                 X_calling_sequence;
    --
