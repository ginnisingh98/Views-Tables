--------------------------------------------------------
--  DDL for Package Body IGS_PS_TCH_RESP_HIST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_TCH_RESP_HIST_PKG" as
/* $Header: IGSPI70B.pls 115.6 2002/12/23 04:52:40 smvk ship $ */
   l_rowid VARCHAR2(25);
  old_references IGS_PS_TCH_RESP_HIST_ALL%RowType;
  new_references IGS_PS_TCH_RESP_HIST_ALL%RowType;
...
END check_constraints;
 ...
 PROCEDURE Check_Parent_Existance AS
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_TRH_OU_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_TRH_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID in NUMBER
    ...
    cursor C is select ROWID from IGS_PS_TCH_RESP_HIST_ALL
      where UNIT_CD = X_UNIT_CD
      and VERSION_NUMBER = X_VERSION_NUMBER
      and OU_START_DT = X_OU_START_DT
      and HIST_START_DT = X_HIST_START_DT
...
begin
  ...
  insert into IGS_PS_TCH_RESP_HIST_ALL (
    UNIT_CD,
    VERSION_NUMBER,
    ORG_UNIT_CD,
    OU_START_DT,
...
begin
  ...
  update IGS_PS_TCH_RESP_HIST_ALL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_TCH_RESP_HIST_ALL
     where UNIT_CD = X_UNIT_CD
     and VERSION_NUMBER = X_VERSION_NUMBER
     and OU_START_DT = X_OU_START_DT
     and HIST_START_DT = X_HIST_START_DT
...
begin
  ...
  delete from IGS_PS_TCH_RESP_HIST_ALL
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
