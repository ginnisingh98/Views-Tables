--------------------------------------------------------
--  DDL for Package Body HR_ICX_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_ICX_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hr_icx_ins.';  -- Global package name
-- ----------------------------------------------------------------------------
-- |-------------------------------< ins_or_sel >-----------------------------|
-- ----------------------------------------------------------------------------
...
--
  l_item_context_id      hr_item_contexts.item_context_id%type;
  l_concatenated_segments  varchar2(2000);
  l_id_flex_num            hr_item_contexts.id_flex_num%type;
...
begin
  ...
  End if;
  --
  IF l_segs_changed and l_id_flex_num is null THEN
...
-- error message
  ...
  End if;
  --
  IF l_id_flex_num is null and l_segs_changed = false  THEN
    null;
...
Begin
  ...
  --
  --
  -- Insert the row into: hr_item_contexts
  --
...
Begin
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.item_context_id;
  Close C_Sel1;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
