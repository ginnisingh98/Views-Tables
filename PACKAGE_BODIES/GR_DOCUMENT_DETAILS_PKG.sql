--------------------------------------------------------
--  DDL for Package Body GR_DOCUMENT_DETAILS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_DOCUMENT_DETAILS_PKG" AS
/*$Header: GRHIDDB.pls 115.6 2002/10/28 19:40:44 mgrosser ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_document_details
   		  	     (text_line_number,
				  document_text_id,
				  print_font,
				  print_size,
...
EXCEPTION
   ...
   WHEN Label_Exists_Error THEN
      ...
      FND_MESSAGE.SET_TOKEN('CODE',
         		            p_document_text_id || ' ' || p_text_line_number,
            			    FALSE);
      IF FND_API.To_Boolean(p_called_by_form) THEN
         APP_EXCEPTION.Raise_Exception;
	  ELSE
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_document_details
	  SET	 text_line_number				 = p_text_line_number,
			 document_text_id	  			 = p_document_text_id,
			 print_font	  					 = p_print_font,
			 print_size	  					 = p_print_size,
			 text_line	  					 = p_text_line,
...
CURSOR c_lock_document_details
 IS
   SELECT	*
   FROM		gr_document_details
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockDetailRcd	  c_lock_document_details%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_document_details;
   FETCH c_lock_document_details INTO LockDetailRcd;
   IF c_lock_document_details%NOTFOUND THEN
	  CLOSE c_lock_document_details;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_document_details
   WHERE  	   rowid = p_rowid;
...
/*	Check the document code */
   ...
   GR_DOCUMENT_PRINT_PKG.Check_Primary_Key
   					(p_document_text_id,
					 'F',
					 l_rowid,
					 l_key_exists);
...
**		  p_text_line_number is the line number
...
CURSOR c_get_print_rowid
 IS
   ...
   WHERE  dd.document_text_id = p_document_text_id
   AND	  dd.text_line_number = p_text_line_number;
PrintRecord			   c_get_print_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   l_msg_data := p_document_text_id || ' ' || p_text_line_number;
