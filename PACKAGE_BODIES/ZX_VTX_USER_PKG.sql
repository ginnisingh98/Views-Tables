--------------------------------------------------------
--  DDL for Package Body ZX_VTX_USER_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ZX_VTX_USER_PKG" AS
/* $Header: zxvtxuserpkgb.pls 120.42.12010000.9 2010/01/29 20:58:34 ssanka ship $ */

/* ======================================================================*
 | Global Data Types                                                     |
 * ======================================================================*/

G_PKG_NAME                  CONSTANT VARCHAR2(30) := 'ZX_VTX_USER_PKG';
...
G_MODULE_NAME               CONSTANT VARCHAR2(30) := 'ZX.PLSQL.ZX_VTX_USER_PKG.';

   g_string                 VARCHAR2(80);
   l_view_name              VARCHAR2(200);
   i                        PLS_INTEGER;
   l_in_out_flag            VARCHAR2(1);     -- Bug 5506031
   ...
   TYPE salesrep_tbl_type is table of NUMBER index by VARCHAR2(100);
   p_salesrep_tbl  salesrep_tbl_type;
...
/* Bug 4668932 */
   PROCEDURE derive_trx_line_type;
   ...
   PROCEDURE derive_customer_class;
   ...
   BEGIN
    ...
    l_header_ext_attr1  := fnd_profile.value_specific('ZX_TAXVDR_CASESENSITIVE');
    ...
    IF ( G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
       ...
       FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,
            ' l_header_ext_attr1 = ' || l_header_ext_attr1);
    END IF;
	...
	ELSE
	   ...
	   END IF;
        ...
        LOOP
           ...
           IF ( G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
               FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,
                ' trx_line_id(i) = ' || trx_line_id_tab(i));
           END IF;
	   ...
	   */
	   ...
	   END IF;
	   IF x_error_status <> FND_API.G_RET_STS_SUCCESS THEN
              IF (g_level_exception >= g_current_runtime_level ) THEN
                   FND_LOG.STRING(G_LEVEL_EXCEPTION,G_MODULE_NAME||l_api_name,SQLERRM);
              END IF;
	      x_error_status := FND_API.G_RET_STS_UNEXP_ERROR;
...
BEGIN
   ...
   application_id , event_class_code and api name and adjusted doc trx id*/
    ...
    IF p_application_id = 222 THEN     -- Receivables
       ...
       ELSIF p_api_name = 'CALCULATE_TAX' THEN  -- Manual invoice
          IF p_event_class_code in ('INVOICE') THEN
             IF p_line_level_action = 'COPY_AND_CREATE' THEN   -- Recurring Invoice
                x_view_name := 'TAX_LINES_RECURR_INVOICE_V_V';
             ELSE
          ...
          ELSIF p_event_class_code in ('CREDIT_MEMO') THEN
             IF p_adjusted_doc_trx_id IS NOT NULL THEN   -- Applied Credit Memo
                x_view_name := 'tax_lines_cm_v_V CREDITMEMO';
             ELSE                                        -- On Account CM
                x_view_name := 'TAX_LINES_CREATE_V_V INVOICE';
             END IF;
          ELSIF p_event_class_code in ('INVOICE_ADJUSTMENT') THEN
                x_view_name := 'TAX_ADJUSTMENTS_V_V';
          END IF;
       ELSIF p_api_name in ('UPDATE_DET_FACTORS_HDR', 'UPDATE_LINE_DET_FACTORS') THEN  -- Line negation
          ...
          ELSIF p_event_class_code in ('CREDIT_MEMO') THEN
             ...
             END IF;
          END IF;
       ELSIF p_api_name in ('OVERRIDE_TAX') THEN
    ...
    ELSIF p_application_id in (660, 300) THEN
          x_view_name := 'OE_TAX_LINES_SUMMARY_V_V';
    ELSE
          x_view_name := 'ASO_TAX_LINES_SUMMARY_V_V'; ---Default View Name Assignment
    END IF;
  ...
  /*===========================================================================+
   ...
   BEGIN
	...
	IF l_tax_account_source_tax IS NOT NULL THEN
		...
		ELSIF l_tax_account_source_tax = 'DISTRICT' THEN
			l_exempt_percent     := arp_district_exempt_pct_tab(p_position);
			l_exempt_reason      := arp_district_exempt_rs_tab(p_position);
		END IF;
		...
		ELSIF tax_identifier_tab(i) = 'DISTRICT' THEN
		      arp_district_exempt_pct_tab(p_position) := l_exempt_percent;
		      arp_district_exempt_rs_tab(p_position) := l_exempt_reason;
		END IF;
		...
		-- adding code to populate exemption details in partner calculated tax lines
		  ...
		  AND ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).tax_provider_id
		   ...
		   ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).di_exempt_reason_code :=
                             arp_district_exempt_rs_tab(p_position);
	...
	ELSE  /* There is No Source Tax */
	    ...
	    IF event_class_code_tab(p_position) = 'CREDIT_MEMO'
		   ...
		   to approprirately insert into the relevant nested tables */
		    ...
		    WHEN NO_DATA_FOUND THEN
		      ...
		      IF tax_identifier_tab(i) = 'DISTRICT' THEN
		        BEGIN
		          SELECT NVL(TAX_EXMPT_SOURCE_TAX, TAX_ACCOUNT_SOURCE_TAX) --Bug 8724051
		          INTO l_tax_account_source_tax
		          FROM ZX_SCO_TAXES_B_V
	    ...
	    /* End of changes */
               ...
               l_event_class_rec.internal_organization_id := internal_org_id_tab(p_position);
            IF ( G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
               FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name, 'int org id = ' || to_char(l_event_class_rec.internal_organization_id ));
            END IF;
               ...
               ZX_TCM_GET_EXEMPT_PKG.get_tax_exemptions(
			       ...
			       p_bill_to_cust_acct_id		=>    p_bill_third_pty_acct_id,
                               -- Fixed for 6798559
                               p_bill_to_party_site_ptp_id      =>    p_bill_to_site_tax_prof ,
                               -- Fixed for 6798559
            ...
            IF x_exempt_record.exemption_id is NOT NULL OR
	       (x_exempt_record.percent_exempt IS NOT NULL AND event_class_code_tab(p_position) = 'CREDIT_MEMO') THEN
		  ...
		  use this condition without in any way harming the regular processing.
			  ...
			  call the ARP_TAX_VIEW_VERTEX.GET_EXEMPTIONS in order to populate the exemption records table
               ...
               */
               IF NOT exemptions_info_tab.EXISTS(NVL(x_exempt_record.exemption_id, -99)) THEN
                 IF x_exempt_record.exemption_id IS NOT NULL THEN
                    ...
                    IF exemptions_info_tab(x_exempt_record.exemption_id).certificate_number IS NULL THEN
                      /* If the user extensible procedure returned a null then use the default exempt
                        certificate number extracted into the exemptions record*/
                      exemptions_info_tab(x_exempt_record.exemption_id).certificate_number := SUBSTRB(x_exempt_record.exempt_certificate_number,1,15);
                    END IF;
                    ...
                    IF exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_pct IS NULL THEN
                      exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_pct := x_exempt_record.percent_exempt;
                    END IF;

                    IF exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_reason IS NULL THEN
                      exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_reason := SUBSTRB(x_exempt_record.exempt_reason_code,1,1);
                    END IF;
                 END IF;
               END IF;
               IF x_exempt_record.exemption_id IS NOT NULL THEN
                 ...
                 ELSIF tax_identifier_tab(i) = 'DISTRICT' THEN
                   arp_district_exempt_pct_tab(p_position) := exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_pct;
                   arp_district_exempt_rs_tab(p_position) := exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_reason;
                 END IF;
                 cert_num_tab(p_position) := nvl(p_cert_num,exemptions_info_tab(x_exempt_record.exemption_id).certificate_number);
	         ...
	         IF tax_identifier_tab(i) = 'STATE' THEN
                 ...
                 ELSIF tax_identifier_tab(i) = 'DISTRICT' THEN
                   arp_district_exempt_pct_tab(p_position) := x_exempt_record.percent_exempt;
                   arp_district_exempt_rs_tab(p_position) := SUBSTRB(x_exempt_record.exempt_reason_code,1,1);
                 END IF;
		 ...
		 ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).exempt_certificate_number := cert_num_tab(p_position);
	         ...
	         IF x_exempt_record.exemption_id IS NOT NULL THEN
	           ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).st_exempt_reason_code :=
                   ...
                   ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).di_exempt_reason_code :=
                               NVL(exemptions_info_tab(x_exempt_record.exemption_id).district_exempt_reason,x_exempt_record.exempt_reason_code);
		 ...
		 ELSIF (x_exempt_record.percent_exempt IS NOT NULL AND event_class_code_tab(p_position) = 'CREDIT_MEMO') THEN
		   ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).st_exempt_reason_code := x_exempt_record.exempt_reason_code;
                   ...
                   ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).di_exempt_reason_code := x_exempt_record.exempt_reason_code;
                   ...
                   ZX_GLOBAL_STRUCTURES_PKG.ptnr_exemption_tbl(l_ptnr_exemption_indx).di_exempt_reason := x_exempt_record.exempt_reason_code;
		 END IF;
               END IF;
...
END POPULATE_EXEMPTION_DETAILS;
   ...
   EXCEPTION WHEN COLLECTION_IS_NULL THEN
    ...
    |
    | DESCRIPTION
    |         This procedure is used to initialize the exemption tables
    |         in order to avoid the no data found exception during runtime
    |
    |
    | SCOPE - PRIVATE
    |
    ...
    BEGIN
	   ...
	   /*Initializing all nested tables pertaining to exemptions
	   so that we do not hit a no data found issue at runtime
	   incase some of these tables do not get populated at all
	   positions*/

	   FOR i in 1..NVL(trx_id_tab.last,0)
	   LOOP

		arp_state_exempt_percent_tab(i) := NULL;
		arp_county_exempt_pct_tab(i)    := NULL;
		arp_city_exempt_pct_tab(i)      := NULL;
		arp_district_exempt_pct_tab(i)  := NULL;
		arp_state_exempt_reason_tab(i)  := NULL;
		arp_county_exempt_reason_tab(i) := NULL;
		arp_city_exempt_reason_tab(i)   := NULL;
		arp_district_exempt_rs_tab(i)   := NULL;
...
/*===========================================================================+
 ...
 |                                                                           |
 | MODIFICATION HISTORY                                                      |
 |     01-NOV-2005  Santosh Vaze      Created for bug 4668932                |
 |                                                                           |
 +===========================================================================*/
...
IS
   ...
   l_api_name                CONSTANT VARCHAR2(30) := 'DERIVE_TRX_LINE_TYPE';
...
BEGIN
   ...
   arp_trx_line_type_tab(i):= ARP_TAX_VIEW_VERTEX.TRX_LINE_TYPE
                                 (l_view_name,
                                  trx_id_tab(i),
                                  trx_line_id_tab(i))  ;
   IF (arp_trx_line_type_tab(i) is NULL) THEN
      arp_trx_line_type_tab(i) := 'SALE';
   END IF;
...
BEGIN
   ...
   IF l_view_name = 'TAX_ADJUSTMENTS_V_V' THEN
      BEGIN
         arp_audit_flag_tab(i)  := ARP_TAX_VIEW_VERTEX.AUDIT_FLAG
                                      (l_view_name,
                                       trx_id_tab(i),
                                       trx_line_id_tab(i));
      EXCEPTION WHEN OTHERS THEN
...
IS
   ...
   l_org_id                  RA_CUSTOMER_TRX_LINES_ALL.ORG_ID%TYPE;
...
BEGIN
   ...
   IF l_view_name <> 'TAX_ADJUSTMENTS_V_V' THEN
      ...
      BEGIN
         arp_product_code_tab(i) := ARP_TAX_VIEW_VERTEX.PRODUCT_CODE(l_view_name
                                       , trx_id_tab(i)
                                       , trx_line_id_tab(i)
                                       , l_product_id
                                       , l_memo_line_id);
      ...
      IF arp_product_code_tab(i) IS NULL THEN
        ...
        EXCEPTION
         WHEN OTHERS THEN
            l_org_id := internal_org_id_tab(i);
        END;

/* Bug 5612024
      IF MO_GLOBAL.get_current_org_id <> nvl(l_org_id, -1) THEN
	 MO_GLOBAL.Set_Policy_Context('S', l_org_id);
      END IF;
*/
      l_master_org_id := oe_sys_parameters.value('MASTER_ORGANIZATION_ID', l_org_id);
...
BEGIN
   ...
   BEGIN --bug6523242
     IF ship_to_loc_id_tab(i) IS NULL THEN
	   arp_ship_to_add_tab(i) :=  ARP_TAX_VIEW_VERTEX.SHIP_TO_ADDRESS_CODE
						(l_view_name,
						trx_id_tab(i),
						trx_line_id_tab(i),
						ship_to_address_id_tab(i),
						bill_to_location_id_tab(i),
						g_trx_date,
...
/* Bug 4668932 */
   l_in_out_flag := '1';     -- Bug 5506031
   IF arp_ship_to_add_tab(i) IS NULL THEN
      BEGIN
	 ...
	 arp_ship_to_add_tab(i) := NULL;
      ...
      IF NOT check_geocode(substr(nvl(arp_ship_to_add_tab(i),'XXXXXXXXX'), 2,9)) THEN
        IF ship_to_loc_id_tab(i) IS NULL THEN
           ZX_TCM_GEO_JUR_PKG.get_tax_jurisdictions(
                             p_location_id          => bill_to_location_id_tab(i),
        ...
        ELSE
         ZX_TCM_GEO_JUR_PKG.get_tax_jurisdictions(
                             p_location_id          => ship_to_loc_id_tab(i),
                             p_location_type        => 'SHIP_TO',
                             p_tax                  => 'CITY',
		             p_tax_regime_code      => tax_regime_code_tab(i),
                             p_trx_date             => g_trx_date,
         ...
         IF x_ret_status = FND_API.G_RET_STS_SUCCESS THEN
            IF l_jurisdictions_found = 'Y' THEN
               IF l_tax_jurisdiction_rec.tax_jurisdiction_code IS NOT NULL THEN
                  l_jur_code := substr(l_tax_jurisdiction_rec.tax_jurisdiction_code, 4);
               ELSE
                  BEGIN
                    SELECT count(*)
                      INTO l_jur_count
                  ...
                  END;
               END IF;
             IF check_geocode(nvl(l_jur_code,'XXXXXXXXX')) THEN
                arp_ship_to_add_tab(i) := l_in_out_flag ||l_jur_code;
             ELSE
              IF l_jur_count > 1 THEN
                arp_ship_to_add_tab(i) := NVL(l_in_out_flag,1)||nvl(l_jur_code,'XXXXXXXXX');
              ELSE
                arp_ship_to_add_tab(i) := NULL;
              END IF;
             END IF;
...
BEGIN
   ...
   arp_ship_from_add_tab(i) :=  ARP_TAX_VIEW_VERTEX.SHIP_FROM_ADDRESS_CODE
                                   (l_view_name,
                                    trx_id_tab(i),
                                    trx_line_id_tab(i),
   ...
   IF (arp_ship_from_add_tab(i) is NULL) THEN
      BEGIN
        l_sfr_in_out_flag := NULL;
        IF l_ship_from_party_id IS NOT NULL THEN
          BEGIN
        ...
        IF l_ship_from_party_id IS NULL AND l_sfr_geocode IS NULL THEN
          l_flag := TRUE;
          BEGIN
	 ...
	 EXCEPTION
	    ...
	    l_flag := FALSE;
	 END;
    	 IF l_flag AND l_inventory_item_id IS NULL THEN
          /* Bug 5612024
		         IF MO_GLOBAL.get_current_org_id <> nvl(l_org_id, -1) THEN
		            MO_GLOBAL.Set_Policy_Context('S', l_org_id);
		         END IF;
       ...
       IF l_sfr_geocode IS NULL THEN
	 arp_ship_from_add_tab(i):= arp_tax_view_vertex.USE_SHIP_TO;
       ELSE
         IF NOT Check_Geocode(l_sfr_geocode) THEN
           arp_ship_from_add_tab(i):=ARP_TAX_VIEW_VERTEX.USE_SHIP_TO;
         ELSE
            IF l_sfr_in_out_flag IS NULL THEN
              arp_ship_from_add_tab(i) := 'X'||l_sfr_geocode;
            ELSE
              IF l_sfr_in_out_flag like 'N%' THEN
                arp_ship_from_add_tab(i) := '0'||l_sfr_geocode;
              ELSE
                arp_ship_from_add_tab(i) :=
                l_sfr_in_out_flag||l_sfr_geocode;
              END IF;
            END IF;
         END IF;
       END IF;
...
IS
   ...
   l_sales_repid             RA_CUSTOMER_TRX_ALL.primary_salesrep_id%TYPE;
...
BEGIN
   ...
   IF event_class_code_tab(i) = 'INVOICE_ADJUSTMENT' THEN
      BEGIN
         IF adjusted_doc_trx_id_tab(i) IS NOT NULL THEN
            IF p_salesrep_tbl.EXISTS(adjusted_doc_trx_id_tab(i)) THEN
               l_sales_repid := p_salesrep_tbl(adjusted_doc_trx_id_tab(i));
            ELSE
               BEGIN
               ...
               EXCEPTION
                  WHEN NO_DATA_FOUND THEN
                     l_sales_repid := NULL;
               END;
               p_salesrep_tbl(adjusted_doc_trx_id_tab(i)) := l_sales_repid;
            END IF;
         END IF;
      EXCEPTION
            WHEN NO_DATA_FOUND THEN
   ...
   ELSE
      IF trx_id_tab(i) IS NOT NULL THEN
        IF trx_line_id_tab(i) IS NOT NULL
              AND l_view_name = 'OE_TAX_LINES_SUMMARY_V_V' THEN
          BEGIN
            SELECT salesrep_id
            INTO l_sales_repid
        ...
        ELSE
          ...
          END;
        END IF;
        p_salesrep_tbl(trx_id_tab(i)) := l_sales_repid;
      END IF;
   END IF;

   arp_poa_add_code_tab(i)  :=  ARP_TAX_VIEW_VERTEX.POA_ADDRESS_CODE
                                   (l_view_name,
                                    trx_id_tab(i),
                                    trx_line_id_tab(i),
                                    l_sales_repid);
   IF (arp_poa_add_code_tab(i) is NULL) THEN
      IF l_sales_repid IS NOT NULL THEN
         IF poa_add_code_cache_tbl.EXISTS(l_sales_repid) THEN
            arp_poa_add_code_tab(i) := poa_add_code_cache_tbl(l_sales_repid);
         ELSE
            BEGIN
            ...
            EXCEPTION
               WHEN OTHERS THEN
     	          arp_poa_add_code_tab(i) := ARP_TAX_VIEW_VERTEX.USE_SHIP_TO;
            END;

            IF l_poa_geocode IS NULL THEN
               arp_poa_add_code_tab(i) := ARP_TAX_VIEW_VERTEX.USE_SHIP_TO;
            ELSE
               IF NOT Check_Geocode(l_poa_geocode) THEN
                  arp_poa_add_code_tab(i) := ARP_TAX_VIEW_VERTEX.USE_SHIP_TO;
               ELSE
                  IF l_poa_in_out_flag IS NULL THEN
                     arp_poa_add_code_tab(i) := l_in_out_flag||l_poa_geocode;
                  ELSE
                     arp_poa_add_code_tab(i) := l_poa_in_out_flag||l_poa_geocode;
                  END IF;
               END IF;
            END IF;
            poa_add_code_cache_tbl(l_sales_repid) := arp_poa_add_code_tab(i);
         END IF;
      ELSE
     	 arp_poa_add_code_tab(i) := ARP_TAX_VIEW_VERTEX.USE_SHIP_TO;
      END IF;
...
BEGIN
   ...
   arp_customer_code_tab(i) := ARP_TAX_VIEW_VERTEX.CUSTOMER_CODE
                                  (l_view_name,
                                   trx_id_tab(i),
                                   trx_line_id_tab(i));
...
IS
   ...
   l_api_name                CONSTANT VARCHAR2(30) := 'DERIVE_CUSTOMER_CLASS';
...
BEGIN
   ...
   l_ptp_id := nvl(ship_to_tx_id_tab(i),bill_to_party_tax_id_tab(i));
   IF l_ptp_id IS NOT NULL THEN  --- Bug 6024643
   IF p_party_id_tbl.EXISTS(l_ptp_id) THEN
      l_party_id := p_party_id_tbl(l_ptp_id);
   ELSE
      SELECT party_id
        INTO l_party_id
        FROM zx_party_tax_profile
       WHERE party_tax_profile_id = l_ptp_id;
      p_party_id_tbl(l_ptp_id) := l_party_id;
   END IF;
   END IF; --- Bug 6024643
   BEGIN
      arp_customer_class_tab(i) :=  ARP_TAX_VIEW_VERTEX.CUSTOMER_CLASS
                                       (l_view_name,
                                        trx_id_tab(i),
                                        trx_line_id_tab(i),
                                        l_party_id);
...
BEGIN
   ...
   arp_division_code_tab(i)  := ARP_TAX_VIEW_VERTEX.DIVISION_CODE
                                   (l_view_name,
                                    trx_id_tab(i),
                                    trx_line_id_tab(i));
...
BEGIN
   ...
   arp_transaction_date_tab(i) :=  ARP_TAX_VIEW_VERTEX.TRANSACTION_DATE
                                                              (l_view_name,
                                                              trx_id_tab(i),
                                                              trx_line_id_tab(i));
...
BEGIN
   ...
   arp_company_code_tab(i)  := ARP_TAX_VIEW_VERTEX.COMPANY_CODE
                                   (l_view_name,
                                    trx_id_tab(i),
                                    trx_line_id_tab(i));
