--------------------------------------------------------
--  DDL for Package Body ARP_UPDATE_LINE_COVER
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ARP_UPDATE_LINE_COVER" AS
/* $Header: ARTCTLUB.pls 120.7 2005/10/07 18:34:58 ralat ship $ */
...
/*===========================================================================+
 ...
 |     29-NOV-95  Martin Johnson      Created                                |
 |     17-JAN-96  Martin Johnson      Changed parameters                     |
 |                                    p_accounting_rule_duration and         |
 |                                    p_rule_start_date to be IN OUT NOCOPY  |
 |     13-FEB-96  Martin Johnson      Added IN OUT NOCOPY parameters         |
 ...
 |					p_gross_unit_selling_price           |
 |					p_amount_includes_tax_flag           |
 |				      changed parameters to OUT:             |
 |					p_net_extended_amount                |
 |					p_net_unit_selling_price             |
 |                                                                           |
 |    Rel 11.5 Changes:                                                      |
 |     10-JAN-99   Saloni Shah        added warehouse_id                     |
 |     22-MAR-99   Debbie Jancis      added translated description for MLS   |
 |     18-APR-05   Debbie Jancis      ETax added additional ship to id's     |
 |                                    for ship to at the line level          |
 |								             |
 +===========================================================================*/
...
BEGIN
  ...
  IF PG_DEBUG in ('Y', 'C') THEN
     arp_util.debug('arp_update_line_cover.update_line_cover()+');
  END IF;
...
--  l_line_rec.extended_amount := p_extended_amount;
  ...
  --
  -- do not update, as the field is populated by insert procedure
  --
  ...
  l_line_rec.unit_selling_price := 	p_gross_unit_selling_price;
  l_line_rec.extended_amount :=    	p_gross_extended_amount;
  ...
  -- ETax - line level ship to information
  ...
  -- Rel. 11: Return new net amounts to form
  ...
  IF  arp_rounding.correct_dist_rounding_errors(
                                        ...
                                        'N') = 0 -- FALSE
  THEN
     arp_util.debug('EXCEPTION:  arp_update_line_cover.update_line_cover()');
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    IF PG_DEBUG in ('Y', 'C') THEN
       arp_util.debug('EXCEPTION:  arp_update_line_cover.update_line_cover()');
       arp_util.debug('------- parameters for update_line_cover() ' ||
                   '---------');
       arp_util.debug('update_line_cover: ' || 'p_form_name = ' || p_form_name);
       arp_util.debug('update_line_cover: ' || 'p_form_version = ' || p_form_version);
       arp_util.debug('update_line_cover: ' || 'p_customer_trx_line_id = ' || p_customer_trx_line_id);
       arp_util.debug('update_line_cover: ' || 'p_foreign_currency_code = ' || p_foreign_currency_code);
       arp_util.debug('update_line_cover: ' || 'p_exchange_rate = ' || p_exchange_rate);
       arp_util.debug('update_line_cover: ' || 'p_recalculate_tax_flag = ' ||
                   arp_trx_util.boolean_to_varchar2(p_recalculate_tax_flag));
       arp_util.debug('update_line_cover: ' || 'p_rerun_autoacc_flag = ' ||
                   arp_trx_util.boolean_to_varchar2(p_rerun_autoacc_flag));
       arp_util.debug('update_line_cover: ' || 'p_customer_trx_id = ' || p_customer_trx_id);
       arp_util.debug('update_line_cover: ' || 'p_line_number = ' || p_line_number);
       arp_util.debug('update_line_cover: ' || 'p_line_type = ' || p_line_type);
       arp_util.debug('update_line_cover: ' || 'p_quantity_credited = ' || p_quantity_credited);
       arp_util.debug('update_line_cover: ' || 'p_quantity_invoiced = ' || p_quantity_invoiced);
       arp_util.debug('update_line_cover: ' || 'p_quantity_ordered = ' || p_quantity_ordered);
    END IF;
--    arp_util.debug('p_net_unit_selling_price = ' || p_net_unit_selling_price);
    IF PG_DEBUG in ('Y', 'C') THEN
       arp_util.debug('update_line_cover: ' || 'p_unit_standard_price = ' || p_unit_standard_price);
    END IF;
--    arp_util.debug('p_net_extended_amount = ' || p_net_extended_amount);
    IF PG_DEBUG in ('Y', 'C') THEN
       arp_util.debug('update_line_cover: ' || 'p_memo_line_id = ' || p_memo_line_id);
       arp_util.debug('update_line_cover: ' || 'p_inventory_item_id = ' || p_inventory_item_id);
       arp_util.debug('update_line_cover: ' || 'p_item_exception_rate_id = ' || p_item_exception_rate_id);
       arp_util.debug('update_line_cover: ' || 'p_description = ' || p_description);
       arp_util.debug('update_line_cover: ' || 'p_item_context = ' || p_item_context);
       arp_util.debug('update_line_cover: ' || 'p_initial_customer_trx_line_id = ' ||
                                      p_initial_customer_trx_line_id);
       arp_util.debug('update_line_cover: ' || 'p_link_to_cust_trx_line_id = ' ||
                                      p_link_to_cust_trx_line_id);
       arp_util.debug('update_line_cover: ' || 'p_previous_customer_trx_id = ' ||
                                      p_previous_customer_trx_id);
       arp_util.debug('update_line_cover: ' || 'p_prev_customer_trx_line_id = ' ||
                                      p_prev_customer_trx_line_id);
       arp_util.debug('update_line_cover: ' || 'p_accounting_rule_duration = ' ||
                                      p_accounting_rule_duration);
       arp_util.debug('update_line_cover: ' || 'p_accounting_rule_id = ' || p_accounting_rule_id);
       arp_util.debug('update_line_cover: ' || 'p_rule_start_date = ' || p_rule_start_date);
       arp_util.debug('update_line_cover: ' || 'p_autorule_complete_flag = ' || p_autorule_complete_flag);
       arp_util.debug('update_line_cover: ' || 'p_autorule_duration_processed = ' ||
                                      p_autorule_duration_processed);
       arp_util.debug('update_line_cover: ' || 'p_reason_code = ' || p_reason_code);
       arp_util.debug('update_line_cover: ' || 'p_last_period_to_credit = ' || p_last_period_to_credit);
       arp_util.debug('update_line_cover: ' || 'p_sales_order = ' || p_sales_order);
       arp_util.debug('update_line_cover: ' || 'p_sales_order_date = ' || p_sales_order_date);
       arp_util.debug('update_line_cover: ' || 'p_sales_order_line = ' || p_sales_order_line);
       arp_util.debug('update_line_cover: ' || 'p_sales_order_revision = ' || p_sales_order_revision);
       arp_util.debug('update_line_cover: ' || 'p_sales_order_source = ' || p_sales_order_source);
       arp_util.debug('update_line_cover: ' || 'p_vat_tax_id = ' || p_vat_tax_id);
       arp_util.debug('update_line_cover: ' || 'p_tax_exempt_flag = ' || p_tax_exempt_flag);
       arp_util.debug('update_line_cover: ' || 'p_sales_tax_id = ' || p_sales_tax_id);
       arp_util.debug('update_line_cover: ' || 'p_location_segment_id = ' || p_location_segment_id);
       arp_util.debug('update_line_cover: ' || 'p_tax_exempt_number = ' || p_tax_exempt_number);
       arp_util.debug('update_line_cover: ' || 'p_tax_exempt_reason_code = ' || p_tax_exempt_reason_code);
       arp_util.debug('update_line_cover: ' || 'p_tax_vendor_return_code = ' || p_tax_vendor_return_code);
       arp_util.debug('update_line_cover: ' || 'p_taxable_flag = ' || p_taxable_flag);
       arp_util.debug('update_line_cover: ' || 'p_tax_exemption_id = ' || p_tax_exemption_id);
       arp_util.debug('update_line_cover: ' || 'p_tax_precedence = ' || p_tax_precedence);
       arp_util.debug('update_line_cover: ' || 'p_tax_rate = ' || p_tax_rate);
       arp_util.debug('update_line_cover: ' || 'p_uom_code = ' || p_uom_code);
       arp_util.debug('update_line_cover: ' || 'p_autotax = ' || p_autotax);
       arp_util.debug('update_line_cover: ' || 'p_movement_id = ' || p_movement_id);
       arp_util.debug('update_line_cover: ' || 'p_default_ussgl_trx_code = ' ||
                      p_default_ussgl_trx_code);
       arp_util.debug('update_line_cover: ' || 'p_default_ussgl_trx_code_cntxt = ' ||
                      p_default_ussgl_trx_code_cntxt);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_context = ' || p_interface_line_context);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute1 = ' ||
                      p_interface_line_attribute1);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute2 = ' ||
                      p_interface_line_attribute2);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute3 = ' ||
                      p_interface_line_attribute3);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute4 = ' ||
                      p_interface_line_attribute4);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute5 = ' ||
                      p_interface_line_attribute5);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute6 = ' ||
                      p_interface_line_attribute6);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute7 = ' ||
                      p_interface_line_attribute7);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute8 = ' ||
                      p_interface_line_attribute8);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute9 = ' ||
                      p_interface_line_attribute9);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute10 = ' ||
                      p_interface_line_attribute10);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute11 = ' ||
                      p_interface_line_attribute11);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute12 = ' ||
                      p_interface_line_attribute12);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute13 = ' ||
                      p_interface_line_attribute13);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute14 = ' ||
                      p_interface_line_attribute14);
       arp_util.debug('update_line_cover: ' || 'p_interface_line_attribute15 = ' ||
                      p_interface_line_attribute15);
       arp_util.debug('update_line_cover: ' || 'p_attribute_category = ' || p_attribute_category);
       arp_util.debug('update_line_cover: ' || 'p_attribute1 = ' || p_attribute1);
       arp_util.debug('update_line_cover: ' || 'p_attribute2 = ' || p_attribute2);
       arp_util.debug('update_line_cover: ' || 'p_attribute3 = ' || p_attribute3);
       arp_util.debug('update_line_cover: ' || 'p_attribute4 = ' || p_attribute4);
       arp_util.debug('update_line_cover: ' || 'p_attribute5 = ' || p_attribute5);
       arp_util.debug('update_line_cover: ' || 'p_attribute6 = ' || p_attribute6);
       arp_util.debug('update_line_cover: ' || 'p_attribute7 = ' || p_attribute7);
       arp_util.debug('update_line_cover: ' || 'p_attribute8 = ' || p_attribute8);
       arp_util.debug('update_line_cover: ' || 'p_attribute9 = ' || p_attribute9);
       arp_util.debug('update_line_cover: ' || 'p_attribute10 = ' || p_attribute10);
       arp_util.debug('update_line_cover: ' || 'p_attribute11 = ' || p_attribute11);
       arp_util.debug('update_line_cover: ' || 'p_attribute12 = ' || p_attribute12);
       arp_util.debug('update_line_cover: ' || 'p_attribute13 = ' || p_attribute13);
       arp_util.debug('update_line_cover: ' || 'p_attribute14 = ' || p_attribute14);
       arp_util.debug('update_line_cover: ' || 'p_attribute15 = ' || p_attribute15);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute_category = ' || p_global_attribute_category);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute1 = ' || p_global_attribute1);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute2 = ' || p_global_attribute2);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute3 = ' || p_global_attribute3);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute4 = ' || p_global_attribute4);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute5 = ' || p_global_attribute5);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute6 = ' || p_global_attribute6);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute7 = ' || p_global_attribute7);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute8 = ' || p_global_attribute8);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute9 = ' || p_global_attribute9);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute10 = ' || p_global_attribute10);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute11 = ' || p_global_attribute11);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute12 = ' || p_global_attribute12);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute13 = ' || p_global_attribute13);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute14 = ' || p_global_attribute14);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute15 = ' || p_global_attribute15);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute16 = ' || p_global_attribute16);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute17 = ' || p_global_attribute17);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute18 = ' || p_global_attribute18);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute19 = ' || p_global_attribute19);
       arp_util.debug('update_line_cover: ' || 'p_global_attribute20 = ' || p_global_attribute20);
       arp_util.debug('update_line_cover: ' || 'p_warehouse_id = ' || p_warehouse_id);
       arp_util.debug('update_line_cover: ' || 'p_translated_description =  ' || p_translated_description);
       arp_util.debug('update_line_cover: ' || 'p_payment_set_id =  ' || p_payment_set_id);
       arp_util.debug('update_line_cover: ' || 'p_rule_end_date =  ' || p_rule_end_date);
    END IF;

  RAISE;

END update_line_cover;
