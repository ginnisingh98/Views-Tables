--------------------------------------------------------
--  DDL for Package Body OE_LINE_ADJ_PCFWK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_LINE_ADJ_PCFWK" AS
/* $Header: OEXKLADB.pls 120.0 2005/05/31 22:55:37 appldev noship $ */

-- Globals
-------------------------------------------
 g_application_id     constant number := 660;
 g_entity_id          constant number := 8;
 g_entity_short_name  constant varchar2(15) := 'LINE_ADJ';
-------------------------------------------
...
IF p_operation = OE_PC_GLOBALS.CREATE_OP THEN
    ...
    FOR c_rec in C_CREATE_OP LOOP
       ...
       IF (l_condition_count = 0 OR l_validation_result = OE_PC_GLOBALS.YES) then
