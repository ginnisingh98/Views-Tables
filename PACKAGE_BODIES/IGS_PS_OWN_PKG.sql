--------------------------------------------------------
--  DDL for Package Body IGS_PS_OWN_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_OWN_PKG" AS
/* $Header: IGSPI27B.pls 120.0 2005/06/01 22:10:53 appldev noship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_PS_OWN%RowType;
  new_references IGS_PS_OWN%RowType;
  ...
  */
  BEGIN
    ...
    IGS_HE_PROG_OU_CC_PKG.get_fk_igs_ps_own (
  ...
  PROCEDURE BeforeRowInsertUpdateDelete1(
	...
	v_course_cd	IGS_PS_OWN.course_cd%TYPE;
	v_version_number	IGS_PS_OWN.version_number%TYPE;
	v_percentage	IGS_PS_OWN.percentage%TYPE;
  BEGIN
	-- Set variables.
	IF p_deleting THEN
	...
	IF p_inserting THEN
	-- As part of the bug# 1956374 changed to the below call from IGS_PS_VAL_COw.crsp_val_ou_sys_sts
	...
	IF p_updating THEN
		IF old_references.percentage <> new_references.percentage THEN
			...
			IGS_PS_GEN_007.CRSP_INS_COW_HIST(
				old_references.course_cd,
				old_references.version_number,
				old_references.org_unit_cd,
  ...
  BEGIN
	...
	END IF;
  	...
  	IF upper(column_name)= 'PERCENTAGE' OR
		column_name is null THEN
		IF (new_references.percentage < 0 OR new_references.percentage  > 100 )
		THEN
            	Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
            	IGS_GE_MSG_STACK.ADD;
            	App_Exception.Raise_Exception;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_COW_CRV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_COW_OU_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_OWN
      where COURSE_CD = X_COURSE_CD
      and OU_START_DT = X_OU_START_DT
      and ORG_UNIT_CD = X_ORG_UNIT_CD
      and VERSION_NUMBER = X_VERSION_NUMBER;
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_OWN
     where COURSE_CD = X_COURSE_CD
     and OU_START_DT = X_OU_START_DT
     and ORG_UNIT_CD = X_ORG_UNIT_CD
     and VERSION_NUMBER = X_VERSION_NUMBER
