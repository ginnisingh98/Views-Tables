--------------------------------------------------------
--  DDL for Package Body IGS_EN_PIG_S_SETUP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_EN_PIG_S_SETUP_PKG" AS
/* $Header: IGSEI67B.pls 115.5 2003/02/24 14:17:40 npalanis noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_en_pig_s_setup%ROWTYPE;
  new_references igs_en_pig_s_setup%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_EN_PIGES_PIG_FK');
      igs_ge_msg_stack.ADD;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    INSERT INTO igs_en_pig_s_setup (
      group_id,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_en_pig_s_setup
    WHERE ROWID = x_rowid;
