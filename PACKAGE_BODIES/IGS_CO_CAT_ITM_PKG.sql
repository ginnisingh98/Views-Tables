--------------------------------------------------------
--  DDL for Package Body IGS_CO_CAT_ITM_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_CO_CAT_ITM_PKG" AS
/* $Header: IGSLI03B.pls 115.6 2002/11/29 01:02:48 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_co_cat_itm%ROWTYPE;
  new_references igs_co_cat_itm%ROWTYPE;
  ...
  PROCEDURE set_column_values (
    ...
    x_block_when_encumbered_ind         IN     VARCHAR2    DEFAULT NULL,
  ...
  */
  ...
  BEGIN
    ...
    new_references.block_when_encumbered_ind         := x_block_when_encumbered_ind;
  ...
  */
  BEGIN
    ...
    IF (column_name IS NULL) THEN
      NULL;
    ELSIF (UPPER(column_name) = 'BLOCK_WHEN_ENCUMBERED_IND') THEN
      new_references.block_when_encumbered_ind := column_value;
    END IF;

    IF (UPPER(column_name) = 'BLOCK_WHEN_ENCUMBERED_IND' OR column_name IS NULL) THEN
      IF NOT (new_references.block_when_encumbered_ind IN ('Y', 'N'))  THEN
  ...
  */
  BEGIN
    ...
    IF (((old_references.correspondence_cat = new_references.correspondence_cat)) OR
      ...
      NULL;
    ELSIF NOT igs_co_cat_pkg.get_pk_for_validation (
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_CO_CORT_CCI_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  PROCEDURE insert_row (
    ...
    x_block_when_encumbered_ind         IN     VARCHAR2,
  ...
  */
  ...
  BEGIN
    ...
    before_dml(
      ...
      x_block_when_encumbered_ind         => NVL (x_block_when_encumbered_ind,'Y' ),
    ...
    INSERT INTO igs_co_cat_itm (
      ...
      new_references.block_when_encumbered_ind,
      x_last_update_date,
      x_last_updated_by,
      x_last_update_date,
      x_last_updated_by,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ((tlinfo.logical_delete_dt = x_logical_delete_dt) OR ((tlinfo.logical_delete_dt IS NULL) AND (X_logical_delete_dt IS NULL)))
        AND (tlinfo.block_when_encumbered_ind = x_block_when_encumbered_ind)
       ) THEN
      NULL;
    ELSE
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_co_cat_itm
      SET
        logical_delete_dt                 = new_references.logical_delete_dt,
        block_when_encumbered_ind         = new_references.block_when_encumbered_ind,
  ...
  */
    CURSOR c1 IS
      ...
      WHERE    correspondence_cat                = x_correspondence_cat
      AND      correspondence_type               = x_correspondence_type
      AND      create_dt                        = NVL (x_create_dt, 'Y' );
