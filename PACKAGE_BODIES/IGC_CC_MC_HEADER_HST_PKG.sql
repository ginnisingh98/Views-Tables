--------------------------------------------------------
--  DDL for Package Body IGC_CC_MC_HEADER_HST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGC_CC_MC_HEADER_HST_PKG" as
/* $Header: IGCCMHHB.pls 120.3.12000000.1 2007/08/20 12:13:22 mbremkum ship $  */

 G_PKG_NAME CONSTANT VARCHAR2(30):= 'IGC_CC_MC_HEADER_HST_PKG ';
...
-- -----------------------------------------------------------------
-- Insert the CC MRC Header History record as requested.
-- -----------------------------------------------------------------
       INSERT INTO IGC_CC_MC_HEADER_HISTORY (
                           CC_Header_Id,
                           Set_Of_Books_Id,
                           cc_version_num,
                           cc_version_action,
...
-- to the caller.
-- -------------------------------------------------------------------
    OPEN C_header_mrc_hst_rowid ;
    FETCH C_header_mrc_hst_rowid
    INTO p_Rowid;
-- -------------------------------------------------------------------
-- If no ROWID can be obtained then exit the procedure with a failure
-- -------------------------------------------------------------------
    if (C_header_mrc_hst_rowid %NOTFOUND) then
      CLOSE C_header_mrc_hst_rowid ;
...
-- -----------------------------------------------------------------
    ...
    WHEN FND_API.G_EXC_ERROR THEN
       ...
       IF (C_header_mrc_hst_rowid %ISOPEN) THEN
           CLOSE C_header_mrc_hst_rowid  ;
       END IF;
  ...
  BEGIN
    ...
    if (
           ...
           AND (   (Recinfo.cc_version_num =  p_cc_version_num)
                OR (    (Recinfo.cc_version_num IS NULL)
                    AND (p_cc_version_num IS NULL)))
           AND (   (Recinfo.cc_version_action =  p_cc_version_action)
                OR (    (Recinfo.cc_version_action IS NULL)
                    AND (p_cc_version_action IS NULL)))
...
/* ================================================================================
  ...
  BEGIN
    ...
    UPDATE IGC_CC_MC_HEADER_HISTORY
    SET
...
/* ================================================================================
 ...
 PROCEDURE Delete_Row(
  ...
  DELETE FROM IGC_CC_MC_HEADER_HISTORY
  WHERE rowid = p_Rowid;
