--------------------------------------------------------
--  DDL for Package Body BEN_PYD_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PYD_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pyd_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_ptip_dpnt_cvg_ctfn_f',
	   ...
	   p_base_key_value	=> p_rec.ptip_dpnt_cvg_ctfn_id);
    --
    ben_pyd_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_ptip_dpnt_cvg_ctfn_f Row
    --
    ...
    --
    ben_pyd_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_pyd_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.ptip_dpnt_cvg_ctfn_id,
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'ben_ptip_dpnt_cvg_ctfn_f'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pyd_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    ben_pyd_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.ptip_id = hr_api.g_number) then
    p_rec.ptip_id :=
    ben_pyd_shd.g_old_rec.ptip_id;
  End If;
  If (p_rec.pfd_flag = hr_api.g_varchar2) then
    p_rec.pfd_flag :=
    ben_pyd_shd.g_old_rec.pfd_flag;
  End If;
  ...
  If (p_rec.dpnt_cvg_ctfn_typ_cd = hr_api.g_varchar2) then
    ...
    ben_pyd_shd.g_old_rec.dpnt_cvg_ctfn_typ_cd;
  End If;
  If (p_rec.rlshp_typ_cd = hr_api.g_varchar2) then
    p_rec.rlshp_typ_cd :=
    ben_pyd_shd.g_old_rec.rlshp_typ_cd;
  End If;
  If (p_rec.rqd_flag = hr_api.g_varchar2) then
    p_rec.rqd_flag :=
    ben_pyd_shd.g_old_rec.rqd_flag;
  End If;
  If (p_rec.pyd_attribute_category = hr_api.g_varchar2) then
    p_rec.pyd_attribute_category :=
    ben_pyd_shd.g_old_rec.pyd_attribute_category;
  End If;
  If (p_rec.pyd_attribute1 = hr_api.g_varchar2) then
    p_rec.pyd_attribute1 :=
    ben_pyd_shd.g_old_rec.pyd_attribute1;
  End If;
  If (p_rec.pyd_attribute2 = hr_api.g_varchar2) then
    p_rec.pyd_attribute2 :=
    ben_pyd_shd.g_old_rec.pyd_attribute2;
  End If;
  If (p_rec.pyd_attribute3 = hr_api.g_varchar2) then
    p_rec.pyd_attribute3 :=
    ben_pyd_shd.g_old_rec.pyd_attribute3;
  End If;
  If (p_rec.pyd_attribute4 = hr_api.g_varchar2) then
    p_rec.pyd_attribute4 :=
    ben_pyd_shd.g_old_rec.pyd_attribute4;
  End If;
  If (p_rec.pyd_attribute5 = hr_api.g_varchar2) then
    p_rec.pyd_attribute5 :=
    ben_pyd_shd.g_old_rec.pyd_attribute5;
  End If;
  If (p_rec.pyd_attribute6 = hr_api.g_varchar2) then
    p_rec.pyd_attribute6 :=
    ben_pyd_shd.g_old_rec.pyd_attribute6;
  End If;
  If (p_rec.pyd_attribute7 = hr_api.g_varchar2) then
    p_rec.pyd_attribute7 :=
    ben_pyd_shd.g_old_rec.pyd_attribute7;
  End If;
  If (p_rec.pyd_attribute8 = hr_api.g_varchar2) then
    p_rec.pyd_attribute8 :=
    ben_pyd_shd.g_old_rec.pyd_attribute8;
  End If;
  If (p_rec.pyd_attribute9 = hr_api.g_varchar2) then
    p_rec.pyd_attribute9 :=
    ben_pyd_shd.g_old_rec.pyd_attribute9;
  End If;
  If (p_rec.pyd_attribute10 = hr_api.g_varchar2) then
    p_rec.pyd_attribute10 :=
    ben_pyd_shd.g_old_rec.pyd_attribute10;
  End If;
  If (p_rec.pyd_attribute11 = hr_api.g_varchar2) then
    p_rec.pyd_attribute11 :=
    ben_pyd_shd.g_old_rec.pyd_attribute11;
  End If;
  If (p_rec.pyd_attribute12 = hr_api.g_varchar2) then
    p_rec.pyd_attribute12 :=
    ben_pyd_shd.g_old_rec.pyd_attribute12;
  End If;
  If (p_rec.pyd_attribute13 = hr_api.g_varchar2) then
    p_rec.pyd_attribute13 :=
    ben_pyd_shd.g_old_rec.pyd_attribute13;
  End If;
  If (p_rec.pyd_attribute14 = hr_api.g_varchar2) then
    p_rec.pyd_attribute14 :=
    ben_pyd_shd.g_old_rec.pyd_attribute14;
  End If;
  If (p_rec.pyd_attribute15 = hr_api.g_varchar2) then
    p_rec.pyd_attribute15 :=
    ben_pyd_shd.g_old_rec.pyd_attribute15;
  End If;
  If (p_rec.pyd_attribute16 = hr_api.g_varchar2) then
    p_rec.pyd_attribute16 :=
    ben_pyd_shd.g_old_rec.pyd_attribute16;
  End If;
  If (p_rec.pyd_attribute17 = hr_api.g_varchar2) then
    p_rec.pyd_attribute17 :=
    ben_pyd_shd.g_old_rec.pyd_attribute17;
  End If;
  If (p_rec.pyd_attribute18 = hr_api.g_varchar2) then
    p_rec.pyd_attribute18 :=
    ben_pyd_shd.g_old_rec.pyd_attribute18;
  End If;
  If (p_rec.pyd_attribute19 = hr_api.g_varchar2) then
    p_rec.pyd_attribute19 :=
    ben_pyd_shd.g_old_rec.pyd_attribute19;
  End If;
  If (p_rec.pyd_attribute20 = hr_api.g_varchar2) then
    p_rec.pyd_attribute20 :=
    ben_pyd_shd.g_old_rec.pyd_attribute20;
  End If;
  If (p_rec.pyd_attribute21 = hr_api.g_varchar2) then
    p_rec.pyd_attribute21 :=
    ben_pyd_shd.g_old_rec.pyd_attribute21;
  End If;
  If (p_rec.pyd_attribute22 = hr_api.g_varchar2) then
    p_rec.pyd_attribute22 :=
    ben_pyd_shd.g_old_rec.pyd_attribute22;
  End If;
  If (p_rec.pyd_attribute23 = hr_api.g_varchar2) then
    p_rec.pyd_attribute23 :=
    ben_pyd_shd.g_old_rec.pyd_attribute23;
  End If;
  If (p_rec.pyd_attribute24 = hr_api.g_varchar2) then
    p_rec.pyd_attribute24 :=
    ben_pyd_shd.g_old_rec.pyd_attribute24;
  End If;
  If (p_rec.pyd_attribute25 = hr_api.g_varchar2) then
    p_rec.pyd_attribute25 :=
    ben_pyd_shd.g_old_rec.pyd_attribute25;
  End If;
  If (p_rec.pyd_attribute26 = hr_api.g_varchar2) then
    p_rec.pyd_attribute26 :=
    ben_pyd_shd.g_old_rec.pyd_attribute26;
  End If;
  If (p_rec.pyd_attribute27 = hr_api.g_varchar2) then
    p_rec.pyd_attribute27 :=
    ben_pyd_shd.g_old_rec.pyd_attribute27;
  End If;
  If (p_rec.pyd_attribute28 = hr_api.g_varchar2) then
    p_rec.pyd_attribute28 :=
    ben_pyd_shd.g_old_rec.pyd_attribute28;
  End If;
  If (p_rec.pyd_attribute29 = hr_api.g_varchar2) then
    p_rec.pyd_attribute29 :=
    ben_pyd_shd.g_old_rec.pyd_attribute29;
  End If;
  If (p_rec.pyd_attribute30 = hr_api.g_varchar2) then
    p_rec.pyd_attribute30 :=
    ben_pyd_shd.g_old_rec.pyd_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_pyd_shd.convert_args
  (
  p_ptip_dpnt_cvg_ctfn_id,
  null,
  null,
  p_business_group_id,
  p_ptip_id,
