--------------------------------------------------------
--  DDL for Package Body FND_OAM_BF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FND_OAM_BF_PKG" as
/* $Header: AFOAMFSB.pls 115.0 2003/10/31 03:58:21 ppradhan noship $ */
procedure LOAD_ROW (
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from FND_OAM_BF
    where BIZ_FLOW_KEY = X_BIZ_FLOW_KEY
    ;
begin
  insert into FND_OAM_BF (
    BIZ_FLOW_KEY,
    MONITORED_FLAG,
    IS_TOP_LEVEL,
    CREATION_DATE,
...
begin
  ...
  update FND_OAM_BF_TL T set (
      FLOW_DISPLAY_NAME,
      DESCRIPTION
    ) = (select
      B.FLOW_DISPLAY_NAME,
      B.DESCRIPTION
    from FND_OAM_BF_TL B
    where B.BIZ_FLOW_KEY = T.BIZ_FLOW_KEY
  ...
  where (
      ...
      SUBT.BIZ_FLOW_KEY,
      SUBT.LANGUAGE
    from FND_OAM_BF_TL SUBB, FND_OAM_BF_TL SUBT
  ...
  insert into FND_OAM_BF_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from FND_OAM_BF_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
