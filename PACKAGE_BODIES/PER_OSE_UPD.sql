--------------------------------------------------------
--  DDL for Package Body PER_OSE_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_OSE_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_ose_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  -- Update the per_org_structure_elements Row
  --
  update per_org_structure_elements
    set
     org_structure_element_id        = p_rec.org_structure_element_id
    ,organization_id_parent          = p_rec.organization_id_parent
    ,organization_id_child           = p_rec.organization_id_child
    ,request_id                      = p_rec.request_id
    ,program_application_id          = p_rec.program_application_id
    ,program_id                      = p_rec.program_id
    ,program_update_date             = p_rec.program_update_date
    ,object_version_number           = p_rec.object_version_number
    ,position_control_enabled_flag  = p_rec.position_control_enabled_flag
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    per_ose_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.organization_id_parent = hr_api.g_number) then
    p_rec.organization_id_parent :=
    per_ose_shd.g_old_rec.organization_id_parent;
  End If;
  If (p_rec.org_structure_version_id = hr_api.g_number) then
    ...
    per_ose_shd.g_old_rec.org_structure_version_id;
  End If;
  If (p_rec.organization_id_child = hr_api.g_number) then
    p_rec.organization_id_child :=
    per_ose_shd.g_old_rec.organization_id_child;
  End If;
  If (p_rec.position_control_enabled_flag = hr_api.g_varchar2) then
    p_rec.position_control_enabled_flag :=
    per_ose_shd.g_old_rec.position_control_enabled_flag;
  End If;
...
--
  ...
  l_old_parent_org_id	per_org_structure_elements.ORGANIZATION_ID_PARENT%type;
...
Begin
  ...
  --
  convert_defs(p_rec);
  per_ose_bus.update_validate
     (p_rec
     ,p_effective_date
     );

  --Bug no 2720039 starts here
  select ORGANIZATION_ID_PARENT into l_old_parent_org_id from per_org_structure_elements x
  where ORG_STRUCTURE_VERSION_ID = p_rec.org_structure_version_id
  and not exists (select null from per_org_structure_elements
  where ORGANIZATION_ID_CHILD = x.ORGANIZATION_ID_PARENT)
  and rownum=1;
  --Bug no 2720039 ends here
  --
  -- Call the supporting pre-update operation
  --
  per_ose_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_ose_upd.update_dml(p_rec);
  --
  ...
  Putting extra check on Top Node in the organization hierarchy so that it can not be updated.*/

  select ORGANIZATION_ID_PARENT into l_new_parent_org_id from per_org_structure_elements x
  where ORG_STRUCTURE_VERSION_ID = p_rec.org_structure_version_id
  and not exists (select null from per_org_structure_elements
  where ORGANIZATION_ID_CHILD = x.ORGANIZATION_ID_PARENT)
  and rownum=1;
hr_utility.set_location('old_parent_id '||l_old_parent_org_id||'new_parent_id '||l_new_parent_org_id,10);
if (l_old_parent_org_id<>l_new_parent_org_id) then
	raise l_myexception;
end if;

--Bug no 2720039 starts here

  per_ose_upd.post_update
     (p_rec
     );
End upd;
--
