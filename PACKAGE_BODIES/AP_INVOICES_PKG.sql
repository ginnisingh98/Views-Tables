--------------------------------------------------------
--  DDL for Package Body AP_INVOICES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_INVOICES_PKG" AS
/* $Header: apiinceb.pls 120.49.12010000.13 2010/12/24 04:17:13 pgayen ship $ */
...
/*Bug5090887 commented the Group by  */
     ...
     select SUM(decode(wfapproval_status,'NOT REQUIRED',1,0)) ,
            ...
            SUM(decode(wfapproval_status,'REAPPROVE',1,0)) ,
     ...
     If l_reapprove>0 then return('REAPPROVE'); end if;
