--------------------------------------------------------
--  DDL for Package Body OKC_CRORDER_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_CRORDER_ASMBLR_PVT" AS
/* $Header: OKCRCOKB.pls 120.0 2005/05/25 19:30:12 appldev noship $ */
...
--
   ...
   l_order_number             varchar2(120) := to_char(p_order_number);
--
...
--
    ...
    FOR acn_rec IN acn_cur LOOP
      ...
      FETCH k_cur INTO k_rec;
       ...
       ELSIF acn_rec.element_name = 'ORDER_NUMBER' THEN
          l_tbl.extend;
          l_tbl(i).element_name := 'ORDER_NUMBER';
          l_tbl(i).element_value := P_ORDER_NUMBER;
       END IF;
       i := i+1;
      CLOSE k_cur;
     END LOOP;
