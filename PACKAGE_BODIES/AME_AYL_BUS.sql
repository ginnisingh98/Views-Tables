--------------------------------------------------------
--  DDL for Package Body AME_AYL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_AYL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_ayl_bus.';  -- Global package name
--
...
Procedure set_security_group_id
  ...
  -- ame_action_types_tl and PER_BUSINESS_GROUPS_PERF
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'ACTION_TYPE_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  --
  if (( nvl(ame_ayl_bus.g_action_type_id, hr_api.g_number)
       = p_action_type_id)
  and ( nvl(ame_ayl_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := ame_ayl_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    ame_ayl_bus.g_action_type_id              := p_action_type_id;
    ame_ayl_bus.g_language                    := p_language;
    ame_ayl_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT ame_ayl_shd.api_updating
