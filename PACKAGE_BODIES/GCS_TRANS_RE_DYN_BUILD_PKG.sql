--------------------------------------------------------
--  DDL for Package Body GCS_TRANS_RE_DYN_BUILD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GCS_TRANS_RE_DYN_BUILD_PKG" AS
 /* $Header: gcsxltredb.pls 120.3 2007/06/28 12:31:47 vkosuri noship $ */
...
--
  -- The API name
  g_api		CONSTANT VARCHAR2(50) := 'gcs.plsql.GCS_TRANS_RE_DYN_BUILD_PKG';
...
';
  ...
  -- Example
  --   GCS_TRANS_RE_DYN_BUILD_PKG.Module_Log_Write
  -- Notes
  --
  ...
  -- Example
  --   GCS_TRANS_RE_DYN_BUILD_PKG.Write_To_Log
  -- Notes
  --
...
--
  ...
  BEGIN
    ...
    -- Bugfix 5725759: Initialize the variables that determines the required dimensions.
    GCS_TRANS_DYN_BUILD_PKG.Initialize_Dimensions;

    -- Create the package body
    body_block :=
   'CREATE OR REPLACE PACKAGE BODY GCS_TRANS_RE_DYNAMIC_PKG AS

      -- The API name
      g_api             VARCHAR2(50) := ''gcs.plsql.GCS_TRANS_RE_DYNAMIC_PKG'';
...
'';
    ...
    --
      ...
      --   p_action_type  Entered, Exited Successfully, or Exited with Failure
      -- Example
      --   GCS_TRANS_RE_DYNAMIC_PKG.Module_Log_Write
      -- Notes
      --
      PROCEDURE Module_Log_Write
        (p_module       VARCHAR2,
      ...
      --   p_text		Text to write
      -- Example
      --   GCS_TRANS_RE_DYNAMICPKG.Write_To_Log
      -- Notes
      --
      PROCEDURE Write_To_Log
        (p_module	VARCHAR2,
    ...
    -- Public procedures
      ...
      --
      PROCEDURE Roll_Forward_Retained_Earnings
        ...
        module        VARCHAR2(50) := ''ROLL_FORWARD_RETAINED_EARNINGS'';
      BEGIN
        write_to_log(module, FND_LOG.LEVEL_PROCEDURE,g_module_enter);

        --Bugfix 6111815: Only Roll Forward Rates Where Standard RE Rate Flag is Y
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''        AND    ghr1.', ' = ghr.', ''' || g_nl ||', line_num, 'F');

    --Bugfix 6111815: Only Roll Forward Rates Where Standard RE Rate Flag is Y
    body_block :=
   '''        AND    ghr1.cal_period_id = '' || p_prev_period_id || '')'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''        AND    ghr3.', ' = ghr2.', ''' || g_nl ||', line_num, 'F');

    body_block :=
   '''        AND    ghr3.from_currency = ghr2.from_currency'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('     AND    ghr1.', ' = ghr.', '', line_num, 'F');

    --Bugfix 6111815: Only Roll Forward Rates Where Standard RE Rate Flag is Y
    body_block :=
   '     AND    ghr1.cal_period_id = p_prev_period_id)
        WHERE  ghr.rowid IN (
         SELECT ghr3.rowid
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('     AND    ghr3.', ' = ghr2.', '', line_num, 'F');

    body_block :=
   '     AND    ghr3.from_currency = ghr2.from_currency
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''        AND    ghr2.', ' (+)= ghr3.', ''' || g_nl ||', line_num, 'F');
   ...
   '''       )'');
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('           AND    ghr2.', ' (+)= ghr3.', '', line_num, 'F');
   ...
   '          );
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_comma_list('''ghr.', ' '' ||', '''NULL, '' ||', line_num, 'F');

    body_block :=
   '''ghr.translated_rate, ghr.translated_amount, ''''P'''', ''''N'''', '' ||
    ...
    ''AND    ghr.line_item_id = '' || p_hier_li_id || g_nl ||
   ...
   '''    AND    ghr1.update_flag = ''''N'''''' || g_nl ||
    ...
    ''    AND    ghr1.to_currency = ghr.to_currency);'' || g_nl );
        ...
        -- Bugfix 6111815: Adeded Standard RE Rate Flag as part of insert
   ...
   '      ghr.translated_rate, ghr.translated_amount, ''P'', ''N'',
        ...
        AND    ghr.line_item_id = p_hier_li_id
   ...
   '    AND    ghr1.update_flag = ''N''
      ...
      END Roll_Forward_Retained_Earnings;


      --
      PROCEDURE Trans_RE_First_Per
        (p_hier_dataset_code    NUMBER,
         p_source_system_code NUMBER,
         p_ledger_id       NUMBER,
         p_cal_period_id   NUMBER,
        ...
        module    VARCHAR2(50) := ''TRANS_RE_FIRST_PER'';
      BEGIN
        write_to_log(module, FND_LOG.LEVEL_PROCEDURE,g_module_enter);
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_comma_list('''fb.', ' '' ||', '''NULL, '' ||', line_num, 'F');
   ...
   '''0, 0, 0, 0,'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_comma_list('''          fb_in.', ' '' ||', '', line_num, 'F');
   ...
   '''          fb_in.line_item_id) fb,'' || g_nl ||
    ...
    --Bugfix 6111815: Remove the outer join
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''AND    ghr.', ' = fb.', ''' || g_nl ||', line_num, 'F');
  ...
  '              fb_in.line_item_id) fb,
   ...
   --Bugfix 6111815: Removed Outer Join
   line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('      AND    ghr.', ' = fb.', '', line_num, 'F');
  ...
  '       ;
   ...
   '        0, 0, 0, 0,
    ...
    --Bugfix 6111815: Removed Added Standard RE Rate Flag
    body_block :=
   ...
   '      ;
        ...
        --Bugfix 6111815: Do not check if any data was translated
        /*
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
          ...
          module_log_write(module, g_module_failure);
          raise GCS_TRANSLATION_PKG.GCS_CCY_SUBPROGRAM_RAISED;
      END Trans_RE_First_Per;


      --
      PROCEDURE Trans_RE_Subseq_Per
        (p_hier_dataset_code       NUMBER,
         p_cal_period_id      NUMBER,
         p_prev_period_id     NUMBER,
         p_entity_id          NUMBER,
        ...
        module    VARCHAR2(50) := ''TRANS_RE_SUBSEQ_PER'';
      BEGIN
        module_log_write(module, g_module_enter);
   ...
   'g_nl ||
    ...
    body_block :=
   '''          SUM(ptd_debit_balance_e) sum_ptd_debit_balance_e,'' || g_nl ||
   ...
   '''          fb_in.line_item_id) fb,'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''AND    fbp.', ' (+)= fb.', ''' || g_nl ||', line_num, 'F');
   ...
   '        round(nvl(ghr.translated_amount * 0,
    ...
    body_block :=
   '                SUM(ptd_debit_balance_e) sum_ptd_debit_balance_e,
   ...
   '             fb_in.line_item_id) fb,
          ...
          AND    fbp.line_item_id (+)= fb.line_item_id
          AND    fb.line_item_id = p_hier_li_id';
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('      AND    fbp.', ' (+)= fb.', '', line_num, 'F');
   ...
   'g_nl ||
    ...
    body_block :=
   '''          ptd_debit_balance_e sum_ptd_debit_balance_e,'' || g_nl ||
   ...
   '''AND    ghr.entity_id = '' || p_entity_id || g_nl ||
    ...
    --Bugfix 6111815: Removing Outer Join Condition
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''AND    ghr.', ' = fb.', ''' || g_nl ||', line_num, 'F');
   ...
   '        round(nvl(ghr.translated_amount * 0,
    ...
    body_block :=
   '                ptd_debit_balance_e sum_ptd_debit_balance_e,
    ...
    --Bugfix 6111815: Removed Outer Joins and Added Standard RE Rate Flag Condition
    body_block :=
   '      AND    ghr.entity_id = p_entity_id
          AND    ghr.hierarchy_id = p_hierarchy_id
          AND    ghr.from_currency = p_from_ccy
   ...
   '      ;
        ...
        --Bugfix 6111815: Do not Raise No Data Found anymore
        /*
        -- No data was found to translate.
        IF SQL%ROWCOUNT = 0 THEN
          raise GCS_CCY_NO_DATA;
      ...
      EXCEPTION
        ...
        WHEN OTHERS THEN
          ...
          module_log_write(module, g_module_failure);
          raise GCS_TRANSLATION_PKG.GCS_CCY_SUBPROGRAM_RAISED;
      END Trans_RE_Subseq_Per;

    END GCS_TRANS_RE_DYNAMIC_PKG;';
    ...
    ad_ddl.create_plsql_object(appl, 'APPS', 'GCS_TRANS_RE_DYNAMIC_PKG', 1, line_num - 1, 'FALSE', err);
