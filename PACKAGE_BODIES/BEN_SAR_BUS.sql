--------------------------------------------------------
--  DDL for Package Body BEN_SAR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_SAR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_sar_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_svc_area_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   svc_area_rt_id PK of record being inserted or updated.
...
--
Procedure chk_svc_area_rt_id(p_svc_area_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_svc_area_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_sar_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_svc_area_rt_id                => p_svc_area_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_svc_area_rt_id,hr_api.g_number)
     <>  ben_sar_shd.g_old_rec.svc_area_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_sar_shd.constraint_error('BEN_SVC_AREA_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_svc_area_rt_id is not null then
      --
...
--   Additionally this procedure will check that svc_area_id is unique
--   within the Variable profile.
--
-- Pre-Conditions
--   None.
--
...
Procedure chk_svc_area_id(
  ...
  cursor c1 is
    select null
    from   ben_svc_area_f a
    where  a.svc_area_id = p_svc_area_id
  ...
  cursor c3 is
         ...
         where svc_area_id = p_svc_area_id
           ...
           and svc_area_rt_id <> nvl(p_svc_area_rt_id,hr_api.g_number)
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_sar_shd.g_old_rec.svc_area_id,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if p_svc_area_id value exists in ben_svc_area_f table
    --
    open c1;
      --
      ...
      if c1%notfound then
        ...
        close c1;
        --
        -- raise error as FK does not relate to PK in ben_svc_area_f
        -- table.
        --
        ben_pzr_shd.constraint_error('BEN_SVC_AREA_RT_PK');
        --
      end if;
      --
    close c1;
    ...
    if c3%found then
      close c3;
      --
      -- raise error as this postal zip range already exists for this profile
    --
     fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
     fnd_message.set_token('VAR1','Service Area criteria');
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and svc_area_rt_id  <> nvl(p_svc_area_rt_id,-1)
                   and p_effective_date between effective_start_date
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_svc_area_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_sar_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
function return_legislation_code
  (p_svc_area_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_svc_area_rt_f b
    where b.svc_area_rt_id      = p_svc_area_rt_id
