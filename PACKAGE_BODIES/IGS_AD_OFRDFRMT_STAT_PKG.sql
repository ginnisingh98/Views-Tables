--------------------------------------------------------
--  DDL for Package Body IGS_AD_OFRDFRMT_STAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_OFRDFRMT_STAT_PKG" as
/* $Header: IGSAI26B.pls 115.6 2003/10/30 13:11:48 akadam ship $ */


 l_rowid VARCHAR2(25);
  old_references IGS_AD_OFRDFRMT_STAT%RowType;
  new_references IGS_AD_OFRDFRMT_STAT%RowType;
  ...
  BEGIN
    ...
    new_references.s_adm_offer_dfrmnt_status := x_s_adm_offer_dfrmnt_status;
  ...
  BEGIN
	-- Validate Admission offer deferment status closed ind.
	IF p_inserting OR ((old_references.s_adm_offer_dfrmnt_status <>
				 new_references.s_adm_offer_dfrmnt_status) OR
			(old_references.closed_ind = 'Y' AND new_references.closed_ind = 'N')) THEN
		IF IGS_AD_VAL_AODS.admp_val_saods_clsd(
					new_references.s_adm_offer_dfrmnt_status,
...
ELSIF upper(Column_name) = 'S_ADM_OFFER_DFRMNT_STATUS' THEN
  new_references.S_ADM_OFFER_DFRMNT_STATUS:= COLUMN_VALUE ;
...
IF upper(Column_name) = 'S_ADM_OFFER_DFRMNT_STATUS' OR COLUMN_NAME IS NULL THEN
  IF new_references.S_ADM_OFFER_DFRMNT_STATUS<> upper(new_references.S_ADM_OFFER_DFRMNT_STATUS) then
    Fnd_Message.Set_Name('IGS','IGS_GE_INVALID_VALUE');
    IGS_GE_MSG_STACK.ADD;
    App_Exception.Raise_Exception ;
  END IF;
...
END IF ;
 ...
 END Check_Constraints;
  ...
  BEGIN
    ...
    IF (((old_references.s_adm_offer_dfrmnt_status = new_references.s_adm_offer_dfrmnt_status)) OR
        ((new_references.s_adm_offer_dfrmnt_status IS NULL))) THEN
      NULL;
    ELSE
	IF NOT IGS_LOOKUPS_VIEW_PKG.Get_PK_For_Validation (
	'ADM_OFFER_DFRMNT_STATUS',new_references.s_adm_offer_dfrmnt_status
	) THEN
     Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
     IGS_GE_MSG_STACK.ADD;
     App_Exception.Raise_Exception;
  ...
  BEGIN
    ...
    IGS_AD_PS_APPL_INST_PKG.GET_FK_IGS_AD_OFRDFRMT_STAT (
  ...
  END Check_Child_Existance;
   ...
   as
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     IGS_AD_OFRDFRMT_STAT
      WHERE    adm_offer_dfrmnt_status = x_adm_offer_dfrmnt_status AND
  ...
  PROCEDURE GET_FK_IGS_LOOKUPS_VIEW(
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     IGS_AD_OFRDFRMT_STAT
      WHERE    s_adm_offer_dfrmnt_status = x_s_adm_offer_dfrmnt_status ;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
       Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_AODS_SAODS_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_OFRDFRMT_STAT
...
Before_DML(
    ...
    x_s_adm_offer_dfrmnt_status => X_S_ADM_OFFER_DFRMNT_STATUS ,
  ...
  insert into IGS_AD_OFRDFRMT_STAT (
    ADM_OFFER_DFRMNT_STATUS,
    DESCRIPTION,
    S_ADM_OFFER_DFRMNT_STATUS,
    SYSTEM_DEFAULT_IND,
    CLOSED_IND,
    CREATION_DATE,
    CREATED_BY,
...
begin
  ...
  if ( (tlinfo.DESCRIPTION = X_DESCRIPTION)
      AND (tlinfo.S_ADM_OFFER_DFRMNT_STATUS = X_S_ADM_OFFER_DFRMNT_STATUS)
...
Before_DML(
  ...
  update IGS_AD_OFRDFRMT_STAT set
    DESCRIPTION = NEW_REFERENCES.DESCRIPTION,
    S_ADM_OFFER_DFRMNT_STATUS = NEW_REFERENCES.S_ADM_OFFER_DFRMNT_STATUS,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_OFRDFRMT_STAT
     where ADM_OFFER_DFRMNT_STATUS = X_ADM_OFFER_DFRMNT_STATUS
  ;
...
Before_DML(
  ...
  delete from IGS_AD_OFRDFRMT_STAT
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
