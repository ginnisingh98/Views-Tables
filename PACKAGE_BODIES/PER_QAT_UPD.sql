--------------------------------------------------------
--  DDL for Package Body PER_QAT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_QAT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_qat_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the per_qualifications_tl Row
  --
  update per_qualifications_tl
    set
...
Begin
  ...
  --
  per_qat_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_qat_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_qat_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
  ...
  cursor csr_upd_langs is
    select qat.language
     ...
     where qat.qualification_id = p_qualification_id
       and p_language_code in (qat.language
                              ,qat.source_lang);
  --
