--------------------------------------------------------
--  DDL for Package Body PER_PSV_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PSV_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_psv_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_pos_structure_version_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_pos_structure_version_id  in  number) is
--
...
Begin
  ...
  --
  per_psv_ins.g_pos_structure_version_id_i := p_pos_structure_version_id;
  --
...
Procedure insert_dml
  (p_rec in out nocopy per_psv_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  -- Insert the row into: per_pos_structure_versions
  --
...
--
  Cursor C_Sel1 is select per_pos_structure_versions_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_pos_structure_versions
     where pos_structure_version_id =
             per_psv_ins.g_pos_structure_version_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (per_psv_ins.g_pos_structure_version_id_i is not null) Then
    --
    ...
    p_rec.pos_structure_version_id :=
      per_psv_ins.g_pos_structure_version_id_i;
    per_psv_ins.g_pos_structure_version_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.pos_structure_version_id;
    Close C_Sel1;
  End If;
  --
...
Begin
  ...
  --
  per_psv_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  per_psv_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  per_psv_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  -- p_rec structure.
  --
per_psv_bus.get_business_group_id
  (p_position_structure_id      =>  p_position_structure_id
  ,p_business_group_id              =>  l_business_group_id);

  l_rec :=
  per_psv_shd.convert_args
    (null
    ,l_business_group_id
    ,p_position_structure_id
    ,p_date_from
  ...
  --
  p_pos_structure_version_id := l_rec.pos_structure_version_id;
