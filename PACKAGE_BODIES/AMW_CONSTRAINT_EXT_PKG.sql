--------------------------------------------------------
--  DDL for Package Body AMW_CONSTRAINT_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMW_CONSTRAINT_EXT_PKG" as
/* $Header: amwtcnxb.pls 120.0 2005/12/28 08:43:19 appldev noship $ */


procedure ADD_LANGUAGE
is
begin
  delete from AMW_CONSTRAINT_EXT_TL T
  where not exists
    (select NULL
    from AMW_CONSTRAINT_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update AMW_CONSTRAINT_EXT_TL T set (
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from AMW_CONSTRAINT_EXT_TL SUBB, AMW_CONSTRAINT_EXT_TL SUBT
  ...
  insert into AMW_CONSTRAINT_EXT_TL (
    LANGUAGE,
    SOURCE_LANG,
    LAST_UPDATED_BY,
    LAST_UPDATE_DATE,
    ...
    B.TL_EXT_ATTR31,B.TL_EXT_ATTR32,B.TL_EXT_ATTR33,B.TL_EXT_ATTR34,B.TL_EXT_ATTR35,
    B.TL_EXT_ATTR36,B.TL_EXT_ATTR37,B.TL_EXT_ATTR38,B.TL_EXT_ATTR39,B.TL_EXT_ATTR40
  from AMW_CONSTRAINT_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from AMW_CONSTRAINT_EXT_TL T
    where T.EXTENSION_ID = B.EXTENSION_ID
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;


end AMW_CONSTRAINT_EXT_PKG;
