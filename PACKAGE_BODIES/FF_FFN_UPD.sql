--------------------------------------------------------
--  DDL for Package Body FF_FFN_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FF_FFN_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ff_ffn_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy ff_ffn_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  -- Update the ff_functions Row
  --
  update ff_functions
    set
     class                           = p_rec.class
    ,name                            = p_rec.name
    ,alias_name                      = p_rec.alias_name
    ,data_type                       = p_rec.data_type
    ,definition                      = p_rec.definition
...
Begin
  ...
  begin
    ...
    ff_ffn_rku.after_update
      ...
      ,p_class                       => p_rec.class
      ,p_name                        => p_rec.name
      ,p_alias_name                  => p_rec.alias_name
      ,p_data_type                   => p_rec.data_type
      ,p_definition                  => p_rec.definition
      ,p_description                 => p_rec.description
      ,p_object_version_number       => p_rec.object_version_number
      ,p_business_group_id_o         => ff_ffn_shd.g_old_rec.business_group_id
      ,p_legislation_code_o          => ff_ffn_shd.g_old_rec.legislation_code
      ,p_class_o                     => ff_ffn_shd.g_old_rec.class
      ,p_name_o                      => ff_ffn_shd.g_old_rec.name
      ,p_alias_name_o                => ff_ffn_shd.g_old_rec.alias_name
      ,p_data_type_o                 => ff_ffn_shd.g_old_rec.data_type
      ,p_definition_o                => ff_ffn_shd.g_old_rec.definition
      ,p_description_o               => ff_ffn_shd.g_old_rec.description
      ,p_object_version_number_o     => ff_ffn_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  End If;
  If (p_rec.alias_name = hr_api.g_varchar2) then
    p_rec.alias_name :=
  ...
  End If;
  If (p_rec.definition = hr_api.g_varchar2) then
    p_rec.definition :=
...
Begin
  ...
  --
  ff_ffn_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ff_ffn_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ff_ffn_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
