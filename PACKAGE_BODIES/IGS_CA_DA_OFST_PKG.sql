--------------------------------------------------------
--  DDL for Package Body IGS_CA_DA_OFST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_CA_DA_OFST_PKG" AS
/* $Header: IGSCI09B.pls 115.4 2002/11/28 23:01:40 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_CA_DA_OFST%RowType;
  new_references IGS_CA_DA_OFST%RowType;
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_dao_as_iu
  -- AFTER INSERT OR UPDATE
  -- ON IGS_CA_DA_OFST

  PROCEDURE AfterStmtInsertUpdate2(
...
v_message_name	varchar2(30);
  BEGIN
  	...
  	IF p_inserting
  	THEN
  		-- Validate insert of date alias offset
  		IF IGS_CA_VAL_DAO.calp_val_dao_ins(new_references.dt_alias,
 ...
 BEGIN
		...
		If upper(Column_name)= 'DAY_OFFSET' Or column_name is null then
			If new_references.day_offset < -999 or new_references.day_offset > 999 Then
		...
		If upper(Column_name)= 'WEEK_OFFSET' Or column_name is null then
			If new_references.week_offset < -99 or new_references.week_offset > 99 Then
					  Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
					  IGS_GE_MSG_STACK.ADD;
        			  App_Exception.Raise_Exception;
			End if;
		...
		If upper(Column_name)= 'MONTH_OFFSET' Or column_name is null then
			If new_references.month_offset < -99 or new_references.month_offset > 99 Then
					  Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
					  IGS_GE_MSG_STACK.ADD;
        			  App_Exception.Raise_Exception;
			End if;
		...
		If upper(Column_name)= 'YEAR_OFFSET' Or column_name is null then
			If new_references.year_offset < -99 or new_references.year_offset > 99 Then
					  Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
					  IGS_GE_MSG_STACK.ADD;
        			  App_Exception.Raise_Exception;
			End if;
  ...
  BEGIN
    ...
    IF (((old_references.offset_dt_alias = new_references.offset_dt_alias)) OR
  ...
  BEGIN
    ...
    IGS_CA_DA_OFFCNT_PKG.GET_FK_IGS_CA_DA_OFST (
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_CA_DAO_DA_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      AfterStmtInsertUpdate2 ( p_inserting => TRUE );
    ELSIF (p_action = 'UPDATE') THEN
      -- Call all the procedures related to After Update.
      AfterStmtInsertUpdate2 ( p_updating => TRUE );
    ELSIF (p_action = 'DELETE') THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_CA_DA_OFST
      where DT_ALIAS = X_DT_ALIAS
      and OFFSET_DT_ALIAS = X_OFFSET_DT_ALIAS;
...
procedure LOCK_ROW (
  ...
  X_OFFSET_DT_ALIAS in VARCHAR2,
  X_DAY_OFFSET NUMBER,
  X_WEEK_OFFSET NUMBER,
  X_MONTH_OFFSET NUMBER,
  X_YEAR_OFFSET NUMBER
) AS
  cursor c1 is select
	DT_ALIAS,
  	OFFSET_DT_ALIAS,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_CA_DA_OFST
     where DT_ALIAS = X_DT_ALIAS
     and OFFSET_DT_ALIAS = X_OFFSET_DT_ALIAS
  ;
begin
