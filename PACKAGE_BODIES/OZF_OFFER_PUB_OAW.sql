--------------------------------------------------------
--  DDL for Package Body OZF_OFFER_PUB_OAW
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OZF_OFFER_PUB_OAW" as
  /* $Header: ozfaofpb.pls 120.1 2005/09/29 10:48 gramanat noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).activity_product_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).act_product_used_by_id := a2(indx);
          ...
          t(ddindx).category_id := a9(indx);
          t(ddindx).category_set_id := a10(indx);
          t(ddindx).attribute_category := a11(indx);
          t(ddindx).level_type_code := a12(indx);
          t(ddindx).excluded_flag := a13(indx);
          t(ddindx).line_lumpsum_amount := a14(indx);
          t(ddindx).line_lumpsum_qty := a15(indx);
          t(ddindx).scan_value := a16(indx);
          t(ddindx).uom_code := a17(indx);
          t(ddindx).adjustment_flag := a18(indx);
          t(ddindx).scan_unit_forecast := a19(indx);
          t(ddindx).channel_id := a20(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a22 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).activity_product_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).act_product_used_by_id;
          ...
          a9(indx) := t(ddindx).category_id;
          a10(indx) := t(ddindx).category_set_id;
          a11(indx) := t(ddindx).attribute_category;
          a12(indx) := t(ddindx).level_type_code;
          a13(indx) := t(ddindx).excluded_flag;
          a14(indx) := t(ddindx).line_lumpsum_amount;
          a15(indx) := t(ddindx).line_lumpsum_qty;
          a16(indx) := t(ddindx).scan_value;
          a17(indx) := t(ddindx).uom_code;
          a18(indx) := t(ddindx).adjustment_flag;
          a19(indx) := t(ddindx).scan_unit_forecast;
          a20(indx) := t(ddindx).channel_id;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).offer_discount_line_id := a0(indx);
          t(ddindx).parent_discount_line_id := a1(indx);
          t(ddindx).volume_from := a2(indx);
          t(ddindx).volume_to := a3(indx);
          t(ddindx).volume_operator := a4(indx);
          t(ddindx).volume_type := a5(indx);
          t(ddindx).volume_break_type := a6(indx);
          t(ddindx).discount := a7(indx);
          ...
          t(ddindx).precedence := a12(indx);
          t(ddindx).bucket := a13(indx);
          t(ddindx).scan_value := a14(indx);
          t(ddindx).scan_data_quantity := a15(indx);
          t(ddindx).scan_unit_forecast := a16(indx);
          t(ddindx).channel_id := a17(indx);
          t(ddindx).adjustment_flag := a18(indx);
          t(ddindx).start_date_active := a19(indx);
          ...
          t(ddindx).offer_id := a28(indx);
          t(ddindx).off_discount_product_id := a29(indx);
          t(ddindx).parent_off_disc_prod_id := a30(indx);
          t(ddindx).product_level := a31(indx);
          t(ddindx).product_id := a32(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a34 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).offer_discount_line_id;
          a1(indx) := t(ddindx).parent_discount_line_id;
          a2(indx) := t(ddindx).volume_from;
          a3(indx) := t(ddindx).volume_to;
          a4(indx) := t(ddindx).volume_operator;
          a5(indx) := t(ddindx).volume_type;
          a6(indx) := t(ddindx).volume_break_type;
          a7(indx) := t(ddindx).discount;
          ...
          a12(indx) := t(ddindx).precedence;
          a13(indx) := t(ddindx).bucket;
          a14(indx) := t(ddindx).scan_value;
          a15(indx) := t(ddindx).scan_data_quantity;
          a16(indx) := t(ddindx).scan_unit_forecast;
          a17(indx) := t(ddindx).channel_id;
          a18(indx) := t(ddindx).adjustment_flag;
          ...
          a28(indx) := t(ddindx).offer_id;
          a29(indx) := t(ddindx).off_discount_product_id;
          a30(indx) := t(ddindx).parent_off_disc_prod_id;
          a31(indx) := t(ddindx).product_level;
          a32(indx) := t(ddindx).product_id;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).off_discount_product_id := a0(indx);
          t(ddindx).parent_off_disc_prod_id := a1(indx);
          t(ddindx).product_level := a2(indx);
          t(ddindx).product_id := a3(indx);
          ...
          t(ddindx).offer_discount_line_id := a8(indx);
          t(ddindx).offer_id := a9(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a16 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).off_discount_product_id;
          a1(indx) := t(ddindx).parent_off_disc_prod_id;
          a2(indx) := t(ddindx).product_level;
          a3(indx) := t(ddindx).product_id;
          ...
          a8(indx) := t(ddindx).offer_discount_line_id;
          a9(indx) := t(ddindx).offer_id;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          t(ddindx).offer_discount_line_id := a0(indx);
          t(ddindx).parent_discount_line_id := a1(indx);
          t(ddindx).offer_id := a2(indx);
          t(ddindx).volume_from := a3(indx);
          t(ddindx).volume_to := a4(indx);
          t(ddindx).volume_operator := a5(indx);
          t(ddindx).volume_type := a6(indx);
          t(ddindx).volume_break_type := a7(indx);
          t(ddindx).discount := a8(indx);
          t(ddindx).discount_type := a9(indx);
          t(ddindx).start_date_active := a10(indx);
          t(ddindx).end_date_active := a11(indx);
    ...
    a14 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        while true loop
          a0(indx) := t(ddindx).offer_discount_line_id;
          a1(indx) := t(ddindx).parent_discount_line_id;
          a2(indx) := t(ddindx).offer_id;
          a3(indx) := t(ddindx).volume_from;
          a4(indx) := t(ddindx).volume_to;
          a5(indx) := t(ddindx).volume_operator;
          a6(indx) := t(ddindx).volume_type;
          a7(indx) := t(ddindx).volume_break_type;
          a8(indx) := t(ddindx).discount;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).act_budget_id := a0(indx);
          t(ddindx).budget_id := a1(indx);
          t(ddindx).budget_amount := a2(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a3 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).act_budget_id;
          a1(indx) := t(ddindx).budget_id;
          a2(indx) := t(ddindx).budget_amount;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).offer_id := a13(indx);
          t(ddindx).discount_by_code := a14(indx);
          t(ddindx).formula_id := a15(indx);
          t(ddindx).offr_disc_struct_name_id := a16(indx);
          t(ddindx).name := a17(indx);
          t(ddindx).description := a18(indx);
          t(ddindx).operation := a19(indx);
          t(ddindx).pbh_index := a20(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
    ...
    a20 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a13(indx) := t(ddindx).offer_id;
          a14(indx) := t(ddindx).discount_by_code;
          a15(indx) := t(ddindx).formula_id;
          a16(indx) := t(ddindx).offr_disc_struct_name_id;
          a17(indx) := t(ddindx).name;
          a18(indx) := t(ddindx).description;
          a19(indx) := t(ddindx).operation;
          a20(indx) := t(ddindx).pbh_index;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).offer_discount_line_id := a2(indx);
          t(ddindx).offer_id := a3(indx);
          t(ddindx).object_version_number := a4(indx);
    ...
    a11 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a2(indx) := t(ddindx).offer_discount_line_id;
          a3(indx) := t(ddindx).offer_id;
          a4(indx) := t(ddindx).object_version_number;
  ...
  begin
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p17(ddp_modifier_line_tbl, p8_a0
      , p8_a1
      , p8_a2
      , p8_a3
      , p8_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p19(ddp_qualifier_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p14(ddp_budget_tbl, p10_a0
      , p10_a1
      , p10_a2
      , p10_a3
      );

    ozf_offer_pub_oaw.rosetta_table_copy_in_p2(ddp_act_product_tbl, p11_a0
      , p11_a1
      , p11_a2
      , p11_a3
      , p11_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p4(ddp_discount_tbl, p12_a0
      , p12_a1
      , p12_a2
      , p12_a3
      , p12_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p8(ddp_excl_tbl, p13_a0
      , p13_a1
      , p13_a2
      , p13_a3
      , p13_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p10(ddp_offer_tier_tbl, p14_a0
      , p14_a1
      , p14_a2
      , p14_a3
      , p14_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p6(ddp_prod_tbl, p15_a0
      , p15_a1
      , p15_a2
      , p15_a3
      , p15_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p12(ddp_na_qualifier_tbl, p16_a0
      , p16_a1
      , p16_a2
      , p16_a3
      , p16_a4
  ...
  begin
    ...
    ddp_modifier_list_rec.offer_id := p6_a0;
    ddp_modifier_list_rec.qp_list_header_id := p6_a1;
    ddp_modifier_list_rec.offer_type := p6_a2;
    ddp_modifier_list_rec.offer_code := p6_a3;
    ddp_modifier_list_rec.activity_media_id := p6_a4;
    ddp_modifier_list_rec.reusable := p6_a5;
    ddp_modifier_list_rec.user_status_id := p6_a6;
    ddp_modifier_list_rec.owner_id := p6_a7;
    ddp_modifier_list_rec.wf_item_key := p6_a8;
    ddp_modifier_list_rec.customer_reference := p6_a9;
    ddp_modifier_list_rec.buying_group_contact_id := p6_a10;
    ddp_modifier_list_rec.object_version_number := p6_a11;
    ddp_modifier_list_rec.perf_date_from := p6_a12;
    ddp_modifier_list_rec.perf_date_to := p6_a13;
    ddp_modifier_list_rec.status_code := p6_a14;
    ddp_modifier_list_rec.status_date := p6_a15;
    ddp_modifier_list_rec.modifier_level_code := p6_a16;
    ddp_modifier_list_rec.order_value_discount_type := p6_a17;
    ddp_modifier_list_rec.lumpsum_amount := p6_a18;
    ddp_modifier_list_rec.lumpsum_payment_type := p6_a19;
    ddp_modifier_list_rec.custom_setup_id := p6_a20;
    ddp_modifier_list_rec.offer_amount := p6_a21;
    ddp_modifier_list_rec.budget_amount_tc := p6_a22;
    ddp_modifier_list_rec.budget_amount_fc := p6_a23;
    ...
    ddp_modifier_list_rec.start_date_active := p6_a43;
    ddp_modifier_list_rec.end_date_active := p6_a44;
    ...
    ddp_modifier_list_rec.start_date_active_first := p6_a51;
    ddp_modifier_list_rec.end_date_active_first := p6_a52;
    ddp_modifier_list_rec.active_date_first_type := p6_a53;
    ddp_modifier_list_rec.start_date_active_second := p6_a54;
    ddp_modifier_list_rec.end_date_active_second := p6_a55;
    ddp_modifier_list_rec.active_date_second_type := p6_a56;
    ddp_modifier_list_rec.active_flag := p6_a57;
    ddp_modifier_list_rec.max_no_of_uses := p6_a58;
    ddp_modifier_list_rec.budget_source_id := p6_a59;
    ddp_modifier_list_rec.budget_source_type := p6_a60;
    ddp_modifier_list_rec.offer_used_by_id := p6_a61;
    ddp_modifier_list_rec.offer_used_by := p6_a62;
    ddp_modifier_list_rec.ql_qualifier_type := p6_a63;
    ddp_modifier_list_rec.ql_qualifier_id := p6_a64;
    ddp_modifier_list_rec.distribution_type := p6_a65;
    ddp_modifier_list_rec.amount_limit_id := p6_a66;
    ddp_modifier_list_rec.uses_limit_id := p6_a67;
    ...
    ddp_modifier_list_rec.na_rule_header_id := p6_a79;
    ddp_modifier_list_rec.sales_method_flag := p6_a80;
    ddp_modifier_list_rec.global_flag := p6_a81;
    ddp_modifier_list_rec.orig_org_id := p6_a82;

    ozf_offer_pub_oaw.rosetta_table_copy_in_p21(ddp_vo_pbh_tbl, p7_a0
      , p7_a1
      , p7_a2
      , p7_a3
      , p7_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p21(ddp_vo_dis_tbl, p8_a0
      , p8_a1
      , p8_a2
      , p8_a3
      , p8_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p23(ddp_vo_prod_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p19(ddp_qualifier_tbl, p10_a0
      , p10_a1
      , p10_a2
      , p10_a3
      , p10_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p25(ddp_vo_mo_tbl, p11_a0
      , p11_a1
      , p11_a2
      , p11_a3
      , p11_a4
    ...
    ozf_offer_pub_oaw.rosetta_table_copy_in_p14(ddp_budget_tbl, p12_a0
      , p12_a1
      , p12_a2
      , p12_a3
      );
