--------------------------------------------------------
--  DDL for Package Body HR_LOT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_LOT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package              varchar2(33)	:= '  hr_lot_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  -- Update the hr_locations_all_tl Row
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy hr_lot_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    p_rec.source_lang :=
    hr_lot_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.location_code = hr_api.g_varchar2) then
    p_rec.location_code :=
    hr_lot_shd.g_old_rec.location_code;
  End If;
  If (p_rec.description = hr_api.g_varchar2) then
    p_rec.description :=
    hr_lot_shd.g_old_rec.description;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
begin
   ...
   --
   l_business_group_id := hr_lot_shd.return_value_business_group_id
