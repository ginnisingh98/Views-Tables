--------------------------------------------------------
--  DDL for Package Body BEN_PCD_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCD_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcd_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_per_cm_prvdd_f Row
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_pcd_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.per_cm_prvdd_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pcd_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.rqstd_flag = hr_api.g_varchar2) then
    p_rec.rqstd_flag :=
    ben_pcd_shd.g_old_rec.rqstd_flag;
  End If;
  If (p_rec.inspn_rqd_flag = hr_api.g_varchar2) then
    p_rec.inspn_rqd_flag :=
    ben_pcd_shd.g_old_rec.inspn_rqd_flag;
  End If;
  If (p_rec.resnd_rsn_cd = hr_api.g_varchar2) then
    p_rec.resnd_rsn_cd :=
    ben_pcd_shd.g_old_rec.resnd_rsn_cd;
  End If;
  If (p_rec.resnd_cmnt_txt = hr_api.g_varchar2) then
    p_rec.resnd_cmnt_txt :=
    ben_pcd_shd.g_old_rec.resnd_cmnt_txt;
  End If;
  If (p_rec.per_cm_prvdd_stat_cd = hr_api.g_varchar2) then
    p_rec.per_cm_prvdd_stat_cd :=
    ben_pcd_shd.g_old_rec.per_cm_prvdd_stat_cd;
  End If;
  If (p_rec.cm_dlvry_med_cd = hr_api.g_varchar2) then
    p_rec.cm_dlvry_med_cd :=
    ben_pcd_shd.g_old_rec.cm_dlvry_med_cd;
  End If;
  If (p_rec.cm_dlvry_mthd_cd = hr_api.g_varchar2) then
    p_rec.cm_dlvry_mthd_cd :=
    ben_pcd_shd.g_old_rec.cm_dlvry_mthd_cd;
  End If;
  If (p_rec.sent_dt = hr_api.g_date) then
    p_rec.sent_dt :=
    ben_pcd_shd.g_old_rec.sent_dt;
  End If;
  If (p_rec.instnc_num = hr_api.g_number) then
    p_rec.instnc_num :=
    ben_pcd_shd.g_old_rec.instnc_num;
  End If;
  If (p_rec.to_be_sent_dt = hr_api.g_date) then
    p_rec.to_be_sent_dt :=
    ben_pcd_shd.g_old_rec.to_be_sent_dt;
  End If;
  If (p_rec.dlvry_instn_txt = hr_api.g_varchar2) then
    p_rec.dlvry_instn_txt :=
    ben_pcd_shd.g_old_rec.dlvry_instn_txt;
  End If;
  If (p_rec.per_cm_id = hr_api.g_number) then
    p_rec.per_cm_id :=
    ben_pcd_shd.g_old_rec.per_cm_id;
  End If;
  If (p_rec.address_id = hr_api.g_number) then
    p_rec.address_id :=
    ben_pcd_shd.g_old_rec.address_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_pcd_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pcd_attribute_category = hr_api.g_varchar2) then
    p_rec.pcd_attribute_category :=
    ben_pcd_shd.g_old_rec.pcd_attribute_category;
  End If;
  If (p_rec.pcd_attribute1 = hr_api.g_varchar2) then
    p_rec.pcd_attribute1 :=
    ben_pcd_shd.g_old_rec.pcd_attribute1;
  End If;
  If (p_rec.pcd_attribute2 = hr_api.g_varchar2) then
    p_rec.pcd_attribute2 :=
    ben_pcd_shd.g_old_rec.pcd_attribute2;
  End If;
  If (p_rec.pcd_attribute3 = hr_api.g_varchar2) then
    p_rec.pcd_attribute3 :=
    ben_pcd_shd.g_old_rec.pcd_attribute3;
  End If;
  If (p_rec.pcd_attribute4 = hr_api.g_varchar2) then
    p_rec.pcd_attribute4 :=
    ben_pcd_shd.g_old_rec.pcd_attribute4;
  End If;
  If (p_rec.pcd_attribute5 = hr_api.g_varchar2) then
    p_rec.pcd_attribute5 :=
    ben_pcd_shd.g_old_rec.pcd_attribute5;
  End If;
  If (p_rec.pcd_attribute6 = hr_api.g_varchar2) then
    p_rec.pcd_attribute6 :=
    ben_pcd_shd.g_old_rec.pcd_attribute6;
  End If;
  If (p_rec.pcd_attribute7 = hr_api.g_varchar2) then
    p_rec.pcd_attribute7 :=
    ben_pcd_shd.g_old_rec.pcd_attribute7;
  End If;
  If (p_rec.pcd_attribute8 = hr_api.g_varchar2) then
    p_rec.pcd_attribute8 :=
    ben_pcd_shd.g_old_rec.pcd_attribute8;
  End If;
  If (p_rec.pcd_attribute9 = hr_api.g_varchar2) then
    p_rec.pcd_attribute9 :=
    ben_pcd_shd.g_old_rec.pcd_attribute9;
  End If;
  If (p_rec.pcd_attribute10 = hr_api.g_varchar2) then
    p_rec.pcd_attribute10 :=
    ben_pcd_shd.g_old_rec.pcd_attribute10;
  End If;
  If (p_rec.pcd_attribute11 = hr_api.g_varchar2) then
    p_rec.pcd_attribute11 :=
    ben_pcd_shd.g_old_rec.pcd_attribute11;
  End If;
  If (p_rec.pcd_attribute12 = hr_api.g_varchar2) then
    p_rec.pcd_attribute12 :=
    ben_pcd_shd.g_old_rec.pcd_attribute12;
  End If;
  If (p_rec.pcd_attribute13 = hr_api.g_varchar2) then
    p_rec.pcd_attribute13 :=
    ben_pcd_shd.g_old_rec.pcd_attribute13;
  End If;
  If (p_rec.pcd_attribute14 = hr_api.g_varchar2) then
    p_rec.pcd_attribute14 :=
    ben_pcd_shd.g_old_rec.pcd_attribute14;
  End If;
  If (p_rec.pcd_attribute15 = hr_api.g_varchar2) then
    p_rec.pcd_attribute15 :=
    ben_pcd_shd.g_old_rec.pcd_attribute15;
  End If;
  If (p_rec.pcd_attribute16 = hr_api.g_varchar2) then
    p_rec.pcd_attribute16 :=
    ben_pcd_shd.g_old_rec.pcd_attribute16;
  End If;
  If (p_rec.pcd_attribute17 = hr_api.g_varchar2) then
    p_rec.pcd_attribute17 :=
    ben_pcd_shd.g_old_rec.pcd_attribute17;
  End If;
  If (p_rec.pcd_attribute18 = hr_api.g_varchar2) then
    p_rec.pcd_attribute18 :=
    ben_pcd_shd.g_old_rec.pcd_attribute18;
  End If;
  If (p_rec.pcd_attribute19 = hr_api.g_varchar2) then
    p_rec.pcd_attribute19 :=
    ben_pcd_shd.g_old_rec.pcd_attribute19;
  End If;
  If (p_rec.pcd_attribute20 = hr_api.g_varchar2) then
    p_rec.pcd_attribute20 :=
    ben_pcd_shd.g_old_rec.pcd_attribute20;
  End If;
  If (p_rec.pcd_attribute21 = hr_api.g_varchar2) then
    p_rec.pcd_attribute21 :=
    ben_pcd_shd.g_old_rec.pcd_attribute21;
  End If;
  If (p_rec.pcd_attribute22 = hr_api.g_varchar2) then
    p_rec.pcd_attribute22 :=
    ben_pcd_shd.g_old_rec.pcd_attribute22;
  End If;
  If (p_rec.pcd_attribute23 = hr_api.g_varchar2) then
    p_rec.pcd_attribute23 :=
    ben_pcd_shd.g_old_rec.pcd_attribute23;
  End If;
  If (p_rec.pcd_attribute24 = hr_api.g_varchar2) then
    p_rec.pcd_attribute24 :=
    ben_pcd_shd.g_old_rec.pcd_attribute24;
  End If;
  If (p_rec.pcd_attribute25 = hr_api.g_varchar2) then
    p_rec.pcd_attribute25 :=
    ben_pcd_shd.g_old_rec.pcd_attribute25;
  End If;
  If (p_rec.pcd_attribute26 = hr_api.g_varchar2) then
    p_rec.pcd_attribute26 :=
    ben_pcd_shd.g_old_rec.pcd_attribute26;
  End If;
  If (p_rec.pcd_attribute27 = hr_api.g_varchar2) then
    p_rec.pcd_attribute27 :=
    ben_pcd_shd.g_old_rec.pcd_attribute27;
  End If;
  If (p_rec.pcd_attribute28 = hr_api.g_varchar2) then
    p_rec.pcd_attribute28 :=
    ben_pcd_shd.g_old_rec.pcd_attribute28;
  End If;
  If (p_rec.pcd_attribute29 = hr_api.g_varchar2) then
    p_rec.pcd_attribute29 :=
    ben_pcd_shd.g_old_rec.pcd_attribute29;
  End If;
  If (p_rec.pcd_attribute30 = hr_api.g_varchar2) then
    p_rec.pcd_attribute30 :=
    ben_pcd_shd.g_old_rec.pcd_attribute30;
  End If;
   If (p_rec.request_id = hr_api.g_number) then
    p_rec.request_id :=
    ben_pcd_shd.g_old_rec.request_id;
  End If;
  If (p_rec.program_application_id = hr_api.g_number) then
    p_rec.program_application_id :=
    ben_pcd_shd.g_old_rec.program_application_id;
  End If;
  If (p_rec.program_id = hr_api.g_number) then
    p_rec.program_id :=
    ben_pcd_shd.g_old_rec.program_id;
  End If;
