--------------------------------------------------------
--  DDL for Package Body OE_CONFIG_VALIDATION_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_CONFIG_VALIDATION_PVT" AS
/* $Header: OEXVBOMB.pls 120.5 2005/10/19 09:14 abalan noship $ */

G_PKG_NAME      CONSTANT    VARCHAR2(30):='OE_CONFIG_VALIDATION_PVT';
G_Ship_From_Org_Id          NUMBER;  -- 4319370
...
BEGIN
  ...
  BEGIN
    -- 4319370
    SELECT component_sequence_id, ordered_quantity,ship_from_org_id
    INTO   l_top_bill_sequence_id, l_model_qty, G_Ship_From_Org_Id
    FROM   oe_order_lines
...
BEGIN
  ...
  BEGIN
    ...
    SELECT open_flag
    INTO l_open_flag
    FROM oe_order_lines_all /* MOAC_SQL_CHANGE */
    where line_id =
    (SELECT line_id
     FROM   oe_order_lines
  ...
  LOOP
    ...
    IF p_options_tbl(I).bom_item_type = 2 THEN
      ...
      ELSE
       ...
       -- 4319370 Make sure the original item type is also option class
        l_inventory_item_id := substr( p_options_tbl(I).component_code, instr(p_options_tbl(I).component_code, '-', -1)+1);
       --   Load Item to check the original item type
          OE_Order_Cache.Load_Item (l_inventory_item_id
                                      ,g_ship_from_org_id);

       IF OE_ORDER_CACHE.g_item_rec.bom_item_type = 2 THEN
      ...
      END IF; -- if qty = 0
      END IF ; --if original is not model
    END IF; -- if a class
