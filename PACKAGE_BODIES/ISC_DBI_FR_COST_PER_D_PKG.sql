--------------------------------------------------------
--  DDL for Package Body ISC_DBI_FR_COST_PER_D_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_FR_COST_PER_D_PKG" AS
/*$Header: ISCRGC1B.pls 120.1 2006/06/14 12:17:44 abhdixi noship $
    /*----------------------------------------------------
    ...
    /* Rated Freight Cost per Unit Distance Report */
    ...
    /*------------------------------------------------
    Rated Freight Cost per Unit Distance Report Function
    -------------------------------------------------*/
...
--   l_filter_where  := ' ISC_MEASURE_4 IS NOT NULL AND ISC_MEASURE_1 IS NOT NULL';
  ...
  END;
	...
	nvl(c_freight_distance_total,0) 	ISC_MEASURE_15,
            ...
            ' || isc_dbi_sutil_pkg.change_str (
                    p_new_numerator     => 'c_freight_distance_total',
                    p_old_numerator     => 'p_freight_distance_total',
                    p_denominator       => 'p_freight_distance_total',
                    p_measure_name      => 'ISC_MEASURE_16') || ', -- Grand Total Rated Fr Distance Change
            ' || isc_dbi_sutil_pkg.rate_str (
                    p_numerator     => 'c_freight_distance',
                    p_denominator   => 'c_freight_distance_total',
                    p_rate_type     => 'PERCENT',
                    p_measure_name  => 'ISC_MEASURE_6') || ', -- Percent of Total Fr Distance
            ' || isc_dbi_sutil_pkg.rate_str (
                    p_numerator     => 'c_freight_cost_total',
                    p_denominator   => 'c_freight_cost_total',
                    p_rate_type     => 'PERCENT',
            ...
            ' || isc_dbi_sutil_pkg.rate_str (
                    ...
                    p_denominator   => 'c_freight_distance_total',
                    p_rate_type     => 'RATIO',
                    p_measure_name  => 'ISC_MEASURE_18') || ', -- Grand Total Fr Cost per Distance
            ...
            ' || isc_dbi_sutil_pkg.change_rate_str (
                    p_new_numerator     => 'c_freight_cost_total',
                    p_new_denominator   => 'c_freight_distance_total',
                    p_old_numerator     => 'p_freight_cost_total',
                    p_old_denominator   => 'p_freight_distance_total',
                    p_rate_type         => 'RATIO',
                    p_measure_name      => 'ISC_MEASURE_19'); -- Grand Total Fr Cost per Distance Change
