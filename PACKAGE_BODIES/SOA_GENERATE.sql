--------------------------------------------------------
--  DDL for Package Body SOA_GENERATE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."SOA_GENERATE" as
/* $Header: SOAGENRB.pls 120.2.12010000.2 2009/08/13 12:43:16 abhverma ship $ */


procedure create_class_entry_detail
	(
...
procedure create_function_entry_detail
	(
...
/**
Procedure for creating a Derived Entry for a given Base Entry Interface
**/
...
begin
	...
	loop
		...
		begin
	                create_function_derived_entry(p_base_class_id,c1.function_id,p_entry_type,x_derived_class_id,p_err_code,p_err_message);
		exception
...
end create_class_derived_entry;


/**
Procedure for creating a Derived Entry for all functions of a base class Entry
**/
		...
		if c_base_function_entry%FOUND then
			...
			if ( c_base_class_entry_rec.scope_type = 'PUBLIC' AND c_base_function_entry_rec.irep_scope = 'PUBLIC' ) then
				...
				if ( c_base_class_entry_rec.class_type = 'XMLGATEWAY' ) then
					...
					Derived
					p_entry_type:oracle.apps.ar.Ar.ProcessInvoice			oracle.apps.ar.Ar

					Function Name
					p_entry_type:oracle.apps.ar.Ar.ProcessInvoice.ProcessInvoice	oracle.apps.ar.Ar.ProcessInvoice
					**/
					...
					if ( i_delimitor <> 0 ) then
						...
						create_class_entry_detail
						(
						c_base_class_entry_rec.class_id,
						p_entry_type ||':'||i_derived_class_name,
						...
						**/
						...
						create_function_entry_detail
						(
						p_entry_type ||':'||i_derived_class_name||':'||i_operation,
				...
				elsif ( c_base_class_entry_rec.class_type = 'SERVICEBEAN' or c_base_class_entry_rec.class_type = 'WSDL' or
				     c_base_class_entry_rec.class_type = 'PLSQL' or c_base_class_entry_rec.class_type = 'CONCURRENTPROGRAM' or
				     c_base_class_entry_rec.class_type = 'EDI' or c_base_class_entry_rec.class_type = 'JAVA' ) then
...
when others then
	...
	raise program_exit;
end create_function_derived_entry;
...
is
	cursor c_derived_class(c_class_name in varchar, c_entry_type in varchar) is
	select *
	from fnd_irep_classes
...
begin
	...
	open c_derived_class(p_class_name,p_entry_type);
...
is
	cursor c_derived_method(c_function_name in varchar) is
	select *
	from fnd_form_functions
...
begin
	open c_derived_method(p_function_name);
...
when others then
	...
	raise program_exit;
end create_function_entry_detail;
...
is
	...
	cursor c_object(c_object_id in number) is
 	select count(*) l_count
 	from fnd_objects
 	where object_id = c_object_id;


        cursor c_object_name(c_object_id in number) is
        select obj_name ,irep_name
        from fnd_objects
        where object_id = c_object_id;

 	c_object_rec c_object%ROWTYPE;

	cursor 	c_methods(c_object_id in number) is
	select function_id
	from fnd_form_functions
	where irep_class_id = c_object_id ;

	method_count pls_integer := 0;
	p_base_object_name varchar2(430) := null;
	x_irep_name varchar2(500) := null;


begin

	if (p_base_object_id is null ) then
		p_err_code := -1;
		p_err_message := wf_core.translate('WF_WS_MISSING_CLASS_ID');
		raise program_exit;
	end if;
	...
	if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
		FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create_object_derived_entry', 'Process Object ID='||p_base_object_id);
	end if;

	open c_object(p_base_object_id);
	fetch c_object into c_object_rec;
	close c_object;

        open c_object_name(p_base_object_id);
        fetch c_object_name into p_base_object_name,x_irep_name;
        close c_object_name;

	if c_object_rec.l_count = 0 then
		p_err_code := -1;
	...
	for c1 in c_methods(p_base_object_id)
	loop
		method_count := method_count + 1;


		if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
			FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create_object_derived_entry', 'Processing FunctionID='||c1.function_id);
		end if;


		begin
	                create_obj_func_derived_entry(p_base_object_id,c1.function_id,p_entry_type,x_derived_class_id,p_err_code,p_err_message);
		exception
		when ignore_rec  then
			null;
		when program_exit then
		...
		when others then
			...
			--wf_core.token('BaseClassId',p_base_object_id);
                        --wf_core.token('BaseClassName', p_base_object_name);
			--wf_core.token('FunctionId',c1.function_id);
			--wf_core.token('SqlErr',SQLERRM);
			p_err_message := wf_core.translate('WF_WS_CLASS_FUNC_ITER');
			raise program_exit;
...
when others then
	...
	wf_core.token('SqlErr',SQLERRM);
	p_err_message := wf_core.translate('WF_WS_CLASS_DERIVED');
end create_object_derived_entry;

/**
Procedure for creating a Derived Entry for all functions of a base Object Entry
Base entry is in fnd_objects and its derived entry would be made in fnd_irep_classes
**/
...
is
	...
	cursor c_base_object_entry(c_base_object_id in number) is
	select 	*
	from fnd_objects_vl
	where object_id = c_base_object_id;

	c_base_object_entry_rec c_base_object_entry%ROWTYPE;
...
begin
	...
	**/
	...
	if ( (p_base_object_id is null) or (p_base_function_id is null)) then
		p_err_code := -1;
		p_err_message := wf_core.translate('WF_WS_MISSING_CLASS_FUNC_ID');
		raise program_exit;
	end if;
	...
	/*Get the Base Entry object**/
	open c_base_object_entry(p_base_object_id);
	fetch c_base_object_entry into c_base_object_entry_rec;

	if c_base_object_entry%NOTFOUND then

		if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
			FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create.obj.function.derived.entry', 'Base Object ID='||p_base_object_id||' does not exist');
		end if;
	...
	elsif c_base_object_entry%FOUND then


		if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
			FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create.obj.function.derived.entry', 'Base object ID='||p_base_object_id||' exists');
		end if;
		...
		if c_base_function_entry%FOUND then
			...
			if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
				FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create.obj.function.derived.entry', 'Base Function='||p_base_function_id||' exist');
			end if;

			if ( c_base_function_entry_rec.irep_class_id <> p_base_object_id ) then
				p_err_code :=-1;
				p_err_message := wf_core.translate('WF_WS_CLASS_FUNC_MISMATCH');
				raise program_exit;
			end if;

			if ( c_base_object_entry_rec.irep_scope = 'PUBLIC' AND c_base_function_entry_rec.irep_scope = 'PUBLIC' ) then



				if ( c_base_object_entry_rec.irep_object_type = 'INTERFACETABLE' or c_base_object_entry_rec.irep_object_type = 'INTERFACEVIEW') then

					if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
						FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create.obj.function.derived.entry', 'OBJECT Type:INTERFACETABLE/INTERFACEVIEW ');
					end if;

					i_function_name := c_base_object_entry_rec.irep_name;
					...
					create_class_entry_detail
					(
					c_base_object_entry_rec.object_id,
					p_entry_type||':'||i_derived_class_name,
					i_derived_class_name,
					c_base_object_entry_rec.created_by,
					c_base_object_entry_rec.creation_date,
					null,
					p_entry_type,
					c_base_object_entry_rec.irep_product,
					null,
					'N',
					'N',
					...
					c_base_object_entry_rec.irep_source_file_product,
					c_base_object_entry_rec.irep_source_file_path,
					c_base_object_entry_rec.irep_source_file_name,
					c_base_object_entry_rec.irep_source_file_version,
					c_base_object_entry_rec.irep_description,
					c_base_object_entry_rec.irep_xml_description,
					c_base_object_entry_rec.irep_standard,
					c_base_object_entry_rec.irep_standard_version,
					c_base_object_entry_rec.irep_standard_spec,
					null,
					null,
					null,
                                        null,
				...
				if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
					FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create.obj.function.derived.entry', 'ObjectID,FunctionID:'
					||c_base_object_entry_rec.object_id||','||c_base_function_entry_rec.function_id||' Not a public interface/method to be exposed');
				end if;
			...
			if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
				FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create.obj.function.derived.entry', ' No records found for FunctionID='||c_base_function_entry_rec.function_id);
			end if;
...
when others then
	...
	p_err_message :=wf_core.translate('WF_WS_GENERIC_CLASS_ERROR')||'Error in Creating Derived Entry for Base FUNCTION_ID='||p_base_function_id||SQLERRM;
	raise program_exit;
end create_obj_func_derived_entry;
...
begin
	...
	if c_base_object_entry%NOTFOUND then
		...
		if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
			FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create_object_function_base_entry', 'Base Object ID='||p_base_object_id||' does not exist');
		end if;
		...
		if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
			FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create_object_function_base_entry', 'Base Object ID='||p_base_object_id||' exists');
		end if;


		if ( c_base_object_entry_rec.irep_scope = 'PUBLIC' ) then


			if ( c_base_object_entry_rec.irep_object_type = 'INTERFACETABLE' or c_base_object_entry_rec.irep_object_type = 'INTERFACEVIEW') then

				if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
					FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create_object_function_base_entry', 'Object Type:INTERFACETABLE/INTERFACEVIEW ');
				end if;


				--dbms_output.put_line('Function ='||p_function_name);
				if ( (p_function_name is null ) or (length(p_function_name) is NULL ) ) then
					p_err_code := -1;
					p_err_message :=wf_core.translate('WF_WS_MISSING_IREP_METHOD_NAME');
					raise program_exit;
				end if;
				...
				--todo find if functionLanguage entry is mandatory.

			end if;
		else
			if( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL ) then
				FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'oracle.apps.fnd.wf.ws.create_object_function_base_entry', 'objectID:'||c_base_object_entry_rec.object_id||' Not a public interface to be exposed');
			end if;
	...
	p_err_code := 0;
	-- todo create new tokens for successful message.
	--wf_core.token('BaseClassId',p_base_object_id);
	--p_err_message :=wf_core.translate('WF_WS_SUCCESS');
...
when others then
	p_err_code := -1;
	p_err_message :=wf_core.translate('WF_WS_GENERIC_object_ERROR')||'Error in Creating Base Entry for Base object_ID='||p_base_object_id||SQLERRM;
	raise program_exit;
end create_obj_function_base_entry;
