--------------------------------------------------------
--  DDL for Package Body BEN_PCX_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCX_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcx_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_pl_bnf_ctfn_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   pl_bnf_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_pl_bnf_ctfn_id(p_pl_bnf_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pl_bnf_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_pcx_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pl_bnf_ctfn_id                => p_pl_bnf_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pl_bnf_ctfn_id,hr_api.g_number)
     <>  ben_pcx_shd.g_old_rec.pl_bnf_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_pcx_shd.constraint_error('BEN_PL_BNF_CTFN_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pl_bnf_ctfn_id is not null then
      --
  ...
  --
End chk_pl_bnf_ctfn_id;
--
-- ----------------------------------------------------------------------------
-- |---------------< chk_bnf_ctfn_typ_cd_unique >------------------------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--     p_bnf_ctfn_typ_cd       is dependent certification type code
--     p_rlshp_typ_cd          is relationship type code
--     p_pl_bnf_ctfn_id        is primary key for ben_pl_bnf_ctfn_f
--     p_pl_id
--     p_business_group_id
...
--
-- ----------------------------------------------------------------------------
Procedure chk_bnf_ctfn_typ_cd_unique
...
is
l_proc      varchar2(72) := g_package||'chk_bnf_ctfn_typ_cd_unique';
l_dummy    char(1);
cursor c1 is select null
               from ben_pl_bnf_ctfn_f
              Where pl_bnf_ctfn_id <> nvl(p_pl_bnf_ctfn_id,-1)
                and nvl(bnf_ctfn_typ_cd,0) = nvl(p_bnf_ctfn_typ_cd,0)
...
End chk_bnf_ctfn_typ_cd_unique;
--
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_pl_bnf_ctfn_id                in number,
  ...
  if (l_api_updating
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        fnd_message.set_token('TYPE_ID',-25);
        fnd_message.raise_error;
        --
      end if;
      --
...
--   Internal table handler use only.
--
Procedure chk_pfd_flag(p_pl_bnf_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_pfd_flag
      <> nvl(ben_pcx_shd.g_old_rec.pfd_flag,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_lack_ctfn_sspnd_enrt_flag(p_pl_bnf_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_pcx_shd.g_old_rec.lack_ctfn_sspnd_enrt_flag,hr_api.g_varchar2)
      or not l_api_updating)
...
-- In Parameters
--   pl_bnf_ctfn_id PK of record being inserted or updated.
--   bnf_ctfn_typ_cd Value of lookup code.
--   effective_date effective date
--   object_version_number Object version number of record being
--                         inserted or updated.
--
...
--   Internal table handler use only.
--
Procedure chk_bnf_ctfn_typ_cd(p_pl_bnf_ctfn_id                in number,
                            p_bnf_ctfn_typ_cd               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_bnf_ctfn_typ_cd';
  l_api_updating boolean;
  --
Begin
  --
  ...
  --
  if (l_api_updating
      and p_bnf_ctfn_typ_cd
      <> nvl(ben_pcx_shd.g_old_rec.bnf_ctfn_typ_cd,hr_api.g_varchar2)
      or not l_api_updating)
      and p_bnf_ctfn_typ_cd is not null then
    --
    -- check if value of lookup falls within lookup type.
    --
    if hr_api.not_exists_in_hr_lookups
          (p_lookup_type    => 'BEN_BNF_CTFN_TYP',
           p_lookup_code    => p_bnf_ctfn_typ_cd,
           p_effective_date => p_effective_date) then
      --
      -- raise error as does not exist as lookup
      --
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    hr_api.mandatory_arg_error
      (p_api_name       => l_proc,
       p_argument       => 'pl_bnf_ctfn_id',
       p_argument_value => p_pl_bnf_ctfn_id);
    --
    --
    --
  End If;
...
function return_legislation_code
  (p_pl_bnf_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pl_bnf_ctfn_f b
    where b.pl_bnf_ctfn_id      = p_pl_bnf_ctfn_id
