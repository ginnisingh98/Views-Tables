--------------------------------------------------------
--  DDL for Package Body HZ_PARTY_SITES_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_PARTY_SITES_EXT_PKG" as
/* $Header: ARHEXPSB.pls 120.4 2006/02/09 10:17:54 jhuang noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from HZ_PARTY_SITES_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into HZ_PARTY_SITES_EXT_B (
    EXTENSION_ID,
    PARTY_SITE_ID,
    ATTR_GROUP_ID,
    C_EXT_ATTR1,
  ...
  insert into HZ_PARTY_SITES_EXT_TL (
    TL_EXT_ATTR2,
    TL_EXT_ATTR3,
    TL_EXT_ATTR4,
    TL_EXT_ATTR5,
...
begin
  ...
  close c;
  if (    (recinfo.PARTY_SITE_ID = X_PARTY_SITE_ID)
...
begin
  update HZ_PARTY_SITES_EXT_B set
  ...
  update HZ_PARTY_SITES_EXT_TL set
...
begin
  delete from HZ_PARTY_SITES_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  insert into /*+ append parallel(tt) */ HZ_PARTY_SITES_EXT_TL tt (
    TL_EXT_ATTR2,
    TL_EXT_ATTR3,
    TL_EXT_ATTR4,
    TL_EXT_ATTR5,
  ...
  (SELECT /*+ no_merge ordered parallel(b) */
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
   FROM HZ_PARTY_SITES_EXT_TL B ,
        FND_LANGUAGES L
   WHERE L.INSTALLED_FLAG IN ( 'I','B' )
   AND B.LANGUAGE = USERENV ( 'LANG' )
  ) v, HZ_PARTY_SITES_EXT_TL t
