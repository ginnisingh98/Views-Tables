--------------------------------------------------------
--  DDL for Package Body BEN_CTU_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CTU_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ctu_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |--------------------------< chk_cm_typ_usg_id >---------------------------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   cm_typ_usg_id PK of record being inserted or updated.
...
--
Procedure chk_cm_typ_usg_id(p_cm_typ_usg_id               in number,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_cm_typ_usg_id';
...
Begin
  ...
  --
  l_api_updating := ben_ctu_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_cm_typ_usg_id               => p_cm_typ_usg_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_cm_typ_usg_id,hr_api.g_number)
     <>  ben_ctu_shd.g_old_rec.cm_typ_usg_id) then
    --
    -- raise error as PK has changed
    --
    ben_ctu_shd.constraint_error('BEN_CM_TYP_USG_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_cm_typ_usg_id is not null then
      --
...
-- In Parameters
--   cm_typ_usg_id PK of record being inserted or updated.
--   all_r_any_cd Value of lookup code.
...
--   Internal table handler use only.
--
Procedure chk_all_r_any_cd(p_cm_typ_usg_id              in number,
                           p_all_r_any_cd               in varchar2,
                           p_effective_date             in date,
                           p_object_version_number      in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_all_r_any_cd';
  l_api_updating boolean;
  --
Begin
  --
  ...
  if (l_api_updating
      and p_all_r_any_cd
      <> nvl(ben_ctu_shd.g_old_rec.all_r_any_cd,hr_api.g_varchar2)
    ...
    if hr_api.not_exists_in_hr_lookups
          (p_lookup_type    => 'BEN_ALL_R_ANY',
           p_lookup_code    => p_all_r_any_cd,
           p_effective_date => p_effective_date) then
      --
      -- raise error as does not exist as lookup
      --
      fnd_message.set_name('BEN','BEN_91628_LOOKUP_TYPE_GENERIC');
      fnd_message.set_token('FIELD','p_all_r_any');
      fnd_message.set_token('TYPE','BEN_ALL_R_ANY');
...
-- In Parameters
--   cm_typ_usg_id PK of record being inserted or updated.
--   cm_usg_rl Value of formula rule id.
--   effective_date effective date
--   object_version_number Object version number of record being
--                         inserted or updated.
--
...
--   Internal table handler use only.
--
Procedure chk_cm_usg_rl(p_cm_typ_usg_id               in number,
                        p_cm_usg_rl                   in number,
                        p_effective_date              in date,
                        p_business_group_id           in number,
                        p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_cm_usg_rl';
  l_api_updating boolean;
  --
Begin
  --
  ...
  --
  if (l_api_updating
      and nvl(p_cm_usg_rl,hr_api.g_number)
      <> ben_ctu_shd.g_old_rec.cm_usg_rl
      or not l_api_updating)
      and p_cm_usg_rl is not null then
    --
    -- check if value of formula rule is valid.
    --
    if not benutils.formula_exists
         (p_formula_id        => p_cm_usg_rl,
          p_formula_type_id   => -352,
          p_business_group_id => p_business_group_id,
          p_effective_date    => p_effective_date) then
      --
      fnd_message.set_name('BEN','BEN_91471_FORMULA_NOT_FOUND');
      fnd_message.set_token('ID',p_cm_usg_rl);
      fnd_message.set_token('TYPE_ID',-352);
      fnd_message.raise_error;
      --
    end if;
    --
...
function return_legislation_code
  (p_cm_typ_usg_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_cm_typ_usg_f b
    where b.cm_typ_usg_id      = p_cm_typ_usg_id
