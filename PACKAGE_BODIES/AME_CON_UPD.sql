--------------------------------------------------------
--  DDL for Package Body AME_CON_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_CON_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_con_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the ame_conditions Row
    --
    update  ame_conditions
    set
     condition_id                         = p_rec.condition_id
    ,condition_type                       = p_rec.condition_type
    ,attribute_id                         = p_rec.attribute_id
    ,parameter_one                        = p_rec.parameter_one
    ,parameter_two                        = p_rec.parameter_two
    ,parameter_three                      = p_rec.parameter_three
    ,include_lower_limit                  = p_rec.include_lower_limit
    ,include_upper_limit                  = p_rec.include_upper_limit
    ,security_group_id                    = p_rec.security_group_id
    ,condition_key                        = p_rec.condition_key
    ,object_version_number                = p_rec.object_version_number
    where   condition_id  =   p_rec.condition_id
...
Begin
  ...
  begin
    ...
    ame_con_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_condition_type_o
      => ame_con_shd.g_old_rec.condition_type
      ,p_attribute_id_o
      => ame_con_shd.g_old_rec.attribute_id
      ,p_parameter_one_o
      => ame_con_shd.g_old_rec.parameter_one
      ,p_parameter_two_o
      => ame_con_shd.g_old_rec.parameter_two
      ,p_parameter_three_o
      => ame_con_shd.g_old_rec.parameter_three
      ,p_include_lower_limit_o
      => ame_con_shd.g_old_rec.include_lower_limit
      ,p_include_upper_limit_o
      => ame_con_shd.g_old_rec.include_upper_limit
      ,p_start_date_o
      => ame_con_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_con_shd.g_old_rec.end_date
      ,p_security_group_id_o
      => ame_con_shd.g_old_rec.security_group_id
      ,p_condition_key_o
      => ame_con_shd.g_old_rec.condition_key
      ,p_object_version_number_o
      => ame_con_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  --
  If (p_rec.condition_type = hr_api.g_varchar2) then
    p_rec.condition_type :=
    ame_con_shd.g_old_rec.condition_type;
  End If;
  If (p_rec.attribute_id = hr_api.g_number) then
    ...
    ame_con_shd.g_old_rec.attribute_id;
  End If;
  If (p_rec.parameter_one = hr_api.g_varchar2) then
    p_rec.parameter_one :=
    ame_con_shd.g_old_rec.parameter_one;
  End If;
  If (p_rec.parameter_two = hr_api.g_varchar2) then
    p_rec.parameter_two :=
    ame_con_shd.g_old_rec.parameter_two;
  End If;
  If (p_rec.parameter_three = hr_api.g_varchar2) then
    p_rec.parameter_three :=
    ame_con_shd.g_old_rec.parameter_three;
  End If;
  If (p_rec.include_lower_limit = hr_api.g_varchar2) then
    p_rec.include_lower_limit :=
    ame_con_shd.g_old_rec.include_lower_limit;
  End If;
  If (p_rec.include_upper_limit = hr_api.g_varchar2) then
    p_rec.include_upper_limit :=
    ame_con_shd.g_old_rec.include_upper_limit;
  End If;
  If (p_rec.security_group_id = hr_api.g_number) then
    ...
    ame_con_shd.g_old_rec.security_group_id;
  End If;
  If (p_rec.condition_key = hr_api.g_varchar2) then
    p_rec.condition_key :=
    ame_con_shd.g_old_rec.condition_key;
  End If;
  --
...
Begin
  ...
  --
  ame_con_upd.convert_defs(p_rec);
  --
  ame_con_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
Procedure upd
  ...
  ,p_condition_type               in     varchar2  default hr_api.g_varchar2
  ,p_attribute_id                 in     number    default hr_api.g_number
  ,p_condition_key                in     varchar2  default hr_api.g_varchar2
