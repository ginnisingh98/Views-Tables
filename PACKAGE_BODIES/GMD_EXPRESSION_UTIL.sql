--------------------------------------------------------
--  DDL for Package Body GMD_EXPRESSION_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMD_EXPRESSION_UTIL" AS
/* $Header: GMDPEXPB.pls 120.2 2007/11/29 16:49:43 rlnagara ship $ */
...
operator_tab	Operator_Table := Operator_Table('+', '-', '/', '*', '(', ')', 'SUM', 'ISUM', 'BSUM',
  ...
  *********************************************************************************/
  ...
  BEGIN
    ...
    IF is_parameter (P_orgn_id		=> P_orgn_id,
                     P_parameter 	=> P_key,
                     X_parm_id		=> l_parm_id,
  ...
  *********************************************************************************/
  ...
  (     p_orgn_id		IN	   NUMBER	,
    ...
    CURSOR Cur_parameter (V_orgn_id NUMBER, V_parameter VARCHAR2) IS
  ...
  BEGIN
    ...
    ELSE
      ...
      OPEN Cur_parameter (P_orgn_id, P_Parameter);
  ...
  *********************************************************************************/
  ...
  BEGIN
    ...
    WHILE i <= l_table_count LOOP
      ...
      IF l_expression_tab(i).expression_type = 0 THEN
      ...
      ELSIF l_expression_tab(i).expression_type = 2 THEN
        l_start := i + 1;
        l_bracket_count := 0;
        /* Bug No.6181501 - Start*/
        l_exp_tab.delete(1,l_count);
        l_count := 0;
        /* Bug No.6181501 - Start*/
      /*  FOR k in i+2..l_expression_tab.COUNT LOOP */
        FOR k in i+2..l_table_count LOOP /* Bug No.6181501 l_expression_tab.COUNT to l_table_count  */
          l_end := k;
