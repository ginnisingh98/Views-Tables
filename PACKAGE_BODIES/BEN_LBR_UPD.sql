--------------------------------------------------------
--  DDL for Package Body BEN_LBR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LBR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lbr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_ler_bnft_rstrn_f Row
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_lbr_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.ler_id = hr_api.g_number) then
    p_rec.ler_id :=
    ben_lbr_shd.g_old_rec.ler_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_lbr_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_lbr_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.plip_id = hr_api.g_number) then
    p_rec.plip_id :=
    ben_lbr_shd.g_old_rec.plip_id;
  End If;
  If (p_rec.lbr_attribute_category = hr_api.g_varchar2) then
    p_rec.lbr_attribute_category :=
    ben_lbr_shd.g_old_rec.lbr_attribute_category;
  End If;
  If (p_rec.lbr_attribute1 = hr_api.g_varchar2) then
    p_rec.lbr_attribute1 :=
    ben_lbr_shd.g_old_rec.lbr_attribute1;
  End If;
  If (p_rec.lbr_attribute2 = hr_api.g_varchar2) then
    p_rec.lbr_attribute2 :=
    ben_lbr_shd.g_old_rec.lbr_attribute2;
  End If;
  If (p_rec.lbr_attribute3 = hr_api.g_varchar2) then
    p_rec.lbr_attribute3 :=
    ben_lbr_shd.g_old_rec.lbr_attribute3;
  End If;
  If (p_rec.lbr_attribute4 = hr_api.g_varchar2) then
    p_rec.lbr_attribute4 :=
    ben_lbr_shd.g_old_rec.lbr_attribute4;
  End If;
  If (p_rec.lbr_attribute5 = hr_api.g_varchar2) then
    p_rec.lbr_attribute5 :=
    ben_lbr_shd.g_old_rec.lbr_attribute5;
  End If;
  If (p_rec.lbr_attribute6 = hr_api.g_varchar2) then
    p_rec.lbr_attribute6 :=
    ben_lbr_shd.g_old_rec.lbr_attribute6;
  End If;
  If (p_rec.lbr_attribute7 = hr_api.g_varchar2) then
    p_rec.lbr_attribute7 :=
    ben_lbr_shd.g_old_rec.lbr_attribute7;
  End If;
  If (p_rec.lbr_attribute8 = hr_api.g_varchar2) then
    p_rec.lbr_attribute8 :=
    ben_lbr_shd.g_old_rec.lbr_attribute8;
  End If;
  If (p_rec.lbr_attribute9 = hr_api.g_varchar2) then
    p_rec.lbr_attribute9 :=
    ben_lbr_shd.g_old_rec.lbr_attribute9;
  End If;
  If (p_rec.lbr_attribute10 = hr_api.g_varchar2) then
    p_rec.lbr_attribute10 :=
    ben_lbr_shd.g_old_rec.lbr_attribute10;
  End If;
  If (p_rec.lbr_attribute11 = hr_api.g_varchar2) then
    p_rec.lbr_attribute11 :=
    ben_lbr_shd.g_old_rec.lbr_attribute11;
  End If;
  If (p_rec.lbr_attribute12 = hr_api.g_varchar2) then
    p_rec.lbr_attribute12 :=
    ben_lbr_shd.g_old_rec.lbr_attribute12;
  End If;
  If (p_rec.lbr_attribute13 = hr_api.g_varchar2) then
    p_rec.lbr_attribute13 :=
    ben_lbr_shd.g_old_rec.lbr_attribute13;
  End If;
  If (p_rec.lbr_attribute14 = hr_api.g_varchar2) then
    p_rec.lbr_attribute14 :=
    ben_lbr_shd.g_old_rec.lbr_attribute14;
  End If;
  If (p_rec.lbr_attribute15 = hr_api.g_varchar2) then
    p_rec.lbr_attribute15 :=
    ben_lbr_shd.g_old_rec.lbr_attribute15;
  End If;
  If (p_rec.lbr_attribute16 = hr_api.g_varchar2) then
    p_rec.lbr_attribute16 :=
    ben_lbr_shd.g_old_rec.lbr_attribute16;
  End If;
  If (p_rec.lbr_attribute17 = hr_api.g_varchar2) then
    p_rec.lbr_attribute17 :=
    ben_lbr_shd.g_old_rec.lbr_attribute17;
  End If;
  If (p_rec.lbr_attribute18 = hr_api.g_varchar2) then
    p_rec.lbr_attribute18 :=
    ben_lbr_shd.g_old_rec.lbr_attribute18;
  End If;
  If (p_rec.lbr_attribute19 = hr_api.g_varchar2) then
    p_rec.lbr_attribute19 :=
    ben_lbr_shd.g_old_rec.lbr_attribute19;
  End If;
  If (p_rec.lbr_attribute20 = hr_api.g_varchar2) then
    p_rec.lbr_attribute20 :=
    ben_lbr_shd.g_old_rec.lbr_attribute20;
  End If;
  If (p_rec.lbr_attribute21 = hr_api.g_varchar2) then
    p_rec.lbr_attribute21 :=
    ben_lbr_shd.g_old_rec.lbr_attribute21;
  End If;
  If (p_rec.lbr_attribute22 = hr_api.g_varchar2) then
    p_rec.lbr_attribute22 :=
    ben_lbr_shd.g_old_rec.lbr_attribute22;
  End If;
  If (p_rec.lbr_attribute23 = hr_api.g_varchar2) then
    p_rec.lbr_attribute23 :=
    ben_lbr_shd.g_old_rec.lbr_attribute23;
  End If;
  If (p_rec.lbr_attribute24 = hr_api.g_varchar2) then
    p_rec.lbr_attribute24 :=
    ben_lbr_shd.g_old_rec.lbr_attribute24;
  End If;
  If (p_rec.lbr_attribute25 = hr_api.g_varchar2) then
    p_rec.lbr_attribute25 :=
    ben_lbr_shd.g_old_rec.lbr_attribute25;
  End If;
  If (p_rec.lbr_attribute26 = hr_api.g_varchar2) then
    p_rec.lbr_attribute26 :=
    ben_lbr_shd.g_old_rec.lbr_attribute26;
  End If;
  If (p_rec.lbr_attribute27 = hr_api.g_varchar2) then
    p_rec.lbr_attribute27 :=
    ben_lbr_shd.g_old_rec.lbr_attribute27;
  End If;
  If (p_rec.lbr_attribute28 = hr_api.g_varchar2) then
    p_rec.lbr_attribute28 :=
    ben_lbr_shd.g_old_rec.lbr_attribute28;
  End If;
  If (p_rec.lbr_attribute29 = hr_api.g_varchar2) then
    p_rec.lbr_attribute29 :=
    ben_lbr_shd.g_old_rec.lbr_attribute29;
  End If;
  If (p_rec.lbr_attribute30 = hr_api.g_varchar2) then
    p_rec.lbr_attribute30 :=
    ben_lbr_shd.g_old_rec.lbr_attribute30;
  End If;
  If (p_rec.susp_if_ctfn_not_prvd_flag = hr_api.g_varchar2) then
    p_rec.susp_if_ctfn_not_prvd_flag :=
    ben_lbr_shd.g_old_rec.susp_if_ctfn_not_prvd_flag;
  End If;
  If (p_rec.ctfn_determine_cd = hr_api.g_varchar2) then
    p_rec.ctfn_determine_cd :=
    ben_lbr_shd.g_old_rec.ctfn_determine_cd;
  End If;
