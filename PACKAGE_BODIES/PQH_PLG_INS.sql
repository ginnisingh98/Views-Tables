--------------------------------------------------------
--  DDL for Package Body PQH_PLG_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_PLG_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_plg_ins.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< insert_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure insert_dml(p_rec in out nocopy pqh_plg_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Insert the row into: pqh_process_log
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_insert(p_rec  in out nocopy pqh_plg_shd.g_rec_type) is
--
  l_proc  varchar2(72) := g_package||'pre_insert';
--
  Cursor C_Sel1 is select pqh_process_log_s.nextval from sys.dual;
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.process_log_id;
  Close C_Sel1;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
...
Begin
  ...
  --
  pqh_plg_bus.insert_validate(p_rec
  ,p_effective_date);
  --
...
Begin
  ...
  --
  p_process_log_id := l_rec.process_log_id;
