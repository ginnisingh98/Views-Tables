--------------------------------------------------------
--  DDL for Package Body IRC_RSE_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_RSE_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  irc_rse_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'IRC_ALL_RECRUITING_SITE_PK') Then
...
Begin
  --
  If (p_recruiting_site_id is null and
  ...
  Else
    If (p_recruiting_site_id
        = irc_rse_shd.g_old_rec.recruiting_site_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into irc_rse_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> irc_rse_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  l_rec.posting_username                 := p_posting_username;
  l_rec.posting_password                 := p_posting_password;
  l_rec.internal                         := p_internal;
  l_rec.external                         := p_external;
  l_rec.third_party                      := p_third_party;
  l_rec.posting_cost                     := p_posting_cost;
  l_rec.posting_cost_period              := p_posting_cost_period;
  l_rec.posting_cost_currency            := p_posting_cost_currency;
  l_rec.stylesheet              := p_stylesheet;
  ...
  l_rec.posting_impl_class               := p_posting_impl_class;
  --
