--------------------------------------------------------
--  DDL for Package Body CE_CEXAVTRX_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CE_CEXAVTRX_XMLP_PKG" AS
/* $Header: CEXAVTRXB.pls 120.0.12010000.2 2008/08/10 14:26:15 csutaria ship $ */
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        WHERE L.LOOKUP_TYPE = 'TRAN_ORG_TYPE'
      ...
      EXCEPTION
        WHEN OTHERS THEN
          SET_NAME('CE'
                  ,'CE_INVALID_BANK_ACC');
		  FND_MESSAGE.SET_NAME('CE', 'CE_INVALID_BANK_ACC');
          L_MESSAGE := GET;
          RAISE_APPLICATION_ERROR(-20101
                                 ,NULL);
      END;
      UNSET_ALL;
      IF (P_TYPE = 'PAYMENTS') THEN
        P_SOURCE_VIEW := 'CE_200_TRANSACTIONS_V';
      ELSIF (P_TYPE = 'RECEIPTS') THEN
        P_SOURCE_VIEW := 'CE_222_TRANSACTIONS_V';
      ELSIF (P_TYPE = 'JE_LINES') THEN
        P_SOURCE_VIEW := 'CE_101_TRANSACTIONS_V';
      ELSIF (P_TYPE = 'ROI_LINES') THEN
        P_SOURCE_VIEW := 'CE_999_TRANSACTIONS_V';
      ELSIF (P_TYPE = 'XTR_LINES') THEN
        P_SOURCE_VIEW := 'CE_185_TRANSACTIONS_V';
      ELSIF (P_TYPE = 'PAYROLLS') THEN
        P_SOURCE_VIEW := 'CE_801_TRANSACTIONS_V';
      ELSIF (P_TYPE = 'AR_AND_AP') THEN
        P_SOURCE_VIEW := 'CE_AVAILABLE_TRANSACTIONS_V';
        SET_200;
        SET_222;
  ...
  FUNCTION C_BANK_ACCOUNT_AMOUNTFORMULA(TYPE IN VARCHAR2
                                       ...
                                       ,BANK_ACCOUNT_AMOUNT IN NUMBER) RETURN NUMBER IS
  BEGIN
    IF (TYPE in ('JE_LINE','STATEMENT')) THEN
  ...
  FUNCTION C_ACCOUNT_AMOUNTFORMULA(ACCOUNT_AMOUNT IN NUMBER) RETURN NUMBER IS
  BEGIN
    RETURN (ROUND(ACCOUNT_AMOUNT
                ,2));
    RETURN NULL;
  ...
  BEGIN
    IF P_ORDER_BY = 'AMOUNT' THEN
      RETURN ('ORDER BY 1 ASC,2 ASC,3 ASC,4 ASC,17 ASC,5 ASC,22 ASC , 11 , 15');
    ELSE
      RETURN ('ORDER BY 1 ASC,2 ASC,3 ASC,4 ASC,17 ASC,5 ASC,22 ASC , 9');
    END IF;
    RETURN NULL;
...
X0 := CE_AUTO_BANK_MATCH.GET_999;
  ...
  /*  STPROC.INIT('begin CE_AUTO_BANK_MATCH.SET_INVERSE_RATE(:INVERSE_RATE); end;');
    ...
    STPROC.EXECUTE;*/
CE_AUTO_BANK_MATCH.SET_INVERSE_RATE(INVERSE_RATE);
  ...
  BEGIN
   /* STPROC.INIT('begin :X0 := CE_AUTO_BANK_MATCH.GET_INVERSE_RATE; end;');
    ...
    STPROC.RETRIEVE(1
                   ,X0);*/
X0 := CE_AUTO_BANK_MATCH.GET_INVERSE_RATE;
    RETURN X0;
  ...
  BEGIN
   /* STPROC.INIT('begin :X0 := CE_AUTO_BANK_MATCH.GET_VAT_TAX_ID; end;');
    ...
    STPROC.RETRIEVE(1
                   ,X0);*/
--X0 := CE_AUTO_BANK_MATCH.GET_VAT_TAX_ID;
    RETURN X0;
  END GET_VAT_TAX_ID;
 ...
 /*   STPROC.INIT('begin :X0 := CE_AUTO_BANK_MATCH.CONVERT_AMOUNT_TOLERANCE(:AMOUNT_TO_CONVERT); end;');
    ...
    STPROC.RETRIEVE(1
                   ,X0);*/
X0 := CE_AUTO_BANK_MATCH.CONVERT_AMOUNT_TOLERANCE(AMOUNT_TO_CONVERT);
    RETURN X0;
  ...
  BEGIN
   /* STPROC.INIT('declare X0rv BOOLEAN; begin X0rv := CE_AUTO_BANK_MATCH.VALIDATE_PAYMENT_METHOD; :X0 := sys.diutil.bool_to_int(X0rv); end;');
    ...
    STPROC.RETRIEVE(1
                   ,X0);*/
X0rv := CE_AUTO_BANK_MATCH.VALIDATE_PAYMENT_METHOD;
--X0_1 := sys.diutil.bool_to_int(X0rv);
    RETURN X0_1;
  ...
  BEGIN
    ...
    STPROC.RETRIEVE(2
                   ,X0);*/
X0rv := CE_AUTO_BANK_MATCH.TRX_VALIDATION(NO_OF_CURRENCIES);
--X0_1 := sys.diutil.bool_to_int(X0rv);
    RETURN X0_1;
  END TRX_VALIDATION;
  ...
  BEGIN
    ...
    STPROC.EXECUTE;*/
CE_AUTO_BANK_MATCH.CALC_ACTUAL_TOLERANCE;
...
CE_AUTO_BANK_MATCH.MATCH_PROCESS;
  ...
  BEGIN
   /* STPROC.INIT('begin CE_AUTO_BANK_MATCH.LOCK_TRANSACTION(:X_RECONCILE_FLAG, :X_CALL_MODE, :X_TRX_TYPE, :X_CLEARING_TRX_TYPE, :X_TRX_ROWID, :X_BATCH_BA_AMOUNT); end;');
    ...
    STPROC.EXECUTE;*/
CE_AUTO_BANK_MATCH.LOCK_TRANSACTION(X_RECONCILE_FLAG, X_CALL_MODE, X_TRX_TYPE, X_CLEARING_TRX_TYPE, X_TRX_ROWID, X_BATCH_BA_AMOUNT);
  END LOCK_TRANSACTION;

  PROCEDURE DEBUG(LINE IN VARCHAR2) IS
  BEGIN
   /* STPROC.INIT('begin CEP_STANDARD.DEBUG(:LINE);
                end;');
    STPROC.BIND_I(LINE);
    STPROC.EXECUTE;*/
CEP_STANDARD.DEBUG(LINE);
  ...
  BEGIN
   /* STPROC.INIT('begin CEP_STANDARD.ENABLE_DEBUG;
                end;');
    STPROC.EXECUTE;*/
CEP_STANDARD.ENABLE_DEBUG;
  ...
  BEGIN
  /*  STPROC.INIT('begin CEP_STANDARD.DISABLE_DEBUG;
                end;');
    STPROC.EXECUTE;*/
...
X0 := CEP_STANDARD.GET_WINDOW_SESSION_TITLE(p_org_id, p_legal_entity_id );
  ...
  FUNCTION GET_EFFECTIVE_DATE(P_BANK_ACCOUNT_ID IN NUMBER
    ...
    X0 DATE;
  BEGIN
    /*STPROC.INIT('begin :X0 := CEP_STANDARD.GET_EFFECTIVE_DATE(:P_BANK_ACCOUNT_ID, :P_TRX_CODE, :P_RECEIPT_DATE);
                 end;');
    STPROC.BIND_O(X0);
