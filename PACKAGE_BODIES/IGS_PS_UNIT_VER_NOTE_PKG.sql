--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_VER_NOTE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_VER_NOTE_PKG" as
/* $Header: IGSPI94B.pls 115.3 2002/11/29 02:42:57 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_PS_UNIT_VER_NOTE%RowType;
  new_references IGS_PS_UNIT_VER_NOTE%RowType;
...
IF (cur_rowid%FOUND) THEN
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UVN_CNT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UVN_NOTE_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UVN_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_UNIT_VER_NOTE
      where UNIT_CD = X_UNIT_CD
      and REFERENCE_NUMBER = X_REFERENCE_NUMBER
      and VERSION_NUMBER = X_VERSION_NUMBER;
...
begin
  ...
  insert into IGS_PS_UNIT_VER_NOTE (
    UNIT_CD,
    VERSION_NUMBER,
    REFERENCE_NUMBER,
    CRS_NOTE_TYPE,
...
begin
  ...
  update IGS_PS_UNIT_VER_NOTE set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_UNIT_VER_NOTE
     where UNIT_CD = X_UNIT_CD
     and REFERENCE_NUMBER = X_REFERENCE_NUMBER
     and VERSION_NUMBER = X_VERSION_NUMBER
  ;
...
begin
  ...
  delete from IGS_PS_UNIT_VER_NOTE
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
