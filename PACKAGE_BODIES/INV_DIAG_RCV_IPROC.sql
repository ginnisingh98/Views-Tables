--------------------------------------------------------
--  DDL for Package Body INV_DIAG_RCV_IPROC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_DIAG_RCV_IPROC" AS
/* $Header: INVREQ1B.pls 120.1 2007/12/19 16:30:14 ssadasiv noship $ */
...
BEGIN
  str := 'This diagnostic test will collect setup and transactional data for a specific purchase requisition.'||'<BR/>'||
         'The data collected can aid in the resolution of issues involving purchase requisition.'||'<BR/>'||
         '<b>Parameters</b>'||'<BR/>'||
         'This script requires either the combination of Requisition Number and Operating Unit Id or Receipt Number and Organization Id'||
...
BEGIN
  ...
  tempInput := JTF_DIAGNOSTIC_ADAPTUTIL.addInput(tempInput,'Requisition Number','LOV-oracle.apps.inv.diag.lov.POReqNumberLov');
  tempInput := JTF_DIAGNOSTIC_ADAPTUTIL.addInput(tempInput,'Requisition Line Number','LOV-oracle.apps.inv.diag.lov.ReqlineLov');    -- LOV-oracle.apps.inv.diag.lov.ItemLov
...
PROCEDURE runtest(inputs IN  JTF_DIAG_INPUTTBL,
   ...
   l_req_number      po_headers_all.segment1%TYPE;
 ...
 BEGIN
  ...
  -- accept input
  l_req_number      := Trim(JTF_DIAGNOSTIC_ADAPTUTIL.getInputValue('Requisition Number',inputs));
...
ELSIF l_operating_id IS NULL THEN
      ...
      RETURN;
ELSE
IF (l_receipt_num IS NOT NULL AND l_req_number IS NULL AND l_org_id IS NULL) THEN
    JTF_DIAGNOSTIC_COREAPI.ErrorPrint('Some of the required inputs are missing.');
    JTF_DIAGNOSTIC_COREAPI.ActionErrorPrint('Please refer to the parameters section for mandatory parameters for this test.');
    statusStr  := 'FAILURE';
    report       := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
...
IF l_operating_id IS NOT NULL AND l_req_number IS NOT NULL AND l_receipt_num IS NOT NULL AND l_org_id IS NOT NULL
   AND l_line_num IS NOT NULL THEN
...
WHERE   prh.type_lookup_code IN ('PURCHASE','INTERNAL')
and prh.requisition_header_id = prl.requisition_header_id
and prl.requisition_line_id = prd.requisition_line_id
AND prl.line_num=Nvl(l_line_num,prl.line_num)
...
IF l_operating_id IS NOT NULL AND l_req_number IS NOT NULL then
...
------------------------------------------------------------------------
IF l_req_number     IS NOT NULL AND
   l_operating_id   IS NOT NULL AND
   l_receipt_num    IS NOT NULL AND
   l_org_id         IS NOT NULL AND
   l_skip_rest = 'N' THEN

   INV_DIAG_RCV_IPROC_COMMON.build_req_all_sql(l_operating_id,l_req_number,l_line_num,l_receipt_num,l_org_id,po_sql);
   l_skip_rest := 'Y';
END if;
...
IF l_req_number     IS NOT NULL  AND
   ...
   INV_DIAG_RCV_IPROC_COMMON.build_req_sql(l_operating_id,l_req_number,l_line_num,po_sql);
   l_skip_rest := 'Y';
END if;
...
IF l_receipt_num    IS NOT NULL AND
   ...
   INV_DIAG_RCV_IPROC_COMMON.build_req_rcv_sql(l_receipt_num,l_org_id,po_sql);
   l_skip_rest := 'Y';
END if;
...
<tr>'||'
<td><a href="#PO_REQUISITION_HEADERS_ALL">PO_REQUISITION_HEADERS_ALL</a></td>'||'
<td><a href="#PO_REQUISITION_LINES_ALL">PO_REQUISITION_LINES_ALL</a></td>'||'
<td><a href="#PO_REQ_DISTRIBUTIONS_ALL">PO_REQ_DISTRIBUTIONS_ALL</a></td>'||'
</tr>'||'
<tr>'||'
...
<tr>'||'
<td><a href="#PO_DISTRIBUTIONS_ALL">PO_DISTRIBUTIONS_ALL</a></td>'||'
...
sql_text := po_sql(1);
JTF_DIAGNOSTIC_COREAPI.insert_html('<a name="PO_REQUISITION_HEADERS_ALL"></a>');
JTF_DIAGNOSTIC_COREAPI.insert_html('<b>PO_REQUISITION_HEADERS_ALL</b>&nbsp;&nbsp;<a href="#INDEX OF QUERIES">[Top]</a>');
dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sql_text,'');
JTF_DIAGNOSTIC_COREAPI.BRPrint;

sql_text := po_sql(2);
JTF_DIAGNOSTIC_COREAPI.insert_html('<a name="PO_REQUISITION_LINES_ALL"></a>');
JTF_DIAGNOSTIC_COREAPI.insert_html('<b>PO_REQUISITION_LINES_ALL</b>&nbsp;&nbsp;<a href="#INDEX OF QUERIES">[Top]</a>');
dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sql_text,'');
JTF_DIAGNOSTIC_COREAPI.BRPrint;

sql_text := po_sql(3);
JTF_DIAGNOSTIC_COREAPI.insert_html('<a name="PO_REQ_DISTRIBUTIONS_ALL"></a>');
JTF_DIAGNOSTIC_COREAPI.insert_html('<b>PO_REQ_DISTRIBUTIONS_ALL</b>&nbsp;&nbsp;<a href="#INDEX OF QUERIES">[Top]</a>');
dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sql_text,'');
JTF_DIAGNOSTIC_COREAPI.BRPrint;
...
EXCEPTION
  when others then
    ...
    fixInfo := 'Unexpected Exception in INVREQ1B.pls';
    isFatal := 'FALSE';
    report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
    reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
END runTest;
