--------------------------------------------------------
--  DDL for Package Body BEN_XER_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_XER_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_xer_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_xer_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_ext_data_elmt_in_rcd Row
  --
  update ben_ext_data_elmt_in_rcd
  set
  ext_data_elmt_in_rcd_id           = p_rec.ext_data_elmt_in_rcd_id,
  seq_num                           = p_rec.seq_num,
  strt_pos                          = p_rec.strt_pos,
  dlmtr_val                         = p_rec.dlmtr_val,
  ...
  where ext_data_elmt_in_rcd_id = p_rec.ext_data_elmt_in_rcd_id;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_xer_shd.g_rec_type) is
--
...
--
 ...
 ,p_seq_num_o                     =>ben_xer_shd.g_old_rec.seq_num
 ,p_strt_pos_o                    =>ben_xer_shd.g_old_rec.strt_pos
 ,p_dlmtr_val_o                   =>ben_xer_shd.g_old_rec.dlmtr_val
 ,p_rqd_flag_o                    =>ben_xer_shd.g_old_rec.rqd_flag
 ,p_sprs_cd_o                     =>ben_xer_shd.g_old_rec.sprs_cd
 ,p_any_or_all_cd_o               =>ben_xer_shd.g_old_rec.any_or_all_cd
 ,p_ext_data_elmt_id_o            =>ben_xer_shd.g_old_rec.ext_data_elmt_id
 ,p_ext_rcd_id_o                  =>ben_xer_shd.g_old_rec.ext_rcd_id
 ,p_business_group_id_o           =>ben_xer_shd.g_old_rec.business_group_id
 ,p_legislation_code_o            =>ben_xer_shd.g_old_rec.legislation_code
 ,p_object_version_number_o       =>ben_xer_shd.g_old_rec.object_version_number
 ,p_hide_flag_o                   =>ben_xer_shd.g_old_rec.hide_flag
      );
    --
  exception
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_xer_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.seq_num = hr_api.g_number) then
    p_rec.seq_num :=
    ben_xer_shd.g_old_rec.seq_num;
  End If;
  If (p_rec.strt_pos = hr_api.g_number) then
    p_rec.strt_pos :=
    ben_xer_shd.g_old_rec.strt_pos;
  End If;
  If (p_rec.dlmtr_val = hr_api.g_varchar2) then
    p_rec.dlmtr_val :=
    ben_xer_shd.g_old_rec.dlmtr_val;
  End If;
  If (p_rec.rqd_flag = hr_api.g_varchar2) then
    p_rec.rqd_flag :=
    ben_xer_shd.g_old_rec.rqd_flag;
  End If;
  If (p_rec.sprs_cd = hr_api.g_varchar2) then
    p_rec.sprs_cd :=
    ben_xer_shd.g_old_rec.sprs_cd;
  End If;
  If (p_rec.any_or_all_cd = hr_api.g_varchar2) then
    p_rec.any_or_all_cd :=
    ben_xer_shd.g_old_rec.any_or_all_cd;
  End If;
  If (p_rec.ext_data_elmt_id = hr_api.g_number) then
    p_rec.ext_data_elmt_id :=
    ben_xer_shd.g_old_rec.ext_data_elmt_id;
  End If;
  If (p_rec.ext_rcd_id = hr_api.g_number) then
    p_rec.ext_rcd_id :=
    ben_xer_shd.g_old_rec.ext_rcd_id;
  End If;
  ...
  If (p_rec.hide_flag = hr_api.g_varchar2) then
    p_rec.hide_flag :=
    ben_xer_shd.g_old_rec.hide_flag;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_xer_bus.update_validate(p_rec
  ,p_effective_date);
  --
