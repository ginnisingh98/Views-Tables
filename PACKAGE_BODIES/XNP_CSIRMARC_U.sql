--------------------------------------------------------
--  DDL for Package Body XNP_CSIRMARC_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIRMARC_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIRMARC' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIRMARC' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIRMARC' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIRMARC.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIRMARC', l_consumer_list );
  END IF;
...
NULL ;
DECLARE
      l_txn_type      varchar2(30) := 'CSIRMARC';
      l_txn_type_id   number := 53;
      l_txn_id        number;
