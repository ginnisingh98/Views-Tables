--------------------------------------------------------
--  DDL for Package Body BEN_EDD_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EDD_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_edd_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_dsblty_dgr_prte_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_dsblty_dgr_prte_id PK of record being inserted or updated.
...
--
Procedure chk_elig_dsblty_dgr_prte_id(p_elig_dsblty_dgr_prte_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_dsblty_dgr_prte_id';
...
Begin
  ...
  --
  l_api_updating := ben_edd_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_dsblty_dgr_prte_id                => p_elig_dsblty_dgr_prte_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_dsblty_dgr_prte_id,hr_api.g_number)
     <>  ben_edd_shd.g_old_rec.elig_dsblty_dgr_prte_id) then
    --
    -- raise error as PK has changed
    --
    ben_edd_shd.constraint_error('BEN_ELIG_dsblty_dgr_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_dsblty_dgr_prte_id is not null then
      --
  ...
  --
End chk_elig_dsblty_dgr_prte_id;
--
--
/*-- ----------------------------------------------------------------------------
-- |------< chk_person_type_id >------|
...
--
-- In Parameters
--   p_elig_dsblty_dgr_prte_id PK
--   p_degree   ID of FK column
--   p_eligy_prfl_id
--   p_business_group_id
...
--   Internal table handler use only.
--
Procedure chk_person_type_id (p_elig_dsblty_dgr_prte_id  in number,
                              p_degree            in number,
  ...
  cursor c3 is
     select null
       from ben_elig_dsblty_dgr_prte_f
         where degree = p_degree
           and eligy_prfl_id = p_eligy_prfl_id
           and elig_dsblty_dgr_prte_id <> nvl(p_elig_dsblty_dgr_prte_id,hr_api.g_number)
  ...
  cursor c1 is
    ...
    where  a.person_type_id =to_number( p_degree)
...
Begin
  ...
  --
  if (l_api_updating
     and nvl(p_degree,hr_api.g_number)
     <> nvl(ben_edd_shd.g_old_rec.degree,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if degree value exists in per_person_types table
    --
    open c1;
      --
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_edd_shd.constraint_error('BEN_ELIG_dsblty_dgr_PRTE_FK2');
        --
      end if;
      --
    close c1;
    ...
    if (l_system_person_type <> 'EMP' and l_system_person_type <> 'EMP_APL'
      ...
      if c2%found then
        ...
        -- ben_edd_shd.constraint_error('BEN_ELIG_dsblty_dgr_PRTE_FK2');
           fnd_message.set_name('BEN', 'BEN_91387_INV_dsblty_dgrE');
           fnd_message.raise_error;
        --
      end if;
      --
...
--   Internal table handler use only.
--
Procedure chk_degree(p_elig_dsblty_dgr_prte_id	 in number,
		         p_degree            in number,
		         p_business_group_id     in number,
		         p_effective_date        in date,
		         p_object_version_number in number) is
  ...
  cursor c1 is
    ...
    where  ppt.system_person_type = p_degree
  ...
  if (l_api_updating
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        --
        close c1;
        fnd_message.set_name('PER','HR_7513_dsblty_dgrE_INVALID');
        fnd_message.raise_error;
        --
      end if;
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_dsblty_dgr_prte_id                in number,
                            p_excld_flag               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_edd_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
Begin
  ...
  --
  /*chk_person_type_id (p_elig_dsblty_dgr_prte_id  => p_rec.elig_dsblty_dgr_prte_id,
...
function return_legislation_code
  (p_elig_dsblty_dgr_prte_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_dsblty_dgr_prte_f b
    where b.elig_dsblty_dgr_prte_id      = p_elig_dsblty_dgr_prte_id
