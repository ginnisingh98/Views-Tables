--------------------------------------------------------
--  DDL for Package Body GCS_TRANS_HRATES_DYN_BUILD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GCS_TRANS_HRATES_DYN_BUILD_PKG" AS
/* $Header: gcsxlthratesdb.pls 120.3 2007/06/28 12:30:41 vkosuri noship $ */
...
--
  -- The API name
  g_api		CONSTANT VARCHAR2(50) := 'gcs.plsql.GCS_TRANS_HRATES_DYN_BUILD_PKG';
...
';
  ...
  -- Example
  --   GCS_TRANS_HRATES_DYN_BUILD_PKG.Module_Log_Write
  -- Notes
  --
  ...
  -- Example
  --   GCS_TRANS_HRATES_DYN_BUILD_PKG.Write_To_Log
  -- Notes
  --
...
--
-- Public procedures
-- Bugfix 5725759: When fem balances is joined with gcs_historical_rates,
-- historical rates dimensions are used.
--
  ...
  BEGIN
    ...
    -- Bugfix 7525759: Initialize the variables that determines the required dimensions.
    GCS_TRANS_DYN_BUILD_PKG.Initialize_Dimensions;

    -- Create the package body
    body_block :=
   'CREATE OR REPLACE PACKAGE BODY GCS_TRANS_HRATES_DYNAMIC_PKG AS
...
'';
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''        AND    ghr1.', ' = ghr.', ''' || g_nl ||', line_num, 'H');

    --Bugfix 6111815: Adeded Standard RE Rate Flag
    body_block :=
   '''        AND    ghr1.cal_period_id = '' || p_prev_period_id || '')'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''        AND    ghr3.', ' = ghr2.', ''' || g_nl ||', line_num, 'H');
   ...
   '''        AND    ghr3.from_currency = ghr2.from_currency'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('     AND    ghr1.', ' = ghr.', '', line_num, 'H');
   ...
   '    AND    ghr1.cal_period_id = p_prev_period_id)
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('     AND    ghr3.', ' = ghr2.', '', line_num, 'H');
   ...
   '     AND    ghr3.from_currency = ghr2.from_currency
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''        AND    ghr2.', ' (+)= ghr3.', ''' || g_nl ||', line_num, 'H');
   ...
   '''       )'');
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('           AND    ghr2.', ' (+)= ghr3.', '', line_num, 'H');
   ...
   '          );
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_comma_list('''ghr.', ' '' ||', '''NULL, '' ||', line_num, 'H');
   ...
   '''          SUM(ytd_debit_balance_e) sum_ytd_debit_balance_e,'' || g_nl ||
    ...
    --Bugfix 6111815: Remove p_hier_li_id condition
    body_block :=
   '''          fb_in.line_item_id) fb,'' || g_nl ||
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('''AND    ghr.', ' (+) = fb.', ''' || g_nl ||', line_num, 'H');
   ...
   '                SUM(ytd_debit_balance_e) sum_ytd_debit_balance_e,
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_comma_list('                fb_in.', '', '', line_num, 'F');
    --Bugfix 6111815: Remove p_hier_li_id condition and added NOT EXISTS clause
    body_block :=
   '             fb_in.line_item_id) fb,
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('      AND    ghr.', ' (+) = fb.', '', line_num, 'H');

    --Bugfix 6111815: Added  Retained Earnings Exclusion
    body_block :=
   '
    ...
    line_num := GCS_TRANS_DYN_BUILD_PKG.build_join_list('      AND    ghr_retained.', ' = fb.', '', line_num, 'F');
   ...
   '''0, 0, 0, 0,'' || g_nl ||
    ...
    --Bugfix 6111815: Removed p_hier_li_id condition and added NOT EXISTS clause
    body_block :=
   ...
   '''          SUM(ptd_debit_balance_e) sum_ptd_debit_balance_e,'' || g_nl ||
    ...
    --Bugfix 6111815: Removed p_hier_li_id and added NOT EXISTS clause
    body_block :=
   '''          fb_in.line_item_id) fb,'' || g_nl ||
  ...
  '                 SUM(ptd_debit_balance_e) sum_ptd_debit_balance_e,
   ...
   'g_nl ||
    ...
    --Bugfix 6111815: Removed p_hier_li_id Condition
    body_block :=
   ...
   '    );
    ...
    ad_ddl.create_plsql_object(appl, 'APPS', 'GCS_TRANS_HRATES_DYNAMIC_PKG', 1, line_num - 1 , 'FALSE', err);
