--------------------------------------------------------
--  DDL for Package Body AME_ITL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ITL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_itl_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_item_class_id               number         default null;
...
Procedure set_security_group_id
  ...
  -- ame_item_classes_tl and PER_BUSINESS_GROUPS_PERF
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'ITEM_CLASS_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  --
  if (( nvl(ame_itl_bus.g_item_class_id, hr_api.g_number)
       = p_item_class_id)
  and ( nvl(ame_itl_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := ame_itl_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    ame_itl_bus.g_item_class_id               := p_item_class_id;
    ame_itl_bus.g_language                    := p_language;
    ame_itl_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT ame_itl_shd.api_updating
