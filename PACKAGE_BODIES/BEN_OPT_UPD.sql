--------------------------------------------------------
--  DDL for Package Body BEN_OPT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_OPT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_opt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_opt_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.opt_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_opt_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    ben_opt_shd.g_old_rec.name;
  End If;
  If (p_rec.cmbn_ptip_opt_id = hr_api.g_number) then
    p_rec.cmbn_ptip_opt_id :=
    ben_opt_shd.g_old_rec.cmbn_ptip_opt_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_opt_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.opt_attribute_category = hr_api.g_varchar2) then
    p_rec.opt_attribute_category :=
    ben_opt_shd.g_old_rec.opt_attribute_category;
  End If;
  If (p_rec.opt_attribute1 = hr_api.g_varchar2) then
    p_rec.opt_attribute1 :=
    ben_opt_shd.g_old_rec.opt_attribute1;
  End If;
  If (p_rec.opt_attribute2 = hr_api.g_varchar2) then
    p_rec.opt_attribute2 :=
    ben_opt_shd.g_old_rec.opt_attribute2;
  End If;
  If (p_rec.opt_attribute3 = hr_api.g_varchar2) then
    p_rec.opt_attribute3 :=
    ben_opt_shd.g_old_rec.opt_attribute3;
  End If;
  If (p_rec.opt_attribute4 = hr_api.g_varchar2) then
    p_rec.opt_attribute4 :=
    ben_opt_shd.g_old_rec.opt_attribute4;
  End If;
  If (p_rec.opt_attribute5 = hr_api.g_varchar2) then
    p_rec.opt_attribute5 :=
    ben_opt_shd.g_old_rec.opt_attribute5;
  End If;
  If (p_rec.opt_attribute6 = hr_api.g_varchar2) then
    p_rec.opt_attribute6 :=
    ben_opt_shd.g_old_rec.opt_attribute6;
  End If;
  If (p_rec.opt_attribute7 = hr_api.g_varchar2) then
    p_rec.opt_attribute7 :=
    ben_opt_shd.g_old_rec.opt_attribute7;
  End If;
  If (p_rec.opt_attribute8 = hr_api.g_varchar2) then
    p_rec.opt_attribute8 :=
    ben_opt_shd.g_old_rec.opt_attribute8;
  End If;
  If (p_rec.opt_attribute9 = hr_api.g_varchar2) then
    p_rec.opt_attribute9 :=
    ben_opt_shd.g_old_rec.opt_attribute9;
  End If;
  If (p_rec.opt_attribute10 = hr_api.g_varchar2) then
    p_rec.opt_attribute10 :=
    ben_opt_shd.g_old_rec.opt_attribute10;
  End If;
  If (p_rec.opt_attribute11 = hr_api.g_varchar2) then
    p_rec.opt_attribute11 :=
    ben_opt_shd.g_old_rec.opt_attribute11;
  End If;
  If (p_rec.opt_attribute12 = hr_api.g_varchar2) then
    p_rec.opt_attribute12 :=
    ben_opt_shd.g_old_rec.opt_attribute12;
  End If;
  If (p_rec.opt_attribute13 = hr_api.g_varchar2) then
    p_rec.opt_attribute13 :=
    ben_opt_shd.g_old_rec.opt_attribute13;
  End If;
  If (p_rec.opt_attribute14 = hr_api.g_varchar2) then
    p_rec.opt_attribute14 :=
    ben_opt_shd.g_old_rec.opt_attribute14;
  End If;
  If (p_rec.opt_attribute15 = hr_api.g_varchar2) then
    p_rec.opt_attribute15 :=
    ben_opt_shd.g_old_rec.opt_attribute15;
  End If;
  If (p_rec.opt_attribute16 = hr_api.g_varchar2) then
    p_rec.opt_attribute16 :=
    ben_opt_shd.g_old_rec.opt_attribute16;
  End If;
  If (p_rec.opt_attribute17 = hr_api.g_varchar2) then
    p_rec.opt_attribute17 :=
    ben_opt_shd.g_old_rec.opt_attribute17;
  End If;
  If (p_rec.opt_attribute18 = hr_api.g_varchar2) then
    p_rec.opt_attribute18 :=
    ben_opt_shd.g_old_rec.opt_attribute18;
  End If;
  If (p_rec.opt_attribute19 = hr_api.g_varchar2) then
    p_rec.opt_attribute19 :=
    ben_opt_shd.g_old_rec.opt_attribute19;
  End If;
  If (p_rec.opt_attribute20 = hr_api.g_varchar2) then
    p_rec.opt_attribute20 :=
    ben_opt_shd.g_old_rec.opt_attribute20;
  End If;
  If (p_rec.opt_attribute21 = hr_api.g_varchar2) then
    p_rec.opt_attribute21 :=
    ben_opt_shd.g_old_rec.opt_attribute21;
  End If;
  If (p_rec.opt_attribute22 = hr_api.g_varchar2) then
    p_rec.opt_attribute22 :=
    ben_opt_shd.g_old_rec.opt_attribute22;
  End If;
  If (p_rec.opt_attribute23 = hr_api.g_varchar2) then
    p_rec.opt_attribute23 :=
    ben_opt_shd.g_old_rec.opt_attribute23;
  End If;
  If (p_rec.opt_attribute24 = hr_api.g_varchar2) then
    p_rec.opt_attribute24 :=
    ben_opt_shd.g_old_rec.opt_attribute24;
  End If;
  If (p_rec.opt_attribute25 = hr_api.g_varchar2) then
    p_rec.opt_attribute25 :=
    ben_opt_shd.g_old_rec.opt_attribute25;
  End If;
  If (p_rec.opt_attribute26 = hr_api.g_varchar2) then
    p_rec.opt_attribute26 :=
    ben_opt_shd.g_old_rec.opt_attribute26;
  End If;
  If (p_rec.opt_attribute27 = hr_api.g_varchar2) then
    p_rec.opt_attribute27 :=
    ben_opt_shd.g_old_rec.opt_attribute27;
  End If;
  If (p_rec.opt_attribute28 = hr_api.g_varchar2) then
    p_rec.opt_attribute28 :=
    ben_opt_shd.g_old_rec.opt_attribute28;
  End If;
  If (p_rec.opt_attribute29 = hr_api.g_varchar2) then
    p_rec.opt_attribute29 :=
    ben_opt_shd.g_old_rec.opt_attribute29;
  End If;
  If (p_rec.opt_attribute30 = hr_api.g_varchar2) then
    p_rec.opt_attribute30 :=
    ben_opt_shd.g_old_rec.opt_attribute30;
  End If;
  If (p_rec.rqd_perd_enrt_nenrt_uom = hr_api.g_varchar2) then
    p_rec.rqd_perd_enrt_nenrt_uom :=
    ben_opt_shd.g_old_rec.rqd_perd_enrt_nenrt_uom;
  End If;
  If (p_rec.rqd_perd_enrt_nenrt_val = hr_api.g_number) then
    p_rec.rqd_perd_enrt_nenrt_val :=
    ben_opt_shd.g_old_rec.rqd_perd_enrt_nenrt_val;
  End If;
  If (p_rec.rqd_perd_enrt_nenrt_rl = hr_api.g_number) then
    p_rec.rqd_perd_enrt_nenrt_rl :=
    ben_opt_shd.g_old_rec.rqd_perd_enrt_nenrt_rl;
  End If;
  If (p_rec.invk_wv_opt_flag = hr_api.g_varchar2) then
    p_rec.invk_wv_opt_flag :=
    ben_opt_shd.g_old_rec.invk_wv_opt_flag;
  End If;
  If (p_rec.short_name = hr_api.g_varchar2) then
      p_rec.short_name :=
      ben_opt_shd.g_old_rec.short_name;
  End If;
  If (p_rec.short_code = hr_api.g_varchar2) then
      p_rec.short_code :=
      ben_opt_shd.g_old_rec.short_code;
  End If;

  If (p_rec.legislation_code = hr_api.g_varchar2) then
      p_rec.legislation_code :=
          ben_opt_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.legislation_subgroup = hr_api.g_varchar2) then
      p_rec.legislation_subgroup :=
          ben_opt_shd.g_old_rec.legislation_subgroup;
  End If;

  If (p_rec.group_opt_id  = hr_api.g_number) then
      p_rec.group_opt_id :=
          ben_opt_shd.g_old_rec.group_opt_id;
  End If;

  If (p_rec.component_reason = hr_api.g_varchar2) then
      p_rec.component_reason :=
      ben_opt_shd.g_old_rec.component_reason;
  End If;
  If (p_rec.mapping_table_name = hr_api.g_varchar2) then
      p_rec.mapping_table_name :=
      ben_opt_shd.g_old_rec.mapping_table_name;
  End If;
  If (p_rec.mapping_table_pk_id = hr_api.g_number) then
      p_rec.mapping_table_pk_id :=
      ben_opt_shd.g_old_rec.mapping_table_pk_id;
  End If;
