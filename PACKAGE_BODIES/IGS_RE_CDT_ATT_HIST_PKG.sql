--------------------------------------------------------
--  DDL for Package Body IGS_RE_CDT_ATT_HIST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_RE_CDT_ATT_HIST_PKG" as
/* $Header: IGSRI03B.pls 120.1 2005/07/04 00:41:15 appldev ship $ */
  -------------------------------------------------------------------------------------------
  --Change History:
  --Who         When            What
  --smadathi    24-AUG-2001     Bug No. 1956374 .The call to igs_re_val_cah.genp_val_sdtt_sess
  --                            is changed to igs_as_val_suaap.genp_val_sdtt_sess
  --svanukur    17-feb-2004    added logic after call to IGS_RE_VAL_CAH.resp_val_cah_hist_dt to
  --                             not raise an exceptio if the message is IGS_RE_ATND_HIST_STRT_CRS_ATM
  --svanukur    13-APR-2004      removed the call to resp_val_cah_strt_dt since this procedure was modified
  --                             to return true in all cases.  as part of bug 3544986
  -------------------------------------------------------------------------------------------
  l_rowid VARCHAR2(25);
  old_references IGS_RE_CDT_ATT_HIST_ALL%RowType;
  new_references IGS_RE_CDT_ATT_HIST_ALL%RowType;
  ...
  PROCEDURE BeforeRowInsertUpdateDelete1(
    ...
    p_deleting IN BOOLEAN DEFAULT FALSE
        ...
        v_person_id             IGS_RE_CDT_ATT_HIST.person_id%TYPE;
        v_ca_sequence_number    IGS_RE_CDT_ATT_HIST.ca_sequence_number%TYPE;
  BEGIN
        ...
        IF igs_as_val_suaap.genp_val_sdtt_sess('ENRP_INS_CA_TRNSFR') THEN
                ...
                IF p_inserting OR
                        ...
                        -- Validate history end date.
                        IF p_inserting OR
                                (p_updating AND
                                (new_references.hist_end_dt <> old_references.hist_end_dt) OR
                                (new_references.hist_start_dt <> old_references.hist_start_dt)) THEN
                                    IF IGS_RE_VAL_CAH.resp_val_cah_end_dt (
                                        new_references.person_id,
                                        new_references.ca_sequence_number,
                        ...
                        END IF;
                END IF;
                -- Save rowid to validate all IGS_RE_CANDIDATURE attendance histories are continuous
                -- Do not validate at database level  if being updated via form RESF3211
                IF igs_as_val_suaap.genp_val_sdtt_sess('RESP_VAL_CAH_HIST_DT') THEN
                        IF p_inserting OR
                                p_updating THEN
                                v_person_id := new_references.person_id;
                                v_ca_sequence_number := new_references.ca_sequence_number;
                        ELSE
                                v_person_id := old_references.person_id;
                                v_ca_sequence_number := old_references.ca_sequence_number;
                        END IF;

                        IF igs_as_val_suaap.genp_val_sdtt_sess('ENRP_INS_CA_TRNSFR') THEN
                                IF IGS_RE_VAL_CAH.resp_val_cah_hist_dt(
                                                v_person_id,
                                                v_ca_sequence_number,
                                                'Y', -- validate first history start date
                                                v_message_name) = FALSE THEN
                                                        Fnd_Message.Set_Name ('IGS', v_message_name);
                                                        IGS_GE_MSG_STACK.ADD;
                                                        App_Exception.Raise_Exception;
 ...
 END Check_Constraints ;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_RE_CAH_ATT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_RE_CAH_CA_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID in NUMBER
    ...
    cursor C is select ROWID from IGS_RE_CDT_ATT_HIST_ALL
      where PERSON_ID = X_PERSON_ID
      and CA_SEQUENCE_NUMBER = X_CA_SEQUENCE_NUMBER
      and SEQUENCE_NUMBER = X_SEQUENCE_NUMBER;
...
begin
  ...
  END IF;
  insert into IGS_RE_CDT_ATT_HIST_ALL (
    PERSON_ID,
    CA_SEQUENCE_NUMBER,
    SEQUENCE_NUMBER,
    HIST_START_DT,
...
begin
  ...
  if ( (tlinfo.HIST_START_DT = X_HIST_START_DT)
...
begin
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
  update IGS_RE_CDT_ATT_HIST_ALL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_RE_CDT_ATT_HIST_ALL
     where PERSON_ID = X_PERSON_ID
     and CA_SEQUENCE_NUMBER = X_CA_SEQUENCE_NUMBER
     and SEQUENCE_NUMBER = X_SEQUENCE_NUMBER
  ;
...
begin
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
  delete from IGS_RE_CDT_ATT_HIST_ALL
  where ROWID = X_ROWID;
  if (sql%notfound) then
     fnd_message.set_name ('IGS', 'IGS_SC_POLICY_UPD_DEL_EXCEP');
     igs_ge_msg_stack.add;
