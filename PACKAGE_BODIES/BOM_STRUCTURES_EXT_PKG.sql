--------------------------------------------------------
--  DDL for Package Body BOM_STRUCTURES_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BOM_STRUCTURES_EXT_PKG" AS
/* $Header: BOMPSUAB.pls 115.0 2004/02/03 13:28:28 hgelli noship $ */
...
PROCEDURE INSERT_ROW (
  ...
  CURSOR C IS SELECT ROWID FROM BOM_STRUCTURES_EXT_B
    WHERE EXTENSION_ID = X_EXTENSION_ID
    ;
BEGIN
  INSERT INTO BOM_STRUCTURES_EXT_B (
    N_EXT_ATTR3,
    N_EXT_ATTR4,
    N_EXT_ATTR5,
    N_EXT_ATTR6,
  ...
  INSERT INTO BOM_STRUCTURES_EXT_TL (
    EXTENSION_ID,
    BILL_SEQUENCE_ID,
    BILL_REVISION_ID,
    STRUCTURE_TYPE_ID,
...
BEGIN
  ...
  END IF;
  CLOSE c;
  IF (    ((recinfo.N_EXT_ATTR3 = X_N_EXT_ATTR3)
      ...
      AND ((recinfo.BILL_REVISION_ID = X_BILL_REVISION_ID)
           OR ((recinfo.BILL_REVISION_ID IS NULL) AND (X_BILL_REVISION_ID IS NULL)))
      ...
      AND ((recinfo.ACD_TYPE = X_ACD_TYPE)
           OR ((recinfo.ACD_TYPE IS NULL) AND (X_ACD_TYPE IS NULL)))
...
BEGIN
  UPDATE BOM_STRUCTURES_EXT_B SET
    ...
    BILL_REVISION_ID = X_BILL_REVISION_ID,
  ...
  UPDATE BOM_STRUCTURES_EXT_TL SET
...
BEGIN
  DELETE FROM BOM_STRUCTURES_EXT_TL
  WHERE EXTENSION_ID = X_EXTENSION_ID;
  ...
  DELETE FROM BOM_STRUCTURES_EXT_B
  WHERE EXTENSION_ID = X_EXTENSION_ID;
...
IS
BEGIN
  DELETE FROM BOM_STRUCTURES_EXT_TL T
  WHERE NOT EXISTS
    (SELECT NULL
    FROM BOM_STRUCTURES_EXT_B B
    WHERE B.EXTENSION_ID = T.EXTENSION_ID
    );

  UPDATE BOM_STRUCTURES_EXT_TL T SET (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  WHERE (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    FROM BOM_STRUCTURES_EXT_TL SUBB, BOM_STRUCTURES_EXT_TL SUBT
  ...
  INSERT INTO BOM_STRUCTURES_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  FROM BOM_STRUCTURES_EXT_TL B, FND_LANGUAGES L
  WHERE L.INSTALLED_FLAG IN ('I', 'B')
