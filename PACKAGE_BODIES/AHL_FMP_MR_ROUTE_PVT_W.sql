--------------------------------------------------------
--  DDL for Package Body AHL_FMP_MR_ROUTE_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_FMP_MR_ROUTE_PVT_W" as
  /* $Header: AHLWMRUB.pls 120.0 2005/05/26 01:27:56 appldev noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).mr_route_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).mr_header_id := a2(indx);
          t(ddindx).mr_title := a3(indx);
          t(ddindx).mr_version_number := a4(indx);
          t(ddindx).route_id := a5(indx);
          t(ddindx).route_number := a6(indx);
          t(ddindx).route_revision_number := a7(indx);
          t(ddindx).route_description := a8(indx);
          t(ddindx).product_type := a9(indx);
          t(ddindx).operator := a10(indx);
          t(ddindx).stage := a11(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a33 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).mr_route_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).mr_header_id;
          a3(indx) := t(ddindx).mr_title;
          a4(indx) := t(ddindx).mr_version_number;
          a5(indx) := t(ddindx).route_id;
          a6(indx) := t(ddindx).route_number;
          a7(indx) := t(ddindx).route_revision_number;
          a8(indx) := t(ddindx).route_description;
          a9(indx) := t(ddindx).product_type;
          a10(indx) := t(ddindx).operator;
          a11(indx) := t(ddindx).stage;
  ...
  as
    ddp_x_mr_route_tbl ahl_fmp_mr_route_pvt.mr_route_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_fmp_mr_route_pvt_w.rosetta_table_copy_in_p1(ddp_x_mr_route_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    ahl_fmp_mr_route_pvt.process_mr_route(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      p_default,
    ...
    ahl_fmp_mr_route_pvt_w.rosetta_table_copy_out_p1(ddp_x_mr_route_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
