--------------------------------------------------------
--  DDL for Package Body OKC_KL_TERM_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_KL_TERM_ASMBLR_PVT" AS
/* $Header: OKCRKLTB.pls 120.0 2005/05/25 22:36:43 appldev noship $ */
...
l_msg_tbl := okc_aq_pvt.msg_tab_typ();
 ...
 ELSIF corr_rec.element_name = 'KL_TERM_DATE' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
  --l_msg_tbl(i).element_value   := P_kl_term_date;
  IF corr_rec.format_mask IS NOT NULL THEN
       l_msg_tbl(i).element_value := to_char(p_kl_term_date,
		      corr_rec.format_mask);
  ELSE
       l_msg_tbl(i).element_value := to_char(p_kl_term_date,'DD-MON-YY');
  END IF;
 ELSIF corr_rec.element_name = 'KL_TERM_REASON' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
  l_msg_tbl(i).element_value   := P_kl_term_reason;
