--------------------------------------------------------
--  DDL for Package Body AHL_LTP_SPACE_CAPB_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_LTP_SPACE_CAPB_PVT" AS
/* $Header: AHLVSPCB.pls 115.10 2002/12/04 19:12:08 ssurapan noship $ */

G_PKG_NAME  VARCHAR2(30)  := 'AHL_LTP_SPACE_CAPB_PVT';
...
EXCEPTION
       WHEN NO_DATA_FOUND THEN
         x_return_status:= Fnd_Api.G_RET_STS_ERROR;
         x_error_msg_code:= 'AHL_LTP_ORG_ID_NOT_EXISTS';
       WHEN TOO_MANY_ROWS THEN
         x_return_status:= Fnd_Api.G_RET_STS_ERROR;
         x_error_msg_code:= 'AHL_LTP_ORG_ID_NOT_EXISTS';
       WHEN OTHERS THEN
...
EXCEPTION
       WHEN NO_DATA_FOUND THEN
         x_return_status:= Fnd_Api.G_RET_STS_ERROR;
         x_error_msg_code:= 'AHL_LTP_DEPT_ID_NOT_EXIST';
       WHEN TOO_MANY_ROWS THEN
         x_return_status:= Fnd_Api.G_RET_STS_ERROR;
         x_error_msg_code:= 'AHL_LTP_DEPT_ID_NOT_EXIST';
       WHEN OTHERS THEN
...
BEGIN
   ...
   END IF;
       IF l_dummy IS NOT NULL THEN
        Fnd_Message.SET_NAME('AHL','AHL_LTP_SPACE_NAME_EXISTS');
        Fnd_Msg_Pub.ADD;
        RAISE Fnd_Api.G_EXC_ERROR;
        END IF;
   END IF;
...
BEGIN
 ...
 -- Check required parameters
     ...
     THEN
          ...
          IF Fnd_Msg_Pub.check_msg_level (Fnd_Msg_Pub.G_MSG_LVL_ERROR)
          THEN
               Fnd_Message.set_name('AHL', 'AHL_LTP_SPACE_NAME_NOT_EXIST');
               Fnd_Msg_Pub.ADD;
          END IF;
          x_return_status := Fnd_Api.G_RET_STS_ERROR;
     END IF;
...
/*
  ...
  IF (p_space_rec.space_id <> FND_API.G_MISS_NUM AND
    ...
    IF l_space_id IS NOT NULL THEN
       Fnd_Message.set_name('AHL', 'AHL_LTP_SPACE_ASSIGN_EXISTS');
       Fnd_Msg_Pub.ADD;
     ELSE
        OPEN space_name_cur(p_space_rec.space_name,p_space_rec.department_id);
        FETCH space_name_cur INTO l_space_name;
...
BEGIN
  ...
  SAVEPOINT create_space;
   ...
   -- Debug info.
   IF G_DEBUG='Y' THEN
   AHL_DEBUG_PUB.debug( 'enter ahl_ltp_space_capb_pvt.Create Space ','+SPCBL+');
   END IF;
   -- Standard call to check for call compatibility.
...
---------------------------End of Body---------------------------------------
 ...
 WHEN FND_API.G_EXC_UNEXPECTED_ERROR THEN
   ...
   IF G_DEBUG='Y' THEN
      ...
      AHL_DEBUG_PUB.debug( 'ahl_ltp_space_capb_pvt.Create Space ','+SPCBL+');
        -- Check if API is called in debug mode. If yes, disable debug.
      AHL_DEBUG_PUB.disable_debug;
   END IF;
WHEN FND_API.G_EXC_ERROR THEN
...
WHEN OTHERS THEN
    ...
    IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.G_MSG_LVL_UNEXP_ERROR)
    THEN
    fnd_msg_pub.add_exc_msg(p_pkg_name        =>  'AHL_LTP_SPACE_CAPB_PVT',
   ...
   --Check for space name
    ...
    END IF;
        ...
        END IF;
         ...
         IF NVL(l_return_status, 'X') <> 'S'
         THEN
            Fnd_Message.SET_NAME('AHL','AHL_LTP_SP_STATUS_NOT_EXIST');
            Fnd_Msg_Pub.ADD;
         END IF;
        END IF;
        -- Id presents
  ...
  SAVEPOINT delete_space;
   ...
   IF l_visit_id IS NOT NULL THEN
    ...
    IF l_visit_status_code <> 'CLOSED' THEN
       FND_MESSAGE.set_name('AHL', 'AHL_LTP_SP_VISITS_ASSIGNED');
       FND_MSG_PUB.add;
      RAISE Fnd_Api.G_EXC_ERROR;
     ELSE
       UPDATE AHL_SPACES_B
...
EXCEPTION
       WHEN NO_DATA_FOUND THEN
         x_return_status:= Fnd_Api.G_RET_STS_ERROR;
         x_error_msg_code:= 'AHL_LTP_ITEM_NOT_EXISTS';
       WHEN TOO_MANY_ROWS THEN
         x_return_status:= Fnd_Api.G_RET_STS_ERROR;
         x_error_msg_code:= 'AHL_LTP_ITEM_NOT_EXISTS';
       WHEN OTHERS THEN
...
PROCEDURE Assign_Space_Capbl_Rec (
   p_space_capbl_rec      IN  AHL_LTP_SPACE_CAPB_PUB.Space_capbl_rec,
...
BEGIN
 ...
 -- Check required parameters
     ...
     THEN
          ...
          IF Fnd_Msg_Pub.check_msg_level (Fnd_Msg_Pub.G_MSG_LVL_ERROR)
          THEN
               Fnd_Message.set_name('AHL', 'AHL_LTP_VISIT_TYPE_NOT_EXIST');
               Fnd_Msg_Pub.ADD;
          END IF;
          x_return_status := Fnd_Api.G_RET_STS_ERROR;
     END IF;
     ...
     THEN
          ...
          IF Fnd_Msg_Pub.check_msg_level (Fnd_Msg_Pub.G_MSG_LVL_ERROR)
          THEN
               Fnd_Message.set_name('AHL', 'AHL_LTP_INV_ITEM_NOT_EXIST');
               Fnd_Msg_Pub.ADD;
          END IF;
          x_return_status := Fnd_Api.G_RET_STS_ERROR;
     END IF;
...
/*   IF p_validation_mode = Jtf_Plsql_Api.g_create
   ...
   THEN
      IF Ahl_Utility_Pvt.check_uniqueness(
         ...
         IF Fnd_Msg_Pub.check_msg_level(Fnd_Msg_Pub.g_msg_lvl_error)
               THEN
            Fnd_Message.set_name('AHL', 'AHL_LTP_SPACE_CAPBL_DUPL_ID');
            Fnd_Msg_Pub.ADD;
         END IF;
         x_return_status := Fnd_Api.g_ret_sts_error;
      END IF;
...
CURSOR get_org_cur (c_space_id IN NUMBER)
 ...
 BEGIN
  ...
  SAVEPOINT create_space_capblts;
   ...
   -- Debug info.
   IF G_DEBUG='Y' THEN
       AHL_DEBUG_PUB.debug( 'enter ahl_ltp_space_capb_pvt.Create Space Capblts ','+SPCBL+');
    END IF;
   -- Standard call to check for call compatibility.
   IF FND_API.to_boolean(p_init_msg_list)
   THEN
     ...
     p_x_space_capblts_rec.organization_id := l_organization_id;
      ...
      IF (p_x_space_capblts_rec.item_description IS NOT NULL AND
         ...
         (p_x_space_capblts_rec.inventory_item_id IS NOT NULL AND
          ...
          IF NVL(l_return_status,'x') <> 'S'
          THEN
              Fnd_Message.SET_NAME('AHL','AHL_LTP_ITEM_NOT_EXISTS');
...
---------------------------End of Body---------------------------------------
 ...
 WHEN FND_API.G_EXC_UNEXPECTED_ERROR THEN
   ...
   IF G_DEBUG='Y' THEN
            AHL_DEBUG_PUB.log_app_messages (
                x_msg_count, x_msg_data, 'ERROR' );
            AHL_DEBUG_PUB.debug( 'ahl_ltp_space_capb_pvt.Create Space Capblts','+SPCBL+');
        -- Check if API is called in debug mode. If yes, disable debug.
        AHL_DEBUG_PUB.disable_debug;
    END IF;
WHEN FND_API.G_EXC_ERROR THEN
