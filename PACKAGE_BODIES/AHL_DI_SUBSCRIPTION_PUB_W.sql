--------------------------------------------------------
--  DDL for Package Body AHL_DI_SUBSCRIPTION_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_DI_SUBSCRIPTION_PUB_W" as
  /* $Header: AHLSUBWB.pls 115.10 2003/07/28 07:30:49 pbarman noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).subscription_id := a0(indx);
          t(ddindx).document_id := a1(indx);
          t(ddindx).status_code := a2(indx);
          t(ddindx).status_desc := a3(indx);
          t(ddindx).requested_by_party_id := a4(indx);
          t(ddindx).requested_by_pty_number := a5(indx);
          t(ddindx).requested_by_pty_name := a6(indx);
          t(ddindx).quantity := a7(indx);
          t(ddindx).frequency_code := a8(indx);
          t(ddindx).frequency_desc := a9(indx);
          t(ddindx).subscribed_frm_party_id := a10(indx);
          t(ddindx).subscribed_frm_pty_number := a11(indx);
          t(ddindx).subscribed_frm_pty_name := a12(indx);
          t(ddindx).start_date := rosetta_g_miss_date_in_map(a13(indx));
          t(ddindx).end_date := rosetta_g_miss_date_in_map(a14(indx));
          t(ddindx).purchase_order_no := a15(indx);
          t(ddindx).subscription_type_code := a16(indx);
          t(ddindx).subscription_type_desc := a17(indx);
          t(ddindx).media_type_code := a18(indx);
          t(ddindx).media_type_desc := a19(indx);
          ...
          t(ddindx).language := a36(indx);
          t(ddindx).source_lang := a37(indx);
          t(ddindx).comments := a38(indx);
          t(ddindx).object_version_number := a39(indx);
          t(ddindx).delete_flag := a40(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  if t is null or t.count = 0 then
    ...
    a40 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).subscription_id;
          a1(indx) := t(ddindx).document_id;
          a2(indx) := t(ddindx).status_code;
          a3(indx) := t(ddindx).status_desc;
          a4(indx) := t(ddindx).requested_by_party_id;
          a5(indx) := t(ddindx).requested_by_pty_number;
          a6(indx) := t(ddindx).requested_by_pty_name;
          a7(indx) := t(ddindx).quantity;
          a8(indx) := t(ddindx).frequency_code;
          a9(indx) := t(ddindx).frequency_desc;
          a10(indx) := t(ddindx).subscribed_frm_party_id;
          a11(indx) := t(ddindx).subscribed_frm_pty_number;
          a12(indx) := t(ddindx).subscribed_frm_pty_name;
          a13(indx) := t(ddindx).start_date;
          a14(indx) := t(ddindx).end_date;
          a15(indx) := t(ddindx).purchase_order_no;
          a16(indx) := t(ddindx).subscription_type_code;
          a17(indx) := t(ddindx).subscription_type_desc;
          a18(indx) := t(ddindx).media_type_code;
          a19(indx) := t(ddindx).media_type_desc;
          ...
          a36(indx) := t(ddindx).language;
          a37(indx) := t(ddindx).source_lang;
          a38(indx) := t(ddindx).comments;
          a39(indx) := t(ddindx).object_version_number;
          a40(indx) := t(ddindx).delete_flag;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_x_subscription_tbl ahl_di_subscription_pub.subscription_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_di_subscription_pub_w.rosetta_table_copy_in_p1(ddp_x_subscription_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
    ...
    ahl_di_subscription_pub.create_subscription(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validate_only,
      p_validation_level,
      ddp_x_subscription_tbl,
      p_module_type,
      x_return_status,
      x_msg_count,
      x_msg_data);
    ...
    ahl_di_subscription_pub_w.rosetta_table_copy_out_p1(ddp_x_subscription_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ahl_di_subscription_pub.modify_subscription(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validate_only,
      p_validation_level,
