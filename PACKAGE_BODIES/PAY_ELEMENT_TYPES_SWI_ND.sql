--------------------------------------------------------
--  DDL for Package Body PAY_ELEMENT_TYPES_SWI_ND
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ELEMENT_TYPES_SWI_ND" As
/* $Header: paycreleswi.pkb 120.3 2006/02/28 14:29 ndorai noship $ */
--
-- Package variables
--
...
Begin
  ...
  -- Call API
  --
  pay_element_types_mig.create_element_type
...
-- ---------------------------------------------------------------------------+
-- |--------------------------< create_class_balance_feeds >-----------------|+
-- ---------------------------------------------------------------------------+
Procedure create_class_balance_feeds
    (p_balance_name VARCHAR2,
     p_business_group_id NUMBER,
     p_legislation_code VARCHAR2,
     p_effective_start_date DATE,
     p_effective_end_date DATE) AS
  l_row_id             rowid;
  ...
  --
    cursor c1_get_reg_earn_feeds is
     ...
     WHERE BT.BALANCE_TYPE_ID = BF.BALANCE_TYPE_ID
       ...
       and nvl(bt.business_group_id,p_business_group_id) = p_business_group_id
       and nvl(bt.legislation_code, 'US') = 'US'
       and bt.balance_name   = 'Regular Earnings'
       and bc.balance_type_id = bt.balance_type_id
    ...
    cursor c2_balance_type is
     select balance_type_id
      ...
      where business_group_id =  p_business_group_id
        and ( balance_name like p_balance_name || ' Eligible Comp'
              OR balance_name like p_balance_name || ' Roth Eligible Comp'
              OR balance_name like p_balance_name || ' AT Eligible Comp');
      ...
      for c2_rec in c2_balance_type loop
        begin
          select count(*) into l_count
           ...
           where balance_type_id = c2_rec.balance_type_id
             and input_value_id = c1_rec.input_value_id
             and business_group_id = p_business_group_id
             and effective_start_date = p_effective_start_date
             and effective_end_date = p_effective_end_date
             and nvl(legislation_code,'US') = c1_rec.legislation_code;
          if l_count = 0 then
...
END create_class_balance_feeds;
end pay_element_types_swi_nd;
