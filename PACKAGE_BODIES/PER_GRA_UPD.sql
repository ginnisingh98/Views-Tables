--------------------------------------------------------
--  DDL for Package Body PER_GRA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_GRA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_gra_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy per_gra_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the per_cagr_grades Row
  --
  update per_cagr_grades
  set
  cagr_grade_id                     = p_rec.cagr_grade_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in per_gra_shd.g_rec_type,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update(p_rec in per_gra_shd.g_rec_type,
	              p_effective_date   in date) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy per_gra_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.cagr_grade_structure_id = hr_api.g_number) then
    p_rec.cagr_grade_structure_id :=
    per_gra_shd.g_old_rec.cagr_grade_structure_id;
  End If;
  If (p_rec.cagr_grade_def_id = hr_api.g_number) then
    p_rec.cagr_grade_def_id :=
    per_gra_shd.g_old_rec.cagr_grade_def_id;
  End If;
  If (p_rec.sequence = hr_api.g_number) then
    p_rec.sequence :=
    per_gra_shd.g_old_rec.sequence;
  End If;
...
Procedure upd
  (
  p_rec        in out nocopy per_gra_shd.g_rec_type
...
Begin
  ...
  --
  convert_defs(p_rec);
  per_gra_bus.update_validate(p_rec	=> p_rec,
  			      p_effective_date	=> p_effective_date);
  --
...
Procedure upd
  ...
  p_cagr_grade_structure_id      in number           default hr_api.g_number,
