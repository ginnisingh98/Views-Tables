--------------------------------------------------------
--  DDL for Package Body MSC_PEG_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MSC_PEG_UTIL" AS
/*  $Header: MSCPEGB.pls 120.0 2005/05/25 18:49:16 appldev noship $ */
...
Procedure get_suptree_sup_values(p_plan_id    IN NUMBER,
                  ...
                  x_itemorg_pegnode_rec  OUT NOCOPY MSC_PEG_UTIL.itemorg_pegnode_rec,
                            p_supply_pegging  IN NUMBER DEFAULT 0) -- demand pegging ( up peg)
IS
...
Procedure get_suptree_dem_values(p_plan_id  IN NUMBER,
                      ...
                      p_supply_pegging        IN NUMBER DEFAULT 0) IS
...
fetch inventory_item_id INTO l_item_id;
...
close bom_item_type;
 ...
 /*
  bug #3525195  modify a  SQL statement to get dependant demands for supply root node .
*/
 select  item_org,
         demand_qty,
         demand_date,
...
Procedure get_demtree_dem_values( p_plan_id                IN NUMBER,
                                  p_transaction_id         IN NUMBER,
                                  x_itemorg_pegnode_rec    OUT NOCOPY  MSC_PEG_UTIL.itemorg_pegnode_rec,                                  p_instance_id            IN NUMBER,
                                  p_organization_id        IN NUMBER,
                                  p_bom_item_type          IN NUMBER)              IS
