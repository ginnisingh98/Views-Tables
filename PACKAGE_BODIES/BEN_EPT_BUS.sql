--------------------------------------------------------
--  DDL for Package Body BEN_EPT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EPT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ept_bus.';  -- Global package name


Procedure chk_valid_fctr (p_eligy_prfl_id         in number
			  ,p_person_type_id       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_valid_fctr';
  l_person_type_cd       varchar2(30);
  l_dummy        varchar2(1);
--
-- Bug:1631182 - Not supporing per_typ_cd, instead use person_type_id for
-- supporting user created person types
--
  ...
  cursor c1 is
    select null
      ...
      where  b.eligy_prfl_id = p_eligy_prfl_id
        and  b.per_typ_cd NOT IN ('EMP', 'EMP_APL')
        and  b.excld_flag = 'N';
  -- */
  --
  cursor c1 is
...
Begin
  ...
  --
	open c2;
	fetch c2 into l_person_type_cd;
	close c2;

if (l_person_type_cd not in ('EMP', 'EMP_APL')) then
   ...
   fetch c1 into l_dummy;
      if c1%found then
        ...
        close c1;
        --
        -- raise error as this eligibility factor is invalid
        -- create generic message here
        --
        fnd_message.set_name('BEN', 'BEN_94611_INV_FCTR');
...
--
-- In Parameters
--   elig_per_typ_prte_id PK of record being inserted or updated.
...
--
Procedure chk_elig_per_typ_prte_id(p_elig_per_typ_prte_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_per_typ_prte_id';
...
Begin
  ...
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  l_api_updating := ben_ept_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_per_typ_prte_id                => p_elig_per_typ_prte_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_per_typ_prte_id,hr_api.g_number)
     <>  ben_ept_shd.g_old_rec.elig_per_typ_prte_id) then
    --
    -- raise error as PK has changed
    --
    ben_ept_shd.constraint_error('BEN_ELIG_PER_TYP_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_per_typ_prte_id is not null then
      --
...
--
-- In Parameters
--   p_elig_per_typ_prte_id PK
--   p_per_typ_cd   ID of FK column
--   p_eligy_prfl_id
--   p_business_group_id
...
--   Internal table handler use only.
--
-- Not supporting per_typ_cd
--
Procedure chk_person_type_id (p_elig_per_typ_prte_id  in number,
  ...
  cursor c3 is
     select null
       from ben_elig_per_typ_prte_f
         where per_typ_cd = p_per_typ_cd
           and eligy_prfl_id = p_eligy_prfl_id
           and elig_per_typ_prte_id <> nvl(p_elig_per_typ_prte_id,hr_api.g_number)
  ...
  cursor c1 is
    ...
    where  a.person_type_id =to_number( p_per_typ_cd)
...
Begin
  ...
  --
  if (l_api_updating
     and nvl(p_per_typ_cd,hr_api.g_number)
     <> nvl(ben_ept_shd.g_old_rec.per_typ_cd,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if per_typ_cd value exists in per_person_types table
    --
    open c1;
      --
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_ept_shd.constraint_error('BEN_ELIG_PER_TYP_PRTE_FK2');
        --
      end if;
      --
    close c1;
...
-- In Parameters
--   elig_per_typ_prte_id PK of record being inserted or updated.
--   per_typ_cd Value of lookup code.
...
--
Procedure chk_per_typ_cd(p_elig_per_typ_prte_id	 in number,
		         --p_per_typ_cd            in varchar2,
		         ...
		         p_object_version_number in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_per_typ_cd';
  l_api_updating boolean;
  l_dummy        varchar2(1);
--
-- Bug 1631182 - To support user created person types needed to use
-- person_type_id insted of per_typ_cd
--
/*  --
  cursor c1 is
    select null
    from   per_person_types ppt
    where  ppt.system_person_type = p_per_typ_cd
...
/*  --
  if (l_api_updating
      and p_per_typ_cd
      <>nvl(ben_ept_shd.g_old_rec.per_typ_cd,hr_api.g_varchar2)
      or not l_api_updating)
      and p_per_typ_cd is not null then
    -- */
  --
  if (l_api_updating
      and p_person_type_id
      <>nvl(ben_ept_shd.g_old_rec.person_type_id,hr_api.g_number)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_per_typ_prte_id                in number,
                            p_excld_flag               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_ept_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
--
-- ---------------------------------------------------------------------------
-- |-----------------------< chk_duplicate_ordr_num >---------------------------|   --Bug 2358518
-- ---------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   p_elig_per_typ_prte_id    elig_per_typ_prte_id
...
is
  ...
  cursor c1 is
    select null
    from   ben_elig_per_typ_prte_f
    where  elig_per_typ_prte_id <> nvl(p_elig_per_typ_prte_id,-1)
    and    eligy_prfl_id = p_eligy_prfl_id
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    hr_api.mandatory_arg_error
      (p_api_name       => l_proc,
       p_argument       => 'elig_per_typ_prte_id',
       p_argument_value => p_elig_per_typ_prte_id);
    --
    --
    --
  End If;
...
--Bug No 5054294
  chk_valid_fctr(p_eligy_prfl_id          => p_rec.eligy_prfl_id,
...
function return_legislation_code
  (p_elig_per_typ_prte_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_per_typ_prte_f b
    where b.elig_per_typ_prte_id      = p_elig_per_typ_prte_id
