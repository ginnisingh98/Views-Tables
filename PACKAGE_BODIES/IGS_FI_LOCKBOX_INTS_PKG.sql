--------------------------------------------------------
--  DDL for Package Body IGS_FI_LOCKBOX_INTS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_FI_LOCKBOX_INTS_PKG" AS
/* $Header: IGSSID7B.pls 115.0 2003/06/19 06:38:00 agairola noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_fi_lockbox_ints%ROWTYPE;
  new_references igs_fi_lockbox_ints%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.transmission_record_count         := x_transmission_record_count;
    new_references.transmission_amt                  := x_transmission_amt;
    new_references.lockbox_name                      := x_lockbox_name;
    new_references.lockbox_batch_count               := x_lockbox_batch_count;
    new_references.lockbox_record_count              := x_lockbox_record_count;
    new_references.lockbox_amt                       := x_lockbox_amt;
    new_references.batch_name                        := x_batch_name;
    new_references.batch_amt                         := x_batch_amt;
    new_references.batch_record_count                := x_batch_record_count;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.record_identifier_cd = x_record_identifier_cd)
        ...
        AND ((tlinfo.transmission_record_count = x_transmission_record_count) OR ((tlinfo.transmission_record_count IS NULL) AND (X_transmission_record_count IS NULL)))
        AND ((tlinfo.transmission_amt = x_transmission_amt) OR ((tlinfo.transmission_amt IS NULL) AND (X_transmission_amt IS NULL)))
        AND ((tlinfo.lockbox_name = x_lockbox_name) OR ((tlinfo.lockbox_name IS NULL) AND (X_lockbox_name IS NULL)))
        AND ((tlinfo.lockbox_batch_count = x_lockbox_batch_count) OR ((tlinfo.lockbox_batch_count IS NULL) AND (X_lockbox_batch_count IS NULL)))
        AND ((tlinfo.lockbox_record_count = x_lockbox_record_count) OR ((tlinfo.lockbox_record_count IS NULL) AND (X_lockbox_record_count IS NULL)))
        AND ((tlinfo.lockbox_amt = x_lockbox_amt) OR ((tlinfo.lockbox_amt IS NULL) AND (X_lockbox_amt IS NULL)))
        AND ((tlinfo.batch_name = x_batch_name) OR ((tlinfo.batch_name IS NULL) AND (X_batch_name IS NULL)))
        AND ((tlinfo.batch_amt = x_batch_amt) OR ((tlinfo.batch_amt IS NULL) AND (X_batch_amt IS NULL)))
        AND ((tlinfo.batch_record_count = x_batch_record_count) OR ((tlinfo.batch_record_count IS NULL) AND (X_batch_record_count IS NULL)))
        AND ((tlinfo.item_number = x_item_number) OR ((tlinfo.item_number IS NULL) AND (X_item_number IS NULL)))
        AND ((tlinfo.receipt_amt = x_receipt_amt) OR ((tlinfo.receipt_amt IS NULL) AND (X_receipt_amt IS NULL)))
