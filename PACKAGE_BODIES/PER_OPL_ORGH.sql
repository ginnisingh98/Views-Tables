--------------------------------------------------------
--  DDL for Package Body PER_OPL_ORGH
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_OPL_ORGH" AS
/* $Header: perporgh.pkb 115.1 2004/06/17 03:25:22 vanantha noship $ */
...
BEGIN
  ...
  FOR i IN 1..g_cs_rows_to_insert LOOP
    ...
    BEGIN
      ...
      INSERT INTO /*+ PARALLEL(<table name>, DEFAULT, DEFAULT) */ PER_org_hrchy_summary
...
/* Get PER schema name - get_app_info populates l_schema */
  IF fnd_installation.get_app_info('PER',l_dummy1, l_dummy2, l_schema) THEN

  /* Empty out organization hierarchy tables */
    l_sql_stmt := 'TRUNCATE TABLE ' || l_schema || '.PER_ORG_HRCHY_SUMMARY';
  ...
  /* Gather index stats */
--    fnd_stats.gather_table_stats(l_schema, 'PER_CS_orgH');
