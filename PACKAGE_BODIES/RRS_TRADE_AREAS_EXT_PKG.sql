--------------------------------------------------------
--  DDL for Package Body RRS_TRADE_AREAS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."RRS_TRADE_AREAS_EXT_PKG" as
/* $Header: RRSTEXTB.pls 120.2.12010000.2 2009/01/05 18:41:18 sunarang ship $ */
...
select nvl(X_EXTENSION_ID ,EGO_EXTFWK_S.nextval)
  ...
  insert into RRS_TRADE_AREAS_EXT_B (
    EXTENSION_ID,
    TRADE_AREA_ID,
    GROUP_ID,
    ATTR_GROUP_ID,
  ...
  insert into RRS_TRADE_AREAS_EXT_TL (
    EXTENSION_ID,
    TRADE_AREA_ID,
    GROUP_ID,
    ATTR_GROUP_ID,
...
begin
  ...
  close c;
  if (    ((recinfo.UOM_EXT_ATTR14 = X_UOM_EXT_ATTR14)
      ...
      AND (recinfo.TRADE_AREA_ID = X_TRADE_AREA_ID)
...
begin
  update RRS_TRADE_AREAS_EXT_B set
  ...
  update RRS_TRADE_AREAS_EXT_TL set
...
begin
  delete from RRS_TRADE_AREAS_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from RRS_TRADE_AREAS_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  delete from RRS_TRADE_AREAS_EXT_TL T
  where not exists
    (select NULL
    from RRS_TRADE_AREAS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update RRS_TRADE_AREAS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from RRS_TRADE_AREAS_EXT_TL SUBB, RRS_TRADE_AREAS_EXT_TL SUBT
  ...
  insert into RRS_TRADE_AREAS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from RRS_TRADE_AREAS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
