--------------------------------------------------------
--  DDL for Package Body XNP_CSIEAMRR_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIEAMRR_U" AS 
PROCEDURE CREATE_MSG  (   XNP$INSTANCE_ID NUMBER,
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIEAMRR' ;
  ...
  XNP_XML_UTILS.write_leaf_element ( 'INSTANCE_ID', XNP$INSTANCE_ID );
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIEAMRR' ) ;
    IF ( XNP$INSTANCE_ID IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - INSTANCE_ID' ;
     fnd_message.set_name('XNP','XNP_MISSING_MANDATORY_ATTR');
     fnd_message.set_token('ATTRIBUTE','INSTANCE_ID' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIEAMRR' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIEAMRR.create_msg()::' || SQLERRM ;
END ;
PROCEDURE PUBLISH  (   XNP$INSTANCE_ID NUMBER,
  XNP$ORGANIZATION_ID NUMBER,
  XNP$WIP_ENTITY_ID NUMBER,
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIEAMRR', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_EVT_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$INSTANCE_ID=>XNP$INSTANCE_ID,
...
END ;
PROCEDURE SEND  (   XNP$INSTANCE_ID NUMBER,
  XNP$ORGANIZATION_ID NUMBER,
  XNP$WIP_ENTITY_ID NUMBER,
  x_message_id OUT  NUMBER,
  x_error_code OUT  NUMBER,
...
NULL ;
DECLARE
   l_txn_type varchar2(8) := 'CSIEAMRR';
   l_txn_type_id number := 93;
 ...
 BEGIN
   ...
   xnp_xml_utils.decode(
     p_msg_text => p_msg_text, p_tag => 'INSTANCE_ID', x_value => l_instance_id);
   IF l_instance_id is null THEN
     l_error_rec.error_text := 'xnp_xml_utils.decode failed. could not derive instance_id.';
     RAISE dpl_error;
   END IF;
   l_error_rec.source_id := l_instance_id;
   debug('  instance_id : '||l_instance_id);
   -- call the new routine here
   init_debug_file;
 ...
 EXCEPTION
   WHEN dpl_error THEN
     rollback to dpl_sp;
     init_debug_file;
     debug('  Transaction '||l_instance_id||' failed. '||l_error_rec.error_text);
