--------------------------------------------------------
--  DDL for Package Body OKL_SETUPOPTRULES_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_SETUPOPTRULES_PUB_W" as
  /* $Header: OKLUSORB.pls 115.2 2002/12/24 04:19:23 sgorantl noship $ */
  ...
  as
    ddp_orlv_rec okl_setupoptrules_pub.orlv_rec_type;
    ddx_no_data_found boolean;
    ddx_orlv_rec okl_setupoptrules_pub.orlv_rec_type;
  ...
  begin
    ...
    okl_setupoptrules_pub.get_rec(ddp_orlv_rec,
      x_return_status,
      x_msg_data,
      ddx_no_data_found,
      ddx_orlv_rec);
  ...
  if ddx_no_data_found is null
    ...
    p4_a0 := rosetta_g_miss_num_map(ddx_orlv_rec.id);
    p4_a1 := rosetta_g_miss_num_map(ddx_orlv_rec.object_version_number);
    p4_a2 := rosetta_g_miss_num_map(ddx_orlv_rec.opt_id);
    p4_a3 := rosetta_g_miss_num_map(ddx_orlv_rec.srd_id_for);
    p4_a4 := ddx_orlv_rec.rgr_rgd_code;
    p4_a5 := ddx_orlv_rec.rgr_rdf_code;
    p4_a6 := rosetta_g_miss_num_map(ddx_orlv_rec.lrg_lse_id);
    p4_a7 := rosetta_g_miss_num_map(ddx_orlv_rec.lrg_srd_id);
    p4_a8 := ddx_orlv_rec.overall_instructions;
    p4_a9 := rosetta_g_miss_num_map(ddx_orlv_rec.created_by);
    p4_a10 := ddx_orlv_rec.creation_date;
    p4_a11 := rosetta_g_miss_num_map(ddx_orlv_rec.last_updated_by);
    p4_a12 := ddx_orlv_rec.last_update_date;
    p4_a13 := rosetta_g_miss_num_map(ddx_orlv_rec.last_update_login);
  end;
  ...
  as
    ddp_optv_rec okl_setupoptrules_pub.optv_rec_type;
    ddp_orlv_rec okl_setupoptrules_pub.orlv_rec_type;
    ddx_orlv_rec okl_setupoptrules_pub.orlv_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_setupoptrules_pub.insert_optrules(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_optv_rec okl_setupoptrules_pub.optv_rec_type;
    ddp_orlv_tbl okl_setupoptrules_pub.orlv_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_setupoptrules_pub.delete_optrules(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
