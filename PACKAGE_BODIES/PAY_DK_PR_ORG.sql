--------------------------------------------------------
--  DDL for Package Body PAY_DK_PR_ORG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_DK_PR_ORG" AS
 /* $Header: pydkprorg.pkb 120.9.12000000.2 2007/05/08 07:12:02 saurai noship $ */
...
PROCEDURE GET_ALL_PARAMETERS(
	...
	SELECT
    	fnd_date.canonical_to_date(PAY_DK_PR_ORG.GET_PARAMETER(legislative_parameters,'FROM_DATE'))
		,fnd_date.canonical_to_date(PAY_DK_PR_ORG.GET_PARAMETER(legislative_parameters,'TO_DATE'))
	  	,PAY_DK_PR_ORG.GET_PARAMETER(legislative_parameters,'PAYROLL_ID')
	  	,PAY_DK_PR_ORG.GET_PARAMETER(legislative_parameters,'ORGANIZATION_ID')
		,PAY_DK_PR_ORG.GET_PARAMETER(legislative_parameters,'ELEMENT_SET_ID')
		,PAY_DK_PR_ORG.GET_PARAMETER(legislative_parameters,'ARCHIVE')
		,effective_date
		,business_group_id
...
BEGIN
	 ...
	 FETCH csr_parameter_info  INTO	p_fromdate,p_todate,p_payroll_id ,p_organization_id,
...
and    action_information1         = 'PYDKPRORGA'
...
IS
 SELECT
	SUM (pay_balance_pkg.get_value(pay_dk_pr_org.get_defined_balance_id
...
WHERE
	...
	AND paaf.organization_id = p_organization_id
...
( SELECT
	...
	,pay_dk_pr_org.costed_code(prr.run_result_id,pivf.input_value_id) costed_code
	,pivf.input_value_id  input_value_id
	,to_char(ppa1.date_earned,'DD-MON-RRRR')    date_earned
	,pay_balance_pkg.get_value(pay_dk_pr_org.get_defined_balance_id
        (pivf.input_value_id,'_ASG_RUN')
...
WHERE
	paaf.business_group_id = l_business_group_id -- BG Check
	AND paaf.organization_id = l_organization_id
...
GROUP BY
	...
	,date_earned
	,costed_code
ORDER BY payroll_id,organization_id,ele_type_id;
...
END IF;
 ...
 PAY_DK_PR_ORG.GET_ALL_PARAMETERS(
		 pactid
		,l_business_group_id
		,l_effective_date
		,l_archive
...
BEGIN
	...
	FROM hr_all_organization_units haou
	WHERE haou.organization_id=l_organization_id;
EXCEPTION
	WHEN OTHERS THEN
	NULL;
END;
...
-- Archive the REPORT DETAILS(input Parameters)
	...
	,p_action_information1          => 'PYDKPRORGA'	 -- Conc Prg Short Name
	...
	,p_action_information7          => l_organization_name   --organization name
    ,p_action_information8          => l_element_set_name);	 -- assignment set name
...
IF (csr_rec.pay_value > 0) THEN -- to archive only elements which have pay value > 0
  ...
  -- Creating Archive Entries
	...
	,p_tax_unit_id                  => NULL			  -- Legal Employer ID
	,p_jurisdiction_code            =>  NULL		  -- Tax Municipality ID
	,p_action_information1          => 'PYDKPRORGA'	  --Con Program Short Name
	,p_action_information2          => csr_rec.payroll_id		-- Payroll ID
	,p_action_information3          => csr_rec.payroll_name		-- payroll name
	,p_action_information4          => csr_rec.organization_id	-- Organization id
	,p_action_information5          => csr_rec.organization_name	-- Organization name
	,p_action_information6         =>  csr_rec.ele_type_id		-- element type id
	,p_action_information7         =>  csr_rec.ele_proc_prior		-- processing priority
	,p_action_information8         =>  csr_rec.ele_name		-- element name
	,p_action_information9         =>  csr_rec.date_earned		-- date_earned
	,p_action_information10         => csr_rec.costed_code		-- Costed value
	/*Storing in Canonical format to fix issues due to varying numeric formats*/
	,p_action_information11        =>  fnd_number.number_to_canonical(csr_rec.pay_value) -- Pay value
	,p_action_information12        =>  fnd_number.number_to_canonical(l_ele_ytd)         -- element YTD
     );
END IF ;
END LOOP;
...
l_tot_org_run NUMBER := 0;
l_tot_org_ytd NUMBER :=0;
l_IANA_charset VARCHAR2 (50);

CURSOR csr_pr_org IS
SELECT
	...
	,pai2.action_information4 org_id
	,pai2.action_information5 org_name
	,pai2.action_information6 ele_type_id
	,pai2.action_information7 ele_proc_prior
	,pai2.action_information8 ele_name
	,pai2.action_information9 date_earned
	,pai2.action_information10 costed_code
	,fnd_number.canonical_to_number(pai2.action_information11) pay_value
	,fnd_number.canonical_to_number(pai2.action_information12) balance_amount
FROM
...
ORDER BY
	...
	,fnd_date.string_to_date(pai2.action_information9,'DD-MON-YYYY');
...
FOR csr_rec IN csr_pr_org LOOP

IF (csr_rec.payroll_id <> l_payroll_id) THEN
  IF (csr_pr_org%rowcount <> 1) THEN
    ...
    l_tot_org_run := l_tot_org_run + l_tot_ele_run;
    l_tot_org_ytd := l_tot_org_ytd + l_tot_ele_ytd;
    l_tot_ele_run := 0;
    l_tot_ele_ytd := 0;
	p_xml:= p_xml || '</ELEMENT_RECORD>';
    p_xml:=p_xml || '<Tot_org_run>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_org_run,2),'999G999G990D99')),' ') || '</Tot_org_run>';
    p_xml:=p_xml || '<Tot_org_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_org_ytd,2),'999G999G990D99')),' ') || '</Tot_org_ytd>';
    l_tot_org_run := 0;
    l_tot_org_ytd := 0;
...
IF( l_ele_type_id <> csr_rec.ele_type_id AND csr_rec.payroll_id = l_payroll_id) THEN
    p_xml:=p_xml || '<Tot_ele_run>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_run,2),'999G999G990D99')),' ') || '</Tot_ele_run>';
    p_xml:=p_xml || '<Tot_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_ytd,2),'999G999G990D99')),' ') || '</Tot_ele_ytd>';
    l_tot_org_run := l_tot_org_run + l_tot_ele_run;
    l_tot_org_ytd := l_tot_org_ytd + l_tot_ele_ytd;
