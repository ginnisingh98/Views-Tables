--------------------------------------------------------
--  DDL for Package Body XNP_CONTROL_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CONTROL_U" AS
/* $Header: XNPCTRLB.pls 120.2 2006/02/13 07:43:35 dputhiye ship $*/
...
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('CONTROL',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CONTROL' ;
  ...
  XNP_XML_UTILS.write_element ( 'OPERATION', XNP$OPERATION );
  XNP_XML_UTILS.write_element ( 'OP_DATA', XNP$OP_DATA );
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CONTROL.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CONTROL', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_OUT_MSG_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$OPERATION=>XNP$OPERATION,
    XNP$OP_DATA=>XNP$OP_DATA,
    x_msg_header=>l_msg_header,
