--------------------------------------------------------
--  DDL for Package Body PAY_GB_P11D_EDI_2005
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_GB_P11D_EDI_2005" AS
/* $Header: pygbp11d05.pkb 120.8 2006/03/01 03:22 kthampan noship $ */
  ...
  is
	 ...
	 and    pai_person.action_information13 = p_emp_ref
  ...
  begin
         ...
         for asset in get_data loop
             if asset.cash_equivalent > 0 then
                l_count := l_count + 1;
  ...
  begin
         ...
         for payment in get_data loop
             ...
             if payment.cash_equivalent > 0 then
                l_count := l_count + 1;
  ...
  begin
         ...
         edi_currency        := 'GBP';
	 edi_cat             := rpad('E',18);
         edi_tax_qualifier12 := rpad('71',3);

         open get_data;
         fetch get_data into l_taxable_payments;
         close get_data;

         if l_taxable_payments > 0 then
            p_edi_rec1  := edi_ftx1a || edi_cat || rpad(' ', 71) || rpad(' ', 71) ||
                           rpad(' ', 70) || fnd_global.local_chr(10);
            p_edi_rec2  := edi_tax1 || edi_tax_qualifier12 || fnd_global.local_chr(10);
  ...
  begin
	 ...
	 p_edi_rec14:= edi_att3 || edi_tax_qualifier5 || rpad(l_make_of_car || ' ' || l_model,35) || fnd_global.local_chr(10);
         ...
         if (to_number(edi_date_from) >= to_number(edi_tax_year_start) and
             to_number(edi_date_to) <= to_number(edi_tax_year_end)) or
            (l_free_fuel_reinstated <> 'N') then
  ...
  begin
       ...
       if (to_number(p_value1) > 0 or
           to_number(p_value2) > 0)  then
  ...
  is
	 ...
	 and    pai.action_context_id = paa.assignment_action_id
	 and    pai.action_context_type = 'AAP'
	 and    pai.action_information_category = 'VANS 2002_03'
	 and    pai_person.action_context_id = paa.assignment_action_id
	 and    pai_person.action_information_category = 'GB EMPLOYEE DETAILS'
	 and    pai_person.action_information13 = p_emp_ref
	 and    pai_person.action_context_type = 'AAP';
    ...
    begin
         ...
         for non1a in get_data('OTHER ITEMS NON 1A','GB_OTHER_ITEMS_NON_1A')loop
             if non1a.description = 'DIRECTOR TAX PAID NOT DEDUCTED' then
                ...
                if non1a.cash_equivalent > 0 then
  ...
  begin
        ...
        if (l_trvlnsubs_cash_equivalent > 0 or
            l_entertain_cash_equivalent > 0 or
            l_bustrvl_cash_equivalent   > 0 or
            l_hometel_cash_equivalent   > 0 or
            l_nonqreloc_cash_equivalent > 0 or
            l_other_cash_equivalent     > 0) then
            ...
            if l_entertain_cash_equivalent > 0 then
  ...
  begin
       ...
       if pay_gb_eoy_magtape.validate_input(p_employer_name,'EDI_SURNAME') > 0 then
  ...
  begin
       ...
       /* Perform EDI Validation */
       /* Removed validation. Validations are now done at archive level */
       /*
       ...
       if pay_gb_eoy_magtape.validate_input(l_addr2,'EDI_SURNAME') > 0 then
          p_error_msg5  := ':Employee id ' || l_employee_no || ' has illegal Character(s) in Address Line2' ;
          p_error_count := p_error_count + 1;
       end if;
  ...
  begin
       ...
       if p_benefit_type = 'VANS 2002_03' then
  ...
  is
       ...
       and    pai_person.action_context_id = paa.assignment_action_id
       and    pai_person.action_information_category = 'GB EMPLOYEE DETAILS'
       and    pai_person.action_information13 = p_employer_ref
       and    pai_person.action_context_type = 'AAP';
  begin
       l_pact_id := p_pact_id;
