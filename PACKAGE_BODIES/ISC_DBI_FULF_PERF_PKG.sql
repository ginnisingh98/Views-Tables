--------------------------------------------------------
--  DDL for Package Body ISC_DBI_FULF_PERF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_FULF_PERF_PKG" AS
/* $Header: ISCRG97B.pls 120.0 2005/05/25 17:24:06 appldev noship $ */
...
BEGIN
  ...
  IF (l_cust IS NULL OR l_cust = '' OR l_cust = 'All')
    ...
    ELSE
      ...
      l_cust_flag := 0;
      l_drill := '''pFunctionName=ISC_DBI_BOOK_DETAIL&VIEW_BY_NAME=VIEW_BY_ID&pParamIds=Y''';
  END IF;
  ...
  l_measures := 'ISC_MEASURE_1,ISC_MEASURE_2,ISC_MEASURE_3,ISC_MEASURE_4,ISC_MEASURE_5,
		 ...
		 ISC_MEASURE_21,ISC_MEASURE_22,ISC_MEASURE_24,ISC_MEASURE_25,ISC_MEASURE_27,ISC_MEASURE_28';
 ...
 SELECT	items.value						VIEWBY,
	items.id						VIEWBYID,
 	'||l_drill||'
								ISC_ATTRIBUTE_2, -- drill on Booked Value
...
(SELECT	c.inv_org_id,	';
 ...
 SELECT	cust.value						VIEWBY,
	cust.id							VIEWBYID,
 	''pFunctionName=ISC_DBI_BOOK_DETAIL&VIEW_BY_NAME=VIEW_BY_ID&pParamIds=Y''
								ISC_ATTRIBUTE_2, -- drill on Booked Value
	NULL							ISC_ATTRIBUTE_3, -- drill across url
	NULL							ISC_ATTRIBUTE_4, -- item description
	NULL							ISC_ATTRIBUTE_5, -- item uom
...
(SELECT	c.customer_id,	';
 ...
 SELECT	ecat.value					VIEWBY,
	...
	decode(ecat.leaf_node_flag, ''Y'',
		''pFunctionName=ISC_DBI_FULF_PERF&VIEW_BY_NAME=VIEW_BY_ID&VIEW_BY=ITEM+ENI_ITEM_ORG&pParamIds=Y'',
		''pFunctionName=ISC_DBI_FULF_PERF&VIEW_BY_NAME=VIEW_BY_ID&VIEW_BY=ITEM+ENI_ITEM_VBH_CAT&pParamIds=Y'')
...
(SELECT	c.item_category_id,	';
  ...
  l_stmt := l_select_stmt || '
	c.curr_booked_qty					ISC_MEASURE_1, -- book qty
	c.curr_booked_value					ISC_MEASURE_2, -- book
	(c.curr_booked_value-c.prev_booked_value)
	  / decode(c.prev_booked_value, 0, NULL,
		   abs(c.prev_booked_value)) * 100		ISC_MEASURE_3, -- book change
	c.curr_fulfill_qty					ISC_MEASURE_4, -- fulf qty
	c.curr_fulfill_value					ISC_MEASURE_5, -- fulf
	(c.curr_fulfill_value-c.prev_fulfill_value)
	  / decode(c.prev_fulfill_value, 0, NULL,
		   abs(c.prev_fulfill_value)) * 100		ISC_MEASURE_6, -- fulf change
	c.curr_booked_value
	  / decode(c.curr_fulfill_value, 0, NULL,
		   c.curr_fulfill_value)			ISC_MEASURE_7, -- book to fulf r
	c.curr_booked_value
	  / decode(c.curr_fulfill_value, 0, NULL,
		   c.curr_fulfill_value) -
	c.prev_booked_value
	  / decode(c.prev_fulfill_value, 0, NULL,
		   c.prev_fulfill_value)			ISC_MEASURE_8, -- book to fulf r change
	sum(c.curr_booked_value) over ()			ISC_MEASURE_9, -- gd total book
	(sum(c.curr_booked_value) over () - sum(c.prev_booked_value) over ())
	  ...
	  * 100							ISC_MEASURE_10, -- gd total book change
	sum(c.curr_fulfill_value) over ()			ISC_MEASURE_11, -- gd total fulf
	(sum(c.curr_fulfill_value) over () - sum(c.prev_fulfill_value) over ())
	  ...
	  * 100							ISC_MEASURE_12, -- gd total fulf change
	sum(c.curr_booked_value) over ()
	  / decode(sum(c.curr_fulfill_value) over (), 0, NULL,
		   sum(c.curr_fulfill_value) over ())		ISC_MEASURE_13, -- gd total book to fulf r
	sum(c.curr_booked_value) over ()
	  / decode(sum(c.curr_fulfill_value) over (), 0, NULL,
		   sum(c.curr_fulfill_value) over ()) -
	sum(c.prev_booked_value) over ()
	  / decode(sum(c.prev_fulfill_value) over (), 0, NULL,
		   sum(c.prev_fulfill_value) over ())		ISC_MEASURE_14, -- gd total book to fulf r change
	c.curr_booked_value					ISC_MEASURE_15, -- KPI book
	c.prev_booked_value					ISC_MEASURE_16, -- KPI book prior
	c.curr_fulfill_value					ISC_MEASURE_17, -- KPI fulf
	c.prev_fulfill_value					ISC_MEASURE_18, -- KPI fulf prior
	c.curr_booked_value
	  / decode(c.curr_fulfill_value, 0, NULL,
		   c.curr_fulfill_value)			ISC_MEASURE_19, -- KPI book to fulf r
	c.prev_booked_value
	  / decode(c.prev_fulfill_value, 0, NULL,
		   c.prev_fulfill_value)			ISC_MEASURE_20, -- KPI book to fulf r prior
	sum(c.curr_booked_value) over ()
	  / decode(sum(c.curr_fulfill_value) over (), 0, NULL,
		   sum(c.curr_fulfill_value) over ())		ISC_MEASURE_21, -- KPI gd total book to fulf r
	sum(c.prev_booked_value) over ()
	  / decode(sum(c.prev_fulfill_value) over (), 0, NULL,
		   sum(c.prev_fulfill_value) over ())		ISC_MEASURE_22, -- KPI gd total book to fulf r prior
	sum(c.curr_booked_value) over ()			ISC_MEASURE_24, -- KPI gd total book
	sum(c.prev_booked_value) over ()			ISC_MEASURE_25, -- KPI gd total book prior
	sum(c.curr_fulfill_value) over ()			ISC_MEASURE_27, -- KPI gd total fulf
	sum(c.prev_fulfill_value) over ()			ISC_MEASURE_28 -- KPI gd total fulf prior
