--------------------------------------------------------
--  DDL for Package Body JTF_RS_GRP_RELATE_AUD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_RS_GRP_RELATE_AUD_PKG" as
/* $Header: jtfrstqb.pls 120.0 2005/05/11 08:22:30 appldev ship $ */
procedure INSERT_ROW (
  X_ROWID in out NOCOPY VARCHAR2,
  X_GROUP_RELATE_AUDIT_ID in NUMBER,
  X_GROUP_RELATE_ID in NUMBER,
  X_NEW_GROUP_ID in NUMBER,
  X_OLD_GROUP_ID in NUMBER,
  X_NEW_RELATED_GROUP_ID in NUMBER,
  X_OLD_RELATED_GROUP_ID in NUMBER,
  X_NEW_RELATION_TYPE in VARCHAR2,
  X_OLD_RELATION_TYPE in VARCHAR2,
  ...
  cursor C is select ROWID from JTF_RS_GRP_RELATE_AUD
    where GROUP_RELATE_AUDIT_ID = X_GROUP_RELATE_AUDIT_ID
    ;
begin
  insert into JTF_RS_GRP_RELATE_AUD (
    GROUP_RELATE_AUDIT_ID,
    GROUP_RELATE_ID,
    NEW_GROUP_ID,
    OLD_GROUP_ID,
    NEW_RELATED_GROUP_ID,
    OLD_RELATED_GROUP_ID,
    NEW_RELATION_TYPE,
    OLD_RELATION_TYPE,
    ...
    X_GROUP_RELATE_AUDIT_ID,
    X_GROUP_RELATE_ID,
    X_NEW_GROUP_ID,
    X_OLD_GROUP_ID,
    X_NEW_RELATED_GROUP_ID,
    X_OLD_RELATED_GROUP_ID,
    X_NEW_RELATION_TYPE,
    X_OLD_RELATION_TYPE,
...
procedure LOCK_ROW (
  ...
  cursor c1 is select
      ...
      NEW_OBJECT_VERSION_NUMBER,
      OLD_OBJECT_VERSION_NUMBER
    from JTF_RS_GRP_RELATE_AUD
    where GROUP_RELATE_AUDIT_ID = X_GROUP_RELATE_AUDIT_ID
    for update of GROUP_RELATE_AUDIT_ID nowait;
tlinfo c1%rowtype;
begin
 open c1;
   ...
   close c1;
      ...
      if (
           (tlinfo.GROUP_RELATE_ID = X_GROUP_RELATE_ID)
          ...
          AND ((tlinfo.NEW_RELATED_GROUP_ID = X_NEW_RELATED_GROUP_ID)
               OR ((tlinfo.NEW_RELATED_GROUP_ID is null) AND (X_NEW_RELATED_GROUP_ID is null)))
          AND ((tlinfo.OLD_RELATED_GROUP_ID = X_OLD_RELATED_GROUP_ID)
               OR ((tlinfo.OLD_RELATED_GROUP_ID is null) AND (X_OLD_RELATED_GROUP_ID is null)))
          AND ((tlinfo.NEW_RELATION_TYPE = X_NEW_RELATION_TYPE)
               OR ((tlinfo.NEW_RELATION_TYPE is null) AND (X_NEW_RELATION_TYPE is null)))
          AND ((tlinfo.OLD_RELATION_TYPE = X_OLD_RELATION_TYPE)
               OR ((tlinfo.OLD_RELATION_TYPE is null) AND (X_OLD_RELATION_TYPE is null)))
...
begin
  update JTF_RS_GRP_RELATE_AUD set
    GROUP_RELATE_ID = X_GROUP_RELATE_ID,
    NEW_GROUP_ID = X_NEW_GROUP_ID,
    OLD_GROUP_ID = X_OLD_GROUP_ID,
    NEW_RELATED_GROUP_ID = X_NEW_RELATED_GROUP_ID,
    OLD_RELATED_GROUP_ID = X_OLD_RELATED_GROUP_ID,
    NEW_RELATION_TYPE = X_NEW_RELATION_TYPE,
    OLD_RELATION_TYPE = X_OLD_RELATION_TYPE,
...
begin
  delete from JTF_RS_GRP_RELATE_AUD
  where GROUP_RELATE_AUDIT_ID = X_GROUP_RELATE_AUDIT_ID;
