--------------------------------------------------------
--  DDL for Package Body AME_ACF_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ACF_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_acf_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_application_id_i  number   default null;
g_action_type_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  ame_acf_ins.g_application_id_i := p_application_id;
  ame_acf_ins.g_action_type_id_i := p_action_type_id;
  --
...
Procedure dt_insert_dml
  (p_rec                     in out nocopy ame_acf_shd.g_rec_type
...
--
  Cursor C_Sel1 Is
    select t.creation_date
     ...
     where t.action_type_id = p_rec.action_type_id
 and    t.application_id = p_rec.application_id
       and t.start_date = ame_acf_shd.g_old_rec.start_date
...
--
  ...
  l_created_by          ame_action_type_config.created_by%TYPE;
  l_creation_date       ame_action_type_config.creation_date%TYPE;
  l_last_update_date    ame_action_type_config.last_update_date%TYPE;
  l_last_updated_by     ame_action_type_config.last_updated_by%TYPE;
  l_last_update_login   ame_action_type_config.last_update_login%TYPE;
...
Begin
 ...
 ,p_application_id =>  p_rec.application_id
  ...
  --
  --
  -- Insert the row into: ame_action_type_config
  --
...
--   action types belonging to a rule type. Used whenever order number is
--   passed as null.
--
-- Prerequisites:
--   None.
    ...
    if(l_temp_count = 0) then
      ...
      select (nvl(max(order_number), 0) + 1)
        ...
        where
          ...
          ame_action_type_config.application_id = p_application_id and
          ame_action_type_usages.rule_type = l_rule_type and
          p_effective_date between ame_action_type_config.start_date and
            nvl(ame_action_type_config.end_date - ame_util.oneSecond, p_effective_date) and
          p_effective_date between ame_action_type_usages.start_date and
            nvl(ame_action_type_usages.end_date - ame_util.oneSecond, p_effective_date);
    end if;
  else
...
--
 -- Cursor C_Sel1 is select ame_action_type_config_s.nextval from sys.dual;
--
 Cursor C_Sel2 is
    Select null
      from ame_action_type_config
     where application_id =
             ame_acf_ins.g_application_id_i
       and action_type_id =
             ame_acf_ins.g_action_type_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
    If (ame_acf_ins.g_application_id_i is not null or
      ame_acf_ins.g_action_type_id_i is not null) Then
    --
    ...
    p_rec.application_id :=
      ame_acf_ins.g_application_id_i;
    ame_acf_ins.g_application_id_i := null;
    p_rec.action_type_id :=
      ame_acf_ins.g_action_type_id_i;
    ame_acf_ins.g_action_type_id_i := null;
  END If;
  /*
        Else
    --
  ...
  End If;
  */
  --The following call generates max(order_number) if order number is passed
  --as null. Else it calls chk_order_number internally. Only for insert.
  l_order_number := p_rec.order_number;
  generate_order_number
  (
    p_effective_date => p_effective_date,
          p_application_id => p_rec.application_id,
  ...
  p_rec.order_number := l_order_number;
  --
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'AME_ACTION_TYPE_CONFIG'
        ,p_hook_type   => 'AI');
      --
  end;
  --
...
Begin
  ...
  --
  -- Having converted the arguments into the ame_acf_rec
  ...
  --
  /*
  p_action_type_id := l_rec.action_type_id;
