--------------------------------------------------------
--  DDL for Package Body HR_TEMPLATE_CANVASES_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_TEMPLATE_CANVASES_B_PKG" as
/* $Header: hrtculct.pkb 115.2 2002/12/11 07:04:49 raranjan noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from HR_TEMPLATE_CANVASES_B
...
procedure LOCK_ROW (
  ...
  cursor c1 is select
    ...
    where TEMPLATE_CANVAS_ID = X_TEMPLATE_CANVAS_ID
    for update of TEMPLATE_CANVAS_ID nowait;
begin
  for tlinfo in c1 loop
          if (tlinfo.OBJECT_VERSION_NUMBER = X_OBJECT_VERSION_NUMBER)
          AND (tlinfo.TEMPLATE_WINDOW_ID = X_TEMPLATE_WINDOW_ID)
...
begin
  update HR_TEMPLATE_CANVASES_B set
...
begin
 ...
 exception
   when no_data_found then
     select hr_template_canvases_b_s.nextval
     into x_template_canvas_id
     from dual;
 end;
...
end HR_TEMPLATE_CANVASES_B_PKG;
