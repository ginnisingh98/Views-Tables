--------------------------------------------------------
--  DDL for Package Body IGS_PE_COURSE_EXCL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PE_COURSE_EXCL_PKG" AS
 /* $Header: IGSNI13B.pls 115.3 2002/11/29 01:17:09 nsidana ship $ */



  l_rowid VARCHAR2(25);
  old_references IGS_PE_COURSE_EXCL%RowType;
  new_references IGS_PE_COURSE_EXCL%RowType;
  ...
  PROCEDURE Set_Column_Values (
    ...
    x_pce_start_dt IN DATE DEFAULT NULL,
  ...
  BEGIN
    ...
    new_references.pce_start_dt := x_pce_start_dt;
...
v_message_name  varchar2(30);
  BEGIN
	...
	IF (new_references.pce_start_dt IS NOT NULL) AND
		(p_inserting OR (p_updating AND
		(NVL(old_references.pce_start_dt, IGS_GE_DATE.IGSDATE('1900/01/01'))
		<> new_references.pce_start_dt)))
		THEN
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_pce_ar_iu
  -- AFTER INSERT OR UPDATE
  -- ON IGS_PE_COURSE_EXCL
  -- FOR EACH ROW
  ...
  BEGIN
	-- Validate for open ended IGS_PE_PERSON IGS_PS_COURSE exclusion records.
...
UPPER(new_references.s_encmb_effect_type) Then
 ...
 END IF;
  ...
  FUNCTION Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      ...
      AND      pce_start_dt = x_pce_start_dt
      FOR UPDATE NOWAIT;
 ...
 PROCEDURE GET_FK_IGS_PS_COURSE (
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PCE_CRS_FK');
       IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
 ...
 END GET_FK_IGS_PS_COURSE;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PCE_PEE_FK');
       IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
    ...
    Set_Column_Values (
     ...
     BeforeRowInsertUpdate1 ( p_inserting => TRUE );
      IF  Get_PK_For_Validation (
   		...
   		new_references.course_cd ,
    		new_references.pce_start_dt ) THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PE_COURSE_EXCL
      where PERSON_ID = X_PERSON_ID
      and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
      and PEN_START_DT = X_PEN_START_DT
      and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PE_COURSE_EXCL
     where PERSON_ID = X_PERSON_ID
     and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
     and PEN_START_DT = X_PEN_START_DT
     and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
 Before_DML(
  ...
  delete from IGS_PE_COURSE_EXCL
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
