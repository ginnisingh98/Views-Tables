--------------------------------------------------------
--  DDL for Package Body XNP_CSENICRT_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSENICRT_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSENICRT' ;
  ...
  XNP_XML_UTILS.write_leaf_element ( 'PARTY_SITE_ID', XNP$PARTY_SITE_ID );
  XNP_XML_UTILS.write_leaf_element ( 'WORK_ORDER_NUMBER', XNP$WORK_ORDER_NUMBER );
  XNP_XML_UTILS.write_leaf_element ( 'PROJECT_ID', XNP$PROJECT_ID );
  XNP_XML_UTILS.write_leaf_element ( 'TASK_ID', XNP$TASK_ID );
  XNP_XML_UTILS.write_leaf_element ( 'TRANSACTION_DATE', XNP$TRANSACTION_DATE );
  XNP_XML_UTILS.write_leaf_element ( 'NETWORK_LOC_ID', XNP$NETWORK_LOC_ID );
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSENICRT' ) ;
    ...
    XNP_XML_UTILS.write_leaf_element ( 'QUANTITY', XNP$QUANTITY ) ;
    XNP_XML_UTILS.write_leaf_element ( 'WORK_ORDER_NUMBER', XNP$WORK_ORDER_NUMBER ) ;
    IF ( XNP$PROJECT_ID IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - PROJECT_ID' ;
     fnd_message.set_name('XNP','XNP_MISSING_MANDATORY_ATTR');
    ...
    XNP_XML_UTILS.write_leaf_element ( 'PARTY_SITE_ID', XNP$PARTY_SITE_ID ) ;
    XNP_XML_UTILS.write_leaf_element ( 'NETWORK_LOC_ID', XNP$NETWORK_LOC_ID ) ;
    IF ( XNP$TASK_ID IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - TASK_ID' ;
     fnd_message.set_name('XNP','XNP_MISSING_MANDATORY_ATTR');
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'TRANSACTED_BY', XNP$TRANSACTED_BY ) ;
    XNP_XML_UTILS.end_segment ( 'CSENICRT' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSENICRT.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSENICRT', l_consumer_list );
  END IF;
...
/* create the XML message */
  CREATE_MSG (
    ...
    XNP$PARTY_SITE_ID=>XNP$PARTY_SITE_ID,
    XNP$WORK_ORDER_NUMBER=>XNP$WORK_ORDER_NUMBER,
    XNP$PROJECT_ID=>XNP$PROJECT_ID,
    XNP$TASK_ID=>XNP$TASK_ID,
    XNP$TRANSACTION_DATE=>XNP$TRANSACTION_DATE,
    XNP$NETWORK_LOC_ID=>XNP$NETWORK_LOC_ID,
    x_msg_header=>l_msg_header,
