--------------------------------------------------------
--  DDL for Package Body HR_FTP_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_FTP_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_ftp_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_ftp_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_form_tab_pages_b Row
  --
  update hr_form_tab_pages_b
    set
     form_tab_page_id                = p_rec.form_tab_page_id
    ,object_version_number           = p_rec.object_version_number
    ,form_canvas_id                  = p_rec.form_canvas_id
    ,tab_page_name                   = p_rec.tab_page_name
    ,display_order                   = p_rec.display_order
...
Begin
  ...
  If (p_rec.form_canvas_id = hr_api.g_number) then
    ...
    hr_ftp_shd.g_old_rec.form_canvas_id;
  End If;
  If (p_rec.tab_page_name = hr_api.g_varchar2) then
    p_rec.tab_page_name :=
    hr_ftp_shd.g_old_rec.tab_page_name;
  End If;
  If (p_rec.display_order = hr_api.g_number) then
    p_rec.display_order :=
    hr_ftp_shd.g_old_rec.display_order;
  End If;
...
Begin
  ...
  --
  hr_ftp_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_ftp_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_ftp_upd.post_update
     (p_effective_date
     ,p_rec
     );
End upd;
