--------------------------------------------------------
--  DDL for Package Body ISC_DBI_PDUE_PRM_DETAIL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_PDUE_PRM_DETAIL_PKG" AS
/* $Header: ISCRGA8B.pls 120.1 2005/10/18 13:08:21 hprathur noship $ */
...
-- Retrieve record to get bucket ranges
  ...
  IF (l_low IS NULL)
    ...
    ELSE l_bucket_low_where := '
	    AND	fact.days_late_promise >= :ISC_LOW';
  END IF;

  IF (l_high IS NULL)
    THEN l_bucket_high_where := '';
    ELSE l_bucket_high_where := '
	    AND	fact.days_late_promise < :ISC_HIGH';
  END IF;
...
(SELECT	fact.customer_id					CUSTOMER_ID,
	...
	fact.days_late_promise					ISC_MEASURE_2, -- days late
...
END ISC_DBI_PDUE_PRM_DETAIL_PKG;
