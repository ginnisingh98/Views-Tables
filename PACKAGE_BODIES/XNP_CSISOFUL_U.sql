--------------------------------------------------------
--  DDL for Package Body XNP_CSISOFUL_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSISOFUL_U" AS 
PROCEDURE CREATE_MSG  (   XNP$ORDER_LINE_ID NUMBER,
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSISOFUL' ;
  ...
  XNP_XML_UTILS.write_leaf_element ( 'ORDER_LINE_ID', XNP$ORDER_LINE_ID );
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSISOFUL' ) ;
    IF ( XNP$ORDER_LINE_ID IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - ORDER_LINE_ID' ;
     fnd_message.set_name('XNP','XNP_MISSING_MANDATORY_ATTR');
     fnd_message.set_token('ATTRIBUTE','ORDER_LINE_ID' ) ;
     x_error_message := fnd_message.get ; 
      RAISE e_MISSING_MANDATORY_DATA ;
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORDER_LINE_ID', XNP$ORDER_LINE_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSISOFUL' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSISOFUL.create_msg()::' || SQLERRM ;
END ;
PROCEDURE PUBLISH  (   XNP$ORDER_LINE_ID NUMBER,
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSISOFUL', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_EVT_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$ORDER_LINE_ID=>XNP$ORDER_LINE_ID,
    x_msg_header=>l_msg_header,
...
END ;
PROCEDURE SEND  (   XNP$ORDER_LINE_ID NUMBER,
  x_message_id OUT  NUMBER,
  x_error_code OUT  NUMBER,
  x_error_message OUT VARCHAR2,  
...
NULL ;
DECLARE
      l_txn_type    varchar2(30) := 'CSISOFUL';
    ...
    BEGIN
      ...
      xnp_xml_utils.decode(
        p_msg_text => p_msg_text, p_tag => 'ORDER_LINE_ID', x_value => l_txn_id);
      IF l_txn_id is null THEN
        l_error_rec.error_text := 'xnp_xml_utils.decode failed. Could not decode the txn id.';
        RAISE dpl_error;
      END IF;
      --l_error_rec.inv_material_transaction_id := l_txn_id; commented for bug 2865384
