--------------------------------------------------------
--  DDL for Package Body ENG_CHANGES_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ENG_CHANGES_EXT_PKG" as
/* $Header: ENGUCEB.pls 115.1 2003/05/30 10:58:54 akumar noship $ */
procedure ADD_LANGUAGE
is
begin

delete from ENG_CHANGES_EXT_TL T
  where not exists
    (select NULL
    from ENG_CHANGES_EXT_TL B
    where
           B.EXTENSION_ID = T.EXTENSION_ID
       and B.CHANGE_ID = T.CHANGE_ID
  ...
  update ENG_CHANGES_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.ATTR_GROUP_ID,
      SUBT.LANGUAGE
    from ENG_CHANGES_EXT_TL SUBB, ENG_CHANGES_EXT_TL SUBT
    where SUBB.EXTENSION_ID = SUBT.EXTENSION_ID
    and SUBB.CHANGE_ID = SUBT.CHANGE_ID
