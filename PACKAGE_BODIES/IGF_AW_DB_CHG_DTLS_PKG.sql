--------------------------------------------------------
--  DDL for Package Body IGF_AW_DB_CHG_DTLS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_AW_DB_CHG_DTLS_PKG" AS
/* $Header: IGFWI64B.pls 120.1 2005/09/28 05:31:35 appldev noship $ */

  l_rowid VARCHAR2(25);
  old_references igf_aw_db_chg_dtls%ROWTYPE;
  new_references igf_aw_db_chg_dtls%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.pymnt_prd_start_date              := x_pymnt_prd_start_date;
    new_references.note_message                      := x_note_message;
    new_references.batch_id_txt                          := x_batch_id_txt;
    new_references.ack_date                          := x_ack_date;
    new_references.booking_id_txt                        := x_booking_id_txt;
    new_references.booking_date                      := x_booking_date;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_AW_AWD_DBCHG_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.disb_accepted_amt = x_disb_accepted_amt)
        ...
        AND ((tlinfo.disb_date = x_disb_date) OR ((tlinfo.disb_date IS NULL) AND (X_disb_date IS NULL)))
        ...
        AND ((tlinfo.pymnt_prd_start_date = x_pymnt_prd_start_date) OR ((tlinfo.pymnt_prd_start_date IS NULL) AND (X_pymnt_prd_start_date IS NULL)))
        AND ((tlinfo.note_message = x_note_message) OR ((tlinfo.note_message IS NULL) AND (X_note_message IS NULL)))
        AND ((tlinfo.batch_id_txt = x_batch_id_txt) OR ((tlinfo.batch_id_txt IS NULL) AND (X_batch_id_txt IS NULL)))
        AND ((tlinfo.ack_date = x_ack_date) OR ((tlinfo.ack_date IS NULL) AND (X_ack_date IS NULL)))
        AND ((tlinfo.booking_id_txt = x_booking_id_txt) OR ((tlinfo.booking_id_txt IS NULL) AND (X_booking_id_txt IS NULL)))
        AND ((tlinfo.booking_date = x_booking_date) OR ((tlinfo.booking_date IS NULL) AND (X_booking_date IS NULL)))
       ) THEN
      NULL;
    ELSE
