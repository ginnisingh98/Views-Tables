--------------------------------------------------------
--  DDL for Package Body CSM_AD_SIMPLE_SRCH_EVENT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSM_AD_SIMPLE_SRCH_EVENT_PKG" AS
/* $Header: csmeassb.pls 120.5 2008/02/22 08:52:06 trajasek noship $ */
/*** Globals ***/
g_ad_simple_sch_acc_tab_name          CONSTANT VARCHAR2(30) := 'CSM_AD_SIMPLE_SEARCH_ACC';
g_ad_simple_sch_table_name            CONSTANT VARCHAR2(30) := 'CSM_AD_SIMPLE_SEARCH_VIEW';
g_ad_simple_sch_seq_name              CONSTANT VARCHAR2(30) := 'CSM_AD_SIMPLE_SEARCH_ACC_S';
g_ad_simple_sch_pubi_name             CONSTANT VARCHAR2(30) := 'CSM_AD_SIMPLE_SEARCH';

g_ad_simple_sch_pkg_name              CONSTANT VARCHAR2(30) := 'CSM_AD_SIMPLE_SRCH_EVENT_PKG';
g_ad_simple_sch_api_name              CONSTANT VARCHAR2(30) := 'REFRESH_ACC';

g_access_list                           asg_download.access_list;

PROCEDURE GET_ACCESS_LIST(p_access_id IN CSM_AD_SIMPLE_SEARCH_ACC.ACCESS_ID%TYPE ,
                          p_count IN NUMBER)
IS
BEGIN
...
TYPE l_conc_rec IS RECORD
 (
 ACCESS_ID   CSM_AD_SIMPLE_SEARCH_ACC.ACCESS_ID%TYPE,
 ID          CSM_AD_SIMPLE_SEARCH_VIEW.ID%TYPE
 );
...
BEGIN
  ...
  --delete--
   ...
   LOOP
      IF l_user_list.COUNT > 0 THEN
        ...
        l_dummy := asg_download.mark_dirty(g_ad_simple_sch_pubi_name,g_access_list,l_user_list, 'D', sysdate );
      END IF;
  ...
  --update--
   ...
   LOOP
      IF l_user_list.COUNT > 0 THEN
        GET_ACCESS_LIST(l_tab(I).ACCESS_ID,l_user_list.COUNT);
        l_dummy := asg_download.mark_dirty(g_ad_simple_sch_pubi_name,g_access_list,l_user_list, 'U', sysdate );
      END IF;
   ...
   LOOP
      ...
      IF l_user_list.COUNT > 0 THEN
        ...
        l_dummy:= ASG_DOWNLOAD.mark_dirty(g_ad_simple_sch_pubi_name,g_access_list,l_user_list,'I',SYSDATE);
      END IF;
   ...
   p_status  := 'FINE';
   p_message := 'CSM_AD_SIMPLE_SRCH_EVENT_PKG.REFRESH_ACC executed successfully';
...
EXCEPTION
   WHEN OTHERS THEN
      ...
      p_status := 'ERROR';
      p_message := 'Error in CSM_AD_SIMPLE_SRCH_EVENT_PKG.REFRESH_ACC: ' || l_sqlerrno || ':' || l_sqlerrmsg;
      ROLLBACK;
      csm_util_pkg.log('CSM_AD_SIMPLE_SRCH_EVENT_PKG.REFRESH_ACC ERROR : ' || l_sqlerrno || ':' || l_sqlerrmsg);
