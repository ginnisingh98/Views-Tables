--------------------------------------------------------
--  DDL for Package Body GMO_INSTR_SET_DEFN_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMO_INSTR_SET_DEFN_PKG" as
/* $Header: GMOINSDB.pls 120.0 2005/06/29 04:21:30 shthakke noship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from GMO_INSTR_SET_DEFN_B
    where INSTRUCTION_SET_ID = X_INSTRUCTION_SET_ID
    ;
begin
  insert into GMO_INSTR_SET_DEFN_B (
    INSTRUCTION_SET_ID,
    INSTRUCTION_TYPE,
    INSTR_SET_NAME,
    ENTITY_NAME,
  ...
  insert into GMO_INSTR_SET_DEFN_TL (
    INSTRUCTION_SET_ID,
    INSTR_SET_DESC,
    CREATION_DATE,
    CREATED_BY,
...
begin
  ...
  close c;
  if (    ((recinfo.INSTRUCTION_TYPE = X_INSTRUCTION_TYPE)
...
is
begin
  delete from GMO_INSTR_SET_DEFN_TL T
  where not exists
    (select NULL
    from GMO_INSTR_SET_DEFN_B B
    where B.INSTRUCTION_SET_ID = T.INSTRUCTION_SET_ID
    );

  update GMO_INSTR_SET_DEFN_TL T set (
      INSTR_SET_DESC
    ) = (select
      B.INSTR_SET_DESC
    from GMO_INSTR_SET_DEFN_TL B
    where B.INSTRUCTION_SET_ID = T.INSTRUCTION_SET_ID
  ...
  where (
      ...
      SUBT.LANGUAGE
    from GMO_INSTR_SET_DEFN_TL SUBB, GMO_INSTR_SET_DEFN_TL SUBT
  ...
  insert into GMO_INSTR_SET_DEFN_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GMO_INSTR_SET_DEFN_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
