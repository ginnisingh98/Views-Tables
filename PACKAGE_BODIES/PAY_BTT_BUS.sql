--------------------------------------------------------
--  DDL for Package Body PAY_BTT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_BTT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_btt_bus.';  -- Global package name
--
...
Begin
  ...
  --
  --
  if (( nvl(pay_btt_bus.g_balance_type_id, hr_api.g_number)
       = p_balance_type_id)
  and ( nvl(pay_btt_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pay_btt_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_btt_bus.g_balance_type_id             := p_balance_type_id;
    pay_btt_bus.g_language                    := p_language;
    pay_btt_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_btt_shd.api_updating
