--------------------------------------------------------
--  DDL for Package Body IGI_DOS_SOURCES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_DOS_SOURCES_PKG" AS
/* $Header: igidosqb.pls 120.5.12000000.2 2007/06/14 05:42:15 pshivara ship $ */
...
l_unexp_level   number := FND_LOG.LEVEL_UNEXPECTED ;
  ...
  l_rowid VARCHAR2(25);
  old_references igi_dos_sources%ROWTYPE;
  new_references igi_dos_sources%ROWTYPE;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_sources_pkg.set_column_values.Msg1',FALSE);
      END IF;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_sources_pkg.check_parent_existance.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
  ...
  */
  BEGIN
    ...
    igi_dos_destinations_pkg.get_fk_igi_dos_sources (
      old_references.source_id
    );


    igi_dos_source_usages_pkg.get_fk_igi_dos_sources (
      old_references.source_id
    );
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_sources_pkg.get_fk_igi_dos_doc_types.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
      RETURN;
...
-- bug 3199481, start block
        IF (l_unexp_level >= l_debug_level) THEN
           FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_sources_pkg.before_dml.Msg1',FALSE);
        END IF;
-- bug 3199481, end block
        app_exception.raise_exception;
      END IF;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_sources_pkg.lock_row.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      CLOSE c1;
      app_exception.raise_exception;
    ...
    IF (
        (tlinfo.dossier_id = x_dossier_id)
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_sources_pkg.lock_row.Msg2',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
