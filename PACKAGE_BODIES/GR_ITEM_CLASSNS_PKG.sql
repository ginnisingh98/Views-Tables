--------------------------------------------------------
--  DDL for Package Body GR_ITEM_CLASSNS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_ITEM_CLASSNS_PKG" AS
/*$Header: GRHIIHCB.pls 115.6 2002/10/25 19:59:08 methomas ship $*/
...
/*     Initialization Routine */
   ...
   x_oracle_error := 0;
   x_msg_data := NULL;
   l_msg_token := p_item_code || ' ' || p_hazard_classification_code;
...
CURSOR c_lock_item_classns
 IS
   SELECT	*
   FROM		gr_item_classns
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockClassnRcd	  c_lock_item_classns%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_item_classns;
   FETCH c_lock_item_classns INTO LockClassnRcd;
   IF c_lock_item_classns%NOTFOUND THEN
	  CLOSE c_lock_item_classns;
...
**		'H' - Delete all rows for the specified hazard classification.
*/
   IF p_delete_option = 'I' THEN
	  ...
	  ELSE
   	  ...
   	  END IF;
   ELSIF p_delete_option = 'H' THEN
      IF p_hazard_classification_code IS NULL THEN
...
/*   Hazard Classification */
   ...
   GR_EUROHAZARDS_B_PKG.Check_Primary_Key
						(p_hazard_classification_code,
						 'F',
						 l_rowid,
						 l_key_exists);
...
**	      p_hazard_classification_code is the second part of the key
...
CURSOR c_get_item_classn_rowid
 IS
   SELECT ihc.rowid
   FROM	  gr_item_classns ihc
   WHERE  ihc.item_code = p_item_code
   AND	  ihc.hazard_classification_code = p_hazard_classification_code;
ItemClassnRecord		c_get_item_classn_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_item_code || ' ' || p_hazard_classification_code;

   x_key_exists := 'F';
   OPEN c_get_item_classn_rowid;
   FETCH c_get_item_classn_rowid INTO ItemClassnRecord;
   IF c_get_item_classn_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := ItemClassnRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_item_classn_rowid;
