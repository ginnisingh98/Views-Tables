--------------------------------------------------------
--  DDL for Package Body CAC_VIEW_AVAIL_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CAC_VIEW_AVAIL_PVT" AS
/* $Header: cacvavb.pls 120.3 2006/04/27 06:57:12 sbarat noship $ */
...
** - ResourceID
** - ResourcType
** - InitialValue (1 for available, 0 for unavailable)
...
BEGIN
  ...
  THEN
    l_ResourceName := jtf_task_utl.get_owner(p_object_type_code => p_ResourceType
                                            ,p_object_id => p_ResourceID);
  ELSE
...
BEGIN
  IF p_free_busy = 'BUSY' THEN
    l_slot_code := 0;
  ELSIF p_free_busy = 'FREE' THEN
    l_slot_code := 1;
  ELSIF p_free_busy = 'TENTATIVE' THEN
    l_slot_code := 2;
  ELSE
    l_slot_code := 0;
  END IF;

  FOR i IN p_FirstSlot..p_LastSlot
  LOOP
    p_list(p_StartRecord + (i-1) ).SlotAvailable := l_slot_code;
  END LOOP;
  ...
  *****************************************************************************/
  ...
  LOOP <<RESOURCES>>
    ...
    ***************************************************************************/
    ...
    LOOP <<RESOURCE_TASKS>>
    /* amigupta, If the Resource Type is PARTY_RELATIONSHIP i.e Contact don't check the availability and combined availability
    */
    IF( p_RSList(i).ResourceType <> 'PARTY_RELATIONSHIP')
     THEN
      ...
      ***************************************************************************/
      ...
      THEN
        ...
        *************************************************************************/
        ...
        IF (l_ItemDisplayType IN (1,5) AND r_ResourceTask.FreeBusyType <> 'FREE')
        THEN
          /*************************************************************************
          ** This procedure will determine what slot are unavailable because of
          ** the task that is fetched
...
IS
  ...
  CURSOR c_resources (b_task_id NUMBER) IS
...
BEGIN
  ...
  *****************************************************************************/
  ...
  FOR rec IN c_resources (p_task_id)
  LOOP
    i := i + 1;
    l_resource_list(i).resourceID   := rec.resource_id;
    l_resource_list(i).resourceType := rec.resource_type_code;
    l_resource_list(i).resourceName := rec.resource_name;
  END LOOP;

  CHECK_AVAILABILITY
  ( p_api_version         => p_api_version
  , p_init_msg_list       => p_init_msg_list
  , p_RSList              => l_resource_list
  , p_StartDateTime       => p_StartDateTime
  , p_EndDateTime         => p_EndDateTime
  , p_SlotSize            => p_SlotSize
  , x_NumberOfSlots       => x_NumberOfSlots
  , x_AvailbltyList       => x_AvailbltyList
  , x_TotalAvailbltyList  => x_TotalAvailbltyList
