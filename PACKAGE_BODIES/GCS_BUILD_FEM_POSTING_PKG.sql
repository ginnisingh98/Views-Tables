--------------------------------------------------------
--  DDL for Package Body GCS_BUILD_FEM_POSTING_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GCS_BUILD_FEM_POSTING_PKG" AS
/* $Header: gcsfempdb.pls 120.8 2007/03/27 20:25:46 skamdar noship $ */
--
-- Package
--   Create_Package
-- Purpose
...
--
  ...
  BEGIN
    ...
    IF (FND_LOG.G_CURRENT_RUNTIME_LEVEL <= FND_LOG.LEVEL_PROCEDURE) THEN
      FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'GCS_BUILD_FEM_POSTING_PKG' || '.' || 'CREATE_PACKAGE',
                     GCS_UTILITY_PKG.g_module_enter || 'CREATE_PACKAGE' ||
                     to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));
    END IF;
    --FND_FILE.PUT_LINE(FND_FILE.LOG, GCS_UTILITY_PKG.g_module_enter || 'CREATE_PACKAGE' || to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));


     -- Create the package body
body:=
'CREATE OR REPLACE PACKAGE BODY GCS_DYN_FEM_POSTING_PKG AS
...
';
     ...
     r := GCS_DYNAMIC_UTIL_PKG.Build_Comma_List('     ', GCS_UTILITY_PKG.g_nl, '', r);
...
';
     ...
     r := GCS_DYNAMIC_UTIL_PKG.Build_Comma_List('   GLE.', GCS_UTILITY_PKG.g_nl, '', r);
body:=
...
';
        ...
        r := GCS_DYNAMIC_UTIL_PKG.Build_Comma_List('   GFB.', GCS_UTILITY_PKG.g_nl, '', r);
...
';
     ...
     r := GCS_DYNAMIC_UTIL_PKG.Build_Fem_Comma_List('     ', GCS_UTILITY_PKG.g_nl, '', r, 'N');
...
';
         ...
         r := r + 1;
         END LOOP;
    r := GCS_DYNAMIC_UTIL_PKG.Build_Fem_Comma_List('        ', GCS_UTILITY_PKG.g_nl, '', r, 'Y');
body:=
'       l_req_id,
        p_object_id,
        l_req_id,
...
';
        ...
        r := GCS_DYNAMIC_UTIL_PKG.Build_Fem_Comma_List('FB.', GCS_UTILITY_PKG.g_nl, '', r, 'N');

        body:= 'FB.CREATED_BY_REQUEST_ID,
...
';
        ...
        r := GCS_DYNAMIC_UTIL_PKG.Build_Fem_Comma_List('GFB.', GCS_UTILITY_PKG.g_nl, '', r, 'Y');

        body := ' GFB.CREATED_BY_REQUEST_ID,
...
';
   ...
   ad_ddl.create_plsql_object(GCS_DYNAMIC_UTIL_PKG.g_applsys_username, 'GCS', 'GCS_DYN_FEM_POSTING_PKG',1, r - 1, 'FALSE', err);

    -- dbms_output.put_line('Error' || AD_DDL.error_buf);

    IF (FND_LOG.G_CURRENT_RUNTIME_LEVEL <= FND_LOG.LEVEL_PROCEDURE) THEN
      FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'GCS_BUILD_FEM_POSTING_PKG' || '.' || 'CREATE_PACKAGE',
                     GCS_UTILITY_PKG.g_module_success || 'Create package' ||
                     to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));
    END IF;
    -- FND_FILE.PUT_LINE(FND_FILE.LOG, GCS_UTILITY_PKG.g_module_success || 'CREATE PACKAGE' || to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));
  ...
  EXCEPTION
    WHEN OTHERS THEN
      IF (FND_LOG.G_CURRENT_RUNTIME_LEVEL <= FND_LOG.LEVEL_UNEXPECTED) THEN
        ...
        FND_LOG.STRING(FND_LOG.LEVEL_UNEXPECTED,
                       'GCS_BUILD_FEM_POSTING_PKG' || '.' || 'CREATE_PACKAGE',
                       GCS_UTILITY_PKG.g_module_failure || 'CREATE_PACKAGE' ||
                       to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));
      END IF;
      -- FND_FILE.PUT_LINE(FND_FILE.LOG, GCS_UTILITY_PKG.g_module_failure ||
      --                  'CREATE_PACKAGE' || to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));
  END Create_Package;
