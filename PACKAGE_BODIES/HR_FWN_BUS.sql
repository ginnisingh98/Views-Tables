--------------------------------------------------------
--  DDL for Package Body HR_FWN_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_FWN_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_fwn_bus.';  -- Global package name
--
...
Begin
  ...
  --
  IF NOT hr_fwn_shd.api_updating
  ...
  IF (nvl(p_rec.application_id,hr_api.g_number) <>
      nvl(hr_fwn_shd.g_old_rec.application_id,hr_api.g_number)
     ) THEN
  ...
  IF (nvl(p_rec.form_id,hr_api.g_number) <>
      nvl(hr_fwn_shd.g_old_rec.form_id,hr_api.g_number)
     ) THEN
     l_argument := 'form_window_id';
     RAISE l_error;
  END IF;
...
-- |----------------------------< chk_window_name >---------------------------|
-- ----------------------------------------------------------------------------
Procedure chk_window_name
  (p_form_window_id               in number
  ,p_object_version_number        in number
  ,p_window_name                  in varchar2
  ) is
  --
  l_proc                         varchar2(72) := g_package || 'chk_window_name';
  l_api_updating                 boolean;
  --
Begin
  hr_utility.set_location('Entering:'||l_proc, 10);
  ...
  hr_api.mandatory_arg_error
    (p_api_name                     => l_proc
    ,p_argument                     => 'window_name'
    ,p_argument_value               => p_window_name
    );
  --
  -- Check value is in uppercase
  --
  if p_window_name <> upper(p_window_name) then
    fnd_message.set_name('PAY', 'HR_6153_ALL_PROCEDURE_FAIL');
    fnd_message.set_token('PROCEDURE', l_proc);
    fnd_message.set_token('STEP','10');
    fnd_message.raise_error;
...
Begin
  ...
  chk_window_name
    (p_form_window_id               => p_rec.form_window_id
    ,p_object_version_number        => p_rec.object_version_number
    ,p_window_name                  => p_rec.window_name
    );
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End insert_validate;
