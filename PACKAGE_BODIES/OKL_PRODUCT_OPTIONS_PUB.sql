--------------------------------------------------------
--  DDL for Package Body OKL_PRODUCT_OPTIONS_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_PRODUCT_OPTIONS_PUB" AS
/* $Header: OKLPPONB.pls 115.9 2004/04/13 10:57:34 rnaik noship $ */

  PROCEDURE add_language IS
  BEGIN
--    okl_pon_pvt.add_language;
    NULL;
  END add_language;
  ...
  BEGIN
    x_return_status := FND_API.G_RET_STS_SUCCESS;
    SAVEPOINT lock_product_options;
    l_ponv_rec := p_ponv_rec;

    okl_pon_pvt.lock_row(
                               p_api_version   => p_api_version
                              ,p_init_msg_list => p_init_msg_list
                              ,x_return_status => l_return_status
                              ,x_msg_count     => x_msg_count
  ...
  EXCEPTION
    WHEN FND_API.G_EXC_ERROR THEN
      ROLLBACK TO lock_product_options;
      x_return_status := FND_API.G_RET_STS_ERROR;
    ...
    WHEN OTHERS THEN
      FND_MSG_PUB.ADD_EXC_MSG('OKL_PRODUCT_OPTIONS_PUB','lock_product_options');
      -- store SQL error message on message stack for caller
      FND_MSG_PUB.Count_and_get(p_encoded => OKC_API.G_FALSE
                               ,p_count   => x_msg_count
                               ,p_data    => x_msg_data);
  ...
  PROCEDURE validate_product_options(
    ...
    l_data                            VARCHAR2(100);
    l_count                           NUMBER;
    l_api_name                        CONSTANT VARCHAR2(30)  := 'validate_product_options';
    l_return_status                   VARCHAR2(1)    := FND_API.G_RET_STS_SUCCESS;
  BEGIN
    x_return_status := FND_API.G_RET_STS_SUCCESS;
    SAVEPOINT validate_product_options;
    l_ponv_rec := p_ponv_rec;



    okl_pon_pvt.validate_row(
                               p_api_version   => p_api_version
                              ,p_init_msg_list => p_init_msg_list
                              ,x_return_status => l_return_status
                              ,x_msg_count     => x_msg_count
  ...
  EXCEPTION
    WHEN FND_API.G_EXC_ERROR THEN
      ROLLBACK TO validate_product_options;
      x_return_status := FND_API.G_RET_STS_ERROR;
    ...
    WHEN OTHERS THEN
      FND_MSG_PUB.ADD_EXC_MSG('OKL_PRODUCT_OPTIONS_PUB','validate_product_options');
      -- store SQL error message on message stack for caller
      FND_MSG_PUB.Count_and_get(p_encoded => OKC_API.G_FALSE
                               ,p_count   => x_msg_count
                               ,p_data    => x_msg_data);
