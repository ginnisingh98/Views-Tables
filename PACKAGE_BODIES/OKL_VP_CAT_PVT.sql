--------------------------------------------------------
--  DDL for Package Body OKL_VP_CAT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_VP_CAT_PVT" AS
/* $Header: OKLSCARB.pls 115.13 2003/12/16 09:44:29 kmotepal noship $ */
/*+++++++++++++Start of hand code +++++++++++++++++*/
...
/*-------------Commented in favor of hand code------
    ...
    FUNCTION validate_foreign_keys (
      ...
      l_row_notfound                 BOOLEAN := TRUE;
    BEGIN
      IF (p_catv_rec.CHR_ID IS NOT NULL)
      THEN
        OPEN okc_chrv_pk_csr(p_catv_rec.CHR_ID);
        ...
        END IF;
      END IF;
      IF (p_catv_rec.CLE_ID IS NOT NULL)
      THEN
        OPEN okc_clev_pk_csr(p_catv_rec.CLE_ID);
        ...
        END IF;
      END IF;
      IF (p_catv_rec.CAT_ID IS NOT NULL)
      THEN
        OPEN okc_catv_pk_csr(p_catv_rec.CAT_ID);
        ...
        END IF;
      END IF;
      IF ((p_catv_rec.SAV_SAE_ID IS NOT NULL) AND
          (p_catv_rec.SAV_SAV_RELEASE IS NOT NULL))
      THEN
        OPEN okc_savv_pk_csr(p_catv_rec.SAV_SAE_ID,
                             p_catv_rec.SAV_SAV_RELEASE);
        FETCH okc_savv_pk_csr INTO l_okc_savv_pk;
        l_row_notfound := okc_savv_pk_csr%NOTFOUND;
        ...
        END IF;
      END IF;
      IF (p_catv_rec.SBT_CODE IS NOT NULL)
      THEN
        OPEN fnd_common_lookup_pk_csr(p_catv_rec.SBT_CODE);
  ...
  BEGIN
    l_return_status := validate_foreign_keys (p_catv_rec);
    RETURN (l_return_status);
  END Validate_Record;
---------------End of the commented code-----------*/
/*+++++++++++++Start of hand code +++++++++++++++++*/
...
--          l_okc_k_articles_tl_rec.text,
  ...
  PROCEDURE update_row(
    ...
    BEGIN
      ...
      END IF;
      IF (x_cat_rec.security_group_id = OKC_API.G_MISS_NUM)
      THEN
        x_cat_rec.security_group_id := l_cat_rec.security_group_id;
      END IF;
      RETURN(l_return_status);
...
--      END IF;
      ...
      END IF;
      IF (x_okc_k_articles_tl_rec.security_group_id = OKC_API.G_MISS_NUM)
      THEN
        x_okc_k_articles_tl_rec.security_group_id := l_okc_k_articles_tl_rec.security_group_id;
      END IF;
...
END; -- Package Body OKL_VP_CAT_PVT
