--------------------------------------------------------
--  DDL for Package Body BEN_BENCLSUM_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BENCLSUM_XMLP_PKG" AS
/* $Header: BENCLSUMB.pls 120.1 2007/12/10 08:26:36 vjaganat noship $ */
...
function CF_STANDARD_HEADERFormula return Number is
  ...
  l_all := substrb(fnd_message.get,1,80);
   begin
    ...
    begin
                select meaning into CP_CLOSE_ACTION_ITEMS_FLAG
    from hr_lookups a,
          ben_benefit_actions b
    where a.lookup_type = 'YES_NO'
    and   a.lookup_code = b.close_uneai_flag
    and   b.request_id = P_CONCURRENT_REQUEST_ID;
    ...
    begin
                select b.uneai_effective_date into CP_ACTION_ITEM_EFFECTIVE_DATE
    from  ben_benefit_actions b
    where b.request_id = P_CONCURRENT_REQUEST_ID;

    exception when no_data_found then
    null;
  end;
    begin
                select b.lf_evt_ocrd_dt into CP_ASSIGNED_LIFE_EVENT_DATE
    from  ben_benefit_actions b
    where b.request_id = P_CONCURRENT_REQUEST_ID;
...
function CF_SUMMARY_EVENTFormula return Number is
  ...
  begin
    ...
    (p_concurrent_request_id             => P_CONCURRENT_REQUEST_ID
    ,p_cd_1  => 'CLSNNOACTN',   p_val_1  => CP_CLSNNOACTN
    ,p_cd_2  => 'CLSNODEF',     p_val_2  => CP_CLSNODEF
    ,p_cd_3  => 'CLSNDEFNOCHG', p_val_3  => CP_CLSNDEFNOCHG
    ,p_cd_4  => 'CLSNDEFWCHG',  p_val_4  => CP_CLSNDEFWCHG
    ,p_cd_5  => 'CLSDEFNOCHG',  p_val_5  => CP_CLSDEFNOCHG
    ,p_cd_6  => 'CLSDEFWCHG',   p_val_6  => CP_CLSDEFWCHG
    ,p_cd_7  => 'XXXXXXXXXX',   p_val_7  => l_val
    ,p_cd_8  => 'XXXXXXXXXX',   p_val_8  => l_val
    ,p_cd_9  => 'XXXxxxxxxx',   p_val_9  => l_val
    ,p_cd_10 => 'XXxxxxxxxX',   p_val_10 => l_val
    );
  CP_CLSNNOACTN := nvl(CP_CLSNNOACTN,0);
  CP_CLSNODEF  := nvl(CP_CLSNODEF,0);
  CP_CLSNDEFNOCHG  := NVL(CP_CLSNDEFNOCHG,0);
  CP_CLSNDEFWCHG   := NVL(CP_CLSNDEFWCHG,0);
  CP_CLSDEFNOCHG   := NVL(CP_CLSDEFNOCHG,0);
  CP_CLSDEFWCHG    := NVL(CP_CLSDEFWCHG,0);
  CP_TOTCLSDEF     := CP_CLSDEFNOCHG + CP_CLSDEFWCHG;
  CP_TOTCLSNDEF    := CP_CLSNDEFNOCHG + CP_CLSNDEFWCHG;
  CP_TOT           := CP_CLSNNOACTN + CP_CLSNODEF + CP_TOTCLSDEF + CP_TOTCLSNDEF;
    return 1;
  end;
...
--Functions to refer Oracle report placeholders--
 ...
 Function CP_MODE_p return varchar2 is
	Begin
	 return CP_MODE;
	 END;
 Function CP_CLSNDEFNOCHG_p return number is
	Begin
	 return CP_CLSNDEFNOCHG;
	 END;
 Function CP_CLSNDEFWCHG_p return number is
	Begin
	 return CP_CLSNDEFWCHG;
	 END;
 ...
 Function CP_PERSONS_UNPROCESSED_p return number is
	Begin
	 ...
	 END;
 Function CP_CLSDEFNOCHG_p return number is
	Begin
	 return CP_CLSDEFNOCHG;
	 END;
 Function CP_CLSNNOACTN_p return number is
	Begin
	 return CP_CLSNNOACTN;
	 END;
 Function CP_CLSDEFWCHG_p return number is
	Begin
	 return CP_CLSDEFWCHG;
	 END;
 Function CP_CLSNODEF_p return number is
	Begin
	 return CP_CLSNODEF;
	 END;
 Function CP_TOTCLSDEF_p return number is
	Begin
	 return CP_TOTCLSDEF;
	 END;
 Function CP_TOTCLSNDEF_p return number is
	Begin
	 return CP_TOTCLSNDEF;
	 END;
 ...
 Function CP_LOCATION_p return varchar2 is
	Begin
	 return CP_LOCATION;
	 END;
 Function CP_ASSIGNED_LIFE_EVENT_DATE_p return varchar2 is
	Begin
	 return CP_ASSIGNED_LIFE_EVENT_DATE;
	 END;
 Function CP_ACTION_ITEM_EFFECTIVE_DATE1 return date is
	Begin
	 return CP_ACTION_ITEM_EFFECTIVE_DATE;
	 END;
 Function CP_AUDIT_LOG_FLAG_p return varchar2 is
	Begin
	 return CP_AUDIT_LOG_FLAG;
	 END;
 Function CP_CLOSE_ACTION_ITEMS_FLAG_p return varchar2 is
	Begin
	 return CP_CLOSE_ACTION_ITEMS_FLAG;
	 END;
 Function CP_CLOSE_MODE_p return varchar2 is
	Begin
	 return CP_CLOSE_MODE;
	 END;
END BEN_BENCLSUM_XMLP_PKG ;
