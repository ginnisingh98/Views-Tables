--------------------------------------------------------
--  DDL for Package Body FA_FASCOSTD_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FA_FASCOSTD_XMLP_PKG" AS
/* $Header: FASCOSTDB.pls 120.0.12010000.1 2008/07/28 13:16:34 appldev ship $ */
...
IF (P_REPORT_TYPE = 'CIP COST') THEN
      RP_Report_Name := ':CIP Detail Report:';
ELSE  RP_Report_Name := ':Cost Detail Report:';
END IF;
...
function ctr_out_of_balanceformula(CTR_BEGIN in number, CTR_ADD in number, CTR_REVAL in number, CTR_RECLASS in number, CTR_RETIRE in number,
CTR_ADJUST in number, CTR_TRANS in number, CTR_CAPITAL in number, CTR_END in number) return varchar2 is
begin
...
MOCK_TOTAL := NVL(CTR_BEGIN,0) + NVL(CTR_ADD,0) + NVL(CTR_REVAL,0)
	+ NVL(CTR_RECLASS,0) - NVL(CTR_RETIRE,0) + NVL(CTR_ADJUST,0)
	+ NVL(CTR_TRANS,0) - NVL(CTR_CAPITAL,0);
...
function acct_out_of_balanceformula(ACCT_BEGIN in number, ACCT_ADD in number, ACCT_REVAL in number, ACCT_RECLASS in number, ACCT_RETIRE in number,
ACCT_ADJUST in number, ACCT_TRANS in number, ACCT_CAPITAL in number, ACCT_END in number) return varchar2 is
begin
...
function bal_out_of_balanceformula(BAL_BEGIN in number, BAL_ADD in number, BAL_REVAL in number, BAL_RECLASS in number, BAL_RETIRE in number,
BAL_ADJUST in number, BAL_TRANS in number, BAL_CAPITAL in number, BAL_END in number) return varchar2 is
begin
...
function rp_out_of_balanceformula(RP_BEGIN in number, RP_ADD in number, RP_REVAL in number, RP_RECLASS in number, RP_RETIRE in number,
RP_ADJUST in number, RP_TRANS in number, RP_CAPITAL in number, RP_END in number) return varchar2 is
begin
...
--Functions to refer Oracle report placeholders--
 ...
 Function RP_CTR_LPROMPT_p return varchar2 is
	Begin
	 ...
	 END;
 Function P_PERIOD_FROM_p return varchar2 is
	Begin
	 return P_PERIOD_FROM;
	 END;
 Function P_PERIOD_TO_p return varchar2 is
	Begin
	 return P_PERIOD_TO;
	 END;
