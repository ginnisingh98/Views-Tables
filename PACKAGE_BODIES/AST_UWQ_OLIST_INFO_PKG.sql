--------------------------------------------------------
--  DDL for Package Body AST_UWQ_OLIST_INFO_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AST_UWQ_OLIST_INFO_PKG" AS
/* $Header: astuoinb.pls 120.3 2006/01/12 01:57:22 rkumares ship $ */

--Purpose:  This package will be used for displaying information within the work panel for the opportunity node
...
BEGIN
    ...
    if  p_action_key in ('astuoinb_cust_notes', 'astuoinb_prof_cust_notes') then  -- begin of main "if"

	     l_object_id := l_party_id;
	     l_object_code := 'PARTY%';

    elsif p_action_key in ('astuoinb_cont_notes', 'astuoinb_prof_cont_notes') then  -- begin of main "if"

	     l_object_id := l_contact_party_id;
	     l_object_code := 'PARTY%';

	elsif p_action_key in ('astuoinb_opp_notes', 'astuoinb_prof_opp_notes') then  -- begin of main "if"

	   l_object_id := l_lead_id;
	   l_object_code := 'OPPORTUNITY';

    end if;

	if p_action_key in ('astuoinb_cust_notes','astuoinb_cont_notes', 'astuoinb_opp_notes') then

	    l_cur_ind := 'DESC'  ;
	elsif p_action_key in ('astuoinb_prof_cust_notes','astuoinb_prof_cont_notes', 'astuoinb_prof_opp_notes') then
	...
	-----Notes in descending order ( latest to the oldest )
     if l_object_id is not null and l_cur_ind = 'DESC' then
		...
		--Code added for BugFix#4451689 --Start
			...
			--modified with c2_rec.entered_date instead of c2_rec.creation_date_date for bug#4177915
