--------------------------------------------------------
--  DDL for Package Body IGI_RPI_UPDATE_LINES_D_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_RPI_UPDATE_LINES_D_PKG" as
--- $Header: igiruldb.pls 120.4.12000000.1 2007/08/31 05:53:52 mbremkum ship $
  ...
  BEGIN
    ...
    if (
           ...
           AND ((Recinfo.standing_charge_id       =  X_standing_charge_id)
                OR ((Recinfo.standing_charge_id is null)
                     AND (X_standing_charge_id is null)))
           AND ((Recinfo.line_item_id             =  X_line_item_id)
                OR ((Recinfo.line_item_id is null)
                     AND (X_line_item_id is null)))
