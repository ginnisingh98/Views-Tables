--------------------------------------------------------
--  DDL for Package Body PQH_CEI_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_CEI_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_cei_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PQH_CORPS_EXTRA_INFO_PK') Then
...
Function api_updating
  ...
  Cursor C_Sel1 is
    ...
    where       corps_extra_info_id = p_corps_extra_info_id;
  --
  l_fct_ret     boolean;
  --
Begin
  --
  If (p_corps_extra_info_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_corps_extra_info_id
        = pqh_cei_shd.g_old_rec.corps_extra_info_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into pqh_cei_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> pqh_cei_shd.g_old_rec.object_version_number) Then
