--------------------------------------------------------
--  DDL for Package Body PAY_CA_DATABASE_ITEMS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CA_DATABASE_ITEMS_PKG" as
/* $Header: pycadbip.pkb 120.0 2005/05/29 03:28:26 appldev noship $ */
--
...
-- Call main package with legislation code defaulted to 'CA'
create_db_item(          p_name,
                         p_description,
                         p_data_type,
                         p_null_allowed,
                         p_definition_text,
                         p_user_entity_name,
                         p_user_entity_description,
                         p_route_name,
                         p_param_value1,
                         p_param_value2,
                         p_route_description,
...
--
  ...
  hr_utility.trace ('Legislation_code: ' || p_legislation_code);
...
--
  hr_utility.trace(' selected route..');
...
--
    ...
    hr_utility.trace('after insert of route');
...
--
      ...
      hr_utility.trace('after insert of context');
...
--
       ...
       hr_utility.trace('after insert of param');
...
--
    ...
    hr_utility.trace('selected entity id'||to_char(l_user_entity_id));
...
--                p_creator_type =>       'B',
   ...
   hr_utility.trace('insereted user entity'||p_user_entity_name);
...
EXCEPTION
    ...
    hr_utility.trace('user entity is not found');
    when others then
    hr_utility.trace('error in others of select of entity');
    raise;
END ;
-- Add any route parameter values
--
    IF p_param_value1 is not null THEN
--
...
--
    END IF;
--
    IF p_param_value2 is not null THEN
--
      INSERT into ff_route_parameter_values
        (route_parameter_id,
         user_entity_id,
...
--
  --END;
--
hr_utility.trace('Alls well am going to create dbi');
