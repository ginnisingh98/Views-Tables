--------------------------------------------------------
--  DDL for Package Body GMD_HOLD_REASONS_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMD_HOLD_REASONS_PVT" as
/* $Header: GMDVHRPB.pls 115.1 2002/12/03 17:05:51 cnagarba noship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from GMD_HOLD_REASONS_B
    where QCHOLD_RES_CODE = X_QCHOLD_RES_CODE
    ;
begin
  ...
  and not exists
    (select NULL
    from GMD_HOLD_REASONS_TL T
    where T.QCHOLD_RES_CODE = X_QCHOLD_RES_CODE
...
procedure LOCK_ROW (
  ...
  cursor c is select
      ...
      ATTRIBUTE30,
      ATTRIBUTE_CATEGORY
    from GMD_HOLD_REASONS_B
    where QCHOLD_RES_CODE = X_QCHOLD_RES_CODE
    for update of QCHOLD_RES_CODE nowait;
  recinfo c%rowtype;

  cursor c1 is select
      QCHOLD_RES_DESC,
      decode(LANGUAGE, userenv('LANG'), 'Y', 'N') BASELANG
    from GMD_HOLD_REASONS_TL
    where QCHOLD_RES_CODE = X_QCHOLD_RES_CODE
...
begin
  ...
  for tlinfo in c1 loop
    if (tlinfo.BASELANG = 'Y') then
      if (    (tlinfo.QCHOLD_RES_DESC = X_QCHOLD_RES_DESC)
      ) then
        null;
      else
        fnd_message.set_name('FND', 'FORM_RECORD_CHANGED');
...
begin
  ...
  update GMD_HOLD_REASONS_TL set
    QCHOLD_RES_DESC = X_QCHOLD_RES_DESC,
    LAST_UPDATE_DATE = X_LAST_UPDATE_DATE,
    LAST_UPDATED_BY = X_LAST_UPDATED_BY,
    LAST_UPDATE_LOGIN = X_LAST_UPDATE_LOGIN,
...
begin
  delete from GMD_HOLD_REASONS_TL
  where QCHOLD_RES_CODE = X_QCHOLD_RES_CODE;
  ...
  delete from GMD_HOLD_REASONS_B
  where QCHOLD_RES_CODE = X_QCHOLD_RES_CODE;
...
is
begin
  delete from GMD_HOLD_REASONS_TL T
  where not exists
    (select NULL
    from GMD_HOLD_REASONS_B B
    where B.QCHOLD_RES_CODE = T.QCHOLD_RES_CODE
    );

  update GMD_HOLD_REASONS_TL T set (
      QCHOLD_RES_DESC
    ) = (select
      B.QCHOLD_RES_DESC
    from GMD_HOLD_REASONS_TL B
    where B.QCHOLD_RES_CODE = T.QCHOLD_RES_CODE
  ...
  where (
      ...
      SUBT.QCHOLD_RES_CODE,
      SUBT.LANGUAGE
    from GMD_HOLD_REASONS_TL SUBB, GMD_HOLD_REASONS_TL SUBT
    where SUBB.QCHOLD_RES_CODE = SUBT.QCHOLD_RES_CODE
    and SUBB.LANGUAGE = SUBT.SOURCE_LANG
    and (SUBB.QCHOLD_RES_DESC <> SUBT.QCHOLD_RES_DESC
  ));
  ...
  insert into GMD_HOLD_REASONS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GMD_HOLD_REASONS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from GMD_HOLD_REASONS_TL T
    where T.QCHOLD_RES_CODE = B.QCHOLD_RES_CODE
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;

end GMD_HOLD_REASONS_PVT;
