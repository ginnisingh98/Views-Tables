--------------------------------------------------------
--  DDL for Package Body OTA_ENROLL_REVIEW_SS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_ENROLL_REVIEW_SS" AS
 /* $Header: otrevwrs.pkb 120.2 2008/01/08 14:04:35 aabalakr noship $*/

   g_package      varchar2(30)   := 'OTA_ENROLL_REVIEW_SS';
  ...
  Begin
    -- Setting sshr510 attribute.
    --
    WF_ENGINE.setitemattrtext(p_item_type,
                              p_item_key,
                              'HR_REVIEW_TEMPLATE_RN_ATTR',
                              'OTA_TP_REVIEW_RN_NTF');
    --
  ...
  End;
    ...
    l_approval_mode :=wf_engine.getItemAttrText(itemtype => p_item_type
			                                 ,itemkey  => p_item_key
			                                 ,aname    => 'HR_RUNTIME_APPROVAL_REQ_FLAG');



   if l_approval_mode = 'YES_DYNAMIC' and  p_from='REVIEW' then
...
PROCEDURE process_api2
  ...
  l_person_details		OTA_trng_enroll_SS.csr_person_to_enroll_details%ROWTYPE;
  l_specialInstruction          ota_delegate_bookings.SPECIAL_BOOKING_INSTRUCTIONS%TYPE;
...
hr_utility.set_location('OTA_ENROLL_REVIEW_SS' ,100);
...
--                ,p_name                => 'P_PERSON_ID'));
        ...
        l_person_details := ota_trng_enroll_ss.Get_Person_To_Enroll_Details(p_person_id => l_delegate_id);
        ...
        l_restricted_assignment_id := ota_trng_enroll_ss.CHK_DELEGATE_OK_FOR_EVENT(p_delegate_id => l_delegate_id
...
--                            hr_transaction_web.get_transaction_id
        ...
        l_cancel_boolean := ota_trng_enroll_ss.Chk_Event_Cancelled_for_Person(p_event_id           => l_eventid
        ...
        IF (l_cancel_boolean) THEN
         -- Call Cancel procedure to cancel the Finance if person Re-enroll
          ota_trng_enroll_ss.cancel_finance(l_booking_id);
        END IF;


      l_booking_status_row := ota_trng_enroll_ss.Get_Booking_Status_for_web(
...
--              RAISE OTA_TRNG_ENROLL_SS.g_mesg_on_stack_exception ;
...
hr_utility.set_location('OTA_ENROLL_REVIEW_SS' ,200);
...
--            p_update_finance_line parameter changed from 'N' to 'Y'.
  */
     ...
     END IF;
        ...
        EXCEPTION
-- Bug#2227738 hdshah change the exception
--              WHEN OTA_TRNG_ENROLL_SS.g_mesg_on_stack_exception THEN
--                    NULL;
