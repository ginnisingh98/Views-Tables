--------------------------------------------------------
--  DDL for Package Body IGF_SL_LOR_LOC_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_SL_LOR_LOC_PKG" AS
/* $Header: IGFLI11B.pls 120.1 2006/04/19 08:23:35 bvisvana noship $ */

  l_rowid VARCHAR2(25);
  old_references igf_sl_lor_loc_all%ROWTYPE;
  new_references igf_sl_lor_loc_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.chg_batch_id                      := x_chg_batch_id;
    ...
    new_references.p_ssn_chg_date                    := x_p_ssn_chg_date;
    ...
    new_references.p_dob_chg_date                    := x_p_dob_chg_date;
    ...
    new_references.s_ssn_chg_date                    := x_s_ssn_chg_date;
    ...
    new_references.s_local_addr_chg_date             := x_s_local_addr_chg_date;
    new_references.s_email_addr                      := x_s_email_addr;
    new_references.s_date_of_birth                   := x_s_date_of_birth;
    new_references.s_dob_chg_date                    := x_s_dob_chg_date;
  ...
  */
  BEGIN
    ...
    IF (((old_references.origination_id = new_references.origination_id)) OR
      ...
      NULL;
    ELSIF NOT igf_sl_lor_pkg.get_pk_for_validation (
                new_references.origination_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SL_LORL_LOR_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_AW_AWD_LORLOC_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_AP_FABASE_LORLOC_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
    ...
    l_org_id                     igf_sl_lor_loc_all.org_id%TYPE DEFAULT igf_aw_gen.get_org_id;
  ...
  BEGIN
    ...
    INSERT INTO igf_sl_lor_loc_all(
      loan_id,
      origination_id,
      loan_number,
      loan_type,
      ...
      new_references.s_local_addr_chg_date,
      new_references.s_email_addr,
      new_references.s_date_of_birth,
      new_references.s_dob_chg_date,
  ...
  BEGIN
    ...
    UPDATE igf_sl_lor_loc_all
      SET
        ...
        chg_batch_id                      = new_references.chg_batch_id,
        ...
        p_ssn_chg_date                    = new_references.p_ssn_chg_date,
        ...
        p_dob_chg_date                    = new_references.p_dob_chg_date,
        ...
        s_ssn_chg_date                    = new_references.s_ssn_chg_date,
        ...
        s_local_addr_chg_date             = new_references.s_local_addr_chg_date,
        s_email_addr                      = new_references.s_email_addr,
        s_date_of_birth                   = new_references.s_date_of_birth,
        s_dob_chg_date                    = new_references.s_dob_chg_date,
        ...
        pnote_batch_id                    = x_pnote_batch_id,
        pnote_ack_date                    = x_pnote_ack_date,
        pnote_mpn_ind                     = x_pnote_mpn_ind,
  ...
  */
  BEGIN
    ...
    DELETE FROM igf_sl_lor_loc_all
    WHERE rowid = x_rowid;
...
AS
  ...
  ||  Purpose : To insert in the history table also whenever
  ||  Known limitations, enhancements or remarks :
  ||  Change History :
  ||  Who             When            What
  ||  (reverse chronological order - newest change first)
  ||  bvisvana        28-Feb-2006     Build FA161. TBH Impact change.
  */
l_rowid   ROWID;
...
IF p_action = 'INSERT' THEN
  l_source := 'SYSTEM';
ELSIF p_action='UPDATE' THEN
   IF NVL(new_references.loan_status,'*')     = 'S'
   OR NVL(new_references.loan_chg_status,'*') = 'S' THEN
      l_source := 'SYSTEM';
   ELSE
      l_source := 'RESPONSE';
   END IF;
...
igf_sl_lor_loc_history_pkg.insert_row(
   x_rowid                             =>      l_rowid,
   x_loan_id                           =>      new_references.loan_id,
   x_origination_id                    =>      new_references.origination_id,
   x_loan_number                       =>      new_references.loan_number,
   x_loan_type                         =>      new_references.loan_type,
   x_loan_amt_offered                  =>      new_references.loan_amt_offered,
   x_loan_amt_accepted                 =>      new_references.loan_amt_accepted,
   x_loan_per_begin_date               =>      new_references.loan_per_begin_date,
   x_loan_per_end_date                 =>      new_references.loan_per_end_date,
   x_acad_yr_begin_date                =>      new_references.acad_yr_begin_date,
   x_acad_yr_end_date                  =>      new_references.acad_yr_end_date,
   x_loan_status                       =>      new_references.loan_status,
   x_loan_status_date                  =>      new_references.loan_status_date,
   x_loan_chg_status                   =>      new_references.loan_chg_status,
   x_loan_chg_status_date              =>      new_references.loan_chg_status_date,
   x_req_serial_loan_code              =>      new_references.req_serial_loan_code,
   x_act_serial_loan_code              =>      new_references.act_serial_loan_code,
   x_active                            =>      new_references.active,
   x_active_date                       =>      new_references.active_date,
   x_sch_cert_date                     =>      new_references.sch_cert_date,
   x_orig_status_flag                  =>      new_references.orig_status_flag,
   x_orig_batch_id                     =>      new_references.orig_batch_id,
   x_orig_batch_date                   =>      new_references.orig_batch_date,
   x_orig_ack_date                     =>      new_references.orig_ack_date,
   x_credit_override                   =>      new_references.credit_override,
   x_credit_decision_date              =>      new_references.credit_decision_date,
   x_pnote_delivery_code               =>      new_references.pnote_delivery_code,
   x_pnote_status                      =>      new_references.pnote_status,
   x_pnote_status_date                 =>      new_references.pnote_status_date,
   x_pnote_id                          =>      new_references.pnote_id,
   x_pnote_print_ind                   =>      new_references.pnote_print_ind,
   x_pnote_accept_amt                  =>      new_references.pnote_accept_amt,
   x_pnote_accept_date                 =>      new_references.pnote_accept_date,
   x_p_signature_code                  =>      new_references.p_signature_code,
   x_p_signature_date                  =>      new_references.p_signature_date,
   x_s_signature_code                  =>      new_references.s_signature_code,
   x_unsub_elig_for_heal               =>      new_references.unsub_elig_for_heal,
   x_disclosure_print_ind              =>      new_references.disclosure_print_ind,
   x_orig_fee_perct                    =>      new_references.orig_fee_perct,
   x_borw_confirm_ind                  =>      new_references.borw_confirm_ind,
   x_borw_interest_ind                 =>      new_references.borw_interest_ind,
   x_unsub_elig_for_depnt              =>      new_references.unsub_elig_for_depnt,
   x_guarantee_amt                     =>      new_references.guarantee_amt,
   x_guarantee_date                    =>      new_references.guarantee_date,
   x_guarnt_adj_ind                    =>      new_references.guarnt_adj_ind,
   x_guarnt_amt_redn_code              =>      new_references.guarnt_amt_redn_code,
   x_guarnt_status_code                =>      new_references.guarnt_status_code,
   x_guarnt_status_date                =>      new_references.guarnt_status_date,
   x_lend_apprv_denied_code            =>      new_references.lend_apprv_denied_code,
   x_lend_apprv_denied_date            =>      new_references.lend_apprv_denied_date,
   x_lend_status_code                  =>      new_references.lend_status_code,
   x_lend_status_date                  =>      new_references.lend_status_date,
   x_grade_level_code                  =>      new_references.grade_level_code,
   x_enrollment_code                   =>      new_references.enrollment_code,
   x_anticip_compl_date                =>      new_references.anticip_compl_date,
   x_borw_lender_id                    =>      new_references.borw_lender_id,
   x_guarantor_id                      =>      new_references.guarantor_id,
   x_prc_type_code                     =>      new_references.prc_type_code,
   x_rec_type_ind                      =>      new_references.rec_type_ind,
   x_cl_loan_type                      =>      new_references.cl_loan_type,
   x_cl_seq_number                     =>      new_references.cl_seq_number,
   x_last_resort_lender                =>      new_references.last_resort_lender,
   x_lender_id                         =>      new_references.lender_id,
   x_lend_non_ed_brc_id                =>      new_references.lend_non_ed_brc_id,
   x_recipient_id                      =>      new_references.recipient_id,
   x_recipient_type                    =>      new_references.recipient_type,
   x_recip_non_ed_brc_id               =>      new_references.recip_non_ed_brc_id,
   x_cl_rec_status                     =>      new_references.cl_rec_status,
   x_cl_rec_status_last_update         =>      new_references.cl_rec_status_last_update,
   x_alt_prog_type_code                =>      new_references.alt_prog_type_code,
   x_alt_appl_ver_code                 =>      new_references.alt_appl_ver_code,
   x_borw_outstd_loan_code             =>      new_references.borw_outstd_loan_code,
   x_mpn_confirm_code                  =>      new_references.mpn_confirm_code,
   x_resp_to_orig_code                 =>      new_references.resp_to_orig_code,
   x_appl_loan_phase_code              =>      new_references.appl_loan_phase_code,
   x_appl_loan_phase_code_chg          =>      new_references.appl_loan_phase_code_chg,
   x_tot_outstd_stafford               =>      new_references.tot_outstd_stafford,
   x_tot_outstd_plus                   =>      new_references.tot_outstd_plus,
   x_alt_borw_tot_debt                 =>      new_references.alt_borw_tot_debt,
   x_act_interest_rate                 =>      new_references.act_interest_rate,
   x_service_type_code                 =>      new_references.service_type_code,
   x_rev_notice_of_guarnt              =>      new_references.rev_notice_of_guarnt,
   x_sch_refund_amt                    =>      new_references.sch_refund_amt,
   x_sch_refund_date                   =>      new_references.sch_refund_date,
   x_uniq_layout_vend_code             =>      new_references.uniq_layout_vend_code,
   x_uniq_layout_ident_code            =>      new_references.uniq_layout_ident_code,
   x_p_person_id                       =>      new_references.p_person_id,
   x_p_ssn                             =>      new_references.p_ssn,
   x_p_last_name                       =>      new_references.p_last_name,
   x_p_first_name                      =>      new_references.p_first_name,
   x_p_middle_name                     =>      new_references.p_middle_name,
   x_p_permt_addr1                     =>      new_references.p_permt_addr1,
   x_p_permt_addr2                     =>      new_references.p_permt_addr2,
   x_p_permt_city                      =>      new_references.p_permt_city,
   x_p_permt_state                     =>      new_references.p_permt_state,
   x_p_permt_zip                       =>      new_references.p_permt_zip,
   x_p_permt_addr_chg_date             =>      new_references.p_permt_addr_chg_date,
   x_p_permt_phone                     =>      new_references.p_permt_phone,
   x_p_email_addr                      =>      new_references.p_email_addr,
   x_p_date_of_birth                   =>      new_references.p_date_of_birth,
   x_p_license_num                     =>      new_references.p_license_num,
   x_p_license_state                   =>      new_references.p_license_state,
   x_p_citizenship_status              =>      new_references.p_citizenship_status,
   x_p_alien_reg_num                   =>      new_references.p_alien_reg_num,
   x_p_default_status                  =>      new_references.p_default_status,
   x_p_foreign_postal_code             =>      new_references.p_foreign_postal_code,
   x_p_state_of_legal_res              =>      new_references.p_state_of_legal_res,
   x_p_legal_res_date                  =>      new_references.p_legal_res_date,
   x_s_ssn                             =>      new_references.s_ssn,
   x_s_last_name                       =>      new_references.s_last_name,
   x_s_first_name                      =>      new_references.s_first_name,
   x_s_middle_name                     =>      new_references.s_middle_name,
   x_s_permt_addr1                     =>      new_references.s_permt_addr1,
   x_s_permt_addr2                     =>      new_references.s_permt_addr2,
   x_s_permt_city                      =>      new_references.s_permt_city,
   x_s_permt_state                     =>      new_references.s_permt_state,
   x_s_permt_zip                       =>      new_references.s_permt_zip,
   x_s_permt_addr_chg_date             =>      new_references.s_permt_addr_chg_date,
   x_s_permt_phone                     =>      new_references.s_permt_phone,
   x_s_local_addr1                     =>      new_references.s_local_addr1,
   x_s_local_addr2                     =>      new_references.s_local_addr2,
   x_s_local_city                      =>      new_references.s_local_city,
   x_s_local_state                     =>      new_references.s_local_state,
   x_s_local_zip                       =>      new_references.s_local_zip,
   x_s_email_addr                      =>      new_references.s_email_addr,
   x_s_date_of_birth                   =>      new_references.s_date_of_birth,
   x_s_license_num                     =>      new_references.s_license_num,
   x_s_license_state                   =>      new_references.s_license_state,
   x_s_depncy_status                   =>      new_references.s_depncy_status,
   x_s_default_status                  =>      new_references.s_default_status,
   x_s_citizenship_status              =>      new_references.s_citizenship_status,
   x_s_alien_reg_num                   =>      new_references.s_alien_reg_num,
   x_s_foreign_postal_code             =>      new_references.s_foreign_postal_code,
   x_pnote_batch_id                    =>      new_references.pnote_batch_id,
   x_pnote_ack_date                    =>      new_references.pnote_ack_date,
   x_pnote_mpn_ind                     =>      new_references.pnote_mpn_ind,
   x_award_id                          =>      new_references.award_id,
   x_base_id                           =>      new_references.base_id,
   x_loan_key_num                      =>      new_references.loan_key_num,
   x_fin_award_year                    =>      new_references.fin_award_year,
   x_cps_trans_num                     =>      new_references.cps_trans_num,
   x_pymt_servicer_amt                 =>      new_references.pymt_servicer_amt,
   x_pymt_servicer_date                =>      new_references.pymt_servicer_date,
   x_book_loan_amt                     =>      new_references.book_loan_amt,
   x_book_loan_amt_date                =>      new_references.book_loan_amt_date,
   x_s_chg_ssn                         =>      new_references.s_chg_ssn,
   x_s_chg_last_name                   =>      new_references.s_chg_last_name,
   x_b_chg_ssn                         =>      new_references.b_chg_ssn,
   x_b_chg_last_name                   =>      new_references.b_chg_last_name,
   x_note_message                      =>      new_references.note_message,
   x_full_resp_code                    =>      new_references.full_resp_code,
   x_s_permt_county                    =>      new_references.s_permt_county,
   x_b_permt_county                    =>      new_references.b_permt_county,
   x_s_permt_country                   =>      new_references.s_permt_country,
   x_b_permt_country                   =>      new_references.b_permt_country,
   x_crdt_decision_status              =>      new_references.crdt_decision_status,
   x_b_chg_birth_date                  =>      new_references.b_chg_birth_date,
   x_s_chg_birth_date                  =>      new_references.s_chg_birth_date,
   x_external_loan_id_txt              =>      new_references.external_loan_id_txt        ,
   x_deferment_request_code            =>      new_references.deferment_request_code      ,
   x_eft_authorization_code            =>      new_references.eft_authorization_code      ,
   x_requested_loan_amt                =>      new_references.requested_loan_amt          ,
   x_actual_record_type_code           =>      new_references.actual_record_type_code     ,
   x_reinstatement_amt                 =>      new_references.reinstatement_amt           ,
   x_lender_use_txt                    =>      new_references.lender_use_txt              ,
   x_guarantor_use_txt                 =>      new_references.guarantor_use_txt           ,
   x_fls_approved_amt                  =>      new_references.fls_approved_amt            ,
   x_flu_approved_amt                  =>      new_references.flu_approved_amt            ,
   x_flp_approved_amt                  =>      new_references.flp_approved_amt            ,
   x_alt_approved_amt                  =>      new_references.alt_approved_amt            ,
   x_loan_app_form_code                =>      new_references.loan_app_form_code          ,
   x_alt_borrower_ind_flag             =>      new_references.alt_borrower_ind_flag       ,
   x_school_id_txt                     =>      new_references.school_id_txt               ,
   x_cost_of_attendance_amt            =>      new_references.cost_of_attendance_amt      ,
   x_EXPECT_FAMILY_CONTRIBUTE_AMT      =>      new_references.expect_family_contribute_amt,
   x_established_fin_aid_amount        =>      new_references.established_fin_aid_amount  ,
   x_BOROWER_ELECTRONIC_SIGN_FLAG      =>      new_references.borower_electronic_sign_flag,
   x_student_electronic_sign_flag      =>      new_references.student_electronic_sign_flag,
   x_BOROWER_CREDIT_AUTHORIZ_FLAG      =>      new_references.borower_credit_authoriz_flag,
   x_mpn_type_flag                     =>      new_references.mpn_type_flag,
   x_school_use_txt                    =>      new_references.school_use_txt,
   x_document_id_txt                   =>      new_references.document_id_txt,
   x_atd_entity_id_txt                 =>      new_references.atd_entity_id_txt,
   x_rep_entity_id_txt                 =>      new_references.rep_entity_id_txt,
   x_source_entity_id_txt              =>      new_references.source_entity_id_txt,
   x_interest_rebate_percent_num       =>      new_references.interest_rebate_percent_num,
   x_esign_src_typ_cd                  =>      new_references.esign_src_typ_cd,
   x_loansh_id                         =>      l_seq,
