--------------------------------------------------------
--  DDL for Package Body IGS_PS_UOFR_WLST_PRF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UOFR_WLST_PRF_PKG" AS
/* $Header: IGSPI97B.pls 120.1 2005/08/17 05:49:57 appldev ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_ps_uofr_wlst_prf%RowType;
  new_references igs_ps_uofr_wlst_prf%RowType;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.unit_ofr_waitlist_pref_id := x_unit_ofr_wl_pref_id;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (((old_references.unit_ofr_waitlist_priority_id = new_references.unit_ofr_waitlist_priority_id)) OR
        ((new_references.unit_ofr_waitlist_priority_id IS NULL))) THEN
      NULL;
    ELSIF NOT Igs_Ps_Uofr_Wlst_Pri_Pkg.Get_PK_For_Validation (
        		new_references.unit_ofr_waitlist_priority_id
        )  THEN
	 Fnd_Message.Set_Name ('FND','FORM_RECORD_DELETED');
      IGS_GE_MSG_STACK.ADD;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_uofr_wlst_prf
      WHERE    unit_ofr_waitlist_pref_id = x_unit_ofr_wl_pref_id
      FOR UPDATE NOWAIT;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      ...
      AND      unit_ofr_waitlist_priority_id = x_unit_ofr_wl_priority_id 	and      ((l_rowid is null) or (rowid <> l_rowid))
  ...
  Change History
  Who             When            What
  shtatiko        18-AUG-2003     Enh# 2520994, Changed message name to
                                  IGS_PS_USEC_EXISTS
  (reverse chronological order - newest change first)
  ***************************************************************/
  ...
  END Get_FK_Igs_Ps_Uofr_Wlst_Pri;
    ...
    IF (p_action = 'INSERT') THEN
      ...
      Null;
	     IF Get_Pk_For_Validation(
    		new_references.unit_ofr_waitlist_pref_id)  THEN
	       Fnd_Message.Set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
      IGS_GE_MSG_STACK.ADD;
	       App_Exception.Raise_Exception;
	     END IF;
...
l_rowid:=NULL;
  ...
  ***************************************************************/
    ...
    cursor C is select ROWID from IGS_PS_UOFR_WLST_PRF
             where                 UNIT_OFR_WAITLIST_PREF_ID= X_UNIT_OFR_WL_PREF_ID
;
 ...
 begin
   ...
   SELECT IGS_PS_UOFR_WLST_PRF_S.NEXTVAL
   INTO   X_UNIT_OFR_WL_PREF_ID
   FROM dual;
   Before_DML(
 		p_action=>'INSERT',
 		x_rowid=>X_ROWID,
 	       x_unit_ofr_wl_pref_id=>X_UNIT_OFR_WL_PREF_ID,
     ...
     insert into IGS_PS_UOFR_WLST_PRF (
		UNIT_OFR_WAITLIST_PREF_ID
		...
		,LAST_UPDATE_LOGIN
	        ...
	        NEW_REFERENCES.UNIT_OFR_WAITLIST_PREF_ID
	        ,NEW_REFERENCES.UNIT_OFR_WAITLIST_PRIORITY_ID
...
begin
  ...
  close c1;
if ( (  tlinfo.UNIT_OFR_WAITLIST_PRIORITY_ID = X_UNIT_OFR_WL_PRIORITY_ID)
 ...
 begin
   ...
   Before_DML(
 		...
 		x_rowid=>X_ROWID,
	       ...
	       x_last_updated_by=>X_LAST_UPDATED_BY,
	       x_last_update_login=>X_LAST_UPDATE_LOGIN);
   update IGS_PS_UOFR_WLST_PRF set
      UNIT_OFR_WAITLIST_PRIORITY_ID =  NEW_REFERENCES.UNIT_OFR_WAITLIST_PRIORITY_ID,
  ...
  ***************************************************************/
    ...
    cursor c1 is select ROWID from IGS_PS_UOFR_WLST_PRF
             where     UNIT_OFR_WAITLIST_PREF_ID= X_UNIT_OFR_WL_PREF_ID
;
begin
	open c1;
...
x_rowid => X_ROWID
 ...
 delete from IGS_PS_UOFR_WLST_PRF
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
