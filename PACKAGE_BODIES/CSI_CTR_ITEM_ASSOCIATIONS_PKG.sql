--------------------------------------------------------
--  DDL for Package Body CSI_CTR_ITEM_ASSOCIATIONS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSI_CTR_ITEM_ASSOCIATIONS_PKG" as
/* $Header: csitciab.pls 120.3 2008/04/03 21:53:54 devijay ship $*/

G_PKG_NAME CONSTANT VARCHAR2(30)  := 'CSI_CTR_ITEM_ASSOCIATIONS_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'csitciab.pls';
...
PROCEDURE Insert_Row(
   ...
   CURSOR C1 IS
   SELECT CSI_CTR_ITEM_ASSOCIATIONS_S.nextval
   FROM   dual;
BEGIN
   IF (px_CTR_ASSOCIATION_ID IS NULL) OR (px_CTR_ASSOCIATION_ID = FND_API.G_MISS_NUM) then
      OPEN C1;
      FETCH C1 INTO px_CTR_ASSOCIATION_ID;
      CLOSE C1;
   END IF;

   INSERT INTO CSI_CTR_ITEM_ASSOCIATIONS(
 	 CTR_ASSOCIATION_ID
	 ,GROUP_ID
	 ,INVENTORY_ITEM_ID
   ...
   VALUES(
	...
	,decode(p_ASSOCIATED_TO_GROUP, FND_API.G_MISS_CHAR, NULL,p_ASSOCIATED_TO_GROUP)
...
PROCEDURE Update_Row(
 ...
 BEGIN
    UPDATE CSI_CTR_ITEM_ASSOCIATIONS
    SET    GROUP_ID = decode(p_GROUP_ID, NULL, GROUP_ID, FND_API.G_MISS_NUM, NULL, p_GROUP_ID)
	   ...
	   ,ASSOCIATED_TO_GROUP = decode(p_ASSOCIATED_TO_GROUP, NULL, ASSOCIATED_TO_GROUP, FND_API.G_MISS_CHAR, NULL,  p_ASSOCIATED_TO_GROUP)
	   ,MAINT_ORGANIZATION_ID = decode(p_MAINT_ORGANIZATION_ID, NULL,MAINT_ORGANIZATION_ID, FND_API.G_MISS_NUM, NULL,  p_MAINT_ORGANIZATION_ID)
...
PROCEDURE Delete_Row(p_CTR_ASSOCIATION_ID  NUMBER)  IS
BEGIN
   DELETE FROM CSI_CTR_ITEM_ASSOCIATIONS
   WHERE  CTR_ASSOCIATION_ID = p_CTR_ASSOCIATION_ID;
   IF (SQL%NOTFOUND) then
       RAISE NO_DATA_FOUND;
   END IF;
...
PROCEDURE Lock_Row(
   ...
   WHERE  CTR_ASSOCIATION_ID = p_CTR_ASSOCIATION_ID
   FOR UPDATE of CTR_ASSOCIATION_ID NOWAIT;
   Recinfo C1%ROWTYPE;
BEGIN
   OPEN C1;
   ...
   IF (
           (      Recinfo.CTR_ASSOCIATION_ID  = p_CTR_ASSOCIATION_ID )
      ...
      AND (    ( Recinfo.COUNTER_ID = p_COUNTER_ID)
       ...
       AND (    ( Recinfo.USAGE_RATE = p_USAGE_RATE)
            OR (    ( Recinfo.USAGE_RATE IS NULL )
                AND (  p_USAGE_RATE IS NULL )))
       -- AND (    ( Recinfo.ASSOCIATION_TYPE = p_ASSOCIATION_TYPE)
       --     OR (    ( Recinfo.ASSOCIATION_TYPE IS NULL )
       --         AND (  p_ASSOCIATION_TYPE IS NULL )))
       AND (    ( Recinfo.USE_PAST_READING = p_USE_PAST_READING)
            OR (    ( Recinfo.USE_PAST_READING IS NULL )
                AND (  p_USE_PAST_READING IS NULL )))
       AND (    ( Recinfo.ASSOCIATED_TO_GROUP = p_ASSOCIATED_TO_GROUP)
            OR (    ( Recinfo.ASSOCIATED_TO_GROUP IS NULL )
                AND (  p_ASSOCIATED_TO_GROUP IS NULL )))
