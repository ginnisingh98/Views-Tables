--------------------------------------------------------
--  DDL for Package Body FA_FASRSVES_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FA_FASRSVES_XMLP_PKG" AS
/* $Header: FASRSVESB.pls 120.0.12010000.1 2008/07/28 13:17:32 appldev ship $ */
...
ELSE IF (P_REPORT_TYPE = 'RESERVE') THEN
	RP_Report_Name := ':Reserve Summary Report:';
ELSE RP_Report_Name := 'REPORT ERROR';
END IF; END IF;
...
THEN
  fnd_client_info.set_currency_context(p_ca_set_of_books_id_1);
END IF;
...
function out_of_balanceformula(BEGIN_P in number, ADDITION in number, DEPRECIATION in number, RECLASS in number, RETIREMENT in number, ADJUST in number, TRANSFER in number, END_P in number) return varchar2 is
begin
...
MOCK_TOTAL := NVL(BEGIN_P,0) + NVL(ADDITION,0) + NVL(DEPRECIATION,0)
...
begin
p_ca_set_of_books_id_1:=nvl(p_ca_set_of_books_id,-1);
IF p_ca_set_of_books_id_1 <> -1999
THEN
...
function G_SETUPGroupFilter return boolean is
begin
...
--Functions to refer Oracle report placeholders--
 ...
 Function RP_CTR_APROMPT_p return varchar2 is
	Begin
	 ...
	 END;
	 --modified
 Function PERIOD_FROM_p(p_period1 varchar2) return varchar2 is
	Begin
	 period_from := p_period1;
	 return PERIOD_FROM;
	 END;
 Function PERIOD_TO_p(p_period2 varchar2) return varchar2 is
	Begin
	period_to := p_period2;
	 return PERIOD_TO;
	 END;
