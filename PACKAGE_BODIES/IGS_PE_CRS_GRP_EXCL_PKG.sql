--------------------------------------------------------
--  DDL for Package Body IGS_PE_CRS_GRP_EXCL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PE_CRS_GRP_EXCL_PKG" AS
 /* $Header: IGSNI14B.pls 115.5 2003/02/20 10:29:33 shtatiko ship $ */



  l_rowid VARCHAR2(25);
  old_references IGS_PE_CRS_GRP_EXCL%RowType;
  new_references IGS_PE_CRS_GRP_EXCL%RowType;
  ...
  BEGIN
    ...
    new_references.pcge_start_dt := x_pcge_start_dt;
...
v_message_name  varchar2(30);
  BEGIN
	...
	IF (new_references.pcge_start_dt IS NOT NULL) THEN
		IF p_inserting OR (p_updating AND
		(NVL(old_references.pcge_start_dt, IGS_GE_DATE.IGSDATE('1900/01/01'))
		<> new_references.pcge_start_dt))
		THEN
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_pcge_ar_iu
  -- AFTER INSERT OR DELETE OR UPDATE
  -- ON IGS_PE_CRS_GRP_EXCL
  -- FOR EACH ROW
  ...
  BEGIN
	-- Validate for open ended IGS_PE_PERSON crs grp exclusion records.
	IF new_references.expiry_dt IS NULL THEN
		 -- Save the rowid of the current row.
               	IF IGS_EN_VAL_PCGE.enrp_val_pcge_open (
		...
		v_rowid_saved := TRUE;
		-- Cannot call enrp_val_pcge_open because trigger will be mutating.
	END IF;
 ...
 END IF;
  ...
  FUNCTION Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      ...
      AND      pcge_start_dt = x_pcge_start_dt
      FOR UPDATE NOWAIT;
...
END Get_PK_For_Validation;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PCGE_PEE_FK');
       IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
  ...
  BEGIN
    ...
    new_references.course_group_cd,
    new_references.pcge_start_dt) THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PE_CRS_GRP_EXCL
      where PERSON_ID = X_PERSON_ID
      and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
      and PEN_START_DT = X_PEN_START_DT
      and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
  ...
  Before_DML(
   ...
   x_pcge_start_dt=>X_PCGE_START_DT,
  ...
  insert into IGS_PE_CRS_GRP_EXCL (
    PERSON_ID,
    ENCUMBRANCE_TYPE,
    PEN_START_DT,
    S_ENCMB_EFFECT_TYPE,
...
begin
  ...
  update IGS_PE_CRS_GRP_EXCL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PE_CRS_GRP_EXCL
     where PERSON_ID = X_PERSON_ID
     and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
     and PEN_START_DT = X_PEN_START_DT
     and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
 Before_DML(
  ...
  delete from IGS_PE_CRS_GRP_EXCL
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
