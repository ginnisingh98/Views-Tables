--------------------------------------------------------
--  DDL for Package Body HR_LEI_FLEX
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_LEI_FLEX" as
/* $Header: hrleifli.pkb 115.0 99/07/17 16:40:33 porting ship $ */
--
-- ----------------------------------------------------------------------------
-- |                     Private Global Definitions                           |
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '   hr_lei_flex.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |--------------------------< val_info_type_y >------------------------------|
-- ----------------------------------------------------------------------------
...
--
--  Post Success:
--    Processing of hr_lei_flex continues.
--
--  Post failure:
...
/*
  ...
  if <reference field value> is not null then
    ...
    --
*/
    if p_rec.lei_attribute1 is not null then
      raise l_error;
    elsif p_rec.lei_attribute2 is not null then
      raise l_error;
    elsif p_rec.lei_attribute3 is not null then
      raise l_error;
    elsif p_rec.lei_attribute4 is not null then
      raise l_error;
    elsif p_rec.lei_attribute5 is not null then
      raise l_error;
    elsif p_rec.lei_attribute6 is not null then
      raise l_error;
    elsif p_rec.lei_attribute7 is not null then
      raise l_error;
    elsif p_rec.lei_attribute8 is not null then
      raise l_error;
    elsif p_rec.lei_attribute9 is not null then
      raise l_error;
    elsif p_rec.lei_attribute10 is not null then
      raise l_error;
    elsif p_rec.lei_attribute11 is not null then
      raise l_error;
    elsif p_rec.lei_attribute12 is not null then
      raise l_error;
    elsif p_rec.lei_attribute13 is not null then
      raise l_error;
    elsif p_rec.lei_attribute14 is not null then
      raise l_error;
    elsif p_rec.lei_attribute15 is not null then
      raise l_error;
    elsif p_rec.lei_attribute16 is not null then
      raise l_error;
    elsif p_rec.lei_attribute17 is not null then
      raise l_error;
    elsif p_rec.lei_attribute18 is not null then
      raise l_error;
    elsif p_rec.lei_attribute19 is not null then
      raise l_error;
    elsif p_rec.lei_attribute20 is not null then
      raise l_error;
    end if;
  --
  /*
