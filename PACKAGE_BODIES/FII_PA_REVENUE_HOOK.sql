--------------------------------------------------------
--  DDL for Package Body FII_PA_REVENUE_HOOK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FII_PA_REVENUE_HOOK" as
/* $Header: FIIPA16B.pls 120.0 2002/08/24 05:00:33 appldev noship $ */
...
function pre_fact_coll return boolean is
  ...
  cursor task_c is
    ...
    where log.event_type   = 'DNRM:FII_PA_REVENUE_F'
...
begin
  ...
  update fii_pa_revenue_fstg  fstg
  set project_org_fk =
      (
  ...
  where collection_status = 'READY'
    ...
    and project_fk in
        (
          ...
          where event_type   = 'DNRM:FII_PA_REVENUE_F'
