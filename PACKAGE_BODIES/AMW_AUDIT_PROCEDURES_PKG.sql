--------------------------------------------------------
--  DDL for Package Body AMW_AUDIT_PROCEDURES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMW_AUDIT_PROCEDURES_PKG" as
/*$Header: amwprcdb.pls 120.0 2005/05/31 20:05:18 appldev noship $*/
procedure INSERT_ROW (
  X_ROWID in out nocopy VARCHAR2,
  X_AUDIT_PROCEDURE_REV_ID in NUMBER,
  ...
  X_AUDIT_PROCEDURE_REV_NUM in NUMBER,
  X_END_DATE in DATE,
  ...
  cursor C is select ROWID from AMW_AUDIT_PROCEDURES_B
    where AUDIT_PROCEDURE_REV_ID = X_AUDIT_PROCEDURE_REV_ID
    ;
begin
  insert into AMW_AUDIT_PROCEDURES_B (
    ...
    X_AUDIT_PROCEDURE_REV_ID,
    X_AUDIT_PROCEDURE_REV_NUM,
    X_END_DATE,
    X_APPROVAL_DATE,
  ...
  and not exists
    (select NULL
    from AMW_AUDIT_PROCEDURES_TL T
    where T.AUDIT_PROCEDURE_REV_ID = X_AUDIT_PROCEDURE_REV_ID
...
procedure LOCK_ROW (
  ...
  cursor c is select
    ...
    where AUDIT_PROCEDURE_REV_ID = X_AUDIT_PROCEDURE_REV_ID
    for update of AUDIT_PROCEDURE_REV_ID nowait;
  recinfo c%rowtype;
...
begin
  ...
  close c;
  if (    ((recinfo.PROJECT_ID = X_PROJECT_ID)
      ...
      AND (recinfo.AUDIT_PROCEDURE_ID = X_AUDIT_PROCEDURE_ID)
      AND (recinfo.AUDIT_PROCEDURE_REV_NUM = X_AUDIT_PROCEDURE_REV_NUM)
...
begin
  update AMW_AUDIT_PROCEDURES_B set
    ...
    AUDIT_PROCEDURE_REV_NUM = X_AUDIT_PROCEDURE_REV_NUM,
    END_DATE = X_END_DATE,
  ...
  update AMW_AUDIT_PROCEDURES_TL set
    NAME = X_NAME,
...
begin
  delete from AMW_AUDIT_PROCEDURES_TL
  where AUDIT_PROCEDURE_REV_ID = X_AUDIT_PROCEDURE_REV_ID;
  ...
  delete from AMW_AUDIT_PROCEDURES_B
  where AUDIT_PROCEDURE_REV_ID = X_AUDIT_PROCEDURE_REV_ID;
...
is
begin
  delete from AMW_AUDIT_PROCEDURES_TL T
  where not exists
    (select NULL
    from AMW_AUDIT_PROCEDURES_B B
    where B.AUDIT_PROCEDURE_REV_ID = T.AUDIT_PROCEDURE_REV_ID
    );

  update AMW_AUDIT_PROCEDURES_TL T set (
      NAME,
      DESCRIPTION
    ) = (select
      B.NAME,
  ...
  where (
      T.AUDIT_PROCEDURE_REV_ID,
      T.LANGUAGE
  ) in (select
      SUBT.AUDIT_PROCEDURE_REV_ID,
      SUBT.LANGUAGE
    from AMW_AUDIT_PROCEDURES_TL SUBB, AMW_AUDIT_PROCEDURES_TL SUBT
    where SUBB.AUDIT_PROCEDURE_REV_ID = SUBT.AUDIT_PROCEDURE_REV_ID
  ...
  insert into AMW_AUDIT_PROCEDURES_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from AMW_AUDIT_PROCEDURES_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from AMW_AUDIT_PROCEDURES_TL T
    where T.AUDIT_PROCEDURE_REV_ID = B.AUDIT_PROCEDURE_REV_ID
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;



end AMW_AUDIT_PROCEDURES_PKG;
