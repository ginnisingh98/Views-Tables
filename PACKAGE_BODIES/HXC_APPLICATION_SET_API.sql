--------------------------------------------------------
--  DDL for Package Body HXC_APPLICATION_SET_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_APPLICATION_SET_API" as
/* $Header: hxcapsapi.pkb 120.2 2005/09/23 08:05:28 sechandr noship $ */
--
-- Package Variables
--
g_package  varchar2(33) := '  hxc_application_set_api.';
...
begin
  ...
  --
  if g_debug then
	l_proc := g_package||' create_application_set';
	hr_utility.set_location('Entering:'|| l_proc, 10);
  end if;
  --
  -- Issue a savepoint if operating in validation only mode
  --
  savepoint create_application_set;
  --
  hxc_application_set_api.chk_name
	(    p_name		=> p_name
  ...
  --
  begin
    hxc_application_set_BK_1.create_application_set_b
  ...
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'create_application_set'
        ,p_hook_type   => 'BP'
        );
  end;
  --
...
hxc_heg_ins.ins (
  ...
  ,p_entity_group_id 	=> l_application_set_id
...
--
  ...
  --
  begin
    hxc_application_set_BK_1.create_application_set_a
...
exception
  ...
  when hr_api.validate_enabled then
    ...
    --
    ROLLBACK TO create_application_set;
    --
...
begin
  ...
  --
  if g_debug then
	 l_proc:= g_package||' update_application_set';
	 hr_utility.set_location('Entering:'|| l_proc, 10);
  end if;
  --
  -- Issue a savepoint if operating in validation only mode
  --
  savepoint update_application_set;
  --
  hxc_application_set_api.chk_name
	(    p_name		=> p_name
	,    p_entity_group_id	=> p_application_set_id );
  ...
  --
  begin
    hxc_application_set_BK_2.update_application_set_b
	  (p_application_set_id             => p_application_set_id
	  ,p_object_version_number  => p_object_version_number
	  ,p_name                   => p_name
  );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'update_application_set'
        ,p_hook_type   => 'BP'
        );
  end;
  --
...
--
  ...
  --
  begin
    hxc_application_set_BK_2.update_application_set_a
	  (p_application_set_id => p_application_set_id
	  ,p_object_version_number  => l_object_version_number
	  ,p_name                   => p_name
  );
...
exception
  ...
  when hr_api.validate_enabled then
    ...
    -- we must rollback to the savepoint
    --
    ROLLBACK TO update_application_set;
    --
    -- Only set output warning arguments
    -- (Any key or derived arguments must be set to null
    -- when validation only mode is being used.)
...
begin
  --
  if g_debug then
	 l_proc:= g_package||'delete_application_set';
	 hr_utility.set_location('Entering:'|| l_proc, 10);
  end if;
  --
  -- Issue a savepoint if operating in validation only mode
  --
  savepoint delete_application_set;
  --
	hxc_application_set_api.chk_delete (
				 p_entity_group_id => p_application_set_id
			    ,    p_entity_type     => g_entity_type );
  ...
  begin
  --
    hxc_application_set_BK_3.delete_application_set_b
	  (p_application_set_id => p_application_set_id
	  ,p_object_version_number    => p_object_version_number
	  );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'delete_application_set_b'
        ,p_hook_type   => 'BP'
        );
  end;
  --
  ...
  begin
  --
  hxc_application_set_BK_3.delete_application_set_a
	  (p_application_set_id            => p_application_set_id
	  ,p_object_version_number => p_object_version_number
	  );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'delete_application_set_a'
        ,p_hook_type   => 'AP'
        );
  end;
  --
...
exception
  ...
  when hr_api.validate_enabled then
    ...
    -- we must rollback to the savepoint
    --
    ROLLBACK TO delete_application_set;
    --
  when others then
    --
    -- A validation or unexpected error has occured
...
THEN
--
      hr_utility.set_message(809, 'HXC_HEG_AS_NAME_MAND');
...
THEN
--
      hr_utility.set_message(809, 'HXC_HEG_AS_NAME_DUP');
      hr_utility.raise_error;
--
END IF;
--
...
WHERE	EXISTS (
	...
	WHERE	preference_code = 'TS_PER_APPLICATION_SET'
...
IF l_exists <> 0 THEN
--
      hr_utility.set_message(809, 'HXC_HEG_AS_IN_USE');
      hr_utility.raise_error;
--
END IF;
--
