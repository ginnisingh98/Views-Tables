--------------------------------------------------------
--  DDL for Package Body IGC_CC_ACCT_LINE_HISTORY_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGC_CC_ACCT_LINE_HISTORY_PKG" as
/* $Header: IGCCALHB.pls 120.3.12000000.4 2007/10/19 07:23:49 smannava ship $  */

    G_PKG_NAME CONSTANT VARCHAR2(30):= 'IGC_CC_ACCT_LINE_HISTORY_PKG';
...
PROCEDURE Insert_Row(
   ...
   CURSOR C_acct_hst_row_id IS
     SELECT Rowid
       FROM IGC_CC_ACCT_LINE_HISTORY
      WHERE CC_Acct_Line_Id     = p_CC_Acct_Line_Id
        AND CC_Acct_Version_Num = p_CC_Acct_Version_Num;
...
-- -----------------------------------------------------------------
-- Insert the account line history record as requested.
-- -----------------------------------------------------------------
   INSERT
     INTO IGC_CC_ACCT_LINE_HISTORY
            (CC_Acct_Line_Id,
             CC_Header_Id,
             Parent_Header_Id,
             Parent_Acct_Line_Id,
...
-- to the caller.
-- -------------------------------------------------------------------
   OPEN C_acct_hst_row_id ;
   FETCH C_acct_hst_row_id
    INTO p_Rowid;

-- -------------------------------------------------------------------
-- If no ROWID can be obtained then exit the procedure with a failure
-- -------------------------------------------------------------------
   IF (C_acct_hst_row_id %NOTFOUND) then
      RAISE FND_API.G_EXC_ERROR ;
   END IF;

   CLOSE C_acct_hst_row_id ;
...
-- ------------------------------------------------------------------
-- MRC Handler For IGC_CC_ACCT_LINE_HISTORY to insert all MRC records for
-- the reporting set of books for this PRIMARY set of books.
-- ------------------------------------------------------------------
      IGC_CC_MC_MAIN_HST_PVT.get_rsobs_ACCT_LINES (
                                 l_api_version,
                                 FND_API.G_FALSE,
                                 FND_API.G_FALSE,
                                 p_validation_level,
...
EXCEPTION
   ...
   WHEN FND_API.G_EXC_ERROR THEN
    ...
    IF (C_acct_hst_row_id %ISOPEN) THEN
       CLOSE C_acct_hst_row_id ;
    END IF;
...
BEGIN
   ...
   UPDATE IGC_CC_ACCT_LINE_HISTORY
      SET CC_Acct_Line_Id               =  p_CC_Acct_Line_Id,
