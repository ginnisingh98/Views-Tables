--------------------------------------------------------
--  DDL for Package Body PER_QUA_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_QUA_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_qua_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_qualification_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_qualification_id  in  number) is
--
...
Begin
  ...
  --
  per_qua_ins.g_qualification_id_i := p_qualification_id;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure insert_dml(p_rec in out nocopy per_qua_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Insert the row into: per_qualifications
  --
  insert into per_qualifications
  (	qualification_id,
	business_group_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_insert(p_rec  in out nocopy per_qua_shd.g_rec_type) is
--
  l_proc  varchar2(72) := g_package||'pre_insert';
  l_exists varchar2(1);
--
  Cursor C_Sel1 is select per_qualifications_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
         select null
                from per_qualifications
                where qualification_id = per_qua_ins.g_qualification_id_i;
--

Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  if per_qua_ins.g_qualification_id_i is not null then
    --
    ...
    If C_Sel2%found then
      ...
      fnd_message.set_token('TABLE_NAME','per_qualifications');
    ...
    --
    p_rec.qualification_id :=
      per_qua_ins.g_qualification_id_i;
    per_qua_ins.g_qualification_id_i := null;
    --
  else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.qualification_id;
    Close C_Sel1;
    --
  end if;
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_insert(p_rec in per_qua_shd.g_rec_type,
...
Begin
  ...
  --
  per_qua_bus.insert_validate(p_rec,p_effective_date);
  --
...
Begin
  ...
  --
  p_qualification_id := l_rec.qualification_id;
