--------------------------------------------------------
--  DDL for Package Body GR_EXPOSURE_AUTHS_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_EXPOSURE_AUTHS_TL_PKG" AS
/*$Header: GRHIEATB.pls 115.12 2002/10/29 20:45:25 mgrosser ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_exposure_auths_tl
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ROLLBACK TO SAVEPOINT Insert_Row;
	  l_msg_token := p_exposure_authority_code || ' ' || p_language;
	  x_return_status := 'E';
...
CURSOR c_get_descs
 IS
   SELECT	eat.exposure_authority_desc,
            eat.created_by,
			eat.creation_date,
			eat.last_updated_by,
			eat.last_update_date,
			eat.last_update_login
   FROM	    gr_exposure_auths_tl eat
   WHERE	eat.exposure_authority_code = p_exposure_authority_code
   AND		eat.language = l_language;
AuthDesc				c_get_descs%ROWTYPE;
...
/* Remove translations with no base row */
  ...
  delete from GR_EXPOSURE_AUTHS_TL T
  where not exists
    (select NULL
    from GR_EXPOSURE_AUTHS_B B
    where B.EXPOSURE_AUTHORITY_CODE = T.EXPOSURE_AUTHORITY_CODE
    );

/* Redefault translations from the source language  */

   update gr_exposure_auths_tl t set (
    exposure_authority_desc ) =
    ( select
      B.EXPOSURE_AUTHORITY_DESC
      from GR_EXPOSURE_AUTHS_TL B
      where B.EXPOSURE_AUTHORITY_CODE = T.EXPOSURE_AUTHORITY_CODE
      and B.LANGUAGE = T.SOURCE_LANG)
   where (
      T.EXPOSURE_AUTHORITY_CODE,
      T.LANGUAGE
   ) in (select
         SUBT.EXPOSURE_AUTHORITY_CODE,
         SUBT.LANGUAGE
         from GR_EXPOSURE_AUTHS_TL SUBB, GR_EXPOSURE_AUTHS_TL SUBT
         where SUBB.EXPOSURE_AUTHORITY_CODE = SUBT.EXPOSURE_AUTHORITY_CODE
         and SUBB.LANGUAGE = SUBT.SOURCE_LANG
         and (SUBB.EXPOSURE_AUTHORITY_DESC <> SUBT.EXPOSURE_AUTHORITY_DESC
          or (SUBB.EXPOSURE_AUTHORITY_DESC is null and SUBT.EXPOSURE_AUTHORITY_DESC is not null)
          or (SUBB.EXPOSURE_AUTHORITY_DESC is not null and SUBT.EXPOSURE_AUTHORITY_DESC is null)
  ));
...
*/
   ...
   OPEN c_get_descs;
   FETCH c_get_descs INTO AuthDesc;
   IF c_get_descs%NOTFOUND THEN
      CLOSE c_get_descs;
      RAISE Language_Missing_Error;
   ELSE
      l_base_desc := AuthDesc.exposure_authority_desc;
	  l_created_by := AuthDesc.created_by;
	  l_creation_date := AuthDesc.creation_date;
	  l_last_updated_by := AuthDesc.last_updated_by;
	  l_last_update_date := AuthDesc.last_update_date;
	  l_last_update_login := AuthDesc.last_update_login;
      CLOSE c_get_descs;
   END IF;

/*	Read fnd_languages for the installed and base languages.
**	For those that are found, read the auths tl table.
...
CURSOR c_lock_auths_tl
 IS
   SELECT	last_update_date
   FROM		gr_exposure_auths_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockAuthRcd	  c_lock_auths_tl%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_auths_tl;
   FETCH c_lock_auths_tl INTO LockAuthRcd;
   IF c_lock_auths_tl%NOTFOUND THEN
	  CLOSE c_lock_auths_tl;
	  RAISE No_Data_Found_Error;
   END IF;
   CLOSE c_lock_auths_tl;

   IF LockAuthRcd.last_update_date <> p_last_update_date THEN
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_exposure_auths_tl
   WHERE  	   rowid = p_rowid;
...
/*		  p_exposure_authority_code is the authority code to check.
...
CURSOR c_get_auths_tl_rowid
 IS
   SELECT eat.rowid
   FROM	  gr_exposure_auths_tl eat
   WHERE  eat.exposure_authority_code = p_exposure_authority_code
   AND	  eat.language = p_language;
AuthTLRecord			   c_get_auths_tl_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_exposure_authority_code || ' ' || p_language;

   x_key_exists := 'F';
   OPEN c_get_auths_tl_rowid;
   FETCH c_get_auths_tl_rowid INTO AuthTLRecord;
   IF c_get_auths_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := AuthTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_auths_tl_rowid;
...
BEGIN
	UPDATE GR_EXPOSURE_AUTHS_TL SET
		EXPOSURE_AUTHORITY_DESC = X_EXPOSURE_AUTHORITY_DESC,
		SOURCE_LANG = USERENV('LANG'),
		LAST_UPDATE_DATE = sysdate,
		LAST_UPDATED_BY = 0,
		LAST_UPDATE_LOGIN = 0
	WHERE (EXPOSURE_AUTHORITY_CODE = X_EXPOSURE_AUTHORITY_CODE)
...
BEGIN
	...
	IF Cur_rowid%FOUND THEN
		GR_EXPOSURE_AUTHS_TL_PKG.UPDATE_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_ROWID => l_row_id
			,P_EXPOSURE_AUTHORITY_CODE => X_EXPOSURE_AUTHORITY_CODE
			,P_LANGUAGE => X_LANGUAGE
			,P_EXPOSURE_AUTHORITY_DESC => X_EXPOSURE_AUTHORITY_DESC
			...
			,X_MSG_DATA => l_msg_data);
	ELSE
		GR_EXPOSURE_AUTHS_TL_PKG.INSERT_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_EXPOSURE_AUTHORITY_CODE => X_EXPOSURE_AUTHORITY_CODE
			,P_LANGUAGE => X_LANGUAGE
...
begin
  ...
  insert into GR_EXPOSURE_AUTHS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GR_EXPOSURE_AUTHS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from GR_EXPOSURE_AUTHS_TL T
    where T.EXPOSURE_AUTHORITY_CODE = B.EXPOSURE_AUTHORITY_CODE
