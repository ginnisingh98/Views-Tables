--------------------------------------------------------
--  DDL for Package Body PAY_AE_PAYROLL_REGISTER
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_AE_PAYROLL_REGISTER" AS
/* $Header: pyaepyrg.pkb 120.8.12010000.4 2008/12/08 11:58:45 bkeshary ship $ */
...
TYPE rec_det IS RECORD       ( r_assact_id	number(15),
				...
				r_title		varchar2(240),
				r_first_name 	varchar2(240),
				r_family_name 	varchar2(240),
				r_payroll_name	varchar2(80),
				r_nationality	varchar2(80),
...
org_ded_sum_tot 	number(12,3):=0;
org_earn_sum_tot 	number(12,3):=0;
--org_earn_sum_last	number(12,3) :=0;
org_earn_sum_last	varchar2(40);
l_temp_sum 		number(13,2);
l_tot_sum 		number(13,2);
l_arch_assact_1		number;
l_d_temp_sum 		number(13,2);
l_d_tot_sum 		number(13,2);
...
/* Modifyig the cursor for performance issue for Bug 7597523 */
...
and    ppa1.report_type = 'AE_ARCHIVE'
AND    ppa1.action_status = 'C'
AND    paa1.action_status = 'C'
AND    ptp.end_date = l_effective_date
AND    ppf.payroll_id = ptp.payroll_id
...
/* Modifying this cursor for bug 6712796
...
/*Modifying this cursor for bug 7307152
...
AND      pai_ele.action_information_category = 'EMEA ELEMENT DEFINITION'
AND      pai_ele.action_information7 IN ('E')
AND      pet.assignment_action_id = paa.assignment_action_id
AND      pet.element_type_id  = pai_ele.action_information2
AND      pai_ele.action_information5 NOT IN ('F')
AND      pet.input_value_id = pai_ele.action_information3
AND      paa.assignment_action_id in (SELECT        pai_emp.action_context_id arch_assact
FROM          per_time_periods ptp
            ,pay_action_information pai_emp
            ,pay_assignment_actions paa1
		...
		LOOP
			...
			If csr_get_details % notfound then
				...
				exit when csr_get_details%notfound;
					...
					vXMLTable(vCtr).TagValue := substr(tab_dets_data(j).r_first_name,1,120) ;
					vCtr := vCtr + 1;
...
/******************/
			...
			if p_payroll_id is null then
				...
				vCtr := vCtr + 1;
				OPEN csr_get_organization_name(p_organization_id);
				FETCH csr_get_organization_name into l_sum_org_val;
				CLOSE csr_get_organization_name;
				vXMLTable(vCtr).TagName := 'organization_name_summary_value';
				vXMLTable(vCtr).TagValue := nvl(l_sum_org_val,' ');
				vCtr := vCtr + 1;
				...
				vXMLTable(vCtr).TagValue := to_char(nvl(org_earn_sum_tot,0),lg_format_mask);
				vCtr := vCtr + 1;
				...
				vXMLTable(vCtr).TagValue := to_char(nvl(org_ded_sum_tot,0),lg_format_mask);
				vCtr := vCtr + 1;
				...
				vXMLTable(vCtr).TagValue := to_char((nvl(org_earn_sum_tot,0) - nvl(org_ded_sum_tot,0)),lg_format_mask);
				vCtr := vCtr + 1;
		...
		if nvl(tab_sum_data.count,0) > 0 then
			...
			LOOP
				...
				ELSE
/*****split the cursors into two cursors *************/
				 /* open csr_get_sum_earn_only_org (p_organization_id , tab_sum_data(i).payroll_id, p_effective_date);
				  ...
				  LOOP
				  	fetch csr_SEOO_SPLIT_1 into l_arch_assact_1;
				  	If csr_SEOO_SPLIT_1%NOTFOUND then
				  		close csr_SEOO_SPLIT_1;
				  		EXIT;
				  	End If;
				  	if l_arch_assact_1 is not null then
				  		open csr_SEOO_SPLIT_2(l_arch_assact_1);
				  		fetch csr_SEOO_SPLIT_2 into l_temp_sum;
				  		close csr_SEOO_SPLIT_2;
				  		l_tot_sum := l_tot_sum + l_temp_sum;
				  		l_temp_sum := 0;
				  	End If;
				  END LOOP;
				  org_earn_sum_last := to_char(l_tot_sum);
                                END IF;
				IF p_org_structure_version_id IS NOT NULL THEN
...
/*****split the cursors into two cursors *************/
				...
				vXMLTable(vCtr).TagValue := to_char(nvl(to_number(org_ded_sum_last),0),lg_format_mask);
				vCtr := vCtr + 1;
				vXMLTable(vCtr).TagName := 'total_pay_p_label';
				vXMLTable(vCtr).TagValue := get_lookup_meaning('KW_FORM_LABELS','TOTAL_PAY_LABEL');
				vCtr := vCtr + 1;
				vXMLTable(vCtr).TagName := 'total_pay_p_value';
				vXMLTable(vCtr).TagValue := to_char(nvl(to_number(org_earn_sum_last),0)- nvl(to_number(org_ded_sum_last),0),lg_format_mask);
				vCtr := vCtr + 1;
			...
			dbms_lob.writeAppend( l_xfdf_string, length(l_str1), l_str1 );
        		FOR ctr_table IN vXMLTable.FIRST .. vXMLTable.LAST LOOP
        			...
        			if (l_str9 is not null) then
				        /* Added CDATA to handle special characters Bug No: 6676325*/
					l_str9 := '<![CDATA['||l_str9||']]>';
...
IS
	BEGIN
		If p_report = 'MAIN' then
		/*Changing thequery for performance issue for bug 7597523 */
			...
			WHERE  file_id = (SELECT MAX(file_id)
			                  FROM    fnd_lobs
                	                         WHERE   file_name like '%PAY_PRG_ar_AE.pdf'); */
			SELECT file_data
			INTO   p_pdf_blob
			FROM   fnd_lobs
			WHERE  file_id =
			  ( SELECT MAX(file_id)
			  from FND_LOBS
			  WHERE PROGRAM_NAME = 'PAY_PRG_ar_AE.pdf'
