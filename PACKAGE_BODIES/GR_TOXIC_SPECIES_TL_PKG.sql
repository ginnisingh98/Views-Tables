--------------------------------------------------------
--  DDL for Package Body GR_TOXIC_SPECIES_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_TOXIC_SPECIES_TL_PKG" AS
/*$Header: GRHITSTB.pls 115.10 2002/10/28 21:40:46 gkelly ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_toxic_species_tl
   		  	     (toxic_species_code,
				  language,
				  source_lang,
				  toxic_species_desc,
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ROLLBACK TO SAVEPOINT Insert_Row;
	  l_msg_token := p_toxic_species_code || ' ' || p_language;
	  x_return_status := 'E';
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_toxic_species_tl
	  SET	 toxic_species_code		 	 	 = p_toxic_species_code,
	  		 language						 = p_language,
			 source_lang					 = p_source_lang,
			 toxic_species_desc		 		 = p_toxic_species_desc,
...
CURSOR c_get_descs
 IS
   SELECT	tst.toxic_species_desc,
            tst.created_by,
			tst.creation_date,
			tst.last_updated_by,
			tst.last_update_date,
			tst.last_update_login
   FROM	    gr_toxic_species_tl tst
   WHERE	tst.toxic_species_code = p_toxic_species_code
   AND		tst.language = l_language;
SpeciesDesc				c_get_descs%ROWTYPE;
...
/* Remove translations with no base row */
  ...
  delete from GR_TOXIC_SPECIES_TL T
  where not exists
    (select NULL
    from GR_TOXIC_SPECIES_B B
    where B.TOXIC_SPECIES_CODE = T.TOXIC_SPECIES_CODE
    );

/* Redefault translations from the source language  */

   update gr_toxic_species_tl t set (
    toxic_species_desc ) =
    ( select
      B.TOXIC_SPECIES_DESC
      from GR_TOXIC_SPECIES_TL B
      where B.TOXIC_SPECIES_CODE = T.TOXIC_SPECIES_CODE
      and B.LANGUAGE = T.SOURCE_LANG)
   where (
      T.TOXIC_SPECIES_CODE,
      T.LANGUAGE
   ) in (select
         SUBT.TOXIC_SPECIES_CODE,
         SUBT.LANGUAGE
         from GR_TOXIC_SPECIES_TL SUBB, GR_TOXIC_SPECIES_TL SUBT
         where SUBB.TOXIC_SPECIES_CODE = SUBT.TOXIC_SPECIES_CODE
         and SUBB.LANGUAGE = SUBT.SOURCE_LANG
         and (SUBB.TOXIC_SPECIES_DESC <> SUBT.TOXIC_SPECIES_DESC
          or (SUBB.TOXIC_SPECIES_DESC is null and SUBT.TOXIC_SPECIES_DESC is not null)
          or (SUBB.TOXIC_SPECIES_DESC is not null and SUBT.TOXIC_SPECIES_DESC is null)
  ));
...
*/
   ...
   OPEN c_get_descs;
   FETCH c_get_descs INTO SpeciesDesc;
   IF c_get_descs%NOTFOUND THEN
      CLOSE c_get_descs;
      RAISE Language_Missing_Error;
   ELSE
      l_base_desc := SpeciesDesc.toxic_species_desc;
	  l_created_by := SpeciesDesc.created_by;
	  l_creation_date := SpeciesDesc.creation_date;
	  l_last_updated_by := SpeciesDesc.last_updated_by;
	  l_last_update_date := SpeciesDesc.last_update_date;
	  l_last_update_login := SpeciesDesc.last_update_login;
      CLOSE c_get_descs;
   END IF;
...
CURSOR c_lock_species_tl
 IS
   SELECT	last_update_date
   FROM		gr_toxic_species_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockSpeciesRcd	  	c_lock_species_tl%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_species_tl;
   FETCH c_lock_species_tl INTO LockSpeciesRcd;
   IF c_lock_species_tl%NOTFOUND THEN
	  CLOSE c_lock_species_tl;
	  RAISE No_Data_Found_Error;
   END IF;
   CLOSE c_lock_species_tl;


   IF LockSpeciesRcd.last_update_date <> p_last_update_date THEN
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_toxic_species_tl
   WHERE  	   rowid = p_rowid;
...
/*		  p_toxic_species_code is the species code to check.
...
CURSOR c_get_species_tl_rowid
 IS
   SELECT tst.rowid
   FROM	  gr_toxic_species_tl tst
   WHERE  tst.toxic_species_code = p_toxic_species_code
   AND	  tst.language = p_language;
SpeciesTLRecord			   c_get_species_tl_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_toxic_species_code || ' ' || p_language;

   x_key_exists := 'F';
   OPEN c_get_species_tl_rowid;
   FETCH c_get_species_tl_rowid INTO SpeciesTLRecord;
   IF c_get_species_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := SpeciesTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_species_tl_rowid;
...
BEGIN
	UPDATE GR_TOXIC_SPECIES_TL SET
		TOXIC_SPECIES_DESC = X_TOXIC_SPECIES_DESC,
		SOURCE_LANG = USERENV('LANG'),
		LAST_UPDATE_DATE = sysdate,
		LAST_UPDATED_BY = 0,
		LAST_UPDATE_LOGIN = 0
	WHERE (TOXIC_SPECIES_CODE = X_TOXIC_SPECIES_CODE)
...
BEGIN
	...
	IF Cur_rowid%FOUND THEN
		GR_TOXIC_SPECIES_TL_PKG.UPDATE_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_ROWID => l_row_id
			,P_TOXIC_SPECIES_CODE => X_TOXIC_SPECIES_CODE
			,P_LANGUAGE => X_LANGUAGE
			,P_SOURCE_LANG => X_SOURCE_LANG
			,P_TOXIC_SPECIES_DESC => X_TOXIC_SPECIES_DESC
			...
			,X_MSG_DATA => l_msg_data);
	ELSE
		GR_TOXIC_SPECIES_TL_PKG.INSERT_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_TOXIC_SPECIES_CODE => X_TOXIC_SPECIES_CODE
			,P_LANGUAGE => X_LANGUAGE
...
begin
  ...
  insert into GR_TOXIC_SPECIES_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GR_TOXIC_SPECIES_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from GR_TOXIC_SPECIES_TL T
    where T.TOXIC_SPECIES_CODE = B.TOXIC_SPECIES_CODE
