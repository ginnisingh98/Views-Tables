--------------------------------------------------------
--  DDL for Package Body BEN_CPO_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CPO_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_cpo_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_popl_org_f',
	   ...
	   p_base_key_value	=> p_rec.popl_org_id);
    --
    ben_cpo_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_popl_org_f Row
    --
    ...
    --
    ben_cpo_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_cpo_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.popl_org_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_cpo_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    ben_cpo_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.cstmr_num = hr_api.g_number) then
    p_rec.cstmr_num :=
    ben_cpo_shd.g_old_rec.cstmr_num;
  End If;
  If (p_rec.plcy_r_grp = hr_api.g_varchar2) then
    p_rec.plcy_r_grp :=
    ben_cpo_shd.g_old_rec.plcy_r_grp;
  End If;
  If (p_rec.pgm_id = hr_api.g_number) then
    p_rec.pgm_id :=
    ben_cpo_shd.g_old_rec.pgm_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_cpo_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.organization_id = hr_api.g_number) then
    p_rec.organization_id :=
    ben_cpo_shd.g_old_rec.organization_id;
  End If;
  If (p_rec.cpo_attribute_category = hr_api.g_varchar2) then
    p_rec.cpo_attribute_category :=
    ben_cpo_shd.g_old_rec.cpo_attribute_category;
  End If;
  If (p_rec.cpo_attribute1 = hr_api.g_varchar2) then
    p_rec.cpo_attribute1 :=
    ben_cpo_shd.g_old_rec.cpo_attribute1;
  End If;
  If (p_rec.cpo_attribute2 = hr_api.g_varchar2) then
    p_rec.cpo_attribute2 :=
    ben_cpo_shd.g_old_rec.cpo_attribute2;
  End If;
  If (p_rec.cpo_attribute3 = hr_api.g_varchar2) then
    p_rec.cpo_attribute3 :=
    ben_cpo_shd.g_old_rec.cpo_attribute3;
  End If;
  If (p_rec.cpo_attribute4 = hr_api.g_varchar2) then
    p_rec.cpo_attribute4 :=
    ben_cpo_shd.g_old_rec.cpo_attribute4;
  End If;
  If (p_rec.cpo_attribute5 = hr_api.g_varchar2) then
    p_rec.cpo_attribute5 :=
    ben_cpo_shd.g_old_rec.cpo_attribute5;
  End If;
  If (p_rec.cpo_attribute6 = hr_api.g_varchar2) then
    p_rec.cpo_attribute6 :=
    ben_cpo_shd.g_old_rec.cpo_attribute6;
  End If;
  If (p_rec.cpo_attribute7 = hr_api.g_varchar2) then
    p_rec.cpo_attribute7 :=
    ben_cpo_shd.g_old_rec.cpo_attribute7;
  End If;
  If (p_rec.cpo_attribute8 = hr_api.g_varchar2) then
    p_rec.cpo_attribute8 :=
    ben_cpo_shd.g_old_rec.cpo_attribute8;
  End If;
  If (p_rec.cpo_attribute9 = hr_api.g_varchar2) then
    p_rec.cpo_attribute9 :=
    ben_cpo_shd.g_old_rec.cpo_attribute9;
  End If;
  If (p_rec.cpo_attribute10 = hr_api.g_varchar2) then
    p_rec.cpo_attribute10 :=
    ben_cpo_shd.g_old_rec.cpo_attribute10;
  End If;
  If (p_rec.cpo_attribute11 = hr_api.g_varchar2) then
    p_rec.cpo_attribute11 :=
    ben_cpo_shd.g_old_rec.cpo_attribute11;
  End If;
  If (p_rec.cpo_attribute12 = hr_api.g_varchar2) then
    p_rec.cpo_attribute12 :=
    ben_cpo_shd.g_old_rec.cpo_attribute12;
  End If;
  If (p_rec.cpo_attribute13 = hr_api.g_varchar2) then
    p_rec.cpo_attribute13 :=
    ben_cpo_shd.g_old_rec.cpo_attribute13;
  End If;
  If (p_rec.cpo_attribute14 = hr_api.g_varchar2) then
    p_rec.cpo_attribute14 :=
    ben_cpo_shd.g_old_rec.cpo_attribute14;
  End If;
  If (p_rec.cpo_attribute15 = hr_api.g_varchar2) then
    p_rec.cpo_attribute15 :=
    ben_cpo_shd.g_old_rec.cpo_attribute15;
  End If;
  If (p_rec.cpo_attribute16 = hr_api.g_varchar2) then
    p_rec.cpo_attribute16 :=
    ben_cpo_shd.g_old_rec.cpo_attribute16;
  End If;
  If (p_rec.cpo_attribute17 = hr_api.g_varchar2) then
    p_rec.cpo_attribute17 :=
    ben_cpo_shd.g_old_rec.cpo_attribute17;
  End If;
  If (p_rec.cpo_attribute18 = hr_api.g_varchar2) then
    p_rec.cpo_attribute18 :=
    ben_cpo_shd.g_old_rec.cpo_attribute18;
  End If;
  If (p_rec.cpo_attribute19 = hr_api.g_varchar2) then
    p_rec.cpo_attribute19 :=
    ben_cpo_shd.g_old_rec.cpo_attribute19;
  End If;
  If (p_rec.cpo_attribute20 = hr_api.g_varchar2) then
    p_rec.cpo_attribute20 :=
    ben_cpo_shd.g_old_rec.cpo_attribute20;
  End If;
  If (p_rec.cpo_attribute21 = hr_api.g_varchar2) then
    p_rec.cpo_attribute21 :=
    ben_cpo_shd.g_old_rec.cpo_attribute21;
  End If;
  If (p_rec.cpo_attribute22 = hr_api.g_varchar2) then
    p_rec.cpo_attribute22 :=
    ben_cpo_shd.g_old_rec.cpo_attribute22;
  End If;
  If (p_rec.cpo_attribute23 = hr_api.g_varchar2) then
    p_rec.cpo_attribute23 :=
    ben_cpo_shd.g_old_rec.cpo_attribute23;
  End If;
  If (p_rec.cpo_attribute24 = hr_api.g_varchar2) then
    p_rec.cpo_attribute24 :=
    ben_cpo_shd.g_old_rec.cpo_attribute24;
  End If;
  If (p_rec.cpo_attribute25 = hr_api.g_varchar2) then
    p_rec.cpo_attribute25 :=
    ben_cpo_shd.g_old_rec.cpo_attribute25;
  End If;
  If (p_rec.cpo_attribute26 = hr_api.g_varchar2) then
    p_rec.cpo_attribute26 :=
    ben_cpo_shd.g_old_rec.cpo_attribute26;
  End If;
  If (p_rec.cpo_attribute27 = hr_api.g_varchar2) then
    p_rec.cpo_attribute27 :=
    ben_cpo_shd.g_old_rec.cpo_attribute27;
  End If;
  If (p_rec.cpo_attribute28 = hr_api.g_varchar2) then
    p_rec.cpo_attribute28 :=
    ben_cpo_shd.g_old_rec.cpo_attribute28;
  End If;
  If (p_rec.cpo_attribute29 = hr_api.g_varchar2) then
    p_rec.cpo_attribute29 :=
    ben_cpo_shd.g_old_rec.cpo_attribute29;
  End If;
  If (p_rec.cpo_attribute30 = hr_api.g_varchar2) then
    p_rec.cpo_attribute30 :=
    ben_cpo_shd.g_old_rec.cpo_attribute30;
  End If;
