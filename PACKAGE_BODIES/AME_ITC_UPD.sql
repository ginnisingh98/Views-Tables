--------------------------------------------------------
--  DDL for Package Body AME_ITC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ITC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_itc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the ame_item_classes Row
    --
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    ame_itc_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_item_class_id          =>  p_rec.item_class_id
...
Begin
  ...
  begin
    ...
    ame_itc_rku.after_update
      ...
      ,p_name_o                  => ame_itc_shd.g_old_rec.name
      ,p_start_date_o            => ame_itc_shd.g_old_rec.start_date
      ,p_end_date_o              => ame_itc_shd.g_old_rec.end_date
      ,p_object_version_number_o => ame_itc_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  --
  ame_itc_upd.convert_defs(p_rec);
  --
  ame_itc_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
