--------------------------------------------------------
--  DDL for Package Body WSH_ITM_USERS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WSH_ITM_USERS_PKG" AS
   /* $Header: WSHITUTB.pls 115.6 2002/12/12 12:07:21 bradha ship $ */
  --  Global constant holding the package name
  G_PKG_NAME      CONSTANT VARCHAR2(30) := 'WSH_ITM_USERS_PKG';
 ...
 /*  --
   -- Purpose
   --  Insert a row into WSH_ITM_USERS_SETUP entity
   --
   -- Input Parameters
   --   p_api_version
   ...
   --   p_ITM_USER_SEQUENCE_ID  - ITM sequence Id ( PK)
   --*/
...
IS
   l_ITM_USER_SEQUENCE_ID NUMBER ;
   l_api_name        CONSTANT VARCHAR2(30)      := 'Insert_Row';
   l_api_version     number := 1.0;
begin
--dbms_output.put_line('begin api');
  -- Standard Start of API savepoint
  SAVEPOINT  WSH_ITM_USERS_PKG;
  ...
  select wsh_itm_user_setups_s.nextval into l_ITM_USER_SEQUENCE_ID from dual;
  /* Validate input parameters if any */

  -- Insert a row into wsh_itm_users entity with all detail information
    insert into wsh_itm_user_setups
    ( ITM_USER_SEQUENCE_ID
     ,VENDOR_ID
     ,APPLICATION_ID
  ...
  END IF;
--dbms_output.put_line('Seq Id got it '||l_ITM_USER_SEQUENCE_ID||'success');
  x_return_status := fnd_api.g_ret_sts_success;
  p_ITM_USER_SEQUENCE_ID := l_ITM_USER_SEQUENCE_ID;
  -- End of API body
...
/*----------------------------------------------------------*/
 ...
 /*  --
   ...
   --     p_ORGANIZATION_ID   --Org
   --     p_APPLICATION_USER_ID  -- Application User
   --     p_USER_ID -- Clear Cross User
   --     p_ENCRYPTED_USER_PASSWORD -- Password
   --     p_ITM_USER_SEQUENCE_ID  -- CC Seq Id
   --
   --
   -- Output Parameters
   --   x_return_status
...
--dbms_output.put_line('begin');
  ...
  -- for the given  seq id

 update wsh_itm_user_setups
 SET
 ...
 ,LAST_UPDATE_LOGIN        = FND_GLOBAL.login_id
  where itm_user_sequence_id = p_itm_user_sequence_id;
...
--dbms_output.put_line('begin');
  ...
  -- Delete a row from wsh_itm_users entity
  -- for the given cc seq id

 DELETE from wsh_itm_user_setups
 WHERE itm_user_sequence_id = p_itm_user_sequence_id ;
  IF SQL%NOTFOUND THEN
...
IS
   CURSOR lock_row IS
   SELECT *
   FROM wsh_itm_user_setups
   WHERE ITM_USER_SEQUENCE_ID = p_ITM_USER_SEQUENCE_ID
   FOR UPDATE OF ITM_USER_SEQUENCE_ID NOWAIT;
...
BEGIN
  ...
  -- Check Lock a row into wsh_itm_users entity with all detail information
  ...
  FETCH lock_row into Recinfo;
      ...
      IF        ((Recinfo.ITM_USER_SEQUENCE_ID = p_ITM_USER_SEQUENCE_ID)
            AND
                (Recinfo.vendor_id = p_vendor_id)
            AND ((Recinfo.master_organization_id =p_master_organization_id)
                 OR
            ...
            AND ((Recinfo.application_user_id =p_application_user_id)
                 OR
                ((Recinfo.application_user_id is null)
                 AND (p_application_user_id is null )))
            AND (Recinfo.encrypted_user_password =p_encrypted_user_password)
            AND (Recinfo.application_id =p_APPLICATION_ID)) THEN
          RETURN;
      ELSE
 ...
 EXCEPTION
    ...
    WHEN others THEN
      ...
      FND_MESSAGE.Set_Token('PACKAGE', 'WSH_ITM_USERS_PKG.LOCK_ROW');
