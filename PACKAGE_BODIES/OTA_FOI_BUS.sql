--------------------------------------------------------
--  DDL for Package Body OTA_FOI_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_FOI_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_foi_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_forum_id                    number         default null;
...
Procedure set_security_group_id
  ...
  -- ota_frm_obj_inclusions and PER_BUSINESS_GROUPS_PERF
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'FORUM_ID')
...
Begin
  ...
  --
  if (( nvl(ota_foi_bus.g_forum_id, hr_api.g_number)
       = p_forum_id)
  and ( nvl(ota_foi_bus.g_object_id, hr_api.g_number)
       = p_object_id)
  and ( nvl(ota_foi_bus.g_object_type, hr_api.g_varchar2)
       = p_object_type)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := ota_foi_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    ota_foi_bus.g_forum_id                    := p_forum_id;
    ota_foi_bus.g_object_id                   := p_object_id;
    ota_foi_bus.g_object_type                 := p_object_type;
    ota_foi_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT ota_foi_shd.api_updating
      (p_forum_id                          => p_rec.forum_id
...
Begin
  ...
  WHEN app_exception.application_exception THEN
               ...
               IF hr_multi_message.exception_add(
                    p_associated_column1    => 'OTA_FRM_OBJ_INCLUSIONS.forum_id',
                    p_associated_column2    => 'OTA_FRM_OBJ_INCLUSIONS.object_id')
                                                              THEN
...
--
Procedure check_forum_category_dates
  (
   p_forum_id           in    number
  , p_object_id        in number
  ) is
  ...
  CURSOR csr_cat_start_end_date is
   ...
   CURSOR csr_forum_start_end_date IS
  ...
  l_cat_end_date          date;
  l_forum_start_date date;
  l_forum_end_date   date;

Begin
  hr_utility.set_location(' Entering:' || l_proc,10);
  --
  IF hr_multi_message.no_exclusive_error
          (p_check_column1   => 'OTA_FORUMS_B.START_DATE_ACTIVE'
          ,p_check_column2   => 'OTA_FORUMS_B.END_DATE_ACTIVE'
          ,p_associated_column1   => 'OTA_FORUMS_B.START_DATE_ACTIVE'
          ,p_associated_column2   => 'OTA_FORUMS_B.END_DATE_ACTIVE'
        ) THEN
     --
     OPEN csr_cat_start_end_date;
     FETCH csr_cat_start_end_date into l_cat_start_date, l_cat_end_date;

     OPEN csr_forum_start_end_date;
     FETCH csr_forum_start_end_date into l_forum_start_date, l_forum_end_date;

     IF csr_cat_start_end_date%FOUND  AND csr_forum_start_end_date%FOUND THEN
        CLOSE csr_cat_start_end_date;
	 CLOSE csr_forum_start_end_date;
        IF ( l_cat_start_date > l_forum_start_date
             or l_cat_end_date < l_forum_end_date
           ) THEN
          --
          fnd_message.set_name      ( 'OTA','OTA_443785_FRM_OUTOF_CAT_DATES');
	  fnd_message.raise_error;
          --
        End IF;
     ELSE
...
Exception
  ...
  --
End check_forum_category_dates;
--
-- ----------------------------------------------------------------------------
...
-- PUBLIC
-- Description:
--   There can be only one primary category for a forum..
--
...
WHEN app_exception.application_exception THEN
               ...
               IF hr_multi_message.exception_add(
                    p_associated_column1    => 'OTA_FRM_OBJ_INCLUSIONS.PRIMARY_FLAG')
...
-- PUBLIC
-- Description:
--   Check if th category forum already has a primary category.
...
Begin
  ...
  if sel_primary_category%found then
    ...
    fnd_message.set_name('OTA', 'OTA_443940_FRM_DEL_PRIMARY');
    fnd_message.raise_error;
  end if;
  close sel_primary_category;
  --
...
Begin
  ...
  WHEN app_exception.application_exception THEN
               ...
               IF hr_multi_message.exception_add(
                    p_associated_column1    => 'OTA_FRM_OBJ_INCLUSIONS.START_DATE_ACTIVE'
                    ,p_associated_column2    => 'OTA_FRM_OBJ_INCLUSIONS.END_DATE_ACTIVE')
                                           THEN
...
Begin
  ...
  -- Validate Dependent Attributes
    check_unique_key( p_forum_id
                    , p_object_id );

   if p_rec.primary_flag = 'Y' then
        check_multiple_primary_ctgr(p_forum_id);
      check_forum_category_dates(p_forum_id       => p_rec.forum_id
                             , p_object_id => p_rec.object_id);
  end if;
...
Begin
  ...
  -- Call all supporting business operations
  --
  --  check_if_primary_category(p_forum_id     => p_rec.forum_id,
  --                          p_object_id    => p_rec.object_id);
