--------------------------------------------------------
--  DDL for Package Body HR_DTY_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_DTY_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hr_dty_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  update hr_document_types
    ...
    ,authorization_required          = p_rec.authorization_required
    ,warning_period                  = p_rec.warning_period
...
Begin
  ...
  If (p_rec.category_code = hr_api.g_varchar2) then
    ...
    hr_dty_shd.g_old_rec.category_code;
  End If;
  If (p_rec.sub_category_code = hr_api.g_varchar2) then
    p_rec.sub_category_code :=
    hr_dty_shd.g_old_rec.sub_category_code;
  End If;
  If (p_rec.active_inactive_flag = hr_api.g_varchar2) then
    p_rec.active_inactive_flag :=
    hr_dty_shd.g_old_rec.active_inactive_flag;
  End If;
  If (p_rec.multiple_occurences_flag = hr_api.g_varchar2) then
    p_rec.multiple_occurences_flag :=
    hr_dty_shd.g_old_rec.multiple_occurences_flag;
  End If;
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    ...
    hr_dty_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.authorization_required = hr_api.g_varchar2) then
    p_rec.authorization_required :=
    hr_dty_shd.g_old_rec.authorization_required;
  End If;
  If (p_rec.warning_period = hr_api.g_number) then
    p_rec.warning_period :=
    hr_dty_shd.g_old_rec.warning_period;
  End If;
...
Begin
  ...
  --
  hr_dty_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_dty_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_dty_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure upd
  ...
  ,p_authorization_required       in     varchar2  default hr_api.g_varchar2
