--------------------------------------------------------
--  DDL for Package Body IGS_AS_EXM_MTRL_TYPE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_EXM_MTRL_TYPE_PKG" as
/* $Header: IGSDI43B.pls 115.7 2003/05/19 04:43:37 ijeddy ship $ */

 l_rowid VARCHAR2(25);
  old_references IGS_AS_EXM_MTRL_TYPE%RowType;
  new_references IGS_AS_EXM_MTRL_TYPE%RowType;
...
PROCEDURE Check_Child_Existance as
  BEGIN
    ...
    IGS_AS_ITM_EXAM_MTRL_PKG.GET_FK_IGS_AS_EXM_MTRL_TYPE (
      OLD_references.exam_material_type
      );
...
BEGIN
	...
	ELSIF upper(Column_name) = 'COMMENTS' then
	           new_references.COMMENTS:= column_value;
      END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AS_EXM_MTRL_TYPE
      where EXAM_MATERIAL_TYPE = X_EXAM_MATERIAL_TYPE;
...
begin
 ...
 x_last_update_login=>X_LAST_UPDATE_LOGIN
  ...
  insert into IGS_AS_EXM_MTRL_TYPE (
    EXAM_MATERIAL_TYPE,
    DESCRIPTION,
    CLOSED_IND,
    COMMENTS,
...
begin
  ...
  update IGS_AS_EXM_MTRL_TYPE set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AS_EXM_MTRL_TYPE
     where EXAM_MATERIAL_TYPE = X_EXAM_MATERIAL_TYPE
  ;
begin
  open c1;
...
begin
  ...
  delete from IGS_AS_EXM_MTRL_TYPE
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
