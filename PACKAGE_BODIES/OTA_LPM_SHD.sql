--------------------------------------------------------
--  DDL for Package Body OTA_LPM_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LPM_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_lpm_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'OTA_LEARNING_PATH_MEMBERS_UK1') Then
...
Begin
  --
  If (p_learning_path_member_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_learning_path_member_id
        = ota_lpm_shd.g_old_rec.learning_path_member_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ota_lpm_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ota_lpm_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  l_rec.course_sequence                  := p_course_sequence;
