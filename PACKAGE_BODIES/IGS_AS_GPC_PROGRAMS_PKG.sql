--------------------------------------------------------
--  DDL for Package Body IGS_AS_GPC_PROGRAMS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_GPC_PROGRAMS_PKG" AS
/* $Header: IGSDI53B.pls 115.6 2002/11/28 23:24:01 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_as_gpc_programs%ROWTYPE;
  new_references igs_as_gpc_programs%ROWTYPE;

  PROCEDURE set_column_values (
    p_action                            IN     VARCHAR2,
    x_rowid                             IN     VARCHAR2    DEFAULT NULL,
    x_gpc_program_id                    IN     NUMBER      DEFAULT NULL,
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.gpc_program_id                    := x_gpc_program_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_as_gpc_programs
      WHERE    gpc_program_id = x_gpc_program_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      ...
      WHERE   ((course_cd = x_course_cd) AND
                     (course_version_number = x_version_number));
  ...
  BEGIN
    ...
    IF cur_rowid%FOUND THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AS_GRP_CVR_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
...
BEGIN
 ...
 IF  cur_rowid%FOUND THEN
   ...
   Fnd_Message.Set_Name ('IGS','IGS_AS_GRP_LVAL_FK');
   IGS_GE_MSG_STACK.ADD ;
...
END GET_FK_IGS_LOOKUPS_VIEW  ;
  ...
  */
  ...
  PROCEDURE insert_row (
    ...
    x_gpc_program_id                    IN OUT NOCOPY NUMBER,
    x_grading_period_cd                 IN     VARCHAR2,
    x_course_cd                         IN     VARCHAR2,
    x_course_version_number             IN     NUMBER,
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_as_gpc_programs_s.NEXTVAL
    INTO      x_gpc_program_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_gpc_program_id                    => x_gpc_program_id,
    ...
    INSERT INTO igs_as_gpc_programs (
      gpc_program_id,
      grading_period_cd,
      course_cd,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ...
        AND (tlinfo.course_version_number = x_course_version_number)
       ) THEN
      NULL;
    ELSE
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_as_gpc_programs
      SET
        grading_period_cd                 = new_references.grading_period_cd,
        course_cd                         = new_references.course_cd,
        course_version_number             = new_references.course_version_number,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_as_gpc_programs
    WHERE rowid = x_rowid;
