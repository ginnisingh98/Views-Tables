--------------------------------------------------------
--  DDL for Package Body JTA_CAL_APPOINTMENT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTA_CAL_APPOINTMENT_PVT" AS
/* $Header: jtavcab.pls 120.1 2005/07/02 02:24:26 appldev ship $ */
...
/*
Bug # 4190108, amigupta, Created one overloaded method which takes one parameter free_busy_type
*/
PROCEDURE create_appointment (
 ...
 IS
    ...
    l_api_name	 CONSTANT VARCHAR2(30) := 'CREATE_APPOINTMENT';
    BEGIN
    SAVEPOINT create_appointment;
    ...
    THEN
	ROLLBACK TO create_appointment;
	x_return_status := fnd_api.g_ret_sts_unexp_error;
    WHEN OTHERS
    THEN
	ROLLBACK TO create_appointment;
