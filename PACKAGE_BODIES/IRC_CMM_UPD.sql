--------------------------------------------------------
--  DDL for Package Body IRC_CMM_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_CMM_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  irc_cmm_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the irc_comm_messages Row
  --
  update irc_comm_messages
    set
     communication_message_id        = p_rec.communication_message_id
    ,parent_id                       = p_rec.parent_id
    ,communication_topic_id          = p_rec.communication_topic_id
    ,message_subject                 = p_rec.message_subject
    ,message_body                    = p_rec.message_body
    ,message_post_date               = p_rec.message_post_date
    ,sender_type                     = p_rec.sender_type
    ,sender_id                       = p_rec.sender_id
    ,document_type                   = p_rec.document_type
    ,document_id                     = p_rec.document_id
    ,deleted_flag                    = p_rec.deleted_flag
...
Begin
  ...
  If (p_rec.parent_id = hr_api.g_number) then
    ...
    irc_cmm_shd.g_old_rec.parent_id;
  End If;
  If (p_rec.communication_topic_id = hr_api.g_number) then
    p_rec.communication_topic_id :=
    irc_cmm_shd.g_old_rec.communication_topic_id;
  End If;
  If (p_rec.message_subject = hr_api.g_varchar2) then
    p_rec.message_subject :=
    irc_cmm_shd.g_old_rec.message_subject;
  End If;
  If (p_rec.message_body = hr_api.g_varchar2) then
    ...
    irc_cmm_shd.g_old_rec.message_body;
  End If;
  If (p_rec.message_post_date = hr_api.g_date) then
    p_rec.message_post_date :=
    irc_cmm_shd.g_old_rec.message_post_date;
  End If;
  If (p_rec.sender_type = hr_api.g_varchar2) then
    p_rec.sender_type :=
    irc_cmm_shd.g_old_rec.sender_type;
  End If;
  If (p_rec.sender_id = hr_api.g_number) then
    p_rec.sender_id :=
    irc_cmm_shd.g_old_rec.sender_id;
  End If;
  If (p_rec.document_type = hr_api.g_varchar2) then
    p_rec.document_type :=
    irc_cmm_shd.g_old_rec.document_type;
  End If;
  If (p_rec.document_id = hr_api.g_number) then
    p_rec.document_id :=
    irc_cmm_shd.g_old_rec.document_id;
  End If;
  If (p_rec.deleted_flag = hr_api.g_varchar2) then
    p_rec.deleted_flag :=
    irc_cmm_shd.g_old_rec.deleted_flag;
  End If;
  --
...
Begin
  ...
  --
  irc_cmm_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  irc_cmm_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  irc_cmm_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure upd
  ...
  ,p_communication_topic_id       in     number    default hr_api.g_number
  ,p_message_subject              in     varchar2  default hr_api.g_varchar2
  ,p_message_post_date            in     date      default hr_api.g_date
  ,p_sender_type                  in     varchar2  default hr_api.g_varchar2
  ,p_sender_id                    in     number    default hr_api.g_number
