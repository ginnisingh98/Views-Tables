--------------------------------------------------------
--  DDL for Package Body HXC_ULA_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_ULA_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_ula_ins.';  -- Global package name
...
Begin
  ...
  --
  --
  -- Insert the row into: hxc_layouts
  --
  insert into hxc_layouts
      (layout_id
      ,layout_name
      ,application_id
      ,layout_type
...
--
  l_proc  varchar2(72) ;
--
  Cursor C_Sel1 is select hxc_layouts_s.nextval from sys.dual;
--
Begin
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.layout_id;
  Close C_Sel1;
  --
  if g_debug then
  	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_ula_bus.insert_validate
     (p_rec
     );
  --
  -- Call the supporting pre-insert operation
  --
  hxc_ula_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_ula_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_ula_ins.post_insert
     (p_rec
     );
  --
  if g_debug then
...
Begin
  ...
  --
  -- Having converted the arguments into the hxc_ula_rec
  ...
  --
  -- insert the row into the tl table
  --
  -- call the api to insert the row into hxc_layouts_tl
  --
  hxc_ult_ins.ins_tl
   (p_language_code => USERENV('LANG')
   ,p_layout_id => l_rec.layout_id
