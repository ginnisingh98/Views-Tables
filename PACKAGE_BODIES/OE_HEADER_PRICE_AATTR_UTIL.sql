--------------------------------------------------------
--  DDL for Package Body OE_HEADER_PRICE_AATTR_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_HEADER_PRICE_AATTR_UTIL" AS
/* $Header: OEXUHAAB.pls 120.0 2005/06/01 01:20:19 appldev noship $ */

G_PKG_NAME              CONSTANT VARCHAR2(30) := 'Oe_Header_Price_Aattr_util';
...
BEGIN
	 Query_Rows
      ...
      x_Header_Adj_Att_Rec := l_Header_Adj_Att_Tbl(1);
...
BEGIN
	...
	FOR iml_rec IN l_price_Adj_att_csr LOOP
 	...
 	x_Header_Adj_Att_Tbl(l_count).PRICE_ADJUSTMENT_ID :=	iml_rec.PRICE_ADJUSTMENT_ID;
	x_Header_Adj_Att_Tbl(l_count).PRICING_CONTEXT	:=	iml_rec.PRICING_CONTEXT;
 	x_Header_Adj_Att_Tbl(l_count).PRICING_ATTRIBUTE	:=	iml_rec.PRICING_ATTRIBUTE;
 	x_Header_Adj_Att_Tbl(l_count).CREATION_DATE		:=	iml_rec.CREATION_DATE;
 	x_Header_Adj_Att_Tbl(l_count).CREATED_BY		:=	iml_rec.CREATED_BY;
 	...
 	x_Header_Adj_Att_Tbl(l_count).PROGRAM_APPLICATION_ID	:=iml_rec.PROGRAM_APPLICATION_ID;
 	x_Header_Adj_Att_Tbl(l_count).PROGRAM_ID			:= iml_rec.PROGRAM_ID;
 	x_Header_Adj_Att_Tbl(l_count).PROGRAM_UPDATE_DATE 	:= iml_rec.PROGRAM_UPDATE_DATE;
 	x_Header_Adj_Att_Tbl(l_count).REQUEST_ID			:= iml_rec.REQUEST_ID;
 	x_Header_Adj_Att_Tbl(l_count).PRICING_ATTR_VALUE_FROM 	:= iml_rec.PRICING_ATTR_VALUE_FROM;
	x_Header_Adj_Att_Tbl(l_count).PRICING_ATTR_VALUE_TO	:= iml_rec.PRICING_ATTR_VALUE_TO;
 	x_Header_Adj_Att_Tbl(l_count).COMPARISON_OPERATOR		:= iml_rec.COMPARISON_OPERATOR;
 	x_Header_Adj_Att_Tbl(l_count).FLEX_TITLE			:= iml_rec.FLEX_TITLE;
 	x_Header_Adj_Att_Tbl(l_count).PRICE_ADJ_ATTRIB_ID 	:= iml_rec.PRICE_ADJ_ATTRIB_ID;
 	x_Header_Adj_Att_Tbl(l_count).LOCK_CONTROL 			:= iml_rec.LOCK_CONTROL;

     -- set values for non-DB fields
 	x_Header_Adj_Att_Tbl(l_count).db_flag 		:= FND_API.G_TRUE;
 	x_Header_Adj_Att_Tbl(l_count).operation 	:= FND_API.G_MISS_CHAR;
 	x_Header_Adj_Att_Tbl(l_count).return_status 	:= FND_API.G_MISS_CHAR;
  ...
  IF ( p_price_Adj_attrib_id IS NOT NULL
	  ...
	  AND
	  (x_Header_Adj_att_tbl.COUNT = 0 )
	THEN
		RAISE NO_DATA_FOUND;
  END IF;


--  RETURN l_Header_Adj_att_tbl;
...
BEGIN
	...
	( 	p_Header_Adj_Att_Rec.PRICE_ADJUSTMENT_ID
 	 ...
 	 p_Header_Adj_Att_rec.lock_control := l_lock_control;
...
l_Header_Adj_Att_rec   OE_Order_PUB.Header_Adj_Att_Rec_Type := p_x_Header_Adj_Att_rec;
--
l_debug_level CONSTANT NUMBER := oe_debug_pub.g_debug_level;
--
BEGIN

    IF l_Header_Adj_Att_rec.PRICE_ADJUSTMENT_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PRICE_ADJUSTMENT_ID := p_old_Header_Adj_Att_rec.PRICE_ADJUSTMENT_ID;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_CONTEXT = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_CONTEXT := p_old_Header_Adj_Att_rec.PRICING_CONTEXT;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_ATTRIBUTE = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_ATTRIBUTE := p_old_Header_Adj_Att_rec.PRICING_ATTRIBUTE;
    END IF;

    IF l_Header_Adj_Att_rec.CREATION_DATE = FND_API.G_MISS_DATE THEN
        l_Header_Adj_Att_rec.CREATION_DATE := p_old_Header_Adj_Att_rec.CREATION_DATE;
    END IF;

    IF l_Header_Adj_Att_rec.CREATED_BY = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.CREATED_BY := p_old_Header_Adj_Att_rec.CREATED_BY;
    END IF;
    ...
    IF l_Header_Adj_Att_rec.PROGRAM_APPLICATION_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PROGRAM_APPLICATION_ID := p_old_Header_Adj_Att_rec.PROGRAM_APPLICATION_ID;
    END IF;

    IF l_Header_Adj_Att_rec.PROGRAM_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PROGRAM_ID := p_old_Header_Adj_Att_rec.PROGRAM_ID;
    END IF;
    ...
    IF l_Header_Adj_Att_rec.REQUEST_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.REQUEST_ID := p_old_Header_Adj_Att_rec.REQUEST_ID;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_FROM = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_FROM := p_old_Header_Adj_Att_rec.PRICING_ATTR_VALUE_FROM;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_TO = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_TO := p_old_Header_Adj_Att_rec.PRICING_ATTR_VALUE_TO;
    END IF;

    IF l_Header_Adj_Att_rec.COMPARISON_OPERATOR = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.COMPARISON_OPERATOR := p_old_Header_Adj_Att_rec.COMPARISON_OPERATOR;
    END IF;

    IF l_Header_Adj_Att_rec.FLEX_TITLE = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.FLEX_TITLE := p_old_Header_Adj_Att_rec.FLEX_TITLE;
    END IF;

    IF l_Header_Adj_Att_rec.PRICE_ADJ_ATTRIB_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PRICE_ADJ_ATTRIB_ID := p_old_Header_Adj_Att_rec.PRICE_ADJ_ATTRIB_ID;
    END IF;

    -- RETURN l_Header_Adj_Att_rec;
...
BEGIN
    ...
    IF l_Header_Adj_Att_rec.PRICE_ADJUSTMENT_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PRICE_ADJUSTMENT_ID := Null;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_CONTEXT = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_CONTEXT := Null;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_ATTRIBUTE = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_ATTRIBUTE := Null;
    END IF;

    IF l_Header_Adj_Att_rec.CREATION_DATE = FND_API.G_MISS_DATE THEN
        l_Header_Adj_Att_rec.CREATION_DATE := Null;
    END IF;

    IF l_Header_Adj_Att_rec.CREATED_BY = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.CREATED_BY := Null;
    END IF;
    ...
    IF l_Header_Adj_Att_rec.PROGRAM_APPLICATION_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PROGRAM_APPLICATION_ID := Null;
    END IF;

    IF l_Header_Adj_Att_rec.PROGRAM_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PROGRAM_ID := Null;
    END IF;
    ...
    IF l_Header_Adj_Att_rec.REQUEST_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.REQUEST_ID := Null;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_FROM = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_FROM := Null;
    END IF;

    IF l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_TO = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.PRICING_ATTR_VALUE_TO := Null;
    END IF;

    IF l_Header_Adj_Att_rec.COMPARISON_OPERATOR = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.COMPARISON_OPERATOR := Null;
    END IF;

    IF l_Header_Adj_Att_rec.FLEX_TITLE = FND_API.G_MISS_CHAR THEN
        l_Header_Adj_Att_rec.FLEX_TITLE := Null;
    END IF;

    IF l_Header_Adj_Att_rec.PRICE_ADJ_ATTRIB_ID = FND_API.G_MISS_NUM THEN
        l_Header_Adj_Att_rec.PRICE_ADJ_ATTRIB_ID := Null;
    END IF;
...
BEGIN
   ...
   -- x_Header_Adj_Att_rec := p_Header_Adj_Att_rec;
    p_x_Header_Adj_Att_rec := p_x_Header_Adj_Att_rec;
...
BEGIN
    ...
    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'ENTERING OE_HEADER_PRICE_AATTR_UTIL.LOCK_ROW.' , 1 ) ;
    END IF;
    SAVEPOINT Lock_Row;
    ...
    ELSE
      l_price_adj_attrib_id := p_x_Header_Adj_Att_rec.price_adj_attrib_id;
      l_lock_control := p_x_Header_Adj_Att_rec.lock_control;
    END IF;
    ...
    ,x_Header_Adj_Att_rec	=> p_x_Header_Adj_Att_rec
    );


    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'QUERIED LOCK_CONTROL: '|| P_X_HEADER_ADJ_ATT_REC.LOCK_CONTROL , 1 ) ;
    END IF;
    ...
    THEN
        ...
        p_x_Header_Adj_Att_rec.return_status     := FND_API.G_RET_STS_SUCCESS;
    ...
    IF      OE_GLOBALS.Equal(p_x_Header_Adj_Att_rec.lock_control,
                             l_lock_control)
    THEN
    ...
    ELSE
        ...
        p_x_Header_Adj_Att_rec.return_status       := FND_API.G_RET_STS_ERROR;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        p_x_Header_Adj_Att_rec.return_status := FND_API.G_RET_STS_UNEXP_ERROR;
...
IS
  ...
  CURSOR lock_att_hdr(p_price_adjustment_id IN NUMBER) IS
  SELECT price_adj_attrib_id
  FROM   oe_price_adj_attribs
  WHERE  price_adjustment_id = p_price_adjustment_id
  FOR UPDATE NOWAIT;
...
BEGIN
    ...
    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'ENTERING OE_HEADER_PRICE_AATTR_UTIL.LOCK_ROWS.' , 1 ) ;
    END IF;
  ...
  BEGIN
    IF p_price_adjustment_id <> FND_API.G_MISS_NUM THEN
	 SAVEPOINT LOCK_ROWS;
	 OPEN lock_att_hdr(p_price_adjustment_id);

	 LOOP
	   FETCH lock_att_hdr INTO l_price_adj_attrib_id;
	   EXIT WHEN lock_att_hdr%NOTFOUND;
      END LOOP;
      CLOSE lock_att_hdr;
    END IF;
  EXCEPTION
  ...
  EXCEPTION
    ...
    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'EXITING OE_HEADER_PRICE_AATTR_UTIL.LOCK_ROWS.' , 1 ) ;
    END IF;
