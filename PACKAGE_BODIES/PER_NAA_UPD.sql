--------------------------------------------------------
--  DDL for Package Body PER_NAA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_NAA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_naa_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  -- Update the per_nl_absence_actions Row
  --
  update per_nl_absence_actions
    set
     absence_attendance_id           = p_rec.absence_attendance_id
    ,expected_date                   = p_rec.expected_date
    ,description                     = p_rec.description
    ,actual_start_date               = p_rec.actual_start_date
    ,actual_end_date                 = p_rec.actual_end_date
    ,holder                          = p_rec.holder
    ,comments                        = p_rec.comments
    ,document_file_name              = p_rec.document_file_name
...
Begin
  ...
  If (p_rec.absence_attendance_id = hr_api.g_number) then
    ...
    per_naa_shd.g_old_rec.absence_attendance_id;
  End If;
  If (p_rec.expected_date = hr_api.g_date) then
    p_rec.expected_date :=
    per_naa_shd.g_old_rec.expected_date;
  End If;
  ...
  If (p_rec.actual_end_date = hr_api.g_date) then
    ...
    per_naa_shd.g_old_rec.actual_end_date;
  End If;
  If (p_rec.holder = hr_api.g_varchar2) then
    p_rec.holder :=
    per_naa_shd.g_old_rec.holder;
  End If;
  If (p_rec.comments = hr_api.g_varchar2) then
    ...
    per_naa_shd.g_old_rec.comments;
  End If;
  If (p_rec.document_file_name = hr_api.g_varchar2) then
    p_rec.document_file_name :=
    per_naa_shd.g_old_rec.document_file_name;
  End If;
  If (p_rec.last_updated_by = hr_api.g_number) then
    p_rec.last_updated_by :=
    per_naa_shd.g_old_rec.last_updated_by;
  End If;
...
Begin
  ...
  --
  per_naa_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_naa_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_naa_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
