--------------------------------------------------------
--  DDL for Package Body BEN_EDC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EDC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_edc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_dsbld_stat_cvg_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_dsbld_stat_cvg_id PK of record being inserted or updated.
...
--
Procedure chk_elig_dsbld_stat_cvg_id(p_elig_dsbld_stat_cvg_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_dsbld_stat_cvg_id';
...
Begin
  ...
  --
  l_api_updating := ben_edc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_dsbld_stat_cvg_id                => p_elig_dsbld_stat_cvg_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_dsbld_stat_cvg_id,hr_api.g_number)
     <>  ben_edc_shd.g_old_rec.elig_dsbld_stat_cvg_id) then
    --
    -- raise error as PK has changed
    --
    ben_edc_shd.constraint_error('BEN_DSBL_STAT_CVG_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_dsbld_stat_cvg_id is not null then
      --
  ...
  --
End chk_elig_dsbld_stat_cvg_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_dsbld_cd >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_dsbld_cd(p_elig_dsbld_stat_cvg_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_dsbld_cd
      <> nvl(ben_edc_shd.g_old_rec.dsbld_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_cvg_thru_rl(p_elig_dsbld_stat_cvg_id                in number,
...
--   Internal table handler use only.
--
Procedure chk_cvg_thru_cd(p_elig_dsbld_stat_cvg_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_cvg_thru_cd
      <> nvl(ben_edc_shd.g_old_rec.cvg_thru_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_cvg_strt_rl(p_elig_dsbld_stat_cvg_id                in number,
...
--   Internal table handler use only.
--
Procedure chk_cvg_strt_cd(p_elig_dsbld_stat_cvg_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_cvg_strt_cd
      <> nvl(ben_edc_shd.g_old_rec.cvg_strt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_cvg_strt_dt_dependency(p_elig_dsbld_stat_cvg_id  	in number,
                            	p_cvg_strt_cd           	in varchar2,
 				p_cvg_strt_rl 			in number,
                            	p_effective_date          	in date,
                           	p_object_version_number    	in number) is
...
Begin
  ...
  if (l_api_updating
      	and (nvl(p_cvg_strt_cd, hr_api.g_varchar2) <>
		      	nvl(ben_edc_shd.g_old_rec.cvg_strt_cd,hr_api.g_varchar2) or
 	     nvl(p_cvg_strt_rl, hr_api.g_number) <>
			nvl(ben_edc_shd.g_old_rec.cvg_strt_rl,hr_api.g_number))
      	or not l_api_updating) then
...
--   Internal table handler use only.
--
Procedure chk_cvg_thru_dt_dependency(p_elig_dsbld_stat_cvg_id  	in number,
                            	   p_cvg_thru_cd           	in varchar2,
				   p_cvg_thru_rl                in number,
                            	   p_effective_date          	in date,
                           	   p_object_version_number    	in number) is
...
Begin
  ...
  if (l_api_updating
      	and (nvl(p_cvg_thru_cd, hr_api.g_varchar2) <>
		      	nvl(ben_edc_shd.g_old_rec.cvg_thru_cd,hr_api.g_varchar2) or
 	     nvl(p_cvg_thru_rl, hr_api.g_number) <>
			nvl(ben_edc_shd.g_old_rec.cvg_thru_rl,hr_api.g_number))
      	or not l_api_updating) then
...
--   p_elig_dsbld_stat_cvg_id - primary key of the table
--   p_dsbld_cd - duplicate value to be checked
...
--   Internal table handler use only.
--
Procedure chk_duplicate_rows(p_elig_dsbld_stat_cvg_id in number,
                             p_dsbld_cd in varchar2,
  ...
  dummy varchar2(1);
  cursor c1 is select null from ben_elig_dsbld_stat_cvg_f
     where (elig_dsbld_stat_cvg_id <> p_elig_dsbld_stat_cvg_id or p_elig_dsbld_stat_cvg_id is null) and
           dsbld_cd  = p_dsbld_cd and
...
function return_legislation_code
  (p_elig_dsbld_stat_cvg_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_dsbld_stat_cvg_f b
    where b.elig_dsbld_stat_cvg_id      = p_elig_dsbld_stat_cvg_id
