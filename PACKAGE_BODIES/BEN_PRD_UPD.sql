--------------------------------------------------------
--  DDL for Package Body BEN_PRD_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PRD_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_prd_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_prd_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.use_parnt_ded_sched_flag = hr_api.g_varchar2) then
    p_rec.use_parnt_ded_sched_flag :=
    ben_prd_shd.g_old_rec.use_parnt_ded_sched_flag;
  End If;
  If (p_rec.asn_on_chc_of_parnt_flag = hr_api.g_varchar2) then
    p_rec.asn_on_chc_of_parnt_flag :=
    ben_prd_shd.g_old_rec.asn_on_chc_of_parnt_flag;
  End If;
  If (p_rec.use_parnt_prtl_mo_cd_flag = hr_api.g_varchar2) then
    p_rec.use_parnt_prtl_mo_cd_flag :=
    ben_prd_shd.g_old_rec.use_parnt_prtl_mo_cd_flag;
  End If;
  If (p_rec.alloc_sme_as_parnt_flag = hr_api.g_varchar2) then
    p_rec.alloc_sme_as_parnt_flag :=
    ben_prd_shd.g_old_rec.alloc_sme_as_parnt_flag;
  End If;
  If (p_rec.use_parnt_pymt_sched_flag = hr_api.g_varchar2) then
    p_rec.use_parnt_pymt_sched_flag :=
    ben_prd_shd.g_old_rec.use_parnt_pymt_sched_flag;
  End If;
  If (p_rec.no_cmbnd_mx_amt_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_cmbnd_mx_amt_dfnd_flag :=
    ben_prd_shd.g_old_rec.no_cmbnd_mx_amt_dfnd_flag;
  End If;
  If (p_rec.cmbnd_mx_amt = hr_api.g_number) then
    p_rec.cmbnd_mx_amt :=
    ben_prd_shd.g_old_rec.cmbnd_mx_amt;
  End If;
  If (p_rec.cmbnd_mn_amt = hr_api.g_number) then
    p_rec.cmbnd_mn_amt :=
    ben_prd_shd.g_old_rec.cmbnd_mn_amt;
  End If;
  If (p_rec.cmbnd_mx_pct_num = hr_api.g_number) then
    p_rec.cmbnd_mx_pct_num :=
    ben_prd_shd.g_old_rec.cmbnd_mx_pct_num;
  End If;
  If (p_rec.cmbnd_mn_pct_num = hr_api.g_number) then
    p_rec.cmbnd_mn_pct_num :=
    ben_prd_shd.g_old_rec.cmbnd_mn_pct_num;
  End If;
  If (p_rec.no_cmbnd_mn_amt_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_cmbnd_mn_amt_dfnd_flag :=
    ben_prd_shd.g_old_rec.no_cmbnd_mn_amt_dfnd_flag;
  End If;
  If (p_rec.no_cmbnd_mn_pct_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_cmbnd_mn_pct_dfnd_flag :=
    ben_prd_shd.g_old_rec.no_cmbnd_mn_pct_dfnd_flag;
  End If;
  If (p_rec.no_cmbnd_mx_pct_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_cmbnd_mx_pct_dfnd_flag :=
    ben_prd_shd.g_old_rec.no_cmbnd_mx_pct_dfnd_flag;
  End If;
  If (p_rec.parnt_acty_base_rt_id = hr_api.g_number) then
    p_rec.parnt_acty_base_rt_id :=
    ben_prd_shd.g_old_rec.parnt_acty_base_rt_id;
  End If;
  If (p_rec.chld_acty_base_rt_id = hr_api.g_number) then
    p_rec.chld_acty_base_rt_id :=
    ben_prd_shd.g_old_rec.chld_acty_base_rt_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_prd_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.prd_attribute_category = hr_api.g_varchar2) then
    p_rec.prd_attribute_category :=
    ben_prd_shd.g_old_rec.prd_attribute_category;
  End If;
  If (p_rec.prd_attribute1 = hr_api.g_varchar2) then
    p_rec.prd_attribute1 :=
    ben_prd_shd.g_old_rec.prd_attribute1;
  End If;
  If (p_rec.prd_attribute2 = hr_api.g_varchar2) then
    p_rec.prd_attribute2 :=
    ben_prd_shd.g_old_rec.prd_attribute2;
  End If;
  If (p_rec.prd_attribute3 = hr_api.g_varchar2) then
    p_rec.prd_attribute3 :=
    ben_prd_shd.g_old_rec.prd_attribute3;
  End If;
  If (p_rec.prd_attribute4 = hr_api.g_varchar2) then
    p_rec.prd_attribute4 :=
    ben_prd_shd.g_old_rec.prd_attribute4;
  End If;
  If (p_rec.prd_attribute5 = hr_api.g_varchar2) then
    p_rec.prd_attribute5 :=
    ben_prd_shd.g_old_rec.prd_attribute5;
  End If;
  If (p_rec.prd_attribute6 = hr_api.g_varchar2) then
    p_rec.prd_attribute6 :=
    ben_prd_shd.g_old_rec.prd_attribute6;
  End If;
  If (p_rec.prd_attribute7 = hr_api.g_varchar2) then
    p_rec.prd_attribute7 :=
    ben_prd_shd.g_old_rec.prd_attribute7;
  End If;
  If (p_rec.prd_attribute8 = hr_api.g_varchar2) then
    p_rec.prd_attribute8 :=
    ben_prd_shd.g_old_rec.prd_attribute8;
  End If;
  If (p_rec.prd_attribute9 = hr_api.g_varchar2) then
    p_rec.prd_attribute9 :=
    ben_prd_shd.g_old_rec.prd_attribute9;
  End If;
  If (p_rec.prd_attribute10 = hr_api.g_varchar2) then
    p_rec.prd_attribute10 :=
    ben_prd_shd.g_old_rec.prd_attribute10;
  End If;
  If (p_rec.prd_attribute11 = hr_api.g_varchar2) then
    p_rec.prd_attribute11 :=
    ben_prd_shd.g_old_rec.prd_attribute11;
  End If;
  If (p_rec.prd_attribute12 = hr_api.g_varchar2) then
    p_rec.prd_attribute12 :=
    ben_prd_shd.g_old_rec.prd_attribute12;
  End If;
  If (p_rec.prd_attribute13 = hr_api.g_varchar2) then
    p_rec.prd_attribute13 :=
    ben_prd_shd.g_old_rec.prd_attribute13;
  End If;
  If (p_rec.prd_attribute14 = hr_api.g_varchar2) then
    p_rec.prd_attribute14 :=
    ben_prd_shd.g_old_rec.prd_attribute14;
  End If;
  If (p_rec.prd_attribute15 = hr_api.g_varchar2) then
    p_rec.prd_attribute15 :=
    ben_prd_shd.g_old_rec.prd_attribute15;
  End If;
  If (p_rec.prd_attribute16 = hr_api.g_varchar2) then
    p_rec.prd_attribute16 :=
    ben_prd_shd.g_old_rec.prd_attribute16;
  End If;
  If (p_rec.prd_attribute17 = hr_api.g_varchar2) then
    p_rec.prd_attribute17 :=
    ben_prd_shd.g_old_rec.prd_attribute17;
  End If;
  If (p_rec.prd_attribute18 = hr_api.g_varchar2) then
    p_rec.prd_attribute18 :=
    ben_prd_shd.g_old_rec.prd_attribute18;
  End If;
  If (p_rec.prd_attribute19 = hr_api.g_varchar2) then
    p_rec.prd_attribute19 :=
    ben_prd_shd.g_old_rec.prd_attribute19;
  End If;
  If (p_rec.prd_attribute20 = hr_api.g_varchar2) then
    p_rec.prd_attribute20 :=
    ben_prd_shd.g_old_rec.prd_attribute20;
  End If;
  If (p_rec.prd_attribute21 = hr_api.g_varchar2) then
    p_rec.prd_attribute21 :=
    ben_prd_shd.g_old_rec.prd_attribute21;
  End If;
  If (p_rec.prd_attribute22 = hr_api.g_varchar2) then
    p_rec.prd_attribute22 :=
    ben_prd_shd.g_old_rec.prd_attribute22;
  End If;
  If (p_rec.prd_attribute23 = hr_api.g_varchar2) then
    p_rec.prd_attribute23 :=
    ben_prd_shd.g_old_rec.prd_attribute23;
  End If;
  If (p_rec.prd_attribute24 = hr_api.g_varchar2) then
    p_rec.prd_attribute24 :=
    ben_prd_shd.g_old_rec.prd_attribute24;
  End If;
  If (p_rec.prd_attribute25 = hr_api.g_varchar2) then
    p_rec.prd_attribute25 :=
    ben_prd_shd.g_old_rec.prd_attribute25;
  End If;
  If (p_rec.prd_attribute26 = hr_api.g_varchar2) then
    p_rec.prd_attribute26 :=
    ben_prd_shd.g_old_rec.prd_attribute26;
  End If;
  If (p_rec.prd_attribute27 = hr_api.g_varchar2) then
    p_rec.prd_attribute27 :=
    ben_prd_shd.g_old_rec.prd_attribute27;
  End If;
  If (p_rec.prd_attribute28 = hr_api.g_varchar2) then
    p_rec.prd_attribute28 :=
    ben_prd_shd.g_old_rec.prd_attribute28;
  End If;
  If (p_rec.prd_attribute29 = hr_api.g_varchar2) then
    p_rec.prd_attribute29 :=
    ben_prd_shd.g_old_rec.prd_attribute29;
  End If;
  If (p_rec.prd_attribute30 = hr_api.g_varchar2) then
    p_rec.prd_attribute30 :=
    ben_prd_shd.g_old_rec.prd_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_prd_shd.convert_args
  (
  p_paird_rt_id,
  null,
  null,
  p_use_parnt_ded_sched_flag,
  p_asn_on_chc_of_parnt_flag,
  p_use_parnt_prtl_mo_cd_flag,
  p_alloc_sme_as_parnt_flag,
  p_use_parnt_pymt_sched_flag,
  p_no_cmbnd_mx_amt_dfnd_flag,
  p_cmbnd_mx_amt,
  p_cmbnd_mn_amt,
  p_cmbnd_mx_pct_num,
  p_cmbnd_mn_pct_num,
  p_no_cmbnd_mn_amt_dfnd_flag,
  p_no_cmbnd_mn_pct_dfnd_flag,
  p_no_cmbnd_mx_pct_dfnd_flag,
  p_parnt_acty_base_rt_id,
  p_chld_acty_base_rt_id,
  p_business_group_id,
  p_prd_attribute_category,
  p_prd_attribute1,
  p_prd_attribute2,
  p_prd_attribute3,
  p_prd_attribute4,
