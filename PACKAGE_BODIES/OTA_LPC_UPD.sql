--------------------------------------------------------
--  DDL for Package Body OTA_LPC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LPC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_lpc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the ota_lp_sections Row
  --
  update ota_lp_sections
    set
     learning_path_section_id        = p_rec.learning_path_section_id
    ,learning_path_id                = p_rec.learning_path_id
    ,section_sequence                = p_rec.section_sequence
    ,completion_type_code            = p_rec.completion_type_code
...
Begin
  ...
  If (p_rec.learning_path_id = hr_api.g_number) then
    ...
    ota_lpc_shd.g_old_rec.learning_path_id;
  End If;
  If (p_rec.section_sequence = hr_api.g_number) then
    p_rec.section_sequence :=
    ota_lpc_shd.g_old_rec.section_sequence;
  End If;
  If (p_rec.completion_type_code = hr_api.g_varchar2) then
    p_rec.completion_type_code :=
    ota_lpc_shd.g_old_rec.completion_type_code;
  End If;
...
Begin
  ...
  --
  ota_lpc_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ota_lpc_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ota_lpc_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
