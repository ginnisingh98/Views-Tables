--------------------------------------------------------
--  DDL for Package Body IGS_PE_ATHLETIC_PRG_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PE_ATHLETIC_PRG_PKG" AS
/* $Header: IGSNI86B.pls 120.1 2005/06/28 05:11:24 appldev ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_pe_athletic_prg%ROWTYPE;
  new_references igs_pe_athletic_prg%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.athletic_prg_id                   := x_athletic_prg_id;
    new_references.person_id                         := x_person_id;
    new_references.athletic_prg_code                 := x_athletic_prg_code;
    new_references.rating                            := x_rating;
    new_references.start_date                        := x_start_date;
    new_references.end_date                          := x_end_date;
    new_references.recruited_ind                     := x_recruited_ind;
    new_references.participating_ind                 := x_participating_ind;
  ...
  */
  BEGIN
    ...
    IF (((old_references.athletic_prg_code = new_references.athletic_prg_code)) OR
        ((new_references.athletic_prg_code IS NULL))) THEN
      NULL;

    --kumma, 2608360 replaced igs_ad_code_classes_pkg with igs_lookups_view_pkg
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PE_PAP_ADCC_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PE_PAP_HZ_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ...
        AND (tlinfo.athletic_prg_code = x_athletic_prg_code)
        AND ((tlinfo.rating = x_rating) OR ((tlinfo.rating IS NULL) AND (X_rating IS NULL)))
        AND (tlinfo.start_date = x_start_date)
        AND ((tlinfo.end_date = x_end_date) OR ((tlinfo.end_date IS NULL) AND (X_end_date IS NULL)))
        AND (tlinfo.recruited_ind = x_recruited_ind)
        AND (tlinfo.participating_ind = x_participating_ind)
