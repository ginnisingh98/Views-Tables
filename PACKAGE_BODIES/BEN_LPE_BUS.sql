--------------------------------------------------------
--  DDL for Package Body BEN_LPE_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LPE_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lpe_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ler_chg_pl_nip_enrt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   ler_chg_pl_nip_enrt_id PK of record being inserted or updated.
...
--
Procedure chk_ler_chg_pl_nip_enrt_id
                    (p_ler_chg_pl_nip_enrt_id      in number,
                     p_effective_date              in date,
                     p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ler_chg_pl_nip_enrt_id';
...
Begin
  ...
  --
  l_api_updating := ben_lpe_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ler_chg_pl_nip_enrt_id                => p_ler_chg_pl_nip_enrt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ler_chg_pl_nip_enrt_id,hr_api.g_number)
     <>  ben_lpe_shd.g_old_rec.ler_chg_pl_nip_enrt_id) then
    --
    -- raise error as PK has changed
    --
    ben_lpe_shd.constraint_error('BEN_LER_CHG_PL_NIP_ENRT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ler_chg_pl_nip_enrt_id is not null then
      --
  ...
  --
End chk_ler_chg_pl_nip_enrt_id;
--
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_auto_enrt_mthd_rl(p_ler_chg_pl_nip_enrt_id             in number,
...
--   Internal table handler use only.
--
Procedure chk_enrt_mthd_cd(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_enrt_mthd_cd
      <> nvl(ben_lpe_shd.g_old_rec.enrt_mthd_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_stl_elig_cant_chg_flag(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_lpe_shd.g_old_rec.stl_elig_cant_chg_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_enrt_cd(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_enrt_cd
      <> nvl(ben_lpe_shd.g_old_rec.enrt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_enrt_rl(p_ler_chg_pl_nip_enrt_id             in number,
                             p_enrt_rl                     in number,
                             p_effective_date              in date,
...
--   Internal table handler use only.
--
Procedure chk_dflt_flag(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_dflt_flag
      <> nvl(ben_lpe_shd.g_old_rec.dflt_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_dflt_enrt_rl(p_ler_chg_pl_nip_enrt_id        in number,
...
--   Internal table handler use only.
--
Procedure chk_dflt_enrt_cd(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_dflt_enrt_cd
      <> nvl(ben_lpe_shd.g_old_rec.dflt_enrt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_crnt_enrt_prclds_chg_flag(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_lpe_shd.g_old_rec.crnt_enrt_prclds_chg_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_tco_chg_enrt_cd(p_ler_chg_pl_nip_enrt_id                in number,
...
Begin
  ...
  --
  if (l_api_updating
      and p_tco_chg_enrt_cd is not null
      and p_tco_chg_enrt_cd
      <> nvl(ben_lpe_shd.g_old_rec.tco_chg_enrt_cd,hr_api.g_varchar2)
      or not l_api_updating)and p_tco_chg_enrt_cd is not null  then
    --
    -- check if value of lookup falls within lookup type.
    --
    --
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    hr_api.mandatory_arg_error
      (p_api_name       => l_proc,
       p_argument       => 'ler_chg_pl_nip_enrt_id',
       p_argument_value => p_ler_chg_pl_nip_enrt_id);
    --
    If (dt_api.rows_exist
          (p_base_table_name => 'ben_ler_chg_pl_nip_rl_f',
           p_base_key_column => 'ler_chg_pl_nip_enrt_id',
           p_base_key_value  => p_ler_chg_pl_nip_enrt_id,
           p_from_date       => p_validation_start_date,
           p_to_date         => p_validation_end_date)) Then
      l_table_name := 'ben_ler_chg_pl_nip_rl_f';
      Raise l_rows_exist;
    End If;
    --
  End If;
...
function return_legislation_code
  (p_ler_chg_pl_nip_enrt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_ler_chg_pl_nip_enrt_f b
    where b.ler_chg_pl_nip_enrt_id      = p_ler_chg_pl_nip_enrt_id
