--------------------------------------------------------
--  DDL for Package Body IGS_AD_DEPLVL_PRG_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_DEPLVL_PRG_PKG" AS
/* $Header: IGSAIG6B.pls 115.5 2003/10/30 13:17:54 akadam noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ad_deplvl_prg%ROWTYPE;
  new_references igs_ad_deplvl_prg%ROWTYPE;
  ...
  */
  BEGIN
    ...
    IF (((old_references.program_code = new_references.program_code) AND
         (old_references.version_number = new_references.version_number)) OR
        ((new_references.program_code IS NULL) OR
  ...
  */
    CURSOR cur_rowid IS
      ...
      AND      program_code = x_program_code
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AD_DEPLVL_PRG_FK1');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ad_deplvl_prg
      WHERE   ((program_code = x_course_cd) AND
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AD_DLPG_PG_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ((tlinfo.closed_ind = x_closed_ind) OR ((tlinfo.closed_ind IS NULL) AND (X_closed_ind IS NULL)))
       ) THEN
      NULL;
    ELSE
