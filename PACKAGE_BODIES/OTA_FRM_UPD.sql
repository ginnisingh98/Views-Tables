--------------------------------------------------------
--  DDL for Package Body OTA_FRM_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_FRM_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_frm_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy ota_frm_shd.g_rec_type
  ) is
--
...
Begin
  ...
  update ota_forums_b
    ...
    ,message_type_flag               = p_rec.message_type_flag
    ,allow_html_flag                 = p_rec.allow_html_flag
...
Begin
  ...
  begin
    ...
    ota_frm_rku.after_update
      ...
      ,p_message_type_flag
      => p_rec.message_type_flag
      ,p_allow_html_flag
      => p_rec.allow_html_flag
      ...
      => ota_frm_shd.g_old_rec.business_group_id
      ,p_start_date_active_o
      => ota_frm_shd.g_old_rec.start_date_active
      ,p_end_date_active_o
      => ota_frm_shd.g_old_rec.end_date_active
      ,p_message_type_flag_o
      => ota_frm_shd.g_old_rec.message_type_flag
      ,p_allow_html_flag_o
      => ota_frm_shd.g_old_rec.allow_html_flag
      ,p_allow_attachment_flag_o
      => ota_frm_shd.g_old_rec.allow_attachment_flag
      ,p_auto_notification_flag_o
      => ota_frm_shd.g_old_rec.auto_notification_flag
      ,p_public_flag_o
      => ota_frm_shd.g_old_rec.public_flag
      ,p_object_version_number_o
      => ota_frm_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.end_date_active = hr_api.g_date) then
    ...
    ota_frm_shd.g_old_rec.end_date_active;
  End If;
  If (p_rec.message_type_flag = hr_api.g_varchar2) then
    p_rec.message_type_flag :=
    ota_frm_shd.g_old_rec.message_type_flag;
  End If;
  If (p_rec.allow_html_flag = hr_api.g_varchar2) then
    p_rec.allow_html_flag :=
    ota_frm_shd.g_old_rec.allow_html_flag;
  End If;
...
Begin
  ...
  --
  ota_frm_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ota_frm_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ota_frm_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
