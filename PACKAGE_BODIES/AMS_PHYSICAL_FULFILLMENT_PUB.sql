--------------------------------------------------------
--  DDL for Package Body AMS_PHYSICAL_FULFILLMENT_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMS_PHYSICAL_FULFILLMENT_PUB" AS
 /* $Header: amsvfulb.pls 120.2 2006/05/07 23:02:03 anchaudh ship $ */
...
BEGIN
 ...
 -- Process Order Header
    ...
    --anchaudh added the following line for bug fix #5137205.
    l_qte_header_rec.order_type_id := ASO_UTILITY_PVT.GET_OU_ATTRIBUTE_VALUE(ASO_UTILITY_PVT.G_DEFAULT_ORDER_TYPE,p_order_header_rec.organization_id) ;
...
--
-- SHIP_FROM_ORG_ID added by ABHOLA
--
    l_hd_shipment_tbl(1).SHIP_FROM_ORG_ID := p_order_header_rec.SHIP_FROM_ORG_ID;
...
--    SHIP_FROM_ORG_ID added by ABHOLA
--
      l_ln_shipment_tbl(shipment_index).SHIP_FROM_ORG_ID := p_order_line_tbl(i).SHIP_FROM_ORG_ID;
  ...
  END IF;
 END LOOP;
      --by: musman: MOAC uptake bug:4421261 Fix
      MO_GLOBAL.SET_POLICY_CONTEXT ('S',p_order_header_rec.organization_id);
 ...
 WHEN OTHERS THEN
     ...
     THEN
        Fnd_Msg_Pub.Add_Exc_Msg( 'AMS_PHYSICAL_FULFILLMENT_PUB','fulfill_collateral_request');
     END IF;
 ...
 END fulfill_collateral_request;
