--------------------------------------------------------
--  DDL for Package Body BEN_XCR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_XCR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_xcr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  -- Update the ben_ext_crit_prfl Row
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_xcr_shd.g_rec_type) is
--
...
Begin
  ...
  End If;
  If (p_rec.xcr_attribute_category = hr_api.g_varchar2) then
    p_rec.xcr_attribute_category :=
    ben_xcr_shd.g_old_rec.xcr_attribute_category;
  End If;
  If (p_rec.xcr_attribute1 = hr_api.g_varchar2) then
    p_rec.xcr_attribute1 :=
    ben_xcr_shd.g_old_rec.xcr_attribute1;
  End If;
  If (p_rec.xcr_attribute2 = hr_api.g_varchar2) then
    p_rec.xcr_attribute2 :=
    ben_xcr_shd.g_old_rec.xcr_attribute2;
  End If;
  If (p_rec.xcr_attribute3 = hr_api.g_varchar2) then
    p_rec.xcr_attribute3 :=
    ben_xcr_shd.g_old_rec.xcr_attribute3;
  End If;
  If (p_rec.xcr_attribute4 = hr_api.g_varchar2) then
    p_rec.xcr_attribute4 :=
    ben_xcr_shd.g_old_rec.xcr_attribute4;
  End If;
  If (p_rec.xcr_attribute5 = hr_api.g_varchar2) then
    p_rec.xcr_attribute5 :=
    ben_xcr_shd.g_old_rec.xcr_attribute5;
  End If;
  If (p_rec.xcr_attribute6 = hr_api.g_varchar2) then
    p_rec.xcr_attribute6 :=
    ben_xcr_shd.g_old_rec.xcr_attribute6;
  End If;
  If (p_rec.xcr_attribute7 = hr_api.g_varchar2) then
    p_rec.xcr_attribute7 :=
    ben_xcr_shd.g_old_rec.xcr_attribute7;
  End If;
  If (p_rec.xcr_attribute8 = hr_api.g_varchar2) then
    p_rec.xcr_attribute8 :=
    ben_xcr_shd.g_old_rec.xcr_attribute8;
  End If;
  If (p_rec.xcr_attribute9 = hr_api.g_varchar2) then
    p_rec.xcr_attribute9 :=
    ben_xcr_shd.g_old_rec.xcr_attribute9;
  End If;
  If (p_rec.xcr_attribute10 = hr_api.g_varchar2) then
    p_rec.xcr_attribute10 :=
    ben_xcr_shd.g_old_rec.xcr_attribute10;
  End If;
  If (p_rec.xcr_attribute11 = hr_api.g_varchar2) then
    p_rec.xcr_attribute11 :=
    ben_xcr_shd.g_old_rec.xcr_attribute11;
  End If;
  If (p_rec.xcr_attribute12 = hr_api.g_varchar2) then
    p_rec.xcr_attribute12 :=
    ben_xcr_shd.g_old_rec.xcr_attribute12;
  End If;
  If (p_rec.xcr_attribute13 = hr_api.g_varchar2) then
    p_rec.xcr_attribute13 :=
    ben_xcr_shd.g_old_rec.xcr_attribute13;
  End If;
  If (p_rec.xcr_attribute14 = hr_api.g_varchar2) then
    p_rec.xcr_attribute14 :=
    ben_xcr_shd.g_old_rec.xcr_attribute14;
  End If;
  If (p_rec.xcr_attribute15 = hr_api.g_varchar2) then
    p_rec.xcr_attribute15 :=
    ben_xcr_shd.g_old_rec.xcr_attribute15;
  End If;
  If (p_rec.xcr_attribute16 = hr_api.g_varchar2) then
    p_rec.xcr_attribute16 :=
    ben_xcr_shd.g_old_rec.xcr_attribute16;
  End If;
  If (p_rec.xcr_attribute17 = hr_api.g_varchar2) then
    p_rec.xcr_attribute17 :=
    ben_xcr_shd.g_old_rec.xcr_attribute17;
  End If;
  If (p_rec.xcr_attribute18 = hr_api.g_varchar2) then
    p_rec.xcr_attribute18 :=
    ben_xcr_shd.g_old_rec.xcr_attribute18;
  End If;
  If (p_rec.xcr_attribute19 = hr_api.g_varchar2) then
    p_rec.xcr_attribute19 :=
    ben_xcr_shd.g_old_rec.xcr_attribute19;
  End If;
  If (p_rec.xcr_attribute20 = hr_api.g_varchar2) then
    p_rec.xcr_attribute20 :=
    ben_xcr_shd.g_old_rec.xcr_attribute20;
  End If;
  If (p_rec.xcr_attribute21 = hr_api.g_varchar2) then
    p_rec.xcr_attribute21 :=
    ben_xcr_shd.g_old_rec.xcr_attribute21;
  End If;
  If (p_rec.xcr_attribute22 = hr_api.g_varchar2) then
    p_rec.xcr_attribute22 :=
    ben_xcr_shd.g_old_rec.xcr_attribute22;
  End If;
  If (p_rec.xcr_attribute23 = hr_api.g_varchar2) then
    p_rec.xcr_attribute23 :=
    ben_xcr_shd.g_old_rec.xcr_attribute23;
  End If;
  If (p_rec.xcr_attribute24 = hr_api.g_varchar2) then
    p_rec.xcr_attribute24 :=
    ben_xcr_shd.g_old_rec.xcr_attribute24;
  End If;
  If (p_rec.xcr_attribute25 = hr_api.g_varchar2) then
    p_rec.xcr_attribute25 :=
    ben_xcr_shd.g_old_rec.xcr_attribute25;
  End If;
  If (p_rec.xcr_attribute26 = hr_api.g_varchar2) then
    p_rec.xcr_attribute26 :=
    ben_xcr_shd.g_old_rec.xcr_attribute26;
  End If;
  If (p_rec.xcr_attribute27 = hr_api.g_varchar2) then
    p_rec.xcr_attribute27 :=
    ben_xcr_shd.g_old_rec.xcr_attribute27;
  End If;
  If (p_rec.xcr_attribute28 = hr_api.g_varchar2) then
    p_rec.xcr_attribute28 :=
    ben_xcr_shd.g_old_rec.xcr_attribute28;
  End If;
  If (p_rec.xcr_attribute29 = hr_api.g_varchar2) then
    p_rec.xcr_attribute29 :=
    ben_xcr_shd.g_old_rec.xcr_attribute29;
  End If;
  If (p_rec.xcr_attribute30 = hr_api.g_varchar2) then
    p_rec.xcr_attribute30 :=
    ben_xcr_shd.g_old_rec.xcr_attribute30;
  End If;
