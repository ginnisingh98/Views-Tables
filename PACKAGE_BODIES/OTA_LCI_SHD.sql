--------------------------------------------------------
--  DDL for Package Body OTA_LCI_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LCI_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_lci_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'OTA_LP_CAT_INCLUSIONS_UK1') Then
...
Begin
  ...
  If (p_learning_path_id is null and
      p_category_usage_id is null and
  ...
  Else
    If (p_learning_path_id
        = ota_lci_shd.g_old_rec.learning_path_id and
        p_category_usage_id
        = ota_lci_shd.g_old_rec.category_usage_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ota_lci_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ota_lci_shd.g_old_rec.object_version_number) Then
