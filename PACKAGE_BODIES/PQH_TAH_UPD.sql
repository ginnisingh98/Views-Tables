--------------------------------------------------------
--  DDL for Package Body PQH_TAH_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_TAH_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_tah_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the pqh_ss_approval_history Row
  --
...
Begin
  ...
  If (p_rec.transaction_effective_date = hr_api.g_date) then
    ...
    pqh_tah_shd.g_old_rec.transaction_effective_date;
  End If;
  If (p_rec.action = hr_api.g_varchar2) then
    p_rec.action :=
    pqh_tah_shd.g_old_rec.action;
  End If;
  If (p_rec.user_name = hr_api.g_varchar2) then
    p_rec.user_name :=
    pqh_tah_shd.g_old_rec.user_name;
  End If;
  If (p_rec.transaction_item_type = hr_api.g_varchar2) then
    p_rec.transaction_item_type :=
    pqh_tah_shd.g_old_rec.transaction_item_type;
  End If;
  If (p_rec.transaction_item_key = hr_api.g_varchar2) then
    p_rec.transaction_item_key :=
    pqh_tah_shd.g_old_rec.transaction_item_key;
  End If;
  If (p_rec.effective_date_option = hr_api.g_varchar2) then
    p_rec.effective_date_option :=
    pqh_tah_shd.g_old_rec.effective_date_option;
  End If;
  If (p_rec.orig_system = hr_api.g_varchar2) then
    p_rec.orig_system :=
    pqh_tah_shd.g_old_rec.orig_system;
  End If;
  If (p_rec.orig_system_id = hr_api.g_number) then
    p_rec.orig_system_id :=
    pqh_tah_shd.g_old_rec.orig_system_id;
  End If;
  If (p_rec.notification_id = hr_api.g_number) then
    p_rec.notification_id :=
    pqh_tah_shd.g_old_rec.notification_id;
  End If;
  If (p_rec.user_comment = hr_api.g_varchar2) then
    p_rec.user_comment :=
    pqh_tah_shd.g_old_rec.user_comment;
  End If;
  --
End convert_defs;
--
...
Procedure upd
  ...
  ,p_transaction_item_type        in     varchar2  default hr_api.g_varchar2
  ,p_transaction_item_key         in     varchar2  default hr_api.g_varchar2
  ,p_effective_date_option        in     varchar2  default hr_api.g_varchar2
  ,p_orig_system                  in     varchar2  default hr_api.g_varchar2
  ,p_orig_system_id               in     number    default hr_api.g_number
