--------------------------------------------------------
--  DDL for Package Body GMA_FM_TEXT_TBL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMA_FM_TEXT_TBL_PKG" AS
/* $Header: GMAFMTXB.pls 115.6 2003/06/09 17:22:26 kmoizudd ship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from FM_TEXT_TBL_TL
...
-- Bug #1775354 (JKB)
-- bug #2747352 (kmoizudd) Modified the Where clause without ROW_ID
-- bug #2998701 (kmoizudd) removed  the old rowid STMT.
...
is
begin
  update FM_TEXT_TBL_TL T set (
      TEXT
    ) = (select
      B.TEXT
    from FM_TEXT_TBL_TL B
  ...
  where (
      ...
      SUBT.LINE_NO,
      SUBT.LANGUAGE
    from FM_TEXT_TBL_TL SUBB, FM_TEXT_TBL_TL SUBT
  ...
  insert into FM_TEXT_TBL_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from FM_TEXT_TBL_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
...
procedure LOAD_ROW (
 ...
 BEGIN
   ...
   GMA_FM_TEXT_TBL_PKG.UPDATE_ROW ( X_ROW_ID => l_row_id,
...
-- Bug #1712111 (JKB)
 ...
 EXCEPTION
  ...
  GMA_FM_TEXT_TBL_PKG.INSERT_ROW (  X_ROWID => l_row_id,
                                    X_TEXT_CODE => X_TEXT_CODE,
                                    X_LANG_CODE => X_LANG_CODE,
                                    X_PARAGRAPH_CODE => X_PARAGRAPH_CODE,
                                    X_SUB_PARACODE => X_SUB_PARACODE,
