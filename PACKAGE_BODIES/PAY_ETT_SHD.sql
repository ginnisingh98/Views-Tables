--------------------------------------------------------
--  DDL for Package Body PAY_ETT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ETT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_ett_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PAY_ELEMENT_TYPES_F_TL_PK') Then
...
Begin
  --
  If (p_element_type_id is null or
      p_language is null
     ) Then
    --
  ...
  Else
    If (p_element_type_id
        = pay_ett_shd.g_old_rec.element_type_id and
        p_language
        = pay_ett_shd.g_old_rec.language
       ) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into pay_ett_shd.g_old_rec;
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'pay_element_types_f_tl');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
--
-- This procedure has been added after running the UNIX program tltblgen.
--
-- ----------------------------------------------------------------------------
