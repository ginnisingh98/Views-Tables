--------------------------------------------------------
--  DDL for Package Body PER_POSITION_INFO_TYPES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_POSITION_INFO_TYPES_PKG" as
/* $Header: perposit.pkb 115.3 2002/12/06 16:18:12 eumenyio ship $ */
--------------------------------------------------------------------------------
...
CURSOR C1 IS
 	select  1
 	from    PER_POSITION_INFO_types t
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from PER_POSITION_INFO_TYPES
...
begin
  update PER_POSITION_INFO_TYPES set
...
begin
  ...
  delete from PER_POSITION_INFO_TYPES
  where INFORMATION_TYPE = X_INFORMATION_TYPE;
...
is
  l_proc                        VARCHAR2(61) := 'PER_POSITION_INFO_TYPES_PKG.LOAD_ROW';
  l_rowid                       rowid;
  l_request_id                  PER_POSITION_INFO_types.request_id%TYPE;
  l_progam_application_id       PER_POSITION_INFO_types.program_application_id%TYPE;
  l_program_id                  PER_POSITION_INFO_types.program_id%TYPE;
  l_program_update_date         PER_POSITION_INFO_types.program_update_date%TYPE;
  l_created_by                  PER_POSITION_INFO_types.created_by%TYPE             := 0;
  l_creation_date               PER_POSITION_INFO_types.creation_date%TYPE          := SYSDATE;
  l_last_update_date            PER_POSITION_INFO_types.last_update_date%TYPE       := SYSDATE;
  l_last_updated_by             PER_POSITION_INFO_types.last_updated_by%TYPE         := 0;
  l_last_update_login           PER_POSITION_INFO_types.last_update_login%TYPE      := 0;
begin
...
is
begin
  UPDATE per_position_info_types
...
END PER_POSITION_INFO_TYPES_PKG;
