--------------------------------------------------------
--  DDL for Package Body BEN_EOU_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EOU_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_eou_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_org_unit_prte_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_org_unit_prte_id PK of record being inserted or updated.
...
--
Procedure chk_elig_org_unit_prte_id(p_elig_org_unit_prte_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_org_unit_prte_id';
...
Begin
  ...
  --
  l_api_updating := ben_eou_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_org_unit_prte_id                => p_elig_org_unit_prte_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_org_unit_prte_id,hr_api.g_number)
     <>  ben_eou_shd.g_old_rec.elig_org_unit_prte_id) then
    --
    -- raise error as PK has changed
    --
    ben_eou_shd.constraint_error('BEN_ELIG_ORG_UNIT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_org_unit_prte_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_org_unit_prte_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_eou_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_organization_id (p_elig_org_unit_prte_id          in number,
  ...
  cursor c1 is
    ...
    where  a.organization_id = p_organization_id
      ...
      and a.internal_external_flag = 'INT';
  --
--
  cursor c3 is
     select null
       from ben_elig_org_unit_prte_f
         where organization_id = p_organization_id
           ...
           and elig_org_unit_prte_id <> nvl(p_elig_org_unit_prte_id,hr_api.g_number)
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_eou_shd.g_old_rec.organization_id,hr_api.g_number)
     or not l_api_updating) then
    --
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        close c1;
        --
        -- raise error as FK does not relate to PK in per_organizations
        -- table.
        --
        ben_eou_shd.constraint_error('BEN_ELIG_ORG_UNIT_FK1');
        --
      end if;
      --
    close c1;
...
--
-- In Parameters
--   p_elig_org_unit_prte_id     elig_org_unit_prte_id
...
is
  ...
  cursor c1 is
    select null
    from   ben_elig_org_unit_prte_f
    where  elig_org_unit_prte_id <> nvl(p_elig_org_unit_prte_id,-1)
    and    eligy_prfl_id = p_eligy_prfl_id
...
Begin
  ...
  --
  chk_organization_id (p_elig_org_unit_prte_id => p_rec.elig_org_unit_prte_id,
...
function return_legislation_code
  (p_elig_org_unit_prte_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_org_unit_prte_f b
    where b.elig_org_unit_prte_id      = p_elig_org_unit_prte_id
