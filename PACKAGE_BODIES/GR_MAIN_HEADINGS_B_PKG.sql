--------------------------------------------------------
--  DDL for Package Body GR_MAIN_HEADINGS_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_MAIN_HEADINGS_B_PKG" AS
/*$Header: GRHIMHBB.pls 115.3 2002/10/28 16:42:58 methomas ship $*/
...
/*	  Now call the check foreign key procedure */
   ...
   Check_Foreign_Keys
			     (p_main_heading_code,
				  p_main_heading_print_font,
				  p_main_heading_print_size,
				  l_return_status,
				  l_oracle_error,
				  l_msg_data);
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_main_headings_b
   		  	     (main_heading_code,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_main_headings_b
	  SET	 main_heading_code               = p_main_heading_code,
			 main_heading_print_font         = p_main_heading_print_font,
			 main_heading_print_size         = p_main_heading_print_size,
...
CURSOR c_lock_main_hdg_b
 IS
   SELECT	*
   FROM		gr_main_headings_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockMainHdgRcd	  c_lock_main_hdg_b%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_main_hdg_b;
   FETCH c_lock_main_hdg_b INTO LockMainHdgRcd;
   IF c_lock_main_hdg_b%NOTFOUND THEN
	  CLOSE c_lock_main_hdg_b;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_main_headings_b
   WHERE  	   rowid = p_rowid;
...
CURSOR c_get_doc_hdgs
 IS
   ...
   WHERE  dh.main_heading_code = p_main_heading_code;
...
CURSOR c_get_item_doc
 IS
   ...
   WHERE  idd.main_heading_code = p_main_heading_code;
...
CURSOR c_get_main_hdg_b_rowid
 IS
   SELECT mhb.rowid
   FROM	  gr_main_headings_b mhb
   WHERE  mhb.main_heading_code = p_main_heading_code;
MainHdgBRecord			   c_get_main_hdg_b_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_main_heading_code;

   x_key_exists := 'F';
   OPEN c_get_main_hdg_b_rowid;
   FETCH c_get_main_hdg_b_rowid INTO MainHdgBRecord;
   IF c_get_main_hdg_b_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := MainHdgBRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_main_hdg_b_rowid;
