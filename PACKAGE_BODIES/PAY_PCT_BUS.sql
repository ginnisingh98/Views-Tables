--------------------------------------------------------
--  DDL for Package Body PAY_PCT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PCT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_pct_bus.';  -- Global package name
--
...
Begin
  ...
  --
  --
  if (( nvl(pay_pct_bus.g_user_column_id, hr_api.g_number)
       = p_user_column_id)
  and ( nvl(pay_pct_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pay_pct_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_pct_bus.g_user_column_id              := p_user_column_id;
    pay_pct_bus.g_language                    := p_language;
    pay_pct_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_pct_shd.api_updating
