--------------------------------------------------------
--  DDL for Package Body EGO_ITEM_USERATTRS_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."EGO_ITEM_USERATTRS_PUB" AS
/* $Header: EGOPIUAB.pls 120.1 2008/03/11 11:16:49 deegupta ship $ */

               ------------------------------------
               -- Global Variables and Constants --
               ------------------------------------

   G_PKG_NAME                CONSTANT VARCHAR2(30) := 'EGO_ITEM_USERATTRS_PUB';
...
IS
BEGIN
/* Solving Perf. Bug 4191046 */
/* The following delete and update statements are commented out */
/* as a quick workaround to fix the time-consuming table handler issue */
/*
  delete from EGO_MTL_SY_ITEMS_EXT_TL T
  where not exists
    (select NULL
    from EGO_MTL_SY_ITEMS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update EGO_MTL_SY_ITEMS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
    ...
    (select
      ...
      B.TL_EXT_ATTR39,
      B.TL_EXT_ATTR40
    from EGO_MTL_SY_ITEMS_EXT_TL B
    where B.EXTENSION_ID = T.EXTENSION_ID
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from EGO_MTL_SY_ITEMS_EXT_TL SUBB, EGO_MTL_SY_ITEMS_EXT_TL SUBT
...
*/
  insert into EGO_MTL_SY_ITEMS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from EGO_MTL_SY_ITEMS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from EGO_MTL_SY_ITEMS_EXT_TL T
