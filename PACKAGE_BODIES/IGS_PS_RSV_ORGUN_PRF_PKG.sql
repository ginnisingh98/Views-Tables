--------------------------------------------------------
--  DDL for Package Body IGS_PS_RSV_ORGUN_PRF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_RSV_ORGUN_PRF_PKG" AS
/* $Header: IGSPI1VB.pls 120.1 2005/08/18 07:16:04 appldev ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_rsv_orgun_prf%ROWTYPE;
  new_references igs_ps_rsv_orgun_prf%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.rsv_org_unit_prf_id               := x_rsv_org_unit_prf_id;
  ...
  */
  BEGIN
    ...
    IF (((old_references.rsv_org_unit_pri_id = new_references.rsv_org_unit_pri_id)) OR
        ((new_references.rsv_org_unit_pri_id IS NULL))) THEN
      NULL;
    ELSIF NOT igs_ps_rsv_ogpri_pkg.get_pk_for_validation (
                new_references.rsv_org_unit_pri_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
  ...
  */
  ...
  END get_fk_igs_ps_rsv_ogpri_all;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_ROPF_CRV_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_ROPF_CSTT_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_ROPF_US_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.rsv_org_unit_pri_id = x_rsv_org_unit_pri_id)
