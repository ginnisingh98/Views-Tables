--------------------------------------------------------
--  DDL for Package Body PV_PRGM_PMT_MODE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PV_PRGM_PMT_MODE_PKG" as
/* $Header: pvxtppmb.pls 120.0 2005/05/27 16:26:42 appldev noship $ */
-- ===============================================================
-- Start of Comments
-- Package name
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'PV_PRGM_PMT_MODE_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'pvxtppmb.pls';
...
BEGIN
   ...
   INSERT INTO PV_PROGRAM_PAYMENT_MODE(
         ...
         VALUES (
           -- DECODE( px_program_payment_mode_id, FND_API.g_miss_num, NULL, px_program_payment_mode_id)
           -- ,DECODE( p_program_id, FND_API.g_miss_num, NULL, p_program_id)
           -- ,DECODE( p_geo_hierarchy_id, FND_API.g_miss_num, NULL, p_geo_hierarchy_id)
           -- ,DECODE( p_mode_of_payment, FND_API.g_miss_char, NULL, p_mode_of_payment)
           ...
           -- ,DECODE( p_object_version_number, FND_API.g_miss_num, NULL, p_object_version_number)
            ...
            DECODE( px_program_payment_mode_id, NULL,px_program_payment_mode_id,FND_API.g_miss_num, NULL, px_program_payment_mode_id)
           ...
           ,DECODE( p_mode_of_payment, NULL,p_mode_of_payment,FND_API.g_miss_char, NULL, p_mode_of_payment)
	   ...
	   ,DECODE( p_creation_date, NULL, p_creation_date, FND_API.g_miss_date, NULL, p_creation_date)
           ...
           ,DECODE( p_mode_type, NULL,p_mode_type,FND_API.g_miss_char, NULL, p_mode_type)
	   );
END Insert_Row;
...
PROCEDURE Update_Row(
 ...
 BEGIN
    ...
    Update PV_PROGRAM_PAYMENT_MODE
    SET
       -- program_payment_mode_id = DECODE( p_program_payment_mode_id, FND_API.g_miss_num, program_payment_mode_id, p_program_payment_mode_id)
       -- ,program_id              = DECODE( p_program_id, FND_API.g_miss_num, program_id, p_program_id)
       -- ,geo_hierarchy_id        = DECODE( p_geo_hierarchy_id, FND_API.g_miss_num, geo_hierarchy_id, p_geo_hierarchy_id)
       -- ,mode_of_payment         = DECODE( p_mode_of_payment, FND_API.g_miss_char, mode_of_payment, p_mode_of_payment)
       ...
       -- ,object_version_number   = DECODE( p_object_version_number, FND_API.g_miss_num, object_version_number, p_object_version_number+1)
        ...
        program_payment_mode_id = DECODE( p_program_payment_mode_id, NULL, program_payment_mode_id, FND_API.g_miss_num, NULL, p_program_payment_mode_id)
       ...
       ,mode_of_payment         = DECODE( p_mode_of_payment, NULL, mode_of_payment, FND_API.g_miss_char, NULL, p_mode_of_payment)
       ...
       ,mode_type         = DECODE( p_mode_type, NULL,p_mode_type,FND_API.g_miss_char, NULL, p_mode_type)
...
PROCEDURE Delete_Row(
 ...
 BEGIN
   ...
   DELETE FROM PV_PROGRAM_PAYMENT_MODE
    WHERE program_payment_mode_id = p_program_payment_mode_id
     AND object_version_number = p_object_version_number;
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
        ...
        WHERE PROGRAM_PAYMENT_MODE_ID =  px_program_payment_mode_id
        FOR UPDATE of PROGRAM_PAYMENT_MODE_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN c;
    ...
    CLOSE C;
    IF (
           (      Recinfo.program_payment_mode_id = px_program_payment_mode_id)
       ...
       AND (    ( Recinfo.mode_of_payment = p_mode_of_payment)
            OR (    ( Recinfo.mode_of_payment IS NULL )
                AND (  p_mode_of_payment IS NULL )))
       ...
       AND (    ( Recinfo.mode_type = p_mode_type)
            OR (    ( Recinfo.mode_type IS NULL )
                AND (  p_mode_type IS NULL )))
       ) THEN
       RETURN;
   ELSE
