--------------------------------------------------------
--  DDL for Package Body PER_PGV_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PGV_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_pgv_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PER_GEN_HIERARCHY_VER_UK1') Then
...
Begin
  --
  If (p_hierarchy_version_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_hierarchy_version_id
        = per_pgv_shd.g_old_rec.hierarchy_version_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into per_pgv_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> per_pgv_shd.g_old_rec.object_version_number) Then
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'per_gen_hierarchy_versions');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
