--------------------------------------------------------
--  DDL for Package Body HXC_TCC_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_TCC_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_tcc_ins.';  -- Global package name
...
Begin
  ...
  --
  -- Insert the row into: hxc_time_category_comps
  --

  insert into hxc_time_category_comps
  ...
  Values
          ...
          ,p_rec.ref_time_category_id
          ,p_rec.component_type_id
...
if g_debug then
	hr_utility.trace(SUBSTR(SQLERRM,1,60));
end if;
    hxc_tcc_shd.g_api_dml := false;   -- Unset the api dml status
...
if g_debug then
	hr_utility.trace(SUBSTR(SQLERRM,1,60));
	hr_utility.trace(SUBSTR(SQLERRM,61,120));
end if;
    hxc_tcc_shd.g_api_dml := false;   -- Unset the api dml status
    Raise;
End insert_dml;
...
--
  l_proc  varchar2(72);
--
  Cursor C_Sel1 is select hxc_time_category_comps_s.nextval from sys.dual;
--
Begin
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.time_category_comp_id;
  Close C_Sel1;
  --
  if g_debug then
  	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_tcc_bus.insert_validate
     (p_rec
     );
  --
  -- Call the supporting pre-insert operation
  --
  hxc_tcc_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_tcc_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_tcc_ins.post_insert
     (p_rec
     );

  IF ( p_rec.type IN ( 'AN', 'MC_VS' ) )
  THEN
      hxc_time_category_utils_pkg.insert_time_category_comp_sql ( p_rec );
...
Procedure ins
  ...
  ,p_type                         in varchar2
  ,p_time_category_comp_id        out nocopy number
...
Begin
  ...
  -- Having converted the arguments into the hxc_tcc_rec
  ...
  --
  p_time_category_comp_id := l_rec.time_category_comp_id;
