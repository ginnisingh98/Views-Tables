--------------------------------------------------------
--  DDL for Package Body PAY_KR_YEA20020101_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KR_YEA20020101_PKG" as
/* $Header: pykryea1.pkb 120.3 2006/12/21 06:54:08 pdesu noship $ */
--
procedure yea(
...
begin
	...
	if p_yea_info.non_resident_flag = 'N' then
		...
		or p_yea_info.lt_housing_loan_interest_repay > 0 then
                        ...
                        --
			p_yea_info.max_housing_exp_tax_exem := trunc((p_yea_info.housing_saving + p_yea_info.housing_loan_repay) * l_housinexp_tax_exem_per )
	...
	if p_yea_info.non_resident_flag = 'N' then
		...
		if p_yea_info.credit_card_exp > 0 then
			l_dummy := p_yea_info.credit_card_exp - trunc(greatest(p_yea_info.taxable, 0) * l_cre_card_exp_tax_exem_per1);
			if l_dummy > 0 then
				p_yea_info.credit_card_exp_tax_exem := least(trunc(l_dummy * l_cre_card_exp_tax_exem_per2), l_cre_card_exp_tax_exem_lim);
	...
	if p_yea_info.non_resident_flag = 'N' then
		...
		-- Stock Saving Tax Break
                -- Commented out this calculation for YEA Stats Update 2002
                -- Stock Saving Tax Break is obsolete now (Bug 2577751)
		----------------------------------------------------------------
--		if p_yea_info.stock_saving > 0 then
--			p_yea_info.stock_saving_tax_break := trunc(p_yea_info.stock_saving * l_stock_savintax_break_per);
...
--		end if;
	...
	----------------------------------------------------------------
        --Bug 2605158 : truncating the last Won digit after the final calculation of
        --              annual_itax , annual_rtax and annual_stax.
