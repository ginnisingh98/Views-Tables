--------------------------------------------------------
--  DDL for Package Body AP_APXPAYSC_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_APXPAYSC_XMLP_PKG" AS
/* $Header: APXPAYSCB.pls 120.0 2007/12/27 08:19:58 vjaganat noship $ */
...
CP_END_DUE_DATE := to_char(P_END_DUE_DATE,'DD-MON-YY');
...
BEGIN
  ...
  IF (get_report_curr_data() <> TRUE) THEN        RAISE init_failure;
  END IF;
  IF (p_debug_switch in ('y','Y')) THEN
     /*SRW.MESSAGE('5','After Get_Report_Curr_Data');*/null;
 ...
 /* IF (get_date_format() <> TRUE) THEN        RAISE init_failure;
  ...
  --COMMENTED BY SWAPNA
  IF (p_debug_switch in ('y','Y')) THEN
     /*SRW.MESSAGE('6','After Get_Date_format');*/null;
...
FUNCTION get_report_curr_data RETURN boolean IS
...
BEGIN
  ...
  WHERE   c.currency_code = p_payment_currency;

  h_currency := p_payment_currency;
  h_currency_precision := prec;
