--------------------------------------------------------
--  DDL for Package Body GR_PHRASE_GROUP_CODES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_PHRASE_GROUP_CODES_PKG" AS
/*$Header: GRHIPGCB.pls 115.7 2002/10/28 20:19:31 methomas ship $*/
PROCEDURE Insert_Row
	   			 (p_commit IN VARCHAR2,
				  p_called_by_form IN VARCHAR2,
				  p_phrase_group_code IN VARCHAR2,
				  p_phrase_group_description IN VARCHAR2,
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_phrase_group_codes
   		  	     (phrase_group_code,
   		  	      phrase_group_description,
				  created_by,
				  creation_date,
				  last_updated_by,
				  last_update_date,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_phrase_group_codes
	  SET	 phrase_group_code	 			 = p_phrase_group_code,
	         phrase_group_description 	 	 = p_phrase_group_description,
...
RECORD_CHANGED_ERROR	 	EXCEPTION;  /* added, cleichte 10-20-99 */


/*   Define the cursors */

CURSOR c_lock_phrase_group_codes
 IS
   SELECT	last_update_date
   FROM		gr_phrase_group_codes
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockGroupRcd	  c_lock_phrase_group_codes%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_phrase_group_codes;
   FETCH c_lock_phrase_group_codes INTO LockGroupRcd;
   IF c_lock_phrase_group_codes%NOTFOUND THEN
	  CLOSE c_lock_phrase_group_codes;
	  RAISE No_Data_Found_Error;
   END IF;
   CLOSE c_lock_phrase_group_codes;

   IF LockGroupRcd.last_update_date <> p_last_update_date THEN
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_phrase_group_codes
   WHERE  	   rowid = p_rowid;
...
/*	 Define the Cursors */
/*   Phrase Concentrations Table */

CURSOR c_get_phrase_grp_assoc
 IS
   SELECT	COUNT(*)
   FROM		gr_phrase_group_assoc
   WHERE	phrase_group_code = p_phrase_group_code;
...
/*   Phrase Group Associations table */

   l_record_count := 0;
   l_code_block := 'c_get_phrase_grp_assoc';
   OPEN c_get_phrase_grp_assoc;
   FETCH c_get_phrase_grp_assoc INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
	  l_msg_data := l_msg_data || 'gr_phrase_group_assoc, ';
   END IF;
   CLOSE c_get_phrase_grp_assoc;
...
/*   Item Document Details */
   ...
   l_record_count := 0;
   l_code_block := 'c_get_item_doc_details';
   OPEN c_get_item_doc_details;
   FETCH c_get_item_doc_details INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
	  l_msg_data := l_msg_data || 'gr_item_document_dtls, ';
   END IF;
   CLOSE c_get_item_doc_details;
...
/*		  p_phrase_group_code is the phrase group code to check.
...
CURSOR c_get_phrase_group_rowid
 IS
   SELECT pgc.rowid
   FROM	  gr_phrase_group_codes pgc
   WHERE  pgc.phrase_group_code = p_phrase_group_code;
GroupRecord			   c_get_phrase_group_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_phrase_group_rowid;
   FETCH c_get_phrase_group_rowid INTO GroupRecord;
   IF c_get_phrase_group_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := GroupRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_phrase_group_rowid;
