--------------------------------------------------------
--  DDL for Package Body AP_APXRPRPR_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_APXRPRPR_XMLP_PKG" AS
/* $Header: APXRPRPRB.pls 120.0 2007/12/27 08:29:19 vjaganat noship $ */
...
BEGIN
  ...
  IF (p_debug_switch = 'Y') THEN
     /*SRW.BREAK;*/null;
  END IF;
  IF p_select_option = 'N' THEN
     p_where := (' WHERE   p.vendor_id      = v.vendor_id
                    AND     gl1.currency_code = p.invoice_currency_code
                    AND     p.vendor_site_id = s.vendor_site_id
                    AND     alc.lookup_type(+) = ''HOLD CODE''
                    AND     alc.lookup_code(+) = p.hold_lookup_code
                 ');
  ELSE
    p_where := (' WHERE  p.vendor_id = v.vendor_id
                   AND    gl1.currency_code = p.invoice_currency_code
                   AND    p.vendor_site_id = s.vendor_site_id
                   AND    nvl(num_of_periods_rem,0) <> 0
		   AND    nvl(expiry_date, sysdate+1) > sysdate
                   AND    alc.lookup_type(+) = ''HOLD CODE''
                   AND    alc.lookup_code(+) = p.hold_lookup_code
                ');
  END IF;
  IF p_order_by_option = 'Create Date' THEN
     p_orderby := ('rec_pay_period_type,
	             gl1.name,
                     p.creation_date
                  ');
  ELSE
