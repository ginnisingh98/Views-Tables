--------------------------------------------------------
--  DDL for Package Body PAY_PGR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PGR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_pgr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    ,grade_or_spinal_point_id             = p_rec.grade_or_spinal_point_id
    ,rate_type                            = p_rec.rate_type
...
Begin
  ...
  --
  pay_pgr_upd.convert_defs(p_rec);
  --
  pay_pgr_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
Begin
  ...
  pay_pgr_shd.convert_args
    ...
    ,hr_api.g_number  --p_rate_id
    ,hr_api.g_number  --p_grade_or_spinal_point_id
