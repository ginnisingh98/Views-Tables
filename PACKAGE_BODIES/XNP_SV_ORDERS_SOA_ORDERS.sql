--------------------------------------------------------
--  DDL for Package Body XNP_SV_ORDERS$SOA_ORDERS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_SV_ORDERS$SOA_ORDERS" as
/* $Header: XNPSVO4B.pls 120.0 2005/05/30 11:46:22 appldev noship $ */
...
--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.Startup
--
-- Description: Entry point for the 'SOA_ORDERS' module
...
--------------------------------------------------------------------------------
   ...
   begin
      ...
      XNP_WSGL.RegisterURL('xnp_sv_orders$soa_orders.startup');
   ...
   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             DEF_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.Startup');
   end;

--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.QueryHits
--
...
--------------------------------------------------------------------------------
   ...
   begin
      ...
      if not PreQuery(P_SV_SOA_ID) then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_ERROR, XNP_cg$errors.GetErrors,
                             'Monitor Ordering Subscriptions'||' : '||'Order Workitems', DEF_BODY_ATTRIBUTES);
         return -1;
      end if;
   ...
   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             DEF_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.QueryHits');
         return -1;
   end;--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.BuildSQL
--
-- Description: Builds the SQL for the 'SOA_ORDERS' module component (Order Workitems).
...
--------------------------------------------------------------------------------
   ...
   begin
      ...
      begin
         XNP_WSGL.BuildWhere(P_SV_SOA_ID, 'SOG.SV_SOA_ID', XNP_WSGL.TYPE_NUMBER, I_WHERE);
      exception
         when others then
            XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_ERROR_QRY, SQLERRM,
                                'Monitor Ordering Subscriptions'||' : '||'Order Workitems', DEF_BODY_ATTRIBUTES, NULL,
      ...
      ZONE_SQL := ZONE_SQL || ' ORDER BY SOG.SV_SOA_ID';
      return true;

   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             DEF_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.BuildSQL');
         return false;
   end;


--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.QueryList
--
-- Description: This procedure builds the Record list for the 'SOA_ORDERS'
...
--------------------------------------------------------------------------------
   ...
   begin
      ...
      XNP_WSGL.RegisterURL('xnp_sv_orders$soa_orders.querylist');
      XNP_WSGL.AddURLParam('P_SV_SOA_ID', P_SV_SOA_ID);
      ...
      XNP_WSGL.OpenPageHead('Monitor Ordering Subscriptions'||' : '||'Order Workitems');
      ...
      htp.p(htf.header(1,fnd_message.get_string('XNP','WEB_XNPSVORD_ORDERS_TITLE')));
	 ...
	 null;
         ...
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_ERROR, XNP_WSGL.MsgGetText(215,XNP_WSGLM.MSG215_NO_MULTIPLE_SUBMITS),
                             'Monitor Ordering Subscriptions'||' : '||'Order Workitems', RL_BODY_ATTRIBUTES);
         XNP_WSGL.ClosePageBody;
         return;
      end if;
      ...
      htp.formOpen(curl => 'xnp_sv_orders$soa_orders.querylist', cattributes => 'NAME="frmZero"');
   ...
   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             RL_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.QueryList');
         XNP_WSGL.ClosePageBody;
   end;


--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.PreQuery
--
-- Description: Provides place holder for code to be run prior to a query
--              for the 'SOA_ORDERS' module component  (Order Workitems).
--
-- Parameters:  None
--
...
--------------------------------------------------------------------------------
   ...
   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             DEF_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.PreQuery');
         return FALSE;
   end;

--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.PostQuery
--
...
--------------------------------------------------------------------------------
   ...
   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             DEF_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.PostQuery');
          return FALSE;
   end;

--------------------------------------------------------------------------------
-- Name:        xnp_sv_orders$soa_orders.CreateListJavaScript
--
-- Description:
--
-- Parameters:
...
--------------------------------------------------------------------------------
   ...
   exception
      when others then
         XNP_WSGL.DisplayMessage(XNP_WSGL.MESS_EXCEPTION, SQLERRM, 'Monitor Ordering Subscriptions'||' : '||'Order Workitems',
                             RL_BODY_ATTRIBUTES, 'xnp_sv_orders$soa_orders.CreateListJavaScript');
   end;
end;
