--------------------------------------------------------
--  DDL for Package Body IRC_SEARCH_CRITERIA_BE5
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_SEARCH_CRITERIA_BE5" as 
...
p_object_version_number        number) is
  ...
  --
  l_proc varchar2(72):='  irc_search_criteria_be5.update_vacancy_criteria_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.irc.api.search_criteria.update_vacancy_criteria';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<max_salary>';
    l_text:=l_text||fnd_number.number_to_canonical(p_max_salary);
    l_text:=l_text||'</max_salary>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<min_qual_level>';
    l_text:=l_text||fnd_number.number_to_canonical(p_min_qual_level);
    l_text:=l_text||'</min_qual_level>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<max_qual_level>';
    l_text:=l_text||fnd_number.number_to_canonical(p_max_qual_level);
    l_text:=l_text||'</max_qual_level>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<description>';
