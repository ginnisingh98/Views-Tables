--------------------------------------------------------
--  DDL for Package Body BEN_PRB_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PRB_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_prb_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_pl_regy_bod_f Row
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_prb_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    ben_prb_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_prb_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.rptg_grp_id = hr_api.g_number) then
    p_rec.rptg_grp_id :=
    ben_prb_shd.g_old_rec.rptg_grp_id;
  End If;
 If (p_rec.organization_id = hr_api.g_number) then
    p_rec.organization_id :=
    ben_prb_shd.g_old_rec.organization_id;
  End If;
  If (p_rec.quald_dt = hr_api.g_date) then
    p_rec.quald_dt :=
    ben_prb_shd.g_old_rec.quald_dt;
  End If;
  If (p_rec.quald_flag = hr_api.g_varchar2) then
    p_rec.quald_flag :=
    ben_prb_shd.g_old_rec.quald_flag;
  End If;
  If (p_rec.regy_pl_name = hr_api.g_varchar2) then
    p_rec.regy_pl_name :=
    ben_prb_shd.g_old_rec.regy_pl_name;
  End If;
  If (p_rec.aprvd_trmn_dt = hr_api.g_date) then
    p_rec.aprvd_trmn_dt :=
    ben_prb_shd.g_old_rec.aprvd_trmn_dt;
  End If;
  If (p_rec.prb_attribute_category = hr_api.g_varchar2) then
    p_rec.prb_attribute_category :=
    ben_prb_shd.g_old_rec.prb_attribute_category;
  End If;
  If (p_rec.prb_attribute1 = hr_api.g_varchar2) then
    p_rec.prb_attribute1 :=
    ben_prb_shd.g_old_rec.prb_attribute1;
  End If;
  If (p_rec.prb_attribute2 = hr_api.g_varchar2) then
    p_rec.prb_attribute2 :=
    ben_prb_shd.g_old_rec.prb_attribute2;
  End If;
  If (p_rec.prb_attribute3 = hr_api.g_varchar2) then
    p_rec.prb_attribute3 :=
    ben_prb_shd.g_old_rec.prb_attribute3;
  End If;
  If (p_rec.prb_attribute4 = hr_api.g_varchar2) then
    p_rec.prb_attribute4 :=
    ben_prb_shd.g_old_rec.prb_attribute4;
  End If;
  If (p_rec.prb_attribute5 = hr_api.g_varchar2) then
    p_rec.prb_attribute5 :=
    ben_prb_shd.g_old_rec.prb_attribute5;
  End If;
  If (p_rec.prb_attribute6 = hr_api.g_varchar2) then
    p_rec.prb_attribute6 :=
    ben_prb_shd.g_old_rec.prb_attribute6;
  End If;
  If (p_rec.prb_attribute7 = hr_api.g_varchar2) then
    p_rec.prb_attribute7 :=
    ben_prb_shd.g_old_rec.prb_attribute7;
  End If;
  If (p_rec.prb_attribute8 = hr_api.g_varchar2) then
    p_rec.prb_attribute8 :=
    ben_prb_shd.g_old_rec.prb_attribute8;
  End If;
  If (p_rec.prb_attribute9 = hr_api.g_varchar2) then
    p_rec.prb_attribute9 :=
    ben_prb_shd.g_old_rec.prb_attribute9;
  End If;
  If (p_rec.prb_attribute10 = hr_api.g_varchar2) then
    p_rec.prb_attribute10 :=
    ben_prb_shd.g_old_rec.prb_attribute10;
  End If;
  If (p_rec.prb_attribute11 = hr_api.g_varchar2) then
    p_rec.prb_attribute11 :=
    ben_prb_shd.g_old_rec.prb_attribute11;
  End If;
  If (p_rec.prb_attribute12 = hr_api.g_varchar2) then
    p_rec.prb_attribute12 :=
    ben_prb_shd.g_old_rec.prb_attribute12;
  End If;
  If (p_rec.prb_attribute13 = hr_api.g_varchar2) then
    p_rec.prb_attribute13 :=
    ben_prb_shd.g_old_rec.prb_attribute13;
  End If;
  If (p_rec.prb_attribute14 = hr_api.g_varchar2) then
    p_rec.prb_attribute14 :=
    ben_prb_shd.g_old_rec.prb_attribute14;
  End If;
  If (p_rec.prb_attribute15 = hr_api.g_varchar2) then
    p_rec.prb_attribute15 :=
    ben_prb_shd.g_old_rec.prb_attribute15;
  End If;
  If (p_rec.prb_attribute16 = hr_api.g_varchar2) then
    p_rec.prb_attribute16 :=
    ben_prb_shd.g_old_rec.prb_attribute16;
  End If;
  If (p_rec.prb_attribute17 = hr_api.g_varchar2) then
    p_rec.prb_attribute17 :=
    ben_prb_shd.g_old_rec.prb_attribute17;
  End If;
  If (p_rec.prb_attribute18 = hr_api.g_varchar2) then
    p_rec.prb_attribute18 :=
    ben_prb_shd.g_old_rec.prb_attribute18;
  End If;
  If (p_rec.prb_attribute19 = hr_api.g_varchar2) then
    p_rec.prb_attribute19 :=
    ben_prb_shd.g_old_rec.prb_attribute19;
  End If;
  If (p_rec.prb_attribute20 = hr_api.g_varchar2) then
    p_rec.prb_attribute20 :=
    ben_prb_shd.g_old_rec.prb_attribute20;
  End If;
  If (p_rec.prb_attribute21 = hr_api.g_varchar2) then
    p_rec.prb_attribute21 :=
    ben_prb_shd.g_old_rec.prb_attribute21;
  End If;
  If (p_rec.prb_attribute22 = hr_api.g_varchar2) then
    p_rec.prb_attribute22 :=
    ben_prb_shd.g_old_rec.prb_attribute22;
  End If;
  If (p_rec.prb_attribute23 = hr_api.g_varchar2) then
    p_rec.prb_attribute23 :=
    ben_prb_shd.g_old_rec.prb_attribute23;
  End If;
  If (p_rec.prb_attribute24 = hr_api.g_varchar2) then
    p_rec.prb_attribute24 :=
    ben_prb_shd.g_old_rec.prb_attribute24;
  End If;
  If (p_rec.prb_attribute25 = hr_api.g_varchar2) then
    p_rec.prb_attribute25 :=
    ben_prb_shd.g_old_rec.prb_attribute25;
  End If;
  If (p_rec.prb_attribute26 = hr_api.g_varchar2) then
    p_rec.prb_attribute26 :=
    ben_prb_shd.g_old_rec.prb_attribute26;
  End If;
  If (p_rec.prb_attribute27 = hr_api.g_varchar2) then
    p_rec.prb_attribute27 :=
    ben_prb_shd.g_old_rec.prb_attribute27;
  End If;
  If (p_rec.prb_attribute28 = hr_api.g_varchar2) then
    p_rec.prb_attribute28 :=
    ben_prb_shd.g_old_rec.prb_attribute28;
  End If;
  If (p_rec.prb_attribute29 = hr_api.g_varchar2) then
    p_rec.prb_attribute29 :=
    ben_prb_shd.g_old_rec.prb_attribute29;
  End If;
  If (p_rec.prb_attribute30 = hr_api.g_varchar2) then
    p_rec.prb_attribute30 :=
    ben_prb_shd.g_old_rec.prb_attribute30;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
