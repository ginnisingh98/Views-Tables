--------------------------------------------------------
--  DDL for Package Body GR_COVER_LETTERS_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_COVER_LETTERS_B_PKG" AS
/*$Header: GRHICLBB.pls 115.3 2002/10/24 21:17:01 mgrosser ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_cover_letters_b
   		  	     (cover_letter_id,
				  orgn_code,
				  created_by,
				  creation_date,
...
CURSOR c_lock_letters_b
 IS
   SELECT	*
   FROM		gr_cover_letters_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockLetterRcd	  c_lock_letters_b%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_letters_b;
   FETCH c_lock_letters_b INTO LockLetterRcd;
   IF c_lock_letters_b%NOTFOUND THEN
	  CLOSE c_lock_letters_b;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_cover_letters_b
   WHERE  	   rowid = p_rowid;
...
/*     Initialization Routine */
   ...
   OPEN c_get_letter_text;
   FETCH c_get_letter_text INTO l_record_count;
   IF l_record_count <> 0 THEN
     l_return_status := 'E';
     l_msg_data := l_msg_data || ' gr_cover_letters_tl';
   END IF;
   CLOSE c_get_letter_text;
   IF l_return_status <> 'S' THEN
...
CURSOR c_get_letters_b_rowid
 IS
   SELECT clb.rowid
   FROM	  gr_cover_letters_b clb
   WHERE  clb.cover_letter_id = p_cover_letter_id;
LetterBRecord			   c_get_letters_b_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_cover_letter_id;

   x_key_exists := 'F';
   OPEN c_get_letters_b_rowid;
   FETCH c_get_letters_b_rowid INTO LetterBRecord;
   IF c_get_letters_b_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := LetterBRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_letters_b_rowid;
