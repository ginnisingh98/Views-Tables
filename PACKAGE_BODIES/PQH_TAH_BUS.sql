--------------------------------------------------------
--  DDL for Package Body PQH_TAH_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_TAH_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_tah_bus.';  -- Global package name
--
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'APPROVAL_HISTORY_ID')
      ,p_associated_column2
        => nvl(p_associated_column2,'TRANSACTION_HISTORY_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  /*if (( nvl(pqh_tah_bus.g_approval_history_id, hr_api.g_number)
       = p_approval_history_id)
  and ( nvl(pqh_tah_bus.g_transaction_history_id, hr_api.g_number)
    ...
    -- variable.
    --
    l_legislation_code := pqh_tah_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pqh_tah_bus.g_approval_history_id         := p_approval_history_id;
    pqh_tah_bus.g_transaction_history_id      := p_transaction_history_id;
    pqh_tah_bus.g_legislation_code  := l_legislation_code;
  end if;*/
...
Begin
  ...
  --
  IF NOT pqh_tah_shd.api_updating
