--------------------------------------------------------
--  DDL for Package Body PER_RAA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_RAA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_raa_upd.';  -- Global package name
g_recruiting_site_response boolean := false;
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the per_recruitment_activities Row
  --
  update per_recruitment_activities
    ...
    ,run_by_organization_id          = p_rec.run_by_organization_id
    ,internal_contact_person_id      = p_rec.internal_contact_person_id
    ,parent_recruitment_activity_id  = p_rec.parent_recruitment_activity_id
    ...
    ,date_closing                    = p_rec.date_closing
    ,date_end                        = p_rec.date_end
    ,external_contact                = p_rec.external_contact
    ,planned_cost                    = p_rec.planned_cost
    ,recruiting_site_id              = p_rec.recruiting_site_id
    ,recruiting_site_response        = empty_clob()
    ,last_posted_date                = p_rec.last_posted_date
    ,type                            = p_rec.type
  ...
  --
  --
    if (g_recruiting_site_response
       and dbms_lob.getlength(l_recruiting_site_response)<=32767
       and dbms_lob.instr(l_recruiting_site_response,p_rec.recruiting_site_response)<>1)
    then
      hr_utility.set_location(l_proc, 10);
      dbms_lob.trim(l_recruiting_site_response,0);
...
Begin
  ...
  If (p_rec.authorising_person_id = hr_api.g_number) then
    ...
    per_raa_shd.g_old_rec.authorising_person_id;
  End If;
  If (p_rec.run_by_organization_id = hr_api.g_number) then
    p_rec.run_by_organization_id :=
    per_raa_shd.g_old_rec.run_by_organization_id;
  End If;
  If (p_rec.internal_contact_person_id = hr_api.g_number) then
    ...
    per_raa_shd.g_old_rec.internal_contact_person_id;
  End If;
  If (p_rec.parent_recruitment_activity_id = hr_api.g_number) then
    p_rec.parent_recruitment_activity_id :=
    per_raa_shd.g_old_rec.parent_recruitment_activity_id;
  End If;
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    ...
    per_raa_shd.g_old_rec.name;
  End If;
  If (p_rec.actual_cost = hr_api.g_varchar2) then
    p_rec.actual_cost :=
    per_raa_shd.g_old_rec.actual_cost;
  End If;
  ...
  If (p_rec.contact_telephone_number = hr_api.g_varchar2) then
    ...
    per_raa_shd.g_old_rec.contact_telephone_number;
  End If;
  If (p_rec.date_closing = hr_api.g_date) then
    p_rec.date_closing :=
    per_raa_shd.g_old_rec.date_closing;
  End If;
  ...
  If (p_rec.external_contact = hr_api.g_varchar2) then
    ...
    per_raa_shd.g_old_rec.external_contact;
  End If;
  If (p_rec.planned_cost = hr_api.g_varchar2) then
    p_rec.planned_cost :=
    per_raa_shd.g_old_rec.planned_cost;
  End If;
  If (p_rec.recruiting_site_id = hr_api.g_number) then
    p_rec.recruiting_site_id :=
    per_raa_shd.g_old_rec.recruiting_site_id;
  End If;
  If (p_rec.recruiting_site_response = hr_api.g_varchar2) then
    p_rec.recruiting_site_response :=
    per_raa_shd.g_old_rec.recruiting_site_response;
  Else
    g_recruiting_site_response:=true;
  End If;
  if (p_rec.last_posted_date=hr_api.g_date) then
    p_rec.last_posted_date :=
    per_raa_shd.g_old_rec.last_posted_date;
  end if;
...
Begin
  ...
  --
  per_raa_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_raa_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_raa_upd.post_update
     (p_rec
     );
  -- Call to raise any errors on multi-message list
  hr_multi_message.end_validation_set;
