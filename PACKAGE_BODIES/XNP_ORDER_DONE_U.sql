--------------------------------------------------------
--  DDL for Package Body XNP_ORDER_DONE_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_ORDER_DONE_U" AS
/*$Header: XNPORDNB.pls 115.0 2000/02/03 22:42:05 pkm ship        $*/
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'ORDER_DONE' ;
...
BEGIN
  x_error_code := 0 ;
  x_error_message := NULL ;
/* check if the recipient list is NULL */
  l_recipient_list := p_recipient_list ;
  IF (l_recipient_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'ORDER_DONE', l_recipient_list );

  END IF;
  l_consumer_list := p_consumer_list ;
  IF (l_consumer_list IS NULL) THEN
    l_consumer_list := l_recipient_list ;
  END IF;
