--------------------------------------------------------
--  DDL for Package Body OPI_DBI_WIP_COMP_INIT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OPI_DBI_WIP_COMP_INIT_PKG" AS
/*$Header: OPIDCOMPLIB.pls 120.0 2005/05/24 17:59:10 appldev noship $ */
...
EURO_MISSING_AT_START CONSTANT NUMBER := -3;
...
s_pkg_name CONSTANT VARCHAR2 (50) := 'opi_dbi_wip_comp_init_pkg';
...
/*  collect_wip_completions_init
    ...
    data in the initial run can be reported simultaneously.
...
IS
    ...
    l_proc_name CONSTANT VARCHAR2 (60) := 'collect_wip_completions_init';
    l_stmt_id NUMBER;
...
BEGIN
    ...
    --                   'MLOG$_OPI_DBI_WIP_COMP_F');

    l_stmt_id := 25;
    EXECUTE IMMEDIATE ('TRUNCATE TABLE ' || s_opi_schema || '.' ||
                       'OPI_DBI_WIP_COMP_F PURGE MATERIALIZED VIEW LOG');
    ...
    BEGIN
        ...
        -- STAGING TABLE.
        collect_init_opi_wip_comp (errbuf, retcode, l_global_start_date);
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            ...
            BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                               l_proc_name || ' ' ||
                                               '#' || l_stmt_id || ': ' ||
                                 'Unable to collect OPI WIP completions data in initial load into staging table.');
    ...
    BEGIN
        ...
        -- COMMIT DATA AS NEEDED!!! WRAPPER WILL NOT COMMIT DATA FOR
        -- STAGING TABLE.
        opi_dbi_wip_comp_opm_pkg.collect_init_opm_wip_comp (errbuf, retcode,
                                                            l_global_start_date);
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            ...
            BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                               l_proc_name || ' ' ||
                                               '#' || l_stmt_id || ': ' ||
                                 'Unable to collect OPM WIP completions data in initial load into staging table.');
    ...
    l_stmt_id := 100;
    compute_wip_comp_conv_rates (errbuf, retcode, s_opi_schema);
    ...
    BIS_COLLECTION_UTILITIES.PUT_LINE
        ('WIP Completions Initial load terminated successfully.');
    retcode := s_SUCCESS;
    errbuf := '';
    return;
...
EXCEPTION
    ...
    WHEN schema_info_not_found THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion ETL Initial Load failed to get OPI schema info.';
        return;
    ...
    WHEN global_setup_missing THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completions Initial Load could not find global setup of global start date and global currency code.';
        return;
    ...
    WHEN data_extraction_failed THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion Initial Load data extraction failed for OPI, OPM or both. Check previous messages for errors. ';
        return;
    ...
    WHEN OTHERS THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion ETLs Initial load Failed.';
        return;


END collect_wip_completions_init;


/*  collect_init_opi_wip_comp

    Initial collection of WIP completion data from MMT/MTA for discrete (OPI)
    organizations.
...
EXCEPTION
    ...
    WHEN cannot_run_initial_load THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Completion initial load concurrent program should not be running. Try running the incremental load request set if the initial request set has already been run.');
        BIS_COLLECTION_UTILITIES.PUT_LINE ('If not, you will need to run the initial load request set.');

        retcode := s_ERROR;
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion initial load concurrent program should not be running. Try running the incremental load request set if the initial request set has already been run. If not, you will need to run the initial load request set.';
        RAISE;  -- propagate exception to wrapper.
    ...
    WHEN run_common_module THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Completion initial load concurrent program is running out of turn. Please submit the initial load request set for initial data collection.');
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion ETLs Initial merge to fact table failed.';
        RAISE;  -- propagate exception to wrapper.
...
/*++++++++++++++++++++++++++++++++++++++++*/
/* Utilities for initial and incremental load
/*++++++++++++++++++++++++++++++++++++++++*/

/*  compute_wip_comp_conv_rates
    ...
    OPI_DBI_WIP_COMP_STG using the fii_currency.get_global_rate_primary
...
IS
    ...
    l_global_currency_code VARCHAR2 (10);
...
BEGIN
    ...
    -- If all rates not found, then raise an exception
    l_stmt_id := 50;
...
EXCEPTION
    ...
    WHEN global_curr_code_not_found THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Completion ETLs global currency code not found.');
    ...
    WHEN global_rate_type_not_found THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Completions ETL Initial load global rate type not found.');
    ...
    WHEN missing_conversion_rates THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Completion ETLs Initial Load found missing currency rates.');

        retcode := s_ERROR;
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion ETLs found missing currency rates.';
    ...
    WHEN OTHERS THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Completion ETLs conversion rate computation failed.';
        RAISE;  -- propagate exception to wrapper.

END compute_wip_comp_conv_rates;
