--------------------------------------------------------
--  DDL for Package Body BEN_DCR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_DCR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_dcr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_dpnt_cvg_rqd_rlshp_f',
	   ...
	   p_base_key_value	=> p_rec.dpnt_cvg_rqd_rlshp_id);
    --
    ben_dcr_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_dpnt_cvg_rqd_rlshp_f Row
    --
    ...
    --
    ben_dcr_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_dcr_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.dpnt_cvg_rqd_rlshp_id,
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'ben_dpnt_cvg_rqd_rlshp_f'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_dcr_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    ben_dcr_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.per_relshp_typ_cd = hr_api.g_varchar2) then
    p_rec.per_relshp_typ_cd :=
    ben_dcr_shd.g_old_rec.per_relshp_typ_cd;
  End If;
  If (p_rec.cvg_strt_dt_cd = hr_api.g_varchar2) then
    p_rec.cvg_strt_dt_cd :=
    ben_dcr_shd.g_old_rec.cvg_strt_dt_cd;
  End If;
  If (p_rec.cvg_thru_dt_rl = hr_api.g_number) then
    p_rec.cvg_thru_dt_rl :=
    ben_dcr_shd.g_old_rec.cvg_thru_dt_rl;
  End If;
  If (p_rec.cvg_thru_dt_cd = hr_api.g_varchar2) then
    p_rec.cvg_thru_dt_cd :=
    ben_dcr_shd.g_old_rec.cvg_thru_dt_cd;
  End If;
  If (p_rec.cvg_strt_dt_rl = hr_api.g_number) then
    p_rec.cvg_strt_dt_rl :=
    ben_dcr_shd.g_old_rec.cvg_strt_dt_rl;
  End If;
  If (p_rec.dpnt_cvg_eligy_prfl_id = hr_api.g_number) then
    p_rec.dpnt_cvg_eligy_prfl_id :=
    ben_dcr_shd.g_old_rec.dpnt_cvg_eligy_prfl_id;
  End If;
  If (p_rec.dcr_attribute_category = hr_api.g_varchar2) then
    p_rec.dcr_attribute_category :=
    ben_dcr_shd.g_old_rec.dcr_attribute_category;
  End If;
  If (p_rec.dcr_attribute1 = hr_api.g_varchar2) then
    p_rec.dcr_attribute1 :=
    ben_dcr_shd.g_old_rec.dcr_attribute1;
  End If;
  If (p_rec.dcr_attribute2 = hr_api.g_varchar2) then
    p_rec.dcr_attribute2 :=
    ben_dcr_shd.g_old_rec.dcr_attribute2;
  End If;
  If (p_rec.dcr_attribute3 = hr_api.g_varchar2) then
    p_rec.dcr_attribute3 :=
    ben_dcr_shd.g_old_rec.dcr_attribute3;
  End If;
  If (p_rec.dcr_attribute4 = hr_api.g_varchar2) then
    p_rec.dcr_attribute4 :=
    ben_dcr_shd.g_old_rec.dcr_attribute4;
  End If;
  If (p_rec.dcr_attribute5 = hr_api.g_varchar2) then
    p_rec.dcr_attribute5 :=
    ben_dcr_shd.g_old_rec.dcr_attribute5;
  End If;
  If (p_rec.dcr_attribute6 = hr_api.g_varchar2) then
    p_rec.dcr_attribute6 :=
    ben_dcr_shd.g_old_rec.dcr_attribute6;
  End If;
  If (p_rec.dcr_attribute7 = hr_api.g_varchar2) then
    p_rec.dcr_attribute7 :=
    ben_dcr_shd.g_old_rec.dcr_attribute7;
  End If;
  If (p_rec.dcr_attribute8 = hr_api.g_varchar2) then
    p_rec.dcr_attribute8 :=
    ben_dcr_shd.g_old_rec.dcr_attribute8;
  End If;
  If (p_rec.dcr_attribute9 = hr_api.g_varchar2) then
    p_rec.dcr_attribute9 :=
    ben_dcr_shd.g_old_rec.dcr_attribute9;
  End If;
  If (p_rec.dcr_attribute10 = hr_api.g_varchar2) then
    p_rec.dcr_attribute10 :=
    ben_dcr_shd.g_old_rec.dcr_attribute10;
  End If;
  If (p_rec.dcr_attribute11 = hr_api.g_varchar2) then
    p_rec.dcr_attribute11 :=
    ben_dcr_shd.g_old_rec.dcr_attribute11;
  End If;
  If (p_rec.dcr_attribute12 = hr_api.g_varchar2) then
    p_rec.dcr_attribute12 :=
    ben_dcr_shd.g_old_rec.dcr_attribute12;
  End If;
  If (p_rec.dcr_attribute13 = hr_api.g_varchar2) then
    p_rec.dcr_attribute13 :=
    ben_dcr_shd.g_old_rec.dcr_attribute13;
  End If;
  If (p_rec.dcr_attribute14 = hr_api.g_varchar2) then
    p_rec.dcr_attribute14 :=
    ben_dcr_shd.g_old_rec.dcr_attribute14;
  End If;
  If (p_rec.dcr_attribute15 = hr_api.g_varchar2) then
    p_rec.dcr_attribute15 :=
    ben_dcr_shd.g_old_rec.dcr_attribute15;
  End If;
  If (p_rec.dcr_attribute16 = hr_api.g_varchar2) then
    p_rec.dcr_attribute16 :=
    ben_dcr_shd.g_old_rec.dcr_attribute16;
  End If;
  If (p_rec.dcr_attribute17 = hr_api.g_varchar2) then
    p_rec.dcr_attribute17 :=
    ben_dcr_shd.g_old_rec.dcr_attribute17;
  End If;
  If (p_rec.dcr_attribute18 = hr_api.g_varchar2) then
    p_rec.dcr_attribute18 :=
    ben_dcr_shd.g_old_rec.dcr_attribute18;
  End If;
  If (p_rec.dcr_attribute19 = hr_api.g_varchar2) then
    p_rec.dcr_attribute19 :=
    ben_dcr_shd.g_old_rec.dcr_attribute19;
  End If;
  If (p_rec.dcr_attribute20 = hr_api.g_varchar2) then
    p_rec.dcr_attribute20 :=
    ben_dcr_shd.g_old_rec.dcr_attribute20;
  End If;
  If (p_rec.dcr_attribute21 = hr_api.g_varchar2) then
    p_rec.dcr_attribute21 :=
    ben_dcr_shd.g_old_rec.dcr_attribute21;
  End If;
  If (p_rec.dcr_attribute22 = hr_api.g_varchar2) then
    p_rec.dcr_attribute22 :=
    ben_dcr_shd.g_old_rec.dcr_attribute22;
  End If;
  If (p_rec.dcr_attribute23 = hr_api.g_varchar2) then
    p_rec.dcr_attribute23 :=
    ben_dcr_shd.g_old_rec.dcr_attribute23;
  End If;
  If (p_rec.dcr_attribute24 = hr_api.g_varchar2) then
    p_rec.dcr_attribute24 :=
    ben_dcr_shd.g_old_rec.dcr_attribute24;
  End If;
  If (p_rec.dcr_attribute25 = hr_api.g_varchar2) then
    p_rec.dcr_attribute25 :=
    ben_dcr_shd.g_old_rec.dcr_attribute25;
  End If;
  If (p_rec.dcr_attribute26 = hr_api.g_varchar2) then
    p_rec.dcr_attribute26 :=
    ben_dcr_shd.g_old_rec.dcr_attribute26;
  End If;
  If (p_rec.dcr_attribute27 = hr_api.g_varchar2) then
    p_rec.dcr_attribute27 :=
    ben_dcr_shd.g_old_rec.dcr_attribute27;
  End If;
  If (p_rec.dcr_attribute28 = hr_api.g_varchar2) then
    p_rec.dcr_attribute28 :=
    ben_dcr_shd.g_old_rec.dcr_attribute28;
  End If;
  If (p_rec.dcr_attribute29 = hr_api.g_varchar2) then
    p_rec.dcr_attribute29 :=
    ben_dcr_shd.g_old_rec.dcr_attribute29;
  End If;
  If (p_rec.dcr_attribute30 = hr_api.g_varchar2) then
    p_rec.dcr_attribute30 :=
    ben_dcr_shd.g_old_rec.dcr_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_dcr_shd.convert_args
  (
  p_dpnt_cvg_rqd_rlshp_id,
  null,
  null,
  p_business_group_id,
  p_per_relshp_typ_cd,
