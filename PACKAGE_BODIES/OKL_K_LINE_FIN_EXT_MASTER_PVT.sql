--------------------------------------------------------
--  DDL for Package Body OKL_K_LINE_FIN_EXT_MASTER_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_K_LINE_FIN_EXT_MASTER_PVT" AS
/* $Header: OKLRLEMB.pls 120.0.12010000.9 2009/09/25 22:05:23 sechawla noship $ */
 ...
 |
 | DESCRIPTION
 |    This procedure identifies contracts for Contract Line Financial Report, based
...
begin
  ...
  if l_req_data is not null and l_req_data = '1' then
    ...
    p_ret_add_layout := FND_REQUEST.add_layout(	template_appl_name=>'OKL',
						template_code=> P_REPORT_TEMPLATE_NAME, --'OKLLNFINEXT',
						template_language=>P_REPORT_LANGUAGE, --'en',
						template_territory=>'00',
						output_format=>P_REPORT_FORMAT); --'RTF');
    ...
    report_request_id := FND_REQUEST.SUBMIT_REQUEST(
                          ...
                          argument4   =>	P_BOOK_CLASS,
                          argument5   =>	P_LEASE_PRODUCT,
                          argument6   =>	P_CONTRACT_NUMBER,
                          argument7  =>	P_CONTRACT_STATUS,
                          argument8   =>	P_CONTRACT_LINE_STATUS,
                          argument9   =>	P_CONTRACT_LINE_TYPE,
                          argument10  =>	P_CUSTOMER_NAME,
                          argument11   =>	P_CUSTOMER_NUMBER,
                          argument12   =>	P_VENDOR_NAME,
                          argument13   =>	P_VENDOR_NUMBER,
                          argument14   =>	P_FA_INFO_YN,
                          argument15   =>	P_TAX_BOOK,
                          argument16   =>	P_DELETE_DATA_YN
                         );
...
--	write_to_log('P_TEMPLATE_CODE ='|| P_TEMPLATE_CODE); --sechawla 7628379
                ...
                -- l_req_counter = number of workers used ?
          		...
          		if l_req_counter < (p_num_processes+1) then
          			...
          			request_id := FND_REQUEST.SUBMIT_REQUEST(
                          application => 'OKL',
                          program     => 'OKL_K_LINE_FIN_REP_CHILD', --short name for child conc program
                          sub_request => TRUE,
                          argument1   => P_OPERATING_UNIT,
                          --argument2   => P_REPORT_DATE, sechawla 25-sep-09 8890513
                          argument2   => P_START_DATE_FROM,
                          ...
                          argument15   => P_TAX_BOOK,
                          argument16   => P_DELETE_DATA_YN,
             		  argument17  => P_NUM_PROCESSES,
             		  argument18  => l_seq_next||'-'||j
                         );
