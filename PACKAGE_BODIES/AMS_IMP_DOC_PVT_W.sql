--------------------------------------------------------
--  DDL for Package Body AMS_IMP_DOC_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMS_IMP_DOC_PVT_W" as
  /* $Header: amswidob.pls 115.3 2002/11/12 23:44:30 jieli noship $ */
  ...
  procedure rosetta_table_copy_in_p2(t OUT NOCOPY ams_imp_doc_pvt.imp_doc_tbl_type, a0 JTF_NUMBER_TABLE
    , a1 JTF_NUMBER_TABLE
    , a2 JTF_NUMBER_TABLE
    , a3 JTF_NUMBER_TABLE
    , a4 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).imp_document_id := rosetta_g_miss_num_map(a0(indx));
          ...
          t(ddindx).file_type := a8(indx);
          t(ddindx).file_size := rosetta_g_miss_num_map(a9(indx));
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  procedure rosetta_table_copy_out_p2(t ams_imp_doc_pvt.imp_doc_tbl_type, a0 OUT NOCOPY JTF_NUMBER_TABLE
  ...
  if t is null or t.count = 0 then
    ...
    a9 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := rosetta_g_miss_num_map(t(ddindx).imp_document_id);
          ...
          a8(indx) := t(ddindx).file_type;
          a9(indx) := rosetta_g_miss_num_map(t(ddindx).file_size);
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  procedure create_imp_doc(p_api_version_number  NUMBER
  ...
  as
    ddp_imp_doc_rec ams_imp_doc_pvt.imp_doc_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ddp_imp_doc_rec.imp_document_id := rosetta_g_miss_num_map(p7_a0);
    ...
    ddp_imp_doc_rec.file_type := p7_a8;
    ddp_imp_doc_rec.file_size := rosetta_g_miss_num_map(p7_a9);


    -- here's the delegated call to the old PL/SQL routine
    ams_imp_doc_pvt.create_imp_doc(p_api_version_number,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      x_return_status,
  ...
  procedure update_imp_doc(p_api_version_number  NUMBER
    , p_init_msg_list  VARCHAR2
    , p_commit  VARCHAR2
    , p_validation_level  NUMBER
    , x_return_status OUT NOCOPY  VARCHAR2
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ams_imp_doc_pvt.update_imp_doc(p_api_version_number,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      x_return_status,
  ...
  begin
    ...
    ddp_imp_doc_rec.imp_document_id := rosetta_g_miss_num_map(p3_a0);
    ...
    ddp_imp_doc_rec.file_type := p3_a8;
    ddp_imp_doc_rec.file_size := rosetta_g_miss_num_map(p3_a9);
    ...
    -- here's the delegated call to the old PL/SQL routine
    ams_imp_doc_pvt.validate_imp_doc(p_api_version_number,
      p_init_msg_list,
      p_validation_level,
      ddp_imp_doc_rec,
      p_validation_mode,
  ...
  begin
    ...
    -- copy data to the local IN or IN-OUT args, if any
    ddp_imp_doc_rec.imp_document_id := rosetta_g_miss_num_map(p0_a0);
    ...
    ddp_imp_doc_rec.file_type := p0_a8;
    ddp_imp_doc_rec.file_size := rosetta_g_miss_num_map(p0_a9);



    -- here's the delegated call to the old PL/SQL routine
    ams_imp_doc_pvt.check_imp_doc_items(ddp_imp_doc_rec,
      p_validation_mode,
      x_return_status);
  ...
  begin
    ...
    ddp_imp_doc_rec.imp_document_id := rosetta_g_miss_num_map(p5_a0);
    ...
    ddp_imp_doc_rec.file_type := p5_a8;
    ddp_imp_doc_rec.file_size := rosetta_g_miss_num_map(p5_a9);

    -- here's the delegated call to the old PL/SQL routine
    ams_imp_doc_pvt.validate_imp_doc_rec(p_api_version_number,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
