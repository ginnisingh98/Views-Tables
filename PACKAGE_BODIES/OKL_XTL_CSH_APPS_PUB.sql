--------------------------------------------------------
--  DDL for Package Body OKL_XTL_CSH_APPS_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_XTL_CSH_APPS_PUB" AS
/* $Header: OKLPXCAB.pls 115.8 2004/04/13 11:27:03 rnaik noship $ */
...
lp_xcav_rec  xcav_rec_type;
lx_xcav_rec  xcav_rec_type;
...
lp_xcav_rec :=  p_xcav_rec;
lx_xcav_rec :=  x_xcav_rec;
...
lp_xcav_rec := lx_xcav_rec;




--Assign value to OUT variables
x_xcav_rec  := lp_xcav_rec;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
      x_return_status := FND_API.G_RET_STS_UNEXP_ERROR;
      FND_MSG_PUB.ADD_EXC_MSG('OKL_XTL_CSH_APPS_PUB','lock_xtl_csh_apps');
      FND_MSG_PUB.count_and_get(
             p_count   => x_msg_count
            ,p_data    => x_msg_data);
END lock_xtl_csh_apps;
...
SAVEPOINT xtl_csh_apps_validate;
...
EXCEPTION
    ...
    WHEN FND_API.G_EXC_ERROR THEN
      ROLLBACK TO xtl_csh_apps_validate;
      x_return_status := FND_API.G_RET_STS_ERROR;
      x_msg_count := l_msg_count ;
      x_msg_data := l_msg_data ;
      FND_MSG_PUB.count_and_get(
    ...
    WHEN OTHERS THEN
      ...
      x_msg_count := l_msg_count ;
      x_msg_data := l_msg_data ;
      FND_MSG_PUB.ADD_EXC_MSG('OKL_XTL_CSH_APPS_PUB','validate_xtl_csh_apps');
      FND_MSG_PUB.count_and_get(
             p_count   => x_msg_count
            ,p_data    => x_msg_data);
END validate_xtl_csh_apps;
