--------------------------------------------------------
--  DDL for Package Body BEN_CPL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CPL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_cpl_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_cmbn_plip_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   cmbn_plip_id PK of record being inserted or updated.
...
--
Procedure chk_cmbn_plip_id(p_cmbn_plip_id                in number,
...
Begin
  ...
  --
  l_api_updating := ben_cpl_shd.api_updating
  ...
  if (l_api_updating
     ...
     <>  ben_cpl_shd.g_old_rec.cmbn_plip_id) then
    --
    -- raise error as PK has changed
    --
    ben_cpl_shd.constraint_error('BEN_CMBN_PLIP_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_cmbn_plip_id is not null then
      --
...
function return_legislation_code
  (p_cmbn_plip_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_cmbn_plip_f b
    where b.cmbn_plip_id      = p_cmbn_plip_id
