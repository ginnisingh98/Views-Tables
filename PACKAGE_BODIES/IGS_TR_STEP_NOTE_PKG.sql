--------------------------------------------------------
--  DDL for Package Body IGS_TR_STEP_NOTE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_TR_STEP_NOTE_PKG" AS
/* $Header: IGSTI04B.pls 115.4 2002/11/29 04:14:34 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_tr_step_note%ROWTYPE;
  new_references igs_tr_step_note%ROWTYPE;
  ...
  BEGIN
    ...
    IF (((old_references.tracking_id = new_references.tracking_id) AND
         (old_references.tracking_step_id = new_references.tracking_step_id)) OR
        ((new_references.tracking_id IS NULL) OR
         (new_references.tracking_step_id IS NULL))) THEN
      NULL;
    ELSE
      IF NOT igs_tr_step_pkg.get_pk_for_validation (
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_TR_TSN_NOTE_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_TR_TSN_TNT_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_TR_TSN_TRST_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
