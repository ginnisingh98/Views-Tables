--------------------------------------------------------
--  DDL for Package Body BEN_VPR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_VPR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_vpr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_vrbl_rt_prfl_rl_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   vrbl_rt_prfl_rl_id PK of record being inserted or updated.
...
--
Procedure chk_vrbl_rt_prfl_rl_id(p_vrbl_rt_prfl_rl_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_vrbl_rt_prfl_rl_id';
...
Begin
  ...
  --
  l_api_updating := ben_vpr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_vrbl_rt_prfl_rl_id                => p_vrbl_rt_prfl_rl_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_vrbl_rt_prfl_rl_id,hr_api.g_number)
     <>  ben_vpr_shd.g_old_rec.vrbl_rt_prfl_rl_id) then
    --
    -- raise error as PK has changed
    --
    ben_vpr_shd.constraint_error('BEN_VARBL_RT_PRFL_RL_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_vrbl_rt_prfl_rl_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_drvbl_fctr_apls_flag(p_vrbl_rt_prfl_rl_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_vpr_shd.g_old_rec.drvbl_fctr_apls_flag,hr_api.g_varchar2)
...
--
-- |--------------------< chk_duplicate_ordr_to_aply_num >----------------------------|
-- ----------------------------------------------------------------------------
--
-- Description
--
...
--
-- ----------------------------------------------------------------------------
procedure chk_duplicate_ordr_to_aply_num
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and vrbl_rt_prfl_rl_id <> nvl(p_vrbl_rt_prfl_rl_id,-1)
...
End chk_duplicate_ordr_to_aply_num;
...
--   Internal table handler use only.
--
Procedure chk_formula_id(p_vrbl_rt_prfl_rl_id              in number,
...
--
-- In Parameters
--   p_vrbl_rt_prfl_rl_id    PK of record being inserted or updated.
--   p_formula_id   	     Value of FK.
...
and vrbl_rt_prfl_rl_id <> nvl(p_vrbl_rt_prfl_rl_id,hr_api.g_number)
and business_group_id + 0 = p_business_group_id
...
--
  ...
  --
  if (l_api_updating
      and p_formula_id <> nvl(ben_vpr_shd.g_old_rec.formula_id,hr_api.g_number)
      or not l_api_updating) then
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Rule criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Rule',TRUE);
...
function return_legislation_code
  (p_vrbl_rt_prfl_rl_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_vrbl_rt_prfl_rl_f b
    where b.vrbl_rt_prfl_rl_id      = p_vrbl_rt_prfl_rl_id
