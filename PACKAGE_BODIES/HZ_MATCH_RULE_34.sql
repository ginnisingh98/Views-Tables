--------------------------------------------------------
--  DDL for Package Body HZ_MATCH_RULE_34
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_MATCH_RULE_34" AS
/*=======================================================================+
...
END;
  ...
  FUNCTION GET_PARTY_SCORE 
      ...
      ,p_table_TX43 VARCHAR2
      ,p_table_TX42 VARCHAR2
      ,p_table_TX44 VARCHAR2
      ,p_table_TX46 VARCHAR2
      ,p_table_TX47 VARCHAR2
      ,p_table_TX48 VARCHAR2
      ,p_table_TX156 VARCHAR2
      ,p_table_TX157 VARCHAR2
  ) RETURN NUMBER IS
    total NUMBER := 0;
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      RETURN total;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX32, p_table_TX32,36) THEN
      total := total+60;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      END IF;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX35, p_table_TX35,39) THEN
      total := total+100;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      END IF;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX42, p_table_TX42,46) THEN
      total := total+30;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
        RETURN total;
      END IF;
    ELSIF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX43, p_table_TX43,47) THEN
      total := total+24;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      END IF;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX48, p_table_TX48,53) THEN
      total := total+10;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ...
      END IF;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX156, p_table_TX156,121) THEN
      total := total+20;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ...
      END IF;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX157, p_table_TX157,122) THEN
      total := total+30;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
  ...
  FUNCTION GET_PARTY_SITES_SCORE (
      ...
      ,p_table_TX13 VARCHAR2
      ,p_table_TX14 VARCHAR2
      ,p_table_TX15 VARCHAR2
      ,p_table_TX20 VARCHAR2
      ,p_table_TX21 VARCHAR2
  ...
  BEGIN
    ...
    FOR J IN 1..g_party_site_stage_list.COUNT LOOP
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX9, p_table_TX9,(50000*(J-1)+72)) THEN
        l_current_score:=l_current_score+ 30;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
      ELSIF -- l_current_score<24 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX10, p_table_TX10,(50000*(J-1)+73)) THEN
        l_current_score:=l_current_score+ 24;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX11, p_table_TX11,(50000*(J-1)+74)) THEN
        l_current_score:=l_current_score+ 35;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX13, p_table_TX13,(50000*(J-1)+76)) THEN
        l_current_score:=l_current_score+ 14;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
      ...
      ELSIF -- l_current_score<16 AND 
        ...
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX20, p_table_TX20,(50000*(J-1)+83)) THEN
        l_current_score:=l_current_score+ 20;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
      ELSIF -- l_current_score<16 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX21, p_table_TX21,(50000*(J-1)+84)) THEN
        l_current_score:=l_current_score+ 16;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
  ...
  BEGIN
    ...
    FOR J IN 1..g_contact_pt_stage_list.COUNT LOOP
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX1, p_table_TX1,(50000*(J-1)+110)) THEN
        ...
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX3, p_table_TX3,(50000*(J-1)+112)) THEN
        l_current_score:=l_current_score+ 30;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX4, p_table_TX4,(50000*(J-1)+113)) THEN
  ...
  IF p_attribute = 'ALL_ACCOUNT_NAMES' THEN
     ...
     l_matched_value := HZ_PARTY_ACQUIRE.get_account_info (p_record_id, p_entity,'ALL_ACCOUNT_NAMES','Y');
  ...
  IF p_attribute = 'PARTY_NUMBER' THEN
      ...
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'ALL_ACCOUNT_NUMBERS' THEN

     l_matched_value := HZ_PARTY_ACQUIRE.get_account_info (p_record_id, p_entity,'ALL_ACCOUNT_NUMBERS','Y');
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'PARTY_ALL_NAMES' THEN
     ...
     l_matched_value := HZ_PARTY_ACQUIRE.get_known_as (p_record_id, p_entity,'PARTY_ALL_NAMES','Y');
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'TAX_NAME' THEN

      SELECT party_type INTO l_party_type 
      FROM HZ_PARTIES
      WHERE party_id = p_record_id;
      IF l_party_type = 'ORGANIZATION' THEN
        SELECT TAX_NAME INTO l_matched_value 
  ...
  IF p_attribute = 'SIC_CODE_TYPE' THEN
      ...
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'CATEGORY_CODE' THEN

      SELECT party_type INTO l_party_type 
      FROM HZ_PARTIES
      WHERE party_id = p_record_id;
      IF l_party_type = 'ORGANIZATION' THEN
        SELECT CATEGORY_CODE INTO l_matched_value 
        FROM HZ_PARTIES
        WHERE party_id = p_record_id 
        and rownum = 1;
      ELSIF l_party_type = 'PERSON' THEN
      ...
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'REFERENCE_USE_FLAG' THEN

      SELECT party_type INTO l_party_type 
      FROM HZ_PARTIES
      WHERE party_id = p_record_id;
      IF l_party_type = 'ORGANIZATION' THEN
        SELECT REFERENCE_USE_FLAG INTO l_matched_value 
        FROM HZ_PARTIES
        WHERE party_id = p_record_id 
        and rownum = 1;
      ELSIF l_party_type = 'PERSON' THEN
      ...
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'CORPORATION_CLASS' THEN

      SELECT party_type INTO l_party_type 
      FROM HZ_PARTIES
      WHERE party_id = p_record_id;
      IF l_party_type = 'ORGANIZATION' THEN
        SELECT CORPORATION_CLASS INTO l_matched_value 
        FROM HZ_ORGANIZATION_PROFILES
        WHERE party_id = p_record_id 
        and effective_end_date is null
        and rownum = 1;
  ...
  IF p_attribute = 'POSTAL_CODE' THEN
     ...
     END;
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'PROVINCE' THEN
     BEGIN
       SELECT HZ_LOCATIONS.PROVINCE
       INTO l_matched_value 
       FROM HZ_PARTY_SITES, HZ_LOCATIONS
       WHERE HZ_PARTY_SITES.party_site_id = p_record_id
       AND HZ_PARTY_SITES.location_id = HZ_LOCATIONS.location_id and rownum=1;
  ...
  IF p_attribute = 'STATE' THEN
     ...
     END;
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'COUNTY' THEN
     BEGIN
       SELECT HZ_LOCATIONS.COUNTY
       INTO l_matched_value 
       FROM HZ_PARTY_SITES, HZ_LOCATIONS
       WHERE HZ_PARTY_SITES.party_site_id = p_record_id
       AND HZ_PARTY_SITES.location_id = HZ_LOCATIONS.location_id and rownum=1;
  ...
  IF p_attribute = 'FLEX_FORMAT_PHONE_NUMBER' THEN
     l_matched_value := HZ_PARTY_ACQUIRE.get_phone_number (p_record_id, p_entity,'FLEX_FORMAT_PHONE_NUMBER','Y');
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'PHONE_AREA_CODE' THEN
     BEGIN
       SELECT PHONE_AREA_CODE INTO l_matched_value 
     ...
     END;
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'PHONE_COUNTRY_CODE' THEN
     BEGIN
       SELECT PHONE_COUNTRY_CODE INTO l_matched_value 
       FROM HZ_CONTACT_POINTS 
       WHERE contact_point_id = p_record_id and rownum=1;
     EXCEPTION
       WHEN NO_DATA_FOUND THEN
...
END;
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX32, p_table_TX32,(50000*(p_idx-1)+36)) THEN
      l_score :=60;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','ALL_ACCOUNT_NAMES');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX35, p_table_TX35,(50000*(p_idx-1)+39)) THEN
      l_score :=100;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','ALL_ACCOUNT_NUMBERS');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX42, p_table_TX42,(50000*(p_idx-1)+46)) THEN
      l_score :=30;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ELSIF l_current_score<24 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX43, p_table_TX43,(50000*(p_idx-1)+47)) THEN
      l_score :=24;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','TAX_NAME');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'TAX_NAME',
           'PARTY', p_search_rec.TAX_NAME,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX48, p_table_TX48,(50000*(p_idx-1)+53)) THEN
      l_score :=10;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','CATEGORY_CODE');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'CATEGORY_CODE',
           'PARTY', p_search_rec.CATEGORY_CODE,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX156, p_table_TX156,(50000*(p_idx-1)+121)) THEN
      l_score :=20;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','REFERENCE_USE_FLAG');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'REFERENCE_USE_FLAG',
           'PARTY', p_search_rec.REFERENCE_USE_FLAG,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX157, p_table_TX157,(50000*(p_idx-1)+122)) THEN
      l_score :=30;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','CORPORATION_CLASS');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'CORPORATION_CLASS',
           'PARTY', p_search_rec.CORPORATION_CLASS,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX13, p_table_TX13,(50000*(p_idx-1)+76)) THEN
      l_score :=14;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY_SITES','PROVINCE');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'PROVINCE',
           'PARTY_SITES', p_search_rec.PROVINCE,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX20, p_table_TX20,(50000*(p_idx-1)+83)) THEN
      l_score :=20;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ELSIF l_current_score<16 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX21, p_table_TX21,(50000*(p_idx-1)+84)) THEN
      l_score :=16;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY_SITES','COUNTY');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'COUNTY',
           'PARTY_SITES', p_search_rec.COUNTY,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX3, p_table_TX3,(50000*(p_idx-1)+112)) THEN
      l_score :=30;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'CONTACT_POINTS','PHONE_AREA_CODE');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'PHONE_AREA_CODE',
           'CONTACT_POINTS', p_search_rec.PHONE_AREA_CODE,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX4, p_table_TX4,(50000*(p_idx-1)+113)) THEN
      l_score :=10;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'CONTACT_POINTS','PHONE_COUNTRY_CODE');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'PHONE_COUNTRY_CODE',
           'CONTACT_POINTS', p_search_rec.PHONE_COUNTRY_CODE,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
...
POP_CONTACT_POINTS_COND_REC(p_contact_point_list);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      threshold := round((72/805)*(l_entered_max_score+l_ps_entered_max_score+l_ct_entered_max_score+l_cpt_entered_max_score));
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
  ...
  IF l_denorm_str IS NOT NULL THEN
    IF (p_non_denorm_max_score+10)>=p_threshold THEN
      l_denorm_str := NULL;
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, p_match_str, l_party_contains_str);
    ELSIF (p_non_denorm_max_score+p_denorm_max_score+10)>=p_threshold THEN
  ...
  END IF;
    ...
    -- Setup query string for CATEGORY_CODE
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
          g_party_stage_rec.TX48,'A22',l_prim_temp);
  ...
  END IF;
    ...
    -- Setup query string for REFERENCE_USE_FLAG
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
          g_party_stage_rec.TX156,'A53',l_prim_temp);

  IF l_denorm_str IS NOT NULL THEN
    IF (p_non_denorm_max_score+20)>=p_threshold THEN
      l_denorm_str := NULL;
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, p_match_str, l_party_contains_str);
    ELSIF (p_non_denorm_max_score+p_denorm_max_score+20)>=p_threshold THEN
      HZ_DQM_SEARCH_UTIL.add_attribute_with_denorm(l_prim_temp, p_match_str, l_denorm_str, l_party_contains_str);
      l_denorm_str := NULL;
    END IF;
  ELSE
  ...
  END IF;
    ...
    -- Setup query string for TAX_NAME
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE
          g_party_stage_rec.TX43,'A17',l_prim_temp);

  IF l_denorm_str IS NOT NULL THEN
    IF (p_non_denorm_max_score+30)>=p_threshold THEN
      l_denorm_str := NULL;
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, p_match_str, l_party_contains_str);
    ELSIF (p_non_denorm_max_score+p_denorm_max_score+30)>=p_threshold THEN
      HZ_DQM_SEARCH_UTIL.add_attribute_with_denorm(l_prim_temp, p_match_str, l_denorm_str, l_party_contains_str);
      l_denorm_str := NULL;
    END IF;
  ELSE
  ...
  END IF;
    ...
    -- Setup query string for CORPORATION_CLASS
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
          g_party_stage_rec.TX157,'A54',l_prim_temp);
  ...
  END IF;
    ...
    -- Setup query string for ALL_ACCOUNT_NAMES
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE SPECIAL + WRNAMES
          g_party_stage_rec.TX33,'A11',l_prim_temp);
  ...
  IF l_denorm_str IS NOT NULL THEN
    IF (p_non_denorm_max_score+80)>=p_threshold THEN
      l_denorm_str := NULL;
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, p_match_str, l_party_contains_str);
    ELSIF (p_non_denorm_max_score+p_denorm_max_score+80)>=p_threshold THEN
      HZ_DQM_SEARCH_UTIL.add_attribute_with_denorm(l_prim_temp, p_match_str, l_denorm_str, l_party_contains_str);
      l_denorm_str := NULL;
    END IF;
  ELSE
  ...
  END IF;
    ...
    -- Setup query string for ALL_ACCOUNT_NUMBERS
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT SPECIAL
          g_party_stage_rec.TX35,'A13',l_prim_temp);
  ...
  BEGIN
    ...
    FOR I IN 1..g_party_site_stage_list.COUNT LOOP
      ...
      -- Setup query string for PROVINCE
      l_prim_temp := null;
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE
            g_party_site_stage_list(I).TX13,'A31',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE
            g_party_site_stage_list(I).TX13,'D_PS',l_prim_temp_den);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp_den, ' AND ', l_den_contains_str_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
      ...
      -- Setup query string for COUNTY
      l_prim_temp := null;
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE
            g_party_site_stage_list(I).TX21,'A35',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE
            g_party_site_stage_list(I).TX21,'D_PS',l_prim_temp_den);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp_den, ' AND ', l_den_contains_str_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
...
END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     OPEN x_cursor FOR 
      SELECT PARTY_ID , TX32, TX34, TX35, TX39, TX158, TX40, TX43, TX42, TX44, TX46, TX47, TX48, TX156, TX157
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      l_sqlstr := 'SELECT PARTY_ID , TX32, TX34, TX35, TX39, TX158, TX40, TX43, TX42, TX44, TX46, TX47, TX48, TX156, TX157 FROM hz_staged_parties stage ';
      ...
      l_sqlstr := l_sqlstr || '     (TX33 like :TX33||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX34 like :TX34||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX35 like :TX35||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX158 like :TX158||'' %'' ';
      l_sqlstr := l_sqlstr || '      OR TX40 like :TX40||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX43 like :TX43||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
     ...
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX48 like :TX48||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX156 like :TX156||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX157 like :TX157||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
     l_sqlstr := l_sqlstr || ' ) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     ...
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX13||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX15||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX21||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     ...
     ,g_party_stage_rec.TX47
     ,g_party_stage_rec.TX48
     ,g_party_stage_rec.TX48
     ,g_party_stage_rec.TX156
     ,g_party_stage_rec.TX156
     ,g_party_stage_rec.TX157
     ,g_party_stage_rec.TX157
     ...
     ,g_party_site_stage_list(1).TX11
     ,g_party_site_stage_list(1).TX13
     ,g_party_site_stage_list(1).TX13
     ,g_party_site_stage_list(1).TX15
     ,g_party_site_stage_list(1).TX15
     ,g_party_site_stage_list(1).TX21
     ,g_party_site_stage_list(1).TX21
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGE_PARTIES_T1) */ PARTY_ID , TX32, TX34, TX35, TX39, TX158, TX40, TX43, TX42, TX44, TX46, TX47, TX48, TX156, TX157
          FROM HZ_STAGED_PARTIES stage
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_PARTIES_U1) */ stage.PARTY_ID , TX32, TX34, TX35, TX39, TX158, TX40, TX43, TX42, TX44, TX46, TX47, TX48, TX156, TX157
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
     IF p_search_ctx_id IS NULL THEN
       l_sqlstr := 'SELECT   ' || l_hint || ' PARTY_ID , TX32, TX34, TX35, TX39, TX158, TX40, TX43, TX42, TX44, TX46, TX47, TX48, TX156, TX157'||
                   ' FROM HZ_STAGED_PARTIES stage'||
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_sites = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID, TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, stage.PARTY_ID, stage.ORG_CONTACT_ID, TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID, TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22
          FROM HZ_STAGED_PARTY_SITES stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT  PARTY_SITE_ID, stage.PARTY_ID, ORG_CONTACT_ID, TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22
          ...
          OR (l_search_merged = 'N' AND nvl(stage.status_flag, 'A') = 'A')       )  
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, r.subject_id, stage.ORG_CONTACT_ID, TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
       l_sqlstr := 'SELECT  /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
         ...
         ' AND (ORG_CONTACT_ID IS NULL ' ||
       	...
       	' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
         ' UNION ' ||
       			 'SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
       ...
       ELSE 
       l_sqlstr := 'SELECT ' || l_hint ||' PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX9, TX10, TX11, TX13, TX14, TX15, TX20, TX21, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACTS_N1) */ ORG_CONTACT_ID, PARTY_ID
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT ORG_CONTACT_ID, PARTY_ID
          FROM HZ_STAGED_CONTACTS stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
          SELECT 1 FROM HZ_STAGED_PARTIES p
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
          OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_CONTACTS_N1) */ ORG_CONTACT_ID, stage.PARTY_ID
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       ELSE 
               l_status_sql := ' AND nvl(p.status,''A'') in (''A'')'; 
       END IF; 
       l_sqlstr := 'SELECT   ' || l_hint || ' ORG_CONTACT_ID, PARTY_ID '||
                   ' FROM HZ_STAGED_CONTACTS stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                 ' AND EXISTS ('||
                 ' SELECT 1 FROM HZ_STAGED_PARTIES p ' || 
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX3, TX4
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N3) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX3, TX4
          FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage 
          WHERE contains( concat_col, p_contains_str)>0 
          AND d.search_context_id = p_search_ctx_id 
          AND( (l_search_merged ='Y' )  
 ...
 THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_cpts = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX3, TX4
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ stage.CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX1, TX3, TX4
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX3, TX4
          FROM HZ_STAGED_CONTACT_POINTS stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
          SELECT 1 FROM HZ_STAGED_PARTIES p
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX3, TX4
            FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage
            WHERE contains( concat_col, p_contains_str)>0
            AND d.search_context_id = p_search_ctx_id
            AND d.party_id = stage.party_id
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, r.subject_id, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX1, TX3, TX4
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       ELSE
		p_restrict_sql1 := 'stage1.'||p_restrict_sql;
       END IF; 
       	l_sqlstr := ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX3, TX4'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ...
                   ' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
                   ' UNION ' ||
                   ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX3, TX4'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ' AND( ('''||l_search_merged||''' =''Y'' )  '||
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '||
       ...
       OPEN x_cursor FOR l_sqlstr USING p_contains_str,
                    p_dup_party_id, p_dup_party_id, p_contains_str, p_dup_party_id, p_dup_party_id;
       ELSE 
       	l_sqlstr := 'SELECT   ' || l_hint ||' CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX3, TX4'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                 ' AND EXISTS ('||
                 ' SELECT 1 FROM HZ_STAGED_PARTIES p ' || 
...
PROCEDURE find_parties (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_cur INTO
         l_party_id , l_TX32, l_TX34, l_TX35, l_TX39, l_TX158, l_TX40, l_TX43, l_TX42, l_TX44, l_TX46, l_TX47, l_TX48, l_TX156, l_TX157;
      EXIT WHEN l_party_cur%NOTFOUND;
      l_index := map_id(l_party_id);
  l_TX35_new:=RTRIM(LTRIM(l_TX35));
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
      l_score := GET_PARTY_SCORE(l_TX32,l_TX34,l_TX35_new,l_TX39,l_TX158,l_TX40,l_TX43,l_TX42,l_TX44,l_TX46,l_TX47,l_TX48,l_TX156,l_TX157);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_site_cur INTO
         l_party_site_id, l_ps_party_id, l_ps_contact_id , l_TX9, l_TX10, l_TX11, l_TX13, l_TX14, l_TX15, l_TX20, l_TX21, l_TX22;
      ...
      IF l_use_contact_addr_info OR l_ps_contact_id IS NULL THEN
        ...
        l_score := GET_PARTY_SITES_SCORE(l_match_idx,l_TX9,l_TX10,l_TX11,l_TX13,l_TX14,l_TX15,l_TX20,l_TX21,l_TX22);
        IF NOT H_SCORES.EXISTS(l_index) THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_contact_pt_cur INTO
         l_contact_pt_id,  l_cpt_type, l_cpt_party_id,  l_cpt_ps_id, l_cpt_contact_id , l_TX1, l_TX3, l_TX4;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      IF l_use_contact_cpt_info OR l_cpt_contact_id IS NULL THEN
        ...
        l_score := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX1,l_TX3,l_TX4);
        IF NOT H_SCORES.EXISTS(l_index) THEN
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    IF (p_index=5 AND call_order(5) <> 'NONE' AND H_PARTY_ID.COUNT=0) THEN
     l_threshold :=  round(( l_entered_max_score / 805) * 72); 
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
    l_party_contains_str := INIT_PARTY_QUERY(l_match_str, l_denorm_str, l_party_max_score, l_denorm_max_score, l_non_denorm_max_score, round((72/805)*l_entered_max_score));
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END LOOP;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'In find_parties. This is a Search Rule. Evaluating Matches. Threshold : 9 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      IF (H_SCORES(l_party_id).TOTAL_SCORE/l_entered_max_score)>=(72/805) THEN
            INSERT INTO HZ_MATCHED_PARTIES_GT (SEARCH_CONTEXT_ID, PARTY_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ...
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_parties;
...
PROCEDURE find_persons (
  ...
  BEGIN
    ...
    END IF;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Evaluating Matches. Threshold : 9 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
    IF l_match_str = ' OR ' THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_persons');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_persons;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_party_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_duplicate_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_duplicate_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_duplicate_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.find_duplicate_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
BEGIN
  ...
  END IF;
  IF AttrVals.EXISTS(11) THEN
    l_party_rec.ALL_ACCOUNT_NAMES:= AttrVals(11);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.ALL_ACCOUNT_NAMES '||AttrVals(11),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(13) THEN
    l_party_rec.ALL_ACCOUNT_NUMBERS:= AttrVals(13);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.ALL_ACCOUNT_NUMBERS '||AttrVals(13),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(17) THEN
    l_party_rec.TAX_NAME:= AttrVals(17);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.TAX_NAME '||AttrVals(17),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(22) THEN
    l_party_rec.CATEGORY_CODE:= AttrVals(22);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.CATEGORY_CODE '||AttrVals(22),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(31) THEN
    l_party_site_list(1).PROVINCE:= AttrVals(31);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_site_list(1).PROVINCE '||AttrVals(31),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(35) THEN
    l_party_site_list(1).COUNTY:= AttrVals(35);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_site_list(1).COUNTY '||AttrVals(35),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(46) THEN
    l_cpt_type := 'PHONE';
    FIRST := FALSE;
    FOR I in 1..l_cpt_list.COUNT LOOP
      IF l_cpt_list(I).CONTACT_POINT_TYPE=l_cpt_type THEN
        l_cpt_list(I).PHONE_AREA_CODE:= AttrVals(46);
        FIRST := TRUE;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||I||').CONTACT_POINT_TYPE '||l_cpt_type,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||I||').PHONE_AREA_CODE '||AttrVals(46),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      END IF;
    END LOOP;
    IF not FIRST THEN
      l_idx := l_cpt_list.COUNT+1;
      l_cpt_list(l_idx).CONTACT_POINT_TYPE:=l_cpt_type;
      l_cpt_list(l_idx).PHONE_AREA_CODE:= AttrVals(46);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||l_idx||').CONTACT_POINT_TYPE '||l_cpt_type,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||l_idx||').PHONE_AREA_CODE '||AttrVals(46),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    END IF;
  END IF;

  IF AttrVals.EXISTS(47) THEN
    l_cpt_type := 'PHONE';
    FIRST := FALSE;
    FOR I in 1..l_cpt_list.COUNT LOOP
      IF l_cpt_list(I).CONTACT_POINT_TYPE=l_cpt_type THEN
        l_cpt_list(I).PHONE_COUNTRY_CODE:= AttrVals(47);
        FIRST := TRUE;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||I||').CONTACT_POINT_TYPE '||l_cpt_type,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||I||').PHONE_COUNTRY_CODE '||AttrVals(47),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      END IF;
    END LOOP;
    IF not FIRST THEN
      l_idx := l_cpt_list.COUNT+1;
      l_cpt_list(l_idx).CONTACT_POINT_TYPE:=l_cpt_type;
      l_cpt_list(l_idx).PHONE_COUNTRY_CODE:= AttrVals(47);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||l_idx||').CONTACT_POINT_TYPE '||l_cpt_type,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||l_idx||').PHONE_COUNTRY_CODE '||AttrVals(47),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    END IF;
  END IF;

  IF AttrVals.EXISTS(53) THEN
    l_party_rec.REFERENCE_USE_FLAG:= AttrVals(53);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.REFERENCE_USE_FLAG '||AttrVals(53),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;

  IF AttrVals.EXISTS(54) THEN
    l_party_rec.CORPORATION_CLASS:= AttrVals(54);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.CORPORATION_CLASS '||AttrVals(54),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.call_api_dynamic');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_party_sites (
  ...
  BEGIN
    ...
    IF l_match_str = ' AND ' OR (
        ((l_cpt_max_score/l_entered_max_score)<(72/805))
        ) THEN
    ...
    END IF;
 IF l_match_str = ' OR ' THEN
IF (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=( 72/805 ) THEN
 ...
 ELSE
    IF H_SCORES(l_party_site_id).PARTY_SITE_SCORE>0 AND
       ...
       (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=(72/805) THEN
      INSERT INTO HZ_MATCHED_PARTY_SITES_GT (SEARCH_CONTEXT_ID, PARTY_ID, PARTY_SITE_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.get_matching_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contacts (
  ...
  BEGIN
    ...
    END IF;
        IF (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(72/805) THEN
      ...
      ELSE
           ...
           (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(72/805) THEN
          INSERT INTO HZ_MATCHED_CONTACTS_GT (SEARCH_CONTEXT_ID, PARTY_ID, ORG_CONTACT_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.get_matching_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contact_points (
  ...
  BEGIN
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      LOOP
        ...
        IF (p_dup_contact_point_id IS NULL OR (
            ...
            H_SCORE(cnt) := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX1,l_TX3,l_TX4);
            cnt := cnt+1;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF (H_SCORE(I)/l_entered_max_score) >= (72/805) THEN
        INSERT INTO HZ_MATCHED_CPTS_GT(SEARCH_CONTEXT_ID,CONTACT_POINT_ID,PARTY_ID,SCORE) VALUES (
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.get_matching_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_score_details (
  ...
  l_TX157 VARCHAR2(2000);
  l_max_TX157 VARCHAR2(2000);
  l_TX158 VARCHAR2(2000);
  l_max_TX158 VARCHAR2(2000);
  l_TX20 VARCHAR2(2000);
  l_max_TX20 VARCHAR2(2000);
  l_TX21 VARCHAR2(2000);
  l_max_TX21 VARCHAR2(2000);
  l_TX22 VARCHAR2(2000);
  l_max_TX22 VARCHAR2(2000);
  l_TX3 VARCHAR2(2000);
  l_max_TX3 VARCHAR2(2000);
  l_TX32 VARCHAR2(2000);
  l_max_TX32 VARCHAR2(2000);
  l_TX34 VARCHAR2(2000);
  l_max_TX34 VARCHAR2(2000);
  l_TX35 VARCHAR2(2000);
  l_max_TX35 VARCHAR2(2000);
  l_TX39 VARCHAR2(2000);
  ...
  l_TX42 VARCHAR2(2000);
  l_max_TX42 VARCHAR2(2000);
  l_TX43 VARCHAR2(2000);
  l_max_TX43 VARCHAR2(2000);
  l_TX44 VARCHAR2(2000);
  ...
  l_TX35_new:=RTRIM(LTRIM(l_TX35));
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
          INSERT_PARTY_SCORE(p_party_id, p_party_id, l_search_ctx_id, p_party_search_rec, g_party_stage_rec, l_TX32,l_TX34,l_TX35_new,l_TX39,l_TX158,l_TX40,l_TX43,l_TX42,l_TX44,l_TX46,l_TX47,l_TX48,l_TX156,l_TX157,1);
    END LOOP;
    CLOSE l_party_cur;
    ...
    IF l_party_site_contains_str IS NOT NULL THEN
      ...
      IF l_max_score>0 THEN
        INSERT_PARTY_SITES_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_party_site_list(l_max_idx), g_party_site_stage_list(l_max_idx) ,l_max_TX9,l_max_TX10,l_max_TX11,l_max_TX13,l_max_TX14,l_max_TX15,l_max_TX20,l_max_TX21,l_max_TX22,l_max_idx);
      END IF;
    END IF;
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      END LOOP;
      IF l_max_score>0 THEN
        INSERT_CONTACT_POINTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_point_list(l_max_idx), g_contact_pt_stage_list(l_max_idx) ,l_max_TX1,l_max_TX3,l_max_TX4,l_max_idx);
      END IF;
      CLOSE l_contact_pt_cur;
    END IF;
    IF fnd_log.level_procedure>=fnd_log.g_current_runtime_level THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_34.get_score_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    ELSIF l_party_type = 'PERSON' THEN
      SELECT 
       ...
       ,translate(HZ_PARTIES.PARTY_TYPE, '%',' ')
       ,translate(HZ_PARTIES.REFERENCE_USE_FLAG, '%',' ')
       ,translate(HZ_PERSON_PROFILES.TAX_NAME, '%',' ')
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
    x_party_search_rec.ALL_ACCOUNT_NAMES:=
       HZ_PARTY_ACQUIRE.get_account_info(p_party_id , 'PARTY','ALL_ACCOUNT_NAMES');
    x_party_search_rec.ALL_ACCOUNT_NUMBERS:=
       HZ_PARTY_ACQUIRE.get_account_info(p_party_id , 'PARTY','ALL_ACCOUNT_NUMBERS');
...
PROCEDURE get_party_site_rec (
  ...
  CURSOR c_party_sites(cp_party_site_id NUMBER) IS
    SELECT party_site_id
      ...
      ,translate(HZ_LOCATIONS.COUNTY, '%',' ')
      ,translate(HZ_LOCATIONS.POSTAL_CODE, '%',' ')
      ,translate(HZ_LOCATIONS.PROVINCE, '%',' ')
    ...
    FOR I IN 1..p_party_site_ids.COUNT LOOP
      ...
      LOOP
        FETCH c_party_sites INTO
           ...
           ,x_party_site_list(J).COUNTY
           ,x_party_site_list(J).POSTAL_CODE
           ,x_party_site_list(J).PROVINCE
...
PROCEDURE get_contact_point_rec (
  ...
  CURSOR c_cpts(cp_contact_point_id NUMBER) IS
    SELECT contact_point_id, contact_point_type
      ,translate(HZ_CONTACT_POINTS.PHONE_AREA_CODE, '%',' ')
      ,translate(HZ_CONTACT_POINTS.PHONE_COUNTRY_CODE, '%',' ')
    FROM HZ_CONTACT_POINTS
    ...
    FOR I in 1..p_contact_point_ids.COUNT LOOP
      ...
      LOOP
        FETCH c_cpts INTO
             ...
             ,x_contact_point_list(J).PHONE_AREA_CODE
             ,x_contact_point_list(J).PHONE_COUNTRY_CODE
        ;
...
BEGIN
   ...
   IF p_search_ctx THEN
    ...
    IF p_search_rec.ALL_ACCOUNT_NAMES IS NOT NULL 
    OR p_search_rec.PARTY_NUMBER IS NOT NULL 
    OR p_search_rec.ALL_ACCOUNT_NUMBERS IS NOT NULL 
    OR p_search_rec.TAX_NAME IS NOT NULL 
    OR p_search_rec.TAX_REFERENCE IS NOT NULL 
    OR p_search_rec.SIC_CODE IS NOT NULL 
    OR p_search_rec.SIC_CODE_TYPE IS NOT NULL 
    OR p_search_rec.CATEGORY_CODE IS NOT NULL 
    OR p_search_rec.REFERENCE_USE_FLAG IS NOT NULL 
    OR p_search_rec.CORPORATION_CLASS IS NOT NULL 
    THEN
      g_other_party_level_attribs:=TRUE;
    END IF;
     x_entered_max_score:=0;
    IF p_search_rec.ALL_ACCOUNT_NAMES IS NOT NULL THEN 
    ...
    END IF;
    IF p_search_rec.ALL_ACCOUNT_NUMBERS IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+100;
    END IF;
    IF p_search_rec.PARTY_ALL_NAMES IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+60;
    END IF;
    IF p_search_rec.TAX_NAME IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+30;
    END IF;
    ...
    IF p_search_rec.SIC_CODE_TYPE IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+10;
    END IF;
    IF p_search_rec.CATEGORY_CODE IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+10;
    END IF;
    IF p_search_rec.REFERENCE_USE_FLAG IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+20;
    END IF;
    IF p_search_rec.CORPORATION_CLASS IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+30;
    END IF;
    END IF;
  IF p_search_ctx THEN
    IF p_search_rec.ALL_ACCOUNT_NAMES IS NOT NULL THEN 
      x_stage_rec.TX33 := 
        HZ_TRANS_PKG.WRNAMES_CLEANSE(
             p_search_rec.ALL_ACCOUNT_NAMES
             ,null,'ALL_ACCOUNT_NAMES'
             ,'PARTY','SEARCH');
    ELSE
      x_stage_rec.TX33 := '';
    END IF;
    ...
    IF p_search_rec.ALL_ACCOUNT_NUMBERS IS NOT NULL THEN 
      x_stage_rec.TX35 := 
        HZ_TRANS_PKG.RM_SPLCHAR(
             p_search_rec.ALL_ACCOUNT_NUMBERS
             ,null,'ALL_ACCOUNT_NUMBERS'
             ,'PARTY','SEARCH');
    ELSE
      x_stage_rec.TX35 := '';
    END IF;
  ...
  IF p_search_ctx THEN
    IF p_search_rec.CORPORATION_CLASS IS NOT NULL THEN 
      x_stage_rec.TX157 := 
        HZ_TRANS_PKG.EXACT(
             p_search_rec.CORPORATION_CLASS
  ...
  IF p_search_ctx THEN
    IF p_search_rec.REFERENCE_USE_FLAG IS NOT NULL THEN 
      x_stage_rec.TX156 := 
        HZ_TRANS_PKG.EXACT(
             p_search_rec.REFERENCE_USE_FLAG
   ...
   g_ps_den_only:=TRUE;
    ...
    FOR I IN 1..p_search_list.COUNT LOOP
      ...
      IF p_search_list(I).CITY IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+30;
      END IF;
      IF p_search_list(I).POSTAL_CODE IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+35;
      END IF;
      IF p_search_list(I).PROVINCE IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+20;
      END IF;
      IF p_search_list(I).STATE IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+20;
      END IF;
      IF p_search_list(I).COUNTY IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+20;
      END IF;
...
BEGIN
  ...
  IF p_search_ctx THEN
    ...
    FOR I IN 1..p_search_list.COUNT LOOP
      ...
      END IF;
      IF p_search_list(I).PHONE_AREA_CODE IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+30;
      END IF;
      IF p_search_list(I).PHONE_COUNTRY_CODE IS NOT NULL THEN 
  ...
  FOR I IN 1..p_search_list.COUNT LOOP
    IF p_search_ctx THEN
      IF p_search_list(I).PHONE_AREA_CODE IS NOT NULL THEN 
        x_stage_list(I).TX3 := 
          HZ_TRANS_PKG.RM_SPLCHAR(
             p_search_list(I).PHONE_AREA_CODE
  ...
  FOR I IN 1..p_search_list.COUNT LOOP
    IF p_search_ctx THEN
      IF p_search_list(I).PHONE_COUNTRY_CODE IS NOT NULL THEN 
        x_stage_list(I).TX4 := 
          HZ_TRANS_PKG.RM_SPLCHAR(
             p_search_list(I).PHONE_COUNTRY_CODE
...
p_contact_point_list(I).FLEX_FORMAT_PHONE_NUMBER IS NOT NULL 
    ...
    IF p_party_search_rec.ALL_ACCOUNT_NAMES IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.PARTY_NUMBER IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.ALL_ACCOUNT_NUMBERS IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.PARTY_ALL_NAMES IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.TAX_NAME IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    ...
    IF p_party_search_rec.SIC_CODE_TYPE IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.CATEGORY_CODE IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.REFERENCE_USE_FLAG IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.CORPORATION_CLASS IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    ...
    END LOOP;
    FOR I IN 1..p_party_site_list.COUNT LOOP
      IF p_party_site_list(I).PROVINCE IS NOT NULL THEN 
        RETURN TRUE;
      END IF;
    END LOOP;
    FOR I IN 1..p_party_site_list.COUNT LOOP
    ...
    END LOOP;
    FOR I IN 1..p_party_site_list.COUNT LOOP
      IF p_party_site_list(I).COUNTY IS NOT NULL THEN 
        RETURN TRUE;
      END IF;
    END LOOP;
    FOR I IN 1..p_party_site_list.COUNT LOOP
...
PROCEDURE check_party_site_cond(
  ...
  BEGIN
    ...
    x_secondary:= FALSE;
    FOR I IN 1..p_party_site_list.COUNT LOOP
      IF p_party_site_list(I).CITY IS NOT NULL 
         OR p_party_site_list(I).POSTAL_CODE IS NOT NULL
         OR p_party_site_list(I).PROVINCE IS NOT NULL
         OR p_party_site_list(I).STATE IS NOT NULL
         OR p_party_site_list(I).COUNTY IS NOT NULL
    ...
    FOR I IN 1..p_contact_point_list.COUNT LOOP
      IF p_contact_point_list(I).FLEX_FORMAT_PHONE_NUMBER IS NOT NULL 
         OR p_contact_point_list(I).PHONE_AREA_CODE IS NOT NULL
         OR p_contact_point_list(I).PHONE_COUNTRY_CODE IS NOT NULL
      THEN
        x_secondary := TRUE;
      END IF;
      EXIT WHEN x_secondary;
