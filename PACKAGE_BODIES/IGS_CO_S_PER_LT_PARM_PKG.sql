--------------------------------------------------------
--  DDL for Package Body IGS_CO_S_PER_LT_PARM_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_CO_S_PER_LT_PARM_PKG" AS
/* $Header: IGSLI26B.pls 115.2 2002/11/29 01:08:18 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_co_s_per_lt_parm_all%ROWTYPE;
  new_references igs_co_s_per_lt_parm_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.parameter_value                   := x_parameter_value;
    new_references.letter_repeating_group_cd         := x_letter_repeating_group_cd;
    new_references.splrg_sequence_number             := x_splrg_sequence_number;
  ...
  */
  BEGIN
    ...
    IF (column_name IS NULL) THEN
      NULL;
    ELSIF (UPPER(column_name) = 'SPLRG_SEQUENCE_NUMBER') THEN
      new_references.splrg_sequence_number := igs_ge_number.to_num (column_value);
    ELSIF (UPPER(column_name) = 'SPL_SEQUENCE_NUMBER') THEN
      new_references.spl_sequence_number := igs_ge_number.to_num (column_value);
    END IF;

    IF (UPPER(column_name) = 'SPLRG_SEQUENCE_NUMBER' OR column_name IS NULL) THEN
      IF NOT (new_references.splrg_sequence_number BETWEEN 1
              AND 9999999999)  THEN
  ...
  */
  BEGIN
    ...
    IF (((old_references.letter_order_number = new_references.letter_order_number) AND
        ...
        ((new_references.letter_order_number IS NULL) OR
         (new_references.correspondence_type IS NULL) OR
         (new_references.letter_reference_number IS NULL))) THEN
      NULL;
    ELSIF NOT igs_co_ltr_param_pkg.get_pk_for_validation (
    ...
    IF (((old_references.letter_order_number = new_references.letter_order_number) AND
      ...
      NULL;
    ELSIF NOT igs_co_ltr_pr_rpt_gr_pkg.get_pk_for_validation (
                new_references.letter_order_number,
                new_references.correspondence_type,
                new_references.letter_reference_number,
    ...
    IF (((old_references.person_id = new_references.person_id) AND
         ...
         (old_references.letter_repeating_group_cd = new_references.letter_repeating_group_cd) AND
         (old_references.splrg_sequence_number = new_references.splrg_sequence_number)) OR
        ((new_references.person_id IS NULL) OR
         (new_references.correspondence_type IS NULL) OR
         (new_references.letter_reference_number IS NULL) OR
         (new_references.spl_sequence_number IS NULL) OR
         (new_references.letter_repeating_group_cd IS NULL) OR
         (new_references.splrg_sequence_number IS NULL))) THEN
      NULL;
    ELSIF NOT igs_co_s_perlt_rptgp_pkg.get_pk_for_validation (
                ...
                new_references.spl_sequence_number,
                new_references.letter_repeating_group_cd,
                new_references.splrg_sequence_number
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
  ...
  */
    CURSOR cur_rowid IS
      ...
      AND      letter_parameter_type = x_letter_parameter_type
  ...
  */
    CURSOR cur_rowid IS
      ...
      WHERE   ((correspondence_type = x_correspondence_type) AND
               ...
               (letter_order_number = x_letter_order_number));
  ...
  */
    CURSOR cur_rowid IS
      ...
      WHERE   ((correspondence_type = x_correspondence_type) AND
               (letter_reference_number = x_letter_reference_number) AND
               (letter_repeating_group_cd = x_letter_repeating_group_cd) AND
               (letter_order_number = x_letter_order_number));
  ...
  END get_fk_igs_co_ltr_pr_rpt_gr;
  ...
  */
    CURSOR cur_rowid IS
      ...
      WHERE   ((person_id = x_person_id) AND
               ...
               (letter_repeating_group_cd = x_letter_repeating_group_cd) AND
               (splrg_sequence_number = x_sequence_number));
  ...
  */
  ...
  BEGIN
    ...
    before_dml(
      ...
      x_splrg_sequence_number             => x_splrg_sequence_number,
    ...
    INSERT INTO igs_co_s_per_lt_parm_all (
      person_id,
      correspondence_type,
      ...
      new_references.letter_parameter_type,
      new_references.sequence_number,
      new_references.parameter_value,
      new_references.letter_repeating_group_cd,
      new_references.splrg_sequence_number,
      new_references.letter_order_number,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ((tlinfo.parameter_value = x_parameter_value) OR ((tlinfo.parameter_value IS NULL) AND (X_parameter_value IS NULL)))
        AND ((tlinfo.letter_repeating_group_cd = x_letter_repeating_group_cd) OR ((tlinfo.letter_repeating_group_cd IS NULL) AND (X_letter_repeating_group_cd IS NULL)))
        AND ((tlinfo.splrg_sequence_number = x_splrg_sequence_number) OR ((tlinfo.splrg_sequence_number IS NULL) AND (X_splrg_sequence_number IS NULL)))
        AND (tlinfo.letter_order_number = x_letter_order_number)
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_co_s_per_lt_parm_all
      SET
        parameter_value                   = new_references.parameter_value,
        letter_repeating_group_cd         = new_references.letter_repeating_group_cd,
        splrg_sequence_number             = new_references.splrg_sequence_number,
        letter_order_number               = new_references.letter_order_number,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_co_s_per_lt_parm_all
    WHERE rowid = x_rowid;
