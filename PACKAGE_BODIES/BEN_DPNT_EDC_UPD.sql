--------------------------------------------------------
--  DDL for Package Body BEN_DPNT_EDC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_DPNT_EDC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_dpnt_edc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_dpnt_edc_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.dpnt_eligy_crit_values_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_dpnt_edc_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.dpnt_cvg_eligy_prfl_id = hr_api.g_number) then
      p_rec.dpnt_cvg_eligy_prfl_id := ben_dpnt_edc_shd.g_old_rec.dpnt_cvg_eligy_prfl_id;
  end if;
  If (p_rec.eligy_criteria_dpnt_id = hr_api.g_number) then
      p_rec.eligy_criteria_dpnt_id := ben_dpnt_edc_shd.g_old_rec.eligy_criteria_dpnt_id;
  end if;
  If (p_rec.number_value1 = hr_api.g_number) then
      p_rec.number_value1 := ben_dpnt_edc_shd.g_old_rec.number_value1;
  end if;
  If (p_rec.number_value2 = hr_api.g_number) then
      p_rec.number_value2 := ben_dpnt_edc_shd.g_old_rec.number_value2;
  end if;
  If (p_rec.char_value1 = hr_api.g_varchar2) then
      p_rec.char_value1 := ben_dpnt_edc_shd.g_old_rec.char_value1;
  end if;
  If (p_rec.char_value2 = hr_api.g_varchar2) then
      p_rec.char_value2 := ben_dpnt_edc_shd.g_old_rec.char_value2;
  end if;
  If (p_rec.date_value1 = hr_api.g_date) then
      p_rec.date_value1 := ben_dpnt_edc_shd.g_old_rec.date_value1;
  end if;
  If (p_rec.date_value2 = hr_api.g_date) then
      p_rec.date_value2 := ben_dpnt_edc_shd.g_old_rec.date_value2;
  end if;
  If (p_rec.excld_flag = hr_api.g_varchar2) then
      p_rec.excld_flag := ben_dpnt_edc_shd.g_old_rec.excld_flag;
  end if;
  If (p_rec.business_group_id = hr_api.g_number) then
      p_rec.business_group_id := ben_dpnt_edc_shd.g_old_rec.business_group_id;
  end if;
  If (p_rec.edc_attribute_category = hr_api.g_varchar2) then
      p_rec.edc_attribute_category := ben_dpnt_edc_shd.g_old_rec.edc_attribute_category;
  end if;
  If (p_rec.edc_attribute1 = hr_api.g_varchar2) then
      p_rec.edc_attribute1 := ben_dpnt_edc_shd.g_old_rec.edc_attribute1;
  end if;
  If (p_rec.edc_attribute2 = hr_api.g_varchar2) then
      p_rec.edc_attribute2 := ben_dpnt_edc_shd.g_old_rec.edc_attribute2;
  end if;
  If (p_rec.edc_attribute3 = hr_api.g_varchar2) then
      p_rec.edc_attribute3 := ben_dpnt_edc_shd.g_old_rec.edc_attribute3;
  end if;
  If (p_rec.edc_attribute4 = hr_api.g_varchar2) then
      p_rec.edc_attribute4 := ben_dpnt_edc_shd.g_old_rec.edc_attribute4;
  end if;
  If (p_rec.edc_attribute5 = hr_api.g_varchar2) then
      p_rec.edc_attribute5 := ben_dpnt_edc_shd.g_old_rec.edc_attribute5;
  end if;
  If (p_rec.edc_attribute6 = hr_api.g_varchar2) then
      p_rec.edc_attribute6 := ben_dpnt_edc_shd.g_old_rec.edc_attribute6;
  end if;
  If (p_rec.edc_attribute7 = hr_api.g_varchar2) then
      p_rec.edc_attribute7 := ben_dpnt_edc_shd.g_old_rec.edc_attribute7;
  end if;
  If (p_rec.edc_attribute8 = hr_api.g_varchar2) then
      p_rec.edc_attribute8 := ben_dpnt_edc_shd.g_old_rec.edc_attribute8;
  end if;
  If (p_rec.edc_attribute9 = hr_api.g_varchar2) then
      p_rec.edc_attribute9 := ben_dpnt_edc_shd.g_old_rec.edc_attribute9;
  end if;
  If (p_rec.edc_attribute10 = hr_api.g_varchar2) then
      p_rec.edc_attribute10 := ben_dpnt_edc_shd.g_old_rec.edc_attribute10;
  end if;
  If (p_rec.edc_attribute11 = hr_api.g_varchar2) then
      p_rec.edc_attribute11 := ben_dpnt_edc_shd.g_old_rec.edc_attribute11;
  end if;
  If (p_rec.edc_attribute12 = hr_api.g_varchar2) then
      p_rec.edc_attribute12 := ben_dpnt_edc_shd.g_old_rec.edc_attribute12;
  end if;
  If (p_rec.edc_attribute13 = hr_api.g_varchar2) then
      p_rec.edc_attribute13 := ben_dpnt_edc_shd.g_old_rec.edc_attribute13;
  end if;
  If (p_rec.edc_attribute14 = hr_api.g_varchar2) then
      p_rec.edc_attribute14 := ben_dpnt_edc_shd.g_old_rec.edc_attribute14;
  end if;
  If (p_rec.edc_attribute15 = hr_api.g_varchar2) then
      p_rec.edc_attribute15 := ben_dpnt_edc_shd.g_old_rec.edc_attribute15;
  end if;
  If (p_rec.edc_attribute16 = hr_api.g_varchar2) then
      p_rec.edc_attribute16 := ben_dpnt_edc_shd.g_old_rec.edc_attribute16;
  end if;
  If (p_rec.edc_attribute17 = hr_api.g_varchar2) then
      p_rec.edc_attribute17 := ben_dpnt_edc_shd.g_old_rec.edc_attribute17;
  end if;
  If (p_rec.edc_attribute18 = hr_api.g_varchar2) then
      p_rec.edc_attribute18 := ben_dpnt_edc_shd.g_old_rec.edc_attribute18;
  end if;
  If (p_rec.edc_attribute19 = hr_api.g_varchar2) then
      p_rec.edc_attribute19 := ben_dpnt_edc_shd.g_old_rec.edc_attribute19;
  end if;
  If (p_rec.edc_attribute20 = hr_api.g_varchar2) then
      p_rec.edc_attribute20 := ben_dpnt_edc_shd.g_old_rec.edc_attribute20;
  end if;
  If (p_rec.edc_attribute21 = hr_api.g_varchar2) then
      p_rec.edc_attribute21 := ben_dpnt_edc_shd.g_old_rec.edc_attribute21;
  end if;
  If (p_rec.edc_attribute22 = hr_api.g_varchar2) then
      p_rec.edc_attribute22 := ben_dpnt_edc_shd.g_old_rec.edc_attribute22;
  end if;
  If (p_rec.edc_attribute23 = hr_api.g_varchar2) then
      p_rec.edc_attribute23 := ben_dpnt_edc_shd.g_old_rec.edc_attribute23;
  end if;
  If (p_rec.edc_attribute24 = hr_api.g_varchar2) then
      p_rec.edc_attribute24 := ben_dpnt_edc_shd.g_old_rec.edc_attribute24;
  end if;
  If (p_rec.edc_attribute25 = hr_api.g_varchar2) then
      p_rec.edc_attribute25 := ben_dpnt_edc_shd.g_old_rec.edc_attribute25;
  end if;
  If (p_rec.edc_attribute26 = hr_api.g_varchar2) then
      p_rec.edc_attribute26 := ben_dpnt_edc_shd.g_old_rec.edc_attribute26;
  end if;
  If (p_rec.edc_attribute27 = hr_api.g_varchar2) then
      p_rec.edc_attribute27 := ben_dpnt_edc_shd.g_old_rec.edc_attribute27;
  end if;
  If (p_rec.edc_attribute28 = hr_api.g_varchar2) then
      p_rec.edc_attribute28 := ben_dpnt_edc_shd.g_old_rec.edc_attribute28;
  end if;
  If (p_rec.edc_attribute29 = hr_api.g_varchar2) then
      p_rec.edc_attribute29 := ben_dpnt_edc_shd.g_old_rec.edc_attribute29;
  end if;
  If (p_rec.edc_attribute30 = hr_api.g_varchar2) then
      p_rec.edc_attribute30 := ben_dpnt_edc_shd.g_old_rec.edc_attribute30;
  end if;
  If (p_rec.Char_value3 = hr_api.g_varchar2) then
      p_rec.Char_value3 := ben_dpnt_edc_shd.g_old_rec.Char_value3;
  end if;
  If (p_rec.Char_value4 = hr_api.g_varchar2) then
      p_rec.Char_value4 := ben_dpnt_edc_shd.g_old_rec.Char_value4;
  end if;
  If (p_rec.Number_value3 = hr_api.g_number) then
      p_rec.Number_value3 := ben_dpnt_edc_shd.g_old_rec.Number_value3;
  end if;
  If (p_rec.Number_value4 = hr_api.g_number) then
      p_rec.Number_value4 := ben_dpnt_edc_shd.g_old_rec.Number_value4;
  end if;
  If (p_rec.Date_value3 = hr_api.g_date) then
      p_rec.Date_value3 := ben_dpnt_edc_shd.g_old_rec.Date_value3;
  end if;
  If (p_rec.Date_value4 = hr_api.g_date) then
      p_rec.Date_value4 := ben_dpnt_edc_shd.g_old_rec.Date_value4;
  end if;
