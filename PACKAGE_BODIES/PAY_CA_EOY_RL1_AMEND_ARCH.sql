--------------------------------------------------------
--  DDL for Package Body PAY_CA_EOY_RL1_AMEND_ARCH
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CA_EOY_RL1_AMEND_ARCH" AS
/* $Header: pycarl1ca.pkb 120.1.12010000.3 2009/08/24 11:16:38 aneghosh ship $ */
/*
   ******************************************************************
   *                                                                *
   ...
   ******************************************************************
    ...
    Description : This procedure is used by RL1 Amendment Pre-Process
    ...
    17-NOV-2003 SSouresr   115.1            Modified range_cursor for situation
                                            where no person or assignment set
                                            has been specified.
    01-SEP-2004 SSouresr   115.4            Added changes for multi assignments
    02-SEP-2004 SSouresr   115.5            Changed to use the function get_parameter
                                            to retrieve PRE_ORGANIZATION_ID
    15-NOV-2004 SSouresr   115.6            Using per_all_people_f instead of per_people_f
    07-MAR-2005 SSouresr   115.7            Removed extra p_payroll_action_id from
                                            range_cursor
    11-OCT-2006 meshah     115.8    5528944 Added order by clause in the range
                                            cursor queries.
    18-JUN-2009 aneghosh   115.9    8316783 Modified locking mechanism.
    24-AUG-2009 aneghosh   115.10   8316783 Removed the locking of RL1_AMD_MAG and
                                            RL1_XML_MAG.
  *****************************************************************************/

   gv_package        VARCHAR2(100) := 'pay_ca_eoy_rl1_amend_arch';
  ...
  ******************************************************************************/
  ...
  IS
    cursor c_payroll_Action_info (cp_payroll_action_id in number) is
      select  to_number(pay_ca_eoy_rl1_amend_arch.get_parameter(
                          'PRE_ORGANIZATION_ID',ppa.legislative_parameters)),
              to_number(pay_ca_eoy_rl1_amend_arch.get_parameter(
  ...
  ************************************************************/
  ...
  IS
    ...
    ln_rl1amend_asg_action  NUMBER := 0;
    lv_year                 VARCHAR2(4);
...
OR  pay_ca_eoy_rl1_amend_arch.get_parameter('TRANSMITTER_PRE',
                     ppa.legislative_parameters) = to_char(ln_pre_org_id))
...
in ('RL1', 'CAEOY_RL1_AMEND_PP','RL1_AMD_MAG','RL1_XML_MAG')
      and to_char(ppa.effective_date,'YYYY') = lv_year
      and ppa.business_group_id+0 = ln_business_group_id
--      and pay_ca_eoy_rl1_amend_arch.get_parameter('PRE_ORGANIZATION_ID',
--                     ppa.legislative_parameters) = to_char(ln_pre_org_id)
   ...
   IS
     ...
     CURSOR get_prev_rl1_amend_dtls (cp_person_id      in number
...
OR  pay_ca_eoy_rl1_amend_arch.get_parameter('TRANSMITTER_PRE',
      ...
      and ppa.report_type in ('RL1', 'CAEOY_RL1_AMEND_PP')
   ...
   BEGIN
      ...
      /* Actions will be created if
         ...
         #1 there is a YEPP action not followed by a
            RL1 Amendment Archiver action

         #2 there is already a Previous RL1 Amendment Archiver
            action for this person
      */

        open get_prev_rl1_amend_dtls(p_person_id
                                    ,ln_pre_org_id
                                    ,ld_end_date);
        fetch get_prev_rl1_amend_dtls into  lv_report_type
        ...
        if get_prev_rl1_amend_dtls%notfound then

           hr_utility.trace('get_prev_rl1_amend_dtls Not Found');
           ...
           lv_record_name := 'RL1 Amendment Pre-Process';
        ...
        if get_prev_rl1_amend_dtls%found then
           hr_utility.trace('get_prev_rl1_amend_dtls Found');

           if lv_report_type in ('RL1', 'CAEOY_RL1_AMEND_PP') then

               /* Create an assignment action for this person */
               select pay_assignment_actions_s.nextval
                 into ln_rl1amend_asg_action
                 from dual;
               hr_utility.trace('New RL1 Amendment Action = ' ||
                                   to_char(ln_rl1amend_asg_action));

               /* Insert into pay_assignment_actions. */
               -- hr_utility.trace('creating asg. action');
               hr_nonrun_asact.insact(ln_rl1amend_asg_action
                                     ,ln_primary_assignment_id
                                     ,p_payroll_action_id
                                     ,p_chunk
                                     ,null);
               ...
               /* Update the serial number column with the person id
                  so that the RL1 Amendment report will not have
               ...
               update pay_assignment_actions aa
                ...
                where  aa.assignment_action_id = ln_rl1amend_asg_action;

               /* Interlock the YEPP or Previous RL1 Amendment Archiver
                   action for current RL1 Amendment Pre-process action */

               hr_utility.trace('Locking Action = ' || to_char(ln_rl1amend_asg_action));
               hr_utility.trace('Locked Action = '  || to_char(ln_asg_act_to_lock));
               hr_nonrun_asact.insint(ln_rl1amend_asg_action
                                     ,ln_asg_act_to_lock);
        ...
        close get_prev_rl1_amend_dtls;
        hr_utility.trace('closed get_prev_rl1_amend_dtls');
   END action_creation;
...
Begin
--hr_utility.trace_on(null,'RL1AMENDPP');
null;
