--------------------------------------------------------
--  DDL for Package Body HZ_MATCH_RULE_61
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_MATCH_RULE_61" AS
/*=======================================================================+
...
POP_CONTACT_POINTS_COND_REC(p_contact_point_list);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      threshold := round((255/340)*(l_entered_max_score+l_ps_entered_max_score+l_ct_entered_max_score+l_cpt_entered_max_score));
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
...
END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_sites = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, stage.PARTY_ID, stage.ORG_CONTACT_ID
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID
          FROM HZ_STAGED_PARTY_SITES stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT  PARTY_SITE_ID, stage.PARTY_ID, ORG_CONTACT_ID
          ...
          OR (l_search_merged = 'N' AND nvl(stage.status_flag, 'A') = 'A')       )  
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, r.subject_id, stage.ORG_CONTACT_ID
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
       l_sqlstr := 'SELECT  /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID '||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
         ...
         ' AND (ORG_CONTACT_ID IS NULL ' ||
       	...
       	' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
         ' UNION ' ||
       			 'SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID '||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
       ...
       ELSE 
       l_sqlstr := 'SELECT ' || l_hint ||' PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID '||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX8, TX7
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N3) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX8, TX7
          FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage 
          WHERE contains( concat_col, p_contains_str)>0 
          AND d.search_context_id = p_search_ctx_id 
          AND( (l_search_merged ='Y' )  
 ...
 THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_cpts = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX8, TX7
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ stage.CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX8, TX7
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX8, TX7
          FROM HZ_STAGED_CONTACT_POINTS stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
          SELECT 1 FROM HZ_STAGED_PARTIES p
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX8, TX7
            FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage
            WHERE contains( concat_col, p_contains_str)>0
            AND d.search_context_id = p_search_ctx_id
            AND d.party_id = stage.party_id
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, r.subject_id, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX8, TX7
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       ELSE
		p_restrict_sql1 := 'stage1.'||p_restrict_sql;
       END IF; 
       	l_sqlstr := ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX8, TX7'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ...
                   ' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
                   ' UNION ' ||
                   ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX8, TX7'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ' AND( ('''||l_search_merged||''' =''Y'' )  '||
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '||
       ...
       OPEN x_cursor FOR l_sqlstr USING p_contains_str,
                    p_dup_party_id, p_dup_party_id, p_contains_str, p_dup_party_id, p_dup_party_id;
       ELSE 
       	l_sqlstr := 'SELECT   ' || l_hint ||' CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX8, TX7'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                 ' AND EXISTS ('||
                 ' SELECT 1 FROM HZ_STAGED_PARTIES p ' || 
...
PROCEDURE find_parties (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_site_cur INTO
         l_party_site_id, l_ps_party_id, l_ps_contact_id ;
      ...
      IF l_use_contact_addr_info OR l_ps_contact_id IS NULL THEN
        ...
        l_score := GET_PARTY_SITES_SCORE(l_match_idx);
        IF NOT H_SCORES.EXISTS(l_index) THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_contact_pt_cur INTO
         l_contact_pt_id,  l_cpt_type, l_cpt_party_id,  l_cpt_ps_id, l_cpt_contact_id , l_TX8, l_TX7;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      IF l_use_contact_cpt_info OR l_cpt_contact_id IS NULL THEN
        ...
        l_score := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX8,l_TX7);
        IF NOT H_SCORES.EXISTS(l_index) THEN
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    IF (p_index=5 AND call_order(5) <> 'NONE' AND H_PARTY_ID.COUNT=0) THEN
     l_threshold :=  round(( l_entered_max_score / 340) * 255); 
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
    l_party_contains_str := INIT_PARTY_QUERY(l_match_str, l_denorm_str, l_party_max_score, l_denorm_max_score, l_non_denorm_max_score, round((255/340)*l_entered_max_score));
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END LOOP;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'In find_parties. This is a Search Rule. Evaluating Matches. Threshold : 75 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      IF (H_SCORES(l_party_id).TOTAL_SCORE/l_entered_max_score)>=(255/340) THEN
            INSERT INTO HZ_MATCHED_PARTIES_GT (SEARCH_CONTEXT_ID, PARTY_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ...
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_parties;
...
PROCEDURE find_persons (
  ...
  BEGIN
    ...
    END IF;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Evaluating Matches. Threshold : 75 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
    IF l_match_str = ' OR ' THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_persons');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_persons;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_party_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_duplicate_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_duplicate_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_duplicate_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.find_duplicate_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.call_api_dynamic');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_party_sites (
  ...
  BEGIN
    ...
    IF l_match_str = ' AND ' OR (
        ((l_cpt_max_score/l_entered_max_score)<(255/340))
        ) THEN
    ...
    END IF;
 IF l_match_str = ' OR ' THEN
IF (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=( 255/340 ) THEN
 ...
 ELSE
    IF H_SCORES(l_party_site_id).PARTY_SITE_SCORE>0 AND
       ...
       (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=(255/340) THEN
      INSERT INTO HZ_MATCHED_PARTY_SITES_GT (SEARCH_CONTEXT_ID, PARTY_ID, PARTY_SITE_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.get_matching_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contacts (
  ...
  BEGIN
    ...
    END IF;
        IF (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(255/340) THEN
      ...
      ELSE
           ...
           (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(255/340) THEN
          INSERT INTO HZ_MATCHED_CONTACTS_GT (SEARCH_CONTEXT_ID, PARTY_ID, ORG_CONTACT_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.get_matching_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contact_points (
  ...
  BEGIN
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      LOOP
        ...
        IF (p_dup_contact_point_id IS NULL OR (
            ...
            H_SCORE(cnt) := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX8,l_TX7);
            cnt := cnt+1;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF (H_SCORE(I)/l_entered_max_score) >= (255/340) THEN
        INSERT INTO HZ_MATCHED_CPTS_GT(SEARCH_CONTEXT_ID,CONTACT_POINT_ID,PARTY_ID,SCORE) VALUES (
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.get_matching_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
  ...
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
    ...
    IF l_party_site_contains_str IS NOT NULL THEN
      ...
      IF l_max_score>0 THEN
        INSERT_PARTY_SITES_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_party_site_list(l_max_idx), g_party_site_stage_list(l_max_idx) ,l_max_idx);
      END IF;
    END IF;
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      END LOOP;
      IF l_max_score>0 THEN
        INSERT_CONTACT_POINTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_point_list(l_max_idx), g_contact_pt_stage_list(l_max_idx) ,l_max_TX8,l_max_TX7,l_max_idx);
      END IF;
      CLOSE l_contact_pt_cur;
    END IF;
    IF fnd_log.level_procedure>=fnd_log.g_current_runtime_level THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_61.get_score_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
p_contact_point_list(I).URL IS NOT NULL 
 ) THEN
...
PROCEDURE check_party_site_cond(
  ...
  BEGIN
    ...
    END LOOP;
    FOR I IN 1..p_contact_point_list.COUNT LOOP
      IF p_contact_point_list(I).URL IS NOT NULL 
      THEN
        x_primary := TRUE;
      END IF;
      EXIT WHEN x_primary;
