--------------------------------------------------------
--  DDL for Package Body BIX_PMV_AI_ATWTR_PRTLT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_PMV_AI_ATWTR_PRTLT_PKG" AS
/*$Header: bixitwrp.plb 120.0 2005/05/25 17:22:41 appldev noship $ */
...
p_custom_output := BIS_QUERY_ATTRIBUTES_TBL();
  ...
  poa_dbi_util_pkg.add_column(bix_col_tab,'call_talk_time','talk' ,'N',2,'XTD');
  ...
  --pass l_xtd here. This
  poa_dbi_util_pkg.add_column(p_cols,'0'     ,'talk','N',2,'XTD');
  poa_dbi_util_pkg.add_column(p_cols,'0'     ,'wrap','N',2,'XTD');
  poa_dbi_util_pkg.add_column(p_cols,'call_cont_calls_handled_tot_na'    ,'hand','N',2,'XTD');
  ...
  l_union_all_text := 'UNION ALL '||bix_pmv_dbi_utl_pkg.get_continued_measures
   ...
   l_sqltext :=  ' SELECT name VIEWBY,
                         -- SUM(p_talk)/DECODE(SUM(p_hand),0,null,SUM(p_hand))           BIX_PMV_AI_PREVAVGTALK,
                          SUM(curr_talk)/DECODE(SUM(curr_hand),0,null,SUM(curr_hand))  BIX_PMV_AI_AVGTALK    ,
                         -- SUM(p_wrap)/DECODE(SUM(p_hand),0,null,SUM(p_hand))           BIX_PMV_AI_PREVAVGWRAP,
                          SUM(curr_wrap)/DECODE(SUM(curr_hand),0,null,SUM(curr_hand))  BIX_PMV_AI_AVGWRAP
...
END GET_SQL;
END BIX_PMV_AI_ATWTR_PRTLT_PKG;
