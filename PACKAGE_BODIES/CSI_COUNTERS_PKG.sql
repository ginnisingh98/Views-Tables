--------------------------------------------------------
--  DDL for Package Body CSI_COUNTERS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSI_COUNTERS_PKG" as
/* $Header: csitctib.pls 120.5 2008/04/02 23:11:40 devijay ship $*/

G_PKG_NAME CONSTANT VARCHAR2(30)  := 'CSI_COUNTERS_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'csitctib.pls';
...
BEGIN
   ...
   VALUES(
	...
	,decode(p_INITIAL_READING_DATE, FND_API.G_MISS_DATE, NULL,p_INITIAL_READING_DATE)
	...
	,decode(p_CREATED_FROM_COUNTER_TMPL_ID, FND_API.G_MISS_NUM, NULL,p_CREATED_FROM_COUNTER_TMPL_ID)
...
PROCEDURE Update_Row(
 ...
 BEGIN
    ...
    SET    GROUP_ID = decode(p_GROUP_ID, NULL, GROUP_ID, FND_API.G_MISS_NUM, NULL, p_GROUP_ID)
	   ...
	   ,CREATED_FROM_COUNTER_TMPL_ID = decode(p_CREATED_FROM_COUNTER_TMPL_ID, NULL, CREATED_FROM_COUNTER_TMPL_ID,FND_API.G_MISS_NUM, NULL, p_CREATED_FROM_COUNTER_TMPL_ID)
    ...
    SET    source_lang        = userenv('LANG'),
           ...
           last_update_date   = decode( p_last_update_date, NULL,last_update_date, fnd_api.g_miss_date, last_update_date,  p_last_update_date),
           last_update_login  = decode( p_last_update_login, NULL,last_update_login, fnd_api.g_miss_num, FND_GLOBAL.USER_ID,  p_last_update_login)
    WHERE counter_id = p_counter_id
...
BEGIN
   DELETE FROM csi_counters_tl t
   WHERE NOT EXISTS (SELECT NULL
                     FROM   csi_counters_b b
                     WHERE  b.counter_id = t.counter_id);

   UPDATE csi_counters_tl t
   SET    (name,description) = (SELECT b.name,
                                       b.description
                                FROM   csi_counters_tl b
                                WHERE  b.counter_id = t.counter_id
                                AND    b.language  = t.source_lang)
   WHERE (t.counter_id,t.language) IN  (SELECT subt.counter_id,
                                               subt.language
                                       FROM    csi_counters_tl subb, csi_counters_tl subt
   ...
   SELECT  b.counter_id,
           ...
           l.language_code,
           b.source_lang
   FROM  csi_counters_tl b, fnd_languages l
   WHERE l.installed_flag in ('I', 'B')
   AND   b.language = userenv('LANG')
   AND   NOT EXISTS (SELECT NULL
                     FROM   csi_counters_tl t
