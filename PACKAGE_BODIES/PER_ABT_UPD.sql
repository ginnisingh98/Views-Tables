--------------------------------------------------------
--  DDL for Package Body PER_ABT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ABT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_abt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy per_abt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the per_abs_attendance_types_tl Row
  --
  update per_abs_attendance_types_tl
    set
...
Begin
  ...
  --
  per_abt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_abt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_abt_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
  ...
  cursor csr_upd_langs is
    select abt.language
     ...
     where abt.absence_attendance_type_id = p_absence_attendance_type_id
       and p_language_code in (abt.language
                              ,abt.source_lang);
  --
