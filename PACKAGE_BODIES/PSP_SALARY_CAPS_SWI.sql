--------------------------------------------------------
--  DDL for Package Body PSP_SALARY_CAPS_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSP_SALARY_CAPS_SWI" As
/* $Header: PSPSCSWB.pls 120.0 2005/11/20 23:56 dpaudel noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'psp_salary_caps_swi.';
--
-- ----------------------------------------------------------------------------
-- |---------------------------< create_salary_cap >--------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_salary_cap_swi;
  --
  ...
  --
  psp_psc_ins.set_base_key_value
    (p_salary_cap_id => p_salary_cap_id
    );
  --
  -- Call API
  --
  psp_salary_caps_api.create_salary_cap
    ...
    ,p_salary_cap_id                => l_salary_cap_id
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_salary_cap_swi;
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_salary_cap_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  psp_salary_caps_api.delete_salary_cap
    (p_validate                     => l_validate
    ,p_salary_cap_id                => p_salary_cap_id
    ,p_object_version_number        => p_object_version_number
    ,p_return_status                => l_return_status
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_salary_cap_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_object_version_number        := l_object_version_number;
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint update_salary_cap_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  psp_salary_caps_api.update_salary_cap
    (p_validate                     => l_validate
    ,p_salary_cap_id                => p_salary_cap_id
    ,p_funding_source_code          => p_funding_source_code
    ,p_start_date                   => p_start_date
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_salary_cap_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_object_version_number        := l_object_version_number;
