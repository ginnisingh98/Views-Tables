--------------------------------------------------------
--  DDL for Package Body PQH_BST_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_BST_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_bst_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_budget_set_id >------|
-- ----------------------------------------------------------------------------
...
--
Procedure chk_budget_set_id(p_budget_set_id                in number,
...
Begin
  ...
  --
  l_api_updating := pqh_bst_shd.api_updating
    (p_budget_set_id                => p_budget_set_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_budget_set_id,hr_api.g_number)
     <>  pqh_bst_shd.g_old_rec.budget_set_id) then
    --
    -- raise error as PK has changed
    --
    pqh_bst_shd.constraint_error('PQH_BUDGET_SETS_PK');
    --
...
-- In Parameters
--   p_budget_set_id PK
--   p_budget_period_id ID of FK column
...
--   Internal table handler use only.
--
Procedure chk_budget_period_id (p_budget_set_id          in number,
  ...
  cursor c1 is
    select null
    from   pqh_budget_periods a
    where  a.budget_period_id = p_budget_period_id;
  --
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  ...
  if (l_api_updating
     ...
     <> nvl(pqh_bst_shd.g_old_rec.budget_period_id,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if budget_period_id value exists in pqh_budget_periods table
    --
    open c1;
      --
      ...
      if c1%notfound then
        ...
        close c1;
        --
        -- raise error as FK does not relate to PK in pqh_budget_periods
        -- table.
        --
        pqh_bst_shd.constraint_error('PQH_BUDGET_SETS_FK2');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_dflt_budget_set_id (p_budget_set_id          in number,
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(pqh_bst_shd.g_old_rec.dflt_budget_set_id,hr_api.g_number)
     or not l_api_updating) and
     p_dflt_budget_set_id is not null then
    --
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        pqh_bst_shd.constraint_error('PQH_BUDGET_SETS_FK1');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_budget_unit3_value_type_cd(p_budget_set_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(pqh_bst_shd.g_old_rec.budget_unit3_value_type_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_budget_unit2_value_type_cd(p_budget_set_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(pqh_bst_shd.g_old_rec.budget_unit2_value_type_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_budget_unit1_value_type_cd(p_budget_set_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(pqh_bst_shd.g_old_rec.budget_unit1_value_type_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
-- |---------------------------< insert_validate >----------------------------|
-- ----------------------------------------------------------------------------
Procedure insert_validate(p_rec in pqh_bst_shd.g_rec_type
...
-- |---------------------------< update_validate >----------------------------|
-- ----------------------------------------------------------------------------
Procedure update_validate(p_rec in pqh_bst_shd.g_rec_type
                         ,p_effective_date in date) is
--
...
-- |---------------------------< delete_validate >----------------------------|
-- ----------------------------------------------------------------------------
Procedure delete_validate(p_rec in pqh_bst_shd.g_rec_type
                         ,p_effective_date in date) is
--
