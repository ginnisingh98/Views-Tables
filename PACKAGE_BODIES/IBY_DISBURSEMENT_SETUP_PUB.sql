--------------------------------------------------------
--  DDL for Package Body IBY_DISBURSEMENT_SETUP_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IBY_DISBURSEMENT_SETUP_PUB" AS
/*$Header: ibyfdstb.pls 120.5.12010000.16 2010/06/17 05:00:31 pschalla ship $*/
   ...
   IF p_ext_payee_tab.COUNT > 0 THEN
     ...
     ELSIF (((p_ext_payee_tab(counter).Payer_Org_Type IS NOT NULL) or
             (p_ext_payee_tab(counter).Payer_ORG_ID IS NOT NULL)) and
              ...
              p_ext_payee_tab(counter).Payee_Party_Site_Id IS NULL)
           ) THEN
       IF ( G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
...
PROCEDURE Validate_External_Payee (
  ...
  BEGIN
    ...
    IF (p_ext_payee_rec.Pay_Reason_Code IS NOT NULL) THEN
      OPEN pmt_reasons_csr(p_ext_payee_rec.Pay_Reason_Code,l_payee_country);
    ...
    IF (p_ext_payee_rec.Delivery_Channel IS NOT NULL) THEN
      OPEN dlv_channels_csr(p_ext_payee_rec.Delivery_Channel,l_payee_country);
...
/* If Bank account is already existing in the application*/
   ELSIF (l_dup_acct_id is not null) THEN
	   ...
	   END IF;
           ...
           /* If party is owner of that bank account*/
           IF (x_return_status = FND_API.G_RET_STS_SUCCESS) THEN
