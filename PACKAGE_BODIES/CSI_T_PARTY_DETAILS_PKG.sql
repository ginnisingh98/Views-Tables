--------------------------------------------------------
--  DDL for Package Body CSI_T_PARTY_DETAILS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSI_T_PARTY_DETAILS_PKG" as
/* $Header: csittptb.pls 120.1 2005/06/17 01:52:35 appldev  $ */
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'CSI_T_PARTY_DETAILS_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'csittptb.pls';
...
PROCEDURE Insert_Row(
 ...
 IS
   CURSOR C2 IS SELECT CSI_T_PARTY_DETAILS_S.nextval FROM sys.dual;
BEGIN
   If (px_TXN_PARTY_DETAIL_ID IS NULL) OR (px_TXN_PARTY_DETAIL_ID = FND_API.G_MISS_NUM) then
       OPEN C2;
       FETCH C2 INTO px_TXN_PARTY_DETAIL_ID;
       CLOSE C2;
   End If;
   INSERT INTO CSI_T_PARTY_DETAILS(
           TXN_PARTY_DETAIL_ID,
           TXN_LINE_DETAIL_ID,
           PARTY_SOURCE_TABLE,
           PARTY_SOURCE_ID,
           ...
           px_TXN_PARTY_DETAIL_ID,
           decode( p_TXN_LINE_DETAIL_ID, FND_API.G_MISS_NUM, NULL, p_TXN_LINE_DETAIL_ID),
           decode( p_PARTY_SOURCE_TABLE, FND_API.G_MISS_CHAR, NULL, p_PARTY_SOURCE_TABLE),
           decode( p_PARTY_SOURCE_ID, FND_API.G_MISS_NUM, NULL, p_PARTY_SOURCE_ID),
           decode( p_RELATIONSHIP_TYPE_CODE, FND_API.G_MISS_CHAR, NULL, p_RELATIONSHIP_TYPE_CODE),
           decode( p_CONTACT_FLAG, FND_API.G_MISS_CHAR, NULL, p_CONTACT_FLAG),
           ...
           decode( p_PRIMARY_FLAG, FND_API.G_MISS_CHAR, NULL, p_PRIMARY_FLAG),
           decode( p_PREFERRED_FLAG, FND_API.G_MISS_CHAR, NULL, p_PREFERRED_FLAG));
End Insert_Row;
...
PROCEDURE Update_Row(
 ...
 BEGIN
    ...
    SET
              ...
              PARTY_SOURCE_TABLE = decode( p_PARTY_SOURCE_TABLE, FND_API.G_MISS_CHAR, PARTY_SOURCE_TABLE, p_PARTY_SOURCE_TABLE),
              PARTY_SOURCE_ID = decode( p_PARTY_SOURCE_ID, FND_API.G_MISS_NUM, PARTY_SOURCE_ID, p_PARTY_SOURCE_ID),
              ...
              CONTACT_FLAG = decode( p_CONTACT_FLAG, FND_API.G_MISS_CHAR, CONTACT_FLAG, p_CONTACT_FLAG),
              ...
              PREFERRED_FLAG = decode( p_PREFERRED_FLAG, FND_API.G_MISS_CHAR, PREFERRED_FLAG, p_PREFERRED_FLAG)
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
        ...
        WHERE TXN_PARTY_DETAIL_ID =  p_TXN_PARTY_DETAIL_ID
        FOR UPDATE of TXN_PARTY_DETAIL_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN C;
    ...
    CLOSE C;
    if (
           (      Recinfo.TXN_PARTY_DETAIL_ID = p_TXN_PARTY_DETAIL_ID)
       AND (    ( Recinfo.TXN_LINE_DETAIL_ID = p_TXN_LINE_DETAIL_ID)
            OR (    ( Recinfo.TXN_LINE_DETAIL_ID IS NULL )
                AND (  p_TXN_LINE_DETAIL_ID IS NULL )))
       AND (    ( Recinfo.PARTY_SOURCE_TABLE = p_PARTY_SOURCE_TABLE)
            OR (    ( Recinfo.PARTY_SOURCE_TABLE IS NULL )
                AND (  p_PARTY_SOURCE_TABLE IS NULL )))
       AND (    ( Recinfo.PARTY_SOURCE_ID = p_PARTY_SOURCE_ID)
            OR (    ( Recinfo.PARTY_SOURCE_ID IS NULL )
                AND (  p_PARTY_SOURCE_ID IS NULL )))
       ...
       AND (    ( Recinfo.CONTACT_FLAG = p_CONTACT_FLAG)
            OR (    ( Recinfo.CONTACT_FLAG IS NULL )
                AND (  p_CONTACT_FLAG IS NULL )))
       ...
       AND (    ( Recinfo.PREFERRED_FLAG = p_PREFERRED_FLAG)
            OR (    ( Recinfo.PREFERRED_FLAG IS NULL )
                AND (  p_PREFERRED_FLAG IS NULL )))
       ) then
       return;
   else
