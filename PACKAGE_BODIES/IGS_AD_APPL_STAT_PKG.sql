--------------------------------------------------------
--  DDL for Package Body IGS_AD_APPL_STAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_APPL_STAT_PKG" as
/* $Header: IGSAI12B.pls 115.8 2003/10/30 13:11:12 akadam ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_AD_APPL_STAT%RowType;
  new_references IGS_AD_APPL_STAT%RowType;
  ...
  BEGIN
    ...
    new_references.s_adm_appl_status := x_s_adm_appl_status;
  ...
  BEGIN
	-- Validate Admission application status closed ind.
	IF p_inserting OR ((old_references.s_adm_appl_status <>
				 new_references.s_adm_appl_status) OR
			(old_references.closed_ind = 'Y' AND new_references.closed_ind = 'N')) THEN
		IF IGS_AD_VAL_AAS.admp_val_saas_clsd(
					new_references.s_adm_appl_status,
  ...
  BEGIN
	...
	ELSIF upper(Column_name) = 'S_ADM_APPL_STATUS' Then
    		new_references.s_adm_appl_status := column_value;
	END IF;

	IF upper(column_name) = 'ADM_APPL_STATUS' OR column_name is null Then
     		IF new_references.adm_appl_status <> UPPER(new_references.adm_appl_status ) Then
	...
	IF upper(column_name) = 'S_ADM_APPL_STATUS'  OR column_name is null Then

    		IF new_references.s_adm_appl_status <> UPPER(new_references.s_adm_appl_status ) Then
  ...
  BEGIN
    ...
    IF (((old_references.s_adm_appl_status = new_references.s_adm_appl_status)) OR
        ((new_references.s_adm_appl_status IS NULL))) THEN
      NULL;
    ELSE
      IF NOT IGS_LOOKUPS_VIEW_PKG.Get_PK_For_Validation (
        'ADM_APPL_STATUS',
	  new_references.s_adm_appl_status ) THEN
     		Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
     		IGS_GE_MSG_STACK.ADD;
		App_Exception.Raise_Exception;
      END IF;
  ...
  BEGIN
    ...
    IGS_AD_APPL_PKG.GET_FK_IGS_AD_APPL_STAT (
  ...
  RETURN BOOLEAN  AS
	CURSOR cur_rowid IS
      	SELECT   rowid
      	FROM     IGS_AD_APPL_STAT
      	WHERE    adm_appl_status = x_adm_appl_status AND
 ...
 PROCEDURE GET_FK_IGS_LOOKUPS_VIEW(
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     IGS_AD_APPL_STAT
      WHERE    s_adm_appl_status = x_s_adm_appl_status ;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_AAS_SLV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
	...
	IF (p_action = 'INSERT') THEN
      	...
      	IF  Get_PK_For_Validation (
          		new_references.adm_appl_status ) THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_APPL_STAT
...
begin
  ...
  Before_DML(
   ...
   x_s_adm_appl_status=>X_S_ADM_APPL_STATUS,
...
begin
  ...
  if ( (tlinfo.DESCRIPTION = X_DESCRIPTION)
      AND (tlinfo.S_ADM_APPL_STATUS = X_S_ADM_APPL_STATUS)
...
begin
  ...
  update IGS_AD_APPL_STAT set
    DESCRIPTION = NEW_REFERENCES.DESCRIPTION,
    S_ADM_APPL_STATUS = NEW_REFERENCES.S_ADM_APPL_STATUS,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_APPL_STAT
     where ADM_APPL_STATUS = X_ADM_APPL_STATUS
  ;
begin
  open c1;
