--------------------------------------------------------
--  DDL for Package Body IGS_AD_UNIT_OU_STAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_UNIT_OU_STAT_PKG" as
/* $Header: IGSAI38B.pls 115.5 2003/10/30 13:12:47 akadam ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_AD_UNIT_OU_STAT%RowType;
  new_references IGS_AD_UNIT_OU_STAT%RowType;
  ...
  BEGIN
	-- Validate System admission outcome status closed ind.
	...
	IF (new_references.closed_ind = 'Y' AND new_references.system_default_ind = 'Y') THEN
			...
			App_Exception.Raise_Exception;
	END IF;
	-- Validate System admission outcome status unit outcome ind.
	IF p_inserting OR (old_references.s_adm_outcome_status <>
				 new_references.s_adm_outcome_status) THEN
		IF IGS_AD_VAL_AUOS.admp_val_saos_uniout(
					new_references.s_adm_outcome_status,
					v_message_name) = FALSE THEN
		    Fnd_Message.Set_Name('IGS', v_message_name);
			IGS_GE_MSG_STACK.ADD;
  ...
  BEGIN
    ...
    IGS_AD_PS_APLINSTUNT_PKG.GET_FK_IGS_AD_UNIT_OU_STAT (
...
function Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     IGS_AD_UNIT_OU_STAT
      WHERE    adm_unit_outcome_status = x_adm_unit_outcome_status AND
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_AUOS_SLV_FK');
	  IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
NULL;
  -- Call all the procedures related to After Insert are removed
  END After_DML;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_UNIT_OU_STAT
...
begin
  ...
  insert into IGS_AD_UNIT_OU_STAT (
...
update IGS_AD_UNIT_OU_STAT set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_UNIT_OU_STAT
     where ADM_UNIT_OUTCOME_STATUS = X_ADM_UNIT_OUTCOME_STATUS
  ;
begin
  open c1;
...
begin
  ...
  delete from IGS_AD_UNIT_OU_STAT
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
