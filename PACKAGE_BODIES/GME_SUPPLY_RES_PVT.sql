--------------------------------------------------------
--  DDL for Package Body GME_SUPPLY_RES_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_SUPPLY_RES_PVT" AS
/*  $Header: GMEORESB.pls 120.6.12010000.2 2009/06/15 20:25:49 srpuri ship $
 +=========================================================================+
...
*/
 ...
 BEGIN
  ...
  For batch_line in get_batch_line(p_FPO_batch_id) Loop
     ...
     IF g_debug <= gme_debug.g_log_procedure THEN
        gme_debug.put_line(g_pkg_name||'.'||l_api_name ||' FPO reservation cound '||l_res_count);
     ...
     Fetch get_new_batch_line Into l_batch_line_id, l_planned_qty;
     ...
     For each_whse in check_whse(l_fpo_batch_line_id) Loop
        ...
        for i in 1..l_fpo_rsv_count Loop
           ...
           */
           ...
           ELSE     -- R12 restriting over reserve
...
/*Bug#7422975 Added the bwlo conidion to convert the secondary
  * quantity only when the secondary uom is not null */
            ...
            IF l_txn_lot is not NULL and l_rsv_rec.secondary_uom_code is not
null THEN
          ...
          ELSE
            ...
            -- ==============================================================================================
            IF l_rsv_rec.demand_source_type_id = inv_reservation_global.g_source_type_oe THEN
              GME_SUPPLY_RES_PVT.notify_CSR
                ( P_Batch_id               =>    NULL
