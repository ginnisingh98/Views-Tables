--------------------------------------------------------
--  DDL for Package Body WSH_ITM_PARAMETER_SETUPS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WSH_ITM_PARAMETER_SETUPS_PKG" as
/* $Header: WSHITTHB.pls 120.0.12010000.2 2010/05/25 08:32:39 gbhargav ship $ */
  --  Global constant holding the package name
  G_PKG_NAME      CONSTANT VARCHAR2(30) := 'WSH_ITM_PARAMETER_SETUPS_PKG';
...
--dbms_output.put_line('begin api');
  ...
  SAVEPOINT  WSH_ITM_PARAMETER_SETUPS_PKG;
...
EXCEPTION
   WHEN FND_API.G_EXC_ERROR THEN
     ROLLBACK TO WSH_ITM_PARAMETER_SETUPS_PKG;
	x_return_status := FND_API.G_RET_STS_ERROR;
	FND_MSG_PUB.Count_And_Get
       (       p_count         =>      x_msg_count,
	          p_data          =>      x_msg_data
 ...
 /*  --
   -- Purpose
   --  Check Lock a row of WSH_ITM_PARAMETER_SETUPS entity
...
begin
  ...
  -- Check Lock a row of wsh_itm_parameter_setups
  ...
  close c;
     ...
     IF     ((recinfo.PARAMETER_NAME = p_PARAMETER_NAME)
        ...
        AND (recinfo.USER_SETTABLE = p_USER_SETTABLE)) THEN
        RETURN;
     ELSE
  ...
  for tlinfo in c1 loop
    IF (tlinfo.BASELANG = 'Y') then
      IF  (((tlinfo.USER_PARAMETER_NAME = p_USER_PARAMETER_NAME) OR
          ((tlinfo.USER_PARAMETER_NAME is null) AND (p_USER_PARAMETER_NAME is null)))
         AND ((tlinfo.DESCRIPTION = p_DESCRIPTION) OR
             ((tlinfo.DESCRIPTION is null) AND (p_DESCRIPTION is null)))) then
        RETURN;
      ELSE
        FND_MESSAGE.Set_Name('FND', 'FORM_RECORD_CHANGED');
        app_exception.raise_exception;
 ...
 EXCEPTION
    ...
    WHEN others THEN
      ...
      FND_MESSAGE.Set_Token('PACKAGE', 'WSH_ITM_PARAMETER_SETUPS_PKG.LOCK_ROW');
...
--dbms_output.put_line('begin');
  ...
  -- Update a row into WSH_ITM_parameter_setups entity with all detail information
  -- for the given parameter id
...
--dbms_output.put_line('begin');
  ...
  -- Delete a row from WSH_ITM_parameter_setups entity
  -- for the given parameter id
...
BEGIN
   ...
   BEGIN
	  ...
	  update WSH_ITM_PARAMETER_SETUPS_B set
	    ...
	    --VALUE = x_VALUE, --Bug 9738287  Do not override value field with seeded value.
