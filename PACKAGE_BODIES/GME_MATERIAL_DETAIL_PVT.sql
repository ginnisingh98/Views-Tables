--------------------------------------------------------
--  DDL for Package Body GME_MATERIAL_DETAIL_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_MATERIAL_DETAIL_PVT" AS
/* $Header: GMEVMTLB.pls 120.28.12010000.2 2009/10/01 14:02:32 gmurator ship $ */
           ...
           IF p_item_rec.mtl_transactions_enabled_flag = 'Y' THEN
             ...
             IF l_mmln_tbl.COUNT > 0 THEN
               IF l_mmln_tbl(1).lot_number IS NOT NULL THEN
                 ...
                 l_mmln_tbl(1).secondary_transaction_quantity := NULL;
               END IF;
             END IF;
 ...
 PROCEDURE validate_cost_alloc(
  ...
  /*Sunitha Ch. Bug# 5391396  restructured the code and also added the code that will
    handle  the rescheduling batch/step when update yield Type of the Child batch is done.*/
      ...
      IF     l_old_scrap <> l_new_scrap
           AND l_status = gme_common_pvt.g_batch_pending
           AND l_old_plan_qty = l_new_plan_qty THEN
           l_old_plan_qty := x_material_detail_rec.plan_qty
                             / (1 + l_old_scrap);
           x_material_detail_rec.plan_qty :=
                      l_old_plan_qty
                      * (1 + x_material_detail_rec.scrap_factor);
      ELSIF     l_old_scrap <> l_new_scrap
              AND l_status = gme_common_pvt.g_batch_wip
              AND l_old_wip_plan = l_new_wip_plan THEN
           l_old_plan_qty :=
...
/* 5391396 seperate the condition */
      ...
      /* Bug 4867497 added subinventory locator and qty logic and moved this out of the       IF l_mtl_dtl_rec.release_type <> l_db_mtl_dtl_rec.release_type THEN condn*/

      -- ToDo Put an IF condition to check if material_requirement_date has changed or release_type has changed. Only if either has changed, then we need to do the re-schedule stuff.
    ...
    /*Sunitha Ch. Bug#5391396  rescheduling batch/step when update yield Type
	   ...
	   IF(l_mtl_dtl_rec.release_type = gme_common_pvt.g_mtl_autobystep_release AND
             gme_common_pvt.is_material_assoc_to_step
                                            (l_mtl_dtl_rec.material_detail_id ))  THEN
   ...
   BEGIN
      ...
      IF p_material_detail_rec.plan_qty IS NOT NULL THEN
         IF l_batch_status = gme_common_pvt.g_batch_pending THEN
            ...
            ELSE
               x_material_detail_rec.plan_qty :=
                                               p_material_detail_rec.plan_qty;
               validate_plan_qty
   ...
   BEGIN
      ...
      IF NOT( p_batch_header_rec.batch_status = gme_common_pvt.g_batch_pending OR
       ...
       IF p_material_detail_rec.line_type = gme_common_pvt.g_line_type_prod THEN
         --if product is primary product then donot allow deletion
