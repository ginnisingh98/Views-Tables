--------------------------------------------------------
--  DDL for Package Body JTF_DSPMGRVALIDATION_GRP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_DSPMGRVALIDATION_GRP" AS
/* $Header: JTFGDVDB.pls 115.12 2004/07/09 18:49:46 applrt ship $ */
...
EXCEPTION
	...
	WHEN l_ath_not_exists_exception THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF','JTF_DSP_ATH_NOT_EXISTS');
...
BEGIN
  ...
  THEN
   FND_MESSAGE.set_name('JTF','JTF_DSP_CONTEXT_NOT_EXISTS');
...
BEGIN
   ...
   IF valid_context_cur%NOTFOUND then
     FND_MESSAGE.set_name('JTF','JTF_DSP_CONTEXT_TYPE_INVLD');
...
*/
   ...
   IF valid_object_cur%NOTFOUND then
     FND_MESSAGE.set_name('JTF','JTF_DSP_LGLCTNT_OBJTYPE_INVLD');
...
if not l_return_status then
	FND_MESSAGE.set_name('JTF','JTF_DSP_CONTEXT_ACCNAME_EXISTS');
...
BEGIN
   ...
   THEN
 	  FND_MESSAGE.set_name('JTF','JTF_DSP_CATEGORY_NOT_EXISTS');
...
BEGIN
  ...
  THEN
   FND_MESSAGE.set_name('JTF','JTF_DSP_ITEM_NOT_EXISTS');
...
BEGIN
  ...
  THEN
   FND_MESSAGE.set_name('JTF','JTF_DSP_SECTION_NOT_EXISTS');
...
l_return_status := jtf_dspmgrvalidation_grp.check_valid_object_type(l_object_type);
...
if l_object_type = 'I'
then
   l_return_status :=  jtf_dspmgrvalidation_grp.check_item_exists(p_object_id);
elsif l_object_type = 'C'
then
   l_return_status := jtf_dspmgrvalidation_grp.check_category_exists(p_object_id);
elsif l_object_type = 'S'
then
   l_return_status := jtf_dspmgrvalidation_grp.check_section_exists(p_object_id);
end if;
...
BEGIN
 ...
 l_item_type := trim(p_item_type);
   ...
   if p_applicable_to is null or p_applicable_to = FND_API.g_miss_char then
   ...
   if l_return_status = false then
		FND_MESSAGE.set_name('JTF','JTF_DSP_DLV_TYPE_NOT_EXISTS');
...
EXCEPTION
	...
	WHEN l_dlv_not_exists_exception THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF','JTF_DSP_DLV_NOT_EXISTS');
...
if p_object_version_number = FND_API.G_MISS_NUM  then
  ...
  THEN
   FND_MESSAGE.set_name('JTF','JTF_DSP_LGLCTNT_ID_NOT_EXISTS');
...
BEGIN
	...
	IF TRIM(p_file_name) IS NULL THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF', 'JTF_DSP_ATH_FILENAME_REQ');
			FND_MSG_PUB.add;
		END IF;
...
EXCEPTION
	...
	WHEN l_dup_file_exception THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF','JTF_DSP_ATH_FILENAME_EXISTS');
...
BEGIN
	...
	IF TRIM(p_access_name) IS NULL THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF', 'JTF_DSP_DLV_ACCNAME_REQ');
			FND_MSG_PUB.add;
		END IF;
...
EXCEPTION
	...
	WHEN l_dup_access_exception THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF', 'JTF_DSP_DLV_ACCNAME_EXISTS');
...
----    sysdate between a.start_date_active and nvl(A.end_date_active,sysdate) and
...
EXCEPTION
	...
	WHEN l_lang_not_supp_exception THEN
		IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
			FND_MESSAGE.set_name('JTF', 'JTF_MSITE_LANG_NOT_SUPP' );
			FND_MESSAGE.set_token('ID', TO_CHAR(p_msite_id));
...
if p_deliverable_id is not null and  p_item_id is not null  then
  ...
  if l_return_status = false then
   	FND_MESSAGE.set_name('JTF', 'JTF_DSP_ITEM_DLV_INVLD');
	FND_MESSAGE.set_token('ITEM_ID', p_item_id);
	FND_MESSAGE.set_token('ID', p_deliverable_id);
	FND_MSG_PUB.add;
   END IF;
else
   	FND_MESSAGE.set_name('JTF', 'JTF_DSP_ITEM_DLV_REQ');
	FND_MSG_PUB.add;
END IF;
...
--    2. If not,  message JTF_DSP_CATEGORY_DLV_INVALID
--        is pushed on the stack
---------------------------------------------------------------------
...
if p_deliverable_id is not null and  p_category_id is not null  then
  ...
  IF category_dlv_cur%NOTFOUND THEN
	CLOSE category_dlv_cur;
   	FND_MESSAGE.set_name('JTF', 'JTF_DSP_CATEGORY_DLV_INVLD');
  ...
  END IF;
   	...
   	FND_MESSAGE.set_name('JTF', 'JTF_DSP_CATEGORY_DLV_REQ');
	FND_MSG_PUB.add;
END IF;
...
BEGIN
	...
	if l_msite_id is null then
		FND_MESSAGE.set_name('JTF', 'JTF_MSITE_MASTER_NOT_EXISTS');
		FND_MSG_PUB.add;
	END IF;
...
BEGIN
     ...
     IF NOT l_return_status THEN
          IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.g_msg_lvl_unexp_error) THEN
               FND_MESSAGE.set_name('JTF','JTF_DSP_DLV_ATH_INVLD');
               FND_MESSAGE.set_token('ID', TO_CHAR(p_deliverable_id));
...
-- 2. If Access Name already exists, message JTF_MSITE_DUP_ACCNAME is
--      pushed on stack.
---------------------------------------------------------------------
...
Begin
  ...
  If C_Exists%FOUND Then
   l_status := FALSE ;
   IF FND_MSG_PUB.check_msg_level(FND_MSG_PUB.G_MSG_LVL_UNEXP_ERROR) THEN
      FND_MESSAGE.set_name('JTF','JTF_MSITE_DUP_ACCNAME');
