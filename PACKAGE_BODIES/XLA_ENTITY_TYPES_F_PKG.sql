--------------------------------------------------------
--  DDL for Package Body XLA_ENTITY_TYPES_F_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_ENTITY_TYPES_F_PKG" AS
/* $Header: xlathent.pkb 120.19.12010000.1 2008/07/29 10:09:29 appldev ship $ */
/*======================================================================+
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.xla_entity_types_f_pkg';
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    xla_exceptions_pkg.raise_message
      (p_location   => 'xla_entity_types_f_pkg.trace');
END trace;
...
INSERT INTO xla_entity_types_b
(creation_date
,created_by
,application_id
,entity_code
,enabled_flag
,enable_gapless_events_flag
,last_update_date
,last_updated_by
,last_update_login)
VALUES
...
,x_entity_code
,x_enabled_flag
,x_enable_gapless_events_flag
,x_last_update_date
,x_last_updated_by
,x_last_update_login)
;

INSERT INTO xla_entity_types_tl
(created_by
,last_update_date
,last_updated_by
,last_update_login
...
IF ( (recinfo.enabled_flag                     = x_enabled_flag)
    AND (recinfo.enable_gapless_events_flag = x_enable_gapless_events_flag)
                   ) THEN
   NULL;
ELSE
...
UPDATE xla_entity_types_b
   SET
       last_update_date                 = x_last_update_date
      ,enabled_flag                     = x_enabled_flag
      ,enable_gapless_events_flag    = x_enable_gapless_events_flag
...
DELETE FROM xla_entity_types_tl
WHERE application_id                   = x_application_id
  AND entity_code                      = x_entity_code;
...
DELETE FROM xla_entity_types_b
WHERE application_id                   = x_application_id
  AND entity_code                      = x_entity_code;
...
DELETE FROM xla_entity_types_tl T
WHERE  NOT EXISTS
      (SELECT NULL
       FROM   xla_entity_types_b                 b
       WHERE  b.application_id                   = t.application_id
         AND  b.entity_code                      = t.entity_code);

UPDATE xla_entity_types_tl   t
SET   (name
      ,description)
   = (SELECT b.name
            ,b.description
      FROM   xla_entity_types_tl                b
      WHERE  b.application_id                   = t.application_id
        AND  b.entity_code                      = t.entity_code
...
WHERE (t.application_id
    ...
    IN (SELECT subt.application_id
              ,subt.entity_code
              ,subt.language
        FROM   xla_entity_types_tl                    subb
              ,xla_entity_types_tl                    subt
...
EXCEPTION
   ...
   WHEN OTHERS THEN
   xla_exceptions_pkg.raise_message
      (p_location   => 'xla_entity_types_f_pkg.load_row');
