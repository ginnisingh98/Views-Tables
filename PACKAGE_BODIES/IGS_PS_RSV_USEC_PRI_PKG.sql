--------------------------------------------------------
--  DDL for Package Body IGS_PS_RSV_USEC_PRI_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_RSV_USEC_PRI_PKG" AS
/* $Header: IGSPI1WB.pls 115.5 2003/03/24 15:00:44 sarakshi ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_rsv_usec_pri%ROWTYPE;
  new_references igs_ps_rsv_usec_pri%ROWTYPE;
  ...
  */
  BEGIN
    ...
    igs_ps_rsv_usec_prf_pkg.get_fk_igs_ps_rsv_usec_pri (
      old_references.rsv_usec_pri_id
    );
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_USPR_UOO_UFK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_rsv_usec_pri_s.NEXTVAL
    INTO      x_rsv_usec_pri_id
    FROM      dual;
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_rsv_usec_pri
    WHERE rowid = x_rowid;
