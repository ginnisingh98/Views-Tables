--------------------------------------------------------
--  DDL for Package Body AHL_FMP_MR_INTERVAL_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_FMP_MR_INTERVAL_PVT_W" as
  /* $Header: AHLWMITB.pls 120.2.12010000.2 2009/08/23 02:14:02 bachandr ship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).mr_interval_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).counter_id := a2(indx);
          t(ddindx).counter_name := a3(indx);
          t(ddindx).interval_value := a4(indx);
          t(ddindx).earliest_due_value := a5(indx);
          t(ddindx).start_value := a6(indx);
          t(ddindx).stop_value := a7(indx);
          t(ddindx).start_date := rosetta_g_miss_date_in_map(a8(indx));
          t(ddindx).stop_date := rosetta_g_miss_date_in_map(a9(indx));
          t(ddindx).tolerance_before := a10(indx);
          t(ddindx).tolerance_after := a11(indx);
          t(ddindx).reset_value := a12(indx);
          ...
          t(ddindx).dml_operation := a34(indx);
          t(ddindx).duedate_rule_code := a35(indx);
          t(ddindx).duedate_rule_meaning := a36(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  if t is null or t.count = 0 then
    ...
    a36 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).mr_interval_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).counter_id;
          a3(indx) := t(ddindx).counter_name;
          a4(indx) := t(ddindx).interval_value;
          a5(indx) := t(ddindx).earliest_due_value;
          a6(indx) := t(ddindx).start_value;
          a7(indx) := t(ddindx).stop_value;
          a8(indx) := t(ddindx).start_date;
          a9(indx) := t(ddindx).stop_date;
          a10(indx) := t(ddindx).tolerance_before;
          a11(indx) := t(ddindx).tolerance_after;
          a12(indx) := t(ddindx).reset_value;
          ...
          a34(indx) := t(ddindx).dml_operation;
          a35(indx) := t(ddindx).duedate_rule_code;
          a36(indx) := t(ddindx).duedate_rule_meaning;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_x_interval_tbl ahl_fmp_mr_interval_pvt.interval_tbl_type;
    ddp_x_threshold_rec ahl_fmp_mr_interval_pvt.threshold_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_fmp_mr_interval_pvt_w.rosetta_table_copy_in_p2(ddp_x_interval_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    ddp_x_threshold_rec.mr_effectivity_id := p10_a0;
    ddp_x_threshold_rec.object_version_number := p10_a1;
    ddp_x_threshold_rec.threshold_date := rosetta_g_miss_date_in_map(p10_a2);
    ddp_x_threshold_rec.program_duration := p10_a3;
    ddp_x_threshold_rec.program_duration_uom_code := p10_a4;



    -- here's the delegated call to the old PL/SQL routine
    ahl_fmp_mr_interval_pvt.process_interval(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      p_default,
    ...
    ahl_fmp_mr_interval_pvt_w.rosetta_table_copy_out_p2(ddp_x_interval_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    p10_a0 := ddp_x_threshold_rec.mr_effectivity_id;
    p10_a1 := ddp_x_threshold_rec.object_version_number;
    p10_a2 := ddp_x_threshold_rec.threshold_date;
    p10_a3 := ddp_x_threshold_rec.program_duration;
    p10_a4 := ddp_x_threshold_rec.program_duration_uom_code;
