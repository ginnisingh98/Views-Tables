--------------------------------------------------------
--  DDL for Package Body HZ_FIN_REPORTS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_FIN_REPORTS_EXT_PKG" as
/* $Header: ARHEXFRB.pls 120.4 2006/02/09 10:09:35 jhuang noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from HZ_FIN_REPORTS_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into HZ_FIN_REPORTS_EXT_B (
    EXTENSION_ID,
    FINANCIAL_REPORT_ID,
    ATTR_GROUP_ID,
    C_EXT_ATTR1,
  ...
  insert into HZ_FIN_REPORTS_EXT_TL (
    TL_EXT_ATTR13,
    TL_EXT_ATTR14,
    TL_EXT_ATTR15,
    TL_EXT_ATTR16,
...
begin
  ...
  close c;
  if (    (recinfo.FINANCIAL_REPORT_ID = X_FINANCIAL_REPORT_ID)
...
begin
  update HZ_FIN_REPORTS_EXT_B set
    FINANCIAL_REPORT_ID = X_FINANCIAL_REPORT_ID,
  ...
  update HZ_FIN_REPORTS_EXT_TL set
...
begin
  delete from HZ_FIN_REPORTS_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from HZ_FIN_REPORTS_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  insert into /*+ append parallel(tt) */ HZ_FIN_REPORTS_EXT_TL tt (
    TL_EXT_ATTR13,
    TL_EXT_ATTR14,
    TL_EXT_ATTR15,
    TL_EXT_ATTR16,
  ...
  (SELECT /*+ no_merge ordered parallel(b) */
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
   FROM HZ_FIN_REPORTS_EXT_TL B ,
        FND_LANGUAGES L
   WHERE L.INSTALLED_FLAG IN ( 'I','B' )
   AND B.LANGUAGE = USERENV ( 'LANG' )
  ) v, HZ_FIN_REPORTS_EXT_TL t
