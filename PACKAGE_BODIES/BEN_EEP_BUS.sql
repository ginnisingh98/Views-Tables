--------------------------------------------------------
--  DDL for Package Body BEN_EEP_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EEP_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_eep_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_enrld_anthr_pl_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_enrld_anthr_pl_id PK of record being inserted or updated.
...
--
Procedure chk_elig_enrld_anthr_pl_id(p_elig_enrld_anthr_pl_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_enrld_anthr_pl_id';
...
Begin
  ...
  --
  l_api_updating := ben_eep_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_enrld_anthr_pl_id                => p_elig_enrld_anthr_pl_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_enrld_anthr_pl_id,hr_api.g_number)
     <>  ben_eep_shd.g_old_rec.elig_enrld_anthr_pl_id) then
    --
    -- raise error as PK has changed
    --
    ben_eep_shd.constraint_error('BEN_ELIG_ENRLD_ANTHR_PL_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_enrld_anthr_pl_id is not null then
      --
  ...
  --
End chk_elig_enrld_anthr_pl_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_pl_id >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_pl_id (p_elig_enrld_anthr_pl_id   in number,
                       p_pl_id                  in number,
  ...
  cursor c3 is
     select null
       ...
       where  eep.pl_id = p_pl_id
       and eep.eligy_prfl_id = p_eligy_prfl_id
       and eep.elig_enrld_anthr_pl_id <> nvl(p_elig_enrld_anthr_pl_id,hr_api.g_number)

       and eep.business_group_id + 0 = p_business_group_id
       and p_validation_start_date <= eep.effective_end_date
       and p_validation_end_date >= eep.effective_start_date;
--
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  ...
  if (l_api_updating
     ...
     <> nvl(ben_eep_shd.g_old_rec.pl_id,hr_api.g_number)
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_eep_shd.constraint_error('BEN_ELIG_ENRLD_ANTHR_PL_DT2');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_enrl_det_dt_cd(p_elig_enrld_anthr_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_eep_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_enrld_anthr_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_eep_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--
-- In Parameters
--   p_elig_enrld_anthr_pl_id      elig_enrld_anthr_pl_id
...
is
  ...
  cursor c1 is
    ...
    where  elig_enrld_anthr_pl_id <> nvl(p_elig_enrld_anthr_pl_id,-1)
...
function return_legislation_code
  (p_elig_enrld_anthr_pl_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_enrld_anthr_pl_f b
    where b.elig_enrld_anthr_pl_id      = p_elig_enrld_anthr_pl_id
