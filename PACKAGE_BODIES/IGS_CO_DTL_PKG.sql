--------------------------------------------------------
--  DDL for Package Body IGS_CO_DTL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_CO_DTL_PKG" AS
/* $Header: IGSLI05B.pls 115.6 2002/11/29 01:03:21 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_co_dtl%ROWTYPE;
  new_references igs_co_dtl%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.s_cord_format_type                := x_s_cord_format_type;
    new_references.cord_text                         := x_cord_text;
  ...
  */
  BEGIN
    ...
    ELSIF (UPPER(column_name) = 'S_CORD_FORMAT_TYPE') THEN
      new_references.s_cord_format_type := column_value;
    END IF;
    ...
    IF (UPPER(column_name) = 'S_CORD_FORMAT_TYPE' OR column_name IS NULL) THEN
      IF NOT (new_references.s_cord_format_type IN ('TEXT', 'POSTSCRIPT', 'UNDEFINED', 'OLE', 'REFERENCE'))  THEN
        fnd_message.set_name('IGS','IGS_GE_INVALID_VALUE');
        igs_ge_msg_stack.add;
        app_exception.raise_exception;
      END IF;
  ...
  */
  BEGIN
    ...
    IF (((old_references.correspondence_type = new_references.correspondence_type) AND
         (old_references.reference_number = new_references.reference_number)) OR
        ((new_references.correspondence_type IS NULL) OR
         (new_references.reference_number IS NULL))) THEN
      NULL;
    ELSIF NOT igs_co_itm_pkg.get_pk_for_validation (
  ...
  */
  BEGIN
    ...
    igs_co_dtl_ole_pkg.get_fk_igs_co_dtl (
  ...
  */
    CURSOR cur_rowid IS
      ...
      WHERE   ((correspondence_type = x_correspondence_type) AND
               (reference_number = x_reference_number));
  ...
  */
  ...
  BEGIN
    ...
    before_dml(
      ...
      x_s_cord_format_type                => x_s_cord_format_type,
      x_cord_text                         => x_cord_text,
    ...
    INSERT INTO igs_co_dtl (
      ...
      new_references.correspondence_type,
      new_references.reference_number,
      new_references.s_cord_format_type,
      new_references.cord_text,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.s_cord_format_type = x_s_cord_format_type)
        AND ((tlinfo.cord_text = x_cord_text) OR ((tlinfo.cord_text IS NULL) AND (X_cord_text IS NULL)))
        AND ((tlinfo.external_reference = x_external_reference) OR ((tlinfo.external_reference IS NULL) AND (X_external_reference IS NULL)))
       ) THEN
      NULL;
    ELSE
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_co_dtl
      SET
        s_cord_format_type                = new_references.s_cord_format_type,
        cord_text                         = new_references.cord_text,
