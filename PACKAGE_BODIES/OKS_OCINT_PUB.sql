--------------------------------------------------------
--  DDL for Package Body OKS_OCINT_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKS_OCINT_PUB" AS
/* $Header: OKSPOCIB.pls 120.27.12010000.3 2009/06/08 08:36:20 cgopinee ship $ */
----------------------------------------------------------------------------------------
...
--For Creating Contract
      ...
      CURSOR l_contract_csr (p_ordlineid NUMBER)
      IS
      ...
      CURSOR l_contact_csr (p_line_id NUMBER)
      IS
...
/*COMMENTED AND MODIFIED FOR BUG6181908 -- FP BUG6006309
...
*/
	...
	Cursor l_custprod_csr (p_ordlineid Number
      ...
      CURSOR l_organization_csr (p_org_id NUMBER)
      IS
         SELECT master_organization_id
           FROM oe_system_parameters_all
          WHERE org_id = p_org_id;

      CURSOR l_serv_ref_csr (p_ordline_id NUMBER)
      IS
         SELECT service_reference_type_code, service_reference_line_id
           FROM okx_order_lines_v
          WHERE id1 = p_ordline_id;

      CURSOR l_contract_line_csr (p_subline_id NUMBER)
      IS
         SELECT cle_id
           FROM okc_k_lines_b
          WHERE ID = p_subline_id;

      /*cgopinee bugfix for bug 8276025*/
      CURSOR l_pricing_qty_csr(p_ordline_id Number)
      IS
...
--General
      ...
      PROCEDURE create_contract (
         p_reproc_line_rec                oks_rep_pvt.repv_rec_type,
	 ...
	 /* Fix Ends */
         ...
         fnd_file.put_line
                        (fnd_file.LOG,
                            'OC INTERFACE :- get_contract_header_info status '
                         || l_return_status
                        );
         ...
         THEN
            ...
            THEN
               ...
               LOOP
                  ...
                  fnd_file.put_line (fnd_file.LOG,
                                     'GET CONTRACT HDR FAILURE ' || l_msg_data
                                    );
               END LOOP;
            END IF;
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- get_k_service_line status '
                            || l_return_status
                           );
         ...
         THEN
            ...
            THEN
               ...
               LOOP
                  ...
                  l_msg_data := fnd_message.get;
                  fnd_file.put_line (fnd_file.LOG,
                                     'GET ORDER LINE FAILURE ' || l_msg_data
                                    );
               END LOOP;
            END IF;
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- get_renew_rules status '
                            || l_return_status
                           );
         ...
         THEN
            ...
            LOOP
               ...
               fnd_file.put_line (fnd_file.LOG,
                                  'RENEWAL RULE FAILURE ' || l_msg_data
                                 );
            END LOOP;
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- get_K_order_details status '
                            || l_return_status
                           );
...
--mmadhavi added following logic for Partial Periods Computation
         ...
         IF l_period_start IS NOT NULL AND l_period_type IS NOT NULL
         THEN
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- get_period_defaults '
                            || l_return_status
                           );
         fnd_file.put_line
                     (fnd_file.LOG,
                         'OC INTERFACE :- get_period_defaults l_period_start '
                      || l_period_start
                     );
         fnd_file.put_line
                      (fnd_file.LOG,
                          'OC INTERFACE :- get_period_defaults l_period_type '
                       || l_period_type
                      );
         ...
         THEN
            ...
            LOOP
               ...
               fnd_file.put_line (fnd_file.LOG,
                                  'get_period_defaults FAILURE ' || l_msg_data
                                 );
            END LOOP;
...
--mmadhavi end Partial Periods logic
         IF    l_renewal_rec.chr_id IS NOT NULL
            OR l_renewal_rec.link_chr_id IS NOT NULL
         THEN
            l_hdr_rec.merge_type := 'LTC';
            l_hdr_rec.merge_object_id :=
                        NVL (l_renewal_rec.chr_id, l_renewal_rec.link_chr_id);
         ELSE
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- Create K Hdr status '
                            || l_return_status
                           );
         ...
         fnd_file.put_line (fnd_file.LOG,
                            'OC INTERFACE :- party name ' || l_party_name
                           );

         IF fnd_profile.VALUE ('OKS_CONTRACT_CATEGORY') = 'SERVICE'
         THEN
         ...
         ELSIF fnd_profile.VALUE ('OKS_CONTRACT_CATEGORY') = 'SUBSCRIPTION'
         THEN
         ...
         ELSIF    fnd_profile.VALUE ('OKS_CONTRACT_CATEGORY') = 'WARRANTY'
               OR fnd_profile.VALUE ('OKS_CONTRACT_CATEGORY') IS NULL
         THEN
...
--changing l_extwar_rec.merge...to l_hdr_rec
         IF l_hdr_rec.merge_object_id IS NOT NULL
         THEN
            ...
            IF l_hdr_scs_code <> fnd_profile.VALUE ('OKS_CONTRACT_CATEGORY')
            THEN
         ...
         FOR l_contact_rec IN l_contact_csr (p_reproc_line_rec.order_line_id)
         LOOP
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- SalesCredit at Header '
                            || l_return_status
                           );
         ...
         THEN
            ...
            LOOP
               ...
               fnd_file.put_line (fnd_file.LOG,
                                     'READ SALES CREDIT ERROR IN HEADER'
                                  || l_msg_data
                                 );
            END LOOP;
...
--mmadhavi end Partial Periods Computation
         ...
         /* Added by sjanakir for Bug#7445270 */
         l_hdr_rec.ar_interface_yn       := 'Y';
         ...
         THEN
            ...
            THEN
               ...
               LOOP
                  ...
                  l_msg_data := fnd_message.get;
                  fnd_file.put_line (fnd_file.LOG,
                                     'Create_k_hdr FAILURE ' || l_msg_data
                                    );
               END LOOP;
            END IF;
         ...
         -- Fix for Bug 2805292
         IF l_hdr_rec.order_hdr_id IS NOT NULL
            AND l_hdr_rec.merge_type = 'NEW'
         THEN
            ...
            fnd_file.put_line
               (fnd_file.LOG,
                   '(OKS_EXTWARPRGM_PVT).okc_oc_int_pub.create_k_relationships '
                || l_return_status
               );
            ...
            THEN
               ...
               LOOP
                  ...
                  fnd_file.put_line (fnd_file.LOG,
                                     'K HDR CREATION ERROR ' || l_msg_data
                                    );
               END LOOP;
         ...
         fnd_file.put_line (fnd_file.LOG,
                            'OC INTERFACE :- SalesCredit ' || l_return_status
                           );
         ...
         THEN
            ...
            LOOP
               ...
               fnd_file.put_line (fnd_file.LOG,
                                  'READ SALES CREDIT ERROR ' || l_msg_data
                                 );
            END LOOP;
         ...
         IF l_salescredit_tbl_in.count = 0 THEN
            ...
            fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- SalesCredit at Header - defaulting for order line id = '||p_reproc_line_rec.order_id
                            || l_return_status
                           );
         ...
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- Create K Line ID =  '
                            || NVL (l_lineid, -1234)
                           );
         fnd_file.put_line (fnd_file.LOG,
                               'OC INTERFACE :- Create K Line status '
                            || l_return_status
                           );
...
--Copy line id for Reprocessing
         ...
         FOR cp_ctr IN 1 .. l_cp_tbl.COUNT
         LOOP
            IF (l_error <> 'Y')
            THEN
               ...
               FETCH l_cp_csr
                INTO l_cp_id, l_cp_inventory, l_cp_name, l_cp_desc, l_cp_qty,
                     l_cp_uom;
...
----mmadhavi added bom_explosion
               ...
               /*cgopinee bugfix for bug8276025*/
               OPEN l_pricing_qty_csr(p_reproc_line_rec.order_line_id);
               FETCH l_pricing_qty_csr into l_pricing_qty;
               CLOSE l_pricing_qty_csr;

               IF l_serv_ref_rec.service_reference_type_code = 'ORDER'
               THEN
                  ...
                  IF l_inv_item_id = l_cp_inventory
                  THEN
                     ...
                     /* Divide the tax amount only when the pricing_qty is not same as cp_qty
                     due to profile CSI_AUTO_SPLIT_INSTANCES - bug8330411*/
                     IF (l_pricing_qty<>l_cp_qty)
                     THEN
                         l_covd_rec.tax_amount := l_k_line_rec.tax_amount/l_pricing_qty;   -- Bug# 5274971
                     ELSE
...
--bug 3761489
        ...
        --
               ...
               fnd_file.put_line
                        (fnd_file.LOG,
                            'l_BOM_instance_flag = '|| l_BOM_instance_flag
                        );
               ...
               IF l_BOM_instance_flag = 'Y' THEN
                  l_BOM_instance_id := l_covlvl_id;
                   ...
                   fnd_file.put_line
                        (fnd_file.LOG,
                            'l_BOM_CovLvl_id = '|| TO_CHAR (l_BOM_instance_id)
                        );
               ...
               IF l_update_line = 'Y' THEN
                  l_eff_line_upd_flag := 'Y';
                   ...
                   fnd_file.put_line
                        (fnd_file.LOG,
                            'l_eff_line_upd_flag = '|| l_eff_line_upd_flag
                        );
               ...
               fnd_file.put_line
                        (fnd_file.LOG,
                            'OC INTERFACE :- Create K Covd Line Subline ID = '
                         || TO_CHAR (l_covlvl_id)
                        );
               fnd_file.put_line
                              (fnd_file.LOG,
                                  'OC INTERFACE :- Create K Covd Line status '
                               || l_return_status
                              );
               ...
               THEN
                  ...
                  THEN
                     ...
                     LOOP
                        ...
                        fnd_file.put_line
                                        (fnd_file.LOG,
                                            'Create_K_Covered_levels FAILURE '
                                         || l_msg_data
                                        );
                     END LOOP;
                  END IF;
               ...
               l_inst_dtls_rec.new_contact_start_date :=
                                                     l_hdrdates_rec.start_date;
               l_inst_dtls_rec.new_contract_end_date :=
                                                       l_hdrdates_rec.end_date;
               l_inst_dtls_rec.new_service_line_id := l_lineid;
               l_inst_dtls_rec.new_service_start_date :=
               ...
               fnd_file.put_line (fnd_file.LOG,
                                     ' instance history details Status  : '
                                  || l_return_status
                                 );
               x_return_status := l_return_status;
...
--Added for fix of bug# 5165947
         ...
         ELSE
            ...
            fnd_file.put_line (fnd_file.LOG,
                                'l_prev_line_amt  : '||to_char(l_prev_line_amt)
                               );
         END IF;
...
--Added for fix of bug# 5274971
         ...
         THEN
            ...
            IF     l_period_start IS NOT NULL
               AND l_period_type IS NOT NULL
               AND l_period_start = 'CALENDAR'
            THEN
               l_sll_tbl (1).uom_code := 'DAY';
               l_sll_tbl (1).uom_per_period :=
                              l_k_line_rec.srv_edt - l_k_line_rec.srv_sdt + 1;
            ELSE
            ...
            fnd_file.put_line (fnd_file.LOG,
                                  'Create_Contract :- CREATE_BCL_FOR_OM '
                               || l_return_status
                              );
...
--Added for fix of bug# 5165947
         ...
         ELSE
            ...
            fnd_file.put_line (fnd_file.LOG,
                                'l_curr_line_amt  : '||to_char(l_curr_line_amt)
                               );
            ...
            if nvl(l_curr_line_amt,0) <> nvl(l_prev_line_amt,0) THEN
              ...
              fnd_file.put_line (fnd_file.LOG,
                                'l_eff_line_upd_flag (final)  : '||l_eff_line_upd_flag
                               );
 ...
 --Added for fix of bug# 5165947
               ...
               ELSE
                  ...
                  fnd_file.put_line
                                  (fnd_file.LOG,
                                      'Create_Contract :- Create_Bill_Sch_CP '
                                   || l_return_status
                                  );
