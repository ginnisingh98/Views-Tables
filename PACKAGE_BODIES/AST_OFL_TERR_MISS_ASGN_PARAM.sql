--------------------------------------------------------
--  DDL for Package Body AST_OFL_TERR_MISS_ASGN_PARAM
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AST_OFL_TERR_MISS_ASGN_PARAM" AS
/* $Header: astrtspb.pls 115.9 2002/02/07 15:24:46 pkm ship      $ */
...
g_image_prefix varchar2(250) := '/OA_MEDIA/'||icx_sec.getid(icx_sec.pv_language_code)||'/';
   ...
   ctr1           integer        := 0;
   ctr2           integer        := 0;
   --G_DATE_FORMAT  varchar2(20)   := ast_ofl_util_pkg.get_date_format;
...
-----------------------------------------------------------------------------------------------
   begin
     ...
     htp.htmlopen;
     htp.headOpen;
     htp.title('Missing Territory Assignments Report');
     htp.headClose;
     ...
     htp.tableData( '<FONT size=+1 face="times new roman">' || 'Missing Territory Assignments Report', cnowrap => 'TRUE');
...
-----------------------------------------------------------------------------------------------
procedure terr_miss_param is

-----------------------------------------------------------------------------------------------
--
--  PROCEDURE: Missing Territory Assignments Report
--
...
if (icx_sec.validateSession) then
       ...
       htp.FormOpen(owa_util.Get_Owa_Service_Path||'AST_OFL_TERR_MISS_ASGN_report.report_wrapper',
 ...
 BEGIN
      ...
      htp.tableRowOpen;
      htp.formOpen( curl => l_agent||'/AST_OFL_TERR_MISS_ASGN_report.report_wrapper', cmethod => 'GET',
