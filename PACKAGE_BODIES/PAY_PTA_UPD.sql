--------------------------------------------------------
--  DDL for Package Body PAY_PTA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PTA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_pta_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the pay_dated_tables Row
  --
  update pay_dated_tables
    ...
    ,surrogate_key_name              = p_rec.surrogate_key_name
    ,start_date_name                 = p_rec.start_date_name
    ,end_date_name                   = p_rec.end_date_name
    ,business_group_id               = p_rec.business_group_id
    ,legislation_code                = p_rec.legislation_code
    ,object_version_number           = p_rec.object_version_number
    ,dyn_trigger_type                = p_rec.dyn_trigger_type
    ,dyn_trigger_package_name        = p_rec.dyn_trigger_package_name
    ,dyn_trig_pkg_generated          = p_rec.dyn_trig_pkg_generated
...
Begin
  ...
  If (p_rec.application_id = hr_api.g_number) then
    ...
    pay_pta_shd.g_old_rec.application_id;
  End If;
  If (p_rec.surrogate_key_name = hr_api.g_varchar2) then
    p_rec.surrogate_key_name :=
    pay_pta_shd.g_old_rec.surrogate_key_name;
  End If;
  If (p_rec.start_date_name = hr_api.g_varchar2) then
    p_rec.start_date_name :=
    pay_pta_shd.g_old_rec.start_date_name;
  End If;
  If (p_rec.end_date_name = hr_api.g_varchar2) then
    p_rec.end_date_name :=
    pay_pta_shd.g_old_rec.end_date_name;
  End If;
  ...
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    ...
    pay_pta_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.dyn_trigger_type = hr_api.g_varchar2) then
    p_rec.dyn_trigger_type :=
    pay_pta_shd.g_old_rec.dyn_trigger_type;
  End If;
  If (p_rec.dyn_trigger_package_name = hr_api.g_varchar2) then
    p_rec.dyn_trigger_package_name :=
    pay_pta_shd.g_old_rec.dyn_trigger_package_name;
  End If;
  If (p_rec.dyn_trig_pkg_generated = hr_api.g_varchar2) then
    p_rec.dyn_trig_pkg_generated :=
    pay_pta_shd.g_old_rec.dyn_trig_pkg_generated;
  End If;
  --
...
Begin
  ...
  --
  convert_defs(p_rec);
  pay_pta_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  pay_pta_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pay_pta_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pay_pta_upd.post_update
     (p_rec
     );
End upd;
--
