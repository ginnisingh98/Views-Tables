--------------------------------------------------------
--  DDL for Package Body IGS_AS_SPCL_CONS_CAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_SPCL_CONS_CAT_PKG" as
/* $Header: IGSDI47B.pls 115.7 2003/05/19 04:44:12 ijeddy ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_AS_SPCL_CONS_CAT%RowType;
  new_references IGS_AS_SPCL_CONS_CAT%RowType;
  ...
  BEGIN
    ...
    IGS_AS_SPL_CONS_APPL_PKG.GET_FK_IGS_AS_SPCL_CONS_OUT(
      old_references.spcl_consideration_cat
      );
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AS_SPCL_CONS_CAT
      where SPCL_CONSIDERATION_CAT = X_SPCL_CONSIDERATION_CAT;
...
begin
 ...
 Before_DML(
  ...
  insert into IGS_AS_SPCL_CONS_CAT (
...
begin
 ...
 Before_DML(
  ...
  update IGS_AS_SPCL_CONS_CAT set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AS_SPCL_CONS_CAT
     where SPCL_CONSIDERATION_CAT = X_SPCL_CONSIDERATION_CAT
  ;
begin
  open c1;
...
begin
 Before_DML(
  ...
  delete from IGS_AS_SPCL_CONS_CAT
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
