--------------------------------------------------------
--  DDL for Package Body PER_CHK_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CHK_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_chk_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (p_constraint_name = 'PER_CHECKLIST_ITEMS_PK') Then
    fnd_message.set_name('PAY', 'HR_7220_INVALID_PRIMARY_KEY');
    fnd_message.raise_error;
  ElsIf (p_constraint_name = 'PER_CHECKLIST_ITEMS_U1') Then
    fnd_message.set_name('PER', 'HR_52779_CHK_ITEM_DUP_VAL');
    fnd_message.raise_error;
  Else
...
Begin
  ...
  --
  If (
	p_checklist_item_id is null and
  ...
  Else
    If (
	p_checklist_item_id = g_old_rec.checklist_item_id and
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'per_checklist_items');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  l_rec.item_code                        := p_item_code;
  l_rec.date_due                         := p_date_due;
  l_rec.date_done                        := p_date_done;
