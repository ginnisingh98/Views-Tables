--------------------------------------------------------
--  DDL for Package Body PSA_MF_TRX_DIST_ALL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSA_MF_TRX_DIST_ALL_PKG" AS
/* $Header: PSAMFTDB.pls 120.5 2006/09/13 13:51:32 agovil ship $ */
--===========================FND_LOG.START=====================================
...
g_path        VARCHAR2(50)  := 'PSA.PLSQL.PSAMFTDB.PSA_MF_TRX_DIST_ALL_PKG.';
--===========================FND_LOG.END=======================================
...
begin
  ...
  insert into PSA_MF_TRX_DIST_ALL (
...
begin
  ...
  if ( (tlinfo.MF_RECEIVABLES_CCID = X_MF_RECEIVABLES_CCID)
      AND ((tlinfo.POSTING_CONTROL_ID = X_POSTING_CONTROL_ID)
           OR ((tlinfo.POSTING_CONTROL_ID is null)
               AND (X_POSTING_CONTROL_ID is null)))
      AND ((tlinfo.PREV_MF_RECEIVABLES_CCID = X_PREV_MF_RECEIVABLES_CCID)
           OR ((tlinfo.PREV_MF_RECEIVABLES_CCID is null)
               AND (X_PREV_MF_RECEIVABLES_CCID is null)))
...
begin
  ...
  update PSA_MF_TRX_DIST_ALL set
    MF_RECEIVABLES_CCID = X_MF_RECEIVABLES_CCID,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from PSA_MF_TRX_DIST_ALL
     where CUST_TRX_LINE_GL_DIST_ID = X_CUST_TRX_LINE_GL_DIST_ID
  ;
  dummy c1%rowtype;
begin
