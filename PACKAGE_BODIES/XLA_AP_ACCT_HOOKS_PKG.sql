--------------------------------------------------------
--  DDL for Package Body XLA_AP_ACCT_HOOKS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_AP_ACCT_HOOKS_PKG" AS
-- $Header: xlaapaph.pkb 120.2 2006/06/28 05:48:48 vinaykum noship $
/*===========================================================================+
|  Copyright (c) 2003 Oracle Corporation BelmFont, California, USA           |
|                          ALL rights reserved.                              |
+============================================================================+
| PACKAGE NAME                                                               |
|     xla_ap_acct_hooks_pkg                                                  |
|                                                                            |
| DESCRIPTION                                                                |
|     Call accounting program integration APIs for Account Payables          |
|                                                                            |
| HISTORY                                                                    |
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.xla_ap_acct_hooks_pkg';
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_ap_acct_hooks_pkg.trace');
END trace;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_ap_acct_hooks_pkg.print_logfile');
...
BEGIN
   ...
   WHEN  p_event_name = 'preaccounting'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call preaccounting API for Account Payables'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ap_sla_processing_pkg.preaccounting
   ...
   WHEN p_event_name = 'extract'        THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call extract API for Account Payables'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;
   ...
   WHEN p_event_name = 'postprocessing'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call postprocessing API for Account Payables'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ap_sla_processing_pkg.postprocessing
            ( p_application_id     => p_application_id
             ,p_accounting_mode    => p_accounting_mode);
   ...
   WHEN p_event_name = 'postaccounting'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call postaccounting API for Account Payables'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ap_sla_processing_pkg.postaccounting
            ( p_application_id     => p_application_id
             ,p_ledger_id          => p_ledger_id
             ,p_process_category   => p_process_category
             ,p_end_date           => p_end_date
...
WHEN xla_exceptions_pkg.application_exception THEN
   ...
   xla_exceptions_pkg.raise_message
      ...
      ,p_token_1        => 'LOCATION'
      ,p_value_1        => 'xla_ap_acct_hooks_pkg.main'
      ,p_token_2        => 'ERROR'
