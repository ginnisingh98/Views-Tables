--------------------------------------------------------
--  DDL for Package Body EAM_NETWORK_ASSETS_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."EAM_NETWORK_ASSETS_PVT" as
/* $Header: EAMVNWKB.pls 115.1 2002/11/22 22:56:01 aan noship $ */
 -- Start of comments
 ...
 -- End of comments
   ...
   g_pkg_name    CONSTANT VARCHAR2(30):= 'eam_network_assets_pvt';
...
PROCEDURE LOCK_ROW(
  ...
  BEGIN
   ...
   -- API body
    ...
    CLOSE C;
    if (
       (Recinfo.NETWORK_ASSOCIATION_ID =  P_NETWORK_ASSOCIATION_ID)
       AND (   (Recinfo.ORGANIZATION_ID =  P_ORGANIZATION_ID)
            OR (    (Recinfo.ORGANIZATION_ID IS NULL)
                AND (P_ORGANIZATION_ID IS NULL)))
       AND (Recinfo.NETWORK_OBJECT_TYPE =  P_NETWORK_OBJECT_TYPE)
       AND (Recinfo.NETWORK_OBJECT_ID =  P_NETWORK_OBJECT_ID)
       AND (Recinfo.MAINTENANCE_OBJECT_TYPE =  P_MAINTENANCE_OBJECT_TYPE)
       AND (Recinfo.MAINTENANCE_OBJECT_ID =  P_MAINTENANCE_OBJECT_ID)
       AND (Recinfo.NETWORK_ITEM_ID =  P_NETWORK_ITEM_ID)
       AND (Recinfo.NETWORK_SERIAL_NUMBER =  P_NETWORK_SERIAL_NUMBER)
