--------------------------------------------------------
--  DDL for Package Body PQH_FR_STAT_SITUATIONS_BK2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_FR_STAT_SITUATIONS_BK2" as
/* $Header: pqstsapi.pkb 120.0 2005/05/29 02:43:03 generated noship $ */
-- Code generated by the Oracle HRMS API Hook Pre-processor
-- Created on 2007/01/04 09:35:43 (YYYY/MM/DD HH24:MI:SS)
procedure UPDATE_STATUTORY_SITUATION_A
(P_EFFECTIVE_DATE in DATE
,P_BUSINESS_GROUP_ID in NUMBER
,P_SITUATION_NAME in VARCHAR2
,P_TYPE_OF_PS in VARCHAR2
,P_SITUATION_TYPE in VARCHAR2
,P_SUB_TYPE in VARCHAR2
,P_SOURCE in VARCHAR2
,P_LOCATION in VARCHAR2
,P_REASON in VARCHAR2
,P_IS_DEFAULT in VARCHAR2
,P_DATE_FROM in DATE
,P_DATE_TO in DATE
,P_REQUEST_TYPE in VARCHAR2
,P_EMPLOYEE_AGREEMENT_NEEDED in VARCHAR2
,P_MANAGER_AGREEMENT_NEEDED in VARCHAR2
,P_PRINT_ARRETTE in VARCHAR2
,P_RESERVE_POSITION in VARCHAR2
,P_ALLOW_PROGRESSIONS in VARCHAR2
,P_EXTEND_PROBATION_PERIOD in VARCHAR2
,P_REMUNERATION_PAID in VARCHAR2
,P_PAY_SHARE in NUMBER
,P_PAY_PERIODS in NUMBER
,P_FREQUENCY in VARCHAR2
,P_FIRST_PERIOD_MAX_DURATION in NUMBER
,P_MIN_DURATION_PER_REQUEST in NUMBER
,P_MAX_DURATION_PER_REQUEST in NUMBER
,P_MAX_DURATION_WHOLE_CAREER in NUMBER
,P_RENEWABLE_ALLOWED in VARCHAR2
,P_MAX_NO_OF_RENEWALS in NUMBER
,P_MAX_DURATION_PER_RENEWAL in NUMBER
,P_MAX_TOT_CONTINUOUS_DURATION in NUMBER
,P_STATUTORY_SITUATION_ID in NUMBER
,P_OBJECT_VERSION_NUMBER in NUMBER
,P_REMUNERATE_ASSIGN_STATUS_ID in NUMBER
)is
begin
hr_utility.set_location('Entering: PQH_FR_STAT_SITUATIONS_BK2.UPDATE_STATUTORY_SITUATION_A', 10);
hr_utility.set_location(' Leaving: PQH_FR_STAT_SITUATIONS_BK2.UPDATE_STATUTORY_SITUATION_A', 20);
end UPDATE_STATUTORY_SITUATION_A;
procedure UPDATE_STATUTORY_SITUATION_B
(P_EFFECTIVE_DATE in DATE
,P_BUSINESS_GROUP_ID in NUMBER
,P_SITUATION_NAME in VARCHAR2
,P_TYPE_OF_PS in VARCHAR2
,P_SITUATION_TYPE in VARCHAR2
,P_SUB_TYPE in VARCHAR2
,P_SOURCE in VARCHAR2
,P_LOCATION in VARCHAR2
,P_REASON in VARCHAR2
,P_IS_DEFAULT in VARCHAR2
,P_DATE_FROM in DATE
,P_DATE_TO in DATE
,P_REQUEST_TYPE in VARCHAR2
,P_EMPLOYEE_AGREEMENT_NEEDED in VARCHAR2
,P_MANAGER_AGREEMENT_NEEDED in VARCHAR2
,P_PRINT_ARRETTE in VARCHAR2
,P_RESERVE_POSITION in VARCHAR2
,P_ALLOW_PROGRESSIONS in VARCHAR2
,P_EXTEND_PROBATION_PERIOD in VARCHAR2
,P_REMUNERATION_PAID in VARCHAR2
,P_PAY_SHARE in NUMBER
,P_PAY_PERIODS in NUMBER
,P_FREQUENCY in VARCHAR2
,P_FIRST_PERIOD_MAX_DURATION in NUMBER
,P_MIN_DURATION_PER_REQUEST in NUMBER
,P_MAX_DURATION_PER_REQUEST in NUMBER
,P_MAX_DURATION_WHOLE_CAREER in NUMBER
,P_RENEWABLE_ALLOWED in VARCHAR2
,P_MAX_NO_OF_RENEWALS in NUMBER
,P_MAX_DURATION_PER_RENEWAL in NUMBER
,P_MAX_TOT_CONTINUOUS_DURATION in NUMBER
,P_STATUTORY_SITUATION_ID in NUMBER
,P_OBJECT_VERSION_NUMBER in NUMBER
,P_REMUNERATE_ASSIGN_STATUS_ID in NUMBER
)is
begin
hr_utility.set_location('Entering: PQH_FR_STAT_SITUATIONS_BK2.UPDATE_STATUTORY_SITUATION_B', 10);
hr_utility.set_location(' Leaving: PQH_FR_STAT_SITUATIONS_BK2.UPDATE_STATUTORY_SITUATION_B', 20);
end UPDATE_STATUTORY_SITUATION_B;
end PQH_FR_STAT_SITUATIONS_BK2;

/
