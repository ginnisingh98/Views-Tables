--------------------------------------------------------
--  DDL for Package Body ICX_CAT_POPULATE_BPA_GRP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ICX_CAT_POPULATE_BPA_GRP" AS
/* $Header: ICXGPPBB.pls 120.1 2005/12/13 15:28:01 sbgeorge noship $*/

-- Constants
G_PKG_NAME      CONSTANT VARCHAR2(30):='ICX_CAT_POPULATE_BPA_GRP';
...
BEGIN
  ...
  SAVEPOINT populateOnlineBlankets_sp;
  ...
  l_err_loc := 600;
  ICX_CAT_POPULATE_PODOCS_PVT.populateOnlineBlankets(p_key);
...
EXCEPTION
  WHEN OTHERS THEN
    BEGIN
      ROLLBACK TO populateOnlineBlankets_sp;
    EXCEPTION
      WHEN OTHERS THEN
...
BEGIN
  ...
  l_err_loc := 200;
  -- Standard Start of API savepoint
  SAVEPOINT populateOnlineOrgAssgnmnts_sp;
  ...
  l_err_loc := 600;
  ICX_CAT_POPULATE_PODOCS_PVT.populateOnlineOrgAssgnmnts(p_key);
...
EXCEPTION
  WHEN OTHERS THEN
    BEGIN
      ROLLBACK TO populateOnlineOrgAssgnmnts_sp;
    EXCEPTION
      WHEN OTHERS THEN
        IF (FND_LOG.LEVEL_EXCEPTION >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
          FND_LOG.string(FND_LOG.LEVEL_EXCEPTION,
