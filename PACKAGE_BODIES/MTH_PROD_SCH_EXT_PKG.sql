--------------------------------------------------------
--  DDL for Package Body MTH_PROD_SCH_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MTH_PROD_SCH_EXT_PKG" as
/* $Header: mthuwtlb.pls 120.0.12010000.3 2009/08/21 09:54:35 sdonthu ship $ */
/*
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from MTH_PRODUCTION_SCHEDULES_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into MTH_PRODUCTION_SCHEDULES_EXT_B (
    D_EXT_ATTR4,
    D_EXT_ATTR5,
    D_EXT_ATTR6,
    D_EXT_ATTR7,
  ...
  insert into MTH_PRODUCTION_SCHEDULE_EXT_TL (
    EXTENSION_ID,
    ATTR_GROUP_ID,
    WORKORDER_PK_KEY,
    TL_EXT_ATTR1,
  ...
  and not exists
    (select NULL
    from MTH_PRODUCTION_SCHEDULE_EXT_TL T
...
procedure LOCK_ROW (
  ...
  cursor c is select
      ...
      D_EXT_ATTR2,
      D_EXT_ATTR3
    from MTH_PRODUCTION_SCHEDULES_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
  ...
  cursor c1 is select
    ...
    from MTH_PRODUCTION_SCHEDULE_EXT_TL
    where EXTENSION_ID = X_EXTENSION_ID
...
begin
  ...
  close c;
  if (    ((recinfo.D_EXT_ATTR4 = X_D_EXT_ATTR4)
      ...
      AND ((recinfo.WORKORDER_PK_KEY = X_WORKORDER_PK_KEY)
           OR ((recinfo.WORKORDER_PK_KEY is null) AND (X_WORKORDER_PK_KEY is null)))
...
begin
  update MTH_PRODUCTION_SCHEDULES_EXT_B set
    ...
    WORKORDER_PK_KEY = X_WORKORDER_PK_KEY,
  ...
  update MTH_PRODUCTION_SCHEDULE_EXT_TL set
...
begin
  delete from MTH_PRODUCTION_SCHEDULE_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from MTH_PRODUCTION_SCHEDULES_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  delete from MTH_PRODUCTION_SCHEDULE_EXT_TL T
  where not exists
    (select NULL
    from MTH_PRODUCTION_SCHEDULES_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update MTH_PRODUCTION_SCHEDULE_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
      ...
      B.TL_EXT_ATTR29,
      B.TL_EXT_ATTR30
    from MTH_PRODUCTION_SCHEDULE_EXT_TL B
    where B.EXTENSION_ID = T.EXTENSION_ID
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from MTH_PRODUCTION_SCHEDULE_EXT_TL SUBB, MTH_PRODUCTION_SCHEDULE_EXT_TL SUBT
  ...
  insert into MTH_PRODUCTION_SCHEDULE_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from MTH_PRODUCTION_SCHEDULE_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
