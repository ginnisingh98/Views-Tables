--------------------------------------------------------
--  DDL for Package Body PAY_KR_YEA20060101_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KR_YEA20060101_PKG" as
/* $Header: pykryea5.pkb 120.8 2007/01/02 11:12:52 pdesu noship $ */
-- Constant
	-- Bug 3966549
	...
	if (p_yea_info.nationality = 'F' and p_yea_info.fixed_tax_rate = 'Y') then
		...
		if p_yea_info.non_resident_flag = 'N' then
			...
			if p_yea_info.corp_pension_prem > 0 then
			        p_yea_info.corp_pension_prem_tax_exem := least(p_yea_info.corp_pension_prem+p_yea_info.pers_pension_saving,l_corp_pen_prem_tax_exem_lim)- p_yea_info.pers_pension_saving_tax_exem;
			        if p_yea_info.corp_pension_prem_tax_exem < 0 then
			           p_yea_info.corp_pension_prem_tax_exem := 0;
			        end if;
			        p_yea_info.taxation_base := p_yea_info.taxation_base - p_yea_info.corp_pension_prem_tax_exem;
			end if;
			----------------------------------------------------------------
			...
			----------------------------------------------------------------
			p_yea_info.total_credit_card_exp :=p_yea_info.emp_cre_card_direct_exp2004 + p_yea_info.dpnt_cre_card_direct_exp2004
			                                   + p_yea_info.giro_tuition_paid_exp2004 + p_yea_info.cash_receipt_exp2005;                -- Bug 5682131
