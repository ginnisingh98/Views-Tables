--------------------------------------------------------
--  DDL for Package Body BIX_PMV_EMC_ART_PRTLT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_PMV_EMC_ART_PRTLT_PKG" AS
/*$Header: bixeartb.plb 120.0 2005/05/25 17:19:18 appldev noship $ */
 ...
 */
select name VIEWBY,
ROUND((nvl(sum(CURR_ART),0)/SUM(CURR_REPLD))/3600,1) BIX_EMC_ARTIME,
ROUND((nvl(sum(PREV_ART),0)/SUM(PREV_REPLD))/3600,1) BIX_EMC_PRARTIME
from
(
...
) over (order by cal.start_date) PREV_ART
,
SUM(
	 CASE when (cal.start_date between &BIS_CURRENT_REPORT_START_DATE and &BIS_CURRENT_ASOF_DATE
	 	  and cal.report_date = least(cal.end_date,&BIS_CURRENT_ASOF_DATE))
...
/* Selects measures for all years in time range from previous report start date to current as of date*/
   select fii1.name,fii1.start_date,fii1.end_date,cal.report_Date,cal.period_type_id,cal.time_id
   from fii_time_ent_year fii1, fii_time_rpt_struct cal
   where
   fii1.start_date BETWEEN &BIS_PREVIOUS_REPORT_START_DATE AND &BIS_CURRENT_ASOF_DATE
   and cal.report_date between fii1.start_date and fii1.end_date
   and cal.report_date in (least(fii1.end_date, &BIS_CURRENT_ASOF_DATE) , &BIS_PREVIOUS_ASOF_DATE)
   AND  bitAND(cal.record_type_id,&BIS_NESTED_PATTERN ) = cal.record_type_id
   order by fii1.sequence
)cal,(
...
l_sqltext := '
      SELECT fii604.name                                  VIEWBY,
             ROUND((nvl(sum(CURR_ART),0)/SUM(CURR_REPLD))/3600,1) BIX_EMC_ARTIME,
		   NULL                                                 BIX_EMC_PRARTIME
      FROM
            ( SELECT fii604.name                               NAME,
                     sum(AGENT_RESP_TIME_IN_PERIOD)         CURR_ART,
                     sum(EMAILS_REPLIED_IN_PERIOD)          CURR_REPLD
              FROM  '||l_period_type||'	fii604,
                    bix_email_details_mv eml,
				fii_time_rpt_struct cal
              WHERE eml.time_id        = cal.time_id
