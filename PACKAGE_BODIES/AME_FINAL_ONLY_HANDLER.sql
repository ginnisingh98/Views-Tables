--------------------------------------------------------
--  DDL for Package Body AME_FINAL_ONLY_HANDLER
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_FINAL_ONLY_HANDLER" as
/* $Header: ameefoha.pkb 120.3 2007/12/20 20:02:23 prasashe noship $ */
  /* package variables */
  approverCategories ame_util.charList;
  finalApproverFound boolean;
      ...
      */
      ...
      exception
        when noSupervisorException then
          ...
          ame_util.runtimeException(packageNameIn => 'ame_final_only_handler',
    ...
    begin
      ...
      */
      ...
      /*
        ...
        starting point or a COA firstAuthority approver.  If not, check whether the requestor has enough
      ...
      */
      ...
      loop
        ...
        */
        ...
        category with that of prev approver to see if category change has occurred If yes
          ...
          */
          loop
            ...
            if COAInsertee.name is null then
              ...
              ame_absolute_job_level_handler.getJobLevelAndSupervisor(personIdIn => prevApprover.orig_system_id,
                                       jobLevelOut => tempJobLevel,
                                       supervisorIdOut => tempSupervisorId);
              getCatSourceAndAuthority(personIdIn => prevApprover.orig_system_id,
                                       jobLevelIn => tempJobLevel,
                                       supervisorIdIn => tempSupervisorId,
                                       categoryOut => prevApprover.approver_category,
                                       sourceOut => prevApprover.source,
                                       hasFinalAuthorityYNOut => tempHasFinalAuthorityYN,
                                       supervisorJobLevelOut => tempSupervisorJobLevel,
                                       nextSupervisorIdOut => tempNextSupervisorId);
            ...
            if ((prevApprover.approval_status is null) or
                (prevApprover.approval_status in
                ...
                (prevApprover.approver_category = ame_util.approvalApproverCategory and
                 prevApprover.approval_status = ame_util.notifiedStatus) )
                then
                if(not finalAuthorityFound and
                  tempHasFinalAuthorityYN = ame_util.booleanTrue) then
                  finalAuthorityFound := true;
