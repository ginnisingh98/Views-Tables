--------------------------------------------------------
--  DDL for Package Body CN_ADJ_DISP_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CN_ADJ_DISP_PUB_W" as
  /* $Header: cnwadjb.pls 115.8 2002/11/25 23:27:26 mblum ship $ */
  ...
  procedure rosetta_table_copy_in_p1(t out nocopy cn_adj_disp_pub.adj_tbl_type, a0 JTF_VARCHAR2_TABLE_100
    , a1 JTF_DATE_TABLE
    , a2 JTF_NUMBER_TABLE
    , a3 JTF_DATE_TABLE
    , a4 JTF_DATE_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).adjust_date := rosetta_g_miss_date_in_map(a9(indx));
          t(ddindx).calc_status_disp := a10(indx);
          t(ddindx).currency_code := a11(indx);
          t(ddindx).sales_credit := a12(indx);
          t(ddindx).commission := a13(indx);
          ...
          t(ddindx).customer_id := a114(indx);
          t(ddindx).customer_name := a115(indx);
          t(ddindx).customer_number := a116(indx);
          t(ddindx).bill_to_address_id := a117(indx);
          t(ddindx).ship_to_address_id := a118(indx);
          t(ddindx).bill_to_contact_id := a119(indx);
          t(ddindx).ship_to_contact_id := a120(indx);
          t(ddindx).rollup_date := rosetta_g_miss_date_in_map(a121(indx));
          t(ddindx).comments := a122(indx);
          t(ddindx).reason_code := a123(indx);
          t(ddindx).reason := a124(indx);
          t(ddindx).quota_id := a125(indx);
          t(ddindx).quota_name := a126(indx);
          t(ddindx).revenue_class_id := a127(indx);
          t(ddindx).revenue_class_name := a128(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  procedure rosetta_table_copy_out_p1(t cn_adj_disp_pub.adj_tbl_type, a0 out nocopy JTF_VARCHAR2_TABLE_100
    ...
    , a121 out nocopy JTF_DATE_TABLE
    , a122 out nocopy JTF_VARCHAR2_TABLE_1800
    ...
    a128 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a9(indx) := t(ddindx).adjust_date;
          a10(indx) := t(ddindx).calc_status_disp;
          a11(indx) := t(ddindx).currency_code;
          a12(indx) := t(ddindx).sales_credit;
          a13(indx) := t(ddindx).commission;
          ...
          a114(indx) := t(ddindx).customer_id;
          a115(indx) := t(ddindx).customer_name;
          a116(indx) := t(ddindx).customer_number;
          a117(indx) := t(ddindx).bill_to_address_id;
          a118(indx) := t(ddindx).ship_to_address_id;
          a119(indx) := t(ddindx).bill_to_contact_id;
          a120(indx) := t(ddindx).ship_to_contact_id;
          a121(indx) := t(ddindx).rollup_date;
          a122(indx) := t(ddindx).comments;
          a123(indx) := t(ddindx).reason_code;
          a124(indx) := t(ddindx).reason;
          a125(indx) := t(ddindx).quota_id;
          a126(indx) := t(ddindx).quota_name;
          a127(indx) := t(ddindx).revenue_class_id;
          a128(indx) := t(ddindx).revenue_class_name;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  procedure get_adj(p_api_version  NUMBER
    ...
    , p20_a114 out nocopy JTF_NUMBER_TABLE
    , p20_a115 out nocopy JTF_VARCHAR2_TABLE_100
    , p20_a116 out nocopy JTF_VARCHAR2_TABLE_100
    , p20_a117 out nocopy JTF_NUMBER_TABLE
    , p20_a118 out nocopy JTF_NUMBER_TABLE
    , p20_a119 out nocopy JTF_NUMBER_TABLE
    , p20_a120 out nocopy JTF_NUMBER_TABLE
    , p20_a121 out nocopy JTF_DATE_TABLE
    , p20_a122 out nocopy JTF_VARCHAR2_TABLE_1800
    , p20_a123 out nocopy JTF_VARCHAR2_TABLE_100
    , p20_a124 out nocopy JTF_VARCHAR2_TABLE_100
    , p20_a125 out nocopy JTF_NUMBER_TABLE
    , p20_a126 out nocopy JTF_VARCHAR2_TABLE_100
    , p20_a127 out nocopy JTF_NUMBER_TABLE
    , p20_a128 out nocopy JTF_VARCHAR2_TABLE_100
  ...
  as
    ...
    ddx_adj_tbl cn_adj_disp_pub.adj_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    cn_adj_disp_pub.get_adj(p_api_version,
      p_init_msg_list,
      p_validation_level,
      x_return_status,
      x_msg_count,
    ...
    cn_adj_disp_pub_w.rosetta_table_copy_out_p1(ddx_adj_tbl, p20_a0
      , p20_a1
      , p20_a2
      , p20_a3
      , p20_a4
