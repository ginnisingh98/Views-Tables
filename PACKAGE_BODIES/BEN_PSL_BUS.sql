--------------------------------------------------------
--  DDL for Package Body BEN_PSL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PSL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ben_psl_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   per_info_chg_cs_ler_id PK of record being inserted or updated.
...
--
Procedure chk_per_info_chg_cs_ler_id(p_per_info_chg_cs_ler_id       in number,
...
Begin
  ...
  --
  l_api_updating := ben_psl_shd.api_updating
  ...
  if (l_api_updating
     ...
     <>  ben_psl_shd.g_old_rec.per_info_chg_cs_ler_id) then
    --
    -- raise error as PK has changed
    --
    ben_psl_shd.constraint_error('BEN_PER_INFO_CHG_CS_LER_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_per_info_chg_cs_ler_id is not null then
      --
...
-- In Parameters
--   per_info_chg_cs_ler_id PK of record being inserted or updated.
--   per_info_chg_cs_ler_rl Value of formula rule id.
...
--   Internal table handler use only.
--
Procedure chk_per_info_chg_cs_ler_rl(p_per_info_chg_cs_ler_id      in number,
                             p_per_info_chg_cs_ler_rl              in number,
                             p_business_group_id           in number,
                             p_effective_date              in date,
                             p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_per_info_chg_cs_ler_rl';
  l_api_updating boolean;
  l_dummy        varchar2(1);
  --
  cursor c1 is
    ...
    where  ff.formula_id = p_per_info_chg_cs_ler_rl
...
Begin
  ...
  --
  if (l_api_updating
      and nvl(p_per_info_chg_cs_ler_rl,hr_api.g_number)
      <> ben_psl_shd.g_old_rec.per_info_chg_cs_ler_rl
      or not l_api_updating)
      and p_per_info_chg_cs_ler_rl is not null then
    --
  ...
  hr_utility.set_location('Leaving:'||l_proc,10);
  --
end chk_per_info_chg_cs_ler_rl;
--
-- ----------------------------------------------------------------------------
...
--   column are entered and that the combo of table, column, new value and
--   old value is unique.
--
-- Pre Conditions
--   None.
...
--   Internal table handler use only.
--
Procedure chk_table_column_val(p_per_info_chg_cs_ler_id  in number,
                           ...
                           p_object_version_number       in number) is
  --
CURSOR c1 (p_per_info_chg_cs_ler_id     number
    ...
    WHERE   per_info_chg_cs_ler_id   <> nvl(p_per_info_chg_cs_ler_id,
                                            hr_api.g_number)
...
--   Internal table handler use only.
--
Procedure chk_name_unique( p_per_info_chg_cs_ler_id  in number,
                           p_name                     in varchar2,
                           p_business_group_id           in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
...
-- Access Status
--   Internal table handler use only.
Procedure chk_whatif_lbl_unique( p_per_info_chg_cs_ler_id  in number,
...
CURSOR c1 (p_per_info_chg_cs_ler_id  number
    ...
    AND     per_info_chg_cs_ler_id   <> nvl(p_per_info_chg_cs_ler_id,
                                            hr_api.g_number)
    AND     business_group_id + 0  = p_business_group_id
    AND     p_effective_date between effective_start_date
    AND     effective_end_date;
...
Begin
	...
	if p_whatif_lbl_txt is not null then
		hr_utility.set_location(' What IF Label is not null'||l_proc, 5);
			open c1(p_per_info_chg_cs_ler_id  =>  p_per_info_chg_cs_ler_id
...
function return_legislation_code
  (p_per_info_chg_cs_ler_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_per_info_chg_cs_ler_f b
    where b.per_info_chg_cs_ler_id      = p_per_info_chg_cs_ler_id
