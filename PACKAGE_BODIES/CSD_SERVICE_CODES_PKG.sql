--------------------------------------------------------
--  DDL for Package Body CSD_SERVICE_CODES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSD_SERVICE_CODES_PKG" as
/* $Header: csdtcscb.pls 115.4 2003/11/05 00:00:52 gilam noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from CSD_SERVICE_CODES_B
    where SERVICE_CODE_ID = PX_SERVICE_CODE_ID;

begin


  select CSD_SERVICE_CODES_S1.nextval
  into PX_SERVICE_CODE_ID
  from dual;


  insert into CSD_SERVICE_CODES_B (
    SERVICE_CODE_ID,
  ...
  insert into CSD_SERVICE_CODES_TL (
    SERVICE_CODE_ID,
    CREATED_BY,
    CREATION_DATE,
    LAST_UPDATED_BY,
  ...
  and not exists
    (select NULL
    from CSD_SERVICE_CODES_TL T
    where T.SERVICE_CODE_ID = PX_SERVICE_CODE_ID
  ...
  */
  ...
  cursor c is select
    ...
    */
    ...
    for update of SERVICE_CODE_ID nowait;
  recinfo c%rowtype;

  --commented out the cursor for TL table
  /*
  cursor c1 is select
      NAME,
      DESCRIPTION,
      decode(LANGUAGE, userenv('LANG'), 'Y', 'N') BASELANG
    from CSD_SERVICE_CODES_TL
    where SERVICE_CODE_ID = P_SERVICE_CODE_ID
...
begin
  ...
  close c;
  if (
         (recinfo.SERVICE_CODE_ID = P_SERVICE_CODE_ID)
      ...
      AND (recinfo.SERVICE_CODE = P_SERVICE_CODE)
  ...
  --commented out the rest of the record from TL
  /*
...
begin
  update CSD_SERVICE_CODES_B set
         ...
         ,SERVICE_CODE = decode( P_SERVICE_CODE, FND_API.G_MISS_CHAR, NULL, NULL, SERVICE_CODE, P_SERVICE_CODE)
  ...
  update CSD_SERVICE_CODES_TL set
          CREATED_BY = decode( P_CREATED_BY, FND_API.G_MISS_NUM, NULL, NULL, CREATED_BY, P_CREATED_BY)
         ,CREATION_DATE = decode( P_CREATION_DATE, FND_API.G_MISS_DATE, NULL, NULL, CREATION_DATE, P_CREATION_DATE)
         ,LAST_UPDATED_BY = decode( P_LAST_UPDATED_BY, FND_API.G_MISS_NUM, NULL, NULL, LAST_UPDATED_BY, P_LAST_UPDATED_BY)
         ,LAST_UPDATE_DATE = decode( P_LAST_UPDATE_DATE, FND_API.G_MISS_DATE, NULL, NULL, LAST_UPDATE_DATE, P_LAST_UPDATE_DATE)
...
begin
  delete from CSD_SERVICE_CODES_TL
  where SERVICE_CODE_ID = P_SERVICE_CODE_ID;
  ...
  delete from CSD_SERVICE_CODES_B
  where SERVICE_CODE_ID = P_SERVICE_CODE_ID;
...
is
begin
  delete from CSD_SERVICE_CODES_TL T
  where not exists
    (select NULL
    from CSD_SERVICE_CODES_B B
    where B.SERVICE_CODE_ID = T.SERVICE_CODE_ID
    );

  update CSD_SERVICE_CODES_TL T set (
      NAME,
      DESCRIPTION
    ) = (select
      B.NAME,
      B.DESCRIPTION
    from CSD_SERVICE_CODES_TL B
    where B.SERVICE_CODE_ID = T.SERVICE_CODE_ID
  ...
  where (
      ...
      SUBT.SERVICE_CODE_ID,
      SUBT.LANGUAGE
    from CSD_SERVICE_CODES_TL SUBB, CSD_SERVICE_CODES_TL SUBT
    where SUBB.SERVICE_CODE_ID = SUBT.SERVICE_CODE_ID
  ...
  insert into CSD_SERVICE_CODES_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from CSD_SERVICE_CODES_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from CSD_SERVICE_CODES_TL T
    where T.SERVICE_CODE_ID = B.SERVICE_CODE_ID
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;
