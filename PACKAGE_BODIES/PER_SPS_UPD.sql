--------------------------------------------------------
--  DDL for Package Body PER_SPS_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_SPS_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_sps_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the per_spinal_point_steps_f Row
    --
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    per_sps_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.spinal_point_id = hr_api.g_number) then
    p_rec.spinal_point_id :=
    per_sps_shd.g_old_rec.spinal_point_id;
  End If;
...
Begin
  ...
  --
  per_sps_upd.convert_defs(p_rec);
  --
  per_sps_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
