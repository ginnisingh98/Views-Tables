--------------------------------------------------------
--  DDL for Package Body PER_ASG_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ASG_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)    := '  per_asg_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    -- Start of fix 518655
    if nvl(per_asg_shd.g_old_rec.assignment_number, hr_api.g_varchar2) <>
       nvl(p_rec.assignment_number, hr_api.g_varchar2) then
           --
...
Begin
  ...
  --
-- Bug 3629308, 4041992 Starts Here
...
Begin
 ...
 If (p_rec.applicant_rank = hr_api.g_number) then
     ...
     per_asg_shd.g_old_rec.applicant_rank;
  End If;
 If (p_rec.grade_ladder_pgm_id = hr_api.g_number) then
 ...
 If (p_rec.supervisor_assignment_id = hr_api.g_number) then
     ...
     per_asg_shd.g_old_rec.supervisor_assignment_id;
  End If;
 If (p_rec.vendor_site_id = hr_api.g_number) then
     p_rec.vendor_site_id :=
     per_asg_shd.g_old_rec.vendor_site_id;
  End If;
 If (p_rec.po_header_id = hr_api.g_number) then
     p_rec.po_header_id :=
     per_asg_shd.g_old_rec.po_header_id;
  End If;
 If (p_rec.po_line_id = hr_api.g_number) then
     p_rec.po_line_id :=
     per_asg_shd.g_old_rec.po_line_id;
  End If;
 If (p_rec.projected_assignment_end = hr_api.g_date) then
