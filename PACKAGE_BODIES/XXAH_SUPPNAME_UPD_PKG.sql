--------------------------------------------------------
--  DDL for Package Body XXAH_SUPPNAME_UPD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XXAH_SUPPNAME_UPD_PKG" 
AS
   /***************************************************************************
   *                           IDENTIFICATION
   *                           ==============
   * NAME              : APPS.XXAH_SUPPNAME_UPD_PKG
   * DESCRIPTION       : PACKAGE TO  Update NFR Supplier name
   ****************************************************************************
   *                           CHANGE HISTORY
   *                           ==============
   * DATE             VERSION     DONE BY
   * 25-APR-2019        1.0       Menaka      Supplier name update for NFR Suppliers (RFC 65)
   ****************************************************************************/
   ...
   IS
      ...
      IS
         SELECT ROWID,
                ...
                supplier_name,
                new_supplier_name
           FROM APPS.XXAH_SUPPNAME_UPDATE
          WHERE NEW_SUPPLIER_NAME IS NOT NULL AND conversion_status = 'N'  ;
...
fnd_file.put_line(fnd_file.log,'--NEW_SUPPLIER_NAME => '||l_party_id||l_new_name||l_object_version_number);
        IF (v_return_status <> fnd_api.g_ret_sts_success)
         ...
         ELSE
         begin
            update APPS.XXAH_SUPPNAME_UPDATE set CONVERSION_STATUS='P' where supplier_number= p_supp_num;
            commit;
            exception when others then
   ...
   IS
      ...
      IS
           SELECT *
             FROM APPS.XXAH_SUPPNAME_UPDATE ORDER BY  CONVERSION_STATUS DESC;
   ...
   BEGIN
      ...
      LOOP
         ...
         IF r_rec.CONVERSION_STATUS = null
         THEN
            IF l_noupd = 'N'
            THEN
               FND_FILE.PUT_LINE (
...
begin
      UPDATE XXAH_SUPPNAME_UPDATE
         SET CONVERSION_STATUS = 'E', ERROR_LOG = p_message
