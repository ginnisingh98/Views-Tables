--------------------------------------------------------
--  DDL for Package Body GR_LABEL_CLASSES_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_LABEL_CLASSES_TL_PKG" AS
/*$Header: GRHILCTB.pls 120.1 2006/06/16 21:39:53 pbamb noship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_label_classes_tl
   		  	     (label_class_code,
				  language,
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ROLLBACK TO SAVEPOINT Insert_Row;
	  l_msg_token := p_label_class_code;
	  x_return_status := 'E';
...
/*	  Now call the check foreign key procedure */
   ...
   ELSE
	  ...
	  SET	 label_class_code                = p_label_class_code,
			 language	                     = p_language,
			 label_class_description	     = p_label_class_description,
...
L_MSG_DATA		  		VARCHAR2(2000);
L_MSG_TOKEN		  		VARCHAR2(100);
L_BASE_DESC             GR_LABEL_CLASSES_TL.label_class_description%TYPE;
...
CURSOR c_get_label_class
 IS
   SELECT	lct.label_class_description,
            lct.created_by,
			lct.creation_date,
			lct.last_updated_by,
			lct.last_update_date,
			lct.last_update_login
   FROM	    gr_label_classes_tl lct
   WHERE	lct.label_class_code = p_label_class_code
   AND		lct.language = l_language;
LabelClassRecord				c_get_label_class%ROWTYPE;
...
/*	Initialization Routine  */
   ...
   x_oracle_error := 0;
   x_msg_data := NULL;
   l_msg_token := p_label_class_code || ' ' || p_language;

/* Remove translations with no base row */

  delete from GR_LABEL_CLASSES_TL T
  where not exists
    (select NULL
    from GR_LABEL_CLASSES_B B
    where B.LABEL_CLASS_CODE = T.LABEL_CLASS_CODE
    );

/* Redefault translations from the source language  */

   update gr_label_classes_tl t set (
    label_class_description ) =
    ( select
      B.LABEL_CLASS_DESCRIPTION
      from GR_LABEL_CLASSES_TL B
      where B.LABEL_CLASS_CODE = T.LABEL_CLASS_CODE
   ...
   where (
      ...
      T.LANGUAGE
         ...
         SUBT.LABEL_CLASS_CODE,
         SUBT.LANGUAGE
         from GR_LABEL_CLASSES_TL SUBB, GR_LABEL_CLASSES_TL SUBT
         where SUBB.LABEL_CLASS_CODE = SUBT.LABEL_CLASS_CODE
         and SUBB.LANGUAGE = SUBT.SOURCE_LANG
         and (SUBB.LABEL_CLASS_DESCRIPTION <> SUBT.LABEL_CLASS_DESCRIPTION
          or (SUBB.LABEL_CLASS_DESCRIPTION is null and SUBT.LABEL_CLASS_DESCRIPTION is not null)
          or (SUBB.LABEL_CLASS_DESCRIPTION is not null and SUBT.LABEL_CLASS_DESCRIPTION is null)
  ));

/*	Open the language cursor and get the description entered from the
**	user environment variable. And insert missing translation defaults
*/
   l_language := p_language;
   OPEN c_get_label_class;
   FETCH c_get_label_class INTO LabelClassRecord;
   IF c_get_label_class%NOTFOUND THEN
      CLOSE c_get_label_class;
      RAISE Language_Missing_Error;
   ELSE
	  l_base_desc := LabelClassRecord.label_class_description;
	  l_created_by := LabelClassRecord.created_by;
	  l_creation_date := LabelClassRecord.creation_date;
	  l_last_updated_by := LabelClassRecord.last_updated_by;
	  l_last_update_date := LabelClassRecord.last_update_date;
	  l_last_update_login := LabelClassRecord.last_update_login;
      CLOSE c_get_label_class;
   END IF;
...
CURSOR c_lock_label_classes_tl
 IS
   SELECT	*
   FROM		gr_label_classes_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockLabelClassRcd	  c_lock_label_classes_tl%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_label_classes_tl;
   FETCH c_lock_label_classes_tl INTO LockLabelClassRcd;
   IF c_lock_label_classes_tl%NOTFOUND THEN
	  CLOSE c_lock_label_classes_tl;
...
/*   Initialization Routine */
   ...
   DELETE FROM gr_label_classes_tl
   WHERE 	   label_class_code = p_label_class_code;
...
/*		  p_label_class_code is the code to check.
...
CURSOR c_get_label_class_tl_rowid
 IS
   SELECT lct.rowid
   FROM	  gr_label_classes_tl lct
   WHERE  lct.label_class_code = p_label_class_code
   AND	  lct.language = p_language;
ClassTLRecord			   c_get_label_class_tl_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_label_class_code || ' ' || p_language;

   x_key_exists := 'F';
   OPEN c_get_label_class_tl_rowid;
   FETCH c_get_label_class_tl_rowid INTO ClassTLRecord;
   IF c_get_label_class_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := ClassTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_label_class_tl_rowid;
...
BEGIN
	UPDATE GR_LABEL_CLASSES_TL SET
		LABEL_CLASS_DESCRIPTION = X_LABEL_CLASS_DESCRIPTION,
		SOURCE_LANG = USERENV('LANG'),
		LAST_UPDATE_DATE = sysdate,
		LAST_UPDATED_BY = 0,
		LAST_UPDATE_LOGIN = 0
	WHERE (LABEL_CLASS_CODE = X_LABEL_CLASS_CODE)
...
BEGIN
	...
	IF Cur_rowid%FOUND THEN
		GR_LABEL_CLASSES_TL_PKG.UPDATE_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_ROWID => l_row_id
			,P_LABEL_CLASS_CODE => X_LABEL_CLASS_CODE
			,P_LANGUAGE => X_LANGUAGE
			,P_LABEL_CLASS_DESCRIPTION => X_LABEL_CLASS_DESCRIPTION
...
PROCEDURE load_row (X_LABEL_CLASS_CODE IN VARCHAR2
		    ,X_LABEL_CLASS_DESCRIPTION IN VARCHAR2
	...
	ELSE
	   insert into GR_LABEL_CLASSES_TL (
	        ...
	        0,
              ...
              and not exists
                (select NULL
                from GR_LABEL_CLASSES_TL T
                where T.LABEL_CLASS_CODE = X_LABEL_CLASS_CODE
...
begin
  ...
  insert into GR_LABEL_CLASSES_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GR_LABEL_CLASSES_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from GR_LABEL_CLASSES_TL T
    where T.LABEL_CLASS_CODE = B.LABEL_CLASS_CODE
    and T.LANGUAGE = L.LANGUAGE_CODE);
