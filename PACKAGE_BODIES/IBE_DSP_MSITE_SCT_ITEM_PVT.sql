--------------------------------------------------------
--  DDL for Package Body IBE_DSP_MSITE_SCT_ITEM_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IBE_DSP_MSITE_SCT_ITEM_PVT" AS
/* $Header: IBEVCMIB.pls 120.0 2005/05/30 02:35:27 appldev noship $ */
...
G_PKG_NAME  CONSTANT VARCHAR2(30):= 'IBE_DSP_MSITE_SCT_ITEM_PVT';
G_FILE_NAME CONSTANT VARCHAR2(12):= 'IBEVCMIB.pls';
...
IS
  ...
  CURSOR c IS SELECT rowid FROM ibe_dsp_msite_sct_items
    WHERE mini_site_section_item_id = x_mini_site_section_item_id;
  CURSOR c2 IS SELECT ibe_dsp_msite_sct_items_s1.nextval FROM dual;
...
BEGIN
  ...
  -- insert base
  INSERT INTO ibe_dsp_msite_sct_items
    (
...
BEGIN
  ...
  -- update base
  UPDATE ibe_dsp_msite_sct_items SET
...
BEGIN
  ...
  THEN
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_INVALID_SI_ID');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_SCT_NO_MSITE_ID');
     ...
     WHEN OTHERS THEN
       ...
       FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_SCT_GET_MSITE_ID');
       FND_MESSAGE.Set_Token('MINI_SITE_ID', p_mini_site_id);
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
  END;
  ...
  BEGIN
    SELECT section_item_id INTO l_section_item_id FROM ibe_dsp_section_items
      WHERE section_item_id = p_section_item_id;
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_SCT_NO_SI_ID');
...
BEGIN
  ...
  IF (p_mini_site_section_item_id IS NULL) THEN
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_NULL_PRIMARY_KEY');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
...
BEGIN
  ...
  ELSIF (l_return_status = FND_API.G_RET_STS_ERROR) THEN
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_MSI_INVALID_CREATE');
    FND_MSG_PUB.Add;
  ...
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_MSI_INSERT_FAIL');
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_ERROR;
     WHEN OTHERS THEN
       FND_MESSAGE.Set_Name('FND', 'SQL_PLSQL_ERROR');
...
BEGIN
  ...
  THEN
    ...
    BEGIN
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
           FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_NO_MSI_ID');
           FND_MESSAGE.Set_Token('MINI_SITE_ID', p_mini_site_id);
           FND_MESSAGE.Set_Token('SECTION_ITEM_ID', p_section_item_id);
           FND_MSG_PUB.Add;
           RAISE FND_API.G_EXC_ERROR;
  ...
  ELSE
    ...
    -- mini_site_id and section_item_id is specified
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_NO_MSI_IDS_SPEC');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  IF (l_return_status = FND_API.G_RET_STS_UNEXP_ERROR) THEN
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
  ELSIF (l_return_status = FND_API.G_RET_STS_ERROR) THEN
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_MSI_INVALID_UPDATE');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;            -- invalid
  END IF;
  ...
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_MSI_UPDATE_FAIL');
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_ERROR;
     WHEN OTHERS THEN
       FND_MESSAGE.Set_Name('FND', 'SQL_PLSQL_ERROR');
