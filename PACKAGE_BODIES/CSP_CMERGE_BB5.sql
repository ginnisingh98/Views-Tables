--------------------------------------------------------
--  DDL for Package Body CSP_CMERGE_BB5
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSP_CMERGE_BB5" AS
/* $Header: cscm105b.pls 115.1 99/07/16 08:47:53 porting s $ */
...
/* This procedure handles the merge process for the CS_MASS_NOTIFICATION_TXNS_TEMP.
...
/* Put the header in the report to identify the block to be run */
        ...
        arp_message.set_line('CP_CMERGE_BB5.MERGE()+');

        IF ( process_mode = 'LOCK' ) Then
	        arp_message.set_name('AR', 'AR_LOCKING_TABLE');
     	  	arp_message.set_token('TABLE_NAME', 'CS_MASS_NOTIFICATION_TXNS_TEMP',FALSE );
        	message_text := 'The locking is done in block CSP_CMERGE_BB5';
        	arp_message.set_line(message_text);
        ELSE
        	arp_message.set_name('AR', 'AR_UPDATING_TABLE');
        	arp_message.set_token('TABLE_NAME', 'CS_MASS_NOTIFICATION_TXNS_TEMP',FALSE );
        	message_text := 'The merge is done in block CSP_CMERGE_BB5';
        	arp_message.set_line(message_text);
        END IF;

/* merge the CS_MASS_NOTIFICATION_TXNS_TEMP table update the customer_id */
...
/* Report that the process for CS_MASS_NOTIFICATION_TXNS_TEMP is complete */

        IF ( process_mode = 'LOCK' ) Then
        	message_text := '** LOCKING completed for table CS_MASS_NOTIFICATION_TXNS_TEMP **';
        	arp_message.set_line(message_text);
        ELSE
        	message_text := '** MERGE completed for table CS_MASS_NOTIFICATION_TXNS_TEMP **';
        	arp_message.set_line(message_text);
        END IF;
...
/* This process updates the customer_id of the CS_MASS_NOTIFICATION_TXNS_TEMP table */
...
/* number of rows updated */
        ...
        FROM   CS_MASS_NOTIFICATION_TXNS_TEMP yt, RA_CUSTOMER_MERGES RACM
        WHERE
  ...
  BEGIN
        ...
        ELSE
             ...
             UPDATE CS_MASS_NOTIFICATION_TXNS_TEMP yt
             SET
