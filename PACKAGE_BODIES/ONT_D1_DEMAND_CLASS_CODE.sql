--------------------------------------------------------
--  DDL for Package Body ONT_D1_DEMAND_CLASS_CODE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ONT_D1_DEMAND_CLASS_CODE" AS
...
--  DESCRIPTION
--  
--      Body of package ONT_D1_DEMAND_CLASS_CODE
--  
--  NOTES
--  
--  HISTORY
...
G_PKG_NAME                    CONSTANT VARCHAR2(30) := 'ONT_D1_DEMAND_CLASS_CODE';
...
BEGIN
  ...
  END IF;
    ...
    l_return_value := ONT_ORDER_TYPE_Def_Util.g_cached_record.DEMAND_CLASS_CODE;
    END IF;
    IF l_return_value IS NOT NULL THEN
       GOTO RETURN_VALUE;
  END IF;
