--------------------------------------------------------
--  DDL for Package Body HR_TPT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_TPT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_tpt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the hr_tab_page_properties_tl Row
  --
  update hr_tab_page_properties_tl
    set
...
Begin
  ...
  --
  convert_defs(p_rec);
  hr_tpt_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hr_tpt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_tpt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_tpt_upd.post_update
     (p_rec
     );
End upd;
--
  ...
  cursor csr_upd_langs is
    select tpt.language
     ...
     where tpt.tab_page_property_id = p_tab_page_property_id
       and p_language_code in (tpt.language
                              ,tpt.source_lang);
  --
