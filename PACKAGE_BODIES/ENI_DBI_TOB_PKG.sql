--------------------------------------------------------
--  DDL for Package Body ENI_DBI_TOB_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ENI_DBI_TOB_PKG" AS
/* $Header: ENITOBPB.pls 120.0 2005/05/26 19:36:18 appldev noship $ */
...
-- the following three parameters, looping through the parameters table here, instead.
  ...
  ELSE
    ...
    THEN
      l_category_where := ' AND edh.parent_id = :CATEGORY
                            AND edh.child_id = fact.item_category_id ';
  ...
  THEN
    l_curr_select := ' fact.booked_amt_g		      ENI_ATTRIBUTE9, ' ;
  ELSIF INSTR(l_curr,ENI_DBI_UTIL_PKG.get_curr_sec) > 0
  THEN
    l_curr_select := ' fact.booked_amt_g1	        ENI_ATTRIBUTE9, ' ;
  ELSE
    l_curr_select := ' fact.booked_amt_g	        ENI_ATTRIBUTE9, ' ;
  END IF;
  ...
  l_stmt := '
 SELECT
   CUST.VALUE ENI_ATTRIBUTE2,
   ENI_ATTRIBUTE3 ,
   ENI_ATTRIBUTE4 ,
   ENI_ATTRIBUTE9 ,
   ENI_ATTRIBUTE7 ,
   ENI_ATTRIBUTE8 ,
   item.value ENI_ATTRIBUTE6 ,
   enit.value ENI_ATTRIBUTE5 ,
   nvl(r.resource_name, decode(	fact.resource_id, -1,
   ''Unassigned'',
   NULL))			ENI_ATTRIBUTE11, -- Sales Person
   g.group_name						ENI_ATTRIBUTE12  -- Sales Group
FROM
(
  SELECT
    fact.order_number				ENI_ATTRIBUTE3, -- Order number
    fact.line_number					ENI_ATTRIBUTE4, -- Order line number
    '|| l_curr_select || '
    fact.booked_date					ENI_ATTRIBUTE7, -- Booked date
    fact.schedule_date				ENI_ATTRIBUTE8, -- Scheduled ship date
    fact.customer_id ,
    fact.sales_grp_id ,
    fact.resource_id ,
    fact.item_id ,
  ...
  WHERE
    ...
    || l_category_where
    || l_extra_where || '
  ORDER BY ENI_ATTRIBUTE9 desc ) fact,
  ENI_ITEM_VBH_NODES_V enit,
  ENI_ITEM_V item,
...
WHERE
  ...
  AND fact.item_category_id = enit.id
  AND	enit.parent_id = enit.child_id
  AND fact.customer_id = cust.id
  AND fact.sales_grp_id = g.group_id
  AND	fact.resource_id = r.resource_id(+)
  '|| l_extra_outer_where ||'
