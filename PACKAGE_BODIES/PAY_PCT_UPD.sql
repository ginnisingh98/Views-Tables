--------------------------------------------------------
--  DDL for Package Body PAY_PCT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PCT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_pct_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy pay_pct_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the pay_user_columns_tl Row
  --
  update pay_user_columns_tl
    ...
    ,user_column_name                = p_rec.user_column_name
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    ...
    pay_pct_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.user_column_name = hr_api.g_varchar2) then
    p_rec.user_column_name :=
    pay_pct_shd.g_old_rec.user_column_name;
  End If;
  --
...
Begin
  ...
  --
  pay_pct_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pay_pct_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  --pay_pct_upd.post_update(p_rec);
  --
  -- Call to raise any errors on multi-message list
  hr_multi_message.end_validation_set;
End upd;
  ...
  cursor csr_upd_langs is
    select pct.language
     ...
     where pct.user_column_id = p_user_column_id
       and p_language_code in (pct.language
                              ,pct.source_lang);
  --
