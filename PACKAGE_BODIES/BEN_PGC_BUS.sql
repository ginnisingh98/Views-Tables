--------------------------------------------------------
--  DDL for Package Body BEN_PGC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PGC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pgc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_pgm_dpnt_cvg_ctfn_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   pgm_dpnt_cvg_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_pgm_dpnt_cvg_ctfn_id(p_pgm_dpnt_cvg_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pgm_dpnt_cvg_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_pgc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pgm_dpnt_cvg_ctfn_id                => p_pgm_dpnt_cvg_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pgm_dpnt_cvg_ctfn_id,hr_api.g_number)
     <>  ben_pgc_shd.g_old_rec.pgm_dpnt_cvg_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_pgc_shd.constraint_error('BEN_PGM_CTFN_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pgm_dpnt_cvg_ctfn_id is not null then
      --
...
--   the value is unique within parent (pgm) and business group.
--
-- Pre Conditions
--   None.
--
...
--   Internal table handler use only.
--
Procedure chk_dpnt_cvg_ctfn_typ_cd(p_pgm_dpnt_cvg_ctfn_id in number,
  ...
  cursor chk_unique is
     select null
        ...
        where nvl(dpnt_cvg_ctfn_typ_cd,0)  = nvl(p_dpnt_cvg_ctfn_typ_cd,0)
          ...
          and pgm_dpnt_cvg_ctfn_id<> nvl(p_pgm_dpnt_cvg_ctfn_id, hr_api.g_number)
          and pgm_id = p_pgm_id
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_pgc_shd.g_old_rec.dpnt_cvg_ctfn_typ_cd,hr_api.g_varchar2)
...
--
-- Description
--   This procedure is used to check that the lookup value is valid. Also
--   only one value of 'Y' is allowed per parent.
--
-- Pre Conditions
--   None.
--
...
--   Internal table handler use only.
--
Procedure chk_pfd_flag(p_pgm_dpnt_cvg_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_pfd_flag
      <> nvl(ben_pgc_shd.g_old_rec.pfd_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_rqd_flag(p_pgm_dpnt_cvg_ctfn_id          in number,
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_pgm_dpnt_cvg_ctfn_id                in number,
...
--   Internal table handler use only.
--
Procedure chk_lack_ctfn_sspnd_enrt_flag(p_pgm_dpnt_cvg_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_pgc_shd.g_old_rec.lack_ctfn_sspnd_enrt_flag,hr_api.g_varchar2)
      or not l_api_updating)
...
function return_legislation_code
  (p_pgm_dpnt_cvg_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pgm_dpnt_cvg_ctfn_f b
    where b.pgm_dpnt_cvg_ctfn_id      = p_pgm_dpnt_cvg_ctfn_id
