--------------------------------------------------------
--  DDL for Package Body CSTPPAHK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSTPPAHK" AS
/* $Header: CSTPAHKB.pls 115.8 2002/11/09 00:26:49 awwang ship $ */
...
EXCEPTION
  ...
  when others then
    o_err_num := SQLCODE;
    o_err_msg := 'CSTPPAQH.ACQ_COST_HOOK:' || substrb(SQLERRM,1,150);
    return 0;
...
BEGIN
  ...
  o_error_num := -1;
  o_hook_cost := 0;
  o_error_msg := 'CSTPPAQH.ACQ_RECEIPT_COST_HOOK-No_Receipt_cost error';
  return;
...
EXCEPTION
  ...
  when others then
    o_error_num := -1;
    o_error_msg := 'CSTPPAQH.ACQ_RECEIPT_COST_HOOK:' ||to_char(SQLCODE) ||
                   substrb(SQLERRM,1,150);
    return;

END acq_receipt_cost_hook;
