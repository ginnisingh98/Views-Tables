--------------------------------------------------------
--  DDL for Package Body PAY_APR_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_APR_INS" as
...
--
g_process_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  pay_apr_ins.g_process_id_i := p_process_id;
  --
...
Begin
  ...
  --
  -- Insert the row into: pay_au_processes
  --
  insert into pay_au_processes
      (process_id
      ,short_name
      ,name
      ,enabled_flag
...
--
  Cursor C_Sel1 is select pay_au_processes_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from pay_au_processes
     where process_id =
             pay_apr_ins.g_process_id_i;
--
  l_proc   varchar2(72);
  l_exists varchar2(1);
--
Begin
  ...
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (pay_apr_ins.g_process_id_i is not null) Then
    --
    ...
    p_rec.process_id :=
      pay_apr_ins.g_process_id_i;
    pay_apr_ins.g_process_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.process_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  pay_apr_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  pay_apr_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  pay_apr_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
--
begin
  g_package  := '  pay_apr_ins.';  -- Global package name
end pay_apr_ins;
