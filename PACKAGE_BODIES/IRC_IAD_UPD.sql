--------------------------------------------------------
--  DDL for Package Body IRC_IAD_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_IAD_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  irc_iad_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the irc_assignment_details_f Row
    --
    update  irc_assignment_details_f
    set
     assignment_details_id                = p_rec.assignment_details_id
    ,assignment_id                        = p_rec.assignment_id
    ,details_version                      = p_rec.details_version
    ,latest_details                       = p_rec.latest_details
    ,attempt_id                           = p_rec.attempt_id
    ,qualified                            = p_rec.qualified
    ,considered                           = p_rec.considered
    ,object_version_number                = p_rec.object_version_number
    where   assignment_details_id = p_rec.assignment_details_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    hr_utility.set_location(l_proc, 10);
    irc_iad_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
  ...
  Else
    -- Correction mode
    --
    -- Reset the Latest Details flag for the current effective date range rows.
  ...
  End If;
  --
  -- We need to insert a new row, so get a new surrogate key if necessary.
  --
...
Begin
  ...
  begin
    ...
    irc_iad_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_assignment_id_o
      => irc_iad_shd.g_old_rec.assignment_id
      ,p_effective_start_date_o
      => irc_iad_shd.g_old_rec.effective_start_date
      ,p_effective_end_date_o
      => irc_iad_shd.g_old_rec.effective_end_date
      ,p_details_version_o
      => irc_iad_shd.g_old_rec.details_version
      ,p_latest_details_o
      => irc_iad_shd.g_old_rec.latest_details
      ,p_attempt_id_o
      => irc_iad_shd.g_old_rec.attempt_id
      ,p_qualified_o
      => irc_iad_shd.g_old_rec.qualified
      ,p_considered_o
      => irc_iad_shd.g_old_rec.considered
      ,p_object_version_number_o
      => irc_iad_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.effective_end_date = hr_api.g_date) then
    ...
    irc_iad_shd.g_old_rec.effective_end_date;
  End If;
  If (p_rec.details_version = hr_api.g_number) then
    p_rec.details_version :=
    irc_iad_shd.g_old_rec.details_version;
  End If;
  If (p_rec.latest_details = hr_api.g_varchar2) then
    p_rec.latest_details :=
    irc_iad_shd.g_old_rec.latest_details;
  End If;
  If (p_rec.attempt_id = hr_api.g_number) then
    ...
    irc_iad_shd.g_old_rec.attempt_id;
  End If;
  If (p_rec.qualified = hr_api.g_varchar2) then
    p_rec.qualified :=
    irc_iad_shd.g_old_rec.qualified;
  End If;
  If (p_rec.considered = hr_api.g_varchar2) then
    p_rec.considered :=
    irc_iad_shd.g_old_rec.considered;
  End If;
  --
...
Begin
  ...
  --
  irc_iad_upd.convert_defs(p_rec);
  --
  irc_iad_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
  ...
  --
  -- No need to call update_dml as we never overwrite the existing
  -- row data but instead create a new version even for correction mode
  --
