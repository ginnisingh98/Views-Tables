--------------------------------------------------------
--  DDL for Package Body GHR_CCA_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GHR_CCA_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ghr_cca_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_compl_appeal_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_compl_appeal_id  in  number) is
--
...
Begin
  ...
  --
  ghr_cca_ins.g_compl_appeal_id_i := p_compl_appeal_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: ghr_compl_appeals
  --
  insert into ghr_compl_appeals
      (compl_appeal_id
      ,complaint_id
      ,appeal_date
      ,appealed_to
...
--
  Cursor C_Sel1 is select ghr_compl_appeals_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from ghr_compl_appeals
     where compl_appeal_id =
             ghr_cca_ins.g_compl_appeal_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (ghr_cca_ins.g_compl_appeal_id_i is not null) Then
    --
    ...
    p_rec.compl_appeal_id :=
      ghr_cca_ins.g_compl_appeal_id_i;
    ghr_cca_ins.g_compl_appeal_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.compl_appeal_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  ghr_cca_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  ghr_cca_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  ghr_cca_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  p_compl_appeal_id := l_rec.compl_appeal_id;
