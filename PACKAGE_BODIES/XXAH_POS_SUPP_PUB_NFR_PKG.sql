--------------------------------------------------------
--  DDL for Package Body XXAH_POS_SUPP_PUB_NFR_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XXAH_POS_SUPP_PUB_NFR_PKG" AS
--'XXAH_POS_NFRSUPP_PUB_JOB_PKG.PUBLISH_SUPP_EVENT_JOB' 
...
SELECT ap.party_id
                ...
                and ap.party_id in (select  aps.party_id
from ap_suppliers aps,ap_supplier_sites_all apss
where aps.vendor_id=apss.vendor_id
and apss.vendor_site_id in (select site_id from XXAH_SUPP_NFR_UPDATE
 where site_id  in (378731,
263978,
264265,
553731,
264369,
...
--(select site_id from XXAH_SUPP_NFR_UPDATE ));
    ...
    BEGIN
        ...
        FROM   (SELECT ap.party_id
                FROM   ap_suppliers ap,
                POS_XXAH_SUPPLIER_TY_AGV pxs
                WHERE  ap.party_id =  pxs.party_id and   pxs.XXAH_SUPPLIER_TYPE_ATT in ('NFR','Trade')
                and ap.party_id in (select  aps.party_id
from ap_suppliers aps,ap_supplier_sites_all apss
where aps.vendor_id=apss.vendor_id
and apss.vendor_site_id in (select site_id from XXAH_SUPP_NFR_UPDATE where site_id  in (378731,
263978,
264265,
553731,
264369,
