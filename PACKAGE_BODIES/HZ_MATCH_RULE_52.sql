--------------------------------------------------------
--  DDL for Package Body HZ_MATCH_RULE_52
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_MATCH_RULE_52" AS
/*=======================================================================+
    ...
    FOR J IN 1..g_contact_stage_list.COUNT LOOP
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_stage_list(J).TX2, p_table_TX2,(50000*(J-1)+89)) THEN
        ...
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          ...
          RETURN l_current_score;
        END IF;
      ELSIF -- l_current_score<18 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_contact_stage_list(J).TX23, p_table_TX23,(50000*(J-1)+136)) THEN
        l_current_score:=l_current_score+ 18;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
        END IF;
  ...
  BEGIN
    ...
    FOR J IN 1..g_contact_pt_stage_list.COUNT LOOP
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX7, p_table_TX7,(50000*(J-1)+116)) THEN
        ...
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX10, p_table_TX10,(50000*(J-1)+120)) THEN
      ...
      ELSIF -- l_current_score<70 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX158, p_table_TX158,(50000*(J-1)+128)) THEN
        l_current_score:=l_current_score+ 70;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
...
END;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ELSIF l_current_score<18 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX23, p_table_TX23,(50000*(p_idx-1)+136)) THEN
      l_score :=18;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX10, p_table_TX10,(50000*(p_idx-1)+120)) THEN
      l_score :=70;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ELSIF l_current_score<70 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX158, p_table_TX158,(50000*(p_idx-1)+128)) THEN
      l_score :=70;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
...
END;
  ...
  BEGIN
    IF p_contains_str IS NULL THEN
      OPEN x_cursor FOR 
        SELECT PARTY_ID , TX59, TX2, lpad(rtrim(TX41),9,chr(48)), TX45
    ...
    ELSIF p_restrict_sql IS NULL OR p_search_ctx_id IS NOT NULL THEN
      IF p_search_ctx_id IS NULL THEN
        OPEN x_cursor FOR 
          SELECT /*+ ORDERED */ stage.PARTY_ID , stage.TX59, stage.TX2, lpad(rtrim(stage.TX41),9,chr(48)), stage.TX45
...
-- do an or between all the transformations of an attribute -- 
(
(srch.TX41 is not null and stage.TX41 like srch.TX41 || '%')
)
)
          AND ((g_party_stage_rec.TX36 IS NULL OR g_party_stage_rec.TX36||' ' =  stage.TX36) AND (g_party_stage_rec.TX46 IS NULL OR g_party_stage_rec.TX46||' ' =  stage.TX46))
          AND (nvl(p_search_merged,'N')='Y' OR nvl(stage.status,'A') in ('A'))
          AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id);
      ELSE
        OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_PARTIES_U1) */ stage.PARTY_ID , stage.TX59, stage.TX2, lpad(rtrim(stage.TX41),9,chr(48)), stage.TX45
...
(srch.TX41 is not null and stage.TX41 like srch.TX41 || '%')
    ...
    ELSE
     IF p_search_ctx_id IS NULL THEN
       l_sqlstr := 'SELECT /*+ ORDERED */ stage.PARTY_ID , stage.TX59, stage.TX2, lpad(rtrim(stage.TX41),9,chr(48)), stage.TX45'||
                   ' FROM HZ_SRCH_PARTIES srch, HZ_STAGED_PARTIES stage'||
                   ' WHERE'||
            'srch.batch_id = -1'||
            'AND'||
...
')'||
')'||
                   ' AND ((:TX36 IS NULL OR :TX36||'' '' =  stage.TX36) AND (:TX46 IS NULL OR :TX46||'' '' =  stage.TX46))'||
                   ' AND ('||p_restrict_sql||')' ||
       ...
       OPEN x_cursor FOR l_sqlstr USING p_contains_str
                              ...
                              ,g_party_stage_rec.TX46,g_party_stage_rec.TX46
...
(srch.TX26 is not null and stage.TX26 like srch.TX26 || '%')
        ...
        AND EXISTS (
          ...
          AND ((g_party_stage_rec.TX36 IS NULL OR g_party_stage_rec.TX36||' ' =  p.TX36) AND (g_party_stage_rec.TX46 IS NULL OR g_party_stage_rec.TX46||' ' =  p.TX46)))
...
')'||
                 ' AND p.party_id = stage.party_id ' || 
                 ' AND ((:TX36 IS NULL OR :TX36||'' '' =  p.TX36) AND (:TX46 IS NULL OR :TX46||'' '' =  p.TX46))) ' || 
                   ' AND ('||p_restrict_sql||')' ||
                   ' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) '; 
       OPEN x_cursor FOR l_sqlstr USING p_contains_str
                              ,g_party_stage_rec.TX36,g_party_stage_rec.TX36
  ...
  BEGIN
     IF p_party_id IS NOT NULL THEN
       OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACTS_N1) */ stage.ORG_CONTACT_ID, stage.PARTY_ID, stage.TX23, stage.TX2, stage.TX22
          FROM HZ_STAGED_CONTACTS stage, HZ_SRCH_CONTACTS srch
          WHERE
            srch.batch_id = -1
            AND
            (
...
-- do an or between all the transformations of an attribute -- 
(
(srch.TX23 is not null and stage.TX23 like srch.TX23 || '%')
)
)
        AND EXISTS (
          SELECT /*+ INDEX(p HZ_STAGED_PARTIES_U1) */  1 FROM HZ_STAGED_PARTIES p
    ...
    ELSIF p_restrict_sql IS NULL OR p_search_ctx_id IS NOT NULL THEN
      IF p_search_ctx_id IS NULL THEN
        OPEN x_cursor FOR 
          SELECT /*+ USE_NL(srch stage)  */ stage.ORG_CONTACT_ID, stage.PARTY_ID, stage.TX23, stage.TX2, stage.TX22
          FROM  HZ_SRCH_CONTACTS srch, HZ_STAGED_CONTACTS stage
          WHERE
            srch.batch_id = -1
            AND
            (
...
(srch.TX23 is not null and stage.TX23 like srch.TX23 || '%')
      ...
      ELSE
          OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_CONTACTS_N1) */ stage.ORG_CONTACT_ID, stage.PARTY_ID, stage.TX23, stage.TX2, stage.TX22
            FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACTS stage, HZ_SRCH_CONTACTS srch
            WHERE
            srch.batch_id = -1
            AND
            (
...
(srch.TX23 is not null and stage.TX23 like srch.TX23 || '%')
      ...
      END IF;
    ELSE
       l_sqlstr := 'SELECT /*+ USE_NL(srch stage)  */  stage.ORG_CONTACT_ID, stage.PARTY_ID , stage.TX23, stage.TX2, stage.TX22'||
                   ' FROM HZ_SRCH_CONTACTS srch, HZ_STAGED_CONTACTS stage'||
                   ' WHERE '||
            'srch.batch_id = -1'||
            'AND'||
            '('||
-------CONTACTS LEVEL ACQUISITION ON NON-FILTER ATTRIBUTES USING B-TREE INDEXES ---------
-- do an or between all the transformations of an attribute -- 
'('||
'(srch.TX23 is not null and stage.TX23 like srch.TX23 || ''%'')'||
')'||
')'||
  ...
  BEGIN
     IF p_party_id IS NOT NULL THEN
       OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ stage.CONTACT_POINT_ID, stage.PARTY_ID,stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , stage.TX5, stage.TX7, stage.TX10, stage.TX158
...
-- do an or between all the transformations of an attribute -- 
(
(srch.TX7 is not null and stage.TX7 like srch.TX7 || '%')
)
)
        AND EXISTS (
          SELECT /*+ INDEX(p HZ_STAGED_PARTIES_U1) */ 1 FROM HZ_STAGED_PARTIES p
    ...
    ELSIF p_restrict_sql IS NULL OR p_search_ctx_id IS NOT NULL THEN
      IF p_search_ctx_id IS NULL THEN
        OPEN x_cursor FOR 
          SELECT /*+ USE_NL(srch stage)  */  stage.CONTACT_POINT_ID, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , stage.TX5, stage.TX7, stage.TX10, stage.TX158
...
(srch.TX7 is not null and stage.TX7 like srch.TX7 || '%')
      ...
      ELSE
          OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ stage.CONTACT_POINT_ID, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , stage.TX5, stage.TX7, stage.TX10, stage.TX158
...
(srch.TX7 is not null and stage.TX7 like srch.TX7 || '%')
      ...
      END IF;
    ELSE
       l_sqlstr := 'SELECT /*+ USE_NL(srch stage)  */ stage.CONTACT_POINT_ID, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID  , stage.TX5, stage.TX7, stage.TX10, stage.TX158'||
                   ' FROM HZ_SRCH_CPTS srch, HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE'||
            'srch.batch_id = -1'||
            'AND'||
  ...
  BEGIN
    ...
    LOOP 
      FETCH l_party_cur INTO
         l_party_id , l_TX59, l_TX2, l_TX41, l_TX45;
      EXIT WHEN l_party_cur%NOTFOUND;
      l_index := map_id(l_party_id);
      l_score := GET_PARTY_SCORE(l_TX59,l_TX2,l_TX41,l_TX45);
  ...
  BEGIN
    ...
    LOOP 
      FETCH l_contact_cur INTO
         l_org_contact_id, l_ct_party_id , l_TX23, l_TX2, l_TX22;
      EXIT WHEN l_contact_cur%NOTFOUND;
      l_index := map_id(l_ct_party_id);
      l_score := GET_CONTACTS_SCORE(l_match_idx,l_TX23,l_TX2,l_TX22);
      IF NOT H_SCORES.EXISTS(l_index) THEN
  ...
  BEGIN
    ...
    LOOP 
      FETCH l_contact_pt_cur INTO
         l_contact_pt_id, l_cpt_party_id,  l_cpt_ps_id, l_cpt_contact_id , l_TX5, l_TX7, l_TX10, l_TX158;
      ...
      IF l_use_contact_cpt_info OR l_ps_contact_id IS NOT NULL THEN
        ...
        l_score := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX5,l_TX7,l_TX10,l_TX158);
        IF NOT H_SCORES.EXISTS(l_index) THEN
    ...
    END IF;
      IF H_SCORES(l_party_id).TOTAL_SCORE>=175 THEN
          ...
          ELSE
            ...
            EXCEPTION 
              WHEN NO_DATA_FOUND THEN
                IF H_SCORES(l_party_id).TOTAL_SCORE>=300 THEN
                  l_merge_flag := 'Y';
                ELSE
                  l_merge_flag := 'N';
                END IF;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ROLLBACK to find_parties;
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_parties;
...
PROCEDURE find_persons (
  ...
  BEGIN
    OPEN x_cursor FOR 
      SELECT /*+ INDEX(stage HZ_STAGED_CONTACTS_U1) */ ORG_CONTACT_ID, PARTY_ID, TX23, TX2, TX22
  ...
  BEGIN
    ...
    END IF;
      IF (H_SCORES(l_party_id).TOTAL_SCORE/l_entered_max_score)>=(175/785) THEN
            INSERT INTO HZ_MATCHED_PARTIES_GT (SEARCH_CONTEXT_ID, PARTY_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ROLLBACK to find_persons;
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_persons');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_persons;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_party_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_duplicate_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_duplicate_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_duplicate_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.find_duplicate_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.call_api_dynamic');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_party_sites (
  ...
  BEGIN
    ...
    END IF;
      IF l_match_str = ' OR ' THEN
        IF (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=(175/785) THEN
      ...
      ELSE
           ...
           (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=(175/785) THEN
          INSERT INTO HZ_MATCHED_PARTY_SITES_GT (SEARCH_CONTEXT_ID, PARTY_ID, PARTY_SITE_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ROLLBACK to get_matching_party_sites ;
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.get_matching_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contacts (
  ...
  BEGIN
    ...
    END IF;
        IF (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(175/785) THEN
      ...
      ELSE
           ...
           (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(175/785) THEN
          INSERT INTO HZ_MATCHED_CONTACTS_GT (SEARCH_CONTEXT_ID, PARTY_ID, ORG_CONTACT_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ROLLBACK to get_matching_contacts ;
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.get_matching_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      LOOP
        ...
        IF (p_dup_contact_point_id IS NULL OR (
            ...
            H_SCORE(cnt) := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX5,l_TX7,l_TX10,l_TX158);
            cnt := cnt+1;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
...
ROLLBACK to get_matching_contact_points ;
    ...
    END IF;
        IF (H_SCORE(I)/l_entered_max_score)>=(175/785) THEN
        INSERT INTO HZ_MATCHED_CPTS_GT(SEARCH_CONTEXT_ID,CONTACT_POINT_ID,PARTY_ID,SCORE) VALUES (
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ROLLBACK to get_matching_contact_points ;
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.get_matching_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
  ...
  BEGIN
    ...
    LOOP 
        FETCH l_party_cur INTO
           l_party_id , l_TX59, l_TX2, l_TX41, l_TX45;
        EXIT WHEN l_party_cur%NOTFOUND;
          INSERT_PARTY_SCORE(p_party_id, p_party_id, l_search_ctx_id, p_party_search_rec, g_party_stage_rec, l_TX59,l_TX2,l_TX41,l_TX45,1);
    END LOOP;
    CLOSE l_party_cur;
    ...
    IF l_contact_contains_str IS NOT NULL THEN
      ...
      CLOSE l_contact_cur;
      IF l_max_score>0 THEN
        INSERT_CONTACTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_list(l_max_idx), g_contact_stage_list(l_max_idx) ,l_max_TX23,l_max_TX2,l_max_TX22,l_max_idx);
      END IF;
    END IF;
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      END LOOP;
      IF l_max_score>0 THEN
        INSERT_CONTACT_POINTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_point_list(l_max_idx), g_contact_pt_stage_list(l_max_idx) ,l_max_TX5,l_max_TX7,l_max_TX10,l_max_TX158,l_max_idx);
      END IF;
      CLOSE l_contact_pt_cur;
    END IF;
...
EXCEPTION
 ...
 --------------- DELETE FROM ALL SRCH TABLES ---------------------
    ...
    DELETE FROM HZ_SRCH_CPTS WHERE batch_id = -1 ;
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_52.get_score_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    IF l_party_type = 'ORGANIZATION' THEN
      SELECT 
       ...
       ,HZ_ORGANIZATION_PROFILES.SIC_CODE
      INTO 
...
BEGIN
  ...
  END IF;
IF p_search_ctx THEN
     insert into HZ_SRCH_PARTIES(batch_id,party_id, party_osr,party_os,TX41,TX45,TX59,TX2,TX36,TX46) values(-1,-1,-1,-1,x_stage_rec.TX41,x_stage_rec.TX45,x_stage_rec.TX59,x_stage_rec.TX2,x_stage_rec.TX36,x_stage_rec.TX46);
END IF;
EXCEPTION
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_TRANSFORM_PROC_ERROR');
...
BEGIN
  ...
  FOR I IN 1..p_search_list.COUNT LOOP
    ...
    ELSE
      IF p_search_list(I).CONTACT_NAME IS NOT NULL THEN 
        tmp := HZ_TRANS_PKG.BASIC_WRPERSON(
             x_stage_list(I).TX23
             ,null,'CONTACT_NAME'
             ,'CONTACTS','SCORE');
        IF tmp IS NOT NULL THEN
          x_stage_list(I).TX23 := tmp;
        END IF;
      END IF;
    END IF;
  END LOOP;
...
IF p_search_ctx THEN
  FOR I IN 1..p_search_list.COUNT LOOP
              insert into HZ_SRCH_CONTACTS(batch_id,party_id, party_osr,party_os, org_contact_id,contact_osr, contact_os,new_party_flag ,TX23,TX2,TX22) values(-1,-1,-1,-1,-1,-1,-1,'Y',x_stage_list(I).TX23,x_stage_list(I).TX2,x_stage_list(I).TX22);
  END LOOP;
END IF;
EXCEPTION
  WHEN OTHERS THEN
...
IF p_search_ctx THEN
  FOR I IN 1..p_search_list.COUNT LOOP
              insert into HZ_SRCH_CPTS(batch_id,party_id, party_osr,party_os, contact_point_id,contact_pt_osr, contact_pt_os,contact_point_type,new_party_flag ,TX5,TX158,TX10,TX7) values(-1,-1,-1,-1,-1,-1,-1,-1,'Y',x_stage_list(I).TX5,x_stage_list(I).TX158,x_stage_list(I).TX10,x_stage_list(I).TX7);
  END LOOP;
END IF; 
EXCEPTION
  WHEN OTHERS THEN
...
PROCEDURE check_contact_cond(
  ...
  BEGIN
    ...
    FOR I IN 1..p_contact_list.COUNT LOOP
      IF p_contact_list(I).CONTACT_NAME IS NOT NULL 
         OR p_contact_list(I).JOB_TITLE IS NOT NULL 
      THEN
        x_secondary := TRUE;
      END IF;
      EXIT WHEN x_secondary;
