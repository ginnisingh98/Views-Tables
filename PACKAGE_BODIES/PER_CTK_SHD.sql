--------------------------------------------------------
--  DDL for Package Body PER_CTK_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CTK_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_ctk_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PER_TASKS_IN_CHECKLIST_PK') Then
...
Function api_updating
  ...
  Cursor C_Sel1 is
    ...
    where       task_in_checklist_id = p_task_in_checklist_id;
  --
  l_fct_ret     boolean;
  --
Begin
  --
  If (p_task_in_checklist_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_task_in_checklist_id
        = per_ctk_shd.g_old_rec.task_in_checklist_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into per_ctk_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> per_ctk_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  l_rec.checklist_task_name              := p_checklist_task_name;
  l_rec.eligibility_object_id            := p_eligibility_object_id;
  l_rec.eligibility_profile_id           := p_eligibility_profile_id;
  l_rec.ame_attribute_identifier         := p_ame_attribute_identifier;
  l_rec.description                      := p_description;
  l_rec.mandatory_flag                   := p_mandatory_flag;
  l_rec.target_duration                  := p_target_duration;
  l_rec.target_duration_uom              := p_target_duration_uom;
