--------------------------------------------------------
--  DDL for Package Body BEN_PCM_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCM_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcm_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_per_cm_f',
	   ...
	   p_base_key_value	=> p_rec.per_cm_id);
    --
    ben_pcm_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_per_cm_f Row
    --
    update  ben_per_cm_f
    set
    ...
    --
    ben_pcm_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_pcm_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.per_cm_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pcm_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.lf_evt_ocrd_dt = hr_api.g_date) then
    ...
    ben_pcm_shd.g_old_rec.lf_evt_ocrd_dt;
  End If;
  If (p_rec.rqstbl_untl_dt = hr_api.g_date) then
    p_rec.rqstbl_untl_dt :=
    ben_pcm_shd.g_old_rec.rqstbl_untl_dt;
  End If;
  If (p_rec.ler_id = hr_api.g_number) then
    p_rec.ler_id :=
    ben_pcm_shd.g_old_rec.ler_id;
  End If;
  If (p_rec.per_in_ler_id = hr_api.g_number) then
    p_rec.per_in_ler_id :=
    ben_pcm_shd.g_old_rec.per_in_ler_id;
  End If;
  If (p_rec.prtt_enrt_actn_id = hr_api.g_number) then
    p_rec.prtt_enrt_actn_id :=
    ben_pcm_shd.g_old_rec.prtt_enrt_actn_id;
  End If;
  If (p_rec.person_id = hr_api.g_number) then
    p_rec.person_id :=
    ben_pcm_shd.g_old_rec.person_id;
  End If;
  If (p_rec.bnf_person_id = hr_api.g_number) then
    p_rec.bnf_person_id :=
    ben_pcm_shd.g_old_rec.bnf_person_id;
  End If;
  If (p_rec.dpnt_person_id = hr_api.g_number) then
    p_rec.dpnt_person_id :=
    ben_pcm_shd.g_old_rec.dpnt_person_id;
  End If;
  If (p_rec.cm_typ_id = hr_api.g_number) then
    p_rec.cm_typ_id :=
    ben_pcm_shd.g_old_rec.cm_typ_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_pcm_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pcm_attribute_category = hr_api.g_varchar2) then
    p_rec.pcm_attribute_category :=
    ben_pcm_shd.g_old_rec.pcm_attribute_category;
  End If;
  If (p_rec.pcm_attribute1 = hr_api.g_varchar2) then
    p_rec.pcm_attribute1 :=
    ben_pcm_shd.g_old_rec.pcm_attribute1;
  End If;
  If (p_rec.pcm_attribute2 = hr_api.g_varchar2) then
    p_rec.pcm_attribute2 :=
    ben_pcm_shd.g_old_rec.pcm_attribute2;
  End If;
  If (p_rec.pcm_attribute3 = hr_api.g_varchar2) then
    p_rec.pcm_attribute3 :=
    ben_pcm_shd.g_old_rec.pcm_attribute3;
  End If;
  If (p_rec.pcm_attribute4 = hr_api.g_varchar2) then
    p_rec.pcm_attribute4 :=
    ben_pcm_shd.g_old_rec.pcm_attribute4;
  End If;
  If (p_rec.pcm_attribute5 = hr_api.g_varchar2) then
    p_rec.pcm_attribute5 :=
    ben_pcm_shd.g_old_rec.pcm_attribute5;
  End If;
  If (p_rec.pcm_attribute6 = hr_api.g_varchar2) then
    p_rec.pcm_attribute6 :=
    ben_pcm_shd.g_old_rec.pcm_attribute6;
  End If;
  If (p_rec.pcm_attribute7 = hr_api.g_varchar2) then
    p_rec.pcm_attribute7 :=
    ben_pcm_shd.g_old_rec.pcm_attribute7;
  End If;
  If (p_rec.pcm_attribute8 = hr_api.g_varchar2) then
    p_rec.pcm_attribute8 :=
    ben_pcm_shd.g_old_rec.pcm_attribute8;
  End If;
  If (p_rec.pcm_attribute9 = hr_api.g_varchar2) then
    p_rec.pcm_attribute9 :=
    ben_pcm_shd.g_old_rec.pcm_attribute9;
  End If;
  If (p_rec.pcm_attribute10 = hr_api.g_varchar2) then
    p_rec.pcm_attribute10 :=
    ben_pcm_shd.g_old_rec.pcm_attribute10;
  End If;
  If (p_rec.pcm_attribute11 = hr_api.g_varchar2) then
    p_rec.pcm_attribute11 :=
    ben_pcm_shd.g_old_rec.pcm_attribute11;
  End If;
  If (p_rec.pcm_attribute12 = hr_api.g_varchar2) then
    p_rec.pcm_attribute12 :=
    ben_pcm_shd.g_old_rec.pcm_attribute12;
  End If;
  If (p_rec.pcm_attribute13 = hr_api.g_varchar2) then
    p_rec.pcm_attribute13 :=
    ben_pcm_shd.g_old_rec.pcm_attribute13;
  End If;
  If (p_rec.pcm_attribute14 = hr_api.g_varchar2) then
    p_rec.pcm_attribute14 :=
    ben_pcm_shd.g_old_rec.pcm_attribute14;
  End If;
  If (p_rec.pcm_attribute15 = hr_api.g_varchar2) then
    p_rec.pcm_attribute15 :=
    ben_pcm_shd.g_old_rec.pcm_attribute15;
  End If;
  If (p_rec.pcm_attribute16 = hr_api.g_varchar2) then
    p_rec.pcm_attribute16 :=
    ben_pcm_shd.g_old_rec.pcm_attribute16;
  End If;
  If (p_rec.pcm_attribute17 = hr_api.g_varchar2) then
    p_rec.pcm_attribute17 :=
    ben_pcm_shd.g_old_rec.pcm_attribute17;
  End If;
  If (p_rec.pcm_attribute18 = hr_api.g_varchar2) then
    p_rec.pcm_attribute18 :=
    ben_pcm_shd.g_old_rec.pcm_attribute18;
  End If;
  If (p_rec.pcm_attribute19 = hr_api.g_varchar2) then
    p_rec.pcm_attribute19 :=
    ben_pcm_shd.g_old_rec.pcm_attribute19;
  End If;
  If (p_rec.pcm_attribute20 = hr_api.g_varchar2) then
    p_rec.pcm_attribute20 :=
    ben_pcm_shd.g_old_rec.pcm_attribute20;
  End If;
  If (p_rec.pcm_attribute21 = hr_api.g_varchar2) then
    p_rec.pcm_attribute21 :=
    ben_pcm_shd.g_old_rec.pcm_attribute21;
  End If;
  If (p_rec.pcm_attribute22 = hr_api.g_varchar2) then
    p_rec.pcm_attribute22 :=
    ben_pcm_shd.g_old_rec.pcm_attribute22;
  End If;
  If (p_rec.pcm_attribute23 = hr_api.g_varchar2) then
    p_rec.pcm_attribute23 :=
    ben_pcm_shd.g_old_rec.pcm_attribute23;
  End If;
  If (p_rec.pcm_attribute24 = hr_api.g_varchar2) then
    p_rec.pcm_attribute24 :=
    ben_pcm_shd.g_old_rec.pcm_attribute24;
  End If;
  If (p_rec.pcm_attribute25 = hr_api.g_varchar2) then
    p_rec.pcm_attribute25 :=
    ben_pcm_shd.g_old_rec.pcm_attribute25;
  End If;
  If (p_rec.pcm_attribute26 = hr_api.g_varchar2) then
    p_rec.pcm_attribute26 :=
    ben_pcm_shd.g_old_rec.pcm_attribute26;
  End If;
  If (p_rec.pcm_attribute27 = hr_api.g_varchar2) then
    p_rec.pcm_attribute27 :=
    ben_pcm_shd.g_old_rec.pcm_attribute27;
  End If;
  If (p_rec.pcm_attribute28 = hr_api.g_varchar2) then
    p_rec.pcm_attribute28 :=
    ben_pcm_shd.g_old_rec.pcm_attribute28;
  End If;
  If (p_rec.pcm_attribute29 = hr_api.g_varchar2) then
    p_rec.pcm_attribute29 :=
    ben_pcm_shd.g_old_rec.pcm_attribute29;
  End If;
  If (p_rec.pcm_attribute30 = hr_api.g_varchar2) then
    p_rec.pcm_attribute30 :=
    ben_pcm_shd.g_old_rec.pcm_attribute30;
  End If;
  If (p_rec.request_id = hr_api.g_number) then
    p_rec.request_id :=
    ben_pcm_shd.g_old_rec.request_id;
  End If;
  If (p_rec.program_application_id = hr_api.g_number) then
    p_rec.program_application_id :=
    ben_pcm_shd.g_old_rec.program_application_id;
  End If;
  If (p_rec.program_id = hr_api.g_number) then
    p_rec.program_id :=
    ben_pcm_shd.g_old_rec.program_id;
  End If;
