--------------------------------------------------------
--  DDL for Package Body INV_PICK_SLIP_REPORT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_PICK_SLIP_REPORT" AS
  /* $Header: INVPKSLB.pls 120.6.12010000.3 2009/11/16 20:46:02 pupakare ship $ */
  ...
  FUNCTION chk_wms_install(p_organization_id IN NUMBER)
    RETURN VARCHAR2 IS
    l_msg_count         NUMBER;
  ...
  BEGIN
    IF wms_install.check_install(x_return_status => l_return_status, x_msg_count => l_msg_count, x_msg_data => l_msg_data
  ...
  Created By :
  Date Created on :
  Purpose : This procedure will be called from Move Order Pick Slip Report.
  Change History
  Who             When            What
  ------------------------------------------------------------------------
  Nalin Kumar     22-Nov-2004     Modified the Procedure and added parameter p_request_id
                                  to fix Bug# 4003379.
  ------------------------------------------------------------------------
...
/*    FOR v_mo_line_rec IN c_move_order_lines(v_mo_header_rec.header_id, v_mo_header_rec.move_order_type) LOOP */
     ...
     --END of new code added to fix Bug# 4003379
         ...
         IF l_debug = 1 THEN
           mydebug('l_tracking_quantity_ind     : ' ||l_tracking_quantity_ind,
l_api_name);
  ...
  , p_pick_slip_group_rule_id VARCHAR2
  ) RETURN NUMBER IS
