--------------------------------------------------------
--  DDL for Package Body OKC_CEVT_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_CEVT_ASMBLR_PVT" AS
/* $Header: OKCRCERB.pls 120.0 2005/05/25 17:59:51 appldev noship $ */
...
PROCEDURE acn_assemble(
 ...
 l_chr_modifier  	varchar2(120);
 l_cont_evt_date	date;

 --Select all the attributes for the contingent event recorded
 ...
 AND acn.correlation = 'CONT_EVT_REC';

 --Select the contract attributes
 ...
 SELECT khr.id k_id,
	...
	khr.sts_code
  FROM   okc_k_headers_v khr,okc_subclasses_v cls
...
BEGIN
 ...
 l_return_status := OKC_API.START_ACTIVITY
  ...
  --The correlation for contingent event occurrence
  l_corrid_rec.corrid := 'CONT_EVT_REC' ;
 ...
 FOR k1_rec in chr_csr(p_chr_id       => p_contract_id) LOOP
           k := 1;
	      --Initialize the table
  	      l_msg_tbl := okc_aq_pvt.msg_tab_typ();
		...
		ELSIF elements_rec.element_name = 'CONT_EVT_NAME' THEN
  			l_msg_tbl.extend;
  			l_msg_tbl(k).element_name    := elements_rec.element_name;
  			l_msg_tbl(k).element_value   := p_cont_evt_name;
		ELSIF elements_rec.element_name = 'CONT_EVT_DATE' THEN
  			l_msg_tbl.extend;
  			l_msg_tbl(k).element_name    := elements_rec.element_name;
			IF elements_rec.format_mask IS NOT NULL THEN
				l_cont_evt_date := to_char(p_cont_evt_date, elements_rec.format_mask);
   			ELSE
				l_cont_evt_date := to_char(p_cont_evt_date, 'DD-MON-YY');
			END IF;
  			l_msg_tbl(k).element_value   := l_cont_evt_date;
    		END IF;
