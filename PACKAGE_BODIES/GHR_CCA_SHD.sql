--------------------------------------------------------
--  DDL for Package Body GHR_CCA_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GHR_CCA_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ghr_cca_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'GHR_COMPL_APPEALS_FK1') Then
    fnd_message.set_name('GHR', 'GHR_38700_INVALID_COMPLAINT');
    fnd_message.set_token('PROCEDURE', l_proc);
    fnd_message.set_token('STEP','5');
    fnd_message.raise_error;
  ElsIf (p_constraint_name = 'GHR_COMPL_APPEALS_PK') Then
...
Function api_updating
  ...
  Cursor C_Sel1 is
    select
      ...
      ,docket_num
      ,org_notified_of_appeal
      ,agency_recvd_req_for_files
      ,files_due
      ,files_forwd
      ,agcy_recvd_appellant_brief
      ,agency_brief_due
      ,appellant_brief_forwd_org
      ,org_forwd_brief_to_agency
      ,agency_brief_forwd
      ,decision_date
      ,dec_recvd_by_agency
      ,decision
    ...
    from        ghr_compl_appeals
    where       compl_appeal_id = p_compl_appeal_id;
  --
  l_fct_ret     boolean;
  --
Begin
  --
  If (p_compl_appeal_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_compl_appeal_id
        = ghr_cca_shd.g_old_rec.compl_appeal_id and
        p_object_version_number
        = ghr_cca_shd.g_old_rec.object_version_number
       ) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ghr_cca_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ghr_cca_shd.g_old_rec.object_version_number) Then
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'ghr_compl_appeals');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  l_rec.appealed_to                      := p_appealed_to;
  ...
  l_rec.org_notified_of_appeal           := p_org_notified_of_appeal;
  l_rec.agency_recvd_req_for_files       := p_agency_recvd_req_for_files;
  l_rec.files_due                        := p_files_due;
  l_rec.files_forwd                      := p_files_forwd;
  l_rec.agcy_recvd_appellant_brief       := p_agcy_recvd_appellant_brief;
  l_rec.agency_brief_due                 := p_agency_brief_due;
  l_rec.appellant_brief_forwd_org        := p_appellant_brief_forwd_org;
  l_rec.org_forwd_brief_to_agency        := p_org_forwd_brief_to_agency;
