--------------------------------------------------------
--  DDL for Package Body HR_IPT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_IPT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_ipt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_ipt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_item_properties_tl Row
  --
  update hr_item_properties_tl
    set
    ...
    ,information_prompt              = p_rec.information_prompt
    ,label                           = p_rec.label
    ,prompt_text                     = p_rec.prompt_text
    ,tooltip_text                    = p_rec.tooltip_text
...
Begin
  ...
  begin
    ...
    hr_ipt_rku.after_update
      ...
      ,p_default_value
      => p_rec.default_value
      ,p_information_prompt
      => p_rec.information_prompt
      ,p_label
      => p_rec.label
      ,p_prompt_text
      => p_rec.prompt_text
      ...
      => p_rec.tooltip_text
      ,p_source_lang_o
      => hr_ipt_shd.g_old_rec.source_lang
      ,p_default_value_o
      => hr_ipt_shd.g_old_rec.default_value
      ,p_information_prompt_o
      => hr_ipt_shd.g_old_rec.information_prompt
      ,p_label_o
      => hr_ipt_shd.g_old_rec.label
      ,p_prompt_text_o
      => hr_ipt_shd.g_old_rec.prompt_text
      ,p_tooltip_text_o
      => hr_ipt_shd.g_old_rec.tooltip_text
      );
    --
  exception
    --
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'HR_ITEM_PROPERTIES_TL'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
Begin
  ...
  If (p_rec.default_value = hr_api.g_varchar2) then
    ...
    hr_ipt_shd.g_old_rec.default_value;
  End If;
  If (p_rec.information_prompt = hr_api.g_varchar2) then
    p_rec.information_prompt :=
    hr_ipt_shd.g_old_rec.information_prompt;
  End If;
  If (p_rec.label = hr_api.g_varchar2) then
    ...
    hr_ipt_shd.g_old_rec.label;
  End If;
  If (p_rec.prompt_text = hr_api.g_varchar2) then
    p_rec.prompt_text :=
    hr_ipt_shd.g_old_rec.prompt_text;
  End If;
  If (p_rec.tooltip_text = hr_api.g_varchar2) then
    p_rec.tooltip_text :=
    hr_ipt_shd.g_old_rec.tooltip_text;
  End If;
  --
...
Begin
  ...
  --
  convert_defs(p_rec);
  hr_ipt_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hr_ipt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_ipt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_ipt_upd.post_update
     (p_rec
     );
End upd;
--
...
Procedure upd_tl
  ...
  cursor csr_upd_langs is
    select ipt.language
     ...
     where ipt.item_property_id = p_item_property_id
