--------------------------------------------------------
--  DDL for Package Body PAY_SA_HIST_ARCH_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_SA_HIST_ARCH_PKG" AS
/* $Header: pysapupg.pkb 120.0.12010000.2 2009/05/31 10:16:08 krreddy noship $ */
...
begin
  ...
  pay_sa_hist_arch_pkg.get_parameters (
...
select distinct paa.assignment_action_id,
  ...
  where paa.SOURCE_ACTION_ID is null
   ...
   for asgrec in get_arch_action_det(l_payroll_id,l_bg_id,l_start_date,l_end_date)
   loop
...
procedure archive_historic_data (p_assactid       in number,
	...
	CURSOR get_run_action_det (p_locking_action_id NUMBER) IS
...
CURSOR get_prl_actid (p_assignment_action_id NUMBER) IS
			SELECT payroll_action_id
			FROM   pay_assignment_actions
			WHERE  assignment_Action_id = p_assignment_action_id;

    l_assignment_action_id pay_assignment_actions.assignment_action_id%type;
    l_run_asg_action_id pay_assignment_actions.assignment_action_id%type;
...
begin
	...
	hr_utility.set_location('Archived assignment_action_id - l_assignment_action_id: '
	   ...
	   open get_run_action_det(l_assignment_action_id);
            fetch get_run_action_det into l_assignment_id, l_run_asg_action_id, l_effective_date;
           close get_run_action_det;

	   open get_prl_actid(l_assignment_action_id);
            fetch get_prl_actid into l_payroll_action_id;
           close get_prl_actid;

    --Calling archive procedure to archive payments and deductions details for
        --this assignment_action_id
	PAY_SA_ARCHIVE.ARCHIVE_MAIN_ELEMENTS	 (p_archive_assact_id     => l_assignment_action_id,
						  p_run_assact_id  => l_run_asg_action_id,
