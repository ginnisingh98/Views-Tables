--------------------------------------------------------
--  DDL for Package Body PER_ES_CREATE_EMPLOYEE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ES_CREATE_EMPLOYEE" as
/* $Header: peesempp.pkb 120.3 2006/09/14 14:20:13 mgettins noship $ */
g_package   VARCHAR2(30) := 'PER_ES_CREATE_EMPLOYEE.';
...
l_validate_profile  VARCHAR2(20);
l_proc              VARCHAR2(72) := g_package||'CREATE_ES_EMPLOYEE';
...
BEGIN
   ...
   IF hr_utility.chk_product_install('Oracle Human Resources', 'ES') THEN
     --
        ...
        l_validate_profile := FND_PROFILE.VALUE('PER_NATIONAL_IDENTIFIER_VALIDATION');
       ...
       END IF;
        --
        IF l_validate_profile <> 'NONE' THEN --No Validation
            ...
            ELSE
                ...
                IF l_national_identifier = 'N' THEN
        ...
        IF  p_per_information2 IS NOT NULL THEN
            ...
            IF  get_lookup_type%NOTFOUND THEN
                ...
                hr_utility.set_message_token(800,'FIELD',hr_general.decode_lookup('ES_FORM_LABELS','IDENTIFIER_TYPE'));
                hr_utility.raise_error;
            ELSIF l_validate_profile <> 'NONE' THEN
                l_identifier_value := hr_es_utility.validate_identifier(p_per_information2,p_per_information3);
            END IF;
            CLOSE get_lookup_type;
        END IF;

        IF  l_validate_profile <> 'NONE' and p_per_information2 IS NOT NULL and p_per_information3 IS NOT NULL and p_national_identifier is not null THEN
            OPEN get_lookup_type(p_per_information2);
            FETCH get_lookup_type into l_identifier_type;
