--------------------------------------------------------
--  DDL for Package Body HR_JOB_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_JOB_BE1" as 
...
p_name                         varchar2) is
  ...
  --
  l_proc varchar2(72):='  hr_job_be1.create_job_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.job.create_job';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<approval_authority>';
    l_text:=l_text||fnd_number.number_to_canonical(p_approval_authority);
    l_text:=l_text||'</approval_authority>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<benchmark_job_flag>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_benchmark_job_flag);
    l_text:=l_text||'</benchmark_job_flag>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<benchmark_job_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_benchmark_job_id);
    l_text:=l_text||'</benchmark_job_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<emp_rights_flag>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_emp_rights_flag);
    l_text:=l_text||'</emp_rights_flag>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_group_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_job_group_id);
    l_text:=l_text||'</job_group_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information_category>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information_category);
    l_text:=l_text||'</job_information_category>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information1>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information1);
    l_text:=l_text||'</job_information1>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information2>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information2);
    l_text:=l_text||'</job_information2>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information3>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information3);
    l_text:=l_text||'</job_information3>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information4>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information4);
    l_text:=l_text||'</job_information4>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information5>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information5);
    l_text:=l_text||'</job_information5>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information6>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information6);
    l_text:=l_text||'</job_information6>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information7>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information7);
    l_text:=l_text||'</job_information7>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information8>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information8);
    l_text:=l_text||'</job_information8>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information9>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information9);
    l_text:=l_text||'</job_information9>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information10>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information10);
    l_text:=l_text||'</job_information10>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information11>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information11);
    l_text:=l_text||'</job_information11>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information12>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information12);
    l_text:=l_text||'</job_information12>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information13>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information13);
    l_text:=l_text||'</job_information13>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information14>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information14);
    l_text:=l_text||'</job_information14>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information15>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information15);
    l_text:=l_text||'</job_information15>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information16>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information16);
    l_text:=l_text||'</job_information16>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information17>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information17);
    l_text:=l_text||'</job_information17>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information18>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information18);
    l_text:=l_text||'</job_information18>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information19>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information19);
    l_text:=l_text||'</job_information19>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_information20>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_job_information20);
    l_text:=l_text||'</job_information20>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<segment1>';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<job_definition_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_job_definition_id);
    l_text:=l_text||'</job_definition_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<name>';
