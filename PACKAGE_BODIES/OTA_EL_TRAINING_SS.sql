--------------------------------------------------------
--  DDL for Package Body OTA_EL_TRAINING_SS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_EL_TRAINING_SS" AS
 /* $Header: oteltwrs.pkb 120.2 2008/01/08 14:02:04 aabalakr noship $*/

   g_package      varchar2(30)   := 'OTA_EL_TRAINING_SS';
...
PROCEDURE process_api2
  ...
  l_person_details		OTA_EL_ENROLL_SS.csr_person_to_enroll_details%ROWTYPE;
  l_specialInstruction          ota_delegate_bookings.SPECIAL_BOOKING_INSTRUCTIONS%TYPE;
...
WHERE	evt.event_id = l_event_id
 ...
 --variable l_eventid is replaced with l_event_id as it is passed to the cursor
...
--                ,p_name                => 'P_PERSON_ID'));
        ...
        l_person_details := OTA_EL_ENROLL_SS.Get_Person_To_Enroll_Details(p_person_id => l_delegate_id);
        ...
        l_restricted_assignment_id := OTA_EL_ENROLL_SS.CHK_DELEGATE_OK_FOR_EVENT(p_delegate_id => l_delegate_id
...
--                            hr_transaction_web.get_transaction_id
        ...
        l_cancel_boolean := OTA_EL_ENROLL_SS.Chk_Event_Cancelled_for_Person(p_event_id           => l_eventid
        ...
        IF (l_cancel_boolean) THEN
         -- Call Cancel procedure to cancel the Finance if person Re-enroll
          OTA_EL_ENROLL_SS.cancel_finance(l_booking_id);
        END IF;


      l_booking_status_row := OTA_EL_ENROLL_SS.Get_Booking_Status_for_web(
