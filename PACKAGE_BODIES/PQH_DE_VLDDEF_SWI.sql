--------------------------------------------------------
--  DDL for Package Body PQH_DE_VLDDEF_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_DE_VLDDEF_SWI" As
/* $Header: pqdefswi.pkb 115.1 2002/11/27 23:43:08 rpasapul noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'pqh_de_vlddef_swi.';
--
-- ----------------------------------------------------------------------------
-- |---------------------------< delete_vldtn_defn >--------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_vldtn_defn_swi;
  --
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to delete_vldtn_defn_swi;
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint insert_vldtn_defn_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Register Surrogate ID or user key values
  --
  pqh_def_ins.set_base_key_value
    (p_wrkplc_vldtn_id => p_wrkplc_vldtn_id
    );
  --
  -- Call API
  --
  pqh_de_vlddef_api.insert_vldtn_defn
    (p_validate                     => l_validate
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to insert_vldtn_defn_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint update_vldtn_defn_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  pqh_de_vlddef_api.update_vldtn_defn
    (p_validate                     => l_validate
    ,p_effective_date               => p_effective_date
    ,p_business_group_id            => p_business_group_id
    ,p_validation_name              => p_validation_name
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_vldtn_defn_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
