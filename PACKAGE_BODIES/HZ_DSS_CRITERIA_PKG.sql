--------------------------------------------------------
--  DDL for Package Body HZ_DSS_CRITERIA_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_DSS_CRITERIA_PKG" AS
/* $Header: ARHPDSCB.pls 115.2 2002/10/10 01:42:09 chsaulit noship $ */
...
BEGIN
    ...
    BEGIN
      ...
      VALUES (
        DECODE(x_secured_item_id,
               FND_API.G_MISS_NUM, HZ_DSS_CRITERIA_S.NEXTVAL,
               NULL, HZ_DSS_CRITERIA_S.NEXTVAL,
               x_secured_item_id),
        DECODE(x_status,
    ...
    EXCEPTION
      WHEN DUP_VAL_ON_INDEX THEN
        IF INSTR(SQLERRM, 'HZ_DSS_CRITERIA_U1') <> 0 THEN
        DECLARE
          l_count             NUMBER;
          l_dummy             VARCHAR2(1);
        BEGIN
          ...
          WHILE l_count > 0 LOOP
            SELECT HZ_DSS_CRITERIA_S.NEXTVAL
            INTO x_secured_item_id FROM dual;
            BEGIN
              SELECT 'Y' INTO l_dummy
              FROM HZ_DSS_CRITERIA
              WHERE secured_item_id = x_secured_item_id;
              l_count := 1;
            EXCEPTION
...
BEGIN
    ...
    IF (
        ( ( Recinfo.secured_item_id = x_secured_item_id )
        OR ( ( Recinfo.secured_item_id IS NULL )
          AND (  x_secured_item_id IS NULL ) ) )
...
BEGIN
    ...
    SELECT
      NVL(secured_item_id, FND_API.G_MISS_NUM),
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
      ...
      FND_MESSAGE.SET_TOKEN('RECORD', 'dss_secured_rel');
      FND_MESSAGE.SET_TOKEN('VALUE', TO_CHAR(x_secured_item_id));
      FND_MSG_PUB.ADD;
