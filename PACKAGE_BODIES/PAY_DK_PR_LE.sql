--------------------------------------------------------
--  DDL for Package Body PAY_DK_PR_LE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_DK_PR_LE" AS
/* $Header: pydkprle.pkb 120.9.12000000.2 2007/05/08 07:09:22 saurai noship $ */
...
PROCEDURE GET_ALL_PARAMETERS(
	...
	SELECT
    	fnd_date.canonical_to_date(PAY_DK_PR_LE.GET_PARAMETER(legislative_parameters,'FROM_DATE'))
		,fnd_date.canonical_to_date(PAY_DK_PR_LE.GET_PARAMETER(legislative_parameters,'TO_DATE'))
	  	,PAY_DK_PR_LE.GET_PARAMETER(legislative_parameters,'PAYROLL_ID')
	  	,PAY_DK_PR_LE.GET_PARAMETER(legislative_parameters,'LEGAL_EMPLOYER_ID')
		,PAY_DK_PR_LE.GET_PARAMETER(legislative_parameters,'ELEMENT_SET_ID')
		,PAY_DK_PR_LE.GET_PARAMETER(legislative_parameters,'ARCHIVE')
		,effective_date
		,business_group_id
...
BEGIN
	 ...
	 FETCH csr_parameter_info  INTO	p_fromdate,p_todate,p_payroll_id ,p_legal_employer_id,
...
and    action_information1         = 'PYDKPRLEA'
...
IS
 SELECT
	SUM (pay_balance_pkg.get_value(pay_dk_pr_le.get_defined_balance_id
...
SELECT
	...
	,haou1.NAME legal_employer_name
	...
	,pay_dk_pr_le.costed_code(prr.run_result_id,pivf.input_value_id) costed_code
	,pivf.input_value_id  input_value_id
	,to_char(ppa1.date_earned,'DD-MON-RRRR')    date_earned
	,pay_balance_pkg.get_value(pay_dk_pr_le.get_defined_balance_id
        (pivf.input_value_id,'_ASG_RUN')
...
WHERE
	paaf.business_group_id = l_business_group_id -- BG Check
	AND paa1.tax_unit_id = nvl(l_legal_employer_id,paa1.tax_unit_id)
	AND ppf.payroll_id = nvl(l_payroll_id,ppf.payroll_id)
	AND  paaf.assignment_status_type_id = 1 -- to check for active assignments
	AND  paaf.payroll_id = ppf.payroll_id
	AND paa1.tax_unit_id= haou1.organization_id
	AND paaf.organization_id = haou2.organization_id
	AND paa1.assignment_id = paaf.assignment_id
	AND paa1.action_status = 'C'  -- Completed
	...
	,date_earned
	,costed_code
ORDER BY tax_unit_id,payroll_id,organization_id,ele_proc_prior,ele_type_id;
...
END IF;
 ...
 PAY_DK_PR_LE.GET_ALL_PARAMETERS(
		 pactid
		,l_business_group_id
		,l_effective_date
		,l_archive
...
BEGIN
	SELECT haou.name INTO l_legal_employer_name
	FROM hr_all_organization_units haou
	WHERE haou.organization_id=l_legal_employer_id;
EXCEPTION
	WHEN OTHERS THEN
	NULL;
END;
...
-- Archive the REPORT DETAILS
	...
	,p_action_information1          => 'PYDKPRLEA'	-- Conc Prg Short Name
	...
	,p_action_information7          => l_legal_employer_name -- Legal Employer Name
...
IF (csr_rec.pay_value > 0) THEN -- to archive only elements which have pay value > 0
  ...
  pay_action_information_api.create_action_information (
	...
	,p_jurisdiction_code            => NULL			        -- Tax Municipality ID
	,p_action_information1          => 'PYDKPRLEA'		        --Con Program Short Name
	,p_action_information2          => csr_rec.payroll_id		-- PayrollID
	,p_action_information3          => csr_rec.payroll_name		-- Payroll Name
	,p_action_information4          => csr_rec.legal_employer_name	-- Legal Employer Name
	,p_action_information5          => csr_rec.organization_id	-- Organization ID
	,p_action_information6          => csr_rec.organization_name	-- Organization Name
	,p_action_information7          =>  csr_rec.ele_type_id		-- element type id
	,p_action_information8          =>  csr_rec.ele_proc_prior	-- processing priority
	,p_action_information9          =>  csr_rec.ele_name		-- element name
	,p_action_information10         => csr_rec.date_earned		-- date_earned
	,p_action_information11         =>  csr_rec.costed_code		-- Costed Value
	/*Storing in Canonical format to fix issues due to varying numeric formats*/
	,p_action_information12         => fnd_number.number_to_canonical(csr_rec.pay_value) -- Pay Value
	,p_action_information13         => fnd_number.number_to_canonical(l_ele_ytd)         -- element YTD
	);
...
l_flag NUMBER :=0;
l_leg_emp_id NUMBER :=-999 ;
l_payroll_id NUMBER :=-999;
l_org_id NUMBER := -999;
l_tot_ele_run NUMBER := 0;
l_tot_ele_ytd NUMBER :=0;
l_tot_org_ele_run NUMBER := 0;
l_tot_org_ele_ytd NUMBER :=0;
l_tot_le_ele_run NUMBER := 0;
l_tot_le_ele_ytd NUMBER :=0;
...
SELECT
	...
	,pai2.tax_unit_id leg_emp_id
	,pai2.action_information4 leg_emp_name
	,pai2.action_information5 org_id
	,pai2.action_information6 org_name
	,pai2.action_information7 ele_type_id
	,pai2.action_information8 ele_proc_prior
	,pai2.action_information9 ele_name
	,pai2.action_information10 date_earned
	,pai2.action_information11 costed_code
	,fnd_number.canonical_to_number(pai2.action_information12) pay_value
	,fnd_number.canonical_to_number(pai2.action_information13) balance_amount
FROM
...
ORDER BY
	...
	,TO_NUMBER(pai2.action_information8)
	,TO_NUMBER(pai2.action_information7)
	,fnd_date.string_to_date(pai2.action_information10,'dd-mon-rrrr');
...
FOR csr_rec IN csr_pr_le LOOP

IF (csr_rec.payroll_id <> l_payroll_id) THEN
  IF (csr_pr_le%rowcount <> 1) THEN
    ...
    l_tot_org_ele_run := l_tot_org_ele_run + l_tot_ele_run;
    l_tot_org_ele_ytd := l_tot_org_ele_ytd + l_tot_ele_ytd;
    l_tot_ele_run := 0;
    l_tot_ele_ytd := 0;
	p_xml:= p_xml || '</ELEMENT_RECORD>';
	p_xml:=p_xml || '<Tot_org_ele_run>' || NVL(TRIM(TO_CHAR(ROUND (l_tot_org_ele_run,2),'999G999G990D99')),' ')|| '</Tot_org_ele_run>';
    p_xml:=p_xml || '<Tot_org_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_org_ele_ytd,2),'999G999G990D99')),' ')|| '</Tot_org_ele_ytd>';
    l_tot_le_ele_run := l_tot_le_ele_run + l_tot_org_ele_run;
    l_tot_le_ele_ytd := l_tot_le_ele_ytd + l_tot_org_ele_ytd;
    l_tot_org_ele_run := 0;
    l_tot_org_ele_ytd := 0;
	p_xml := p_xml ||'</ORGANIZATION_RECORD>';
	p_xml:=p_xml || '<Tot_le_ele_run>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_le_ele_run,2),'999G999G990D99')),' ')|| '</Tot_le_ele_run>';
    p_xml:=p_xml || '<Tot_le_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_le_ele_ytd,2),'999G999G990D99')),' ')|| '</Tot_le_ele_ytd>';
    l_tot_le_ele_run := 0;
    l_tot_le_ele_ytd := 0;
	p_xml := p_xml ||'</LEGAL_EMPLOYER_RECORD>';
	p_xml := p_xml ||'</PAYROLL_RECORD>';
	p_xml := p_xml ||'<PAYROLL_RECORD>';
	p_xml := p_xml ||'<LEGAL_EMPLOYER_RECORD>';
	p_xml := p_xml ||'<ORGANIZATION_RECORD>';
	p_xml:=p_xml || '<Report_Period>' || csr_rec.report_period || '</Report_Period>';
	p_xml:=p_xml || '<Legal_Employer>' || hr_dk_utility.REPLACE_SPECIAL_CHARS(csr_rec.leg_emp_name) || '</Legal_Employer>';
	p_xml:=p_xml || '<Payroll_Name>' || hr_dk_utility.REPLACE_SPECIAL_CHARS(csr_rec.payroll_name) || '</Payroll_Name>';
	p_xml:= p_xml || '<ELEMENT_RECORD>';
 END IF;
END IF;
IF (csr_rec.leg_emp_id <> l_leg_emp_id AND csr_rec.payroll_id = l_payroll_id) THEN
  IF (csr_pr_le%rowcount <> 1) THEN
    p_xml:=p_xml || '<Tot_ele_run>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_run,2),'999G999G990D99')),' ') || '</Tot_ele_run>';
    p_xml:=p_xml || '<Tot_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_ytd,2),'999G999G990D99')),' ') || '</Tot_ele_ytd>';
     l_tot_org_ele_run := l_tot_org_ele_run + l_tot_ele_run;
	...
	p_xml:= p_xml || '</ELEMENT_RECORD>';
	p_xml:=p_xml || '<Tot_org_ele_run>' || NVL(TRIM(TO_CHAR(ROUND (l_tot_org_ele_run,2),'999G999G990D99')),' ')|| '</Tot_org_ele_run>';
    p_xml:=p_xml || '<Tot_org_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_org_ele_ytd,2),'999G999G990D99')),' ')|| '</Tot_org_ele_ytd>';     l_tot_le_ele_run := l_tot_le_ele_run + l_tot_org_ele_run;
    l_tot_le_ele_ytd := l_tot_le_ele_ytd + l_tot_org_ele_ytd;
    l_tot_org_ele_run := 0;
    l_tot_org_ele_ytd := 0;
	p_xml := p_xml ||'</ORGANIZATION_RECORD>';
...
IF (csr_rec.org_id <> l_org_id AND csr_rec.leg_emp_id = l_leg_emp_id
	AND csr_rec.payroll_id = l_payroll_id) THEN
  IF (csr_pr_le%rowcount <> 1) THEN
    p_xml:=p_xml || '<Tot_ele_run>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_run,2),'999G999G990D99')),' ') || '</Tot_ele_run>';
    p_xml:=p_xml || '<Tot_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_ytd,2),'999G999G990D99')),' ') || '</Tot_ele_ytd>';
    l_tot_org_ele_run := l_tot_org_ele_run + l_tot_ele_run;
...
IF( l_ele_type_id <> csr_rec.ele_type_id
	AND csr_rec.org_id = l_org_id AND csr_rec.leg_emp_id = l_leg_emp_id
	AND csr_rec.payroll_id = l_payroll_id)
THEN
    p_xml:=p_xml || '<Tot_ele_run>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_run,2),'999G999G990D99')),' ') || '</Tot_ele_run>';
    p_xml:=p_xml || '<Tot_ele_ytd>' || NVL(TRIM(TO_CHAR(ROUND(l_tot_ele_ytd,2),'999G999G990D99')),' ') || '</Tot_ele_ytd>';
    l_tot_org_ele_run := l_tot_org_ele_run + l_tot_ele_run;
...
END IF;
	...
	p_xml:=p_xml || '<Organization_Name>' || hr_dk_utility.REPLACE_SPECIAL_CHARS(csr_rec.org_name) || '</Organization_Name>';
...
l_org_id:=csr_rec.org_id;
l_leg_emp_id:=csr_rec.leg_emp_id;
...
IF (l_flag = 1) THEN
	...
	p_xml:=p_xml || '</START>';
ELSIF (l_flag = 0) THEN
p_xml:=p_xml||'<PAYROLL_RECORD><LEGAL_EMPLOYER_RECORD><ORGANIZATION_RECORD><ELEMENT_RECORD><ELEMENT_RECORD_PER_RUN></ELEMENT_RECORD_PER_RUN></ELEMENT_RECORD></ORGANIZATION_RECORD></LEGAL_EMPLOYER_RECORD></PAYROLL_RECORD></START>';
END IF;
