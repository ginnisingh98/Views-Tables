--------------------------------------------------------
--  DDL for Package Body OKC_SCHR_PD_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_SCHR_PD_ASMBLR_PVT" AS
/* $Header: OKCRSPRB.pls 120.0 2005/05/26 09:47:35 appldev noship $ */
...
PROCEDURE acn_assemble(
 ...
 --Select all the attributes for the scheduled planned date reached
 ...
 AND acn.correlation = 'SHR_PDATE_REACH';

 --Select the rule attributes for the resolved time value id
...
BEGIN
 ...
 l_return_status := OKC_API.START_ACTIVITY
  ...
  --The correlation for the scheduled planned date reached event
  l_corrid_rec.corrid := 'SHR_PDATE_REACH' ;
...
IF OKC_K_SIGN_ASMBLR_PVT.isActionEnabled(l_corrid_rec.corrid) = 'Y' THEN
  FOR rule_rec in rul_cur(p_rtve_id => p_rtv_id) LOOP
     FOR k1_rec in chr_csr(p_chr_id   => rule_rec.dnz_chr_id) LOOP
		...
		ELSIF elements_rec.element_name = 'PDATE' THEN
  			l_msg_tbl.extend;
  			l_msg_tbl(k).element_name    := elements_rec.element_name;
			IF elements_rec.format_mask IS NOT NULL THEN
				l_planned_date := to_char(p_planned_date, elements_rec.format_mask);
   			ELSE
				l_planned_date := to_char(p_planned_date, 'DD-MON-YY');
			END IF;
  			l_msg_tbl(k).element_value   := l_planned_date;
    		END IF;
		k := k + 1;
		END LOOP;
