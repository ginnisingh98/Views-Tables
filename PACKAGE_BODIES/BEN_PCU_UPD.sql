--------------------------------------------------------
--  DDL for Package Body BEN_PCU_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCU_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcu_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_per_cm_usg_f',
	   ...
	   p_base_key_value	=> p_rec.per_cm_usg_id);
    --
    ben_pcu_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_per_cm_usg_f Row
    --
    ...
    --
    ben_pcu_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_pcu_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.per_cm_usg_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pcu_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.per_cm_id = hr_api.g_number) then
    p_rec.per_cm_id :=
    ben_pcu_shd.g_old_rec.per_cm_id;
  End If;
  If (p_rec.cm_typ_usg_id = hr_api.g_number) then
    p_rec.cm_typ_usg_id :=
    ben_pcu_shd.g_old_rec.cm_typ_usg_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_pcu_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pcu_attribute_category = hr_api.g_varchar2) then
    p_rec.pcu_attribute_category :=
    ben_pcu_shd.g_old_rec.pcu_attribute_category;
  End If;
  If (p_rec.pcu_attribute1 = hr_api.g_varchar2) then
    p_rec.pcu_attribute1 :=
    ben_pcu_shd.g_old_rec.pcu_attribute1;
  End If;
  If (p_rec.pcu_attribute2 = hr_api.g_varchar2) then
    p_rec.pcu_attribute2 :=
    ben_pcu_shd.g_old_rec.pcu_attribute2;
  End If;
  If (p_rec.pcu_attribute3 = hr_api.g_varchar2) then
    p_rec.pcu_attribute3 :=
    ben_pcu_shd.g_old_rec.pcu_attribute3;
  End If;
  If (p_rec.pcu_attribute4 = hr_api.g_varchar2) then
    p_rec.pcu_attribute4 :=
    ben_pcu_shd.g_old_rec.pcu_attribute4;
  End If;
  If (p_rec.pcu_attribute5 = hr_api.g_varchar2) then
    p_rec.pcu_attribute5 :=
    ben_pcu_shd.g_old_rec.pcu_attribute5;
  End If;
  If (p_rec.pcu_attribute6 = hr_api.g_varchar2) then
    p_rec.pcu_attribute6 :=
    ben_pcu_shd.g_old_rec.pcu_attribute6;
  End If;
  If (p_rec.pcu_attribute7 = hr_api.g_varchar2) then
    p_rec.pcu_attribute7 :=
    ben_pcu_shd.g_old_rec.pcu_attribute7;
  End If;
  If (p_rec.pcu_attribute8 = hr_api.g_varchar2) then
    p_rec.pcu_attribute8 :=
    ben_pcu_shd.g_old_rec.pcu_attribute8;
  End If;
  If (p_rec.pcu_attribute9 = hr_api.g_varchar2) then
    p_rec.pcu_attribute9 :=
    ben_pcu_shd.g_old_rec.pcu_attribute9;
  End If;
  If (p_rec.pcu_attribute10 = hr_api.g_varchar2) then
    p_rec.pcu_attribute10 :=
    ben_pcu_shd.g_old_rec.pcu_attribute10;
  End If;
  If (p_rec.pcu_attribute11 = hr_api.g_varchar2) then
    p_rec.pcu_attribute11 :=
    ben_pcu_shd.g_old_rec.pcu_attribute11;
  End If;
  If (p_rec.pcu_attribute12 = hr_api.g_varchar2) then
    p_rec.pcu_attribute12 :=
    ben_pcu_shd.g_old_rec.pcu_attribute12;
  End If;
  If (p_rec.pcu_attribute13 = hr_api.g_varchar2) then
    p_rec.pcu_attribute13 :=
    ben_pcu_shd.g_old_rec.pcu_attribute13;
  End If;
  If (p_rec.pcu_attribute14 = hr_api.g_varchar2) then
    p_rec.pcu_attribute14 :=
    ben_pcu_shd.g_old_rec.pcu_attribute14;
  End If;
  If (p_rec.pcu_attribute15 = hr_api.g_varchar2) then
    p_rec.pcu_attribute15 :=
    ben_pcu_shd.g_old_rec.pcu_attribute15;
  End If;
  If (p_rec.pcu_attribute16 = hr_api.g_varchar2) then
    p_rec.pcu_attribute16 :=
    ben_pcu_shd.g_old_rec.pcu_attribute16;
  End If;
  If (p_rec.pcu_attribute17 = hr_api.g_varchar2) then
    p_rec.pcu_attribute17 :=
    ben_pcu_shd.g_old_rec.pcu_attribute17;
  End If;
  If (p_rec.pcu_attribute18 = hr_api.g_varchar2) then
    p_rec.pcu_attribute18 :=
    ben_pcu_shd.g_old_rec.pcu_attribute18;
  End If;
  If (p_rec.pcu_attribute19 = hr_api.g_varchar2) then
    p_rec.pcu_attribute19 :=
    ben_pcu_shd.g_old_rec.pcu_attribute19;
  End If;
  If (p_rec.pcu_attribute20 = hr_api.g_varchar2) then
    p_rec.pcu_attribute20 :=
    ben_pcu_shd.g_old_rec.pcu_attribute20;
  End If;
  If (p_rec.pcu_attribute21 = hr_api.g_varchar2) then
    p_rec.pcu_attribute21 :=
    ben_pcu_shd.g_old_rec.pcu_attribute21;
  End If;
  If (p_rec.pcu_attribute22 = hr_api.g_varchar2) then
    p_rec.pcu_attribute22 :=
    ben_pcu_shd.g_old_rec.pcu_attribute22;
  End If;
  If (p_rec.pcu_attribute23 = hr_api.g_varchar2) then
    p_rec.pcu_attribute23 :=
    ben_pcu_shd.g_old_rec.pcu_attribute23;
  End If;
  If (p_rec.pcu_attribute24 = hr_api.g_varchar2) then
    p_rec.pcu_attribute24 :=
    ben_pcu_shd.g_old_rec.pcu_attribute24;
  End If;
  If (p_rec.pcu_attribute25 = hr_api.g_varchar2) then
    p_rec.pcu_attribute25 :=
    ben_pcu_shd.g_old_rec.pcu_attribute25;
  End If;
  If (p_rec.pcu_attribute26 = hr_api.g_varchar2) then
    p_rec.pcu_attribute26 :=
    ben_pcu_shd.g_old_rec.pcu_attribute26;
  End If;
  If (p_rec.pcu_attribute27 = hr_api.g_varchar2) then
    p_rec.pcu_attribute27 :=
    ben_pcu_shd.g_old_rec.pcu_attribute27;
  End If;
  If (p_rec.pcu_attribute28 = hr_api.g_varchar2) then
    p_rec.pcu_attribute28 :=
    ben_pcu_shd.g_old_rec.pcu_attribute28;
  End If;
  If (p_rec.pcu_attribute29 = hr_api.g_varchar2) then
    p_rec.pcu_attribute29 :=
    ben_pcu_shd.g_old_rec.pcu_attribute29;
  End If;
  If (p_rec.pcu_attribute30 = hr_api.g_varchar2) then
    p_rec.pcu_attribute30 :=
    ben_pcu_shd.g_old_rec.pcu_attribute30;
  End If;
   If (p_rec.request_id = hr_api.g_number) then
    p_rec.request_id :=
    ben_pcu_shd.g_old_rec.request_id;
  End If;
  If (p_rec.program_application_id = hr_api.g_number) then
    p_rec.program_application_id :=
    ben_pcu_shd.g_old_rec.program_application_id;
  End If;
  If (p_rec.program_id = hr_api.g_number) then
    p_rec.program_id :=
    ben_pcu_shd.g_old_rec.program_id;
  End If;
