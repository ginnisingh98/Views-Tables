--------------------------------------------------------
--  DDL for Package Body PAY_ELEMENT_TYPES_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ELEMENT_TYPES_BE1" as 
...
p_comment_id                   number,
p_processing_priority_warning  boolean) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  pay_element_types_be1.create_element_type_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.pay.api.element_types.create_element_type';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<classification_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_classification_id);
    l_text:=l_text||'</classification_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<element_name>';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<input_currency_code>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_input_currency_code);
    l_text:=l_text||'</input_currency_code>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<output_currency_code>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_output_currency_code);
    l_text:=l_text||'</output_currency_code>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<processing_type>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_processing_type);
    l_text:=l_text||'</processing_type>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<legislation_subgroup>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_legislation_subgroup);
    l_text:=l_text||'</legislation_subgroup>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<default_uom>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_default_uom);
    l_text:=l_text||'</default_uom>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
