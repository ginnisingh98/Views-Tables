--------------------------------------------------------
--  DDL for Package Body PAY_PUT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PUT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_put_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  -- Update the pay_user_tables Row
  --
...
Begin
  ...
  begin
    ...
    pay_put_rku.after_update
      ...
      ,p_range_or_match_o
      => pay_put_shd.g_old_rec.range_or_match
      ,p_user_key_units_o
      => pay_put_shd.g_old_rec.user_key_units
      ,p_user_table_name
      => p_rec.user_table_name
      ,p_user_table_name_o
      => pay_put_shd.g_old_rec.user_table_name
      ,p_user_row_title
      => p_rec.user_row_title
      ,p_user_row_title_o
      => pay_put_shd.g_old_rec.user_row_title
...
Begin
  ...
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    ...
    pay_put_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.range_or_match = hr_api.g_varchar2) then
    p_rec.range_or_match :=
    pay_put_shd.g_old_rec.range_or_match;
  End If;
  If (p_rec.user_key_units = hr_api.g_varchar2) then
    p_rec.user_key_units :=
    pay_put_shd.g_old_rec.user_key_units;
  End If;
  If (p_rec.user_table_name = hr_api.g_varchar2) then
    p_rec.user_table_name :=
    pay_put_shd.g_old_rec.user_table_name;
  End If;
  If (p_rec.user_row_title = hr_api.g_varchar2) then
    p_rec.user_row_title :=
    pay_put_shd.g_old_rec.user_row_title;
  End If;
  --
...
Begin
  ...
  --
  pay_put_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pay_put_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pay_put_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  -- business process.
  --
  pay_put_upd.upd(p_effective_date, l_rec);
