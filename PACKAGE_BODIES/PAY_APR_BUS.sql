--------------------------------------------------------
--  DDL for Package Body PAY_APR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_APR_BUS" as
...
g_process_id                  number         default null;
--
--  ---------------------------------------------------------------------------
  ...
  cursor csr_sec_grp is
    select pbg.security_group_id,
      ...
      from per_business_groups_perf pbg
         , pay_au_processes apr
     where apr.process_id = p_process_id
       and pbg.business_group_id (+) = apr.business_group_id;
  --
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'PROCESS_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(pay_apr_bus.g_process_id, hr_api.g_number)
       = p_process_id) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pay_apr_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_apr_bus.g_process_id                  := p_process_id;
    pay_apr_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_apr_shd.api_updating
      (p_process_id                        => p_rec.process_id
...
Begin
  ...
  -- Only proceed with the validation if a row exists for the current
  -- record in the HR Schema.
  -- To check if the enabled flag is 'Y' or 'N'
  if not (p_enabled_flag = 'Y' or p_enabled_flag = 'N') then
...
Begin
  ...
  chk_enabled_flag(p_rec.enabled_flag);
  --
  --
...
--
begin
  g_package  := '  pay_apr_bus.';  -- Global package name
end pay_apr_bus;
