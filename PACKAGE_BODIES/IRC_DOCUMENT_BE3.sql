--------------------------------------------------------
--  DDL for Package Body IRC_DOCUMENT_BE3
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_DOCUMENT_BE3" as 
...
p_purge                        varchar2) is
  ...
  --
  l_proc varchar2(72):='  irc_document_be3.delete_document_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.irc.api.document.delete_document';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<purge>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_purge);
    l_text:=l_text||'</purge>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</document>';
    --
