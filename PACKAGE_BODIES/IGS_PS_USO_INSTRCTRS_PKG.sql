--------------------------------------------------------
--  DDL for Package Body IGS_PS_USO_INSTRCTRS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_USO_INSTRCTRS_PKG" AS
/* $Header: IGSPI2PB.pls 115.4 2003/03/21 08:03:15 sarakshi ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_uso_instrctrs%ROWTYPE;
  new_references igs_ps_uso_instrctrs%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.uso_instructor_id                 := x_uso_instructor_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_uso_instrctrs
      WHERE    uso_instructor_id = x_uso_instructor_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      ...
      AND      instructor_id = x_instructor_id
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_USO_INS_FK1');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_HZP_INS_FK2');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.uso_instructor_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_uso_instrctrs_s.NEXTVAL
    INTO      x_uso_instructor_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_uso_instructor_id                 => x_uso_instructor_id,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_uso_instrctrs
    WHERE rowid = x_rowid;
