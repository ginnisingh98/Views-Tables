--------------------------------------------------------
--  DDL for Package Body BEN_ESW_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ESW_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ben_esw_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the ben_elig_scre_wtg_f Row
    --
    update  ben_elig_scre_wtg_f
    set
     elig_scre_wtg_id                     = p_rec.elig_scre_wtg_id
    ...
    ,crit_tab_short_name                  = p_rec.crit_tab_short_name
    ,crit_tab_pk_id                       = p_rec.crit_tab_pk_id
    ,computed_score                       = p_rec.computed_score
...
Begin
  ...
  begin
    ...
    ben_esw_rku.after_update
      ...
      => p_rec.crit_tab_short_name
      ,p_crit_tab_pk_id
      => p_rec.crit_tab_pk_id
      ,p_computed_score
      => p_rec.computed_score
      ,p_benefit_action_id
...
Begin
  ...
  If (p_rec.elig_per_opt_id = hr_api.g_number) then
    ...
    ben_esw_shd.g_old_rec.elig_per_opt_id;
  End If;
  If (p_rec.elig_rslt_id = hr_api.g_number) then
    p_rec.elig_rslt_id :=
    ben_esw_shd.g_old_rec.elig_rslt_id;
  End If;
  ...
  If (p_rec.eligy_prfl_id = hr_api.g_number) then
    ...
    ben_esw_shd.g_old_rec.eligy_prfl_id;
  End If;
  If (p_rec.crit_tab_short_name = hr_api.g_varchar2) then
    p_rec.crit_tab_short_name :=
    ben_esw_shd.g_old_rec.crit_tab_short_name;
  End If;
  If (p_rec.crit_tab_pk_id = hr_api.g_number) then
    p_rec.crit_tab_pk_id :=
    ben_esw_shd.g_old_rec.crit_tab_pk_id;
  End If;
  If (p_rec.computed_score = hr_api.g_number) then
    p_rec.computed_score :=
    ben_esw_shd.g_old_rec.computed_score;
  End If;
...
Begin
  ...
  --
  ben_esw_upd.convert_defs(p_rec);
  --
  ben_esw_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
Procedure upd
  ...
  ,p_elig_rslt_id                 in     number    default hr_api.g_number
  ,p_per_in_ler_id                in     number    default hr_api.g_number
  ,p_eligy_prfl_id                 in     number    default hr_api.g_number
  ,p_crit_tab_short_name          in     varchar2  default hr_api.g_varchar2
  ,p_crit_tab_pk_id               in     number    default hr_api.g_number
