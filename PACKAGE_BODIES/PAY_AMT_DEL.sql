--------------------------------------------------------
--  DDL for Package Body PAY_AMT_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_AMT_DEL" as
...
Begin
  ...
  --
  -- Delete the pay_au_module_types row.
  --
  delete from pay_au_module_types
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_delete(p_rec in pay_amt_shd.g_rec_type) is
--
  l_proc  varchar2(72);
--
Begin
...
-- {End Of Comments}
-- -----------------------------------------------------------------------------
Procedure post_delete(p_rec in pay_amt_shd.g_rec_type) is
--
  l_proc  varchar2(72);
--
Begin
...
Begin
  ...
  --
  pay_amt_bus.delete_validate(p_rec);
  --
  ...
  --
  pay_amt_del.pre_delete(p_rec);
  --
  -- Delete the row.
  --
  pay_amt_del.delete_dml(p_rec);
  --
  -- Call the supporting post-delete operation
  --
  pay_amt_del.post_delete(p_rec);
  --
  -- Call to raise any errors on multi-message list
  hr_multi_message.end_validation_set;
  --
...
Begin
  ...
  -- business process
  --
  pay_amt_del.del(l_rec);
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End del;
--
begin
  g_package  := '  pay_amt_del.';  -- Global package name
end pay_amt_del;
