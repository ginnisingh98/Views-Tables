--------------------------------------------------------
--  DDL for Package Body WIP_WDJ_DIAG_CORRUPT_OSP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WIP_WDJ_DIAG_CORRUPT_OSP" as
/* $Header: WIPDJ02B.pls 120.1 2008/05/01 04:06:03 shjindal noship $ */
PROCEDURE init is
BEGIN
null;
END init;
...
WIP_DIAG_DISCRETE_LOT.corrupt_osp_txn_wdj(l_org_id, report, reportClob);
...
BEGIN
name := 'Invalid OSP txns';
END getComponentName;

PROCEDURE getTestDesc(descStr OUT NOCOPY VARCHAR2) IS
BEGIN
descStr :='This health check diagnostic identifies potential data inconsistency in OSP transactions.<BR>'||
          'Run this diagnostic when OSP Delivery does not charge OSP resource or Quantity on Operations '||
          'are not in sync with move transactions. <BR>'||'Please see the diagnostic output for recommended'||
          ' actions. <BR>'||'It is recommended to run this health check periodically.';
