--------------------------------------------------------
--  DDL for Package Body OTA_CERT_ENROLLMENT_BE2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_CERT_ENROLLMENT_BE2" as 
...
p_enrollment_date              date) is
  ...
  --
  l_proc varchar2(72):='  ota_cert_enrollment_be2.update_cert_enrollment_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.ota.api.cert_enrollment.update_cert_enrollment';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<cert_enrollment_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_cert_enrollment_id);
    l_text:=l_text||'</cert_enrollment_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<certification_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_certification_id);
    l_text:=l_text||'</certification_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<person_id>';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<certification_status_code>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_certification_status_code);
    l_text:=l_text||'</certification_status_code>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<completion_date>';
    l_text:=l_text||fnd_date.date_to_canonical(p_completion_date);
    l_text:=l_text||'</completion_date>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<unenrollment_date>';
    l_text:=l_text||fnd_date.date_to_canonical(p_unenrollment_date);
    l_text:=l_text||'</unenrollment_date>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<expiration_date>';
    l_text:=l_text||fnd_date.date_to_canonical(p_expiration_date);
    l_text:=l_text||'</expiration_date>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<earliest_enroll_date>';
    l_text:=l_text||fnd_date.date_to_canonical(p_earliest_enroll_date);
    l_text:=l_text||'</earliest_enroll_date>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<enrollment_date>';
    l_text:=l_text||fnd_date.date_to_canonical(p_enrollment_date);
    l_text:=l_text||'</enrollment_date>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</cert_enrollment>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
