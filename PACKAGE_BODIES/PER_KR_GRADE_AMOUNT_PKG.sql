--------------------------------------------------------
--  DDL for Package Body PER_KR_GRADE_AMOUNT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_KR_GRADE_AMOUNT_PKG" AS
/* $Header: pekrsg02.pkb 115.1 2002/12/03 09:36:23 viagarwa noship $ */
-------------------------------------------------------------------------------------
...
,p_attribute30            VARCHAR2
  ...
  --
  CURSOR c_s1 IS
    SELECT per_kr_grade_amount_s.NEXTVAL
    FROM   dual;
  --
BEGIN
  --
  OPEN c_s1;
  FETCH c_s1 INTO p_grade_amount_id;
  ...
  CLOSE c_s1;
  --
  INSERT INTO per_kr_grade_amount_f
    (grade_amount_id
...
,p_attribute30           VARCHAR2
  ...
  CURSOR c1 IS
    SELECT grade_amount_id
          ...
          ,attribute29
          ,attribute30
    FROM per_kr_grade_amount
    WHERE rowid = chartorowid(p_row_id)
    FOR UPDATE OF grade_amount_id NOWAIT;
  --
  recinfo    c1%ROWTYPE;
  --
BEGIN
  ...
  CLOSE c1;
  --
  IF (     (recinfo.grade_amount_id      = p_grade_amount_id)
     ...
     AND   (  (recinfo.grade_amount = p_grade_amount)
           OR (   (recinfo.grade_amount IS NULL)
              AND (p_grade_amount IS NULL)))
...
BEGIN
  --
  DELETE FROM per_kr_grade_amount_f
  WHERE rowid = chartorowid(p_row_id);
  --
  IF (SQL%NOTFOUND) THEN
    RAISE NO_DATA_FOUND;
