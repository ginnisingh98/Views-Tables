--------------------------------------------------------
--  DDL for Package Body OTA_LP_MEMBER_ENROLLMENT_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LP_MEMBER_ENROLLMENT_SWI" As
/* $Header: otlmeswi.pkb 120.0.12010000.2 2009/05/14 07:31:42 pekasi ship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'ota_lp_member_enrollment_swi.';
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_lp_member_enr_swi;
  --
  ...
  --
  ota_lme_ins.set_base_key_value
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_lp_member_enr_swi;
    --
...
PROCEDURE update_lp_member_enrollment
  ...
  --
  -- Other variables
  l_proc    varchar2(72) := g_package ||'update_lp_member_enrollment';
Begin
  hr_utility.set_location(' Entering:' || l_proc,10);
  --
  -- Issue a savepoint
  --
  savepoint update_lp_member_enr_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_lp_member_enr_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_lp_member_enr_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  ota_lp_member_enrollment_api.delete_lp_member_enrollment
    (p_lp_member_enrollment_id      => p_lp_member_enrollment_id
    ,p_object_version_number        => p_object_version_number
    ,p_validate                     => l_validate
    );
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_lp_member_enr_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_return_status := hr_multi_message.get_return_status_disable;
