--------------------------------------------------------
--  DDL for Package Body PAY_SID_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_SID_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_sid_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the pay_ie_prsi_details_f Row
    --
    update  pay_ie_prsi_details_f
    set
     prsi_details_id                      = p_rec.prsi_details_id
    ,object_version_number                = p_rec.object_version_number
    ,assignment_id                        = p_rec.assignment_id
    ,contribution_class                   = p_rec.contribution_class
    ,overridden_subclass                  = p_rec.overridden_subclass
    ,soc_ben_flag                         = p_rec.soc_ben_flag
    ,soc_ben_start_date                   = p_rec.soc_ben_start_date
    ,overridden_ins_weeks                 = p_rec.overridden_ins_weeks
    ,non_standard_ins_weeks               = p_rec.non_standard_ins_weeks
    ,exemption_start_date                 = p_rec.exemption_start_date
    ,exemption_end_date                   = p_rec.exemption_end_date
    ,cert_issued_by                       = p_rec.cert_issued_by
    ,director_flag                        = p_rec.director_flag
    ,request_id                           = p_rec.request_id
    ,program_application_id               = p_rec.program_application_id
    ,program_id                           = p_rec.program_id
    ,program_update_date                  = p_rec.program_update_date
    ,community_flag                       = p_rec.community_flag
...
Begin
  ...
  begin
    ...
    pay_sid_rku.after_update
      ...
      => p_rec.contribution_class
      ,p_overridden_subclass
      => p_rec.overridden_subclass
      ,p_soc_ben_flag
      => p_rec.soc_ben_flag
      ,p_soc_ben_start_date
      => p_rec.soc_ben_start_date
      ,p_overridden_ins_weeks
      => p_rec.overridden_ins_weeks
      ,p_non_standard_ins_weeks
      => p_rec.non_standard_ins_weeks
      ,p_exemption_start_date
      => p_rec.exemption_start_date
      ,p_exemption_end_date
      => p_rec.exemption_end_date
      ,p_cert_issued_by
      => p_rec.cert_issued_by
      ,p_director_flag
      => p_rec.director_flag
      ,p_request_id
      => p_rec.request_id
...
Begin
  ...
  If (p_rec.assignment_id = hr_api.g_number) then
    ...
    pay_sid_shd.g_old_rec.assignment_id;
  End If;
  If (p_rec.contribution_class = hr_api.g_varchar2) then
    p_rec.contribution_class :=
    pay_sid_shd.g_old_rec.contribution_class;
  End If;
  If (p_rec.overridden_subclass = hr_api.g_varchar2) then
    p_rec.overridden_subclass :=
    pay_sid_shd.g_old_rec.overridden_subclass;
  End If;
  If (p_rec.soc_ben_flag = hr_api.g_varchar2) then
    p_rec.soc_ben_flag :=
    pay_sid_shd.g_old_rec.soc_ben_flag;
  End If;
  If (p_rec.soc_ben_start_date = hr_api.g_date) then
    p_rec.soc_ben_start_date :=
    pay_sid_shd.g_old_rec.soc_ben_start_date;
  End If;
  If (p_rec.overridden_ins_weeks = hr_api.g_number) then
    p_rec.overridden_ins_weeks :=
    pay_sid_shd.g_old_rec.overridden_ins_weeks;
  End If;
  If (p_rec.non_standard_ins_weeks = hr_api.g_number) then
    p_rec.non_standard_ins_weeks :=
    pay_sid_shd.g_old_rec.non_standard_ins_weeks;
  End If;
  If (p_rec.exemption_start_date = hr_api.g_date) then
    p_rec.exemption_start_date :=
    pay_sid_shd.g_old_rec.exemption_start_date;
  End If;
  If (p_rec.exemption_end_date = hr_api.g_date) then
    p_rec.exemption_end_date :=
    pay_sid_shd.g_old_rec.exemption_end_date;
  End If;
  If (p_rec.cert_issued_by = hr_api.g_varchar2) then
    p_rec.cert_issued_by :=
    pay_sid_shd.g_old_rec.cert_issued_by;
  End If;
  If (p_rec.director_flag = hr_api.g_varchar2) then
    p_rec.director_flag :=
    pay_sid_shd.g_old_rec.director_flag;
  End If;
  ...
  If (p_rec.program_update_date = hr_api.g_date) then
    ...
    pay_sid_shd.g_old_rec.program_update_date;
  End If;
  If (p_rec.community_flag = hr_api.g_varchar2) then
    p_rec.community_flag :=
    pay_sid_shd.g_old_rec.community_flag;
  End If;
  --
...
Begin
  ...
  --
  pay_sid_upd.convert_defs(p_rec);
  --
  pay_sid_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
