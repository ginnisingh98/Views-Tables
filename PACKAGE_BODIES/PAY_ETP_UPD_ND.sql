--------------------------------------------------------
--  DDL for Package Body PAY_ETP_UPD_ND
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ETP_UPD_ND" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_etp_upd_nd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Exception
  When hr_api.check_integrity_violated Then
    ...
    pay_etp_shd_nd.g_api_dml := false;   -- Unset the api dml status
    pay_etp_shd_nd.constraint_error
...
Begin
  ...
  ElsIf (p_rec.comment_id is not null and p_rec.comments <>
        pay_etp_shd_nd.g_old_rec.comments) then
...
Begin
  ...
  --
  pay_etp_upd_nd.convert_defs(p_rec);
  --
  pay_etp_bus_nd.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
