--------------------------------------------------------
--  DDL for Package Body PAY_ELEMENT_TYPES_BE3
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ELEMENT_TYPES_BE3" as 
...
p_processing_rules_warning     boolean) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  pay_element_types_be3.delete_element_type_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.pay.api.element_types.delete_element_type';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<balance_feeds_warning>';
if(P_BALANCE_FEEDS_WARNING) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</balance_feeds_warning>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<processing_rules_warning>';
if(P_PROCESSING_RULES_WARNING) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</processing_rules_warning>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</element_types>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
