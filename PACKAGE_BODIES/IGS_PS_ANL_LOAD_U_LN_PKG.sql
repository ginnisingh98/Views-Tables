--------------------------------------------------------
--  DDL for Package Body IGS_PS_ANL_LOAD_U_LN_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_ANL_LOAD_U_LN_PKG" AS
  /* $Header: IGSPI05B.pls 115.3 2002/11/29 01:53:31 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_ANL_LOAD_U_LN%RowType;
  new_references IGS_PS_ANL_LOAD_U_LN%RowType;
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_calul_br_iud
  -- BEFORE INSERT OR DELETE OR UPDATE
  -- ON IGS_PS_ANL_LOAD_U_LN
  -- FOR EACH ROW
  ...
  PROCEDURE BeforeRowInsertUpdateDelete1(
	...
	v_course_cd			IGS_PS_ANL_LOAD_U_LN.course_cd%TYPE;
	v_crv_version_number	IGS_PS_ANL_LOAD_U_LN.crv_version_number%TYPE;
  BEGIN

	-- Set variables.
	IF p_deleting THEN
		v_course_cd := old_references.course_cd;
		v_crv_version_number := old_references.crv_version_number;
	ELSE -- p_inserting or p_updating
		v_course_cd := new_references.course_cd;
		v_crv_version_number := new_references.crv_version_number;
	END IF;
	...
	IF  IGS_PS_VAL_CRS.crsp_val_iud_crv_dtl (
		...
		App_Exception.Raise_Exception;
	END IF;
	-- Validate the IGS_PS_UNIT version.  IGS_PS_UNIT version is not updateable.
	IF p_inserting THEN
...
BEGIN
  ...
  BEGIN
    ...
    IF (((old_references.course_cd = new_references.course_cd) AND
         ...
         (old_references.yr_num = new_references.yr_num) AND
         (old_references.effective_start_dt = new_references.effective_start_dt)) OR
        ((new_references.course_cd IS NULL) OR
         (new_references.crv_version_number IS NULL) OR
         (new_references.yr_num IS NULL) OR
         (new_references.effective_start_dt IS NULL))) THEN
      NULL;
    ELSE
      IF NOT IGS_PS_ANL_LOAD_PKG.Get_PK_For_Validation (
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CALUL_CAL_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
       Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CALUL_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
	...
	IF Get_PK_For_Validation (
      ...
      new_references.effective_start_dt ,
      new_references.unit_cd ,
      new_references.uv_version_number) THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_ANL_LOAD_U_LN
      where COURSE_CD = X_COURSE_CD
      and CRV_VERSION_NUMBER = X_CRV_VERSION_NUMBER
      and EFFECTIVE_START_DT = X_EFFECTIVE_START_DT
      and YR_NUM = X_YR_NUM
...
Before_DML (
  ...
  insert into IGS_PS_ANL_LOAD_U_LN (
    COURSE_CD,
    CRV_VERSION_NUMBER,
    YR_NUM,
    EFFECTIVE_START_DT,
...
Before_DML (
  ...
  delete from IGS_PS_ANL_LOAD_U_LN
  where ROWID = X_ROWID;
