--------------------------------------------------------
--  DDL for Package Body BIM_EDW_LEADS_F_SIZE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIM_EDW_LEADS_F_SIZE" AS
/* $Header: bimszflb.pls 115.0 2001/03/14 12:02:30 pkm ship       $*/
...
CURSOR c_cnt_rows IS
   ...
   from (
	...
	FROM AS_SALES_LEAD_LINES ASLL,
	 ...
	 BIM_EDW_LEADS_INC INC,
	 EDW_LOCAL_INSTANCE INST
        WHERE ASL.SALES_LEAD_ID = ASLL.SALES_LEAD_ID
         AND ASLL.SALES_LEAD_LINE_ID = INC.PRIMARY_KEY
         AND NVL(ASLL.SOURCE_PROMOTION_ID,
	 -999) = BSCD.SOURCE_CODE_ID AND ( ( ASL.LAST_UPDATE_DATE
	> TO_DATE( '1000/01/01',
	 'YYYY/MM/DD' ) ) OR ( ASLL.LAST_UPDATE_DATE > TO_DATE(
	'1000/01/01',
	 'YYYY/MM/DD' ) ) )
	and
...
PROCEDURE est_row_len(p_from_date DATE,
 ...
 x_INSTANCE NUMBER;
  ...
  CURSOR c_1 IS
	SELECT
	avg(nvl(vsize( SALES_LEAD_LINE_ID ), 0))
  ...
  BEGIN
	...
	4*ceil(x_CUSTOMER_ID + 1) +
	2*ceil(x_CURRENCY_CODE + 1) +
    ...
    x_total := x_total + 15*ceil(x_INSTANCE + 1);
