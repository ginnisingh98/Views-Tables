--------------------------------------------------------
--  DDL for Package Body AP_ACCOUNTING_EVENTS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_ACCOUNTING_EVENTS_PKG" AS
/* $Header: apeventb.pls 120.54.12010000.40 2010/06/07 23:50:33 gagrawal ship $ */
...
-----------------------------------------------------------------------------
  ...
  FROM   (
select accounting_date , accounting_event_id , invoice_line_number,
invoice_id , amount , line_type_lookup_code , parent_reversal_id ,
set_of_books_id , org_id , prepay_distribution_id ,
charge_applicable_to_dist_id , cancellation_flag ,
match_status_flag , awt_invoice_payment_id
...
BEGIN
  ...
  IF (p_event_type IN (PAYMENT_CANCELLED_TYPE, REFUND_CANCELLED_TYPE)
 		                AND l_when_to_acct = 'CLEARING ONLY') THEN  --bug 8771563
  ...
  LOOP
    ...
    IF ((P_event_type = MANUAL_PAYMENT_ADJUSTED_TYPE AND
        ...
        (P_event_type = UPGRADED_MAN_PAY_ADJ_TYPE AND
         l_event_info.event_type_code = UPGRADED_MAN_PAY_ADJ_TYPE)) THEN
...
IS
  ...
  CURSOR no_action_prepay_dists IS
  SELECT accounting_event_id
  FROM   ap_invoice_distributions
  WHERE  invoice_id = p_invoice_id
  AND    line_type_lookup_code = 'PREPAY'
  ...
  -- bug 4748638
  CURSOR non_preapy_dist_grp_count IS
  SELECT accounting_date
  FROM  ap_invoice_distributions
  WHERE invoice_id = p_invoice_id
  AND   line_type_lookup_code <> 'PREPAY'
  ...
  -- bug fix for 4881719
  CURSOR all_unaccounted_dists_cur IS
  SELECT accounting_event_id
  FROM  ap_invoice_distributions
  WHERE invoice_id = p_invoice_id
    AND posted_flag <> 'Y';
...
EXCEPTION
  WHEN OTHERS THEN
    IF (inv_cancel_event_dists%ISOPEN) THEN
      CLOSE inv_cancel_event_dists;
    END IF;
    IF (no_action_prepay_dists%ISOPEN) THEN
      CLOSE no_action_prepay_dists;
    END IF;
    IF (non_preapy_dist_grp_count%ISOPEN) THEN
      CLOSE non_preapy_dist_grp_count;
    END IF;
    IF (all_unaccounted_dists_cur%ISOPEN) THEN
      CLOSE all_unaccounted_dists_cur;
    END IF;
    IF (SQLCODE <> -20001) THEN
       FND_MESSAGE.SET_NAME('SQLAP','AP_DEBUG');
       FND_MESSAGE.SET_TOKEN('ERROR',SQLERRM);
