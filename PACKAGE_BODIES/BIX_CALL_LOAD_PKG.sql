--------------------------------------------------------
--  DDL for Package Body BIX_CALL_LOAD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_CALL_LOAD_PKG" AS
/*$Header: bixcaloa.plb 120.5 2006/08/08 11:37:33 pubalasu noship $ */
...
l_count := 0;
  ...
  --IF (g_debug_flag = 'Y') THEN
--write_log ('Entered check_if_processed');
  --END IF;
...
/* This has been added for marketing mv */
l_start_date_time_pr Date;
l_start_date_time_julian_pr integer;
...
--write_log('Completed CALL level TALK time at '||
 ...
 ELSE      -- for if which checks if this is a new media_id
   ...
   THEN
      ...
      --
	 /****Change on 09/04 - consider all the queue segments
	  not just the first queue segment.
...
--insert into bixtest values (counter|| 'media  ' || l_source_record.media_id(i));
--insert into bixtest values (counter|| 'resource  ' || l_source_record.resource_id(i));
--insert into bixtest values (counter|| 'SEG START ' || l_segment_start);
--insert into bixtest values (counter|| 'SEG END ' || l_segment_end);
--insert into bixtest values (counter|| 'SECS is   ' || l_secs);
--commit;
...
then -- item added/created
   ...
   then  -- Opportunity
      l_target_record.agent_opportunities_created(counter) := l_target_record.agent_opportunities_created(counter)+ 1;
...
/****************** Not needed as we do this in ROLLUP itslef *****************
LOOP
...
--
  ...
  truncate_table('BIX_CALL_DETAILS_STG');
  truncate_table('BIX_CALL_DETAILS_F');


  cleanup_oltp; --001 close open medias and other processings before getting
                --the end collection date
...
g_collect_start_date := to_date(fnd_profile.value('BIS_GLOBAL_START_DATE'),
						  'MM/DD/YYYY');
