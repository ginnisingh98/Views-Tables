--------------------------------------------------------
--  DDL for Package Body CSE_IN_SERVICE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSE_IN_SERVICE_PKG" AS
-- $Header: CSEINSVB.pls 120.0.12010000.2 2008/09/20 00:15:55 fli ship $
...
PROCEDURE Decode_Message(
   ...
   l_Api_Name   CONSTANT  VARCHAR2(30) := 'CSE_IN_SERVICE_PKG';
   l_Item_Id                NUMBER;
...
BEGIN
   ...
   X_In_Service_Attr_Rec.Item_Id               := l_Item_Id;
   X_In_Service_Attr_Rec.Revision              := l_Revision;
   X_In_Service_Attr_Rec.Lot_Number            := l_Lot_Number;
   X_In_Service_Attr_Rec.Serial_Number         := l_Serial_Number;
   X_In_Service_Attr_Rec.Quantity              := l_Quantity;
   X_In_Service_Attr_Rec.Network_Location_Id   := l_Netwk_Locn_Id;
   X_In_Service_Attr_Rec.Party_Site_Id         := l_Party_Site_Id;
   X_In_Service_Attr_Rec.Work_Order_Number     := l_Work_Order_Number;
   X_In_Service_Attr_Rec.Transaction_Date      := TO_DATE(l_Transaction_Date_Str, 'YYYY/MM/DD HH24:MI:SS');
   X_In_Service_Attr_Rec.Transacted_By         := l_Transacted_By;
   X_In_Service_Attr_Rec.Message_id            := P_Msg_Header.Message_Id;
...
BEGIN
 ...
 IF (l_debug = 'Y') THEN
    CSE_DEBUG_PUB.ADD('*****  IN SERVICE TRANSACTION ******');
    CSE_DEBUG_PUB.ADD('-----------------------------------------------');
 END IF;
...
END IF;
  ...
  -- l_Network_Location_Code := P_In_Service_Attr_Rec.Network_Location_Code;
    If P_In_Service_Attr_Rec.Network_Location_id is not null then
      l_hz_location_id    :=P_In_Service_Attr_Rec.Network_Location_id ;
      l_location_type_code:='HZ_LOCATIONS';
    elsif  P_In_Service_Attr_Rec.party_site_id is not null  then
      l_hz_location_id :=P_In_Service_Attr_Rec.party_site_id ;
...
--   X_Error_Message          => l_Error_Message);
 ...
 --  END IF;
   ...
   l_Instance_Query_Rec.Inventory_Item_Id := P_In_Service_Attr_Rec.Item_Id;
   l_Instance_Query_Rec.Inventory_Revision:= P_In_Service_Attr_Rec.Revision;
   l_Instance_Query_Rec.Lot_Number        := P_In_Service_Attr_Rec.Lot_Number;
   l_Instance_Query_Rec.Serial_Number := P_In_Service_Attr_Rec.Serial_Number;
  ...
  IF(l_Instance_Header_Tbl_Out.COUNT=0) THEN
   IF (l_debug = 'Y') THEN
      CSE_DEBUG_PUB.ADD('source instance not found');
   END IF;
      fnd_message.set_name('CSE','CSE_SRC_RECORD_NOTFOUND');
      fnd_message.set_token('ITEM',P_In_Service_Attr_Rec.Item_Id);
   ...
   IF (l_debug = 'Y') THEN
      CSE_DEBUG_PUB.ADD('multiple source instances found');
   END IF;
   ...
   IF(P_In_Service_Attr_Rec.Quantity >
      l_Instance_Header_Tbl_Out(i).Quantity)  THEN
   IF (l_debug = 'Y') THEN
      CSE_DEBUG_PUB.ADD('not enough source quantity');
   END IF;
   ...
   --Step 4,If the Item is serialized,
    ...
    IF(P_In_Service_Attr_Rec.Serial_Number IS NOT NULL) THEN
       ...
       l_Instance_Rec.Instance_Usage_Code:= CSE_DATASTRUCTURES_PUB.G_IN_SERVICE;
       ...
       l_Txn_Rec.Source_Transaction_Date:= p_In_Service_Attr_Rec.Transaction_Date;
       l_Txn_Rec.Transaction_Type_Id := l_transaction_type_id;
       l_Txn_Rec.Source_Group_Ref := P_In_Service_Attr_Rec.Work_Order_Number;
       l_Txn_Rec.Transaction_Quantity := P_In_Service_Attr_Rec.Quantity;
       l_Txn_Rec.Transacted_By := P_In_Service_Attr_Rec.Transacted_By;
       l_Txn_Rec.Transaction_Status_Code:= CSE_DATASTRUCTURES_PUB.G_PENDING;
       l_Txn_Rec.Message_Id := P_In_Service_Attr_Rec.Message_Id;
...
-- Step 5  ,if the item is non serialized
 ...
 IF(P_In_Service_Attr_Rec.Serial_Number IS  NULL) THEN

--Step 5.1 ,Check for the existence of the destination record

   IF (l_debug = 'Y') THEN
      CSE_DEBUG_PUB.ADD('NON Serialized Initailizing Query Record for the destination instance - step5.1');
   END IF;
...
-- update source instance quantity
      ...
      END IF;
       ...
       l_Instance_Rec.Quantity              := (l_Source_Quantity -
                                                P_In_Service_Attr_Rec.Quantity);
       ...
       END IF;
       IF (l_debug = 'Y') THEN
          CSE_DEBUG_PUB.ADD('Before update of source instance qty - step 5.3');
       END IF;
 ...
 IF(l_Instance_Header_Tbl_Out.COUNT>0) THEN
       IF (l_debug = 'Y') THEN
          CSE_DEBUG_PUB.ADD('Before util.get_destination_instance');
       END IF;
...
-- If destination item instance is found
  ...
  l_D_object_version_number:=l_Dest_Instance_Rec.object_version_number;
   ...
   END IF;
    ...
    l_Instance_Rec.Quantity          := (P_In_Service_Attr_Rec.Quantity +
...
END IF;
       ...
       l_Instance_Rec.Inventory_Item_id  := P_In_Service_Attr_Rec.Item_id;
       l_Instance_Rec.Inventory_Revision := P_In_Service_Attr_Rec.Revision;
       ...
       l_Instance_Rec.Inv_organization_id := l_Inv_Organization_Id;
       l_Instance_Rec.Serial_Number := P_In_Service_Attr_Rec.Serial_Number;
       ...
       l_Instance_Rec.Lot_Number := P_In_Service_Attr_Rec.Lot_Number;
       l_Instance_Rec.Quantity := P_In_Service_Attr_Rec.Quantity;
...
PROCEDURE update_eib_instances(
   ...
   X_Error_Message         OUT NOCOPY VARCHAR2) IS
BEGIN
 IF NOT p_in_service_attr_tbl.COUNT = 0 THEN
 	FOR i IN p_in_service_attr_tbl.FIRST .. p_in_service_attr_tbl.LAST
        LOOP
             IF p_in_service_attr_tbl.EXISTS(i) THEN
 	     update_ib_repository(p_in_service_attr_tbl(i),
                      x_return_status,
                      x_error_message);
             END IF;
 	END LOOP;
