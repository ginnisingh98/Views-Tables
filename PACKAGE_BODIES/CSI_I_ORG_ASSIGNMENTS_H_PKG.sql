--------------------------------------------------------
--  DDL for Package Body CSI_I_ORG_ASSIGNMENTS_H_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSI_I_ORG_ASSIGNMENTS_H_PKG" as
/* $Header: csitorgb.pls 115.17 2003/09/04 00:21:31 sguthiva ship $ */


g_pkg_name CONSTANT VARCHAR2(30):= 'CSI_I_ORG_ASSIGNMENTS_H_PKG';

PROCEDURE Insert_Row(
          px_INSTANCE_OU_HISTORY_ID   IN OUT NOCOPY NUMBER,
 ...
 IS
   CURSOR C2 IS SELECT CSI_I_ORG_ASSIGNMENTS_H_S.nextval FROM sys.dual;
BEGIN
   If (px_INSTANCE_OU_HISTORY_ID IS NULL) OR (px_INSTANCE_OU_HISTORY_ID = FND_API.G_MISS_NUM) then
       OPEN C2;
       FETCH C2 INTO px_INSTANCE_OU_HISTORY_ID;
       CLOSE C2;
   End If;
...
PROCEDURE Update_Row(
          p_INSTANCE_OU_HISTORY_ID    NUMBER,
          p_INSTANCE_OU_ID    NUMBER,
          p_TRANSACTION_ID    NUMBER,
          p_OLD_OPERATING_UNIT_ID    NUMBER,
          p_NEW_OPERATING_UNIT_ID    NUMBER,
 ...
 BEGIN
    ...
    where INSTANCE_OU_HISTORY_ID = p_INSTANCE_OU_HISTORY_ID;
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
        ...
        WHERE INSTANCE_OU_HISTORY_ID =  p_INSTANCE_OU_HISTORY_ID
        FOR UPDATE of INSTANCE_OU_HISTORY_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN C;
    ...
    CLOSE C;
    if (
           (      Recinfo.INSTANCE_OU_HISTORY_ID = p_INSTANCE_OU_HISTORY_ID)
       ...
       AND (    ( Recinfo.OLD_OPERATING_UNIT_ID = p_OLD_OPERATING_UNIT_ID)
            OR (    ( Recinfo.OLD_OPERATING_UNIT_ID IS NULL )
                AND (  p_OLD_OPERATING_UNIT_ID IS NULL )))
       AND (    ( Recinfo.NEW_OPERATING_UNIT_ID = p_NEW_OPERATING_UNIT_ID)
            OR (    ( Recinfo.NEW_OPERATING_UNIT_ID IS NULL )
                AND (  p_NEW_OPERATING_UNIT_ID IS NULL )))
...
End CSI_I_ORG_ASSIGNMENTS_H_PKG;
