--------------------------------------------------------
--  DDL for Package Body IGS_AS_GPC_CLS_STNDG_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_GPC_CLS_STNDG_PKG" AS
/* $Header: IGSDI51B.pls 115.7 2002/11/28 23:23:29 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_as_gpc_cls_stndg%ROWTYPE;
  new_references igs_as_gpc_cls_stndg%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.gpc_cls_stndg_id                  := x_gpc_cls_stndg_id;
  ...
  */
  BEGIN
    ...
    ELSIF NOT igs_pr_class_std_pkg.get_uk_For_validation (
                new_references.class_standing
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
...
BEGIN
 ...
 IF ( cur_rowid%FOUND )THEN
   ...
   Fnd_Message.Set_Name ('IGS','IGS_AS_GCS_LVAL_FK');
   IGS_GE_MSG_STACK.ADD ;
...
END GET_FK_IGS_LOOKUPS_VIEW  ;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_as_gpc_cls_stndg
      WHERE    gpc_cls_stndg_id = x_gpc_cls_stndg_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AS_GCS_PCS_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.gpc_cls_stndg_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_as_gpc_cls_stndg_s.NEXTVAL
    INTO      x_gpc_cls_stndg_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_gpc_cls_stndg_id                  => x_gpc_cls_stndg_id,
    ...
    INSERT INTO igs_as_gpc_cls_stndg (
      gpc_cls_stndg_id,
      grading_period_cd,
      class_standing,
      creation_date,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_as_gpc_cls_stndg
    WHERE rowid = x_rowid;
