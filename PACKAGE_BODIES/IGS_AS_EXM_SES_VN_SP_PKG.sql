--------------------------------------------------------
--  DDL for Package Body IGS_AS_EXM_SES_VN_SP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_EXM_SES_VN_SP_PKG" AS
/* $Header: IGSDI13B.pls 115.4 2002/11/28 23:13:32 nsidana ship $ */
--w.r.t BUG #1956374 , Procedure assp_val_est_closed reference is changed
  l_rowid VARCHAR2(25);
  old_references IGS_AS_EXM_SES_VN_SP%RowType;
  new_references IGS_AS_EXM_SES_VN_SP%RowType;
...
PROCEDURE BeforeRowInsertUpdate1(
  ...
  BEGIN
	IF p_inserting THEN
		...
		-- Validate that an exam instance exists for the venue.
		IF IGS_AS_VAL_ESVS.assp_val_ei_venue(new_references.exam_cal_type,
						new_references.exam_ci_sequence_number,
						new_references.dt_alias,
						new_references.dai_sequence_number,
						new_references.start_time,
...
IGS_GE_MSG_STACK.ADD;
    ...
    ELSIF NOT IGS_AS_EXM_SPRVSRTYP_PKG.Get_PK_For_Validation (
        new_references.exam_supervisor_type )THEN
	    Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
IGS_GE_MSG_STACK.ADD;
	    APP_EXCEPTION.RAISE_EXCEPTION;
	END IF;
...
IGS_GE_MSG_STACK.ADD;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_AS_ESVS_ESE_UFK');
IGS_GE_MSG_STACK.ADD;
	       Close cur_rowid;
      APP_EXCEPTION.RAISE_EXCEPTION;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_AS_ESVS_EST_FK');
IGS_GE_MSG_STACK.ADD;
	    Close cur_rowid;
      APP_EXCEPTION.RAISE_EXCEPTION;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_AS_ESVS_ESU_FK');
IGS_GE_MSG_STACK.ADD;
	        Close cur_rowid;
      APP_EXCEPTION.RAISE_EXCEPTION;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AS_EXM_SES_VN_SP
      where PERSON_ID = X_PERSON_ID
      and EXAM_CAL_TYPE = X_EXAM_CAL_TYPE
      and EXAM_CI_SEQUENCE_NUMBER = X_EXAM_CI_SEQUENCE_NUMBER
      and DT_ALIAS = X_DT_ALIAS
...
IGS_GE_MSG_STACK.ADD;
  ...
  insert into IGS_AS_EXM_SES_VN_SP (
    PERSON_ID,
    EXAM_CAL_TYPE,
...
IGS_GE_MSG_STACK.ADD;
  ...
  if ( (tlinfo.ESE_ID = X_ESE_ID)
      AND (trunc(tlinfo.EXAM_SUPERVISOR_TYPE) = X_EXAM_SUPERVISOR_TYPE)
      AND ((trunc(tlinfo.OVERRIDE_START_TIME) = X_OVERRIDE_START_TIME)
           OR ((trunc(tlinfo.OVERRIDE_START_TIME) is null)
               AND (trunc(X_OVERRIDE_START_TIME) is null)))
      AND ((trunc(tlinfo.OVERRIDE_END_TIME) = X_OVERRIDE_END_TIME)
           OR ((trunc(tlinfo.OVERRIDE_END_TIME) is null)
               AND (trunc(X_OVERRIDE_END_TIME) is null)))
  ) then
    null;
  else
...
IGS_GE_MSG_STACK.ADD;
  ...
  update IGS_AS_EXM_SES_VN_SP set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AS_EXM_SES_VN_SP
     where PERSON_ID = X_PERSON_ID
     and EXAM_CAL_TYPE = X_EXAM_CAL_TYPE
     and EXAM_CI_SEQUENCE_NUMBER = X_EXAM_CI_SEQUENCE_NUMBER
     and DT_ALIAS = X_DT_ALIAS
...
begin
  ...
  delete from IGS_AS_EXM_SES_VN_SP
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
