--------------------------------------------------------
--  DDL for Package Body XNP_XDP_ORDER_DONE_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_XDP_ORDER_DONE_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'XDP_ORDER_DONE' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'XDP_ORDER_DONE' ) ;
  l_loop_index_1 := 0 ;
  FOR XNP$XDP_ORDER_DONE  IN (SELECT STATUS_CODE FROM
XDP_ORDER_HEADERS WHERE
ORDER_ID=P_ORDER_ID)
 LOOP 
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORDER_ID', p_order_id ) ;
    XNP_XML_UTILS.write_leaf_element ( 'ORDER_STATUS', XNP$XDP_ORDER_DONE.STATUS_CODE ) ;
    XNP_XML_UTILS.write_leaf_element ( 'SDP_RESULT_CODE', XNP$XDP_ORDER_DONE.STATUS_CODE ) ;
    EXIT ;
  END LOOP ;
  IF ( l_loop_index_1 = 0 ) THEN
       fnd_message.set_name('XNP', 'XNP_MISSING_MANDATORY_DATA' );
       fnd_message.set_token( 'ELEMENT',', ORDER_ID' ) ;
      x_error_message := fnd_message.get ; 
    RAISE e_MISSING_MANDATORY_DATA ;
  END IF ;
    XNP_XML_UTILS.end_segment ( 'XDP_ORDER_DONE' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'XDP_ORDER_DONE.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'XDP_ORDER_DONE', l_consumer_list );
  END IF;
