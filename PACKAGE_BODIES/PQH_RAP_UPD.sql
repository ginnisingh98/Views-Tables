--------------------------------------------------------
--  DDL for Package Body PQH_RAP_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_RAP_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package   constant varchar2(33) := '  pqh_rap_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the pqh_rank_process_approvals Row
  --
  update pqh_rank_process_approvals
    set
     rank_process_approval_id        = p_rec.rank_process_approval_id
    ,rank_process_id                 = p_rec.rank_process_id
    ,approval_date                   = p_rec.approval_date
    ,supervisor_id                   = p_rec.supervisor_id
    ,system_rank                     = p_rec.system_rank
    ,population_count                = p_rec.population_count
    ,proposed_rank                   = p_rec.proposed_rank
...
Begin
  ...
  --
  If (p_rec.rank_process_id = hr_api.g_number) then
    p_rec.rank_process_id :=
    pqh_rap_shd.g_old_rec.rank_process_id;
  End If;
  ...
  If (p_rec.supervisor_id = hr_api.g_number) then
    ...
    pqh_rap_shd.g_old_rec.supervisor_id;
  End If;
  If (p_rec.system_rank = hr_api.g_number) then
    p_rec.system_rank :=
    pqh_rap_shd.g_old_rec.system_rank;
  End If;
  If (p_rec.population_count = hr_api.g_number) then
    p_rec.population_count :=
    pqh_rap_shd.g_old_rec.population_count;
  End If;
  If (p_rec.proposed_rank = hr_api.g_number) then
    p_rec.proposed_rank :=
    pqh_rap_shd.g_old_rec.proposed_rank;
  End If;
  --
...
Begin
  ...
  --
  pqh_rap_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pqh_rap_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pqh_rap_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure upd
  ...
  ,p_rank_process_id              in     number    default hr_api.g_number
