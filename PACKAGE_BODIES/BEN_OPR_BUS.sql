--------------------------------------------------------
--  DDL for Package Body BEN_OPR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_OPR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_opr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_othr_ptip_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   othr_ptip_rt_id PK of record being inserted or updated.
...
--
Procedure chk_othr_ptip_rt_id(p_othr_ptip_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_othr_ptip_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_opr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_othr_ptip_rt_id      => p_othr_ptip_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_othr_ptip_rt_id,hr_api.g_number)
     <>  ben_opr_shd.g_old_rec.othr_ptip_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_opr_shd.constraint_error('BEN_OTHR_PTIP_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_othr_ptip_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_ptip_id (p_othr_ptip_rt_id   in number,
                       p_ptip_id                  in number,
  ...
  cursor c3 is
     select null
       ...
       where  opr.ptip_id = p_ptip_id
       and opr.vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
       and opr.othr_ptip_rt_id <> nvl(p_othr_ptip_rt_id,hr_api.g_number)

       and opr.business_group_id + 0 = p_business_group_id
       and p_validation_start_date <= opr.effective_end_date
       and p_validation_end_date >= opr.effective_start_date;
--
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  ...
  if (l_api_updating
     ...
     <> nvl(ben_opr_shd.g_old_rec.ptip_id,hr_api.g_number)
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_opr_shd.constraint_error('BEN_OTHR_PTIP_RT_DT2');
        --
      end if;
      --
    close c1;
  ...
  --
    ...
    if c3%found then
     ...
     fnd_message.set_token('VAR1','Eligible in Another Plan Type in Program criteria');
...
--   Internal table handler use only.
--
Procedure chk_only_pls_subj_cobra_flag(p_othr_ptip_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_opr_shd.g_old_rec.only_pls_subj_cobra_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_othr_ptip_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_opr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and othr_ptip_rt_id   <> nvl(p_othr_ptip_rt_id  ,-1)
...
function return_legislation_code
  (p_othr_ptip_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_othr_ptip_rt_f b
    where b.othr_ptip_rt_id      = p_othr_ptip_rt_id
