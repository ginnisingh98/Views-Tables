--------------------------------------------------------
--  DDL for Package Body PSP_PRT_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSP_PRT_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  psp_prt_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_template_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  psp_prt_ins.g_template_id_i := p_template_id;
  --
...
Begin
  ...
  --
  -- Insert the row into: psp_report_templates
  --
  insert into psp_report_templates
      (template_id
      ,template_name
      ,business_group_id
      ,set_of_books_id
...
--
  Cursor C_Sel1 is select psp_report_templates_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from psp_report_templates
     where template_id =
             psp_prt_ins.g_template_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (psp_prt_ins.g_template_id_i is not null) Then
    --
    ...
    p_rec.template_id :=
      psp_prt_ins.g_template_id_i;
    psp_prt_ins.g_template_id_i := null;
  Else
    --
...
Begin
  ...
  --
  psp_prt_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  psp_prt_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  psp_prt_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  -- Having converted the arguments into the psp_prt_rec
