--------------------------------------------------------
--  DDL for Package Body OTA_TPS_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_TPS_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ota_tps_ins.';  -- Global package name
--
g_training_plan_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_training_plan_id  in  number) is
--
...
Begin
  ...
  --
  ota_tps_ins.g_training_plan_id_i := p_training_plan_id;
  --
...
Begin
  ...
  --
  -- Insert the row into: ota_training_plans
  --
  insert into ota_training_plans
      (training_plan_id
      ,time_period_id
      ,plan_status_type_id
      ,organization_id
...
--
  Cursor C_Sel1 is select ota_training_plans_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from ota_training_plans
     where training_plan_id =
             ota_tps_ins.g_training_plan_id_i;
--
  l_exists varchar2(1);

Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  If (ota_tps_ins.g_training_plan_id_i is not null) Then
    --
    ...
    p_rec.training_plan_id :=
      ota_tps_ins.g_training_plan_id_i;
    ota_tps_ins.g_training_plan_id_i := null;
  Else
    --
  ...
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.training_plan_id;
  Close C_Sel1;
...
Begin
  ...
  --
  ota_tps_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  ota_tps_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  ota_tps_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  -- Having converted the arguments into the ota_tps_rec
  ...
  --
  p_training_plan_id := l_rec.training_plan_id;
