--------------------------------------------------------
--  DDL for Package Body BEN_XCT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_XCT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_xct_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_xct_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_ext_crit_typ Row
  --
  update ben_ext_crit_typ
  set
  ...
  where ext_crit_typ_id = p_rec.ext_crit_typ_id;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_xct_shd.g_rec_type) is
--
...
--
 ...
 ,p_crit_typ_cd_o                 =>ben_xct_shd.g_old_rec.crit_typ_cd
 ,p_ext_crit_prfl_id_o            =>ben_xct_shd.g_old_rec.ext_crit_prfl_id
 ,p_business_group_id_o           =>ben_xct_shd.g_old_rec.business_group_id
 ,p_legislation_code_o            =>ben_xct_shd.g_old_rec.legislation_code
 ,p_object_version_number_o       =>ben_xct_shd.g_old_rec.object_version_number
 ,p_excld_flag_o                  =>ben_xct_shd.g_old_rec.excld_flag
      );
    --
  exception
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_xct_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.crit_typ_cd = hr_api.g_varchar2) then
    p_rec.crit_typ_cd :=
    ben_xct_shd.g_old_rec.crit_typ_cd;
  End If;
  If (p_rec.ext_crit_prfl_id = hr_api.g_number) then
    p_rec.ext_crit_prfl_id :=
    ben_xct_shd.g_old_rec.ext_crit_prfl_id;
  End If;
  ...
  If (p_rec.excld_flag = hr_api.g_varchar2) then
    p_rec.excld_flag :=
    ben_xct_shd.g_old_rec.excld_flag;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_xct_bus.update_validate(p_rec
  ,p_effective_date);
  --
