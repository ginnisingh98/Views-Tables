--------------------------------------------------------
--  DDL for Package Body PO_PRICE_ADJ_DRAFT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PO_PRICE_ADJ_DRAFT_PVT" AS
/* $Header: PO_PRICE_ADJ_DRAFT_PVT.plb 120.0.12010000.1 2009/06/01 23:33:50 ababujan noship $ */

d_pkg_name CONSTANT varchar2(50) :=
  PO_LOG.get_package_base('PO_PRICE_ADJ_DRAFT_PVT');
...
--  given the draft_id or draft_id + price_adjustment_id
--    If only draft_id is provided, this program returns FND_API.G_TRUE for
--  any draft changes in this table for the draft
--    If the whole primary key is provided (draft_id + price_adjustment id), then
...
--  price adjustment record unique identifier
--IN OUT:
--OUT:
--Returns:
--  FND_API.G_TRUE if there are draft changes
...
--  None
--Function:
--  Process price adjustment draft records and merge them to transaction table. It
...
BEGIN
  ...
  IF (p_draft_info.price_adj_changed = FND_API.G_FALSE) THEN
    IF (PO_LOG.d_stmt) THEN
  ...
  d_position := 10;
  -- Merge Changes
  PO_PRICE_ADJ_DRAFT_PKG.merge_changes
