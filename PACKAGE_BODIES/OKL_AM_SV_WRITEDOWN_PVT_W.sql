--------------------------------------------------------
--  DDL for Package Body OKL_AM_SV_WRITEDOWN_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_AM_SV_WRITEDOWN_PVT_W" as
  /* $Header: OKLESVWB.pls 120.1 2005/07/07 12:46:43 asawanka noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).p_new_salvage_value := rosetta_g_miss_num_map(a1(indx));
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
    ...
    a1 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a1(indx) := rosetta_g_miss_num_map(t(ddindx).p_new_salvage_value);
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_assets_tbl okl_am_sv_writedown_pvt.assets_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_am_sv_writedown_pvt.create_salvage_value_trx(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
