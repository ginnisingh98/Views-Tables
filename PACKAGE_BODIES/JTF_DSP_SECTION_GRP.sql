--------------------------------------------------------
--  DDL for Package Body JTF_DSP_SECTION_GRP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_DSP_SECTION_GRP" AS
/* $Header: JTFGCSCB.pls 115.24 2004/07/09 18:49:21 applrt ship $ */
...
G_PKG_NAME  CONSTANT VARCHAR2(30):='JTF_DSP_SECTION_GRP';
G_FILE_NAME CONSTANT VARCHAR2(12):='JTFGCSCB.pls';
...
IS
  ...
  CURSOR c IS SELECT rowid FROM jtf_dsp_sections_b
    WHERE section_id = x_section_id;
  CURSOR c2 IS SELECT jtf_dsp_sections_b_s1.nextval FROM dual;
...
BEGIN
  ...
  -- insert base
  INSERT INTO jtf_dsp_sections_b
  (
  section_id,
  ...
  -- insert tl
  INSERT INTO jtf_dsp_sections_tl
  (
  last_update_login,
  display_name,
  description,
...
BEGIN
  ...
  UPDATE jtf_dsp_sections_tl SET
    object_version_number = object_version_number + 1,
...
BEGIN
  ...
  DELETE FROM jtf_dsp_sections_tl
  WHERE section_id = p_section_id;
  ...
  DELETE FROM jtf_dsp_sections_b
  WHERE section_id = p_section_id;
...
IS
  ...
  IS SELECT lookup_code FROM fnd_lookup_values
    WHERE lookup_type = 'JTF_SECTION_STATUS' AND
...
BEGIN
  ...
  IF ((p_display_name IS NULL) OR
      (p_display_name = FND_API.G_MISS_CHAR))
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_DSP_NAME');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_SCT_TYPE');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_STATUS');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_START_DATE');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    ...
    THEN
      FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_AVL_FLAG');
      FND_MSG_PUB.Add;
      RAISE FND_API.G_EXC_ERROR;
    END IF;
  END IF;
  ...
  THEN
    ...
    ELSIF (x_return_status = FND_API.G_RET_STS_ERROR) THEN
      FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_AUTO_PLACE');
  ...
  THEN
    ...
    IF (x_return_status = FND_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
    ELSIF (x_return_status = FND_API.G_RET_STS_ERROR) THEN
      FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_ORDER_BY');
  ...
  ELSIF (l_return_status = FND_API.G_RET_STS_SUCCESS) THEN
      FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_DUPLICATE_SECT');
      FND_MSG_PUB.Add;
  ...
  THEN
    BEGIN
      SELECT context_id INTO l_display_context_id FROM jtf_dsp_context_b
        WHERE context_id = p_display_context_id;
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
         FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NO_DSP_CTX');
         FND_MESSAGE.Set_Token('DISPLAY_CONTEXT', p_display_context_id);
         FND_MSG_PUB.Add;
         RAISE FND_API.G_EXC_ERROR;
       WHEN OTHERS THEN
         FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_GET_DSP_CTX');
         FND_MSG_PUB.Add;
         RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
    END;
  END IF;
  ...
  THEN
    ...
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
         FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NO_DLVRBL');
         FND_MESSAGE.Set_Token('DELIVERABLE', p_deliverable_id);
         FND_MSG_PUB.Add;
         RAISE FND_API.G_EXC_ERROR;
       WHEN OTHERS THEN
         FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_GET_DLVRBL');
         FND_MSG_PUB.Add;
         RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
    END;
  END IF;
  ...
  BEGIN
    SELECT lookup_code INTO l_tmp_str FROM fnd_lookup_values
      WHERE lookup_type = 'JTF_SECTION_TYPE' AND
            lookup_code = p_section_type_code AND
            language = USERENV('LANG');
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NO_SCT_TYPE');
       FND_MESSAGE.Set_Token('SECTION_TYPE', p_section_type_code);
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_ERROR;
     WHEN OTHERS THEN
       FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_GET_SCT_TYPE');
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
  END;
  ...
  IF (c3%NOTFOUND) THEN
    CLOSE c3;
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NO_STATUS');
...
BEGIN
  ...
  IF (p_section_id IS NULL) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NULL_SCT_ID');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  IF (p_display_name IS NULL) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NULL_DSP_NAME');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  IF (p_section_type_code IS NULL) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NULL_SCT_TYPE');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  IF (p_status_code IS NULL) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NULL_STATUS');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  IF (p_start_date_active IS NULL) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NULL_START_DATE');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
...
BEGIN
  ...
  ELSIF (l_return_status = FND_API.G_RET_STS_ERROR) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INVALID_CREATE');
    FND_MSG_PUB.Add;
  ...
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INSERT_FAIL');
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_ERROR;
     WHEN OTHERS THEN
       FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_INSERT_FAIL');
...
BEGIN
  ...
  THEN
    ...
    -- query for section id
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
           FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NO_SCT_ACSS_NAME');
           FND_MESSAGE.Set_Token('ACCESS_NAME', p_access_name);
           FND_MSG_PUB.Add;
           RAISE FND_API.G_EXC_ERROR;
         WHEN OTHERS THEN
           FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_GET_SCT_ACSS_NAME');
           FND_MESSAGE.Set_Token('ACCESS_NAME', p_access_name);
           FND_MSG_PUB.Add;
           RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
      END;
  ELSE
    ...
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_NO_ID_OR_ACSS');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  EXCEPTION
     WHEN NO_DATA_FOUND THEN
       FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_UPDATE_FAIL');
       FND_MSG_PUB.Add;
       RAISE FND_API.G_EXC_ERROR;
     WHEN OTHERS THEN
       FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_UPDATE_FAIL');
...
IS
  ...
  CURSOR c1(l_c_child_section_id IN NUMBER) IS
    SELECT mini_site_section_section_id FROM jtf_dsp_msite_sct_sects
...
BEGIN
  ...
  IF (c3%FOUND) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_MSITE_REF');
  ...
  -- delete from jtf_dsp_msite_sct_sects table
  FOR r1 in c1(l_section_id) LOOP

    JTF_DSP_MSITE_SCT_SECT_PVT.Delete_MSite_Section_Section
      (
  ...
  -- delete for jtf_dsp_section_items table
  FOR r2 in c2(l_section_id) LOOP

    JTF_DSP_SECTION_ITEM_PVT.Delete_Section_Item
      (
      p_api_version                  => p_api_version,
      p_init_msg_list                => FND_API.G_FALSE,
      p_commit                       => FND_API.G_FALSE,
  ...
  JTF_LOGICALCONTENT_GRP.Delete_Section(l_section_id);

  -- delete for jtf_dsp_sections_b and _tl tables
...
is
begin
  delete from JTF_DSP_SECTIONS_TL T
  where not exists
    (select NULL
  ...
  update JTF_DSP_SECTIONS_TL T set (
      DISPLAY_NAME,
      DESCRIPTION,
      LONG_DESCRIPTION,
      KEYWORDS
  ...
  where (
      ...
      SUBT.SECTION_ID,
      SUBT.LANGUAGE
    from JTF_DSP_SECTIONS_TL SUBB, JTF_DSP_SECTIONS_TL SUBT
  ...
  insert into JTF_DSP_SECTIONS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from JTF_DSP_SECTIONS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
