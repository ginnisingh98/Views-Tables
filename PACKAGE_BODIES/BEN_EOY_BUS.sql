--------------------------------------------------------
--  DDL for Package Body BEN_EOY_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EOY_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_eoy_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_othr_ptip_prte_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_othr_ptip_prte_id PK of record being inserted or updated.
...
--
Procedure chk_elig_othr_ptip_prte_id(p_elig_othr_ptip_prte_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_othr_ptip_prte_id';
...
Begin
  ...
  --
  l_api_updating := ben_eoy_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_othr_ptip_prte_id      => p_elig_othr_ptip_prte_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_othr_ptip_prte_id,hr_api.g_number)
     <>  ben_eoy_shd.g_old_rec.elig_othr_ptip_prte_id) then
    --
    -- raise error as PK has changed
    --
    ben_eoy_shd.constraint_error('BEN_ELIG_OTHR_PTIP_PRTE_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_othr_ptip_prte_id is not null then
      --
  ...
  --
End chk_elig_othr_ptip_prte_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_ptip_id >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_ptip_id (p_elig_othr_ptip_prte_id   in number,
                       p_ptip_id                  in number,
  ...
  cursor c3 is
     select null
       ...
       where  eoy.ptip_id = p_ptip_id
       and eoy.eligy_prfl_id = p_eligy_prfl_id
       and eoy.elig_othr_ptip_prte_id <> nvl(p_elig_othr_ptip_prte_id,hr_api.g_number)

       and eoy.business_group_id + 0 = p_business_group_id
       and p_validation_start_date <= eoy.effective_end_date
       and p_validation_end_date >= eoy.effective_start_date;
--
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  ...
  if (l_api_updating
     ...
     <> nvl(ben_eoy_shd.g_old_rec.ptip_id,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if ptip_id value exists in the plan type in program table
    open c1;
      --
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_eoy_shd.constraint_error('BEN_ELIG_OTHR_PTIP_PRTE_DT2');
        --
      end if;
      --
    close c1;
  ...
  --
    ...
    if c3%found then
      close c3;
      --
      -- raise error as this ptip already exists for this profile
  --
...
--   Internal table handler use only.
--
Procedure chk_only_pls_subj_cobra_flag(p_elig_othr_ptip_prte_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_eoy_shd.g_old_rec.only_pls_subj_cobra_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_othr_ptip_prte_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_eoy_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--
-- In Parameters
--   p_elig_othr_ptip_prte_id      elig_othr_ptip_prte_id
...
is
  ...
  cursor c1 is
    ...
    where  elig_othr_ptip_prte_id <> nvl(p_elig_othr_ptip_prte_id,-1)
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    hr_api.mandatory_arg_error
      (p_api_name       => l_proc,
       p_argument       => 'elig_othr_ptip_prte_id',
       p_argument_value => p_elig_othr_ptip_prte_id);
    --
    --
    --
  End If;
...
function return_legislation_code
  (p_elig_othr_ptip_prte_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_othr_ptip_prte_f b
    where b.elig_othr_ptip_prte_id      = p_elig_othr_ptip_prte_id
