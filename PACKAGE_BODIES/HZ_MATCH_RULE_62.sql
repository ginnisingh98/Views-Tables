--------------------------------------------------------
--  DDL for Package Body HZ_MATCH_RULE_62
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_MATCH_RULE_62" AS
/*=======================================================================+
...
END;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      END IF;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(case(instr(g_party_stage_rec.TX41,'%')) when 0 then g_party_stage_rec.TX41 else ltrim(g_party_stage_rec.TX41,chr(48)) END, case(instr(g_party_stage_rec.TX41,'%')) when 0 then  p_table_TX41 else ltrim( p_table_TX41,chr(48)) END,45) THEN
      total := total+60;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
  ...
  BEGIN
    ...
    FOR J IN 1..g_party_site_stage_list.COUNT LOOP
      ...
      ELSIF -- l_current_score<16 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX13, p_table_TX13,(50000*(J-1)+76)) THEN
  ...
  BEGIN
    ...
    FOR J IN 1..g_contact_pt_stage_list.COUNT LOOP
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX1, p_table_TX1,(50000*(J-1)+110)) THEN
        ...
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX7, p_table_TX7,(50000*(J-1)+116)) THEN
        l_current_score:=l_current_score+ 60;
        IF g_score_until_thresh AND (l_current_score)>=g_thres_score THEN
          x_matchidx:=J;
          RETURN l_current_score;
      ...
      ELSIF -- l_current_score<48 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_contact_pt_stage_list(J).TX8, p_table_TX8,(50000*(J-1)+117)) THEN
  ...
  IF p_attribute = 'PARTY_ALL_NAMES' THEN
     ...
     l_matched_value := HZ_PARTY_ACQUIRE.get_known_as (p_record_id, p_entity,'PARTY_ALL_NAMES','Y');
      RETURN l_matched_value;
    END IF;
  IF p_attribute = 'DUNS_NUMBER_C' THEN

      SELECT party_type INTO l_party_type 
      FROM HZ_PARTIES
      WHERE party_id = p_record_id;
      IF l_party_type = 'ORGANIZATION' THEN
        SELECT DUNS_NUMBER_C INTO l_matched_value 
  ...
  IF p_attribute = 'URL' THEN
     BEGIN
       SELECT URL INTO l_matched_value 
...
END;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(case(instr(g_party_stage_rec.TX41,'%')) when 0 then g_party_stage_rec.TX41 else ltrim(g_party_stage_rec.TX41,chr(48)) END, case(instr(g_party_stage_rec.TX41,'%')) when 0 then  p_table_TX41 else ltrim( p_table_TX41,chr(48)) END,(50000*(p_idx-1)+45)) THEN
      l_score :=60;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'PARTY','DUNS_NUMBER_C');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'DUNS_NUMBER_C',
           'PARTY', p_search_rec.DUNS_NUMBER_C,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ELSIF l_current_score<16 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX13, p_table_TX13,(50000*(p_idx-1)+76)) THEN
      l_score :=16;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX7, p_table_TX7,(50000*(p_idx-1)+116)) THEN
      l_score :=60;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ELSIF l_current_score<48 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX8, p_table_TX8,(50000*(p_idx-1)+117)) THEN
      l_score :=48;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score > 0',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      l_attrib_value := get_attrib_val(p_record_id,'CONTACT_POINTS','URL');
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ...',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      INSERT INTO HZ_PARTY_SCORE_DTLS_GT (PARTY_ID, RECORD_ID, SEARCH_CONTEXT_ID,
                ATTRIBUTE,ENTITY,ENTERED_VALUE, MATCHED_VALUE, ASSIGNED_SCORE)
      VALUES (
           p_party_id,p_record_id,p_search_ctx_id,'URL',
           'CONTACT_POINTS', p_search_rec.URL,
           l_attrib_value,l_score);
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Inserting into HZ_PARTY_SCORE_DTLS_GT ... Done',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
...
POP_CONTACT_POINTS_COND_REC(p_contact_point_list);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      threshold := round((448/560)*(l_entered_max_score+l_ps_entered_max_score+l_ct_entered_max_score+l_cpt_entered_max_score));
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
  ...
  IF l_denorm_str IS NOT NULL THEN
    IF (p_non_denorm_max_score+40)>=p_threshold THEN
      l_denorm_str := NULL;
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, p_match_str, l_party_contains_str);
    ELSIF (p_non_denorm_max_score+p_denorm_max_score+40)>=p_threshold THEN
  ...
  END IF;
    ...
    -- Setup query string for DUNS_NUMBER_C
    l_prim_temp := null;
    IF g_party_stage_rec.TX41 IS NOT NULL THEN
     IF ltrim(g_party_stage_rec.TX41,'0') IS NOT NULL THEN
      FOR I in lengthb(ltrim(g_party_stage_rec.TX41,'0'))..9 LOOP
        HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
          lpad(ltrim(g_party_stage_rec.TX41,'0'),I,chr(48)),'A16',l_prim_temp);
      END LOOP;
     END IF;
    END IF;
  ...
  BEGIN
    ...
    FOR I IN 1..g_contact_pt_stage_list.COUNT LOOP
      ...
      l_prim_temp := null;
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- CLEANSE (URL) + WRDOMAIN
            g_contact_pt_stage_list(I).TX8,'A49',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
...
END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     OPEN x_cursor FOR 
      SELECT PARTY_ID , TX34, TX35, TX39, TX40, lpad(rtrim(TX41),9,chr(48)), TX45
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      l_sqlstr := 'SELECT PARTY_ID , TX34, TX35, TX39, TX40, lpad(rtrim(TX41),9,chr(48)), TX45 FROM hz_staged_parties stage ';
     ...
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (TX41 like :TX41||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || '     '||p_match_str||' (:attr IS NULL OR ';
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGE_PARTIES_T1) */ PARTY_ID , TX34, TX35, TX39, TX40, lpad(rtrim(TX41),9,chr(48)), TX45
          FROM HZ_STAGED_PARTIES stage
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_PARTIES_U1) */ stage.PARTY_ID , TX34, TX35, TX39, TX40, lpad(rtrim(TX41),9,chr(48)), TX45
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
     IF p_search_ctx_id IS NULL THEN
       l_sqlstr := 'SELECT   ' || l_hint || ' PARTY_ID , TX34, TX35, TX39, TX40, lpad(rtrim(TX41),9,chr(48)), TX45'||
                   ' FROM HZ_STAGED_PARTIES stage'||
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_sites = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID, TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, stage.PARTY_ID, stage.ORG_CONTACT_ID, TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID, TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22
          FROM HZ_STAGED_PARTY_SITES stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT  PARTY_SITE_ID, stage.PARTY_ID, ORG_CONTACT_ID, TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22
          ...
          OR (l_search_merged = 'N' AND nvl(stage.status_flag, 'A') = 'A')       )  
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, r.subject_id, stage.ORG_CONTACT_ID, TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
       l_sqlstr := 'SELECT  /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
         ...
         ' AND (ORG_CONTACT_ID IS NULL ' ||
       	...
       	' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
         ' UNION ' ||
       			 'SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
       ...
       ELSE 
       l_sqlstr := 'SELECT ' || l_hint ||' PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX3, TX4, TX9, TX10, TX11, TX12, TX13, TX14, TX15, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX8, TX7
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N3) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX8, TX7
          FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage 
          WHERE contains( concat_col, p_contains_str)>0 
          AND d.search_context_id = p_search_ctx_id 
          AND( (l_search_merged ='Y' )  
 ...
 THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_cpts = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX8, TX7
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ stage.CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX1, TX8, TX7
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX8, TX7
          FROM HZ_STAGED_CONTACT_POINTS stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
          SELECT 1 FROM HZ_STAGED_PARTIES p
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX8, TX7
            FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage
            WHERE contains( concat_col, p_contains_str)>0
            AND d.search_context_id = p_search_ctx_id
            AND d.party_id = stage.party_id
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, r.subject_id, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX1, TX8, TX7
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       ELSE
		p_restrict_sql1 := 'stage1.'||p_restrict_sql;
       END IF; 
       	l_sqlstr := ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX8, TX7'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ...
                   ' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
                   ' UNION ' ||
                   ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX8, TX7'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ' AND( ('''||l_search_merged||''' =''Y'' )  '||
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '||
       ...
       OPEN x_cursor FOR l_sqlstr USING p_contains_str,
                    p_dup_party_id, p_dup_party_id, p_contains_str, p_dup_party_id, p_dup_party_id;
       ELSE 
       	l_sqlstr := 'SELECT   ' || l_hint ||' CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX8, TX7'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                 ' AND EXISTS ('||
                 ' SELECT 1 FROM HZ_STAGED_PARTIES p ' || 
...
PROCEDURE find_parties (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_cur INTO
         l_party_id , l_TX34, l_TX35, l_TX39, l_TX40, l_TX41, l_TX45;
  ...
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
      l_score := GET_PARTY_SCORE(l_TX34,l_TX35_new,l_TX39,l_TX40,l_TX41,l_TX45);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_site_cur INTO
         l_party_site_id, l_ps_party_id, l_ps_contact_id , l_TX3, l_TX4, l_TX9, l_TX10, l_TX11, l_TX12, l_TX13, l_TX14, l_TX15, l_TX22;
      ...
      IF l_use_contact_addr_info OR l_ps_contact_id IS NULL THEN
        ...
        l_score := GET_PARTY_SITES_SCORE(l_match_idx,l_TX3,l_TX4,l_TX9,l_TX10,l_TX11,l_TX12,l_TX13,l_TX14,l_TX15,l_TX22);
        IF NOT H_SCORES.EXISTS(l_index) THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_contact_pt_cur INTO
         l_contact_pt_id,  l_cpt_type, l_cpt_party_id,  l_cpt_ps_id, l_cpt_contact_id , l_TX1, l_TX8, l_TX7;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      IF l_use_contact_cpt_info OR l_cpt_contact_id IS NULL THEN
        ...
        l_score := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX1,l_TX8,l_TX7);
        IF NOT H_SCORES.EXISTS(l_index) THEN
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    IF (p_index=5 AND call_order(5) <> 'NONE' AND H_PARTY_ID.COUNT=0) THEN
     l_threshold :=  round(( l_entered_max_score / 560) * 448); 
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
    l_party_contains_str := INIT_PARTY_QUERY(l_match_str, l_denorm_str, l_party_max_score, l_denorm_max_score, l_non_denorm_max_score, round((448/560)*l_entered_max_score));
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END LOOP;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'In find_parties. This is a Search Rule. Evaluating Matches. Threshold : 80 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      IF (H_SCORES(l_party_id).TOTAL_SCORE/l_entered_max_score)>=(448/560) THEN
            INSERT INTO HZ_MATCHED_PARTIES_GT (SEARCH_CONTEXT_ID, PARTY_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ...
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_parties;
...
PROCEDURE find_persons (
  ...
  BEGIN
    ...
    END IF;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Evaluating Matches. Threshold : 80 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
    IF l_match_str = ' OR ' THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_persons');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_persons;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_party_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_duplicate_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_duplicate_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_duplicate_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.find_duplicate_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
BEGIN
  ...
  IF AttrVals.EXISTS(16) THEN
    l_party_rec.DUNS_NUMBER_C:= AttrVals(16);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_party_rec.DUNS_NUMBER_C '||AttrVals(16),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
  END IF;
  ...
  IF AttrVals.EXISTS(49) THEN
    l_cpt_type := 'WEB';
    FIRST := FALSE;
    FOR I in 1..l_cpt_list.COUNT LOOP
      IF l_cpt_list(I).CONTACT_POINT_TYPE=l_cpt_type THEN
        l_cpt_list(I).URL:= AttrVals(49);
        FIRST := TRUE;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||I||').CONTACT_POINT_TYPE '||l_cpt_type,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||I||').URL '||AttrVals(49),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      END IF;
    END LOOP;
    IF not FIRST THEN
      l_idx := l_cpt_list.COUNT+1;
      l_cpt_list(l_idx).CONTACT_POINT_TYPE:=l_cpt_type;
      l_cpt_list(l_idx).URL:= AttrVals(49);
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||l_idx||').CONTACT_POINT_TYPE '||l_cpt_type,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
      hz_utility_v2pub.debug(p_message=>'l_cpt_list('||l_idx||').URL '||AttrVals(49),p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    END IF;
  END IF;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.call_api_dynamic');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_party_sites (
  ...
  BEGIN
    ...
    IF l_match_str = ' AND ' OR (
        ((l_cpt_max_score/l_entered_max_score)<(448/560))
        ) THEN
    ...
    END IF;
 IF l_match_str = ' OR ' THEN
IF (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=( 448/560 ) THEN
 ...
 ELSE
    IF H_SCORES(l_party_site_id).PARTY_SITE_SCORE>0 AND
       ...
       (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=(448/560) THEN
      INSERT INTO HZ_MATCHED_PARTY_SITES_GT (SEARCH_CONTEXT_ID, PARTY_ID, PARTY_SITE_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.get_matching_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contacts (
  ...
  BEGIN
    ...
    END IF;
        IF (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(448/560) THEN
      ...
      ELSE
           ...
           (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(448/560) THEN
          INSERT INTO HZ_MATCHED_CONTACTS_GT (SEARCH_CONTEXT_ID, PARTY_ID, ORG_CONTACT_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.get_matching_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contact_points (
  ...
  BEGIN
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      LOOP
        ...
        IF (p_dup_contact_point_id IS NULL OR (
            ...
            H_SCORE(cnt) := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX1,l_TX8,l_TX7);
            cnt := cnt+1;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF (H_SCORE(I)/l_entered_max_score) >= (448/560) THEN
        INSERT INTO HZ_MATCHED_CPTS_GT(SEARCH_CONTEXT_ID,CONTACT_POINT_ID,PARTY_ID,SCORE) VALUES (
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.get_matching_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_score_details (
  ...
  l_TX41 VARCHAR2(2000);
  l_max_TX41 VARCHAR2(2000);
  l_TX45 VARCHAR2(2000);
  l_max_TX45 VARCHAR2(2000);
  l_TX7 VARCHAR2(2000);
  l_max_TX7 VARCHAR2(2000);
  l_TX8 VARCHAR2(2000);
  ...
  l_TX35_new:=RTRIM(LTRIM(l_TX35));
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
          INSERT_PARTY_SCORE(p_party_id, p_party_id, l_search_ctx_id, p_party_search_rec, g_party_stage_rec, l_TX34,l_TX35_new,l_TX39,l_TX40,l_TX41,l_TX45,1);
    END LOOP;
    CLOSE l_party_cur;
    ...
    IF l_party_site_contains_str IS NOT NULL THEN
      ...
      IF l_max_score>0 THEN
        INSERT_PARTY_SITES_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_party_site_list(l_max_idx), g_party_site_stage_list(l_max_idx) ,l_max_TX3,l_max_TX4,l_max_TX9,l_max_TX10,l_max_TX11,l_max_TX12,l_max_TX13,l_max_TX14,l_max_TX15,l_max_TX22,l_max_idx);
      END IF;
    END IF;
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      END LOOP;
      IF l_max_score>0 THEN
        INSERT_CONTACT_POINTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_point_list(l_max_idx), g_contact_pt_stage_list(l_max_idx) ,l_max_TX1,l_max_TX8,l_max_TX7,l_max_idx);
      END IF;
      CLOSE l_contact_pt_cur;
    END IF;
    IF fnd_log.level_procedure>=fnd_log.g_current_runtime_level THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_62.get_score_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_contact_point_rec (
  ...
  CURSOR c_cpts(cp_contact_point_id NUMBER) IS
    SELECT contact_point_id, contact_point_type
      ,translate(HZ_CONTACT_POINTS.URL, '%',' ')
    FROM HZ_CONTACT_POINTS
...
BEGIN
   ...
   IF p_search_ctx THEN
    ...
    OR p_search_rec.DUNS_NUMBER_C IS NOT NULL 
    ...
    IF p_search_rec.PARTY_ALL_NAMES IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+40;
    END IF;
    IF p_search_rec.DUNS_NUMBER_C IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+60;
    END IF;
...
BEGIN
  ...
  IF p_search_ctx THEN
    ...
    FOR I IN 1..p_search_list.COUNT LOOP
      ...
      END IF;
      IF p_search_list(I).URL IS NOT NULL THEN 
        l_current_max_score := l_current_max_score+60;
      END IF;
      x_modify := 'N';
      FOR J IN 1..l_cnt_pt_type.count LOOP
  ...
  FOR I IN 1..p_search_list.COUNT LOOP
    ...
    ELSE
      IF p_search_list(I).URL IS NOT NULL THEN 
        tmp := HZ_TRANS_PKG.CLEANSED_URL(
             x_stage_list(I).TX8
             ,null,'URL'
             ,'CONTACT_POINTS','SCORE');
        IF tmp IS NOT NULL THEN
          x_stage_list(I).TX8 := tmp;
        END IF;
      END IF;
    END IF;
  END LOOP;
...
OR p_contact_point_list(I).URL IS NOT NULL 
 ) THEN
    ...
    IF p_party_search_rec.PARTY_ALL_NAMES IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    IF p_party_search_rec.DUNS_NUMBER_C IS NOT NULL THEN 
      RETURN TRUE;
    END IF;
    ...
    END LOOP;
    FOR I IN 1..p_contact_point_list.COUNT LOOP
      IF p_contact_point_list(I).URL IS NOT NULL THEN 
        RETURN TRUE;
      END IF;
    END LOOP;
    RETURN FALSE;
