--------------------------------------------------------
--  DDL for Package Body BEN_LNC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LNC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lnc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ler_enrt_ctfn_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   ler_enrt_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_ler_enrt_ctfn_id(p_ler_enrt_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ler_enrt_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_lnc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ler_enrt_ctfn_id                => p_ler_enrt_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ler_enrt_ctfn_id,hr_api.g_number)
     <>  ben_lnc_shd.g_old_rec.ler_enrt_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_lnc_shd.constraint_error('BEN_LER_ENRT_CTFN_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ler_enrt_ctfn_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_ler_enrt_ctfn_id          in number,
...
--   Internal table handler use only.
--
Procedure chk_enrt_ctfn_typ_cd(p_ler_enrt_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_lnc_shd.g_old_rec.enrt_ctfn_typ_cd,hr_api.g_varchar2)
...
--     p_ler_enrt_ctfn_id      is primary key for ben_ler_enrt_ctfn_f
...
cursor c1 is select null
              ...
              Where ler_enrt_ctfn_id <> nvl(p_ler_enrt_ctfn_id,-1)
                and nvl(enrt_ctfn_typ_cd,0) = nvl(p_enrt_ctfn_typ_cd,0)
                and ler_rqrs_enrt_ctfn_id = p_ler_rqrs_enrt_ctfn_id
...
--   Internal table handler use only.
--
Procedure chk_rqd_flag(p_ler_enrt_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_rqd_flag
      <> nvl(ben_lnc_shd.g_old_rec.rqd_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    --
    If ((nvl(p_ler_rqrs_enrt_ctfn_id, hr_api.g_number) <> hr_api.g_number) and
      ...
      l_table_name := 'ben_ler_rqrs_enrt_ctfn_f';
...
function return_legislation_code
  (p_ler_enrt_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_ler_enrt_ctfn_f b
    where b.ler_enrt_ctfn_id      = p_ler_enrt_ctfn_id
