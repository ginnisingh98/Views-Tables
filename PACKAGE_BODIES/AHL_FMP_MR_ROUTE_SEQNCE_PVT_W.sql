--------------------------------------------------------
--  DDL for Package Body AHL_FMP_MR_ROUTE_SEQNCE_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_FMP_MR_ROUTE_SEQNCE_PVT_W" as
  /* $Header: AHLWMRSB.pls 120.0 2005/05/26 00:13:25 appldev noship $ */
  ...
  procedure rosetta_table_copy_in_p1(t out nocopy ahl_fmp_mr_route_seqnce_pvt.mr_route_seq_tbl, a0 JTF_NUMBER_TABLE
    , a1 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).mr_route_sequence_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).mr_route_number := a2(indx);
          t(ddindx).mr_route_revision := a3(indx);
          t(ddindx).mr_route_id := a4(indx);
          t(ddindx).mr_header_id := a5(indx);
          t(ddindx).mr_title := a6(indx);
          t(ddindx).mr_version_number := a7(indx);
          t(ddindx).route_number := a8(indx);
          t(ddindx).route_revision_number := a9(indx);
          t(ddindx).related_mr_route_id := a10(indx);
          t(ddindx).sequence_code := a11(indx);
  ...
  procedure rosetta_table_copy_out_p1(t ahl_fmp_mr_route_seqnce_pvt.mr_route_seq_tbl, a0 out nocopy JTF_NUMBER_TABLE
  ...
  if t is null or t.count = 0 then
    ...
    a33 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).mr_route_sequence_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).mr_route_number;
          a3(indx) := t(ddindx).mr_route_revision;
          a4(indx) := t(ddindx).mr_route_id;
          a5(indx) := t(ddindx).mr_header_id;
          a6(indx) := t(ddindx).mr_title;
          a7(indx) := t(ddindx).mr_version_number;
          a8(indx) := t(ddindx).route_number;
          a9(indx) := t(ddindx).route_revision_number;
          a10(indx) := t(ddindx).related_mr_route_id;
          a11(indx) := t(ddindx).sequence_code;
  ...
  procedure process_mr_route_seq(p_api_version  NUMBER
  ...
  as
    ddp_x_mr_route_seq_tbl ahl_fmp_mr_route_seqnce_pvt.mr_route_seq_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_fmp_mr_route_seqnce_pvt_w.rosetta_table_copy_in_p1(ddp_x_mr_route_seq_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    ahl_fmp_mr_route_seqnce_pvt.process_mr_route_seq(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      p_default,
    ...
    ahl_fmp_mr_route_seqnce_pvt_w.rosetta_table_copy_out_p1(ddp_x_mr_route_seq_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
  ...
  procedure validate_route_stage_seq(p_mr_route_id  NUMBER
    , p_route_stage_upd  number
  )

  as
    ddp_route_stage_upd boolean;
    ddindx binary_integer; indx binary_integer;
  begin

    -- copy data to the local IN or IN-OUT args, if any

    if p_route_stage_upd is null
      then ddp_route_stage_upd := null;
    elsif p_route_stage_upd = 0
      then ddp_route_stage_upd := false;
    else ddp_route_stage_upd := true;
    end if;

    -- here's the delegated call to the old PL/SQL routine
    ahl_fmp_mr_route_seqnce_pvt.validate_route_stage_seq(p_mr_route_id,
      ddp_route_stage_upd);
...
end ahl_fmp_mr_route_seqnce_pvt_w;
