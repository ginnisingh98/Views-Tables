--------------------------------------------------------
--  DDL for Package Body BEN_PCX_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCX_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcx_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_pl_bnf_ctfn_f Row
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pcx_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_pcx_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.bnf_ctfn_typ_cd = hr_api.g_varchar2) then
    p_rec.bnf_ctfn_typ_cd :=
    ben_pcx_shd.g_old_rec.bnf_ctfn_typ_cd;
  End If;
  If (p_rec.lack_ctfn_sspnd_enrt_flag = hr_api.g_varchar2) then
    p_rec.lack_ctfn_sspnd_enrt_flag :=
    ben_pcx_shd.g_old_rec.lack_ctfn_sspnd_enrt_flag;
  End If;
  If (p_rec.pfd_flag = hr_api.g_varchar2) then
    p_rec.pfd_flag :=
    ben_pcx_shd.g_old_rec.pfd_flag;
  End If;
  If (p_rec.rqd_flag = hr_api.g_varchar2) then
    p_rec.rqd_flag :=
    ben_pcx_shd.g_old_rec.rqd_flag;
  End If;
  If (p_rec.ctfn_rqd_when_rl = hr_api.g_number) then
    p_rec.ctfn_rqd_when_rl :=
    ben_pcx_shd.g_old_rec.ctfn_rqd_when_rl;
  End If;
  If (p_rec.bnf_typ_cd = hr_api.g_varchar2) then
    p_rec.bnf_typ_cd :=
    ben_pcx_shd.g_old_rec.bnf_typ_cd;
  End If;
  If (p_rec.rlshp_typ_cd = hr_api.g_varchar2) then
    p_rec.rlshp_typ_cd :=
    ben_pcx_shd.g_old_rec.rlshp_typ_cd;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_pcx_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pcx_attribute_category = hr_api.g_varchar2) then
    p_rec.pcx_attribute_category :=
    ben_pcx_shd.g_old_rec.pcx_attribute_category;
  End If;
  If (p_rec.pcx_attribute1 = hr_api.g_varchar2) then
    p_rec.pcx_attribute1 :=
    ben_pcx_shd.g_old_rec.pcx_attribute1;
  End If;
  If (p_rec.pcx_attribute2 = hr_api.g_varchar2) then
    p_rec.pcx_attribute2 :=
    ben_pcx_shd.g_old_rec.pcx_attribute2;
  End If;
  If (p_rec.pcx_attribute3 = hr_api.g_varchar2) then
    p_rec.pcx_attribute3 :=
    ben_pcx_shd.g_old_rec.pcx_attribute3;
  End If;
  If (p_rec.pcx_attribute4 = hr_api.g_varchar2) then
    p_rec.pcx_attribute4 :=
    ben_pcx_shd.g_old_rec.pcx_attribute4;
  End If;
  If (p_rec.pcx_attribute5 = hr_api.g_varchar2) then
    p_rec.pcx_attribute5 :=
    ben_pcx_shd.g_old_rec.pcx_attribute5;
  End If;
  If (p_rec.pcx_attribute6 = hr_api.g_varchar2) then
    p_rec.pcx_attribute6 :=
    ben_pcx_shd.g_old_rec.pcx_attribute6;
  End If;
  If (p_rec.pcx_attribute7 = hr_api.g_varchar2) then
    p_rec.pcx_attribute7 :=
    ben_pcx_shd.g_old_rec.pcx_attribute7;
  End If;
  If (p_rec.pcx_attribute8 = hr_api.g_varchar2) then
    p_rec.pcx_attribute8 :=
    ben_pcx_shd.g_old_rec.pcx_attribute8;
  End If;
  If (p_rec.pcx_attribute9 = hr_api.g_varchar2) then
    p_rec.pcx_attribute9 :=
    ben_pcx_shd.g_old_rec.pcx_attribute9;
  End If;
  If (p_rec.pcx_attribute10 = hr_api.g_varchar2) then
    p_rec.pcx_attribute10 :=
    ben_pcx_shd.g_old_rec.pcx_attribute10;
  End If;
  If (p_rec.pcx_attribute11 = hr_api.g_varchar2) then
    p_rec.pcx_attribute11 :=
    ben_pcx_shd.g_old_rec.pcx_attribute11;
  End If;
  If (p_rec.pcx_attribute12 = hr_api.g_varchar2) then
    p_rec.pcx_attribute12 :=
    ben_pcx_shd.g_old_rec.pcx_attribute12;
  End If;
  If (p_rec.pcx_attribute13 = hr_api.g_varchar2) then
    p_rec.pcx_attribute13 :=
    ben_pcx_shd.g_old_rec.pcx_attribute13;
  End If;
  If (p_rec.pcx_attribute14 = hr_api.g_varchar2) then
    p_rec.pcx_attribute14 :=
    ben_pcx_shd.g_old_rec.pcx_attribute14;
  End If;
  If (p_rec.pcx_attribute15 = hr_api.g_varchar2) then
    p_rec.pcx_attribute15 :=
    ben_pcx_shd.g_old_rec.pcx_attribute15;
  End If;
  If (p_rec.pcx_attribute16 = hr_api.g_varchar2) then
    p_rec.pcx_attribute16 :=
    ben_pcx_shd.g_old_rec.pcx_attribute16;
  End If;
  If (p_rec.pcx_attribute17 = hr_api.g_varchar2) then
    p_rec.pcx_attribute17 :=
    ben_pcx_shd.g_old_rec.pcx_attribute17;
  End If;
  If (p_rec.pcx_attribute18 = hr_api.g_varchar2) then
    p_rec.pcx_attribute18 :=
    ben_pcx_shd.g_old_rec.pcx_attribute18;
  End If;
  If (p_rec.pcx_attribute19 = hr_api.g_varchar2) then
    p_rec.pcx_attribute19 :=
    ben_pcx_shd.g_old_rec.pcx_attribute19;
  End If;
  If (p_rec.pcx_attribute20 = hr_api.g_varchar2) then
    p_rec.pcx_attribute20 :=
    ben_pcx_shd.g_old_rec.pcx_attribute20;
  End If;
  If (p_rec.pcx_attribute21 = hr_api.g_varchar2) then
    p_rec.pcx_attribute21 :=
    ben_pcx_shd.g_old_rec.pcx_attribute21;
  End If;
  If (p_rec.pcx_attribute22 = hr_api.g_varchar2) then
    p_rec.pcx_attribute22 :=
    ben_pcx_shd.g_old_rec.pcx_attribute22;
  End If;
  If (p_rec.pcx_attribute23 = hr_api.g_varchar2) then
    p_rec.pcx_attribute23 :=
    ben_pcx_shd.g_old_rec.pcx_attribute23;
  End If;
  If (p_rec.pcx_attribute24 = hr_api.g_varchar2) then
    p_rec.pcx_attribute24 :=
    ben_pcx_shd.g_old_rec.pcx_attribute24;
  End If;
  If (p_rec.pcx_attribute25 = hr_api.g_varchar2) then
    p_rec.pcx_attribute25 :=
    ben_pcx_shd.g_old_rec.pcx_attribute25;
  End If;
  If (p_rec.pcx_attribute26 = hr_api.g_varchar2) then
    p_rec.pcx_attribute26 :=
    ben_pcx_shd.g_old_rec.pcx_attribute26;
  End If;
  If (p_rec.pcx_attribute27 = hr_api.g_varchar2) then
    p_rec.pcx_attribute27 :=
    ben_pcx_shd.g_old_rec.pcx_attribute27;
  End If;
  If (p_rec.pcx_attribute28 = hr_api.g_varchar2) then
    p_rec.pcx_attribute28 :=
    ben_pcx_shd.g_old_rec.pcx_attribute28;
  End If;
  If (p_rec.pcx_attribute29 = hr_api.g_varchar2) then
    p_rec.pcx_attribute29 :=
    ben_pcx_shd.g_old_rec.pcx_attribute29;
  End If;
  If (p_rec.pcx_attribute30 = hr_api.g_varchar2) then
    p_rec.pcx_attribute30 :=
    ben_pcx_shd.g_old_rec.pcx_attribute30;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
