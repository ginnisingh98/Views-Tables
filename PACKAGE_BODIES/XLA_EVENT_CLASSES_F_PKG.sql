--------------------------------------------------------
--  DDL for Package Body XLA_EVENT_CLASSES_F_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_EVENT_CLASSES_F_PKG" AS
/* $Header: xlathevc.pkb 120.18.12010000.1 2008/07/29 10:09:35 appldev ship $ */
/*======================================================================+
...
|                                                                       |
| DESCRIPTION                                                           |
|    Forms PL/SQL Wrapper for xla_event_classes                         |
|                                                                       |
| HISTORY                                                               |
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.xla_event_classes_f_pkg';
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    xla_exceptions_pkg.raise_message
      (p_location   => 'xla_event_classes_f_pkg.trace');
END trace;
...
INSERT INTO xla_event_classes_b
(creation_date
,created_by
,application_id
,entity_code
...
INSERT INTO xla_event_classes_tl
(creation_date
,entity_code
,event_class_code
,name
...
DELETE FROM xla_event_classes_tl
WHERE application_id                   = x_application_id
  AND entity_code                      = x_entity_code
  AND event_class_code                 = x_event_class_code;
...
DELETE FROM xla_event_classes_b
WHERE application_id                   = x_application_id
  AND entity_code                      = x_entity_code
  AND event_class_code                 = x_event_class_code;
...
DELETE FROM xla_event_classes_tl T
WHERE  NOT EXISTS
      (SELECT NULL
       FROM   xla_event_classes_b                b
       WHERE  b.application_id                   = t.application_id
         AND  b.entity_code                      = t.entity_code
         AND  b.event_class_code                 = t.event_class_code);

UPDATE xla_event_classes_tl   t
SET   (name
      ,description)
   = (SELECT b.name
            ,b.description
      FROM   xla_event_classes_tl               b
      WHERE  b.application_id                   = t.application_id
        AND  b.entity_code                      = t.entity_code
        AND  b.event_class_code                 = t.event_class_code
...
WHERE (t.application_id
    ...
    IN (SELECT subt.application_id
              ...
              ,subt.language
        FROM   xla_event_classes_tl                   subb
              ,xla_event_classes_tl                   subt
...
BEGIN
  ...
  BEGIN
    ...
    IF (fnd_load_util.upload_test(f_luby, f_ludate, db_luby, db_ludate, NULL)) THEN
      xla_event_classes_f_pkg.update_row
...
EXCEPTION
   ...
   WHEN OTHERS THEN
   xla_exceptions_pkg.raise_message
      (p_location   => 'xla_event_classes_f_pkg.load_row');
