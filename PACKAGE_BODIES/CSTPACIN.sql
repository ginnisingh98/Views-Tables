--------------------------------------------------------
--  DDL for Package Body CSTPACIN
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSTPACIN" as
/* $Header: CSTACINB.pls 120.8.12000000.2 2007/08/17 21:35:19 hyu ship $ */
...
is
  ...
  -- OPM INVCONV umoogala Process-Discrete Xfer Flag
...
begin
  ...
  */
  ...
  SELECT MOD(SUM(DECODE(mp.process_enabled_flag,'Y',1,2)), 2)
   ...
   WHERE organization_id = i_txn_org_id
      OR organization_id = i_txfr_org_id;
  ...
  /*------------------------------------------------------------+
   ...
   |     Bug 5400930: Without this condition, control is going to
  ...
  if ((i_txn_action_id = 3) or
    ...
    if (l_pd_txfr_ind = 0) then
     ...
     end if;  -- OPM INVCONV l_pd_txfr_ind = 0
   end if;
  ...
  /*------------------------------------------------------------+
   ...
   +------------------------------------------------------------*/
  /* Bug #2002105. Txf_cost_group_id is -1 if value in database is NULL.*/
   ...
   end if; /*Checking for average org*/
  ...
  /*
  ** call the average cost processor to cost transactions
  */

  CSTPAVCP.cost_processor(i_org_id,
                          i_txn_id,
                          l_layer_id,
			  i_cost_type_id,
			  i_cost_method,
  ...
  */
  ...
  /*
  ** call the material distribution processor
  */
...
/* BUG#6125007 - Changing the condition
 ...
 */
  ...
  IF    (NOT (      I_TXN_ACTION_ID = 12
                AND I_FOB_POINT     = 2
                AND I_TXN_ORG_ID    <> I_ORG_ID
                AND l_std_txfr_flag =  'N' )
    AND  NOT (      I_TXN_ACTION_ID = 21
                AND I_FOB_POINT     = 1
                AND I_TXN_ORG_ID    = I_ORG_ID
                AND l_std_txfr_flag = 'N'))
     OR
         ( I_TXN_ACTION_ID = 21    AND
           I_FOB_POINT     = 1     AND
           I_TXN_ORG_ID <> I_ORG_ID AND
           l_std_txfr_flag = 'N' )
  THEN
    l_stmt_num := 180;
    ...
    IF l_return_status <> 'S' THEN
      l_err_num := -1;
      l_err_code := 'Error raising SLA Event for transaction: '||to_char(i_txn_id);    RAISE FND_API.g_exc_unexpected_error;
    END IF;
...
EXCEPTION
  when cost_inv_txn_error then
    ...
    o_err_msg := 'CSTPACIN.COST_INV_TXN:' || l_err_msg;
  when OTHERS then
    o_err_num := SQLCODE;
    o_err_msg := 'CSTPACIN.COST_INV_TXN: (' || to_char(l_stmt_num) || '): '
