--------------------------------------------------------
--  DDL for Package Body PAY_BCT_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_BCT_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_bct_ins.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< insert_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
--
  ...
  -- Insert the row into: pay_batch_control_totals
  --

  insert into pay_batch_control_totals
      (batch_control_id
      ,batch_id
      ,control_status
      ,control_total
...
--
  Cursor C_Sel1 is select pay_batch_control_totals_s.nextval from sys.dual;
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.batch_control_id;
  Close C_Sel1;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
...
Begin
  ...
  --
  pay_bct_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  pay_bct_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  pay_bct_ins.post_insert
     (p_session_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  -- Having converted the arguments into the pay_bct_rec
  ...
  --
  p_batch_control_id := l_rec.batch_control_id;
