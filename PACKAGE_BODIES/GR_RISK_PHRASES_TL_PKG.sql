--------------------------------------------------------
--  DDL for Package Body GR_RISK_PHRASES_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_RISK_PHRASES_TL_PKG" AS
/*$Header: GRHIRPTB.pls 120.1 2006/06/16 21:41:20 pbamb noship $*/
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ROLLBACK TO SAVEPOINT Insert_Row;
	  l_msg_token := p_risk_phrase_code || ' ' || p_language;
	  x_return_status := 'E';
...
*/
   ...
   ELSE
      l_base_desc := PhraseDesc.risk_description;
...
**	For those that are found, read the risk phrase tl table.
...
CURSOR c_lock_risk_tl
 IS
   SELECT	last_update_date
   FROM		gr_risk_phrases_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockRiskRcd	  c_lock_risk_tl%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_risk_tl;
   FETCH c_lock_risk_tl INTO LockRiskRcd;
   IF c_lock_risk_tl%NOTFOUND THEN
	  CLOSE c_lock_risk_tl;
...
CURSOR c_get_risk_tl_rowid
 IS
   SELECT rpt.rowid
   FROM	  gr_risk_phrases_tl rpt
   WHERE  rpt.risk_phrase_code = p_risk_phrase_code
   AND	  rpt.language = p_language;
RiskTLRecord			   c_get_risk_tl_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_risk_phrase_code || ' ' || p_language;

   x_key_exists := 'F';
   OPEN c_get_risk_tl_rowid;
   FETCH c_get_risk_tl_rowid INTO RiskTLRecord;
   IF c_get_risk_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := RiskTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_risk_tl_rowid;
