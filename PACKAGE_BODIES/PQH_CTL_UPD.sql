--------------------------------------------------------
--  DDL for Package Body PQH_CTL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_CTL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_ctl_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy pqh_ctl_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_transaction_categories_tl Row
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in pqh_ctl_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update(p_rec in pqh_ctl_shd.g_rec_type) is
--
...
--
  ...
  begin
    --
    pqh_ctl_rku.after_update
      (
  p_transaction_category_id       =>p_rec.transaction_category_id
 ,p_name                          =>p_rec.name
 ,p_language                      =>p_rec.language
 ,p_source_lang                   =>p_rec.source_lang
 ,p_name_o                        =>pqh_ctl_shd.g_old_rec.name
 ,p_language_o                    =>pqh_ctl_shd.g_old_rec.language
 ,p_source_lang_o                 =>pqh_ctl_shd.g_old_rec.source_lang
      );
    --
  exception
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy pqh_ctl_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    pqh_ctl_shd.g_old_rec.name;
  End If;
  If (p_rec.source_lang = hr_api.g_varchar2) then
    p_rec.source_lang :=
    pqh_ctl_shd.g_old_rec.source_lang;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  pqh_ctl_bus.update_validate(p_rec);
  --
...
cursor csr_upd_langs is
     select ctl.language
      ...
      where ctl.transaction_category_id = p_transaction_category_id
        AND p_language_code in (ctl.language,ctl.source_lang);
--
...
--
    ...
    and transaction_category_id  = l_transaction_category_id ;
--
end translate_row;
--
--
