--------------------------------------------------------
--  DDL for Package Body AME_ATR_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ATR_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_atr_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_ATTRIBUTES_PK') Then
...
Begin
  ...
  Else
    If (p_attribute_id =
        ame_atr_shd.g_old_rec.attribute_id and
        p_object_version_number =
        ame_atr_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_atr_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_atr_shd.g_old_rec.end_date;
...
Begin
  ...
  l_rec.name                             := p_name;
  l_rec.attribute_type                   := p_attribute_type;
  l_rec.line_item                        := p_line_item;
