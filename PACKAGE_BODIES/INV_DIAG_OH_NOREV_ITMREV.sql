--------------------------------------------------------
--  DDL for Package Body INV_DIAG_OH_NOREV_ITMREV
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_DIAG_OH_NOREV_ITMREV" as
/* $Header: INVDO04B.pls 120.0.12000000.1 2007/06/22 01:02:00 musinha noship $ */
...
BEGIN
   ...
   IF l_org_id IS NOT NULL THEN
      ...
      dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sqltxt,'Onhand without Revision for item revision controlled');
...
EXCEPTION
  when others then
    ...
    fixInfo := 'Unexpected Exception in INVDO04B.pls';
    isFatal := 'FALSE';
    report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
    reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
END runTest;
...
BEGIN
descStr := 'Items is revision controlled and Onhand without revision';
END getTestDesc;

PROCEDURE getTestName(name OUT NOCOPY VARCHAR2) IS
BEGIN
name := 'Onhand NoRev Item Rev';
END getTestName;
