--------------------------------------------------------
--  DDL for Package Body ARH_DQM_SRCH_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ARH_DQM_SRCH_PKG" AS
/*$Header: ARHDSRHB.pls 120.5 2005/06/16 21:11:28 jhuang noship $*/
...
--fnd_client_info.set_org_context('458');
  ...
  -------------------------------------
  IF NOT arh_dqm_srch_pkg.IsNull(l_party_site) THEN
    l_party_site_list(1)      := l_party_site;
  END IF;

  IF NOT arh_dqm_srch_pkg.IsNull(l_contact) THEN
    l_contact_list(1)         := l_contact;
  END IF;

  l_contact_point_list      := arh_dqm_srch_pkg.Contact_point_list( p_contact_point =>l_contact_point);
