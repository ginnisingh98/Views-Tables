--------------------------------------------------------
--  DDL for Package Body PER_CPN_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CPN_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_cpn_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy per_cpn_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the per_competences Row
  -- ngundura made changes as per pa requirements
  -- competence_alias added in the update statement
  update per_competences
  set
  ...
  certification_required            = p_rec.certification_required,
  evaluation_method                 = p_rec.evaluation_method,
  renewal_period_frequency          = p_rec.renewal_period_frequency,
  renewal_period_units              = p_rec.renewal_period_units,
  min_level			    = p_rec.min_level,
  max_level			    = p_rec.max_level,
  ...
  competence_definition_id          = p_rec.competence_definition_id
 ,competence_cluster                = p_rec.competence_cluster
 ,unit_standard_id                  = p_rec.unit_standard_id
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in per_cpn_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update(p_rec in per_cpn_shd.g_rec_type) is
--
...
Begin
  ...
  --
  begin
  -- ngundura made changes as per pa requirement
  -- added two more parameters for competence_alias
     per_cpn_rku.after_update	(
      ...
      p_max_level              => p_rec.max_level               ,
      p_min_level              => p_rec.min_level               ,
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy per_cpn_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    per_cpn_shd.g_old_rec.name;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    per_cpn_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.description = hr_api.g_varchar2) then
    p_rec.description :=
    per_cpn_shd.g_old_rec.description;
  End If;
  If (p_rec.date_from = hr_api.g_date) then
    p_rec.date_from :=
    per_cpn_shd.g_old_rec.date_from;
  End If;
  If (p_rec.date_to = hr_api.g_date) then
    p_rec.date_to :=
    per_cpn_shd.g_old_rec.date_to;
  End If;
  If (p_rec.behavioural_indicator = hr_api.g_varchar2) then
    p_rec.behavioural_indicator :=
    per_cpn_shd.g_old_rec.behavioural_indicator;
  End If;
  If (p_rec.certification_required = hr_api.g_varchar2) then
    p_rec.certification_required :=
    per_cpn_shd.g_old_rec.certification_required;
  End If;
  If (p_rec.evaluation_method = hr_api.g_varchar2) then
    p_rec.evaluation_method :=
    per_cpn_shd.g_old_rec.evaluation_method;
  End If;
  If (p_rec.renewal_period_frequency = hr_api.g_number) then
    p_rec.renewal_period_frequency :=
    per_cpn_shd.g_old_rec.renewal_period_frequency;
  End If;
  If (p_rec.renewal_period_units = hr_api.g_varchar2) then
    p_rec.renewal_period_units :=
    per_cpn_shd.g_old_rec.renewal_period_units;
  End If;
  If (p_rec.min_level = hr_api.g_number) then
    p_rec.min_level :=
    per_cpn_shd.g_old_rec.min_level;
  End If;
  If (p_rec.max_level = hr_api.g_number) then
    p_rec.max_level :=
    per_cpn_shd.g_old_rec.max_level;
  End If;
  If (p_rec.rating_scale_id = hr_api.g_number) then
    p_rec.rating_scale_id :=
    per_cpn_shd.g_old_rec.rating_scale_id;
  End If;
  If (p_rec.attribute_category = hr_api.g_varchar2) then
    p_rec.attribute_category :=
    per_cpn_shd.g_old_rec.attribute_category;
  End If;
  If (p_rec.attribute1 = hr_api.g_varchar2) then
    p_rec.attribute1 :=
    per_cpn_shd.g_old_rec.attribute1;
  End If;
  If (p_rec.attribute2 = hr_api.g_varchar2) then
    p_rec.attribute2 :=
    per_cpn_shd.g_old_rec.attribute2;
  End If;
  If (p_rec.attribute3 = hr_api.g_varchar2) then
    p_rec.attribute3 :=
    per_cpn_shd.g_old_rec.attribute3;
  End If;
  If (p_rec.attribute4 = hr_api.g_varchar2) then
    p_rec.attribute4 :=
    per_cpn_shd.g_old_rec.attribute4;
  End If;
  If (p_rec.attribute5 = hr_api.g_varchar2) then
    p_rec.attribute5 :=
    per_cpn_shd.g_old_rec.attribute5;
  End If;
  If (p_rec.attribute6 = hr_api.g_varchar2) then
    p_rec.attribute6 :=
    per_cpn_shd.g_old_rec.attribute6;
  End If;
  If (p_rec.attribute7 = hr_api.g_varchar2) then
    p_rec.attribute7 :=
    per_cpn_shd.g_old_rec.attribute7;
  End If;
  If (p_rec.attribute8 = hr_api.g_varchar2) then
    p_rec.attribute8 :=
    per_cpn_shd.g_old_rec.attribute8;
  End If;
  If (p_rec.attribute9 = hr_api.g_varchar2) then
    p_rec.attribute9 :=
    per_cpn_shd.g_old_rec.attribute9;
  End If;
  If (p_rec.attribute10 = hr_api.g_varchar2) then
    p_rec.attribute10 :=
    per_cpn_shd.g_old_rec.attribute10;
  End If;
  If (p_rec.attribute11 = hr_api.g_varchar2) then
    p_rec.attribute11 :=
    per_cpn_shd.g_old_rec.attribute11;
  eND iF;
  If (p_rec.attribute12 = hr_api.g_varchar2) then
    p_rec.attribute12 :=
    per_cpn_shd.g_old_rec.attribute12;
  End If;
  If (p_rec.attribute13 = hr_api.g_varchar2) then
    p_rec.attribute13 :=
    per_cpn_shd.g_old_rec.attribute13;
  End If;
  If (p_rec.attribute14 = hr_api.g_varchar2) then
    p_rec.attribute14 :=
    per_cpn_shd.g_old_rec.attribute14;
  End If;
  If (p_rec.attribute15 = hr_api.g_varchar2) then
    p_rec.attribute15 :=
    per_cpn_shd.g_old_rec.attribute15;
  End If;
  If (p_rec.attribute16 = hr_api.g_varchar2) then
    p_rec.attribute16 :=
    per_cpn_shd.g_old_rec.attribute16;
  End If;
  If (p_rec.attribute17 = hr_api.g_varchar2) then
    p_rec.attribute17 :=
    per_cpn_shd.g_old_rec.attribute17;
  End If;
  If (p_rec.attribute18 = hr_api.g_varchar2) then
    p_rec.attribute18 :=
    per_cpn_shd.g_old_rec.attribute18;
  End If;
  If (p_rec.attribute19 = hr_api.g_varchar2) then
    p_rec.attribute19 :=
    per_cpn_shd.g_old_rec.attribute19;
  End If;
  If (p_rec.attribute20 = hr_api.g_varchar2) then
    p_rec.attribute20 :=
    per_cpn_shd.g_old_rec.attribute20;
  End If;
  -- ngundura changes as per pa requirements
  If (p_rec.competence_alias = hr_api.g_varchar2) then
    p_rec.competence_alias :=
    per_cpn_shd.g_old_rec.competence_alias;
  End If;
  If (p_rec.competence_definition_id = hr_api.g_number) then
    p_rec.competence_definition_id :=
    per_cpn_shd.g_old_rec.competence_definition_id;
  End If;
  --
  -- BUG3356369
  --
  If (p_rec.competence_cluster = hr_api.g_varchar2) then
    p_rec.competence_cluster :=
    per_cpn_shd.g_old_rec.competence_cluster;
  End If;
  If (p_rec.unit_standard_id = hr_api.g_varchar2) then
    p_rec.unit_standard_id :=
    per_cpn_shd.g_old_rec.unit_standard_id;
  End If;
  If (p_rec.credit_type = hr_api.g_varchar2) then
    p_rec.credit_type :=
    per_cpn_shd.g_old_rec.credit_type;
  End If;
  If (p_rec.credits = hr_api.g_number) then
    p_rec.credits :=
    per_cpn_shd.g_old_rec.credits;
  End If;
  If (p_rec.level_type = hr_api.g_varchar2) then
    p_rec.level_type :=
    per_cpn_shd.g_old_rec.level_type;
  End If;
  If (p_rec.level_number = hr_api.g_number) then
    p_rec.level_number :=
    per_cpn_shd.g_old_rec.level_number;
  End If;
  If (p_rec.field      = hr_api.g_varchar2) then
    p_rec.field      :=
    per_cpn_shd.g_old_rec.field     ;
  End If;
  If (p_rec.sub_field = hr_api.g_varchar2) then
    p_rec.sub_field :=
    per_cpn_shd.g_old_rec.sub_field;
  End If;
  If (p_rec.provider = hr_api.g_varchar2) then
    p_rec.provider :=
    per_cpn_shd.g_old_rec.provider;
  End If;
  If (p_rec.qa_organization = hr_api.g_varchar2) then
    p_rec.qa_organization :=
    per_cpn_shd.g_old_rec.qa_organization;
  End If;
  If (p_rec.information_category = hr_api.g_varchar2) then
    p_rec.information_category :=
    per_cpn_shd.g_old_rec.information_category;
  End If;
  If (p_rec.information1 = hr_api.g_varchar2) then
    p_rec.information1 :=
    per_cpn_shd.g_old_rec.information1;
  End If;
  If (p_rec.information2 = hr_api.g_varchar2) then
    p_rec.information2 :=
    per_cpn_shd.g_old_rec.information2;
  End If;
  If (p_rec.information3 = hr_api.g_varchar2) then
    p_rec.information3 :=
    per_cpn_shd.g_old_rec.information3;
  End If;
  If (p_rec.information4 = hr_api.g_varchar2) then
    p_rec.information4 :=
    per_cpn_shd.g_old_rec.information4;
  End If;
  If (p_rec.information5 = hr_api.g_varchar2) then
    p_rec.information5 :=
    per_cpn_shd.g_old_rec.information5;
  End If;
  If (p_rec.information6 = hr_api.g_varchar2) then
    p_rec.information6 :=
    per_cpn_shd.g_old_rec.information6;
  End If;
  If (p_rec.information7 = hr_api.g_varchar2) then
    p_rec.information7 :=
    per_cpn_shd.g_old_rec.information7;
  End If;
  If (p_rec.information8 = hr_api.g_varchar2) then
    p_rec.information8 :=
    per_cpn_shd.g_old_rec.information8;
  End If;
  If (p_rec.information9 = hr_api.g_varchar2) then
    p_rec.information9 :=
    per_cpn_shd.g_old_rec.information9;
  End If;
  If (p_rec.information10 = hr_api.g_varchar2) then
    p_rec.information10 :=
    per_cpn_shd.g_old_rec.information10;
  End If;
  If (p_rec.information11 = hr_api.g_varchar2) then
    p_rec.information11 :=
    per_cpn_shd.g_old_rec.information11;
  End If;
  If (p_rec.information12 = hr_api.g_varchar2) then
    p_rec.information12 :=
    per_cpn_shd.g_old_rec.information12;
  End If;
  If (p_rec.information13 = hr_api.g_varchar2) then
    p_rec.information13 :=
    per_cpn_shd.g_old_rec.information13;
  End If;
  If (p_rec.information14 = hr_api.g_varchar2) then
    p_rec.information14 :=
    per_cpn_shd.g_old_rec.information14;
  End If;
  If (p_rec.information15 = hr_api.g_varchar2) then
    p_rec.information15 :=
    per_cpn_shd.g_old_rec.information15;
  End If;
  If (p_rec.information16 = hr_api.g_varchar2) then
    p_rec.information16 :=
    per_cpn_shd.g_old_rec.information16;
  End If;
  If (p_rec.information17 = hr_api.g_varchar2) then
    p_rec.information17 :=
    per_cpn_shd.g_old_rec.information17;
  End If;
  If (p_rec.information18 = hr_api.g_varchar2) then
    p_rec.information18 :=
    per_cpn_shd.g_old_rec.information18;
  End If;
  If (p_rec.information19 = hr_api.g_varchar2) then
    p_rec.information19 :=
    per_cpn_shd.g_old_rec.information19;
  End If;
  If (p_rec.information20 = hr_api.g_varchar2) then
    p_rec.information20 :=
    per_cpn_shd.g_old_rec.information20;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  per_cpn_bus.update_validate(p_rec,p_effective_date);
  --
...
Procedure upd
  ...
  p_min_level			 in number	     default hr_api.g_number,
  p_max_level                    in number           default hr_api.g_number,
...
Begin
 ...
 ,p_competence_cluster                -- BUG3356369
 ,p_unit_standard_id
 ,p_credit_type
 ,p_credits
 ,p_level_type
