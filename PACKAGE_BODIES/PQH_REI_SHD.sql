--------------------------------------------------------
--  DDL for Package Body PQH_REI_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_REI_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_rei_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PQH_ROLE_EXTRA_INFO_FK1') Then
    hr_utility.set_message(800, 'HR_INV_INFO_TYPE');
    hr_utility.raise_error;
  Elsif (p_constraint_name = 'PQH_ROLE_EXTRA_INFO_FK2') Then
    hr_utility.set_message(8302, 'PQH_INV_ROLE_ID');
    hr_utility.raise_error;
  ElsIf (p_constraint_name = 'PQH_ROLE_EXTRA_INFO_PK') Then
...
Begin
  --
  If (p_role_extra_info_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_role_extra_info_id
        = pqh_rei_shd.g_old_rec.role_extra_info_id and
        p_object_version_number
        = pqh_rei_shd.g_old_rec.object_version_number
       ) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into pqh_rei_shd.g_old_rec;
...
Exception
  When HR_Api.Object_Locked then
    ...
    hr_utility.set_message_token('TABLE_NAME', 'pqh_role_extra_info');
    hr_utility.raise_error;
End lck;
