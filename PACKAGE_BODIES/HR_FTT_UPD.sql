--------------------------------------------------------
--  DDL for Package Body HR_FTT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_FTT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_ftt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_ftt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_form_tab_pages_tl Row
  --
  update hr_form_tab_pages_tl
    set
     form_tab_page_id                = p_rec.form_tab_page_id
    ,language                        = p_rec.language
    ,source_lang                     = p_rec.source_lang
    ,user_tab_page_name              = p_rec.user_tab_page_name
...
Begin
  ...
  begin
    ...
    hr_ftt_rku.after_update
      ...
      ,p_source_lang
      => p_rec.source_lang
      ,p_user_tab_page_name
      => p_rec.user_tab_page_name
      ,p_description
      => p_rec.description
      ,p_source_lang_o
      => hr_ftt_shd.g_old_rec.source_lang
      ,p_user_tab_page_name_o
      => hr_ftt_shd.g_old_rec.user_tab_page_name
      ,p_description_o
      => hr_ftt_shd.g_old_rec.description
      );
    --
  exception
    --
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'HR_FORM_TAB_PAGES_TL'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    ...
    hr_ftt_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.user_tab_page_name = hr_api.g_varchar2) then
    p_rec.user_tab_page_name :=
    hr_ftt_shd.g_old_rec.user_tab_page_name;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  hr_ftt_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hr_ftt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_ftt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_ftt_upd.post_update
     (p_rec
     );
End upd;
--
...
Procedure upd_tl
  ...
  cursor csr_upd_langs is
    select ftt.language
     ...
     where ftt.form_tab_page_id = p_form_tab_page_id
       and p_language_code in (ftt.language
                              ,ftt.source_lang);
  --
