--------------------------------------------------------
--  DDL for Package Body AME_ACF_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ACF_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_acf_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  begin
    ...
    ame_acf_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_voting_regime_o
      => ame_acf_shd.g_old_rec.voting_regime
      ,p_order_number_o
      => ame_acf_shd.g_old_rec.order_number
      ,p_chain_ordering_mode_o
      => ame_acf_shd.g_old_rec.chain_ordering_mode
      ,p_start_date_o
      => ame_acf_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_acf_shd.g_old_rec.end_date
...
Begin
  ...
  --
  If (p_rec.voting_regime = hr_api.g_varchar2) then
    p_rec.voting_regime :=
    ame_acf_shd.g_old_rec.voting_regime;
  End If;
  If (p_rec.order_number = hr_api.g_number) then
    ...
    ame_acf_shd.g_old_rec.order_number;
  End If;
  If (p_rec.chain_ordering_mode = hr_api.g_varchar2) then
    p_rec.chain_ordering_mode :=
    ame_acf_shd.g_old_rec.chain_ordering_mode;
  End If;
  --
...
Begin
  ...
  --
  ame_acf_upd.convert_defs(p_rec);
  --
  ame_acf_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
