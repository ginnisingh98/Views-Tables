--------------------------------------------------------
--  DDL for Package Body ARP_PROCESS_CREDIT_UPD_COVER
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ARP_PROCESS_CREDIT_UPD_COVER" AS
/* $Header: ARTCCMUB.pls 115.8 2003/10/10 14:27:37 mraymond ship $ */
...
BEGIN
    IF PG_DEBUG in ('Y', 'C') THEN
       arp_util.debug('arp_process_credit_cover.update_header_cover()+');
    END IF;
   ...
   l_cm_header.doc_sequence_id          := p_doc_sequence_id; -- Bug 1115787 doc_sequence_id also is added
   ...
   l_cm_header.comments := p_comments;
    ...
    --
    arp_process_credit.update_header(
             p_form_name,
             p_form_version,
             p_customer_trx_id,
             l_cm_header,
             ...
             p_freight_amount,
             p_credit_amount,
             p_cr_txn_invoicing_rule_id,
             p_rederive_credit_info,
             p_rerun_aa,
             p_rerun_cm_module,
             p_compute_tax,
             p_tax_percent,
