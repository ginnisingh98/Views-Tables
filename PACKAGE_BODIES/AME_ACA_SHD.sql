--------------------------------------------------------
--  DDL for Package Body AME_ACA_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ACA_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_aca_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_CALLING_APPS_PK') Then
...
Begin
  ...
  Else
    If (p_application_id =
        ame_aca_shd.g_old_rec.application_id and
        p_object_version_number =
        ame_aca_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_aca_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_aca_shd.g_old_rec.end_date;
...
Begin
  ...
  --
  l_rec.fnd_application_id               := p_fnd_application_id;
  ...
  l_rec.line_item_id_query               := p_line_item_id_query;
