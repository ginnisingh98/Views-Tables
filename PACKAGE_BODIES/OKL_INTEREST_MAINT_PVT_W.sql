--------------------------------------------------------
--  DDL for Package Body OKL_INTEREST_MAINT_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_INTEREST_MAINT_PVT_W" as
  /* $Header: OKLEINMB.pls 120.1 2005/07/11 12:50:42 dkagrawa noship $ */
  ...
  function overlap_exists(p_idx_type  VARCHAR2
    , p_idx_name  VARCHAR2
    , p_datetime_valid  date
    , p_datetime_invalid  date
  ) return varchar2

  as
    ddp_datetime_valid date;
    ddp_datetime_invalid date;
  ...
  begin
    ...
    ddp_datetime_valid := rosetta_g_miss_date_in_map(p_datetime_valid);

    ddp_datetime_invalid := rosetta_g_miss_date_in_map(p_datetime_invalid);

    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := okl_interest_maint_pvt.overlap_exists(p_idx_type,
      p_idx_name,
      ddp_datetime_valid,
      ddp_datetime_invalid);
  ...
  function overlap_exists(p_idx_id  NUMBER
    , p_datetime_valid  date
    , p_datetime_invalid  date
  ) return varchar2
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := okl_interest_maint_pvt.overlap_exists(p_idx_id,
      ddp_datetime_valid,
      ddp_datetime_invalid);
  ...
  as
    ddp_idxv_rec okl_interest_maint_pvt.idxv_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_interest_maint_pvt.int_hdr_ins_updt(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_idxv_tbl okl_interest_maint_pvt.idxv_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
  ...
  as
    ddp_ivev_rec okl_interest_maint_pvt.ivev_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_interest_maint_pvt.int_dtl_ins_updt(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_ivev_tbl okl_interest_maint_pvt.ivev_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
