--------------------------------------------------------
--  DDL for Package Body WIP_BIS_YDSP_ALERT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WIP_BIS_YDSP_ALERT" AS
/* $Header: wipbiaqb.pls 115.11 2004/04/14 10:56:42 achandak ship $ */
...
PROCEDURE PostLevelActuals( target_level_id  in number,
  ...
  /* FROM clause */
  p_from              VARCHAR2(1000) := 'WIP_BIS_PROD_DEPT_YIELD_V';
  p_from_geo          VARCHAR2(1000) := 'sys.dual';

  /* FROM clause */
  p_where_geo         VARCHAR2(1000) := '1=1 ';
...
BEGIN
  ...
  if dimension1_level = 2 then
    ...
    p_from_geo := 'bis_territory_hierarchies_v ';
    p_where_geo := 'TEMP.country_code = bth.child_territory_code ' ||
                   'and bth.parent_territory_type = ''AREA'' ' ||
    ...
    p_where_geo := 'TEMP.country_code = bth.child_territory_code ' ||
                   'and bth.parent_territory_type = ''AREA'' ' ||
                   'and bth.child_territory_type = ''COUNTRY'' ';
    p_groupby_geo := 'bth.parent_territory_code, TEMP.country_code ';
  end if;


  /* PRODUCT */

  if dimension2_level = 1 then
     p_from                 := 'WIP_BIS_PROD_DEPT_YIELD_V';
  else
     p_from                 := 'WIP_BIS_PROD_ASSY_YIELD_V';
  end if;
...
BEGIN
    ...
    l_subject := fnd_message.get_string('WIP', 'WIP_BIS_WK_SUBJECT');
    ...
    l_prod_cat := fnd_message.get_string('WIP', 'WIP_PROD_CATEGORY')
           || ': ' || prod;
    l_prod := fnd_message.get_string('BOM', 'CST_PRODUCT_LABEL') ||
           ': ' || item;
    ...
    l_param := 'P_PARAM_FROM_DATE=' ||
                ...
                '*P_PARAM_ORG_LEVEL=' || org_level ||
                '*P_PARAM_ORGANIZATION=' || org_level_value ||
                '*P_PARAM_GEO_LEVEL=' || dimension1_level ||
                '*P_PARAM_GEOGRAPHY=' || dimension1_level_value ||
                '*P_PARAM_PROD_LEVEL=' || dimension2_level ||
                '*P_PARAM_PRODUCT=' || dimension2_level_value ||
                '*P_PARAM_PLAN_ID=' || plan_id ||
                '*P_PARAM_VIEW_BY=' || 1 ||
                '*paramform=NO*';

    if dimension2_level = 3 then  /* item */
        l_param := l_param || 'P_PARAM_CAT_ID=' || prod_id || '*';
    end if;
...
BEGIN
  ...
  if dimension1_level = 2 then    /* area */
      ...
      p_from_geo := 'bis_territory_hierarchies_v';
      p_context_geo := 'AREA';
      p_where_trgt_geo := 'and TEMP.country_code = bth.child_territory_code ' ||
                          'and bth.parent_territory_type = ''AREA'' ' ||
                          'and bth.child_territory_type = ''COUNTRY'' ' ||
                          'and trgt.dim1_level_value_id = ' ||
                          'bth.parent_territory_code ';
      ...
      p_where_trgt_geo := 'and TEMP.country_code = bth.child_territory_code ' ||
                          'and bth.parent_territory_type = ''AREA'' ' ||
                          'and bth.child_territory_type = ''COUNTRY'' ' ||
                          'and WIP_BIS_COMMON.get_segment(trgt.dim1_level_value_id, ''+'',' ||
                          ' 1) = bth.child_territory_code ';
      p_groupby_geo := 'bth.parent_territory_code, ' ||
                       'bth.parent_territory_name, ' ||
                       'TEMP.country_code, bth.child_territory_name ';
  end if;
...
BEGIN
  ...
  FOR tl_rec in get_target_level LOOP
    ...
    WIP_BIS_YDSP_ALERT.PostLevelActuals(tl_rec.target_level_id,
                                        tl_rec.time_value,
                                        tl_rec.org_value,
                                tl_rec.geo_value,
                                        tl_rec.prod_value);
    WIP_BIS_YDSP_ALERT.CompareLevelTarget(tl_rec.target_level_id,
                                          tl_rec.time_value,
                                  tl_rec.org_value,
                                          tl_rec.geo_value,
                                  tl_rec.prod_value);
