--------------------------------------------------------
--  DDL for Package Body ASO_PARTY_RLTSHIP_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ASO_PARTY_RLTSHIP_PVT" as
/* $Header: asovparb.pls 120.1 2005/06/29 12:42:44 appldev ship $ */
-- Start of Comments
-- Package name     : ASO_PARTY_RLTSHIP_PVT
-- Purpose          :
-- History          :
-- NOTE             :
-- End of Comments
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'ASO_PARTY_RLTSHIP_PVT';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'asovparb.pls';

G_USER_ID         NUMBER := FND_GLOBAL.USER_ID;
G_LOGIN_ID        NUMBER := FND_GLOBAL.CONC_LOGIN_ID;

-- Hint: Primary key needs to be returned.
PROCEDURE Create_party_rltship(
...
--l_identity_sales_member_rec AS_SALES_MEMBER_PUB.Sales_member_rec_Type;
 BEGIN
     ...
     -- ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW, 'Private API: ' || l_api_name || 'start');
      ...
      THEN
          -- Debug message
 --         ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW,'Private API: Validate_party_rltship');

          -- Invoke validation procedures
          Validate_party_rltship(
   ...
   --   ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW, 'Private API: Calling create table handler');
      ...
      -- Invoke table handler(ASO_PARTY_RELATIONSHIPS_PKG.Insert_Row)
	 ...
	 p_OBJECT_VERSION_NUMBER => p_PARTY_RLTSHIP_rec.OBJECT_VERSION_NUMBER);
      -- x_PARTY_RELATIONSHIP_ID := px_PARTY_RELATIONSHIP_ID;
...
-- Local Variables
--l_identity_sales_member_rec   AS_SALES_MEMBER_PUB.Sales_member_rec_Type;
l_ref_PARTY_RLTSHIP_rec  ASO_quote_pub.PARTY_RLTSHIP_Rec_Type;
l_tar_PARTY_RLTSHIP_rec  ASO_quote_pub.PARTY_RLTSHIP_Rec_Type := P_PARTY_RLTSHIP_Rec;
l_rowid  ROWID;
 BEGIN
      -- Standard Start of API savepoint
 ...
 --     ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW, 'Private API: ' || l_api_name || 'start');
      ...
      --
-- Un-comment the following statements when AS_CALLOUT_PKG is ready.
      Open C_Get_party_rltship( l_tar_PARTY_RLTSHIP_rec.PARTY_RELATIONSHIP_ID);

      Fetch C_Get_party_rltship into
       --        l_rowid,
               l_ref_PARTY_RLTSHIP_rec.PARTY_RELATIONSHIP_ID,
               l_ref_PARTY_RLTSHIP_rec.CREATION_DATE,
               l_ref_PARTY_RLTSHIP_rec.CREATED_BY,
       ...
       If ( C_Get_party_rltship%NOTFOUND) Then
           IF FND_MSG_PUB.Check_Msg_Level (FND_MSG_PUB.G_MSG_LVL_ERROR)
           THEN
               FND_MESSAGE.Set_Name('ASO', 'API_MISSING_UPDATE_TARGET');
               FND_MESSAGE.Set_Token ('INFO', 'party_rltship', FALSE);
               FND_MSG_PUB.Add;
           END IF;
           raise FND_API.G_EXC_ERROR;
       END IF;
...
--      ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW, 'Private API: Calling update table handler');
      ...
      -- Invoke table handler(ASO_PARTY_RELATIONSHIPS_PKG.Update_Row)
...
--l_identity_sales_member_rec  AS_SALES_MEMBER_PUB.Sales_member_rec_Type;
 ...
 --     ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW, 'Private API: ' || l_api_name || 'start');
      ...
      --
-- Un-comment the following statements when AS_CALLOUT_PKG is ready.
      -- Invoke table handler(ASO_PARTY_RELATIONSHIPS_PKG.Delete_Row)
...
PROCEDURE Validate_PARTY_RLTSHIP_ID (
    P_Init_Msg_List              IN   VARCHAR2     := FND_API.G_FALSE,
...
BEGIN
      ...
      IF(p_PARTY_RELATIONSHIP_ID is NULL)
      THEN
          --ASO_UTILITY_PVT.Print('ERROR', 'Private party_rltship API: -Violate NOT NULL constraint(PARTY_RELATIONSHIP_ID)');
      ...
      THEN
          -- Hint: Validate data
          -- IF p_PARTY_RELATIONSHIP_ID is not NULL and p_PARTY_RELATIONSHIP_ID <> G_MISS_CHAR
...
PROCEDURE Validate_QUOTE_OBJECT_TYPE (
    P_Init_Msg_List              IN   VARCHAR2     := FND_API.G_FALSE,
    P_Validation_mode            IN   VARCHAR2,
    P_QUOTE_OBJECT_TYPE                IN   VARCHAR2,
    X_Return_Status              OUT NOCOPY /* file.sql.39 change */    VARCHAR2,
    X_Msg_Count                  OUT NOCOPY /* file.sql.39 change */    NUMBER,
    X_Msg_Data                   OUT NOCOPY /* file.sql.39 change */    VARCHAR2
    )
...
BEGIN
      ...
      THEN
          -- Hint: Validate data
          -- IF p_QUOTE_OBJECT_TYPE is not NULL and p_QUOTE_OBJECT_TYPE <> G_MISS_CHAR
          -- verify if data is valid
          -- if data is not valid : x_return_status := FND_API.G_RET_STS_ERROR;
          NULL;
      ELSIF(p_validation_mode = ASO_UTILITY_PVT.G_UPDATE)
...
END Validate_QUOTE_OBJECT_TYPE;
...
BEGIN
      ...
      THEN
          --ASO_UTILITY_PVT.Print('ERROR', 'Private party_rltship API: -Violate NOT NULL constraint(RELATIONSHIP_TYPE_CODE)');
          x_return_status := FND_API.G_RET_STS_ERROR;
      END IF;
...
PROCEDURE Validate_RELATED_OBJ_TYPE_CODE (
    P_Init_Msg_List              IN   VARCHAR2     := FND_API.G_FALSE,
    P_Validation_mode            IN   VARCHAR2,
    P_RELATED_OBJECT_TYPE_CODE                IN   VARCHAR2,
    X_Return_Status              OUT NOCOPY /* file.sql.39 change */    VARCHAR2,
    X_Msg_Count                  OUT NOCOPY /* file.sql.39 change */    NUMBER,
    X_Msg_Data                   OUT NOCOPY /* file.sql.39 change */    VARCHAR2
    )
...
BEGIN
      ...
      THEN
          -- Hint: Validate data
          -- IF p_RELATED_OBJECT_TYPE_CODE is not NULL and p_RELATED_OBJECT_TYPE_CODE <> G_MISS_CHAR
          -- verify if data is valid
          -- if data is not valid : x_return_status := FND_API.G_RET_STS_ERROR;
          NULL;
      ELSIF(p_validation_mode = ASO_UTILITY_PVT.G_UPDATE)
...
END Validate_RELATED_OBJ_TYPE_CODE;
...
BEGIN
      ...
      -- validate NOT NULL column
      IF(p_QUOTE_HEADER_ID is NULL)
      THEN
          --ASO_UTILITY_PVT.Print('ERROR', 'Private party_rltship API: -Violate NOT NULL constraint(QUOTE_HEADER_ID)');
          x_return_status := FND_API.G_RET_STS_ERROR;
      END IF;
      ...
      THEN
          -- Hint: Validate data
          -- IF p_QUOTE_HEADER_ID is not NULL and p_QUOTE_HEADER_ID <> G_MISS_CHAR
          -- verify if data is valid
          -- if data is not valid : x_return_status := FND_API.G_RET_STS_ERROR;
          NULL;
      ELSIF(p_validation_mode = ASO_UTILITY_PVT.G_UPDATE)
...
BEGIN
      ...
      THEN
          --ASO_UTILITY_PVT.Print('ERROR', 'Private party_rltship API: -Violate NOT NULL constraint(QUOTE_LINE_ID)');
          x_return_status := FND_API.G_RET_STS_ERROR;
      END IF;
...
BEGIN
      ...
      THEN
          --ASO_UTILITY_PVT.Print('ERROR', 'Private party_rltship API: -Violate NOT NULL constraint(OBJECT_TYPE_CODE)');
          x_return_status := FND_API.G_RET_STS_ERROR;
      END IF;
...
BEGIN
      ...
      THEN
          --ASO_UTILITY_PVT.Print('ERROR', 'Private party_rltship API: -Violate NOT NULL constraint(OBJECT_ID)');
          x_return_status := FND_API.G_RET_STS_ERROR;
      END IF;
...
PROCEDURE Validate_PARTY_RLTSHIP_rec(
    P_Init_Msg_List              IN   VARCHAR2     := FND_API.G_FALSE,
    P_Validation_mode            IN   VARCHAR2,
    P_PARTY_RLTSHIP_Rec     IN    ASO_quote_PUB.PARTY_RLTSHIP_Rec_Type,
    X_Return_Status              OUT NOCOPY /* file.sql.39 change */    VARCHAR2,
...
END Validate_PARTY_RLTSHIP_Rec;

PROCEDURE Validate_party_rltship(
    P_Init_Msg_List              IN   VARCHAR2     := FND_API.G_FALSE,
    P_Validation_level           IN   NUMBER := FND_API.G_VALID_LEVEL_FULL,
    P_Validation_mode            IN   VARCHAR2,
    P_PARTY_RLTSHIP_Rec     IN    ASO_quote_PUB.PARTY_RLTSHIP_Rec_Type,
...
IS
l_api_name   CONSTANT VARCHAR2(30) := 'Validate_party_rltship';
 BEGIN
...
--      --ASO_UTILITY_PVT.Print(FND_MSG_PUB.G_MSG_LVL_DEBUG_LOW, 'Private API: ' || l_api_name || 'start');
  ...
  /*        Validate_REQUEST_ID(
          ...
          Validate_PROGRAM_APPLICATION_ID(
              p_init_msg_list          => FND_API.G_FALSE,
              p_validation_mode        => p_validation_mode,
              p_PROGRAM_APPLICATION_ID   => P_PARTY_RLTSHIP_Rec.PROGRAM_APPLICATION_ID,
              x_return_status          => x_return_status,
          ...
          Validate_QUOTE_OBJECT_TYPE(
              p_init_msg_list          => FND_API.G_FALSE,
              p_validation_mode        => p_validation_mode,
              p_QUOTE_OBJECT_TYPE   => P_PARTY_RLTSHIP_Rec.QUOTE_OBJECT_TYPE,
              x_return_status          => x_return_status,
              x_msg_count              => x_msg_count,
              x_msg_data               => x_msg_data);
          IF x_return_status <> FND_API.G_RET_STS_SUCCESS THEN
          ...
          Validate_QUOTE_OBJECT_ID(
              p_init_msg_list          => FND_API.G_FALSE,
              p_validation_mode        => p_validation_mode,
              p_QUOTE_OBJECT_ID   => P_PARTY_RLTSHIP_Rec.QUOTE_OBJECT_ID,
              x_return_status          => x_return_status,
              x_msg_count              => x_msg_count,
              x_msg_data               => x_msg_data);
          IF x_return_status <> FND_API.G_RET_STS_SUCCESS THEN
...
*/
          ...
          END IF;
/*
          Validate_RELATED_OBJ_TYPE_CODE(
              p_init_msg_list          => FND_API.G_FALSE,
              p_validation_mode        => p_validation_mode,
              p_RELATED_OBJECT_TYPE_CODE   => P_PARTY_RLTSHIP_Rec.RELATED_OBJECT_TYPE_CODE,
              x_return_status          => x_return_status,
              x_msg_count              => x_msg_count,
              x_msg_data               => x_msg_data);
          IF x_return_status <> FND_API.G_RET_STS_SUCCESS THEN
          ...
          Validate_RELATED_OBJECT_ID(
              p_init_msg_list          => FND_API.G_FALSE,
              p_validation_mode        => p_validation_mode,
              p_RELATED_OBJECT_ID   => P_PARTY_RLTSHIP_Rec.RELATED_OBJECT_ID,
              x_return_status          => x_return_status,
              x_msg_count              => x_msg_count,
              x_msg_data               => x_msg_data);
          IF x_return_status <> FND_API.G_RET_STS_SUCCESS THEN
...
END Validate_party_rltship;

End ASO_PARTY_RLTSHIP_PVT;
