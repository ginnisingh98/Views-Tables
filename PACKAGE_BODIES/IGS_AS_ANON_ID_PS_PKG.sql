--------------------------------------------------------
--  DDL for Package Body IGS_AS_ANON_ID_PS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_ANON_ID_PS_PKG" AS
/* $Header: IGSDI64B.pls 115.1 2002/11/28 23:27:01 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_as_anon_id_ps%ROWTYPE;
  new_references igs_as_anon_id_ps%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AS_APS_SCA_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
