--------------------------------------------------------
--  DDL for Package Body INV_DIAG_WDD_STAGED
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_DIAG_WDD_STAGED" AS
/* $Header: INVDP04B.pls 120.0.12000000.1 2007/06/22 01:16:52 musinha noship $ */
  ...
  dummy_num := JTF_DIAGNOSTIC_COREAPI.display_sql(sqltxt,'Details of staged / shipped (and not inv interfaced) serials not marked');
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      fixInfo := 'Unexpected Exception in INVDP04B.pls';
      isFatal := 'FALSE';
      report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
      reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
  END runTest;
  ...
  BEGIN
  descStr := 'Details of Staged delivery details';
  END getTestDesc;
  ...
  BEGIN
    name := 'Details of Staged delivery details';
  END getTestName;
