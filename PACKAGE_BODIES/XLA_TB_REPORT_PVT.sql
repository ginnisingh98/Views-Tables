--------------------------------------------------------
--  DDL for Package Body XLA_TB_REPORT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_TB_REPORT_PVT" AS
/* $Header: xlatbrpt.pkb 120.30.12010000.8 2009/04/17 09:21:54 svellani ship $ */
/*======================================================================+
...
|    21-Oct-08 Vijaya         7376639: Excluded the Boundary Conditions |
|                             for inclusion of applied transactions,for |
|                             which Original Transactions aren't in the |
|                             given start date and end date.            |
|    09-Feb-08 Nitesh         7646239: Introduced parameters            |
...
--
--  Original invoices are not in the date range specified
--  Display Applied Trx Information
--
...
BEGIN
   ...
   --
   -- Assign template sql C_INSERT_GT_SELECT to a local variable
   -- to replace strings
   --
   ...
   IF p_account_from IS NOT NULL THEN
      ...
      --
      -- Applied only transaction for the case invoice is out of the range
      --
   ...
   ELSE
      ...
      --
      -- Display Applied Transactions when the original invoice is outside the range
      --
   ...
   --
   IF p_defn_info.balance_side_code = 'C' THEN
      ...
      --
      -- Display Applied Transactions when the original invoice is outside the range
      -- Do not need $tb_amount_col$ as original invoice amount is always zero.
      --
      l_insert_gt_appld_sql := REPLACE (l_insert_gt_appld_sql
                                 ,'$amount_col$'
                                 ,C_CR_APPLIED_AMT_COL);
   ...
   ELSIF p_defn_info.balance_side_code = 'D' THEN
   ...
   IF l_party_type_code IN ('C','S') THEN
 ...
 /*     l_insert_gt_sql := replace_party_string
   ...
   ELSIF l_party_type_code IS NULL THEN
      ...
      --
      -- Support only supplier for applied
      --
      l_insert_gt_appld_sql := replace_party_string
                                (p_party_id        => l_party_id
                                ,p_party_type_code => 'S'
      ...
      --
      -- 7118890
      -- Support Supplier Only for Performance reasons
      --
   ...
   --
   -- 7118890
   -- Support Supplier Only for Performance
   --
   IF l_party_type_code IS NOT NULL THEN --Added for Bug 7646239
   ...
   END IF; --Added for Bug 7646239

/*   IF l_party_type_code IS NOT NULL THEN
   ...
   ELSE
      ...
      --
      -- Union of two sqls (for 'Customer' and 'Supplier')
      --
...
BEGIN
   ...
   --
   -- replace placeholders for gl_balances
   --
   ...
   --
   --The logic is for the old upgrade strategy
   --
...
BEGIN
   ...
   /*
    bug#8216458 commenting the check that is NVL(p_security_flag,'N') = 'Y'
    If security is defined the p_security_flag is passed default 'Y' by the concurrent program
    and the security context is set using xla_security_pkg.set_security_context(l_application_id).
    For payables and receivables the security context is always set from Accounting Setup -> Subledger Application
    Use Security Option checked and the policy function specified.
    For Cost Management application this setup is not done, p_security_flag is NULL,
    therefore context is not set and OABL report is not fetching any output.
   */
   --IF NVL(p_security_flag,'N') = 'Y' THEN  bug#8216458
