--------------------------------------------------------
--  DDL for Package Body IGS_PS_FAC_WL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_FAC_WL_PKG" AS
/* $Header: IGSPI3JB.pls 120.0 2005/06/01 20:11:29 appldev noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_fac_wl%ROWTYPE;
  new_references igs_ps_fac_wl%ROWTYPE;
  ...
  */
  ...
  ||  (reverse chronological order - newest change first)
  */
  CURSOR cur_hz IS SELECT 1 FROM HZ_PARTIES
                WHERE PARTY_ID = new_references.person_id;
  rec_hz   cur_hz%rowtype;
  BEGIN
    ...
    ELSE
      OPEN cur_hz;
      FETCH cur_hz INTO rec_hz;
      IF cur_hz%NOTFOUND THEN
            fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
            igs_ge_msg_stack.add;
            app_exception.raise_exception;
      END IF;
  ...
  */
  BEGIN
    ...
    igs_ps_fac_asg_task_pkg.get_fk_igs_ps_fac_wl (
      old_references.fac_wl_id
    );

    igs_ps_fac_ovr_wl_pkg.get_fk_igs_ps_fac_wl (
      old_references.fac_wl_id
    );
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_FWL_CI_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_fac_wl_s.NEXTVAL
    INTO      x_fac_wl_id
    FROM      dual;
  ...
  ||  Change History :
  ||  Who             When            What
  ||  sarakshi        08-Apr-2002     column std_exp_wl is made null,as a part of bug:2277132
  ||  (reverse chronological order - newest change first)
  */
    CURSOR c1 IS
      SELECT
  ...
  BEGIN
    ...
    IF (
        ...
        AND (tlinfo.calendar_cat = x_calendar_cat)
