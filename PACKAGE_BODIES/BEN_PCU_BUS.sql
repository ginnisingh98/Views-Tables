--------------------------------------------------------
--  DDL for Package Body BEN_PCU_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCU_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcu_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_per_cm_usg_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   per_cm_usg_id PK of record being inserted or updated.
...
--
Procedure chk_per_cm_usg_id(p_per_cm_usg_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_per_cm_usg_id';
...
Begin
  ...
  --
  l_api_updating := ben_pcu_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_per_cm_usg_id                => p_per_cm_usg_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_per_cm_usg_id,hr_api.g_number)
     <>  ben_pcu_shd.g_old_rec.per_cm_usg_id) then
    --
    -- raise error as PK has changed
    --
    ben_pcu_shd.constraint_error('BEN_PER_CM_USG_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_per_cm_usg_id is not null then
      --
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    --
    If ((nvl(p_cm_typ_usg_id, hr_api.g_number) <> hr_api.g_number) and
...
function return_legislation_code
  (p_per_cm_usg_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_per_cm_usg_f b
    where b.per_cm_usg_id      = p_per_cm_usg_id
