--------------------------------------------------------
--  DDL for Package Body IEM_CONCURRENT_MDT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IEM_CONCURRENT_MDT_PVT" as
/* $Header: iempconb.pls 115.11 2004/06/01 23:36:43 appldev shipped $*/

G_PKG_NAME CONSTANT varchar2(30) :='IEM_CONCURRENT_MDT_PVT ';
...
BEGIN
   ...
   FOR i in 1..p_number_of_threads loop
	 ...
	 l_request_id := fnd_request.submit_request('IEM', 'IEMMDTWW', '',p_delay_worker_start_time,FALSE,1,'F','T','MAILPREPROC','IEM_MAIL','NO_WAIT',p_number_of_msgs);
...
-- Removed Retry portion  KBeagle 20-11-02
...
/*
   if (p_schedule_retry = 'Y') then
    	...
    	l_request_id := fnd_request.submit_request('IEM', 'IEMMDTWR', '','',FALSE,1,'F','T','MAILPREPROC','IEM_MAIL');
...
PROCEDURE SyncFolder(ERRBUF   OUT NOCOPY     VARCHAR2,
 ...
 begin
   ...
   for i in 1..email_account_id_list.count loop
        ...
        EXECUTE IMMEDIATE l_str using OUT NOCOPY l_ret,l_email_user,l_domain,l_pass;
   	    IF l_ret <> 0 THEN
                    ...
                    open v_cur;
                        LOOP
                           ...
                           l_folder := '/'||l_user_name||'/Drafts';
                            ...
                            EXECUTE IMMEDIATE l_str using OUT NOCOPY l_ret,l_folder;
                            IF l_ret <> 0 THEN
