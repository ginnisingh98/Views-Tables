--------------------------------------------------------
--  DDL for Package Body OKC_IGS_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_IGS_PVT" AS
/* $Header: OKCSIGSB.pls 120.0 2005/05/25 23:00:15 appldev noship $ */
...
*/
  ---------------------------------------------------------------------------
  -- FUNCTION get_rec for: OKC_TIME_IG_STARTEND_V
  ---------------------------------------------------------------------------
  FUNCTION get_rec (
    p_igsv_rec                     IN igsv_rec_type,
    x_no_data_found                OUT NOCOPY BOOLEAN
  ) RETURN igsv_rec_type IS
    CURSOR okc_igsv_pk_csr (p_id                 IN NUMBER) IS
    SELECT
            ID,
            OBJECT_VERSION_NUMBER,
--Bug 3122962            SFWT_FLAG,
            ...
            LAST_UPDATE_DATE,
            LAST_UPDATE_LOGIN
      FROM Okc_Time_IG_STARTend_V
     WHERE okc_time_IG_STARTend_v.id = p_id;
    l_okc_igsv_pk                  okc_igsv_pk_csr%ROWTYPE;
    l_igsv_rec                     igsv_rec_type;
  BEGIN
    x_no_data_found := TRUE;
    -- Get current database values
    OPEN okc_igsv_pk_csr (p_igsv_rec.id);
    FETCH okc_igsv_pk_csr INTO
              l_igsv_rec.ID,
              l_igsv_rec.OBJECT_VERSION_NUMBER,
--Bug 3122962              l_igsv_rec.SFWT_FLAG,
              l_igsv_rec.TVE_ID_STARTED,
              l_igsv_rec.TVE_ID_ENDED,
              l_igsv_rec.TVE_ID_LIMITED,
              l_igsv_rec.DNZ_CHR_ID,
              l_igsv_rec.TZE_ID,
              l_igsv_rec.DESCRIPTION,
              l_igsv_rec.SHORT_DESCRIPTION,
              l_igsv_rec.COMMENTS,
              l_igsv_rec.ATTRIBUTE_CATEGORY,
              l_igsv_rec.ATTRIBUTE1,
              l_igsv_rec.ATTRIBUTE2,
              l_igsv_rec.ATTRIBUTE3,
              l_igsv_rec.ATTRIBUTE4,
              l_igsv_rec.ATTRIBUTE5,
              l_igsv_rec.ATTRIBUTE6,
              l_igsv_rec.ATTRIBUTE7,
              l_igsv_rec.ATTRIBUTE8,
              l_igsv_rec.ATTRIBUTE9,
              l_igsv_rec.ATTRIBUTE10,
              l_igsv_rec.ATTRIBUTE11,
              l_igsv_rec.ATTRIBUTE12,
              l_igsv_rec.ATTRIBUTE13,
              l_igsv_rec.ATTRIBUTE14,
              l_igsv_rec.ATTRIBUTE15,
              l_igsv_rec.CREATED_BY,
              l_igsv_rec.CREATION_DATE,
              l_igsv_rec.LAST_UPDATED_BY,
              l_igsv_rec.LAST_UPDATE_DATE,
              l_igsv_rec.LAST_UPDATE_LOGIN;
    x_no_data_found := okc_igsv_pk_csr%NOTFOUND;
    CLOSE okc_igsv_pk_csr;
    RETURN(l_igsv_rec);
  END get_rec;
  ...
  FUNCTION get_rec (
    ...
    l_row_notfound                 BOOLEAN := TRUE;
  BEGIN
    RETURN(get_rec(p_igsv_rec, l_row_notfound));
  END get_rec;

  -------------------------------------------------------------
  -- FUNCTION null_out_defaults for: OKC_TIME_IG_STARTEND_V --
  -------------------------------------------------------------
  FUNCTION null_out_defaults (
    p_igsv_rec	IN igsv_rec_type
  ) RETURN igsv_rec_type IS
    l_igsv_rec	igsv_rec_type := p_igsv_rec;
  BEGIN
    IF (l_igsv_rec.object_version_number = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.object_version_number := NULL;
    END IF;
/*    IF (l_igsv_rec.sfwt_flag = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.sfwt_flag := NULL;
    END IF;
*/
    IF (l_igsv_rec.tve_id_started = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.tve_id_started := NULL;
    END IF;
    IF (l_igsv_rec.tve_id_ended = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.tve_id_ended := NULL;
    END IF;
    IF (l_igsv_rec.tve_id_limited = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.tve_id_limited := NULL;
    END IF;
    IF (l_igsv_rec.dnz_chr_id = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.dnz_chr_id := NULL;
    END IF;
    IF (l_igsv_rec.tze_id = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.tze_id := NULL;
    END IF;
    IF (l_igsv_rec.description = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.description := NULL;
    END IF;
    IF (l_igsv_rec.short_description = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.short_description := NULL;
    END IF;
    IF (l_igsv_rec.comments = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.comments := NULL;
    END IF;
    IF (l_igsv_rec.attribute_category = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute_category := NULL;
    END IF;
    IF (l_igsv_rec.attribute1 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute1 := NULL;
    END IF;
    IF (l_igsv_rec.attribute2 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute2 := NULL;
    END IF;
    IF (l_igsv_rec.attribute3 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute3 := NULL;
    END IF;
    IF (l_igsv_rec.attribute4 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute4 := NULL;
    END IF;
    IF (l_igsv_rec.attribute5 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute5 := NULL;
    END IF;
    IF (l_igsv_rec.attribute6 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute6 := NULL;
    END IF;
    IF (l_igsv_rec.attribute7 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute7 := NULL;
    END IF;
    IF (l_igsv_rec.attribute8 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute8 := NULL;
    END IF;
    IF (l_igsv_rec.attribute9 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute9 := NULL;
    END IF;
    IF (l_igsv_rec.attribute10 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute10 := NULL;
    END IF;
    IF (l_igsv_rec.attribute11 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute11 := NULL;
    END IF;
    IF (l_igsv_rec.attribute12 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute12 := NULL;
    END IF;
    IF (l_igsv_rec.attribute13 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute13 := NULL;
    END IF;
    IF (l_igsv_rec.attribute14 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute14 := NULL;
    END IF;
    IF (l_igsv_rec.attribute15 = OKC_API.G_MISS_CHAR) THEN
      l_igsv_rec.attribute15 := NULL;
    END IF;
    IF (l_igsv_rec.created_by = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.created_by := NULL;
    END IF;
    IF (l_igsv_rec.creation_date = OKC_API.G_MISS_DATE) THEN
      l_igsv_rec.creation_date := NULL;
    END IF;
    IF (l_igsv_rec.last_updated_by = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.last_updated_by := NULL;
    END IF;
    IF (l_igsv_rec.last_update_date = OKC_API.G_MISS_DATE) THEN
      l_igsv_rec.last_update_date := NULL;
    END IF;
    IF (l_igsv_rec.last_update_login = OKC_API.G_MISS_NUM) THEN
      l_igsv_rec.last_update_login := NULL;
    END IF;
    RETURN(l_igsv_rec);
...
*/
  ...
  ---------------------------------------------------------------------------
  -----------------------------------------------------
  -- Validate_Attributes for:OKC_TIME_IG_STARTEND_V --
  -----------------------------------------------------

  PROCEDURE Validate_DNZ_Chr_ID (
      x_return_status                OUT NOCOPY VARCHAR2,
      p_igsv_rec                     IN igsv_rec_type) IS
    ...
    BEGIN
      ...
      IF (p_igsv_rec.dnz_chr_id IS NOT NULL AND
          p_igsv_rec.dnz_chr_id <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_chrv_pk_csr(p_igsv_rec.dnz_chr_id);
  ...
  PROCEDURE Validate_Tve_Id_Started (
      ...
      SELECT  '1'
        FROM OKC_TIMEVALUES
        WHERE id  = p_id
        AND TVE_TYPE IN ('TGD', 'TGN');
      l_okc_tvev_pk                  okc_tvev_pk_csr%ROWTYPE;
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_igsv_rec.TVE_ID_STARTED IS NOT NULL  AND
          p_igsv_rec.TVE_ID_STARTED <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_tvev_pk_csr(p_igsv_rec.TVE_ID_STARTED);
  ...
  PROCEDURE Validate_Tve_Id_Ended (
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_igsv_rec.TVE_ID_ENDED IS NOT NULL  AND
          p_igsv_rec.TVE_ID_ENDED <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_tvev_pk_csr(p_igsv_rec.TVE_ID_ENDED);
        FETCH okc_tvev_pk_csr INTO l_okc_tvev_pk;
        l_row_notfound := okc_tvev_pk_csr%NOTFOUND;
        CLOSE okc_tvev_pk_csr;
        IF (l_row_notfound) THEN
        ...
        END IF;
      ELSE
         OKC_API.set_message(G_APP_NAME, G_REQUIRED_VALUE,G_COL_NAME_TOKEN,'TVE_ID_ENDED');
         x_return_status := OKC_API.G_RET_STS_ERROR;
      END IF;
    EXCEPTION
      WHEN item_not_found_error THEN
  ...
  PROCEDURE Validate_Tve_Id_Limited (
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_igsv_rec.TVE_ID_LIMITED IS NOT NULL AND
          p_igsv_rec.TVE_ID_LIMITED <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_tvev_pk_csr(p_igsv_rec.TVE_ID_LIMITED);
        FETCH okc_tvev_pk_csr INTO l_okc_tvev_pk;
        l_row_notfound := okc_tvev_pk_csr%NOTFOUND;
        CLOSE okc_tvev_pk_csr;
        IF (l_row_notfound) THEN
  ...
  PROCEDURE Validate_Time_Zone_Id (
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_igsv_rec.TZE_ID IS NOT NULL AND
          p_igsv_rec.TZE_ID <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okx_timezones_v_pk_csr(p_igsv_rec.TZE_ID);
...
/*
  PROCEDURE Validate_SFWT_Flag (
    ...
    p_igsv_rec                     IN igsv_rec_type) IS
  BEGIN
    IF upper(p_igsv_rec.sfwt_flag) = 'Y' OR
       upper(p_igsv_rec.sfwt_flag) = 'N'
    THEN
       IF p_igsv_rec.sfwt_flag = 'Y' OR
          p_igsv_rec.sfwt_flag = 'N'
       THEN
         x_return_status := OKC_API.G_RET_STS_SUCCESS;
       ELSE
...
*/
  ...
  BEGIN
    IF p_igsv_rec.id = OKC_API.G_MISS_NUM OR
       p_igsv_rec.id IS NULL
    THEN
      OKC_API.set_message(G_APP_NAME, G_REQUIRED_VALUE,G_COL_NAME_TOKEN,'id');
      x_return_status := OKC_API.G_RET_STS_ERROR;
    ELSIF p_igsv_rec.object_version_number = OKC_API.G_MISS_NUM OR
          p_igsv_rec.object_version_number IS NULL
    THEN
      OKC_API.set_message(G_APP_NAME, G_REQUIRED_VALUE,G_COL_NAME_TOKEN,'object_version_number');
      x_return_status := OKC_API.G_RET_STS_ERROR;
    END IF;
    IF p_igsv_rec.dnz_chr_id <> 0 Then
    ...
    IF (l_return_status <> OKC_API.G_RET_STS_SUCCESS) THEN
      ...
      END IF;
    END IF;
    IF p_igsv_rec.tve_id_limited <> OKC_API.G_MISS_NUM AND
       p_igsv_rec.tve_id_limited IS NOT NULL
    THEN
...
*/
  ...
  ---------------------------------------------------------------------------
  -------------------------------------------------
  -- Validate_Record for:OKC_TIME_IG_STARTEND_V --
  -------------------------------------------------
...
*/
  ...
  ---------------------------------------------------------------------------
  ----------------------------------------------
  -- validate_row for:OKC_TIME_IG_STARTEND_V --
  ----------------------------------------------
  PROCEDURE validate_row(
...
--Bug 3122962    l_okc_timevalues_tl_rec        okc_timevalues_tl_rec_type;
  BEGIN
    ...
    l_return_status := Validate_Attributes(l_igsv_rec);
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := Validate_Record(l_igsv_rec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
  ...
  END validate_row;
  ------------------------------------------
  -- PL/SQL TBL validate_row for:IGSV_TBL --
  ------------------------------------------
  PROCEDURE validate_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
    ...
    x_msg_count                    OUT NOCOPY NUMBER,
    x_msg_data                     OUT NOCOPY VARCHAR2,
    p_igsv_tbl                     IN igsv_tbl_type) IS
    ...
    IF (p_igsv_tbl.COUNT > 0) THEN
      ...
      LOOP
        validate_row (
          ...
          p_igsv_rec                     => p_igsv_tbl(i));
        EXIT WHEN (i = p_igsv_tbl.LAST);
        i := p_igsv_tbl.NEXT(i);
      END LOOP;
    END IF;
  EXCEPTION
...
*/
  --------------------------------------------
  -- insert_row for:OKC_TIME_IG_STARTEND_V --
  --------------------------------------------
  PROCEDURE insert_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
    ...
    x_msg_data                     OUT NOCOPY VARCHAR2,
    p_igsv_rec                     IN igsv_rec_type,
    x_igsv_rec                     OUT NOCOPY igsv_rec_type) IS

    l_api_version                 CONSTANT NUMBER := 1;
    l_api_name                     CONSTANT VARCHAR2(30) := 'V_insert_row';
    l_return_status                VARCHAR2(1) := OKC_API.G_RET_STS_SUCCESS;
    l_igsv_rec                     igsv_rec_type;
    l_def_igsv_rec                 igsv_rec_type;
    l_tve_rec                      tve_rec_type;
...
--Bug 3122962    lx_okc_timevalues_tl_rec       okc_timevalues_tl_rec_type;
    ...
    FUNCTION fill_who_columns (
      ...
      l_igsv_rec	igsv_rec_type := p_igsv_rec;
    BEGIN
      l_igsv_rec.CREATION_DATE := SYSDATE;
      l_igsv_rec.CREATED_BY := FND_GLOBAL.USER_ID;
      l_igsv_rec.LAST_UPDATE_DATE := l_igsv_rec.CREATION_DATE;
      l_igsv_rec.LAST_UPDATED_BY := FND_GLOBAL.USER_ID;
      l_igsv_rec.LAST_UPDATE_LOGIN := FND_GLOBAL.LOGIN_ID;
      RETURN(l_igsv_rec);
    END fill_who_columns;
    ------------------------------------------------
    -- Set_Attributes for:OKC_TIME_IG_STARTEND_V --
    ------------------------------------------------
    FUNCTION Set_Attributes (
      p_igsv_rec IN  igsv_rec_type,
      ...
      l_return_status                VARCHAR2(1) := OKC_API.G_RET_STS_SUCCESS;
    BEGIN
      x_igsv_rec := p_igsv_rec;
      x_igsv_rec.OBJECT_VERSION_NUMBER := 1;
--Bug 3122962      x_igsv_rec.SFWT_FLAG := 'N';
      RETURN(l_return_status);
    END Set_Attributes;
  BEGIN
    l_return_status := OKC_API.START_ACTIVITY(l_api_name,
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_igsv_rec := null_out_defaults(p_igsv_rec);
    -- Set primary key value
    l_igsv_rec.ID := get_seq_id;
    --- Setting item attributes
    l_return_status := Set_Attributes(
      l_igsv_rec,                        -- IN
      l_def_igsv_rec);                   -- OUT
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_def_igsv_rec := fill_who_columns(l_def_igsv_rec);
    --- Validate all non-missing attributes (Item Level Validation)
    l_return_status := Validate_Attributes(l_def_igsv_rec);
    --- If any errors happen abort API
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := Validate_Record(l_def_igsv_rec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    ...
    --------------------------------------
    migrate(l_def_igsv_rec, l_tve_rec);
--Bug 3122962    migrate(l_def_igsv_rec, l_okc_timevalues_tl_rec);
    --------------------------------------------
    ...
    ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    migrate(lx_tve_rec, l_def_igsv_rec);
/*    insert_row(
      p_init_msg_list,
      x_return_status,
      x_msg_count,
    ...
    ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    migrate(lx_okc_timevalues_tl_rec, l_def_igsv_rec);
*/    -- Set OUT values
    x_igsv_rec := l_def_igsv_rec;
    OKC_API.END_ACTIVITY(x_msg_count, x_msg_data);
  EXCEPTION
    WHEN OKC_API.G_EXCEPTION_ERROR THEN
      x_return_status := OKC_API.HANDLE_EXCEPTIONS
  ...
  END insert_row;
  ----------------------------------------
  -- PL/SQL TBL insert_row for:IGSV_TBL --
  ----------------------------------------
  PROCEDURE insert_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
    ...
    x_igsv_tbl                     OUT NOCOPY igsv_tbl_type) IS
...
*/
  ------------------------------------------
  -- lock_row for:OKC_TIME_IG_STARTEND_V --
  ------------------------------------------
  PROCEDURE lock_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
...
--Bug 3122962    l_okc_timevalues_tl_rec        okc_timevalues_tl_rec_type;
  BEGIN
    ...
    -- Move VIEW record to "Child" records
    --------------------------------------
    migrate(p_igsv_rec, l_tve_rec);
--Bug 3122962    migrate(p_igsv_rec, l_okc_timevalues_tl_rec);
    --------------------------------------------
...
*/
  ...
  END lock_row;
  --------------------------------------
  -- PL/SQL TBL lock_row for:IGSV_TBL --
  --------------------------------------
  PROCEDURE lock_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
...
*/
  --------------------------------------------
  -- update_row for:OKC_TIME_IG_STARTEND_V --
  --------------------------------------------
  PROCEDURE update_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
...
--Bug 3122962    lx_okc_timevalues_tl_rec       okc_timevalues_tl_rec_type;
    ...
    FUNCTION fill_who_columns (
      ...
      l_igsv_rec	igsv_rec_type := p_igsv_rec;
    BEGIN
      l_igsv_rec.LAST_UPDATE_DATE := SYSDATE;
      l_igsv_rec.LAST_UPDATED_BY := FND_GLOBAL.USER_ID;
      l_igsv_rec.LAST_UPDATE_LOGIN := FND_GLOBAL.LOGIN_ID;
      RETURN(l_igsv_rec);
    END fill_who_columns;
    ...
    BEGIN
      x_igsv_rec := p_igsv_rec;
      -- Get current database values
      l_igsv_rec := get_rec(p_igsv_rec, l_row_notfound);
      IF (l_row_notfound) THEN
        l_return_status := OKC_API.G_RET_STS_UNEXP_ERROR;
      END IF;
      IF (x_igsv_rec.id = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.id := l_igsv_rec.id;
      END IF;
      IF (x_igsv_rec.object_version_number = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.object_version_number := l_igsv_rec.object_version_number;
      END IF;
/*      IF (x_igsv_rec.sfwt_flag = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.sfwt_flag := l_igsv_rec.sfwt_flag;
      END IF;
*/
      IF (x_igsv_rec.tve_id_started = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.tve_id_started := l_igsv_rec.tve_id_started;
      END IF;
      IF (x_igsv_rec.tve_id_ended = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.tve_id_ended := l_igsv_rec.tve_id_ended;
      END IF;
      IF (x_igsv_rec.tve_id_limited = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.tve_id_limited := l_igsv_rec.tve_id_limited;
      END IF;
      IF (x_igsv_rec.dnz_chr_id = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.dnz_chr_id := l_igsv_rec.dnz_chr_id;
      END IF;
      IF (x_igsv_rec.tze_id = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.tze_id := l_igsv_rec.tze_id;
      END IF;
      IF (x_igsv_rec.description = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.description := l_igsv_rec.description;
      END IF;
      IF (x_igsv_rec.short_description = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.short_description := l_igsv_rec.short_description;
      END IF;
      IF (x_igsv_rec.comments = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.comments := l_igsv_rec.comments;
      END IF;
      IF (x_igsv_rec.attribute_category = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute_category := l_igsv_rec.attribute_category;
      END IF;
      IF (x_igsv_rec.attribute1 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute1 := l_igsv_rec.attribute1;
      END IF;
      IF (x_igsv_rec.attribute2 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute2 := l_igsv_rec.attribute2;
      END IF;
      IF (x_igsv_rec.attribute3 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute3 := l_igsv_rec.attribute3;
      END IF;
      IF (x_igsv_rec.attribute4 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute4 := l_igsv_rec.attribute4;
      END IF;
      IF (x_igsv_rec.attribute5 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute5 := l_igsv_rec.attribute5;
      END IF;
      IF (x_igsv_rec.attribute6 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute6 := l_igsv_rec.attribute6;
      END IF;
      IF (x_igsv_rec.attribute7 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute7 := l_igsv_rec.attribute7;
      END IF;
      IF (x_igsv_rec.attribute8 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute8 := l_igsv_rec.attribute8;
      END IF;
      IF (x_igsv_rec.attribute9 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute9 := l_igsv_rec.attribute9;
      END IF;
      IF (x_igsv_rec.attribute10 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute10 := l_igsv_rec.attribute10;
      END IF;
      IF (x_igsv_rec.attribute11 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute11 := l_igsv_rec.attribute11;
      END IF;
      IF (x_igsv_rec.attribute12 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute12 := l_igsv_rec.attribute12;
      END IF;
      IF (x_igsv_rec.attribute13 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute13 := l_igsv_rec.attribute13;
      END IF;
      IF (x_igsv_rec.attribute14 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute14 := l_igsv_rec.attribute14;
      END IF;
      IF (x_igsv_rec.attribute15 = OKC_API.G_MISS_CHAR)
      THEN
        x_igsv_rec.attribute15 := l_igsv_rec.attribute15;
      END IF;
      IF (x_igsv_rec.created_by = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.created_by := l_igsv_rec.created_by;
      END IF;
      IF (x_igsv_rec.creation_date = OKC_API.G_MISS_DATE)
      THEN
        x_igsv_rec.creation_date := l_igsv_rec.creation_date;
      END IF;
      IF (x_igsv_rec.last_updated_by = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.last_updated_by := l_igsv_rec.last_updated_by;
      END IF;
      IF (x_igsv_rec.last_update_date = OKC_API.G_MISS_DATE)
      THEN
        x_igsv_rec.last_update_date := l_igsv_rec.last_update_date;
      END IF;
      IF (x_igsv_rec.last_update_login = OKC_API.G_MISS_NUM)
      THEN
        x_igsv_rec.last_update_login := l_igsv_rec.last_update_login;
      END IF;
      RETURN(l_return_status);
    END populate_new_record;
    ------------------------------------------------
...
--Bug 3122962      x_igsv_rec.SFWT_FLAG := upper(p_igsv_rec.SFWT_FLAG);
      x_igsv_rec.OBJECT_VERSION_NUMBER := NVL(x_igsv_rec.OBJECT_VERSION_NUMBER, 0) + 1;
      RETURN(l_return_status);
    END Set_Attributes;
  BEGIN
    l_return_status := OKC_API.START_ACTIVITY(l_api_name,
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := populate_new_record(l_igsv_rec, l_def_igsv_rec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
...
*/
  ...
  END update_row;
  ----------------------------------------
  -- PL/SQL TBL update_row for:IGSV_TBL --
  ----------------------------------------
  PROCEDURE update_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
...
*/
  --------------------------------------------
  -- delete_row for:OKC_TIME_IG_STARTEND_V --
  --------------------------------------------
  PROCEDURE delete_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
...
--Bug 3122962    l_okc_timevalues_tl_rec        okc_timevalues_tl_rec_type;
  ...
  BEGIN
    ...
    -- Move VIEW record to "Child" records
    --------------------------------------
--Bug 3122962    migrate(l_igsv_rec, l_okc_timevalues_tl_rec);
    migrate(l_igsv_rec, l_tve_rec);
    --------------------------------------------
...
*/
  ...
  END delete_row;
  ----------------------------------------
  -- PL/SQL TBL delete_row for:IGSV_TBL --
  ----------------------------------------
  PROCEDURE delete_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 ,
