--------------------------------------------------------
--  DDL for Package Body IGS_AS_ANON_METHOD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_ANON_METHOD_PKG" AS
/* $Header: IGSDI60B.pls 115.1 2002/11/28 23:25:57 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_as_anon_method%ROWTYPE;
  new_references igs_as_anon_method%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.method                            := x_method;
  ...
  */
  BEGIN
    ...
    IF (((old_references.load_cal_type = new_references.load_cal_type)) OR
        ((new_references.load_cal_type IS NULL))) THEN
      NULL;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_as_anon_method
      WHERE    load_cal_type = x_load_cal_type
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AS_ANON_ATYP_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_as_anon_method
      WHERE   ((load_cal_type = x_cal_type));
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_AS_ANON_CAT_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    INSERT INTO igs_as_anon_method (
      load_cal_type,
      method,
      assessment_type,
      creation_date,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.method = x_method)
        AND ((tlinfo.assessment_type = x_assessment_type) OR ((tlinfo.assessment_type IS NULL) AND (X_assessment_type IS NULL)))
       ) THEN
      NULL;
    ELSE
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_as_anon_method
      SET
        method                            = new_references.method,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_as_anon_method
    WHERE rowid = x_rowid;
