--------------------------------------------------------
--  DDL for Package Body CN_COLLECT_PAYMENTS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CN_COLLECT_PAYMENTS" AS
-- $Header: cncopmtb.pls 120.3 2006/01/19 04:00:33 apink noship $
...
BEGIN
  ...
  dbms_output.put_line('Error: Package cn_collect_payments'||cached_org_append||' is probably missing');
...
--   collect
-- Purpose
--   This procedure is the router for collecting payments
--
-- History
--   02-Jan-97	Jcheng	Router Procedure for collecting payments
--
...
begin
  ...
  statement := 'begin cn_collect_payments'||cached_org_append||
