--------------------------------------------------------
--  DDL for Package Body IGS_PR_ORG_STAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PR_ORG_STAT_PKG" AS
/* $Header: IGSQI36B.pls 115.4 2002/11/29 03:23:57 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_pr_org_stat%ROWTYPE;
  new_references igs_pr_org_stat%ROWTYPE;
  ...
  */
  BEGIN
    ...
    igs_pr_org_stat_ref_pkg.get_fk_igs_pr_org_stat (
  ...
  */
    CURSOR cur_rowid IS
      ...
      FROM     igs_pr_org_stat
      WHERE    org_unit_cd = x_org_unit_cd
      AND      display_order = x_display_order
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PR_ORST_STTY_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    after_dml(x_org_unit_cd);
  END insert_row;

  PROCEDURE after_dml(
  	x_org_unit_cd 	IN 	VARCHAR2) AS
  /*
  ||  Created By : nbehera
  ||  Created On : 15-NOV-2001
  ||  Purpose : Checks for more than one record with the same indicator
  ...
  */
  ...
  CURSOR c1(l_org_unit_cd igs_pr_org_stat.org_unit_cd%TYPE) IS
  SELECT   orst.timeframe,
           orst.standard_ind
  FROM     igs_pr_org_stat  orst
  WHERE    orst.org_unit_cd = l_org_unit_cd
  AND      (  orst.standard_ind = 'Y');

  CURSOR c2 IS
  SELECT   orst.timeframe
  FROM 	   igs_pr_org_stat  orst
  WHERE    orst.standard_ind = 'Y'
    AND    orst.org_unit_cd = x_org_unit_cd
  GROUP BY orst.timeframe
  HAVING   COUNT(orst.timeframe) > 1;
  ...
  dummy		VARCHAR2(50);
 BEGIN
  FOR l_c1_rec IN c1(x_org_unit_cd) LOOP
