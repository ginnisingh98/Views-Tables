--------------------------------------------------------
--  DDL for Package Body GR_RECIPIENT_INFO_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_RECIPIENT_INFO_PKG" AS
/*$Header: GRHIRECB.pls 115.6 2002/10/25 17:22:14 gkelly ship $*/
...
/*	  Now call the check foreign key procedure */
   ...
   Check_Foreign_Keys
			     (p_recipient_code,
				  ...
				  p_document_code,
				  p_territory_code,
				  p_print_recipient_product_code,
				  p_update_address_flag,
				  p_disclose_all_ingred_flag,
				  p_address_to_print,
				  p_document_print_frequency,
				  p_disclosure_code,
				  p_region_code,
				  p_cover_letter_id,
				  p_additional_documents_flag,
				  p_additional_address_flag,
				  p_shipping_address,
				  p_invoice_address,
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_recipient_info
   		  	     (recipient_code,
				  recipient_name,
				  document_code,
				  territory_code,
				  print_recipient_product_code,
				  update_address_flag,
				  disclose_all_ingredients_flag,
				  address_to_print,
				  document_print_frequency,
				  disclosure_code,
				  region_code,
				  cover_letter_id,
				  additional_documents_flag,
				  additional_address_flag,
				  shipping_address,
				  invoice_address,
				  attribute_category,
				  attribute1,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_recipient_info
	  SET	 recipient_code 	 	 	 	 = p_recipient_code,
			 recipient_name	  				 = p_recipient_name,
			 document_code	  				 = p_document_code,
			 territory_code	  				 = p_territory_code,
			 print_recipient_product_code	 = p_print_recipient_product_code,
			 update_address_flag	  		 = p_update_address_flag,
			 disclose_all_ingredients_flag	 = p_disclose_all_ingred_flag,
			 address_to_print	  			 = p_address_to_print,
			 document_print_frequency	  	 = p_document_print_frequency,
			 disclosure_code	  			 = p_disclosure_code,
			 region_code	  				 = p_region_code,
			 cover_letter_id	  			 = p_cover_letter_id,
			 additional_documents_flag	  	 = p_additional_documents_flag,
			 additional_address_flag	     = p_additional_address_flag,
...
CURSOR c_lock_recipient
 IS
   SELECT	last_update_date
   FROM		gr_recipient_info
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockRecipientRcd	  c_lock_recipient%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_recipient;
   FETCH c_lock_recipient INTO LockRecipientRcd;
   IF c_lock_recipient%NOTFOUND THEN
	  CLOSE c_lock_recipient;
	  RAISE No_Data_Found_Error;
   END IF;
   CLOSE c_lock_recipient;


   IF LockRecipientRcd.last_update_date <> p_last_update_date THEN
...
CURSOR c_get_territory
 IS
   SELECT	ter.territory_code
   FROM		fnd_territories ter
   WHERE	ter.territory_code = p_territory_code;
TerrRecord			c_get_territory%ROWTYPE;
...
/*		Region Code */
   ...
   IF p_region_code IS NOT NULL THEN
      l_key_exists := 'T';
      GR_REGIONS_PKG.Check_Primary_Key
					(p_region_code,
					 'F',
					 l_rowid,
					 l_key_exists);
...
/*   Territory Code */
   ...
   OPEN c_get_territory;
   FETCH c_get_territory INTO TerrRecord;
   IF c_get_territory%NOTFOUND THEN
      x_return_status := 'E';
	  FND_MESSAGE.SET_NAME('GR',
	   					   'GR_RECORD_NOT_FOUND');
	  FND_MESSAGE.SET_TOKEN('CODE',
...
/* 	Now read the cursors to make sure the recipient code isn't used. */
...
/*		  p_recipient_code is the recipient code to check.
...
CURSOR c_get_recipient_rowid
 IS
   SELECT rec.rowid
   FROM	  gr_recipient_info rec
   WHERE  rec.recipient_code = p_recipient_code;
RecipientRecord			   c_get_recipient_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   l_msg_data := p_recipient_code;
   OPEN c_get_recipient_rowid;
   FETCH c_get_recipient_rowid INTO RecipientRecord;
   IF c_get_recipient_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := RecipientRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_recipient_rowid;
