--------------------------------------------------------
--  DDL for Package Body BEN_REG_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_REG_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_reg_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_delete_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE_NEXT_CHANGE') then
    ...
    --
    --  Delete from MLS table.
    --
    delete from ben_regn_f_tl
    where       regn_id = p_rec.regn_id
    and   effective_start_date = p_validation_start_date;
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'ZAP') then
    --
    p_rec.effective_start_date := ben_reg_shd.g_old_rec.effective_start_date;
    --
