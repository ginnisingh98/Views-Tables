--------------------------------------------------------
--  DDL for Package Body POA_EDW_ALINES_F_C
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."POA_EDW_ALINES_F_C" AS
/* $Header: poafpalb.pls 120.1 2005/06/13 12:53:24 sriswami noship $ */
...
-----------------------------------------------------------
 ...
 BEGIN
    ...
    IF (FND_INSTALLATION.GET_APP_INFO('POA', l_status, l_industry, l_poa_schema)) THEN
       l_stmt := 'TRUNCATE TABLE ' || l_poa_schema || '.POA_EDW_ALINES_INC';
...
-----------------------------------------------------------
 ...
 l_fact_name     Varchar2(30) := 'POA_EDW_ALINES_F';
 l_staging_table Varchar2(30) := 'POA_EDW_ALINES_FSTG';
