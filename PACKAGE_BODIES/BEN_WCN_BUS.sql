--------------------------------------------------------
--  DDL for Package Body BEN_WCN_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_WCN_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_wcn_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_wv_prtn_rsn_ctfn_pl_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   wv_prtn_rsn_ctfn_pl_id PK of record being inserted or updated.
...
--
Procedure chk_wv_prtn_rsn_ctfn_pl_id(p_wv_prtn_rsn_ctfn_pl_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_wv_prtn_rsn_ctfn_pl_id';
...
Begin
  ...
  --
  l_api_updating := ben_wcn_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_wv_prtn_rsn_ctfn_pl_id                => p_wv_prtn_rsn_ctfn_pl_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_wv_prtn_rsn_ctfn_pl_id,hr_api.g_number)
     <>  ben_wcn_shd.g_old_rec.wv_prtn_rsn_ctfn_pl_id) then
    --
    -- raise error as PK has changed
    --
    ben_wcn_shd.constraint_error('BEN_WV_PRTN_RSN_CTFN_PL_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_wv_prtn_rsn_ctfn_pl_id is not null then
      --
  ...
  --
End chk_wv_prtn_rsn_ctfn_pl_id;
--
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_wv_prtn_ctfn_typ_cd(p_wv_prtn_rsn_ctfn_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_wcn_shd.g_old_rec.wv_prtn_ctfn_typ_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_wv_prtn_rsn_ctfn_pl_id    in number,
...
--   Internal table handler use only.
--
Procedure chk_rqd_flag(p_wv_prtn_rsn_ctfn_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_rqd_flag
      <> nvl(ben_wcn_shd.g_old_rec.rqd_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_lack_ctfn_sspnd_wvr_flag(p_wv_prtn_rsn_ctfn_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_wcn_shd.g_old_rec.lack_ctfn_sspnd_wvr_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_pfd_flag(p_wv_prtn_rsn_ctfn_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_pfd_flag
      <> nvl(ben_wcn_shd.g_old_rec.pfd_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    --
    If ((nvl(p_wv_prtn_rsn_pl_id, hr_api.g_number) <> hr_api.g_number) and
...
function return_legislation_code
  (p_wv_prtn_rsn_ctfn_pl_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_wv_prtn_rsn_ctfn_pl_f b
    where b.wv_prtn_rsn_ctfn_pl_id      = p_wv_prtn_rsn_ctfn_pl_id
