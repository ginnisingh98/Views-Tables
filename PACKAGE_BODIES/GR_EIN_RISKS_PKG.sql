--------------------------------------------------------
--  DDL for Package Body GR_EIN_RISKS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_EIN_RISKS_PKG" AS
/*$Header: GRHIEIRB.pls 115.3 2002/10/29 16:43:45 mgrosser noship $*/
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ...
      FND_MESSAGE.SET_TOKEN('CODE',
         		     p_european_index_number||' '||p_risk_phrase_code,
            		     FALSE);
      IF FND_API.To_Boolean(p_called_by_form) THEN
         APP_EXCEPTION.Raise_Exception;
      ELSE
...
CURSOR c_lock_ein_risk
 IS
   SELECT	last_update_date
   FROM		gr_ein_risks
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockEinRcd	  c_lock_ein_risk%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_ein_risk;
   FETCH c_lock_ein_risk INTO LockEinRcd;
   IF c_lock_ein_risk%NOTFOUND THEN
	  CLOSE c_lock_ein_risk;
...
RiskClassRecord			c_get_risk%ROWTYPE;
...
/*   Check the Risk phrase codes */

   OPEN c_get_risk;
   FETCH c_get_risk INTO RiskClassRecord;
   IF c_get_risk%NOTFOUND THEN
...
CURSOR c_get_eir_rowid
 IS
   SELECT eir.rowid
   FROM	  gr_ein_risks eir
   WHERE  eir.european_index_number = p_european_index_number
   AND    eir.risk_phrase_code = p_risk_phrase_code;
EirRecord			   c_get_eir_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_eir_rowid;
   FETCH c_get_eir_rowid INTO EirRecord;
   IF c_get_eir_rowid%FOUND THEN
      x_key_exists := 'T';
      x_rowid := EirRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_eir_rowid;
