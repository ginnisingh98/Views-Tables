--------------------------------------------------------
--  DDL for Package Body POS_COMPARE_REVISIONS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."POS_COMPARE_REVISIONS" AS
/* $Header: POSPOCMB.pls 120.20.12010000.4 2014/06/13 10:55:43 pneralla ship $ */
...
WHEN others THEN
	PO_MESSAGE_S.SQL_ERROR(
		'PO_COMPARE_REVISIONS.GET_ORG_NAME',
		v_progress,
		sqlcode );
	RAISE;
...
/* Bug 12899225 :  More than one row will be returned by this query if vendor site condition is not used.
...
/*Bug:18938136:
Adding cursor to fetch location from hr_locations or hz_locations to handle drop ship POs*/
CURSOR cur_loc
IS
...
OPEN cur_loc;
    LOOP
      FETCH cur_loc INTO v_location_code;
 ...
 *
 * PURPOSE
 * Resolves the price differential type
 *
 * ARGUMENTS
 * p_pricediff_code     Unique in PO_PRICE_DIFF_LOOKUPS_V
 *
 * NOTES
 * Return NULL if no matching records were found.
 *
 * HISTORY
 * 21-OCT-03    Amitabh Mitra created
 ********************************************************************/
FUNCTION get_pricediff_type( p_pricediff_code IN VARCHAR2 )
        RETURN VARCHAR2 AS

v_pricediff_dsp varchar2(240);
v_progress      varchar2(3);
...
WHEN others THEN
        PO_MESSAGE_S.SQL_ERROR(
                'PO_COMPARE_REVISIONS.GET_PRICEDIFF_TYPE',
                v_progress,
                sqlcode );
        RAISE;

END get_pricediff_type;
 ...
 *	Start Date
 *	End Date
 *      Deliverables Change Date
 *      Articles Change Date
 *
 * HISTORY
 * 08-AUG-97	Nilo Paredes	Created.
...
c_po_num		po_headers_archive_all.segment1%TYPE;
...
/* bug 5509777 - Do amount comparison only when PO Type is not in 'BLANKET', 'CONTRACT' */
IF (p_po_from.type_lookup_code <> 'BLANKET' AND p_po_from.type_lookup_code <> 'CONTRACT') THEN

-- using new methods for calculating amount - bug 5398214
/*
...
*/
v_amount_from := POS_TOTALS_PO_SV.get_po_archive_total(
			c_po_header_id,
			p_po_from.revision_num, p_po_from.type_lookup_code );
v_amount_to := POS_TOTALS_PO_SV.get_po_archive_total(
			c_po_header_id,
			p_po_to.revision_num, p_po_to.type_lookup_code );
...
*/
		...
		null,
		null,
		p_po_from.acceptance_due_date,
		p_po_to.acceptance_due_date
		);
--end bug 4179194
END IF;
...
END IF;
/* Compare Article Change Date and Deliverables Change Date for
   Compare to Previous and Compare to Last Signed only */

--compare date for ALL also
--fix for bug # 3616301
if (p_comparison_flag in ('PREVIOUS','LASTSIGN', 'ALL')) then
v_progress := '330';
/* Deliverables Changed Date */
IF NVL( p_po_from.conterms_deliv_upd_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) <>
	NVL( p_po_to.conterms_deliv_upd_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) THEN
...
v_progress := '340';
/* Articles Changed Date */
IF NVL( p_po_from.conterms_articles_upd_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) <>
	NVL( p_po_to.conterms_articles_upd_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) THEN
/*
	bug 4140325
...
c_release_id		NUMBER;
c_revision_num		NUMBER;
c_line_id		NUMBER := NULL;c_line_num		NUMBER := NULL;
c_location_id		NUMBER := NULL;
c_location_num		NUMBER := NULL;
c_distribution_id	NUMBER := NULL;
c_distribution_num	NUMBER := NULL;
...
v_amount_from		NUMBER;
v_amount_to		NUMBER;
v_po_num		po_headers_archive_all.segment1%TYPE;
...
/* Amount -- new code to compare amount - bug 5452301 */
v_amount_from := POS_TOTALS_PO_SV.get_release_archive_total(
			c_release_id,
			p_release_from.revision_num);
v_amount_to := POS_TOTALS_PO_SV.get_release_archive_total(
			c_release_id,
			p_release_to.revision_num );
...
*/
		...
		null,
		null,
		p_release_from.acceptance_due_date,
		p_release_to.acceptance_due_date
		);
--end bug 4179194
END IF;
...
*/
		...
		null,
		null,
		p_release_from.release_date,
		p_release_to.release_date
		);
--end bug 4179194
END IF;
...
c_level_altered		pos_po_revisions_gt.level_altered%TYPE := 'ICX_HEADER';
c_po_header_id		NUMBER;
c_po_num		po_headers_archive_all.segment1%TYPE;
c_release_id		NUMBER;
c_revision_num		NUMBER;
...
v_po_num		po_headers_archive_all.segment1%TYPE;
c_org_name		hr_all_organization_units_tl.name%TYPE;
c_entity_id	NUMBER;
c_entity_type	po_price_differentials_archive.entity_type%TYPE;
c_price_diff_num	po_price_differentials_archive.price_differential_num%TYPE;
...
IF p_pdiffs_to.price_differential_id IS NULL THEN
	RETURN;
END IF;


c_entity_id		:= p_pdiffs_to.entity_id;
c_entity_type 		:= p_pdiffs_to.entity_type;
c_revision_num		:= p_pdiffs_to.revision_num;
c_price_diff_num        := p_pdiffs_to.price_differential_num;

IF NVL( p_pdiffs_from.revision_num, -99 ) =
	NVL( p_pdiffs_to.revision_num, -99 ) THEN
	RETURN;
END IF;

if (c_entity_type = 'PO LINE' OR c_entity_type = 'BLANKET LINE') then
...
IF nvl(p_pdiffs_from.price_type,-99) <> nvl(p_pdiffs_to.price_type,-99)  THEN
...
IF nvl(p_pdiffs_from.multiplier,-99) <> nvl(p_pdiffs_to.multiplier,-99)  THEN
	...
	END IF;
IF nvl(p_pdiffs_from.min_multiplier,-99) <> nvl(p_pdiffs_to.min_multiplier,-99)  THEN
	...
	END IF;
IF nvl(p_pdiffs_from.max_multiplier,-99) <> nvl(p_pdiffs_to.max_multiplier,-99)  THEN
...
WHEN others THEN
	PO_MESSAGE_S.SQL_ERROR(
		'PO_COMPARE_REVISIONS.COMPARE_PDIFS',
		v_progress,
		sqlcode );
	RAISE;

END compare_price_diffs;
...
v_po_num		po_headers_archive_all.segment1%TYPE;
c_org_name		hr_all_organization_units_tl.name%TYPE;
c_enabled_org_id	NUMBER;

BEGIN


IF p_ga_ass_to.organization_id IS NULL THEN
	RETURN;
END IF;


c_po_header_id		:= p_ga_ass_to.po_header_id;
c_enabled_org_id 	:= p_ga_ass_to.organization_id;
...
c_revision_num		:= p_ga_ass_to.revision_num;


IF NVL( p_ga_ass_from.revision_num, -99 ) =
	NVL( p_ga_ass_to.revision_num, -99 ) THEN
	RETURN;
END IF;



IF nvl(p_ga_ass_from.purchasing_org_id,-99) <> nvl(p_ga_ass_to.purchasing_org_id,-99)  THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
IF nvl(p_ga_ass_from.vendor_site_id,-99) <> nvl(p_ga_ass_to.vendor_site_id,-99)  THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
IF nvl(p_ga_ass_from.enabled_flag,-99) <> nvl(p_ga_ass_to.enabled_flag,-99)  THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
WHEN others THEN
	PO_MESSAGE_S.SQL_ERROR(
		'PO_COMPARE_REVISIONS.COMPARE_GA_ORG_ASSIGNMENTS',
		v_progress,
		sqlcode );
	RAISE;
...
c_job_id                NUMBER := NULL;
c_org_id                NUMBER := NULL;
c_item                  mtl_system_items_kfv.concatenated_segments%TYPE;
c_job                   per_jobs.name%TYPE;
...
c_job_id		:= p_line_to.job_id;
c_org_id		:= p_line_to.org_id;

v_progress := '410';

if (c_item_id is not null) then
	c_item := get_item(c_item_id, c_org_id);
end if;

if (c_job_id is not null) then
	c_job := get_job(c_job_id);
end if;
...
/* Job */
IF NVL( p_line_from.job_id, -99 )  <> NVL( p_line_to.job_id, -99 )
THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
...
/* Shopping category */
IF NVL( p_line_from.ip_category_id, -99 )  <> NVL( p_line_to.ip_category_id, -99 )
THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
...
IF NVL( p_line_from.CONTRACTOR_FIRST_NAME,'') <>
        NVL( p_line_to.CONTRACTOR_FIRST_NAME,'') THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
v_progress := '620';
IF NVL( p_line_from.CONTRACTOR_LAST_NAME,'') <>
        NVL( p_line_to.CONTRACTOR_LAST_NAME,'') THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
v_progress := '630';
IF NVL( p_line_from.start_date,
                TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) <>
        NVL( p_line_to.start_date,
                TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
...
*/
		...
		null,
		null,
		p_line_from.start_date,
		p_line_to.start_date,
		c_item,
		c_job
		);
--end bug 4179194
...
*/
		...
		null,
		null,
		p_line_from.expiration_date,
		p_line_to.expiration_date,
		c_item,
		c_job
		);
--end bug 4179194
...
v_progress := '650';
/* Compare Amount field on lines for fixed price services*/
IF NVL( p_line_from.amount, -99 ) <>
        NVL( p_line_to.amount, -99 ) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
IF NVL( p_line_from.not_to_exceed_price, -99 ) <>
        NVL( p_line_to.not_to_exceed_price, -99 ) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
--start complex work ECO 5106340

IF NVL( p_line_from.RETAINAGE_RATE, -99 ) <>
        NVL( p_line_to.RETAINAGE_RATE, -99 ) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
IF NVL( p_line_from.MAX_RETAINAGE_AMOUNT, -99 ) <>
        NVL( p_line_to.MAX_RETAINAGE_AMOUNT, -99 ) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
IF NVL( p_line_from.PROGRESS_PAYMENT_RATE, -99 ) <>
        NVL( p_line_to.PROGRESS_PAYMENT_RATE, -99 ) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
...
IF NVL( p_line_from.RECOUPMENT_RATE, -99 ) <>
        NVL( p_line_to.RECOUPMENT_RATE, -99 ) THEN
        insert_changes(
                p_sequence,
                c_po_header_id,
                c_release_id,
 ...
 *				and shipment price.
 * 14-FEB-2003  Amitabh Mitra   Added start_date and end_date
 *                              for blankets
 ********************************************************************/
...
c_po_header_id		NUMBER;
c_release_id		NUMBER;
c_po_num		po_headers_archive_all.segment1%TYPE := NULL;
c_line_id		NUMBER;
c_line_num		NUMBER := NULL;
...
v_from_payment_type     VARCHAR(100)  := NULL;
v_to_payment_type       VARCHAR2(100) := NULL;
BEGIN
...
--start advance amount
/* for shipments of payment type ADVANCE and shipment type PREPAYMENT, we show change in amount as
change in Advance Amount at line level
This code replaces Advance Amount Code in compare_lines - search for ADVAMT in this file
*/

IF ( (NVL(p_loc_from.payment_type, ' ') = 'ADVANCE' and NVL(p_loc_from.shipment_type, ' ') = 'PREPAYMENT' ) or
     (NVL(p_loc_to.payment_type, ' ') = 'ADVANCE' and NVL(p_loc_to.shipment_type, ' ') = 'PREPAYMENT' ) ) THEN

  IF (NVL( p_loc_from.amount, -99 ) <>
	NVL( p_loc_to.amount, -99 )) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
/* Do not show shipments of type 'ADVANCE', 'DELIVERY'*/
IF (NVL(p_loc_from.payment_type, ' ') in ('ADVANCE', 'DELIVERY')) THEN
	RETURN;
END IF;
--end complex work
...
*/
		...
		null,
		null,
		p_loc_from.last_accept_date,
		p_loc_to.last_accept_date,
		c_item,
		c_job
                );
--end bug 4179194
...
--start fix bug 4696960
--for releases show change in price_override as Unit Price Change

IF ((NVL( p_loc_from.price_override, -99 ) <>
	NVL( p_loc_to.price_override, -99 )) and p_loc_from.shipment_type = 'PRICE BREAK' and c_release_id is null) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
IF ((NVL( p_loc_from.price_override, -99 ) <>
	NVL( p_loc_to.price_override, -99 )) AND p_loc_from.shipment_type <> 'PRICE BREAK' /*(--Bug 13508885) */ /* (--bug 5204345 -- and c_release_id is not null)*/) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
-- Added start_date and end_date for blanket price breaks

/* Start Effective date */
IF NVL( p_loc_from.start_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) <>
	NVL( p_loc_to.start_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
...
/* End Effective Date */
IF NVL( p_loc_from.end_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) <>
	NVL( p_loc_to.end_date,
		TO_DATE( '01/01/1000', 'DD/MM/YYYY' )) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
...
IF (NVL( p_loc_from.work_approver_id, -99 ) <>
	NVL( p_loc_to.work_approver_id, -99 )) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
IF NVL( p_loc_from.description, ' ' ) <>
	NVL( p_loc_to.description, ' ' ) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
...
IF NVL( p_loc_from.payment_type, ' ' ) <>
	NVL( p_loc_to.payment_type, ' ' ) THEN

	if (p_loc_from.payment_type = 'MILESTONE') THEN
		v_from_payment_type := fnd_message.get_String('POS','POS_PROG_PAY_MILE');
	elsif (p_loc_from.payment_type = 'RATE') THEN
		v_from_payment_type := fnd_message.get_String('POS','POS_PROG_PAY_RATE');
	elsif (p_loc_from.payment_type = 'LUMPSUM') THEN
		v_from_payment_type := fnd_message.get_String('POS','POS_PROG_PAY_LUMP');
	else
		v_from_payment_type := p_loc_from.payment_type;
	end if;

	if (p_loc_to.payment_type = 'MILESTONE') THEN
		v_to_payment_type := fnd_message.get_String('POS','POS_PROG_PAY_MILE');
	elsif (p_loc_to.payment_type = 'RATE') THEN
		v_to_payment_type := fnd_message.get_String('POS','POS_PROG_PAY_RATE');
	elsif (p_loc_to.payment_type = 'LUMPSUM') THEN
		v_to_payment_type := fnd_message.get_String('POS','POS_PROG_PAY_LUMP');
	else
		v_to_payment_type := p_loc_to.payment_type;
	end if;
...
/*
IF (p_loc_from.payment_type = 'MILESTONE' || p_loc_to.payment_type = 'MILESTONE' ) THEN

IF (Get_ship_val_percent_revision (p_loc_from.line_location_id, p_loc_from.revision_num) <>
	Get_ship_val_percent_revision (p_loc_to.line_location_id, p_loc_to.revision_num) ) THEN
	insert_changes(
		p_sequence,
		c_po_header_id,
		c_release_id,
 ...
 *
 * HISTORY
 * 15-Mar-05	Abhishek Trivedi	Created
 ********************************************************************/
...
WHEN others THEN
	PO_MESSAGE_S.SQL_ERROR(
		'PO_COMPARE_REVISIONS.GET_REQUESTOR',
		v_progress,
		sqlcode );
	RAISE;
 ...
 *
 * ARGUMENTS
 * p_category_id 	Unique identifier for shopping category
 *
 * NOTES
 * Return NULL if no matching records were found.
 *
 * HISTORY
 * 10-May-05	Abhishek Trivedi	Created
 ********************************************************************/
FUNCTION get_shopping_category( p_category_id IN NUMBER )
	RETURN VARCHAR2 AS

v_category_name	icx_cat_categories_v.category_name%TYPE;
v_progress	varchar2(3);
...
WHEN others THEN
	PO_MESSAGE_S.SQL_ERROR(
		'PO_COMPARE_REVISIONS.GET_SHOPPING_CATEGORY',
		v_progress,
		sqlcode );
	RAISE;

END get_shopping_category;
 ...
 *
 * PURPOSE
 * Resolves the owner name.
 *
 * ARGUMENTS
 * p_work_approver_id	Unique identifier in fnd_user.
 *
 * NOTES
 * Return NULL if no matching records were found.
 *
 * HISTORY
 * 09-May-06	Abhishek Trivedi	Created
 ********************************************************************/
...
WHEN others THEN
	PO_MESSAGE_S.SQL_ERROR(
		'PO_COMPARE_REVISIONS.GET_OWNER',
		v_progress,
		sqlcode );
	RAISE;
...
WHEN others THEN
        PO_MESSAGE_S.SQL_ERROR(
                'PO_COMPARE_REVISIONS.GET_NEXTVAL',
                '112',
                sqlcode );
        RAISE;
 ...
 *
 * ARGUMENTS
 * p_job_id	job_id from po_lines_all
 *
 * NOTES
 * Return NULL if no matching records were found.
 *
 ...
 *
 * ARGUMENTS
 * p_item_id	item_id from po_lines_all
 * p_org_id	org_id from po_lines_all
 *
 * NOTES
 * Return NULL if no matching records were found.
 *
...
/*********************************************************************
 * NAME
 * GET_LINE_ADV_AMOUNT_REVISION
 *
 * PURPOSE
 * Calculates the advance amount from shipments
 *
 * ARGUMENTS
 * p_po_line_id	po_line_id
 * p_revision_num	revision number
 *
 * NOTES
 * Return NULL if no matching records were found.
 *
...
RETURN NUMBER as
  ...
  l_api_name	CONSTANT VARCHAR2(30) := 'GET_LINE_ADV_AMOUNT_REVISION';
  l_api_version	CONSTANT NUMBER := 1.0;

  l_advance_amt 	NUMBER := 0;
  x_advance_amt 	NUMBER := 0;
...
END GET_LINE_ADV_AMOUNT_REVISION;

/*********************************************************************
 * NAME
 * GET_SHIP_VAL_PERCENT_REVISION
 *
 * PURPOSE
 * Calculates value percent for a shipment
 *
 * ARGUMENTS
 * p_po_line_location_id	po_line_id
 * p_revision_num	revision number
 *
 * NOTES
 * Return NULL if no matching records were found.
...
END Get_ship_val_percent_revision;
