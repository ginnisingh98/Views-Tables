--------------------------------------------------------
--  DDL for Package Body BEN_BNB_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BNB_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_bnb_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_bnfts_bal_f Row
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_bnb_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.bnfts_bal_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_bnb_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    ben_bnb_shd.g_old_rec.name;
  End If;
  If (p_rec.bnfts_bal_usg_cd = hr_api.g_varchar2) then
    p_rec.bnfts_bal_usg_cd :=
    ben_bnb_shd.g_old_rec.bnfts_bal_usg_cd;
  End If;
--            Next 8 lines added for nnmntry_uom , uom
  If (p_rec.nnmntry_uom = hr_api.g_varchar2) then
    p_rec.nnmntry_uom  :=
    ben_bnb_shd.g_old_rec.nnmntry_uom;
  End If;
  If (p_rec.uom = hr_api.g_varchar2) then
    p_rec.uom  :=
    ben_bnb_shd.g_old_rec.uom;
  End If;
  If (p_rec.bnfts_bal_desc = hr_api.g_varchar2) then
    p_rec.bnfts_bal_desc :=
    ben_bnb_shd.g_old_rec.bnfts_bal_desc;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_bnb_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.bnb_attribute_category = hr_api.g_varchar2) then
    p_rec.bnb_attribute_category :=
    ben_bnb_shd.g_old_rec.bnb_attribute_category;
  End If;
  If (p_rec.bnb_attribute1 = hr_api.g_varchar2) then
    p_rec.bnb_attribute1 :=
    ben_bnb_shd.g_old_rec.bnb_attribute1;
  End If;
  If (p_rec.bnb_attribute2 = hr_api.g_varchar2) then
    p_rec.bnb_attribute2 :=
    ben_bnb_shd.g_old_rec.bnb_attribute2;
  End If;
  If (p_rec.bnb_attribute3 = hr_api.g_varchar2) then
    p_rec.bnb_attribute3 :=
    ben_bnb_shd.g_old_rec.bnb_attribute3;
  End If;
  If (p_rec.bnb_attribute4 = hr_api.g_varchar2) then
    p_rec.bnb_attribute4 :=
    ben_bnb_shd.g_old_rec.bnb_attribute4;
  End If;
  If (p_rec.bnb_attribute5 = hr_api.g_varchar2) then
    p_rec.bnb_attribute5 :=
    ben_bnb_shd.g_old_rec.bnb_attribute5;
  End If;
  If (p_rec.bnb_attribute6 = hr_api.g_varchar2) then
    p_rec.bnb_attribute6 :=
    ben_bnb_shd.g_old_rec.bnb_attribute6;
  End If;
  If (p_rec.bnb_attribute7 = hr_api.g_varchar2) then
    p_rec.bnb_attribute7 :=
    ben_bnb_shd.g_old_rec.bnb_attribute7;
  End If;
  If (p_rec.bnb_attribute8 = hr_api.g_varchar2) then
    p_rec.bnb_attribute8 :=
    ben_bnb_shd.g_old_rec.bnb_attribute8;
  End If;
  If (p_rec.bnb_attribute9 = hr_api.g_varchar2) then
    p_rec.bnb_attribute9 :=
    ben_bnb_shd.g_old_rec.bnb_attribute9;
  End If;
  If (p_rec.bnb_attribute10 = hr_api.g_varchar2) then
    p_rec.bnb_attribute10 :=
    ben_bnb_shd.g_old_rec.bnb_attribute10;
  End If;
  If (p_rec.bnb_attribute11 = hr_api.g_varchar2) then
    p_rec.bnb_attribute11 :=
    ben_bnb_shd.g_old_rec.bnb_attribute11;
  End If;
  If (p_rec.bnb_attribute12 = hr_api.g_varchar2) then
    p_rec.bnb_attribute12 :=
    ben_bnb_shd.g_old_rec.bnb_attribute12;
  End If;
  If (p_rec.bnb_attribute13 = hr_api.g_varchar2) then
    p_rec.bnb_attribute13 :=
    ben_bnb_shd.g_old_rec.bnb_attribute13;
  End If;
  If (p_rec.bnb_attribute14 = hr_api.g_varchar2) then
    p_rec.bnb_attribute14 :=
    ben_bnb_shd.g_old_rec.bnb_attribute14;
  End If;
  If (p_rec.bnb_attribute15 = hr_api.g_varchar2) then
    p_rec.bnb_attribute15 :=
    ben_bnb_shd.g_old_rec.bnb_attribute15;
  End If;
  If (p_rec.bnb_attribute16 = hr_api.g_varchar2) then
    p_rec.bnb_attribute16 :=
    ben_bnb_shd.g_old_rec.bnb_attribute16;
  End If;
  If (p_rec.bnb_attribute17 = hr_api.g_varchar2) then
    p_rec.bnb_attribute17 :=
    ben_bnb_shd.g_old_rec.bnb_attribute17;
  End If;
  If (p_rec.bnb_attribute18 = hr_api.g_varchar2) then
    p_rec.bnb_attribute18 :=
    ben_bnb_shd.g_old_rec.bnb_attribute18;
  End If;
  If (p_rec.bnb_attribute19 = hr_api.g_varchar2) then
    p_rec.bnb_attribute19 :=
    ben_bnb_shd.g_old_rec.bnb_attribute19;
  End If;
  If (p_rec.bnb_attribute20 = hr_api.g_varchar2) then
    p_rec.bnb_attribute20 :=
    ben_bnb_shd.g_old_rec.bnb_attribute20;
  End If;
  If (p_rec.bnb_attribute21 = hr_api.g_varchar2) then
    p_rec.bnb_attribute21 :=
    ben_bnb_shd.g_old_rec.bnb_attribute21;
  End If;
  If (p_rec.bnb_attribute22 = hr_api.g_varchar2) then
    p_rec.bnb_attribute22 :=
    ben_bnb_shd.g_old_rec.bnb_attribute22;
  End If;
  If (p_rec.bnb_attribute23 = hr_api.g_varchar2) then
    p_rec.bnb_attribute23 :=
    ben_bnb_shd.g_old_rec.bnb_attribute23;
  End If;
  If (p_rec.bnb_attribute24 = hr_api.g_varchar2) then
    p_rec.bnb_attribute24 :=
    ben_bnb_shd.g_old_rec.bnb_attribute24;
  End If;
  If (p_rec.bnb_attribute25 = hr_api.g_varchar2) then
    p_rec.bnb_attribute25 :=
    ben_bnb_shd.g_old_rec.bnb_attribute25;
  End If;
  If (p_rec.bnb_attribute26 = hr_api.g_varchar2) then
    p_rec.bnb_attribute26 :=
    ben_bnb_shd.g_old_rec.bnb_attribute26;
  End If;
  If (p_rec.bnb_attribute27 = hr_api.g_varchar2) then
    p_rec.bnb_attribute27 :=
    ben_bnb_shd.g_old_rec.bnb_attribute27;
  End If;
  If (p_rec.bnb_attribute28 = hr_api.g_varchar2) then
    p_rec.bnb_attribute28 :=
    ben_bnb_shd.g_old_rec.bnb_attribute28;
  End If;
  If (p_rec.bnb_attribute29 = hr_api.g_varchar2) then
    p_rec.bnb_attribute29 :=
    ben_bnb_shd.g_old_rec.bnb_attribute29;
  End If;
  If (p_rec.bnb_attribute30 = hr_api.g_varchar2) then
    p_rec.bnb_attribute30 :=
    ben_bnb_shd.g_old_rec.bnb_attribute30;
  End If;
