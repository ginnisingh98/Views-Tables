--------------------------------------------------------
--  DDL for Package Body ISC_DBI_FR_COST_PER_W_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_FR_COST_PER_W_PKG" AS
/*$Header: ISCRGBQB.pls 120.0 2005/05/25 17:18:31 appldev noship $
    /*----------------------------------------------------
        Declare PRIVATE procedures and functions for package
    -----------------------------------------------------*/
    /* Rated Freight Cost per Unit Weight Report */
    ...
    /*------------------------------------------------
    Rated Freight Cost per Unit Weight Report Function
    -------------------------------------------------*/
    ...
    BEGIN
        ...
        poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     => 'freight_cost_wt_'||l_currency,
        ...
        poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     => 'freight_weight_g',
                                     p_alias_name   => 'freight_weight',
                                     p_grand_total  => 'Y',
                                     p_prior_code   => poa_dbi_util_pkg.BOTH_PRIORS,
                                     p_to_date_type => 'XTD');
...
--   l_filter_where  := ' (ISC_MEASURE_1 IS NOT NULL OR ISC_MEASURE_9 IS NOT NULL)';
