--------------------------------------------------------
--  DDL for Package Body PAY_PSD_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PSD_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_psd_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_sii_details_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_sii_details_id  in  number) is
--
...
Begin
  ...
  --
  pay_psd_ins.g_sii_details_id_i := p_sii_details_id;
  --
...
--
  Cursor C_Sel1 Is
    select t.created_by,
           t.creation_date
    from   pay_pl_sii_details_f t
    where  t.sii_details_id       = p_rec.sii_details_id
    and    t.effective_start_date =
             pay_psd_shd.g_old_rec.effective_start_date
    and    t.effective_end_date   = (p_validation_start_date - 1);
--
  l_proc                varchar2(72) := g_package||'dt_insert_dml';
  l_created_by          pay_pl_sii_details_f.created_by%TYPE;
  l_creation_date       pay_pl_sii_details_f.creation_date%TYPE;
  l_last_update_date    pay_pl_sii_details_f.last_update_date%TYPE;
  l_last_updated_by     pay_pl_sii_details_f.last_updated_by%TYPE;
  l_last_update_login   pay_pl_sii_details_f.last_update_login%TYPE;
--
Begin
  ...
  --
  --
  -- Insert the row into: pay_pl_sii_details_f
  --
  insert into pay_pl_sii_details_f
      (sii_details_id
...
--
  Cursor C_Sel1 is select pay_pl_sii_details_s.nextval from sys.dual;
--
 Cursor C_Sel2 is
    Select null
      from pay_pl_sii_details_f
     where sii_details_id =
             pay_psd_ins.g_sii_details_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
    If (pay_psd_ins.g_sii_details_id_i is not null) Then
    --
    ...
    p_rec.sii_details_id :=
      pay_psd_ins.g_sii_details_id_i;
    pay_psd_ins.g_sii_details_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.sii_details_id;
    Close C_Sel1;
  End If;
  --
  --
...
Begin
  ...
  --
  if p_rec.contract_category in ('CIVIL','TERM_NORMAL','LUMP','F_LUMP') then

  -- Here we are locking the corresponding Assignment record if
  --  a) the Contract category is "CIVIL"  since the SII record for Civil Contracts are stored at the Assignment level
  ...
  --  c) the Contract category is "LUMP" or "F_LUMP"
  ...
  dt_api.validate_dt_mode
    ...
    ,p_parent_key_value1       => p_rec.per_or_asg_id
    ,p_enforce_foreign_locking => true
    ,p_validation_start_date   => l_validation_start_date
    ,p_validation_end_date     => l_validation_end_date
    );

 elsif p_rec.contract_category = 'NORMAL' then
...
Procedure ins
  ...
  ,p_program_login_id               in     number   default null
...
Begin
  ...
  --
  -- Having converted the arguments into the pay_psd_rec
  ...
  --
  p_sii_details_id                   := l_rec.sii_details_id;
