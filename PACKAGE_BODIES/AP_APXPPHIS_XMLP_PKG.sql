--------------------------------------------------------
--  DDL for Package Body AP_APXPPHIS_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_APXPPHIS_XMLP_PKG" AS
/* $Header: APXPPHISB.pls 120.0 2007/12/27 08:23:33 vjaganat noship $ */
...
FUNCTION  get_nls_strings     RETURN BOOLEAN IS
   nls_all       ap_lookup_codes.displayed_field%TYPE;    nls_type      po_lookup_codes.displayed_field%TYPE;    nls_yes       fnd_lookups.meaning%TYPE;     nls_no        fnd_lookups.meaning%TYPE;
BEGIN
...
LP_START_PAYMENT_DATE := TO_CHAR(P_START_PAYMENT_DATE, 'DD-MON-YY');
LP_END_PAYMENT_DATE := TO_CHAR(P_END_PAYMENT_DATE, 'DD-MON-YY');
...
BEGIN
  ...
  END IF;
   ...
   END IF;
   IF (p_debug_switch = 'Y') THEN
      /*SRW.MESSAGE('5','After Get_Cover_Page_Values');*/null;
...
return(P_PRINT_DETAIL_FLAG = 'Y');  return (TRUE);
end;
