--------------------------------------------------------
--  DDL for Package Body IGS_PR_OU_UNIT_SET_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PR_OU_UNIT_SET_PKG" AS
/* $Header: IGSQI08B.pls 115.6 2003/02/25 09:09:24 anilk ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PR_OU_UNIT_SET_ALL%RowType;
  new_references IGS_PR_OU_UNIT_SET_ALL%RowType;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PR_POUS_PRO_FK');
      IGS_GE_MSG_STACK.ADD;
	  Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PR_POUS_US_FK');
      IGS_GE_MSG_STACK.ADD;
	  Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID IN NUMBER
    ...
    cursor C is select ROWID from IGS_PR_OU_UNIT_SET_ALL
      where PROGRESSION_RULE_CAT = X_PROGRESSION_RULE_CAT
      and PRA_SEQUENCE_NUMBER = X_PRA_SEQUENCE_NUMBER
      and PRO_SEQUENCE_NUMBER = X_PRO_SEQUENCE_NUMBER
      and UNIT_SET_CD = X_UNIT_SET_CD
...
Before_DML (
  ...
  insert into IGS_PR_OU_UNIT_SET_ALL (
...
Before_DML (
  ...
  delete from IGS_PR_OU_UNIT_SET_ALL
  where ROWID  = X_ROWID;
