--------------------------------------------------------
--  DDL for Package Body IGI_DOS_TRX_SOURCES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_DOS_TRX_SOURCES_PKG" AS
/* $Header: igidostb.pls 120.7.12000000.2 2007/06/14 05:55:29 pshivara ship $ */
...
l_unexp_level   number;
  ...
  l_rowid VARCHAR2(25);
  old_references igi_dos_trx_sources%ROWTYPE;
  new_references igi_dos_trx_sources%ROWTYPE;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.set_column_values.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
    ...
    new_references.budget_org_id                     := x_budget_org_id;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.check_parent_existance.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.check_parent_existance.Msg2',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
  ...
  */
  BEGIN
    ...
    igi_dos_trx_dest_pkg.get_fk_igi_dos_trx_sources (
      old_references.source_trx_id
    );

   /* sowsubra  06-JUN-2002 start */
   /* Commented the following code as this  igi_dos_trx_sources_hist_pkg
   	is currently not present  */

    /*igi_dos_trx_sources_hist_pkg.get_fk_igi_dos_trx_sources (
      old_references.source_trx_id );
	*/
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.get_fk_igi_dos_doc_types.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
      RETURN;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.get_fk_igi_dos_trx_headers.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
      RETURN;
...
-- bug 3199481, start block
        IF (l_unexp_level >= l_debug_level) THEN
           FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.before_dml.Msg1',FALSE);
        END IF;
-- bug 3199481, end block
        app_exception.raise_exception;
      END IF;
...
-- bug 3199481, start block
        IF (l_unexp_level >= l_debug_level) THEN
           FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.before_dml.Msg2',FALSE);
        END IF;
-- bug 3199481, end block
        app_exception.raise_exception;
      END IF;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.insert_row.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;

    SELECT    igi_dos_trx_sources_s.NEXTVAL
    INTO      x_source_trx_id
    FROM      dual;
    ...
    before_dml(
      ...
      x_budget_org_id                     => x_budget_org_id,
    ...
    INSERT INTO igi_dos_trx_sources (
      sob_id,
      trx_id,
      source_trx_id,
      source_id,
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.lock_row.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      CLOSE c1;
      app_exception.raise_exception;
    ...
    IF (
        ...
        AND ((tlinfo.budget_org_id = x_budget_org_id) OR ((tlinfo.budget_org_id IS NULL) AND (X_budget_org_id IS NULL)))
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.lock_row.Msg2',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_trx_sources_pkg.update_row.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
    ...
    UPDATE igi_dos_trx_sources
      SET
        ...
        budget_org_id                     = new_references.budget_org_id,
  ...
  */
  BEGIN
   ...
   -- before_dml (
    ...
    DELETE FROM igi_dos_trx_sources
    WHERE rowid = x_rowid;
