--------------------------------------------------------
--  DDL for Package Body PER_PREV_JOB_INFO_TYPES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PREV_JOB_INFO_TYPES_PKG" as
/* $Header: perpjxit.pkb 115.1 2002/12/06 15:20:42 eumenyio ship $ */
--------------------------------------------------------------------------------
...
CURSOR C1 IS
 	select  1
 	from    PER_PREV_JOB_INFO_TYPES t
...
procedure INSERT_ROW (
  ...
  X_MULTIPLE_OCCURANCES_FLAG in VARCHAR2,
  ...
  cursor C is select ROWID from PER_PREV_JOB_INFO_TYPES
...
begin
  ...
  if (    (recinfo.ACTIVE_INACTIVE_FLAG = X_ACTIVE_INACTIVE_FLAG)
      AND (recinfo.MULTIPLE_OCCURANCES_FLAG = X_MULTIPLE_OCCURANCES_FLAG)
...
begin
  update PER_PREV_JOB_INFO_TYPES set
    ACTIVE_INACTIVE_FLAG = X_ACTIVE_INACTIVE_FLAG,
    MULTIPLE_OCCURANCES_FLAG = X_MULTIPLE_OCCURANCES_FLAG,
...
begin
  ...
  delete from PER_PREV_JOB_INFO_TYPES
  where INFORMATION_TYPE = X_INFORMATION_TYPE;
...
is
  l_proc                        VARCHAR2(61) := 'PER_PREV_JOB_INFO_TYPES_PKG.LOAD_ROW';
  l_rowid                       rowid;
  l_progam_application_id       PER_PEOPLE_INFO_types.program_application_id%TYPE;
  l_program_id                  PER_PEOPLE_INFO_types.program_id%TYPE;
  l_program_update_date         PER_PEOPLE_INFO_types.program_update_date%TYPE;
...
begin
  ...
  begin
    UPDATE_ROW
      ...
      ,X_MULTIPLE_OCCURANCES_FLAG => X_MULTIPLE_OCCURANCES_FLAG
...
END PER_PREV_JOB_INFO_TYPES_PKG;
