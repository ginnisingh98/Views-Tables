--------------------------------------------------------
--  DDL for Package Body PAY_MX_CURRENCY_CONVERSION
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_MX_CURRENCY_CONVERSION" AS
/* $Header: paymxcurrconv.pkb 120.1 2005/11/20 22:20:58 vmehta noship $ */
--
/*
   ******************************************************************
   *                                                                *
   ...
   ******************************************************************
    ...
    -----------
     ...
     20-Nov-2005 vmehta    115.1             ota_activity_version does not have
                                             business_group_id in older version
                                             of the table.
...
*/
  ...
  CURSOR get_all_bgs( cp_old_currency VARCHAR2 ) IS
    SELECT hoi.organization_id, hou.name
      FROM hr_organization_units hou,
           hr_organization_information hoi,
           hr_organization_information hoi1
     WHERE hoi.org_information_context  = 'Business Group Information'
       AND hoi.org_information9         = 'MX'
       AND hoi.org_information10        = cp_old_currency
       ...
       AND NOT EXISTS ( SELECT 1
                         ...
                         WHERE pps.patch_number = to_char(hoi.organization_id)
                           AND pps.patch_name = 'Currency_Conversion_for_Mexico'
                           AND pps.legislation_code = 'MX'
                           AND pps.status = 'C' );
  ...
  BEGIN
    ...
    OPEN  get_all_bgs( lv_old_currency );
    LOOP
      ...
      VALUES      (pay_patch_status_s.nextval, to_char(ln_business_group_id),
                   'Currency_Conversion_for_Mexico', sysdate, 'C',
                   lv_business_group_name,  sysdate, 'MX');
