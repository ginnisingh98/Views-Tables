--------------------------------------------------------
--  DDL for Package Body PQH_BCE_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_BCE_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_bce_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy pqh_bce_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_bdgt_cmmtmnt_elmnts Row
  --
  update pqh_bdgt_cmmtmnt_elmnts
  set
  bdgt_cmmtmnt_elmnt_id             = p_rec.bdgt_cmmtmnt_elmnt_id,
  budget_id                         = p_rec.budget_id,
  actual_commitment_type            = p_rec.actual_commitment_type,
  element_type_id                   = p_rec.element_type_id,
  salary_basis_flag                 = p_rec.salary_basis_flag,
  element_input_value_id            = p_rec.element_input_value_id,
  balance_type_id                   = p_rec.balance_type_id,
  frequency_input_value_id          = p_rec.frequency_input_value_id,
  formula_id                        = p_rec.formula_id,
  dflt_elmnt_frequency              = p_rec.dflt_elmnt_frequency,
  overhead_percentage               = p_rec.overhead_percentage,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in pqh_bce_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy pqh_bce_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.budget_id = hr_api.g_number) then
    p_rec.budget_id :=
    pqh_bce_shd.g_old_rec.budget_id;
  End If;
  If (p_rec.actual_commitment_type = hr_api.g_varchar2) then
    p_rec.actual_commitment_type :=
    pqh_bce_shd.g_old_rec.actual_commitment_type;
  End If;
  If (p_rec.element_type_id = hr_api.g_number) then
    p_rec.element_type_id :=
    pqh_bce_shd.g_old_rec.element_type_id;
  End If;
  If (p_rec.salary_basis_flag = hr_api.g_varchar2) then
    p_rec.salary_basis_flag :=
    pqh_bce_shd.g_old_rec.salary_basis_flag;
  End If;
  If (p_rec.element_input_value_id = hr_api.g_number) then
    p_rec.element_input_value_id :=
    pqh_bce_shd.g_old_rec.element_input_value_id;
  End If;
  If (p_rec.balance_type_id = hr_api.g_number) then
    p_rec.balance_type_id :=
    pqh_bce_shd.g_old_rec.balance_type_id;
  End If;
  If (p_rec.frequency_input_value_id = hr_api.g_number) then
    p_rec.frequency_input_value_id :=
    pqh_bce_shd.g_old_rec.frequency_input_value_id;
  End If;
  If (p_rec.formula_id = hr_api.g_number) then
    p_rec.formula_id :=
    pqh_bce_shd.g_old_rec.formula_id;
  End If;
  If (p_rec.dflt_elmnt_frequency = hr_api.g_varchar2) then
    p_rec.dflt_elmnt_frequency :=
    pqh_bce_shd.g_old_rec.dflt_elmnt_frequency;
  End If;
  If (p_rec.overhead_percentage = hr_api.g_number) then
    p_rec.overhead_percentage :=
    pqh_bce_shd.g_old_rec.overhead_percentage;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  pqh_bce_bus.update_validate(p_rec
  ,p_effective_date);
  --
