--------------------------------------------------------
--  DDL for Package Body ENG_CHANGE_TEXT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ENG_CHANGE_TEXT_PVT" AS
/* $Header: ENGVIDXB.pls 120.2 2006/02/21 00:52:53 lkasturi noship $ */

G_PKG_NAME	CONSTANT  VARCHAR2(30)  :=  'ENG_CHANGE_TEXT_PVT';
...
g_Log_File		VARCHAR2(30)  :=  'eng_change_text_tl_srs.log';
g_Out_File		VARCHAR2(30)  :=  'eng_cchange_text_tl_srs.out';
...
IS
   ...
   l_pre_name          ctxsys.ctx_preferences.pre_name%TYPE; -- Bug 4958959
BEGIN
   ...
   FOR sub_lexer_rec IN		( SELECT pre_owner, pre_name
                       		  FROM ctxsys.ctx_preferences
                       		  WHERE pre_name LIKE 'ENG_CHANGE_LEXER%' )
   LOOP
      ad_ctx_ddl.drop_preference (sub_lexer_rec.pre_owner ||'.'|| sub_lexer_rec.pre_name);
   END LOOP;

   l_pre_name := 'ENG_CHANGE_WORDLIST'; -- Bug 4958959
   ...
   FOR stoplist_rec IN		( SELECT spl_owner, spl_name
                       		  ...
                       		  WHERE spl_name = 'ENG_CHANGE_STOPLIST' )
   LOOP
      ad_ctx_ddl.Drop_Stoplist (stoplist_rec.spl_owner || '.ENG_CHANGE_STOPLIST');
   END LOOP;

   FOR section_group_rec IN	( SELECT sgp_owner, sgp_name
                       		  FROM ctxsys.ctx_section_groups
                       		  WHERE sgp_name = 'ENG_CHANGE_SECTION_GROUP' )
   LOOP
      ad_ctx_ddl.Drop_Section_Group (section_group_rec.sgp_owner ||'.'|| section_group_rec.sgp_name);
   END LOOP;

   l_pre_name := 'ENG_CHANGE_DATASTORE'; -- Bug 4958959
   ...
   l_pre_name := 'ENG_CHANGE_STORAGE'; -- Bug 4958959
   ...
   FROM all_tables
   WHERE owner = g_Prod_Schema AND table_name = 'ENG_CHANGE_IMTEXT_TL';
   ...
   WHERE owner = g_Prod_Schema
     AND index_name = 'ENG_CHANGE_IMTEXT_TL_U1'
     AND table_name = 'ENG_CHANGE_IMTEXT_TL';

   Log_Line (l_api_name || ': creating STORAGE preference ...');

   ad_ctx_ddl.create_preference (g_Pref_Owner || '.ENG_CHANGE_STORAGE', 'BASIC_STORAGE');

   ad_ctx_ddl.set_attribute (g_Pref_Owner || '.ENG_CHANGE_STORAGE',
   ...
   ad_ctx_ddl.Create_Preference (g_Pref_Owner || '.ENG_CHANGE_DATASTORE', 'USER_DATASTORE');

   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_DATASTORE', 'OUTPUT_TYPE', 'CLOB');
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_DATASTORE', 'PROCEDURE', '"ENG_CHANGE_TEXT_CTX_PKG"."Get_Change_Text_CLOB"');

   --ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_DATASTORE', 'OUTPUT_TYPE', 'VARCHAR2');
   --ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_DATASTORE', 'PROCEDURE', '"ENG_CHANGE_TEXT_CTX_PKG"."Get_Change_Text_VARCHAR2"');
   ...
   --ad_ctx_ddl.Create_Section_Group (g_Pref_Owner || '.ENG_CHANGE_STOPLIST', 'AUTO_SECTION_GROUP');
   ...
   ad_ctx_ddl.Create_Stoplist (g_Pref_Owner || '.ENG_CHANGE_STOPLIST');
   ...
   ad_ctx_ddl.Create_Preference (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'BASIC_WORDLIST');

   -- Enable prefix indexing to improve performance for wildcard searches
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'PREFIX_INDEX', 'TRUE');
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'PREFIX_MIN_LENGTH', 2);
   --ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'PREFIX_LENGTH_MIN', 2);
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'PREFIX_MAX_LENGTH', 32);
   --ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'PREFIX_LENGTH_MAX', 32);
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'WILDCARD_MAXTERMS', 5000);
   ...
   --
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'SUBSTRING_INDEX', 'FALSE');
   ...
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'STEMMER', 'AUTO');
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'FUZZY_MATCH', 'AUTO');
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'FUZZY_SCORE', 40);
   ad_ctx_ddl.Set_Attribute (g_Pref_Owner || '.ENG_CHANGE_WORDLIST', 'FUZZY_NUMRESULTS', 120);
   ...
   LOOP
      Lexer_Name := g_Pref_Owner || '.ENG_CHANGE_LEXER_' || Lang_Code_List(i);

      IF ( Lang_Code_List(i) = 'JA' ) THEN

         -- Use JAPANESE_LEXER if db charset is UTF8, JA16SJIS, or JA16EUC.
         IF ( ENG_CHANGE_TEXT_UTIL.get_DB_Version_Num >= 9.0 ) THEN
      ...
      ELSIF ( Lang_Code_List(i) IN ('ZHS', 'ZHT') ) THEN
         ...
         IF ( ENG_CHANGE_TEXT_UTIL.get_DB_Version_Num >= 9.2 ) THEN
   ...
   ad_ctx_ddl.Create_Preference (g_Pref_Owner || '.ENG_CHANGE_MULTI_LEXER', 'MULTI_LEXER');
   ...
   LOOP
      ...
      IF ( Lang_Code_List(i) = 'US' ) THEN
         ...
         ad_ctx_ddl.Add_Sub_Lexer ( g_Pref_Owner || '.ENG_CHANGE_MULTI_LEXER', 'DEFAULT'
                                  , g_Pref_Owner || '.ENG_CHANGE_LEXER_US' );
      ELSE
         ad_ctx_ddl.Add_Sub_Lexer
         (  lexer_name    =>  g_Pref_Owner || '.ENG_CHANGE_MULTI_LEXER'
...
-- -----------------------------------------------------------------------------
--			   Process_Change_Text_Index
-- -----------------------------------------------------------------------------

PROCEDURE Process_Change_Text_Index
(
   p_Index_Name		IN           VARCHAR2
,  p_Action		IN           VARCHAR2
,  x_return_status	OUT  NOCOPY  VARCHAR2 -- Bug#4055218: Datatype of x_return_status changed to VARCHAR2
	                                      -- as in 8i DB Varchar2 data can't be assigned to CLOB.
)
IS
   l_api_name		CONSTANT  VARCHAR2(30)  :=  'Process_Change_Text_Index';
   l_return_status		VARCHAR2(1);
...
BEGIN
   ...
   -- Check for existing indexes in the ENG product, APPS Universal, and interMedia schemas.
   --
   ...
   IF ( l_create_index ) THEN
      ...
      IF ( ENG_CHANGE_TEXT_UTIL.get_DB_Version_Num >= 9.2 ) THEN
         ...
         Log_Line (l_api_name || ': DB version: ' || ENG_CHANGE_TEXT_UTIL.get_DB_Version_Str || ', using parallel clause: ' || l_index_parallel);
      END IF;
      ...
      BEGIN
         ...
         ENG_CHANGE_TEXT_UTIL.Set_Context ('CREATE_INDEX');

         sql_stmt :=
            'CREATE INDEX ' || g_Index_Owner || '.' || p_Index_Name              ||
            ' ON ' || g_Prod_Schema || '.ENG_CHANGE_IMTEXT_TL (text)               ' ||
            ' INDEXTYPE IS CTXSYS.context                                      ' ||
            ' PARAMETERS                                                       ' ||
            ' (''DATASTORE      ' || g_Pref_Owner || '.ENG_CHANGE_DATASTORE      ' ||
            '  WORDLIST         ' || g_Pref_Owner || '.ENG_CHANGE_WORDLIST       ' ||
            '  STOPLIST         ' || g_Pref_Owner || '.ENG_CHANGE_STOPLIST       ' ||
            '  LEXER            ' || g_Pref_Owner || '.ENG_CHANGE_MULTI_LEXER    ' ||
            '  LANGUAGE COLUMN  language                                       ' ||
            --'  SECTION GROUP    ' || g_Pref_Owner || '.ENG_CHANGE_STOPLIST  ' ||
            '  SECTION GROUP    CTXSYS.NULL_SECTION_GROUP  '                     ||
            '  STORAGE          ' || g_Pref_Owner || '.ENG_CHANGE_STORAGE        ' ||
...
/*
      ...
      ENG_CHANGE_TEXT_UTIL.Incremental_Sync
      (
...
*/
      ...
      END IF;
       ...
       Log_Line (l_api_name || 'Completed building Change Management Text Index.');

      -- Check the created index status

         Log_Line (l_api_name || 'Checking Status of Change Management Text Index.');

         SELECT idx_docid_count, idx_status
           INTO l_idx_docid_count, l_idx_status
         FROM ctxsys.ctx_indexes
         WHERE idx_owner = g_Index_Owner AND idx_name = 'ENG_CHANGE_IMTEXT_TL_CTX1'
           AND idx_table = 'ENG_CHANGE_IMTEXT_TL';
...
END Process_Change_Text_Index;

-- -----------------------------------------------------------------------------
--			     Build_Change_Text_Index
-- -----------------------------------------------------------------------------

PROCEDURE Build_Change_Text_Index
(
...
IS
   l_api_name		CONSTANT  VARCHAR2(30)  :=  'Build_Change_Text_Index';
   l_Index_Name			VARCHAR2(30)    :=  'ENG_CHANGE_IMTEXT_TL_CTX1';
   l_return_status		VARCHAR2(1);
BEGIN
   ...
   Log_Line (l_api_name || ': calling Process_Change_Text_Index ...');
