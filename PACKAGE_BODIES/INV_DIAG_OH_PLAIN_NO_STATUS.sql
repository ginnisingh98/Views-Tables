--------------------------------------------------------
--  DDL for Package Body INV_DIAG_OH_PLAIN_NO_STATUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_DIAG_OH_PLAIN_NO_STATUS" as
/* $Header: INVDOH1B.pls 120.0.12010000.2 2009/03/18 09:34:08 aambulka noship $ */
...
BEGIN
  ...
  sqltxt :=' SELECT   mp.organization_code' ||
   ...
   dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sqltxt,'Onhand with status not populated for Onhand material status tracked organization for non serial controlled Item');
...
EXCEPTION
  when others then
    ...
    fixInfo := 'Unexpected Exception in INVDOH1B.pls';
    isFatal := 'FALSE';
    report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
    reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
END runTest;
...
BEGIN
   descStr := 'Onhand with status not populated for Onhand material status tracked organization for non serial controlled Item';
END getTestDesc;

PROCEDURE getTestName(name OUT NOCOPY VARCHAR2) IS
BEGIN
   name := 'Onhand Plain Item Status Not populated';
END getTestName;
...
END getTestMode;
END INV_DIAG_OH_PLAIN_NO_STATUS;
