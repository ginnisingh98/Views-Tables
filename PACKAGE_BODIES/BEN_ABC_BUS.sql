--------------------------------------------------------
--  DDL for Package Body BEN_ABC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ABC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_abc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_acty_base_rt_ctfn_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   acty_base_rt_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_acty_base_rt_ctfn_id(p_acty_base_rt_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_acty_base_rt_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_abc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_acty_base_rt_ctfn_id                => p_acty_base_rt_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_acty_base_rt_ctfn_id,hr_api.g_number)
     <>  ben_abc_shd.g_old_rec.acty_base_rt_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_abc_shd.constraint_error('BEN_acty_base_rt_ctfn_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_acty_base_rt_ctfn_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_enrt_ctfn_typ_cd(p_acty_base_rt_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_abc_shd.g_old_rec.enrt_ctfn_typ_cd,hr_api.g_varchar2)
...
--     p_acty_base_rt_ctfn_id          is primary key for ben_acty_base_rt_ctfn_f
--     p_acty_base_rt_id
--     p_business_group_id
...
cursor c1 is select null
              ...
              Where acty_base_rt_ctfn_id <> nvl(p_acty_base_rt_ctfn_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_rqd_flag(p_acty_base_rt_ctfn_id           in number,
...
Begin
  ...
  if (l_api_updating
      and p_rqd_flag
      <> nvl(ben_abc_shd.g_old_rec.rqd_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_acty_base_rt_ctfn_id              in number,
...
function return_legislation_code
  (p_acty_base_rt_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_acty_base_rt_ctfn_f b
    where b.acty_base_rt_ctfn_id      = p_acty_base_rt_ctfn_id
