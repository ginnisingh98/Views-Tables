--------------------------------------------------------
--  DDL for Package Body PSP_PSO_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSP_PSO_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  psp_pso_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_salary_cap_override_id      number         default null;
--
--  ---------------------------------------------------------------------------
...
Procedure set_security_group_id
  ...
  -- psp_salary_cap_overrides and PER_BUSINESS_GROUPS_PERF
...
begin
  ...
  hr_api.mandatory_arg_error
    (p_api_name           => l_proc
    ,p_argument           => 'salary_cap_override_id'
    ,p_argument_value     => p_salary_cap_override_id
    );
  --
  open csr_sec_grp;
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'SALARY_CAP_OVERRIDE_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(psp_pso_bus.g_salary_cap_override_id, hr_api.g_number)
       = p_salary_cap_override_id) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := psp_pso_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    psp_pso_bus.g_salary_cap_override_id      := p_salary_cap_override_id;
    psp_pso_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT psp_pso_shd.api_updating
      (p_salary_cap_override_id            => p_rec.salary_cap_override_id
...
--
  ...
  PROCEDURE validate_dates( p_salary_cap_override_id	IN	NUMBER
...
--Cursor date_selection_csr: Select the dates for the accounts(other than the current one) of this organization.
  ...
  BEGIN
	...
	IF (l_start_flag = false) OR  (l_end_flag = FALSE) THEN
		fnd_message.set_name ('PSP', 'PSP_SC_NOT_DEFINED');
		fnd_message.raise_error;
	END IF;
	...
	LOOP
		...
		IF p_start_date BETWEEN l_start_date AND l_end_date THEN
			OPEN project_number_csr;
			FETCH  project_number_csr INTO l_project_number;
			CLOSE project_number_csr;

			fnd_message.set_name ('PSP', 'PSP_SC_BEGINDATE_OVERLAP_PRJ');
	...
	LOOP
		...
		IF p_end_date BETWEEN l_start_date AND l_end_date THEN
			...
			fnd_message.set_name('PSP', 'PSP_SC_ENDDATE_OVERLAP_PRJ');
			fnd_message.set_token('BEGIN_DATE', TO_CHAR(l_start_date ));
			fnd_message.set_token('END_DATE', l_end_date);
			fnd_message.set_token('PROJECT_NUMBER', l_project_number);
		        fnd_message.raise_error;
...
Begin
  ...
  --
  --
  validate_dates( p_salary_cap_override_id	=>	p_rec.salary_cap_override_id
