--------------------------------------------------------
--  DDL for Package Body OTA_DELEGATE_BOOKING_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_DELEGATE_BOOKING_BE1" as 
...
p_create_finance_line          varchar2,
...
p_is_mandatory_enrollment      varchar2,
p_booking_id                   number) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  ota_delegate_booking_be1.create_delegate_booking_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.ota.api.delegate_booking.create_delegate_booking';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<create_finance_line>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_create_finance_line);
    l_text:=l_text||'</create_finance_line>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<is_mandatory_enrollment>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_is_mandatory_enrollment);
    l_text:=l_text||'</is_mandatory_enrollment>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<booking_id>';
...
end create_delegate_booking_a;
end ota_delegate_booking_be1;
