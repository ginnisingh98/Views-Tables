--------------------------------------------------------
--  DDL for Package Body PAY_ELEMENT_LINK_BE3
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ELEMENT_LINK_BE3" as 
...
p_entries_warning              boolean) is
  ...
  --
  l_proc varchar2(72):='  pay_element_link_be3.delete_element_link_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.pay.api.element_link.delete_element_link';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<entries_warning>';
if(P_ENTRIES_WARNING) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</entries_warning>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</element_link>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
