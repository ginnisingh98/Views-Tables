--------------------------------------------------------
--  DDL for Package Body PAY_CPT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CPT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_cpt_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (p_constraint_name = 'PAY_CA_EMP_PROVIN_TAX_RULES_PK') Then
...
Begin
  ...
  If (p_effective_date is null or
      p_emp_province_tax_inf_id is null or
    ...
    l_fct_ret := false;
  Else
    If (p_emp_province_tax_inf_id = g_old_rec.emp_province_tax_inf_id and
...
Begin
  ...
  --
/* Only ZAP mode of deletion is allowed */
...
Begin
  ...
  If (p_datetrack_mode <> 'INSERT') then
	...
	(p_effective_date	   => p_effective_date,
	 ...
	 p_base_table_name	   => 'pay_ca_emp_prov_tax_info_f',
	 p_base_key_column	   => 'emp_province_tax_inf_id',
	 p_base_key_value 	   => p_emp_province_tax_inf_id,
	 p_parent_table_name1      => 'per_all_assignments_f',
	 p_parent_key_column1      => 'assignment_id',
...
Exception
  When HR_Api.Object_Locked then
    ...
    hr_utility.set_message_token('TABLE_NAME', 'pay_ca_emp_prov_tax_info_f');
    hr_utility.raise_error;
...
Begin
  ...
  l_rec.province_code                    := p_province_code;
  ...
  l_rec.deduction_code                   := p_deduction_code;
  l_rec.extra_info_not_provided          := p_extra_info_not_provided;
  l_rec.marriage_status                  := p_marriage_status;
  l_rec.no_of_infirm_dependants          := p_no_of_infirm_dependants;
  l_rec.non_resident_status              := p_non_resident_status;
  l_rec.disability_status                := p_disability_status;
  l_rec.no_of_dependants                 := p_no_of_dependants;
  ...
  l_rec.prov_override_rate               := p_prov_override_rate;
  l_rec.prov_override_amount             := p_prov_override_amount;
  l_rec.prov_exempt_flag                 := p_prov_exempt_flag;
  l_rec.pmed_exempt_flag                 := p_pmed_exempt_flag;
  l_rec.wc_exempt_flag                   := p_wc_exempt_flag;
  l_rec.qpp_exempt_flag                  := p_qpp_exempt_flag;
  l_rec.tax_calc_method                  := p_tax_calc_method;
  l_rec.other_tax_credit                 := p_other_tax_credit;
  ...
  l_rec.prov_lsp_amount            	 := p_prov_lsp_amount;
  l_rec.ppip_exempt_flag                  := p_ppip_exempt_flag;
  --
