--------------------------------------------------------
--  DDL for Package Body XNP_CSICYCNT_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSICYCNT_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSICYCNT' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSICYCNT' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSICYCNT' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSICYCNT.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSICYCNT', l_consumer_list );
  END IF;
...
BEGIN
  ...
  csi_t_gen_utility_pvt.add('CSICYCNT'||'-'||to_char(sysdate,'DD-MON-YYYY HH24:MI:SS'));
