--------------------------------------------------------
--  DDL for Package Body IGS_PS_OFR_NOTE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_OFR_NOTE_PKG" AS
/* $Header: IGSPI21B.pls 115.3 2002/11/29 02:12:05 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_OFR_NOTE%RowType;
  new_references IGS_PS_OFR_NOTE%RowType;
 ...
 BEGIN
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CON_CNT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CON_CO_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CON_NOTE_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_OFR_NOTE
      where COURSE_CD = X_COURSE_CD
      and REFERENCE_NUMBER = X_REFERENCE_NUMBER
      and CAL_TYPE = X_CAL_TYPE
      and VERSION_NUMBER = X_VERSION_NUMBER;
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_OFR_NOTE
     where COURSE_CD = X_COURSE_CD
     and REFERENCE_NUMBER = X_REFERENCE_NUMBER
     and CAL_TYPE = X_CAL_TYPE
     and VERSION_NUMBER = X_VERSION_NUMBER
