--------------------------------------------------------
--  DDL for Package Body AME_STV_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_STV_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_stv_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  begin
    ...
    ame_stv_rku.after_update
      ...
      ,p_end_date_o
      => ame_stv_shd.g_old_rec.end_date
      ,p_security_group_id_o
      => ame_stv_shd.g_old_rec.security_group_id
      ,p_object_version_number_o
      => ame_stv_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
 ...
 ,p_string_value =>  p_rec.string_value
  ...
  --
  ame_stv_upd.convert_defs(p_rec);
  --
  ame_stv_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
