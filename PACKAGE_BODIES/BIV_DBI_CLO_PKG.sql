--------------------------------------------------------
--  DDL for Package Body BIV_DBI_CLO_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIV_DBI_CLO_PKG" 
/* $Header: bivsrvrclob.pls 120.0 2005/05/25 10:55:26 appldev noship $ */
as

  g_closure_rep_func         varchar2(50) := 'BIV_DBI_CLO_TBL_REP';
  g_closure_dbn_rep_func     varchar2(50) := 'BIV_DBI_CLO_DBN_TBL_REP';

  g_closed_detail_rep_func   varchar2(50) := 'BIV_DBI_CLO_DTL_REP';
...
begin
  ...
  poa_dbi_util_pkg.add_column( p_col_tbl    => l_col_tbl
                             , p_col_name   => 'total_time_to_close'
                             , p_alias_name => 'time_to_close'
                             , p_to_date_type => l_to_date_type
                             );

  biv_dbi_tmpl_util.add_bucket_inner_query
  ( p_short_name   => 'BIV_DBI_CLOSURE_CYCLE_TIME'
  , p_col_tbl      => l_col_tbl
  , p_col_name     => 'time_to_close'
...
IF (l_xtd = 'WTD')
  THEN
  l_stmt := l_stmt ||','||'''AS_OF_DATE=''||to_char(cal.end_date,''dd/mm/yyyy'')||''&pFunctionName=BIV_DBI_CLO_TRD_REP&pParamIds=Y&VIEW_BY=TIME+FII_TIME_DAY&FII_TIME_DAY=FII_TIME_DAY'' BIV_DYNAMIC_URL1' || ',NULL BIV_DYNAMIC_URL2';
ELSIF (l_xtd = 'RLW') THEN
  l_stmt := l_stmt ||',NULL BIV_DYNAMIC_URL1 ,'||'''AS_OF_DATE=''||to_char(cal.end_date,''dd/mm/yyyy'')||''&pFunctionName=BIV_DBI_CLO_TRD_REP&pParamIds=Y&VIEW_BY=TIME+FII_TIME_DAY&FII_TIME_DAY=FII_TIME_DAY'' BIV_DYNAMIC_URL2';
  ELSE
...
begin
  ...
  if l_order_by like '% DESC%' then
    if l_order_by like '%BIV_MEASURE11%' then
      l_order_by := 'fact.closed_date desc, fact.incident_id desc';
    else
      l_order_by := 'fact.age desc, fact.incident_id desc';
    end if;
  else
    if l_order_by like '%BIV_MEASURE11%' then
      l_order_by := 'fact.closed_date asc, fact.incident_id asc';
    else
...
, fnd_date.date_to_displaydate(fact.closed_date) biv_measure11 ' ||
  case
...
from
  ( select
    ...
    from
      ' || l_mv || ' fact
    where
        fact.closed_date between &BIS_CURRENT_EFFECTIVE_START_DATE and &BIS_CURRENT_EFFECTIVE_END_DATE
' || l_where_clause || '
  ) fact
' || l_join_from || '
