--------------------------------------------------------
--  DDL for Package Body BIM_EDW_IH_RESLT_M_SIZE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIM_EDW_IH_RESLT_M_SIZE" AS
/* $Header: bimszrtb.pls 115.0 2001/03/14 12:03:46 pkm ship       $*/
...
CURSOR c_cnt_rows IS
   ...
   from (
	select count(*) cnt
	FROM JTF_IH_RESULTS_VL RSLT ,
	EDW_LOCAL_INSTANCE INST
        where RSLT.last_update_date between
...
PROCEDURE est_row_len(p_from_date DATE,
 ...
 X_RESULT_CODE NUMBER;
 X_POSITIVE_RESPONSE_FLAG NUMBER;
 ...
 x_INSTANCE NUMBER;
  ...
  CURSOR c_1 IS
	SELECT
	avg(nvl(vsize(RESULT_ID ), 0)),
	avg(nvl(vsize(SHORT_DESCRIPTION ), 0)),
	avg(nvl(vsize(RESULT_CODE ), 0)),
	avg(nvl(vsize(POSITIVE_RESPONSE_FLAG ), 0)),
  ...
  BEGIN
    ...
    x_total := 4  +
	 ...
	 ceil(X_RESULT_CODE + 1) +
	 ceil(X_POSITIVE_RESPONSE_FLAG + 1) +
...
END;  -- package body BIM_EDW_IH_RESLT_M_SIZE
