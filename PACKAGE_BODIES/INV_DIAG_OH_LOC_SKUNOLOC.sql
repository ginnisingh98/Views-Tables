--------------------------------------------------------
--  DDL for Package Body INV_DIAG_OH_LOC_SKUNOLOC
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_DIAG_OH_LOC_SKUNOLOC" as
/* $Header: INVDO05B.pls 120.0.12000000.1 2007/06/22 01:03:19 musinha noship $ */
...
BEGIN
   ...
   IF l_org_id IS NOT NULL THEN
      ...
      dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sqltxt,'Onhand with locator for SKU not locator controlled');
...
EXCEPTION
  when others then
    ...
    fixInfo := 'Unexpected Exception in INVDO05B.pls';
    isFatal := 'FALSE';
    report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
    reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
END runTest;
...
BEGIN
descStr := 'Onhand with locator when SKU is not locator controlled';
END getTestDesc;

PROCEDURE getTestName(name OUT NOCOPY VARCHAR2) IS
BEGIN
name := 'Onhand Loc SKU NoLoc';
END getTestName;
