--------------------------------------------------------
--  DDL for Package Body POS_CONTROL_REGION_SV
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."POS_CONTROL_REGION_SV" AS
/* $Header: POSCTRLB.pls 115.0 99/08/20 11:09:18 porting sh $*/


  /* -------------- Private Procedures -------------- */
  PROCEDURE InitializeRegion;
  PROCEDURE CloseRegion;
  PROCEDURE PaintButtons(p_position VARCHAR2);
...
--     htp.tableClose;
  ...
  PROCEDURE PaintButtons(p_position VARCHAR2) IS
  BEGIN
  ...
  PROCEDURE PaintSubmitButton(p_position VARCHAR2) IS
  BEGIN
    ...
    IF (upper(p_position) = 'SELECT') THEN
      htp.p('<font class=disabledbutton>');
    ELSE
      htp.p('<a href="javascript:top.submitClicked(top.content)">');
      htp.p('<font class=button>');
    END IF;
   ...
   * ------------------
   */
  PROCEDURE PaintControlRegion(p_position VARCHAR2) IS
  BEGIN
