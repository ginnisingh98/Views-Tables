--------------------------------------------------------
--  DDL for Package Body MTH_PROD_SEGMENTS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MTH_PROD_SEGMENTS_EXT_PKG" as
/* $Header: mthuwstlb.pls 120.0.12010000.3 2009/08/21 09:53:51 sdonthu noship $ */
/*
...
is
begin
  delete from MTH_PRODUCTION_SEGMENTS_EXT_TL T
  where not exists
    (select NULL
    from MTH_PRODUCTION_SEGMENTS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update MTH_PRODUCTION_SEGMENTS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
      ...
      B.TL_EXT_ATTR29,
      B.TL_EXT_ATTR30
    from MTH_PRODUCTION_SEGMENTS_EXT_TL B
    where B.EXTENSION_ID = T.EXTENSION_ID
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from MTH_PRODUCTION_SEGMENTS_EXT_TL SUBB, MTH_PRODUCTION_SEGMENTS_EXT_TL SUBT
  ...
  insert into MTH_PRODUCTION_SEGMENTS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from MTH_PRODUCTION_SEGMENTS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
