--------------------------------------------------------
--  DDL for Package Body PNRX_C_RENT_ROLL_LEASE_EXP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PNRX_C_RENT_ROLL_LEASE_EXP" AS
/* $Header: PNRXCRLB.pls 115.6 2002/11/15 20:50:44 stripath ship $ */

PROCEDURE pn_c_rent_roll_lease_exp(
  errbuf            OUT NOCOPY VARCHAR2,
  retcode           OUT NOCOPY VARCHAR2,
  argument1         IN  VARCHAR2,   --lease_responsible_user
  argument2         IN  VARCHAR2,   --location_code_low
  argument3         IN  VARCHAR2,   --location_code_high
  argument4         IN  VARCHAR2,   --lease_type
  argument5         IN  VARCHAR2,   --lease_number_low
  argument6         IN  VARCHAR2,   --lease_number_high
  argument7         IN  VARCHAR2,   --lease_termination_from
  argument8         IN  VARCHAR2,   --lease_termination_to
  argument9         IN  VARCHAR2,   --lease_status
  argument10        IN  VARCHAR2,   --lease_class 		bug#2099864
...
IS
  ...
  BEGIN
    ...
    pnrx_rent_les.pn_rent_les(
          lease_resp_user         =>argument1,
          location_code_low       =>argument2,
          location_code_high      =>argument3,
          lease_type              =>argument4,
          lease_number_low        =>argument5,
          lease_number_high       =>argument6,
          lease_termination_from  =>FND_DATE.CANONICAL_TO_DATE(argument7),
          lease_termination_to    =>FND_DATE.CANONICAL_TO_DATE(argument8),
          lease_status            =>argument9,
          lease_class             =>argument10,					--bug#2099864
          l_request_id            =>to_number(l_request_id),
          l_user_id               =>to_number(l_user_id),
          retcode                 =>retcode,
          errbuf                  =>errbuf);
    COMMIT;
  END pn_c_rent_roll_lease_exp;
END pnrx_c_rent_roll_lease_exp;
