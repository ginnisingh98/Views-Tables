--------------------------------------------------------
--  DDL for Package Body BEN_DCP_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_DCP_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_dcp_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_dpnt_cvrd_plip_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   dpnt_cvrd_plip_rt_id PK of record being inserted or updated.
...
--
Procedure chk_dpnt_cvrd_plip_rt_id(p_dpnt_cvrd_plip_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_dpnt_cvrd_plip_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_dcp_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_dpnt_cvrd_plip_rt_id                => p_dpnt_cvrd_plip_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_dpnt_cvrd_plip_rt_id,hr_api.g_number)
     <>  ben_dcp_shd.g_old_rec.dpnt_cvrd_plip_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_dcp_shd.constraint_error('BEN_DPNT_CVRD_PLIP_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_dpnt_cvrd_plip_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_enrl_det_dt_cd(p_dpnt_cvrd_plip_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_dcp_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2)
    ...
    if hr_api.not_exists_in_hr_lookups
      ...
      fnd_message.set_token('VALUE',p_enrl_det_dt_cd);
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_dpnt_cvrd_plip_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_dcp_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   p_dpnt_cvrd_plip_rt_id  PK of record being inserted or updated.
--   p_plip_id		     Value of FK
...
and dpnt_cvrd_plip_rt_id <> nvl(p_dpnt_cvrd_plip_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and (p_plip_id <> nvl(ben_dcp_shd.g_old_rec.plip_id,hr_api.g_number)
      or p_enrl_det_dt_cd <> nvl(ben_dcp_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2))
      or not l_api_updating) then
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Covered by Another Plan in Program
		-- criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Covered by Another Plan in Program criteria',TRUE);
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and dpnt_cvrd_plip_rt_id   <> nvl(p_dpnt_cvrd_plip_rt_id  ,-1)
...
function return_legislation_code
  (p_dpnt_cvrd_plip_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_dpnt_cvrd_plip_rt_f b
    where b.dpnt_cvrd_plip_rt_id      = p_dpnt_cvrd_plip_rt_id
