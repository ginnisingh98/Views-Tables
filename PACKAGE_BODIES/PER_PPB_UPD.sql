--------------------------------------------------------
--  DDL for Package Body PER_PPB_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PPB_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_ppb_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy per_ppb_shd.g_rec_type
  ) is
--
  l_proc  varchar2(72) := g_package||'update_dml';
  l_rec per_ppb_shd.g_rec_type;
--
Begin
  ...
  --
  per_ppb_shd.g_api_dml := true;  -- Set the api dml status
  --
  -- Update the per_pay_bases Row
  --
  update per_pay_bases
    set
    ...
    ,annualized_hours                = p_rec.annualized_hours
    ,pay_annualization_factor        = p_rec.pay_annualization_factor
    ,grade_annualization_factor      = p_rec.grade_annualization_factor
    ...
    where pay_basis_id = p_rec.pay_basis_id;
  --
  per_ppb_shd.g_api_dml := false;   -- Unset the api dml status
  --
...
Begin
  ...
  begin
    ...
    per_ppb_rku.after_update
      ...
      => p_rec.attribute20
      ,p_annualized_hours
      => p_rec.annualized_hours
      ,p_pay_annualization_factor
      => p_rec.pay_annualization_factor
      ,p_grade_annualization_factor
      => p_rec.grade_annualization_factor
      ...
      => per_ppb_shd.g_old_rec.business_group_id
      ,p_name_o
      => per_ppb_shd.g_old_rec.name
      ,p_pay_basis_o
      ...
      ,p_annualized_hours_o
      => per_ppb_shd.g_old_rec.annualized_hours
      ,p_pay_annualization_factor_o
      => per_ppb_shd.g_old_rec.pay_annualization_factor
      ,p_grade_annualization_factor_o
      ...
      ,p_information20_o
      => per_ppb_shd.g_old_rec.information20
      ,p_object_version_number_o
      => per_ppb_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    ...
    per_ppb_shd.g_old_rec.name;
  End If;
  If (p_rec.pay_basis = hr_api.g_varchar2) then
    p_rec.pay_basis :=
    per_ppb_shd.g_old_rec.pay_basis;
  End If;
  ...
  If (p_rec.attribute20 = hr_api.g_varchar2) then
    ...
    per_ppb_shd.g_old_rec.attribute20;
  End If;
  If (p_rec.annualized_hours = hr_api.g_number) then
    p_rec.annualized_hours :=
    per_ppb_shd.g_old_rec.annualized_hours;
  End If;
  If (p_rec.pay_annualization_factor = hr_api.g_number) then
    p_rec.pay_annualization_factor :=
    per_ppb_shd.g_old_rec.pay_annualization_factor;
  End If;
  If (p_rec.grade_annualization_factor = hr_api.g_number) then
    p_rec.grade_annualization_factor :=
    per_ppb_shd.g_old_rec.grade_annualization_factor;
  End If;
...
Begin
  ...
  --
  per_ppb_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_ppb_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_ppb_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Procedure upd
  ...
  ,p_annualized_hours             in     number    default hr_api.g_number
