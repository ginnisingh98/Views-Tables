--------------------------------------------------------
--  DDL for Package Body INV_DIAG_OH_LOT_ITMNOLOT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."INV_DIAG_OH_LOT_ITMNOLOT" as
/* $Header: INVDO01B.pls 120.0.12000000.1 2007/06/22 00:57:36 musinha noship $ */
...
BEGIN
  ...
  sqltxt :='select  mp.organization_code|| '' (''||moqd.organization_id ||'')'' "Organization|Code (Id)"' ||
   ...
   dummy_num:= JTF_DIAGNOSTIC_COREAPI.display_sql(sqltxt,'Onhand with Lot number for Item Not Lot Controlled');
...
EXCEPTION
  when others then
    ...
    fixInfo := 'Unexpected Exception in INVDO01B.pls';
    isFatal := 'FALSE';
    report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
    reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
END runTest;
...
BEGIN
   descStr := 'Onhand with lot number when item is not lot controlled';
END getTestDesc;

PROCEDURE getTestName(name OUT NOCOPY VARCHAR2) IS
BEGIN
   name := 'Onhand Lot Item NoLot';
END getTestName;
