--------------------------------------------------------
--  DDL for Package Body ISC_DBI_PDUE_SCH_SUM_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_PDUE_SCH_SUM_PKG" AS
/* $Header: ISCRGA4B.pls 120.0 2005/05/25 17:19:07 appldev noship $ */
...
BEGIN
  ...
  IF (l_bucket IS NULL OR l_bucket = '')
    THEN
      ...
      l_days_late := 'days_late';
      l_line_cnt := 'pdue_line_cnt';
    ELSE
      l_pdue_qty := 'bucket'||l_bucket||'_qty';
      l_pdue_amt := 'bucket'||l_bucket||'_pdue_amt_'||l_curr_suffix;
      l_days_late := 'bucket'||l_bucket||'_days_late';
      l_line_cnt := 'bucket'||l_bucket||'_line_cnt';
  END IF;
...
(SELECT	c.inv_org_id,	';
 ...
 SELECT	ecat.value					VIEWBY,
	...
	decode(ecat.leaf_node_flag, ''Y'',
		''pFunctionName=ISC_DBI_PDUE_SCH_SUM&VIEW_BY_NAME=VIEW_BY_ID&VIEW_BY=ITEM+ENI_ITEM_ORG&pParamIds=Y'',
		''pFunctionName=ISC_DBI_PDUE_SCH_SUM&VIEW_BY_NAME=VIEW_BY_ID&VIEW_BY=ITEM+ENI_ITEM_VBH_CAT&pParamIds=Y'')
  ...
  l_stmt := l_select_stmt || '
	...
	sum(c.curr_days_late) over ()
	  ...
	  WHERE fact.time_snapshot_date_id IN (a.day, b.day)
	    AND fact.late_schedule_flag = 1'||l_flags_where||'
