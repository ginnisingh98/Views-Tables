--------------------------------------------------------
--  DDL for Package Body PSP_POA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSP_POA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  psp_poa_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy psp_poa_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  -- Update the psp_organization_accounts Row
  --
  update psp_organization_accounts
    set
     organization_account_id         = p_rec.organization_account_id
    ...
    ,account_type_code               = p_rec.account_type_code
    ...
    ,poeta_start_date                = p_rec.poeta_start_date
    ,poeta_end_date                  = p_rec.poeta_end_date
    ,object_version_number           = p_rec.object_version_number
    ,funding_source_code             = p_rec.funding_source_code
...
Begin
  ...
  begin
    ...
    psp_poa_rku.after_update
      ...
      ,p_poeta_start_date
      => p_rec.poeta_start_date
      ,p_poeta_end_date
      => p_rec.poeta_end_date
...
Begin
  ...
  If (p_rec.set_of_books_id = hr_api.g_number) then
    ...
    psp_poa_shd.g_old_rec.set_of_books_id;
  End If;
  If (p_rec.account_type_code = hr_api.g_varchar2) then
    p_rec.account_type_code :=
    psp_poa_shd.g_old_rec.account_type_code;
  End If;
  ...
  If (p_rec.organization_id = hr_api.g_number) then
    ...
    psp_poa_shd.g_old_rec.organization_id;
  End If;
  If (p_rec.poeta_start_date = hr_api.g_date) then
    p_rec.poeta_start_date :=
    psp_poa_shd.g_old_rec.poeta_start_date;
  End If;
  If (p_rec.poeta_end_date = hr_api.g_date) then
    p_rec.poeta_end_date :=
    psp_poa_shd.g_old_rec.poeta_end_date;
  End If;
  If (p_rec.funding_source_code = hr_api.g_varchar2) then
    p_rec.funding_source_code :=
    psp_poa_shd.g_old_rec.funding_source_code;
  End If;
  --
...
Begin
  ...
  --
  psp_poa_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  psp_poa_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  psp_poa_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Procedure upd
  ...
  ,p_account_type_code            in     varchar2  default hr_api.g_varchar2
  ...
  ,p_poeta_start_date             in     date      default hr_api.g_date
  ,p_poeta_end_date               in     date      default hr_api.g_date
  ,p_funding_source_code          in     varchar2  default hr_api.g_varchar2
  ) is
--
