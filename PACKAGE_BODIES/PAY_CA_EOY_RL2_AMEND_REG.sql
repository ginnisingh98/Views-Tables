--------------------------------------------------------
--  DDL for Package Body PAY_CA_EOY_RL2_AMEND_REG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CA_EOY_RL2_AMEND_REG" AS
/* $Header: pycarl2cr.pkb 120.1.12010000.8 2009/11/20 12:46:14 aneghosh ship $ */
/*
   ******************************************************************
   *                                                                *
   ...
   ******************************************************************
    ...
    16-OCT-2006 meshah     115.1   5528944  Added order by to the range
                                            cursor queries.
    17-JUN-2009 aneghosh   115.2   8316787  Modified the locking mechanism.
    02-JUL-2009 aneghosh   115.3   8316787  Modified cursor
                                            get_latest_rl2_amend_dtls.
    14-JUL-2009 aneghosh   115.4   5912715  Added support for Amendment paper to be
                                            run under two modes - Unprinted and reprint.
    24-SEP-2009 aneghosh   115.6   8932754  Modified the cursor
                                            get_latest_rl2_amend_dtls.
    09-OCT-2009 aneghosh   115.7   8932598  Modified action_creation to prevent creation
                                            of duplicate assignment actions for the
                                            same employee.
    20-NOV-2009 aneghosh   115.8   9133270  Modified cursors get_latest_rl2_amend_dtls,
                                            get_reprint_rl2_amend_dtls and
                                            get_all_rl2_amend_dtls

  *****************************************************************************/

   gv_package        VARCHAR2(100) := 'pay_ca_eoy_rl2_amend_reg';
   gv_procedure_name VARCHAR2(100);

  /*****************************************************************************
   Name      : get_payroll_action_info
   Purpose   : This returns the Payroll Action level
               information for RL2 Amendment Paper.
   Arguments : p_payroll_action_id - Payroll_Action_id of archiver
               p_start_date        - Start date of RL2 Amendment Paper
               p_end_date          - End date of RL2 Amendment Paper
  ...
  ******************************************************************/
  ...
  /************************************************************
   ...
   Purpose   : This creates the assignment actions for
               ...
               by the RL2 Amendment Report process.
   Arguments :
   Notes     : Calls procedure - get_payroll_action_info
  ************************************************************/
  ...
  IS
    ...
    ln_rl2_amend_reg_asg_action NUMBER := 0;
    lv_year                   VARCHAR2(4);
   ...
   IS
     ...
     CURSOR get_latest_rl2_amend_dtls (cp_person_id  in number
       ...
       and paa.assignment_id = paf.assignment_id
        ...
        and ppa.report_type IN ('CAEOY_RL2_AMEND_PP','PYRL2PRAMEND')
        and exists (select 1
          ...
          AND   not exists
             ( SELECT  pail.locked_action_id
               ...
               WHERE   pact.report_type = 'PYRL2PRAMEND'
 ...
 --       order by paa.assignment_action_id desc;
     ...
     CURSOR get_all_rl2_amend_dtls (cp_person_id  in number
                                   ,cp_pre_org_id in number
                                   ,cp_effective_date in date
                                   ,cp_business_group_id in number) is
        select ppa.report_type,
 ...
 --       order by paa.assignment_action_id desc;
     ...
     CURSOR get_reprint_rl2_amend_dtls (cp_person_id  in number
                                   ,cp_pre_org_id in number
                                   ,cp_effective_date in date
                                   ,cp_business_group_id in number) is
        select ppa.report_type,
...
--  Check mode here
     if lv_print = 'LATEST' then  --- For Amendment Paper Unprinted Mode
                ...
                /* Create an assignment action for this person */
                   ...
                   from dual;
                    ...
                    hr_utility.trace('New RL2 Amend Action = ' ||
                                      to_char(ln_rl2_amend_reg_asg_action));

         open get_latest_rl2_amend_dtls(p_person_id
                                       ,ln_pre_org_id
                                       ,ld_end_date
                                       ,ln_business_group_id);
         ...
         if get_latest_rl2_amend_dtls%notfound then
           ...
           if ln_iteration = 0 then
             ...
             lv_record_name := 'RL2 Amend Paper Report';
             lv_message := 'RL2 Amend Preprocess was not run for this employee';
              ...
              begin
               ...
               /* Insert into pay_assignment_actions. */
                   ...
                   ***********************************************************/
                   ...
                   hr_utility.trace('Province: '||substr(ln_serial_number,1,2));
...
--Added to lock the Amend Archiver
                   hr_nonrun_asact.insint(ln_rl2_amend_reg_asg_action
                                     ,ln_asg_act_to_lock);

                   hr_utility.trace('Locking Action'||ln_rl2_amend_reg_asg_action);
     ...
     if lv_report_type = 'PYRL2PRAMEND' then -- To lock previous paper (unprinted)

                    open c_paa_update_check(ln_rl2_amend_reg_asg_action);
...
--Added to lock the Amend Archiver
     ...
     if lv_print = 'RECENT' then  -- For Amendment Register Recent Mode
         open get_all_rl2_amend_dtls(p_person_id
                                       ,ln_pre_org_id
                                       ,ld_end_date
                                       ,ln_business_group_id);
         ...
         fetch get_all_rl2_amend_dtls into lv_report_type,
                                              ln_primary_assignment_id,
                                              ln_asg_act_to_lock,
                                              ln_pre_id_null;

         if get_all_rl2_amend_dtls%notfound then
           ...
           if ln_iteration = 0 then
             ...
             hr_utility.trace('get_all_rl2_amend_dtls not found');
             hr_utility.trace('p_person_id :'||to_char(p_person_id));

             lv_record_name := 'RL2 Amend Register Report';
             lv_message := 'RL2 Amend Preprocess was not run for this employee';
         ...
         if get_all_rl2_amend_dtls%found then
              ...
              begin
                ...
                hr_utility.trace('lv_prov_amend_flag : '||lv_prov_amend_flag);
                 ...
                 if c_get_prov_amend_flag%FOUND then
                  ...
                  /* Create an assignment action for this person */
                   hr_utility.trace('get_all_rl2_amend_dtls found ');
         ...
         end if; /* get_all_rl2_amend_dtls found*/

         end loop; /* get_all_rl2_amend_dtls loop */

         close get_all_rl2_amend_dtls;
...
-- check Mode here
      if lv_print = 'HISTORICAL' then -- For Amendment Register Historical Mode
         ...
         loop
               ...
               if c_get_prov_amend_flag%FOUND then
                 ...
                 if lv_prov_amend_flag = 'Y' then
                  ...
                  /* Create an assignment action for this person */
                   ...
                   hr_utility.trace('New RL2 Amend Action = ' || ln_rl2_amend_reg_asg_action);
          ...
          end loop; -- loop for get_all_rl2_amend_dtls
          close get_all_rl2_amend_dtls;
      ...
      if lv_print = 'REPRINT' then  -- For Amendment Paper Reprint Mode
         ...
         open get_reprint_rl2_amend_dtls(p_person_id
                                    ,ln_pre_org_id
                                    ,ld_end_date
                                    ,ln_business_group_id);
         lv_report_type := null;
         ...
         loop
              ...
              fetch get_reprint_rl2_amend_dtls into lv_report_type
                                          ,ln_primary_assignment_id
                                          ,ln_asg_act_to_lock
                                          ,ln_pre_id_null;

              if get_reprint_rl2_amend_dtls%notfound then
                 hr_utility.trace('get_reprint_rl2_amend_dtls not found ');
                 exit;
              end if;
              ...
              lv_prov_amend_flag := 'N';
               ...
               if c_get_prov_amend_flag%FOUND then
                 ...
                 if lv_prov_amend_flag = 'Y' and lv_flag_count = 0 then
                   ...
                   hr_utility.trace('get_reprint_rl2_amend_dtls found ');
                   hr_utility.trace('Report Type: '||lv_report_type);
          ...
          end loop; -- loop for get_reprint_rl2_amend_dtls
          close get_reprint_rl2_amend_dtls;
...
FUNCTION get_parameter(name in varchar2,
  ...
  END get_parameter;
Begin
 /*  hr_utility.trace_on(null,'RL2AMEND_REG'); */
 null;
