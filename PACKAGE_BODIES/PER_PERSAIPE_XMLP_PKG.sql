--------------------------------------------------------
--  DDL for Package Body PER_PERSAIPE_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PERSAIPE_XMLP_PKG" AS
/* $Header: PERSAIPEB.pls 120.1 2008/03/13 06:22:00 amakrish noship $ */
 ...
 if p_date is null then
    p_date := fnd_date.date_to_canonical(trunc(SYSDATE));
  end if;
  p_r_date := fnd_date.canonical_to_date(p_date);
--p_r_date:=p_date;
	...
	|| 'prior pose.organization_id_child ' || 'and pose.org_structure_version_id = ' || TO_CHAR(P_ORG_STRUCTURE_VERSION_ID) || ' start with pose.organization_id_parent = ' || TO_CHAR(NVL(P_ORG_ID
