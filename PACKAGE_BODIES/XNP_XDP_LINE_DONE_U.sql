--------------------------------------------------------
--  DDL for Package Body XNP_XDP_LINE_DONE_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_XDP_LINE_DONE_U" AS
/*$Header: XNPLIDNB.pls 120.1 2005/06/09 02:56:28 appldev  $*/
PROCEDURE CREATE_MSG  (   XNP$LINE_ITEM_ID NUMBER,
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'XDP_LINE_DONE' ;
  ...
  XNP_XML_UTILS.write_leaf_element ( 'LINE_ITEM_ID', XNP$LINE_ITEM_ID );
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'XDP_LINE_DONE' ) ;
  l_loop_index_1 := 0 ;

  FOR XNP$XDP_LINE_DONE  IN (SELECT ORDER_ID,
              LINE_ITEM_ID,
              STATUS_CODE
FROM XDP_ORDER_LINE_ITEMS WHERE
LINE_ITEM_ID=XNP$LINE_ITEM_ID)
 LOOP
    l_loop_index_1 := l_loop_index_1 + 1 ;
    IF ( XNP$XDP_LINE_DONE.ORDER_ID IS NULL) THEN
       fnd_message.set_name('XNP', 'XNP_MISSING_MANDATORY_ATTR' );
       fnd_message.set_token( 'ATTRIBUTE','ORDER_ID' ) ;
      x_error_message := fnd_message.get ;
      RAISE e_MISSING_MANDATORY_DATA ;
    END IF ;

    XNP_XML_UTILS.write_leaf_element ( 'ORDER_ID', XNP$XDP_LINE_DONE.ORDER_ID ) ;

    IF ( XNP$LINE_ITEM_ID IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - LINE_ITEM_ID' ;
     fnd_message.set_name('XNP','XNP_MISSING_MANDATORY_ATTR');
     fnd_message.set_token('ATTRIBUTE','LINE_ITEM_ID' ) ;
     x_error_message := fnd_message.get ;
      RAISE e_MISSING_MANDATORY_DATA ;
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'LINE_ITEM_ID', XNP$LINE_ITEM_ID ) ;
    XNP_XML_UTILS.write_leaf_element ( 'LINE_ITEM_STATUS', XNP$XDP_LINE_DONE.STATUS_CODE ) ;
    XNP_XML_UTILS.write_leaf_element ( 'SDP_RESULT_CODE', XNP$XDP_LINE_DONE.STATUS_CODE ) ;

    EXIT ;
  END LOOP ;
  IF ( l_loop_index_1 = 0 ) THEN
       fnd_message.set_name('XNP', 'XNP_MISSING_MANDATORY_DATA' );
       fnd_message.set_token( 'ELEMENT',', ORDER_ID, LINE_ITEM_ID' ) ;
      x_error_message := fnd_message.get ;
    RAISE e_MISSING_MANDATORY_DATA ;
  END IF ;
    XNP_XML_UTILS.end_segment ( 'XDP_LINE_DONE' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'XDP_LINE_DONE.create_msg()::' || SQLERRM ;
END ;

PROCEDURE PUBLISH  (   XNP$LINE_ITEM_ID NUMBER,
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'XDP_LINE_DONE', l_consumer_list );
...
/* create the XML message */
  CREATE_MSG (
    XNP$LINE_ITEM_ID=>XNP$LINE_ITEM_ID,
    x_msg_header=>l_msg_header,
...
END ;
PROCEDURE SEND  (   XNP$LINE_ITEM_ID NUMBER,
  x_message_id OUT NOCOPY  NUMBER,
  x_error_code OUT NOCOPY  NUMBER,
  x_error_message OUT NOCOPY VARCHAR2,
