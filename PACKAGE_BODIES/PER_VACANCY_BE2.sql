--------------------------------------------------------
--  DDL for Package Body PER_VACANCY_BE2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_VACANCY_BE2" as 
...
begin
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<assignment_changed>';
if(P_ASSIGNMENT_CHANGED) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</assignment_changed>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
