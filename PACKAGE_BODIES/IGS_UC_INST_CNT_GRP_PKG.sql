--------------------------------------------------------
--  DDL for Package Body IGS_UC_INST_CNT_GRP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_UC_INST_CNT_GRP_PKG" AS
/* $Header: IGSXI19B.pls 115.8 2003/06/11 10:09:42 smaddali noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_uc_inst_cnt_grp%ROWTYPE;
  new_references igs_uc_inst_cnt_grp%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.ucas_group                        := x_ucas_group;
  ...
  */
  BEGIN
    ...
    IF (((old_references.contact_code = new_references.contact_code)) OR
        ((new_references.contact_code IS NULL))) THEN
      NULL;
    ELSIF NOT igs_uc_inst_conts_pkg.get_pk_for_validation (
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_UC_UCINCG_UCINCO_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    INSERT INTO igs_uc_inst_cnt_grp (
      contact_code,
      updater,
      ucas_group,
      sent_to_ucas,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_uc_inst_cnt_grp
    WHERE rowid = x_rowid;
