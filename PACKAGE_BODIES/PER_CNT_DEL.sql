--------------------------------------------------------
--  DDL for Package Body PER_CNT_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CNT_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_cnt_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< delete_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Delete the per_ri_configurations_tl row.
  --
  delete from per_ri_configurations_tl
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_delete(p_rec in per_cnt_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- -----------------------------------------------------------------------------
Procedure post_delete(p_rec in per_cnt_shd.g_rec_type) is
--
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'PER_RI_CONFIGURATIONS_TL'
        ,p_hook_type   => 'AD');
      --
  end;
  --
...
Begin
  ...
  --
  per_cnt_bus.delete_validate(p_rec);
  --
  ...
  --
  per_cnt_del.pre_delete(p_rec);
  --
  -- Delete the row.
  --
  per_cnt_del.delete_dml(p_rec);
  --
  -- Call the supporting post-delete operation
  --
  per_cnt_del.post_delete(p_rec);
  --
  -- Call to raise any errors on multi-message list
  hr_multi_message.end_validation_set;
  --
...
Begin
  ...
  --
  --
  -- Having converted the arguments into the per_cnt_rec
  -- plsql record structure we must call the corresponding entity
  -- business process
  --
  per_cnt_del.del(l_rec);
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End del;
--
