--------------------------------------------------------
--  DDL for Package Body GR_TOXIC_ROUTES_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_TOXIC_ROUTES_B_PKG" AS
/*$Header: GRHITRB.pls 115.7 2002/10/28 21:31:15 gkelly ship $*/
...
CURSOR c_lock_toxic_routes
 IS
   SELECT	last_update_date
   FROM		gr_toxic_routes_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockRouteRcd	  c_lock_toxic_routes%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_toxic_routes;
   FETCH c_lock_toxic_routes INTO LockRouteRcd;
   IF c_lock_toxic_routes%NOTFOUND THEN
	  CLOSE c_lock_toxic_routes;
...
CURSOR c_get_toxic_route_rowid
 IS
   SELECT tr.rowid
   FROM	  gr_toxic_routes_b tr
   WHERE  tr.toxic_route_code = p_toxic_route_code;
ToxicRouteRecord			   c_get_toxic_route_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_toxic_route_rowid;
   FETCH c_get_toxic_route_rowid INTO ToxicRouteRecord;
   IF c_get_toxic_route_rowid%FOUND THEN
      x_key_exists := 'T';
      x_rowid := ToxicRouteRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_toxic_route_rowid;
