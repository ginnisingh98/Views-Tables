--------------------------------------------------------
--  DDL for Package Body IGS_EN_NSU_DLSTP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_EN_NSU_DLSTP_PKG" AS
/* $Header: IGSEI48B.pls 120.1 2006/02/16 04:25:30 smaddali noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_en_nsu_dlstp_all%ROWTYPE;
  new_references igs_en_nsu_dlstp_all%ROWTYPE;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    ELSIF NOT igs_or_unit_pkg.get_pk_for_str_validation (
                                  new_references.org_unit_code) THEN
        fnd_message.set_name ('FND','FORM_RECORD_DELETED');
        igs_ge_msg_stack.add;
        app_exception.raise_exception;
    END IF;
  ...
  */
  BEGIN
    ...
    igs_en_dl_offset_cons_pkg.get_fk_igs_en_nsu_dlstp_all (
      old_references.non_std_usec_dls_id
    );
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.function_name = x_function_name)
