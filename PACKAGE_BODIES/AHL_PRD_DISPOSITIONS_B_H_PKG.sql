--------------------------------------------------------
--  DDL for Package Body AHL_PRD_DISPOSITIONS_B_H_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_PRD_DISPOSITIONS_B_H_PKG" as
/* $Header: AHLLDSHB.pls 120.0 2005/05/26 00:06:19 appldev noship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from AHL_PRD_DISPOSITIONS_B_H
    where DISPOSITION_H_ID = X_DISPOSITION_H_ID
    ;
begin
  insert into AHL_PRD_DISPOSITIONS_B_H (
    DISPOSITION_H_ID,
    DISPOSITION_ID,
  ...
  insert into AHL_PRD_DISPOSITIONS_TL_H (
    DISPOSITION_H_ID,
    DISPOSITION_ID,
    LAST_UPDATE_DATE,
    LAST_UPDATED_BY,
  ...
  and not exists
    (select NULL
    from AHL_PRD_DISPOSITIONS_TL_H T
    where T.DISPOSITION_H_ID = X_DISPOSITION_H_ID
...
procedure LOCK_ROW (
  ...
  cursor c is select
      ...
      ATTRIBUTE14,
      ATTRIBUTE15
    from AHL_PRD_DISPOSITIONS_B_H
    where DISPOSITION_H_ID = X_DISPOSITION_H_ID
    for update of DISPOSITION_H_ID nowait;
  recinfo c%rowtype;

  cursor c1 is select
      COMMENTS,
      decode(LANGUAGE, userenv('LANG'), 'Y', 'N') BASELANG
    from AHL_PRD_DISPOSITIONS_TL_H
    where DISPOSITION_H_ID = X_DISPOSITION_H_ID
...
begin
  ...
  close c;
  if (    (recinfo.DISPOSITION_ID = X_DISPOSITION_ID)
...
begin
  update AHL_PRD_DISPOSITIONS_B_H set
    DISPOSITION_ID = X_DISPOSITION_ID,
  ...
  update AHL_PRD_DISPOSITIONS_TL_H set
    COMMENTS = X_COMMENTS,
    LAST_UPDATE_DATE = X_LAST_UPDATE_DATE,
    LAST_UPDATED_BY = X_LAST_UPDATED_BY,
    LAST_UPDATE_LOGIN = X_LAST_UPDATE_LOGIN,
...
begin
  delete from AHL_PRD_DISPOSITIONS_TL_H
  where DISPOSITION_H_ID = X_DISPOSITION_H_ID;
  ...
  delete from AHL_PRD_DISPOSITIONS_B_H
  where DISPOSITION_H_ID = X_DISPOSITION_H_ID;
...
is
begin
  delete from AHL_PRD_DISPOSITIONS_TL_H T
  where not exists
    (select NULL
    from AHL_PRD_DISPOSITIONS_B_H B
    where B.DISPOSITION_H_ID = T.DISPOSITION_H_ID
    );

  update AHL_PRD_DISPOSITIONS_TL_H T set (
      COMMENTS
    ) = (select
      B.COMMENTS
    from AHL_PRD_DISPOSITIONS_TL_H B
    where B.DISPOSITION_H_ID = T.DISPOSITION_H_ID
  ...
  where (
      ...
      SUBT.DISPOSITION_H_ID,
      SUBT.LANGUAGE
    from AHL_PRD_DISPOSITIONS_TL_H SUBB, AHL_PRD_DISPOSITIONS_TL_H SUBT
    where SUBB.DISPOSITION_H_ID = SUBT.DISPOSITION_H_ID
  ...
  insert into AHL_PRD_DISPOSITIONS_TL_H (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from AHL_PRD_DISPOSITIONS_TL_H B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from AHL_PRD_DISPOSITIONS_TL_H T
    where T.DISPOSITION_H_ID = B.DISPOSITION_H_ID
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;
