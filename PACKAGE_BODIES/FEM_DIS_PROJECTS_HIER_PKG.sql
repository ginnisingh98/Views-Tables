--------------------------------------------------------
--  DDL for Package Body FEM_DIS_PROJECTS_HIER_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FEM_DIS_PROJECTS_HIER_PKG" as
/* $Header: fem_disproj_pkb.plb 120.0 2005/10/19 19:20:00 appldev noship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from FEM_DIS_PROJECTS_HIER_B
    ...
    ;
begin
  insert into FEM_DIS_PROJECTS_HIER_B (
  ...
  insert into FEM_DIS_PROJECTS_HIER_TL (
    LEVEL9_DESCRIPTION,
    LEVEL10_DESCRIPTION,
    LEVEL11_DESCRIPTION,
    LEVEL12_DESCRIPTION,
  ...
  and not exists
    (select NULL
    from FEM_DIS_PROJECTS_HIER_TL T
...
begin
  ...
  close c;
  if (    (recinfo.LEVEL6_DISPLAY_ORDER_NUM = X_LEVEL6_DISPLAY_ORDER_NUM)
...
begin
  update FEM_DIS_PROJECTS_HIER_B set
  ...
  update FEM_DIS_PROJECTS_HIER_TL set
...
begin
  delete from FEM_DIS_PROJECTS_HIER_TL
  where OBJECT_ID = X_OBJECT_ID
  and OBJECT_DEFINITION_ID = X_OBJECT_DEFINITION_ID
  and VALUE_SET_ID = X_VALUE_SET_ID
  and LEVEL1_ID = X_LEVEL1_ID
  ...
  delete from FEM_DIS_PROJECTS_HIER_B
  where OBJECT_ID = X_OBJECT_ID
  and OBJECT_DEFINITION_ID = X_OBJECT_DEFINITION_ID
  and VALUE_SET_ID = X_VALUE_SET_ID
  and LEVEL1_ID = X_LEVEL1_ID
...
is
begin
  delete from FEM_DIS_PROJECTS_HIER_TL T
  where not exists
    (select NULL
    from FEM_DIS_PROJECTS_HIER_B B
  ...
  update FEM_DIS_PROJECTS_HIER_TL T set (
      OBJECT_NAME,
      OBJECT_DEFINITION_NAME,
      LEVEL1_NAME,
      LEVEL2_NAME,
    ...
    from FEM_DIS_PROJECTS_HIER_TL B
    where B.OBJECT_ID = T.OBJECT_ID
    and B.OBJECT_DEFINITION_ID = T.OBJECT_DEFINITION_ID
    and B.VALUE_SET_ID = T.VALUE_SET_ID
    and B.LEVEL1_ID = T.LEVEL1_ID
  ...
  where (
      ...
      SUBT.LEVEL20_ID,
      SUBT.LANGUAGE
    from FEM_DIS_PROJECTS_HIER_TL SUBB, FEM_DIS_PROJECTS_HIER_TL SUBT
  ...
  insert into FEM_DIS_PROJECTS_HIER_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from FEM_DIS_PROJECTS_HIER_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
