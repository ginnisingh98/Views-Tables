--------------------------------------------------------
--  DDL for Package Body MTH_OTHERS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MTH_OTHERS_EXT_PKG" as
/* $Header: mthuotlb.pls 120.0.12010000.3 2009/08/21 09:52:31 sdonthu ship $ */
/*
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from MTH_OTHERS_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into MTH_OTHERS_EXT_B (
    D_EXT_ATTR7,
    D_EXT_ATTR8,
    D_EXT_ATTR9,
    D_EXT_ATTR10,
...
begin
  ...
  close c;
  if (    ((recinfo.D_EXT_ATTR7 = X_D_EXT_ATTR7)
      ...
      AND ((recinfo.OTHER_PK_KEY = X_OTHER_PK_KEY)
           OR ((recinfo.OTHER_PK_KEY is null) AND (X_OTHER_PK_KEY is null)))
...
is
begin
  delete from MTH_OTHERS_EXT_TL T
  where not exists
    (select NULL
    from MTH_OTHERS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update MTH_OTHERS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from MTH_OTHERS_EXT_TL SUBB, MTH_OTHERS_EXT_TL SUBT
  ...
  insert into MTH_OTHERS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from MTH_OTHERS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
