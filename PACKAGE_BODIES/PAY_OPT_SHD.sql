--------------------------------------------------------
--  DDL for Package Body PAY_OPT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_OPT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_opt_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PAY_ORG_PAYMENT_METHODS_TL_PK') Then
...
Begin
  --
  If (p_org_payment_method_id is null or
      p_language is null
     ) Then
    --
  ...
  Else
    If (p_org_payment_method_id
        = pay_opt_shd.g_old_rec.org_payment_method_id and
        p_language
        = pay_opt_shd.g_old_rec.language
       ) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into pay_opt_shd.g_old_rec;
