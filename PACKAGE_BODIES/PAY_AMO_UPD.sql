--------------------------------------------------------
--  DDL for Package Body PAY_AMO_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_AMO_UPD" as
...
Begin
  ...
  --
  -- Update the pay_au_modules Row
  --
  update pay_au_modules
    set
    ...
    ,package_name                    = p_rec.package_name
    ,procedure_function_name         = p_rec.procedure_function_name
    ,formula_name                    = p_rec.formula_name
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    pay_amo_shd.g_old_rec.name;
  End If;
  If (p_rec.enabled_flag = hr_api.g_varchar2) then
    p_rec.enabled_flag :=
    pay_amo_shd.g_old_rec.enabled_flag;
  End If;
  If (p_rec.module_type_id = hr_api.g_number) then
    p_rec.module_type_id :=
    pay_amo_shd.g_old_rec.module_type_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    pay_amo_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    p_rec.legislation_code :=
    pay_amo_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.description = hr_api.g_varchar2) then
    p_rec.description :=
    pay_amo_shd.g_old_rec.description;
  End If;
  If (p_rec.package_name = hr_api.g_varchar2) then
    p_rec.package_name :=
    pay_amo_shd.g_old_rec.package_name;
  End If;
  If (p_rec.procedure_function_name = hr_api.g_varchar2) then
    p_rec.procedure_function_name :=
    pay_amo_shd.g_old_rec.procedure_function_name;
  End If;
  If (p_rec.formula_name = hr_api.g_varchar2) then
    p_rec.formula_name :=
    pay_amo_shd.g_old_rec.formula_name;
  End If;
  --
End convert_defs;
--
...
Begin
  ...
  --
  pay_amo_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pay_amo_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pay_amo_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
--
begin
  g_package := '  pay_amo_upd.';  -- Global package name
end pay_amo_upd;
