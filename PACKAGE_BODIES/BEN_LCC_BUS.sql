--------------------------------------------------------
--  DDL for Package Body BEN_LCC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LCC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lcc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ler_chg_dpnt_cvg_ctfn_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   ler_chg_dpnt_cvg_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_ler_chg_dpnt_cvg_ctfn_id(p_ler_chg_dpnt_cvg_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ler_chg_dpnt_cvg_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_lcc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ler_chg_dpnt_cvg_ctfn_id                => p_ler_chg_dpnt_cvg_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ler_chg_dpnt_cvg_ctfn_id,hr_api.g_number)
     <>  ben_lcc_shd.g_old_rec.ler_chg_dpnt_cvg_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_lcc_shd.constraint_error('BEN_LER_CHG_DPNT_CTFN_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ler_chg_dpnt_cvg_ctfn_id is not null then
      --
  ...
  --
End chk_ler_chg_dpnt_cvg_ctfn_id;
--
-- ----------------------------------------------------------------------------
...
--   within parent and business group.
--
-- Pre Conditions
--   None.
--
...
--   Internal table handler use only.
--
Procedure chk_dpnt_cvg_ctfn_typ_cd(p_ler_chg_dpnt_cvg_ctfn_id in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_lcc_shd.g_old_rec.dpnt_cvg_ctfn_typ_cd,hr_api.g_varchar2)
      or not l_api_updating)
      and p_dpnt_cvg_ctfn_typ_cd is not null then
    --
...
--     p_ler_chg_dpnt_cvg_ctfn_id   is primary key for ben_ler_chg_dpnt_cvg_ctfn_f
...
cursor c1 is select null
              ...
              Where ler_chg_dpnt_cvg_ctfn_id <> nvl(p_ler_chg_dpnt_cvg_ctfn_id,-1)
                and nvl(dpnt_cvg_ctfn_typ_cd,0) = nvl(p_dpnt_cvg_ctfn_typ_cd,0)
                and nvl(rlshp_typ_cd,0) = nvl(p_rlshp_typ_cd,0)
                and ler_chg_dpnt_cvg_id = p_ler_chg_dpnt_cvg_id
...
--   Internal table handler use only.
--
Procedure chk_rlshp_typ_cd(p_ler_chg_dpnt_cvg_ctfn_id     in number,
                            p_rlshp_typ_cd                in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      and p_rlshp_typ_cd
      <> nvl(ben_lcc_shd.g_old_rec.rlshp_typ_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_ler_chg_dpnt_cvg_ctfn_id    in number,
                               p_business_group_id           in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> ben_lcc_shd.g_old_rec.ctfn_rqd_when_rl
      or not l_api_updating)
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    --
    If ((nvl(p_ler_chg_dpnt_cvg_id, hr_api.g_number) <> hr_api.g_number) and
...
function return_legislation_code
  (p_ler_chg_dpnt_cvg_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_ler_chg_dpnt_cvg_ctfn_f b
    where b.ler_chg_dpnt_cvg_ctfn_id      = p_ler_chg_dpnt_cvg_ctfn_id
