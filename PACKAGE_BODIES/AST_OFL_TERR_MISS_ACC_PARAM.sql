--------------------------------------------------------
--  DDL for Package Body AST_OFL_TERR_MISS_ACC_PARAM
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AST_OFL_TERR_MISS_ACC_PARAM" AS
/* $Header: astrtnpb.pls 115.6 2002/02/07 15:24:29 pkm ship   $ */
...
-----------------------------------------------------------------------------------------------
   begin
     ...
     htp.htmlopen;
     htp.headOpen;
     htp.title('Territories - No Accesses Report');
     htp.headClose;
     ...
     htp.tableData( '<FONT size=+1 face="times new roman">' || 'Territories - No Accesses Report', cnowrap => 'TRUE');
...
-----------------------------------------------------------------------------------------------
procedure terr_miss_acc_param is
...
if (icx_sec.validateSession) then
       ...
       htp.FormOpen(owa_util.Get_Owa_Service_Path||'AST_OFL_TERR_MISS_acc_report.report_wrapper',
...
*/
 ...
 BEGIN
      ...
      htp.tableRowOpen;
      htp.formOpen( curl => l_agent||'/AST_OFL_TERR_MISS_acc_report.report_wrapper', cmethod => 'GET',
