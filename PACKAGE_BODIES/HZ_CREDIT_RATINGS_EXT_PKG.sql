--------------------------------------------------------
--  DDL for Package Body HZ_CREDIT_RATINGS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_CREDIT_RATINGS_EXT_PKG" as
/* $Header: ARHEXCRB.pls 120.4 2006/02/09 10:02:15 jhuang noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from HZ_CREDIT_RATINGS_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into HZ_CREDIT_RATINGS_EXT_B (
    EXTENSION_ID,
    CREDIT_RATING_ID,
    ATTR_GROUP_ID,
    C_EXT_ATTR1,
  ...
  insert into HZ_CREDIT_RATINGS_EXT_TL (
    CREATED_BY,
    CREATION_DATE,
    LAST_UPDATED_BY,
    LAST_UPDATE_DATE,
  ...
  and not exists
    (select NULL
    from HZ_CREDIT_RATINGS_EXT_TL T
...
begin
  ...
  close c;
  if (    (recinfo.CREDIT_RATING_ID = X_CREDIT_RATING_ID)
...
begin
  update HZ_CREDIT_RATINGS_EXT_B set
    CREDIT_RATING_ID = X_CREDIT_RATING_ID,
  ...
  update HZ_CREDIT_RATINGS_EXT_TL set
...
begin
  delete from HZ_CREDIT_RATINGS_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from HZ_CREDIT_RATINGS_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  insert /*+ append parallel(tt) */ into HZ_CREDIT_RATINGS_EXT_TL tt (
    CREATED_BY,
    CREATION_DATE,
    LAST_UPDATED_BY,
    LAST_UPDATE_DATE,
  ...
  (SELECT /*+ no_merge ordered parallel(b) */
     ...
     L.LANGUAGE_CODE,
     B.SOURCE_LANG
   FROM HZ_CREDIT_RATINGS_EXT_TL B ,
        FND_LANGUAGES L
   WHERE L.INSTALLED_FLAG IN ( 'I','B' )
   AND B.LANGUAGE = USERENV ( 'LANG' )
  ) v, HZ_CREDIT_RATINGS_EXT_TL t
