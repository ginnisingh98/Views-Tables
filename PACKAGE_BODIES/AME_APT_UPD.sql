--------------------------------------------------------
--  DDL for Package Body AME_APT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_APT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := 'ame_apt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the ame_approver_types Row
    --
    update  ame_approver_types
    set
     approver_type_id                = p_rec.approver_type_id
    ,orig_system                     = p_rec.orig_system
    ,query_variable_1_label          = p_rec.query_variable_1_label
    ,query_variable_2_label          = p_rec.query_variable_2_label
    ,query_variable_3_label          = p_rec.query_variable_3_label
    ,query_variable_4_label          = p_rec.query_variable_4_label
    ,query_variable_5_label          = p_rec.query_variable_5_label
    ,variable_1_lov_query            = p_rec.variable_1_lov_query
    ,variable_2_lov_query            = p_rec.variable_2_lov_query
    ,variable_3_lov_query            = p_rec.variable_3_lov_query
    ,variable_4_lov_query            = p_rec.variable_4_lov_query
    ,variable_5_lov_query            = p_rec.variable_5_lov_query
    ,query_procedure                 = p_rec.query_procedure
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    ame_apt_shd.upd_effective_end_date
      (p_effective_date            => p_effective_date
      ,p_approver_type_id          => p_rec.approver_type_id
      ,p_new_effective_end_date    => p_validation_start_date
      ,p_validation_start_date     => p_validation_start_date
      ,p_validation_end_date       => p_validation_end_date
...
Begin
  ...
  --
  If (p_rec.orig_system = hr_api.g_varchar2) then
    p_rec.orig_system :=
    ame_apt_shd.g_old_rec.orig_system;
  End If;
  If (p_rec.query_variable_1_label = hr_api.g_varchar2) then
    p_rec.query_variable_1_label :=
    ame_apt_shd.g_old_rec.query_variable_1_label;
  End If;
  If (p_rec.query_variable_2_label = hr_api.g_varchar2) then
    p_rec.query_variable_2_label :=
    ame_apt_shd.g_old_rec.query_variable_2_label;
  End If;
  If (p_rec.query_variable_3_label = hr_api.g_varchar2) then
    p_rec.query_variable_3_label :=
    ame_apt_shd.g_old_rec.query_variable_3_label;
  End If;
  If (p_rec.query_variable_4_label = hr_api.g_varchar2) then
    p_rec.query_variable_4_label :=
    ame_apt_shd.g_old_rec.query_variable_4_label;
  End If;
  If (p_rec.query_variable_5_label = hr_api.g_varchar2) then
    p_rec.query_variable_5_label :=
    ame_apt_shd.g_old_rec.query_variable_5_label;
  End If;
  If (p_rec.variable_1_lov_query = hr_api.g_varchar2) then
    p_rec.variable_1_lov_query :=
    ame_apt_shd.g_old_rec.variable_1_lov_query;
  End If;
  If (p_rec.variable_2_lov_query = hr_api.g_varchar2) then
    p_rec.variable_2_lov_query :=
    ame_apt_shd.g_old_rec.variable_2_lov_query;
  End If;
  If (p_rec.variable_3_lov_query = hr_api.g_varchar2) then
    p_rec.variable_3_lov_query :=
    ame_apt_shd.g_old_rec.variable_3_lov_query;
  End If;
  If (p_rec.variable_4_lov_query = hr_api.g_varchar2) then
    p_rec.variable_4_lov_query :=
    ame_apt_shd.g_old_rec.variable_4_lov_query;
  End If;
  If (p_rec.variable_5_lov_query = hr_api.g_varchar2) then
    p_rec.variable_5_lov_query :=
    ame_apt_shd.g_old_rec.variable_5_lov_query;
  End If;
  If (p_rec.query_procedure = hr_api.g_varchar2) then
    p_rec.query_procedure :=
    ame_apt_shd.g_old_rec.query_procedure;
  End If;
  --
End convert_defs;
--
...
Begin
  ...
  --
  ame_apt_upd.convert_defs(p_rec);
  --
  ame_apt_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
