--------------------------------------------------------
--  DDL for Package Body QP_PRICE_LIST_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."QP_PRICE_LIST_PUB" AS
/* $Header: QPXPPRLB.pls 120.8 2006/10/22 15:43:51 nirmkuma ship $ */

--  Global constant holding the package name

G_PKG_NAME                    CONSTANT VARCHAR2(30) := 'QP_Price_List_PUB';
...
l_p_PRICE_LIST_rec	Price_List_Rec_Type;	  --[prarasto]
l_p_PRICE_LIST_LINE_tbl	Price_List_Line_Tbl_Type; --[prarasto]
l_p_QUALIFIERS_tbl	Qp_Qualifier_Rules_Pub.Qualifiers_Tbl_Type;	  --[prarasto]
l_p_PRICING_ATTR_tbl	Pricing_Attr_Tbl_Type;	  --[prarasto]

BEGIN

--5018856, 5024801, 5024919
 SAVEPOINT PPL;
...
--    MO_GLOBAL.set_policy_context('M', null);--commented as MO_GLOBAL.Init will set_policy_context  to 'M' or 'S' based on profile settings
  ...
  END IF;
	...
	-- bug#3599792 / bug#3957078 / bug4261021
    IF p_PRICE_LIST_LINE_tbl.COUNT <> 0 THEN
       IF p_PRICE_LIST_LINE_tbl(1).list_header_id IS NOT NULL AND
          p_PRICE_LIST_LINE_tbl(1).list_header_id <> FND_API.G_MISS_NUM THEN
        BEGIN
        ...
        EXCEPTION
                WHEN OTHERS THEN
                 oe_debug_pub.add('PRICELIST LINE list_header_id : '||p_PRICE_LIST_LINE_tbl(1).list_header_id);
                 IF p_PRICE_LIST_rec.list_source_code IS NOT NULL
                 AND p_PRICE_LIST_rec.list_source_code <> FND_API.G_MISS_CHAR
                 THEN
                    l_list_source_code := p_PRICE_LIST_rec.list_source_code;
                 END IF;
        END;
       ELSIF  p_PRICE_LIST_LINE_tbl(1).list_line_id IS NOT NULL AND
          p_PRICE_LIST_LINE_tbl(1).list_line_id <> FND_API.G_MISS_NUM THEN
        BEGIN
                select list_source_code into l_list_source_code
                from qp_list_headers_b
        ...
        EXCEPTION
                WHEN OTHERS THEN
		 oe_debug_pub.add('PRICELIST LINE list_line_id : '|| p_PRICE_LIST_LINE_tbl(1).list_line_id);
                 IF p_PRICE_LIST_rec.list_source_code IS NOT NULL
                 AND p_PRICE_LIST_rec.list_source_code <> FND_API.G_MISS_CHAR
                 THEN
                    l_list_source_code := p_PRICE_LIST_rec.list_source_code;
   ...
   ELSE
	...
	END IF;
   END IF;
	-- shulin, allow FTE to use this api since it comes with advanced pricing
  IF l_installed_status IN ('S', 'N') AND l_request_type_code <> 'FTE' THEN  -- shulin, allow 'FTE' to use this API

     IF     l_installed_status = 'S'
        AND nvl(l_list_source_code,'NULL') = QP_GLOBALS.G_ENTITY_BSO -- bug#3599792
     THEN --bug#3385041
        NULL;
     ELSE
  ...
  END IF;
    ...
    FOR i in 1..l_PRICE_LIST_LINE_tbl.COUNT
    LOOP
        IF l_PRICE_LIST_LINE_tbl(i).list_line_type_code = 'PBH' AND
	   l_PRICE_LIST_LINE_tbl(i).operation = QP_GLOBALS.G_OPR_CREATE
	THEN
	   l_PRICE_LIST_LINE_tbl(i).continuous_price_break_flag := 'Y';
	END IF;
    END LOOP;
    ...
    l_p_PRICE_LIST_rec		:= l_PRICE_LIST_rec;	  --[prarasto]
    l_p_PRICE_LIST_LINE_tbl	:= l_PRICE_LIST_LINE_tbl; --[prarasto]
