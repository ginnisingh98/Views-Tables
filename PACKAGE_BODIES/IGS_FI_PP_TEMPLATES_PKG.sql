--------------------------------------------------------
--  DDL for Package Body IGS_FI_PP_TEMPLATES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_FI_PP_TEMPLATES_PKG" AS
/* $Header: IGSSID8B.pls 115.1 2003/09/08 16:55:31 smvk noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_fi_pp_templates%ROWTYPE;
  new_references igs_fi_pp_templates%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.payment_plan_desc                 := x_payment_plan_desc;
    new_references.closed_flag                       := x_closed_flag;
    new_references.installment_period_code           := x_installment_period_code;
    new_references.due_day_of_month                  := x_due_day_of_month;
    new_references.due_end_of_month_flag             := x_due_end_of_month_flag;
    new_references.due_cutoff_day                    := x_due_cutoff_day;
    new_references.processing_fee_type               := x_processing_fee_type;
    new_references.processing_fee_amt                := x_processing_fee_amt;
    new_references.installment_method_flag           := x_installment_method_flag;
    new_references.base_amt                          := x_base_amt;
  ...
  ||  Change History :
  ||  Who             When            What
  ||  smvk            06-Sep-2003     Added the check for the Fk with igs_lookups_view.
  ||  (reverse chronological order - newest change first)
  */
  BEGIN
    ...
    IF (((old_references.installment_period_code = new_references.installment_period_code )) OR
        ((new_references.installment_period_code IS NULL))) THEN
      NULL;
    ELSIF NOT igs_lookups_view_pkg.get_pk_for_validation ('IGS_FI_INSTALLMENT_PERIOD', new_references.installment_period_code
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.payment_plan_desc = x_payment_plan_desc)
        AND (tlinfo.closed_flag = x_closed_flag)
        AND (tlinfo.installment_period_code = x_installment_period_code)
        AND ((tlinfo.due_day_of_month = x_due_day_of_month) OR ((tlinfo.due_day_of_month IS NULL) AND (X_due_day_of_month IS NULL)))
        AND ((tlinfo.due_end_of_month_flag = x_due_end_of_month_flag) OR ((tlinfo.due_end_of_month_flag IS NULL) AND (X_due_end_of_month_flag IS NULL)))
        AND ((tlinfo.due_cutoff_day = x_due_cutoff_day) OR ((tlinfo.due_cutoff_day IS NULL) AND (X_due_cutoff_day IS NULL)))
        ...
        AND (tlinfo.installment_method_flag = x_installment_method_flag)
        AND ((tlinfo.base_amt = x_base_amt) OR ((tlinfo.base_amt IS NULL) AND (X_base_amt IS NULL)))
       ) THEN
      NULL;
    ELSE
