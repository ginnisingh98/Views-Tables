--------------------------------------------------------
--  DDL for Package Body BEN_ASR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ASR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_asr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_asnt_set_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   asnt_set_rt_id PK of record being inserted or updated.
...
--
Procedure chk_asnt_set_rt_id(p_asnt_set_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_asnt_set_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_asr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_asnt_set_rt_id                => p_asnt_set_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_asnt_set_rt_id,hr_api.g_number)
     <>  ben_asr_shd.g_old_rec.asnt_set_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_asr_shd.constraint_error('BEN_ASNT_SET_RTS_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_asnt_set_rt_id is not null then
      --
...
--
-- In Parameters
--   p_asnt_set_rt_id        PK of record being inserted or updated.
--   p_assignment_set_id     Value of lookup code.
...
and asnt_set_rt_id <> nvl(p_asnt_set_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_assignment_set_id <> nvl(ben_asr_shd.g_old_rec.assignment_set_id,hr_api.g_number)
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Assignment Set already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Assignment Set criteria');
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_asnt_set_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_asr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
Begin
   ...
   if c1%found then
      ...
      --fnd_message.set_name('BEN', 'vrbl =' || p_vrbl_rt_prfl_id || 'date=' || p_effective_date || 'bsnsid= ' || p_business_group_id || ' or = ' || p_ordr_num);
      --fnd_message.raise_error;
    null;
   end if;
   close c1;
...
function return_legislation_code
  (p_asnt_set_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_asnt_set_rt_f b
    where b.asnt_set_rt_id      = p_asnt_set_rt_id
