--------------------------------------------------------
--  DDL for Package Body BEN_EDR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EDR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_edr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_elig_dsblty_rsn_prte_f Row
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_edr_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.elig_dsblty_rsn_prte_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_edr_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    ben_edr_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.eligy_prfl_id = hr_api.g_number) then
    p_rec.eligy_prfl_id :=
    ben_edr_shd.g_old_rec.eligy_prfl_id;
  End If;
  If (p_rec.reason = hr_api.g_varchar2) then
    p_rec.reason :=
    ben_edr_shd.g_old_rec.reason;
  End If;
  If (p_rec.ordr_num = hr_api.g_number) then
    p_rec.ordr_num :=
    ben_edr_shd.g_old_rec.ordr_num;
  End If;
  If (p_rec.excld_flag = hr_api.g_varchar2) then
    p_rec.excld_flag :=
    ben_edr_shd.g_old_rec.excld_flag;
  End If;
  If (p_rec.edr_attribute_category = hr_api.g_varchar2) then
    p_rec.edr_attribute_category :=
    ben_edr_shd.g_old_rec.edr_attribute_category;
  End If;
  If (p_rec.edr_attribute1 = hr_api.g_varchar2) then
    p_rec.edr_attribute1 :=
    ben_edr_shd.g_old_rec.edr_attribute1;
  End If;
  If (p_rec.edr_attribute2 = hr_api.g_varchar2) then
    p_rec.edr_attribute2 :=
    ben_edr_shd.g_old_rec.edr_attribute2;
  End If;
  If (p_rec.edr_attribute3 = hr_api.g_varchar2) then
    p_rec.edr_attribute3 :=
    ben_edr_shd.g_old_rec.edr_attribute3;
  End If;
  If (p_rec.edr_attribute4 = hr_api.g_varchar2) then
    p_rec.edr_attribute4 :=
    ben_edr_shd.g_old_rec.edr_attribute4;
  End If;
  If (p_rec.edr_attribute5 = hr_api.g_varchar2) then
    p_rec.edr_attribute5 :=
    ben_edr_shd.g_old_rec.edr_attribute5;
  End If;
  If (p_rec.edr_attribute6 = hr_api.g_varchar2) then
    p_rec.edr_attribute6 :=
    ben_edr_shd.g_old_rec.edr_attribute6;
  End If;
  If (p_rec.edr_attribute7 = hr_api.g_varchar2) then
    p_rec.edr_attribute7 :=
    ben_edr_shd.g_old_rec.edr_attribute7;
  End If;
  If (p_rec.edr_attribute8 = hr_api.g_varchar2) then
    p_rec.edr_attribute8 :=
    ben_edr_shd.g_old_rec.edr_attribute8;
  End If;
  If (p_rec.edr_attribute9 = hr_api.g_varchar2) then
    p_rec.edr_attribute9 :=
    ben_edr_shd.g_old_rec.edr_attribute9;
  End If;
  If (p_rec.edr_attribute10 = hr_api.g_varchar2) then
    p_rec.edr_attribute10 :=
    ben_edr_shd.g_old_rec.edr_attribute10;
  End If;
  If (p_rec.edr_attribute11 = hr_api.g_varchar2) then
    p_rec.edr_attribute11 :=
    ben_edr_shd.g_old_rec.edr_attribute11;
  End If;
  If (p_rec.edr_attribute12 = hr_api.g_varchar2) then
    p_rec.edr_attribute12 :=
    ben_edr_shd.g_old_rec.edr_attribute12;
  End If;
  If (p_rec.edr_attribute13 = hr_api.g_varchar2) then
    p_rec.edr_attribute13 :=
    ben_edr_shd.g_old_rec.edr_attribute13;
  End If;
  If (p_rec.edr_attribute14 = hr_api.g_varchar2) then
    p_rec.edr_attribute14 :=
    ben_edr_shd.g_old_rec.edr_attribute14;
  End If;
  If (p_rec.edr_attribute15 = hr_api.g_varchar2) then
    p_rec.edr_attribute15 :=
    ben_edr_shd.g_old_rec.edr_attribute15;
  End If;
  If (p_rec.edr_attribute16 = hr_api.g_varchar2) then
    p_rec.edr_attribute16 :=
    ben_edr_shd.g_old_rec.edr_attribute16;
  End If;
  If (p_rec.edr_attribute17 = hr_api.g_varchar2) then
    p_rec.edr_attribute17 :=
    ben_edr_shd.g_old_rec.edr_attribute17;
  End If;
  If (p_rec.edr_attribute18 = hr_api.g_varchar2) then
    p_rec.edr_attribute18 :=
    ben_edr_shd.g_old_rec.edr_attribute18;
  End If;
  If (p_rec.edr_attribute19 = hr_api.g_varchar2) then
    p_rec.edr_attribute19 :=
    ben_edr_shd.g_old_rec.edr_attribute19;
  End If;
  If (p_rec.edr_attribute20 = hr_api.g_varchar2) then
    p_rec.edr_attribute20 :=
    ben_edr_shd.g_old_rec.edr_attribute20;
  End If;
  If (p_rec.edr_attribute21 = hr_api.g_varchar2) then
    p_rec.edr_attribute21 :=
    ben_edr_shd.g_old_rec.edr_attribute21;
  End If;
  If (p_rec.edr_attribute22 = hr_api.g_varchar2) then
    p_rec.edr_attribute22 :=
    ben_edr_shd.g_old_rec.edr_attribute22;
  End If;
  If (p_rec.edr_attribute23 = hr_api.g_varchar2) then
    p_rec.edr_attribute23 :=
    ben_edr_shd.g_old_rec.edr_attribute23;
  End If;
  If (p_rec.edr_attribute24 = hr_api.g_varchar2) then
    p_rec.edr_attribute24 :=
    ben_edr_shd.g_old_rec.edr_attribute24;
  End If;
  If (p_rec.edr_attribute25 = hr_api.g_varchar2) then
    p_rec.edr_attribute25 :=
    ben_edr_shd.g_old_rec.edr_attribute25;
  End If;
  If (p_rec.edr_attribute26 = hr_api.g_varchar2) then
    p_rec.edr_attribute26 :=
    ben_edr_shd.g_old_rec.edr_attribute26;
  End If;
  If (p_rec.edr_attribute27 = hr_api.g_varchar2) then
    p_rec.edr_attribute27 :=
    ben_edr_shd.g_old_rec.edr_attribute27;
  End If;
  If (p_rec.edr_attribute28 = hr_api.g_varchar2) then
    p_rec.edr_attribute28 :=
    ben_edr_shd.g_old_rec.edr_attribute28;
  End If;
  If (p_rec.edr_attribute29 = hr_api.g_varchar2) then
    p_rec.edr_attribute29 :=
    ben_edr_shd.g_old_rec.edr_attribute29;
  End If;
  If (p_rec.edr_attribute30 = hr_api.g_varchar2) then
    p_rec.edr_attribute30 :=
    ben_edr_shd.g_old_rec.edr_attribute30;
  End If;
  If (p_rec.criteria_score = hr_api.g_number) then
    p_rec.criteria_score :=
    ben_edr_shd.g_old_rec.criteria_score;
  End If;
  If (p_rec.criteria_weight = hr_api.g_number) then
    p_rec.criteria_weight :=
    ben_edr_shd.g_old_rec.criteria_weight;
  End If;
