--------------------------------------------------------
--  DDL for Package Body XNP_CSIORGDS_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIORGDS_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIORGDS' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIORGDS' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIORGDS' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIORGDS.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIORGDS', l_consumer_list );
  END IF;
...
l_trx_return_status VARCHAR2(1);
...
l_date date;
BEGIN
  l_trx_return_status  := l_fnd_success;
  l_trx_error_message  := NULL;
  IF (l_debug > 0) THEN
    csi_t_gen_utility_pvt.build_file_name(p_file_segment1 => 'csi',
                                          p_file_segment2 => to_char(sysdate,'DDMONYYYY'));
  END IF;
  csi_t_gen_utility_pvt.add(' ');
  csi_t_gen_utility_pvt.add('CSIORGDS'||'-'||to_char(sysdate,'DD-MON-YYYY HH24:MI:SS'));
  csi_t_gen_utility_pvt.add('  XML String: '||p_msg_text);
  savepoint one;
  xnp_xml_utils.decode(p_msg_text => p_msg_text,p_tag => 'MTL_TRANSACTION_ID',x_value => l_mtl_trx_rec.mtl_transaction_id);
  IF l_mtl_trx_rec.mtl_transaction_id is null THEN
    ...
    l_trx_error_rec.source_type          := 'CSIORGDS';
    csi_t_gen_utility_pvt.add('  Inv Transaction ID was NULL so raising Error');
    RAISE e_dpl_error;
  END IF;
  csi_t_gen_utility_pvt.add('  Invoking Trans: '||l_mtl_trx_rec.mtl_transaction_id);
  csi_inv_txnstub_pkg.execute_trx_dpl('CSIORGDS',
         l_mtl_trx_rec.mtl_transaction_id,
         l_trx_return_status,
         l_trx_error_rec);
  ...
  IF (l_debug > 0) THEN
    csi_t_gen_utility_pvt.build_file_name(p_file_segment1 => 'csi',
                                          p_file_segment2 => to_char(sysdate,'DDMONYYYY'));
  END IF;
  csi_t_gen_utility_pvt.add('  Transaction '||l_mtl_trx_rec.mtl_transaction_id||' completed Successfully');
  EXCEPTION
    WHEN e_dpl_error THEN
      csi_t_gen_utility_pvt.add('  Transaction '||l_mtl_trx_rec.mtl_transaction_id||' completed with error'||l_trx_error_rec.error_text);
      l_trx_error_rec.message_id := p_msg_header.message_id;
      l_trx_error_rec.message_string := p_msg_text;
    ...
    WHEN others THEN
      IF l_mtl_trx_rec.mtl_transaction_id IS NOT NULL THEN
        csi_t_gen_utility_pvt.add('Transaction '||l_mtl_trx_rec.mtl_transaction_id||' completed with error'||l_trx_error_rec.error_text);
        l_trx_error_rec.message_id := p_msg_header.message_id;
        l_trx_error_rec.message_string := p_msg_text;
        l_trx_error_rec.error_text := SQLERRM;
        l_trx_error_rec.transaction_type_id  := 143;
        l_trx_error_rec.source_type := 'CSIORGDS';
        l_trx_error_rec.source_id := l_mtl_trx_rec.mtl_transaction_id;
        l_trx_error_rec.inv_material_transaction_id := l_trx_error_rec.source_id;
        rollback to one;
        csi_inv_trxs_pkg.log_csi_error(l_trx_error_rec);
      ELSE
        csi_t_gen_utility_pvt.add('Transaction '||l_mtl_trx_rec.mtl_transaction_id||' completed with error - passing out to calling XNP Program'||l_trx_error_rec.error_text);
        raise;
      END IF;
  END;
END ;
