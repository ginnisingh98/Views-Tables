--------------------------------------------------------
--  DDL for Package Body OE_CNCL_VALIDATE_ADJ
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_CNCL_VALIDATE_ADJ" AS
/* $Header: OEXVCADB.pls 120.0 2005/05/31 23:33:51 appldev noship $ */

--  Global constant holding the package name

G_PKG_NAME                    CONSTANT VARCHAR2(30) := 'OE_CNCL_Validate_Adj';
...
BEGIN
    ...
    IF p_automatic_flag IS NULL OR
       p_automatic_flag = FND_API.G_MISS_CHAR THEN

        RETURN TRUE;
    END IF;

    IF p_automatic_flag NOT IN ('Y','N') THEN
		RAISE NO_DATA_FOUND;
    END IF;
...
BEGIN
    ...
    IF p_Change_Reason_Code IS NULL OR
        p_Change_Reason_Code = FND_API.G_MISS_CHAR
    THEN
...
BEGIN
    ...
    -- JAUTOMO: TO DO: Validation, but how ?
...
BEGIN
    ...
    IF p_Redeemed_Flag NOT IN ('Y','N') THEN
		RAISE NO_DATA_FOUND;
    END IF;
...
BEGIN
    ...
    IF p_Accrual_Flag IS NULL OR
        p_Accrual_Flag = FND_API.G_MISS_CHAR THEN
            RETURN TRUE;
    END IF;

    IF p_Accrual_Flag NOT IN ('Y','N') THEN
		RAISE NO_DATA_FOUND;
    END IF;
...
BEGIN
    ...
    -- JAUTOMO: TO DO: Validation required here ?
