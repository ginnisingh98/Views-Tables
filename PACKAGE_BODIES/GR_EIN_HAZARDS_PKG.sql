--------------------------------------------------------
--  DDL for Package Body GR_EIN_HAZARDS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_EIN_HAZARDS_PKG" AS
/*$Header: GRHIEIHB.pls 115.3 2002/10/29 16:39:46 mgrosser noship $*/
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ...
      FND_MESSAGE.SET_TOKEN('CODE',
         		     p_european_index_number||' '||p_hazard_classification_code,
            		     FALSE);
      IF FND_API.To_Boolean(p_called_by_form) THEN
         APP_EXCEPTION.Raise_Exception;
      ELSE
...
CURSOR c_lock_ein_hazard
 IS
   SELECT	last_update_date
   FROM		gr_ein_hazards
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockEinRcd	  c_lock_ein_hazard%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_ein_hazard;
   FETCH c_lock_ein_hazard INTO LockEinRcd;
   IF c_lock_ein_hazard%NOTFOUND THEN
	  CLOSE c_lock_ein_hazard;
...
/*   Check the hazard classification codes */
   ...
   OPEN c_get_hazards;
   FETCH c_get_hazards INTO HazClassRecord;
   IF c_get_hazards%NOTFOUND THEN
...
CURSOR c_get_eih_rowid
 IS
   SELECT eih.rowid
   FROM	  gr_ein_hazards eih
   WHERE  eih.european_index_number = p_european_index_number
   AND    eih.hazard_classification_code = p_hazard_classification_code;
EihRecord			   c_get_eih_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_eih_rowid;
   FETCH c_get_eih_rowid INTO EihRecord;
   IF c_get_eih_rowid%FOUND THEN
      x_key_exists := 'T';
      x_rowid := EihRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_eih_rowid;
