--------------------------------------------------------
--  DDL for Package Body PER_CTK_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CTK_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_ctk_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the per_tasks_in_checklist Row
  --
  update per_tasks_in_checklist
    set
     task_in_checklist_id            = p_rec.task_in_checklist_id
    ,checklist_id                    = p_rec.checklist_id
    ,checklist_task_name             = p_rec.checklist_task_name
    ,eligibility_object_id           = p_rec.eligibility_object_id
    ,eligibility_profile_id          = p_rec.eligibility_profile_id
    ,ame_attribute_identifier        = p_rec.ame_attribute_identifier
    ,description                     = p_rec.description
    ,mandatory_flag                  = p_rec.mandatory_flag
    ,target_duration                 = p_rec.target_duration
    ,target_duration_uom             = p_rec.target_duration_uom
...
-- for bgid
  cursor c_bg_id(p_checklist_id number) is
  select business_group_id
  from per_checklists
  where checklist_id = p_checklist_id;

 -- for elgibility profile

  cursor c_elig_prfl_dates(cur_p_elig_prfl_id number) is
    select
         effective_start_date,object_version_number
    FROM
        ben_eligy_prfl_f
    WHERE
        trunc(sysdate) between effective_start_date and effective_end_date
    AND stat_cd = 'A'
    AND bnft_cagr_prtn_cd = 'CHKLIST'
    and ELIGY_PRFL_ID=cur_p_elig_prfl_id;
  ...
  l_tab_name varchar2(30) := 'PER_TASKS_IN_CHECKLIST';
  l_col_name varchar2(30) := 'TASK_IN_CHECKLIST_ID';
  l_tic_id   number;
...
Begin
  ...
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
--  hr_utility.set_location('g_old_rec.eligibility_profile_id='||per_ctk_shd.g_old_rec.eligibility_profile_id,10);
--  hr_utility.set_location('p_rec.eligibility_profile_id='||p_rec.eligibility_profile_id,10);
  if ( nvl(per_ctk_shd.g_old_rec.eligibility_profile_id, hr_api.g_number) <> hr_api.g_number  AND
      ...
      -- delete old eligy profile eligy object (child)
        ...
        close c_elig_prfl_elig_obj;
		--hr_utility.set_location('p_rec.l_elig_obj_elig_prfl_id='||l_elig_obj_elig_prfl_id,10);
		--hr_utility.set_location('p_rec.l_prf_obj_ovn='||l_prf_obj_ovn,10);
		--hr_utility.set_location('p_rec.l_effective_date='||l_effective_date,10);
        --hr_utility.set_location('eligy_obj_elig_profile deleted',10);
        ...
        close c_elig_obj;
		--hr_utility.set_location('p_rec.l_obj_ovn='||l_obj_ovn,20);
		--hr_utility.set_location('p_rec.l_effective_date='||to_char(l_effective_date),20);

        p_rec.eligibility_object_id :=null;
      -- create eligy object (parent)
      -- create old eligy profile eligy object (child)

   end if;

     if ((nvl(per_ctk_shd.g_old_rec.eligibility_profile_id, hr_api.g_number)= hr_api.g_number  AND
          nvl(p_rec.eligibility_profile_id, hr_api.g_number) <> hr_api.g_number ))
      OR  -- in this case above we have deleted old records
         (nvl(per_ctk_shd.g_old_rec.eligibility_profile_id, hr_api.g_number) <> hr_api.g_number  AND
          nvl(p_rec.eligibility_profile_id, hr_api.g_number) <> hr_api.g_number AND
          nvl(per_ctk_shd.g_old_rec.eligibility_profile_id, hr_api.g_number) <> nvl(p_rec.eligibility_profile_id, hr_api.g_number))
      then

      open c_elig_prfl_dates(p_rec.eligibility_profile_id);
          fetch c_elig_prfl_dates into l_effective_date,l_prf_ovn;
      close c_elig_prfl_dates;

      Open c_bg_id(p_rec.checklist_id);
          Fetch c_bg_id Into l_bgid;
      Close c_bg_id;

      l_tic_id:=p_rec.checklist_id;
      ...
      ben_elig_obj_api.create_ELIG_OBJ
        (
           ...
           ,p_table_name                     => l_tab_name --'PER_TASKS_IN_CHECKLIST'
           ,p_column_name                    => l_col_name
           ,p_column_value                   => l_tic_id
           ,p_object_version_number          => l_obj_ovn
           ,p_effective_date                 => l_effective_date
       );

     hr_utility.set_location('levaving :'||l_proc, 20);
     p_rec.eligibility_object_id := l_elig_obj_id;
    --
    -- Add Create Eligibility Object logic
    --
    --
    ben_ELIG_OBJ_ELIG_PROFL_api. create_ELIG_OBJ_ELIG_PROFL
     (
      p_validate                       => false
     ,p_elig_obj_id                    => p_rec.eligibility_object_id
     ,p_elig_prfl_id                   => p_rec.eligibility_profile_id
     ,p_mndtry_flag                    => 'N'
     ,p_business_group_id              => l_bgid
     ,p_effective_start_date           => l_effective_start_date
     ,p_effective_end_date             => l_effective_end_date
...
Begin
    ...
    per_ctk_rku.after_update
      ...
      => p_rec.checklist_task_name
      ,p_eligibility_object_id
      => p_rec.eligibility_object_id
      ,p_eligibility_profile_id
      => p_rec.eligibility_profile_id
      ,p_ame_attribute_identifier
      => p_rec.ame_attribute_identifier
      ,p_description
      => p_rec.description
      ,p_mandatory_flag
      => p_rec.mandatory_flag
      ,p_target_duration
      => p_rec.target_duration
      ,p_target_duration_uom
      => p_rec.target_duration_uom
      ,p_task_sequence
      => p_rec.task_sequence
...
Begin
  ...
  --
  If (p_rec.checklist_id = hr_api.g_number) then
    p_rec.checklist_id :=
    per_ctk_shd.g_old_rec.checklist_id;
  End If;
  If (p_rec.checklist_task_name = hr_api.g_varchar2) then
    p_rec.checklist_task_name :=
    per_ctk_shd.g_old_rec.checklist_task_name;
  End If;
  If (p_rec.eligibility_object_id = hr_api.g_number) then
    p_rec.eligibility_object_id :=
    per_ctk_shd.g_old_rec.eligibility_object_id;
  End If;
  If (p_rec.eligibility_profile_id = hr_api.g_number) then
    p_rec.eligibility_profile_id :=
    per_ctk_shd.g_old_rec.eligibility_profile_id;
  End If;
  If (p_rec.ame_attribute_identifier = hr_api.g_varchar2) then
    p_rec.ame_attribute_identifier :=
    per_ctk_shd.g_old_rec.ame_attribute_identifier;
  End If;
  ...
  If (p_rec.mandatory_flag = hr_api.g_varchar2) then
    ...
    per_ctk_shd.g_old_rec.mandatory_flag;
  End If;
  If (p_rec.target_duration = hr_api.g_number) then
    p_rec.target_duration :=
    per_ctk_shd.g_old_rec.target_duration;
  End If;
  If (p_rec.target_duration_uom = hr_api.g_varchar2) then
    p_rec.target_duration_uom :=
    per_ctk_shd.g_old_rec.target_duration_uom;
  End If;
...
Begin
  ...
  --
  per_ctk_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_ctk_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_ctk_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure upd
  ...
  ,p_checklist_task_name          in     varchar2  default hr_api.g_varchar2
