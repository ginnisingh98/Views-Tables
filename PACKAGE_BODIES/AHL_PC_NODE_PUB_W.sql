--------------------------------------------------------
--  DDL for Package Body AHL_PC_NODE_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_PC_NODE_PUB_W" as
  /* $Header: AHLWPCNB.pls 115.6 2002/12/02 14:57:32 pbarman noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).pc_node_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).name := a2(indx);
          t(ddindx).description := a3(indx);
          t(ddindx).pc_header_id := a4(indx);
          t(ddindx).parent_node_id := a5(indx);
          t(ddindx).child_count := a6(indx);
          t(ddindx).operation_status_flag := a7(indx);
          t(ddindx).draft_flag := a8(indx);
          t(ddindx).link_to_node_id := a9(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a26 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).pc_node_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).name;
          a3(indx) := t(ddindx).description;
          a4(indx) := t(ddindx).pc_header_id;
          a5(indx) := t(ddindx).parent_node_id;
          a6(indx) := t(ddindx).child_count;
          a7(indx) := t(ddindx).operation_status_flag;
          a8(indx) := t(ddindx).draft_flag;
          a9(indx) := t(ddindx).link_to_node_id;
  ...
  as
    ddp_x_nodes_tbl ahl_pc_node_pub.pc_node_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_pc_node_pub_w.rosetta_table_copy_in_p8(ddp_x_nodes_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
    ...
    ahl_pc_node_pub.process_nodes(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      p_module_type,
    ...
    ahl_pc_node_pub_w.rosetta_table_copy_out_p8(ddp_x_nodes_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
