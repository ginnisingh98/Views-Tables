--------------------------------------------------------
--  DDL for Package Body HR_ITL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_ITL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hr_itl_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_itl_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_ki_integrations_tl Row
  --
  update hr_ki_integrations_tl
    set
     integration_id                  = p_rec.integration_id
    ,partner_name                    = p_rec.partner_name
    ,service_name                    = p_rec.service_name
...
Begin
  ...
  --
  If (p_rec.partner_name = hr_api.g_varchar2) then
    p_rec.partner_name :=
    hr_itl_shd.g_old_rec.partner_name;
  End If;
  If (p_rec.service_name = hr_api.g_varchar2) then
    p_rec.service_name :=
    hr_itl_shd.g_old_rec.service_name;
  End If;
...
Begin
  ...
  --
  hr_itl_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_itl_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_itl_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
  ...
  cursor csr_upd_langs is
    select itl.language
     ...
     where
       ...
       and p_language_code in (itl.language,itl.source_lang);
  --
