--------------------------------------------------------
--  DDL for Package Body IGF_SL_DL_CHG_SEND_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_SL_DL_CHG_SEND_PKG" AS
/* $Header: IGFLI17B.pls 115.9 2002/11/28 14:25:19 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igf_sl_dl_chg_send_all%ROWTYPE;
  new_references igf_sl_dl_chg_send_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.chg_num                           := x_chg_num;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SL_SCHG_DBTH_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
    ...
    l_org_id                     igf_sl_dl_chg_send_all.org_id%TYPE DEFAULT igf_aw_gen.get_org_id;
  ...
  BEGIN
    ...
    SELECT igf_sl_dl_chg_send_s.nextval INTO
           x_chg_num FROM dual;
    ...
    INSERT INTO igf_sl_dl_chg_send_all (
      chg_num,
      dbth_id,
      loan_number,
      chg_code,
  ...
  */
  BEGIN
    ...
    DELETE FROM igf_sl_dl_chg_send_all
    WHERE rowid = x_rowid;
