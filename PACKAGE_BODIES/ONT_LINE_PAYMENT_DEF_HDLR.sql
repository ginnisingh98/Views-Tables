--------------------------------------------------------
--  DDL for Package Body ONT_LINE_PAYMENT_DEF_HDLR
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ONT_LINE_PAYMENT_DEF_HDLR" AS
...
--  FILENAME
--  
--      ONT_LINE_PAYMENT_Def_Hdlr
--  
--  DESCRIPTION
--  
--      Body of package ONT_LINE_PAYMENT_Def_Hdlr
--  
--  NOTES
--  
--  HISTORY
...
G_PKG_NAME                    CONSTANT VARCHAR2(30) := 'ONT_LINE_PAYMENT_Def_Hdlr';
 
  g_entity_code  varchar2(15) := 'LINE_PAYMENT';
  g_database_object_name varchar2(30) :='OE_AK_LINE_PAYMENTS_V';
...
oe_debug_pub.ADD('Enter ONT_LINE_PAYMENT_Def_Hdlr.Default_Record');
 
IF p_iteration =1 THEN
OE_LINE_PAYMENT_Security.G_Is_Caller_Defaulting := 'Y';
  g_record := p_x_rec;
END IF;
...
--  Get the defaulting api registered in the AK AND default
    l_attr:=l_attr||' 1';
    g_record.PAYMENT_TYPE_CODE := ONT_D1025_PAYMENT_TYPE_CODE.Get_Default_Value(g_record);
    l_attr:=l_attr||' 2';
  ...
  IF g_record.operation = OE_GLOBALS.G_OPR_UPDATE THEN
    ...
    IF NOT OE_GLOBALS.Equal(g_record.payment_type_code, p_in_old_rec.payment_type_code) THEN
      IF OE_LINE_PAYMENT_SECURITY.PAYMENT_TYPE_CODE(p_record => g_record, x_on_operation_action	=> l_action) = OE_PC_GLOBALS.YES THEN
  ...
  IF g_record.PAYMENT_TYPE_CODE IS NOT NULL THEN
    l_attr:=l_attr||' 4';
      ...
      OE_LINE_PAYMENT_CL_DEP_ATTR.PAYMENT_TYPE_CODE(g_record);
  END IF;
END IF;
l_attr:= 'ATTRIBUTE1';
...
--  Get the defaulting api registered in the AK AND default
    l_attr:=l_attr||' 1';
    g_record.CREDIT_CARD_NUMBER := ONT_D1025_CREDIT_CARD_NUMBER.Get_Default_Value(g_record);
    l_attr:=l_attr||' 2';
...
--  Get the defaulting api registered in the AK AND default
    l_attr:=l_attr||' 1';
    g_record.CREDIT_CARD_EXPIRATION_DATE := ONT_D1025_CREDIT_CARD_EXPIRATI.Get_Default_Value(g_record);
    l_attr:=l_attr||' 2';
-- There is no security api registered in the AK dictionary  
      -- There is no validation api registered in the AK dictionary  
END IF;
...
--  Get the defaulting api registered in the AK AND default
    l_attr:=l_attr||' 1';
    g_record.CREDIT_CARD_HOLDER_NAME := ONT_D1025_CREDIT_CARD_HOLDER_N.Get_Default_Value(g_record);
    l_attr:=l_attr||' 2';
-- There is no security api registered in the AK dictionary  
      -- There is no validation api registered in the AK dictionary  
END IF;
...
--  Get the defaulting api registered in the AK AND default
    l_attr:=l_attr||' 1';
    g_record.RECEIPT_METHOD_ID := ONT_D1025_RECEIPT_METHOD_ID.Get_Default_Value(g_record);
    l_attr:=l_attr||' 2';
  -- For UPDATE operations, check security if new defaulted value is not equal to old value
  IF g_record.operation = OE_GLOBALS.G_OPR_UPDATE THEN
    l_attr:=l_attr||' 3';
    IF NOT OE_GLOBALS.Equal(g_record.receipt_method_id, p_in_old_rec.receipt_method_id) THEN
      IF OE_LINE_PAYMENT_SECURITY.RECEIPT_METHOD(p_record => g_record, x_on_operation_action	=> l_action) = OE_PC_GLOBALS.YES THEN
      -- Raise error if security returns YES, operation IS CONSTRAINED
        RAISE FND_API.G_EXC_ERROR;
      END IF;
      OE_GLOBALS.G_ATTR_UPDATED_BY_DEF := 'Y';
  ...
  IF g_record.RECEIPT_METHOD_ID IS NOT NULL THEN
    l_attr:=l_attr||' 4';
      -- if valid, clear dependent attributes
      OE_LINE_PAYMENT_CL_DEP_ATTR.RECEIPT_METHOD(g_record);
  END IF;
END IF;
...
IF p_iteration =1 THEN
OE_LINE_PAYMENT_Security.G_Is_Caller_Defaulting := 'N';
  p_x_rec := g_record;
END IF;
 
oe_debug_pub.ADD('Exit ONT_LINE_PAYMENT_Def_Hdlr.Default_Record');
