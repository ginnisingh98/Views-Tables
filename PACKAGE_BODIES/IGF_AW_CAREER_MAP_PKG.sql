--------------------------------------------------------
--  DDL for Package Body IGF_AW_CAREER_MAP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_AW_CAREER_MAP_PKG" AS
/* $Header: IGFWI46B.pls 115.4 2002/11/28 12:17:14 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igf_aw_career_map_all%ROWTYPE;
  new_references igf_aw_career_map_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.career_level_id                   := x_career_level_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igf_aw_career_map_all
      WHERE    career_level_id = x_career_level_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igf_aw_career_map_all
      WHERE    program_type = x_program_type
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igf_aw_career_map_all
      WHERE   ((program_type = x_course_type));
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_AW_CLV_PTY_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.career_level_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igf_aw_career_map_s.NEXTVAL
    INTO      x_career_level_id
    FROM      dual;
    ...
    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_career_level_id                   => x_career_level_id,
    ...
    INSERT INTO igf_aw_career_map_all (
      career_level_id,
      program_type,
      class_standing,
      org_id,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.program_type = x_program_type)
  ...
  */
  BEGIN
    ...
    DELETE FROM igf_aw_career_map_all
    WHERE rowid = x_rowid;
