--------------------------------------------------------
--  DDL for Package Body GR_SUB_HEADINGS_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_SUB_HEADINGS_B_PKG" AS
/*$Header: GRHISHBB.pls 115.3 2002/10/25 16:05:43 gkelly ship $*/
...
/*	  Now call the check foreign key procedure */
   ...
   Check_Foreign_Keys
			     (p_sub_heading_code,
				  p_sub_heading_print_font,
				  p_sub_heading_print_size,
				  l_return_status,
				  l_oracle_error,
				  l_msg_data);
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_sub_headings_b
   		  	     (sub_heading_code,
				  sub_heading_print_font,
				  sub_heading_print_size,
				  created_by,
				  creation_date,
				  last_updated_by,
				  last_update_date,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_sub_headings_b
	  SET	 sub_heading_code               = p_sub_heading_code,
			 sub_heading_print_font         = p_sub_heading_print_font,
			 sub_heading_print_size         = p_sub_heading_print_size,
...
CURSOR c_lock_sub_hdg_b
 IS
   SELECT	*
   FROM		gr_sub_headings_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockSubHdgRcd	  c_lock_sub_hdg_b%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_sub_hdg_b;
   FETCH c_lock_sub_hdg_b INTO LockSubHdgRcd;
   IF c_lock_sub_hdg_b%NOTFOUND THEN
	  CLOSE c_lock_sub_hdg_b;
...
**   Document Headings
*/
CURSOR c_get_doc_hdgs
 IS
   SELECT COUNT(*)
   FROM   gr_document_headings dh
   WHERE  dh.sub_heading_code = p_sub_heading_code;

/*
**   Item document details
*/
CURSOR c_get_item_doc
 IS
   SELECT COUNT(*)
   FROM   gr_item_document_dtls idd
   WHERE  idd.sub_heading_code = p_sub_heading_code;
...
*/
   OPEN c_get_doc_hdgs;
   FETCH c_get_doc_hdgs INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
      l_msg_data := l_msg_data || ' gr_document_headings,';
   END IF;
   CLOSE c_get_doc_hdgs;
/*
** Check item document details
*/
   l_record_count := 0;
   OPEN c_get_item_doc;
   FETCH c_get_item_doc INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
	  l_msg_data := l_msg_data || ' gr_item_document_dtls,';
   END IF;
   CLOSE c_get_item_doc;
...
CURSOR c_get_sub_hdg_b_rowid
 IS
   SELECT mhb.rowid
   FROM	  gr_sub_headings_b mhb
   WHERE  mhb.sub_heading_code = p_sub_heading_code;
SubHdgBRecord			   c_get_sub_hdg_b_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_sub_heading_code;

   x_key_exists := 'F';
   OPEN c_get_sub_hdg_b_rowid;
   FETCH c_get_sub_hdg_b_rowid INTO SubHdgBRecord;
   IF c_get_sub_hdg_b_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := SubHdgBRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_sub_hdg_b_rowid;
