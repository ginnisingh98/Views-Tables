--------------------------------------------------------
--  DDL for Package Body AHL_FMP_MR_ACTION_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_FMP_MR_ACTION_PVT_W" as
  /* $Header: AHLWMRAB.pls 115.3 2002/12/04 20:35:04 rtadikon noship $ */
  ...
  procedure rosetta_table_copy_in_p1(t out nocopy ahl_fmp_mr_action_pvt.mr_action_tbl, a0 JTF_NUMBER_TABLE
    , a1 JTF_NUMBER_TABLE
    , a2 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).mr_action_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).mr_header_id := a2(indx);
          t(ddindx).mr_action_code := a3(indx);
          t(ddindx).mr_action := a4(indx);
          t(ddindx).plan_id := a5(indx);
          t(ddindx).plan := a6(indx);
  ...
  procedure rosetta_table_copy_out_p1(t ahl_fmp_mr_action_pvt.mr_action_tbl, a0 out nocopy JTF_NUMBER_TABLE
  ...
  if t is null or t.count = 0 then
    ...
    a29 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).mr_action_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).mr_header_id;
          a3(indx) := t(ddindx).mr_action_code;
          a4(indx) := t(ddindx).mr_action;
          a5(indx) := t(ddindx).plan_id;
          a6(indx) := t(ddindx).plan;
  ...
  procedure process_mr_action(p_api_version  NUMBER
    ...
    , p9_a7 in out nocopy JTF_VARCHAR2_TABLE_2000
  ...
  as
    ddp_x_mr_action_tbl ahl_fmp_mr_action_pvt.mr_action_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    ahl_fmp_mr_action_pvt_w.rosetta_table_copy_in_p1(ddp_x_mr_action_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
    ...
    ahl_fmp_mr_action_pvt.process_mr_action(p_api_version,
      p_init_msg_list,
      p_commit,
      p_validation_level,
      p_default,
    ...
    ahl_fmp_mr_action_pvt_w.rosetta_table_copy_out_p1(ddp_x_mr_action_tbl, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
