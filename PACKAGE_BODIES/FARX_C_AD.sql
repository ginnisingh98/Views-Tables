--------------------------------------------------------
--  DDL for Package Body FARX_C_AD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FARX_C_AD" as
/* $Header: farxcadb.pls 120.3.12010000.2 2009/07/19 10:48:37 glchen ship $ */
...
PROCEDURE ADD_PERIOD (
  ...
  argument13        in  varchar2  default  null, -- category segmrnt number
...
begin
 -- for debugging
   ...
   IF (g_print_debug) THEN
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument1:' ||argument1);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument2:' ||argument2);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument3:' ||argument3);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument4:' ||argument4);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument5:' ||argument5);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument6:' ||argument6);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument7:' ||argument7);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument8:' ||argument8);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument9:' ||argument9);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument10:' ||argument10);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument11:' ||argument11);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument12:' ||argument12);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument13:' ||argument13);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument14:' ||argument14);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument15:' ||argument15);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument16:' ||argument16);
   	fa_rx_util_pkg.debug('ADD_PERIOD: ' || 'argument17:' ||argument17);
   END IF;
...
--  and fcr.phase_code = 'R'
--  and fcr.concurrent_program_id = fcp.concurrent_program_id
--  and fcp.concurrent_program_name = 'RXFAADD';

  h_request_id := fnd_global.conc_request_id;
  fnd_profile.get('USER_ID',h_user_id);

--  h_begin_str := substr(argument2,1,instr(argument2,'_')-1);
--  h_end_str := substr(argument3,1,instr(argument3,'_')-1);
--
--  h_date_format := substr(argument2,instr(argument2,'_')+1);
--  h_begin_dpis := to_date(h_begin_str,h_date_format);
--  h_end_dpis := to_date(h_end_str,h_date_format);

h_begin_dpis := to_date(argument2,'YYYY/MM/DD HH24:MI:SS');
h_end_dpis := to_date(argument3,'YYYY/MM/DD HH24:MI:SS');

  farx_ad.add_by_date (
	book => argument1,
	begin_dpis => h_begin_dpis,
	end_dpis => h_end_dpis,
	request_id => h_request_id,
	user_id => h_user_id,
	retcode => retcode,
...
PROCEDURE ADD_RESP (
  ...
  argument3                 in      varchar2,   -- accounting flex struct
  argument4             in      varchar2,       -- begin cost center
  argument5           in      varchar2,	        -- end cost center
  argument6        in  varchar2  default  null,
  argument7        in  varchar2  default  null,
  argument8        in  varchar2  default  null,
  argument9        in  varchar2  default  null,
...
--  and fcr.argument2 = argument2
--  and fcr.argument3 = argument3
--  and fcr.argument4 = argument4
--  and fcr.phase_code = 'R'
--  and fcr.concurrent_program_id = fcp.concurrent_program_id
--  and fcp.concurrent_program_name = 'RXFAADR';
