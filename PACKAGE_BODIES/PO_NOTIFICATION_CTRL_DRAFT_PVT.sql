--------------------------------------------------------
--  DDL for Package Body PO_NOTIFICATION_CTRL_DRAFT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PO_NOTIFICATION_CTRL_DRAFT_PVT" AS
/* $Header: PO_NOTIFICATION_CTRL_DRAFT_PVT.plb 120.2 2005/07/26 16:21 bao noship $ */

d_pkg_name CONSTANT varchar2(50) :=
  PO_LOG.get_package_base('PO_NOTIFICATION_CTRL_DRAFT_PVT');
...
BEGIN
  ...
  FORALL i IN 1..p_draft_id_tbl.COUNT
    ...
    WHERE EXISTS (SELECT 1
                  ...
                  WHERE  PNCD.draft_id = p_draft_id_tbl(i)
                  AND    PNCD.notification_id = NVL(p_notification_id_tbl(i),
...
--  None
--Function:
--  Process notification control draft records and merge them to transaction
--  table. It also performs all additional work related specifically to the
--  merge action
--Parameters:
--IN:
--p_draft_info
...
BEGIN
  ...
  IF (p_draft_info.notification_ctrl_changed = FND_API.G_FALSE) THEN
    IF (PO_LOG.d_stmt) THEN
  ...
  d_position := 10;
  -- Merge Changes
  PO_NOTIFICATION_CTRL_DRAFT_PKG.merge_changes
