--------------------------------------------------------
--  DDL for Package Body XNP_CSIWIPNI_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIWIPNI_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIWIPNI' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIWIPNI' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIWIPNI' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIWIPNI.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIWIPNI', l_consumer_list );
  END IF;
...
NULL ;
DECLARE
      l_txn_type    varchar2(30) := 'CSIWIPNI';
