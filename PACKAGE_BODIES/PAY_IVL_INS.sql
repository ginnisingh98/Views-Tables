--------------------------------------------------------
--  DDL for Package Body PAY_IVL_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_IVL_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_ivl_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_input_value_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  pay_ivl_ins.g_input_value_id_i := p_input_value_id;
  --
...
--
  Cursor C_Sel1 Is
    ...
    where  t.input_value_id       = p_rec.input_value_id
    and    t.effective_start_date =
             pay_ivl_shd.g_old_rec.effective_start_date
    and    t.effective_end_date   = (p_validation_start_date - 1);
--
  l_proc                varchar2(72) := g_package||'dt_insert_dml';
  l_created_by          pay_input_values_f.created_by%TYPE;
  l_creation_date       pay_input_values_f.creation_date%TYPE;
  l_last_update_date    pay_input_values_f.last_update_date%TYPE;
  l_last_updated_by     pay_input_values_f.last_updated_by%TYPE;
  l_last_update_login   pay_input_values_f.last_update_login%TYPE;
--
Begin
  ...
  --
  -- Insert the row into: pay_input_values_f
  --
...
--
  Cursor C_Sel1 is select pay_input_values_s.nextval from sys.dual;
--
 Cursor C_Sel2 is
    Select null
      from pay_input_values_f
     where input_value_id =
             pay_ivl_ins.g_input_value_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
    If (pay_ivl_ins.g_input_value_id_i is not null) Then
    --
    ...
    If C_Sel2%found Then
       ...
       fnd_message.set_token('TABLE_NAME','pay_input_values_f');
       fnd_message.raise_error;
    End If;
    Close C_Sel2;
    --
    ...
    p_rec.input_value_id :=
      pay_ivl_ins.g_input_value_id_i;
    pay_ivl_ins.g_input_value_id_i := null;
  Else
    --
...
--
  ...
  cursor csr_display_sequence is
    select 1 + nvl(max( display_sequence ), 0)
    from pay_input_values_f
...
Begin
  ...
  if p_rec.display_sequence is null then
  open csr_display_sequence;
  fetch csr_display_sequence into p_rec.display_sequence;
  close csr_display_sequence;
  end if;
  --
  -- Insert the row
  --
  pay_ivl_ins.insert_dml
    (p_rec                   => p_rec
    ,p_effective_date        => p_effective_date
    ,p_datetrack_mode        => l_datetrack_mode
    ,p_validation_start_date => l_validation_start_date
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  -- Check for Pay Value special case
  -- whereby 'PAY VALUE' goes into base table
  -- (instead of base language value)
  if (p_name = hr_general.pay_value) then
     l_name := 'Pay Value';
  else
     l_name := p_name;
  end if;
