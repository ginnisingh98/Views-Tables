--------------------------------------------------------
--  DDL for Package Body AME_GCF_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_GCF_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_gcf_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
 ...
 ,p_application_id =>  p_rec.application_id
    ...
    --
    --
    -- Update the ame_approval_group_config Row
    --
...
Begin
  ...
  begin
    ...
    ame_gcf_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_voting_regime_o
      => ame_gcf_shd.g_old_rec.voting_regime
      ,p_order_number_o
      => ame_gcf_shd.g_old_rec.order_number
      ,p_start_date_o
      => ame_gcf_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_gcf_shd.g_old_rec.end_date
      ,p_object_version_number_o
      => ame_gcf_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
 ...
 ,p_application_id =>  p_rec.application_id
  ...
  --
  ame_gcf_upd.convert_defs(p_rec);
  --
  ame_gcf_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
