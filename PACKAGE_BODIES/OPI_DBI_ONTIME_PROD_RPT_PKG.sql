--------------------------------------------------------
--  DDL for Package Body OPI_DBI_ONTIME_PROD_RPT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OPI_DBI_ONTIME_PROD_RPT_PKG" AS
/*$Header: OPIDOTPRPTB.pls 120.2 2005/09/29 23:12 sberi noship $ */
FUNCTION GET_ONTIME_PROD_SEL_CLAUSE(p_view_by_dim IN VARCHAR2, p_join_tbl IN
                                   poa_dbi_util_pkg.POA_DBI_JOIN_TBL)
RETURN VARCHAR2;


/* -------------------------------------------------------------------------------------------
   Procedure Name: GET_ONTIME_PROD_SQL
   ...
   Purpose       : This procedure calls process parameters of the OPI util package to get things
		   ...
		   It also forms the report query by calling the function GET_ONTIME_PROD_SEL_
		   CLAUSE
----------------------------------------------------------------------------------------------
*/
PROCEDURE GET_ONTIME_PROD_SQL(p_param IN BIS_PMV_PAGE_PARAMETER_TBL,
...
-- Add measure columns that need to be aggregated
        ...
        poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     => 'late_prod_cnt' ,
                                     p_alias_name   => 'late_prod_cnt',
				     p_prior_code   => poa_dbi_util_pkg.BOTH_PRIORS,
                                     p_grand_total  => 'Y',
				     p_to_date_type => 'XTD'
                                     );
   	poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     =>'late_prod_val_' || l_cur_suffix,
                                     p_alias_name   =>'late_prod_val',
				     p_prior_code   => poa_dbi_util_pkg.BOTH_PRIORS,
				     p_grand_total  => 'Y',
				     p_to_date_type => 'XTD'
				     );
        poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     => 'ontime_prod_cnt',
                                     p_alias_name   => 'ontime_prod_cnt',
                                     p_prior_code   => poa_dbi_util_pkg.BOTH_PRIORS,
                                     p_grand_total  => 'Y',
				     p_to_date_type => 'XTD'
				     );
	poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     =>'ontime_prod_val_' || l_cur_suffix,
                                     p_alias_name   =>'ontime_prod_val',
				     p_prior_code   => poa_dbi_util_pkg.BOTH_PRIORS,
				     p_grand_total  => 'Y',
				     p_to_date_type => 'XTD'
				     );
    ...
    --{
          poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     => 'late_prod_qty' ,
                                     p_alias_name   => 'late_prod_qty',
				     p_prior_code   => poa_dbi_util_pkg.NO_PRIORS,
                                     p_grand_total  => 'N',
				     p_to_date_type => 'XTD'
				     );
          poa_dbi_util_pkg.add_column (p_col_tbl      => l_col_tbl,
                                     p_col_name     => 'ontime_prod_qty' ,
                                     p_alias_name   => 'ontime_prod_qty',
				     p_prior_code   => poa_dbi_util_pkg.NO_PRIORS,
                                     p_grand_total  => 'N',
				     p_to_date_type => 'XTD'
				     );
   --}
    END IF;
      l_query := GET_ONTIME_PROD_SEL_CLAUSE (p_view_by_dim => l_view_by,
                                             p_join_tbl    => l_join_tbl)
                || ' from
...
-- Passing OPI_AGGREGATION_LEVEL_FLAGS to PMV
    ...
    END GET_ONTIME_PROD_SQL;

 /*
----------------------------------------------------------------------------------------------
   Function Name: GET_ONTIME_PROD_SEL_CLAUSE
   Parameters    : p_view_by_dim(IN parameter), p_join_tbl (IN parameter)
   Purpose       : This function helps in constructing the report query of the On-Time Production
...
BEGIN
	...
	l_sel_clause := l_sel_clause ||
                opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_late_prod_val',
                           p_default_val => 0) || ' OPI_MEASURE3,
           '  || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_ontime_prod_cnt',
	...
	--}
	 ...
	 l_sel_clause := l_sel_clause ||
	         opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'p_ontime_prod_val',
                           p_default_val => 0) || ' OPI_MEASURE6,
           '  || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_ontime_prod_val',
                           p_default_val => 0) || ' OPI_MEASURE7,
           '  || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => '(c_late_prod_cnt+c_ontime_prod_cnt)',
                           p_default_val => 0) || ' OPI_MEASURE8,'|| fnd_global.newline;

         IF (p_view_by_dim = 'ITEM+ENI_ITEM_ORG') THEN
	 --{
	   l_sel_clause := l_sel_clause ||
	          opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => '(c_late_prod_qty+c_ontime_prod_qty)',
                           p_default_val => 0) || ' OPI_MEASURE9,'|| fnd_global.newline;
	--}
         ELSE
	 --{
	    l_sel_clause := l_sel_clause || 'NULL OPI_MEASURE9, '|| fnd_global.newline;
	 --}
	 END IF;

	 l_sel_clause := l_sel_clause ||
                 opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => '(c_late_prod_val+c_ontime_prod_val)',
                           p_default_val => 0) || ' OPI_MEASURE10,
	   '  || opi_dbi_rpt_util_pkg.percent_str(
		                             p_numerator      => 'p_ontime_prod_cnt',
			                     p_denominator    => '(p_late_prod_cnt+p_ontime_prod_cnt)',
			                     p_measure_name   => 'OPI_MEASURE24') || ',
           '  || opi_dbi_rpt_util_pkg.percent_str(
		                             p_numerator      => 'c_ontime_prod_cnt',
			                     p_denominator    => '(c_late_prod_cnt+c_ontime_prod_cnt)',
			                     p_measure_name   => 'OPI_MEASURE11') || ',
	   ' || opi_dbi_rpt_util_pkg.percent_str(
		                             p_numerator      => 'p_ontime_prod_val',
			                     p_denominator    => '(p_late_prod_val+p_ontime_prod_val)',
			                     p_measure_name   => 'OPI_MEASURE12') || ',
	   ' || opi_dbi_rpt_util_pkg.percent_str(
		                             p_numerator      => 'c_ontime_prod_val',
			                     p_denominator    => '(c_late_prod_val+c_ontime_prod_val)',
	   ...
	   ' || opi_dbi_rpt_util_pkg.rate_str (
		 ...
		 opi_dbi_rpt_util_pkg.rate_str (
		                             p_numerator       => 'p_ontime_prod_val',
			                     p_denominator     => '(p_late_prod_val+p_ontime_prod_val)',
			                     p_rate_type       => 'NP') || 'OPI_MEASURE14,
	   ' || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_late_prod_cnt_total',
                           p_default_val => 0) || ' OPI_MEASURE15,
           ' || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_late_prod_val_total',
                           p_default_val => 0) || ' OPI_MEASURE16,
           ' || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_ontime_prod_cnt_total',
                           p_default_val => 0) || ' OPI_MEASURE17,
           ' || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => 'c_ontime_prod_val_total',
                           p_default_val => 0) || ' OPI_MEASURE18,
           ' || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => '(c_late_prod_cnt_total+c_ontime_prod_cnt_total)',
                           p_default_val => 0) || ' OPI_MEASURE19,
           ' || opi_dbi_rpt_util_pkg.nvl_str (
                           p_str         => '(c_late_prod_val_total+c_ontime_prod_val_total)',
                           p_default_val => 0) || ' OPI_MEASURE20,
           ' || opi_dbi_rpt_util_pkg.percent_str(
		                             p_numerator      => 'c_ontime_prod_cnt_total',
			                     p_denominator    => '(c_late_prod_cnt_total+c_ontime_prod_cnt_total)',
			                     p_measure_name   => 'OPI_MEASURE21') || ',
	   ' || opi_dbi_rpt_util_pkg.percent_str(
		                             p_numerator      => 'c_ontime_prod_val_total',
			                     p_denominator    => '(c_late_prod_val_total+c_ontime_prod_val_total)',
	   ...
	   ' || opi_dbi_rpt_util_pkg.rate_str (
			                     ...
			                     p_rate_type       => 'NP') || '-' ||
		opi_dbi_rpt_util_pkg.rate_str (
		                             p_numerator       => 'p_ontime_prod_val_total',
			                     p_denominator     => '(p_late_prod_val_total+p_ontime_prod_val_total)',
			                     p_rate_type       => 'NP') || 'OPI_MEASURE23,
	   ' || opi_dbi_rpt_util_pkg.percent_str(
		           p_numerator      => 'c_ontime_prod_val',
			   p_denominator    => '(c_late_prod_val+c_ontime_prod_val)',
...
END GET_ONTIME_PROD_SEL_CLAUSE;
