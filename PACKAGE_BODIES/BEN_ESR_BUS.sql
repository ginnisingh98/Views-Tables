--------------------------------------------------------
--  DDL for Package Body BEN_ESR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ESR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_esr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ee_stat_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   ee_stat_rt_id PK of record being inserted or updated.
...
--
Procedure chk_ee_stat_rt_id(p_ee_stat_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ee_stat_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_esr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ee_stat_rt_id                => p_ee_stat_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ee_stat_rt_id,hr_api.g_number)
     <>  ben_esr_shd.g_old_rec.ee_stat_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_esr_shd.constraint_error('BEN_EE_STAT_RTS_F');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ee_stat_rt_id is not null then
      --
...
--
-- In Parameters
--   p_ee_stat_rt_id         PK of record being inserted or updated.
--   assignment_status_type_id Value of lookup code.
...
and ee_stat_rt_id <> nvl(p_ee_stat_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_assignment_status_type_id <> nvl(ben_esr_shd.g_old_rec.assignment_status_type_id,hr_api.g_number)
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Assignment Status already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Assignment Status criteria');
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and ee_stat_rt_id <> nvl(p_ee_stat_rt_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_ee_stat_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_esr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    End If;
    --
    /*If ((nvl(p_assignment_status_type_id, hr_api.g_number) <> hr_api.g_number) and
      NOT (dt_api.check_min_max_dates
            (p_base_table_name => 'per_assignment_status_types',
             p_base_key_column => 'assignment_status_type_id',
             p_base_key_value  => p_assignment_status_type_id,
             p_from_date       => p_validation_start_date,
             p_to_date         => p_validation_end_date)))  Then
      l_table_name := 'per_assignment_status_types';
      Raise l_integrity_error;
    End If;*/
    --
  End If;
...
function return_legislation_code
  (p_ee_stat_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_ee_stat_rt_f b
    where b.ee_stat_rt_id      = p_ee_stat_rt_id
