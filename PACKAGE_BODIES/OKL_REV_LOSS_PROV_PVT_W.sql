--------------------------------------------------------
--  DDL for Package Body OKL_REV_LOSS_PROV_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_REV_LOSS_PROV_PVT_W" as
  /* $Header: OKLERPVB.pls 115.3 2003/01/06 19:32:11 sgiyer noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).cntrct_num := a0(indx);
          t(ddindx).reversal_type := a1(indx);
          t(ddindx).reversal_date := rosetta_g_miss_date_in_map(a2(indx));
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  if t is null or t.count = 0 then
    ...
    a2 := JTF_DATE_TABLE();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).cntrct_num;
          a1(indx) := t(ddindx).reversal_type;
          a2(indx) := t(ddindx).reversal_date;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_lprv_rec okl_rev_loss_prov_pvt.lprv_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_rev_loss_prov_pvt.reverse_loss_provisions(p_api_version,
      p_init_msg_list,
      x_msg_count,
      x_msg_data,
      x_return_status,
  ...
  as
    ddp_lprv_tbl okl_rev_loss_prov_pvt.lprv_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
