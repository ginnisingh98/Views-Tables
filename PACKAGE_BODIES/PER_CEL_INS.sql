--------------------------------------------------------
--  DDL for Package Body PER_CEL_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CEL_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_cel_ins.';  -- Global package name
--
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_competence_element_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  per_cel_ins.g_competence_element_id_i := p_competence_element_id;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure insert_dml(p_rec in out nocopy per_cel_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Insert the row into: per_competence_elements
  -- ngundura added object_id and object_name in the insert statement
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_insert(p_rec  in out nocopy per_cel_shd.g_rec_type) is
--
  l_proc  varchar2(72) := g_package||'pre_insert';
  l_exists varchar2(1);
--
  Cursor C_Sel1 is select per_competence_elements_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
         select null
                from per_competence_elements
                where competence_element_id = per_cel_ins.g_competence_element_id_i;
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  if per_cel_ins.g_competence_element_id_i is not null then
    --
    ...
    If C_Sel2%found then
      ...
      fnd_message.set_token('TABLE_NAME','per_competence_elements');
    ...
    --
    p_rec.competence_element_id :=
      per_cel_ins.g_competence_element_id_i;
    per_cel_ins.g_competence_element_id_i := null;
    --
  else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.competence_element_id;
    Close C_Sel1;
    --
  end if;
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_insert(p_rec in per_cel_shd.g_rec_type) is
--
...
Begin
  ...
  -- ngundura Added object id and name as parameters to per_cel_rki.after_insert
  begin
     per_cel_rki.after_insert	(
...
Begin
  ...
  If p_validate then
    ...
    -- Issue the savepoint.
    --
    SAVEPOINT ins_per_cel;
  End If;
  --
  -- Call the supporting insert validate operations
  --
  per_cel_bus.insert_validate(p_rec,p_effective_date);
  --
...
Exception
  When HR_Api.Validate_Enabled Then
    ...
    --
    ROLLBACK TO ins_per_cel;
end ins;
--
-- ----------------------------------------------------------------------------
-- |---------------------------------< ins >----------------------------------|
-- ----------------------------------------------------------------------------
-- ngundura added object_id and name as parameters to the ins procedure.
Procedure ins
  (
...
Begin
  ...
  -- p_rec structure.
  -- ngundura added object_id and name as parameters to convert_args
  hr_utility.set_location(p_type,99);
  l_rec :=
 ...
 -- BUG3356369
  ...
  --
  p_competence_element_id := l_rec.competence_element_id;
