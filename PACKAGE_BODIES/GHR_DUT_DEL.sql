--------------------------------------------------------
--  DDL for Package Body GHR_DUT_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GHR_DUT_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ghr_dut_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_delete_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE_NEXT_CHANGE') then
    ...
    --
        hr_utility.set_location('dt_delete_dml NAR IF ',10);
    delete from ghr_duty_stations_f
  ...
  Else
    ...
    --
    hr_utility.set_location('dt_delete_dml NAR ELSE ',15);
    delete from ghr_duty_stations_f
    where        duty_station_id = p_rec.duty_station_id
    and	  effective_start_date >= p_validation_start_date;
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'ZAP') then
    --
    p_rec.effective_start_date := ghr_dut_shd.g_old_rec.effective_start_date;
    --
