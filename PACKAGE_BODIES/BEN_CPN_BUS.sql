--------------------------------------------------------
--  DDL for Package Body BEN_CPN_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CPN_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_cpn_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_cntng_prtn_prfl_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   cntng_prtn_prfl_rt_id PK of record being inserted or updated.
...
--
Procedure chk_cntng_prtn_prfl_rt_id(p_cntng_prtn_prfl_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_cntng_prtn_prfl_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_cpn_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_cntng_prtn_prfl_rt_id                => p_cntng_prtn_prfl_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_cntng_prtn_prfl_rt_id,hr_api.g_number)
     <>  ben_cpn_shd.g_old_rec.cntng_prtn_prfl_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_cpn_shd.constraint_error('BEN_CNTNG_PRTN_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_cntng_prtn_prfl_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_cntng_frmr_prtt_dsge_rl(p_cntng_prtn_prfl_rt_id     in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> ben_cpn_shd.g_old_rec.cntng_frmr_prtt_dsge_rl
...
--   Internal table handler use only.
--
Procedure chk_cntng_frmr_prtt_rl(p_cntng_prtn_prfl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> ben_cpn_shd.g_old_rec.cntng_frmr_prtt_rl
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_dsge_must_be_redsgd_flag(p_cntng_prtn_prfl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_cpn_shd.g_old_rec.dsge_must_be_redsgd_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_pymt_must_be_rcvd_rl(p_cntng_prtn_prfl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_cpn_shd.g_old_rec.pymt_must_be_rcvd_uom,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_pymt_must_be_rcvd_dep(p_cntng_prtn_prfl_rt_id     in number,
                                           p_pymt_must_be_rcvd_rl        in number,
...
Begin
  ...
  if (l_api_updating
      and nvl(p_pymt_must_be_rcvd_rl,hr_api.g_number)
      <> nvl(ben_cpn_shd.g_old_rec.pymt_must_be_rcvd_rl, hr_api.g_number) or
         nvl(p_pymt_must_be_rcvd_num,hr_api.g_number)
      <> nvl(ben_cpn_shd.g_old_rec.pymt_must_be_rcvd_num, hr_api.g_number) or
...
*/
  ...
  (p_cntng_prtn_prfl_rt_id          => p_rec.cntng_prtn_prfl_rt_id,
   ...
   p_object_version_number => p_rec.object_version_number);
  --
  chk_pymt_must_be_rcvd_dep(p_cntng_prtn_prfl_rt_id  => p_rec.cntng_prtn_prfl_rt_id,
                                   p_pymt_must_be_rcvd_rl     => p_rec.pymt_must_be_rcvd_rl,
...
function return_legislation_code
  (p_cntng_prtn_prfl_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_cntng_prtn_prfl_rt_f b
    where b.cntng_prtn_prfl_rt_id      = p_cntng_prtn_prfl_rt_id
