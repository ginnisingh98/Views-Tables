--------------------------------------------------------
--  DDL for Package Body GMS_REPORT_SF272
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMS_REPORT_SF272" AS
--$Header: gmsgrfcb.pls 120.1 2006/02/14 01:50:15 lveerubh noship $
...
-- Bug 4005755 : splitted the cursor defination to remove outer join.
--               Functionality remains same.
-- Bug fixes 1356234,2701130,2831665 fixes are ported into these splitted new sql's
--
...
L_Report_Start_Date gms_272_history.report_start_date%type;
L_Report_End_Date gms_272_history.report_end_date%type;
...
X_Sum_Raw_Cost         NUMBER(22,5) := 0;
X_Sum_Burden_Cost         NUMBER(22,5) := 0;
X_Document_Number VARCHAR2(30);
X_Status_Code VARCHAR2(1) := NULL;
X_Cash_On_Hand_Beginning NUMBER(22,5) := 0;
X_Total_Actuals NUMBER(22,5) := 0;
X_Version NUMBER;
...
/* To set project_id NULL to be able to use Suresh's view to derive Burden Components and Burden Cost */
     ...
     -- The above line has been commented out NOCOPY for the bug 2442827
...
-- Bug 4005755 : Modified the logic for performance fix

 open c_sum_amount ;
 fetch c_sum_amount into X_Sum_Raw_Cost ;
 close c_sum_amount ;

 open c_sum_burden ;
 fetch c_sum_burden into X_Sum_Burden_Cost ;
 close c_sum_burden ;
...
--Added for Bug 2357578
 OPEN  report_period_date_cur;
  ...
  X_Total_Actuals  :=  NVL(X_Sum_Raw_Cost,0) + NVL(X_Sum_Burden_Cost,0) ;

-- End of fix 4005755

Begin
	/* To get the cash_in_hand at the beginning of the reporting period */
