--------------------------------------------------------
--  DDL for Package Body BEN_ELV_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ELV_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_elv_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_lvg_rsn_prte_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_lvg_rsn_prte_id PK of record being inserted or updated.
...
--
Procedure chk_elig_lvg_rsn_prte_id(p_elig_lvg_rsn_prte_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_lvg_rsn_prte_id';
...
Begin
  ...
  --
  l_api_updating := ben_elv_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_lvg_rsn_prte_id                => p_elig_lvg_rsn_prte_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_lvg_rsn_prte_id,hr_api.g_number)
     <>  ben_elv_shd.g_old_rec.elig_lvg_rsn_prte_id) then
    --
    -- raise error as PK has changed
    --
    ben_elv_shd.constraint_error('BEN_ELIG_LVG_RSN_PRTE_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_lvg_rsn_prte_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_lvg_rsn_cd(p_elig_lvg_rsn_prte_id        in number,
  ...
  cursor c3 is
     ...
     where elv.lvg_rsn_cd = p_lvg_rsn_cd
     and elv.eligy_prfl_id = p_eligy_prfl_id
     and elv.elig_lvg_rsn_prte_id <> nvl(p_elig_lvg_rsn_prte_id,hr_api.g_number)
     and elv.business_group_id + 0 = p_business_group_id
     and p_validation_start_date <= elv.effective_end_date
     and p_validation_end_date >= elv.effective_start_date
     ;
...
Begin
  ...
  if (l_api_updating
      and p_lvg_rsn_cd
      <> nvl(ben_elv_shd.g_old_rec.lvg_rsn_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_lvg_rsn_prte_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_elv_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   p_elig_lvg_rsn_prte_id   elig_lvg_rsn_prte_id
...
is
  ...
  cursor c1 is
    ...
    where  elig_lvg_rsn_prte_id <> nvl(p_elig_lvg_rsn_prte_id,-1)
...
function return_legislation_code
  (p_elig_lvg_rsn_prte_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_lvg_rsn_prte_f b
    where b.elig_lvg_rsn_prte_id      = p_elig_lvg_rsn_prte_id
