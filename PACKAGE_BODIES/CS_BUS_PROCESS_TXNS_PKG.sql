--------------------------------------------------------
--  DDL for Package Body CS_BUS_PROCESS_TXNS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CS_BUS_PROCESS_TXNS_PKG" AS
 /* $Header: cssebptb.pls 115.1 99/07/16 09:00:40 porting s $ */
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from CS_BUS_PROCESS_TXNS
      where BUSINESS_PROCESS_ID = X_BUSINESS_PROCESS_ID
      and TRANSACTION_TYPE_ID = X_TRANSACTION_TYPE_ID;
...
begin
  ...
  insert into CS_BUS_PROCESS_TXNS (
    BUSINESS_PROCESS_ID,
    TRANSACTION_TYPE_ID,
    START_DATE_ACTIVE,
    END_DATE_ACTIVE,
...
procedure LOCK_ROW (
  ...
  cursor c1 is select
    ...
    where BUSINESS_PROCESS_ID = X_BUSINESS_PROCESS_ID
    and TRANSACTION_TYPE_ID = X_TRANSACTION_TYPE_ID
    for update of BUSINESS_PROCESS_ID nowait;
  tlinfo c1%rowtype;
...
begin
  ...
  close c1;
      ...
      if ( ((tlinfo.START_DATE_ACTIVE = X_START_DATE_ACTIVE)
...
begin
  ...
  update CS_BUS_PROCESS_TXNS set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from CS_BUS_PROCESS_TXNS
     where BUSINESS_PROCESS_ID = X_BUSINESS_PROCESS_ID
     and TRANSACTION_TYPE_ID = X_TRANSACTION_TYPE_ID
  ;
  dummy c1%rowtype;
...
begin
  delete from CS_BUS_PROCESS_TXNS
  where BUSINESS_PROCESS_ID = X_BUSINESS_PROCESS_ID
  and TRANSACTION_TYPE_ID = X_TRANSACTION_TYPE_ID;
  if (sql%notfound) then
    raise no_data_found;
