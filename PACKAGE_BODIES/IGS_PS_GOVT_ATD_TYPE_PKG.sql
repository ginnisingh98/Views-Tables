--------------------------------------------------------
--  DDL for Package Body IGS_PS_GOVT_ATD_TYPE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_GOVT_ATD_TYPE_PKG" as
/* $Header: IGSPI57B.pls 115.3 2002/11/29 02:31:59 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_PS_GOVT_ATD_TYPE%RowType;
  new_references IGS_PS_GOVT_ATD_TYPE%RowType;
  ...
  BEGIN
	-- If being closed, validate attendance types.
	IF p_updating AND
	   old_references.closed_ind <> new_references.closed_ind THEN
		IF IGS_PS_VAL_GAT.crsp_val_gat_upd (
...
END check_constraints;
  ...
  BEGIN
    ...
    IGS_EN_ATD_TYPE_PKG.GET_FK_IGS_PS_GOVT_ATD_TYPE (
      old_references.govt_attendance_type
      );
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_GOVT_ATD_TYPE
      where GOVT_ATTENDANCE_TYPE = X_GOVT_ATTENDANCE_TYPE;
...
begin
  ...
  insert into IGS_PS_GOVT_ATD_TYPE (
...
begin
  ...
  update IGS_PS_GOVT_ATD_TYPE set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_GOVT_ATD_TYPE
     where GOVT_ATTENDANCE_TYPE = X_GOVT_ATTENDANCE_TYPE
  ;
begin
  open c1;
...
begin
  ...
  delete from IGS_PS_GOVT_ATD_TYPE
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
