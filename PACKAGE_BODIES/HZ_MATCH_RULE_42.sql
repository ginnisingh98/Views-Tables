--------------------------------------------------------
--  DDL for Package Body HZ_MATCH_RULE_42
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_MATCH_RULE_42" AS
/*=======================================================================+
...
END;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      RETURN total;
    END IF;
    IF HZ_DQM_SEARCH_UTIL.is_match(g_party_stage_rec.TX8, p_table_TX8,12) THEN
      total := total+50;
      IF g_score_until_thresh AND (total)>=g_thres_score THEN 
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'get_party_score returned total =  '||total,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
  ...
  BEGIN
    ...
    FOR J IN 1..g_party_site_stage_list.COUNT LOOP
      l_current_score := 0;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX3, p_table_TX3,(50000*(J-1)+66)) THEN
      ...
      ELSIF -- l_current_score<48 AND 
          HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX4, p_table_TX4,(50000*(J-1)+67)) THEN
      ...
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX11, p_table_TX11,(50000*(J-1)+74)) THEN
        ...
        END IF;
      END IF;
      IF HZ_DQM_SEARCH_UTIL.is_match(g_party_site_stage_list(J).TX12, p_table_TX12,(50000*(J-1)+75)) THEN
...
END;
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX8, p_table_TX8,(50000*(p_idx-1)+12)) THEN
      l_score :=50;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX3, p_table_TX3,(50000*(p_idx-1)+66)) THEN
      l_score :=60;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
      ELSIF l_current_score<48 AND 
          HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX4, p_table_TX4,(50000*(p_idx-1)+67)) THEN
      l_score :=48;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      END IF;
     l_score :=0;
      IF HZ_DQM_SEARCH_UTIL.is_match(p_stage_rec.TX12, p_table_TX12,(50000*(p_idx-1)+75)) THEN
      l_score :=10;
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'l_score is -  '||l_score,p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
...
POP_CONTACT_POINTS_COND_REC(p_contact_point_list);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      threshold := round((528/880)*(l_entered_max_score+l_ps_entered_max_score+l_ct_entered_max_score+l_cpt_entered_max_score));
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
  ...
  END IF;
    ...
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
          g_party_stage_rec.TX42,'A17',l_prim_temp);
  ...
  END IF;
    ...
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT SPECIAL + WRNAMES
          g_party_stage_rec.TX32,'A11',l_prim_temp);

  IF l_denorm_str IS NOT NULL THEN
    IF (p_non_denorm_max_score+50)>=p_threshold THEN
      l_denorm_str := NULL;
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, p_match_str, l_party_contains_str);
    ELSIF (p_non_denorm_max_score+p_denorm_max_score+50)>=p_threshold THEN
      HZ_DQM_SEARCH_UTIL.add_attribute_with_denorm(l_prim_temp, p_match_str, l_denorm_str, l_party_contains_str);
      l_denorm_str := NULL;
    END IF;
  ELSE
  ...
  END IF;
    ...
    -- Setup query string for PARTY_NAME
    l_prim_temp := null;
    HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT SPECIAL + WRNAMES
          g_party_stage_rec.TX8,'A8',l_prim_temp);
  ...
  BEGIN
    ...
    FOR I IN 1..g_party_site_stage_list.COUNT LOOP
      ...
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT SPECIAL + WRADDRESS
            g_party_site_stage_list(I).TX3,'A27',l_prim_temp);
      ...
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
            g_party_site_stage_list(I).TX9,'A29',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
            g_party_site_stage_list(I).TX9,'D_PS',l_prim_temp_den);
      ...
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
            g_party_site_stage_list(I).TX12,'A31',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
            g_party_site_stage_list(I).TX12,'D_PS',l_prim_temp_den);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp_den, ' AND ', l_den_contains_str_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
      ...
      l_prim_temp := null;
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT + WRSTATE
            g_party_site_stage_list(I).TX14,'A32',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT + WRSTATE
            g_party_site_stage_list(I).TX14,'D_PS',l_prim_temp_den);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp_den, ' AND ', l_den_contains_str_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
      ...
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
            g_party_site_stage_list(I).TX20,'A35',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT
            g_party_site_stage_list(I).TX20,'D_PS',l_prim_temp_den);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp_den, ' AND ', l_den_contains_str_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
  ...
  BEGIN
    ...
    FOR I IN 1..g_contact_stage_list.COUNT LOOP
      ...
      l_prim_temp := null;
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- EXACT SPECIAL + WRPERSON
            g_contact_stage_list(I).TX5,'A39',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
  ...
  BEGIN
    ...
    FOR I IN 1..g_contact_pt_stage_list.COUNT LOOP
      ...
      l_prim_temp := null;
      l_prim_temp_den := null;
      HZ_DQM_SEARCH_UTIL.add_transformation( -- FORCE UPPERCASE ONLY
            g_contact_pt_stage_list(I).TX5,'A48',l_prim_temp);
      HZ_DQM_SEARCH_UTIL.add_attribute(l_prim_temp, ' AND ', l_contains_str_temp);
...
END IF;
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     OPEN x_cursor FOR 
      SELECT PARTY_ID , TX8, TX32, TX35, TX42, TX44, TX45, TX46, TX47, TX48, TX156, TX157
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      l_sqlstr := 'SELECT PARTY_ID , TX8, TX32, TX35, TX42, TX44, TX45, TX46, TX47, TX48, TX156, TX157 FROM hz_staged_parties stage ';
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX9||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX11||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX12||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX14||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
      l_sqlstr := l_sqlstr || '     (D_PS like ''% ''||:TX20||'' %'' ';
     l_sqlstr := l_sqlstr || ' )) ';
      l_sqlstr := l_sqlstr || ' AND (:attr IS NULL OR ';
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
     ...
     ,g_party_stage_rec.TX32
     ,g_party_stage_rec.TX32
     ,g_party_stage_rec.TX35
     ,g_party_stage_rec.TX35
     ,g_party_stage_rec.TX42
     ,g_party_stage_rec.TX42
     ...
     ,g_party_stage_rec.TX36
     ,g_party_site_stage_list(1).TX9
     ,g_party_site_stage_list(1).TX9
     ,g_party_site_stage_list(1).TX11
     ,g_party_site_stage_list(1).TX11
     ,g_party_site_stage_list(1).TX12
     ,g_party_site_stage_list(1).TX12
     ,g_party_site_stage_list(1).TX14
     ,g_party_site_stage_list(1).TX14
     ,g_party_site_stage_list(1).TX20
     ,g_party_site_stage_list(1).TX20
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGE_PARTIES_T1) */ PARTY_ID , TX8, TX32, TX35, TX42, TX44, TX45, TX46, TX47, TX48, TX156, TX157
          FROM HZ_STAGED_PARTIES stage
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_PARTIES_U1) */ stage.PARTY_ID , TX8, TX32, TX35, TX42, TX44, TX45, TX46, TX47, TX48, TX156, TX157
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
     IF p_search_ctx_id IS NULL THEN
       l_sqlstr := 'SELECT   ' || l_hint || ' PARTY_ID , TX8, TX32, TX35, TX42, TX44, TX45, TX46, TX47, TX48, TX156, TX157'||
                   ' FROM HZ_STAGED_PARTIES stage'||
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_sites = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID, TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, stage.PARTY_ID, stage.ORG_CONTACT_ID, TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID, TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22
          FROM HZ_STAGED_PARTY_SITES stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT  PARTY_SITE_ID, stage.PARTY_ID, ORG_CONTACT_ID, TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22
          ...
          OR (l_search_merged = 'N' AND nvl(stage.status_flag, 'A') = 'A')       )  
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ stage.PARTY_SITE_ID, r.subject_id, stage.ORG_CONTACT_ID, TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       END IF; 
       l_sqlstr := 'SELECT  /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N1) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
         ...
         ' AND (ORG_CONTACT_ID IS NULL ' ||
       	...
       	' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
         ' UNION ' ||
       			 'SELECT /*+ INDEX(stage HZ_STAGED_PARTY_SITES_N2) */ PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
       ...
       ELSE 
       l_sqlstr := 'SELECT ' || l_hint ||' PARTY_SITE_ID, PARTY_ID, ORG_CONTACT_ID , TX3, TX4, TX9, TX11, TX12, TX14, TX20, TX22'||
                   ' FROM HZ_STAGED_PARTY_SITES stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                   '  AND( ('''||l_search_merged||''' =''Y'' )  '|| 
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '|| 
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACTS_N1) */ ORG_CONTACT_ID, PARTY_ID, TX5, TX2
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT ORG_CONTACT_ID, PARTY_ID, TX5, TX2
          FROM HZ_STAGED_CONTACTS stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
          SELECT 1 FROM HZ_STAGED_PARTIES p
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
          OPEN x_cursor FOR 
            SELECT /*+ ORDERED INDEX(stage HZ_STAGED_CONTACTS_N1) */ ORG_CONTACT_ID, stage.PARTY_ID, TX5, TX2
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       ELSE 
               l_status_sql := ' AND nvl(p.status,''A'') in (''A'')'; 
       END IF; 
       l_sqlstr := 'SELECT   ' || l_hint || ' ORG_CONTACT_ID, PARTY_ID , TX5, TX2'||
                   ' FROM HZ_STAGED_CONTACTS stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                 ' AND EXISTS ('||
                 ' SELECT 1 FROM HZ_STAGED_PARTIES p ' || 
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX5
  ...
  THEN
          OPEN x_cursor FOR 
          SELECT /*+ USE_NL(d stage) ORDERED INDEX(stage HZ_STAGED_CONTACT_POINTS_N3) */ CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX5
          FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage 
          WHERE contains( concat_col, p_contains_str)>0 
          AND d.search_context_id = p_search_ctx_id 
          AND( (l_search_merged ='Y' )  
 ...
 THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       IF p_search_rel_cpts = 'N' THEN
         OPEN x_cursor FOR 
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX5
       ...
       ELSE
         OPEN x_cursor FOR 
          ...
          AND stage.party_id = p_party_id 
          UNION
          SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ stage.CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX1, TX5
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        OPEN x_cursor FOR 
          SELECT CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX5
          FROM HZ_STAGED_CONTACT_POINTS stage
        WHERE contains( concat_col, p_contains_str)>0
        AND EXISTS (
          SELECT 1 FROM HZ_STAGED_PARTIES p
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF p_person_api = 'Y' THEN
          OPEN x_cursor FOR 
            SELECT CONTACT_POINT_ID, stage.contact_point_type, stage.PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID , TX1, TX5
            FROM HZ_DQM_PARTIES_GT d, HZ_STAGED_CONTACT_POINTS stage
            WHERE contains( concat_col, p_contains_str)>0
            AND d.search_context_id = p_search_ctx_id
            AND d.party_id = stage.party_id
            ...
            AND (p_dup_party_id IS NULL OR stage.party_id <> p_dup_party_id)
            UNION 
            SELECT /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, r.subject_id, stage.PARTY_SITE_ID, stage.ORG_CONTACT_ID , TX1, TX5
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
       ...
       ELSE
		p_restrict_sql1 := 'stage1.'||p_restrict_sql;
       END IF; 
       	l_sqlstr := ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N1) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX5'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ...
                   ' AND (:p_dup IS NULL OR stage.party_id <> :p_dup) ' ||
                   ' UNION ' ||
                   ' SELECT   /*+ INDEX(stage HZ_STAGED_CONTACT_POINTS_N2) */ CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX5'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0  '||
                   ' AND( ('''||l_search_merged||''' =''Y'' )  '||
                   '   OR ('''||l_search_merged||''' = ''I'' AND nvl(stage.status_flag, ''A'') in (''A'', ''I''))  '||
       ...
       OPEN x_cursor FOR l_sqlstr USING p_contains_str,
                    p_dup_party_id, p_dup_party_id, p_contains_str, p_dup_party_id, p_dup_party_id;
       ELSE 
       	l_sqlstr := 'SELECT   ' || l_hint ||' CONTACT_POINT_ID, stage.contact_point_type, PARTY_ID, PARTY_SITE_ID, ORG_CONTACT_ID  , TX1, TX5'||
                   ' FROM HZ_STAGED_CONTACT_POINTS stage'||
                   ' WHERE contains( concat_col, :cont)>0'||
                 ' AND EXISTS ('||
                 ' SELECT 1 FROM HZ_STAGED_PARTIES p ' || 
...
PROCEDURE find_parties (
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_cur INTO
         l_party_id , l_TX8, l_TX32, l_TX35, l_TX42, l_TX44, l_TX45, l_TX46, l_TX47, l_TX48, l_TX156, l_TX157;
  ...
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
      l_score := GET_PARTY_SCORE(l_TX8,l_TX32,l_TX35_new,l_TX42,l_TX44,l_TX45,l_TX46,l_TX47,l_TX48,l_TX156,l_TX157);
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_party_site_cur INTO
         l_party_site_id, l_ps_party_id, l_ps_contact_id , l_TX3, l_TX4, l_TX9, l_TX11, l_TX12, l_TX14, l_TX20, l_TX22;
      ...
      IF l_use_contact_addr_info OR l_ps_contact_id IS NULL THEN
        ...
        l_score := GET_PARTY_SITES_SCORE(l_match_idx,l_TX3,l_TX4,l_TX9,l_TX11,l_TX12,l_TX14,l_TX20,l_TX22);
        IF NOT H_SCORES.EXISTS(l_index) THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_contact_cur INTO
         l_org_contact_id, l_ct_party_id , l_TX5, l_TX2;
      EXIT WHEN l_contact_cur%NOTFOUND;
      l_index := map_id(l_ct_party_id);
      l_score := GET_CONTACTS_SCORE(l_match_idx,l_TX5,l_TX2);
      IF NOT H_SCORES.EXISTS(l_index) THEN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    LOOP 
      FETCH l_contact_pt_cur INTO
         l_contact_pt_id,  l_cpt_type, l_cpt_party_id,  l_cpt_ps_id, l_cpt_contact_id , l_TX1, l_TX5;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      ...
      IF l_use_contact_cpt_info OR l_cpt_contact_id IS NULL THEN
        ...
        l_score := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX1,l_TX5);
        IF NOT H_SCORES.EXISTS(l_index) THEN
  ...
  BEGIN
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    IF (p_index=5 AND call_order(5) <> 'NONE' AND H_PARTY_ID.COUNT=0) THEN
     l_threshold :=  round(( l_entered_max_score / 880) * 528); 
  ...
  BEGIN
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
    l_party_contains_str := INIT_PARTY_QUERY(l_match_str, l_denorm_str, l_party_max_score, l_denorm_max_score, l_non_denorm_max_score, round((528/880)*l_entered_max_score));
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END LOOP;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'In find_parties. This is a Search Rule. Evaluating Matches. Threshold : 60 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
      IF (H_SCORES(l_party_id).TOTAL_SCORE/l_entered_max_score)>=(528/880) THEN
            INSERT INTO HZ_MATCHED_PARTIES_GT (SEARCH_CONTEXT_ID, PARTY_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    ...
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_parties;
...
PROCEDURE find_persons (
  ...
  BEGIN
    OPEN x_cursor FOR 
      SELECT /*+ INDEX(stage HZ_STAGED_CONTACTS_U1) */ ORG_CONTACT_ID, PARTY_ID, TX5, TX2
      FROM HZ_STAGED_CONTACTS stage
      WHERE contains( concat_col, p_contains_str)>0
      AND ORG_CONTACT_ID in (
  ...
  BEGIN
    ...
    END IF;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
      hz_utility_v2pub.debug(p_message=>'Evaluating Matches. Threshold : 60 ',p_module_prefix=>'dqm',p_module=>'hz_match_rule_xxx',p_prefix=>NULL,p_msg_level=>fnd_log.level_statement);
    END IF;
    x_num_matches := 0;
    l_num_eval := 0;
    IF l_match_str = ' OR ' THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_persons');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
END find_persons;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_party_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_duplicate_parties');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_duplicate_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_duplicate_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.find_duplicate_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.call_api_dynamic');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_party_sites (
  ...
  BEGIN
    ...
    IF l_match_str = ' AND ' OR (
        ((l_cpt_max_score/l_entered_max_score)<(528/880))
        ) THEN
    ...
    END IF;
 IF l_match_str = ' OR ' THEN
IF (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=( 528/880 ) THEN
 ...
 ELSE
    IF H_SCORES(l_party_site_id).PARTY_SITE_SCORE>0 AND
       ...
       (H_SCORES(l_party_site_id).TOTAL_SCORE/l_entered_max_score)>=(528/880) THEN
      INSERT INTO HZ_MATCHED_PARTY_SITES_GT (SEARCH_CONTEXT_ID, PARTY_ID, PARTY_SITE_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.get_matching_party_sites');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contacts (
  ...
  BEGIN
    ...
    END IF;
        IF (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(528/880) THEN
      ...
      ELSE
           ...
           (H_SCORES(l_org_contact_id).TOTAL_SCORE/l_entered_max_score)>=(528/880) THEN
          INSERT INTO HZ_MATCHED_CONTACTS_GT (SEARCH_CONTEXT_ID, PARTY_ID, ORG_CONTACT_ID, SCORE) 
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.get_matching_contacts');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_matching_contact_points (
  ...
  BEGIN
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      LOOP
        ...
        IF (p_dup_contact_point_id IS NULL OR (
            ...
            H_SCORE(cnt) := GET_CONTACT_POINTS_SCORE(l_match_idx,l_TX1,l_TX5);
            cnt := cnt+1;
    IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    END IF;
        IF (H_SCORE(I)/l_entered_max_score) >= (528/880) THEN
        INSERT INTO HZ_MATCHED_CPTS_GT(SEARCH_CONTEXT_ID,CONTACT_POINT_ID,PARTY_ID,SCORE) VALUES (
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.get_matching_contact_points');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
PROCEDURE get_score_details (
  ...
  l_TX35_new:=RTRIM(LTRIM(l_TX35));
  l_TX35_new:=(CASE l_TX35_new WHEN 'SYNC' THEN HZ_STAGE_MAP_TRANSFORM.den_acc_number (l_party_id) ELSE l_TX35_new END);
          INSERT_PARTY_SCORE(p_party_id, p_party_id, l_search_ctx_id, p_party_search_rec, g_party_stage_rec, l_TX8,l_TX32,l_TX35_new,l_TX42,l_TX44,l_TX45,l_TX46,l_TX47,l_TX48,l_TX156,l_TX157,1);
    END LOOP;
    CLOSE l_party_cur;
    ...
    IF l_party_site_contains_str IS NOT NULL THEN
      ...
      IF l_max_score>0 THEN
        INSERT_PARTY_SITES_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_party_site_list(l_max_idx), g_party_site_stage_list(l_max_idx) ,l_max_TX3,l_max_TX4,l_max_TX9,l_max_TX11,l_max_TX12,l_max_TX14,l_max_TX20,l_max_TX22,l_max_idx);
      END IF;
    END IF;
    ...
    IF l_contact_contains_str IS NOT NULL THEN
      ...
      CLOSE l_contact_cur;
      IF l_max_score>0 THEN
        INSERT_CONTACTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_list(l_max_idx), g_contact_stage_list(l_max_idx) ,l_max_TX5,l_max_TX2,l_max_idx);
      END IF;
    END IF;
    ...
    IF l_contact_pt_contains_str IS NOT NULL THEN
      ...
      END LOOP;
      IF l_max_score>0 THEN
        INSERT_CONTACT_POINTS_SCORE(p_party_id,l_max_id,l_search_ctx_id, p_contact_point_list(l_max_idx), g_contact_pt_stage_list(l_max_idx) ,l_max_TX1,l_max_TX5,l_max_idx);
      END IF;
      CLOSE l_contact_pt_cur;
    END IF;
    IF fnd_log.level_procedure>=fnd_log.g_current_runtime_level THEN
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
    FND_MESSAGE.SET_TOKEN('PROC','HZ_MATCH_RULE_42.get_score_details');
    FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
    FND_MSG_PUB.ADD;
    RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
   ...
   IF fnd_log.level_statement>=fnd_log.g_current_runtime_level THEN
    ...
    IF l_party_type = 'ORGANIZATION' THEN
      SELECT 
       ...
       ,translate(HZ_PARTIES.PARTY_NAME, '%',' ')
...
BEGIN
   ...
   IF p_search_ctx THEN
    ...
    IF p_search_rec.PARTY_NAME IS NOT NULL THEN 
      x_entered_max_score := x_entered_max_score+50;
    END IF;
