--------------------------------------------------------
--  DDL for Package Body OKL_PAM_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_PAM_PVT_W" as
  /* $Header: OKLIPAMB.pls 120.0 2005/07/07 10:40:11 viselvar noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).adj_mat_id := a0(indx);
          t(ddindx).object_version_number := a1(indx);
          t(ddindx).org_id := a2(indx);
          t(ddindx).currency_code := a3(indx);
          t(ddindx).adj_mat_type_code := a4(indx);
          t(ddindx).orig_adj_mat_id := a5(indx);
          t(ddindx).sts_code := a6(indx);
          t(ddindx).effective_from_date := a7(indx);
          t(ddindx).effective_to_date := a8(indx);
          ...
          t(ddindx).created_by := a25(indx);
          t(ddindx).creation_date := a26(indx);
          t(ddindx).last_updated_by := a27(indx);
          t(ddindx).last_update_date := a28(indx);
          t(ddindx).last_update_login := a29(indx);
          t(ddindx).adj_mat_name := a30(indx);
          t(ddindx).adj_mat_desc := a31(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  if t is null or t.count = 0 then
    ...
    a31 := JTF_VARCHAR2_TABLE_2000();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).adj_mat_id;
          a1(indx) := t(ddindx).object_version_number;
          a2(indx) := t(ddindx).org_id;
          a3(indx) := t(ddindx).currency_code;
          a4(indx) := t(ddindx).adj_mat_type_code;
          a5(indx) := t(ddindx).orig_adj_mat_id;
          ...
          a25(indx) := t(ddindx).created_by;
          a26(indx) := t(ddindx).creation_date;
          a27(indx) := t(ddindx).last_updated_by;
          a28(indx) := t(ddindx).last_update_date;
          a29(indx) := t(ddindx).last_update_login;
          a30(indx) := t(ddindx).adj_mat_name;
          a31(indx) := t(ddindx).adj_mat_desc;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          t(ddindx).adj_mat_id := a0(indx);
          t(ddindx).adj_mat_name := a1(indx);
          t(ddindx).object_version_number := a2(indx);
          t(ddindx).org_id := a3(indx);
          t(ddindx).currency_code := a4(indx);
          t(ddindx).adj_mat_type_code := a5(indx);
          t(ddindx).orig_adj_mat_id := a6(indx);
          t(ddindx).sts_code := a7(indx);
          t(ddindx).effective_from_date := a8(indx);
          t(ddindx).effective_to_date := a9(indx);
  ...
  if t is null or t.count = 0 then
    ...
    a30 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          a0(indx) := t(ddindx).adj_mat_id;
          a1(indx) := t(ddindx).adj_mat_name;
          a2(indx) := t(ddindx).object_version_number;
          a3(indx) := t(ddindx).org_id;
          a4(indx) := t(ddindx).currency_code;
          a5(indx) := t(ddindx).adj_mat_type_code;
          a6(indx) := t(ddindx).orig_adj_mat_id;
          a7(indx) := t(ddindx).sts_code;
          a8(indx) := t(ddindx).effective_from_date;
          a9(indx) := t(ddindx).effective_to_date;
  ...
  as
    ddp_pamv_rec okl_pam_pvt.okl_pamv_rec;
    ddx_pamv_rec okl_pam_pvt.okl_pamv_rec;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_pam_pvt.insert_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_pamv_tbl okl_pam_pvt.okl_pamv_tbl;
    ddx_pamv_tbl okl_pam_pvt.okl_pamv_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_pam_pvt_w.rosetta_table_copy_in_p1(ddp_pamv_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
    ...
    okl_pam_pvt_w.rosetta_table_copy_out_p1(ddx_pamv_tbl, p6_a0
      , p6_a1
      , p6_a2
      , p6_a3
      , p6_a4
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_pam_pvt.update_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_pam_pvt.delete_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
