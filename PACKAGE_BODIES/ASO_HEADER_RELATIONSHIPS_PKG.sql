--------------------------------------------------------
--  DDL for Package Body ASO_HEADER_RELATIONSHIPS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ASO_HEADER_RELATIONSHIPS_PKG" as
/* $Header: asothedb.pls 120.1 2005/06/29 12:38:57 appldev ship $ */
-- Start of Comments
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'ASO_HEADER_RELATIONSHIPS_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'asothedb.pls';
...
PROCEDURE Insert_Row(
 ...
 IS
   CURSOR C2 IS SELECT ASO_HEADER_RELATIONSHIPS_S.nextval FROM sys.dual;
BEGIN
   If (px_HEADER_RELATIONSHIP_ID IS NULL) OR (px_HEADER_RELATIONSHIP_ID = FND_API.G_MISS_NUM) then
       OPEN C2;
       FETCH C2 INTO px_HEADER_RELATIONSHIP_ID;
       CLOSE C2;
   End If;
   INSERT INTO ASO_HEADER_RELATIONSHIPS(
           ...
           decode( p_RELATED_HEADER_ID, FND_API.G_MISS_NUM, NULL, p_RELATED_HEADER_ID),
...
PROCEDURE Update_Row(
 ...
 IS
 BEGIN
    Update ASO_HEADER_RELATIONSHIPS
    SET
              ...
              RELATED_HEADER_ID = decode( p_RELATED_HEADER_ID, FND_API.G_MISS_NUM, RELATED_HEADER_ID, p_RELATED_HEADER_ID),
...
PROCEDURE Delete_Row(
 ...
 IS
 BEGIN
   DELETE FROM ASO_HEADER_RELATIONSHIPS
    WHERE HEADER_RELATIONSHIP_ID = p_HEADER_RELATIONSHIP_ID;
   If (SQL%NOTFOUND) then
       RAISE NO_DATA_FOUND;
   End If;
...
RECIPROCAL_FLAG
        ...
        WHERE HEADER_RELATIONSHIP_ID =  p_HEADER_RELATIONSHIP_ID
        FOR UPDATE of HEADER_RELATIONSHIP_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN C;
    ...
    if (
/*
           (      Recinfo.HEADER_RELATIONSHIP_ID = p_HEADER_RELATIONSHIP_ID)
...
/*
       ...
       AND (    ( Recinfo.RELATED_HEADER_ID = p_RELATED_HEADER_ID)
            OR (    ( Recinfo.RELATED_HEADER_ID IS NULL )
                AND (  p_RELATED_HEADER_ID IS NULL )))
