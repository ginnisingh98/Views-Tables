--------------------------------------------------------
--  DDL for Package Body FARX_C_AJ
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FARX_C_AJ" as
/* $Header: farxcajb.pls 120.2.12010000.2 2009/07/19 10:49:34 glchen ship $ */
...
--  and fcp.concurrent_program_name = 'RXFACAJ';
...
--  and fcr.phase_code = 'R'
--  and fcr.concurrent_program_id = fcp.concurrent_program_id
--  and fcp.concurrent_program_name = 'RXFACCR';

h_request_id := fnd_global.conc_request_id;
fnd_profile.get('USER_ID',h_user_id);

  farx_aj.cost_clear_rec (
        book => argument1,
        period => argument2,
        request_id => h_request_id,
	user_id => h_user_id,
