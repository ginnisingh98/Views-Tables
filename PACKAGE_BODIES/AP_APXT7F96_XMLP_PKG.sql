--------------------------------------------------------
--  DDL for Package Body AP_APXT7F96_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_APXT7F96_XMLP_PKG" AS
/* $Header: APXT7F96B.pls 120.0 2007/12/27 08:35:42 vjaganat noship $ */
    ...
    BEGIN
      ...
      IF (P_DEBUG_SWITCH in ('y','Y')) THEN
        /*SRW.MESSAGE('5'
                   ,'After Mail_Sel')*/NULL;
      END IF;
      IF (SEL_COMPANY_NAME <> TRUE) THEN
        RAISE INIT_FAILURE;
      END IF;
      IF (P_DEBUG_SWITCH in ('y','Y')) THEN
        /*SRW.MESSAGE('6'
                   ,'After Sel_Company_Name')*/NULL;
      END IF;
      RETURN (TRUE);
    EXCEPTION
      WHEN OTHERS THEN
  ...
  FUNCTION MAIL_SEL RETURN BOOLEAN IS
  BEGIN
    SELECT
      NUMBER_OF_DOCUMENTS,
      TOTAL_REPORTED,
      TOTAL_WITHHELD
    INTO C_NUMBER_OF_DOCS,C_TOTAL_REPORTED,C_TOTAL_WITHHELD
    FROM
      AP_1096_DATA
  ...
  FUNCTION SEL_COMPANY_NAME RETURN BOOLEAN IS
  BEGIN
    SELECT
      ...
      NVL(HR.REGION_3
         ,HR.REGION_2),
      REPLACE(REPLACE(HR.POSTAL_CODE
                     ,' '
                     ,'')
             ,'-'
             ,''),
      RE.TAX_IDENTIFICATION_NUM
    INTO C_COMPANY_NAME,C_US_LINE2,C_US_LINE3,C_US_CITY,C_US_STATE,C_US_ZIP,C_TIN
    FROM
      HR_LOCATIONS HR,
      AP_REPORTING_ENTITIES RE
    WHERE RE.TAX_ENTITY_ID = P_TAX_ENTITY_ID
      AND RE.LOCATION_ID = HR.LOCATION_ID;
    RETURN (TRUE);
    RETURN NULL;
  EXCEPTION
    WHEN OTHERS THEN
      RETURN (FALSE);
  END SEL_COMPANY_NAME;
  ...
  FUNCTION C_NUMBER_OF_DOCS_P RETURN NUMBER IS
  BEGIN
    RETURN C_NUMBER_OF_DOCS;
  END C_NUMBER_OF_DOCS_P;

  FUNCTION C_TOTAL_REPORTED_P RETURN NUMBER IS
  BEGIN
    RETURN C_TOTAL_REPORTED;
  END C_TOTAL_REPORTED_P;

  FUNCTION C_TOTAL_WITHHELD_P RETURN NUMBER IS
  BEGIN
    RETURN C_TOTAL_WITHHELD;
  END C_TOTAL_WITHHELD_P;
  ...
  FUNCTION C_US_LINE2_P RETURN VARCHAR2 IS
  BEGIN
    RETURN C_US_LINE2;
  END C_US_LINE2_P;

  FUNCTION C_US_LINE3_P RETURN VARCHAR2 IS
  BEGIN
    RETURN C_US_LINE3;
  END C_US_LINE3_P;

  FUNCTION C_US_CITY_P RETURN VARCHAR2 IS
  BEGIN
    RETURN C_US_CITY;
  END C_US_CITY_P;

  FUNCTION C_US_STATE_P RETURN VARCHAR2 IS
  BEGIN
    RETURN C_US_STATE;
  END C_US_STATE_P;

  FUNCTION C_US_ZIP_P RETURN VARCHAR2 IS
  BEGIN
    RETURN C_US_ZIP;
  END C_US_ZIP_P;
