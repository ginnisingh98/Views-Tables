--------------------------------------------------------
--  DDL for Package Body OKC_CHG_REQ_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_CHG_REQ_ASMBLR_PVT" AS
/* $Header: OKCRCRAB.pls 120.0 2005/05/25 22:56:25 appldev noship $ */
...
PROCEDURE acn_assemble(
 ...
 SELECT aae.element_name
  ...
  WHERE acn.id = aae.acn_id
    AND acn.correlation = 'CHREQ_INIT' ;
...
BEGIN
 ...
 l_corrid_rec.corrid := 'CHREQ_INIT' ;
...
l_msg_tbl := okc_aq_pvt.msg_tab_typ();
 ...
 ELSIF corr_rec.element_name = 'CHREQ_ID' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
  l_msg_tbl(i).element_value   := P_chreq_id;
 ELSIF corr_rec.element_name = 'CHREQ_DATE' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
  --l_msg_tbl(i).element_value   := P_chreq_date;
  IF corr_rec.format_mask IS NOT NULL THEN
       l_msg_tbl(i).element_value := to_char(p_chreq_date,
		      corr_rec.format_mask);
  ELSE
       l_msg_tbl(i).element_value := to_char(p_chreq_date,'DD-MON-YY');
  END IF;
 END IF;
