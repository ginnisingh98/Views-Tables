--------------------------------------------------------
--  DDL for Package Body AME_APG_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_APG_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_apg_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Procedure dt_update_dml
  (p_rec                   in out nocopy ame_apg_shd.g_rec_type
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the ame_approval_groups Row
    --
    ...
    ,query_string                         = p_rec.query_string
    ,is_static                            = p_rec.is_static
...
Begin
  ...
  begin
    ...
    ame_apg_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_name_o
      => ame_apg_shd.g_old_rec.name
      ,p_start_date_o
      => ame_apg_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_apg_shd.g_old_rec.end_date
      ,p_description_o
      => ame_apg_shd.g_old_rec.description
      ,p_query_string_o
      => ame_apg_shd.g_old_rec.query_string
      ,p_is_static_o
      => ame_apg_shd.g_old_rec.is_static
      ,p_security_group_id_o
      => ame_apg_shd.g_old_rec.security_group_id
      ,p_object_version_number_o
      => ame_apg_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'AME_APPROVAL_GROUPS'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
Begin
  ...
  If (p_rec.description = hr_api.g_varchar2) then
    ...
    ame_apg_shd.g_old_rec.description;
  End If;
  If (p_rec.query_string = hr_api.g_varchar2) then
    p_rec.query_string :=
    ame_apg_shd.g_old_rec.query_string;
  End If;
  If (p_rec.is_static = hr_api.g_varchar2) then
    p_rec.is_static :=
    ame_apg_shd.g_old_rec.is_static;
  End If;
...
Begin
  ...
  --
  ame_apg_upd.convert_defs(p_rec);
  --
  ame_apg_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
