--------------------------------------------------------
--  DDL for Package Body AME_RULE_USAGES_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_RULE_USAGES_API" AS
/* $Header: ameruapi.pkb 120.2 2005/10/14 04:13:49 ubhat noship $ */
...
is
  X_APPROVER_CATEGORY varchar2(1) := ame_util.approvalApproverCategory;
  X_ITEM_ID NUMBER;
  X_CREATED_BY NUMBER;
...
exception
    when others then
    ame_util.runtimeException('ame_rules_usages_api',
                         'load_row',
                         sqlcode,
                         sqlerrm);
        raise;
