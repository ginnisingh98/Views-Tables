--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNITREQREF_CD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNITREQREF_CD_PKG" AS
/* $Header: IGSPI2GB.pls 115.4 2003/05/09 06:38:06 sarakshi ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_unitreqref_cd%ROWTYPE;
  new_references igs_ps_unitreqref_cd%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.unit_req_ref_cd_id                := x_unit_req_ref_cd_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_unitreqref_cd
      WHERE    unit_req_ref_cd_id = x_unit_req_ref_cd_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_RC_URRC_FK1');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_RCT_URRC_FK2');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.unit_req_ref_cd_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_unitreqref_cd_s.NEXTVAL
    INTO      x_unit_req_ref_cd_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_unit_req_ref_cd_id                => x_unit_req_ref_cd_id,
      x_unit_cd                           => x_unit_cd,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_unitreqref_cd
    WHERE rowid = x_rowid;
