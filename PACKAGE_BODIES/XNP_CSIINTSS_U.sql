--------------------------------------------------------
--  DDL for Package Body XNP_CSIINTSS_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIINTSS_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIINTSS' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIINTSS' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIINTSS' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIINTSS.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIINTSS', l_consumer_list );
  END IF;
...
BEGIN
  ...
  csi_t_gen_utility_pvt.add('CSIINTSS'||'-'||to_char(sysdate,'DD-MON-YYYY HH24:MI:SS'));
  ...
  IF l_mtl_trx_rec.mtl_transaction_id is null THEN
    ...
    l_trx_error_rec.source_type          := 'CSIINTSS';
