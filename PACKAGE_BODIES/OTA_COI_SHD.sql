--------------------------------------------------------
--  DDL for Package Body OTA_COI_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_COI_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_coi_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'OTA_CHAT_OBJ_INCLUSIONS_PK') Then
...
Begin
  ...
  Else
    If (p_chat_id
        = ota_coi_shd.g_old_rec.chat_id and
        p_object_id
        = ota_coi_shd.g_old_rec.object_id and
        p_object_type
        = ota_coi_shd.g_old_rec.object_type and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ota_coi_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ota_coi_shd.g_old_rec.object_version_number) Then
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'ota_chat_obj_inclusions');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
