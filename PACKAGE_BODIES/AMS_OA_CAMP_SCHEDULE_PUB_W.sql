--------------------------------------------------------
--  DDL for Package Body AMS_OA_CAMP_SCHEDULE_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMS_OA_CAMP_SCHEDULE_PUB_W" as
  /* $Header: amsaschb.pls 120.4 2006/05/31 11:36:52 srivikri noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).schedule_id := a0(indx);
          t(ddindx).last_update_date := a1(indx);
          t(ddindx).last_updated_by := a2(indx);
          t(ddindx).creation_date := a3(indx);
          t(ddindx).created_by := a4(indx);
          t(ddindx).last_update_login := a5(indx);
          t(ddindx).object_version_number := a6(indx);
          t(ddindx).campaign_id := a7(indx);
          t(ddindx).user_status_id := a8(indx);
          t(ddindx).status_code := a9(indx);
          t(ddindx).status_date := a10(indx);
          t(ddindx).source_code := a11(indx);
          t(ddindx).use_parent_code_flag := a12(indx);
          t(ddindx).start_date_time := a13(indx);
          t(ddindx).end_date_time := a14(indx);
          t(ddindx).timezone_id := a15(indx);
          t(ddindx).activity_type_code := a16(indx);
          t(ddindx).activity_id := a17(indx);
          t(ddindx).arc_marketing_medium_from := a18(indx);
          t(ddindx).marketing_medium_id := a19(indx);
          t(ddindx).custom_setup_id := a20(indx);
          t(ddindx).triggerable_flag := a21(indx);
          t(ddindx).trigger_id := a22(indx);
          t(ddindx).notify_user_id := a23(indx);
          t(ddindx).approver_user_id := a24(indx);
          t(ddindx).owner_user_id := a25(indx);
          t(ddindx).active_flag := a26(indx);
          t(ddindx).cover_letter_id := a27(indx);
          ...
          t(ddindx).org_id := a33(indx);
          t(ddindx).objective_code := a34(indx);
          t(ddindx).country_id := a35(indx);
          ...
          t(ddindx).budget_amount_tc := a43(indx);
          t(ddindx).budget_amount_fc := a44(indx);
          t(ddindx).language_code := a45(indx);
          t(ddindx).task_id := a46(indx);
          t(ddindx).related_event_from := a47(indx);
          t(ddindx).related_event_id := a48(indx);
          ...
          t(ddindx).related_source_id := a85(indx);
          t(ddindx).query_id := a86(indx);
          ...
          t(ddindx).orig_csch_id := a94(indx);
          t(ddindx).cover_letter_version := a95(indx);
          t(ddindx).usage := a96(indx);
          t(ddindx).purpose := a97(indx);
          t(ddindx).last_activation_date := a98(indx);
          t(ddindx).sales_methodology_id := a99(indx);
          t(ddindx).printer_address := a100(indx);
          t(ddindx).notify_on_activation_flag := a101(indx);
          t(ddindx).sender_display_name := a102(indx);
          t(ddindx).asn_group_id := a103(indx);
          t(ddindx).delivery_mode := a104(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
    ...
    a104 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a7(indx) := t(ddindx).campaign_id;
          a8(indx) := t(ddindx).user_status_id;
          ...
          a17(indx) := t(ddindx).activity_id;
          a18(indx) := t(ddindx).arc_marketing_medium_from;
          a19(indx) := t(ddindx).marketing_medium_id;
          a20(indx) := t(ddindx).custom_setup_id;
          a21(indx) := t(ddindx).triggerable_flag;
          a22(indx) := t(ddindx).trigger_id;
          a23(indx) := t(ddindx).notify_user_id;
          a24(indx) := t(ddindx).approver_user_id;
          a25(indx) := t(ddindx).owner_user_id;
          a26(indx) := t(ddindx).active_flag;
          a27(indx) := t(ddindx).cover_letter_id;
          ...
          a33(indx) := t(ddindx).org_id;
          a34(indx) := t(ddindx).objective_code;
          a35(indx) := t(ddindx).country_id;
          ...
          a43(indx) := t(ddindx).budget_amount_tc;
          a44(indx) := t(ddindx).budget_amount_fc;
          a45(indx) := t(ddindx).language_code;
          a46(indx) := t(ddindx).task_id;
          a47(indx) := t(ddindx).related_event_from;
          a48(indx) := t(ddindx).related_event_id;
          ...
          a85(indx) := t(ddindx).related_source_id;
          a86(indx) := t(ddindx).query_id;
          ...
          a94(indx) := t(ddindx).orig_csch_id;
          a95(indx) := t(ddindx).cover_letter_version;
          a96(indx) := t(ddindx).usage;
          a97(indx) := t(ddindx).purpose;
          a98(indx) := t(ddindx).last_activation_date;
          a99(indx) := t(ddindx).sales_methodology_id;
          a100(indx) := t(ddindx).printer_address;
          a101(indx) := t(ddindx).notify_on_activation_flag;
          a102(indx) := t(ddindx).sender_display_name;
          a103(indx) := t(ddindx).asn_group_id;
          a104(indx) := t(ddindx).delivery_mode;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  begin
    ...
    ddp_schedule_rec.schedule_id := p7_a0;
    ddp_schedule_rec.last_update_date := p7_a1;
    ddp_schedule_rec.last_updated_by := p7_a2;
    ddp_schedule_rec.creation_date := p7_a3;
    ddp_schedule_rec.created_by := p7_a4;
    ddp_schedule_rec.last_update_login := p7_a5;
    ddp_schedule_rec.object_version_number := p7_a6;
    ddp_schedule_rec.campaign_id := p7_a7;
    ddp_schedule_rec.user_status_id := p7_a8;
    ddp_schedule_rec.status_code := p7_a9;
    ddp_schedule_rec.status_date := p7_a10;
    ddp_schedule_rec.source_code := p7_a11;
    ddp_schedule_rec.use_parent_code_flag := p7_a12;
    ddp_schedule_rec.start_date_time := p7_a13;
    ddp_schedule_rec.end_date_time := p7_a14;
    ddp_schedule_rec.timezone_id := p7_a15;
    ddp_schedule_rec.activity_type_code := p7_a16;
    ddp_schedule_rec.activity_id := p7_a17;
    ddp_schedule_rec.arc_marketing_medium_from := p7_a18;
    ddp_schedule_rec.marketing_medium_id := p7_a19;
    ddp_schedule_rec.custom_setup_id := p7_a20;
    ddp_schedule_rec.triggerable_flag := p7_a21;
    ddp_schedule_rec.trigger_id := p7_a22;
    ddp_schedule_rec.notify_user_id := p7_a23;
    ddp_schedule_rec.approver_user_id := p7_a24;
    ddp_schedule_rec.owner_user_id := p7_a25;
    ddp_schedule_rec.active_flag := p7_a26;
    ddp_schedule_rec.cover_letter_id := p7_a27;
    ...
    ddp_schedule_rec.org_id := p7_a33;
    ddp_schedule_rec.objective_code := p7_a34;
    ddp_schedule_rec.country_id := p7_a35;
    ...
    ddp_schedule_rec.budget_amount_tc := p7_a43;
    ddp_schedule_rec.budget_amount_fc := p7_a44;
    ddp_schedule_rec.language_code := p7_a45;
    ddp_schedule_rec.task_id := p7_a46;
    ddp_schedule_rec.related_event_from := p7_a47;
    ddp_schedule_rec.related_event_id := p7_a48;
    ...
    ddp_schedule_rec.related_source_id := p7_a85;
    ddp_schedule_rec.query_id := p7_a86;
    ...
    ddp_schedule_rec.orig_csch_id := p7_a94;
    ddp_schedule_rec.cover_letter_version := p7_a95;
    ddp_schedule_rec.usage := p7_a96;
    ddp_schedule_rec.purpose := p7_a97;
    ddp_schedule_rec.last_activation_date := p7_a98;
    ddp_schedule_rec.sales_methodology_id := p7_a99;
    ddp_schedule_rec.printer_address := p7_a100;
    ddp_schedule_rec.notify_on_activation_flag := p7_a101;
    ddp_schedule_rec.sender_display_name := p7_a102;
    ddp_schedule_rec.asn_group_id := p7_a103;
    ddp_schedule_rec.delivery_mode := p7_a104;
...
end ams_oa_camp_schedule_pub_w;
