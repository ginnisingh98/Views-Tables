--------------------------------------------------------
--  DDL for Package Body IGS_PR_OU_UNIT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PR_OU_UNIT_PKG" as
/* $Header: IGSQI07B.pls 115.7 2003/02/25 09:15:13 anilk ship $ */

l_rowid VARCHAR2(25);
  old_references IGS_PR_OU_UNIT_ALL%RowType;
  new_references IGS_PR_OU_UNIT_ALL%RowType;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PR_POPU_PRO_FK');
      IGS_GE_MSG_STACK.ADD;
	  Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PR_POPU_UN_FK');
      IGS_GE_MSG_STACK.ADD;
	  Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID IN NUMBER
    ...
    cursor C is select ROWID from IGS_PR_OU_UNIT_ALL
      where PROGRESSION_RULE_CAT = X_PROGRESSION_RULE_CAT
      and PRA_SEQUENCE_NUMBER = X_PRA_SEQUENCE_NUMBER
      and PRO_SEQUENCE_NUMBER = X_PRO_SEQUENCE_NUMBER
      and UNIT_CD = X_UNIT_CD;
...
Before_DML (
  ...
  insert into IGS_PR_OU_UNIT_ALL (
...
begin
  ...
  if ( (tlinfo.S_UNIT_TYPE = X_S_UNIT_TYPE)
  ) then
    null;
  else
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PR_OU_UNIT_ALL
     where PROGRESSION_RULE_CAT = X_PROGRESSION_RULE_CAT
     and PRA_SEQUENCE_NUMBER = X_PRA_SEQUENCE_NUMBER
     and PRO_SEQUENCE_NUMBER = X_PRO_SEQUENCE_NUMBER
     and UNIT_CD = X_UNIT_CD
...
Before_DML (
  ...
  delete from IGS_PR_OU_UNIT_ALL
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
...
-- This has been commented as 'S_UNIT_TYPE' has be checked already

/*IF upper(Column_name) = 'S_UNIT_TYPE' OR COLUMN_NAME IS NULL THEN
  IF new_references.S_UNIT_TYPE not in  ('REQUIRED' , 'EXCLUDED' ) then
    Fnd_Message.Set_Name('IGS','IGS_GE_INVALID_VALUE');
      IGS_GE_MSG_STACK.ADD;
    App_Exception.Raise_Exception ;
  END IF;
