--------------------------------------------------------
--  DDL for Package Body BEN_EHC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EHC_UPD" as
/* $Header: beehcrhi.pkb 120.0 2005/05/28 02:14:11 appldev noship $ */
--
-- ----------------------------------------------------------------------------
-- |                     Private Global Definitions                           |
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ehc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_ELIG_HLTH_CVG_PRTE_f',
	   ...
	   p_base_key_value	=> p_rec.ELIG_HLTH_CVG_PRTE_id);
    --
    ben_ehc_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_ELIG_HLTH_CVG_PRTE_f Row
    --
    ...
    --
    ben_ehc_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_ehc_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.ELIG_HLTH_CVG_PRTE_id,
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'ben_ELIG_HLTH_CVG_PRTE_f'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_ehc_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.excld_flag = hr_api.g_varchar2) then
    p_rec.excld_flag :=
    ben_ehc_shd.g_old_rec.excld_flag;
  End If;
  If (p_rec.ordr_num = hr_api.g_number) then
    p_rec.ordr_num :=
    ben_ehc_shd.g_old_rec.ordr_num;
  End If;
  If (p_rec.pl_typ_opt_typ_id = hr_api.g_number) then
    p_rec.pl_typ_opt_typ_id :=
    ben_ehc_shd.g_old_rec.pl_typ_opt_typ_id;
  End If;
  If (p_rec.oipl_id = hr_api.g_number) then
    p_rec.oipl_id :=
    ben_ehc_shd.g_old_rec.oipl_id;
  End If;
  If (p_rec.eligy_prfl_id = hr_api.g_number) then
    p_rec.eligy_prfl_id :=
    ben_ehc_shd.g_old_rec.eligy_prfl_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_ehc_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.ehc_attribute_category = hr_api.g_varchar2) then
    p_rec.ehc_attribute_category :=
    ben_ehc_shd.g_old_rec.ehc_attribute_category;
  End If;
  If (p_rec.ehc_attribute1 = hr_api.g_varchar2) then
    p_rec.ehc_attribute1 :=
    ben_ehc_shd.g_old_rec.ehc_attribute1;
  End If;
  If (p_rec.ehc_attribute2 = hr_api.g_varchar2) then
    p_rec.ehc_attribute2 :=
    ben_ehc_shd.g_old_rec.ehc_attribute2;
  End If;
  If (p_rec.ehc_attribute3 = hr_api.g_varchar2) then
    p_rec.ehc_attribute3 :=
    ben_ehc_shd.g_old_rec.ehc_attribute3;
  End If;
  If (p_rec.ehc_attribute4 = hr_api.g_varchar2) then
    p_rec.ehc_attribute4 :=
    ben_ehc_shd.g_old_rec.ehc_attribute4;
  End If;
  If (p_rec.ehc_attribute5 = hr_api.g_varchar2) then
    p_rec.ehc_attribute5 :=
    ben_ehc_shd.g_old_rec.ehc_attribute5;
  End If;
  If (p_rec.ehc_attribute6 = hr_api.g_varchar2) then
    p_rec.ehc_attribute6 :=
    ben_ehc_shd.g_old_rec.ehc_attribute6;
  End If;
  If (p_rec.ehc_attribute7 = hr_api.g_varchar2) then
    p_rec.ehc_attribute7 :=
    ben_ehc_shd.g_old_rec.ehc_attribute7;
  End If;
  If (p_rec.ehc_attribute8 = hr_api.g_varchar2) then
    p_rec.ehc_attribute8 :=
    ben_ehc_shd.g_old_rec.ehc_attribute8;
  End If;
  If (p_rec.ehc_attribute9 = hr_api.g_varchar2) then
    p_rec.ehc_attribute9 :=
    ben_ehc_shd.g_old_rec.ehc_attribute9;
  End If;
  If (p_rec.ehc_attribute10 = hr_api.g_varchar2) then
    p_rec.ehc_attribute10 :=
    ben_ehc_shd.g_old_rec.ehc_attribute10;
  End If;
  If (p_rec.ehc_attribute11 = hr_api.g_varchar2) then
    p_rec.ehc_attribute11 :=
    ben_ehc_shd.g_old_rec.ehc_attribute11;
  End If;
  If (p_rec.ehc_attribute12 = hr_api.g_varchar2) then
    p_rec.ehc_attribute12 :=
    ben_ehc_shd.g_old_rec.ehc_attribute12;
  End If;
  If (p_rec.ehc_attribute13 = hr_api.g_varchar2) then
    p_rec.ehc_attribute13 :=
    ben_ehc_shd.g_old_rec.ehc_attribute13;
  End If;
  If (p_rec.ehc_attribute14 = hr_api.g_varchar2) then
    p_rec.ehc_attribute14 :=
    ben_ehc_shd.g_old_rec.ehc_attribute14;
  End If;
  If (p_rec.ehc_attribute15 = hr_api.g_varchar2) then
    p_rec.ehc_attribute15 :=
    ben_ehc_shd.g_old_rec.ehc_attribute15;
  End If;
  If (p_rec.ehc_attribute16 = hr_api.g_varchar2) then
    p_rec.ehc_attribute16 :=
    ben_ehc_shd.g_old_rec.ehc_attribute16;
  End If;
  If (p_rec.ehc_attribute17 = hr_api.g_varchar2) then
    p_rec.ehc_attribute17 :=
    ben_ehc_shd.g_old_rec.ehc_attribute17;
  End If;
  If (p_rec.ehc_attribute18 = hr_api.g_varchar2) then
    p_rec.ehc_attribute18 :=
    ben_ehc_shd.g_old_rec.ehc_attribute18;
  End If;
  If (p_rec.ehc_attribute19 = hr_api.g_varchar2) then
    p_rec.ehc_attribute19 :=
    ben_ehc_shd.g_old_rec.ehc_attribute19;
  End If;
  If (p_rec.ehc_attribute20 = hr_api.g_varchar2) then
    p_rec.ehc_attribute20 :=
    ben_ehc_shd.g_old_rec.ehc_attribute20;
  End If;
  If (p_rec.ehc_attribute21 = hr_api.g_varchar2) then
    p_rec.ehc_attribute21 :=
    ben_ehc_shd.g_old_rec.ehc_attribute21;
  End If;
  If (p_rec.ehc_attribute22 = hr_api.g_varchar2) then
    p_rec.ehc_attribute22 :=
    ben_ehc_shd.g_old_rec.ehc_attribute22;
  End If;
  If (p_rec.ehc_attribute23 = hr_api.g_varchar2) then
    p_rec.ehc_attribute23 :=
    ben_ehc_shd.g_old_rec.ehc_attribute23;
  End If;
  If (p_rec.ehc_attribute24 = hr_api.g_varchar2) then
    p_rec.ehc_attribute24 :=
    ben_ehc_shd.g_old_rec.ehc_attribute24;
  End If;
  If (p_rec.ehc_attribute25 = hr_api.g_varchar2) then
    p_rec.ehc_attribute25 :=
    ben_ehc_shd.g_old_rec.ehc_attribute25;
  End If;
  If (p_rec.ehc_attribute26 = hr_api.g_varchar2) then
    p_rec.ehc_attribute26 :=
    ben_ehc_shd.g_old_rec.ehc_attribute26;
  End If;
  If (p_rec.ehc_attribute27 = hr_api.g_varchar2) then
    p_rec.ehc_attribute27 :=
    ben_ehc_shd.g_old_rec.ehc_attribute27;
  End If;
  If (p_rec.ehc_attribute28 = hr_api.g_varchar2) then
    p_rec.ehc_attribute28 :=
    ben_ehc_shd.g_old_rec.ehc_attribute28;
  End If;
  If (p_rec.ehc_attribute29 = hr_api.g_varchar2) then
    p_rec.ehc_attribute29 :=
    ben_ehc_shd.g_old_rec.ehc_attribute29;
  End If;
  If (p_rec.ehc_attribute30 = hr_api.g_varchar2) then
    p_rec.ehc_attribute30 :=
    ben_ehc_shd.g_old_rec.ehc_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_ehc_shd.convert_args
  (
  p_ELIG_HLTH_CVG_PRTE_id,
  null,
  null,
  p_excld_flag,
  p_ordr_num,
