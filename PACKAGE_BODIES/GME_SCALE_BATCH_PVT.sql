--------------------------------------------------------
--  DDL for Package Body GME_SCALE_BATCH_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_SCALE_BATCH_PVT" AS
/* $Header: GMEVSCBB.pls 120.10.12010000.3 2009/06/17 09:14:30 apmishra ship $ */
      ...
      FOR i IN 1 .. p_material_tbl.COUNT LOOP
         IF p_qty_type = 0 THEN
            IF p_material_tbl (i).formulaline_id IS NOT NULL THEN
               /* This loop is not needed really. We will always get only one record. */
               --FPBug#4351032 Begin
               --nsinghi Bug#5674398 Pass the ingredient substitution date rather than the batch plan start date
               -- for pDate parameter
               l_ingr_req_date := gme_api_grp.get_ingr_sub_date(p_material_tbl(i).batch_id,p_material_tbl(i).material_detail_id);

	       gmdfmval_pub.get_substitute_line_item(
	                         pFormulaline_id     =>  p_material_tbl(i).formulaline_id
                                 ,pItem_id            =>  p_material_tbl(i).inventory_item_id
                                 ,pQty                =>  p_material_tbl(i).plan_qty
                                 ,pUom                =>  p_material_tbl(i).dtl_um
                                 ,pScale_multiple     =>  p_material_tbl(i).scale_multiple
                                 ,pDate               =>  l_ingr_req_date
                                 ,xFormulaDetail_tbl  => l_formula_tbl);

               IF g_debug <= gme_debug.g_log_statement THEN
 	         gme_debug.put_line('Orig '||p_material_tbl(i).formulaline_id||' '||p_material_tbl(i).inventory_item_id||' '||
                                     p_material_tbl(i).plan_qty ||' '||p_material_tbl(i).dtl_um);

                 gme_debug.put_line('New '||p_material_tbl(i).formulaline_id||' '||l_formula_tbl(1).item_id||' '||
                                    l_formula_tbl(1).qty ||' '||l_formula_tbl(1).item_um);
               END IF;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
     ...
     IF    p_batch_header_rec.batch_status <> 0       THEN
      IF p_recalc_dates = 'T' THEN
         ...
         -- Navin: PENDING: Verify who is doing the cahnges to gme_api_reschedule_batch.reschedule_batch
