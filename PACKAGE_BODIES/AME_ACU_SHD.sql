--------------------------------------------------------
--  DDL for Package Body AME_ACU_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ACU_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_acu_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_ACTION_USAGES_PK') Then
...
Begin
  ...
  Else
    If (p_action_id =
        ame_acu_shd.g_old_rec.action_id and
      p_rule_id =
        ame_acu_shd.g_old_rec.rule_id and
        p_object_version_number =
        ame_acu_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_acu_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_acu_shd.g_old_rec.end_date;
...
--
-- We need to trap the 2 parent objects viz. ame_rules and ame_actions
--
  begin
    select null
      into l_dummy
