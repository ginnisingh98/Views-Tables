--------------------------------------------------------
--  DDL for Package Body XNP_ADAPTER_ADMIN_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_ADAPTER_ADMIN_U" AS
/*$Header: XNPADMNB.pls 120.2 2005/07/08 07:34:49 appldev ship $*/
...
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('ADAPTER_ADMIN',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'ADAPTER_ADMIN' ;
  ...
  XNP_XML_UTILS.write_element ( 'REQUEST_TYPE', XNP$REQUEST_TYPE );
  XNP_XML_UTILS.write_element ( 'REQUEST_ID', XNP$REQUEST_ID );
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'ADAPTER_ADMIN.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'ADAPTER_ADMIN', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_EVT_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$REQUEST_TYPE=>XNP$REQUEST_TYPE,
    XNP$REQUEST_ID=>XNP$REQUEST_ID,
    x_msg_header=>l_msg_header,
...
NULL ;
declare
l_RequestType varchar2(4000);
l_RequestID number;
l_RequestIDChar varchar2(4000);
begin
xnp_xml_utils.Decode(p_msg_text, 'REQUEST_TYPE', l_RequestType);
xnp_xml_utils.Decode(p_msg_text, 'REQUEST_ID', l_RequestIDChar);
l_RequestID := to_number(l_RequestIDChar);
-- Commented out - sacsharm - 11.5.6
-- This message never published from XDPCORFB and the called pkg is obsolete
--xdp_adapter_admin.administer_adapter( l_RequestType,
--l_RequestID,
--x_error_code,
--x_error_message);
if x_error_code = -20310 then
x_error_code := 0;
end if;
end;
END ;
