--------------------------------------------------------
--  DDL for Package Body PSB_ENTITY_ASSIGNMENT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSB_ENTITY_ASSIGNMENT_PVT" AS
 /* $Header: PSBVEAPB.pls 115.4 2002/11/18 10:57:05 msuram ship $ */

  G_PKG_NAME CONSTANT VARCHAR2(30):= 'PSB_ENTITY_ASSIGNMENT_PVT';
...
procedure INSERT_ROW(
  ...
  --
    cursor C is select ROWID from PSB_ENTITY_ASSIGNMENT
...
BEGIN
  ...
  insert into PSB_ENTITY_ASSIGNMENT (
    ENTITY_SET_ID,
    ENTITY_ID,
    PRIORITY,
    SEVERITY_LEVEL,
...
BEGIN
  ...
  close c1;
      ...
      if ( ((tlinfo.PRIORITY = P_PRIORITY)
	   OR ((tlinfo.PRIORITY is null)
	       AND (P_PRIORITY is null)))
      AND ((tlinfo.SEVERITY_LEVEL = P_SEVERITY_LEVEL)
	   OR ((tlinfo.SEVERITY_LEVEL is null)
...
BEGIN
  ...
  p_return_status := FND_API.G_RET_STS_SUCCESS ;
  --
  update PSB_ENTITY_ASSIGNMENT set
    PRIORITY = P_PRIORITY,
...
BEGIN
  ...
  p_return_status := FND_API.G_RET_STS_SUCCESS ;
  --
  delete from PSB_ENTITY_ASSIGNMENT
  where ENTITY_SET_ID = P_ENTITY_SET_ID
    and ENTITY_ID = P_ENTITY_ID;
