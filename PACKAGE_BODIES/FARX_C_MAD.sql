--------------------------------------------------------
--  DDL for Package Body FARX_C_MAD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FARX_C_MAD" as
-- $Header: farxcmdb.pls 120.2.12010000.2 2009/07/19 13:47:25 glchen ship $
...
PROCEDURE MASS_ADDITIONS (
	...
	argument2	in  varchar2,   -- queue_name
...
--	and	fcp.concurrent_program_name = 'RXFAMAD';

  h_request_id := fnd_global.conc_request_id;
  fnd_profile.get('USER_ID',h_user_id);

	farx_mad.mass_additions (
		book => argument1,
		queue_name => argument2,
