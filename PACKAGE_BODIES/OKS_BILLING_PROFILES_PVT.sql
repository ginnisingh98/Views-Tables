--------------------------------------------------------
--  DDL for Package Body OKS_BILLING_PROFILES_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKS_BILLING_PROFILES_PVT" AS
/* $Header: OKSCBPEB.pls 120.1 2005/10/05 03:25:33 mchoudha noship $ */
  ---------------------------------------------------------------------------
  ...
  END validate_id;
   ...
   ---------------------------------
  -- Validate_Attributes for: billing_type --
  ---------------------------------
 ...
 END validate_billing_type;
  ...
  FUNCTION Validate_Record (
    ...
    FUNCTION validate_foreign_keys (
      ...
      CURSOR okx_parties_v_pk_csr (p_id1 IN NUMBER,
                                   p_id2 IN VARCHAR2) IS
      SELECT 'x'
        FROM Okx_Parties_V
       WHERE okx_parties_v.id1    = p_id1
         AND okx_parties_v.id2    = p_id2;
      l_okx_parties_v_pk             okx_parties_v_pk_csr%ROWTYPE;

      CURSOR okx_site_uses_v_pk_csr (p_id1 IN NUMBER,
                                     p_id2 IN VARCHAR2) IS
      SELECT 'x'
        FROM Okx_Cust_Site_Uses_V
       WHERE okx_cust_site_uses_v.id1 = p_id1
         AND okx_cust_site_uses_v.id2 = p_id2;
      l_okx_site_uses_v_pk           okx_site_uses_v_pk_csr%ROWTYPE;

      CURSOR bpev_fndv_mda_fk_csr (p_lookup_code IN VARCHAR2) IS
      SELECT 'x'
        FROM Fnd_Common_Lookups
       WHERE fnd_common_lookups.lookup_code = p_lookup_code;
      l_bpev_fndv_mda_fk             bpev_fndv_mda_fk_csr%ROWTYPE;

      l_return_status                VARCHAR2(1) := OKC_API.G_RET_STS_SUCCESS;
      l_row_notfound                 BOOLEAN := TRUE;
    BEGIN
    /*
      IF ((p_bpev_rec.MDA_CODE IS NOT NULL)
       AND
          (p_bpev_rec.MDA_CODE <> p_db_bpev_rec.MDA_CODE))
      THEN
        OPEN bpev_fndv_mda_fk_csr (p_bpev_rec.MDA_CODE);
        FETCH bpev_fndv_mda_fk_csr INTO l_bpev_fndv_mda_fk;
        l_row_notfound := bpev_fndv_mda_fk_csr%NOTFOUND;
        CLOSE bpev_fndv_mda_fk_csr;
        IF (l_row_notfound) THEN
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'MDA_CODE');
      ...
      THEN
        ...
        END IF;
      END IF;
      IF (((p_bpev_rec.TCE_CODE_PRI_OFFSET IS NOT NULL) AND
           (p_bpev_rec.UOM_CODE_PRI_OFFSET IS NOT NULL))
       AND
          ((p_bpev_rec.TCE_CODE_PRI_OFFSET <> p_db_bpev_rec.TCE_CODE_PRI_OFFSET) OR
           (p_bpev_rec.UOM_CODE_PRI_OFFSET <> p_db_bpev_rec.UOM_CODE_PRI_OFFSET)))
      THEN
        OPEN okc_tcuv_pk_csr (p_bpev_rec.TCE_CODE_PRI_OFFSET,
                              p_bpev_rec.UOM_CODE_PRI_OFFSET);
        FETCH okc_tcuv_pk_csr INTO l_okc_tcuv_pk;
        l_row_notfound := okc_tcuv_pk_csr%NOTFOUND;
        CLOSE okc_tcuv_pk_csr;
        IF (l_row_notfound) THEN
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'TCE_CODE_PRI_OFFSET');
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'UOM_CODE_PRI_OFFSET');
          RAISE item_not_found_error;
        END IF;
      END IF;
      IF (((p_bpev_rec.TCE_CODE_SEC_OFFSET IS NOT NULL) AND
           (p_bpev_rec.UOM_CODE_SEC_OFFSET IS NOT NULL))
       AND
          ((p_bpev_rec.TCE_CODE_SEC_OFFSET <> p_db_bpev_rec.TCE_CODE_SEC_OFFSET) OR
           (p_bpev_rec.UOM_CODE_SEC_OFFSET <> p_db_bpev_rec.UOM_CODE_SEC_OFFSET)))
      THEN
        OPEN okc_tcuv_pk_csr (p_bpev_rec.TCE_CODE_SEC_OFFSET,
                              p_bpev_rec.UOM_CODE_SEC_OFFSET);
        FETCH okc_tcuv_pk_csr INTO l_okc_tcuv_pk;
        l_row_notfound := okc_tcuv_pk_csr%NOTFOUND;
        CLOSE okc_tcuv_pk_csr;
        IF (l_row_notfound) THEN
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'TCE_CODE_SEC_OFFSET');
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'UOM_CODE_SEC_OFFSET');
          RAISE item_not_found_error;
        END IF;
      END IF;
      IF (((p_bpev_rec.OWNED_PARTY_ID1 IS NOT NULL) AND
           (p_bpev_rec.OWNED_PARTY_ID2 IS NOT NULL))
       AND
          ((p_bpev_rec.OWNED_PARTY_ID1 <> p_db_bpev_rec.OWNED_PARTY_ID1) OR
           (p_bpev_rec.OWNED_PARTY_ID2 <> p_db_bpev_rec.OWNED_PARTY_ID2)))
      THEN
        OPEN okx_parties_v_pk_csr (p_bpev_rec.OWNED_PARTY_ID1,
                                   p_bpev_rec.OWNED_PARTY_ID2);
        FETCH okx_parties_v_pk_csr INTO l_okx_parties_v_pk;
        l_row_notfound := okx_parties_v_pk_csr%NOTFOUND;
        CLOSE okx_parties_v_pk_csr;
        IF (l_row_notfound) THEN
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'OWNED_PARTY_ID1');
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'OWNED_PARTY_ID2');
          RAISE item_not_found_error;
        END IF;
      END IF;
      ...
      THEN
        ...
        END IF;
      END IF;
      IF (((p_bpev_rec.BILL_TO_ADDRESS_ID1 IS NOT NULL) AND
           (p_bpev_rec.BILL_TO_ADDRESS_ID2 IS NOT NULL))
       AND
          ((p_bpev_rec.BILL_TO_ADDRESS_ID1 <> p_db_bpev_rec.BILL_TO_ADDRESS_ID1) OR
           (p_bpev_rec.BILL_TO_ADDRESS_ID2 <> p_db_bpev_rec.BILL_TO_ADDRESS_ID2)))
      THEN
        OPEN okx_site_uses_v_pk_csr (p_bpev_rec.BILL_TO_ADDRESS_ID1,
                                     p_bpev_rec.BILL_TO_ADDRESS_ID2);
        FETCH okx_site_uses_v_pk_csr INTO l_okx_site_uses_v_pk;
        l_row_notfound := okx_site_uses_v_pk_csr%NOTFOUND;
        CLOSE okx_site_uses_v_pk_csr;
        IF (l_row_notfound) THEN
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'BILL_TO_ADDRESS_ID1');
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'BILL_TO_ADDRESS_ID2');
          RAISE item_not_found_error;
        END IF;
      END IF;
      RETURN (l_return_status);
  ...
  BEGIN
    ...
    p_to.uom_code_frequency := p_from.uom_code_sec_offset;
    p_to.uom_code_frequency := p_from.uom_code_pri_offset;
    p_to.tce_code_frequency := p_from.tce_code_frequency;
    p_to.tce_code_frequency := p_from.tce_code_sec_offset;
    p_to.tce_code_frequency := p_from.tce_code_pri_offset;
