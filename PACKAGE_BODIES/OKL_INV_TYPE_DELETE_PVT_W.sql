--------------------------------------------------------
--  DDL for Package Body OKL_INV_TYPE_DELETE_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_INV_TYPE_DELETE_PVT_W" as
  /* $Header: OKLEITDB.pls 120.1 2005/07/11 14:17:54 asawanka noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).group_asset_yn := a4(indx);
          t(ddindx).group_by_contract_yn := a5(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
    ...
    a5 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a4(indx) := t(ddindx).group_asset_yn;
          a5(indx) := t(ddindx).group_by_contract_yn;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_ity_del_rec okl_inv_type_delete_pvt.ity_del_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_inv_type_delete_pvt.delete_type(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_ity_del_tbl okl_inv_type_delete_pvt.ity_del_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
