--------------------------------------------------------
--  DDL for Package Body HR_MX_PERSON_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_MX_PERSON_API" AS
/* $Header: pemxwrpa.pkb 120.0 2005/05/31 11:35 appldev noship $ */
--
-- Package Variables
--
...
begin
  ...
  l_proc  := g_package||'update_mx_person';
 ...
 if g_debug then
  ...
  --
  -- Check that the legislation of the specified business group is 'MX'.
  --
  if l_legislation_code <> 'MX' then
    hr_utility.set_message(801, 'HR_7961_PER_BUS_GRP_INVALID');
    hr_utility.set_message_token('LEG_CODE','MX');
    hr_utility.raise_error;
  end if;
 if g_debug then
 ...
 if g_debug then
  ...
  --
end update_mx_person;

begin
  g_package  := 'hr_mx_person_api.';
