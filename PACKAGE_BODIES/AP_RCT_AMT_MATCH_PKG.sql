--------------------------------------------------------
--  DDL for Package Body AP_RCT_AMT_MATCH_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_RCT_AMT_MATCH_PKG" AS
/*$Header: apamtrcb.pls 120.46.12010000.6 2010/02/08 08:52:36 baole ship $*/
G_PKG_NAME          CONSTANT VARCHAR2(30) := 'AP_RCT_AMT_MATCH_PKG';
...
G_MODULE_NAME           CONSTANT VARCHAR2(100) := 'AP.PLSQL.AP_RCT_AMT_MATCH_PKG.';
--
...
PROCEDURE Get_Total_Proration_Amount(
                  X_Rcv_Transaction_Id        IN      NUMBER,
		  X_Po_Line_Location_Id       IN      NUMBER, --bugfix:5641346
                  X_Match_Amount              IN      NUMBER,
...
BEGIN
   ...
   current_calling_sequence := 'AP_RCT_AMT_MATCH_PKG.ap_amt_match<-'||X_calling_sequence;
...
BEGIN
 ...
 IF(X_Match_Mode IN ('STD-PS','CR-PS')) THEN
   ...
   Get_Total_Proration_Amount
                          ( X_Rcv_Transaction_Id   => x_rcv_transaction_id,
			    X_Po_Line_Location_Id  => x_po_line_location_id, --bugfix:5641346
                            X_Match_Amount    => x_match_amount,
   ...
   LOOP
      ...
      l_debug_info := 'Get distribution level amounts from PO API';
      ...
      RCV_INVOICE_MATCHING_SV.get_DeliverAmount(
                        ...
                        p_receive_transaction_id => X_rcv_transaction_id,
                        p_po_distribution_id     => l_po_distribution_id,
                        x_delivered_amt => l_amount_delivered,
                        x_corrected_amt => l_amount_corrected);

      PO_AP_INVOICE_MATCH_GRP.get_po_dist_amounts(
                    p_api_version               =>1.0,
                    p_po_distribution_id        =>l_po_distribution_id,
                    x_dist_amt_ordered          =>l_amount_ordered,
                    x_dist_amt_cancelled        =>l_amount_cancelled,
	...
	--This is to bring the same behaviour as in PO matching.
	 if(l_amount_ordered <> l_amount_cancelled or  l_matching_case<>'O') then
            l_amount_ordered := nvl(l_amount_ordered,0)- nvl(l_amount_cancelled,0);
  	 end if;


      l_amount_delivered := nvl(l_amount_delivered,0) + nvl(l_amount_corrected,0);

      l_debug_info := 'Get Distribution level Amount billed';

      /* Bug 5355861. using x_dist_tab(l_po_distribution_id)
         instead x_dist_tab(i) */
      AP_MATCHING_UTILS_PKG.Get_Recpt_Dist_Amt_Billed (
                           x_rcv_transaction_id,
                           x_dist_tab(l_po_distribution_id).po_distribution_id,
                           l_amount_billed);
      ...
      IF l_matching_case = 'DB' THEN
        ...
        IF (x_match_mode = 'STD-PS') THEN
           l_amount_invoiced := (X_match_amount * (l_amount_delivered - l_amount_billed)/
                           l_total_amount);

	--bugfix:5639042
        ELSE  /*-ve invoices */
           l_amount_invoiced := (X_match_amount * l_amount_billed/
	                                   l_total_amount);
	END IF;

      ELSIF l_matching_case = 'OB' THEN
        IF(x_match_mode = 'STD-PS') THEN
          l_amount_invoiced := (X_match_amount * (l_amount_ordered - l_amount_billed)/
                                l_total_amount);
      ...
      ELSE
        l_amount_invoiced := (X_match_amount * l_amount_ordered/
                              l_total_amount);
      ...
      l_Debug_info := ' x_dist_tab(l_po_distribution_id).amount ' || x_dist_tab(l_po_distribution_id).amount;
      IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
                    FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,l_debug_info);
		           END IF;
   ...
   --bugfix:5641346 added l_rounding_index to the IF clause
...
*/
     ...
     --Populate dist_code_combination_id information
        ...
        --Contract Payments: Modified the IF condition, to not prevent overlay for Prepayment
        --invoices even if encumbrance is turned on.
...
/*--------------------------------------------------------------------------+
 |GET_TOTAL_PRORATION_AMOUNT :: returns the Total Amount and Receipt Match  |
 |case given the rcv_transaction_id. These values are calculated as follows:|
 |                                                                          |
 |O -- Amount ORDERED on PO shipment                                        |
 |C -- Amount CANCELLED  on PO shipment                                     |
 |D -- Amount DELIVERED on Rcv Transaction                                  |
 |B -- Total Amount BILLED on the rcv_transaction                           |
 |a -- Amount being Invoiced right now (X_Match_Amount)                     |
 |                                                                          |
 |If a < = D - B        then  Case = 'DB' , Total_Amount  = (D - B)         |
 |If a < = (O - C) - B  then  Case = 'OB' , Total_Amount = (O - C) - B      |
 | Else                        Case = 'O'  , Total_Amount = O               |
 +--------------------------------------------------------------------------*/
PROCEDURE Get_Total_Proration_Amount
                          ( X_Rcv_Transaction_Id        IN      NUMBER,
			    X_Po_Line_Location_Id       IN      NUMBER,
...
BEGIN
   ...
   l_debug_info := 'Get total receipt amounts for proration';
   ...
   RCV_INVOICE_MATCHING_SV.Get_ReceiveAmount(
    ...
    PO_AP_INVOICE_MATCH_GRP.Get_po_ship_amounts(p_api_version   => 1.0,
                        p_receive_transaction_id        => X_rcv_transaction_id,
    ...
    l_debug_info := 'Calculating Total Amount and Receipt matching case';

      -- Calculate the Total Amount and Receipt matching case
    IF (X_Match_Amount <= (l_amount_delivered - l_amount_billed)) THEN

        X_total_amount := l_amount_delivered - l_amount_billed;
        X_matching_case := 'DB';

    Elsif (X_Match_Amount <= (l_amount_ordered - l_amount_cancelled) - l_amount_billed)
    Then
        X_total_amount := (l_amount_ordered - l_amount_cancelled)- l_amount_billed;
        X_matching_case := 'OB';
    Else
        X_total_amount := l_amount_ordered;
        X_matching_case := 'O';
    End if;

    l_debug_info := 'x_match_amount,g_match_option,l_amount_billed '||x_match_amount||','||g_match_option||','||l_amount_billed;
    IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
          FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME,l_debug_info);
    END IF;

    --bugfix:5641346
    IF (x_match_amount < 0) THEN

       IF g_match_option = 'R' THEN

          x_total_amount :=  l_amount_billed;
 ...
 WHEN OTHERS THEN
   IF (SQLCODE <> -20001) THEN
     ...
     FND_MESSAGE.SET_TOKEN('PARAMETERS',
                          ' X_Rcv_Transaction_Id = '||TO_CHAR(X_Rcv_Transaction_id)
                          ||', X_Match_Amount = '||X_Match_amount);
     FND_MESSAGE.SET_TOKEN('DEBUG_INFO',l_debug_info);
   END IF;
   APP_EXCEPTION.RAISE_EXCEPTION;
...
/*+=======================================================================+
  |          AMOUNT CORRECTION OF INVOICE MATCHED TO RECEIPT              |
  +=======================================================================+*/

PROCEDURE Amount_Correct_Inv_RCV(
                X_Invoice_Id            IN      NUMBER,
                X_Invoice_Line_Number   IN      NUMBER,
                X_Corrected_Invoice_Id  IN      NUMBER,
                X_Corrected_Line_Number IN      NUMBER,
...
BEGIN
   ...
   l_api_name := 'Amount_Correct_Inv_RCV';

   current_calling_sequence := 'Amount_Correct_Inv_RCV<-'||x_calling_sequence;
...
BEGIN
  ...
  current_calling_sequence := 'Get_Corr_Dist_Proration_Info<-'||x_calling_sequence;
    ...
    -- Bug 5585744. Using invoice_distribution_id in place of po_distribution_id for index
    IF(X_Match_Mode IN ('STD-PS','CR-PS')) THEN
