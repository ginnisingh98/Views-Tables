--------------------------------------------------------
--  DDL for Package Body IGF_AW_FUND_EXCL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_AW_FUND_EXCL_PKG" AS
/* $Header: IGFWI15B.pls 115.8 2002/11/28 14:38:42 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igf_aw_fund_excl_all%ROWTYPE;
  new_references igf_aw_fund_excl_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_AW_FEXC_FMAST_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  ||  (reverse chronological order - newest change first)
  */
     l_org_id           igf_aw_fund_excl_all.org_id%TYPE  DEFAULT igf_aw_gen.get_org_id;
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igf_aw_fund_excl_all
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_AW_FEXC_FCAT_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    INSERT INTO igf_aw_fund_excl_all (
      fund_id,
      fund_code,
      creation_date,
      created_by,
  ...
  */
  BEGIN
    ...
    DELETE FROM igf_aw_fund_excl_all
    WHERE rowid = x_rowid;
