--------------------------------------------------------
--  DDL for Package Body PAY_US_MMREF_LOCAL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_US_MMREF_LOCAL" AS
/* $Header: pyusmmle.pkb 120.3.12010000.2 2008/08/06 14:24:49 svannian ship $ */
/*REM +======================================================================+
...
REM fusman      02-jul-2002 115.9  2296797      Added legislation Code
REM ppanda      04-Nov-2002 115.10              changes made to generalise assignment creation
REM                                             for any given locality
REM asasthan    15-Nov-2002 115.2 2586041       Added Index hint
REM                                             on pay_us_state_tax_info_f
REM ppanda      02-Dec-02   115.3               Nocopy hint added to OUT and IN OUT parameters
REM ppanda      12-Dec-03   115.6  3313007      LOCAL W2 MAG FOR CCA INCLUDES STATE RW RECORD
REM ppanda      09-FEB-03   115.9               Federal/State W-2 Mag tapes should issue warning if GRE
REM                                             is not archived. This issue was fixed for Federal and State
REM                                             W-2. However This is causing a performance issue on Local W-2
REM                                             so to be consistent on W-2 Magnetic Media reporting and
REM                                             performance reason code changed.
REM asasthan    30-JUL02004 115.10  3319904     cursors c_gre_fed, c_gre_state
REM                                             were not being used. removed
REM                                             the cursors and removed +0
REM                                             from c_get_gre.
REM pragupta    22-JUL-2005 115.11  4344862     cursor c_person_in_state removed. It was a redundant
REM                                             cursor and was not used any where in the package.
REM djoshi      14-feb-2005 115.12  5009863    Changed state and federal Cursor
REM                                            to join paa.serial_number
REM ppanda    06-DEC-2006 115.13 4490252    Commented out the code for checking highly compensated
...
END bal_db_item;
 -----------------------------------------------------------------------------
   ...
   --
   --   p_locality_code		Locality Code like DAYTO, PHILA, STLOU and RTCCA
   --
   -- Notes
 ----------------------------------------------------------------------------
       ...
       /* cursor to get if transmitter has been been archived */
        ...
        SELECT  SUBSTR(legislative_parameters,INSTR(legislative_parameters, 'TRANSFER_TRANS_LEGAL_CO_ID=')
         ...
         WHERE report_type = 'W2'
          ...
          AND report_category = 'LC_'||
              pay_us_get_item_data_pkg.GET_CPROG_PARAMETER_VALUE(payroll_action_id,
                                                                 'LC');
--substr(legislative_parameters,instr(legislative_parameters,'LOCALITY_CODE=')
...
/*
        ...
        --
        */
-- End of Code comment for Bug # 3069840
--
...
IS
	...
	-- 1099R GREs.
          CURSOR c_state (c_state_code    varchar2,
		...
		ELSE
		  ...
		  hr_utility.trace('Effective End Date :  '||
			...
			IF l_report_type = 'STW2' THEN
				...
				IF l_w2_box17 > 999999999.99 THEN
...
BEGIN
 ...
 FOR  c1 IN c_yep_tax_unit_ppa_id(p_payroll_action_id )  LOOP
        ...
        if c_sit_check%notfound then
            l_message_text := ' :Missing State Tax Rules For State of :' || p_transfer_state;
            pay_core_utils.push_message(801,'PAY_INVALID_ER_DATA','P');
   ...
   hr_utility.trace(l_message_preprocess || ' ' || l_message_text );
          ...
          if l_info IS NULL THEN
            l_flag := 'N';
            l_message_text := ' NULL EIN For State of :' || p_transfer_state;
            hr_utility.trace(l_message_preprocess || ' ' || l_message_text );
            pay_core_utils.push_message(801,'PAY_INVALID_ER_DATA','P');
            pay_core_utils.push_token('record_name',l_message_preprocess);
            pay_core_utils.push_token('description',l_message_text);
 ...
 END loop;
IF l_flag = 'N' THEN
                l_message_text := 'Flag N Set for Tax Rule Check  ';
                pay_core_utils.push_message(801,'PAY_INVALID_ER_DATA','P');
                pay_core_utils.push_token('record_name',l_message_preprocess);
                pay_core_utils.push_token('description',l_message_text);
                u_message_text := 'Error in EIN or State Tax Rule Missing ';
                hr_utility.set_message(801, 'HR_7998_ALL_EXEMACRO_MESSAGE');
...
WHEN OTHERS THEN
          ...
          l_message_text := 'In State Tax Rules Or EIN Checking ';
          pay_core_utils.push_message(801,'PAY_INVALID_ER_DATA','P');
          pay_core_utils.push_token('record_name',l_message_preprocess);
          pay_core_utils.push_token('description',l_message_text);
          u_message_text := 'Exception : State Tax Rules Or EIN Checking ';
          hr_utility.set_message(801, 'HR_7998_ALL_EXEMACRO_MESSAGE');
          hr_utility.set_message_token('MESSAGE', u_message_text);
          hr_utility.raise_error;
          return 'Y';
