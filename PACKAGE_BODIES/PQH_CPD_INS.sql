--------------------------------------------------------
--  DDL for Package Body PQH_CPD_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_CPD_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_cpd_ins.';  -- Global package name
g_debug    boolean := hr_utility.debug_enabled;
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_corps_definition_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_corps_definition_id  in  number) is
--
  l_proc       varchar2(72);
--
Begin
  ...
  --
  pqh_cpd_ins.g_corps_definition_id_i := p_corps_definition_id;
  --
  if g_debug then
...
Begin
  ...
  --
  -- Insert the row into: pqh_corps_definitions
  --
  insert into pqh_corps_definitions
      (corps_definition_id
      ,business_group_id
      ,name
      ,status_cd
...
--
  Cursor C_Sel1 is select pqh_corps_definitions_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from pqh_corps_definitions
     where corps_definition_id =
             pqh_cpd_ins.g_corps_definition_id_i;
--
  l_proc   varchar2(72);
  l_exists varchar2(1);
--
Begin
  ...
  --
  If (pqh_cpd_ins.g_corps_definition_id_i is not null) Then
    --
    ...
    p_rec.corps_definition_id :=
      pqh_cpd_ins.g_corps_definition_id_i;
    pqh_cpd_ins.g_corps_definition_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.corps_definition_id;
    Close C_Sel1;
  End If;
  --
  if g_debug then
...
Begin
  ...
  --
  pqh_cpd_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  pqh_cpd_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  pqh_cpd_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure ins
  ...
  ,p_corps_type_cd                  in     varchar2 default null
...
Begin
  ...
  --
  p_corps_definition_id := l_rec.corps_definition_id;
