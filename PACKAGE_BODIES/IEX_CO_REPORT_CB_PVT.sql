--------------------------------------------------------
--  DDL for Package Body IEX_CO_REPORT_CB_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IEX_CO_REPORT_CB_PVT" AS
/* $Header: IEXRRCBB.pls 120.1 2004/03/17 18:04:49 jsanju ship $ */
...
FUNCTION get_code_meaning(p_lookup_type IN VARCHAR2
  ...
  BEGIN
    ...
    l_case_rec.last_notification_date := l_date;
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
      null;
    END;
    ...
    BEGIN
    l_date := null;
    SELECT credit_bureau_report_date
    INTO l_date
    FROM OKL_OPEN_INT
    WHERE cas_id = p_cas_id
    AND credit_bureau_report_date IS NOT NULL
    AND rownum = 1;

    l_case_rec.last_report_date := l_date;
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
      null;
    END;
