--------------------------------------------------------
--  DDL for Package Body HR_PERSON_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_PERSON_BE1" as 
...
p_orig_hire_warning            boolean) is
  ...
  --
  l_proc varchar2(72):='  hr_person_be1.update_person_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.person.update_person';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<rehire_authorizor>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_rehire_authorizor);
    l_text:=l_text||'</rehire_authorizor>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<rehire_reason>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_rehire_reason);
    l_text:=l_text||'</rehire_reason>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<suffix>';
