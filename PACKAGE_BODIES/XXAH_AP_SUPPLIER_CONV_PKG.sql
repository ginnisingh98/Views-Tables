--------------------------------------------------------
--  DDL for Package Body XXAH_AP_SUPPLIER_CONV_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XXAH_AP_SUPPLIER_CONV_PKG" 
AS
   /***************************************************************************
   *                           IDENTIFICATION
   *                           ==============
   ...
   * 10-Sep-2015        1.1       Vema Reddy       Added/Modified logic based on RFC-027
   ****************************************************************************/
...
IS
   ...
   lv_row_count                    NUMBER;
   l_row_count                       NUMBER;
   lv_output                       VARCHAR2 (8000);  -- Modified size
   lv_api_msg                      VARCHAR2 (8000); -- Modified size
   v_msg_dummy                     NUMBER;
   ...
   l_party_object_version_number   NUMBER;
                      ...
                      l_vendor_site_name VARCHAR2(15);
                      l_term_id number;
   ...
   IS
    ...
    IS
      SELECT   xsb.*
          FROM xxah_ps_ebs_supp_bank xsb, ap_suppliers asp
...
BEGIN
   ...
   LOOP
      ...
      --if r_create_vendor.purchasing_flag='Y' and r_create_vendor.pay_site_flag='Y' and r_create_vendor.primary_pay_site_flag='Y' then

     -- l_pay_group_lookup_code := '0' || NVL (r_create_vendor.payment_group_code, 4);
       begin
       select distinct payment_group_code, po_match into l_pay_group_code, l_po_match from  xxah_ps_ebs_supp_bank
  ...
  fnd_file.put_line (fnd_file.LOG,' l_pay_group_lookup_code => '||l_pay_group_lookup_code);
      ...
      THEN
         ...
         lv_vendor_site_rec.vendor_site_code     := r_create_vendor.supplier_site_name;
         ...
         lv_vendor_site_rec.zip                 := r_create_vendor.primary_zip_code;
         ...
         ELSE
            ...
            THEN
               ...
               /*IF     r_create_vendor.remit_supplier_name IS NOT NULL
                  AND r_create_vendor.remit_supplier_site IS NOT NULL
               THEN
                        --<Create Relation of Supplier Remitance>--
    fnd_file.put_line(fnd_file.LOG,'lv_party_id '||lv_party_id ||'x_vendor_site_id '||x_vendor_site_id||'remit_supplier_name '||r_create_vendor.remit_supplier_name||'remit_supplier_site '||r_create_vendor.remit_supplier_site);

                  p_remit_relationships (lv_party_id,
                                         x_vendor_site_id,
                                         TRIM(r_create_vendor.remit_supplier_name),
                                         TRIM(r_create_vendor.remit_supplier_site)
                                        );
               END IF;*/
     ...
     END IF;
      ...
      IF lv_branch_id IS NOT NULL AND r_create_vendor.bank_account_number IS NOT NULL
      THEN
         BEGIN
            p_create_bank_acct (lv_bank_id,
                                ...
                                r_create_vendor.bank_account_number,
                                lv_territory,
                                x_vendor_site_id,
                                x_party_site_id,
                                lv_account_id,
                                r_create_vendor.iban,
                                r_create_vendor.check_digits,
         ...
         END;
      END IF;
                -- FND_FILE.PUT_LINE(FND_FILE.LOG,'x_vendor_site_id'||''||x_vendor_site_id);    ST    08/10/2015    Log writing issue
                 --FND_FILE.PUT_LINE(FND_FILE.LOG,'l_supplier_exists_flag'||''||l_supplier_exists_flag);    ST    08/10/2015    Log writing issue
      IF x_vendor_site_id IS NOT NULL
      THEN
         --IF l_supplier_exists_flag <> 'Y'
         --THEN
            --<Supplier Type>--
           ...
           /* BEGIN
               p_uda (lv_party_id,
                      ...
                      r_create_vendor.leaf_commodity,
                      'SUPP_LEVEL',
                      lv_party_site_id,
                      x_vendor_site_id
                     );
...
END IF;
            ...
            BEGIN
               p_uda (lv_party_id,
                      'XXAH_STD_QUAL_SUPP',              -- RFC-027
                      'Registration Type',               -- RFC-027
                      r_create_vendor.STD_ID_QUAL_SUPP,  -- RFC-027
                      r_create_vendor.STD_ID_NUM_SUPP,   -- RFC-027
                      'SUPP_LEVEL',
                      lv_party_site_id,
                      x_vendor_site_id
                     );
         ...
         --THEN
            ...
            If r_create_vendor.STD_ID_NUM_SITE IS NOT NULL THEN

            BEGIN
               p_uda (lv_party_id,
                      'XXAH_STD_QUAL_SITE',             -- RFC-027
                      'Registration Type Site',         -- RFC-027
                      r_create_vendor.STD_ID_QUAL_SITE, -- RFC-027
                      r_create_vendor.STD_ID_NUM_SITE,  -- RFC-027
                      'SUPP_ADDR_SITE_LEVEL',
                      x_party_site_id,
                      x_vendor_site_id
                     );
            ...
            p_update_dff (x_vendor_site_id,
                          ...
                          r_create_vendor.primary_contact_phone_work,
                          ...
                          r_create_vendor.primary_work_country_code,
...
--    select fl.MEANING INTO l_currency_code from fnd_lookups fl
--where fl.lookup_type='XXAH_CURRENCY_CODE'
--and fl.lookup_code=p_territory_code;
...
END;
   ...
   THEN
      COMMIT;
   ELSE
      --fnd_file.put_line (fnd_file.LOG,'Error Message Count : ' || ln_msg_count);ST    08/10/2015    Log writing issue
      fnd_file.put_line (fnd_file.LOG,'Error Message Data  : ' || lv_msg_data);
      --fnd_file.put_line (fnd_file.LOG,'Error Code          : ' || ln_errorcode);ST    08/10/2015    Log writing issue
      --fnd_file.put_line (fnd_file.LOG,'Entering Error Loop ');    ST    08/10/2015    Log writing issue
      ...
      LOOP
       ...
       /*  fnd_file.put_line (fnd_file.LOG,
                           ...
                           );*/    --ST    08/10/2015    Log writing issue
      END LOOP;
...
BEGIN
  ...
  ap_vendor_pub_pkg.update_vendor_site (p_api_version => 1.0,
    ...
    commit;
  ELSE
    --FND_FILE.put_line(FND_FILE.LOG,'Error Message Count : ' || l_msg_count);    ST    08/10/2015    Log writing issue
    --FND_FILE.put_line(FND_FILE.LOG,'Error Message Data  : ' || l_msg_data);    ST    08/10/2015    Log writing issue
    --FND_FILE.put_line(FND_FILE.LOG,'Entering Error Loop of Procedure p_update_dff ');
    FOR i IN 1 .. l_msg_count
    LOOP
      fnd_msg_pub.get(p_msg_index     => i,
                      p_data          => l_msg_data,
      ...
      fnd_message.set_encoded(l_msg_data);
     -- FND_FILE.put_line(FND_FILE.LOG,'Inside Error Loop : ' || i || ', ' || l_msg_data); ST    08/10/2015    Log writing issue
    END LOOP;
    rollback;
...
-- Commented due to Remit to Site relation creation using through User Defined Attributes(UDA)
...
IS
   ...
   l_relationship_ct    NUMBER;
   l_insert_flag        VARCHAR2 (1) := 'Y';
BEGIN

   BEGIN
      SELECT b.vendor_site_id, a.party_id
   ...
   EXCEPTION
      ...
      THEN
         l_insert_flag := 'N';
         fnd_file.put_line (fnd_file.LOG,'Error l_rem_party_id '|| SQLCODE|| ' -ERROR- '|| SQLERRM);
   END;

   SELECT COUNT (*)
     INTO l_relationship_ct
     FROM iby_ext_payee_relationships
    WHERE party_id = p_party_id
      AND supplier_site_id = p_vendor_site_id
      AND remit_party_id = l_rem_party_id
      AND remit_supplier_site_id = l_rem_vend_site_id
      AND TO_DATE IS NULL;

   IF    l_relationship_ct IS NULL
      OR l_relationship_ct = 0 AND l_insert_flag = 'Y'
   THEN
...
EXCEPTION
   ...
   THEN
      fnd_file.put_line(fnd_file.LOG,'+---------------------------------------------------------------------------+');
      fnd_file.put_line(fnd_file.LOG,'Error at P_REMIT_RELATIONSHIPS '|| SQLCODE|| ' -ERROR- '|| SQLERRM);
      fnd_file.put_line(fnd_file.LOG,'+---------------------------------------------------------------------------+');
END P_REMIT_RELATIONSHIPS;*/
...
hz_contact_point_v2pub.create_email_contact_point (
   ...
   IF (x_return_status = fnd_api.g_ret_sts_success) THEN
    COMMIT;
    /*fnd_file.put_line (fnd_file.LOG,'Creation of Customer contact is Successful ');
    fnd_file.put_line (fnd_file.LOG,'Output information ....');
    fnd_file.put_line (fnd_file.LOG,'Cust contact point id         = '||TO_CHAR(x_contact_point_id));
    fnd_file.put_line (fnd_file.LOG,'Return Status Of Api    = '||x_return_status);
    fnd_file.put_line (fnd_file.LOG,'No of err msgs          = '||x_msg_count);
    fnd_file.put_line (fnd_file.LOG,'err information          = '||x_msg_data);*/
ELSE
    --fnd_file.put_line (fnd_file.LOG,'Creation of Customer contact got failed:'||x_msg_data);
    ROLLBACK;
    FOR i IN 1 .. x_msg_count
    LOOP
...
END P_CREATE_EMAIL_CONTACT_POINT;
   ...
   IS
   select * from XXAH_PS_EBS_SUPP_BANK
