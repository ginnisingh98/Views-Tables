--------------------------------------------------------
--  DDL for Package Body PAY_KR_YEA20100101_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KR_YEA20100101_PKG" as
/* $Header: pykryea9.pkb 120.0.12010000.3 2010/04/05 11:59:07 vaisriva noship $ */
-- Constant
	-- Bug 3966549
	c_20040101 			constant date := fnd_date.canonical_to_date('2004/01/01') ;
	-- End of 3966549
	-- Bug 9231094
	c_20100101			constant date := fnd_date.canonical_to_date('2010/01/01') ;
	c_20130101			constant date := fnd_date.canonical_to_date('2013/01/01') ;
	c_20120101			constant date := fnd_date.canonical_to_date('2012/01/01') ;
...
-- Bug 3901912
	...
	l_housinexp_tax_exem_lim3      	number default 0;  -- Bug 8237227
	...
	--
	l_er_smb_exem_pct		number default 0 ; -- Bug 9079450
	l_er_smb_exem_limit		number default 0 ; -- Bug 9079450
	l_er_smb_prev_earn		number default 0 ; -- Bug 9079450
	l_er_smb_cur_earn		number default 0 ; -- Bug 9079450
	l_long_term_hou_pro_saving_lim  number default 0 ; -- Bug 9231094
	l_fw_tax_reduction_rate		number default 0 ; -- Bug 9231094
--
procedure yea(
...
begin
	...
	if l_global_value_flag <> TRUE then
		...
		l_housinexp_tax_exem_lim2     	:= get_globalvalue('KR_YEA_HOUSINEXP_TAX_EXEM_LIM2',p_effective_date);
                l_housinexp_tax_exem_lim3     	:= get_globalvalue('KR_YEA_HOUSINEXP_TAX_EXEM_LIM3',p_effective_date);  -- Bug 8237227
		...
		-- End of 3966549
		l_dir_pay_tax_exem_per2010      := get_globalvalue('KR_YEA_DIRECT_PAY_EXP_2010_TAX_EXEM_PER',p_effective_date); -- Bug 9231094
		-- Bug 9079450: Removed code for investment partnership financing1
		l_inv_part_fin2_tax_exem_per  	:= get_globalvalue('KR_YEA_INVEST_PARTNER_FIN2_TAX_EXEM_PER',p_effective_date);
                l_inv_part_fin3_tax_exem_per  	:= get_globalvalue('KR_YEA_INVEST_PARTNER_FIN3_TAX_EXEM_PER',p_effective_date);    -- Bug 8237227
		-- Bug 9079450: Removed code for investment partnership financing1
		l_inv_part_fin2_tax_exem_lim  	:= get_globalvalue('KR_YEA_INVEST_PARTNER_FIN2_TAX_EXEM_LIM',p_effective_date);
                l_inv_part_fin3_tax_exem_lim  	:= get_globalvalue('KR_YEA_INVEST_PARTNER_FIN3_TAX_EXEM_LIM',p_effective_date);    -- Bug 8237227
		-- Bug 6895093
		...
		-- End of 5682450
		--
		l_er_smb_exem_pct		:= get_globalvalue ('KR_YEA_ER_SMB_EXEM_PCT', p_effective_date);	-- Bug 9079450
		l_er_smb_exem_limit		:= get_globalvalue ('KR_YEA_ER_SMB_EXEM_LIMIT', p_effective_date);	-- Bug 9079450
		--
		-- Bug 9231094
		l_long_term_hou_pro_saving_lim   := get_globalvalue ('KR_YEA_LONG_TERM_HOU_PRO_SAVING_LIM', p_effective_date);
		--
	...
	-- Bug 3172960
	if (p_yea_info.nationality = 'F' and p_yea_info.fixed_tax_rate = 'Y' and p_effective_date < c_20130101) then
		--
		...
		--
		p_yea_info.calc_tax := greatest(trunc((p_yea_info.taxation_base + p_yea_info.non_rep_non_taxable) * l_fw_fixed_tax_rate),0); -- Bug 9079450
		--
		...
		*/
		...
		-- Bug 3172960
		 if p_yea_info.nationality = 'F' then
			...
			p_yea_info.cur_fw_income_exem 	:= p_yea_info.foreign_worker_income_exem;  -- Bug 8644512
			--
			if p_yea_info.foreign_worker_income_exem > 0 then
				...
				p_yea_info.cur_total_ntax_earn	:= nvl(p_yea_info.cur_total_ntax_earn,0)
			...
			-- Bug 8341054 Added Previous employer foreign worker Exemption to the current employer
                           p_yea_info.foreign_worker_income_exem := nvl(p_yea_info.foreign_worker_income_exem,0)
								  + nvl(p_yea_info.prev_foreign_wrkr_inc_exem,0)
		...
		*/
		...
		if p_yea_info.num_of_super_ageds > 0 then
			p_yea_info.aged_tax_exem := p_yea_info.num_of_super_ageds * l_aged_tax_exem1;
		end if;
		p_yea_info.num_of_ageds := nvl(p_yea_info.num_of_super_ageds, 0);
		--
		...
		if p_yea_info.non_resident_flag = 'N' then
			...
			for i in 1..p_yea_info.dpnt_educ_contact_type_tbl.count loop
				...
				if (p_yea_info.dpnt_educ_contact_type_tbl(i) = '3') then -- Bug 9079450
			...
			if l_dummy > 0
			   ...
			   or p_yea_info.lt_housing_loan_intr_repay_1 > 0
                           or p_yea_info.lt_housing_loan_intr_repay_2 > 0   then
				for i in 1..p_yea_info.housing_saving_tbl.count loop
				       if i = 1 then
					     p_yea_info.housing_saving_type := p_yea_info.housing_saving_type_tbl(i);
				       end if;
				       /* Bug 9079478 */
				       if p_yea_info.housing_saving_type_tbl(i) in ('HST1') then
					...
					/* Bug 9231094 */
					elsif p_yea_info.housing_saving_type_tbl(i) in ('HST5')
					       and p_yea_info.taxable_income <= l_long_term_hou_pro_saving_lim
					       and p_effective_date < c_20130101
					  then
					     if p_yea_info.housing_saving_tbl(i) < l_housinsavintype_HST1 then
						    l_dummy := p_yea_info.housing_saving_tbl(i);
					     else
				...
				p_yea_info.housing_loan_repay_exem  :=  least(greatest(p_yea_info.taxable_income2 - l_cuml_special_exem,0), least(l_housinexp_tax_exem_lim,
                                ...
                                p_yea_info.max_housing_exp_tax_exem := nvl(p_yea_info.housing_loan_repay_exem,0) ;
					end if;
				-- End of Bug -- 7142620
				...
				-- Bug 3966549 : Added date check
			        -- Bug 9079478 : removed the date check for long term interest repay
                                if p_yea_info.lt_housing_loan_interest_repay > 0  then
				-- End of 3966549
                                  --  Start of Bug --7142620
				  -- Bug 7692554
				   p_yea_info.max_housing_exp_tax_exem := p_yea_info.max_housing_exp_tax_exem + p_yea_info.lt_housing_loan_interest_repay;
                                   p_yea_info.max_housing_exp_tax_exem := least(p_yea_info.max_housing_exp_tax_exem,l_housinexp_tax_exem_lim1);

				 -- End of Bug -- 7142620

                                end if;

                                -- Long term loan due 15 - 29 years
                                --
                                if p_yea_info.lt_housing_loan_intr_repay_1  > 0 then
                                 --Start of  Bug -- 7142620
				 -- Bug 7692554
				p_yea_info.max_housing_exp_tax_exem := p_yea_info.max_housing_exp_tax_exem + p_yea_info.lt_housing_loan_intr_repay_1 ;
				p_yea_info.max_housing_exp_tax_exem := least(p_yea_info.max_housing_exp_tax_exem,l_housinexp_tax_exem_lim2);
				--End of Bug --7142620
                                end if;
			        -- Start of Bug 8237227
                                -- Long term loan due greater then 30 years
                                --
                                if p_yea_info.lt_housing_loan_intr_repay_2  > 0 then

				p_yea_info.max_housing_exp_tax_exem :=  p_yea_info.max_housing_exp_tax_exem + p_yea_info.lt_housing_loan_intr_repay_2 ;
                                p_yea_info.max_housing_exp_tax_exem := least(p_yea_info.max_housing_exp_tax_exem,l_housinexp_tax_exem_lim3);
                                end if;
                                p_yea_info.lt_housing_loan_intr_exem := greatest(p_yea_info.max_housing_exp_tax_exem - p_yea_info.housing_loan_repay_exem ,0);
                                --
                             	-- End of Bug 8237227
				-- Bug 4119483: Apply upper limit based on cumulative special exemption
				--Start of  Bug -- 7142620
			...
			------------------------------------------------------------------------
			-- Bug 7676136: Obsoleted Marriage, Funeral and Relocation Tax Exemption
			------------------------------------------------------------------------
		...
		if p_yea_info.non_resident_flag = 'N' then
			...
			if p_yea_info.lt_housing_loan_intr_repay_1  > 0 then
				p_yea_info.housing_saving_exem := least(greatest(p_yea_info.taxation_base,0) ,
				   least(l_housinexp_tax_exem_lim2 - nvl(p_yea_info.housing_loan_repay_exem,0) - nvl(p_yea_info.lt_housing_loan_intr_exem,0),
                        ...
                        if p_yea_info.lt_housing_loan_intr_repay_2  > 0 then
				p_yea_info.housing_saving_exem := least(greatest(p_yea_info.taxation_base,0) ,
				   least(l_housinexp_tax_exem_lim3 - nvl(p_yea_info.housing_loan_repay_exem,0) - nvl(p_yea_info.lt_housing_loan_intr_exem,0),
				         l_housinexp_tax_exem_lim - nvl(p_yea_info.housing_loan_repay_exem,0), p_yea_info.housing_saving * l_housinexp_tax_exem_per ));
			end if;
			...
			----------------------------------------------------------------
			-- Bug 9079450: Removed code for investment partnership financing1
			if p_yea_info.invest_partner_fin2 > 0
                        or p_yea_info.invest_partner_fin3 > 0 then
				if p_yea_info.taxable_income > 0 then
					p_yea_info.invest_partner_fin_tax_exem := least(trunc(p_yea_info.invest_partner_fin2 * l_inv_part_fin2_tax_exem_per),
                                        ...
                                        p_yea_info.invest_partner_fin_tax_exem := p_yea_info.invest_partner_fin_tax_exem +
                                                                                  least(trunc(p_yea_info.invest_partner_fin3 * l_inv_part_fin3_tax_exem_per),
                                                                                  trunc(p_yea_info.taxable_income * l_inv_part_fin3_tax_exem_lim));
			...
			if p_yea_info.total_credit_card_exp > 0 and p_effective_date < c_20120101 then
				...
				if l_dummy > 0 then
					-- Bug 9231094
					p_yea_info.credit_card_exp_tax_exem := (l_dummy * ( (p_yea_info.credit_card_exp + p_yea_info.cash_receipt_exp2005 + p_yea_info.giro_tuition_paid_exp2004)/ p_yea_info.total_credit_card_exp) * l_cre_card_tax_exem_per2004 );
					p_yea_info.direct_card_exp_tax_exem := (l_dummy * ( p_yea_info.direct_card_exp / p_yea_info.total_credit_card_exp ) * l_dir_pay_tax_exem_per2010 );
					--
					-- After statement below, l_dummy holds maximum exemption amount
					l_dummy := p_yea_info.credit_card_exp_tax_exem + p_yea_info.direct_card_exp_tax_exem;
					-- End of bug 9231094
					--
					-- End of 3966549
					--
					...
					p_yea_info.total_credit_card_exp_tax_exem := least(p_yea_info.total_credit_card_exp_tax_exem,greatest(p_yea_info.taxation_base,0));
			...
			--
			---------------------------------------------------------------------------
			-- Bug 9079450: Employment Retention Small-Medium Business Income Exemption
			---------------------------------------------------------------------------
			if (p_yea_info.smb_eligibility_flag = 'Y') then

			    l_er_smb_prev_earn := nvl(p_yea_info.prev_smb_eligible_income,0);
			    l_er_smb_cur_earn  := nvl(p_yea_info.cur_smb_eligible_income,0);

			    if (   nvl(p_yea_info.cur_smb_days_worked,0) = 0
				or nvl(p_yea_info.prev_smb_days_worked,0) = 0
				or p_yea_info.emp_join_prev_year = 'X'
				or p_yea_info.emp_leave_cur_year = 'X') then
				p_yea_info.smb_income_exem := 0;
			    else
			        if p_yea_info.emp_join_prev_year = 'Y' then
				   l_er_smb_cur_earn  := l_er_smb_cur_earn
							* (p_yea_info.prev_smb_days_worked / p_yea_info.cur_smb_days_worked);
				end if;
				if p_yea_info.emp_leave_cur_year = 'Y' then
				   l_er_smb_prev_earn := l_er_smb_prev_earn
							* (p_yea_info.cur_smb_days_worked / p_yea_info.prev_smb_days_worked);
				end if;
				--
				p_yea_info.smb_income_exem := greatest(least(round((l_er_smb_prev_earn - l_er_smb_cur_earn)
							   * l_er_smb_exem_pct,0), l_er_smb_exem_limit,
							     greatest(p_yea_info.taxation_base,0)),0);
				p_yea_info.taxation_base := p_yea_info.taxation_base - p_yea_info.smb_income_exem;
			    end if;
			else
				p_yea_info.smb_income_exem := 0;
			end if;
		...
		if l_dummy > 0 then
		        -- Bug 9231094
			if p_effective_date > c_20120101 then
			l_basic_tax_break     := to_number(hruserdt.get_table_value(
							p_bus_group_id		=> p_business_group_id,
							p_table_name		=> 'MAX_BASIC_TAX_BREAK',
							p_col_name		=> 'VALUE',
							p_row_value		=> to_char(p_yea_info.taxable),
							p_effective_date	=> p_effective_date));
			end if;
		...
		----------------------------------------------------------------
                -- Bug 5083240
		p_yea_info.foreign_worker_tax_break := 0;
		if p_yea_info.nationality = 'F'
			...
			l_fw_contr_taxable := trunc(p_yea_info.fw_contr_taxable_earn);
		-- Bug 9231094
		l_fw_tax_reduction_rate :=  get_globalvalue ('KR_FW_TAX_REDUCTION_RATE',p_yea_info.fwtb_contract_date);
			/* Bug 9231094 -- Deduct foreign worker income exemption
                        ...
                        -- Prorate tax breaks
			-- Bug 9231094
			if p_yea_info.fwtb_contract_date < c_20100101 then
			  ...
			  if p_yea_info.fwtb_immigration_purpose = 'G' then
			        ...
			        p_yea_info.foreign_worker_tax_break := greatest(trunc((p_yea_info.calc_tax * l_fw_brk_proration_factor * l_fw_tax_reduction_rate), 0), 0);
				p_yea_info.foreign_worker_tax_break2 := p_yea_info.foreign_worker_tax_break;
			  end if;
			end if;
		...
		if p_yea_info.foreign_worker_tax_break < p_yea_info.calc_tax then

			----------------------------------------------------------------
			-- Annual Tax
			----------------------------------------------------------------
			if p_yea_info.total_tax_break > (p_yea_info.calc_tax - p_yea_info.foreign_worker_tax_break) then
			       p_yea_info.total_tax_break := p_yea_info.calc_tax - p_yea_info.foreign_worker_tax_break;
			end if;
			--
			p_yea_info.annual_itax := trunc(greatest(p_yea_info.calc_tax - p_yea_info.total_tax_break - p_yea_info.foreign_worker_tax_break, 0),0);
			p_yea_info.annual_rtax := trunc(p_yea_info.annual_itax * l_annual_itax_per,0);
