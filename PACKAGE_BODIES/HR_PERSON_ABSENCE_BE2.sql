--------------------------------------------------------
--  DDL for Package Body HR_PERSON_ABSENCE_BE2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_PERSON_ABSENCE_BE2" as 
...
p_del_element_entry_warning    boolean) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  hr_person_absence_be2.update_person_absence_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.person_absence.update_person_absence';
...
l_text:=l_text||'FALSE';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<del_element_entry_warning>';
if(P_DEL_ELEMENT_ENTRY_WARNING) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</del_element_entry_warning>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</person_absence>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
