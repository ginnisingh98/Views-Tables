--------------------------------------------------------
--  DDL for Package Body IEX_PAYMENT_COLL_RPT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IEX_PAYMENT_COLL_RPT_PKG" AS
/* $Header: iexpclrb.pls 120.0.12010000.12 2009/08/20 09:46:58 barathsr noship $ */
  G_PKG_NAME VARCHAR2(100)         :='iex_payment_coll_rpt_pkg';
 -- l_api_name CONSTANT VARCHAR2(30) := 'Payment Collector Report';
...
PROCEDURE gen_xml_data_pcol
			  (
			    ...
			    p_goal_amount NUMBER )
                                IS
  l_api_name CONSTANT VARCHAR2(30) := 'gen_xml_data_pcol';
  l_api_version CONSTANT NUMBER := 1.0;
 ...
 /*  if l_coll_rate is null then
  ...
  IF p_summ_det    = 'DETAIL' THEN
     l_query := 'SELECT l.meaning payment_method,p.ipayment_status payment_status,p.campaign_sched_id campaign_schedule_id ,'||
      'c.source_code,to_char(p.creation_date,''DD-Mon-YYYY'') payment_date,gl_currency_api.convert_amount(r.currency_code,'''||p_currency||''',sysdate,''Corporate'',r.amount) amount,'||
      'p.payment_id,d.delinquency_id,p.resource_id,j.source_name collector,hca.account_number,s.trx_number invoice_id,s.payment_schedule_id,s.class,'||
      'gl_currency_api.convert_amount(r.currency_code,'''||p_currency||''',sysdate,''Corporate'',s.amount_due_remaining) amount_due_remaining,'||
      'gl_currency_api.convert_amount(r.currency_code,'''||p_currency||''',sysdate,''Corporate'',s.amount_due_original) amount_due_original,'||
  ...
  IF p_summ_det          = 'SUMMARY' THEN
    ...
    END IF;
      ...
      l_query:='SELECT j.source_name Collector,'||
    ...
    l_group_by  :=' group by j.source_name';
    l_query:=l_query||l_where;
...
WHEN OTHERS THEN
 ...
 LogMessage (FND_LOG.LEVEL_UNEXPECTED, G_PKG_NAME || '.' || l_api_name || '-end');
END gen_xml_data_pcol;
