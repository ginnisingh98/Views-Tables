--------------------------------------------------------
--  DDL for Package Body BEN_APLD_DPNT_CVG_PRFL_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_APLD_DPNT_CVG_PRFL_API" as
/* $Header: beadeapi.pkb 120.0 2005/05/28 00:21:18 appldev noship $ */
--
-- Package Variables
--
g_package  varchar2(33) := '  ben_Apld_Dpnt_Cvg_Prfl_api.';
--
-- ----------------------------------------------------------------------------
-- |------------------------< create_Apld_Dpnt_Cvg_Prfl >----------------------|
-- ----------------------------------------------------------------------------
--
procedure create_Apld_Dpnt_Cvg_Prfl
  (p_validate                       in  boolean   default false
  ,p_apld_dpnt_cvg_elig_prfl_id     out nocopy number
  ,p_effective_start_date           out nocopy date
  ,p_effective_end_date             out nocopy date
  ,p_business_group_id              in  number    default null
  ,p_apld_dpnt_cvg_elig_rl          in  number    default null
  ,p_mndtry_flag                    in  varchar2  default null
  ,p_dpnt_cvg_eligy_prfl_id         in  number    default null
  ,p_pgm_id                         in  number    default null
  ,p_pl_id                          in  number    default null
  ,p_ptip_id                        in  number    default null
  ,p_ade_attribute_category         in  varchar2  default null
  ,p_ade_attribute1                 in  varchar2  default null
  ,p_ade_attribute2                 in  varchar2  default null
  ,p_ade_attribute3                 in  varchar2  default null
  ,p_ade_attribute4                 in  varchar2  default null
  ,p_ade_attribute5                 in  varchar2  default null
  ,p_ade_attribute6                 in  varchar2  default null
  ,p_ade_attribute7                 in  varchar2  default null
  ,p_ade_attribute8                 in  varchar2  default null
  ,p_ade_attribute9                 in  varchar2  default null
  ,p_ade_attribute10                in  varchar2  default null
  ,p_ade_attribute11                in  varchar2  default null
  ,p_ade_attribute12                in  varchar2  default null
  ,p_ade_attribute13                in  varchar2  default null
  ,p_ade_attribute14                in  varchar2  default null
  ,p_ade_attribute15                in  varchar2  default null
  ,p_ade_attribute16                in  varchar2  default null
  ,p_ade_attribute17                in  varchar2  default null
  ,p_ade_attribute18                in  varchar2  default null
  ,p_ade_attribute19                in  varchar2  default null
  ,p_ade_attribute20                in  varchar2  default null
  ,p_ade_attribute21                in  varchar2  default null
  ,p_ade_attribute22                in  varchar2  default null
  ,p_ade_attribute23                in  varchar2  default null
  ,p_ade_attribute24                in  varchar2  default null
  ,p_ade_attribute25                in  varchar2  default null
  ,p_ade_attribute26                in  varchar2  default null
  ,p_ade_attribute27                in  varchar2  default null
  ,p_ade_attribute28                in  varchar2  default null
  ,p_ade_attribute29                in  varchar2  default null
  ,p_ade_attribute30                in  varchar2  default null
  ,p_object_version_number          out nocopy number
  ,p_effective_date                 in  date
  ) is
  --
  -- Declare cursors and local variables
  --
  l_apld_dpnt_cvg_elig_prfl_id ben_apld_dpnt_cvg_elig_prfl_f.apld_dpnt_cvg_elig_prfl_id%TYPE;
  l_effective_start_date ben_apld_dpnt_cvg_elig_prfl_f.effective_start_date%TYPE;
  l_effective_end_date ben_apld_dpnt_cvg_elig_prfl_f.effective_end_date%TYPE;
  l_proc varchar2(72) := g_package||'create_Apld_Dpnt_Cvg_Prfl';
  l_object_version_number ben_apld_dpnt_cvg_elig_prfl_f.object_version_number%TYPE;
  --
begin
  --
  hr_utility.set_location('Entering:'|| l_proc, 10);
  --
  -- Issue a savepoint if operating in validation only mode
  --
  savepoint create_Apld_Dpnt_Cvg_Prfl;
  --
  hr_utility.set_location(l_proc, 20);
  --
  -- Process Logic
  --
  begin
    --
    -- Start of API User Hook for the before hook of create_Apld_Dpnt_Cvg_Prfl
    --
    ben_Apld_Dpnt_Cvg_Prfl_bk1.create_Apld_Dpnt_Cvg_Prfl_b
      (
       p_business_group_id              =>  p_business_group_id
      ,p_apld_dpnt_cvg_elig_rl          =>  p_apld_dpnt_cvg_elig_rl
      ,p_mndtry_flag                    =>  p_mndtry_flag
      ,p_dpnt_cvg_eligy_prfl_id         =>  p_dpnt_cvg_eligy_prfl_id
      ,p_pgm_id                         =>  p_pgm_id
      ,p_pl_id                          =>  p_pl_id
      ,p_ptip_id                        =>  p_ptip_id
      ,p_ade_attribute_category         =>  p_ade_attribute_category
      ,p_ade_attribute1                 =>  p_ade_attribute1
      ,p_ade_attribute2                 =>  p_ade_attribute2
      ,p_ade_attribute3                 =>  p_ade_attribute3
      ,p_ade_attribute4                 =>  p_ade_attribute4
      ,p_ade_attribute5                 =>  p_ade_attribute5
      ,p_ade_attribute6                 =>  p_ade_attribute6
      ,p_ade_attribute7                 =>  p_ade_attribute7
      ,p_ade_attribute8                 =>  p_ade_attribute8
      ,p_ade_attribute9                 =>  p_ade_attribute9
      ,p_ade_attribute10                =>  p_ade_attribute10
      ,p_ade_attribute11                =>  p_ade_attribute11
      ,p_ade_attribute12                =>  p_ade_attribute12
      ,p_ade_attribute13                =>  p_ade_attribute13
      ,p_ade_attribute14                =>  p_ade_attribute14
      ,p_ade_attribute15                =>  p_ade_attribute15
      ,p_ade_attribute16                =>  p_ade_attribute16
      ,p_ade_attribute17                =>  p_ade_attribute17
      ,p_ade_attribute18                =>  p_ade_attribute18
      ,p_ade_attribute19                =>  p_ade_attribute19
      ,p_ade_attribute20                =>  p_ade_attribute20
      ,p_ade_attribute21                =>  p_ade_attribute21
      ,p_ade_attribute22                =>  p_ade_attribute22
      ,p_ade_attribute23                =>  p_ade_attribute23
      ,p_ade_attribute24                =>  p_ade_attribute24
      ,p_ade_attribute25                =>  p_ade_attribute25
      ,p_ade_attribute26                =>  p_ade_attribute26
      ,p_ade_attribute27                =>  p_ade_attribute27
      ,p_ade_attribute28                =>  p_ade_attribute28
      ,p_ade_attribute29                =>  p_ade_attribute29
      ,p_ade_attribute30                =>  p_ade_attribute30
      ,p_effective_date                 => trunc(p_effective_date)
      );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (
         p_module_name => 'CREATE_Apld_Dpnt_Cvg_Prfl'
        ,p_hook_type   => 'BP'
        );
    --
    -- End of API User Hook for the before hook of create_Apld_Dpnt_Cvg_Prfl
    --
  end;
  --
  ben_ade_ins.ins
    (
     p_apld_dpnt_cvg_elig_prfl_id    => l_apld_dpnt_cvg_elig_prfl_id
    ,p_effective_start_date          => l_effective_start_date
    ,p_effective_end_date            => l_effective_end_date
    ,p_business_group_id             => p_business_group_id
    ,p_apld_dpnt_cvg_elig_rl         => p_apld_dpnt_cvg_elig_rl
    ,p_mndtry_flag                   => p_mndtry_flag
    ,p_dpnt_cvg_eligy_prfl_id        => p_dpnt_cvg_eligy_prfl_id
    ,p_pgm_id                        => p_pgm_id
    ,p_pl_id                         => p_pl_id
    ,p_ptip_id                       => p_ptip_id
    ,p_ade_attribute_category        => p_ade_attribute_category
    ,p_ade_attribute1                => p_ade_attribute1
    ,p_ade_attribute2                => p_ade_attribute2
    ,p_ade_attribute3                => p_ade_attribute3
    ,p_ade_attribute4                => p_ade_attribute4
    ,p_ade_attribute5                => p_ade_attribute5
    ,p_ade_attribute6                => p_ade_attribute6
    ,p_ade_attribute7                => p_ade_attribute7
    ,p_ade_attribute8                => p_ade_attribute8
    ,p_ade_attribute9                => p_ade_attribute9
    ,p_ade_attribute10               => p_ade_attribute10
    ,p_ade_attribute11               => p_ade_attribute11
    ,p_ade_attribute12               => p_ade_attribute12
    ,p_ade_attribute13               => p_ade_attribute13
    ,p_ade_attribute14               => p_ade_attribute14
    ,p_ade_attribute15               => p_ade_attribute15
    ,p_ade_attribute16               => p_ade_attribute16
    ,p_ade_attribute17               => p_ade_attribute17
    ,p_ade_attribute18               => p_ade_attribute18
    ,p_ade_attribute19               => p_ade_attribute19
    ,p_ade_attribute20               => p_ade_attribute20
    ,p_ade_attribute21               => p_ade_attribute21
    ,p_ade_attribute22               => p_ade_attribute22
    ,p_ade_attribute23               => p_ade_attribute23
    ,p_ade_attribute24               => p_ade_attribute24
    ,p_ade_attribute25               => p_ade_attribute25
    ,p_ade_attribute26               => p_ade_attribute26
    ,p_ade_attribute27               => p_ade_attribute27
    ,p_ade_attribute28               => p_ade_attribute28
    ,p_ade_attribute29               => p_ade_attribute29
    ,p_ade_attribute30               => p_ade_attribute30
    ,p_object_version_number         => l_object_version_number
    ,p_effective_date                => trunc(p_effective_date)
    );
  --
  begin
    --
    -- Start of API User Hook for the after hook of create_Apld_Dpnt_Cvg_Prfl
    --
    ben_Apld_Dpnt_Cvg_Prfl_bk1.create_Apld_Dpnt_Cvg_Prfl_a
      (
       p_apld_dpnt_cvg_elig_prfl_id     =>  l_apld_dpnt_cvg_elig_prfl_id
      ,p_effective_start_date           =>  l_effective_start_date
      ,p_effective_end_date             =>  l_effective_end_date
      ,p_business_group_id              =>  p_business_group_id
      ,p_apld_dpnt_cvg_elig_rl          =>  p_apld_dpnt_cvg_elig_rl
      ,p_mndtry_flag                    =>  p_mndtry_flag
      ,p_dpnt_cvg_eligy_prfl_id         =>  p_dpnt_cvg_eligy_prfl_id
      ,p_pgm_id                         =>  p_pgm_id
      ,p_pl_id                          =>  p_pl_id
      ,p_ptip_id                        =>  p_ptip_id
      ,p_ade_attribute_category         =>  p_ade_attribute_category
      ,p_ade_attribute1                 =>  p_ade_attribute1
      ,p_ade_attribute2                 =>  p_ade_attribute2
      ,p_ade_attribute3                 =>  p_ade_attribute3
      ,p_ade_attribute4                 =>  p_ade_attribute4
      ,p_ade_attribute5                 =>  p_ade_attribute5
      ,p_ade_attribute6                 =>  p_ade_attribute6
      ,p_ade_attribute7                 =>  p_ade_attribute7
      ,p_ade_attribute8                 =>  p_ade_attribute8
      ,p_ade_attribute9                 =>  p_ade_attribute9
      ,p_ade_attribute10                =>  p_ade_attribute10
      ,p_ade_attribute11                =>  p_ade_attribute11
      ,p_ade_attribute12                =>  p_ade_attribute12
      ,p_ade_attribute13                =>  p_ade_attribute13
      ,p_ade_attribute14                =>  p_ade_attribute14
      ,p_ade_attribute15                =>  p_ade_attribute15
      ,p_ade_attribute16                =>  p_ade_attribute16
      ,p_ade_attribute17                =>  p_ade_attribute17
      ,p_ade_attribute18                =>  p_ade_attribute18
      ,p_ade_attribute19                =>  p_ade_attribute19
      ,p_ade_attribute20                =>  p_ade_attribute20
      ,p_ade_attribute21                =>  p_ade_attribute21
      ,p_ade_attribute22                =>  p_ade_attribute22
      ,p_ade_attribute23                =>  p_ade_attribute23
      ,p_ade_attribute24                =>  p_ade_attribute24
      ,p_ade_attribute25                =>  p_ade_attribute25
      ,p_ade_attribute26                =>  p_ade_attribute26
      ,p_ade_attribute27                =>  p_ade_attribute27
      ,p_ade_attribute28                =>  p_ade_attribute28
      ,p_ade_attribute29                =>  p_ade_attribute29
      ,p_ade_attribute30                =>  p_ade_attribute30
      ,p_object_version_number          =>  l_object_version_number
      ,p_effective_date                      => trunc(p_effective_date)
      );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'CREATE_Apld_Dpnt_Cvg_Prfl'
        ,p_hook_type   => 'AP'
        );
    --
    -- End of API User Hook for the after hook of create_Apld_Dpnt_Cvg_Prfl
    --
  end;
  --
  hr_utility.set_location(l_proc, 60);
  --
  -- When in validation only mode raise the Validate_Enabled exception
  --
  if p_validate then
    raise hr_api.validate_enabled;
  end if;
  --
  -- Set all output arguments
  --
  p_apld_dpnt_cvg_elig_prfl_id := l_apld_dpnt_cvg_elig_prfl_id;
  p_effective_start_date := l_effective_start_date;
  p_effective_end_date := l_effective_end_date;
  p_object_version_number := l_object_version_number;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 70);
  --
exception
  --
  when hr_api.validate_enabled then
    --
    -- As the Validate_Enabled exception has been raised
    -- we must rollback to the savepoint
    --
    ROLLBACK TO create_Apld_Dpnt_Cvg_Prfl;
    --
    -- Only set output warning arguments
    -- (Any key or derived arguments must be set to null
    -- when validation only mode is being used.)
    --
    p_apld_dpnt_cvg_elig_prfl_id := null;
    p_effective_start_date := null;
    p_effective_end_date := null;
    p_object_version_number  := null;
    hr_utility.set_location(' Leaving:'||l_proc, 80);
    --
  when others then
    --
    -- A validation or unexpected error has occured
    --
    ROLLBACK TO create_Apld_Dpnt_Cvg_Prfl;
    p_apld_dpnt_cvg_elig_prfl_id := null;
    p_effective_start_date := null;
    p_effective_end_date := null;
    p_object_version_number := null ;
    hr_utility.set_location(' Leaving:'||l_proc, 90);
    raise;
    --
end create_Apld_Dpnt_Cvg_Prfl;
-- ----------------------------------------------------------------------------
-- |------------------------< update_Apld_Dpnt_Cvg_Prfl >--- ------------------|
-- ----------------------------------------------------------------------------
--
procedure update_Apld_Dpnt_Cvg_Prfl
  (p_validate                       in  boolean   default false
  ,p_apld_dpnt_cvg_elig_prfl_id     in  number
  ,p_effective_start_date           out nocopy date
  ,p_effective_end_date             out nocopy date
  ,p_business_group_id              in  number    default hr_api.g_number
  ,p_apld_dpnt_cvg_elig_rl          in  number    default hr_api.g_number
  ,p_mndtry_flag                    in  varchar2  default hr_api.g_varchar2
  ,p_dpnt_cvg_eligy_prfl_id         in  number    default hr_api.g_number
  ,p_pgm_id                         in  number    default hr_api.g_number
  ,p_pl_id                          in  number    default hr_api.g_number
  ,p_ptip_id                        in  number    default hr_api.g_number
  ,p_ade_attribute_category         in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute1                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute2                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute3                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute4                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute5                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute6                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute7                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute8                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute9                 in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute10                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute11                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute12                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute13                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute14                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute15                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute16                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute17                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute18                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute19                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute20                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute21                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute22                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute23                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute24                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute25                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute26                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute27                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute28                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute29                in  varchar2  default hr_api.g_varchar2
  ,p_ade_attribute30                in  varchar2  default hr_api.g_varchar2
  ,p_object_version_number          in out nocopy number
  ,p_effective_date                 in  date
  ,p_datetrack_mode                 in  varchar2
  ) is
  --
  -- Declare cursors and local variables
  --
  l_proc varchar2(72) := g_package||'update_Apld_Dpnt_Cvg_Prfl';
  l_object_version_number ben_apld_dpnt_cvg_elig_prfl_f.object_version_number%TYPE;
  l_effective_start_date ben_apld_dpnt_cvg_elig_prfl_f.effective_start_date%TYPE;
  l_effective_end_date ben_apld_dpnt_cvg_elig_prfl_f.effective_end_date%TYPE;
  --
begin
  --
  hr_utility.set_location('Entering:'|| l_proc, 10);
  --
  -- Issue a savepoint if operating in validation only mode
  --
  savepoint update_Apld_Dpnt_Cvg_Prfl;
  --
  hr_utility.set_location(l_proc, 20);
  --
  -- Process Logic
  --
  l_object_version_number := p_object_version_number;
  --
  begin
    --
    -- Start of API User Hook for the before hook of update_Apld_Dpnt_Cvg_Prfl
    --
    ben_Apld_Dpnt_Cvg_Prfl_bk2.update_Apld_Dpnt_Cvg_Prfl_b
      (
       p_apld_dpnt_cvg_elig_prfl_id     =>  p_apld_dpnt_cvg_elig_prfl_id
      ,p_business_group_id              =>  p_business_group_id
      ,p_apld_dpnt_cvg_elig_rl          =>  p_apld_dpnt_cvg_elig_rl
      ,p_mndtry_flag                    =>  p_mndtry_flag
      ,p_dpnt_cvg_eligy_prfl_id         =>  p_dpnt_cvg_eligy_prfl_id
      ,p_pgm_id                         =>  p_pgm_id
      ,p_pl_id                          =>  p_pl_id
      ,p_ptip_id                        =>  p_ptip_id
      ,p_ade_attribute_category         =>  p_ade_attribute_category
      ,p_ade_attribute1                 =>  p_ade_attribute1
      ,p_ade_attribute2                 =>  p_ade_attribute2
      ,p_ade_attribute3                 =>  p_ade_attribute3
      ,p_ade_attribute4                 =>  p_ade_attribute4
      ,p_ade_attribute5                 =>  p_ade_attribute5
      ,p_ade_attribute6                 =>  p_ade_attribute6
      ,p_ade_attribute7                 =>  p_ade_attribute7
      ,p_ade_attribute8                 =>  p_ade_attribute8
      ,p_ade_attribute9                 =>  p_ade_attribute9
      ,p_ade_attribute10                =>  p_ade_attribute10
      ,p_ade_attribute11                =>  p_ade_attribute11
      ,p_ade_attribute12                =>  p_ade_attribute12
      ,p_ade_attribute13                =>  p_ade_attribute13
      ,p_ade_attribute14                =>  p_ade_attribute14
      ,p_ade_attribute15                =>  p_ade_attribute15
      ,p_ade_attribute16                =>  p_ade_attribute16
      ,p_ade_attribute17                =>  p_ade_attribute17
      ,p_ade_attribute18                =>  p_ade_attribute18
      ,p_ade_attribute19                =>  p_ade_attribute19
      ,p_ade_attribute20                =>  p_ade_attribute20
      ,p_ade_attribute21                =>  p_ade_attribute21
      ,p_ade_attribute22                =>  p_ade_attribute22
      ,p_ade_attribute23                =>  p_ade_attribute23
      ,p_ade_attribute24                =>  p_ade_attribute24
      ,p_ade_attribute25                =>  p_ade_attribute25
      ,p_ade_attribute26                =>  p_ade_attribute26
      ,p_ade_attribute27                =>  p_ade_attribute27
      ,p_ade_attribute28                =>  p_ade_attribute28
      ,p_ade_attribute29                =>  p_ade_attribute29
      ,p_ade_attribute30                =>  p_ade_attribute30
      ,p_object_version_number          =>  p_object_version_number
    ,p_effective_date                      => trunc(p_effective_date)
    ,p_datetrack_mode                      => p_datetrack_mode
      );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'UPDATE_Apld_Dpnt_Cvg_Prfl'
        ,p_hook_type   => 'BP'
        );
    --
    -- End of API User Hook for the before hook of update_Apld_Dpnt_Cvg_Prfl
    --
  end;
  --
  ben_ade_upd.upd
    (
     p_apld_dpnt_cvg_elig_prfl_id    => p_apld_dpnt_cvg_elig_prfl_id
    ,p_effective_start_date          => l_effective_start_date
    ,p_effective_end_date            => l_effective_end_date
    ,p_business_group_id             => p_business_group_id
    ,p_apld_dpnt_cvg_elig_rl         => p_apld_dpnt_cvg_elig_rl
    ,p_mndtry_flag                   => p_mndtry_flag
    ,p_dpnt_cvg_eligy_prfl_id        => p_dpnt_cvg_eligy_prfl_id
    ,p_pgm_id                        => p_pgm_id
    ,p_pl_id                         => p_pl_id
    ,p_ptip_id                       => p_ptip_id
    ,p_ade_attribute_category        => p_ade_attribute_category
    ,p_ade_attribute1                => p_ade_attribute1
    ,p_ade_attribute2                => p_ade_attribute2
    ,p_ade_attribute3                => p_ade_attribute3
    ,p_ade_attribute4                => p_ade_attribute4
    ,p_ade_attribute5                => p_ade_attribute5
    ,p_ade_attribute6                => p_ade_attribute6
    ,p_ade_attribute7                => p_ade_attribute7
    ,p_ade_attribute8                => p_ade_attribute8
    ,p_ade_attribute9                => p_ade_attribute9
    ,p_ade_attribute10               => p_ade_attribute10
    ,p_ade_attribute11               => p_ade_attribute11
    ,p_ade_attribute12               => p_ade_attribute12
    ,p_ade_attribute13               => p_ade_attribute13
    ,p_ade_attribute14               => p_ade_attribute14
    ,p_ade_attribute15               => p_ade_attribute15
    ,p_ade_attribute16               => p_ade_attribute16
    ,p_ade_attribute17               => p_ade_attribute17
    ,p_ade_attribute18               => p_ade_attribute18
    ,p_ade_attribute19               => p_ade_attribute19
    ,p_ade_attribute20               => p_ade_attribute20
    ,p_ade_attribute21               => p_ade_attribute21
    ,p_ade_attribute22               => p_ade_attribute22
    ,p_ade_attribute23               => p_ade_attribute23
    ,p_ade_attribute24               => p_ade_attribute24
    ,p_ade_attribute25               => p_ade_attribute25
    ,p_ade_attribute26               => p_ade_attribute26
    ,p_ade_attribute27               => p_ade_attribute27
    ,p_ade_attribute28               => p_ade_attribute28
    ,p_ade_attribute29               => p_ade_attribute29
    ,p_ade_attribute30               => p_ade_attribute30
    ,p_object_version_number         => l_object_version_number
    ,p_effective_date                => trunc(p_effective_date)
    ,p_datetrack_mode                => p_datetrack_mode
    );
  --
  begin
    --
    -- Start of API User Hook for the after hook of update_Apld_Dpnt_Cvg_Prfl
    --
    ben_Apld_Dpnt_Cvg_Prfl_bk2.update_Apld_Dpnt_Cvg_Prfl_a
      (
       p_apld_dpnt_cvg_elig_prfl_id     =>  p_apld_dpnt_cvg_elig_prfl_id
      ,p_effective_start_date           =>  l_effective_start_date
      ,p_effective_end_date             =>  l_effective_end_date
      ,p_business_group_id              =>  p_business_group_id
      ,p_apld_dpnt_cvg_elig_rl          =>  p_apld_dpnt_cvg_elig_rl
      ,p_mndtry_flag                    =>  p_mndtry_flag
      ,p_dpnt_cvg_eligy_prfl_id         =>  p_dpnt_cvg_eligy_prfl_id
      ,p_pgm_id                         =>  p_pgm_id
      ,p_pl_id                          =>  p_pl_id
      ,p_ptip_id                        =>  p_ptip_id
      ,p_ade_attribute_category         =>  p_ade_attribute_category
      ,p_ade_attribute1                 =>  p_ade_attribute1
      ,p_ade_attribute2                 =>  p_ade_attribute2
      ,p_ade_attribute3                 =>  p_ade_attribute3
      ,p_ade_attribute4                 =>  p_ade_attribute4
      ,p_ade_attribute5                 =>  p_ade_attribute5
      ,p_ade_attribute6                 =>  p_ade_attribute6
      ,p_ade_attribute7                 =>  p_ade_attribute7
      ,p_ade_attribute8                 =>  p_ade_attribute8
      ,p_ade_attribute9                 =>  p_ade_attribute9
      ,p_ade_attribute10                =>  p_ade_attribute10
      ,p_ade_attribute11                =>  p_ade_attribute11
      ,p_ade_attribute12                =>  p_ade_attribute12
      ,p_ade_attribute13                =>  p_ade_attribute13
      ,p_ade_attribute14                =>  p_ade_attribute14
      ,p_ade_attribute15                =>  p_ade_attribute15
      ,p_ade_attribute16                =>  p_ade_attribute16
      ,p_ade_attribute17                =>  p_ade_attribute17
      ,p_ade_attribute18                =>  p_ade_attribute18
      ,p_ade_attribute19                =>  p_ade_attribute19
      ,p_ade_attribute20                =>  p_ade_attribute20
      ,p_ade_attribute21                =>  p_ade_attribute21
      ,p_ade_attribute22                =>  p_ade_attribute22
      ,p_ade_attribute23                =>  p_ade_attribute23
      ,p_ade_attribute24                =>  p_ade_attribute24
      ,p_ade_attribute25                =>  p_ade_attribute25
      ,p_ade_attribute26                =>  p_ade_attribute26
      ,p_ade_attribute27                =>  p_ade_attribute27
      ,p_ade_attribute28                =>  p_ade_attribute28
      ,p_ade_attribute29                =>  p_ade_attribute29
      ,p_ade_attribute30                =>  p_ade_attribute30
      ,p_object_version_number          =>  l_object_version_number
      ,p_effective_date                     => trunc(p_effective_date)
      ,p_datetrack_mode                     => p_datetrack_mode
      );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'UPDATE_Apld_Dpnt_Cvg_Prfl'
        ,p_hook_type   => 'AP'
        );
    --
    -- End of API User Hook for the after hook of update_Apld_Dpnt_Cvg_Prfl
    --
  end;
  --
  hr_utility.set_location(l_proc, 60);
  --
  -- When in validation only mode raise the Validate_Enabled exception
  --
  if p_validate then
    raise hr_api.validate_enabled;
  end if;
  --
  -- Set all output arguments
  --
  p_object_version_number := l_object_version_number;
  p_effective_start_date := l_effective_start_date;
  p_effective_end_date := l_effective_end_date;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 70);
  --
exception
  --
  when hr_api.validate_enabled then
    --
    -- As the Validate_Enabled exception has been raised
    -- we must rollback to the savepoint
    --
    ROLLBACK TO update_Apld_Dpnt_Cvg_Prfl;
    --
    -- Only set output warning arguments
    -- (Any key or derived arguments must be set to null
    -- when validation only mode is being used.)
    --
    hr_utility.set_location(' Leaving:'||l_proc, 80);
    --
  when others then
    --
    -- A validation or unexpected error has occured
    --
    ROLLBACK TO update_Apld_Dpnt_Cvg_Prfl;
    p_effective_start_date := null;
    p_effective_end_date := null;
    p_object_version_number := l_object_version_number ;
    hr_utility.set_location(' Leaving:'||l_proc, 90);
    raise;
    --
end update_Apld_Dpnt_Cvg_Prfl;
-- ----------------------------------------------------------------------------
-- |------------------------< delete_Apld_Dpnt_Cvg_Prfl >----------------------|
-- ----------------------------------------------------------------------------
--
procedure delete_Apld_Dpnt_Cvg_Prfl
  (p_validate                       in  boolean  default false
  ,p_apld_dpnt_cvg_elig_prfl_id     in  number
  ,p_effective_start_date           out nocopy date
  ,p_effective_end_date             out nocopy date
  ,p_object_version_number          in out nocopy number
  ,p_effective_date                 in  date
  ,p_datetrack_mode                 in  varchar2
  ) is
  --
  -- Declare cursors and local variables
  --
  l_proc varchar2(72) := g_package||'update_Apld_Dpnt_Cvg_Prfl';
  l_object_version_number ben_apld_dpnt_cvg_elig_prfl_f.object_version_number%TYPE;
  l_effective_start_date ben_apld_dpnt_cvg_elig_prfl_f.effective_start_date%TYPE;
  l_effective_end_date ben_apld_dpnt_cvg_elig_prfl_f.effective_end_date%TYPE;
  --
begin
  --
  hr_utility.set_location('Entering:'|| l_proc, 10);
  --
  -- Issue a savepoint if operating in validation only mode
  --
  savepoint delete_Apld_Dpnt_Cvg_Prfl;
  --
  hr_utility.set_location(l_proc, 20);
  --
  -- Process Logic
  --
  l_object_version_number := p_object_version_number;
  --
  --
  begin
    --
    -- Start of API User Hook for the before hook of delete_Apld_Dpnt_Cvg_Prfl
    --
    ben_Apld_Dpnt_Cvg_Prfl_bk3.delete_Apld_Dpnt_Cvg_Prfl_b
      (
       p_apld_dpnt_cvg_elig_prfl_id     =>  p_apld_dpnt_cvg_elig_prfl_id
      ,p_object_version_number          =>  p_object_version_number
    ,p_effective_date                      => trunc(p_effective_date)
    ,p_datetrack_mode                      => p_datetrack_mode
      );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'DELETE_Apld_Dpnt_Cvg_Prfl'
        ,p_hook_type   => 'BP'
        );
    --
    -- End of API User Hook for the before hook of delete_Apld_Dpnt_Cvg_Prfl
    --
  end;
  --
  ben_ade_del.del
    (
     p_apld_dpnt_cvg_elig_prfl_id    => p_apld_dpnt_cvg_elig_prfl_id
    ,p_effective_start_date          => l_effective_start_date
    ,p_effective_end_date            => l_effective_end_date
    ,p_object_version_number         => l_object_version_number
    ,p_effective_date                => p_effective_date
    ,p_datetrack_mode                => p_datetrack_mode
    );
  --
  begin
    --
    -- Start of API User Hook for the after hook of delete_Apld_Dpnt_Cvg_Prfl
    --
    ben_Apld_Dpnt_Cvg_Prfl_bk3.delete_Apld_Dpnt_Cvg_Prfl_a
      (
       p_apld_dpnt_cvg_elig_prfl_id     =>  p_apld_dpnt_cvg_elig_prfl_id
      ,p_effective_start_date           =>  l_effective_start_date
      ,p_effective_end_date             =>  l_effective_end_date
      ,p_object_version_number          =>  l_object_version_number
    ,p_effective_date                      => trunc(p_effective_date)
    ,p_datetrack_mode                      => p_datetrack_mode
      );
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'DELETE_Apld_Dpnt_Cvg_Prfl'
        ,p_hook_type   => 'AP'
        );
    --
    -- End of API User Hook for the after hook of delete_Apld_Dpnt_Cvg_Prfl
    --
  end;
  --
  hr_utility.set_location(l_proc, 60);
  --
  -- When in validation only mode raise the Validate_Enabled exception
  --
  if p_validate then
    raise hr_api.validate_enabled;
  end if;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 70);
  --
exception
  --
  when hr_api.validate_enabled then
    --
    -- As the Validate_Enabled exception has been raised
    -- we must rollback to the savepoint
    --
    ROLLBACK TO delete_Apld_Dpnt_Cvg_Prfl;
    --
    -- Only set output warning arguments
    -- (Any key or derived arguments must be set to null
    -- when validation only mode is being used.)
    --
    p_effective_start_date := null;
    p_effective_end_date := null;
    --
  when others then
    --
    -- A validation or unexpected error has occured
    --
    ROLLBACK TO delete_Apld_Dpnt_Cvg_Prfl;
    -- NOCOPY Changes
    p_effective_start_date := null;
    p_effective_end_date := null;
    p_object_version_number := l_object_version_number ;
    hr_utility.set_location(' Leaving:'||l_proc, 90);
    raise;
    --
end delete_Apld_Dpnt_Cvg_Prfl;
--
-- ----------------------------------------------------------------------------
-- |-------------------------------< lck >------------------------------------|
-- ----------------------------------------------------------------------------
--
procedure lck
  (
   p_apld_dpnt_cvg_elig_prfl_id                   in     number
  ,p_object_version_number          in     number
  ,p_effective_date                 in     date
  ,p_datetrack_mode                 in     varchar2
  ,p_validation_start_date          out nocopy    date
  ,p_validation_end_date            out nocopy    date
  ) is
  --
  --
  -- Declare cursors and local variables
  --
  l_proc varchar2(72) := g_package||'lck';
  l_validation_start_date date;
  l_validation_end_date date;
  --
begin
  --
  hr_utility.set_location('Entering:'|| l_proc, 10);
  --
  ben_ade_shd.lck
    (
      p_apld_dpnt_cvg_elig_prfl_id                 => p_apld_dpnt_cvg_elig_prfl_id
     ,p_validation_start_date      => l_validation_start_date
     ,p_validation_end_date        => l_validation_end_date
     ,p_object_version_number      => p_object_version_number
     ,p_effective_date             => p_effective_date
     ,p_datetrack_mode             => p_datetrack_mode
    );
  --
  hr_utility.set_location(' Leaving:'||l_proc, 70);
  --
end lck;
--
end ben_Apld_Dpnt_Cvg_Prfl_api;

/
