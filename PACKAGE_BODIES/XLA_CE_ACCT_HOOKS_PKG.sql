--------------------------------------------------------
--  DDL for Package Body XLA_CE_ACCT_HOOKS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_CE_ACCT_HOOKS_PKG" AS
-- $Header: xlaapceh.pkb 120.1 2006/06/28 05:50:52 vinaykum noship $
/*===========================================================================+
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.xla_ce_acct_hooks_pkg';
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_ce_acct_hooks_pkg.trace');
END trace;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_ce_acct_hooks_pkg.print_logfile');
...
BEGIN
   ...
   WHEN  p_event_name = 'preaccounting'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call preaccounting API for Cash Managment'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ce_xla_acct_events_pkg.preaccounting
   ...
   WHEN p_event_name = 'extract'        THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call extract API for Cash Managment'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ce_xla_acct_events_pkg.extract
            ( p_application_id     => p_application_id
             ,p_accounting_mode    => p_accounting_mode);
   ...
   WHEN p_event_name = 'postprocessing'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call postprocessing API for Cash Managment'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ce_xla_acct_events_pkg.postprocessing
            ( p_application_id     => p_application_id
             ,p_accounting_mode    => p_accounting_mode);
   ...
   WHEN p_event_name = 'postaccounting'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call postaccounting API for Cash Managment'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      ce_xla_acct_events_pkg.postaccounting
            ( p_application_id     => p_application_id
             ,p_ledger_id          => p_ledger_id
             ,p_process_category   => p_process_category
             ,p_end_date           => p_end_date
...
WHEN xla_exceptions_pkg.application_exception THEN
   ...
   xla_exceptions_pkg.raise_message
      ...
      ,p_token_1        => 'LOCATION'
      ,p_value_1        => 'xla_ce_acct_hooks_pkg.main'
      ,p_token_2        => 'ERROR'
