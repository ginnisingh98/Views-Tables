--------------------------------------------------------
--  DDL for Package Body CS_SR_PROBLEM_CODE_MAPPING_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CS_SR_PROBLEM_CODE_MAPPING_PKG" as
/* $Header: cspcmapb.pls 115.7 2003/09/18 01:03:36 rhungund noship $ */
...
procedure INSERT_ROW (
  ...
  is
   ...
   cursor c1 is
   select cs_sr_prob_code_mapping_s.nextval
   from dual;

  BEGIN
   if ( PX_PROBLEM_MAP_ID IS NULL ) OR ( PX_PROBLEM_MAP_ID = FND_API.G_MISS_NUM) THEN
      open c1;
      fetch c1 into PX_PROBLEM_MAP_ID;
      close c1;
   end if;

  insert into CS_SR_PROB_CODE_MAPPING (
    PROBLEM_MAP_ID,
    INCIDENT_TYPE_ID,
    INVENTORY_ITEM_ID,
    ORGANIZATION_ID,
...
procedure LOCK_ROW (
  ...
  cursor c1 is select
    ...
    where PROBLEM_MAP_ID = P_PROBLEM_MAP_ID
    for update of PROBLEM_MAP_ID nowait;
begin
  for tlinfo in c1 loop
      if (    (tlinfo.PROBLEM_MAP_ID = P_PROBLEM_MAP_ID)
...
begin
  update CS_SR_PROB_CODE_MAPPING set
...
begin
  delete from CS_SR_PROB_CODE_MAPPING
  where PROBLEM_MAP_ID = P_PROBLEM_MAP_ID;
