--------------------------------------------------------
--  DDL for Package Body BEN_PCT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pct_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
...
--
Procedure chk_pl_gd_r_svc_ctfn_id(p_pl_gd_r_svc_ctfn_id         in number,
                                  p_effective_date              in date,
                                  p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pl_gd_r_svc_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_pct_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pl_gd_r_svc_ctfn_id         => p_pl_gd_r_svc_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pl_gd_r_svc_ctfn_id,hr_api.g_number)
     <>  ben_pct_shd.g_old_rec.pl_gd_r_svc_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_pct_shd.constraint_error('BEN_PL_GD_R_SVC_CTFN_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pl_gd_r_svc_ctfn_id is not null then
      --
...
-- In Parameters
--   pl_gd_r_svc_ctfn_id PK of record being inserted or updated.
--   lack_ctfn_deny_rmbmt_rl Value of formula rule id.
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_pl_gd_r_svc_ctfn_id         in number,
  ...
  cursor c1 is
    ...
    and    ff.formula_type_id = -43 -- BEN_LACK_CTFN_DENY_RMBMT
...
-- In Parameters
--   pl_gd_r_svc_ctfn_id PK of record being inserted or updated.
--   rmbmt_ctfn_typ_cd Value of lookup code.
--   effective_date effective date
--   object_version_number Object version number of record being
--                         inserted or updated.
--
...
--   Internal table handler use only.
--
Procedure chk_rmbmt_ctfn_typ_cd(p_pl_gd_r_svc_ctfn_id         in number,
                                p_rmbmt_ctfn_typ_cd           in varchar2,
                                p_effective_date              in date,
                                p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_rmbmt_ctfn_typ_cd';
  l_api_updating boolean;
  --
Begin
  --
  ...
  --
  if (l_api_updating
      and p_rmbmt_ctfn_typ_cd
      <> nvl(ben_pct_shd.g_old_rec.rmbmt_ctfn_typ_cd,hr_api.g_varchar2)
    ...
    if hr_api.not_exists_in_hr_lookups
          (p_lookup_type    => 'BEN_REIMBMT_CTFN_TYP',
           p_lookup_code    => p_rmbmt_ctfn_typ_cd,
...
-- In Parameters
--   pl_gd_r_svc_ctfn_id PK of record being inserted or updated.
--   lack_ctfn_deny_rmbmt_flag Value of lookup code.
--   effective_date effective date
--   object_version_number Object version number of record being
--                         inserted or updated.
--
...
--   Internal table handler use only.
--
Procedure chk_rqd_flag(p_pl_gd_r_svc_ctfn_id         in number,
...
Begin
  ...
  if (l_api_updating
      and p_rqd_flag
      <> nvl(ben_pct_shd.g_old_rec.rqd_flag,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_pfd_flag(p_pl_gd_r_svc_ctfn_id         in number,
...
Begin
  ...
  if (l_api_updating
      and p_pfd_flag
      <> nvl(ben_pct_shd.g_old_rec.pfd_flag,hr_api.g_varchar2)
      or not l_api_updating)
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    End If;
    If ((nvl(p_pl_gd_or_svc_id, hr_api.g_number) <> hr_api.g_number) and
      NOT (dt_api.check_min_max_dates
...
function return_legislation_code
  (p_pl_gd_r_svc_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pl_gd_r_svc_ctfn_f b
    where b.pl_gd_r_svc_ctfn_id      = p_pl_gd_r_svc_ctfn_id
