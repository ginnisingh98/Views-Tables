--------------------------------------------------------
--  DDL for Package Body OTA_TMT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_TMT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ota_tmt_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'OTA_TP_MEASUREMENT_BUDGET') Then
    fnd_message.set_name('OTA', 'OTA_13805_TMT_INV_BUDGET');
    fnd_message.set_token('PROCEDURE', l_proc);
    fnd_message.set_token('STEP','5');
    fnd_message.raise_error;
  ElsIf (p_constraint_name = 'OTA_TP_MEASUREMENT_COST') Then
    fnd_message.set_name('OTA', 'OTA_13808_TMT_INV_COST');
    fnd_message.set_token('PROCEDURE', l_proc);
    fnd_message.set_token('STEP','10');
    fnd_message.raise_error;
  ElsIf (p_constraint_name = 'OTA_TP_MEASUREMENT_TYPES_PK') Then
    fnd_message.set_name('PAY', 'HR_6153_ALL_PROCEDURE_FAIL');
    fnd_message.set_token('PROCEDURE', l_proc);
    fnd_message.set_token('STEP','15');
    fnd_message.raise_error;
  ElsIf (p_constraint_name = 'OTA_TP_MEASUREMENT_TYPES_UK2') Then
...
Begin
  --
  If (p_tp_measurement_type_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_tp_measurement_type_id
        = ota_tmt_shd.g_old_rec.tp_measurement_type_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ota_tmt_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ota_tmt_shd.g_old_rec.object_version_number) Then
        fnd_message.set_name(801, 'HR_7155_OBJECT_INVALID');
        fnd_message.raise_error;
      End If;
      l_fct_ret := true;
    End If;
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'ota_tp_measurement_types');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  l_rec.tp_measurement_code              := p_tp_measurement_code;
  l_rec.unit                             := p_unit;
  l_rec.reporting_sequence               := p_reporting_sequence;
  l_rec.budget_level                     := p_budget_level;
  l_rec.cost_level                       := p_cost_level;
  l_rec.many_budget_values_flag          := p_many_budget_values_flag;
  l_rec.item_type_usage_id               := p_item_type_usage_id;
