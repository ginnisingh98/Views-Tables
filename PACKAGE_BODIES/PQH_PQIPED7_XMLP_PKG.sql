--------------------------------------------------------
--  DDL for Package Body PQH_PQIPED7_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_PQIPED7_XMLP_PKG" AS
/* $Header: PQIPED7B.pls 120.3 2008/04/16 11:16:39 amakrish noship $ */
     ...
     AND PPP.CHANGE_DATE =
      ...
      CURSOR GET_LINE2_COUNTS IS
       ...
       AND PPP.CHANGE_DATE =
        ...
        CURSOR GET_TMR_LINE2_COUNTS(C_LOOKUP_CODE IN VARCHAR2) IS
        SELECT COUNT(DECODE(PEI.PEI_INFORMATION5,    '2',    DECODE(PEO.SEX,    'M',    1,    NULL),    NULL)) BNHMEN,
           COUNT(DECODE(PEI.PEI_INFORMATION5,    '2',    DECODE(PEO.SEX,    'F',    1,    NULL),    NULL)) BNHWMEN,
           COUNT(DECODE(PEI.PEI_INFORMATION5,    '6',    DECODE(PEO.SEX,    'M',    1,    NULL),    NULL)) AM_ALMEN,
           COUNT(DECODE(PEI.PEI_INFORMATION5,    '6',    DECODE(PEO.SEX,    'F',    1,    NULL),    NULL)) AM_ALWMEN,
	...
	cp_pt	:= l_pt;
            FOR I IN GET_LINE2_COUNTS
             LOOP
              ...
              FOR J IN GET_TMR_LINE2_COUNTS(sc)
               LOOP
