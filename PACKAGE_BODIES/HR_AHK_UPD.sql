--------------------------------------------------------
--  DDL for Package Body HR_AHK_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_AHK_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_ahk_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy hr_ahk_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.api_module_id = hr_api.g_number) then
    p_rec.api_module_id :=
    hr_ahk_shd.g_old_rec.api_module_id;
  End If;
  If (p_rec.api_hook_type = hr_api.g_varchar2) then
    p_rec.api_hook_type :=
    hr_ahk_shd.g_old_rec.api_hook_type;
  End If;
  If (p_rec.hook_package = hr_api.g_varchar2) then
    p_rec.hook_package :=
    hr_ahk_shd.g_old_rec.hook_package;
  End If;
  If (p_rec.hook_procedure = hr_api.g_varchar2) then
    p_rec.hook_procedure :=
    hr_ahk_shd.g_old_rec.hook_procedure;
  End If;
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    p_rec.legislation_code :=
    hr_ahk_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.legislation_package = hr_api.g_varchar2) then
    p_rec.legislation_package :=
    hr_ahk_shd.g_old_rec.legislation_package;
  End If;
  If (p_rec.legislation_function = hr_api.g_varchar2) then
    p_rec.legislation_function :=
    hr_ahk_shd.g_old_rec.legislation_function;
  End If;
  If (p_rec.encoded_error = hr_api.g_varchar2) then
    p_rec.encoded_error :=
    hr_ahk_shd.g_old_rec.encoded_error;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
