--------------------------------------------------------
--  DDL for Package Body CS_PARTY_ROLE_GROUPS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CS_PARTY_ROLE_GROUPS_PKG" as
/* $Header: csxptygb.pls 120.1 2005/08/19 10:27 allau noship $ */
...
begin
  insert into CS_PARTY_ROLE_GROUPS_B (
  ...
  insert into CS_PARTY_ROLE_GROUPS_TL (
    CREATION_DATE,
    CREATED_BY,
    LAST_UPDATE_DATE,
    LAST_UPDATED_BY,
  ...
  and not exists
    (select NULL
    from CS_PARTY_ROLE_GROUPS_TL T
    where T.PARTY_ROLE_GROUP_CODE = PX_PARTY_ROLE_GROUP_CODE
...
begin
  update CS_PARTY_ROLE_GROUPS_B set
  ...
  update CS_PARTY_ROLE_GROUPS_TL set
    NAME = P_NAME,
...
begin
  delete from CS_PARTY_ROLE_GROUPS_TL
  where PARTY_ROLE_GROUP_CODE = P_PARTY_ROLE_GROUP_CODE;
  ...
  delete from CS_PARTY_ROLE_GROUPS_B
  where PARTY_ROLE_GROUP_CODE = P_PARTY_ROLE_GROUP_CODE;
...
is
begin
  delete from CS_PARTY_ROLE_GROUPS_TL T
  where not exists
    (select NULL
    from CS_PARTY_ROLE_GROUPS_B B
    where B.PARTY_ROLE_GROUP_CODE = T.PARTY_ROLE_GROUP_CODE
    );

  update CS_PARTY_ROLE_GROUPS_TL T set (
      NAME,
      DESCRIPTION
    ) = (select
      B.NAME,
      B.DESCRIPTION
    from CS_PARTY_ROLE_GROUPS_TL B
    where B.PARTY_ROLE_GROUP_CODE = T.PARTY_ROLE_GROUP_CODE
    and B.LANGUAGE = T.SOURCE_LANG)
  where (
      T.PARTY_ROLE_GROUP_CODE,
      T.LANGUAGE
  ) in (select
      SUBT.PARTY_ROLE_GROUP_CODE,
      SUBT.LANGUAGE
    from CS_PARTY_ROLE_GROUPS_TL SUBB, CS_PARTY_ROLE_GROUPS_TL SUBT
    where SUBB.PARTY_ROLE_GROUP_CODE = SUBT.PARTY_ROLE_GROUP_CODE
  ...
  insert into CS_PARTY_ROLE_GROUPS_TL (
    ...
    B.LAST_UPDATED_BY,
    B.LAST_UPDATE_LOGIN,
    B.PARTY_ROLE_GROUP_CODE,
    B.NAME,
    B.DESCRIPTION,
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from CS_PARTY_ROLE_GROUPS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from CS_PARTY_ROLE_GROUPS_TL T
    where T.PARTY_ROLE_GROUP_CODE = B.PARTY_ROLE_GROUP_CODE
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;
...
procedure TRANSLATE_ROW ( X_PARTY_ROLE_GROUP_CODE  in  varchar2,
                          X_NAME in varchar2,
...
update CS_PARTY_ROLE_GROUPS_TL set
 ...
 last_update_login = 0,
 source_lang = userenv('LANG')
 where PARTY_ROLE_GROUP_CODE = x_PARTY_ROLE_GROUP_CODE
 and userenv('LANG') in (LANGUAGE,SOURCE_LANG);

end TRANSLATE_ROW;

end CS_PARTY_ROLE_GROUPS_PKG;
