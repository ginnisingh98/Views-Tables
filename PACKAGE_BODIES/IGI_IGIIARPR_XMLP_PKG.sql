--------------------------------------------------------
--  DDL for Package Body IGI_IGIIARPR_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_IGIIARPR_XMLP_PKG" AS
/* $Header: IGIIARPRB.pls 120.0.12010000.3 2008/08/21 11:49:16 sharoy ship $ */
...
WHERE  book_Type_code = p_book_type_code;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RP_REPORT_NAME := 'Inflation Accounting Asset Revaluation Preview Detail Report:';
      /*SRW.MESSAGE(98
  ...
  BEGIN
    FND_PROFILE.GET('IGI_IAC_REVAL_TOLERANCE'
                   ,CF_TOLERANCE_LEVEL);
    RETURN 1;
  ...
  FUNCTION CF_CALC_TOLERANCEFORMULA(S_A_B_REVAL_COST IN NUMBER
                                   ,S_A_A_REVAL_COST IN NUMBER) RETURN NUMBER IS
    GAP NUMBER;
  BEGIN
    CF_B_REVAL_COST := S_A_B_REVAL_COST;
    CF_A_REVAL_COST := S_A_A_REVAL_COST;
    IF CF_TOLERANCE_LEVEL IS NOT NULL AND CF_B_REVAL_COST <> 0 THEN
      GAP := ABS(((NVL(CF_A_REVAL_COST
                    ,0) - NVL(CF_B_REVAL_COST
                    ,0)) / NVL(CF_B_REVAL_COST
