--------------------------------------------------------
--  DDL for Package Body GR_EXPOSURE_TYPES_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_EXPOSURE_TYPES_B_PKG" AS
/*$Header: GRHIETB.pls 115.8 2002/10/29 20:46:41 mgrosser ship $*/
...
CURSOR c_lock_exposure_type
 IS
   SELECT	last_update_date
   FROM		gr_exposure_types_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockExposureRcd	  c_lock_exposure_type%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_exposure_type;
   FETCH c_lock_exposure_type INTO LockExposureRcd;
   IF c_lock_exposure_type%NOTFOUND THEN
	  CLOSE c_lock_exposure_type;
...
CURSOR c_get_expo_type_rowid
 IS
   SELECT et.rowid
   FROM	  gr_exposure_types_b et
   WHERE  et.exposure_type_code = p_exposure_type_code;
ExposureTypeRecord			   c_get_expo_type_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_expo_type_rowid;
   FETCH c_get_expo_type_rowid INTO ExposureTypeRecord;
   IF c_get_expo_type_rowid%FOUND THEN
      x_key_exists := 'T';
      x_rowid := ExposureTypeRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_expo_type_rowid;
