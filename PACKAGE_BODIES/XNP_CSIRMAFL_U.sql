--------------------------------------------------------
--  DDL for Package Body XNP_CSIRMAFL_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIRMAFL_U" AS 
PROCEDURE CREATE_MSG  (   XNP$RMA_LINE_ID NUMBER,
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIRMAFL' ;
  ...
  XNP_XML_UTILS.write_leaf_element ( 'RMA_LINE_ID', XNP$RMA_LINE_ID );
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIRMAFL' ) ;
    IF ( XNP$RMA_LINE_ID IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - RMA_LINE_ID' ;
     fnd_message.set_name('XNP','XNP_MISSING_MANDATORY_ATTR');
     fnd_message.set_token('ATTRIBUTE','RMA_LINE_ID' ) ;
     x_error_message := fnd_message.get ; 
      RAISE e_MISSING_MANDATORY_DATA ;
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'RMA_LINE_ID', XNP$RMA_LINE_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIRMAFL' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIRMAFL.create_msg()::' || SQLERRM ;
END ;
PROCEDURE PUBLISH  (   XNP$RMA_LINE_ID NUMBER,
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIRMAFL', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_EVT_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$RMA_LINE_ID=>XNP$RMA_LINE_ID,
    x_msg_header=>l_msg_header,
...
END ;
PROCEDURE SEND  (   XNP$RMA_LINE_ID NUMBER,
  x_message_id OUT  NUMBER,
  x_error_code OUT  NUMBER,
  x_error_message OUT VARCHAR2,  
...
NULL ;
DECLARE
      l_txn_type    varchar2(30) := 'CSIRMAFL';
    ...
    BEGIN
      ...
      xnp_xml_utils.decode(
        p_msg_text => p_msg_text, p_tag => 'RMA_LINE_ID', x_value => l_txn_id);
