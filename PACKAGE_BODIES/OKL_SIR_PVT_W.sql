--------------------------------------------------------
--  DDL for Package Body OKL_SIR_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_SIR_PVT_W" as
  /* $Header: OKLISIRB.pls 120.1 2005/07/14 12:00:09 asawanka noship $ */
  ...
  procedure rosetta_table_copy_in_p2(t out nocopy okl_sir_pvt.sir_tbl_type, a0 JTF_NUMBER_TABLE
    , a1 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).transaction_number := rosetta_g_miss_num_map(a1(indx));
          t(ddindx).srt_code := a2(indx);
          t(ddindx).effective_pre_tax_yield := rosetta_g_miss_num_map(a3(indx));
          t(ddindx).yield_name := a4(indx);
          t(ddindx).index_number := rosetta_g_miss_num_map(a5(indx));
          t(ddindx).effective_after_tax_yield := rosetta_g_miss_num_map(a6(indx));
          t(ddindx).nominal_pre_tax_yield := rosetta_g_miss_num_map(a7(indx));
          t(ddindx).nominal_after_tax_yield := rosetta_g_miss_num_map(a8(indx));
          t(ddindx).stream_interface_attribute01 := a9(indx);
          t(ddindx).stream_interface_attribute02 := a10(indx);
          t(ddindx).stream_interface_attribute03 := a11(indx);
          t(ddindx).stream_interface_attribute04 := a12(indx);
          t(ddindx).stream_interface_attribute05 := a13(indx);
          t(ddindx).stream_interface_attribute06 := a14(indx);
          t(ddindx).stream_interface_attribute07 := a15(indx);
          t(ddindx).stream_interface_attribute08 := a16(indx);
          t(ddindx).stream_interface_attribute09 := a17(indx);
          t(ddindx).stream_interface_attribute10 := a18(indx);
          t(ddindx).stream_interface_attribute11 := a19(indx);
          t(ddindx).stream_interface_attribute12 := a20(indx);
          t(ddindx).stream_interface_attribute13 := a21(indx);
          t(ddindx).stream_interface_attribute14 := a22(indx);
          t(ddindx).stream_interface_attribute15 := a23(indx);
          t(ddindx).object_version_number := rosetta_g_miss_num_map(a24(indx));
          ...
          t(ddindx).implicit_interest_rate := rosetta_g_miss_num_map(a30(indx));
          t(ddindx).date_processed := rosetta_g_miss_date_in_map(a31(indx));
          t(ddindx).request_id := rosetta_g_miss_num_map(a32(indx));
          t(ddindx).program_application_id := rosetta_g_miss_num_map(a33(indx));
          t(ddindx).program_id := rosetta_g_miss_num_map(a34(indx));
  ...
  procedure rosetta_table_copy_out_p2(t okl_sir_pvt.sir_tbl_type, a0 out nocopy JTF_NUMBER_TABLE
    ...
    a35 := JTF_DATE_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a1(indx) := rosetta_g_miss_num_map(t(ddindx).transaction_number);
          a2(indx) := t(ddindx).srt_code;
          a3(indx) := rosetta_g_miss_num_map(t(ddindx).effective_pre_tax_yield);
          a4(indx) := t(ddindx).yield_name;
          a5(indx) := rosetta_g_miss_num_map(t(ddindx).index_number);
          a6(indx) := rosetta_g_miss_num_map(t(ddindx).effective_after_tax_yield);
          a7(indx) := rosetta_g_miss_num_map(t(ddindx).nominal_pre_tax_yield);
          a8(indx) := rosetta_g_miss_num_map(t(ddindx).nominal_after_tax_yield);
          a9(indx) := t(ddindx).stream_interface_attribute01;
          a10(indx) := t(ddindx).stream_interface_attribute02;
          a11(indx) := t(ddindx).stream_interface_attribute03;
          a12(indx) := t(ddindx).stream_interface_attribute04;
          a13(indx) := t(ddindx).stream_interface_attribute05;
          a14(indx) := t(ddindx).stream_interface_attribute06;
          a15(indx) := t(ddindx).stream_interface_attribute07;
          a16(indx) := t(ddindx).stream_interface_attribute08;
          a17(indx) := t(ddindx).stream_interface_attribute09;
          a18(indx) := t(ddindx).stream_interface_attribute10;
          a19(indx) := t(ddindx).stream_interface_attribute11;
          a20(indx) := t(ddindx).stream_interface_attribute12;
          a21(indx) := t(ddindx).stream_interface_attribute13;
          a22(indx) := t(ddindx).stream_interface_attribute14;
          a23(indx) := t(ddindx).stream_interface_attribute15;
          a24(indx) := rosetta_g_miss_num_map(t(ddindx).object_version_number);
          ...
          a30(indx) := rosetta_g_miss_num_map(t(ddindx).implicit_interest_rate);
          a31(indx) := t(ddindx).date_processed;
          a32(indx) := rosetta_g_miss_num_map(t(ddindx).request_id);
          a33(indx) := rosetta_g_miss_num_map(t(ddindx).program_application_id);
          a34(indx) := rosetta_g_miss_num_map(t(ddindx).program_id);
  ...
  procedure rosetta_table_copy_in_p5(t out nocopy okl_sir_pvt.sirv_tbl_type, a0 JTF_NUMBER_TABLE
    , a1 JTF_NUMBER_TABLE
    , a2 JTF_VARCHAR2_TABLE_100
    , a3 JTF_NUMBER_TABLE
    , a4 JTF_VARCHAR2_TABLE_200
  ...
  procedure rosetta_table_copy_out_p5(t okl_sir_pvt.sirv_tbl_type, a0 out nocopy JTF_NUMBER_TABLE
    , a1 out nocopy JTF_NUMBER_TABLE
    , a2 out nocopy JTF_VARCHAR2_TABLE_100
    , a3 out nocopy JTF_NUMBER_TABLE
    , a4 out nocopy JTF_VARCHAR2_TABLE_200
  ...
  as
    ddp_sirv_rec okl_sir_pvt.sirv_rec_type;
    ddx_sirv_rec okl_sir_pvt.sirv_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_sir_pvt.insert_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_sirv_tbl okl_sir_pvt.sirv_tbl_type;
    ddx_sirv_tbl okl_sir_pvt.sirv_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_sir_pvt.lock_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_sir_pvt.update_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_sir_pvt.delete_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_sir_pvt.validate_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
