--------------------------------------------------------
--  DDL for Package Body WIP_DISCRETE_J_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WIP_DISCRETE_J_PKG" as
/* $Header: wipdjthb.pls 120.2 2005/11/12 03:23:47 panagara noship $ */
...
PROCEDURE Insert_Row(X_Rowid            IN OUT NOCOPY VARCHAR2,
		...
		X_requested_start_date		DATE,
                x_serialization_start_op        NUMBER := null) IS
		/*X_Project_Costed		NUMBER) IS*/


CURSOR C(we_id NUMBER) IS SELECT rowid FROM WIP_DISCRETE_JOBS
	    WHERE  wip_entity_id = we_id
	    AND    organization_id = X_Organization_Id;

CURSOR C2 IS SELECT wip_entities_s.nextval FROM DUAL;

CURSOR C3(we_id NUMBER) IS SELECT rowid FROM WIP_ENTITIES
            WHERE  wip_entity_id = we_id
            AND    organization_id = X_Organization_Id;
...
BEGIN
	...
	INSERT INTO WIP_DISCRETE_JOBS(
		...
		X_overhead_variance_account,
                DECODE(X_scheduled_start_date,
                       NULL,
                       X_scheduled_completion_date,
                       X_scheduled_start_date),
		X_date_released,
		DECODE(X_scheduled_completion_date,
		       NULL,
		       X_scheduled_start_date,
		       X_scheduled_completion_date),
		X_date_completed,
		...
		X_po_creation_time,
                ...
                X_due_date,
                X_due_date_penalty,
                X_due_date_tolerance,
		X_requested_start_date,
                x_serialization_start_op);
		/*X_Project_Costed);*/
...
BEGIN
	...
	scheduled_start_date		=	DECODE(X_scheduled_start_date,
							NULL,
							X_scheduled_completion_date,
							X_scheduled_start_date),
	date_released			=	X_Date_Released,
	scheduled_completion_date	=	DECODE(X_scheduled_completion_date,
							NULL,
							X_scheduled_start_date,
							X_scheduled_completion_date),
	...
	priority                        =       X_priority,
	due_date                        =       X_due_date,
	due_date_penalty                =       X_due_date_penalty,
	due_date_tolerance              =       X_due_date_tolerance,
	requested_start_date		=	X_requested_start_date,
        serialization_start_op          =       x_serialization_start_op
