--------------------------------------------------------
--  DDL for Package Body GR_PHRASE_TYPES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_PHRASE_TYPES_PKG" AS
/*$Header: GRHIPTB.pls 115.5 2002/10/28 20:23:01 methomas ship $*/
PROCEDURE Insert_Row
	   			 (p_commit IN VARCHAR2,
				  p_called_by_form IN VARCHAR2,
				  p_phrase_type_code IN VARCHAR2,
				  p_phrase_type_description IN VARCHAR2,
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_phrase_types
   		  	     (phrase_type_code,
   		  	      phrase_type_description,
				  created_by,
				  creation_date,
				  last_updated_by,
				  last_update_date,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_phrase_types
	  SET	 phrase_type_code	 			 = p_phrase_type_code,
	         phrase_type_description 	 	 = p_phrase_type_description,
...
CURSOR c_lock_phrase_types
 IS
   SELECT	*
   FROM		gr_phrase_types
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockTypeRcd	  c_lock_phrase_types%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_phrase_types;
   FETCH c_lock_phrase_types INTO LockTypeRcd;
   IF c_lock_phrase_types%NOTFOUND THEN
	  CLOSE c_lock_phrase_types;
...
CURSOR c_get_phrase_concs
 IS
   SELECT	COUNT(*)
   FROM		gr_phrase_concs
   WHERE	phrase_type_code = p_phrase_type_code;
...
/*   Phrase Concentrations table */
   ...
   l_record_count := 0;
   l_code_block := 'c_get_phrase_concs';
   OPEN c_get_phrase_concs;
   FETCH c_get_phrase_concs INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
	  l_msg_data := l_msg_data || 'gr_phrase_concs, ';
   END IF;
   CLOSE c_get_phrase_concs;
...
/*		  p_phrase_type_code is the phrase_type code to check.
...
CURSOR c_get_phrase_type_rowid
 IS
   SELECT rc.rowid
   FROM	  gr_phrase_types rc
   WHERE  rc.phrase_type_code = p_phrase_type_code;
TypeRecord			   c_get_phrase_type_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_phrase_type_rowid;
   FETCH c_get_phrase_type_rowid INTO TypeRecord;
   IF c_get_phrase_type_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := TypeRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_phrase_type_rowid;
