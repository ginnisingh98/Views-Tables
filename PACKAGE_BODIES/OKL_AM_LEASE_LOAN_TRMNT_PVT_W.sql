--------------------------------------------------------
--  DDL for Package Body OKL_AM_LEASE_LOAN_TRMNT_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_AM_LEASE_LOAN_TRMNT_PVT_W" as
  /* $Header: OKLELLTB.pls 120.7.12010000.5 2008/11/18 10:35:42 sosharma ship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).p_contract_modifier := a2(indx);
          t(ddindx).p_orig_end_date := rosetta_g_miss_date_in_map(a3(indx));
          t(ddindx).p_contract_version := a4(indx);
          t(ddindx).p_termination_date := rosetta_g_miss_date_in_map(a5(indx));
          t(ddindx).p_termination_reason := a6(indx);
          t(ddindx).p_quote_id := rosetta_g_miss_num_map(a7(indx));
          t(ddindx).p_quote_type := a8(indx);
          t(ddindx).p_quote_reason := a9(indx);
          t(ddindx).p_early_termination_yn := a10(indx);
          t(ddindx).p_control_flag := a11(indx);
          t(ddindx).p_recycle_flag := a12(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
    ...
    a12 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a2(indx) := t(ddindx).p_contract_modifier;
          a3(indx) := t(ddindx).p_orig_end_date;
          a4(indx) := t(ddindx).p_contract_version;
          a5(indx) := t(ddindx).p_termination_date;
          a6(indx) := t(ddindx).p_termination_reason;
          a7(indx) := rosetta_g_miss_num_map(t(ddindx).p_quote_id);
          a8(indx) := t(ddindx).p_quote_type;
          a9(indx) := t(ddindx).p_quote_reason;
          a10(indx) := t(ddindx).p_early_termination_yn;
          a11(indx) := t(ddindx).p_control_flag;
          a12(indx) := t(ddindx).p_recycle_flag;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_term_rec okl_am_lease_loan_trmnt_pvt.term_rec_type;
    ddp_tcnv_rec okl_am_lease_loan_trmnt_pvt.tcnv_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_am_lease_loan_trmnt_pvt.lease_loan_termination(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_term_tbl okl_am_lease_loan_trmnt_pvt.term_tbl_type;
    ddp_tcnv_tbl okl_am_lease_loan_trmnt_pvt.tcnv_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
  ...
  begin
    ...
    ddp_termination_date := rosetta_g_miss_date_in_map(p_termination_date);

    -- here's the delegated call to the old PL/SQL routine
    okl_am_lease_loan_trmnt_pvt.process_discount_subsidy(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  begin
    ...
    -- copy data to the local IN or IN-OUT args, if any
    ddp_term_rec.p_contract_id := rosetta_g_miss_num_map(p0_a0);
    ddp_term_rec.p_contract_number := p0_a1;
    ddp_term_rec.p_contract_modifier := p0_a2;
    ddp_term_rec.p_orig_end_date := rosetta_g_miss_date_in_map(p0_a3);
    ddp_term_rec.p_contract_version := p0_a4;
    ddp_term_rec.p_termination_date := rosetta_g_miss_date_in_map(p0_a5);
    ddp_term_rec.p_termination_reason := p0_a6;
    ddp_term_rec.p_quote_id := rosetta_g_miss_num_map(p0_a7);
    ddp_term_rec.p_quote_type := p0_a8;
    ddp_term_rec.p_quote_reason := p0_a9;
    ddp_term_rec.p_early_termination_yn := p0_a10;
    ddp_term_rec.p_control_flag := p0_a11;
    ddp_term_rec.p_recycle_flag := p0_a12;

    ddp_tcnv_rec.id := rosetta_g_miss_num_map(p1_a0);
    ddp_tcnv_rec.object_version_number := rosetta_g_miss_num_map(p1_a1);
    ddp_tcnv_rec.rbr_code := p1_a2;
    ddp_tcnv_rec.rpy_code := p1_a3;
    ddp_tcnv_rec.rvn_code := p1_a4;
    ddp_tcnv_rec.trn_code := p1_a5;
    ddp_tcnv_rec.khr_id_new := rosetta_g_miss_num_map(p1_a6);
    ddp_tcnv_rec.pvn_id := rosetta_g_miss_num_map(p1_a7);
    ddp_tcnv_rec.pdt_id := rosetta_g_miss_num_map(p1_a8);
    ddp_tcnv_rec.qte_id := rosetta_g_miss_num_map(p1_a9);
    ddp_tcnv_rec.aes_id := rosetta_g_miss_num_map(p1_a10);
    ddp_tcnv_rec.code_combination_id := rosetta_g_miss_num_map(p1_a11);
    ddp_tcnv_rec.tax_deductible_local := p1_a12;
    ddp_tcnv_rec.tax_deductible_corporate := p1_a13;
    ddp_tcnv_rec.date_accrual := rosetta_g_miss_date_in_map(p1_a14);
    ddp_tcnv_rec.accrual_status_yn := p1_a15;
    ddp_tcnv_rec.update_status_yn := p1_a16;
    ddp_tcnv_rec.amount := rosetta_g_miss_num_map(p1_a17);
    ddp_tcnv_rec.currency_code := p1_a18;
    ddp_tcnv_rec.attribute_category := p1_a19;
    ddp_tcnv_rec.attribute1 := p1_a20;
    ddp_tcnv_rec.attribute2 := p1_a21;
    ddp_tcnv_rec.attribute3 := p1_a22;
    ddp_tcnv_rec.attribute4 := p1_a23;
    ddp_tcnv_rec.attribute5 := p1_a24;
    ddp_tcnv_rec.attribute6 := p1_a25;
    ddp_tcnv_rec.attribute7 := p1_a26;
    ddp_tcnv_rec.attribute8 := p1_a27;
    ddp_tcnv_rec.attribute9 := p1_a28;
    ddp_tcnv_rec.attribute10 := p1_a29;
    ddp_tcnv_rec.attribute11 := p1_a30;
    ddp_tcnv_rec.attribute12 := p1_a31;
    ddp_tcnv_rec.attribute13 := p1_a32;
    ddp_tcnv_rec.attribute14 := p1_a33;
    ddp_tcnv_rec.attribute15 := p1_a34;
    ddp_tcnv_rec.tcn_type := p1_a35;
    ddp_tcnv_rec.rjn_code := p1_a36;
    ddp_tcnv_rec.party_rel_id1_old := rosetta_g_miss_num_map(p1_a37);
    ddp_tcnv_rec.party_rel_id2_old := p1_a38;
    ddp_tcnv_rec.party_rel_id1_new := rosetta_g_miss_num_map(p1_a39);
    ddp_tcnv_rec.party_rel_id2_new := p1_a40;
    ddp_tcnv_rec.complete_transfer_yn := p1_a41;
    ddp_tcnv_rec.org_id := rosetta_g_miss_num_map(p1_a42);
    ddp_tcnv_rec.khr_id := rosetta_g_miss_num_map(p1_a43);
    ddp_tcnv_rec.request_id := rosetta_g_miss_num_map(p1_a44);
    ddp_tcnv_rec.program_application_id := rosetta_g_miss_num_map(p1_a45);
    ddp_tcnv_rec.khr_id_old := rosetta_g_miss_num_map(p1_a46);
    ddp_tcnv_rec.program_id := rosetta_g_miss_num_map(p1_a47);
    ddp_tcnv_rec.program_update_date := rosetta_g_miss_date_in_map(p1_a48);
    ddp_tcnv_rec.created_by := rosetta_g_miss_num_map(p1_a49);
    ddp_tcnv_rec.creation_date := rosetta_g_miss_date_in_map(p1_a50);
    ddp_tcnv_rec.last_updated_by := rosetta_g_miss_num_map(p1_a51);
    ddp_tcnv_rec.last_update_date := rosetta_g_miss_date_in_map(p1_a52);
    ddp_tcnv_rec.last_update_login := rosetta_g_miss_num_map(p1_a53);
    ddp_tcnv_rec.try_id := rosetta_g_miss_num_map(p1_a54);
    ddp_tcnv_rec.tsu_code := p1_a55;
    ddp_tcnv_rec.set_of_books_id := rosetta_g_miss_num_map(p1_a56);
    ddp_tcnv_rec.description := p1_a57;
    ddp_tcnv_rec.date_transaction_occurred := rosetta_g_miss_date_in_map(p1_a58);
    ddp_tcnv_rec.trx_number := p1_a59;
    ddp_tcnv_rec.tmt_evergreen_yn := p1_a60;
    ddp_tcnv_rec.tmt_close_balances_yn := p1_a61;
    ddp_tcnv_rec.tmt_accounting_entries_yn := p1_a62;
    ddp_tcnv_rec.tmt_cancel_insurance_yn := p1_a63;
    ddp_tcnv_rec.tmt_asset_disposition_yn := p1_a64;
    ddp_tcnv_rec.tmt_amortization_yn := p1_a65;
    ddp_tcnv_rec.tmt_asset_return_yn := p1_a66;
    ddp_tcnv_rec.tmt_contract_updated_yn := p1_a67;
    ddp_tcnv_rec.tmt_recycle_yn := p1_a68;
    ddp_tcnv_rec.tmt_validated_yn := p1_a69;
    ddp_tcnv_rec.tmt_streams_updated_yn := p1_a70;
    ddp_tcnv_rec.accrual_activity := p1_a71;
    ddp_tcnv_rec.tmt_split_asset_yn := p1_a72;
    ddp_tcnv_rec.tmt_generic_flag1_yn := p1_a73;
    ddp_tcnv_rec.tmt_generic_flag2_yn := p1_a74;
    ddp_tcnv_rec.tmt_generic_flag3_yn := p1_a75;
    ddp_tcnv_rec.currency_conversion_type := p1_a76;
    ddp_tcnv_rec.currency_conversion_rate := rosetta_g_miss_num_map(p1_a77);
    ddp_tcnv_rec.currency_conversion_date := rosetta_g_miss_date_in_map(p1_a78);
    ddp_tcnv_rec.chr_id := rosetta_g_miss_num_map(p1_a79);
    ddp_tcnv_rec.source_trx_id := rosetta_g_miss_num_map(p1_a80);
    ddp_tcnv_rec.source_trx_type := p1_a81;
    ddp_tcnv_rec.canceled_date := rosetta_g_miss_date_in_map(p1_a82);
    ddp_tcnv_rec.legal_entity_id := rosetta_g_miss_num_map(p1_a83);
    ddp_tcnv_rec.accrual_reversal_date := rosetta_g_miss_date_in_map(p1_a84);
    ddp_tcnv_rec.accounting_reversal_yn := p1_a85;
    ddp_tcnv_rec.product_name := p1_a86;
    ddp_tcnv_rec.book_classification_code := p1_a87;
    ddp_tcnv_rec.tax_owner_code := p1_a88;
    ddp_tcnv_rec.tmt_status_code := p1_a89;
    ddp_tcnv_rec.representation_name := p1_a90;
    ddp_tcnv_rec.representation_code := p1_a91;
    ddp_tcnv_rec.upgrade_status_flag := p1_a92;
    ddp_tcnv_rec.transaction_date := rosetta_g_miss_date_in_map(p1_a93);




    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := okl_am_lease_loan_trmnt_pvt.check_service_k_int_needed(ddp_term_rec,
      ddp_tcnv_rec,
      p_partial_yn,
      p_asset_id,
      p_source);
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_am_lease_loan_trmnt_pvt.service_k_integration(ddp_term_rec,
      p_transaction_id,
      ddp_transaction_date,
      p_source,
      p_service_integration_needed);
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := okl_am_lease_loan_trmnt_pvt.check_billing_done(p_contract_id,
      p_contract_number,
      p_quote_number,
      ddp_trn_date);
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_am_lease_loan_trmnt_pvt.get_set_quote_dates(p_qte_id,
      ddp_trn_date,
      x_return_status);
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_am_lease_loan_trmnt_pvt.process_adjustments(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := okl_am_lease_loan_trmnt_pvt.check_int_calc_done(p_contract_id,
      p_contract_number,
      p_quote_number,
      p_source,
      ddp_trn_date);
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_am_lease_loan_trmnt_pvt.process_loan_refunds(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
