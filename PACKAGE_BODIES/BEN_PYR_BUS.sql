--------------------------------------------------------
--  DDL for Package Body BEN_PYR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PYR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pyr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_pyrl_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   pyrl_rt_id PK of record being inserted or updated.
...
--
Procedure chk_pyrl_rt_id(p_pyrl_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pyrl_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_pyr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pyrl_rt_id                => p_pyrl_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pyrl_rt_id,hr_api.g_number)
     <>  ben_pyr_shd.g_old_rec.pyrl_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_pyr_shd.constraint_error('BEN_PYRL_RTS_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pyrl_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_pyrl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_pyr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   payroll_id FK of pointing to PAY_ALL_PAYROLLS_F tables.
...
--   Internal table handler use only.
--
Procedure chk_payroll_id(p_payroll_id           in number,
...
Begin
  ...
  if p_payroll_id is not null then
     ...
     if c1%notfound then
        hr_utility.set_message(801,'Payroll_NotExist');
        hr_utility.raise_error;
     end if;
     close c1;
  end if;
...
--
-- In Parameters
--   p_pyrl_rt_id	     PK of record being inserted or updated.
--   p_payroll_id   	     Value of FK.
...
and pyrl_rt_id <> nvl(p_pyrl_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_payroll_id <> nvl(ben_pyr_shd.g_old_rec.payroll_id,hr_api.g_number)
      or not l_api_updating) then
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Payroll criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Payroll criteria',TRUE);
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and pyrl_rt_id   <> nvl(p_pyrl_rt_id  ,-1)
                   --and payroll_id   <> nvl(p_payroll_id  ,-1)
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    End If;
    --
    --Check for Bug 3219987
    --
    If ((nvl(p_vrbl_rt_prfl_id, hr_api.g_number) <> hr_api.g_number) and
    NOT (dt_api.check_min_max_dates
          (p_base_table_name => 'pay_payrolls_f',
...
function return_legislation_code
  (p_pyrl_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pyrl_rt_f b
    where b.pyrl_rt_id      = p_pyrl_rt_id
