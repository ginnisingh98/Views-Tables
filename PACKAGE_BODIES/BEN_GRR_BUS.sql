--------------------------------------------------------
--  DDL for Package Body BEN_GRR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_GRR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_grr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_grade_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   grade_rt_id PK of record being inserted or updated.
...
--
Procedure chk_grade_rt_id(p_grade_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_grade_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_grr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_grade_rt_id                => p_grade_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_grade_rt_id,hr_api.g_number)
     <>  ben_grr_shd.g_old_rec.grade_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_grr_shd.constraint_error('BEN_GRADE_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_grade_rt_id is not null then
      --
...
--
-- In Parameters
--   grade_id FK of pointing to PER_GRADES tables.
...
Begin
  ...
  if p_grade_id is not null then
     ...
     if c1%notfound then
        hr_utility.set_message(801,'Grade_Id_not_exist');
...
--
-- In Parameters
--   p_grade_rt_id	     PK of record being inserted or updated.
--   p_grade_id		     Value of lookup code.
...
and grade_rt_id <> nvl(p_grade_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_grade_id <> nvl(ben_grr_shd.g_old_rec.grade_id,hr_api.g_number)
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Grade already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Grade criteria',TRUE);
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and grade_rt_id <> nvl(p_grade_rt_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_grade_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_grr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
function return_legislation_code
  (p_grade_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_grade_rt_f b
    where b.grade_rt_id      = p_grade_rt_id
