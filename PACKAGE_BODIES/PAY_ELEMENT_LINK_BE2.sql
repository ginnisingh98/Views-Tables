--------------------------------------------------------
--  DDL for Package Body PAY_ELEMENT_LINK_BE2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ELEMENT_LINK_BE2" as 
...
p_cost_concat_segments_in      varchar2,
p_balance_concat_segments_in   varchar2,
p_object_version_number        number,
p_cost_allocation_keyflex_id   number,
p_balancing_keyflex_id         number,
p_cost_concat_segments_out     varchar2,
p_balance_concat_segments_out  varchar2,
...
p_effective_end_date           date) is
  ...
  --
  l_proc varchar2(72):='  pay_element_link_be2.update_element_link_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.pay.api.element_link.update_element_link';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<comment_id>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_comment_id);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<cost_concat_segments_in>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_cost_concat_segments_in);
    l_text:=l_text||'</cost_concat_segments_in>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<balance_concat_segments_in>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_balance_concat_segments_in);
    l_text:=l_text||'</balance_concat_segments_in>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<cost_concat_segments_out>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_cost_concat_segments_out);
    l_text:=l_text||'</cost_concat_segments_out>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<balance_concat_segments_out>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_balance_concat_segments_out);
    l_text:=l_text||'</balance_concat_segments_out>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
...
end update_element_link_a;
end pay_element_link_be2;
