--------------------------------------------------------
--  DDL for Package Body CSP_REP_HIERARCHIES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSP_REP_HIERARCHIES_PKG" as
/* $Header: csptprhb.pls 120.0 2005/05/24 18:26:33 appldev noship $ */
-- Start of Comments
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'CSP_REP_HIERARCHIES_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'csptephb.pls';
...
PROCEDURE Insert_Row(
 ...
 IS
   CURSOR C2 IS SELECT CSP_REP_HIERARCHIES_S1.nextval FROM sys.dual;
BEGIN
   If (px_HIERARCHY_NODE_ID IS NULL) OR (px_HIERARCHY_NODE_ID = FND_API.G_MISS_NUM) then
       OPEN C2;
       FETCH C2 INTO px_HIERARCHY_NODE_ID;
       CLOSE C2;
   End If;
   INSERT INTO CSP_REP_HIERARCHIES(
           HIERARCHY_NODE_ID,
           CREATED_BY,
           CREATION_DATE,
           LAST_UPDATED_BY,
           ...
           decode( p_TOP_NODE_ID, FND_API.G_MISS_NUM, NULL, p_TOP_NODE_ID),
...
PROCEDURE Update_Row(
 ...
 BEGIN
    ...
    SET
              ...
              TOP_NODE_ID = decode( p_TOP_NODE_ID, FND_API.G_MISS_NUM, TOP_NODE_ID, p_TOP_NODE_ID),
...
PROCEDURE Delete_Row(
 ...
 IS
  ...
  Cursor c1(P_HIERARCHY_NODE_ID Number) is
 select HIERARCHY_NODE_ID from CSP_REP_HIERARCHIES
 start with HIERARCHY_NODE_ID = P_HIERARCHY_NODE_ID
 connect by PARENT_NODE_ID = PRIOR HIERARCHY_NODE_ID;
 ...
 BEGIN
   ...
   -- Update Subinventory for the Hierarchy node
   Update csp_sec_inventories
   set   hierarchy_node_id = NULL
   where hierarchy_node_id = p_HIERARCHY_NODE_ID;

   -- Update Subinventory for all the SubHierarchy nodes
   Update csp_sec_inventories
   set   hierarchy_node_id = NULL
   where hierarchy_node_id in (select rh.hierarchy_node_id
                               from   csp_rep_hierarchies rh
						 where  parent_node_id = p_HIERARCHY_NODE_ID);
   -- Delete all Subhierarchies

   Open c1(p_HIERARCHY_NODE_ID);
   Loop
   Fetch c1 into l_HIERARCHY_NODE_ID;
   Exit when c1%NotFound;
      DELETE FROM CSP_REP_HIERARCHIES
      WHERE PARENT_NODE_ID = l_HIERARCHY_NODE_ID;
   End Loop;
   Close c1;
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
        SELECT *
         FROM CSP_REP_HIERARCHIES
        WHERE HIERARCHY_NODE_ID =  p_HIERARCHY_NODE_ID
        FOR UPDATE of HIERARCHY_NODE_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN C;
    ...
    CLOSE C;
    if (
           (      Recinfo.HIERARCHY_NODE_ID = p_HIERARCHY_NODE_ID)
       ...
       AND (    ( Recinfo.TOP_NODE_ID = p_TOP_NODE_ID)
            OR (    ( Recinfo.TOP_NODE_ID IS NULL )
                AND (  p_TOP_NODE_ID IS NULL )))
