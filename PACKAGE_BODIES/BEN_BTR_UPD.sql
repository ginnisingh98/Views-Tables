--------------------------------------------------------
--  DDL for Package Body BEN_BTR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BTR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_btr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_comp_lvl_acty_rt_f',
	   p_base_key_column	=> 'comp_lvl_acty_rt_id',
	   p_base_key_value	=> p_rec.comp_lvl_acty_rt_id);
    --
    ben_btr_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_comp_lvl_acty_rt_f Row
    --
    ...
    --
    ben_btr_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_btr_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.comp_lvl_acty_rt_id,
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'ben_comp_lvl_acty_rt_f'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_btr_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.dflt_flag = hr_api.g_varchar2) then
    p_rec.dflt_flag :=
    ben_btr_shd.g_old_rec.dflt_flag;
  End If;
  If (p_rec.comp_lvl_fctr_id = hr_api.g_number) then
    p_rec.comp_lvl_fctr_id :=
    ben_btr_shd.g_old_rec.comp_lvl_fctr_id;
  End If;
  If (p_rec.acty_base_rt_id = hr_api.g_number) then
    p_rec.acty_base_rt_id :=
    ben_btr_shd.g_old_rec.acty_base_rt_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_btr_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.btr_attribute_category = hr_api.g_varchar2) then
    p_rec.btr_attribute_category :=
    ben_btr_shd.g_old_rec.btr_attribute_category;
  End If;
  If (p_rec.btr_attribute1 = hr_api.g_varchar2) then
    p_rec.btr_attribute1 :=
    ben_btr_shd.g_old_rec.btr_attribute1;
  End If;
  If (p_rec.btr_attribute2 = hr_api.g_varchar2) then
    p_rec.btr_attribute2 :=
    ben_btr_shd.g_old_rec.btr_attribute2;
  End If;
  If (p_rec.btr_attribute3 = hr_api.g_varchar2) then
    p_rec.btr_attribute3 :=
    ben_btr_shd.g_old_rec.btr_attribute3;
  End If;
  If (p_rec.btr_attribute4 = hr_api.g_varchar2) then
    p_rec.btr_attribute4 :=
    ben_btr_shd.g_old_rec.btr_attribute4;
  End If;
  If (p_rec.btr_attribute5 = hr_api.g_varchar2) then
    p_rec.btr_attribute5 :=
    ben_btr_shd.g_old_rec.btr_attribute5;
  End If;
  If (p_rec.btr_attribute6 = hr_api.g_varchar2) then
    p_rec.btr_attribute6 :=
    ben_btr_shd.g_old_rec.btr_attribute6;
  End If;
  If (p_rec.btr_attribute7 = hr_api.g_varchar2) then
    p_rec.btr_attribute7 :=
    ben_btr_shd.g_old_rec.btr_attribute7;
  End If;
  If (p_rec.btr_attribute8 = hr_api.g_varchar2) then
    p_rec.btr_attribute8 :=
    ben_btr_shd.g_old_rec.btr_attribute8;
  End If;
  If (p_rec.btr_attribute9 = hr_api.g_varchar2) then
    p_rec.btr_attribute9 :=
    ben_btr_shd.g_old_rec.btr_attribute9;
  End If;
  If (p_rec.btr_attribute10 = hr_api.g_varchar2) then
    p_rec.btr_attribute10 :=
    ben_btr_shd.g_old_rec.btr_attribute10;
  End If;
  If (p_rec.btr_attribute11 = hr_api.g_varchar2) then
    p_rec.btr_attribute11 :=
    ben_btr_shd.g_old_rec.btr_attribute11;
  End If;
  If (p_rec.btr_attribute12 = hr_api.g_varchar2) then
    p_rec.btr_attribute12 :=
    ben_btr_shd.g_old_rec.btr_attribute12;
  End If;
  If (p_rec.btr_attribute13 = hr_api.g_varchar2) then
    p_rec.btr_attribute13 :=
    ben_btr_shd.g_old_rec.btr_attribute13;
  End If;
  If (p_rec.btr_attribute14 = hr_api.g_varchar2) then
    p_rec.btr_attribute14 :=
    ben_btr_shd.g_old_rec.btr_attribute14;
  End If;
  If (p_rec.btr_attribute15 = hr_api.g_varchar2) then
    p_rec.btr_attribute15 :=
    ben_btr_shd.g_old_rec.btr_attribute15;
  End If;
  If (p_rec.btr_attribute16 = hr_api.g_varchar2) then
    p_rec.btr_attribute16 :=
    ben_btr_shd.g_old_rec.btr_attribute16;
  End If;
  If (p_rec.btr_attribute17 = hr_api.g_varchar2) then
    p_rec.btr_attribute17 :=
    ben_btr_shd.g_old_rec.btr_attribute17;
  End If;
  If (p_rec.btr_attribute18 = hr_api.g_varchar2) then
    p_rec.btr_attribute18 :=
    ben_btr_shd.g_old_rec.btr_attribute18;
  End If;
  If (p_rec.btr_attribute19 = hr_api.g_varchar2) then
    p_rec.btr_attribute19 :=
    ben_btr_shd.g_old_rec.btr_attribute19;
  End If;
  If (p_rec.btr_attribute20 = hr_api.g_varchar2) then
    p_rec.btr_attribute20 :=
    ben_btr_shd.g_old_rec.btr_attribute20;
  End If;
  If (p_rec.btr_attribute21 = hr_api.g_varchar2) then
    p_rec.btr_attribute21 :=
    ben_btr_shd.g_old_rec.btr_attribute21;
  End If;
  If (p_rec.btr_attribute22 = hr_api.g_varchar2) then
    p_rec.btr_attribute22 :=
    ben_btr_shd.g_old_rec.btr_attribute22;
  End If;
  If (p_rec.btr_attribute23 = hr_api.g_varchar2) then
    p_rec.btr_attribute23 :=
    ben_btr_shd.g_old_rec.btr_attribute23;
  End If;
  If (p_rec.btr_attribute24 = hr_api.g_varchar2) then
    p_rec.btr_attribute24 :=
    ben_btr_shd.g_old_rec.btr_attribute24;
  End If;
  If (p_rec.btr_attribute25 = hr_api.g_varchar2) then
    p_rec.btr_attribute25 :=
    ben_btr_shd.g_old_rec.btr_attribute25;
  End If;
  If (p_rec.btr_attribute26 = hr_api.g_varchar2) then
    p_rec.btr_attribute26 :=
    ben_btr_shd.g_old_rec.btr_attribute26;
  End If;
  If (p_rec.btr_attribute27 = hr_api.g_varchar2) then
    p_rec.btr_attribute27 :=
    ben_btr_shd.g_old_rec.btr_attribute27;
  End If;
  If (p_rec.btr_attribute28 = hr_api.g_varchar2) then
    p_rec.btr_attribute28 :=
    ben_btr_shd.g_old_rec.btr_attribute28;
  End If;
  If (p_rec.btr_attribute29 = hr_api.g_varchar2) then
    p_rec.btr_attribute29 :=
    ben_btr_shd.g_old_rec.btr_attribute29;
  End If;
  If (p_rec.btr_attribute30 = hr_api.g_varchar2) then
    p_rec.btr_attribute30 :=
    ben_btr_shd.g_old_rec.btr_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_btr_shd.convert_args
  (
  p_comp_lvl_acty_rt_id,
  null,
  null,
  p_dflt_flag,
  p_comp_lvl_fctr_id,
