--------------------------------------------------------
--  DDL for Package Body BEN_BUR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BUR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_bur_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_brgng_unit_rt_f Row
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_bur_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.brgng_unit_rt_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_bur_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.vrbl_rt_prfl_id = hr_api.g_number) then
    ...
    ben_bur_shd.g_old_rec.vrbl_rt_prfl_id;
  End If;
  If (p_rec.excld_flag = hr_api.g_varchar2) then
    p_rec.excld_flag :=
    ben_bur_shd.g_old_rec.excld_flag;
  End If;
  If (p_rec.ordr_num = hr_api.g_number) then
    p_rec.ordr_num :=
    ben_bur_shd.g_old_rec.ordr_num;
  End If;
  If (p_rec.brgng_unit_cd = hr_api.g_varchar2) then
    p_rec.brgng_unit_cd :=
    ben_bur_shd.g_old_rec.brgng_unit_cd;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_bur_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.bur_attribute_category = hr_api.g_varchar2) then
    p_rec.bur_attribute_category :=
    ben_bur_shd.g_old_rec.bur_attribute_category;
  End If;
  If (p_rec.bur_attribute1 = hr_api.g_varchar2) then
    p_rec.bur_attribute1 :=
    ben_bur_shd.g_old_rec.bur_attribute1;
  End If;
  If (p_rec.bur_attribute2 = hr_api.g_varchar2) then
    p_rec.bur_attribute2 :=
    ben_bur_shd.g_old_rec.bur_attribute2;
  End If;
  If (p_rec.bur_attribute3 = hr_api.g_varchar2) then
    p_rec.bur_attribute3 :=
    ben_bur_shd.g_old_rec.bur_attribute3;
  End If;
  If (p_rec.bur_attribute4 = hr_api.g_varchar2) then
    p_rec.bur_attribute4 :=
    ben_bur_shd.g_old_rec.bur_attribute4;
  End If;
  If (p_rec.bur_attribute5 = hr_api.g_varchar2) then
    p_rec.bur_attribute5 :=
    ben_bur_shd.g_old_rec.bur_attribute5;
  End If;
  If (p_rec.bur_attribute6 = hr_api.g_varchar2) then
    p_rec.bur_attribute6 :=
    ben_bur_shd.g_old_rec.bur_attribute6;
  End If;
  If (p_rec.bur_attribute7 = hr_api.g_varchar2) then
    p_rec.bur_attribute7 :=
    ben_bur_shd.g_old_rec.bur_attribute7;
  End If;
  If (p_rec.bur_attribute8 = hr_api.g_varchar2) then
    p_rec.bur_attribute8 :=
    ben_bur_shd.g_old_rec.bur_attribute8;
  End If;
  If (p_rec.bur_attribute9 = hr_api.g_varchar2) then
    p_rec.bur_attribute9 :=
    ben_bur_shd.g_old_rec.bur_attribute9;
  End If;
  If (p_rec.bur_attribute10 = hr_api.g_varchar2) then
    p_rec.bur_attribute10 :=
    ben_bur_shd.g_old_rec.bur_attribute10;
  End If;
  If (p_rec.bur_attribute11 = hr_api.g_varchar2) then
    p_rec.bur_attribute11 :=
    ben_bur_shd.g_old_rec.bur_attribute11;
  End If;
  If (p_rec.bur_attribute12 = hr_api.g_varchar2) then
    p_rec.bur_attribute12 :=
    ben_bur_shd.g_old_rec.bur_attribute12;
  End If;
  If (p_rec.bur_attribute13 = hr_api.g_varchar2) then
    p_rec.bur_attribute13 :=
    ben_bur_shd.g_old_rec.bur_attribute13;
  End If;
  If (p_rec.bur_attribute14 = hr_api.g_varchar2) then
    p_rec.bur_attribute14 :=
    ben_bur_shd.g_old_rec.bur_attribute14;
  End If;
  If (p_rec.bur_attribute15 = hr_api.g_varchar2) then
    p_rec.bur_attribute15 :=
    ben_bur_shd.g_old_rec.bur_attribute15;
  End If;
  If (p_rec.bur_attribute16 = hr_api.g_varchar2) then
    p_rec.bur_attribute16 :=
    ben_bur_shd.g_old_rec.bur_attribute16;
  End If;
  If (p_rec.bur_attribute17 = hr_api.g_varchar2) then
    p_rec.bur_attribute17 :=
    ben_bur_shd.g_old_rec.bur_attribute17;
  End If;
  If (p_rec.bur_attribute18 = hr_api.g_varchar2) then
    p_rec.bur_attribute18 :=
    ben_bur_shd.g_old_rec.bur_attribute18;
  End If;
  If (p_rec.bur_attribute19 = hr_api.g_varchar2) then
    p_rec.bur_attribute19 :=
    ben_bur_shd.g_old_rec.bur_attribute19;
  End If;
  If (p_rec.bur_attribute20 = hr_api.g_varchar2) then
    p_rec.bur_attribute20 :=
    ben_bur_shd.g_old_rec.bur_attribute20;
  End If;
  If (p_rec.bur_attribute21 = hr_api.g_varchar2) then
    p_rec.bur_attribute21 :=
    ben_bur_shd.g_old_rec.bur_attribute21;
  End If;
  If (p_rec.bur_attribute22 = hr_api.g_varchar2) then
    p_rec.bur_attribute22 :=
    ben_bur_shd.g_old_rec.bur_attribute22;
  End If;
  If (p_rec.bur_attribute23 = hr_api.g_varchar2) then
    p_rec.bur_attribute23 :=
    ben_bur_shd.g_old_rec.bur_attribute23;
  End If;
  If (p_rec.bur_attribute24 = hr_api.g_varchar2) then
    p_rec.bur_attribute24 :=
    ben_bur_shd.g_old_rec.bur_attribute24;
  End If;
  If (p_rec.bur_attribute25 = hr_api.g_varchar2) then
    p_rec.bur_attribute25 :=
    ben_bur_shd.g_old_rec.bur_attribute25;
  End If;
  If (p_rec.bur_attribute26 = hr_api.g_varchar2) then
    p_rec.bur_attribute26 :=
    ben_bur_shd.g_old_rec.bur_attribute26;
  End If;
  If (p_rec.bur_attribute27 = hr_api.g_varchar2) then
    p_rec.bur_attribute27 :=
    ben_bur_shd.g_old_rec.bur_attribute27;
  End If;
  If (p_rec.bur_attribute28 = hr_api.g_varchar2) then
    p_rec.bur_attribute28 :=
    ben_bur_shd.g_old_rec.bur_attribute28;
  End If;
  If (p_rec.bur_attribute29 = hr_api.g_varchar2) then
    p_rec.bur_attribute29 :=
    ben_bur_shd.g_old_rec.bur_attribute29;
  End If;
  If (p_rec.bur_attribute30 = hr_api.g_varchar2) then
    p_rec.bur_attribute30 :=
    ben_bur_shd.g_old_rec.bur_attribute30;
  End If;
