--------------------------------------------------------
--  DDL for Package Body IGW_PROPOSAL_APPROVAL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGW_PROPOSAL_APPROVAL" as
--$Header: igwprapb.pls 115.21 2002/11/14 18:50:23 vmedikon ship $
...
/* -- Debashis. We don't validate anymore.
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       ...
       fnd_msg_pub.add_exc_msg(p_pkg_name       =>    'IGW_PROPOSAL_APPROVAL',
                            p_procedure_name    =>    'START_APPROVAL_PROCESS',
...
-- Debashis. The block below has been commented out NOCOPY since we do not check
-- for any such condition any more. Anyone can submit proposal
...
begin
  ...
  close get_user_id;
   ...
   -- Assign Signing Official role(role_id=3) to Signing Official and Administrative Official if it does not already exist
