--------------------------------------------------------
--  DDL for Package Body OTA_TPM_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_TPM_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ota_tpm_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy ota_tpm_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  -- Update the ota_training_plan_members Row
  --
  update ota_training_plan_members
    set
     training_plan_member_id         = p_rec.training_plan_member_id
    ,training_plan_id                = p_rec.training_plan_id
    ,activity_version_id             = p_rec.activity_version_id
    ,activity_definition_id          = p_rec.activity_definition_id
    ,member_status_type_id           = p_rec.member_status_type_id
    ,target_completion_date          = p_rec.target_completion_date
    ...
    ,source_function                 = p_rec.source_function
    ,cancellation_reason             = p_rec.cancellation_reason
    ,earliest_start_date             = p_rec.earliest_start_date
...
Begin
  ...
  begin
    ...
    ota_tpm_rku.after_update
      ...
      => p_rec.activity_definition_id
      ,p_member_status_type_id
      => p_rec.member_status_type_id
      ,p_target_completion_date
      => p_rec.target_completion_date
      ,p_business_group_id
...
Begin
  ...
  If (p_rec.activity_version_id = hr_api.g_number) then
    ...
    ota_tpm_shd.g_old_rec.activity_version_id;
  End If;
  If (p_rec.activity_definition_id = hr_api.g_number) then
    p_rec.activity_definition_id :=
    ota_tpm_shd.g_old_rec.activity_definition_id;
  End If;
  If (p_rec.member_status_type_id = hr_api.g_varchar2) then
    p_rec.member_status_type_id :=
    ota_tpm_shd.g_old_rec.member_status_type_id;
  End If;
  If (p_rec.target_completion_date = hr_api.g_date) then
    p_rec.target_completion_date :=
    ota_tpm_shd.g_old_rec.target_completion_date;
  End If;
  ...
  If (p_rec.assignment_id = hr_api.g_number) then
    p_rec.assignment_id :=
    ota_tpm_shd.g_old_rec.assignment_id;
  End If;
  If (p_rec.source_id = hr_api.g_number) then
    p_rec.source_id :=
    ota_tpm_shd.g_old_rec.source_id;
  End If;
  If (p_rec.source_function = hr_api.g_varchar2) then
    p_rec.source_function :=
    ota_tpm_shd.g_old_rec.source_function;
  End If;
  If (p_rec.cancellation_reason = hr_api.g_varchar2) then
    p_rec.cancellation_reason :=
    ota_tpm_shd.g_old_rec.cancellation_reason;
  End If;
  If (p_rec.earliest_start_date = hr_api.g_date) then
    p_rec.earliest_start_date :=
    ota_tpm_shd.g_old_rec.earliest_start_date;
  End If;
  If (p_rec.creator_person_id = hr_api.g_number) then
    p_rec.creator_person_id :=
    ota_tpm_shd.g_old_rec.creator_person_id;
  End If;
  --
End convert_defs;
--
...
Begin
  ...
  --
  ota_tpm_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ota_tpm_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ota_tpm_upd.post_update
     (p_effective_date
     ,p_rec
     );
