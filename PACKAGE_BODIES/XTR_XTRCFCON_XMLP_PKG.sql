--------------------------------------------------------
--  DDL for Package Body XTR_XTRCFCON_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XTR_XTRCFCON_XMLP_PKG" AS
/* $Header: XTRCFCONB.pls 120.1 2007/12/28 12:45:02 npannamp noship $ */
  FUNCTION ROLLING_BAL1FORMULA RETURN VARCHAR2 IS
  BEGIN
    /*EXECUTE IMMEDIATE
      'SELECT ('||OPEN_BAL || '+' || ACCOUNT_BAL|| ') FROM SYS.DUAL' INTO ROLLING_BAL;
    COMMIT;*/null;
    RETURN ('');
  ...
  FUNCTION OPEN_BAL1FORMULA(ccy varchar2) RETURN VARCHAR2 IS
  BEGIN
    EXECUTE IMMEDIATE
      'SELECT NVL(round(sum(NVL(OPENING_BALANCE,0)),0),0)/'||L_amt_unit2||' FROM XTR_BANK_ACCOUNTS_V WHERE CURRENCY = :CYY' INTO OPEN_BAL USING OPEN_BAL1FORMULA.ccy;
--    COMMIT;
    RETURN ('');
  ...
  BEGIN
    ...
    FOR c IN GET_LANGUAGE_DESC LOOP
      ...
      END IF;
    END LOOP;
    LP_PERIOD_FROM:=to_char(P_PERIOD_FROM,'DD-MON-YY');
    LP_PERIOD_TO:=to_char(P_PERIOD_TO,'DD-MON-YY');
    RETURN (TRUE);
  END BEFOREREPORT;
