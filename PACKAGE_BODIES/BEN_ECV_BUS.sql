--------------------------------------------------------
--  DDL for Package Body BEN_ECV_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ECV_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ecv_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_eligy_crit_values_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   eligy_crit_values_id PK of record being inserted or updated.
...
--
procedure chk_eligy_crit_values_id (
                                    p_eligy_crit_values_id      In   Number,
                                    p_effective_date            In   Date,
                                    p_object_version_number     In   Number
									) is
       --
       l_proc varchar2(72) := g_package||'chk_eligy_crit_values_id';
...
Begin
  ...
  --
  l_api_updating := ben_ecv_shd.api_updating
  ...
  --
  if (l_api_updating
      and nvl(p_eligy_crit_values_id,hr_api.g_number)
	      <> ben_ecv_shd.g_old_rec.eligy_crit_values_id) then
    --
    -- raise error as PK has changed
    --
    ben_ecv_shd.constraint_error('BEN_ELIGY_CRIT_VALUES_F_PK');
    --
    elsif not l_api_updating then
    --
    --check if PK is NULL
    --
    if p_eligy_crit_values_id is not null then
      --
...
--
-- In Parameters
--   p_eligy_crit_values_id PK
--   p_eligy_criteria_id ID of FK column
...
--   Internal table handler use only.
--
procedure chk_eligy_criteria_id(
                                p_eligy_crit_values_id  In Number,
      ...
      l_dummy varchar2(1);
      --
      Cursor csr_eligy_criteria_id is
         select NULL
...
begin
  ...
  if (l_api_updating
      and nvl(p_eligy_criteria_id,hr_api.g_number)
          <>  ben_ecv_shd.g_old_rec.eligy_criteria_id
		   or  not l_api_updating) then
     --
     open csr_eligy_criteria_id;
     --
     fetch csr_eligy_criteria_id into l_dummy;
     --
     if csr_eligy_criteria_id%notfound then
        --
        close csr_eligy_criteria_id;
        --
        --Raise an error
        --
       ben_ecv_shd.constraint_error('BEN_ELIGY_CRIT_VALUES_FK1');
      --
     end if;
     --
     close csr_eligy_criteria_id;
...
begin
  ...
  if (l_api_updating
      and nvl(p_eligy_prfl_id,hr_api.g_number)
          <>  ben_ecv_shd.g_old_rec.eligy_prfl_id
		  or  not l_api_updating) then
     --
...
procedure chk_duplicate_eligy_criteria
  ...
  cursor c_eligy_prfl is
     select null
      ...
      where ecv.eligy_prfl_id = p_eligy_prfl_id
        and ecv.eligy_criteria_id = p_eligy_criteria_id
        and ecv.eligy_crit_values_id <> nvl ( p_eligy_crit_values_id, -1)
	and nvl(ecv.number_value1,hr_api.g_number) = nvl(p_number_value1,hr_api.g_number)
	and nvl(ecv.number_value2,hr_api.g_number) = nvl(p_number_value2,hr_api.g_number)
	and nvl(ecv.char_value1,hr_api.g_varchar2) = nvl(p_char_value1,hr_api.g_varchar2)
	and nvl(ecv.char_value2,hr_api.g_varchar2) = nvl(p_char_value2,hr_api.g_varchar2)
	and nvl(ecv.date_value1,hr_api.g_date) = nvl (p_date_value1,hr_api.g_date)
	and nvl(ecv.date_value2,hr_api.g_date) = nvl (p_date_value2,hr_api.g_date)
	and nvl(ecv.number_value3,hr_api.g_number) = nvl(p_number_value3,hr_api.g_number)
	and nvl(ecv.number_value4,hr_api.g_number) = nvl(p_number_value4,hr_api.g_number)
	and nvl(ecv.char_value3,hr_api.g_varchar2) = nvl(p_char_value3,hr_api.g_varchar2)
	and nvl(ecv.char_value4,hr_api.g_varchar2) = nvl(p_char_value4,hr_api.g_varchar2)
	and nvl(ecv.date_value3,hr_api.g_date) = nvl (p_date_value3,hr_api.g_date)
	and nvl(ecv.date_value4,hr_api.g_date) = nvl (p_date_value4,hr_api.g_date)
	and ecv.effective_start_date <= p_validation_end_date
        and ecv.effective_end_date >= p_validation_start_date
        and ecv.business_group_id = p_business_group_id;
  --
begin
  --
  hr_utility.set_location('Entering:'||l_proc, 5);
...
--
-- In Parameters
--   p_eligy_crit_values_id eligy_crit_values_id
...
Procedure chk_duplicate_ordr_num
  ...
  cursor c1 is
     select null
      ...
      where eligy_crit_values_id <> nvl( p_eligy_crit_values_id, -1 )
...
begin
  ...
  open c_eligy_criteria;
    ...
    then
      ...
      --
      elsif l_crit_col1_val_type_cd = 'ORG_HIER'
      then
        --
        if (p_number_value1 is null or p_number_value2 is null)
        then
          --
          fnd_message.set_name('BEN', 'BEN_94142_ORG_HIERARCHY_NULL');
          fnd_message.raise_error;
          --
        else
          --
          ...
          open c_org_stru_ver ;
            --
            fetch c_org_stru_ver into l_dummy;
            --
            if c_org_stru_ver%notfound
            then
              --
              -- Organization Structure Version ID non-existent or inactive
              --
              fnd_message.set_name('BEN', 'BEN_94143_INVALID_ORG_STRU_VER');
              fnd_message.raise_error;
              --
            end if;
            --
          ...
          open c_start_org ;
            --
            fetch c_start_org into l_dummy;
            --
            if c_start_org%notfound
            then
              --
              -- Start Organization does not fall under selected Organization Structure / Hierarchy
              --
              fnd_message.set_name('BEN', 'BEN_94144_INVALID_START_ORG');
              fnd_message.raise_error;
              --
            end if;
            --
      ...
      -- Position Hierarchy
      --
      elsif l_crit_col1_val_type_cd = 'POS_HIER'
      then
        --
        if (p_number_value1 is null or p_number_value2 is null)
        then
          --
          fnd_message.set_name('BEN', 'BEN_94145_POS_HIERARCHY_NULL');
          fnd_message.raise_error;
          --
        else
          --
          -- Validate Position Hierarchy / Structure
          --
          open c_pos_stru_ver ;
            --
            fetch c_pos_stru_ver into l_dummy;
            --
            if c_pos_stru_ver%notfound
            then
              --
              -- Position Structure Version ID non-existent or inactive
              --
              fnd_message.set_name('BEN', 'BEN_94146_INVALID_POS_STRU_VER');
              fnd_message.raise_error;
              --
            end if;
            --
          ...
          open c_start_pos ;
            --
            fetch c_start_pos into l_dummy;
            --
            if c_start_pos%notfound
            then
              --
              -- Start Position does not fall under selected Position Structure / Hierarchy
              --
              fnd_message.set_name('BEN', 'BEN_94147_INVALID_START_POS');
              fnd_message.raise_error;
              --
            end if;
            --
      ...
      -- Organization Hierarchy
      --
      elsif l_crit_col2_val_type_cd = 'ORG_HIER'
      then
        --
        if (p_number_value3 is null or p_number_value4 is null)
        then
          --
          fnd_message.set_name('BEN', 'BEN_94142_ORG_HIERARCHY_NULL');
          fnd_message.raise_error;
          ...
          -- Validate Organization Hierarchy / Structure
          --
          open c_org_stru_ver2 ;
            --
            fetch c_org_stru_ver2 into l_dummy;
            --
            if c_org_stru_ver2%notfound
            then
              --
              -- Organization Structure Version ID non-existent or inactive
              --
            ...
            --
          close c_org_stru_ver2;
          --
          -- Validate Start Organization
          --
          open c_start_org2 ;
            --
            fetch c_start_org2 into l_dummy;
            --
            if c_start_org2%notfound
            then
              --
              -- Start Organization does not fall under selected Organization Structure / Hierarchy
              --
      ...
      -- Position Hierarchy
      --
      elsif l_crit_col2_val_type_cd = 'POS_HIER'
      then
        --
        if (p_number_value3 is null or p_number_value4 is null)
        then
          ...
          -- Validate Position Hierarchy / Structure
          --
          open c_pos_stru_ver2 ;
            --
            fetch c_pos_stru_ver2 into l_dummy;
            --
            if c_pos_stru_ver2%notfound
            then
              --
              -- Position Structure Version ID non-existent or inactive
              --
            ...
            --
          close c_pos_stru_ver2;
          --
          -- Validate Start Position
          --
          open c_start_pos2 ;
            --
            fetch c_start_pos2 into l_dummy;
            --
            if c_start_pos2%notfound
            then
              --
              -- Start Position does not fall under selected Position Structure / Hierarchy
              --
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    --
  /*  If ((nvl(p_eligy_criteria_id, hr_api.g_number) <> hr_api.g_number) and
      NOT (dt_api.check_min_max_dates
            (p_base_table_name => 'ben_eligy_criteria',
             p_base_key_column => 'eligy_criteria_id',
             p_base_key_value  => p_eligy_criteria_id,
...
begin
  ...
  --
  chk_eligy_crit_values_id( p_eligy_crit_values_id         =>  p_rec.eligy_crit_values_id,
                            p_effective_date               =>  p_effective_date,
                            p_object_version_number        =>  p_rec.object_version_number );
  --
  chk_eligy_criteria_id( p_eligy_crit_values_id         =>  p_rec.eligy_crit_values_id,
                         p_eligy_criteria_id            =>  p_rec.eligy_criteria_id,
                         p_effective_date               =>  p_effective_date,
                         p_object_version_number        =>  p_rec.object_version_number );
  --
  chk_eligy_prfl_id( p_eligy_crit_values_id         =>  p_rec.eligy_crit_values_id,
                     p_eligy_prfl_id                =>  p_rec.eligy_prfl_id,
                     p_effective_date               =>  p_effective_date,
                     p_object_version_number        =>  p_rec.object_version_number );
  --
  chk_duplicate_eligy_criteria (p_eligy_crit_values_id       => p_rec.eligy_crit_values_id,
                                p_eligy_prfl_id              => p_rec.eligy_prfl_id,
                                p_eligy_criteria_id          => p_rec.eligy_criteria_id,
  ...
  --
  chk_duplicate_ordr_num ( p_eligy_crit_values_id       => p_rec.eligy_crit_values_id,
                           p_eligy_prfl_id              => p_rec.eligy_prfl_id,
                           ...
                           p_business_group_id          => p_rec.business_group_id );
  --
  chk_required_fields ( p_eligy_crit_values_id       => p_rec.eligy_crit_values_id,
                        p_eligy_criteria_id          => p_rec.eligy_criteria_id,
                        p_number_value1              => p_rec.number_value1,
                        p_char_value1                => p_rec.char_value1,
                        p_date_Value1                => p_rec.date_Value1,
...
begin
  ...
  --
  --
  if p_rec.business_group_id is not null and p_rec.legislation_code is null
  then
    --
    hr_api.validate_bus_grp_id(p_rec.business_group_id);  -- Validate Bus Grp
    --
  ...
  --
  dt_update_validate( p_eligy_criteria_id            => p_rec.eligy_criteria_id,
                      p_eligy_prfl_id                => p_rec.eligy_prfl_id,
...
function return_legislation_code
  (p_eligy_crit_values_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_eligy_crit_values_f b
    where b.eligy_crit_values_id      = p_eligy_crit_values_id
