--------------------------------------------------------
--  DDL for Package Body PAY_BLT_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_BLT_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_blt_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_balance_type_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  pay_blt_ins.g_balance_type_id_i := p_balance_type_id;
  --
...
Begin
  ...
  --
  -- Insert the row into: pay_balance_types
  --
...
--
  Cursor C_Sel1 is select pay_balance_types_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from pay_balance_types
     where balance_type_id =
             pay_blt_ins.g_balance_type_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (pay_blt_ins.g_balance_type_id_i is not null) Then
    --
    ...
    If C_Sel2%found Then
       ...
       fnd_message.set_token('TABLE_NAME','pay_balance_types');
    ...
    p_rec.balance_type_id :=
      pay_blt_ins.g_balance_type_id_i;
    pay_blt_ins.g_balance_type_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.balance_type_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  pay_blt_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  pay_blt_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  pay_blt_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
