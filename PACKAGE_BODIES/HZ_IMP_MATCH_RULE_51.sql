--------------------------------------------------------
--  DDL for Package Body HZ_IMP_MATCH_RULE_51
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_IMP_MATCH_RULE_51" AS
    g_match_rule_id NUMBER := 51;
  ...
  select decode(a.party_type, 'ORGANIZATION', a.organization_name, 'PERSON', a.person_first_name || ' ' || a.person_last_name) as PARTY_NAME , a.PARTY_TYPE, a.JGZZ_FISCAL_CODE, a.party_orig_system, a.party_orig_system_reference, b.party_id, a.rowid, a.party_type
   ...
   LOOP 
    FETCH p_party_cur BULK COLLECT INTO 
       H_8E, H_14E, H_19E, H_P_PARTY_OS , H_P_PARTY_OSR, H_P_PARTY_ID, H_P_ROW_ID, H_P_P_TYPE
          LIMIT g_limit; 
...
----------- SETTING GLOBAL CONDITION RECORD AT THE PARTY_SITES LEVEL ---------
  ...
  select a.EMAIL_ADDRESS, a.party_orig_system, a.party_orig_system_reference, a.cp_orig_system, a.cp_orig_system_reference, a.site_orig_system, a.site_orig_system_reference, b.party_site_id, b.contact_point_id, b.party_id, b.party_action_flag, a.rowid, a.contact_point_type, decode(a.raw_phone_number, null, a.PHONE_COUNTRY_CODE||a.PHONE_AREA_CODE ||a.phone_number, a.raw_phone_number) as raw_phone_number 
   ...
   LOOP 
      FETCH l_cp_cur BULK COLLECT INTO 
       H_48E, H_P_PARTY_OS, H_P_PARTY_OSR, H_P_CP_OS, H_P_CP_OSR, H_P_PS_OS, H_P_PS_OSR, H_P_PARTY_SITE_ID, H_P_CONTACT_POINT_ID, H_P_PARTY_ID, H_P_N_PARTY, H_P_ROW_ID, H_P_CP_TYPE, H_P_CP_R_PH_NO 
       LIMIT g_limit; 
 ...
 PROCEDURE pop_parties_int ( 
  ...
  select decode(a.party_type, 'ORGANIZATION', a.organization_name, 'PERSON', a.person_first_name || ' ' || a.person_last_name) as PARTY_NAME , a.PARTY_TYPE, a.JGZZ_FISCAL_CODE, a.party_orig_system, a.party_orig_system_reference, a.rowid, a.party_type  , a.party_id 
    ...
    LOOP 
    FETCH p_party_cur BULK COLLECT INTO 
       H_8E, H_14E, H_19E, H_P_PARTY_OS , H_P_PARTY_OSR, H_P_ROW_ID, H_P_P_TYPE , H_P_PARTY_ID 
          LIMIT g_limit; 
    IF p_party_cur%NOTFOUND THEN 
      l_last_fetch:=TRUE; 
    END IF; 
...
----------- SETTING GLOBAL CONDITION RECORD AT THE PARTY_SITES LEVEL ---------
  ...
  select a.EMAIL_ADDRESS, a.party_orig_system, a.party_orig_system_reference, a.cp_orig_system, a.cp_orig_system_reference, a.site_orig_system, a.site_orig_system_reference, a.rowid, a.contact_point_type,decode(a.raw_phone_number, null, a.PHONE_COUNTRY_CODE||a.PHONE_AREA_CODE ||a.phone_number,a.raw_phone_number) as raw_phone_number  , a.party_id 
   ...
   LOOP 
       FETCH l_cp_cur BULK COLLECT INTO 
       H_48E, H_P_PARTY_OS, H_P_PARTY_OSR, H_P_CP_OS, H_P_CP_OSR, H_P_PS_OS, H_P_PS_OSR, H_P_ROW_ID, H_P_CP_TYPE, H_P_CP_R_PH_NO  , H_P_PARTY_ID 
       LIMIT g_limit; 
     IF l_cp_cur%NOTFOUND THEN 
       l_last_fetch:=TRUE; 
     END IF; 
...
WHEN OTHERS THEN
         ...
         ELSE
             ...
             FND_MESSAGE.SET_TOKEN('PROC','HZ_IMP_MATCH_RULE_51.tca_join_entities');
...
WHEN OTHERS THEN
         FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
         FND_MESSAGE.SET_TOKEN('PROC','HZ_IMP_MATCH_RULE_51.interface_tca_join_entities');
         FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
         FND_MSG_PUB.ADD;
         RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
WHEN OTHERS THEN
         FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
         FND_MESSAGE.SET_TOKEN('PROC','HZ_IMP_MATCH_RULE_51.interface_join_entities');
         FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
         FND_MSG_PUB.ADD;
         RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
