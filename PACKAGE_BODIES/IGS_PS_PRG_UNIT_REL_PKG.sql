--------------------------------------------------------
--  DDL for Package Body IGS_PS_PRG_UNIT_REL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_PRG_UNIT_REL_PKG" AS
/* $Header: IGSPI2YB.pls 115.5 2002/11/29 02:20:43 nsidana noship $ */


  l_rowid VARCHAR2(25);
  old_references igs_ps_prg_unit_rel%ROWTYPE;
  new_references igs_ps_prg_unit_rel%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.ps_prun_rel_id                    := x_ps_prun_rel_id;
    new_references.student_career_level              := x_student_career_level;
  ...
  */
  BEGIN
    ...
    IF ( get_uk_for_validation (
           new_references.student_career_level,
  ...
  */
  BEGIN
    ...
    IF (((old_references.student_career_level = new_references.student_career_level)) OR
        ((new_references.student_career_level IS NULL))) THEN
      NULL;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_prg_unit_rel
      WHERE    ps_prun_rel_id = x_ps_prun_rel_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_prg_unit_rel
      WHERE    student_career_level = x_student_career_level
      AND      unit_type_id = x_unit_type_id
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_prg_unit_rel
      WHERE   ((unit_type_id = x_unit_type_id));
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_UTL_PUR_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  END get_fk_igs_ps_unit_type_lvl;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_prg_unit_rel
      WHERE   ((student_career_level = x_course_type));
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_PST_PUR_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_prg_unit_rel_s.NEXTVAL
    INTO      x_ps_prun_rel_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_ps_prun_rel_id                    => x_ps_prun_rel_id,
      x_student_career_level              => x_student_career_level,
    ...
    INSERT INTO igs_ps_prg_unit_rel (
      ps_prun_rel_id,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.student_career_level = x_student_career_level)
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_ps_prg_unit_rel
      SET
        student_career_level              = new_references.student_career_level,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_prg_unit_rel
    WHERE rowid = x_rowid;
