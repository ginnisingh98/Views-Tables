--------------------------------------------------------
--  DDL for Package Body PAY_DTE_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_DTE_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_dte_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_datetracked_event_id        number         default null;
--
--  ---------------------------------------------------------------------------
...
Begin
  ...
  --
  if ( nvl(pay_dte_bus.g_datetracked_event_id, hr_api.g_number)
       = p_datetracked_event_id) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := pay_dte_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_dte_bus.g_datetracked_event_id:= p_datetracked_event_id;
    pay_dte_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_dte_shd.api_updating
      (p_datetracked_event_id                 => p_rec.datetracked_event_id
  ...
  if nvl(p_rec.legislation_code, hr_api.g_varchar2) <>
     nvl(pay_dte_shd.g_old_rec.legislation_code, hr_api.g_varchar2)
  then
  ...
  if nvl(p_rec.business_group_id, hr_api.g_number) <>
     nvl(pay_dte_shd.g_old_rec.business_group_id, hr_api.g_number)
  then
    l_argument := 'business_group_id';
    raise l_error;
  end if;
  --
  if nvl(p_rec.event_group_id, hr_api.g_number) <>
     nvl(pay_dte_shd.g_old_rec.event_group_id, hr_api.g_number)
  then
    l_argument := 'event_group_id';
    raise l_error;
  end if;
  --
  EXCEPTION
...
--
-- Description:
--   This procedure validates the column_name exist on the table referred to
--
-- Prerequisites:
--
-- In Parameters:
...
--
-- Description:
--   This procedure validates the update type being passed. For the time
--   being we can only have a value of DATETRACK_UPDATE.
--
-- Prerequisites:
--   The table identified by p_dated_table_id already exists.
--
-- In Parameters:
--   A Pl/Sql record structure.
--
...
Begin
  ...
  hr_utility.set_location(l_proc,30);
  --
  if p_rec.proration_style is not null then
    if hr_api.not_exists_in_hrstanlookups(p_effective_date => p_effective_date
                                         ,p_lookup_type    => 'PAY_PRORATION_STYLE'
				         ,p_lookup_code    => p_rec.proration_style) then
      --
      -- The proration style is not recognised
      --
      fnd_message.set_name('PAY','HR_xxxx_INVALID_PRORATION_STL');
      fnd_message.raise_error;
    end if;
  end if;
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  -- commenting this out as bus grp can be null
