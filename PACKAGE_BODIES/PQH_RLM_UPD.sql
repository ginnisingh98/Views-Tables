--------------------------------------------------------
--  DDL for Package Body PQH_RLM_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_RLM_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_rlm_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy pqh_rlm_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_routing_list_members Row
  --
  update pqh_routing_list_members
  set
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in pqh_rlm_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy pqh_rlm_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.role_id = hr_api.g_number) then
    p_rec.role_id :=
    pqh_rlm_shd.g_old_rec.role_id;
  End If;
  If (p_rec.routing_list_id = hr_api.g_number) then
    p_rec.routing_list_id :=
    pqh_rlm_shd.g_old_rec.routing_list_id;
  End If;
  If (p_rec.seq_no = hr_api.g_number) then
    p_rec.seq_no :=
    pqh_rlm_shd.g_old_rec.seq_no;
  End If;
  If (p_rec.approver_flag = hr_api.g_varchar2) then
    p_rec.approver_flag :=
    pqh_rlm_shd.g_old_rec.approver_flag;
  End If;
  If (p_rec.enable_flag = hr_api.g_varchar2) then
    p_rec.enable_flag :=
    pqh_rlm_shd.g_old_rec.enable_flag;
  End If;
  If (p_rec.user_id = hr_api.g_number) then
    p_rec.user_id :=
    pqh_rlm_shd.g_old_rec.user_id;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  pqh_rlm_bus.update_validate(p_rec
  ,p_effective_date);
  --
