--------------------------------------------------------
--  DDL for Package Body OPI_DBI_WIP_SCRAP_INCR_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OPI_DBI_WIP_SCRAP_INCR_PKG" AS
/*$Header: OPIDSCRAPRB.pls 120.0 2005/05/24 19:13:21 appldev noship $ */
...
s_pkg_name CONSTANT VARCHAR2 (50) := 'opi_dbi_wip_scrap_incr_pkg';
...
/*  collect_wip_scrap_incr

    Wrapper routine for WIP Scrap data extraction for incremental load.
    Since there is no OPM data, this wrapper is somewhat unneeded, but
    by including it, the extension of WIP to OPM (if ever) can be done
    easily.
    ...
    Data extraction can runs upto the staging table level and if an error
    ocurrs the routine ends with error.

    If not, then conversion rates have to be calculated for
    all the WIP scrap data in the staging table.
...
IS
    ...
    l_proc_name CONSTANT VARCHAR2 (60) := 'collect_wip_scrap_incr';
    l_stmt_id NUMBER;
...
BEGIN
    ...
    l_stmt_id := 11;
    IF (NOT (opi_dbi_wip_scrap_init_pkg.check_global_setup ())) THEN
    ...
    BEGIN
        ...
        -- STAGING TABLE.
        collect_incr_opi_wip_scrap (errbuf, retcode, l_global_start_date);
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            ...
            BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                               l_proc_name || ' ' ||
                                               '#' || l_stmt_id || ': ' ||
                                 'Unable to collect OPI WIP Scrap data in incremental load into staging table.');

    END;

    -- If extraction failed, then abort here
    l_stmt_id := 50;
    ...
    BIS_COLLECTION_UTILITIES.PUT_LINE
        ('Scrap Incremental load terminated successfully.');
    retcode := s_SUCCESS;
    errbuf := '';
    return;
...
EXCEPTION
    ...
    WHEN schema_info_not_found THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Scrap ETL Incremental Load failed to get OPI schema info.';
        return;
    ...
    WHEN global_setup_missing THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Scrap Incremental Load could not find global setup of global start date and global currency code.';
        return;
    ...
    WHEN data_extraction_failed THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Scrap Incremental Load data extraction failed. Check previous messages for errors. ';
        return;
    ...
    WHEN OTHERS THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Scrap ETL Incremental load Failed.';
        return;


END collect_wip_scrap_incr;


/*  collect_incr_opi_wip_scrap

    Incremental collection of WIP scrap data from MMT/MTA for discrete
    (OPI) organizations.
...
BEGIN
    ...
    -- Return from scrap transactions (MMT type 91) cause WIP scrap
    ...
    -- return from scrap, we need to use -1 * value from MTA.
    --
...
EXCEPTION
    ...
    WHEN could_not_log_success THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Scrap ETL Incremental load OPI data extraction success could not be logged into log table. Aborting.');
    ...
    WHEN cannot_run_incr_load THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Scrap incremental load concurrent program should not be running.
...
If the initial load request set has already been run successfully, please submit the incremental load request set. If not, please run the initial load request set.';
    ...
    WHEN run_common_module THEN
        ...
        BIS_COLLECTION_UTILITIES.PUT_LINE (s_pkg_name || '.' ||
                                           l_proc_name || ' ' ||
                                           '#' || l_stmt_id || ': ' ||
                              'WIP Scrap incremental load concurrent program is running out of turn. Please submit the incremental load request set for incremental data collection.');
    ...
    WHEN OTHERS THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Scrap ETL Incremental load OPI data extraction failed.';
        RAISE;  -- propagate exception to wrapper.

END collect_incr_opi_wip_scrap;
...
BEGIN
    ...
    l_stmt_id := 10;
    MERGE INTO opi_dbi_wip_scrap_f base
    USING
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        errbuf := s_pkg_name || '.' || l_proc_name || ' ' || '#' ||
                  l_stmt_id || ': ' ||
                  'WIP Scrap ETL Incremental merge to fact table failed.';
        RAISE;  -- propagate exception to wrapper.
