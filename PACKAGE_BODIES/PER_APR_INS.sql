--------------------------------------------------------
--  DDL for Package Body PER_APR_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_APR_INS" as
...
g_package  varchar2(33)	:= '  per_apr_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_appraisal_id_i number default null;
--
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
...
Begin
  ...
  --
  per_apr_ins.g_appraisal_id_i := p_appraisal_id;
  --
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------+
Procedure insert_dml(p_rec in out nocopy per_apr_shd.g_rec_type) is
...
Begin
  ...
  -- Insert the row into: per_appraisals

  insert into per_appraisals
  (	appraisal_id,
	business_group_id,
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------+
Procedure pre_insert(p_rec  in out nocopy per_apr_shd.g_rec_type) is

  l_proc  varchar2(72) := g_package||'pre_insert';
  l_exists      varchar2(1);

  Cursor C_Sel1 is select per_appraisals_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_appraisals
     where appraisal_id =
             per_apr_ins.g_appraisal_id_i;
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  ...
  If (per_apr_ins.g_appraisal_id_i is not null)
  then
    --
    ...
    If C_Sel2%found Then
      ...
      fnd_message.set_token('TABLE_NAME','PER_APPRAISALS');
    ...
    --
      p_rec.appraisal_id := per_apr_ins.g_appraisal_id_i;
      per_apr_ins.g_appraisal_id_i := null;
  Else

    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.appraisal_id;
    Close C_Sel1;
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------+
Procedure post_insert(p_rec in per_apr_shd.g_rec_type) is
...
Begin
  ...
  begin
     per_apr_rki.after_insert	(
       ...
       p_update_appraisal             => p_rec.update_appraisal,
       p_provide_overall_feedback     => p_rec.provide_overall_feedback,
       p_appraisee_comments           => p_rec.appraisee_comments,
...
Begin
  ...
  If p_validate then
    ...
    SAVEPOINT ins_per_apr;
  End If;

  -- Call the supporting insert validate operations

  per_apr_bus.insert_validate(p_rec,p_effective_date);
...
Begin
  ...
  p_appraisal_id := l_rec.appraisal_id;
