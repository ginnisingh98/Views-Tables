--------------------------------------------------------
--  DDL for Package Body EDW_POA_DIST_HOOK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."EDW_POA_DIST_HOOK" as
/*$Header: poahkdtb.pls 115.12 2002/01/24 17:54:27 pkm ship    $ */

function Pre_Fact_Collect(p_object_name varchar2) return boolean  is
 begin
   -- Update UNSPSC
   EDW_OWB_COLLECTION_UTIL.write_to_log_file('Entering PO Distributions Pre Fact Hook Procedure');
   ...
   IF (fnd_profile.value('POA_DNB_HOOKS') = 'N') THEN
     ...
     Update POA_EDW_PO_DIST_FSTG Fact
     set UNSPSC_FK =
        (select Com.Function
