--------------------------------------------------------
--  DDL for Package Body BEN_XWC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_XWC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_xwc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_xwc_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_ext_where_clause Row
  --
  update ben_ext_where_clause
  set
  ext_where_clause_id               = p_rec.ext_where_clause_id,
  seq_num                           = p_rec.seq_num,
  oper_cd                           = p_rec.oper_cd,
  val                               = p_rec.val,
  and_or_cd                         = p_rec.and_or_cd,
  ext_data_elmt_id                  = p_rec.ext_data_elmt_id,
  cond_ext_data_elmt_id             = p_rec.cond_ext_data_elmt_id,
  ...
  cond_ext_data_elmt_in_rcd_id      = p_rec.cond_ext_data_elmt_in_rcd_id
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_xwc_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_xwc_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.seq_num = hr_api.g_number) then
    p_rec.seq_num :=
    ben_xwc_shd.g_old_rec.seq_num;
  End If;
  If (p_rec.oper_cd = hr_api.g_varchar2) then
    p_rec.oper_cd :=
    ben_xwc_shd.g_old_rec.oper_cd;
  End If;
  If (p_rec.val = hr_api.g_varchar2) then
    p_rec.val :=
    ben_xwc_shd.g_old_rec.val;
  End If;
  If (p_rec.and_or_cd = hr_api.g_varchar2) then
    p_rec.and_or_cd :=
    ben_xwc_shd.g_old_rec.and_or_cd;
  End If;
  If (p_rec.ext_data_elmt_id = hr_api.g_number) then
    p_rec.ext_data_elmt_id :=
    ben_xwc_shd.g_old_rec.ext_data_elmt_id;
  End If;
  If (p_rec.cond_ext_data_elmt_id = hr_api.g_number) then
    p_rec.cond_ext_data_elmt_id :=
    ben_xwc_shd.g_old_rec.cond_ext_data_elmt_id;
  End If;
  If (p_rec.ext_rcd_in_file_id = hr_api.g_number) then
    p_rec.ext_rcd_in_file_id :=
    ben_xwc_shd.g_old_rec.ext_rcd_in_file_id;
  End If;
  If (p_rec.ext_data_elmt_in_rcd_id = hr_api.g_number) then
    p_rec.ext_data_elmt_in_rcd_id :=
    ben_xwc_shd.g_old_rec.ext_data_elmt_in_rcd_id;
  End If;
  ...
  End If;
  If (p_rec.cond_ext_data_elmt_in_rcd_id = hr_api.g_number) then
    p_rec.cond_ext_data_elmt_in_rcd_id :=
    ben_xwc_shd.g_old_rec.cond_ext_data_elmt_in_rcd_id;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Procedure upd
  ...
  p_rec        in out nocopy ben_xwc_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_xwc_bus.update_validate(p_rec
  ,p_effective_date);
  --
...
Begin
  ...
  --
  l_rec :=
  ben_xwc_shd.convert_args
  (
  p_ext_where_clause_id,
  p_seq_num,
  p_oper_cd,
  p_val,
  p_and_or_cd,
  p_ext_data_elmt_id,
  p_cond_ext_data_elmt_id,
