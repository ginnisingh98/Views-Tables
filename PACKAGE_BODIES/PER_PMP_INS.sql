--------------------------------------------------------
--  DDL for Package Body PER_PMP_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PMP_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_pmp_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_plan_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  per_pmp_ins.g_plan_id_i := p_plan_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: per_perf_mgmt_plans
  --
  insert into per_perf_mgmt_plans
      (plan_id
...
--
  Cursor C_Sel1 is select per_perf_mgmt_plans_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_perf_mgmt_plans
     where plan_id =
             per_pmp_ins.g_plan_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (per_pmp_ins.g_plan_id_i is not null) Then
    --
  ...
  Else
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.plan_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  per_pmp_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  per_pmp_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  per_pmp_ins.post_insert
     (p_effective_date
     ,p_rec
     ,p_duplicate_name_warning
     ,p_no_life_events_warning
...
--
  ...
  l_proc  varchar2(72) := g_package||'ins';
  l_status_code  CONSTANT varchar2(5) := 'DRAFT';
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
