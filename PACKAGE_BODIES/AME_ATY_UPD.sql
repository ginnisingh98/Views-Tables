--------------------------------------------------------
--  DDL for Package Body AME_ATY_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ATY_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_aty_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the ame_action_types Row
    --
    ...
    ,dynamic_description                  = p_rec.dynamic_description
    ,description_query                    = p_rec.description_query
...
Begin
  ...
  begin
    ...
    ame_aty_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_name_o
      => ame_aty_shd.g_old_rec.name
      ,p_procedure_name_o
      => ame_aty_shd.g_old_rec.procedure_name
      ,p_start_date_o
      => ame_aty_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_aty_shd.g_old_rec.end_date
      ,p_description_o
      => ame_aty_shd.g_old_rec.description
      ,p_security_group_id_o
      => ame_aty_shd.g_old_rec.security_group_id
      ,p_dynamic_description_o
      => ame_aty_shd.g_old_rec.dynamic_description
      ,p_description_query_o
      => ame_aty_shd.g_old_rec.description_query
...
Begin
  ...
  If (p_rec.security_group_id = hr_api.g_number) then
    ...
    ame_aty_shd.g_old_rec.security_group_id;
  End If;
  If (p_rec.dynamic_description = hr_api.g_varchar2) then
    p_rec.dynamic_description :=
    ame_aty_shd.g_old_rec.dynamic_description;
  End If;
  If (p_rec.description_query = hr_api.g_varchar2) then
    p_rec.description_query :=
    ame_aty_shd.g_old_rec.description_query;
  End If;
  --
...
Begin
  ...
  --
  ame_aty_upd.convert_defs(p_rec);
  --
  ame_aty_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
Procedure upd
  ...
  ,p_dynamic_description          in     varchar2  default hr_api.g_varchar2
