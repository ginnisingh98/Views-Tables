--------------------------------------------------------
--  DDL for Package Body IGS_PR_SDT_PR_UNT_ST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PR_SDT_PR_UNT_ST_PKG" AS
/* $Header: IGSQI19B.pls 120.0 2005/07/05 12:07:59 appldev noship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PR_SDT_PR_UNT_ST%RowType;
  new_references IGS_PR_SDT_PR_UNT_ST%RowType;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PR_SPUS_SPO_FK');
      IGS_GE_MSG_STACK.ADD;
	  Close cur_rowid;
      App_Exception.Raise_Exception;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PR_SPUS_US_FK');
      IGS_GE_MSG_STACK.ADD;
	  Close cur_rowid;
      App_Exception.Raise_Exception;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PR_SDT_PR_UNT_ST
      where PERSON_ID = X_PERSON_ID
      and COURSE_CD = X_COURSE_CD
      and SPO_SEQUENCE_NUMBER = X_SPO_SEQUENCE_NUMBER
      and UNIT_SET_CD = X_UNIT_SET_CD
...
Before_DML (
  ...
  END IF;
 insert into IGS_PR_SDT_PR_UNT_ST (
    PERSON_ID,
    COURSE_CD,
    SPO_SEQUENCE_NUMBER,
    UNIT_SET_CD,
...
begin
  ...
  (tlinfo.UNIT_SET_CD = X_UNIT_SET_CD) AND
  (tlinfo.VERSION_NUMBER = X_VERSION_NUMBER)
  )
  then
  	null;
  else
