--------------------------------------------------------
--  DDL for Package Body BEN_BUR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BUR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_bur_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_brgng_unit_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   brgng_unit_rt_id PK of record being inserted or updated.
...
--
Procedure chk_brgng_unit_rt_id(p_brgng_unit_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_brgng_unit_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_bur_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_brgng_unit_rt_id                => p_brgng_unit_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_brgng_unit_rt_id,hr_api.g_number)
     <>  ben_bur_shd.g_old_rec.brgng_unit_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_bur_shd.constraint_error('BEN_BRGNG_UNIT_RTS_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_brgng_unit_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_brgng_unit_cd(p_brgng_unit_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_brgng_unit_cd
      <> nvl(ben_bur_shd.g_old_rec.brgng_unit_cd,hr_api.g_varchar2)
      or not l_api_updating)
      and p_brgng_unit_cd is not null then
    --
...
and brgng_unit_rt_id <>  nvl(p_brgng_unit_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and nvl(p_brgng_unit_cd,hr_api.g_varchar2) <> nvl(ben_bur_shd.g_old_rec.brgng_unit_cd,hr_api.g_varchar2)
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Bargaining Unit already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Bargaining Unit criteria',TRUE);
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_brgng_unit_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_bur_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   ...
                   and (brgng_unit_rt_id <> p_brgng_unit_rt_id or p_brgng_unit_rt_id is null);
--
Begin
   hr_utility.set_location('Entering:'||l_proc, 5);
...
function return_legislation_code
  (p_brgng_unit_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_brgng_unit_rt_f b
    where b.brgng_unit_rt_id      = p_brgng_unit_rt_id
