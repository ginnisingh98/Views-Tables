--------------------------------------------------------
--  DDL for Package Body PSP_ELEMENT_GROUPS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSP_ELEMENT_GROUPS_PKG" as
 /* $Header: PSPSUGRB.pls 115.7 2003/09/08 16:11:18 spchakra ship $ */
procedure INSERT_ROW (
  ...
  X_SET_OF_BOOKS_ID IN NUMBER		-- Introduced for bug fix 3098050
    ...
    cursor C is select ROWID from PSP_ELEMENT_GROUPS
      where ELEMENT_GROUP_ID = X_ELEMENT_GROUP_ID;
...
begin
  ...
  insert into PSP_ELEMENT_GROUPS (
    ELEMENT_GROUP_ID,
    ELEMENT_GROUP_NAME,
    START_DATE_ACTIVE,
    END_DATE_ACTIVE,
...
--	Introduced BG/SOB check for bug fix 3098050
    ...
    for update of ELEMENT_GROUP_ID nowait;
  tlinfo c1%rowtype;
...
begin
  ...
  if ( (tlinfo.ELEMENT_GROUP_NAME = X_ELEMENT_GROUP_NAME)
...
begin
  ...
  update PSP_ELEMENT_GROUPS set
    ELEMENT_GROUP_NAME = X_ELEMENT_GROUP_NAME,
  ...
  where ELEMENT_GROUP_ID = X_ELEMENT_GROUP_ID
--	Introduced BG/SOB for bug fix 3098050
  AND	business_group_id = x_business_group_id
  AND	set_of_books_id = x_set_of_books_id
  ;
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from PSP_ELEMENT_GROUPS
     where ELEMENT_GROUP_ID = X_ELEMENT_GROUP_ID
  ;
  dummy c1%rowtype;
begin
  ...
  if (c1%notfound) then
    ...
    INSERT_ROW (
     ...
     X_MODE,
     X_BUSINESS_GROUP_ID,	-- Introduced for bug fix 3098050
     X_SET_OF_BOOKS_ID);	-- Introdcued for bug fix 3098050
    return;
  end if;
  close c1;
  UPDATE_ROW (
...
begin
  delete from PSP_ELEMENT_GROUPS
  where ELEMENT_GROUP_ID = X_ELEMENT_GROUP_ID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
