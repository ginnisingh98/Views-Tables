--------------------------------------------------------
--  DDL for Package Body OKL_ANSWERS_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_ANSWERS_PUB" AS
/* $Header: OKLPAWRB.pls 115.7 2004/04/13 10:32:16 rnaik noship $ */
...
lp_awrv_rec  awrv_rec_type;
lx_awrv_rec  awrv_rec_type;
...
lp_awrv_rec :=  p_awrv_rec;
lx_awrv_rec :=  x_awrv_rec;
...
l_msg_count NUMBER ;
l_return_status VARCHAR2(1);
lp_awrv_tbl  awrv_tbl_type;
lx_awrv_tbl  awrv_tbl_type;
...
l_msg_count := x_msg_count ;
l_msg_data := x_msg_data ;
lp_awrv_tbl :=  p_awrv_tbl;
lx_awrv_tbl :=  x_awrv_tbl;
...
--Copy value of OUT variable in the IN record type
lp_awrv_tbl := lx_awrv_tbl;



--Assign value to OUT variables
x_awrv_tbl  := lx_awrv_tbl;
x_return_status := l_return_status ;
x_msg_count := l_msg_count ;
x_msg_data := l_msg_data ;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
      x_return_status := FND_API.G_RET_STS_UNEXP_ERROR;
      FND_MSG_PUB.ADD_EXC_MSG('OKL_ANSWERS_PUB','lock_answers');
      FND_MSG_PUB.count_and_get(
             p_count   => x_msg_count
            ,p_data    => x_msg_data);
END lock_answers;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      x_msg_count := l_msg_count ;
      x_msg_data := l_msg_data ;
      FND_MSG_PUB.ADD_EXC_MSG('OKL_ANSWERS_PUB','validate_answers');
      FND_MSG_PUB.count_and_get(
             p_count   => x_msg_count
            ,p_data    => x_msg_data);
