--------------------------------------------------------
--  DDL for Package Body IGS_FI_HIER_ACCOUNTS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_FI_HIER_ACCOUNTS_PKG" AS
/* $Header: IGSSIA8B.pls 115.9 2003/03/19 08:36:23 smadathi ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_fi_hier_accounts%ROWTYPE;
  new_references igs_fi_hier_accounts%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.zero_fill_flag                    := x_zero_fill_flag;
  ...
  FUNCTION check_used_in_ftci (
    x_acct_hier_id                      IN     NUMBER
  ) RETURN BOOLEAN AS
  /*
  ||  Created By : jbegum
  ||  Created On : 8-March-2002
  ||  Purpose : Validates whether the account hierarchy has been asssigned to an FTCI .
  ||            Done as part of bug fix of bug#2148665
  ||  Known limitations, enhancements or remarks :
  ||  Change History :
  ||  Who             When            What
  ||  (reverse chronological order - newest change first)
  */
    CURSOR cur_acct_hier IS
      SELECT 'x'
      FROM igs_fi_f_typ_ca_inst
      WHERE acct_hier_id = x_acct_hier_id;

    l_acct_hier cur_acct_hier%ROWTYPE;

  BEGIN

    OPEN cur_acct_hier;
    FETCH cur_acct_hier INTO l_acct_hier;
    IF (cur_acct_hier%FOUND) THEN
      CLOSE cur_acct_hier;
      RETURN(TRUE);
    ELSE
      CLOSE cur_acct_hier;
  ...
  */
  BEGIN
    ...
    igs_fi_hier_acct_tbl_pkg.get_fk_igs_fi_hier_accounts (
      old_references.acct_hier_id
    );
  ...
  */
  BEGIN
    ...
    IF ( get_uk_for_validation (new_references.name)) THEN
  ...
  */
  ...
  FUNCTION get_uk_for_validation ( x_name IN VARCHAR2
  ) RETURN BOOLEAN AS
  /*
  ||  Created By : SMADATHI
  ||  Created On : 06-MAR-2003
  ...
  */
    ...
    l_v_rowid c_rowid%ROWTYPE;

  BEGIN

    OPEN c_rowid;
    FETCH c_rowid INTO l_v_rowid;
    IF (c_rowid%FOUND) THEN
      CLOSE c_rowid;
        RETURN (TRUE);
        ELSE
       CLOSE c_rowid;
  ...
  */
  BEGIN
    ...
    ELSIF (p_action = 'DELETE') THEN
      ...
      IF ( check_used_in_ftci (
             old_references.acct_hier_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_FI_ACCT_HIER_NO_DEL');
        igs_ge_msg_stack.add;
        app_exception.raise_exception;
      END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ...
        AND (tlinfo.zero_fill_flag = x_zero_fill_flag)
