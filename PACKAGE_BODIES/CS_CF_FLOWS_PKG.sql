--------------------------------------------------------
--  DDL for Package Body CS_CF_FLOWS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CS_CF_FLOWS_PKG" as
/* $Header: CSCFFLWB.pls 120.0 2005/06/01 09:59:55 appldev noship $ */
...
begin
  ...
  if (    ((recinfo.ATTRIBUTE7 = X_ATTRIBUTE7)
      ...
      AND ((recinfo.FLOW_TYPE_CODE = X_FLOW_TYPE_CODE)
  ...
  for tlinfo in c1 loop
    if (tlinfo.BASELANG = 'Y') then
      if (    ((tlinfo.FLOW_DISPLAY_NAME = X_FLOW_DISPLAY_NAME)
               OR ((tlinfo.FLOW_DISPLAY_NAME is null) AND (X_FLOW_DISPLAY_NAME is null)))
      ) then
        null;
      else
        fnd_message.set_name('FND', 'FORM_RECORD_CHANGED');
