--------------------------------------------------------
--  DDL for Package Body BEN_HSR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_HSR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_hsr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_hrly_slrd_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   hrly_slrd_rt_id PK of record being inserted or updated.
...
--
Procedure chk_hrly_slrd_rt_id(p_hrly_slrd_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_hrly_slrd_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_hsr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_hrly_slrd_rt_id                => p_hrly_slrd_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_hrly_slrd_rt_id,hr_api.g_number)
     <>  ben_hsr_shd.g_old_rec.hrly_slrd_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_hsr_shd.constraint_error('BEN_HYRLY_SLRD_RTS_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_hrly_slrd_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_hrly_slrd_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_hsr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
-- In Parameters
--   hrly_slrd_rt_id PK of record being inserted or updated.
--   hrly_slrd_cd Value of lookup code.
--   effective_date effective date
--   object_version_number Object version number of record being
--                         inserted or updated.
--
...
--   Internal table handler use only.
--
Procedure chk_hrly_slrd_cd(p_hrly_slrd_rt_id                in number,
                            p_hrly_slrd_cd               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_hrly_slrd_cd';
  l_api_updating boolean;
  --
Begin
  --
  ...
  if (l_api_updating
      and p_hrly_slrd_cd
      <> nvl(ben_hsr_shd.g_old_rec.hrly_slrd_cd,hr_api.g_varchar2)
      or not l_api_updating)
      and p_hrly_slrd_cd is not null then
    --
    -- check if value of lookup falls within lookup type.
    --
    if hr_api.not_exists_in_hr_lookups
          (p_lookup_type    => 'HOURLY_SALARIED_CODE',
           p_lookup_code    => p_hrly_slrd_cd,
           p_effective_date => p_effective_date) then
      --
      -- raise error as does not exist as lookup
      --
...
--
-- In Parameters
--   p_hrly_slrd_rt_id	     PK of record being inserted or updated.
--   p_hrly_slrd_cd	     Value of lookup code.
...
where hrly_slrd_cd = p_hrly_slrd_cd
and vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
and hrly_slrd_rt_id <> nvl(p_hrly_slrd_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_hrly_slrd_cd <> nvl(ben_hsr_shd.g_old_rec.hrly_slrd_cd,hr_api.g_varchar2)
      or not l_api_updating) then
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Hourly Salaried already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Hourly / Salaried criteria',TRUE);
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and hrly_slrd_rt_id   <> nvl(p_hrly_slrd_rt_id,-1)
                   --and hrly_slrd_cd   <> nvl(p_hrly_slrd_cd,-1)
...
function return_legislation_code
  (p_hrly_slrd_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_hrly_slrd_rt_f b
    where b.hrly_slrd_rt_id      = p_hrly_slrd_rt_id
