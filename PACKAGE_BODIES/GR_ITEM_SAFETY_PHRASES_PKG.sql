--------------------------------------------------------
--  DDL for Package Body GR_ITEM_SAFETY_PHRASES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_ITEM_SAFETY_PHRASES_PKG" AS
/*$Header: GRHIISPB.pls 115.6 2002/10/25 21:11:41 methomas ship $*/
PROCEDURE Insert_Row
				  ...
				  p_temperature_scale IN VARCHAR2,
...
/*     Initialization Routine */
   ...
   x_oracle_error := 0;
   x_msg_data := NULL;
   l_msg_token := p_item_code || ' ' || p_safety_phrase_code;
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_item_safety_phrases
   		  	     (item_code,
				  safety_phrase_code,
				  temperature,
				  temperature_scale,
...
CURSOR c_lock_item_safetys
 IS
   SELECT	*
   FROM		gr_item_safety_phrases
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockSafetyRcd	  c_lock_item_safetys%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_item_safetys;
   FETCH c_lock_item_safetys INTO LockSafetyRcd;
   IF c_lock_item_safetys%NOTFOUND THEN
	  CLOSE c_lock_item_safetys;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_item_safety_phrases
   WHERE  	   rowid = p_rowid;
...
**		'S' - Delete all rows for the specified safety phrase.
*/
   ...
   ELSIF p_delete_option = 'S' THEN
      IF p_safety_phrase_code IS NULL THEN
	     l_msg_token := 'Safety Phrase Code';
		 RAISE Null_Delete_Option_Error;
	  ELSE
...
/*   Risk Phrase */
   ...
   GR_SAFETY_PHRASES_B_PKG.Check_Primary_Key
						(p_safety_phrase_code,
						 'F',
						 l_rowid,
						 l_key_exists);
...
**	      p_safety_phrase_code is the second part of the key
...
CURSOR c_get_safety_phrase_rowid
 IS
   SELECT isp.rowid
   FROM	  gr_item_safety_phrases isp
   WHERE  isp.item_code = p_item_code
   AND	  isp.safety_phrase_code = p_safety_phrase_code;
ItemSafetyRecord		c_get_safety_phrase_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_item_code || ' ' || p_safety_phrase_code;

   x_key_exists := 'F';
   OPEN c_get_safety_phrase_rowid;
   FETCH c_get_safety_phrase_rowid INTO ItemSafetyRecord;
   IF c_get_safety_phrase_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := ItemSafetyRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_safety_phrase_rowid;
...
END GR_ITEM_SAFETY_PHRASES_PKG;
