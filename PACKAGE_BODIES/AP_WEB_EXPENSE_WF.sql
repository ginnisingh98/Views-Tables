--------------------------------------------------------
--  DDL for Package Body AP_WEB_EXPENSE_WF
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_WEB_EXPENSE_WF" AS
/* $Header: apwxwkfb.pls 120.244.12010000.45 2010/06/14 08:34:46 rveliche ship $ */
...
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    -- 9412210: if report is in a currency other than USD then convert to USD
    -- and compare
    if (nvl(l_currency,'USD') <> 'USD') then
       BEGIN
       gl_currency_api.convert_closest_amount( l_currency ,
                                            'USD' ,
                                            SYSDATE ,
                                            null ,
                                            null ,
                                            l_sum_missing_receipts ,
                                            7 ,
                                            l_converted_amount ,
                                            denominator ,
                                            numerator ,
      ...
      EXCEPTION
        WHEN OTHERS THEN
          -- l_sum_missing_receipts cannot be converted, hence use the same
          l_converted_amount := l_sum_missing_receipts;
      END;
    else
      l_converted_amount := l_sum_missing_receipts;
    end if;
    ...
    l_debug_info := 'If Report Amount is Less than AP Limit Return N
                     otherwise return Y';
    -----------------------------------------------------------------
    IF (nvl(l_converted_amount,0) <= nvl(l_sum_limit,0)) THEN
...
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    IF (AP_WEB_UTILITIES_PKG.Oie_Round_Currency(l_orig_exp_report_amt,l_reimb_currency) <>
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    IF (l_receipt_type = 'ORIGINAL') THEN
	    ...
	    ELSE
	      p_result := 'COMPLETE:N';
	    END IF;
    ELSIF (l_receipt_type = 'IMAGE') THEN
	    l_no_receipts_shortpay_id := WF_ENGINE.GetItemAttrNumber(p_item_type,
								     p_item_key,
							    'NO_IMG_RECEIPTS_SHORTPAY_ID');
	    IF (l_no_receipts_shortpay_id IS NOT NULL) THEN
	      p_result := 'COMPLETE:Y';
	    ELSE
	      p_result := 'COMPLETE:N';
	    END IF;
    ELSIF (l_receipt_type = 'BOTH') THEN
           l_no_receipts_shortpay_id := WF_ENGINE.GetItemAttrNumber(p_item_type,
                                                                     p_item_key,
                                                            'NO_BOTH_RECEIPTS_SHORTPAY_ID');
            IF (l_no_receipts_shortpay_id IS NOT NULL) THEN
              p_result := 'COMPLETE:Y';
            ELSE
              p_result := 'COMPLETE:N';
...
BEGIN
  ...
  AP_WEB_UTILITIES_PKG.logProcedure('AP_WEB_EXPENSE_WF', 'start CreateShortPayExpReport');
    ...
    /*Bug#8976900 - Add Reference for attachments from parent expense report
      to the short-paid expense report*/
    --------------------------------------------------------------------
    l_debug_info := 'Creating reference for Header Attachments';
    --------------------------------------------------------------------

    AP_WEB_DB_EXPRPT_PKG.CopyAttachments(p_orig_expense_report_id,
                                         p_new_expense_report_id,
  ...
  ELSE
    ...
    IF (p_shortpay_type = 'NO_RECEIPTS') THEN
	    IF (AP_WEB_DB_EXPLINE_PKG.AddUnverifiedShortpaidLines(
				p_new_expense_report_id, p_orig_expense_report_id)) THEN
		NULL;
	    END IF;
    ELSIF (p_shortpay_type = 'NO_IMAGE_RECEIPTS') THEN
	    IF (AP_WEB_DB_EXPLINE_PKG.AddUnverifiedImgShortpaidLines(
				p_new_expense_report_id, p_orig_expense_report_id)) THEN
		NULL;
	    END IF;
    ELSE
            IF (AP_WEB_DB_EXPLINE_PKG.AddUnverifiedBtShortpaidLines(
				p_new_expense_report_id, p_orig_expense_report_id)) THEN
		NULL;
	    END IF;
    END IF;
...
----------------------------------------------------------------------
  ...
  l_no_img_receipts_shortpay_id	  AP_WEB_DB_EXPRPT_PKG.expHdr_headerID;
  l_no_both_receipts_shortpay_id  AP_WEB_DB_EXPRPT_PKG.expHdr_headerID;
  ...
  l_image_count			  NUMBER := 0;
  l_both_count			  NUMBER := 0;
  l_shortpaid_count               NUMBER := 0;
  l_missing_receipt_count         NUMBER := 0;
  /* jrautiai ADJ Fix - End */
  l_shortpay_suffix		  VARCHAR2(3) := '-1';
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
     ...
     */
    IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumImageShortpaidLines(l_report_header_id, l_image_count)) THEN
      l_image_count := 0;
    END IF;

    IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumBothShortpaidLines(l_report_header_id, l_both_count)) THEN
      l_both_count := 0;
    END IF;

    --l_missing_receipt_count := l_shortpaid_count - l_policy_count - l_image_count - l_both_count;
    IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumOriginalShortpaidLines(l_report_header_id, l_missing_receipt_count)) THEN
      l_missing_receipt_count := 0;
    END IF;
    ...
    l_debug_info := 'Retrieve Policy ShortPay New Expense Report Id';
    -----------------------------------------------------------------
      IF (NOT AP_WEB_DB_EXPRPT_PKG.GetNextExpReportID(l_no_img_receipts_shortpay_id)) THEN
	l_no_img_receipts_shortpay_id := NULL;
      END IF;

      IF (l_policy_count > 0 AND l_missing_receipt_count > 0) THEN
        l_no_receipts_shortpay_doc_num := l_document_number || '-3';
	l_shortpay_suffix := '-4';
      ELSIF (l_missing_receipt_count > 0) THEN
        l_no_receipts_shortpay_doc_num := l_document_number || '-2';
	l_shortpay_suffix := '-3';
      ELSE
        l_no_receipts_shortpay_doc_num := l_document_number || '-1';
	l_shortpay_suffix := '-2';
      ...
      /* jrautiai ADJ Fix Start
       * recalculate the shortpaid totals after it has been created*/
      IF (NOT AP_WEB_DB_EXPLINE_PKG.CalculateAmtsDue(l_no_img_receipts_shortpay_id,
                                                     l_payment_due,
                                                     l_no_receipts_emp_amt,
                                                     l_no_receipts_ccard_amt,
                                                     l_no_receipts_shortpay_total)) THEN
        ...
        l_no_receipts_shortpay_total := 0;
      END IF;
      IF (NOT AP_WEB_DB_EXPRPT_PKG.SetAmtDuesAndTotal(l_no_img_receipts_shortpay_id,
                                                      l_no_receipts_ccard_amt,
                                                      l_no_receipts_emp_amt,
                                                      l_no_receipts_shortpay_total)) THEN
        NULL;
      ...
      WF_ENGINE.SetItemAttrText(p_item_type,
			        p_item_key,
			        'NO_IMG_RECPT_SHORTPAY_DOC_NUM',
			        l_no_receipts_shortpay_doc_num);

      WF_ENGINE.SetItemAttrNumber(p_item_type,
			          p_item_key,
			          'NO_IMG_RECPT_SHORTPAY_TOTAL',
			          l_no_receipts_shortpay_total);

    END IF;

    IF (l_both_count > 0) THEN
    -----------------------------------------------------------------
    l_debug_info := 'Retrieve Policy ShortPay New Expense Report Id';
    -----------------------------------------------------------------
      IF (NOT AP_WEB_DB_EXPRPT_PKG.GetNextExpReportID(l_no_both_receipts_shortpay_id)) THEN
	l_no_img_receipts_shortpay_id := NULL;
      END IF;


      l_no_receipts_shortpay_doc_num := l_document_number || l_shortpay_suffix;
      ...
      CreateShortpayExpReport(l_report_header_id,
			      l_no_both_receipts_shortpay_id,
			      l_no_receipts_shortpay_doc_num,
			      0,
                              0,
			      0,
      ...
      /* jrautiai ADJ Fix Start
       * recalculate the shortpaid totals after it has been created*/
      IF (NOT AP_WEB_DB_EXPLINE_PKG.CalculateAmtsDue(l_no_both_receipts_shortpay_id,
                                                     l_payment_due,
                                                     l_no_receipts_emp_amt,
                                                     l_no_receipts_ccard_amt,
                                                     l_no_receipts_shortpay_total)) THEN
        ...
        l_no_receipts_shortpay_total := 0;
      END IF;
      IF (NOT AP_WEB_DB_EXPRPT_PKG.SetAmtDuesAndTotal(l_no_both_receipts_shortpay_id,
                                                      l_no_receipts_ccard_amt,
                                                      l_no_receipts_emp_amt,
                                                      l_no_receipts_shortpay_total)) THEN
        NULL;
      ...
      WF_ENGINE.SetItemAttrText(p_item_type,
			        p_item_key,
			        'NO_BOTH_RECPT_SHORTPAY_DOC_NUM',
			        l_no_receipts_shortpay_doc_num);

      WF_ENGINE.SetItemAttrNumber(p_item_type,
			          p_item_key,
			          'NO_BOTH_RECPT_SHORTPAY_TOTAL',
			          l_no_receipts_shortpay_total);
    ...
    IF l_no_img_receipts_shortpay_id IS NOT NULL THEN
      AP_WEB_RECEIPT_MANAGEMENT_UTIL.handle_event(l_no_img_receipts_shortpay_id,AP_WEB_RECEIPT_MANAGEMENT_UTIL.C_EVENT_MIR_SHORTPAY);
    END IF;

    IF l_no_both_receipts_shortpay_id IS NOT NULL THEN
      AP_WEB_RECEIPT_MANAGEMENT_UTIL.handle_event(l_no_both_receipts_shortpay_id,AP_WEB_RECEIPT_MANAGEMENT_UTIL.C_EVENT_MBR_SHORTPAY);
    END IF;
   ...
   FOR i in 1 .. l_childItemKeySeq LOOP
    ...
    if ((((l_AmeRejectedChildItemKey is null) or (l_childItemKey <> l_AmeRejectedChildItemKey)) and
       ((l_ap_reject_return = 'N') or
        (l_ap_reject_return = 'Y' and (l_AmeApprovedChildItemKey is null or l_childItemKey <> l_AmeApprovedChildItemKey))) )
        or
        (p_withdraw = 'Y'))
    then
...
BEGIN
  ...
  */
	   ...
	   END IF;
           ...
           AP_WEB_DB_EXPRPT_PKG.CopyAttachments(l_report_header_id,
                                                l_new_report_id,
                                                'OIE_HEADER_ATTACHMENTS');
...
-----------------------------------------------------------------
  ...
  -- jrautiai ADJ Fix
  l_mess                        Wf_Item_Attribute_Values.TEXT_VALUE%TYPE;
  l_missing_receipt_type	VARCHAR2(50);
BEGIN

  AP_WEB_UTILITIES_PKG.logProcedure('AP_WEB_EXPENSE_WF', 'start StartNoReceiptsShortPayProcess');

  l_missing_receipt_type := WF_ENGINE.GetActivityAttrText(p_item_type,
                                                   p_item_key,
                                                   p_actid,
                                                   'MISSING_RECEIPT_TYPE');
  ...
  l_debug_info := 'Retrieve New Expense_Report_ID Item Attribute';
  ------------------------------------------------------------
  IF (l_missing_receipt_type = 'ORIGINAL') THEN
	  l_report_header_id := WF_ENGINE.GetItemAttrNumber(p_item_type,
						    p_item_key,
						    'NO_RECEIPTS_SHORTPAY_ID');
  ...
  ELSIF (l_missing_receipt_type = 'IMAGE') THEN
	  ...
	  FND_MESSAGE.SET_NAME('SQLAP','OIE_MISSING_RECPT_NOTIF_INSTR');

	  FND_MESSAGE.Set_Token('RPT_ID', l_orig_expense_report_num);
          l_mess := FND_MESSAGE.GET;
  ...
  LOOP
    ...
    FOR i in 1..(l_counter -1) LOOP
        ...
        IF l_approver_id = l_manager_id(i) THEN
           ...
           ---------------------------------------------
           -- 3257576
           FND_MESSAGE.Set_Name('SQLAP', 'OIE_CIRCULAR_REF_APRVL_CHAIN');
           p_error_message := FND_MESSAGE.Get;
           FND_MESSAGE.Set_Name('SQLAP', 'OIE_NO_APPROVER_INSTR6');
           p_instructions := FND_MESSAGE.Get;
           FND_MESSAGE.Set_Name('SQLAP', 'OIE_NO_APPROVER_SPL_INSTR');
...
l_document_max            NUMBER:=25000;--namrata
l_debug_info              VARCHAR2(1000);
l_item_type               VARCHAR2(30);
l_report_header_id	  AP_WEB_DB_EXPLINE_PKG.expLines_headerID;
l_adjustment_line	  VARCHAR2(2000);
  	 ...
  	 AP_WEB_DISC_PKG.getPrompts(200,'AP_WEB_WF_LINETABLE',l_title,l_prompts);
         ...
         */
         ...
         ELSE
           ...
           ELSIF (l_adj_code = 'AUDIT') THEN -- no table header for AUDIT
             document := '<br>';
...
BEGIN
  ...
  IF (p_s_command = 'SET_CTX') THEN
    ...
    -- Set Accounting Flexfield validation context
    AP_WEB_DB_GL_INT_PKG.set_aff_validation_org_context(l_n_org_id);
  ...
  ELSIF (p_s_command = 'TEST_CTX') THEN
     ...
     /* Bug 4711393 : Should return NOTSET instead of FALSE in test_ctx mode
      *               so that the actual context can get set via call in
      *               set_ctx mode. This would avoid incorrect deferred state
      */
     IF (nvl(mo_global.get_access_mode, 'NULL') <> 'S') THEN
        p_s_result := 'NOTSET';
     ELSIF l_n_user_id IS NULL
     ...
     ELSIF l_current_user_id IS NULL
       ...
       OR l_current_resp_appl_id IS NULL
       OR l_current_resp_id = -1
       OR l_current_resp_appl_id = -1 THEN
...
BEGIN
  ...
  -- Bug 7155445(sodash) Used the AME API AME_API6.updateApprovalStatus so that the comments of one approver is visible to other approvers
  IF(p_funmode = 'RESPOND') THEN
    ...
    if (l_message_name = 'OIE_AME_EXPENSE_REPORT_APPRVL' OR
        l_message_name = 'OIE_REQ_EXPENSE_REPORT_APPRVL') then
	...
	l_forwarder.name := l_approver_name;
	  ...
	  ELSIF (l_approverResponse = 'NO_RESPONSE' or l_approverResponse IS NULL) THEN -- Bug 9011191(sodash): Added this clause because in case of time out, the value of the attribute RESULT is set as NULL by workflow
	...
	AME_API6.updateApprovalStatus(applicationIdIn    => AP_WEB_DB_UTIL_PKG.GetApplicationID,
      ...
      END IF; -- (nvl(l_AMEEnabled,'N')='Y')
    end if;
  END IF;  --  (p_funmode = 'RESPOND')
...
'||wf_engine.context_user_comment,
    ...
    if (p_funmode in ('TRANSFER') OR (l_AMEEnabled = 'Y')) then
	    ...
	    if (l_TransferNotificationID is not null) then
	      ...
	      WF_NOTIFICATION.SetAttrText(l_TransferNotificationID,
              ...
              WF_NOTIFICATION.SetAttrText(l_TransferNotificationID,
                                          ...
                                          WF_ENGINE.GetItemAttrText(p_item_type,
                                                                    p_item_key,
                                                                    'APPROVER_DISPLAY_NAME'));
              WF_NOTIFICATION.SetAttrNumber(l_TransferNotificationID,
                                            'EXPENSE_REPORT_ID',
	      ...
	      wf_notification.denormalize_notification(l_TransferNotificationID);
	    end if;
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    IF (l_expense_status_code = 'INVOICED') THEN
       ...
       IF (l_holds_setup = 'Y') THEN
          l_expense_status_code := AP_WEB_RECEIPTS_WF.C_PENDING_HOLDS;
       END IF;
    ELSIF (l_expense_status_code = 'REJECTED') THEN

       --Delete the duplicate detection violations
    ...
    IF (AP_WEB_DB_HR_INT_PKG.IsPersonActive(l_employee_id) = 'Y') THEN
      p_result := 'COMPLETE:Y';
    ELSE
      p_result := 'COMPLETE:N';
    END IF;
...
BEGIN
  ...
  if l_wf_exist then
     ...
     -- Bug: 8537082, No workflow exists for reports in error, moved the code here
...
BEGIN
  ...
  IF (l_distance_thresholds_flag is not null) THEN
     IF ((l_distance_uom = C_KILOMETERS) AND (l_distance_unit_code = C_MILES)) THEN
       -- if l_distance_unit_code = C_MILES then
...
BEGIN
  -- Bug: 7330731, Null returned from here causes invalid splits
...
BEGIN
  ...
  FOR i IN 1..l_sl_array_count LOOP
    ...
    IF (i = 1) THEN
      ...
      IF (round(p_mileage_line_array(p_ml_index).trip_distance - l_over_threshold_distance)) <= 0  THEN
...
BEGIN
  ...
  IF (l_mileage_array_count > 0 ) THEN
	...
	-- Bug: 6476888, NO_DATA_FOUND, wrong index used.
	...
	-- Bug: 6617094, Expense Type not populated for CustomCalculateAmount.
 	AP_WEB_DFLEX_PKG.PopulateExpTypeInLineRec(l_mileage_line);
	...
	-- Bug: 6476888, VARRAY can be atomically null, CustomCalculateAmount need not initialize this for Mileage
...
-------------------------------------------------------------------------------
  ...
  CURSOR recpt_cur(p_report_header_id IN NUMBER) IS
...
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    ELSE
      ...
      ELSIF audit_rec.audit_code = 'RECEIPT_BASED' THEN

	OPEN recpt_cur(l_report_header_id);
        FETCH recpt_cur INTO recpt_rec;

        IF recpt_rec.recpt_assign_stage_code = 'MGR_APPR_IMG' THEN

	  select nvl(image_receipts_status,'NOT_REQUIRED') into l_image_receipts_status from
	...
	p_result := 'COMPLETE:RECEIPT_BASED';
...
----------------------------------------------------------------------
  l_debug_info                  VARCHAR2(200);
  l_auditor_name                fnd_user.user_name%type;
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    l_debug_info := 'Set #FROM_ROLE to AP';
    ----------------------------------------------------------------
    -- Setting the from role to the last auditor Bug 8371199 (sodash)
    begin
    ...
    exception
      when no_data_found then
        l_auditor_name :=  WF_ENGINE.GetItemAttrText(p_item_type,
                                                     p_item_key,
                                                     'PAYABLES');
    end;
...
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    IF audit_rec.audit_code = 'PAPERLESS_AUDIT' THEN
      ...
      ------------------------------------------------------------
      AP_WEB_AUDIT_QUEUE_UTILS.enqueue_for_audit(l_report_header_id);
    ELSIF (audit_rec.audit_code = 'AUDIT' OR audit_rec.audit_code = 'RECEIPT_BASED') THEN
      ------------------------------------------------------------
      l_debug_info := 'Add to Audit queue';
      ------------------------------------------------------------
      --Bug#6618522
      AP_WEB_AUDIT_QUEUE_UTILS.assign_to_last_auditor(l_report_header_id);
    END IF;
  ...
  IF (p_funmode = 'RUN') THEN
     ...
     FOR i IN 1 .. l_nextApproversOut.count LOOP
       ...
       WF_ENGINE.SetItemAttrText(p_item_type,
			      ...
			      l_manager_display_name);
      ----------------------------------------------------------------
      -- Bug 7272553: Store approver authority, used during reassign
	...
	WHEN OTHERS THEN
	  if (wf_core.error_name = 'WFENG_ITEM_ATTR') then
       ...
       -- Bug: 7649190, Purpose not shown in the manager approval notification when AME is enabled
       WF_ENGINE.SetItemAttrText(p_item_type,
                                   l_childItemKey,
                                   'PURPOSE',
                                   WF_ENGINE.GetItemAttrText(p_item_type,
...
BEGIN
  ...
  IF (p_funmode = 'RUN') THEN
    ...
    -- Bug 7155445 (sodash) status is updated in postnotif function IsApprovalRequestTransferred
    /*AME_API2.updateApprovalStatus(applicationIdIn    => AP_WEB_DB_UTIL_PKG.GetApplicationID,
                              	    transactionTypeIn  => p_item_type,
                               	    transactionIdIn    => l_AmeMasterItemKey,
...
----------------------------------------------------------------------
l_report_header_id		AP_WEB_DB_EXPRPT_PKG.expHdr_headerID;
l_req_mgr_appr			VARCHAR2(1);
l_img_recpt_status		VARCHAR2(30);
BEGIN
  IF (p_funmode = 'RUN') THEN
	l_report_header_id := WF_ENGINE.GetItemAttrNumber(p_item_type,
                                                    p_item_key,
	...
	ELSE
		...
		IF (l_req_mgr_appr = 'Y' AND l_img_recpt_status NOT IN ('RECEIVED','NOT_REQUIRED')) THEN
  ...
  EXCEPTION
	WHEN NO_DATA_FOUND THEN
		p_result := 'COMPLETE:N';
END WaitForImagedReceipts;

PROCEDURE CompleteReceiptsBlock(p_report_header_id IN VARCHAR2) IS
BEGIN

	WF_ENGINE.CompleteActivity('APEXP',
				p_report_header_id,
                               'HOLD_FOR_IMAGE_RECEIPTS',
                               '');

EXCEPTION
	WHEN OTHERS THEN
		NULL;
END CompleteReceiptsBlock;
...
l_report_header_id		AP_WEB_DB_EXPRPT_PKG.expHdr_headerID;
l_image_receipts_status		VARCHAR2(50);
BEGIN
...
IF (p_funmode = 'RUN') THEN
	...
	l_report_header_id := WF_ENGINE.GetItemAttrNumber(p_item_type,
                                                    p_item_key,
                                                    'EXPENSE_REPORT_ID');
        l_image_receipts_status := WF_ENGINE.GetActivityAttrText(p_item_type,
                                                   p_item_key,
                                                   p_actid,
...
l_report_header_id		AP_WEB_DB_EXPRPT_PKG.expHdr_headerID;
l_orig_receipts_status		VARCHAR2(50);
BEGIN
...
IF (p_funmode = 'RUN') THEN
	...
	l_report_header_id := WF_ENGINE.GetItemAttrNumber(p_item_type,
        ...
        l_orig_receipts_status := WF_ENGINE.GetActivityAttrText(p_item_type,
                                                   p_item_key,
                                                   p_actid,
                                                   'ORIGINAL_RECEIPTS_STATUS');
...
PROCEDURE CheckShortPayRecptType(p_item_type      IN VARCHAR2,
                       ...
                       p_result  OUT NOCOPY VARCHAR2) IS
----------------------------------------------------------------------
l_receipts_issue	number;
l_line_count		number;
l_shortpay_count	number;
...
BEGIN
  IF (p_funmode = 'RUN') THEN
	...
	WHERE report_header_id = l_report_header_id;
        ...
        IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumShortpaidLines(l_report_header_id, l_shortpay_count)) THEN
          l_shortpay_count := 0;
        END IF;
	...
	IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumPolicyShortpaidLines(l_report_header_id, l_policy_count)) THEN
        ...
        IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumOriginalShortpaidLines(l_report_header_id, l_receipts_issue)) THEN
          l_receipts_issue := 0;
        END IF;

        IF (NOT AP_WEB_DB_EXPLINE_PKG.GetNumImageShortpaidLines(l_report_header_id, l_img_count)) THEN
          l_img_count := 0;
        END IF;
        ...
        --l_receipts_issue := l_shortpay_count - l_policy_count - l_img_count - l_both_count;


	IF (l_receipts_issue > 0) THEN
		p_result := 'COMPLETE:ORIGINAL';
	ELSIF(l_img_count > 0) THEN
		p_result := 'COMPLETE:IMAGE';
        ELSIF(l_both_count > 0) THEN
...
END CheckShortPayRecptType;

-------------------------------------------------------------------------------------
FUNCTION GetImageMissingJustification(p_report_header_id IN NUMBER) RETURN VARCHAR2 IS
--------------------------------------------------------------------------------------
l_missing_image_just AP_EXPENSE_REPORT_HEADERS.MISSING_IMG_JUST%TYPE;
BEGIN
 ...
 RETURN l_missing_image_just;
EXCEPTION
 WHEN OTHERS THEN
  return NULL;

END GetImageMissingJustification;
