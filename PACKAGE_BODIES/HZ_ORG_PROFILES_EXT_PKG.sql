--------------------------------------------------------
--  DDL for Package Body HZ_ORG_PROFILES_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_ORG_PROFILES_EXT_PKG" as
/* $Header: ARHEXOPB.pls 120.4 2006/02/09 10:14:48 jhuang noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from HZ_ORG_PROFILES_EXT_B
...
begin
  ...
  close c;
  if (    ((recinfo.N_EXT_ATTR19 = X_N_EXT_ATTR19)
      ...
      AND (recinfo.ORGANIZATION_PROFILE_ID = X_ORGANIZATION_PROFILE_ID)
...
begin
  update HZ_ORG_PROFILES_EXT_B set
    ...
    ORGANIZATION_PROFILE_ID = X_ORGANIZATION_PROFILE_ID,
  ...
  update HZ_ORG_PROFILES_EXT_TL set
...
begin
  delete from HZ_ORG_PROFILES_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from HZ_ORG_PROFILES_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  insert into /*+ append parallel(tt) */ HZ_ORG_PROFILES_EXT_TL tt (
    EXTENSION_ID,
    ORGANIZATION_PROFILE_ID,
    ATTR_GROUP_ID,
    CREATED_BY,
  ...
  (SELECT /*+ no_merge ordered parallel(b) */
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
   FROM HZ_ORG_PROFILES_EXT_TL B ,
        FND_LANGUAGES L
   WHERE L.INSTALLED_FLAG IN ( 'I','B' )
   AND B.LANGUAGE = USERENV ( 'LANG' )
  ) v, HZ_ORG_PROFILES_EXT_TL t
