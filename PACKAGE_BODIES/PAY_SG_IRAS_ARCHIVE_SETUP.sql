--------------------------------------------------------
--  DDL for Package Body PAY_SG_IRAS_ARCHIVE_SETUP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_SG_IRAS_ARCHIVE_SETUP" as
/* $Header: pysgirsu.pkb 120.1.12010000.2 2009/12/09 02:17:22 jalin ship $ */
  g_debug boolean;
  ...
  begin
      ...
      l_rep_req_id := FND_REQUEST.SUBMIT_REQUEST (
 	                                         APPLICATION          =>   'PAY',
                                                 PROGRAM              =>   'PYSGIRAR',
                                                 ARGUMENT1            =>   'ARCHIVE',
                                                 ARGUMENT2            =>   'SG_IRAS_ARCHIVE',
						 ARGUMENT3            =>   'SG',
						 ARGUMENT4            =>   null,
						 ARGUMENT5            =>   null,
						 ARGUMENT6            =>   'REPORT',
						 ARGUMENT7            =>   v_business_group_id,
						 ARGUMENT8            =>   v_magnetic_file_name,
						 ARGUMENT9            =>   null,
						 ARGUMENT10           =>   v_action_parameter_group,
                                                 ARGUMENT11           =>   v_legal_entity,
                                                 ARGUMENT12           =>   v_basis_year,
                                                 ARGUMENT13           =>   'START_DATE='||to_char(v_basis_year)||'/01/01 00:00:00',
                                                 ARGUMENT14           =>   'END_DATE='||to_char(v_basis_year)||'/12/31 00:00:00',
                                                 ARGUMENT15           =>   p_payroll_action_id,
                                                 ARGUMENT16           =>   'BUSINESS_GROUP_ID='||v_business_group_id||' LEGAL_ENTITY_ID='||v_legal_entity||
                                                                           ' SETUP_ACTION_ID='||p_payroll_action_id||'  BASIS_YEAR='||v_basis_year||' '||
                                                                           'END_DATE='||to_char(v_basis_year)||'/12/31 00:00:00'  );
      if g_debug then
...
begin
  l_package_name := 'pay_sg_iras_archive_setup.';
  g_debug        := hr_utility.debug_enabled;
end pay_sg_iras_archive_setup;
