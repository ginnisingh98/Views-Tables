--------------------------------------------------------
--  DDL for Package Body GR_TOXIC_SPECIES_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_TOXIC_SPECIES_B_PKG" AS
/*$Header: GRHITSB.pls 115.9 2002/10/28 21:41:44 gkelly ship $*/
PROCEDURE Insert_Row
	   			 (p_commit IN VARCHAR2,
				  p_called_by_form IN VARCHAR2,
				  p_toxic_species_code IN VARCHAR2,
/* Bug 1323951 M. Thomas 28-Jan-2002 Added the following for Toxicity Calculation */
				  p_hierarchy_seq IN NUMBER,
/* Bug 1323951 M. Thomas 28-Jan-2002 End of the changes  for Toxicity Calculation */
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_toxic_species_b
  	     (toxic_species_code,
/* Bug 1323951 M. Thomas 28-Jan-2002 Added the following for Toxicity Calculation */
  	      hierarchy_seq,
/* Bug 1323951 M. Thomas 28-Jan-2002 End of the changes  for Toxicity Calculation */
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE	gr_toxic_species_b
	  SET		toxic_species_code	= p_toxic_species_code,
/* Bug 1323951 M. Thomas 28-Jan-2002 Added the following for Toxicity Calculation */
	                hierarchy_seq		= p_hierarchy_seq,
/* Bug 1323951 M. Thomas 28-Jan-2002 End of the changes  for Toxicity Calculation */
...
CURSOR c_lock_toxic_species
 IS
   SELECT	last_update_date
   FROM		gr_toxic_species_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockSpeciesRcd	  c_lock_toxic_species%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_toxic_species;
   FETCH c_lock_toxic_species INTO LockSpeciesRcd;
   IF c_lock_toxic_species%NOTFOUND THEN
	  CLOSE c_lock_toxic_species;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_toxic_species_b
   WHERE  	   rowid = p_rowid;
...
CURSOR c_get_toxic_species_rowid
 IS
   SELECT ts.rowid
   FROM	  gr_toxic_species_b ts
   WHERE  ts.toxic_species_code = p_toxic_species_code;
ToxicSpeciesRecord			   c_get_toxic_species_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_toxic_species_rowid;
   FETCH c_get_toxic_species_rowid INTO ToxicSpeciesRecord;
   IF c_get_toxic_species_rowid%FOUND THEN
      x_key_exists := 'T';
      x_rowid := ToxicSpeciesRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_toxic_species_rowid;
