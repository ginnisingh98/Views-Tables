--------------------------------------------------------
--  DDL for Package Body FND_REQUEST
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FND_REQUEST" as
/* $Header: AFCPREQB.pls 120.15.12010000.31 2019/05/07 12:33:16 ckclark ship $ */
...
--
  ...
  --
        ...
        P_RECALC_PARAMETERS     varchar2(1)     := null;
        P_NLS_SORT              varchar2(30)    := 'BINARY';
        ...
        TYPE terr_tab_type is table of varchar2(2) index by varchar2(64);
        P_TERR_TAB terr_tab_type;
...
-- bug 1518406 define a local name
  ...
  begin
      ...
      if (is_trace_enabled(SQL_WAITS)) then
        ...
        -- Default NLS numeric characters
...
-- if number of copies is zero, no need to perform print action so don't insert
...
if curr_copies > 0 then
        ...
        Insert
          Into Fnd_Concurrent_Requests (
                ...
                Node_Name1, Connstr1, Recalc_Parameters,
        ...
        Select cur_request_id, z_security_group_id,
               ...
               P_NODE_NAME1, P_CONNSTR1, P_RECALC_PARAMETERS,
               argtxt2, nargs,
        ...
        if (subloginid is not null) then
          ...
          -- 10118578 - Do not insert if auditing is not turned on
          l_audit_level:= fnd_profile.value('SIGNONAUDIT:LEVEL');
          if (l_audit_level in ('B', 'C', 'D')) then
  ...
  function submit_mls_request (
     ...
     begin
         ...
         -- Return if deferred request was submitted this time
         if P_DEF_REQUEST_ID <> 0 then
            return reqid;
         end if;
         ...
         insert into fnd_run_requests
          ...
          RECALC_PARAMETERS, nls_sort)
        values
         (conc_app_id, conc_prog_id, reqid,
          0, 0, 0,
          null, P_PRINT_STYLE, P_SAVE_OUTPUT, null,
          P_LANGUAGE, P_TERRITORY, P_NUMERIC_CHARACTERS, -1,
          submit_mls_request.description,
          ...
          P_ORG_ID, P_RECALC_PARAMETERS, P_NLS_SORT);
  ...
  begin
        ...
        set_increment_dates_option (increment_dates);

        set_recalc_parameters_option (recalc_parameters);
  ...
  begin
        ...
        P_RECALC_PARAMETERS      := recalc_parameters;
  ...
  -- Added an overload for only 2 parameter processing for function
  function add_printer (printer in varchar2 default null,
                        copies  in number default null
                        ) return boolean is
  begin

     return(add_printer(printer, copies, null));

  end;

  function add_printer (printer in varchar2 default null,
                        copies  in number default null,
                        lang    in varchar2) return boolean is
        printer_typ             varchar2 (30) := null;
        dummy_fld               varchar (2);
        print_together_error    exception;
        printer_error           exception;
  ...
  -- Added an overload for only 1 parameter processing for function
  function add_notification (user          in varchar2
                     ) return boolean is
  begin

     return(add_notification(user,
                             null));
  ...
  -- Added an overload for only 5 paramter processing for function
  ...
  begin
    ...
    return(add_layout(template_appl_name,
                      template_code,
                      template_language,
                      template_territory,
                      output_format,
  ...
  begin
       ...
       if P_DEF_REQUEST_ID <> 0 then
        ...
        -- Also submit normally if this is deferred mode and not more than
        -- 1 language has been previously requested.
        if (P_REQUEST_TYPE = 'C' or
            (P_DEF_REQUEST_ID <> 0 and langcnt <= 1) or
            (P_DEF_REQUEST_ID = 0 and
...
begin
  ...
  begin
     ...
     -- 16279648 - check the array for a value without having to change the current session
     if P_TERR_TAB.exists(UPPER(check_terr)) then
         return P_TERR_TAB(UPPER(check_terr));
     end if;
...
when OTHERS then
  ...
  function add_language (
               ...
               num_char IN VARCHAR2,
               nls_sort IN VARCHAR2 default 'BINARY') return boolean is
  ...
  P_NLS_PARAMETERS(P_NLS_PARAMETERS_COUNT).nls_sort := nls_sort;
...
begin
     ...
     P_TERR_TAB('ALGERIA') := '.,';
     P_TERR_TAB('AMERICA') := '.,';
     P_TERR_TAB('ARGENTINA') := ',.';
     P_TERR_TAB('AUSTRALIA') := '.,';
     P_TERR_TAB('AUSTRIA') := ',.';
     P_TERR_TAB('AZERBAIJAN') := ', ';
     P_TERR_TAB('BAHRAIN') := '.,';
     P_TERR_TAB('BANGLADESH') := '.,';
     P_TERR_TAB('BELGIUM') := ',.';
     P_TERR_TAB('BRAZIL') := ',.';
     P_TERR_TAB('BULGARIA') := '. ';
     P_TERR_TAB('CANADA') := ', ';
     ...
     P_TERR_TAB('CHINA') := '.,';
     P_TERR_TAB('COLOMBIA') := '.,';
     P_TERR_TAB('COSTA RICA') := '.,';
     P_TERR_TAB('CROATIA') := ',.';
     P_TERR_TAB('CYPRUS') := ',.';
     P_TERR_TAB('CZECH REPUBLIC') := ',.';
     P_TERR_TAB('DENMARK') := ',.';
     P_TERR_TAB('DJIBOUTI') := '.,';
     P_TERR_TAB('ECUADOR') := ',.';
     P_TERR_TAB('EGYPT') := '.,';
     P_TERR_TAB('EL SALVADOR') := '.,';
     P_TERR_TAB('ESTONIA') := ', ';
     P_TERR_TAB('FINLAND') := ',.';
     P_TERR_TAB('FRANCE') := ',.';
     P_TERR_TAB('FYR MACEDONIA') := ',.';
     P_TERR_TAB('GERMANY') := ',.';
     P_TERR_TAB('GREECE') := ',.';
     P_TERR_TAB('GUATEMALA') := '.,';
     P_TERR_TAB('HONG KONG') := '.,';
     P_TERR_TAB('HUNGARY') := '. ';
     P_TERR_TAB('ICELAND') := ',.';
     P_TERR_TAB('INDIA') := '.,';
     P_TERR_TAB('INDONESIA') := ',.';
     P_TERR_TAB('IRAQ') := '.,';
     P_TERR_TAB('IRELAND') := '.,';
     P_TERR_TAB('ISRAEL') := '.,';
     P_TERR_TAB('ITALY') := ',.';
     P_TERR_TAB('JAPAN') := '.,';
     P_TERR_TAB('JORDAN') := '.,';
     P_TERR_TAB('KAZAKHSTAN') := ', ';
     P_TERR_TAB('KOREA') := '.,';
     P_TERR_TAB('KUWAIT') := '.,';
     P_TERR_TAB('LATVIA') := ', ';
     P_TERR_TAB('LEBANON') := '.,';
     P_TERR_TAB('LIBYA') := '.,';
     P_TERR_TAB('LITHUANIA') := ',.';
     P_TERR_TAB('LITHUANIA') := ',.';
     P_TERR_TAB('LUXEMBOURG') := ',.';
     P_TERR_TAB('MALAYSIA') := '.,';
     P_TERR_TAB('MAURITANIA') := '.,';
     P_TERR_TAB('MEXICO') := ',.';
     P_TERR_TAB('MOROCCO') := '.,';
     P_TERR_TAB('NEW ZEALAND') := '.,';
     P_TERR_TAB('NICARAGUA') := '.,';
     P_TERR_TAB('NORWAY') := ',.';
     P_TERR_TAB('OMAN') := '.,';
     P_TERR_TAB('PANAMA') := '.,';
     P_TERR_TAB('PERU') := ',.';
     P_TERR_TAB('PHILIPPINES') := '.,';
     P_TERR_TAB('POLAND') := ', ';
     P_TERR_TAB('PORTUGAL') := ',.';
     P_TERR_TAB('PUERTO RICO') := '.,';
     P_TERR_TAB('QATAR') := '.,';
     P_TERR_TAB('ROMANIA') := ',.';
     P_TERR_TAB('RUSSIA') := ', ';
     P_TERR_TAB('SAUDI ARABIA') := '.,';
     P_TERR_TAB('SERBIA AND MONTENEGRO') := ',.';
     P_TERR_TAB('SINGAPORE') := '.,';
     P_TERR_TAB('SLOVAKIA') := ',.';
     P_TERR_TAB('SLOVENIA') := ',.';
     P_TERR_TAB('SOMALIA') := '.,';
     P_TERR_TAB('SOUTH AFRICA') := '.,';
     P_TERR_TAB('SPAIN') := ',.';
     P_TERR_TAB('SUDAN') := '.,';
     P_TERR_TAB('SWEDEN') := ',.';
     P_TERR_TAB('SWITZERLAND') := ',.';
     P_TERR_TAB('SYRIA') := '.,';
     P_TERR_TAB('TAIWAN') := '.,';
     P_TERR_TAB('THAILAND') := '.,';
     P_TERR_TAB('THE NETHERLANDS') := ',.';
     P_TERR_TAB('TUNISIA') := '.,';
     P_TERR_TAB('TURKEY') := ',.';
     P_TERR_TAB('UKRAINE') := ', ';
     P_TERR_TAB('UNITED ARAB EMIRATES') := '.,';
     P_TERR_TAB('UNITED KINGDOM') := '.,';
     P_TERR_TAB('UZBEKISTAN') := ',.';
     P_TERR_TAB('VENEZUELA') := ',.';
     P_TERR_TAB('VIETNAM') := ',.';
     P_TERR_TAB('YEMEN') := '.,';
