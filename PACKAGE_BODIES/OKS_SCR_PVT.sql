--------------------------------------------------------
--  DDL for Package Body OKS_SCR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKS_SCR_PVT" AS
/* $Header: OKSRSCRB.pls 120.0 2005/05/25 18:12:50 appldev noship $ */
  ---------------------------------------------------------------------------
  -- FUNCTION get_seq_id
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- FUNCTION get_rec for: OKS_K_SALES_CREDITS
  ---------------------------------------------------------------------------
  FUNCTION get_rec (
    p_scr_rec                      IN scr_rec_type,
    x_no_data_found                OUT NOCOPY BOOLEAN
  ) RETURN scr_rec_type IS
    CURSOR oks_k_saels_credits_pk_csr (p_id                 IN NUMBER) IS
    SELECT
            ID,
            PERCENT,
            CHR_ID,
     ...
     WHERE oks_k_sales_credits.id = p_id;
    l_oks_k_saels_credits_pk       oks_k_saels_credits_pk_csr%ROWTYPE;
    l_scr_rec                      scr_rec_type;
  BEGIN
    x_no_data_found := TRUE;
    -- Get current database values
    OPEN oks_k_saels_credits_pk_csr (p_scr_rec.id);
    FETCH oks_k_saels_credits_pk_csr INTO
              l_scr_rec.ID,
              l_scr_rec.PERCENT,
              l_scr_rec.CHR_ID,
              l_scr_rec.CLE_ID,
    ...
    x_no_data_found := oks_k_saels_credits_pk_csr%NOTFOUND;
    CLOSE oks_k_saels_credits_pk_csr;
    RETURN(l_scr_rec);
  END get_rec;
  ...
  END get_rec;
  ---------------------------------------------------------------------------
  -- FUNCTION get_rec for: OKS_K_SALES_CREDITS_V
  ---------------------------------------------------------------------------
   FUNCTION get_rec (
    p_scrv_rec    IN scrv_rec_type,
    x_no_data_found                OUT NOCOPY BOOLEAN
  ) RETURN scrv_rec_type IS
  CURSOR oks_scrv_csr (p_id                 IN NUMBER) IS
    SELECT
            ID,
            PERCENT,
            CHR_ID,
      ...
      FROM Oks_K_Sales_Credits
     WHERE oks_k_sales_credits.id = p_id;
    l_scrv_pk       oks_scrv_csr%ROWTYPE;
    l_scrv_rec                      scrv_rec_type;
  BEGIN
    x_no_data_found := TRUE;
    -- Get current database values

    OPEN oks_scrv_csr (p_scrv_rec.id);
    FETCH oks_scrv_csr INTO
              l_scrv_rec.ID,
              l_scrv_rec.PERCENT,
              l_scrv_rec.CHR_ID,
              l_scrv_rec.CLE_ID,
    ...
    x_no_data_found := oks_scrv_csr%NOTFOUND;
  ...
  -----------------------------------------------------------
  -- FUNCTION null_out_defaults for: OKS_K_SALES_CREDITS_V --
  -----------------------------------------------------------
  ...
  BEGIN
    ...
    END IF;
    IF (l_scrv_rec.sales_group_id = OKC_API.G_MISS_NUM) THEN
      l_scrv_rec.sales_group_id := NULL;
    END IF;
  ...
  ---------------------------------------------------------------------------
  ---------------------------------------------------
  -- Validate_Attributes for:OKS_K_SALES_CREDITS_V --
  ---------------------------------------------------
  -----------------------------------------------------
  -- Validate ID--
  -----------------------------------------------------
...
---------------------------------------------------
 ...
 FUNCTION Validate_Attributes (
  ...
  -- call OKC_UTIL.ADD_VIEW to prepare the PL/SQL table to hold columns of view
    ...
    OKC_UTIL.ADD_VIEW('OKS_K_SALES_CREDITS_V',x_return_status);
...
*/
  ...
  -----------------------------------------------
  -- Validate_Record for:OKS_K_SALES_CREDITS_V --
  -----------------------------------------------
  ...
  ---------------------------------------------------------------------------
  --------------------------------------------
  -- validate_row for:OKS_K_SALES_CREDITS_V --
  --------------------------------------------
  PROCEDURE validate_row(
  ...
  END validate_row;
  -----------------------------------------------------------
  -- PL/SQL TBL validate_row for:OKS_K_SALES_CREDITS_V_TBL --
  -----------------------------------------------------------
  PROCEDURE validate_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
  ...
  ---------------------------------------------------------------------------
  ----------------------------------------
  -- insert_row for:OKS_K_SALES_CREDITS --
  ----------------------------------------
  PROCEDURE insert_row(
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
    x_return_status                OUT NOCOPY VARCHAR2,
    ...
    --------------------------------------------
    -- Set_Attributes for:OKS_K_SALES_CREDITS --
    --------------------------------------------
...
--    l_scr_rec.id := get_seq_id;
  ...
  END insert_row;
  ------------------------------------------
  -- insert_row for:OKS_K_SALES_CREDITS_V --
  ------------------------------------------
  PROCEDURE insert_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
    ...
    l_return_status                VARCHAR2(1) := OKC_API.G_RET_STS_SUCCESS;
    l_scrv_rec    scrv_rec_type;
    ldefoksksalescreditsvrec       scrv_rec_type;
    l_scr_rec                      scr_rec_type;
    ...
    ----------------------------------------------
    -- Set_Attributes for:OKS_K_SALES_CREDITS_V --
    ----------------------------------------------
    FUNCTION Set_Attributes (
      p_scrv_rec IN  scrv_rec_type,
  ...
  BEGIN
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    ldefoksksalescreditsvrec     := fill_who_columns(ldefoksksalescreditsvrec);
    ldefoksksalescreditsvrec.id  := get_seq_id ;


    --- Validate all non-missing attributes (Item Level Validation)
    l_return_status := Validate_Attributes(ldefoksksalescreditsvrec);
    --- If any errors happen abort API
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := Validate_Record(ldefoksksalescreditsvrec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    ...
    --------------------------------------
    migrate(ldefoksksalescreditsvrec, l_scr_rec);
    --------------------------------------------
    ...
    ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    migrate(lx_scr_rec, ldefoksksalescreditsvrec);
    -- Set OUT values
    x_scrv_rec := ldefoksksalescreditsvrec;
    OKC_API.END_ACTIVITY(x_msg_count, x_msg_data);
  EXCEPTION
    WHEN OKC_API.G_EXCEPTION_ERROR THEN
      x_return_status := OKC_API.HANDLE_EXCEPTIONS
  ...
  END insert_row;
  ---------------------------------------------------------
  -- PL/SQL TBL insert_row for:OKS_K_SALES_CREDITS_V_TBL --
  ---------------------------------------------------------
  PROCEDURE insert_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
  ...
  ---------------------------------------------------------------------------
  --------------------------------------
  -- lock_row for:OKS_K_SALES_CREDITS --
  --------------------------------------
  PROCEDURE lock_row(
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
    x_return_status                OUT NOCOPY VARCHAR2,
    ...
    l_return_status                VARCHAR2(1) := OKC_API.G_RET_STS_SUCCESS;
    l_object_version_number       OKS_K_SALES_CREDITS.OBJECT_VERSION_NUMBER%TYPE;
    lc_object_version_number      OKS_K_SALES_CREDITS.OBJECT_VERSION_NUMBER%TYPE;
  ...
  END lock_row;
  ----------------------------------------
  -- lock_row for:OKS_K_SALES_CREDITS_V --
  ----------------------------------------
  PROCEDURE lock_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
  ...
  END lock_row;
  -------------------------------------------------------
  -- PL/SQL TBL lock_row for:OKS_K_SALES_CREDITS_V_TBL --
  -------------------------------------------------------
  PROCEDURE lock_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
  ...
  ---------------------------------------------------------------------------
  ----------------------------------------
  -- update_row for:OKS_K_SALES_CREDITS --
  ----------------------------------------
  PROCEDURE update_row(
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
    x_return_status                OUT NOCOPY VARCHAR2,
    ...
    BEGIN
      ...
      END IF;
      IF (x_scr_rec.sales_group_id = OKC_API.G_MISS_NUM)
      THEN
        x_scr_rec.sales_group_id := l_scr_rec.sales_group_id;
      END IF;
      ...
      END IF;
/*
      IF (x_scr_rec.security_group_id = OKC_API.G_MISS_NUM)
      THEN
        x_scr_rec.security_group_id := l_scr_rec.security_group_id;
      END IF;
*/
  ...
  BEGIN
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    UPDATE  OKS_K_SALES_CREDITS
    SET PERCENT = l_def_scr_rec.percent,
        SALES_GROUP_ID = l_def_scr_rec.sales_group_id,
...
--        ,SECURITY_GROUP_ID = l_def_scr_rec.security_group_id
  ...
  END update_row;
  ------------------------------------------
  -- update_row for:OKS_K_SALES_CREDITS_V --
  ------------------------------------------
  PROCEDURE update_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
    ...
    BEGIN
      ...
      END IF;
      IF (x_scrv_rec.sales_group_id = OKC_API.G_MISS_NUM)
      THEN
        x_scrv_rec.sales_group_id := l_scrv_rec.sales_group_id;
      END IF;
  ...
  BEGIN
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := populate_new_record(l_scrv_rec, ldefoksksalescreditsvrec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
  ...
  END update_row;
  ---------------------------------------------------------
  -- PL/SQL TBL update_row for:OKS_K_SALES_CREDITS_V_TBL --
  ---------------------------------------------------------
  PROCEDURE update_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
  ...
  ---------------------------------------------------------------------------
  ----------------------------------------
  -- delete_row for:OKS_K_SALES_CREDITS --
  ----------------------------------------
  PROCEDURE delete_row(
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
    x_return_status                OUT NOCOPY VARCHAR2,
  ...
  END delete_row;
  ------------------------------------------
  -- delete_row for:OKS_K_SALES_CREDITS_V --
  ------------------------------------------
  PROCEDURE delete_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
  ...
  END delete_row;
  ---------------------------------------------------------
  -- PL/SQL TBL delete_row for:OKS_K_SALES_CREDITS_V_TBL --
  ---------------------------------------------------------
  PROCEDURE delete_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2 DEFAULT OKC_API.G_FALSE,
