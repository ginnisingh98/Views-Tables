--------------------------------------------------------
--  DDL for Package Body GMDOPLAP_WF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMDOPLAP_WF_PKG" AS
/* $Header: GMDOPLAB.pls 120.0 2005/05/30 04:13:56 appldev noship $ */
   PROCEDURE wf_init (
   ...
   IS
      ...
      l_itemtype                WF_ITEMS.ITEM_TYPE%TYPE :=  'GMDOPLAP';
      ...
      l_WorkflowProcess   VARCHAR2(30) := 'GMDOPLAP_PROCESS';
      l_count             NUMBER;
      BEGIN
      ...
      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDOPLAP_OPRN_DESC',
         					  avalue => l_oprn_desc);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDOPLAP_OPRN_CLASS',
         					  avalue => l_oprn_class);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDOPLAP_OPRN_UOM',
         					  avalue => l_item_um);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDOPLAP_EFF_START_DATE',
         					  avalue => l_effective_start_date);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDOPLAP_EFF_END_DATE',
  ...
  EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDOPLAP_wf_pkg','wf_init',l_itemtype,l_itemkey,'Initial' );
      raise;
...
/* ######################################################################## */
   ...
   IS
      ...
      l_operation_id      GMD_OPERATIONS_B.oprn_id%TYPE:=to_number(wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_OPERATION_ID'));
      ...
      P_lab_activity_name VARCHAR2(80) := 'GMDOPLAP_NOTI_REQUEST';
     ...
     IF (p_funcmode = 'RUN') THEN
         ...
         THEN
	      ...
	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDOPLAP_ADDL_TEXT',
                                      avalue => l_data_string);

	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDOPLAP_APPROVER',
                                      avalue => p_role);

            p_resultout:='COMPLETE:Y';
        ELSE
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_TARGET_STATUS');
...
EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDOPLAP_wf_pkg','is_approval_req',p_itemtype,p_itemkey,p_role);
      raise;
END is_approval_req;
...
PROCEDURE REMINDAR_CHECK (
      ...
      l_mesg_cnt      number:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDOPLAP_MESG_CNT');
      l_approver      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_APPROVER');
BEGIN
...
BEGIN
     IF (p_funcmode = 'RUN') THEN
          l_start_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_START_STATUS');
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_TARGET_STATUS');
...
PROCEDURE MOREINFO_RESPONSE  (
      ...
      p_funcmode      IN VARCHAR2,
      p_resultout     OUT NOCOPY VARCHAR2) IS
      l_requester     VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_REQUSTER');
BEGIN
...
PROCEDURE APPEND_COMMENTS (
      ...
      l_comment       VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_COMMENT');
      l_mesg_comment  VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_DISP_COMMENT');
      l_performer      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDOPLAP_CURR_PERFORMER');
BEGIN
...
--           l_html_mesg := replace(l_mesg_comment,wf_core.newline,'<BR>'||wf_core.newline);
	   ...
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDOPLAP_DISP_COMMENT',
                                   avalue => l_mesg_comment);
--	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
--                                   itemkey => p_itemkey,
--         			           aname => 'GMDOPLAP_HTML_DISP_COMMENT',
--                                   avalue => l_html_mesg);
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDOPLAP_COMMENT',
                                   avalue => l_comment);
       END IF;
END;
