--------------------------------------------------------
--  DDL for Package Body AS_GAR_OPPTYS_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AS_GAR_OPPTYS_PUB" AS
/* $Header: asxgropb.pls 120.8 2006/02/02 21:29 amagupta noship $ */

---------------------------------------------------------------------------
--    Start of Comments
---------------------------------------------------------------------------
--    PACKAGE NAME:   AS_GAR_OPPTYS_PUB
--    ---------------------------------------------------------------------
--    PURPOSE
--    --------
...
/*-------------------------------------------------------------------------+
 ...
 +-------------------------------------------------------------------------*/
  ...
  G_ENTITY CONSTANT VARCHAR2(13) := 'GAR::OPPTYS::';
/*-------------------------------------------------------------------------*
 |                             PRIVATE VARIABLES
 *-------------------------------------------------------------------------*/
/*-------------------------------------------------------------------------*
...
BEGIN
   ...
   If l_return_status <> FND_API.G_RET_STS_SUCCESS Then
    ...
    AS_GAR_OPPTYS_PUB.EXPLODE_GROUPS_OPPTYS(
          x_errbuf        => l_errbuf,
          x_retcode       => l_retcode,
    ...
    AS_GAR_OPPTYS_PUB.EXPLODE_TEAMS_OPPTYS(
          x_errbuf        => l_errbuf,
          x_retcode       => l_retcode,
          p_terr_globals  => l_terr_globals,
          x_return_status => l_return_status);
    ...
    AS_GAR_OPPTYS_PUB.SET_TEAM_LEAD_OPPTYS(
        x_errbuf        => l_errbuf,
        x_retcode       => l_retcode,
        p_terr_globals  => l_terr_globals,
        x_return_status => l_return_status);
    ...
    AS_GAR_OPPTYS_PUB.INSERT_ACCESSES_OPPTYS(
        x_errbuf        => l_errbuf,
        x_retcode       => l_retcode,
        p_terr_globals  => l_terr_globals,
        x_return_status => l_return_status);
    ...
    AS_GAR_OPPTYS_PUB.INSERT_TERR_ACCESSES_OPPTYS(
        x_errbuf        => l_errbuf,
        x_retcode       => l_retcode,
        p_terr_globals  => l_terr_globals,
        x_return_status => l_return_status);
    ...
    AS_GAR_OPPTYS_PUB.PERFORM_OPPTY_CLEANUP(
              x_errbuf        => l_errbuf,
              x_retcode       => l_retcode,
              p_terr_globals  => l_terr_globals,
              x_return_status => l_return_status);
    ...
    +------------------------------------------------------------------------*/
    AS_GAR.G_TAP_FLAG := 'Y';
    AS_GAR_OPPTYS_PUB.ASSIGN_OPPTY_OWNER(
              x_errbuf        => l_errbuf,
              x_retcode       => l_retcode,
              p_terr_globals  => l_terr_globals,
              x_return_status => l_return_status);
...
BEGIN
   ...
   OPEN   count_res_oppor;
	FETCH  count_res_oppor INTO   l_res_oppr_count;
   CLOSE  count_res_oppor;

   IF l_res_oppr_count > 0 THEN
	...
	**/
	       INSERT INTO JTF_TAE_1001_OPPOR_WINNERS
            (trans_object_id,
     ...
     WHERE j.team_id = t.resource_id
        ...
        AND NOT EXISTS (SELECT 1 FROM JTF_TAE_1001_OPPOR_WINNERS rt1
...
/************************** End Explode Groups Opptys ******************/

/************************** Start Set Opptys Team Leader *****************/
...
IS
	...
	CURSOR team_leader(c_worker_id number) IS
	    SELECT /*+ LEADING(WIN) USE_NL(A WIN) INDEX(A) */ A.lead_id,
		      A.salesforce_id,
		      A.sales_group_id,
		      NVL(WIN.full_access_flag,'N')
	    FROM  AS_ACCESSES_ALL_ALL A,
		      JTF_TAE_1001_OPPOR_WINNERS WIN
	    ...
	    AND   WIN.trans_object_id = A.lead_id
	...
	LOOP -- For l_limit_flag
		...
		OPEN team_leader(l_worker_id);
		    FETCH team_leader BULK COLLECT INTO
			      l_lead_id, l_salesforce_id, l_sales_group_id, l_faf
		    LIMIT l_max_fetches;
		CLOSE team_leader;
		...
		IF l_lead_id.COUNT < l_max_fetches THEN
		   l_limit_flag := TRUE;
		END IF;
		AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_STLEAD || AS_GAR.G_BULK_UPD || AS_GAR.G_START);

		IF  l_lead_id.COUNT > 0 THEN
			l_flag := TRUE;
			l_first := l_lead_id.FIRST;
			l_last := l_first + l_var;
			WHILE l_flag LOOP
				IF l_last > l_lead_id.LAST THEN
					l_last := l_lead_id.LAST;
				END IF;
							...
							IF l_attempts = 3 THEN
								...
								LOOP
									...
									WHEN OTHERS THEN
										...
										AS_GAR.LOG('LEAD_ID/SALESFORCE_ID/SALESGROUP_ID/ORG_ID - ' || l_lead_id(i) || '/' || l_salesforce_id(i) || '/' || l_sales_group_id(i));
									END;
				...
				l_first := l_last + 1;
				l_last := l_first + l_var;
				IF l_first > l_lead_id.LAST THEN
					l_flag := FALSE;
				END IF;
			END LOOP; -- loop for more records within the bulk_size
		END IF; --l_lead_id.count > 0
		AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_STLEAD || AS_GAR.G_END);
		AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_STLEAD || AS_GAR.G_N_ROWS_PROCESSED || l_lead_id.COUNT);
...
IS
	...
	SELECT W.resource_id,
	       ...
	       W.trans_object_id lead_id,
	       MAX(W.full_access_flag) faf,
	       W.ORG_ID
	FROM  JTF_TAE_1001_OPPOR_WINNERS W
					...
					AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_INSACC || AS_GAR.G_BULK_INS || AS_GAR.G_N_ROWS_PROCESSED ||l_first||' to '||l_last);
						...
						(      access_id
						   ...
						   SELECT as_accesses_s.nextval
						    ...
						    FROM  DUAL, AS_LEADS_ALL L, AS_STATUSES_B STS
						    WHERE L.lead_id = l_lead_id(i)
	   					    AND L.status = STS.status_code
						    AND NOT EXISTS ( SELECT  'X'
								       FROM AS_ACCESSES_ALL_ALL AA
								       WHERE AA.sales_lead_id IS NULL
								       AND AA.lead_id IS NOT NULL
								       AND AA.delete_flag IS NULL
								       AND AA.lead_id = l_lead_id(i)
 ...
 +-------------------------------------------------------------------------*/
	...
	LOOP
		...
		BEGIN
			...
			l_lead_id, l_salesforce_id, l_sales_group_id
			LIMIT l_max_fetches;
			-- Initialize variables
			l_flag := TRUE;
			l_first := 0;
			...
			IF l_lead_id.COUNT < l_max_fetches THEN l_limit_flag := TRUE; END IF;
			IF  l_lead_id.COUNT > 0 THEN
				l_flag := TRUE;
				l_first := l_lead_id.FIRST;
				l_last := l_first + l_var;
...
IS
	...
	CURSOR del_oppty_totalmode(c_worker_id number) IS
		SELECT  distinct trans_object_id
		FROM JTF_TAE_1001_OPPOR_TRANS
		WHERE worker_id=c_worker_id;

	CURSOR del_oppty_newmode(c_worker_id number) IS
		SELECT  distinct trans_object_id
		FROM JTF_TAE_1001_OPPOR_NM_TRANS
		WHERE worker_id=c_worker_id;
...
BEGIN
	...
	IF p_terr_globals.run_mode = AS_GAR.G_TOTAL_MODE THEN
		OPEN del_oppty_totalmode(l_worker_id);
	ELSE
		OPEN del_oppty_newmode(l_worker_id);
	END IF;
	LOOP --{L1
		IF (l_limit_flag) THEN EXIT; END IF;
		...
		BEGIN
			IF p_terr_globals.run_mode = AS_GAR.G_TOTAL_MODE THEN
				EXIT WHEN del_oppty_totalmode%NOTFOUND;
				FETCH del_oppty_totalmode BULK COLLECT INTO l_lead_id
				LIMIT l_max_fetches;
			ELSE
				EXIT WHEN del_oppty_newmode%NOTFOUND;
				FETCH del_oppty_newmode BULK COLLECT INTO l_lead_id
				LIMIT l_max_fetches;
			END IF;
						...
						WHILE l_attempts < 3 LOOP --{L4
							BEGIN
								...
								FORALL i in l_first..l_last
									...
									AND NOT EXISTS (SELECT  'X'
									  FROM JTF_TAE_1001_OPPOR_WINNERS W
									  WHERE  W.trans_object_id = ACC.lead_id
									  AND  W.worker_id = l_worker_id
									  AND  W.resource_id = ACC.salesforce_id
									  AND  W.group_id = ACC.sales_group_id)
									AND NOT EXISTS (SELECT 1 FROM AS_SALES_CREDITS SCR
											WHERE SCR.LEAD_ID =  ACC.LEAD_ID
											AND SCR.SALESFORCE_ID = ACC.SALESFORCE_ID
											AND SCR.SALESGROUP_ID = ACC.SALES_GROUP_ID )
									...
									FOR i IN l_first .. l_last LOOP --{L5
										...
										WHEN DUP_VAL_ON_INDEX THEN
											...
											WHEN OTHERS THEN
												...
												AS_GAR.LOG('OPPTY_ID - ' || l_lead_id(i));
											END;
										END;
									END LOOP; --}L5
									COMMIT;
							...
							WHEN OTHERS THEN
								...
								IF del_oppty_totalmode%ISOPEN THEN CLOSE del_oppty_totalmode; END IF;
								IF del_oppty_newmode%ISOPEN THEN CLOSE del_oppty_newmode; END IF;
								x_errbuf  := SQLERRM;
								x_retcode := SQLCODE;
								x_return_status := FND_API.G_RET_STS_ERROR;
								RAISE;
...
IS
    ...
    CURSOR oppty_owner_totalmode(c_worker_id number) IS
    SELECT /*+ index(aaa as_accesses_n3) */ aaa.lead_id ,
    ...
    WHERE  aaa.lead_id IS NOT NULL
    AND    aaa.sales_lead_id IS NULL
    AND    aaa.delete_flag IS NULL
    AND    aaa.lead_id=w.trans_object_id
    AND    aaa.lead_id+0=w.trans_object_id
    GROUP BY aaa.lead_id
    ...
    FROM   JTF_TAE_1001_OPPOR_TRANS w
    WHERE worker_id = c_worker_id
    AND NOT EXISTS
     (SELECT 'x'
      FROM AS_ACCESSES_ALL aaa
      WHERE aaa.lead_id =w.trans_object_id
      AND (aaa.CREATED_BY_TAP_FLAG = 'Y'
      OR   aaa.owner_flag='Y'));


    CURSOR oppty_owner_newmode(c_worker_id number) IS
    SELECT /*+ index(aaa as_accesses_n3) */ aaa.lead_id ,
           max(decode(aaa.created_by_tap_flag,'Y',aaa.access_id,-999)) access_id
    FROM   AS_ACCESSES_ALL_ALL aaa,
           ( SELECT distinct trans_object_id
    ...
    UNION -- Union added for Bug#4035168
    SELECT trans_object_id ,0
    FROM   JTF_TAE_1001_OPPOR_NM_TRANS w
    WHERE worker_id = c_worker_id
    AND NOT EXISTS
     (SELECT 'x'
      FROM AS_ACCESSES_ALL aaa
...
BEGIN
    ...
    l_var      :=p_terr_globals.bulk_size;
    l_max_fetches := p_terr_globals.cursor_limit;
    l_call_pre_uhk := JTF_USR_HKS.Ok_to_execute('AS_TERR_OPPTY_ASSIGNMENT','Assign_Oppty_Owner','B','C');
    IF l_call_pre_uhk
    THEN
        AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'CUSTOM AS PER PROFILE---AS_LEAD_ASSIGNMENT_UHK:- Y---');
        AS_CUSTOM_HOOKS_UHK.Oppty_TOTTAP_Owner_Assignment(
	...
	LOOP
		...
		IF p_terr_globals.run_mode = AS_GAR.G_TOTAL_MODE THEN
			OPEN oppty_owner_totalmode(l_worker_id);
			FETCH oppty_owner_totalmode BULK COLLECT INTO l_lead_id,l_access_id LIMIT l_max_fetches;
			CLOSE oppty_owner_totalmode;
		ELSE
			OPEN oppty_owner_newmode(l_worker_id);
			FETCH oppty_owner_newmode BULK COLLECT INTO l_lead_id,l_access_id LIMIT l_max_fetches;
			CLOSE oppty_owner_newmode;
		END IF;
		l_flag := TRUE;
		l_first := 0;
		l_last := 0;
		...
		IF  l_lead_id.COUNT > 0 THEN
			 ...
			 WHILE l_flag LOOP
				...
				END IF;
				WHILE l_attempts < 3 LOOP
					AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'UPDATE AS_LEADS_ALL');
					BEGIN
						 FORALL i in l_first .. l_last
							...
							SET	sl.object_version_number =  nvl(sl.object_version_number,0) + 1,
								...
								( sl.owner_salesforce_id,
								  sl.owner_sales_group_id
								) =
								( SELECT salesforce_id,sales_group_id
								  FROM as_accesses_all_all
								  WHERE access_id = l_access_id(i)
								)
								WHERE sl.lead_id = l_lead_id(i) ;
							COMMIT;
						l_attempts := 3;
					...
					BEGIN
						...
						IF l_attempts = 3 THEN
							...
							EXCEPTION
							WHEN OTHERS THEN
								AS_GAR.LOG_EXCEPTION(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'ROW-BY-ROW UPDATE OF OPPTYS', SQLERRM, TO_CHAR(SQLCODE));
							END;
							END LOOP;
							COMMIT;
						END IF;
					END; -- end of deadlock exception
					WHEN OTHERS THEN
						AS_GAR.LOG_EXCEPTION(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'UPDATE AS_LEADS', SQLERRM, TO_CHAR(SQLCODE));
						x_errbuf  := SQLERRM;
						x_retcode := SQLCODE;
						x_return_status := FND_API.G_RET_STS_ERROR;
						RAISE;
					END;
				END LOOP;
				AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'UPDATE AS_SALES_CREDITS_DENORM');
				WHILE l_attempts < 3 LOOP
					BEGIN
						 FORALL i in l_first .. l_last
							UPDATE AS_SALES_CREDITS_DENORM scd
								SET	scd.object_version_number =  nvl(scd.object_version_number,0) + 1,
									...
									scd.last_updated_by =  p_terr_globals.user_id,
									scd.last_update_login = p_terr_globals.last_update_login,
									scd.request_id = p_terr_globals.request_id,
									scd.program_application_id = p_terr_globals.prog_appl_id,
									scd.program_id =  p_terr_globals.prog_id,
									scd.program_update_date = SYSDATE,
					...
					BEGIN
						...
						IF l_attempts = 3 THEN
							...
							EXCEPTION
							WHEN OTHERS THEN
								AS_GAR.LOG_EXCEPTION(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'ROW-BY-ROW UPDATE OF SALES CREDITS DENORM', SQLERRM, TO_CHAR(SQLCODE));
							END;
							END LOOP;
							COMMIT;
						END IF;
					END; -- end of deadlock exception
					WHEN OTHERS THEN
						AS_GAR.LOG_EXCEPTION(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || 'UPDATE AS_SALES_CREDITS_DENORM', SQLERRM, TO_CHAR(SQLCODE));
						x_errbuf  := SQLERRM;
						x_retcode := SQLCODE;
						x_return_status := FND_API.G_RET_STS_ERROR;
						RAISE;
			...
			END LOOP;
		END IF; --l_lead_id.count > 0
		AS_GAR.LOG(G_ENTITY || AS_GAR.G_PROCESS || AS_GAR.G_CO || AS_GAR.G_N_ROWS_PROCESSED || l_lead_id.COUNT);
	END LOOP;
	l_limit_flag    := FALSE;
	l_loop_count    := 0;
	l_access_id.delete;
