--------------------------------------------------------
--  DDL for Package Body B_STD_SUB_OP_RES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."B_STD_SUB_OP_RES_PKG" as
/* $Header: BOMPSSRB.pls 120.0.12010000.2 2008/10/15 11:06:18 tbhande ship $ */
...
PROCEDURE Insert_Row(X_Rowid                        IN OUT NOCOPY VARCHAR2,
   ...
   CURSOR C IS SELECT rowid FROM BOM_STD_SUB_OP_RESOURCES
             WHERE standard_operation_id = X_Standard_Operation_Id
             AND   replacement_group_num = X_Replacement_Group_Num;

BEGIN
  INSERT INTO BOM_STD_SUB_OP_RESOURCES(
	  ...
	  X_Schedule_Flag,
          ...
          X_Attribute14,
          X_Attribute15,
          X_Schedule_Seq_Num              -- Bug 7370692
...
PROCEDURE Lock_Row(X_Rowid                                 VARCHAR2,
  ...
  CURSOR C IS
      ...
      WHERE  rowid = X_Rowid
      FOR UPDATE of Standard_Operation_Id, Replacement_Group_Num NOWAIT;
  Recinfo C%ROWTYPE;
BEGIN
  OPEN C;
  ...
  if (
      ...
      AND (   (Recinfo.replacement_group_num = X_replacement_group_num)
           OR (    (Recinfo.replacement_group_num IS NULL)
               AND (X_replacement_group_num IS NULL)))
...
BEGIN
  UPDATE BOM_STD_SUB_OP_RESOURCES
  SET
    ...
    Schedule_Seq_Num                          =    X_Schedule_Seq_Num   -- Bug 7370692
  WHERE rowid = X_rowid;
...
PROCEDURE Delete_Row_Sub(X_Rowid VARCHAR2) IS
BEGIN
  DELETE FROM BOM_STD_SUB_OP_RESOURCES
  WHERE  rowid = X_Rowid;
