--------------------------------------------------------
--  DDL for Package Body HR_FGT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_FGT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_fgt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_fgt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_form_data_groups_tl Row
  --
  update hr_form_data_groups_tl
    set
     form_data_group_id              = p_rec.form_data_group_id
    ,language                        = p_rec.language
    ,source_lang                     = p_rec.source_lang
    ,user_data_group_name            = p_rec.user_data_group_name
...
Begin
  ...
  begin
    ...
    hr_fgt_rku.after_update
      ...
      ,p_source_lang
      => p_rec.source_lang
      ,p_user_data_group_name
      => p_rec.user_data_group_name
      ,p_description
      => p_rec.description
      ,p_source_lang_o
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    ...
    hr_fgt_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.user_data_group_name = hr_api.g_varchar2) then
    p_rec.user_data_group_name :=
    hr_fgt_shd.g_old_rec.user_data_group_name;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  hr_fgt_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hr_fgt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_fgt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_fgt_upd.post_update
     (p_rec
     );
End upd;
--
