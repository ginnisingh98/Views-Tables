--------------------------------------------------------
--  DDL for Package Body HXC_HRR_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_HRR_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_hrr_ins.';  -- Global package name
...
Begin
  ...
  --
  --
  -- Insert the row into: hxc_resource_rules
  --
  insert into hxc_resource_rules
      (resource_rule_id
      ,name
      ,business_group_id
      ,legislation_code
...
--
  l_proc  varchar2(72);
--
  Cursor C_Sel1 is select hxc_resource_rules_s.nextval from sys.dual;
--
Begin
  if g_debug then
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.resource_rule_id;
  Close C_Sel1;
  --
  if g_debug then
	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_hrr_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_hrr_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_hrr_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure ins
  ...
  ,p_resource_rule_id                  out nocopy number
...
Begin
  ...
  --
  -- Having converted the arguments into the hxc_hrr_rec
  ...
  --
  p_resource_rule_id := l_rec.resource_rule_id;
