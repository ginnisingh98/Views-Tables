--------------------------------------------------------
--  DDL for Package Body BEN_BENDEAUD_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BENDEAUD_XMLP_PKG" AS
/* $Header: BENDEAUDB.pls 120.1 2007/12/10 08:28:59 vjaganat noship $ */
...
function CF_STANDARD_HEADERFormula return Number is
  ...
  CP_STATUS                     := l_status;
    CP_BUSINESS_GROUP		 := l_business_group;
        open c_benefit_action_id;
  fetch c_benefit_action_id into cp_benefit_action_id;
  close c_benefit_action_id;
      CP_MODE                       := l_mode;
  ...
  begin
                select name into CP_BENEFIT_GROUP
...
begin
  ...
  select fnd_profile.value('BEN_DSG_NO_CHG') into cp_profile_value from dual;
  return (TRUE);
end;
...
--Functions to refer Oracle report placeholders--
 ...
 Function CP_STATUS_p return varchar2 is
	Begin
	 return CP_STATUS;
	 END;
 Function CP_PROFILE_VALUE_p return varchar2 is
	Begin
	 return CP_PROFILE_VALUE;
	 END;
 ...
 Function CP_LOCATION_p return varchar2 is
	Begin
	 return CP_LOCATION;
	 END;
 Function CP_BENEFIT_GROUP_p return varchar2 is
	Begin
	 return CP_BENEFIT_GROUP;
	 END;
