--------------------------------------------------------
--  DDL for Package Body IGS_RE_CAND_SEO_CLS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_RE_CAND_SEO_CLS_PKG" as
/* $Header: IGSRI02B.pls 120.1 2005/07/04 00:40:58 appldev ship $ */
  -------------------------------------------------------------------------------------------
  --Change History:
  --Who         When            What
  --smadathi    24-AUG-2001     Bug No. 1956374 .The call to igs_re_val_csc.genp_val_sdtt_sess
  --                            is changed to igs_as_val_suaap.genp_val_sdtt_sess.
  -------------------------------------------------------------------------------------------
  l_rowid VARCHAR2(25);
  old_references IGS_RE_CAND_SEO_CLS%RowType;
  new_references IGS_RE_CAND_SEO_CLS%RowType;
  ...
  BEGIN
	...
	IF igs_as_val_suaap.genp_val_sdtt_sess('ENRP_INS_CA_TRNSFR') THEN
		IF p_inserting  THEN
			-- Validate socio-economic objective class code.
			IF IGS_RE_VAL_CSC.resp_val_csc_seocc (
...
||  Changed the reference of "IGS_RE_VAL_CSC.RESP_VAL_CA_CHILDUPD" to program unit "IGS_RE_VAL_CAH.RESP_VAL_CA_CHILDUPD". -- kdande
*/
  ...
  IF upper(column_name) = 'CA_SEQUENCE_NUMBER' OR COLUMN_NAME IS NULL THEN
    IF new_references.CA_SEQUENCE_NUMBER < 1 OR new_references.CA_SEQUENCE_NUMBER > 9999999 then
	  Fnd_Message.Set_Name('IGS','IGS_GE_INVALID_VALUE');
	  IGS_GE_MSG_STACK.ADD;
	  App_Exception.Raise_Exception ;
	END IF;
 ...
 END Check_Constraints ;
  ...
  BEGIN
    ...
    IF (((old_references.SEO_CLASS_CD = new_references.seo_class_cd)) OR
        ((new_references.seo_class_cd IS NULL))) THEN
      NULL;
    ELSE
      IF NOT IGS_RE_SEO_CLASS_CD_PKG.Get_PK_For_Validation (
        new_references.seo_class_cd
        ) THEN
     	     Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
     	     IGS_GE_MSG_STACK.ADD;
  ...
  FUNCTION Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      ...
      AND      seo_class_cd = x_seo_class_cd
      FOR UPDATE NOWAIT;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_RE_CSC_CA_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_RE_CSC_SCC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  END GET_FK_IGS_RE_SEO_CLASS_CD;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_RE_CAND_SEO_CLS
      where PERSON_ID = X_PERSON_ID
      and CA_SEQUENCE_NUMBER = X_CA_SEQUENCE_NUMBER
      and SEO_CLASS_CD = X_SEO_CLASS_CD;
...
begin
  ...
  END IF;
  insert into IGS_RE_CAND_SEO_CLS (
    PERSON_ID,
    CA_SEQUENCE_NUMBER,
    SEO_CLASS_CD,
    PERCENTAGE,
...
begin
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
  update IGS_RE_CAND_SEO_CLS set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_RE_CAND_SEO_CLS
     where PERSON_ID = X_PERSON_ID
     and CA_SEQUENCE_NUMBER = X_CA_SEQUENCE_NUMBER
     and SEO_CLASS_CD = X_SEO_CLASS_CD
  ;
...
begin
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
  delete from IGS_RE_CAND_SEO_CLS
  where ROWID = X_ROWID;
  if (sql%notfound) then
     fnd_message.set_name ('IGS', 'IGS_SC_POLICY_UPD_DEL_EXCEP');
     igs_ge_msg_stack.add;
