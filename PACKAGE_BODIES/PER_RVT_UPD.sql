--------------------------------------------------------
--  DDL for Package Body PER_RVT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_RVT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_rvt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the per_ri_view_reports_tl Row
  --
  update per_ri_view_reports_tl
    set
     workbench_view_report_code      = p_rec.workbench_view_report_code
    ,language                        = p_rec.language
    ,workbench_view_report_name      = p_rec.workbench_view_report_name
    ,wb_view_report_description      = p_rec.wb_view_report_description
...
Begin
  ...
  begin
    ...
    per_rvt_rku.after_update
      ...
      ,p_source_lang
      => p_rec.source_lang
      ,p_workbench_view_report_name_o
      => per_rvt_shd.g_old_rec.workbench_view_report_name
      ,p_wb_view_report_description_o
      => per_rvt_shd.g_old_rec.wb_view_report_description
      ,p_source_lang_o
      => per_rvt_shd.g_old_rec.source_lang
      );
    --
  exception
    --
...
Begin
  ...
  --
  If (p_rec.workbench_view_report_name = hr_api.g_varchar2) then
    p_rec.workbench_view_report_name :=
    per_rvt_shd.g_old_rec.workbench_view_report_name;
  End If;
  If (p_rec.wb_view_report_description = hr_api.g_varchar2) then
    p_rec.wb_view_report_description :=
    per_rvt_shd.g_old_rec.wb_view_report_description;
  End If;
...
Begin
  ...
  --
  per_rvt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_rvt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_rvt_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
  ...
  cursor csr_upd_langs is
    select rvt.language
     ...
     where rvt.workbench_view_report_code = p_workbench_view_report_code
       and p_language_code in (rvt.language ,rvt.source_lang);
  --
