--------------------------------------------------------
--  DDL for Package Body XLA_AR_ACCT_HOOKS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_AR_ACCT_HOOKS_PKG" AS
-- $Header: xlaaparh.pkb 120.2 2006/06/28 05:50:13 vinaykum noship $
/*===========================================================================+
|  Copyright (c) 2003 Oracle Corporation BelmFont, California, USA           |
|                          ALL rights reserved.                              |
+============================================================================+
| PACKAGE NAME                                                               |
|     xla_ar_acct_hooks_pkg                                                  |
|                                                                            |
| DESCRIPTION                                                                |
|     Call accounting program integration APIs for Account Receivables       |
|                                                                            |
| HISTORY                                                                    |
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.xla_ar_acct_hooks_pkg';
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_ar_acct_hooks_pkg.trace');
END trace;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_ar_acct_hooks_pkg.print_logfile');
...
BEGIN
   ...
   WHEN  p_event_name = 'preaccounting'  THEN
      ...
      arp_xla_extract_main_pkg.preaccounting
   ...
   WHEN p_event_name = 'postaccounting'  THEN
      ...
      arp_xla_extract_main_pkg.postaccounting
            ( p_application_id     => p_application_id
             ,p_ledger_id          => p_ledger_id
             ,p_process_category   => p_process_category
             ,p_end_date           => p_end_date
...
WHEN xla_exceptions_pkg.application_exception THEN
   ...
   xla_exceptions_pkg.raise_message
      ...
      ,p_token_1        => 'LOCATION'
      ,p_value_1        => 'xla_ar_acct_hooks_pkg.main'
      ,p_token_2        => 'ERROR'
