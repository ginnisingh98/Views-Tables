--------------------------------------------------------
--  DDL for Package Body IGS_PS_OFR_PAT_NOTE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_OFR_PAT_NOTE_PKG" AS
/* $Header: IGSPI24B.pls 115.4 2002/11/29 02:12:56 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_OFR_PAT_NOTE%RowType;
  new_references IGS_PS_OFR_PAT_NOTE%RowType;
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_copn_br_iud
  -- BEFORE INSERT OR DELETE OR UPDATE
  -- ON IGS_PS_OFR_PAT_NOTE
  -- FOR EACH ROW
  ...
  PROCEDURE BeforeRowInsertUpdateDelete1(
	...
	v_course_cd		IGS_PS_OFR_PAT_NOTE.course_cd%TYPE;
	v_version_number		IGS_PS_OFR_PAT_NOTE.version_number%TYPE;
	v_message_name		VARCHAR2(30);
  BEGIN

	-- Set IGS_PS_OFR_PAT key.
	IF p_inserting THEN
		IGS_PS_GEN_003.CRSP_GET_COP_KEY (
 ...
 BEGIN
  ...
  BEGIN
    ...
    IF (((old_references.course_cd = new_references.course_cd) AND
      ...
      NULL;
    ELSE
      IF NOT IGS_PS_OFR_PAT_PKG.Get_PK_For_Validation (
        new_references.course_cd,
        new_references.version_number,
        new_references.cal_type,
        new_references.ci_sequence_number,
    ...
    IF (((old_references.cop_id = new_references.cop_id)) OR
        ((new_references.cop_id IS NULL))) THEN
      NULL;
    ELSE
      IF NOT IGS_PS_OFR_PAT_PKG.Get_UK_For_Validation (
        new_references.cop_id
        ) THEN
	        Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
	        IGS_GE_MSG_STACK.ADD;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_COPN_CNT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_COPN_COP_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_COPN_NOTE_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_OFR_PAT_NOTE
      where COURSE_CD = X_COURSE_CD
      and CI_SEQUENCE_NUMBER = X_CI_SEQUENCE_NUMBER
      and CAL_TYPE = X_CAL_TYPE
      and VERSION_NUMBER = X_VERSION_NUMBER
...
Before_DML (
  ...
  insert into IGS_PS_OFR_PAT_NOTE (
    COURSE_CD,
    VERSION_NUMBER,
    CAL_TYPE,
    CI_SEQUENCE_NUMBER,
...
Before_DML (
  ...
  update IGS_PS_OFR_PAT_NOTE set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_OFR_PAT_NOTE
     where COURSE_CD = X_COURSE_CD
     and CI_SEQUENCE_NUMBER = X_CI_SEQUENCE_NUMBER
     and CAL_TYPE = X_CAL_TYPE
     and VERSION_NUMBER = X_VERSION_NUMBER
...
Before_DML (
  ...
  delete from IGS_PS_OFR_PAT_NOTE
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
