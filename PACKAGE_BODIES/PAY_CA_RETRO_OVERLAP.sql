--------------------------------------------------------
--  DDL for Package Body PAY_CA_RETRO_OVERLAP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CA_RETRO_OVERLAP" 
--  /* $Header: pycaenro.pkb 120.0.12010000.1 2009/06/03 14:09:05 sneelapa noship $ */
/*
   ...
   ----------- ---------- ------ ------- -----------------------------------
   06-06-2008 sneelapa    120.0          Intial Version
*/
AS
...
-- Checking whether Enhanced Retropay is enabled or not
   ...
   END LOOP;
    ...
    IF lv_qualified = 'Y'
    THEN
        lv_short_name := 'CA_ENHANCED_RETROPAY';
        ...
        THEN
            ...
            -- Checking the status of Enhanced Retro pay
                IF(c_upgrade_status%FOUND) AND (lv_upgrade_status = ln_exists or ln_exists = 'P' )
                THEN
        ...
        ELSE
            fnd_file.put_line(fnd_file.log,'Upgrade process of Enhanced Retropay has not completed ');
...
BEGIN
 gv_package_name := 'PAY_CA_RETRO_OVERLAP';
