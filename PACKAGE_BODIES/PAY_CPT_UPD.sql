--------------------------------------------------------
--  DDL for Package Body PAY_CPT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CPT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_cpt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the pay_ca_emp_prov_tax_info_f Row
    --
    update  pay_ca_emp_prov_tax_info_f
    set
        emp_province_tax_inf_id         = p_rec.emp_province_tax_inf_id,
    legislation_code                = p_rec.legislation_code,
    assignment_id                   = p_rec.assignment_id,
    business_group_id               = p_rec.business_group_id,
    province_code                   = p_rec.province_code,
    ...
    deduction_code                  = p_rec.deduction_code,
    extra_info_not_provided         = p_rec.extra_info_not_provided,
    marriage_status                 = p_rec.marriage_status,
    no_of_infirm_dependants         = p_rec.no_of_infirm_dependants,
    non_resident_status             = p_rec.non_resident_status,
    disability_status               = p_rec.disability_status,
    no_of_dependants                = p_rec.no_of_dependants,
    ...
    prov_override_rate              = p_rec.prov_override_rate,
    prov_override_amount            = p_rec.prov_override_amount,
    prov_exempt_flag                = p_rec.prov_exempt_flag,
    pmed_exempt_flag                = p_rec.pmed_exempt_flag,
    wc_exempt_flag                  = p_rec.wc_exempt_flag,
    qpp_exempt_flag                 = p_rec.qpp_exempt_flag,
    tax_calc_method                 = p_rec.tax_calc_method,
    other_tax_credit                = p_rec.other_tax_credit,
    ...
    prov_lsp_amount                 = p_rec.prov_lsp_amount,
    ppip_exempt_flag                 = p_rec.ppip_exempt_flag
...
--
 ...
 ,p_deduction_code                =>p_rec.deduction_code
 ,p_extra_info_not_provided       =>p_rec.extra_info_not_provided
 ,p_marriage_status               =>p_rec.marriage_status
 ,p_no_of_infirm_dependants       =>p_rec.no_of_infirm_dependants
 ,p_non_resident_status           =>p_rec.non_resident_status
 ,p_disability_status             =>p_rec.disability_status
 ,p_no_of_dependants              =>p_rec.no_of_dependants
 ...
 ,p_prov_override_rate            =>p_rec.prov_override_rate
 ,p_prov_override_amount          =>p_rec.prov_override_amount
 ,p_prov_exempt_flag              =>p_rec.prov_exempt_flag
 ,p_pmed_exempt_flag              =>p_rec.pmed_exempt_flag
 ,p_wc_exempt_flag                =>p_rec.wc_exempt_flag
 ,p_qpp_exempt_flag               =>p_rec.qpp_exempt_flag
 ,p_tax_calc_method               =>p_rec.tax_calc_method
 ,p_other_tax_credit              =>p_rec.other_tax_credit
 ...
 ,p_prov_lsp_amount               =>p_rec.prov_lsp_amount
 ,p_effective_date                =>p_effective_date
 ,p_datetrack_mode                =>p_datetrack_mode
 ,p_validation_start_date         =>p_validation_start_date
 ,p_validation_end_date           =>p_validation_end_date
 ...
 ,p_ppip_exempt_flag               =>p_rec.ppip_exempt_flag
      );
    --
  exception
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy pay_cpt_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    ...
    pay_cpt_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.assignment_id = hr_api.g_number) then
    p_rec.assignment_id :=
    pay_cpt_shd.g_old_rec.assignment_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    pay_cpt_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.province_code = hr_api.g_varchar2) then
    p_rec.province_code :=
    pay_cpt_shd.g_old_rec.province_code;
  End If;
  If (p_rec.jurisdiction_code = hr_api.g_varchar2) then
    p_rec.jurisdiction_code :=
    pay_cpt_shd.g_old_rec.jurisdiction_code;
  End If;
  If (p_rec.tax_credit_amount = hr_api.g_number) then
    p_rec.tax_credit_amount :=
    pay_cpt_shd.g_old_rec.tax_credit_amount;
  End If;
  If (p_rec.basic_exemption_flag = hr_api.g_varchar2) then
    p_rec.basic_exemption_flag :=
    pay_cpt_shd.g_old_rec.basic_exemption_flag;
  End If;
  If (p_rec.deduction_code = hr_api.g_varchar2) then
    p_rec.deduction_code :=
    pay_cpt_shd.g_old_rec.deduction_code;
  End If;
  If (p_rec.extra_info_not_provided = hr_api.g_varchar2) then
    p_rec.extra_info_not_provided :=
    pay_cpt_shd.g_old_rec.extra_info_not_provided;
  End If;
  If (p_rec.marriage_status = hr_api.g_varchar2) then
    p_rec.marriage_status :=
    pay_cpt_shd.g_old_rec.marriage_status;
  End If;
  If (p_rec.no_of_infirm_dependants = hr_api.g_number) then
    p_rec.no_of_infirm_dependants :=
    pay_cpt_shd.g_old_rec.no_of_infirm_dependants;
  End If;
  If (p_rec.non_resident_status = hr_api.g_varchar2) then
    p_rec.non_resident_status :=
    pay_cpt_shd.g_old_rec.non_resident_status;
  End If;
  If (p_rec.disability_status = hr_api.g_varchar2) then
    p_rec.disability_status :=
    pay_cpt_shd.g_old_rec.disability_status;
  End If;
  If (p_rec.no_of_dependants = hr_api.g_number) then
    p_rec.no_of_dependants :=
    pay_cpt_shd.g_old_rec.no_of_dependants;
  End If;
  If (p_rec.annual_dedn = hr_api.g_number) then
    p_rec.annual_dedn :=
    pay_cpt_shd.g_old_rec.annual_dedn;
  End If;
  If (p_rec.total_expense_by_commission = hr_api.g_number) then
    p_rec.total_expense_by_commission :=
    pay_cpt_shd.g_old_rec.total_expense_by_commission;
  End If;
  If (p_rec.total_remnrtn_by_commission = hr_api.g_number) then
    p_rec.total_remnrtn_by_commission :=
    pay_cpt_shd.g_old_rec.total_remnrtn_by_commission;
  End If;
  If (p_rec.prescribed_zone_dedn_amt = hr_api.g_number) then
    p_rec.prescribed_zone_dedn_amt :=
    pay_cpt_shd.g_old_rec.prescribed_zone_dedn_amt;
  End If;
  If (p_rec.additional_tax = hr_api.g_number) then
    p_rec.additional_tax :=
    pay_cpt_shd.g_old_rec.additional_tax;
  End If;
  If (p_rec.prov_override_rate = hr_api.g_number) then
    p_rec.prov_override_rate :=
    pay_cpt_shd.g_old_rec.prov_override_rate;
  End If;
  If (p_rec.prov_override_amount = hr_api.g_number) then
    p_rec.prov_override_amount :=
    pay_cpt_shd.g_old_rec.prov_override_amount;
  End If;
  If (p_rec.prov_exempt_flag = hr_api.g_varchar2) then
    p_rec.prov_exempt_flag :=
    pay_cpt_shd.g_old_rec.prov_exempt_flag;
  End If;
  If (p_rec.pmed_exempt_flag = hr_api.g_varchar2) then
    p_rec.pmed_exempt_flag :=
    pay_cpt_shd.g_old_rec.pmed_exempt_flag;
  End If;
  If (p_rec.wc_exempt_flag = hr_api.g_varchar2) then
    p_rec.wc_exempt_flag :=
    pay_cpt_shd.g_old_rec.wc_exempt_flag;
  End If;
  If (p_rec.qpp_exempt_flag = hr_api.g_varchar2) then
    p_rec.qpp_exempt_flag :=
    pay_cpt_shd.g_old_rec.qpp_exempt_flag;
  End If;
  If (p_rec.tax_calc_method = hr_api.g_varchar2) then
    p_rec.tax_calc_method :=
    pay_cpt_shd.g_old_rec.tax_calc_method;
  End If;
  If (p_rec.other_tax_credit = hr_api.g_number) then
    p_rec.other_tax_credit :=
    pay_cpt_shd.g_old_rec.other_tax_credit;
  End If;
  If (p_rec.ca_tax_information_category = hr_api.g_varchar2) then
    p_rec.ca_tax_information_category :=
    pay_cpt_shd.g_old_rec.ca_tax_information_category;
  End If;
  If (p_rec.ca_tax_information1 = hr_api.g_varchar2) then
    p_rec.ca_tax_information1 :=
    pay_cpt_shd.g_old_rec.ca_tax_information1;
  End If;
  If (p_rec.ca_tax_information2 = hr_api.g_varchar2) then
    p_rec.ca_tax_information2 :=
    pay_cpt_shd.g_old_rec.ca_tax_information2;
  End If;
  If (p_rec.ca_tax_information3 = hr_api.g_varchar2) then
    p_rec.ca_tax_information3 :=
    pay_cpt_shd.g_old_rec.ca_tax_information3;
  End If;
  If (p_rec.ca_tax_information4 = hr_api.g_varchar2) then
    p_rec.ca_tax_information4 :=
    pay_cpt_shd.g_old_rec.ca_tax_information4;
  End If;
  If (p_rec.ca_tax_information5 = hr_api.g_varchar2) then
    p_rec.ca_tax_information5 :=
    pay_cpt_shd.g_old_rec.ca_tax_information5;
  End If;
  If (p_rec.ca_tax_information6 = hr_api.g_varchar2) then
    p_rec.ca_tax_information6 :=
    pay_cpt_shd.g_old_rec.ca_tax_information6;
  End If;
  If (p_rec.ca_tax_information7 = hr_api.g_varchar2) then
    p_rec.ca_tax_information7 :=
    pay_cpt_shd.g_old_rec.ca_tax_information7;
  End If;
  If (p_rec.ca_tax_information8 = hr_api.g_varchar2) then
    p_rec.ca_tax_information8 :=
    pay_cpt_shd.g_old_rec.ca_tax_information8;
  End If;
  If (p_rec.ca_tax_information9 = hr_api.g_varchar2) then
    p_rec.ca_tax_information9 :=
    pay_cpt_shd.g_old_rec.ca_tax_information9;
  End If;
  If (p_rec.ca_tax_information10 = hr_api.g_varchar2) then
    p_rec.ca_tax_information10 :=
    pay_cpt_shd.g_old_rec.ca_tax_information10;
  End If;
  If (p_rec.ca_tax_information11 = hr_api.g_varchar2) then
    p_rec.ca_tax_information11 :=
    pay_cpt_shd.g_old_rec.ca_tax_information11;
  End If;
  If (p_rec.ca_tax_information12 = hr_api.g_varchar2) then
    p_rec.ca_tax_information12 :=
    pay_cpt_shd.g_old_rec.ca_tax_information12;
  End If;
  If (p_rec.ca_tax_information13 = hr_api.g_varchar2) then
    p_rec.ca_tax_information13 :=
    pay_cpt_shd.g_old_rec.ca_tax_information13;
  End If;
  If (p_rec.ca_tax_information14 = hr_api.g_varchar2) then
    p_rec.ca_tax_information14 :=
    pay_cpt_shd.g_old_rec.ca_tax_information14;
  End If;
  If (p_rec.ca_tax_information15 = hr_api.g_varchar2) then
    p_rec.ca_tax_information15 :=
    pay_cpt_shd.g_old_rec.ca_tax_information15;
  End If;
  If (p_rec.ca_tax_information16 = hr_api.g_varchar2) then
    p_rec.ca_tax_information16 :=
    pay_cpt_shd.g_old_rec.ca_tax_information16;
  End If;
  If (p_rec.ca_tax_information17 = hr_api.g_varchar2) then
    p_rec.ca_tax_information17 :=
    pay_cpt_shd.g_old_rec.ca_tax_information17;
  End If;
  If (p_rec.ca_tax_information18 = hr_api.g_varchar2) then
    p_rec.ca_tax_information18 :=
    pay_cpt_shd.g_old_rec.ca_tax_information18;
  End If;
  If (p_rec.ca_tax_information19 = hr_api.g_varchar2) then
    p_rec.ca_tax_information19 :=
    pay_cpt_shd.g_old_rec.ca_tax_information19;
  End If;
  If (p_rec.ca_tax_information20 = hr_api.g_varchar2) then
    p_rec.ca_tax_information20 :=
    pay_cpt_shd.g_old_rec.ca_tax_information20;
  End If;
  If (p_rec.ca_tax_information21 = hr_api.g_varchar2) then
    p_rec.ca_tax_information21 :=
    pay_cpt_shd.g_old_rec.ca_tax_information21;
  End If;
  If (p_rec.ca_tax_information22 = hr_api.g_varchar2) then
    p_rec.ca_tax_information22 :=
    pay_cpt_shd.g_old_rec.ca_tax_information22;
  End If;
  If (p_rec.ca_tax_information23 = hr_api.g_varchar2) then
    p_rec.ca_tax_information23 :=
    pay_cpt_shd.g_old_rec.ca_tax_information23;
  End If;
  If (p_rec.ca_tax_information24 = hr_api.g_varchar2) then
    p_rec.ca_tax_information24 :=
    pay_cpt_shd.g_old_rec.ca_tax_information24;
  End If;
  If (p_rec.ca_tax_information25 = hr_api.g_varchar2) then
    p_rec.ca_tax_information25 :=
    pay_cpt_shd.g_old_rec.ca_tax_information25;
  End If;
  If (p_rec.ca_tax_information26 = hr_api.g_varchar2) then
    p_rec.ca_tax_information26 :=
    pay_cpt_shd.g_old_rec.ca_tax_information26;
  End If;
  If (p_rec.ca_tax_information27 = hr_api.g_varchar2) then
    p_rec.ca_tax_information27 :=
    pay_cpt_shd.g_old_rec.ca_tax_information27;
  End If;
  If (p_rec.ca_tax_information28 = hr_api.g_varchar2) then
    p_rec.ca_tax_information28 :=
    pay_cpt_shd.g_old_rec.ca_tax_information28;
  End If;
  If (p_rec.ca_tax_information29 = hr_api.g_varchar2) then
    p_rec.ca_tax_information29 :=
    pay_cpt_shd.g_old_rec.ca_tax_information29;
  End If;
  If (p_rec.ca_tax_information30 = hr_api.g_varchar2) then
    p_rec.ca_tax_information30 :=
    pay_cpt_shd.g_old_rec.ca_tax_information30;
  End If;
  If (p_rec.prov_lsp_amount = hr_api.g_number) then
    p_rec.prov_lsp_amount :=
    pay_cpt_shd.g_old_rec.prov_lsp_amount;
  End If;
  If (p_rec.ppip_exempt_flag = hr_api.g_varchar2) then
    p_rec.ppip_exempt_flag :=
    pay_cpt_shd.g_old_rec.ppip_exempt_flag;
  End If;
...
Begin
  ...
  --
  l_rec :=
  pay_cpt_shd.convert_args
  (
  p_emp_province_tax_inf_id,
  null,
  null,
  p_legislation_code,
  p_assignment_id,
  ...
  p_additional_tax,
  p_prov_override_rate,
  p_prov_override_amount,
  p_prov_exempt_flag,
  p_pmed_exempt_flag,
  p_wc_exempt_flag,
  p_qpp_exempt_flag,
