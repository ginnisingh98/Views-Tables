--------------------------------------------------------
--  DDL for Package Body IGS_PS_USEC_PRV_GRAD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_USEC_PRV_GRAD_PKG" AS
/* $Header: IGSPI0JB.pls 115.8 2002/11/29 01:57:23 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_ps_usec_prv_grad%RowType;
  new_references igs_ps_usec_prv_grad%RowType;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.unit_section_prev_grade_id := x_unit_section_prev_grade_id;
    new_references.uoo_id := x_uoo_id;
    new_references.grading_schema_code := x_grading_schema_code;
    new_references.grading_schema_version_number := x_grad_schema_version_number;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_usec_prv_grad
      WHERE    unit_section_prev_grade_id = x_unit_section_prev_grade_id
      FOR UPDATE NOWAIT;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_usec_prv_grad
      WHERE    grading_schema_value = x_grading_schema_value
      AND      grading_schema_version_number = x_grad_schema_version_number
      AND      uoo_id = x_uoo_id
      AND      grading_schema_code = x_grading_schema_code 	and      ((l_rowid is null) or (rowid <> l_rowid))
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_USPG_UOO_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_USPG_GSG_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    Set_Column_Values (
      p_action,
      x_rowid,
      x_unit_section_prev_grade_id,
      x_uoo_id,
      x_grading_schema_code,
      x_grad_schema_version_number,
    ...
    IF (p_action = 'INSERT') THEN
      ...
      Null;
	     IF Get_Pk_For_Validation(
    		new_references.unit_section_prev_grade_id)  THEN
	       Fnd_Message.Set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
      IGS_GE_MSG_STACK.ADD;
	       App_Exception.Raise_Exception;
	     END IF;
...
l_rowid:=NULL;
  ...
  ***************************************************************/
    ...
    cursor C is select ROWID from IGS_PS_USEC_PRV_GRAD
             where                 UNIT_SECTION_PREV_GRADE_ID= X_UNIT_SECTION_PREV_GRADE_ID
;
 ...
 begin
   ...
   select igs_ps_usec_prv_grad_s.nextval into x_unit_section_prev_grade_id from dual;
   Before_DML(
 		p_action=>'INSERT',
 		x_rowid=>X_ROWID,
 	       x_unit_section_prev_grade_id=>X_UNIT_SECTION_PREV_GRADE_ID,
 	       x_uoo_id=>X_UOO_ID,
 	       x_grading_schema_code=>X_GRADING_SCHEMA_CODE,
 	       x_grad_schema_version_number=>x_grad_schema_version_number,
     ...
     insert into IGS_PS_USEC_PRV_GRAD (
		UNIT_SECTION_PREV_GRADE_ID
		,UOO_ID
		,GRADING_SCHEMA_CODE
		...
		,LAST_UPDATE_LOGIN
	        ...
	        NEW_REFERENCES.UNIT_SECTION_PREV_GRADE_ID
	        ,NEW_REFERENCES.UOO_ID
	        ,NEW_REFERENCES.GRADING_SCHEMA_CODE
	        ,NEW_REFERENCES.GRADING_SCHEMA_VERSION_NUMBER
	        ,NEW_REFERENCES.GRADING_SCHEMA_VALUE
...
if ( (  tlinfo.UOO_ID = X_UOO_ID)
  ...
  AND (tlinfo.GRADING_SCHEMA_VERSION_NUMBER = x_grad_schema_version_number)
 ...
 begin
   ...
   Before_DML(
 		...
 		x_rowid=>X_ROWID,
	       ...
	       x_last_updated_by=>X_LAST_UPDATED_BY,
	       x_last_update_login=>X_LAST_UPDATE_LOGIN);
   update IGS_PS_USEC_PRV_GRAD set
  ...
  ***************************************************************/
    ...
    cursor c1 is select ROWID from IGS_PS_USEC_PRV_GRAD
             where     UNIT_SECTION_PREV_GRADE_ID= X_UNIT_SECTION_PREV_GRADE_ID
;
begin
	open c1;
...
x_rowid => X_ROWID
 ...
 delete from IGS_PS_USEC_PRV_GRAD
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
