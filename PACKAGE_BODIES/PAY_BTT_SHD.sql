--------------------------------------------------------
--  DDL for Package Body PAY_BTT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_BTT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_btt_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'PAY_BALANCE_TYPES_TL_PK') Then
...
Begin
  --
  If (p_balance_type_id is null or
      p_language is null
     ) Then
    --
    ...
    l_fct_ret := false;
  Else
    If (p_balance_type_id
        = pay_btt_shd.g_old_rec.balance_type_id and
        p_language
        = pay_btt_shd.g_old_rec.language
       ) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into pay_btt_shd.g_old_rec;
...
Exception
  When HR_Api.Object_Locked then
    ...
    fnd_message.set_token('TABLE_NAME', 'pay_balance_types_tl');
    fnd_message.raise_error;
End lck;
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  l_rec.balance_name                     := p_balance_name;
