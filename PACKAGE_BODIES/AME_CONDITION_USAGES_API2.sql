--------------------------------------------------------
--  DDL for Package Body AME_CONDITION_USAGES_API2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_CONDITION_USAGES_API2" AS
/* $Header: amecgapi.pkb 120.1 2005/10/14 04:11:53 ubhat noship $ */
...
procedure KEY_TO_IDS (
  ...
  X_CONDITION_USAGE  out nocopy VARCHAR2,
  ...
  cursor CSR_GET_CONDITION_USAGE
  (
...
begin
    ...
    if (X_CONDITION_ID is not null) and
      ...
      open CSR_GET_CONDITION_USAGE(
        X_CONDITION_ID,
        X_RULE_ID
      );
      fetch CSR_GET_CONDITION_USAGE into X_CONDITION_USAGE;
        if (CSR_GET_CONDITION_USAGE%notfound) then
          X_CONDITION_USAGE := 'NOTFOUND';
        end if;
      close CSR_GET_CONDITION_USAGE;
    end if;
...
-- CONDITION_ID is positive
-- Drive off RULE_KEY and CONDITION_KEY
...
-- the current row was not found insert a new row
   if (X_CONDITION_USAGE = 'NOTFOUND') then
     INSERT_ROW (
       X_RULE_ID_LOAD,
       X_CONDITION_ID_LOAD,
       X_CREATED_BY,
...
-- Drive off CONDITION_ID and RULE_KEY
    X_RULE_ID_LOAD:=null;
    KEY_TO_IDS_3 (
      X_RULE_KEY,
      X_CONDITION_ID,
