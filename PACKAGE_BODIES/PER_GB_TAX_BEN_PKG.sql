--------------------------------------------------------
--  DDL for Package Body PER_GB_TAX_BEN_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_GB_TAX_BEN_PKG" AS
/* $Header: pegbtaxb.pkb 120.2 2006/05/23 13:29:56 kthampan noship $ */

   -- Global package name
   g_package                    CONSTANT VARCHAR2 (33) := '  per_gb_tax_ben_pkg.';
   ...
   --Insert new element entry
-- the constant definition has been moved to the spec so as to set the
-- same default values in header and spec. This constant is now defined in the spec.

--   c_default_action_if_exists   CONSTANT VARCHAR2 (1)  := 'I';

   -- This is the package that is called by the SRS for creating new Batches.
   PROCEDURE upload_tax_ben (
   ...
   IS
      ...
      -- Taxable Benefits variables
      l_benefits_type                VARCHAR2 (256);
      -- Batch Variables
   ...
   BEGIN
      --hr_utility.trace_on(null,'KTBEN');
      hr_utility.set_location (   'Entering:'
                               || l_proc, 10);
      OPEN csr_leg (p_business_group_id);
      FETCH csr_leg INTO p_legislation_code;
      CLOSE csr_leg;
      hr_utility.set_location (   'Legislation = '
                               || p_legislation_code, 20);
      l_filename := p_file_name;
      fnd_profile.get (c_data_exchange_dir, l_location);
      hr_utility.set_location (   'directory = '
                               || l_location, 30);
      ...
      LOOP
         ...
         EXCEPTION
            ...
            THEN
               ...
               hr_utility.set_message (800, 'PER_GB_P11D_78058_ASG_ERR_MSG');
               hr_utility.set_message_token (800, 'TEXT', 'Error at line' || l_batch_seq || ' : Invalid date');
            ...
            THEN
               ...
               -- Set the application error
               hr_utility.set_message (800, 'PER_GB_P11D_78058_ASG_ERR_MSG');
               hr_utility.set_message_token (800, 'TEXT', 'Error at line' || l_batch_seq || ' : Non matching element');
            ...
            THEN
               ...
               -- Set the application error
               hr_utility.set_message (800, 'PER_GB_P11D_78058_ASG_ERR_MSG');
               hr_utility.set_message_token (800, 'TEXT', 'Error at line' || l_batch_seq || ' : Non matching ID');
            ...
            THEN
               ...
               -- Set the application error
               hr_utility.set_message (800, 'PER_GB_P11D_78058_ASG_ERR_MSG');
               hr_utility.set_message_token (800, 'TEXT', 'Error at line' || l_batch_seq || ' : Invalid ID type');
            ...
            THEN
               ...
               -- Set the application error
               hr_utility.set_message (800, 'PER_GB_P11D_78058_ASG_ERR_MSG');
               hr_utility.set_message_token (800, 'TEXT', 'Error at line' || l_batch_seq || ' : Wrong date format');
            ...
            THEN
               ...
               -- Set the application error
               hr_utility.set_message (800, 'PER_GB_P11D_78058_ASG_ERR_MSG');
               hr_utility.set_message_token (800, 'TEXT', 'Error at line' || l_batch_seq || ' : Entry line too long');
   ...
   EXCEPTION
      ...
      THEN
         ...
         retcode := c_error;
         errbuf :=    'Reading Taxable Benefits File ('
                   || l_location
                   || ' -> '
                   || l_filename
                   || ') - Invalid Operation.';
   ...
   BEGIN
      hr_utility.set_location (   'Entering:'
                               || l_proc, 10);
/* MS excel does not let the first two columns to be blank, this meant the
input file created for P11D had to be edited edited manually to enter ',,' in
the first two columns of each line . This was not acceptable
so we are now moving these two column to positioin 3 and 4
and so current 3 and 4 become 1 and 2
*/
/*
...
*/
      -- First field in file is the id type
      l_id_type := get_field (p_line => l_line, p_delimiter => p_delimiter);

      IF NOT correct_type_id (l_id_type)
      THEN
         RAISE e_wrong_id_type;
      END IF;

      -- Second field in file is the id
      ...
      -- Third field in file is the session_date, this should be NULL
      BEGIN
         p_batch_line.session_date :=
               NVL (
                  fnd_date.chardate_to_date (
      ...
      -- Fourth field in file is the batch_id, this should be NULL
      p_batch_line.batch_id := NVL (
                                  get_field (
                                     p_line=> l_line,
                                     p_delimiter=> p_delimiter
      ...
      ELSE
         BEGIN
            -- Field 30 contains the effective_start_date, if empty default to start of UK Tax Year
            ...
            p_batch_line.effective_start_date :=
                  NVL (
                     ...
                     GREATEST (
                        hr_gbnicar.uk_tax_yr_start (p_session_date),
                        l_start_date
                     )
                  );
            -- Field 31 contains the effective_end_date, if empty default to end of UK Tax Year
            -- or the end date of the assignment
            -- If a person has not been terminated, the final_process_date will be null
            -- In that case we should use the date provided (or end of tax year).
            ...
            p_batch_line.effective_end_date :=
                  NVL (
                     ...
                     LEAST (
                        hr_gbnicar.uk_tax_yr_end (p_session_date),
                        NVL (
            ...
            -- if it wasn't provided by the user.  This will solve bug 2058082.
   ...
   -- This procedure will create a batch header for the Taxable Benefits.
   -- The IN Parameters are
   ...
   PROCEDURE create_tax_ben_batch_header (
   ...
   IS
      ...
      c_batch_reference                CONSTANT VARCHAR2 (30)   := 'Batch to upload Taxable Ben';
      c_batch_source                   CONSTANT VARCHAR2 (30)   := 'Created by TaxBen Upload';
      c_comments                       CONSTANT VARCHAR2 (4000)
               := 'Use this batch to upload the Taxable Benefits into the Taxable Benefits Element Entries.';
   BEGIN
      hr_utility.set_location (   'Entering:'
                               || l_proc, 10);
      ...
      ******************************************************************/
      ...
      hr_utility.set_location (   'Leaving:'
                               || l_proc, 20);
   END create_tax_ben_batch_header;

   PROCEDURE create_tax_ben_batch_line (
      p_session_date                 IN       DATE,
   ...
   -- Not used at the moment but might be an extra check we want to do later
   FUNCTION validate_tax_ben_element (
      p_element_name   IN   VARCHAR2,
      p_leg_cd         IN   VARCHAR2
   )
      RETURN BOOLEAN
   IS
      CURSOR csr_p11d_elements (v_element_name VARCHAR2, v_leg_cd VARCHAR2)
      IS
         SELECT 1
      ...
      l_dummy           csr_p11d_elements%ROWTYPE;
      l_tax_ben_el      BOOLEAN;
      l_proc   CONSTANT VARCHAR2 (72)
               :=    g_package
                  || 'validate_tax_ben_element';
   BEGIN
      hr_utility.set_location (   'Entering:'
                               || l_proc, 10);
      OPEN csr_p11d_elements (p_element_name, p_leg_cd);
      FETCH csr_p11d_elements INTO l_dummy;

      IF csr_p11d_elements%FOUND
      THEN
         l_tax_ben_el := TRUE;
      ELSE
         l_tax_ben_el := FALSE;
      ...
      hr_utility.set_location (   'Leaving:'
                               || l_proc, 20);
      RETURN l_tax_ben_el;
   END validate_tax_ben_element;

   PROCEDURE get_assignment_info (
      p_bus_group_id         IN       per_all_assignments_f.business_group_id%TYPE
            DEFAULT NULL,
      p_id_type              IN       VARCHAR2,
      p_id                   IN       VARCHAR2,
      p_effective_date       IN       DATE,
      p_assg_id              OUT NOCOPY      per_all_assignments_f.assignment_id%TYPE,
      p_assg_nr              OUT NOCOPY      per_all_assignments_f.assignment_number%TYPE,
   ...
   IS
      ...
      CURSOR csr_assg_id (
         v_assg_id          per_all_assignments_f.assignment_id%TYPE,
...
/*      CURSOR csr_assg_nr (
         v_assg_nr   per_all_assignments_f.assignment_number%TYPE,
         v_bg_id     per_all_assignments_f.business_group_id%TYPE,
         v_effective_date   DATE
      )
      IS
...
*/
      ...
      CURSOR csr_empl_nr (
         v_empl_nr          per_all_people_f.employee_number%TYPE,
         v_bg_id            per_all_assignments_f.business_group_id%TYPE,
         v_effective_date   DATE
      )
      IS
      ...
      CURSOR csr_ni_nr (
         v_ni_nr            per_all_people_f.national_identifier%TYPE,
         v_bg_id            per_all_assignments_f.business_group_id%TYPE,
         v_effective_date   DATE
      )
      IS
      ...
      l_assg_nr         per_all_assignments_f.assignment_number%TYPE     := NULL;
      l_assg_id         per_all_assignments_f.assignment_id%TYPE         := NULL;
