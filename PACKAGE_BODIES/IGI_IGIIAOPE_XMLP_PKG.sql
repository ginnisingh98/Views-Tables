--------------------------------------------------------
--  DDL for Package Body IGI_IGIIAOPE_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_IGIIAOPE_XMLP_PKG" AS
/* $Header: IGIIAOPEB.pls 120.0.12010000.1 2008/07/29 08:58:36 appldev ship $ */
...
procedure FDRCSID(idstring in varchar2) is
  BEGIN
    IF (idstring is NULL) THEN
      NULL;
    END IF;
        --        FDRCSID('$Header: IGIIAOPEB.pls 120.0.12010000.1 2008/07/29 08:58:36 appldev ship $');
  END;
...
PROCEDURE Insert_info (
  ...
  FUNCTION COST_MOVEMENTFORMULA(BALANCE_TYPE IN VARCHAR2
                               ...
                               ,CC_COST_TRANSFER IN NUMBER) RETURN NUMBER IS
    L_NUM NUMBER := 0;
  BEGIN
    IF (BALANCE_TYPE = 'Cost') THEN
      L_NUM := NVL(ADDITION
