--------------------------------------------------------
--  DDL for Package Body HR_ASSIGNMENT_BE8
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_ASSIGNMENT_BE8" as 
...
p_supervisor_assignment_id     number) is
  ...
  --
  l_proc varchar2(72):='  hr_assignment_be8.create_secondary_apl_asg_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.assignment.create_secondary_apl_asg';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<person_referred_by_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_person_referred_by_id);
    l_text:=l_text||'</person_referred_by_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<source_organization_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_source_organization_id);
    l_text:=l_text||'</source_organization_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<vacancy_id>';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<applicant_rank>';
    l_text:=l_text||fnd_number.number_to_canonical(p_applicant_rank);
    l_text:=l_text||'</applicant_rank>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
...
end create_secondary_apl_asg_a;
