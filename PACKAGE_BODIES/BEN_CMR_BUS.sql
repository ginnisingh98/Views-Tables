--------------------------------------------------------
--  DDL for Package Body BEN_CMR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CMR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_cmr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_cmbn_age_los_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   cmbn_age_los_rt_id PK of record being inserted or updated.
...
--
Procedure chk_cmbn_age_los_rt_id(p_cmbn_age_los_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_cmbn_age_los_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_cmr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_cmbn_age_los_rt_id                => p_cmbn_age_los_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_cmbn_age_los_rt_id,hr_api.g_number)
     <>  ben_cmr_shd.g_old_rec.cmbn_age_los_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_cmr_shd.constraint_error('BEN_CMBN_AGE_LOS_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_cmbn_age_los_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_cmbn_age_los_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_cmr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--   BEN_CMBN_AGE_LGTH_OF_SVC_FCTR table.
--
-- Pre Conditions
--   None.
--
-- In Parameters
--   cmbn_age_los_fctr_id FK of pointing to BEN_CMBN_AGE_LGTH_OF_SVC_FCTR table.
...
Begin
  ...
  if p_cmbn_age_los_fctr_id is not null then
     ...
     if c1%notfound then
        hr_utility.set_message(801,'Cmbn_age_los_fctr');
        hr_utility.raise_error;
     end if;
     close c1;
  end if;
...
--
-- In Parameters
--   p_cmbn_age_los_rt_id    PK of record being inserted or updated.
--   p_cmbn_age_los_fctr_id  Value of FK
...
and cmbn_age_los_rt_id <> nvl(p_cmbn_age_los_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_cmbn_age_los_fctr_id <> nvl(ben_cmr_shd.g_old_rec.cmbn_age_los_fctr_id,hr_api.g_number)
      or not l_api_updating) then
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Combined Age Length of Service criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Combined Age Length of Service criteria',TRUE);
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and cmbn_age_los_rt_id   <> nvl(p_cmbn_age_los_rt_id ,-1)
		   --and cmbn_age_los_fctr_id   <> nvl(p_cmbn_age_los_fctr_id ,-1)
...
Begin
  ...
  (p_cmbn_age_los_rt_id          => p_rec.cmbn_age_los_rt_id,
   ...
   p_object_version_number => p_rec.object_version_number);
  --
  chk_cmbn_age_los_fctr_id(p_cmbn_age_los_fctr_id => p_rec.cmbn_age_los_fctr_id,
...
function return_legislation_code
  (p_cmbn_age_los_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_cmbn_age_los_rt_f b
    where b.cmbn_age_los_rt_id      = p_cmbn_age_los_rt_id
