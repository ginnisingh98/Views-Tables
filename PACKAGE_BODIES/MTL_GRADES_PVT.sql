--------------------------------------------------------
--  DDL for Package Body MTL_GRADES_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MTL_GRADES_PVT" as
/* $Header: INVVGRDB.pls 120.0 2005/05/25 06:14:08 appldev noship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from MTL_GRADES_B
    where GRADE_CODE = X_GRADE_CODE
    ;
begin
  insert into MTL_GRADES_B (
    GRADE_CODE,
    DISABLE_FLAG,
    ATTRIBUTE1,
    ATTRIBUTE2,
  ...
  and not exists
    (select NULL
    from MTL_GRADES_TL T
    where T.GRADE_CODE = X_GRADE_CODE
...
procedure LOCK_ROW (
  ...
  cursor c is select
    ...
    from MTL_GRADES_B
    where GRADE_CODE = X_GRADE_CODE
    for update of GRADE_CODE nowait;
  recinfo c%rowtype;
...
begin
  ...
  update MTL_GRADES_TL T set (
      DESCRIPTION
    ) = (select
      B.DESCRIPTION
    from MTL_GRADES_TL B
    where B.GRADE_CODE = T.GRADE_CODE
  ...
  where (
      ...
      SUBT.GRADE_CODE,
      SUBT.LANGUAGE
    from MTL_GRADES_TL SUBB, MTL_GRADES_TL SUBT
    where SUBB.GRADE_CODE = SUBT.GRADE_CODE
  ...
  insert into MTL_GRADES_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from MTL_GRADES_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from MTL_GRADES_TL T
    where T.GRADE_CODE = B.GRADE_CODE
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;
