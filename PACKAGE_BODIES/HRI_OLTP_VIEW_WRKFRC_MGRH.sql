--------------------------------------------------------
--  DDL for Package Body HRI_OLTP_VIEW_WRKFRC_MGRH
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HRI_OLTP_VIEW_WRKFRC_MGRH" AS
/* $Header: hriovwrkmgr.pkb 120.0.12000000.2 2007/04/12 13:20:29 smohapat noship $ */
...
FUNCTION get_hdc(p_sup_person_id    IN NUMBER,
  ...
  FROM hri_mds_wrkfc_mgrh_c01_ct   wrkfc
  WHERE wrkfc.sup_person_fk = p_sup_person_id
...
FUNCTION get_transfer_info(p_sup_person_id    IN NUMBER,
                           p_from_date        IN DATE,
                           p_to_date          IN DATE,
                           p_worker_type      IN VARCHAR2,
                           p_directs_only     IN VARCHAR2,
  ...
  CURSOR trn_csr(v_direct_record_ind   IN NUMBER) IS
  SELECT
  ...
  FROM
   hri_mdp_mgrh_transfers_ct trn
  ,hri_mb_wrkfc_evt_ct       wevt
  WHERE trn.mgr_sup_person_fk = p_sup_person_id
  ...
  AND trn.direct_record_ind = v_direct_record_ind
...
BEGIN
  ...
  IF (p_directs_only = 'Y') THEN
    l_direct_record_ind := 1;
  ELSE
    l_direct_record_ind := 0;
  END IF;

  OPEN trn_csr(l_direct_record_ind);
  FETCH trn_csr INTO
    l_hdc_trn_in,
    l_hdc_trn_out,
    l_fte_trn_in,
...
BEGIN
  ...
  RETURN get_transfer_info
          (p_sup_person_id => p_sup_person_id,
