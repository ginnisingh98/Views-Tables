--------------------------------------------------------
--  DDL for Package Body AP_UTILITIES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_UTILITIES_PKG" AS
/* $Header: aputilsb.pls 120.30.12010000.13 2009/12/09 08:56:57 asansari ship $ */
     ...
     If (p_invoice_type_lookup_code = 'EXPENSE REPORT') then
      ...
      /*debug_info :=
        '(PA Flexbuild 1 ) pa_acc_gen_wf_pkg.ap_er_generate_account Failed ';
          ...
          --start bug 8980626
          /*p_debug_info := l_errmsg;*/			--bug 8320268
 ...
 IS
   ...
   SELECT sum(nvl(ael.accounted_cr,0) - nvl(ael.accounted_dr,0) )
    ...
    WHERE aid.po_distribution_id = P_po_distribution_id
      ...
      AND ael.encumbrance_line_type not in ('IPV', 'ERV', 'QV','AV')
