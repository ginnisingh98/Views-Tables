--------------------------------------------------------
--  DDL for Package Body OE_UPGRADE_WF
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_UPGRADE_WF" as
/* $Header: OEXIUWFB.pls 115.35 2003/10/20 06:58:42 appldev ship $ */
...
IS
    ...
    SELECT
         ...
         nvl(oemap.activity_name,
              'UPG_ILN_'||to_char(soac.action_id))        instance_label,
...
IS
	...
	AND   spr.action_id IN
		 (SELECT action_id FROM so_cycle_actions
		  WHERE cycle_id = p_cycle_id)
     AND   spr.cycle_action_id IN
          (SELECT cycle_action_id FROM so_Cycle_actions
           WHERE cycle_id = p_cycle_id
           AND   (cycle_id, p_item_type) in
                 (SELECT cycle_id, item_type FROM oe_upgrade_wf_vld_cyc ) )
	...
	/*  Transition from Pick Release and Backorder Release obsoleted */
     ...
     /* Inventory Interface and Comfirm Line should not exist in the same flow */
/*
     and  (sa.action_id <> 11 or
           (sa.action_id = 11 and sa2.action_id <> 8) or
           (not exists (select  'x' from so_action_pre_reqs x, so_cycle_actions y
              where x.cycle_action_id = y.cycle_action_id
              and   y.cycle_id = p_cycle_id
              and   x.action_id <> 11
              and   y.action_id = 8)
           )
          )
...
IS
     ...
     AND    a.cycle_action_id IN
                  (SELECT cycle_action_id
                   FROM   so_Cycle_actions
                   WHERE  (cycle_id, 'OEOL') IN
                        (SELECT cycle_id,item_type
                            FROM oe_upgrade_wf_vld_cyc ) );
   ...
   FOR c2 in c1 LOOP
           ...
           DECLARE
              ...
              SELECT
                     ...
                     nvl(m.activity_name,
                     'UPG_ILN_'||to_char(a.action_id))        instance_label,
         ...
         BEGIN
		   ...
		   -- dbms_output.put_line('======================='||to_char(v_error_level));
                   /* Insert INTO table wf_activity_transitions */
                   oe_upgrade_wf.insert_into_wf_table
                      ( v_from_instance_id,
                        v_to_instance_id,
                        v_result_code,
                        v_level_error
		   ...
		   -- dbms_output.put_line('#######################'||c4.act_name_t);
                         v_to_instance_id := oe_upgrade_wf.get_instance_id
		   ...
		   -- dbms_output.put_line('#######################'||c4.act_seq);
                            oe_upgrade_wf.get_pre_activity(c4.action_id_c3,c4.act_seq,
                                                           v_pre_activity,v_pre_result);

                            v_error_level := 2096;
		   -- dbms_output.put_line('======================='||to_char(v_error_level));
		   -- dbms_output.put_line('#######################'||v_pre_activity);
                            v_from_instance_id := oe_upgrade_wf.get_instance_id
		   ...
		   -- dbms_output.put_line('======================='||to_char(v_error_level));
		     ...
		     --dbms_output.put_line('======================='||to_char(v_error_level));
                     ...
                     -- dbms_output.put_line('*************** = '||c6.act_seq);
						v_post_activity := oe_upgrade_wf.get_post_activity
							  (
							   c6.action_id_c5,
							   c6.act_seq
							   );
               ...
               FOR c4 IN c3 LOOP
                    ...
                    -- dbms_output.put_line('************* ????? **************'||c4.act_seq);
                     ...
                     ELSE
			              ...
			              oe_upgrade_wf.get_pre_activity
                                            (
                                               c4.action_id_c3,
                                               c4.act_seq,
                                               v_pre_activity,
...
BEGIN
	 ...
	 -- dbms_output.put_line('Entered program');
      ...
      v_instance_id := oe_upgrade_wf.get_instance_id
     ...
     BEGIN
         ...
         WHILE  v_flag = 'Y' LOOP
             ...
             FOR c2 IN c1 LOOP
                 v_error_level := 2152;
                 v_flag := 'Y';
                 oe_upgrade_wf.get_icon_x_value
                                 (
                                   c2.icon,
                                   v_icon_row
                                 );
...
8. Filter out nocopy cycles that have no action results in so_action_results, but

        is used as a pre-req in so_action_pre_reqs
9. Filter out nocopy cycles that has the following attribute:

        i   More than one group in its pre-requisites (OR condition)
            AND
        ii  One of the groups has more than one pre-reqs (AND condition)
            AND
        iii Such a group has a header action as one or more of its pre-reqs
            (involves co-ordination activities)
*/
    ...
    WHERE    cycle_id NOT IN
            (select a.cycle_id from so_cycle_actions a
             where exists (Select action_id
                           from so_cycle_actions
    ...
    AND    cycle_id NOT IN
            (select a.cycle_id from so_cycle_actions a
             where not exists (Select sca.action_id
                               from so_cycle_actions sca, so_actions sa
                               where sca.cycle_id = a.cycle_id
                               and sa.action_id = sca.action_id
                               and sa.result_table = 'SO_LINES'))
    AND    cycle_id NOT IN
            (select a.cycle_id from so_cycle_actions a
             where not exists (Select sca.action_id
                               from so_cycle_actions sca, so_actions sa
                               where sca.cycle_id = a.cycle_id
                               and sa.action_id = sca.action_id
                               and sa.result_table = 'SO_HEADERS'))
    AND    cycle_id not in (select cycle_id
                            from so_cycle_actions
                            where action_id in (5,6) )
    AND    cycle_id not in (select distinct sca.cycle_id
                            from so_cycle_actions sca
                            where sca.cycle_action_id in (select sapr.cycle_action_id
                                                          from so_action_pre_reqs sapr
                                                          where (sapr.action_id, sapr.result_id) not in
                                                                     (select action_id, result_id
                                                                      from so_action_results)))
    AND    cycle_id not in (select distinct sa.cycle_id
                            from so_cycle_actions sa, so_action_pre_reqs p, so_actions a
                            where a.result_table = 'SO_LINES'
                            and sa.action_id = a.action_id
                            and p.cycle_action_id = sa.cycle_action_id
                            and exists (select 'more than 1 group'
                                        from so_action_pre_reqs p1
                                        where p1.cycle_action_id = sa.cycle_action_id
                                        having count(distinct group_number) > 1)
                            and exists (select 'Group has more than 1 row'
                                        from so_action_pre_reqs p2
                                        where p2.cycle_action_id = sa.cycle_action_id
                                        group by p2.cycle_action_id, p2.group_number
                                        having count(*) > 1
                                        and exists (select  'Group has header action'
                                                    from so_action_pre_reqs p3, so_actions a1
                                                    where p3.cycle_action_id = p2.cycle_action_id
                                                    and p3.group_number = p2.group_number
                                                    and p3.action_id = a1.action_id
                                                    and a1.result_table = 'SO_HEADERS')))
    AND    cycle_id IN (select cycle_id from oe_upgrade_wf_vld_cyc);

    cursor c3 is
    select cycle_id from so_cycles;

    v_vld_hdr_flag VARCHAR2(1) := 'N';
    v_vld_lin_flag VARCHAR2(1) := 'N';
    v_error_code   NUMBER;
...
BEGIN
	...
	IF V_ERROR_FLAG = 'N' THEN
        OE_UPGRADE_WF.Create_Lookup_Type('OEOH');
        COMMIT;
     END IF;

     -- dbms_output.put_line('************* FLAG2 = '||v_error_flag);
     IF V_ERROR_FLAG = 'N' THEN
        OE_UPGRADE_WF.Create_Lookup_Type('OEOL');
        COMMIT;
     END IF;

     -- dbms_output.put_line('************* FLAG3 = '||v_error_flag);
     IF V_ERROR_FLAG = 'N' THEN
        OE_UPGRADE_WF.Create_Lookup_Code('OEOH');
        COMMIT;
     END IF;

     -- dbms_output.put_line('************* FLAG4 = '||v_error_flag);
     IF V_ERROR_FLAG = 'N' THEN
        OE_UPGRADE_WF.Create_Lookup_Code('OEOL');
        COMMIT;
     END IF;

     -- dbms_output.put_line('************* FLAG5 = '||v_error_flag);
     IF V_ERROR_FLAG = 'N' THEN
         OE_UPGRADE_WF.Create_activity_name('OEOH');
         COMMIT;
     END IF;

     -- dbms_output.put_line('************* FLAG6 = '||v_error_flag);
     IF V_ERROR_FLAG = 'N' THEN
         OE_UPGRADE_WF.Create_activity_name('OEOL');
         COMMIT;
     END IF;
     ...
     for c4 in c3 loop
        ...
        IF V_ERROR_FLAG = 'N' THEN
             OE_UPGRADE_WF.Create_process_name ('OEOH','REG',c4.cycle_id);
        END IF;

        --dbms_output.put_line('************* FLAG11 = '||v_error_flag);
        IF V_ERROR_FLAG = 'N' THEN
             OE_UPGRADE_WF.Create_process_name ('OEOL','REG',c4.cycle_id);
        END IF;
        ...
        IF v_cfg_item = 'Y' THEN
             OE_UPGRADE_WF.Create_process_name ('OEOL','CFG',c4.cycle_id);
        END IF;
        v_cfg_item := 'N';
     ...
     FOR c2 IN c1 LOOP
          ...
          BEGIN
              SELECT 'Y' INTO v_vld_hdr_flag
              FROM   oe_Upgrade_wf_vld_cyc
              WHERE  cycle_id = c2.cycle_id
              AND    item_type = 'OEOH';
          EXCEPTION
              WHEN no_data_found THEN
                   v_vld_hdr_flag := 'N';
          ...
          EXCEPTION
              ...
              WHEN others THEN
                   v_vld_hdr_flag := 'N';
          END;
          IF v_vld_hdr_flag = 'Y' THEN
                -- dbms_output.put_line('************* FLAG11 = '||v_error_flag);
               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_process_activity ('OEOH',c2.cycle_id,'REG');
               END IF;

               -- dbms_output.put_line('************* FLAG12 = '||v_error_flag);
               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_activity_and ('OEOH','REG',c2.cycle_id);
               END IF;
          END IF;

          IF v_vld_lin_flag = 'Y' THEN
                  BEGIN
                    SELECT 'Y'
                    INTO v_cfg_item
                    FROM so_cycle_actions
	          ...
	          IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_process_activity ('OEOL',c2.cycle_id,'REG');
                   IF v_cfg_item = 'Y' THEN
                       -- dbms_output.put_line('create process activity for cfg');
                       OE_UPGRADE_WF.Create_process_activity ('OEOL',c2.cycle_id,'CFG');
                   END IF;
	          ...
	          IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_activity_and ('OEOL','REG',c2.cycle_id);
                   IF v_cfg_item = 'Y' THEN
                       -- dbms_output.put_line('create activity and for cfg ' || to_char(c2.cycle_id));
                       OE_UPGRADE_WF.Create_activity_and ('OEOL','CFG',c2.cycle_id);
                   END IF;
	          END IF;
		 ...
		 and   result_id in (5, 15);
          ...
          IF V_ERROR_FLAG = 'N' THEN
               ...
               OE_UPGRADE_WF.Create_header_line_dependency (c2.cycle_id,'REG');
               IF v_cfg_item = 'Y' THEN
                    -- dbms_output.put_line('create dependency for cfg ' || to_char(c2.cycle_id));
                    OE_UPGRADE_WF.Create_header_line_dependency (c2.cycle_id,'CFG');
               END IF;
          END IF;
          ...
          IF v_vld_hdr_flag = 'Y' THEN
               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_activity_or ('OEOH','REG',c2.cycle_id);
               END IF;
               ...
               IF V_ERROR_FLAG = 'N' THEN
                   ...
                   OE_UPGRADE_WF.Create_activity_transition ('OEOH',c2.cycle_id,'REG');
               END IF;

               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_notification (c2.cycle_id,'REG','OEOH');
               END IF;

               -- dbms_output.put_line('************* FLAG19 = '||v_error_flag);
               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Create_default_transition ('OEOH','REG',c2.cycle_id);
               END IF;

               -- dbms_output.put_line('************* FLAG20 = '||v_error_flag);
               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Close_Open_End (c2.cycle_id,'REG','OEOH');
               END IF;

               -- dbms_output.put_line('************* FLAG21 = '||v_error_flag);
               IF V_ERROR_FLAG = 'N' THEN
                   OE_UPGRADE_WF.Adjust_arrow_geometry ('OEOH','REG',c2.cycle_id);
               END IF;
       ...
       IF v_vld_lin_flag = 'Y' THEN
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Create_activity_or ('OEOL','REG',c2.cycle_id);
                 IF v_cfg_item = 'Y' THEN
                    --dbms_output.put_line('create activity or  for cfg');
                    OE_UPGRADE_WF.Create_activity_or ('OEOL','CFG',c2.cycle_id);
                  END IF;
            END IF;
            ...
            IF V_ERROR_FLAG = 'N' THEN
                 ...
                 OE_UPGRADE_WF.Create_activity_transition ('OEOL',c2.cycle_id,'REG');
                 IF v_cfg_item = 'Y' THEN
                    -- dbms_output.put_line('create activity transition  for cfg');
                    OE_UPGRADE_WF.Create_activity_transition ('OEOL',c2.cycle_id,'CFG');
                  END IF;
            END IF;
            ...
            --dbms_output.put_line('************* FLAG23+ = '||v_error_flag);
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Create_notification (c2.cycle_id,'REG','OEOL');
                 IF v_cfg_item = 'Y' THEN
                     --dbms_output.put_line('create notification  for cfg');
                     OE_UPGRADE_WF.Create_notification (c2.cycle_id,'CFG','OEOL');
                   END IF;
            END IF;

            -- dbms_output.put_line('************* FLAG24 = '||v_error_flag|| ' cycle_id:' || to_char(c2.cycle_id));
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Create_default_transition ('OEOL','REG',c2.cycle_id);
                 IF v_cfg_item = 'Y' THEN
                     -- dbms_output.put_line('create default end for cfg');
                     OE_UPGRADE_WF.Create_default_transition ('OEOL','CFG',c2.cycle_id);
                   END IF;
            END IF;

            --dbms_output.put_line('************* FLAG25 = '||v_error_flag);
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Create_line_start (c2.cycle_id,'REG');
                 IF v_cfg_item = 'Y' THEN
                     OE_UPGRADE_WF.Create_line_start (c2.cycle_id,'CFG');
                 END IF;
            END IF;

            -- dbms_output.put_line('************* FLAG26 = '||v_error_flag);
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Close_Open_End (c2.cycle_id,'REG','OEOL');
                 IF v_cfg_item = 'Y' THEN
                     OE_UPGRADE_WF.Close_Open_End (c2.cycle_id,'CFG','OEOL');
                 END IF;
            END IF;

            -- dbms_output.put_line('************* FLAG26+ = '||v_error_flag);
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Ship_Confirm_Adjusting (c2.cycle_id,'REG');
                 IF v_cfg_item = 'Y' THEN
                     -- dbms_output.put_line('adjust ship confirm for cfg');
                     OE_UPGRADE_WF.Ship_Confirm_Adjusting (c2.cycle_id,'CFG');
                   END IF;
            END IF;
            ...
            IF V_ERROR_FLAG = 'N' THEN
               IF v_cfg_item = 'Y' THEN
                 ...
                 OE_UPGRADE_WF.ATO_Adjusting (c2.cycle_id);
               END IF;
            END IF;

            -- dbms_output.put_line('************* FLAG27 = '||v_error_flag);
            IF V_ERROR_FLAG = 'N' THEN
               OE_UPGRADE_WF.Generic_Flow_Adjusting ('OEOL', c2.cycle_id,'REG');
               IF v_cfg_item = 'Y' THEN
                 -- dbms_output.put_line('adjust ato for cfg');
                 OE_UPGRADE_WF.Generic_Flow_Adjusting ('OEOL', c2.cycle_id,'CFG');
               END IF;
            END IF;
            ...
            IF V_ERROR_FLAG = 'N' THEN
               OE_UPGRADE_WF.Wait_Flow_Adjusting ('OEOL', c2.cycle_id,'REG');
               IF v_cfg_item = 'Y' THEN
                 --dbms_output.put_line('adjust ato for cfg');
                 OE_UPGRADE_WF.Wait_Flow_Adjusting ('OEOL', c2.cycle_id,'CFG');
               END IF;
            END IF;

            -- dbms_output.put_line('************* FLAG29 = '||v_error_flag);
            IF V_ERROR_FLAG = 'N' THEN
                 OE_UPGRADE_WF.Adjust_arrow_geometry ('OEOL','REG',c2.cycle_id);
                 IF v_cfg_item = 'Y' THEN
                     OE_UPGRADE_WF.Adjust_arrow_geometry ('OEOL','CFG',c2.cycle_id);
                 END IF;
            END IF;
