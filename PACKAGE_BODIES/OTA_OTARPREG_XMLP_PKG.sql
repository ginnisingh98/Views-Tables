--------------------------------------------------------
--  DDL for Package Body OTA_OTARPREG_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_OTARPREG_XMLP_PKG" AS
/* $Header: OTARPREGB.pls 120.1 2007/12/07 05:59:53 amakrish noship $ */
...
IF p_event_id IS NULL and p_training_center_id IS NULL and p_course_start_date IS NULL
   and p_course_end_date IS NULL and p_event_type IS NULL THEN
...
IF (p_event_type IS NOT NULL) THEN
    p_and := p_and || ' AND ((UPPER(evt.event_type) LIKE UPPER(:p_event_type||''%''))
                                    OR (UPPER(evt.event_type) LIKE '||'''%'''||
                                        ' AND UPPER(:p_event_type) = '||'''ALL'''||'))';
END IF;
...
function CF_session_dateFormula return Char is
begin
    select fnd_date.date_to_displaydate(p_session_date)    into cp_session_date
  from dual;
...
--Functions to refer Oracle report placeholders--
 ...
 Function C_EVENT_TITLE_p return varchar2 is
	Begin
	 ...
	 END;
 Function CP_current_date_p return varchar2 is
	Begin
	 return CP_current_date;
	 END;
 Function CP_session_date_p return varchar2 is
	Begin
	 return CP_session_date;
	 END;
