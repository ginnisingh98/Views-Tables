--------------------------------------------------------
--  DDL for Package Body XLA_00240_AAD_S_000004_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_00240_AAD_S_000004_PKG" AS
--
/*======================================================================+
...
+=======================================================================+
| Package Name                                                          |
|     XLA_00240_AAD_S_000004_PKG                                        |
|                                                                       |
| DESCRIPTION                                                           |
|     Package generated From Product Accounting Definition              |
|      Name    : Property Manager Normalized Accounting                 |
|      Code    : PN_LEASE_ACCOUNTING                                    |
|      Owner   : PRODUCT                                                |
|      Version :                                                        |
|      AMB Context Code: DEFAULT                                        |
| HISTORY                                                               |
|     Generated at 30-08-2013 at 03:08:55 by user ANONYMOUS             |
+=======================================================================*/
--
--
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.XLA_00240_AAD_S_000004_PKG';
...
EXCEPTION
       ...
       WHEN OTHERS THEN
          xla_exceptions_pkg.raise_message
             (p_location   => 'XLA_00240_AAD_S_000004_PKG.trace');
END trace;
...
WHEN OTHERS  THEN
   xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.ValidateLookupMeaning');
       --
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.GetMeaning');
END GetMeaning;
--
...
l_component_code        := 'PN_EXPENSE_DESCRIPTION';
l_component_type_code   := 'S';
l_component_appl_id     :=  240;
...
l_description           := NULL;
  ...
  IF l_source IS NOT NULL THEN
    ...
    l_description :=  SUBSTR(CONCAT(l_description,' '),1,2000);
  END IF;   
  l_source := SUBSTR(p_source_6,1,1996);
  IF l_source IS NOT NULL THEN
    l_description :=  SUBSTR(CONCAT(l_description,l_source),1,2000);
    l_description :=  SUBSTR(CONCAT(l_description,' '),1,2000);
  END IF;   
...
EXCEPTION
 ...
 WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.Description_1');
END Description_1;
...
l_ledger_language       := xla_ae_journal_entry_pkg.g_cache_ledgers_info.description_language;
l_component_type        := 'AMB_DESCRIPTION';
l_component_code        := 'PN_REVENUE_DESCRIPTION';
l_component_type_code   := 'S';
l_component_appl_id     :=  240;
l_amb_context_code      := 'DEFAULT';
l_source                := NULL;
l_description           := NULL;
  ...
  IF l_source IS NOT NULL THEN
    ...
    l_description :=  SUBSTR(CONCAT(l_description,' '),1,2000);
  END IF;   
  l_source := SUBSTR(p_source_9,1,1996);
  IF l_source IS NOT NULL THEN
    l_description :=  SUBSTR(CONCAT(l_description,l_source),1,2000);
    l_description :=  SUBSTR(CONCAT(l_description,' '),1,2000);
  END IF;   
...
EXCEPTION
 ...
 WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.Description_2');
END Description_2;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.AcctDerRule_3');
END AcctDerRule_3;
--
...
--Payment / Billing Item Identifier
...
l_component_code             := 'PN_ACCR_LIA';
l_component_type_code        := 'S';
l_component_appl_id          :=  240;
l_amb_context_code           := 'DEFAULT';
l_entity_code                := 'TRANSACTION';
l_event_class_code           := 'LEASE_EXPENSE';
l_event_type_code            := 'LEASE_EXPENSE_TRANSFER';
l_line_definition_owner_code := 'S';
l_line_definition_code       := 'PN_LEASE_EXPENSE';
--
...
') =  'ACC'
 THEN 
   ...
   l_rec_acct_attrs.array_char_value(2)  := p_source_11;
   ...
   l_rec_acct_attrs.array_num_value(8)  := p_source_18;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.AcctLineType_4');
END AcctLineType_4;
--
...
--
l_component_type             := 'AMB_JLT';
l_component_code             := 'PN_ACC_AST';
l_component_type_code        := 'S';
l_component_appl_id          :=  240;
l_amb_context_code           := 'DEFAULT';
l_entity_code                := 'TRANSACTION';
l_event_class_code           := 'LEASE_REVENUE';
l_event_type_code            := 'LEASE_REVENUE_TRANSFER';
l_line_definition_owner_code := 'S';
l_line_definition_code       := 'PN_LEASE_REVENUE';
--
l_balance_type_code          := 'A';
l_segment                     := NULL;
l_ccid                        := NULL;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.AcctLineType_5');
END AcctLineType_5;
--
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.AcctLineType_6');
END AcctLineType_6;
--
...
EXCEPTION
  ...
  WHEN OTHERS THEN
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.AcctLineType_7');
END AcctLineType_7;
--
...
C_EVENT_TYPE_CODE    CONSTANT  VARCHAR2(30)  := 'LEASE_EXPENSE_TRANSFER';
C_EVENT_CLASS_CODE   CONSTANT  VARCHAR2(30) := 'LEASE_EXPENSE';
...
SELECT
  ...
  FROM (
       SELECT xet.event_id                  event_id
            , 0                             line_number
            , CASE r
               WHEN 1 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 2 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 3 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 4 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 5 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 6 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 7 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
                WHEN 8 THEN 'PN_XLA_EXTRACT_HEADERS_V' 
            ...
            , CASE r
                ...
                WHEN 2 THEN 'SUPPLIER_SITE' 
                WHEN 3 THEN 'PAYMENT_PURPOSE' 
                WHEN 4 THEN 'PAYMENT_TYPE' 
                WHEN 5 THEN 'LOCATION_CODE' 
                WHEN 6 THEN 'LEASE_NUMBER' 
                WHEN 7 THEN 'PROPERTY_CODE' 
                WHEN 8 THEN 'PAYMENT_ITEM_ID' 
                
                ELSE null
              END                           source_code
            , CASE r
                WHEN 1 THEN TO_CHAR(h1.SUPPLIER_NAME)
                WHEN 2 THEN TO_CHAR(h1.SUPPLIER_SITE)
                WHEN 3 THEN TO_CHAR(h1.PAYMENT_PURPOSE)
                WHEN 4 THEN TO_CHAR(h1.PAYMENT_TYPE)
                WHEN 5 THEN TO_CHAR(h1.LOCATION_CODE)
                WHEN 6 THEN TO_CHAR(h1.LEASE_NUMBER)
                WHEN 7 THEN TO_CHAR(h1.PROPERTY_CODE)
                WHEN 8 THEN TO_CHAR(h1.PAYMENT_ITEM_ID)
                
                ELSE null
              END                           source_value
            , null              source_meaning
        FROM xla_events_gt     xet  
      , PN_XLA_EXTRACT_HEADERS_V  h1
...
SELECT  event_id
  ...
  FROM (
       SELECT xet.event_id                  event_id
            ...
            , CASE r
               WHEN 1 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 2 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 3 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 4 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 5 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 6 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 7 THEN 'PN_XLA_EXTRACT_LINES_V' 
                WHEN 8 THEN 'PN_XLA_EXTRACT_LINES_V' 
            ...
            , CASE r
                ...
                WHEN 2 THEN 'DISTRIBUTION_TYPE' 
                WHEN 3 THEN 'ENTERED_AMOUNT' 
                WHEN 4 THEN 'CURRENCY_CODE' 
                WHEN 5 THEN 'CONVERSION_DATE' 
                WHEN 6 THEN 'CONVERSION_RATE' 
                WHEN 7 THEN 'CONVERSION_RATE_TYPE' 
                WHEN 8 THEN 'ACCOUNTED_AMOUNT' 
                
                ELSE null
              END                           source_code
            , CASE r
                WHEN 1 THEN TO_CHAR(l2.CODE_COMBINATION_ID)
                WHEN 2 THEN TO_CHAR(l2.DISTRIBUTION_TYPE)
                WHEN 3 THEN TO_CHAR(l2.ENTERED_AMOUNT)
                WHEN 4 THEN TO_CHAR(l2.CURRENCY_CODE)
                WHEN 5 THEN TO_CHAR(l2.CONVERSION_DATE)
                WHEN 6 THEN TO_CHAR(l2.CONVERSION_RATE)
                WHEN 7 THEN TO_CHAR(l2.CONVERSION_RATE_TYPE)
                WHEN 8 THEN TO_CHAR(l2.ACCOUNTED_AMOUNT)
                
                ELSE null
              END                           source_value
            , null              source_meaning
         FROM  xla_events_gt     xet  
        , PN_XLA_EXTRACT_LINES_V  l2
            ,(select rownum r from all_objects where rownum <= 8 and owner = p_apps_owner)
        WHERE xet.event_date between p_pad_start_date AND p_pad_end_date
          AND xet.event_type_code = C_EVENT_TYPE_CODE
            AND l2.event_id          = xet.event_id
  AND l2.ledger_id (+)  = p_sla_ledger_id
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      IF (C_LEVEL_EXCEPTION >= g_log_level) THEN
       ...
       END IF;
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.insert_sources_8');
...
l_rows                      NUMBER  := 1000;
l_event_type_name           VARCHAR2(80) := 'Lease Expense Item Transfer';
l_event_class_name          VARCHAR2(80) := 'Lease Expense';
...
TYPE t_array_source_1 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.SUPPLIER_NAME%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_2 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.SUPPLIER_SITE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_3 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.PAYMENT_PURPOSE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_4 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.PAYMENT_TYPE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_5 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.LOCATION_CODE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_6 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.LEASE_NUMBER%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_7 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.PROPERTY_CODE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_12 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.PAYMENT_ITEM_ID%TYPE INDEX BY BINARY_INTEGER;

TYPE t_array_source_10 IS TABLE OF PN_XLA_EXTRACT_LINES_V.CODE_COMBINATION_ID%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_11 IS TABLE OF PN_XLA_EXTRACT_LINES_V.DISTRIBUTION_TYPE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_13 IS TABLE OF PN_XLA_EXTRACT_LINES_V.ENTERED_AMOUNT%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_14 IS TABLE OF PN_XLA_EXTRACT_LINES_V.CURRENCY_CODE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_15 IS TABLE OF PN_XLA_EXTRACT_LINES_V.CONVERSION_DATE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_16 IS TABLE OF PN_XLA_EXTRACT_LINES_V.CONVERSION_RATE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_17 IS TABLE OF PN_XLA_EXTRACT_LINES_V.CONVERSION_RATE_TYPE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_18 IS TABLE OF PN_XLA_EXTRACT_LINES_V.ACCOUNTED_AMOUNT%TYPE INDEX BY BINARY_INTEGER;
...
-- Event Type Code: LEASE_EXPENSE_TRANSFER
-- Event Class Code: LEASE_EXPENSE
    xet.entity_id
  , xet.legal_entity_id
  , xet.entity_code
  ...
  , h1.SUPPLIER_NAME    source_1
  , h1.SUPPLIER_SITE    source_2
  , h1.PAYMENT_PURPOSE    source_3
  , h1.PAYMENT_TYPE    source_4
  , h1.LOCATION_CODE    source_5
  , h1.LEASE_NUMBER    source_6
  , h1.PROPERTY_CODE    source_7
  , h1.PAYMENT_ITEM_ID    source_12
  FROM xla_events_gt     xet 
  , PN_XLA_EXTRACT_HEADERS_V  h1
 WHERE xet.event_date between p_pad_start_date and p_pad_end_date
   and xet.event_type_code = C_EVENT_TYPE_CODE
...
-- Event Class Code: LEASE_EXPENSE
  ...
  , l2.CODE_COMBINATION_ID    source_10
  , l2.DISTRIBUTION_TYPE    source_11
  , l2.ENTERED_AMOUNT    source_13
  , l2.CURRENCY_CODE    source_14
  , l2.CONVERSION_DATE    source_15
  , l2.CONVERSION_RATE    source_16
  , l2.CONVERSION_RATE_TYPE    source_17
  , l2.ACCOUNTED_AMOUNT    source_18
  FROM xla_events_gt     xet 
  , PN_XLA_EXTRACT_LINES_V  l2
...
g_array_event(l_event_id).array_value_char('source_6') := l_array_source_6(hdr_idx);
...
xla_ae_header_pkg.SetHdrDescription(
 ...
 , p_source_6 => g_array_event(l_event_id).array_value_char('source_6')
...
xla_exceptions_pkg.raise_message
      (p_location => 'XLA_00240_AAD_S_000004_PKG.EventType_8');
...
C_EVENT_TYPE_CODE    CONSTANT  VARCHAR2(30)  := 'LEASE_REVENUE_TRANSFER';
C_EVENT_CLASS_CODE   CONSTANT  VARCHAR2(30) := 'LEASE_REVENUE';
p_apps_owner                   VARCHAR2(30);
l_log_module                   VARCHAR2(240);
BEGIN
IF g_log_enabled THEN
...
SELECT
  ...
  FROM (
       SELECT xet.event_id                  event_id
            ...
            , 'S'             source_type_code
            , CASE r
                WHEN 1 THEN 'PAYMENT_PURPOSE' 
                WHEN 2 THEN 'PAYMENT_TYPE' 
                WHEN 3 THEN 'LOCATION_CODE' 
                WHEN 4 THEN 'LEASE_NUMBER' 
                WHEN 5 THEN 'PROPERTY_CODE' 
                WHEN 6 THEN 'CUSTOMER_NAME' 
                WHEN 7 THEN 'CUSTOMER_SITE' 
                WHEN 8 THEN 'PAYMENT_ITEM_ID' 
                
                ELSE null
              END                           source_code
            , CASE r
                WHEN 1 THEN TO_CHAR(h1.PAYMENT_PURPOSE)
                WHEN 2 THEN TO_CHAR(h1.PAYMENT_TYPE)
                WHEN 3 THEN TO_CHAR(h1.LOCATION_CODE)
                WHEN 4 THEN TO_CHAR(h1.LEASE_NUMBER)
                WHEN 5 THEN TO_CHAR(h1.PROPERTY_CODE)
                WHEN 6 THEN TO_CHAR(h1.CUSTOMER_NAME)
                WHEN 7 THEN TO_CHAR(h1.CUSTOMER_SITE)
                WHEN 8 THEN TO_CHAR(h1.PAYMENT_ITEM_ID)
...
EXCEPTION
  ...
  WHEN OTHERS THEN
      IF (C_LEVEL_EXCEPTION >= g_log_level) THEN
       ...
       END IF;
       xla_exceptions_pkg.raise_message
           (p_location => 'XLA_00240_AAD_S_000004_PKG.insert_sources_9');
...
l_result                    BOOLEAN := TRUE;
l_rows                      NUMBER  := 1000;
l_event_type_name           VARCHAR2(80) := 'Lease Revenue Item Transfer';
l_event_class_name          VARCHAR2(80) := 'Lease Revenue';
l_description               VARCHAR2(4000);
l_transaction_reversal      NUMBER;
l_ae_header_id              NUMBER;
l_array_extract_line_num    xla_ae_journal_entry_pkg.t_array_Num;
...
TYPE t_array_source_6 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.LEASE_NUMBER%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_7 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.PROPERTY_CODE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_8 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.CUSTOMER_NAME%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_9 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.CUSTOMER_SITE%TYPE INDEX BY BINARY_INTEGER;
TYPE t_array_source_12 IS TABLE OF PN_XLA_EXTRACT_HEADERS_V.PAYMENT_ITEM_ID%TYPE INDEX BY BINARY_INTEGER;
...
IS
SELECT /*+ leading(xet) cardinality(xet,1) */
-- Event Type Code: LEASE_REVENUE_TRANSFER
-- Event Class Code: LEASE_REVENUE
    xet.entity_id
  , xet.legal_entity_id
  , xet.entity_code
  , xet.transaction_number
  ...
  , h1.LEASE_NUMBER    source_6
  , h1.PROPERTY_CODE    source_7
  , h1.CUSTOMER_NAME    source_8
  , h1.CUSTOMER_SITE    source_9
  , h1.PAYMENT_ITEM_ID    source_12
  FROM xla_events_gt     xet 
  , PN_XLA_EXTRACT_HEADERS_V  h1
 WHERE xet.event_date between p_pad_start_date and p_pad_end_date
...
xla_exceptions_pkg.raise_message
      (p_location => 'XLA_00240_AAD_S_000004_PKG.EventType_9');
...
WHEN OTHERS THEN
   xla_exceptions_pkg.raise_message
      (p_location => 'XLA_00240_AAD_S_000004_PKG.CreateHeadersAndLines');
...
xla_ae_journal_entry_pkg.SetProductAcctDefinition
   (p_product_rule_code      => 'PN_LEASE_ACCOUNTING'
   ,p_product_rule_type_code => 'S'
   ,p_product_rule_version   => ''
   ,p_product_rule_name      => 'Property Manager Normalized Accounting'
   ,p_amb_context_code       => 'DEFAULT'
   );
...
WHEN OTHERS THEN
   ...
   END IF;
   xla_exceptions_pkg.raise_message
      (p_location => 'XLA_00240_AAD_S_000004_PKG.CreateJournalEntries');
...
BEGIN
   ...
   END IF;
--
END XLA_00240_AAD_S_000004_PKG;
--
