--------------------------------------------------------
--  DDL for Package Body QP_QUALIFIER_RULES_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."QP_QUALIFIER_RULES_PUB_W" as
  /* $Header: ozfwqqrb.pls 120.3 2005/08/26 15:39 julou noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).created_from_rule_id := rosetta_g_miss_num_map(a18(indx));
          ...
          t(ddindx).list_header_id := rosetta_g_miss_num_map(a25(indx));
          t(ddindx).list_line_id := rosetta_g_miss_num_map(a26(indx));
          ...
          t(ddindx).qualifier_grouping_no := rosetta_g_miss_num_map(a35(indx));
          t(ddindx).qualifier_id := rosetta_g_miss_num_map(a36(indx));
          t(ddindx).qualifier_precedence := rosetta_g_miss_num_map(a37(indx));
          t(ddindx).qualifier_rule_id := rosetta_g_miss_num_map(a38(indx));
          t(ddindx).request_id := rosetta_g_miss_num_map(a39(indx));
          t(ddindx).start_date_active := rosetta_g_miss_date_in_map(a40(indx));
          t(ddindx).list_type_code := a41(indx);
          t(ddindx).qual_attr_value_from_number := rosetta_g_miss_num_map(a42(indx));
          t(ddindx).qual_attr_value_to_number := rosetta_g_miss_num_map(a43(indx));
          t(ddindx).active_flag := a44(indx);
          t(ddindx).search_ind := rosetta_g_miss_num_map(a45(indx));
          t(ddindx).qualifier_group_cnt := rosetta_g_miss_num_map(a46(indx));
          t(ddindx).header_quals_exist_flag := a47(indx);
          t(ddindx).distinct_row_count := rosetta_g_miss_num_map(a48(indx));
    ...
    a51 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a18(indx) := rosetta_g_miss_num_map(t(ddindx).created_from_rule_id);
          ...
          a25(indx) := rosetta_g_miss_num_map(t(ddindx).list_header_id);
          a26(indx) := rosetta_g_miss_num_map(t(ddindx).list_line_id);
          ...
          a35(indx) := rosetta_g_miss_num_map(t(ddindx).qualifier_grouping_no);
          a36(indx) := rosetta_g_miss_num_map(t(ddindx).qualifier_id);
          a37(indx) := rosetta_g_miss_num_map(t(ddindx).qualifier_precedence);
          a38(indx) := rosetta_g_miss_num_map(t(ddindx).qualifier_rule_id);
          a39(indx) := rosetta_g_miss_num_map(t(ddindx).request_id);
          a40(indx) := t(ddindx).start_date_active;
          a41(indx) := t(ddindx).list_type_code;
          a42(indx) := rosetta_g_miss_num_map(t(ddindx).qual_attr_value_from_number);
          a43(indx) := rosetta_g_miss_num_map(t(ddindx).qual_attr_value_to_number);
          a44(indx) := t(ddindx).active_flag;
          a45(indx) := rosetta_g_miss_num_map(t(ddindx).search_ind);
          a46(indx) := rosetta_g_miss_num_map(t(ddindx).qualifier_group_cnt);
          a47(indx) := t(ddindx).header_quals_exist_flag;
          a48(indx) := rosetta_g_miss_num_map(t(ddindx).distinct_row_count);
