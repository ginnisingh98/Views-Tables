--------------------------------------------------------
--  DDL for Package Body DPP_BPEL_GETOUTBOUNDPL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."DPP_BPEL_GETOUTBOUNDPL" AS
/* $Header: dppvbonb.pls 120.2 2007/12/12 06:22:13 sdasan noship $ */
	FUNCTION PL_TO_SQL7(aPlsqlItem DPP_PRICING_PVT.DPP_PL_NOTIFY_REC_TYPE)
 	RETURN DPP_PRICING_PVT_DPP_PL_NOTIF6 IS
	aSqlItem DPP_PRICING_PVT_DPP_PL_NOTIF6;
	BEGIN
		-- initialize the object
		aSqlItem := DPP_PRICING_PVT_DPP_PL_NOTIF6(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	...
	FUNCTION PL_TO_SQL9(aPlsqlItem DPP_PRICING_PVT.DPP_OBJECT_NAME_TBL_TYPE)
 	RETURN DPPPRICINGPVTDPPPLNOTIF8_DPP_ IS
	aSqlItem DPPPRICINGPVTDPPPLNOTIF8_DPP_;
	BEGIN
	...
	FUNCTION PL_TO_SQL10(aPlsqlItem DPP_PRICING_PVT.DPP_PL_NOTIFY_LINE_REC_TYPE)
 	RETURN DPP_PRICING_PVT_DPP_PL_NOTIF8 IS
	aSqlItem DPP_PRICING_PVT_DPP_PL_NOTIF8;
	BEGIN
		-- initialize the object
		aSqlItem := DPP_PRICING_PVT_DPP_PL_NOTIF8(NULL, NULL, NULL, NULL, NULL);
		aSqlItem.INVENTORY_ITEM_ID := aPlsqlItem.INVENTORY_ITEM_ID;
		aSqlItem.ITEM_NUMBER := aPlsqlItem.ITEM_NUMBER;
		aSqlItem.OBJECT_NAME_TBL := PL_TO_SQL9(aPlsqlItem.OBJECT_NAME_TBL);
	...
	BEGIN
		...
		aPlsqlItem.OBJECT_NAME_TBL := SQL_TO_PL9(aSqlItem.OBJECT_NAME_TBL);
	...
	FUNCTION PL_TO_SQL8(aPlsqlItem DPP_PRICING_PVT.DPP_PL_NOTIFY_LINE_TBL_TYPE)
 	RETURN DPP_PRICING_PVT_DPP_PL_NOTIF7 IS
	aSqlItem DPP_PRICING_PVT_DPP_PL_NOTIF7;
	BEGIN
		-- initialize the table
		aSqlItem := DPP_PRICING_PVT_DPP_PL_NOTIF7();
		aSqlItem.EXTEND(aPlsqlItem.COUNT);
	...
	BEGIN
		FOR I IN 1..aSqlItem.COUNT LOOP
			aPlsqlItem(I) := SQL_TO_PL10(aSqlItem(I));
		END LOOP;
		RETURN aPlsqlItem;
	END SQL_TO_PL8;
   ...
   P_PL_NOTIFY_HDR_REC IN OUT NOCOPY DPP_PRICING_PVT_DPP_PL_NOTIF6,P_PL_NOTIFY_LINE_TBL IN OUT NOCOPY DPP_PRICING_PVT_DPP_PL_NOTIF7) IS
   ...
   BEGIN
      P_PL_NOTIFY_HDR_REC_ := DPP_BPEL_GETOUTBOUNDPL.SQL_TO_PL7(P_PL_NOTIFY_HDR_REC);
      P_PL_NOTIFY_LINE_TBL_ := DPP_BPEL_GETOUTBOUNDPL.SQL_TO_PL8(P_PL_NOTIFY_LINE_TBL);
      APPS.DPP_PRICING_PVT.NOTIFY_OUTBOUNDPRICELISTS(P_API_VERSION,P_INIT_MSG_LIST,P_COMMIT,P_VALIDATION_LEVEL,X_RETURN_STATUS,X_MSG_COUNT,X_MSG_DATA,P_PL_NOTIFY_HDR_REC_,P_PL_NOTIFY_LINE_TBL_);
      P_PL_NOTIFY_HDR_REC := DPP_BPEL_GETOUTBOUNDPL.PL_TO_SQL7(P_PL_NOTIFY_HDR_REC_);
      P_PL_NOTIFY_LINE_TBL := DPP_BPEL_GETOUTBOUNDPL.PL_TO_SQL8(P_PL_NOTIFY_LINE_TBL_);
   END DPP_PRICING_PVT$NOTIFY_OUTBOU;
