--------------------------------------------------------
--  DDL for Package Body IGS_EN_TIMESLOT_RSLT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_EN_TIMESLOT_RSLT_PKG" AS
/* $Header: IGSEI42B.pls 115.4 2002/11/28 23:42:34 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_en_timeslot_rslt%ROWTYPE;
  new_references igs_en_timeslot_rslt%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    CLOSE cur_old_ref_values;
    -- Populate New Values.
    new_references.igs_en_timeslot_rslt_id           := x_igs_en_timeslot_rslt_id;
    new_references.igs_en_timeslot_para_id           := x_igs_en_timeslot_para_id;
    new_references.person_id                         := x_person_id;
    new_references.start_dt_time                     := x_start_dt_time;
    new_references.end_dt_time                       := x_end_dt_time;
  ...
  */
  BEGIN
    IF (((old_references.igs_en_timeslot_para_id = new_references.igs_en_timeslot_para_id)) OR
        ((new_references.igs_en_timeslot_para_id IS NULL))) THEN
      NULL;
    ELSIF NOT igs_en_timeslot_para_pkg.get_pk_for_validation (
  ...
  */
  ...
  BEGIN
    ...
    END IF;
    CLOSE cur_rowid;
  END get_fk_igs_en_timeslot_para;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.igs_en_timeslot_para_id = x_igs_en_timeslot_para_id)
        AND (tlinfo.person_id = x_person_id)
        AND ((tlinfo.start_dt_time = x_start_dt_time) OR ((tlinfo.start_dt_time IS NULL) AND (X_start_dt_time IS NULL)))
        AND ((tlinfo.end_dt_time = x_end_dt_time) OR ((tlinfo.end_dt_time IS NULL) AND (X_end_dt_time IS NULL)))
       ) THEN
      NULL;
    ELSE
