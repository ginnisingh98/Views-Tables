--------------------------------------------------------
--  DDL for Package Body XNP_ERROR_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_ERROR_U" AS
/*$Header: XNPERRRB.pls 120.2 2006/02/13 07:47:08 dputhiye ship $*/


PROCEDURE CREATE_MSG  (   XNP$ERROR_CODE VARCHAR2,
...
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('ERROR',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'ERROR' ;
  ...
  XNP_XML_UTILS.write_element ( 'ERROR_CODE', XNP$ERROR_CODE );
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'ERROR.create_msg()::' || SQLERRM ;
END ;
PROCEDURE PUBLISH  (   XNP$ERROR_CODE VARCHAR2,
  XNP$DESCRIPTION VARCHAR2,
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'ERROR', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_MSG_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$ERROR_CODE=>XNP$ERROR_CODE,
...
END ;
PROCEDURE SEND  (   XNP$ERROR_CODE VARCHAR2,
  XNP$DESCRIPTION VARCHAR2,
  x_message_id OUT NOCOPY  NUMBER,
  x_error_code OUT NOCOPY  NUMBER,
  x_error_message OUT NOCOPY VARCHAR2,
