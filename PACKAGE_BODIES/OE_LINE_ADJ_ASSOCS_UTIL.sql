--------------------------------------------------------
--  DDL for Package Body OE_LINE_ADJ_ASSOCS_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_LINE_ADJ_ASSOCS_UTIL" AS
/* $Header: OEXULASB.pls 120.0 2005/06/01 02:56:45 appldev noship $ */

G_PKG_NAME              CONSTANT VARCHAR2(30) := 'Oe_Line_Adj_Assocs_util';
...
BEGIN
	Query_Rows
     ...
     x_Line_Adj_Assoc_Rec := l_Line_Adj_Assoc_Tbl(1);
...
BEGIN
	...
	FOR iml_rec IN l_price_Adj_assoc_csr LOOP
 	...
 	x_Line_Adj_Assoc_Tbl(l_count).PRICE_ADJUSTMENT_ID :=iml_rec.PRICE_ADJUSTMENT_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).CREATION_DATE	:=	iml_rec.CREATION_DATE;
 	x_Line_Adj_Assoc_Tbl(l_count).CREATED_BY		:=	iml_rec.CREATED_BY;
 	...
 	x_Line_Adj_Assoc_Tbl(l_count).PROGRAM_APPLICATION_ID:=iml_rec.PROGRAM_APPLICATION_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).PROGRAM_ID		:=	iml_rec.PROGRAM_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).PROGRAM_UPDATE_DATE :=iml_rec.PROGRAM_UPDATE_DATE;
 	x_Line_Adj_Assoc_Tbl(l_count).REQUEST_ID		:=	iml_rec.REQUEST_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).PRICE_ADJ_ASSOC_ID :=	iml_rec.PRICE_ADJ_ASSOC_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).LINE_ID :=	iml_rec.LINE_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).RLTD_PRICE_ADJ_ID :=	iml_rec.RLTD_PRICE_ADJ_ID;
 	x_Line_Adj_Assoc_Tbl(l_count).LOCK_CONTROL :=	iml_rec.LOCK_CONTROL;

	-- set values for non-DB fields
 	x_Line_Adj_Assoc_Tbl(l_count).db_flag 		:=	FND_API.G_TRUE;
 	x_Line_Adj_Assoc_Tbl(l_count).operation 	:=	FND_API.G_MISS_CHAR;
 	x_Line_Adj_Assoc_Tbl(l_count).return_status 	:=	FND_API.G_MISS_CHAR;
...
BEGIN
	...
	( 	p_Line_Adj_Assoc_Rec.PRICE_ADJUSTMENT_ID
 	...
 	p_Line_Adj_Assoc_Rec.lock_control := l_lock_control;
...
IF p_price_adjustment_id <> FND_API.G_MISS_NUM then
   ...
   END IF;
     --Modified the below delete statement to fix bug 2626303
...
l_Line_Adj_Assoc_Rec  OE_Order_PUB.Line_Adj_Assoc_Rec_Type := p_x_Line_Adj_Assoc_Rec;
--
l_debug_level CONSTANT NUMBER := oe_debug_pub.g_debug_level;
--
BEGIN

    IF l_Line_Adj_Assoc_Rec.RLTD_PRICE_ADJ_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.RLTD_PRICE_ADJ_ID := p_old_Line_Adj_Assoc_Rec.RLTD_PRICE_ADJ_ID;
    END IF;


    IF l_Line_Adj_Assoc_Rec.PRICE_ADJUSTMENT_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PRICE_ADJUSTMENT_ID := p_old_Line_Adj_Assoc_Rec.PRICE_ADJUSTMENT_ID;
    END IF;

    IF l_Line_Adj_Assoc_Rec.CREATION_DATE = FND_API.G_MISS_DATE THEN
        l_Line_Adj_Assoc_Rec.CREATION_DATE := p_old_Line_Adj_Assoc_Rec.CREATION_DATE;
    END IF;

    IF l_Line_Adj_Assoc_Rec.CREATED_BY = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.CREATED_BY := p_old_Line_Adj_Assoc_Rec.CREATED_BY;
    END IF;
    ...
    IF l_Line_Adj_Assoc_Rec.PROGRAM_APPLICATION_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PROGRAM_APPLICATION_ID := p_old_Line_Adj_Assoc_Rec.PROGRAM_APPLICATION_ID;
    END IF;

    IF l_Line_Adj_Assoc_Rec.PROGRAM_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PROGRAM_ID := p_old_Line_Adj_Assoc_Rec.PROGRAM_ID;
    END IF;
    ...
    IF l_Line_Adj_Assoc_Rec.REQUEST_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.REQUEST_ID := p_old_Line_Adj_Assoc_Rec.REQUEST_ID;
    END IF;

    IF l_Line_Adj_Assoc_Rec.PRICE_ADJ_ASSOC_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PRICE_ADJ_ASSOC_ID := p_old_Line_Adj_Assoc_Rec.PRICE_ADJ_ASSOC_ID;
    END IF;

    IF l_Line_Adj_Assoc_Rec.LINE_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.LINE_ID := p_old_Line_Adj_Assoc_Rec.LINE_ID;
    END IF;

    -- RETURN l_Line_Adj_Assoc_Rec;
...
BEGIN
    ...
    IF l_Line_Adj_Assoc_Rec.RLTD_PRICE_ADJ_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.RLTD_PRICE_ADJ_ID := Null;
    END IF;

    IF l_Line_Adj_Assoc_Rec.PRICE_ADJUSTMENT_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PRICE_ADJUSTMENT_ID := Null;
    END IF;

    IF l_Line_Adj_Assoc_Rec.CREATION_DATE = FND_API.G_MISS_DATE THEN
        l_Line_Adj_Assoc_Rec.CREATION_DATE := Null;
    END IF;

    IF l_Line_Adj_Assoc_Rec.CREATED_BY = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.CREATED_BY := Null;
    END IF;
    ...
    IF l_Line_Adj_Assoc_Rec.PROGRAM_APPLICATION_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PROGRAM_APPLICATION_ID := Null;
    END IF;

    IF l_Line_Adj_Assoc_Rec.PROGRAM_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PROGRAM_ID := Null;
    END IF;
    ...
    IF l_Line_Adj_Assoc_Rec.REQUEST_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.REQUEST_ID := Null;
    END IF;

    IF l_Line_Adj_Assoc_Rec.PRICE_ADJ_ASSOC_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.PRICE_ADJ_ASSOC_ID := Null;
    END IF;

    IF l_Line_Adj_Assoc_Rec.LINE_ID = FND_API.G_MISS_NUM THEN
        l_Line_Adj_Assoc_Rec.LINE_ID := Null;
    END IF;
...
BEGIN
    ...
    p_x_Line_Adj_Assoc_Rec := p_x_Line_Adj_Assoc_Rec;
...
BEGIN
    ...
    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'ENTERING OE_LINE_ADJ_ASSOCS_UTIL.LOCK_ROW.' , 1 ) ;
    END IF;
    SAVEPOINT Lock_Row;
    ...
    ELSE
      l_price_adj_assoc_id := p_x_Line_Adj_Assoc_rec.price_adj_assoc_id;
      l_lock_control := p_x_Line_Adj_Assoc_rec.lock_control;
    END IF;
    ...
    ,x_Line_Adj_Assoc_rec	=> p_x_Line_Adj_Assoc_rec
    );


    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'QUERIED LOCK_CONTROL: '|| P_X_LINE_ADJ_ASSOC_REC.LOCK_CONTROL , 1 ) ;
    END IF;
    ...
    THEN
        ...
        p_x_Line_Adj_Assoc_rec.return_status     := FND_API.G_RET_STS_SUCCESS;
    ...
    IF      OE_GLOBALS.Equal(p_x_Line_Adj_Assoc_rec.lock_control,
                             l_lock_control)
    THEN
    ...
    ELSE
        ...
        p_x_Line_Adj_Assoc_rec.return_status       := FND_API.G_RET_STS_ERROR;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        p_x_Line_Adj_Assoc_Rec.return_status := FND_API.G_RET_STS_UNEXP_ERROR;
...
IS
  ...
  CURSOR lock_assoc_lines(p_price_adjustment_id IN NUMBER) IS
  SELECT price_adj_assoc_id
  FROM   oe_price_adj_assocs
...
BEGIN
    ...
    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'ENTERING OE_LINE_ADJ_ASSOCS_UTIL.LOCK_ROWS.' , 1 ) ;
    END IF;
  ...
  BEGIN
    IF p_price_adjustment_id <> FND_API.G_MISS_NUM THEN
	 SAVEPOINT LOCK_ROWS;
	 OPEN lock_assoc_lines(p_price_adjustment_id);

	 LOOP
	   FETCH lock_assoc_lines INTO l_price_adj_assoc_id;
	   EXIT WHEN lock_assoc_lines%NOTFOUND;
      END LOOP;
      CLOSE lock_assoc_lines;
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
	 ROLLBACK TO LOCK_ROWS;

	 IF lock_assoc_lines%ISOPEN THEN
        CLOSE lock_assoc_lines;
      END IF;
  ...
  EXCEPTION
    ...
    IF l_debug_level  > 0 THEN
        oe_debug_pub.add(  'EXITING OE_LINE_ADJ_ASSOCS_UTIL.LOCK_ROWS.' , 1 ) ;
    END IF;
