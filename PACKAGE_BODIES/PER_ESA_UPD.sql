--------------------------------------------------------
--  DDL for Package Body PER_ESA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ESA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_esa_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy per_esa_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the per_establishment_attendances Row
  --
  ...
  establishment                     = p_rec.establishment,
  attended_start_date               = p_rec.attended_start_date,
  attended_end_date                 = p_rec.attended_end_date,
  full_time                         = p_rec.full_time,
  ...
  party_id                          = p_rec.party_id,  -- HR/TCA merge
  address			    = p_rec.address
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in per_esa_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update(p_rec in per_esa_shd.g_rec_type,
...
Begin
  ...
  begin
    per_esa_rku.after_update
      (p_attendance_id            =>p_rec.attendance_id,
       p_person_id                =>p_rec.person_id,
       p_establishment_id         =>p_rec.establishment_id,
       p_establishment            =>p_rec.establishment,
       p_attended_start_date      =>p_rec.attended_start_date,
       p_attended_end_date        =>p_rec.attended_end_date,
       p_full_time                =>p_rec.full_time,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy per_esa_shd.g_rec_type) is
--
...
Begin
  ...
  End If;
  If (p_rec.establishment = hr_api.g_varchar2) then
    p_rec.establishment :=
    per_esa_shd.g_old_rec.establishment;
  End If;
  If (p_rec.attended_start_date = hr_api.g_date) then
    p_rec.attended_start_date :=
    per_esa_shd.g_old_rec.attended_start_date;
  End If;
  If (p_rec.attended_end_date = hr_api.g_date) then
    p_rec.attended_end_date :=
    per_esa_shd.g_old_rec.attended_end_date;
  End If;
  If (p_rec.full_time = hr_api.g_varchar2) then
    p_rec.full_time :=
    per_esa_shd.g_old_rec.full_time;
  End If;
  If (p_rec.attribute_category = hr_api.g_varchar2) then
    p_rec.attribute_category :=
    per_esa_shd.g_old_rec.attribute_category;
  End If;
  If (p_rec.attribute1 = hr_api.g_varchar2) then
    p_rec.attribute1 :=
    per_esa_shd.g_old_rec.attribute1;
  End If;
  If (p_rec.attribute2 = hr_api.g_varchar2) then
    p_rec.attribute2 :=
    per_esa_shd.g_old_rec.attribute2;
  End If;
  If (p_rec.attribute3 = hr_api.g_varchar2) then
    p_rec.attribute3 :=
    per_esa_shd.g_old_rec.attribute3;
  End If;
  If (p_rec.attribute4 = hr_api.g_varchar2) then
    p_rec.attribute4 :=
    per_esa_shd.g_old_rec.attribute4;
  End If;
  If (p_rec.attribute5 = hr_api.g_varchar2) then
    p_rec.attribute5 :=
    per_esa_shd.g_old_rec.attribute5;
  End If;
  If (p_rec.attribute6 = hr_api.g_varchar2) then
    p_rec.attribute6 :=
    per_esa_shd.g_old_rec.attribute6;
  End If;
  If (p_rec.attribute7 = hr_api.g_varchar2) then
    p_rec.attribute7 :=
    per_esa_shd.g_old_rec.attribute7;
  End If;
  If (p_rec.attribute8 = hr_api.g_varchar2) then
    p_rec.attribute8 :=
    per_esa_shd.g_old_rec.attribute8;
  End If;
  If (p_rec.attribute9 = hr_api.g_varchar2) then
    p_rec.attribute9 :=
    per_esa_shd.g_old_rec.attribute9;
  End If;
  If (p_rec.attribute10 = hr_api.g_varchar2) then
    p_rec.attribute10 :=
    per_esa_shd.g_old_rec.attribute10;
  End If;
  If (p_rec.attribute11 = hr_api.g_varchar2) then
    p_rec.attribute11 :=
    per_esa_shd.g_old_rec.attribute11;
  End If;
  If (p_rec.attribute12 = hr_api.g_varchar2) then
    p_rec.attribute12 :=
    per_esa_shd.g_old_rec.attribute12;
  End If;
  If (p_rec.attribute13 = hr_api.g_varchar2) then
    p_rec.attribute13 :=
    per_esa_shd.g_old_rec.attribute13;
  End If;
  If (p_rec.attribute14 = hr_api.g_varchar2) then
    p_rec.attribute14 :=
    per_esa_shd.g_old_rec.attribute14;
  End If;
  If (p_rec.attribute15 = hr_api.g_varchar2) then
    p_rec.attribute15 :=
    per_esa_shd.g_old_rec.attribute15;
  End If;
  If (p_rec.attribute16 = hr_api.g_varchar2) then
    p_rec.attribute16 :=
    per_esa_shd.g_old_rec.attribute16;
  End If;
  If (p_rec.attribute17 = hr_api.g_varchar2) then
    p_rec.attribute17 :=
    per_esa_shd.g_old_rec.attribute17;
  End If;
  If (p_rec.attribute18 = hr_api.g_varchar2) then
    p_rec.attribute18 :=
    per_esa_shd.g_old_rec.attribute18;
  End If;
  If (p_rec.attribute19 = hr_api.g_varchar2) then
    p_rec.attribute19 :=
    per_esa_shd.g_old_rec.attribute19;
  End If;
  If (p_rec.attribute20 = hr_api.g_varchar2) then
    p_rec.attribute20 :=
    per_esa_shd.g_old_rec.attribute20;
  End If;
  ...
  End If;
  If (p_rec.address = hr_api.g_varchar2) then
    p_rec.address :=
    per_esa_shd.g_old_rec.address;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  per_esa_bus.update_validate(p_rec,p_effective_date);
  --
...
Procedure upd
  ...
  p_full_time                    in varchar2         default hr_api.g_varchar2,
...
Begin
  ...
  --
  l_rec :=
  per_esa_shd.convert_args
  (
  p_attendance_id,
  p_person_id,
  p_establishment_id,
  p_establishment,
  p_attended_start_date,
  p_attended_end_date,
  p_full_time,
