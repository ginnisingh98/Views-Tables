--------------------------------------------------------
--  DDL for Package Body AME_RLU_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_RLU_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_rlu_upd.';  -- Global package name
g_old_start_date date;
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  begin
    ...
    ame_rlu_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_usage_type_o
      => ame_rlu_shd.g_old_rec.usage_type
      ,p_start_date_o
      => ame_rlu_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_rlu_shd.g_old_rec.end_date
      ,p_security_group_id_o
      => ame_rlu_shd.g_old_rec.security_group_id
      ,p_priority_o
      => ame_rlu_shd.g_old_rec.priority
      ,p_approver_category_o
      => ame_rlu_shd.g_old_rec.approver_category
      ,p_object_version_number_o
      => ame_rlu_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.priority = hr_api.g_number) then
    ...
    ame_rlu_shd.g_old_rec.priority;
  End If;
  If (p_rec.approver_category = hr_api.g_varchar2) then
    p_rec.approver_category :=
    ame_rlu_shd.g_old_rec.approver_category;
  End If;
  If (p_rec.start_date = hr_api.g_date) then
     p_rec.start_date := ame_rlu_shd.g_old_rec.start_date;
  End If;
  If (p_rec.end_date = hr_api.g_date) then
    p_rec.end_date := ame_rlu_shd.g_old_rec.end_date;
  End If;
  --
End convert_defs;
--
...
Begin
 ...
 ,p_old_start_date => p_rec.old_start_date
  ...
  --
  ame_rlu_upd.convert_defs(p_rec);
  --
