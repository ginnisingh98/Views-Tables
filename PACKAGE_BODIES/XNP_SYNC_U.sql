--------------------------------------------------------
--  DDL for Package Body XNP_SYNC_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_SYNC_U" AS
/*$Header: XNPMSYNB.pls 120.2 2006/02/13 07:52:31 dputhiye ship $*/
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'SYNC' ) ;
  ...
  XNP_XML_UTILS.end_segment ( 'SYNC' ) ;
/* get the message body */
  XNP_XML_UTILS.get_document ( l_xml_body ) ;
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('SYNC',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'SYNC' ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'SYNC.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'SYNC', l_consumer_list );
  END IF;
