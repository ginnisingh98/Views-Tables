--------------------------------------------------------
--  DDL for Package Body BIX_PMV_AI_CACLASS_PRTLT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_PMV_AI_CACLASS_PRTLT_PKG" AS
/*$Header: bixiclap.plb 115.11 2003/12/25 00:39:52 anasubra noship $ */
...
l_other_class := FND_MESSAGE.GET_STRING('BIX','BIX_PMV_ALLACCT');

IF l_other_class IS NULL OR l_other_class = 'BIX_PMV_ALLACCT'
THEN
   l_other_class := 'Others';
END IF;
...
--commit;
   ...
   l_sqltext := '
            SELECT DECODE(greatest(RANKING,10),10,class,
		                 decode(class,:l_unknown,
                                    ''</a><nobr>''|| :l_unknown ||''</nobr><a href=#>'',
                                    ''</a><nobr>''|| :l_other_class ||'' </nobr><a href=#>''
                                 )
                         ) VIEWBY,
             ...
             --
            SELECT nvl(class,:l_unknown) class,
			    ...
			    decode(class,NULL,11,
                   RANK() OVER (ORDER BY nvl(sum(in_fresh_hand),0) DESC, class)
			          ) RANKING
            FROM
               (
             SELECT classification_value class,
...
l_sqltext := l_sqltext || l_call_where_clause ||
             '
		    GROUP BY mv.classification_value
              UNION ALL
              SELECT classification_value class,
...
l_sqltext := l_sqltext || l_call_where_clause ||
		    ...
		    GROUP BY mv.classification_value
             ...
             GROUP BY DECODE(greatest(RANKING,10),10,RANKING,decode(class,:l_unknown,11,12)),
			   DECODE(greatest(RANKING,10),10,class,
		                         decode(class,:l_unknown,
                                       ''</a><nobr>''|| :l_unknown ||''</nobr><a href=#>'',
                                       ''</a><nobr>''|| :l_other_class ||'' </nobr><a href=#>''
							      )
                                  )
	     ORDER BY DECODE(greatest(RANKING,10),10,RANKING,decode(class,:l_unknown,11,12)),
			DECODE(greatest(RANKING,10),10,class,
		                      decode(class,:l_unknown,
                                       ''</a><nobr>''|| :l_unknown ||''</nobr><a href=#>'',
                                       ''</a><nobr>''|| :l_other_class ||'' </nobr><a href=#>''
...
l_custom_rec.attribute_name := ':l_other_class' ;
l_custom_rec.attribute_value:= l_other_class;
l_custom_rec.attribute_type := BIS_PMV_PARAMETERS_PUB.BIND_TYPE;
l_custom_rec.attribute_data_type := BIS_PMV_PARAMETERS_PUB.VARCHAR2_BIND;
...
l_custom_rec.attribute_value := 'BIX_TELEPHONY+BIX_CALL_CLASSIFICATION';
