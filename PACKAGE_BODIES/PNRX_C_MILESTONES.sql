--------------------------------------------------------
--  DDL for Package Body PNRX_C_MILESTONES
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PNRX_C_MILESTONES" AS
/* $Header: PNRXCMSB.pls 115.2 2002/11/15 20:40:42 stripath ship $ */
...
IS
  ...
  BEGIN
    ...
    pnrx_milestones.pn_milestones(
          lease_number_low          =>argument1,
          lease_number_high         =>argument2,
          location_code_low         =>argument3,
          location_code_high        =>argument4,
          lease_termination_from    =>FND_DATE.CANONICAL_TO_DATE(argument5),
          lease_termination_to      =>FND_DATE.CANONICAL_TO_DATE(argument6),
          responsible_user          =>argument7,
          action_due_date_from      =>FND_DATE.CANONICAL_TO_DATE(argument8),
          action_due_date_to        =>FND_DATE.CANONICAL_TO_DATE(argument9),
          milestone_type            =>argument10,
