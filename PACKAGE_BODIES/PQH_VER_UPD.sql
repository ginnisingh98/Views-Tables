--------------------------------------------------------
--  DDL for Package Body PQH_VER_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_VER_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_ver_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy pqh_ver_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_de_wrkplc_vldtn_vers Row
  --
  update pqh_de_wrkplc_vldtn_vers
    set
     wrkplc_vldtn_ver_id             = p_rec.wrkplc_vldtn_ver_id
    ,wrkplc_vldtn_id                 = p_rec.wrkplc_vldtn_id
    ,version_number                  = p_rec.version_number
    ,business_group_id               = p_rec.business_group_id
    ,tariff_contract_code            = p_rec.tariff_contract_code
    ,tariff_group_code               = p_rec.tariff_group_code
    ,remuneration_job_description    = p_rec.remuneration_job_description
    ,job_group_id                    = p_rec.job_group_id
    ,remuneration_job_id             = p_rec.remuneration_job_id
    ,derived_grade_id                = p_rec.derived_grade_id
    ,derived_case_group_id           = p_rec.derived_case_group_id
    ,derived_subcasgrp_id            = p_rec.derived_subcasgrp_id
    ,user_enterable_grade_id         = p_rec.user_enterable_grade_id
    ,user_enterable_case_group_id    = p_rec.user_enterable_case_group_id
    ,user_enterable_subcasgrp_id     = p_rec.user_enterable_subcasgrp_id
    ,freeze                          = p_rec.freeze
...
Begin
  ...
  begin
    --
    pqh_ver_rku.after_update
      (p_effective_date              => p_effective_date
      ,p_wrkplc_vldtn_ver_id
      => p_rec.wrkplc_vldtn_ver_id
      ,p_wrkplc_vldtn_id
      => p_rec.wrkplc_vldtn_id
      ,p_version_number
      => p_rec.version_number
      ,p_business_group_id
      => p_rec.business_group_id
      ,p_tariff_contract_code
      => p_rec.tariff_contract_code
      ,p_tariff_group_code
      => p_rec.tariff_group_code
      ,p_remuneration_job_description
      => p_rec.remuneration_job_description
      ,p_job_group_id
      => p_rec.job_group_id
      ,p_remuneration_job_id
      => p_rec.remuneration_job_id
      ,p_derived_grade_id
      => p_rec.derived_grade_id
      ,p_derived_case_group_id
      => p_rec.derived_case_group_id
      ,p_derived_subcasgrp_id
      => p_rec.derived_subcasgrp_id
      ,p_user_enterable_grade_id
      => p_rec.user_enterable_grade_id
      ,p_user_enterable_case_group_id
      => p_rec.user_enterable_case_group_id
      ,p_user_enterable_subcasgrp_id
      => p_rec.user_enterable_subcasgrp_id
      ,p_freeze
      => p_rec.freeze
...
Begin
  ...
  --
  If (p_rec.wrkplc_vldtn_id = hr_api.g_number) then
    p_rec.wrkplc_vldtn_id :=
    pqh_ver_shd.g_old_rec.wrkplc_vldtn_id;
  End If;
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    pqh_ver_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.tariff_contract_code = hr_api.g_varchar2) then
    p_rec.tariff_contract_code :=
    pqh_ver_shd.g_old_rec.tariff_contract_code;
  End If;
  If (p_rec.tariff_group_code = hr_api.g_varchar2) then
    p_rec.tariff_group_code :=
    pqh_ver_shd.g_old_rec.tariff_group_code;
  End If;
  If (p_rec.remuneration_job_description = hr_api.g_varchar2) then
    p_rec.remuneration_job_description :=
    pqh_ver_shd.g_old_rec.remuneration_job_description;
  End If;
  If (p_rec.job_group_id = hr_api.g_number) then
    ...
    pqh_ver_shd.g_old_rec.job_group_id;
  End If;
  If (p_rec.remuneration_job_id = hr_api.g_number) then
    p_rec.remuneration_job_id :=
    pqh_ver_shd.g_old_rec.remuneration_job_id;
  End If;
  If (p_rec.derived_grade_id = hr_api.g_number) then
    p_rec.derived_grade_id :=
    pqh_ver_shd.g_old_rec.derived_grade_id;
  End If;
  If (p_rec.derived_case_group_id = hr_api.g_number) then
    p_rec.derived_case_group_id :=
    pqh_ver_shd.g_old_rec.derived_case_group_id;
  End If;
  If (p_rec.derived_subcasgrp_id = hr_api.g_number) then
    p_rec.derived_subcasgrp_id :=
    pqh_ver_shd.g_old_rec.derived_subcasgrp_id;
  End If;
  If (p_rec.user_enterable_grade_id = hr_api.g_number) then
    p_rec.user_enterable_grade_id :=
    pqh_ver_shd.g_old_rec.user_enterable_grade_id;
  End If;
  If (p_rec.user_enterable_case_group_id = hr_api.g_number) then
    p_rec.user_enterable_case_group_id :=
    pqh_ver_shd.g_old_rec.user_enterable_case_group_id;
  End If;
  If (p_rec.user_enterable_subcasgrp_id = hr_api.g_number) then
    p_rec.user_enterable_subcasgrp_id :=
    pqh_ver_shd.g_old_rec.user_enterable_subcasgrp_id;
  End If;
  If (p_rec.freeze = hr_api.g_varchar2) then
    p_rec.freeze :=
    pqh_ver_shd.g_old_rec.freeze;
  End If;
  --
...
Begin
  ...
  --
  pqh_ver_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pqh_ver_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pqh_ver_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
