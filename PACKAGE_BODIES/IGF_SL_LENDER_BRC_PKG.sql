--------------------------------------------------------
--  DDL for Package Body IGF_SL_LENDER_BRC_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_SL_LENDER_BRC_PKG" AS
/* $Header: IGFLI02B.pls 115.6 2003/09/10 05:07:51 veramach ship $ */

  l_rowid VARCHAR2(25);
  old_references igf_sl_lender_brc%ROWTYPE;
  new_references igf_sl_lender_brc%ROWTYPE;
  ...
  */
  BEGIN
    ...
    igf_sl_cl_recipient_pkg.get_fk_igf_sl_lender_brc (
      old_references.lender_id,
      old_references.lend_non_ed_brc_id
    );
  ...
  */
    CURSOR cur_rowid IS
      ...
      AND      lend_non_ed_brc_id = x_lend_non_ed_brc_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SL_LNDB_LND_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
