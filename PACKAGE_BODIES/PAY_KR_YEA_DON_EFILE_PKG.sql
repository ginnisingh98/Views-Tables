--------------------------------------------------------
--  DDL for Package Body PAY_KR_YEA_DON_EFILE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KR_YEA_DON_EFILE_PKG" as
/* $Header: pykrydef.pkb 120.1.12010000.2 2010/01/27 14:10:38 vaisriva ship $ */
--
-- Constants
--
c_package constant varchar2(31) := '  pay_kr_yea_don_efile_pkg.';
--
...
begin
        ...
        begin
       	     --
             g_payroll_action_id := to_number(pay_magtape_generic.get_parameter_value('PAYROLL_ACTION_ID') );
	     ...
	     g_assignment_set_id := to_number(pay_magtape_generic.get_parameter_value('ASSIGNMENT_SET_ID'));
             --
             g_donation_exem_archive := user_entity_id('X_YEA_DONATION_TAX_EXEM');
             --
			...
			if l_report_type       is null then
				g_normal_yea   := 'N';
				g_interim_yea  := 'I';
				g_rep_period_code := '1'; -- Bug 9251570
				...
				g_interim_yea  := 'I';
				g_rep_period_code := '3'; -- Bug 9251570
