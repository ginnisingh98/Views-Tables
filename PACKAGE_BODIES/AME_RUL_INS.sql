--------------------------------------------------------
--  DDL for Package Body AME_RUL_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_RUL_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_rul_ins.';  -- Global package name
--
...
--
  function getNextRuleKey return varchar2 as
    ...
    l_proc              varchar2(72)  :=  g_package||'getNextRuleKey';
    begin
      begin
      ...
      loop
        ...
        select count(*)
          into ruleCount
          from ame_rules
          where upper(rule_key) = upper(newRuleKey1) and
                rownum < 2;
        if ruleCount = 0 then
          exit;
        end if;
...
Begin
  ...
  --
  ame_rul_ins.g_rule_id_i := p_rule_id;
  --
...
Procedure dt_insert_dml
  (p_rec                     in out nocopy ame_rul_shd.g_rec_type
...
--
  Cursor C_Sel1 Is
    ...
    and    t.start_date =
             ame_rul_shd.g_old_rec.start_date
...
--
  ...
  l_created_by          ame_rules.created_by%TYPE;
  l_creation_date       ame_rules.creation_date%TYPE;
  l_last_update_date    ame_rules.last_update_date%TYPE;
  l_last_updated_by     ame_rules.last_updated_by%TYPE;
  l_last_update_login   ame_rules.last_update_login%TYPE;
...
Begin
  ...
  p_rec.object_version_number :=
   ame_rul_shd.get_object_version_number
...
--
  Cursor C_Sel1 is select ame_rules_s.nextval from sys.dual;
--
 Cursor C_Sel2 is
    Select null
      from ame_rules
     where rule_id =
             ame_rul_ins.g_rule_id_i
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
    If (ame_rul_ins.g_rule_id_i is not null) Then
    --
    ...
    If C_Sel2%found Then
       ...
       fnd_message.set_token('TABLE_NAME','ame_rules');
       fnd_message.raise_error;
    End If;
    Close C_Sel2;
    --
  ...
  Else
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.rule_id;
    Close C_Sel1;
  End If;
  --
  -- rule_key
  --   If rule_key field is null, a default value is initialized.
  --
  if p_rec.rule_key is null then
    p_rec.rule_key := getNextRuleKey;
  end if;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
...
Begin
  ...
  if p_rec.start_date is null then
    p_rec.start_date := p_effective_date;
  end if;
  if p_rec.end_date is null then
    p_rec.end_date := ame_utility_pkg.endOfTime;
  end if;

  --
  -- Call the supporting insert validate operations
  --
  ame_rul_bus.insert_validate
    (p_rec                   => p_rec
    ,p_effective_date        => p_effective_date
    ,p_datetrack_mode        => l_datetrack_mode
    ,p_validation_start_date => l_validation_start_date
...
Begin
  ...
  --
  -- Having converted the arguments into the ame_rul_rec
