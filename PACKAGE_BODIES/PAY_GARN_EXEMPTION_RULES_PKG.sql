--------------------------------------------------------
--  DDL for Package Body PAY_GARN_EXEMPTION_RULES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_GARN_EXEMPTION_RULES_PKG" as
/* $Header: pyger01t.pkb 115.0 99/07/17 06:08:28 porting ship $ */
--
  PROCEDURE pre_insert (x_exemption_rule_id     IN OUT NUMBER)
   IS
      CURSOR C2 IS SELECT pay_us_garn_exemption_rules_s.nextval FROM sys.dual;
  BEGIN
      if (X_Exemption_Rule_Id is NULL) then
        OPEN C2;
        FETCH C2 INTO X_Exemption_Rule_Id;
        CLOSE C2;
      end if;
  END pre_insert;
  ...
  BEGIN
    ...
    IF x_garn_category IS NOT NULL THEN
      ...
      END IF;
    END IF;
    IF x_calc_rule IS NOT NULL THEN
      l_lookup_type := 'US_GARN_CHK_CALRULE';
      l_lookup_code := x_calc_rule;
      OPEN lookup_c;
      FETCH lookup_c INTO l_meaning;
      IF lookup_c%NOTFOUND THEN
        CLOSE lookup_c;
        RAISE no_data_found;
      ELSE
        x_d_calc_rule := l_meaning;
        CLOSE lookup_c;
      END IF;
    END IF;
    IF x_dependents_calc_rule IS NOT NULL THEN
      l_lookup_type := 'US_GARN_DEP_CALRULE';
      l_lookup_code := x_dependents_calc_rule;
      OPEN lookup_c;
      FETCH lookup_c INTO l_meaning;
      IF lookup_c%NOTFOUND THEN
        CLOSE lookup_c;
        RAISE no_data_found;
      ELSE
        x_d_dependents_calc_rule := l_meaning;
        CLOSE lookup_c;
      END IF;
    END IF;
