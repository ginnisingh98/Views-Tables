--------------------------------------------------------
--  DDL for Package Body JTF_PF_CONV_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_PF_CONV_PKG" AS
/* $Header: jtfpfconvpkgb.pls 120.14.12010000.2 2010/02/10 19:33:44 rlandows ship $ */
/*===========================================================================+
 ...
 |___________________________________________________________________________|*/
  ...
  PROCEDURE writePFObjectTable(objTab JTF_PF_PAGE_OBJECT_TABLE, write_to_jtf INTEGER, tech_stack_tbl JTF_PF_TECHSTACK_TABLE, track_prps_tbl JTF_PF_TRACKPURPOSE_TABLE, dbdate OUT NOCOPY DATE) IS
	PRAGMA AUTONOMOUS_TRANSACTION;
        enqueue_options     dbms_aq.enqueue_options_t;
        message_properties  dbms_aq.message_properties_t;
        message_handle      RAW(16);
        objTabLocal             JTF_PF_PAGE_OBJECT_TABLE;
	obj	                     JTF_PF_PAGE_OBJECT;
	dummyPageObj         JTF_PF_PAGE_OBJECT;
	seq_num                  INTEGER;

   BEGIN
   objTabLocal             := JTF_PF_PAGE_OBJECT_TABLE();
   FOR i IN 1..objTab.COUNT
    LOOP
      obj := objTab(i);
      objTabLocal.EXTEND;
      SELECT JTF_PF_SEQ.NEXTVAL INTO seq_num FROM dual;
      objTabLocal(i) := JTF_PF_PAGE_OBJECT(JTF_PF_INFO_OBJECT(seq_num,obj.info.Day,obj.info.Timestamp),obj.dpf,obj.ses,obj.jsp,
                                          obj.loc,obj.lang,obj.client,obj.params,
                                          obj.cookies,obj.headers, obj.prodinfo);
    END LOOP;
  SELECT sysdate INTO dbdate FROM dual;
  IF (write_to_jtf = 1) THEN
            FORALL indx IN objTabLocal.FIRST .. objTabLocal.LAST
            INSERT INTO jtf_pf_repository(
			pageobject, object_version_number,
			created_by, creation_date,
			last_updated_by, last_update_date, tech_stack, track_purpose)
		VALUES (objTabLocal(indx), 0, 690, sysdate, 690, sysdate, tech_stack_tbl(indx), track_prps_tbl(indx));
  ...
  END writePFObjectTable;
   ...
   ============================================================================================================*/
   PROCEDURE writePFObject(obj JTF_PF_PAGE_OBJECT,write_to_jtf INTEGER,tech_stack VARCHAR2, track_purpose VARCHAR2, dbdate OUT NOCOPY DATE) IS
	PRAGMA AUTONOMOUS_TRANSACTION;
   ...
   BEGIN
       ...
       IF (write_to_jtf = 1) THEN
           INSERT INTO jtf_pf_repository(
			...
			created_by, creation_date,
			last_updated_by, last_update_date, tech_stack, track_purpose)
		VALUES (dummyPageObj, 0, 690, obj.info.Day, 690, obj.info.Timestamp, tech_stack, track_purpose);
       ELSE
           message_properties.correlation := seq_num;
           message_properties.expiration := 24*60*60; -- messages expire in one day after first dequeue
           dbms_aq.enqueue(queue_name => qname,
  ...
  /* ============================================================================================================
  |  Converts THE param names AND param VALUES, coming IN AS 2 seperate arrays, FROM THE JAVA layer,         |
  |  INTO JTF_PF_PRODPARAMS_TABLE                                                                                |
  |  This IS used TO construct THE JTF_PF_PRODINFO_OBJECT                                                        |
  ===============================================================================================================*/
  FUNCTION GetProdParams (paramNames JTF_VARCHAR2_TABLE_300, paramValues JTF_VARCHAR2_TABLE_4000, paramSz INTEGER)
  RETURN JTF_PF_PRODPARAMS_TABLE IS
	params JTF_PF_PRODPARAMS_TABLE := JTF_PF_PRODPARAMS_TABLE();
  BEGIN
	FOR i IN 1..paramSz LOOP
	    params.EXTEND;
	    params(i) := JTF_PF_NVPAIR(paramNames(i),paramValues(i));
  ...
  =============================================================================================================*/
   ...
   END ;
	...
	END IF;
		-- ADDED to fix bug no-5036611 (START HERE)
		IF 	   (last_migrate_day  is null) THEN
        ...
        INSERT INTO JTF_PF_SESSION_SUMM (day, seqid, sessionid, user_name, userid, sum_exect_jtf, sum_exect_oaf, sum_exect_form, sum_thinkt_jtf, sum_thinkt_oaf, sum_thinkt_form,
                ...
                (SELECT  day, seqid, sessionid, user_name, userid, sum_exect_jtf, sum_exect_oaf, sum_exect_form, sum_thinkt_jtf, sum_thinkt_oaf, sum_thinkt_form,
                 ...
                 WHERE (sessionid IS NULL )
                        or (sessionid IS NOT NULL and sessionid IN
                                (select distinct sessionid from JTF_PF_SES_ACTIVITY))
                );
        ...
        INSERT INTO JTF_PF_SESSION_SUMM (day, seqid, sessionid, user_name, userid, sum_exect_jtf, sum_exect_oaf, sum_exect_form, sum_thinkt_jtf, sum_thinkt_oaf, sum_thinkt_form,
                ...
                (SELECT  day, seqid, sessionid, user_name, userid, sum_exect_jtf, sum_exect_oaf, sum_exect_form, sum_thinkt_jtf, sum_thinkt_oaf, sum_thinkt_form,
                 ...
                 WHERE (sessionid IS NULL and day >= last_migrate_day)
                        or (sessionid IS NOT NULL and sessionid IN
                                (select distinct sessionid from JTF_PF_SES_ACTIVITY where day >= last_migrate_day ))
                );
        INSERT INTO JTF_PF_USER_SUMM (day, user_name, userid, sum_exect_jtf, sum_exect_oaf, sum_exect_form, sum_thinkt_jtf, sum_thinkt_oaf, sum_thinkt_form,
                        cnt_pages_jtf, cnt_pages_oaf, cnt_pages_form, cnt_pages_all, ucnt_ses, ucnt_pages, ucnt_apps, ucnt_resps, ucnt_langs, startt, endt, cnt_fail)
                (SELECT day, user_name, userid, sum_exect_jtf, sum_exect_oaf, sum_exect_form, sum_thinkt_jtf, sum_thinkt_oaf, sum_thinkt_form,
	   ...
	   END IF;
	   -- ADDED To fix bug no-5036611 (END HERE)
  ...
  PROCEDURE CLEAN_DATA(START_DATE DATE) IS
    ...
    CURSOR EBIZ_DATA IS
      SELECT X.PAGEOBJECT.INFO.RECID FROM JTF_PF_REPOSITORY X WHERE
	       X.TRACK_PURPOSE IN (0,1) AND
	        X.LAST_UPDATE_DATE <= LEAST(START_DATE ,(SELECT LAST_RECORD_MIGRATED_TIME FROM JTF_PF_PURGEABLE WHERE TRACK_PURPOSE IN (0,1)));

    CURSOR NON_EBIZ_DATA IS
      SELECT X.PAGEOBJECT.INFO.RECID FROM JTF_PF_REPOSITORY X WHERE
	       X.TRACK_PURPOSE = 4 AND
	       X.LAST_UPDATE_DATE <= LEAST(START_DATE , (SELECT LAST_RECORD_MIGRATED_TIME FROM JTF_PF_PURGEABLE WHERE TRACK_PURPOSE = 4));

    CURSOR WA_EBIZ_DATA IS
      SELECT X.PAGEOBJECT.INFO.RECID FROM JTF_PF_REPOSITORY X WHERE
	       X.TRACK_PURPOSE = 2 AND
	      X.LAST_UPDATE_DATE <= LEAST( START_DATE, (SELECT LAST_RECORD_MIGRATED_TIME FROM JTF_PF_PURGEABLE WHERE TRACK_PURPOSE = 2));

    CURSOR COMMON_DATA IS
      SELECT X.PAGEOBJECT.INFO.RECID FROM JTF_PF_REPOSITORY X WHERE
	       X.TRACK_PURPOSE = 3 AND
	      X.LAST_UPDATE_DATE <= LEAST (START_DATE ,(SELECT MIN(LAST_RECORD_MIGRATED_TIME) FROM JTF_PF_PURGEABLE WHERE TRACK_PURPOSE IN(0,1,2)));
  ...
  BEGIN
    ...
    CLOSE EBIZ_DATA;
     ...
     OPEN NON_EBIZ_DATA;
     LOOP
        FETCH NON_EBIZ_DATA BULK COLLECT INTO RECTABTYPE1 LIMIT 1000;
	     IF RECTABTYPE1.COUNT > 0  THEN
     FORALL I IN RECTABTYPE1.FIRST .. RECTABTYPE1.LAST
          DELETE FROM JTF_PF_REPOSITORY X WHERE X.PAGEOBJECT.INFO.RECID = RECTABTYPE1(I);
    END IF;

       EXIT WHEN NON_EBIZ_DATA%NOTFOUND;
     END LOOP;
    RECTABTYPE1.DELETE;
	CLOSE NON_EBIZ_DATA;
     ...
     OPEN WA_EBIZ_DATA;
     LOOP
        FETCH WA_EBIZ_DATA BULK COLLECT INTO RECTABTYPE1 LIMIT 1000;
	     IF RECTABTYPE1.COUNT > 0  THEN
     FORALL I IN RECTABTYPE1.FIRST .. RECTABTYPE1.LAST
          DELETE FROM JTF_PF_REPOSITORY X WHERE X.PAGEOBJECT.INFO.RECID = RECTABTYPE1(I);
    END IF;

       EXIT WHEN WA_EBIZ_DATA%NOTFOUND;
     END LOOP;
    RECTABTYPE1.DELETE;
	CLOSE WA_EBIZ_DATA;
     ...
     OPEN COMMON_DATA;
     LOOP
        FETCH COMMON_DATA BULK COLLECT INTO RECTABTYPE1 LIMIT 1000;
	     IF RECTABTYPE1.COUNT > 0  THEN
     FORALL I IN RECTABTYPE1.FIRST .. RECTABTYPE1.LAST
          DELETE FROM JTF_PF_REPOSITORY X WHERE X.PAGEOBJECT.INFO.RECID = RECTABTYPE1(I);
    END IF;
       EXIT WHEN COMMON_DATA%NOTFOUND;
     END LOOP;
  ...
  PROCEDURE PURGE_DATA(ERRBUF OUT NOCOPY VARCHAR2, RETCODE OUT NOCOPY NUMBER, start_date_v IN varchar2) IS
	PRAGMA AUTONOMOUS_TRANSACTION;
        start_date date;
  BEGIN
  ...
  BEGIN
    ...
    END IF;
  END;
  PROCEDURE MIGRATED_DATA(TRK_PRPS NUMBER, LST_RCD_MGRTD_TM DATE) IS
    TP NUMBER := -1;
  BEGIN
    SELECT COUNT(TRACK_PURPOSE) INTO TP FROM JTF_PF_PURGEABLE WHERE TRACK_PURPOSE = TRK_PRPS;
    IF (TP > 0) THEN
      UPDATE JTF_PF_PURGEABLE SET LAST_RECORD_MIGRATED_TIME = LST_RCD_MGRTD_TM WHERE TRACK_PURPOSE = TRK_PRPS;
    ELSE
      INSERT INTO JTF_PF_PURGEABLE(TRACK_PURPOSE, LAST_RECORD_MIGRATED_TIME) VALUES(TRK_PRPS, LST_RCD_MGRTD_TM);
    END IF;
  END MIGRATED_DATA;
END;
