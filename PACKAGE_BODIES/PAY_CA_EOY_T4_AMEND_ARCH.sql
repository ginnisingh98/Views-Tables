--------------------------------------------------------
--  DDL for Package Body PAY_CA_EOY_T4_AMEND_ARCH
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_CA_EOY_T4_AMEND_ARCH" AS
/* $Header: pycat4ca.pkb 120.1 2006/10/11 20:39:43 meshah noship $ */
/*
   ******************************************************************
   *                                                                *
   ...
   ******************************************************************
    ...
    Name        : pay_ca_eoy_t4_amend_arch

    Description : This procedure is used by T4 Amendment Pre-Process
                  to archive data for T4 Amendment Paper Report.

    Change List
    -----------
    Date        Name       Vers    Bug No   Description
    ----------- ---------- ------  -------  --------------------------
    05-AUG-2003 SSattini   115.0            Created.
    16-SEP-2003 SSattini   115.1   3144801  Corrected get_eoy_info cursor
                                            to avoid numeric or value error,
                                            corrected the range_cursor sql
                                            string for Asg_set and small
                                            local variable conversions for
                                            hr_utility messages.
    17-SEP-2003 SSattini   115.2   3144801  Corrected variables used
                                            to substitute tokens for messages.
                                            This was causing the numeric/value
                                            error.
    17-NOV-2003 SSouresr   115.3            Modified range_cursor for situation
                                            where no person or assignment set
                                            has been specified.
    01-SEP-2004 SSouresr   115.4            Cleaned up the package by taking out
                                            some unnecessary code and a
                                            redundant join on
                                            per_all_assignments_f.
    01-SEP-2004 SSattini   115.5            Fixed the GSCC errors.
    15-NOV-2004 SSouresr   115.6            Replaced per_people_f with per_all_people_f
  ...
  *****************************************************************************/
   ...
   gv_package        VARCHAR2(100) := 'pay_ca_eoy_t4_amend_arch';
  ...
  ************************************************************/
  ...
  IS
    ...
    ln_t4amend_asg_action   NUMBER := 0;
    lv_year                 VARCHAR2(4);
   ...
   IS
     ...
     CURSOR get_prev_t4_amend_dtls (cp_person_id      in number
   ...
   BEGIN
      ...
      /* Actions will be created if
         ...
         #1 there is a YEPP action not followed by a
            T4 Amendment Archiver action

         #2 there is already a Previous T4 Amendment Archiver
            action for this person
      */

        open get_prev_t4_amend_dtls(p_person_id
                              ,ln_tax_unit_id
                              ,ld_end_date);
        fetch get_prev_t4_amend_dtls into  lv_report_type
        ...
        if get_prev_t4_amend_dtls%notfound then

           hr_utility.trace('get_prev_t4_amend_dtls Not Found');
           ...
           lv_record_name := 'T4 Amendment Pre-Process';
           lv_message := 'Fed YE Preprocess was not run for this employee';
        ...
        if get_prev_t4_amend_dtls%found then
           hr_utility.trace('get_prev_t4_amend_dtls Found');

           if lv_report_type in ('T4', 'CAEOY_T4_AMEND_PP') then

               /* Create an assignment action for this person */
               select pay_assignment_actions_s.nextval
                 into ln_t4amend_asg_action
                 from dual;
               hr_utility.trace('New T4 Amendment Action = ' ||
                                   to_char(ln_t4amend_asg_action));

               /* Insert into pay_assignment_actions. */
               hr_utility.trace('creating asg_action');
               hr_nonrun_asact.insact(ln_t4amend_asg_action
                                     ,ln_primary_assignment_id
                                     ,p_payroll_action_id
                                     ,p_chunk
                                     ,ln_tax_unit_id);
               ...
               /* Interlock the YEPP or Previous T4 Amendment Archiver
                   action for current T4 Amendment Pre-process action */

               hr_utility.trace('Locking Action = ' || to_char(ln_t4amend_asg_action));
               hr_utility.trace('Locked Action = '  || to_char(ln_asg_act_to_lock));
               hr_nonrun_asact.insint(ln_t4amend_asg_action
                                     ,ln_asg_act_to_lock);
        ...
        close get_prev_t4_amend_dtls;
        hr_utility.trace('closed get_prev_t4_amend_dtls');
   END action_creation;
...
Begin
--hr_utility.trace_on(null,'T4AMENDPP');
null;



end pay_ca_eoy_t4_amend_arch;
