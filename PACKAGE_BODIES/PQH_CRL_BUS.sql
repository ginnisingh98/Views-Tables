--------------------------------------------------------
--  DDL for Package Body PQH_CRL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_CRL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_crl_bus.';  -- Global package name
--
...
Function return_legislation_code
  ...
  -- pqh_criteria_rate_defn_tl and PER_BUSINESS_GROUPS_PERF
...
Begin
  ...
  --
  --
  if (( nvl(pqh_crl_bus.g_criteria_rate_defn_id, hr_api.g_number)
       = p_criteria_rate_defn_id)
  and ( nvl(pqh_crl_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pqh_crl_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pqh_crl_bus.g_criteria_rate_defn_id       := p_criteria_rate_defn_id;
    pqh_crl_bus.g_language                    := p_language;
    pqh_crl_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pqh_crl_shd.api_updating
