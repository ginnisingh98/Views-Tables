--------------------------------------------------------
--  DDL for Package Body AME_APT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_APT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := 'ame_apt_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_APPROVER_TYPES_PK') Then
...
Begin
  ...
  Else
    If (p_approver_type_id =
        ame_apt_shd.g_old_rec.approver_type_id and
        p_object_version_number =
        ame_apt_shd.g_old_rec.object_version_number)
    Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_apt_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ame_apt_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_apt_shd.g_old_rec.end_date;
