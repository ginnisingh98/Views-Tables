--------------------------------------------------------
--  DDL for Package Body PER_PSO_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PSO_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_pso_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_security_organization_id    number         default null;
--
--  ---------------------------------------------------------------------------
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'SECURITY_ORGANIZATION_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(per_pso_bus.g_security_organization_id, hr_api.g_number)
       = p_security_organization_id) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := per_pso_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    per_pso_bus.g_security_organization_id    := p_security_organization_id;
    per_pso_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  if csr_organization%notfound then
     fnd_message.set_name('PER', 'HR_289002_INV_ORG_ID');
     fnd_message.raise_error;
  END IF;
...
open csr_unique;
  ...
  if csr_unique%found then
     fnd_message.set_name('PER', 'HR_289773_MULTI_ORG_DUPLICATE');
     fnd_message.raise_error;
  END IF;
...
exception
  when app_exception.application_exception then
    if hr_multi_message.exception_add
         (p_associated_column1      => per_pso_shd.g_tab_nam||'.ORGANIZATION_ID'
         ) then
      hr_utility.set_location(' Leaving:'|| l_proc, 40);
      raise;
    end if;
...
Begin
  --
  open csr_profile;
  fetch csr_profile into l_dummy;
  if csr_profile%notfound then
     fnd_message.set_name('PER', 'HR_289774_INVALID_SEC_PROFILE');
     fnd_message.raise_error;
  END IF;
...
Begin
  ...
  --
  IF NOT per_pso_shd.api_updating
      (p_security_organization_id          => p_rec.security_organization_id
...
Begin
  ...
  --
  chk_security_profile_id(p_rec);
  chk_organization_id(p_rec);
