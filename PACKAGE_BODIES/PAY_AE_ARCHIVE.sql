--------------------------------------------------------
--  DDL for Package Body PAY_AE_ARCHIVE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_AE_ARCHIVE" AS
			/* $Header: pyaeparc.pkb 120.6.12010000.4 2009/12/10 10:13:59 bkeshary ship $ */
			...
			g_package           VARCHAR2(33) := '  PAY_AE_ARCHIVE.';
			...
			SELECT PAY_AE_ARCHIVE.GET_PARAMETER(legislative_parameters,'PAYROLL_ID')
			      ,PAY_AE_ARCHIVE.GET_PARAMETER(legislative_parameters,'CONSOLIDATION_SET_ID')
			      ,PAY_AE_ARCHIVE.GET_PARAMETER(legislative_parameters,'START_DATE')
			      ,PAY_AE_ARCHIVE.GET_PARAMETER(legislative_parameters,'END_DATE')
			      ,effective_date
			      ,business_group_id
			...
			PAY_AE_ARCHIVE.GET_ALL_PARAMETERS(p_payroll_action_id
					,l_business_group_id
					,l_start_date
					,l_end_date
					,l_effective_date
			...
			AND    NOT EXISTS (SELECT /* + ORDERED */ NULL
					   ...
					   AND    appa3.report_type        = 'AE_ARCHIVE')
			AND  NOT EXISTS (  SELECT /* + ORDERED */ NULL
					   FROM   pay_action_interlocks      pai1,
						     pay_assignment_actions     act2,
						     pay_payroll_actions        appa3
			...
			SELECT ppf.person_id person_id,
			       ...
			       ppf.per_information18 nationality,
			       ...
			       ppf.per_information4 alt_first,
			       ppf.per_information5 alt_father,
			       ppf.per_information6 alt_gfather,
			       ppf.per_information7 alt_last,
			       ppf.per_information8 mother_name,
			       ppf.title title,
			...
			FROM   hr_organization_information
			WHERE  organization_id = p_bg_id
			AND    org_information_context = 'AE_HR_BG_INFO';
			...
			l_nationality  VARCHAR2(80); --per_all_people_f.nationality%TYPE;
			 ...
			 /*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS 2');*/
			  ...
			  /*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS 3');*/
				...
				FETCH csr_cost_center INTO l_cost_center;
			       ...
			       /*NATIONALITY*/
			       l_nationality := hr_general.decode_lookup('AE_NATIONALITY',rec_person_details.nationality);
				...
				/* COUNTRY */
				l_country:=PAY_AE_ARCHIVE.get_country_name(rec_primary_address.country);

				l_emp_country:=PAY_AE_ARCHIVE.get_country_name(rec_employer_address.country);

				l_org_country:=PAY_AE_ARCHIVE.get_country_name(rec_org_address.country);

				/*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS 6');*/
				/* Total Earnings YTD */
				/*l_defined_balance_id := PAY_AE_ARCHIVE.GET_DEFINED_BALANCE_ID('TOTAL_EARNINGS_ASG_YTD');
				...
				/*l_defined_balance_id_ded := PAY_AE_ARCHIVE.GET_DEFINED_BALANCE_ID('TOTAL_DEDUCTIONS_ASG_YTD');
				...
				pay_action_information_api.create_action_information (
				 ...
				 l_full_name := rec_person_details.alt_first || ' ' ||  rec_person_details.alt_father || ' ' ||
				                rec_person_details.alt_gfather || ' ' || rec_person_details.alt_last;
				 ...
				 pay_action_information_api.create_action_information (
					 ...
					 ,p_action_information6          => substr(l_full_name,1,60)
					 ,p_action_information7          => l_cost_center
			...
			AND ((et.business_group_id IS NULL AND et.legislation_code = 'AE')
			...
			SELECT rrv.result_value
			              ,rr.run_result_id /*Added for loan*/
		        ...
		        BEGIN
                          ...
                          LOOP
                            ...
                            IF  l_result_value is not null THEN
                              ...
                              ,p_action_information4          => l_result_value --l_formatted_value /* Bug 8201719 */
                              ,p_action_information9          => 'Earnings Element'
                              ,p_assignment_id                => p_assignment_id);
                            END IF;
                          END LOOP;
			...
			l_run_result_id := NULL;
		        ...
		        BEGIN
                          ...
                          LOOP
                            FETCH csr_result_value INTO rec_result_value;
                            EXIT WHEN csr_result_value%NOTFOUND;
                            l_run_result_id := rec_result_value.run_result_id;
                            l_result_value := rec_result_value.result_value;
                            IF  l_result_value is not null THEN
                              DECLARE
                                l_loan_type       VARCHAR2(80) := NULL;
                                l_count_loan  NUMBER := 0;
                              BEGIN
                                SELECT count(*)
                                INTO l_count_loan
                                FROM pay_element_types_f_tl
                                WHERE element_type_id = csr_rec.element_type_id
                                AND element_name = 'Loan Recovery'
                                AND language = 'US';
                                IF l_count_loan = 1 THEN
                                   l_loan_type := pay_ae_soe.ae_loan_type(p_assignment_action_id,l_run_result_id,p_effective_date);
                                END IF;
                                   pay_action_information_api.create_action_information (
                                    p_action_information_id        => l_action_info_id
                                   ,p_action_context_id            => p_archive_assact_id
                                   ...
                                   ,p_action_information4          => l_result_value --l_formatted_value /* Bug 8201719 */
                                   /*Added for showing loan input value*/
                                   ,p_action_information8          => l_loan_type
			...
			/* Cursor to fetch id flex num for ae_bank_details */
			CURSOR csr_get_flexnum IS
			SELECT id_flex_num
			FROM FND_ID_FLEX_STRUCTURES_VL
			WHERE (ID_FLEX_STRUCTURE_CODE = 'AE_BANK_DETAILS')
			...
			l_bank_name fnd_lookup_values.meaning%type;
			l_branch_name fnd_lookup_values.meaning%type;
			l_action_info_id NUMBER;
			l_ovn NUMBER;
			l_org NUMBER;
			...
			EXIT WHEN csr_chk%NOTFOUND;
				...
				l_bank_name := hr_general.decode_lookup('AE_BANK_NAMES',rec_pers_pay.name_id);
				l_branch_name := hr_general.decode_lookup('AE_BRANCH_NAMES',rec_pers_pay.branch);
				...
				pay_action_information_api.create_action_information (
					 ...
					 ,p_action_information6          => l_branch_name
					 ,p_action_information7          => NULL
					 ,p_action_information8          => rec_pers_pay.acct_num
					 ,p_action_information9          => 'PAYMENT' -- For PDF Payslip Grouping
			...
			l_uom fnd_lookup_values.meaning%type := NULL;
			l_action_info_id NUMBER;
			l_ovn NUMBER;
			l_index NUMBER;
			l_tu_id NUMBER;
			...
			---------------------------------------------------
			  ...
			  LOOP
				...
				PAY_AE_ARCHIVE.SETUP_BALANCE_DEFINITIONS(p_balance_name         => rec_get_balance.narrative
