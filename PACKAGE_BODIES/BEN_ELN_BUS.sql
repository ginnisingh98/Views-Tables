--------------------------------------------------------
--  DDL for Package Body BEN_ELN_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ELN_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_eln_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_lgl_enty_prte_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_lgl_enty_prte_id PK of record being inserted or updated.
...
--
Procedure chk_elig_lgl_enty_prte_id(p_elig_lgl_enty_prte_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'elig_lgl_enty_prte_id';
...
Begin
  ...
  --
  l_api_updating := ben_eln_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_lgl_enty_prte_id                => p_elig_lgl_enty_prte_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_lgl_enty_prte_id,hr_api.g_number)
     <>  ben_eln_shd.g_old_rec.elig_lgl_enty_prte_id) then
    --
    -- raise error as PK has changed
    --
    ben_eln_shd.constraint_error('BEN_ELIG_LGL_PRTE_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_lgl_enty_prte_id is not null then
      --
...
--   in the referenced table and unique for Eligy Profile
--
-- Pre-Conditions
--   None.
--
-- In Parameters
--   p_elig_lgl_enty_prte_id PK
...
--   Internal table handler use only.
--
Procedure chk_organization_id (p_elig_lgl_enty_prte_id in number,
     ...
     select null
       from ben_elig_lgl_enty_prte_f
         where organization_id = p_organization_id
           ...
           and elig_lgl_enty_prte_id <> nvl(p_elig_lgl_enty_prte_id,hr_api.g_number)
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_eln_shd.g_old_rec.organization_id,hr_api.g_number)
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_eln_shd.constraint_error('BEN_ELIG_LGL_PRTE_FK1');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_lgl_enty_prte_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_eln_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
--
-- In Parameters
--   p_elig_lgl_enty_prte_id     elig_lgl_enty_prte_id
...
is
  ...
  cursor c1 is
    select null
    from   ben_elig_lgl_enty_prte_f
    where  elig_lgl_enty_prte_id <> nvl(p_elig_lgl_enty_prte_id,-1)
    and    eligy_prfl_id = p_eligy_prfl_id
...
function return_legislation_code
  (p_elig_lgl_enty_prte_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_lgl_enty_prte_f b
    where b.elig_lgl_enty_prte_id      = p_elig_lgl_enty_prte_id
