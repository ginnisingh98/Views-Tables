--------------------------------------------------------
--  DDL for Package Body PAY_SGB_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_SGB_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_sgb_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the pay_shadow_gu_bal_exclusions Row
  --
...
Begin
  ...
  If (p_rec.source_id = hr_api.g_number) then
    p_rec.source_id :=
    pay_sgb_shd.g_old_rec.source_id;
  End If;
  If (p_rec.source_type = hr_api.g_varchar2) then
    p_rec.source_type :=
    pay_sgb_shd.g_old_rec.source_type;
  End If;
  If (p_rec.balance_type_name = hr_api.g_varchar2) then
    p_rec.balance_type_name :=
    pay_sgb_shd.g_old_rec.balance_type_name;
  End If;
  If (p_rec.balance_type_id = hr_api.g_number) then
    p_rec.balance_type_id :=
    pay_sgb_shd.g_old_rec.balance_type_id;
  End If;
  If (p_rec.exclusion_rule_id = hr_api.g_number) then
    p_rec.exclusion_rule_id :=
    pay_sgb_shd.g_old_rec.exclusion_rule_id;
  End If;
  --
End convert_defs;
--
...
Procedure upd
  ...
  ,p_balance_type_name            in     varchar2  default hr_api.g_varchar2
