--------------------------------------------------------
--  DDL for Package Body HXC_ULP_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_ULP_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_ulp_ins.';  -- Global package name
...
Begin
  ...
  --
  --
  -- Insert the row into: hxc_layout_comp_prompts
  --
  insert into hxc_layout_comp_prompts
...
--
  l_proc  varchar2(72) ;
--
  Cursor C_Sel1 is select hxc_layout_comp_prompts_s.nextval from sys.dual;
--
Begin
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.layout_comp_prompt_id;
  Close C_Sel1;
  --
  if g_debug then
  	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_ulp_bus.insert_validate
     (p_rec
     );
  --
  -- Call the supporting pre-insert operation
  --
  hxc_ulp_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_ulp_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_ulp_ins.post_insert
     (p_rec
     );
  --
  if g_debug then
...
Procedure ins
  ...
  ,p_layout_comp_prompt_id             out nocopy number
...
Begin
  ...
  --
  -- Having converted the arguments into the hxc_ulp_rec
  ...
  --
  p_layout_comp_prompt_id := l_rec.layout_comp_prompt_id;
