--------------------------------------------------------
--  DDL for Package Body HR_PER_DEPLYMT_CONTACT_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_PER_DEPLYMT_CONTACT_SWI" As
/* $Header: hrpdcswi.pkb 120.0 2005/09/23 06:46 adhunter noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'hr_per_deplymt_contact_swi.';
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_per_deplymt_contact_swi;
  --
  ...
  --
  hr_pdc_ins.set_base_key_value
    (p_person_deplymt_contact_id => p_per_deplymt_contact_id
    );
  --
  -- Call API
  --
  hr_per_deplymt_contact_api.create_per_deplymt_contact
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_per_deplymt_contact_swi;
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_per_deplymt_contact_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  hr_per_deplymt_contact_api.delete_per_deplymt_contact
    (p_validate                     => l_validate
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_per_deplymt_contact_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_return_status := hr_multi_message.get_return_status_disable;
