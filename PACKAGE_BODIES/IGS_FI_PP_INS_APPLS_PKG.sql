--------------------------------------------------------
--  DDL for Package Body IGS_FI_PP_INS_APPLS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_FI_PP_INS_APPLS_PKG" AS
/* $Header: IGSSIE2B.pls 115.1 2003/09/16 12:25:32 smvk noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_fi_pp_ins_appls%ROWTYPE;
  new_references igs_fi_pp_ins_appls%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.installment_application_id        := x_installment_application_id;
    new_references.application_type_code             := x_application_type_code;
    ...
    new_references.applied_amt                       := x_applied_amt;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_fi_pp_ins_appls
      WHERE    installment_application_id = x_installment_application_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_FI_PPIA_PPIN_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  END get_fk_igs_fi_pp_instlmnts;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.installment_application_id
           )
         ) THEN
  ...
  */
  ...
  BEGIN
    ...
    ELSE
      ...
      fnd_message.set_token ('ROUTINE', 'IGS_FI_PP_INS_APPLS_PKG.INSERT_ROW');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
    END IF;

    x_installment_application_id := NULL;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_installment_application_id        => x_installment_application_id,
      x_application_type_code             => x_application_type_code,
      x_installment_id                    => x_installment_id,
      x_credit_id                         => x_credit_id,
      x_credit_activity_id                => x_credit_activity_id,
      x_applied_amt                       => x_applied_amt,
    ...
    INSERT INTO igs_fi_pp_ins_appls (
      ...
      igs_fi_pp_ins_appls_s.NEXTVAL,
      new_references.application_type_code,
      new_references.installment_id,
      new_references.credit_id,
      new_references.credit_activity_id,
      new_references.applied_amt,
    ...
    ) RETURNING ROWID, installment_application_id INTO x_rowid, x_installment_application_id;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.application_type_code = x_application_type_code)
        ...
        AND (tlinfo.applied_amt = x_applied_amt)
  ...
  */
  ...
  BEGIN
    ...
    ELSE
      fnd_message.set_name( 'FND', 'SYSTEM-INVALID ARGS');
      fnd_message.set_token ('ROUTINE', 'IGS_FI_PP_INS_APPLS_PKG.UPDATE_ROW');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
    END IF;
    ...
    UPDATE igs_fi_pp_ins_appls
      SET
        application_type_code             = new_references.application_type_code,
        installment_id                    = new_references.installment_id,
        credit_id                         = new_references.credit_id,
        credit_activity_id                = new_references.credit_activity_id,
        applied_amt                       = new_references.applied_amt,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_fi_pp_ins_appls
    WHERE rowid = x_rowid;
