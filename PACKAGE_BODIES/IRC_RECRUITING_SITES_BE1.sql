--------------------------------------------------------
--  DDL for Package Body IRC_RECRUITING_SITES_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_RECRUITING_SITES_BE1" as 
--Code generated on 30/08/2013 11:35:51
/* $Header: hrapiwfe.pkb 120.4.12010000.2 2008/09/29 12:54:07 srgnanas ship $*/
procedure create_recruiting_site_a (
...
p_posting_impl_class           varchar2) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  irc_recruiting_sites_be1.create_recruiting_site_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.irc.api.recruiting_sites.create_recruiting_site';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<site_name>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_site_name);
    l_text:=l_text||'</site_name>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<date_from>';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_username>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_posting_username);
    l_text:=l_text||'</posting_username>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_password>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_posting_password);
    l_text:=l_text||'</posting_password>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<internal>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_internal);
    l_text:=l_text||'</internal>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<external>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_external);
    l_text:=l_text||'</external>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<third_party>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_third_party);
    l_text:=l_text||'</third_party>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<redirection_url>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_redirection_url);
    l_text:=l_text||'</redirection_url>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_url>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_posting_url);
    l_text:=l_text||'</posting_url>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_cost>';
    l_text:=l_text||fnd_number.number_to_canonical(p_posting_cost);
    l_text:=l_text||'</posting_cost>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_cost_period>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_posting_cost_period);
    l_text:=l_text||'</posting_cost_period>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_cost_currency>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_posting_cost_currency);
    l_text:=l_text||'</posting_cost_currency>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<stylesheet>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_stylesheet);
    l_text:=l_text||'</stylesheet>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<posting_impl_class>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_posting_impl_class);
    l_text:=l_text||'</posting_impl_class>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</recruiting_sites>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
...
end create_recruiting_site_a;
end irc_recruiting_sites_be1;
