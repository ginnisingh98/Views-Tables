--------------------------------------------------------
--  DDL for Package Body AME_ATU_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ATU_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_atu_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_ATTRIBUTE_USAGES_PK') Then
...
Begin
  ...
  Else
    If (p_attribute_id =
        ame_atu_shd.g_old_rec.attribute_id and
        p_application_id =
        ame_atu_shd.g_old_rec.application_id and
        p_object_version_number =
        ame_atu_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_atu_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_atu_shd.g_old_rec.end_date;
    ...
    p_validation_start_date := ame_atu_shd.g_old_rec.start_date;
    p_validation_end_date   := ame_atu_shd.g_old_rec.end_date;
  end if;
  --
...
Begin
  ...
  l_rec.use_count                        := p_use_count;
  l_rec.user_editable                    := p_user_editable;
