--------------------------------------------------------
--  DDL for Package Body HR_TCP_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_TCP_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_tcp_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code             varchar2(150)  default null;
g_template_item_context_page_i number         default null;
--
--  ---------------------------------------------------------------------------
  ...
  cursor csr_leg_code is
    select tmp.legislation_code
     ...
     where tmp.form_template_id = tim.form_template_id
       ...
       and tic.template_item_context_id = tcp.template_item_context_id
...
Begin
  ...
  --
  if ( nvl(hr_tcp_bus.g_template_item_context_page_i, hr_api.g_number)
       = p_template_item_context_page_i) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := hr_tcp_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    hr_tcp_bus.g_template_item_context_page_i := p_template_item_context_page_i;
    hr_tcp_bus.g_legislation_code := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT hr_tcp_shd.api_updating
      (p_template_item_context_page_i         => p_rec.template_item_context_page_id
  ...
  IF (nvl(p_rec.template_item_context_id,hr_api.g_number) <>
      nvl(hr_tcp_shd.g_old_rec.template_item_context_id,hr_api.g_number)
     ) THEN
  ...
  IF (nvl(p_rec.template_tab_page_id,hr_api.g_number) <>
      nvl(hr_tcp_shd.g_old_rec.template_tab_page_id,hr_api.g_number)
     ) THEN
...
--
-- ----------------------------------------------------------------------------
-- |-----------------------< chk_context_and_tab_page >-----------------------|
-- ----------------------------------------------------------------------------
Procedure chk_context_and_tab_page
  (p_template_item_context_page_i in     number
  ,p_object_version_number        in     number
  ,p_template_item_context_id     in     number
  ,p_template_tab_page_id         in     number
  ...
  cursor csr_template_item_context is
    select tim.form_template_id
      from hr_template_items_b tim
          ,hr_template_item_contexts_b tic
     where tim.template_item_id = tic.template_item_id
  ...
  cursor csr_tab_stacked_canvas is
    select null
     ...
     where p_template_tab_page_id = ttp.template_tab_page_id
       ...
       and tic.template_item_context_id = p_template_item_context_id;
  --
  l_proc                         varchar2(72) := g_package || 'chk_context_and_tab_page';
  l_api_updating                 boolean;
  l_context_template_id          number;
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 10);
  --
  l_api_updating := hr_tcp_shd.api_updating
  ...
  --
  if (  (   l_api_updating
        and (  nvl(hr_tcp_shd.g_old_rec.template_item_context_id,hr_api.g_number) <>
               nvl(p_template_item_context_id,hr_api.g_number)
            or nvl(hr_tcp_shd.g_old_rec.template_tab_page_id,hr_api.g_number) <>
    ...
    fetch csr_template_item_context into l_context_template_id;
    ...
    --
    -- Check context and tab page reference the same template
    --
    if nvl(l_context_template_id,hr_api.g_number) <>
    ...
    --
    -- Check context and tab page reference the same tab stacked canvas
    --
