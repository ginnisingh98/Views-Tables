--------------------------------------------------------
--  DDL for Package Body ASO_SUP_RESPONSE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ASO_SUP_RESPONSE_PKG" AS
/* $Header: asospreb.pls 120.4.12010000.2 2015/08/11 05:37:22 akushwah ship $*/
...
IS
  ...
  cursor c is
    select ROWID
    from  aso_sup_response_B
    where  RESPONSE_ID = PX_RESPONSE_ID ;

  cursor CU_RESPONSE_ID IS
    select aso_sup_response_B_S.NEXTVAL from sys.dual;

Begin

  IF (PX_RESPONSE_ID IS NULL) OR (PX_RESPONSE_ID = FND_API.G_MISS_NUM) THEN
      OPEN CU_RESPONSE_ID;
      FETCH CU_RESPONSE_ID INTO PX_RESPONSE_ID;
      CLOSE CU_RESPONSE_ID;

  END IF;

  insert into ASO_SUP_RESPONSE_B (
  RESPONSE_ID,
  created_by  ,
  creation_date ,
  last_updated_by ,
...
values
  ...
  insert into ASO_SUP_RESPONSE_TL (
    RESPONSE_ID,
    LANGUAGE,
    SOURCE_LANG,
    RESPONSE_NAME,
  ...
  and  not exists
         ( select 'x'
           from  ASO_SUP_RESPONSE_TL  T
           where  T.RESPONSE_ID = PX_RESPONSE_ID
...
update ASO_SUP_RESPONSE_TL
 set
  RESPONSE_NAME = P_RESPONSE_NAME,
...
Begin
 ...
 delete from ASO_SUP_RESPONSE_TL
  where  RESPONSE_ID = P_RESPONSE_ID;
  ...
  delete from ASO_SUP_RESPONSE_B
  where  RESPONSE_ID = P_RESPONSE_ID;
...
SELECT
 ...
 from  ASO_SUP_RESPONSE_B a
 where a.RESPONSE_ID = P_RESPONSE_ID
 for update of a.RESPONSE_ID nowait;
...
recinfo i_csr%rowtype;
  ...
  cursor c1 is
    ...
    where RESPONSE_ID = P_RESPONSE_ID
    for update of RESPONSE_ID nowait;
...
-- only compare to TL column (c1 cursor below).
  ...
  if (
          ((recinfo.RESPONSE_ID = P_RESPONSE_ID)
  ...
  for tlinfo in c1 loop
    if (tlinfo.BASELANG = 'Y') then
      if (    ((tlinfo.RESPONSE_NAME = P_RESPONSE_NAME)
               OR ((tlinfo.RESPONSE_NAME is null) AND (P_RESPONSE_NAME is null)))
...
begin
  ...
  delete from ASO_SUP_RESPONSE_TL T
  where not exists
        ( select NULL
          from  ASO_SUP_RESPONSE_B  B
          where  B.RESPONSE_ID = T.RESPONSE_ID
        );

  update ASO_SUP_RESPONSE_TL T set (
      RESPONSE_NAME,
      DESCRIPTION
    ) = ( select
      B.RESPONSE_NAME,
      B.DESCRIPTION
    from  ASO_SUP_RESPONSE_TL  B
    where  B.RESPONSE_ID = T.RESPONSE_ID
  ...
  where (
      ...
      SUBT.RESPONSE_ID,
      SUBT.LANGUAGE
    from  ASO_SUP_RESPONSE_TL  SUBB,
          ASO_SUP_RESPONSE_TL  SUBT
    where  SUBB.RESPONSE_ID = SUBT.RESPONSE_ID
      and  SUBB.LANGUAGE = SUBT.SOURCE_LANG
      and  ( SUBB.RESPONSE_NAME <> SUBT.RESPONSE_NAME
           or ( SUBB.RESPONSE_NAME is null     and SUBT.RESPONSE_NAME is not null )
           or ( SUBB.RESPONSE_NAME is not null and SUBT.RESPONSE_NAME is null ) )
  ...
  where  L.INSTALLED_FLAG in ('I', 'B')
    ...
    and  not exists
         ( select NULL
           from  ASO_SUP_RESPONSE_TL  T
           where  T.RESPONSE_ID = B.RESPONSE_ID
             and  T.LANGUAGE = L.LANGUAGE_CODE );
...
begin
  ...
  declare
     user_id            number := 0;
     row_id             varchar2(64);
     l_RESPONSE_id NUMBER := p_RESPONSE_id;
...
ASO_SUP_RESPONSE_PKG.UPDATE_ROW (
  P_RESPONSE_ID => P_RESPONSE_ID,
  P_LAST_UPDATE_DATE => sysdate,
  P_LAST_UPDATED_BY => user_id,
  P_LAST_UPDATE_LOGIN => 0,
  P_RESPONSE_NAME => P_RESPONSE_NAME,
 ...
 ASO_SUP_RESPONSE_PKG.INSERT_ROW (
  PX_ROWID => row_id,
  PX_RESPONSE_ID => L_RESPONSE_ID,
...
begin
    ...
    update ASO_SUP_RESPONSE_TL
    set  RESPONSE_NAME = P_RESPONSE_NAME,
         DESCRIPTION = P_DESCRIPTION,
         source_lang = userenv('LANG'),
...
IS
 ...
 l_COMPONENT_RESPONSE_ID NUMBER;
 row_id VARCHAR2(32767);
 row_id1 VARCHAR2(32767);

cursor get_response_id is
SELECT RESPONSE_ID
FROM ASO_SUP_RESPONSE_TL
WHERE RESPONSE_ID = p_RESPONSE_ID;

cursor get_mappings is
select component_response_ID
from aso_sup_comp_resp_map
where component_id = P_RESP_COMPONENT_ID
and response_id = P_RESPONSE_ID;

begin
     if (P_UPLOAD_MODE = 'NLS') then
	   ASO_SUP_RESPONSE_PKG.TRANSLATE_ROW (
                 P_RESPONSE_ID    => P_RESPONSE_ID,
                 P_RESPONSE_NAME  => P_RESPONSE_NAME,
                 P_DESCRIPTION    => P_DESCRIPTION,
                 X_OWNER          => P_OWNER);
       ...
       if ( fnd_load_util.owner_id(P_OWNER) = 120 ) then
          ...
          open get_response_id;
          loop
          fetch get_response_id into l_RESPONSE_ID;
          if get_response_id%FOUND THEN
            -- THIS MEANS RESPONSE ALREADY EXISTS
           --WHERE RESPONSE_NAME like P_RESPONSE_NAME;
           ASO_SUP_RESPONSE_PKG.UPDATE_ROW (
             P_RESPONSE_ID             => P_RESPONSE_ID,
             P_LAST_UPDATE_DATE         => sysdate ,
             ...
             P_ATTRIBUTE20              => P_ATTRIBUTE20
              ...
              exit;
             elsif get_response_id%NOTFOUND THEN
             -- this means this is a new response
           ...
           ASO_SUP_RESPONSE_PKG.INSERT_ROW (
             ...
             close get_response_id;

             open get_mappings;
             loop
             fetch get_mappings into l_component_response_ID;
             if get_mappings%FOUND THEN
             -- THIS MEANS RESPONSE has already been used for that component
                   ASO_SUP_COMP_RESP_MAP_PKG.UPDATE_ROW (
                   P_COMPONENT_RESPONSE_ID   => l_COMPONENT_RESPONSE_ID,
                   P_LAST_UPDATE_DATE         => sysdate ,
                   P_LAST_UPDATED_BY          => l_user_id ,
                   P_LAST_UPDATE_LOGIN        => l_user_id ,
                   P_COMPONENT_ID             => P_RESP_COMPONENT_ID,
                   P_RESPONSE_ID              => P_RESPONSE_ID ,
                   P_DISPLAY_SEQUENCE         => P_RESP_DISPLAY_SEQUENCE,
                   p_context                  => P_CONTEXT,
                   P_ATTRIBUTE1               => P_ATTRIBUTE1,
                   P_ATTRIBUTE2               => P_ATTRIBUTE2,
                   P_ATTRIBUTE3               => P_ATTRIBUTE3,
             ...
             -- THIS MEANS RESPONSE has not been used for that component
                   ASO_SUP_COMP_RESP_MAP_PKG.INSERT_ROW (
                   PX_ROWID                   => row_id1,
                   PX_COMPONENT_RESPONSE_ID   => l_COMPONENT_RESPONSE_ID,
                   P_CREATION_DATE            => sysdate ,
                   P_CREATED_BY               => l_user_id ,
                   P_LAST_UPDATE_DATE         => sysdate ,
                   P_LAST_UPDATED_BY          => l_user_id ,
     ...
     end if; -- end if for the NLS condition check
END LOAD_SEED_ROW;

END; -- Package Body ASO_SUP_RESPONSE_PKG
