--------------------------------------------------------
--  DDL for Package Body PER_FI_POSTALCODE_UPLOAD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_FI_POSTALCODE_UPLOAD" AS
/* $Header: pefipost.pkb 120.0 2005/05/31 08:44:38 appldev noship $ */


-- Global package name
        g_package                    CONSTANT VARCHAR2 (33) := 'POST_FI_UPLOAD';
     ...
     BEGIN
	   ...
	   EXCEPTION
		       ...
		       -- Set the application error
   		       ...
   		       hr_utility.set_message (800, 'HR_376627_FI_INVALID_RECORD');
		       ...
		       -- Set the application error
   		       ...
   		       hr_utility.set_message (800, 'HR_376626_FI_EMPTY_LINE');
		       hr_utility.set_message_token (800, 'LINE_NO', l_batch_seq);
	...
	when others then
	  g_security_group_id := 0;
        ...
        ELSE
            ...
            l_code:= substr(p_line,14,5);
            if l_code is null or is_number(l_code)=false
            or is_number(substr(p_line,14,6))=true
            then
            RAISE e_invalid_record;
            else

            g_meaning:=rtrim(substr(p_line,19,30))||' '||'/'||' '||rtrim(substr(p_line,49,30));
