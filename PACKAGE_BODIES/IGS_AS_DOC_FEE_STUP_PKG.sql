--------------------------------------------------------
--  DDL for Package Body IGS_AS_DOC_FEE_STUP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_DOC_FEE_STUP_PKG" AS
/* $Header: IGSDI69B.pls 115.2 2003/02/11 09:51:38 pathipat noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_as_doc_fee_stup%ROWTYPE;
  new_references igs_as_doc_fee_stup%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.tfee_id                           := x_tfee_id;
    ...
    new_references.payment_type                      := x_payment_type;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_as_doc_fee_stup
      WHERE    ((document_type = x_document_type) OR (document_type IS NULL AND x_document_type IS NULL))
      AND      ((lower_range = x_lower_range) OR (lower_range IS NULL AND x_lower_range IS NULL))
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_as_doc_fee_stup_s.NEXTVAL
    INTO      x_tfee_id
    FROM      dual;
    ...
    INSERT INTO igs_as_doc_fee_stup (
      tfee_id,
      document_type,
      lower_range,
      upper_range,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ((tlinfo.document_type = x_document_type) OR ((tlinfo.document_type IS NULL) AND (X_document_type IS NULL)))
        AND ((tlinfo.lower_range = x_lower_range) OR ((tlinfo.lower_range IS NULL) AND (X_lower_range IS NULL)))
        AND ((tlinfo.upper_range = x_upper_range) OR ((tlinfo.upper_range IS NULL) AND (X_upper_range IS NULL)))
        AND ((tlinfo.payment_type = x_payment_type) OR ((tlinfo.payment_type IS NULL) AND (X_payment_type IS NULL)))
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_as_doc_fee_stup
    WHERE rowid = x_rowid;
