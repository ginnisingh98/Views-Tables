--------------------------------------------------------
--  DDL for Package Body PQH_RCT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_RCT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_rct_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy pqh_rct_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_routing_categories Row
  --
  update pqh_routing_categories
  set
  ...
  routing_list_id                   = p_rec.routing_list_id,
  position_structure_id             = p_rec.position_structure_id,
  override_position_id              = p_rec.override_position_id,
  override_assignment_id            = p_rec.override_assignment_id,
  override_role_id                  = p_rec.override_role_id,
  override_user_id                  = p_rec.override_user_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in pqh_rct_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update(p_effective_date in date,p_rec in pqh_rct_shd.g_rec_type) is
--
...
--
 ...
 ,p_override_assignment_id        =>p_rec.override_assignment_id
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy pqh_rct_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.transaction_category_id = hr_api.g_number) then
    ...
    pqh_rct_shd.g_old_rec.transaction_category_id;
  End If;
  If (p_rec.enable_flag = hr_api.g_varchar2) then
    p_rec.enable_flag :=
    pqh_rct_shd.g_old_rec.enable_flag;
  End If;
  If (p_rec.default_flag = hr_api.g_varchar2) then
    p_rec.default_flag :=
    pqh_rct_shd.g_old_rec.default_flag;
  End If;
  If (p_rec.delete_flag = hr_api.g_varchar2) then
    p_rec.delete_flag :=
    pqh_rct_shd.g_old_rec.delete_flag;
  End If;
  If (p_rec.routing_list_id = hr_api.g_number) then
    p_rec.routing_list_id :=
    pqh_rct_shd.g_old_rec.routing_list_id;
  End If;
  If (p_rec.position_structure_id = hr_api.g_number) then
    p_rec.position_structure_id :=
    pqh_rct_shd.g_old_rec.position_structure_id;
  End If;
  If (p_rec.override_position_id = hr_api.g_number) then
    p_rec.override_position_id :=
    pqh_rct_shd.g_old_rec.override_position_id;
  End If;
  If (p_rec.override_assignment_id = hr_api.g_number) then
    p_rec.override_assignment_id :=
    pqh_rct_shd.g_old_rec.override_assignment_id;
  End If;
  If (p_rec.override_role_id = hr_api.g_number) then
    p_rec.override_role_id :=
    pqh_rct_shd.g_old_rec.override_role_id;
  End If;
  If (p_rec.override_user_id = hr_api.g_number) then
    p_rec.override_user_id :=
    pqh_rct_shd.g_old_rec.override_user_id;
  End If;
...
Procedure upd
  ...
  p_rec        in out nocopy pqh_rct_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  pqh_rct_bus.update_validate(p_rec,p_effective_date);
  --
...
Begin
  ...
  p_override_position_id,
  p_override_assignment_id,
  p_override_role_id,
  p_override_user_id,
