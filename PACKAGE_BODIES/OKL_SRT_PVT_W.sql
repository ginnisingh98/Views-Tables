--------------------------------------------------------
--  DDL for Package Body OKL_SRT_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_SRT_PVT_W" as
  /* $Header: OKLISRTB.pls 120.0 2005/07/07 10:40:32 viselvar noship $ */
  procedure rosetta_table_copy_in_p1(t out nocopy okl_srt_pvt.okl_srtv_tbl, a0 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).std_rate_tmpl_id := a0(indx);
          t(ddindx).template_name := a1(indx);
          t(ddindx).template_desc := a2(indx);
          t(ddindx).object_version_number := a3(indx);
          t(ddindx).org_id := a4(indx);
          t(ddindx).currency_code := a5(indx);
          t(ddindx).rate_card_yn := a6(indx);
          t(ddindx).pricing_engine_code := a7(indx);
          t(ddindx).orig_std_rate_tmpl_id := a8(indx);
          t(ddindx).rate_type_code := a9(indx);
          t(ddindx).frequency_code := a10(indx);
          t(ddindx).index_id := a11(indx);
          t(ddindx).default_yn := a12(indx);
          t(ddindx).sts_code := a13(indx);
          t(ddindx).effective_from_date := a14(indx);
          t(ddindx).effective_to_date := a15(indx);
          t(ddindx).srt_rate := a16(indx);
  ...
  procedure rosetta_table_copy_out_p1(t okl_srt_pvt.okl_srtv_tbl, a0 out nocopy JTF_NUMBER_TABLE
  ...
  if t is null or t.count = 0 then
    ...
    a37 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).std_rate_tmpl_id;
          a1(indx) := t(ddindx).template_name;
          a2(indx) := t(ddindx).template_desc;
          a3(indx) := t(ddindx).object_version_number;
          a4(indx) := t(ddindx).org_id;
          a5(indx) := t(ddindx).currency_code;
          a6(indx) := t(ddindx).rate_card_yn;
          a7(indx) := t(ddindx).pricing_engine_code;
          a8(indx) := t(ddindx).orig_std_rate_tmpl_id;
          a9(indx) := t(ddindx).rate_type_code;
          a10(indx) := t(ddindx).frequency_code;
          a11(indx) := t(ddindx).index_id;
          a12(indx) := t(ddindx).default_yn;
          a13(indx) := t(ddindx).sts_code;
          a14(indx) := t(ddindx).effective_from_date;
          a15(indx) := t(ddindx).effective_to_date;
          a16(indx) := t(ddindx).srt_rate;
  ...
  procedure rosetta_table_copy_in_p3(t out nocopy okl_srt_pvt.okl_srtb_tbl, a0 JTF_NUMBER_TABLE
    , a1 JTF_VARCHAR2_TABLE_300
    , a2 JTF_NUMBER_TABLE
    , a3 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).rate_card_yn := a5(indx);
          t(ddindx).pricing_engine_code := a6(indx);
          t(ddindx).orig_std_rate_tmpl_id := a7(indx);
          t(ddindx).rate_type_code := a8(indx);
          t(ddindx).frequency_code := a9(indx);
          t(ddindx).index_id := a10(indx);
          t(ddindx).default_yn := a11(indx);
          t(ddindx).sts_code := a12(indx);
          t(ddindx).effective_from_date := a13(indx);
          t(ddindx).effective_to_date := a14(indx);
          t(ddindx).srt_rate := a15(indx);
  ...
  procedure rosetta_table_copy_out_p3(t okl_srt_pvt.okl_srtb_tbl, a0 out nocopy JTF_NUMBER_TABLE
    , a1 out nocopy JTF_VARCHAR2_TABLE_300
    ...
    a36 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a5(indx) := t(ddindx).rate_card_yn;
          a6(indx) := t(ddindx).pricing_engine_code;
          a7(indx) := t(ddindx).orig_std_rate_tmpl_id;
          a8(indx) := t(ddindx).rate_type_code;
          a9(indx) := t(ddindx).frequency_code;
          a10(indx) := t(ddindx).index_id;
          a11(indx) := t(ddindx).default_yn;
          a12(indx) := t(ddindx).sts_code;
          a13(indx) := t(ddindx).effective_from_date;
          a14(indx) := t(ddindx).effective_to_date;
          a15(indx) := t(ddindx).srt_rate;
  ...
  procedure rosetta_table_copy_in_p5(t out nocopy okl_srt_pvt.okl_srttl_tbl, a0 JTF_NUMBER_TABLE
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          t(ddindx).std_rate_tmpl_id := a0(indx);
          t(ddindx).template_desc := a1(indx);
  ...
  procedure rosetta_table_copy_out_p5(t okl_srt_pvt.okl_srttl_tbl, a0 out nocopy JTF_NUMBER_TABLE
  ...
  if t is null or t.count = 0 then
    ...
    a9 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          a0(indx) := t(ddindx).std_rate_tmpl_id;
          a1(indx) := t(ddindx).template_desc;
  ...
  as
    ddp_srtv_rec okl_srt_pvt.okl_srtv_rec;
    ddx_srtv_rec okl_srt_pvt.okl_srtv_rec;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_srt_pvt.insert_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
  ...
  as
    ddp_srtv_tbl okl_srt_pvt.okl_srtv_tbl;
    ddx_srtv_tbl okl_srt_pvt.okl_srtv_tbl;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    okl_srt_pvt_w.rosetta_table_copy_in_p1(ddp_srtv_tbl, p5_a0
      , p5_a1
      , p5_a2
      , p5_a3
      , p5_a4
    ...
    okl_srt_pvt_w.rosetta_table_copy_out_p1(ddx_srtv_tbl, p6_a0
      , p6_a1
      , p6_a2
      , p6_a3
      , p6_a4
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    okl_srt_pvt.delete_row(p_api_version,
      p_init_msg_list,
      x_return_status,
      x_msg_count,
      x_msg_data,
