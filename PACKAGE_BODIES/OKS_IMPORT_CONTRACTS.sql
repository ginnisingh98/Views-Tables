--------------------------------------------------------
--  DDL for Package Body OKS_IMPORT_CONTRACTS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKS_IMPORT_CONTRACTS" AS
-- $Header: OKSPKIMPB.pls 120.2 2007/09/06 11:18:38 vmutyala noship $
--+=======================================================================+
...
--+=======================================================================+
--| FILENAME                                                              |
--|     OKSPKIMPB.pls   Created By Vamshi Mutyala                         |
--|                                                                       |
--| DESCRIPTION                                                           |
...
G_PKG_NAME CONSTANT    VARCHAR2(30) := 'OKS_IMPORT_CONTRACTS';
...
IS
  ...
  l_stmt_num              NUMBER := 0;
  l_routine      CONSTANT VARCHAR2(30) := 'Import_contracts';
...
--
 ...
 l_stmt_num := 15;
    SELECT count(1) INTO l_worker_count FROM fnd_concurrent_requests
    WHERE  parent_request_id = G_WORKER_REQ_ID
      AND rownum = 1;

    IF l_row_count > 0 AND l_worker_count = 0 THEN
	 l_stmt_num := 20;
    ...
    ELSE
	 ...
	 l_report_req_id := FND_REQUEST.SUBMIT_REQUEST( 'OKS'
							,'OKS_IMP_REPORT'
			                                ,NULL
			                                ,NULL
			                                ,FALSE
	 ...
	 IF (l_report_req_id = 0) THEN
		...
		FND_MESSAGE.Set_Name('OKS', 'OKS_IMPORT_UNEXPECTED');
		FND_MESSAGE.set_token('ROUTINE', G_MODULE_HEAD ||l_routine);
		FND_MESSAGE.set_token('MESSAGE', 'Unable to submit request for error report '||l_message);
		FND_MSG_PUB.Add;
		RAISE FND_API.G_EXC_ERROR;
	 END IF;
    END IF;
