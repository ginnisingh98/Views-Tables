--------------------------------------------------------
--  DDL for Package Body OTA_CTL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_CTL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_ctl_bus.';  -- Global package name
--
...
g_language                    varchar2(240)  default null;
--
--  ---------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  if (( nvl(ota_ctl_bus.g_certification_id, hr_api.g_number)
       = p_certification_id)
  and ( nvl(ota_ctl_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := ota_ctl_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    ota_ctl_bus.g_certification_id            := p_certification_id;
    ota_ctl_bus.g_language                    := p_language;
    ota_ctl_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT ota_ctl_shd.api_updating
...
Begin
  ...
  ota_ctl_bus.set_security_group_id(p_certification_id => p_certification_id);
  --
