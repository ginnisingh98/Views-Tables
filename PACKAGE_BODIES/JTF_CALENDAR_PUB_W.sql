--------------------------------------------------------
--  DDL for Package Body JTF_CALENDAR_PUB_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_CALENDAR_PUB_W" as
  /* $Header: jtfclawb.pls 120.2 2005/12/30 03:06 abraina ship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        ddindx := 1;
        while true loop
          t(ddindx).shift_construct_id := a0(indx);
          t(ddindx).shift_date := rosetta_g_miss_date_in_map(a1(indx));
          t(ddindx).start_time := a2(indx);
          t(ddindx).end_time := a3(indx);
          t(ddindx).availability_type := a4(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
  ...
  if t is null or t.count = 0 then
    ...
    a4 := JTF_VARCHAR2_TABLE_100();
      ...
      if t.count > 0 then
        ...
        indx := 1;
        while true loop
          a0(indx) := t(ddindx).shift_construct_id;
          a1(indx) := t(ddindx).shift_date;
          a2(indx) := t(ddindx).start_time;
          a3(indx) := t(ddindx).end_time;
          a4(indx) := t(ddindx).availability_type;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  begin
    ...
    jtf_calendar_pub.is_res_available(p_api_version,
      p_init_msg_list,
      p_resource_id,
      p_resource_type,
      ddp_start_date_time,
  ...
  as
    ddp_start_date date;
    ddp_end_date date;
    ddx_shift jtf_calendar_pub.shift_tbl_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    jtf_calendar_pub.get_available_time(p_api_version,
      p_init_msg_list,
      p_resource_id,
      p_resource_type,
      ddp_start_date,
    ...
    jtf_calendar_pub_w.rosetta_table_copy_out_p1(ddx_shift, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    jtf_calendar_pub.get_available_slot(p_api_version,
      p_init_msg_list,
      p_resource_id,
      p_resource_type,
      ddp_start_date_time,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    jtf_calendar_pub.get_resource_shifts(p_api_version,
      p_init_msg_list,
      p_resource_id,
      p_resource_type,
      ddp_start_date,
  ...
  as
    ddp_start_date date;
    ddp_end_date date;
    ddx_shift jtf_calendar_pub.shift_tbl_attributes_type;
    ddindx binary_integer; indx binary_integer;
  begin
    ...
    jtf_calendar_pub_w.rosetta_table_copy_out_p3(ddx_shift, p9_a0
      , p9_a1
      , p9_a2
      , p9_a3
      , p9_a4
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    jtf_calendar_pub.get_res_schedule(p_api_version,
      p_init_msg_list,
      p_resource_id,
      p_resource_type,
      ddp_start_date,
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := jtf_calendar_pub.resourcedt_to_serverdt(ddp_resource_dttime,
      p_resource_tz_id,
      p_server_tz_id);
  ...
  begin
    ...
    -- here's the delegated call to the old PL/SQL routine
    ddrosetta_retval := jtf_calendar_pub.validate_cal_date(p_calendar_id,
      ddp_shift_date);
