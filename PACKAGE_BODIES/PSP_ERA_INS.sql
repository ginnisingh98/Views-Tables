--------------------------------------------------------
--  DDL for Package Body PSP_ERA_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PSP_ERA_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  psp_era_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_effort_report_approval_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_effort_report_approval_id  in  number) is
--
...
Begin
  ...
  --
  psp_era_ins.g_effort_report_approval_id_i := p_effort_report_approval_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: psp_eff_report_approvals
  --
...
--
  Cursor C_Sel1 is select psp_eff_report_approvals_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from psp_eff_report_approvals
     where effort_report_approval_id =
             psp_era_ins.g_effort_report_approval_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (psp_era_ins.g_effort_report_approval_id_i is not null) Then
    --
    ...
    p_rec.effort_report_approval_id :=
      psp_era_ins.g_effort_report_approval_id_i;
    psp_era_ins.g_effort_report_approval_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.effort_report_approval_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  psp_era_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  psp_era_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  psp_era_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  -- Having converted the arguments into the psp_era_rec
  ...
  --
  p_effort_report_approval_id := l_rec.effort_report_approval_id;
