--------------------------------------------------------
--  DDL for Package Body PER_APT_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_APT_DEL" as
...
g_package  varchar2(33)	:= '  per_apt_del.';  -- Global package name
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------+
Procedure delete_dml(p_rec in per_apt_shd.g_rec_type) is
...
Begin
  ...
  -- Delete the per_appraisal_templates row.

  delete from per_appraisal_templates
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------+
Procedure pre_delete(p_rec in per_apt_shd.g_rec_type) is
...
-- {End Of Comments}
-- ---------------------------------------------------------------------------+
Procedure post_delete(p_rec in per_apt_shd.g_rec_type) is
...
Procedure del
  (
  p_rec	      in per_apt_shd.g_rec_type,
...
Begin
  ...
  per_apt_bus.delete_validate(p_rec);
