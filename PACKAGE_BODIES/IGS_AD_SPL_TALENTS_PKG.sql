--------------------------------------------------------
--  DDL for Package Body IGS_AD_SPL_TALENTS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_SPL_TALENTS_PKG" AS
/* $Header: IGSAI97B.pls 120.3 2005/10/03 08:21:07 appldev ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_ad_spl_talents%RowType;
  new_references igs_ad_spl_talents%RowType;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.spl_talent_id := x_spl_talent_id;
    new_references.person_id := x_person_id;
    new_references.admission_appl_number := x_admission_appl_number;
    new_references.special_talent_type_id := x_special_talent_type_id;
  ...
  ***************************************************************/
   ...
   begin
		...
		new_references.special_talent_type_id
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (((old_references.special_talent_type_id = new_references.special_talent_type_id)) OR
        ((new_references.special_talent_type_id IS NULL))) THEN
      NULL;
    ELSIF NOT Igs_Ad_Code_Classes_Pkg.Get_UK2_For_Validation (
         ...
         FND_MESSAGE.SET_TOKEN('ATTRIBUTE',FND_MESSAGE.GET_STRING('IGS','IGS_AD_SPL_TALENT_TYPE'));
         IGS_GE_MSG_STACK.ADD;
         App_Exception.Raise_Exception;
    END IF;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ad_spl_talents
      WHERE    spl_talent_id = x_spl_talent_id
      FOR UPDATE NOWAIT;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ad_spl_talents
      WHERE    special_talent_type_id = x_special_talent_type_id
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ad_spl_talents
      WHERE    special_talent_type_id = x_code_id ;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ASPLT_ACDC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ASPLT_ACAI_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
    ...
    IF (p_action = 'INSERT') THEN
      ...
      Null;
	     IF Get_Pk_For_Validation(
    		new_references.spl_talent_id)  THEN
	       Fnd_Message.Set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
      IGS_GE_MSG_STACK.ADD;
	       App_Exception.Raise_Exception;
	     END IF;
  ...
  ***************************************************************/
    ...
    cursor C is select ROWID from IGS_AD_SPL_TALENTS
             where                 SPL_TALENT_ID= X_SPL_TALENT_ID
;
 ...
 begin
   ...
   Before_DML(
 		p_action=>'INSERT',
 		x_rowid=>X_ROWID,
 	       x_spl_talent_id=>X_SPL_TALENT_ID,
 	       x_person_id=>X_PERSON_ID,
 	       x_admission_appl_number=>X_ADMISSION_APPL_NUMBER,
 	       x_special_talent_type_id=>X_SPECIAL_TALENT_TYPE_ID,
  ...
  END IF;
 INSERT INTO IGS_AD_SPL_TALENTS (
		SPL_TALENT_ID
		,PERSON_ID
...
) RETURNING SPL_TALENT_ID INTO X_SPL_TALENT_ID;
 IF (x_mode = 'S') THEN
...
END INSERT_ROW;
 PROCEDURE LOCK_ROW (
      X_ROWID in  VARCHAR2,
       ...
       x_PERSON_ID IN NUMBER,
       x_ADMISSION_APPL_NUMBER IN NUMBER,
       x_SPECIAL_TALENT_TYPE_ID IN NUMBER  ) AS
  /*************************************************************
  Created By :
  Date Created By :
  Purpose :
...
if ( (  tlinfo.PERSON_ID = X_PERSON_ID)
  ...
  AND (tlinfo.SPECIAL_TALENT_TYPE_ID = X_SPECIAL_TALENT_TYPE_ID)
  ) then
    null;
  else
 ...
 begin
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
  UPDATE IGS_AD_SPL_TALENTS set
      PERSON_ID =  NEW_REFERENCES.PERSON_ID,
      ADMISSION_APPL_NUMBER =  NEW_REFERENCES.ADMISSION_APPL_NUMBER,
      SPECIAL_TALENT_TYPE_ID =  NEW_REFERENCES.SPECIAL_TALENT_TYPE_ID,
...
END UPDATE_ROW;
  ...
  ***************************************************************/
    ...
    cursor c1 is select ROWID from IGS_AD_SPL_TALENTS
             where     SPL_TALENT_ID= X_SPL_TALENT_ID
;
begin
	open c1;
...
x_rowid => X_ROWID
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
 delete from IGS_AD_SPL_TALENTS
 where ROWID = X_ROWID;
  IF (sql%notfound) THEN
     fnd_message.set_name ('IGS', 'IGS_SC_POLICY_UPD_DEL_EXCEP');
     igs_ge_msg_stack.add;
