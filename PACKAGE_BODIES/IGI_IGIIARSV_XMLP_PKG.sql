--------------------------------------------------------
--  DDL for Package Body IGI_IGIIARSV_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_IGIIARSV_XMLP_PKG" AS
/* $Header: IGIIARSVB.pls 120.0.12010000.1 2008/07/29 08:58:52 appldev ship $ */
...
procedure FDRCSID(idstring in varchar2) is
  BEGIN
    IF (idstring is NULL) THEN
      NULL;
    END IF;
        --        FDRCSID('$Header: IGIIARSVB.pls 120.0.12010000.1 2008/07/29 08:58:52 appldev ship $');
  END;
...
PROCEDURE Insert_info (
  ...
  FUNCTION REPORT_NAMEFORMULA(COMPANY_NAME IN VARCHAR2
                             ,RP_CURRENCY_CODE IN VARCHAR2) RETURN CHAR IS
  BEGIN
    DECLARE
...
l_report_name := substr(l_report_name,1,instr(l_report_name,' (XML)'));
      RP_REPORT_NAME := L_REPORT_NAME || '(' || RP_CURRENCY_CODE || ')';
      RETURN (L_REPORT_NAME);
    EXCEPTION
      WHEN OTHERS THEN
        RP_REPORT_NAME := 'Inflation Accounting : Reserve Summary Report';
        /*SRW.MESSAGE(101
                   ,'report name in exception')*/NULL;
