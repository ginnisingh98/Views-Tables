--------------------------------------------------------
--  DDL for Package Body IGS_PS_RSV_USEC_PRF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_RSV_USEC_PRF_PKG" AS
/* $Header: IGSPI1XB.pls 120.1 2005/08/18 07:18:09 appldev ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_rsv_usec_prf%ROWTYPE;
  new_references igs_ps_rsv_usec_prf%ROWTYPE;

  PROCEDURE set_column_values (
    p_action                            IN     VARCHAR2,
    x_rowid                             IN     VARCHAR2    DEFAULT NULL,
    x_rsv_usec_prf_id                   IN     NUMBER      DEFAULT NULL,
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.rsv_usec_prf_id                   := x_rsv_usec_prf_id;
    new_references.rsv_usec_pri_id                   := x_rsv_usec_pri_id;
  ...
  */
  BEGIN
    ...
    IF ( get_uk_for_validation (
           new_references.rsv_usec_pri_id,
  ...
  CURSOR c_priority is
   ...
   from  IGS_PS_RSV_USEC_PRI PRI
   Where PRI.RSV_USEC_PRI_ID = new_references.rsv_usec_pri_id;
  ...
  ||  Change History :
  ||  Who             When            What
  ||  sommukhe    12-AUG-2005   Bug#4377818,changed the c_check_hz_exists, included table igs_pe_hz_parties in
  ...
  */
  BEGIN
    ...
    IF (((old_references.rsv_usec_pri_id = new_references.rsv_usec_pri_id)) OR
        ((new_references.rsv_usec_pri_id IS NULL))) THEN
      NULL;
    ELSIF NOT igs_ps_rsv_usec_pri_pkg.get_pk_for_validation (
                new_references.rsv_usec_pri_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_rsv_usec_prf
      WHERE    rsv_usec_prf_id = x_rsv_usec_prf_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_rsv_usec_prf
      WHERE    rsv_usec_pri_id = x_rsv_usec_pri_id
  ...
  PROCEDURE get_fk_igs_ps_rsv_usec_pri (
    x_rsv_usec_pri_id                   IN     NUMBER
  ) AS
  /*
  ||  Created By : apelleti
  ...
  */
  ...
  END get_fk_igs_ps_rsv_usec_pri;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_USPF_CRV_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_USPF_CSTT_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_USPF_US_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_USPF_HZ_PARTIES_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.rsv_usec_prf_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  PROCEDURE insert_row (
    ...
    x_rsv_usec_prf_id                   IN OUT NOCOPY NUMBER,
    x_rsv_usec_pri_id                   IN     NUMBER,
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_rsv_usec_prf_s.NEXTVAL
    INTO      x_rsv_usec_prf_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_rsv_usec_prf_id                   => x_rsv_usec_prf_id,
      x_rsv_usec_pri_id                   => x_rsv_usec_pri_id,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.rsv_usec_pri_id = x_rsv_usec_pri_id)
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igs_ps_rsv_usec_prf
      SET
        rsv_usec_pri_id                   = new_references.rsv_usec_pri_id,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_rsv_usec_prf
    WHERE rowid = x_rowid;
