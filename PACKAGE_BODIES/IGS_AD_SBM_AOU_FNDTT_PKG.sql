--------------------------------------------------------
--  DDL for Package Body IGS_AD_SBM_AOU_FNDTT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_SBM_AOU_FNDTT_PKG" as
/* $Header: IGSAI63B.pls 115.6 2003/10/30 13:21:29 rghosh ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_AD_SBM_AOU_FNDTT%RowType;
  new_references IGS_AD_SBM_AOU_FNDTT%RowType;
...
PROCEDURE Check_Parent_Existance AS
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFT_FS_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFT_GSC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFT_ITT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFT_OU_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_SBM_AOU_FNDTT
      where SUBMISSION_YR = X_SUBMISSION_YR
      and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
      and ORG_UNIT_CD = X_ORG_UNIT_CD
...
begin
  ...
  insert into IGS_AD_SBM_AOU_FNDTT (
    SUBMISSION_YR,
    SUBMISSION_NUMBER,
    ORG_UNIT_CD,
    OU_START_DT,
...
begin
  ...
  update IGS_AD_SBM_AOU_FNDTT set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_SBM_AOU_FNDTT
     where SUBMISSION_YR = X_SUBMISSION_YR
     and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
     and ORG_UNIT_CD = X_ORG_UNIT_CD
     and OU_START_DT = X_OU_START_DT
...
begin
  ...
  delete from IGS_AD_SBM_AOU_FNDTT
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
