--------------------------------------------------------
--  DDL for Package Body IGS_FI_LOCKBOXES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_FI_LOCKBOXES_PKG" AS
/* $Header: IGSSID1B.pls 120.1 2006/02/01 19:55:48 svuppala noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_fi_lockboxes%ROWTYPE;
  new_references igs_fi_lockboxes%ROWTYPE;

  PROCEDURE set_column_values (
    p_action                            IN     VARCHAR2,
    x_rowid                             IN     VARCHAR2,
    x_lockbox_name                      IN     VARCHAR2,
    x_lockbox_desc                      IN     VARCHAR2,
    x_closed_flag                       IN     VARCHAR2,
    x_gl_date_source_code               IN     VARCHAR2,
    x_default_credit_type_id            IN     NUMBER,
  ...
  */
  ...
  BEGIN
    ...
    new_references.lockbox_desc                      := x_lockbox_desc;
    new_references.closed_flag                       := x_closed_flag;
    new_references.gl_date_source_code               := x_gl_date_source_code;
    new_references.default_credit_type_id            := x_default_credit_type_id;
    new_references.bank_account_id                   := x_bank_account_id;
  ...
  ||  Change History :
  ||  Who             When            What
  || sapanigr       01-FEB-2006     Bug 5010822: The code referencing AP_BANK_ACCOUNT
  ||                                has been commented out.
  ||  (reverse chronological order - newest change first)
  */

/*
  CURSOR cur_bank_account_id (cp_n_bank_account_id ap_bank_accounts.bank_account_id%TYPE) IS
  SELECT bank_account_id
  FROM ap_bank_accounts
  WHERE bank_account_id = cp_n_bank_account_id;
  l_rec_bank_account_id cur_bank_account_id%ROWTYPE;
*/

  BEGIN

    IF (((old_references.default_credit_type_id = new_references.default_credit_type_id)) OR
        ((new_references.default_credit_type_id IS NULL))) THEN
      NULL;
    ELSIF NOT igs_fi_cr_types_pkg.get_pk_for_validation (
                new_references.default_credit_type_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
...
/*
    IF (((old_references.bank_account_id = new_references.bank_account_id)) OR
        ((new_references.bank_account_id IS NULL))) THEN
      NULL;
    ELSE
      OPEN cur_bank_account_id( new_references.bank_account_id );
      FETCH cur_bank_account_id INTO l_rec_bank_account_id;
      IF( cur_bank_account_id%NOTFOUND ) THEN
        CLOSE cur_bank_account_id;
        fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
        igs_ge_msg_stack.add;
        app_exception.raise_exception;
      END IF;
...
*/
  ...
  -- Removed check_child_existance as Deletion is not allowed on this table
  ...
  ||  (reverse chronological order - newest change first)
  */
    -- Enh# 2831582, FOR UPDATE NOWAIT clause is removed to avoid locking issues as deletion is not allowed
    -- on igs_fi_lockboxes table.
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_fi_lockboxes
  ...
  -- Enh# 2831582, Removed get_fk_igs_fi_cr_types_all as deletion is not allowed on igs_fi_cr_types_all.
  ...
  */
  BEGIN
    ...
    set_column_values (
      ...
      x_lockbox_desc,
      x_closed_flag,
      x_gl_date_source_code,
      x_default_credit_type_id,
      x_bank_account_id,
  ...
  */
  ...
  BEGIN
    ...
    ELSE
      ...
      fnd_message.set_token ('ROUTINE', 'IGS_FI_LOCKBOXES_PKG.INSERT_ROW');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
    END IF;
    ...
    before_dml(
      ...
      x_lockbox_desc                      => x_lockbox_desc,
      x_closed_flag                       => x_closed_flag,
      x_gl_date_source_code               => x_gl_date_source_code,
      x_default_credit_type_id            => x_default_credit_type_id,
    ...
    INSERT INTO igs_fi_lockboxes (
      lockbox_name,
      lockbox_desc,
      closed_flag,
      gl_date_source_code,
      default_credit_type_id,
      bank_account_id,
      attribute_category,
      attribute1,
      attribute2,
      ...
      new_references.lockbox_name,
      new_references.lockbox_desc,
      new_references.closed_flag,
      new_references.gl_date_source_code,
      new_references.default_credit_type_id,
      new_references.bank_account_id,
  ...
  */
  ...
  BEGIN
    ...
    CLOSE c1;
    IF (
        (tlinfo.lockbox_desc = x_lockbox_desc)
        AND (tlinfo.closed_flag = x_closed_flag)
        AND (tlinfo.gl_date_source_code = x_gl_date_source_code)
        AND ((tlinfo.default_credit_type_id = x_default_credit_type_id) OR ((tlinfo.default_credit_type_id IS NULL) AND (x_default_credit_type_id IS NULL)))
        AND ((tlinfo.bank_account_id = x_bank_account_id) OR ((tlinfo.bank_account_id IS NULL) AND (x_bank_account_id IS NULL)))
  ...
  */
  ...
  BEGIN
    ...
    ELSE
      fnd_message.set_name( 'FND', 'SYSTEM-INVALID ARGS');
      fnd_message.set_token ('ROUTINE', 'IGS_FI_LOCKBOXES_PKG.UPDATE_ROW');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
    END IF;
    ...
    UPDATE igs_fi_lockboxes
      SET
        lockbox_desc                      = new_references.lockbox_desc,
        closed_flag                       = new_references.closed_flag,
        gl_date_source_code               = new_references.gl_date_source_code,
        default_credit_type_id            = new_references.default_credit_type_id,
        bank_account_id                   = new_references.bank_account_id,
  ...
  */
  ...
  -- Removed delete_row as Deletion is not allowed on this table.

END igs_fi_lockboxes_pkg;
