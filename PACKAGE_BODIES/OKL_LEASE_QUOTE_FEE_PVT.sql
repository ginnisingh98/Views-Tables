--------------------------------------------------------
--  DDL for Package Body OKL_LEASE_QUOTE_FEE_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_LEASE_QUOTE_FEE_PVT" AS
/* $Header: OKLRQUFB.pls 120.22 2006/04/05 07:05:09 ssdeshpa noship $ */
  ...
  ---------------------------------------
  -- PROCEDURE populate_fee_details
  ---------------------------------------
  PROCEDURE populate_fee_details (
    p_fee_rec            IN  OUT NOCOPY fee_rec_type
   ,p_expense_header_rec IN  OUT NOCOPY cashflow_hdr_rec_type
   ,p_expense_level_tbl  IN  OUT NOCOPY cashflow_level_tbl_type
   ,p_payment_level_tbl  IN  cashflow_level_tbl_type
   ,x_return_status      OUT NOCOPY VARCHAR2   ) IS

    l_program_name CONSTANT VARCHAR2(61) := G_PKG_NAME||'.'||'populate_fee_details';

    l_fee_rec               fee_rec_type;
    l_expense_header_rec    cashflow_hdr_rec_type;
    l_expense_level_tbl     cashflow_level_tbl_type;
  ...
  BEGIN
    ...
    --bug 4737394 start
    IF l_fee_rec.fee_type = 'INCOME' THEN
        IF (p_payment_level_tbl.COUNT > 0) THEN
            l_fee_rec.fee_amount := 0;
            FOR i IN p_payment_level_tbl.FIRST..p_payment_level_tbl.LAST LOOP
                IF p_payment_level_tbl.exists(i) THEn
                                        l_fee_rec.fee_amount := l_fee_rec.fee_amount + p_payment_level_tbl(i).periodic_amount * p_payment_level_tbl(i).periods;
                END IF;
            END LOOP;
        END IF;
    END IF;
    ...
    IF l_fee_rec.fee_type IN ('ABSORBED', 'INCOME', 'ROLLOVER') THEN
      l_fee_rec.effective_from := l_k_effective_from;
    END IF;

    IF l_fee_rec.fee_type IN ('SEC_DEPOSIT') THEN
      l_fee_rec.fee_amount := p_payment_level_tbl(1).periodic_amount * p_payment_level_tbl(1).periods;
    END IF;


    IF l_fee_rec.fee_type IN ('ABSORBED') THEN
      l_expense_level_tbl(1).periods  :=  1;
      l_expense_level_tbl(1).periodic_amount  :=  l_fee_rec.fee_amount;
      l_expense_header_rec.frequency_code :=  'M';
    END IF;

    IF l_fee_rec.fee_type IN ('EXPENSE' , 'MISCELLANEOUS') THEN
      l_fee_rec.fee_amount := l_expense_level_tbl(1).periodic_amount * l_expense_level_tbl(1).periods;
    END IF;
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      x_return_status := G_RET_STS_UNEXP_ERROR;
  END populate_fee_details;

  -------------------------------
  -- PROCEDURE validate_quote_fee
  -------------------------------
  PROCEDURE validate_quote_fee (p_fee_rec            IN  fee_rec_type,
                                p_link_asset_tbl     IN  line_relation_tbl_type,
                                p_payment_levels_tbl IN  cashflow_level_tbl_type,
                                p_expense_header_rec IN  cashflow_hdr_rec_type,
                                p_expense_levels_tbl IN  cashflow_level_tbl_type,
                                x_return_status      OUT NOCOPY VARCHAR2,
                                x_msg_count          OUT NOCOPY NUMBER,
                                x_msg_data           OUT NOCOPY VARCHAR2) IS

    l_program_name          CONSTANT VARCHAR2(61) := G_PKG_NAME||'.'||'validate_quote_fee';

    l_quote_start_date   DATE;
    l_quote_end_date     DATE;
    l_last_rec_exp_date     DATE;
    l_mpp                   BINARY_INTEGER;
    lx_return_status        VARCHAR2(1);

  BEGIN

    IF p_fee_rec.fee_type IN ('MISCELLANEOUS', 'INCOME', 'SEC_DEPOSIT') AND
       p_payment_levels_tbl.COUNT = 0 THEN

      OKL_API.SET_MESSAGE (p_app_name => G_APP_NAME,
                           p_msg_name => 'OKL_FEETYPE_PAYMENT_REQD');

      RAISE OKL_API.G_EXCEPTION_ERROR;

    END IF;

    IF p_fee_rec.fee_type = 'SEC_DEPOSIT' THEN

      IF (p_payment_levels_tbl.COUNT > 1) OR (p_payment_levels_tbl(p_payment_levels_tbl.FIRST).periods > 1) THEN

        OKL_API.SET_MESSAGE (p_app_name => G_APP_NAME,
                             p_msg_name => 'OKL_SD_PAYMENT_MULTIPLE');

        RAISE OKL_API.G_EXCEPTION_ERROR;

      END IF;

      IF p_payment_levels_tbl(p_payment_levels_tbl.FIRST).stub_days IS NOT NULL OR
         p_payment_levels_tbl(p_payment_levels_tbl.FIRST).stub_amount IS NOT NULL THEN

        OKL_API.SET_MESSAGE (p_app_name => G_APP_NAME,
                             p_msg_name => 'OKL_SD_PAYMENT_STUB');
    ...
    IF p_fee_rec.fee_type IN ('EXPENSE', 'MISCELLANEOUS') THEN

      IF p_expense_header_rec.frequency_code = 'M' THEN
        l_mpp := 1;
      ELSIF p_expense_header_rec.frequency_code = 'Q' THEN
        l_mpp := 3;
      ELSIF p_expense_header_rec.frequency_code = 'S' THEN
        l_mpp := 6;
      ELSIF p_expense_header_rec.frequency_code = 'A' THEN
        l_mpp := 12;
      END IF;

      -- last recurring expense date was not calculated properly
      -- previously we were just adding the moths per period
      -- we should be adding (moths per period*periods)
      l_last_rec_exp_date := ADD_MONTHS(p_fee_rec.effective_from, l_mpp*(p_expense_levels_tbl(1).periods-1));

      IF l_last_rec_exp_date > l_quote_end_date THEN

        OKL_API.SET_MESSAGE (p_app_name => G_APP_NAME,
                             p_msg_name => 'OKL_RECEXP_PAST_K_END');
    ...
    IF p_fee_rec.fee_type IN ('CAPITALIZED', 'FINANCED') THEN

      IF p_link_asset_tbl.COUNT > 0 AND TRUNC(l_quote_start_date) <> TRUNC(p_fee_rec.effective_from) THEN

        OKL_API.SET_MESSAGE (p_app_name     => G_APP_NAME,
                             p_msg_name     => 'OKL_QTE_ASST_FEE_DATE_MISMATCH');
    ...
    /*IF p_fee_rec.fee_type = 'ROLLOVER' THEN

      check_redundant_rollover (p_chr_id        => p_quote_fee_rec.chr_id,
                                p_cle_id        => p_quote_fee_rec.cle_id,
                                p_qte_id        => p_quote_fee_rec.qte_id,
    ...
    IF p_fee_rec.effective_from NOT BETWEEN l_quote_start_date AND l_quote_end_date THEN

      IF p_fee_rec.fee_type IN ('FINANCED', 'CAPITALIZED') THEN

        OKL_API.SET_MESSAGE (p_app_name     => G_APP_NAME,
                             p_msg_name     => 'OKL_QTE_FUNDING_DATE_NA');

        RAISE OKL_API.G_EXCEPTION_ERROR;

      ELSIF p_fee_rec.fee_type IN ('EXPENSE', 'MISCELLANEOUS') THEN

        OKL_API.SET_MESSAGE (p_app_name     => G_APP_NAME,
                             p_msg_name     => 'OKL_QTE_EXPENSE_DATE_NA');

        RAISE OKL_API.G_EXCEPTION_ERROR;

      ELSIF p_fee_rec.fee_type = 'SEC_DEPOSIT' THEN

        OKL_API.SET_MESSAGE (p_app_name     => G_APP_NAME,
                             p_msg_name     => 'OKL_QTE_DEPOSIT_DATE_NA');
  ...
  ------------------------
  -- PROCEDURE get_fee_rec
  ------------------------
  PROCEDURE get_fee_rec (
    p_fee_id                  IN  NUMBER
   ,x_fee_rec                 OUT NOCOPY okl_fee_pvt.feev_rec_type
   ,x_return_status           OUT NOCOPY VARCHAR2
    ) IS

    l_program_name         CONSTANT VARCHAR2(30) := 'get_fee_rec';
    l_api_name             CONSTANT VARCHAR2(61) := G_PKG_NAME||'.'||l_program_name;
  ...
  ---------------------------------
  PROCEDURE validate_link_assets (p_fee_amount       IN NUMBER,
  ...
  BEGIN
    ...
    IF l_amt_flag = 'Y' AND l_assoc_total <> p_fee_amount THEN
      OKL_API.SET_MESSAGE (p_app_name     => G_APP_NAME,
                           p_msg_name     => 'OKL_LINKASSET_AMT_MISMATCH',
                           p_token1       => 'LINE_COST',
                           p_token1_value => p_fee_amount,
  ...
  PROCEDURE process_link_asset_amounts (
   ...
   ,x_msg_data           OUT NOCOPY VARCHAR2
    ...
    CURSOR c_get_parent_object_code IS
  ...
  BEGIN
    ...
    -- Fee Asset amount will be null in case of 'Solve for Financed Amount' pricing
  ...
  PROCEDURE get_deleted_assoc_assets (p_fee_id                  IN  NUMBER,
    ...
    l_assoc_asset_tbl      assoc_asset_tbl_type;
  ...
  PROCEDURE create_line_associations (
    ...
    l_assoc_assets_tbl   assoc_asset_tbl_type;
    lx_assoc_assets_tbl  assoc_asset_tbl_type;
  ...
  BEGIN
    ...
    FOR i IN l_line_relation_tbl.FIRST .. l_line_relation_tbl.LAST LOOP
      IF l_line_relation_tbl.EXISTS(i) THEN
        l_assoc_assets_tbl(i).related_line_id   := p_fee_id;
  ...
  PROCEDURE update_line_associations (
    ...
    l_assoc_create_tbl          assoc_asset_tbl_type;
      ...
      FOR i IN l_line_relation_tbl.FIRST .. l_line_relation_tbl.LAST LOOP
        ...
        IF l_line_relation_tbl.EXISTS(i) THEN
          ...
          ELSIF UPPER(l_line_relation_tbl(i).record_mode) = 'CREATE' THEN
            ...
            l_assoc_create_tbl(i).related_line_id   := l_line_relation_tbl(i).related_line_id;
  ...
  PROCEDURE create_header (
   ...
   ,x_msg_data                OUT NOCOPY VARCHAR2
    ...
    l_fee_rec              okl_fee_pvt.feev_rec_type;
  ...
  BEGIN
    ...
    l_payment_header_rec.parent_object_id := p_fee_id;
  ...
  BEGIN
    ...
    l_expense_header_rec                  := p_expense_header_rec;
    l_expense_header_rec.parent_object_id := p_fee_id;
    l_expense_level_tbl                   := p_expense_level_tbl;
  ...
  PROCEDURE create_fee (
   ...
   ,x_msg_data                OUT NOCOPY VARCHAR2 ) IS
    ...
    l_program_name         CONSTANT VARCHAR2(30) := 'create_fee';
    l_api_name             CONSTANT VARCHAR2(61) := G_PKG_NAME||'.'||l_program_name;
    ...
    l_fee_rec             fee_rec_type;
    l_expense_level_tbl   okl_lease_quote_cashflow_pvt.cashflow_level_tbl_type;
    l_expense_hdr_rec     okl_lease_quote_cashflow_pvt.cashflow_header_rec_type;
    l_p_id                 NUMBER;
  ...
  BEGIN
    ...
    l_fee_rec := p_fee_rec;
    l_expense_level_tbl := p_expense_level_tbl;
    l_expense_hdr_rec := p_expense_header_rec;

    populate_fee_details (p_fee_rec            =>   l_fee_rec,
    ...
    validate_quote_fee (p_fee_rec            => l_fee_rec,
                        p_link_asset_tbl     => p_assoc_asset_tbl,
                        p_payment_levels_tbl => p_payment_level_tbl,
                        p_expense_header_rec => l_expense_hdr_rec,
                        p_expense_levels_tbl => l_expense_level_tbl,
                        x_return_status      => l_return_status,
                        x_msg_count          => x_msg_count,
                        x_msg_data           => x_msg_data);
    IF l_return_status = G_RET_STS_UNEXP_ERROR THEN
    ...
    IF p_assoc_asset_tbl.COUNT > 0 THEN
      ...
      validate_link_assets (
        p_fee_amount       => l_fee_rec.fee_amount
      ...
      IF (l_derive_assoc_amt = 'Y') THEN
        ...
        process_link_asset_amounts (
          p_quote_id           => l_fee_rec.parent_object_id
         ,p_fee_amount         => l_fee_rec.fee_amount
    ...
    IF l_fee_rec.fee_type IN ('INCOME', 'MISCELLANEOUS', 'SEC_DEPOSIT') THEN
  ...
  ------------------------------
  -- PROCEDURE sync_fee_values
  ------------------------------
  PROCEDURE sync_fee_values(x_fee_rec        IN OUT NOCOPY fee_rec_type,
                            p_input_rec      IN fee_rec_type,
                            x_return_status  OUT NOCOPY VARCHAR2) IS

    l_program_name         CONSTANT VARCHAR2(30) := 'sync_fee_values';
    l_api_name             CONSTANT VARCHAR2(61) := G_PKG_NAME||'.'||l_program_name;

  BEGIN

    IF (p_input_rec.rate_card_id IS NOT NULL) THEN
      x_fee_rec.rate_card_id := p_input_rec.rate_card_id;
    END IF;

    IF (p_input_rec.rate_template_id IS NOT NULL) THEN
      x_fee_rec.rate_template_id := p_input_rec.rate_template_id;
    END IF;

    IF (p_input_rec.structured_pricing IS NOT NULL) THEN
      x_fee_rec.structured_pricing := p_input_rec.structured_pricing;
    END IF;

    IF (p_input_rec.target_arrears IS NOT NULL) THEN
      x_fee_rec.target_arrears := p_input_rec.target_arrears;
    END IF;

    IF (p_input_rec.lease_rate_factor IS NOT NULL) THEN
      x_fee_rec.lease_rate_factor := p_input_rec.lease_rate_factor;
    END IF;
  ...
  PROCEDURE update_fee (
   ...
   ,x_msg_data                OUT NOCOPY VARCHAR2 ) IS
    ...
    l_line_relation_tbl   assoc_asset_tbl_type;
    lx_line_relation_tbl  assoc_asset_tbl_type;
    l_p_id                 NUMBER;
    l_p_code               VARCHAR2(30);
  ...
  BEGIN
    ...
    get_fee_rec (p_fee_id        => p_fee_rec.id
     			,x_fee_rec       => l_fee_rec
			    ,x_return_status => x_return_status);
    IF x_return_status = G_RET_STS_UNEXP_ERROR THEN
      RAISE OKL_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ...
    IF (p_sync_fee_header = 'Y') THEN
      -- Sync Fee Info
      sync_fee_values(x_fee_rec       => l_fee_rec,
                      p_input_rec     => p_fee_rec,
                      x_return_status => x_return_status);
    ...
    IF (p_sync_line_relations = 'Y') THEN
      get_line_relations_tbl (p_fee_id            => l_fee_rec.id
  ...
  BEGIN
    ...
    l_fee_id   := l_fee_rec.id;
    l_fee_type := l_fee_rec.fee_type;
  ...
  -------------------------------
  PROCEDURE get_lrship_tbl (p_source_fee_id           IN  NUMBER
  						   ,p_target_fee_id			  IN  NUMBER
  ...
  BEGIN
    FOR l_db_lrships IN c_db_lrships LOOP
      ...
      x_lrship_tbl(i).related_line_id := p_target_fee_id;
  ...
  -----------------------------------
  PROCEDURE copy_line_associations( p_source_fee_id   IN NUMBER,
  									p_target_fee_id   IN NUMBER,
				            		x_return_status   OUT NOCOPY VARCHAR2,
    ...
    ln_target_quote_id  NUMBER;

  CURSOR c_get_new_asset_id (p_source_asset_id IN NUMBER,
                             p_target_quote_id In NUMBER) IS
  SELECT id
  FROM OKL_ASSETS_B
  ...
  BEGIN
    ...
    get_lrship_tbl (p_source_fee_id    => p_source_fee_id,
    				p_target_fee_id    => p_target_fee_id,
      ...
      FOR i IN l_lrship_tbl.FIRST .. l_lrship_tbl.LAST LOOP
        IF l_lrship_tbl.EXISTS(i) THEN
          OPEN c_get_new_asset_id(p_source_asset_id	 => l_lrship_tbl(i).source_line_id,
                                  p_target_quote_id => ln_target_quote_id);
  ...
  PROCEDURE duplicate_fee (
   ...
   ,x_msg_data                OUT NOCOPY VARCHAR2 ) IS
    ...
    l_program_name         CONSTANT VARCHAR2(30) := 'duplicate_fee';
    l_api_name             CONSTANT VARCHAR2(61) := G_PKG_NAME||'.'||l_program_name;

    l_line_relation_tbl    line_relation_tbl_type;
    lx_line_relation_tbl   line_relation_tbl_type;
  ...
  BEGIN
    ...
    -- Check if the Source and Target Quote's Pricing type are equal and if it
    -- is a Configuration fee, cash flows are not copied.
    IF l_fee_rec.fee_type IN ('FINANCED', 'CAPITALIZED', 'ROLLOVER') THEN
      lb_config_fee := TRUE;
    END IF;
    --
    SELECT expected_start_date,
    ...
    --
    IF (lb_config_fee) THEN
      lv_pricing_type_equal := is_pricing_method_equal(p_source_quote_id => l_fee_rec.parent_object_id,
      ...
      IF (l_parent_object_code <> 'LEASEAPP' AND lv_pricing_type_equal = 'N') THEN
		...
		l_fee_rec.target_frequency := null;
		l_fee_rec.target_amount := null;
		--Bug # 5021937 start
		--Not nullfying all Pricing Parameters
	    l_fee_rec.rate_card_id := null;
        l_fee_rec.rate_template_id := null;
        l_fee_rec.payment_type_id := null;
        --Bug # 5021937 end
      END IF;
    END IF;
    -- End

	-- Assign the target quote id to parent object
    l_fee_rec.parent_object_id := p_target_quote_id;
    l_fee_rec.id := null;
    ...
    IF (lb_copy_lr) THEN
      copy_line_associations( p_source_fee_id	  => p_source_fee_id,
    					      p_target_fee_id   => x_fee_id,
				              x_return_status   => x_return_status,
					          x_msg_count       => x_msg_count,
					          x_msg_data        => x_msg_data );
      IF x_return_status = G_RET_STS_UNEXP_ERROR THEN
    ...
    IF (lb_copy_cashflow) THEN
     ...
     ,p_source_object_id     => p_source_fee_id
     ,p_target_object_id     => x_fee_id
  ...
  PROCEDURE delete_fee (
   ...
   ,x_msg_data                OUT NOCOPY VARCHAR2
    ...
    CURSOR c_fee_sublines (p_fee_type VARCHAR2) IS
      SELECT id
      FROM   okl_line_relationships_b
      WHERE  related_line_type = p_fee_type
      AND    related_line_id = p_fee_id;
  ...
  BEGIN
    ...
    i := 0;
    FOR l_assoc_assets IN c_fee_sublines (p_fee_type => l_fee_type) LOOP
      l_assoc_asset_tbl(i).id := l_assoc_assets.id;
      i := i + 1;
    END LOOP;

    IF l_assoc_asset_tbl.COUNT > 0 THEN
