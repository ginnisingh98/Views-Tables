--------------------------------------------------------
--  DDL for Package Body PAY_SA_ARCHIVE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_SA_ARCHIVE" AS
/* $Header: pysaparc.pkb 120.6.12010000.6 2010/03/29 07:10:26 rbabla ship $ */
			...
			g_package           VARCHAR2(33) := '  PAY_SA_ARCHIVE.';
			...
			PAY_SA_ARCHIVE.GET_ALL_PARAMETERS(p_payroll_action_id
					,l_business_group_id
					,l_start_date
					,l_end_date
					,l_effective_date
			...
			/*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS');*/
			 ...
			 /*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS 2');*/
			  ...
			  /*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS 3');*/
				...
				l_city := null;
				IF rec_primary_address.style = 'SA' THEN
					l_city:= hr_general.decode_lookup('SA_CITY',rec_primary_address.city);
				ELSE
					l_city:= rec_primary_address.city;
				END IF;

				l_country:=PAY_SA_ARCHIVE.get_country_name(rec_primary_address.country);
				l_emp_city := null;
				IF rec_employer_address.style = 'SA' THEN
					l_emp_city:= hr_general.decode_lookup('SA_CITY',rec_employer_address.city);
				ELSE
					l_emp_city:= rec_employer_address.city;
				END IF;

				l_emp_country:=PAY_SA_ARCHIVE.get_country_name(rec_employer_address.country);
				l_org_city := null;
				IF rec_org_address.style = 'SA' THEN
					--l_org_city:= hr_general.decode_lookup('SA_CITY',rec_employer_address.city);
					l_org_city:= hr_general.decode_lookup('SA_CITY',rec_org_address.city);
				ELSE
					--l_org_city:= rec_employer_address.city;
					l_org_city:= rec_org_address.city;
				END IF;

				l_org_country:=PAY_SA_ARCHIVE.get_country_name(rec_org_address.country);



				/*fnd_file.put_line(fnd_file.log,'Entering Procedure ARCHIVE_EMPLOYEE_DETAILS 6');*/

				/* Get Value for GOSI Reference Salary*/
				...
				l_defined_balance_id := PAY_SA_ARCHIVE.GET_DEFINED_BALANCE_ID('GOSI_REFERENCE_EARNINGS_ASG_YTD');
				l_balance_value := pay_balance_pkg.get_value(l_defined_balance_id, p_assignment_action_id);

				--Check for lower and upper limits of the reference salary


				IF(l_balance_value > to_number(l_upper_base)) THEN
					l_balance_value := to_number(l_upper_base);
				ELSIF(l_balance_value < to_number(l_lower_base)) THEN
					l_balance_value := to_number(l_lower_base);
				END IF;
				...
				pay_action_information_api.create_action_information (
				 ...
				 pay_action_information_api.create_action_information (
					 ...
					 ,p_action_information4          => l_nationality
					 ,p_action_information5          => l_cost_center
					 ,p_action_information6          => l_top_org_id
					 ,p_action_information7          => l_currency
					 ,p_action_information8          => rec_employer_address.AL1
					 ,p_action_information9          => rec_person_details.title         -- for payroll register
					 ...
					 ,p_action_information13         => l_formatted_value                -- GOSI Reference Salary
				...
				pay_action_information_api.create_action_information (
							 ...
							 ,p_action_information8          => l_org_city
			...
			/* TO ARCHIVE MAIN ELEMENTS IN CONTEXT EMEA ELEMENT INFO */
			/* New Procedure added for archiving the run result values Bug No : 7648285 */
			------------------------------------------------------------------------------------------------------------
			...
			CURSOR csr_result_value(p_run_assact_id NUMBER,p_payroll_action_id NUMBER) IS
			SELECT rrv.result_value , pai.action_information2, pai.action_information3,pai.action_information5
				FROM   pay_run_result_values rrv
				      ,pay_run_results rr
					 ,pay_assignment_actions paa
					   ,pay_payroll_actions ppa
					   ,pay_action_information pai
			WHERE  rrv.input_value_id = pai.action_information3
			AND    rr.element_type_id = pai.action_information2
			AND    rr.run_result_id = rrv.run_result_id
			AND    rr.assignment_action_id = paa.assignment_action_id
			AND    paa.assignment_action_id = p_run_assact_id
			...
			AND    pai.action_context_id = p_payroll_action_id
			 and pai.action_information6 = 'M'
			  and pai.action_information5 in ('E','D'); /* only check for Earning and deduction elements */
			rec_result_value csr_result_value%ROWTYPE;

			l_result_value		pay_run_result_values.result_value%TYPE := 0;
			l_run_result_id         NUMBER := 0;
			l_action_info_id	NUMBER;
			l_ovn			NUMBER;
			...
			IF g_debug THEN
				hr_utility.set_location(' Archivng earning and deduction elements',325);
			END IF;

			-- Archiving Earnings and deduction Elements

			l_result_value := NULL;

		         OPEN csr_result_value(p_run_assact_id,p_payroll_action_id);
                          LOOP
                            FETCH csr_result_value INTO rec_result_value;
                            EXIT WHEN csr_result_value%NOTFOUND;
                            l_result_value := rec_result_value.result_value;
                            IF  l_result_value is not null THEN
                              ...
                              ,p_action_information1          => rec_result_value.action_information2
                              ,p_action_information2          => rec_result_value.action_information3
                              ,p_action_information3          => rec_result_value.action_information5
			...
			EXIT WHEN csr_chk%NOTFOUND;
				...
				l_bank_name := hr_general.decode_lookup('SA_BANKS',rec_pers_pay.name_id);
            ...
            -- Bug 6915966
            -- Fetch and archive the accruals cursor in loop
             FOR c_leave_bal In csr_leave_balance
             Loop
    			l_accrual_plan_name := c_leave_bal.accrual_plan_name ;
				l_accrual_category  := c_leave_bal.accrual_catgry ;
				l_accrual_uom       := c_leave_bal.accrual_units_of_measure ;
				l_payroll_id        := c_leave_bal.payroll_id ;
				l_business_group_id := c_leave_bal.business_group_id ;
				l_accrual_plan_id   := c_leave_bal.accrual_plan_id ;
			...
			---------------------------------------------------
			  ...
			  LOOP
				...
				PAY_SA_ARCHIVE.SETUP_BALANCE_DEFINITIONS(p_balance_name         => rec_get_balance.narrative
			...
			END IF;
			  ...
			  FOR rec_archive_ids IN csr_archive_ids(p_assignment_action_id) LOOP
			   ...
			   ARCHIVE_ADD_ELEMENT(p_archive_assact_id     => p_assignment_action_id,
			    ...
			    /*fnd_file.put_line(fnd_file.log,'ARCHIVE_MAIN_ELEMENTS');*/
			    /* Bug No : 7648285 */
			   ARCHIVE_MAIN_ELEMENTS (p_archive_assact_id     => p_assignment_action_id,
						  p_run_assact_id  => rec_archive_ids.run_assact_id,
						  p_assignment_id         => rec_archive_ids.run_assgt_id,
						  p_payroll_action_id     => l_archive_payact_id,
						  p_effective_date        => p_effective_date ) ;
