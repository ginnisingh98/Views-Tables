--------------------------------------------------------
--  DDL for Package Body PQP_ERT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQP_ERT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqp_ert_bus.';  -- Global package name
--
...
Begin
  ...
  --
  --
  if (( nvl(pqp_ert_bus.g_exception_report_id, hr_api.g_number)
       = p_exception_report_id)
  and ( nvl(pqp_ert_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pqp_ert_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pqp_ert_bus.g_exception_report_id         := p_exception_report_id;
    pqp_ert_bus.g_language                    := p_language;
    pqp_ert_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pqp_ert_shd.api_updating
...
Begin
  ...
  --
  --
  --set_security_group_id(p_rec.exception_report_id);
  --
