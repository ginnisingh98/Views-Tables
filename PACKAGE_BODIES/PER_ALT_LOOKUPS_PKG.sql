--------------------------------------------------------
--  DDL for Package Body PER_ALT_LOOKUPS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ALT_LOOKUPS_PKG" AS
/*$Header: peraltlk.pkb 115.1 2002/12/05 17:25:02 eumenyio noship $*/
--
...
--
l_procedure := 'Procedure run_process in the package per_alt_lookups_pkg';
--
...
begin
  hr_utility.set_location('perwsalc - Clean_pay_tables ', 1);
  ...
  If (Get_pay_user_tables_id%FOUND) then
     Begin
        hr_utility.set_location('perwsalc - Clean_pay_tables', 5);
        ...
        hr_utility.set_location('perwsalc - delete pay_user_row, column and table rows' , 20);
  ...
  hr_utility.set_location('perwsalc - End Clean_pay_tables', 50);
end;
...
Begin
  ...
  --
  hr_utility.set_location('perwsalc - set_instance_value - Entering',1);
  OPEN Get_pay_user_tables_id (cp_user_table_name,cn_business_group_id);
  FETCH Get_pay_user_tables_id into ln_user_table_id;
  if (Get_pay_user_tables_id%NOTFOUND) Then
     hr_utility.set_location('perwsalc - set_instance_value - unknown table_name : ' || cp_user_table_name, 5);
  Else
     ...
     if (Get_column_id%NOTFOUND) Then
        hr_utility.set_location('perwsalc - set_instance_value - unknown column_name : ' || cp_user_column_name, 10);
     Else
        ...
        if (ln_count_row_id = 0) Then
           hr_utility.set_location('perwsalc - set_instance_value - unknown row_name : ' || cp_row_name, 15);
	            ...
	            -- Check if we have more than one column (different effective dates) for this instance
                    ...
                    Else
                       Begin
                          ...
                          EXCEPTION
                          WHEN OTHERS Then
                             hr_utility.set_location ('perwsalc - set_instance_value - unable to update row into PAY_USER_COLUMN_INSTANCES_F '|| to_char(ln_column_instance_id), 20);
                       End;
                    End if;
                 End;
              end if;
  ...
  hr_utility.set_location('perwsalc - Set_instance_value - Leaving',30);
...
Begin
   ...
   hr_utility.set_location('per_alt_lookups_pkg - Entering update_instance_value', 1);
   hr_utility.set_location('--------------------------------------------------',1);
  ...
  hr_utility.set_location('perwsalc - Updte_Instance_value - Leaving ..',30);
End;
--
