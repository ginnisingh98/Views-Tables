--------------------------------------------------------
--  DDL for Package Body BEN_PCG_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PCG_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pcg_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< delete_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure delete_dml(p_rec in ben_pcg_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Delete the ben_prtt_clm_gd_or_svc_typ row.
  --
...
Procedure chk_reimbmt_status(p_prtt_reimbmt_rqst_id  in number ) is
--
  l_proc  varchar2(72) := g_package||'chk_reimbmt_status';
  cursor c_prc is
   select   prtt_reimbmt_rqst_stat_cd
   from   ben_prtt_reimbmt_rqst_f
   where prtt_reimbmt_rqst_id = p_prtt_reimbmt_rqst_id
   and prtt_reimbmt_rqst_stat_cd in ('APPRVD','VOIDED','PDINFL','PRTLYPD') ;

   l_prtt_reimbmt_rqst_stat_cd ben_prtt_reimbmt_rqst_f.prtt_reimbmt_rqst_stat_cd%TYPE; -- UTF8 varchar2(15) ;
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  open c_prc ;
  fetch c_prc into l_prtt_reimbmt_rqst_stat_cd ;
  if c_prc%found then
    close c_prc ;
    if l_prtt_reimbmt_rqst_stat_cd in ( 'APPRVD','PDINFL','PRTLYPD') then
         fnd_message.set_name('BEN','BEN_92705_REIMB_RQST_APPROVD');
         fnd_message.raise_error;
    end if ;
    if l_prtt_reimbmt_rqst_stat_cd = 'VOIDED' then
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_delete(p_rec in ben_pcg_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_delete(p_rec in ben_pcg_shd.g_rec_type) is
--
...
Procedure del
  (
  p_rec	      in ben_pcg_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  ben_pcg_bus.delete_validate(p_rec);
  --
  -- Call the supporting pre-delete operation
  --
  chk_reimbmt_Status ( p_rec.prtt_reimbmt_rqst_id ) ;
  pre_delete(p_rec);
  --
  -- Delete the row.
  --
