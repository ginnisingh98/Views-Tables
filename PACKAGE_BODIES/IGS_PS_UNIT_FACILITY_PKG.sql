--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_FACILITY_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_FACILITY_PKG" AS
/* $Header: IGSPI0MB.pls 115.8 2003/03/21 07:59:22 sarakshi ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_ps_unit_facility%RowType;
  new_references igs_ps_unit_facility%RowType;

  PROCEDURE Set_Column_Values (
    p_action IN VARCHAR2,
    x_rowid IN VARCHAR2 DEFAULT NULL,
    x_unit_media_id IN NUMBER DEFAULT NULL,
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.unit_media_id := x_unit_media_id;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (((old_references.media_code = new_references.media_code)) OR
        ((new_references.media_code IS NULL))) THEN
      NULL;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_unit_facility
      WHERE    unit_media_id = x_unit_media_id
      FOR UPDATE NOWAIT;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UF_ME_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UF_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
    ...
    IF (p_action = 'INSERT') THEN
      ...
      Null;
	     IF Get_Pk_For_Validation(
    		new_references.unit_media_id)  THEN
	       Fnd_Message.Set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
      IGS_GE_MSG_STACK.ADD;
	       App_Exception.Raise_Exception;
	     END IF;
 ...
 procedure INSERT_ROW (
      X_ROWID in out NOCOPY VARCHAR2,
       x_UNIT_MEDIA_ID IN OUT NOCOPY NUMBER,
       x_UNIT_CODE IN VARCHAR2,
       x_UNIT_VERSION_NUMBER IN NUMBER,
       x_MEDIA_CODE IN VARCHAR2,
  ...
  ***************************************************************/
    ...
    cursor C is select ROWID from IGS_PS_UNIT_FACILITY
             where                 UNIT_MEDIA_ID= X_UNIT_MEDIA_ID
;
 ...
 begin
   ...
   Select IGS_PS_UNIT_FACILITY_S.NEXTVAL into x_unit_media_id from dual;
   Before_DML(
 		p_action=>'INSERT',
 		x_rowid=>X_ROWID,
 	       x_unit_media_id=>X_UNIT_MEDIA_ID,
     ...
     insert into IGS_PS_UNIT_FACILITY (
		UNIT_MEDIA_ID
		,UNIT_CODE
		,UNIT_VERSION_NUMBER
		,MEDIA_CODE
...
if ( (  tlinfo.UNIT_CODE = X_UNIT_CODE)
  ...
  AND (tlinfo.MEDIA_CODE = X_MEDIA_CODE)
  ) then
    null;
  else
  ...
  ***************************************************************/
    ...
    cursor c1 is select ROWID from IGS_PS_UNIT_FACILITY
             where     UNIT_MEDIA_ID= X_UNIT_MEDIA_ID
;
begin
	open c1;
...
x_rowid => X_ROWID
 ...
 delete from IGS_PS_UNIT_FACILITY
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
