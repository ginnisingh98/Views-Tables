--------------------------------------------------------
--  DDL for Package Body PER_RVT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_RVT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_rvt_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_workbench_view_report_code  varchar2(60)   default null;
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'WORKBENCH_VIEW_REPORT_CODE')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  --
  if (( nvl(per_rvt_bus.g_workbench_view_report_code, hr_api.g_varchar2)
       = p_workbench_view_report_code)
  and ( nvl(per_rvt_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := per_rvt_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    per_rvt_bus.g_workbench_view_report_code  := p_workbench_view_report_code;
    per_rvt_bus.g_language                    := p_language;
    per_rvt_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT per_rvt_shd.api_updating
