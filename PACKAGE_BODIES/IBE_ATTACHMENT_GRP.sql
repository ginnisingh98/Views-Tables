--------------------------------------------------------
--  DDL for Package Body IBE_ATTACHMENT_GRP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IBE_ATTACHMENT_GRP" AS
/* $Header: IBEGATHB.pls 120.1 2005/11/17 15:55:21 szou noship $ */
...
PROCEDURE list_attachment (
  ...
  x_ath_id_tbl             OUT NOCOPY NUMBER_TABLE,
  x_dlv_id_tbl             OUT NOCOPY NUMBER_TABLE,
  x_file_name_tbl          OUT NOCOPY VARCHAR2_TABLE_300,

  --added by G. Zhang 04/30/2001 11:18AM
  x_file_id_tbl		OUT  NOCOPY NUMBER_TABLE,
  x_file_ext_tbl		OUT	NOCOPY VARCHAR2_TABLE_20,
  x_dsp_width_tbl		OUT 	NOCOPY NUMBER_TABLE,
  x_dsp_height_tbl	OUT 	NOCOPY NUMBER_TABLE,

  x_version_tbl            OUT  NOCOPY NUMBER_TABLE ) IS
...
PROCEDURE save_attachment (
  ...
  x_msg_count			OUT	NOCOPY NUMBER,
  x_msg_data			OUT	NOCOPY VARCHAR2,
  p_attachment_rec		IN OUT NOCOPY ATTACHMENT_REC_TYPE ) IS
...
-- Modified by G. Zhang 04/30/2001 11:18AM
  ...
  ELSE
  	l_act_attachment_rec.FILE_ID := NULL;
  END IF;
  l_act_attachment_rec.SECURED_FLAG := p_attachment_rec.SECURED_FLAG;  --bug 2633722
  ...
  --added as jtf_amv_attachment_pub requires this if deliverable type
  --is MEDIA.
  If( l_act_attachment_rec.DISPLAY_URL is NULL) Then
    l_act_attachment_rec.DISPLAY_URL := 'not in use';
  End If;
  ...
  --IF NOT IBE_DSPMGRVALIDATION_GRP.check_attachment_filename(
...
PROCEDURE save_attachment (
  ...
  x_msg_count              OUT  NOCOPY NUMBER,
  x_msg_data               OUT  NOCOPY VARCHAR2,
  p_attachment_tbl         IN OUT NOCOPY ATTACHMENT_TBL_TYPE ) IS
...
BEGIN
  ...
  IF NOT IBE_DSPMGRVALIDATION_GRP.check_attachment_exists(
