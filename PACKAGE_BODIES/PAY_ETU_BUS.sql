--------------------------------------------------------
--  DDL for Package Body PAY_ETU_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ETU_BUS" as
/* $Header: pyeturhi.pkb 120.1 2005/11/08 04:51:49 pgongada noship $ */
--
-- ----------------------------------------------------------------------------
-- |                     Private Global Definitions                           |
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_etu_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_element_type_usage_id       number         default null;
--
--  ---------------------------------------------------------------------------
...
begin
  ...
  hr_api.mandatory_arg_error
    (p_api_name           => l_proc
    ,p_argument           => 'element_type_usage_id'
    ,p_argument_value     => p_element_type_usage_id
    );
  --
  open csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(pay_etu_bus.g_element_type_usage_id, hr_api.g_number)
       = p_element_type_usage_id) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := pay_etu_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_etu_bus.g_element_type_usage_id:= p_element_type_usage_id;
    pay_etu_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_etu_shd.api_updating
  ...
  if nvl(p_rec.business_group_id, hr_api.g_number) <>
     nvl(pay_etu_shd.g_old_rec.business_group_id, hr_api.g_number) then
     l_argument := 'business_group_id';
     raise l_error;
  end if;
  hr_utility.set_location(l_proc, 15);
  --
  if nvl(p_rec.element_type_usage_id,hr_api.g_number) <>
     nvl(pay_etu_shd.g_old_rec.element_type_usage_id,hr_api.g_number) then
     l_argument := 'element_type_usage_id';
     raise l_error;
  end if;
  hr_utility.set_location(l_proc, 20);
  --
  if nvl(p_rec.legislation_code,hr_api.g_varchar2) <>
     nvl(pay_etu_shd.g_old_rec.legislation_code,hr_api.g_varchar2) then
  ...
  if nvl(p_rec.element_type_id, hr_api.g_number) <>
     nvl(pay_etu_shd.g_old_rec.element_type_id, hr_api.g_number) then
  ...
  if nvl(p_rec.run_type_id,hr_api.g_number) <>
     nvl(pay_etu_shd.g_old_rec.run_type_id,hr_api.g_number) then
...
/*  if nvl(p_rec.inclusion_flag,hr_api.g_varchar2) <>
     nvl(pay_etu_shd.g_old_rec.inclusion_flag,hr_api.g_varchar2) then
     l_argument := 'inclusion_flag';
     raise l_error;
  end if;
  hr_utility.set_location(l_proc, 40);
  --
  if nvl(p_rec.usage_type,hr_api.g_varchar2) <>
     nvl(pay_etu_shd.g_old_rec.usage_type,hr_api.g_varchar2) then
     l_argument := 'usage_type';
     raise l_error;
  end if;*/
...
l_legislation_code pay_element_type_usages_f.legislation_code%TYPE := Null;
l_exists  varchar2(1);
--
...
--
  IF (((p_element_type_usage_id is not null) and
     nvl(pay_etu_shd.g_old_rec.run_type_id, hr_api.g_number) <>
     nvl(p_run_type_id, hr_api.g_number)) or
     (p_element_type_usage_id is null)) THEN
     --
     hr_utility.set_location(l_proc, 10);
     --
     ...
     --
       ...
       ELSIF hr_startup_data_api_support.g_startup_mode = 'USER' THEN
         hr_utility.set_location(l_proc, 25);
         --
         IF p_element_type_usage_id is not null THEN
         l_legislation_code := pay_etu_bus.return_legislation_code(p_element_type_usage_id);
         ELSE
...
l_exists  varchar2(1);
--
CURSOR csr_chk_user_element_type(p_leg_code varchar2) is
select 'Y'
...
--
CURSOR csr_chk_startup_element_type is
select 'Y'
from   pay_element_types_f pet,
       pay_run_types_f rty
where  pet.element_type_id = p_element_type_id
...
--
CURSOR csr_chk_generic_element_type is
select 'Y'
from   pay_element_types_f pet,
       pay_run_types_f rty
where  pet.element_type_id = p_element_type_id
...
--
-- Only execute the cursor if absolutely necessary.
-- a) During update, the element_type_id has actually changed to another not
--    null value, i,e, the value passed to this procedure is different to the
--    g_old_rec value.
-- b) During insert, the element_type_id is null.
-- Can tell the difference between insert and update by looking at the
-- primary key value. For update it will be not null. For insert it will be
-- null because pre_insert has not been called yet.
--
  IF (((p_element_type_usage_id is not null) and
     nvl(pay_etu_shd.g_old_rec.element_type_id, hr_api.g_number) <>
     nvl(p_element_type_id, hr_api.g_number)) or
     (p_element_type_usage_id is null)) THEN
     --
     hr_utility.set_location(l_proc, 10);
     --
     -- Only need to open the cursor if element_type_id is not null
     --
     ...
     --
       ...
       hr_utility.set_location(l_proc, 15);
       --
         OPEN csr_chk_generic_element_type;
         FETCH csr_chk_generic_element_type INTO l_exists;
           IF csr_chk_generic_element_type%NOTFOUND THEN
           --
             CLOSE csr_chk_generic_element_type;
             hr_utility.set_message(801, 'HR_33588_ETU_INVAL_ET_FOR_MODE');
             hr_utility.raise_error;
             --
           END IF;
         CLOSE csr_chk_generic_element_type;
       ...
       ELSIF hr_startup_data_api_support.g_startup_mode = 'STARTUP' THEN
         hr_utility.set_location(l_proc, 20);
         --
         OPEN  csr_chk_startup_element_type;
         FETCH csr_chk_startup_element_type INTO l_exists;
           IF csr_chk_startup_element_type%NOTFOUND THEN
           --
             CLOSE csr_chk_startup_element_type;
             hr_utility.set_message(801, 'HR_33588_ETU_INVAL_ET_FOR_MODE');
             hr_utility.raise_error;
           END IF;
         CLOSE csr_chk_startup_element_type;
       ...
       ELSIF hr_startup_data_api_support.g_startup_mode = 'USER' THEN
         ...
         END IF;
         --
	 OPEN  csr_chk_user_element_type(l_legislation_code);
         FETCH csr_chk_user_element_type INTO l_exists;
           IF csr_chk_user_element_type%NOTFOUND THEN
           --
             CLOSE csr_chk_user_element_type;
	     hr_utility.set_message(801, 'HR_33590_ETU_INVALID_ELE_TYPE');
             hr_utility.raise_error;
           END IF;
         CLOSE csr_chk_user_element_type;
       --
...
Procedure chk_inclusion_flag
  ...
  ,p_inclusion_flag        in varchar2) IS
--
...
Begin
--
-- As inclusion_flag is validated against the standard lookup 'YES_NO' users
-- will not be able to add new lookup_codes. Thus only need to validate against
...
--
  ...
  THEN
  --
  -- The inclusion_flag for this record is invalid
  --
    fnd_message.set_name('PAY','HR_33585_ETU_INCLUSION_FLAG_N');
    fnd_message.raise_error;
    --
    hr_utility.set_location(l_proc,10);
  END IF;
...
Begin
  ...
  hr_utility.set_location(l_proc, 10);
  --
  chk_run_type_id(p_element_type_usage_id => p_rec.element_type_usage_id
  ...
  hr_utility.set_location(l_proc, 15);
  --
  chk_element_type_id(p_element_type_usage_id => p_rec.element_type_usage_id
  ...
  hr_utility.set_location(l_proc, 20);
  --
  chk_inclusion_flag(p_effective_date        => p_effective_date
                    ,p_validation_start_date => p_validation_start_date
                    ,p_validation_end_date   => p_validation_end_date
...
Begin
  ...
  --
  chk_startup_action(False
                    ,pay_etu_shd.g_old_rec.business_group_id
                    ,pay_etu_shd.g_old_rec.legislation_code);
  --
  dt_delete_validate
    (p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => p_validation_start_date
