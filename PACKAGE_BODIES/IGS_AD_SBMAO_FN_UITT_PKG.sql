--------------------------------------------------------
--  DDL for Package Body IGS_AD_SBMAO_FN_UITT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_SBMAO_FN_UITT_PKG" as
/* $Header: IGSAI62B.pls 115.6 2003/10/30 13:21:20 rghosh ship $ */
l_rowid VARCHAR2(25);
old_references IGS_AD_SBMAO_FN_UITT%RowType;
new_references IGS_AD_SBMAO_FN_UITT%RowType;
...
PROCEDURE BeforeRowInsertUpdate1(
  ...
  BEGIN
	...
	IF p_inserting OR
	    (old_references.unit_int_course_level_cd <> new_references.unit_int_course_level_cd) THEN
		IF IGS_AD_VAL_SAFT.crsp_val_uicl_closed(
...
PROCEDURE Check_Parent_Existance AS
  ...
  FUNCTION Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      ...
      AND      unit_int_course_level_cd = x_unit_int_course_level_cd
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFUT_FS_FK');
IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFUT_GSC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFUT_ITT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFUT_OU_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFUT_UICL_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_SBMAO_FN_UITT
      where SUBMISSION_YR = X_SUBMISSION_YR
      and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
      and ORG_UNIT_CD = X_ORG_UNIT_CD
...
begin
  ...
  insert into IGS_AD_SBMAO_FN_UITT (
    SUBMISSION_YR,
    SUBMISSION_NUMBER,
    ORG_UNIT_CD,
    OU_START_DT,
...
begin
  ...
  Before_DML (
    ...
    update IGS_AD_SBMAO_FN_UITT set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_SBMAO_FN_UITT
     where SUBMISSION_YR = X_SUBMISSION_YR
     and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
     and ORG_UNIT_CD = X_ORG_UNIT_CD
     and OU_START_DT = X_OU_START_DT
...
begin
 ...
 delete from IGS_AD_SBMAO_FN_UITT
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
