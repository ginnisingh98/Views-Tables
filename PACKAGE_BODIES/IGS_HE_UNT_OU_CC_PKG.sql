--------------------------------------------------------
--  DDL for Package Body IGS_HE_UNT_OU_CC_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_HE_UNT_OU_CC_PKG" AS
/* $Header: IGSWI46B.pls 120.0 2005/06/02 04:23:29 appldev noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_he_unt_ou_cc%ROWTYPE;
  new_references igs_he_unt_ou_cc%ROWTYPE;

  PROCEDURE set_column_values (
    p_action                            IN     VARCHAR2,
    x_rowid                             IN     VARCHAR2,
    x_hesa_unit_cc_id                   IN     NUMBER,
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.hesa_unit_cc_id                   := x_hesa_unit_cc_id;
  ...
  */
  BEGIN
    ...
    IF (((old_references.unit_cd = new_references.unit_cd) AND
      ...
      NULL;
    ELSIF NOT igs_ps_tch_resp_pkg.get_pk_for_validation (
                new_references.unit_cd,
                new_references.version_number,
                new_references.org_unit_cd
              ) THEN
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_he_unt_ou_cc
      WHERE    hesa_unit_cc_id = x_hesa_unit_cc_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  PROCEDURE get_fk_igs_ps_tch_resp (
    x_unit_cd                           IN     VARCHAR2,
    x_version_number                    IN     NUMBER,
    x_org_unit_cd                       IN     VARCHAR2
  ) AS
  ...
  */
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.hesa_unit_cc_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  PROCEDURE insert_row (
    ...
    x_hesa_unit_cc_id                   IN OUT NOCOPY NUMBER,
    x_unit_cd                           IN     VARCHAR2,
    x_version_number                    IN     NUMBER,
    x_org_unit_cd                       IN     VARCHAR2,
    x_cost_centre                       IN     VARCHAR2,
  ...
  */
  ...
  BEGIN
    ...
    ELSE
      ...
      fnd_message.set_token ('ROUTINE', 'IGS_HE_UNT_OU_CC_PKG.INSERT_ROW');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
    END IF;

    x_hesa_unit_cc_id := NULL;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_hesa_unit_cc_id                   => x_hesa_unit_cc_id,
      x_unit_cd                           => x_unit_cd,
    ...
    INSERT INTO igs_he_unt_ou_cc (
      ...
      x_last_updated_by,
      x_last_update_login
    ) RETURNING ROWID, hesa_unit_cc_id INTO x_rowid, x_hesa_unit_cc_id;
  ...
  */
  ...
  BEGIN
    ...
    ELSE
      fnd_message.set_name( 'FND', 'SYSTEM-INVALID ARGS');
      fnd_message.set_token ('ROUTINE', 'IGS_HE_UNT_OU_CC_PKG.UPDATE_ROW');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
    END IF;
