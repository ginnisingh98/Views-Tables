--------------------------------------------------------
--  DDL for Package Body PAY_PUR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PUR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_pur_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the pay_user_rows_f Row
    --
    ...
    ,row_low_range_or_name                = p_rec.row_low_range_or_name
    ,display_sequence                     = p_rec.display_sequence
    ,row_high_range                       = p_rec.row_high_range
    ,object_version_number                = p_rec.object_version_number
    where   user_row_id = p_rec.user_row_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    pay_pur_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_base_key_value         => p_rec.user_row_id
...
Begin
  ...
  begin
    ...
    pay_pur_rku.after_update
      ...
      => pay_pur_shd.g_old_rec.effective_end_date
      ,p_business_group_id_o
      => pay_pur_shd.g_old_rec.business_group_id
      ,p_legislation_code_o
      => pay_pur_shd.g_old_rec.legislation_code
      ,p_user_table_id_o
      => pay_pur_shd.g_old_rec.user_table_id
      ,p_row_low_range_or_name_o
      => pay_pur_shd.g_old_rec.row_low_range_or_name
      ,p_display_sequence_o
      => pay_pur_shd.g_old_rec.display_sequence
      ,p_row_high_range_o
      => pay_pur_shd.g_old_rec.row_high_range
      ,p_object_version_number_o
      => pay_pur_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    ...
    pay_pur_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.user_table_id = hr_api.g_number) then
    p_rec.user_table_id :=
    pay_pur_shd.g_old_rec.user_table_id;
  End If;
  If (p_rec.row_low_range_or_name = hr_api.g_varchar2) then
    p_rec.row_low_range_or_name :=
    pay_pur_shd.g_old_rec.row_low_range_or_name;
  End If;
  If (p_rec.display_sequence = hr_api.g_number) then
    ...
    pay_pur_shd.g_old_rec.display_sequence;
  End If;
  If (p_rec.row_high_range = hr_api.g_varchar2) then
    p_rec.row_high_range :=
    pay_pur_shd.g_old_rec.row_high_range;
  End If;
  --
...
Begin
  ...
  --
  pay_pur_upd.convert_defs(p_rec);
  --
  pay_pur_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
