--------------------------------------------------------
--  DDL for Package Body IBE_DSP_MSITE_SCT_SECT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IBE_DSP_MSITE_SCT_SECT_PVT" AS
/* $Header: IBEVCMSB.pls 120.3 2006/06/30 22:33:06 abhandar noship $ */
...
G_PKG_NAME  CONSTANT VARCHAR2(30):= 'IBE_DSP_MSITE_SCT_SECT_PVT';
G_FILE_NAME CONSTANT VARCHAR2(12):= 'IBEVCMSB.pls';
...
BEGIN
  ...
  THEN
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_INVALID_CHILD_SCT_ID');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    IF(p_level_number < 0) THEN
      FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_INVALID_SCT_LVL_NUM');
      FND_MSG_PUB.Add;
      RAISE FND_API.G_EXC_ERROR;
    END IF;
  END IF;
...
BEGIN
  ...
  ELSIF (l_return_status = FND_API.G_RET_STS_ERROR) THEN
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_MSS_INVALID_CREATE');
    FND_MSG_PUB.Add;
...
BEGIN
  ...
  THEN
    ...
    BEGIN
      ...
      EXCEPTION
         WHEN NO_DATA_FOUND THEN
           FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_NO_MSS_ID');
           FND_MESSAGE.Set_Token('MINI_SITE_ID', p_mini_site_id);
           IF (p_parent_section_id IS NOT NULL) THEN
  ...
  ELSE
    ...
    -- mini_site_id, parent_section_id and child_section_id is specified
    FND_MESSAGE.Set_Name('IBE', 'IBE_DSP_NO_MSS_IDS_SPEC');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
...
BEGIN
	 ...
	 Else
	  	...
	  	LOAD_ROW(
			P_OWNER,
            P_MINI_SITE_SECTION_SECTION_ID,
			P_OBJECT_VERSION_NUMBER,
			P_MINI_SITE_ID,
			P_PARENT_SECTION_ID,
			P_CHILD_SECTION_ID,
