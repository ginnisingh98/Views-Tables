--------------------------------------------------------
--  DDL for Package Body PAY_OPT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_OPT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pay_opt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the pay_org_payment_methods_f_tl Row
  --
...
Begin
  ...
  --
  convert_defs(p_rec);
  pay_opt_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  pay_opt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pay_opt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pay_opt_upd.post_update
     (p_rec
     );
End upd;
--
  ...
  cursor csr_upd_langs is
    select opt.language
     ...
     where opt.org_payment_method_id = p_org_payment_method_id
       and p_language_code in (opt.language
                              ,opt.source_lang);
  --
