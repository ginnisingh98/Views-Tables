--------------------------------------------------------
--  DDL for Package Body BIX_KPI_AGENT_RPT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_KPI_AGENT_RPT_PKG" AS
/*$Header: bixxrkab.pls 115.18 2001/09/27 10:42:48 pkm ship     $*/
...
g_denom_rate         NUMBER;
g_num_rate           NUMBER;
g_format_mask        VARCHAR2(30);
g_nls_date_format    VARCHAR2(50);

/* This procedure fetches exchange rate from BIX global currency to */
/* user currency according to the BIX conversion type and sysdate   */
PROCEDURE get_exchange_rate
IS
  l_global_curr  VARCHAR2(15);
  l_user_curr    VARCHAR2(15);
...
BEGIN
  ...
  g_format_mask := fnd_currency.get_format_mask(l_user_curr, 30);

  /* If the user currency and BIX global currency does not match then   */
  /* get the exchange rate according to BIX conversion type and sysdate */
  IF (l_global_curr <> l_user_curr) THEN
    ...
    bix_util_pkg.get_conversion_rate(l_global_curr, l_user_curr, sysdate, l_conv_type,
                                     g_denom_rate, g_num_rate, l_status);
...
BEGIN
  ...
  IF (p_total_row_ind = TRUE) THEN
    ...
    g_sqlstmt := g_sqlstmt ||
		    ' , ''p'' || mem.group_id || ''c'' || to_char(a.resource_id) || ''n''
			 ...
			 , null ';
  ELSE
    /* If we are processing detail row , then col3 = agent id ; col4 = null ; */
  ...
  g_sqlstmt := g_sqlstmt ||
		   '  , SUM(in_calls_handled)
		      ...
		      , trunc(SUM(available_time + in_talk_time + out_talk_time + in_wrap_time + out_wrap_time)
                ...
                , trunc(SUM(service_requests_created))
                , trunc(SUM(service_requests_opened))
                , trunc(SUM(service_requests_closed))
                , trunc(SUM(leads_created))
  ...
  /* Add the filer condition for classification if the user has chosen */
  /* a particular value for classification in the parameter            */
  IF (g_classification_id <> -999) THEN
    g_sqlstmt := g_sqlstmt || 'AND a.classification_id = :classification_id ';
  ELSE
...
BEGIN
  ...
  EXECUTE IMMEDIATE g_sqlstmt USING g_session_id, g_denom_rate, g_num_rate, g_format_mask, g_denom_rate, g_num_rate,
                                     g_format_mask, g_from_date, g_to_date, p_agent_id, g_classification_id, g_site_id;
...
BEGIN
  ...
  EXECUTE IMMEDIATE g_sqlstmt USING g_session_id, g_denom_rate, g_num_rate, g_format_mask, g_denom_rate, g_num_rate,
							  g_format_mask, g_from_date, g_to_date, p_group_id, g_classification_id, g_site_id;
...
BEGIN
  ...
  g_site_id           := nvl(TO_NUMBER(bix_util_pkg.get_parameter_value(p_context, 'P_SITE')),-999);
  g_classification_id := nvl(TO_NUMBER(bix_util_pkg.get_parameter_value(p_context, 'P_CLASSIFICATION')),-999);
...
BEGIN
  ...
  /* Get the exchange rate from BIX global currency to user preferred currency  */
  /* This is needed to convert the lead and opportunity amount to user currency */
  get_exchange_rate();
  ...
  IF (g_parent = 'BIX_KPI_AGENT_RPT') THEN
    RETURN;
  END IF;

  /* g_parent = "not null" : User has navigated to the report either  from the bin or      */
  /*            by clicking the child column in the report which is hperlinked             */
