--------------------------------------------------------
--  DDL for Package Body PER_CNL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CNL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_cnl_bus.';  -- Global package name
--
  ...
  cursor csr_sec_grp is
    select Null,Null --pbg.security_group_id,
     ...
     where cnl.location_id = p_location_id;
       --and pbg.business_group_id (+) = cnl.business_group_id;
  --
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'LOCATION_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(per_cnl_bus.g_location_id, hr_api.g_number)
       = p_location_id) then
    --
    ...
    -- variable.
    --
    l_legislation_code := per_cnl_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    per_cnl_bus.g_location_id                 := p_location_id;
    per_cnl_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT per_cnl_shd.api_updating
...
/*  hr_api.validate_bus_grp_id
    ...
    ,p_associated_column1 => per_cnl_shd.g_tab_nam|| '.BUSINESS_GROUP_ID'); */
  --
...
/*  hr_api.validate_bus_grp_id
    (p_business_group_id => p_rec.business_group_id
    ,p_associated_column1 => per_cnl_shd.g_tab_nam
                              || '.BUSINESS_GROUP_ID');  */
  --
  -- After validating the set of important attributes,
  -- if Multiple Message detection is enabled and at least
