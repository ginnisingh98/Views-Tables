--------------------------------------------------------
--  DDL for Package Body AP_APXPTPTR_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_APXPTPTR_XMLP_PKG" AS
/* $Header: APXPTPTRB.pls 120.0 2007/12/27 08:26:51 vjaganat noship $ */
...
FUNCTION  get_nls_strings     RETURN BOOLEAN IS
   nls_all       ap_lookup_codes.displayed_field%TYPE;    nls_yes       fnd_lookups.meaning%TYPE;     nls_no        fnd_lookups.meaning%TYPE;     nls_termtype  ap_lookup_codes.displayed_field%TYPE;
BEGIN
   nls_all     := '';
   nls_yes     := '';
   nls_no      := '';
   ...
   INTO    nls_yes,  nls_no,  nls_all, nls_termtype
   ...
   WHERE   ly.lookup_type = 'YES_NO'
     ...
     AND   lt.lookup_code = p_termtype;
   c_nls_yes := nls_yes;
   c_nls_no  := nls_no;
   c_nls_all := nls_all;
   c_nls_termtype := nls_termtype;
...
--Functions to refer Oracle report placeholders--
 ...
 Function C_CHART_OF_ACCOUNTS_ID_p return number is
	Begin
	 ...
	 END;
 Function C_NLS_TERMTYPE_p return varchar2 is
	Begin
	 return C_NLS_TERMTYPE;
	 END;
 Function C_NLS_END_OF_REPORT_p return varchar2 is
	Begin
	 return C_NLS_END_OF_REPORT;
	 END;
END AP_APXPTPTR_XMLP_PKG ;
