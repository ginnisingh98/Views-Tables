--------------------------------------------------------
--  DDL for Package Body IGS_AZ_ADVISORS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AZ_ADVISORS_PKG" AS
/* $Header: IGSHI03B.pls 115.4 2003/06/30 05:38:13 kdande noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_az_advisors%ROWTYPE;
  new_references igs_az_advisors%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.advisor_person_id                 := x_advisor_person_id;
    new_references.start_date                        := TRUNC(x_start_date);
    new_references.end_date                          := TRUNC(x_end_date);
    new_references.max_students_num                  := x_max_students_num;
  ...
  */
  BEGIN
    ...
    igs_az_advising_rels_pkg.get_fk_igs_az_advisors (
      old_references.group_advisor_id
    );
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ...
        AND (tlinfo.advisor_person_id = x_advisor_person_id)
        ...
        AND ((tlinfo.max_students_num = x_max_students_num) OR ((tlinfo.max_students_num IS NULL) AND (x_max_students_num IS NULL)))
  ...
  BEGIN
    ...
    IF ((new_references.end_date IS NULL AND old_references.end_date IS NOT NULL) OR
        ...
        ( NVL(trunc(new_references.end_date),trunc(SYSDATE)) <> NVL(trunc(old_references.end_date),trunc(SYSDATE)) ) ) THEN
                igs_az_gen_001.end_date_advisor(
                     p_group_name        => new_references.group_name,
                     p_advisor_person_id => new_references.advisor_person_id,
                     p_end_date          => new_references.end_date,
                     p_calling_mod       => 'T' );
    -- If advisor is being un-end-dated set auto_delete_flag to null
