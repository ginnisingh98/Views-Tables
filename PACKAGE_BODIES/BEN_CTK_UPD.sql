--------------------------------------------------------
--  DDL for Package Body BEN_CTK_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CTK_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ben_ctk_upd.';  -- Global package name
...
Begin
  ...
  --
  -- Update the ben_cwb_person_tasks Row
  --
  update ben_cwb_person_tasks
    set
     group_pl_id                     = p_rec.group_pl_id
    ,lf_evt_ocrd_dt                  = p_rec.lf_evt_ocrd_dt
    ,status_cd                       = p_rec.status_cd
    ,access_cd                       = p_rec.access_cd
    ,task_last_update_date           = p_rec.task_last_update_date
    ,task_last_update_by             = p_rec.task_last_update_by
...
Begin
  ...
  If (p_rec.access_cd = hr_api.g_varchar2) then
    ...
    ben_ctk_shd.g_old_rec.access_cd;
  End If;
  If (p_rec.task_last_update_date = hr_api.g_date) then
    p_rec.task_last_update_date :=
    ben_ctk_shd.g_old_rec.task_last_update_date;
  End If;
  If (p_rec.task_last_update_by = hr_api.g_number) then
    p_rec.task_last_update_by :=
    ben_ctk_shd.g_old_rec.task_last_update_by;
  End If;
  --
...
Begin
  ...
  --
  ben_ctk_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ben_ctk_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ben_ctk_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
