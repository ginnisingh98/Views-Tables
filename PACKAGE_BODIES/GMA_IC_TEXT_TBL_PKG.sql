--------------------------------------------------------
--  DDL for Package Body GMA_IC_TEXT_TBL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMA_IC_TEXT_TBL_PKG" AS
/* $Header: GMAICTXB.pls 115.6 2003/02/24 19:21:05 kmoizudd ship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from IC_TEXT_TBL_TL
...
is
begin
  update IC_TEXT_TBL_TL T set (
      TEXT
    ) = (select
      B.TEXT
    from IC_TEXT_TBL_TL B
  ...
  where (
      ...
      SUBT.LINE_NO,
      SUBT.LANGUAGE
    from IC_TEXT_TBL_TL SUBB, IC_TEXT_TBL_TL SUBT
  ...
  insert into IC_TEXT_TBL_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from IC_TEXT_TBL_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
...
procedure LOAD_ROW (
 ...
 BEGIN
   ...
   GMA_IC_TEXT_TBL_PKG.UPDATE_ROW ( X_ROW_ID => l_row_id,
...
-- Bug #1712111 (JKB)
 ...
 EXCEPTION
  ...
  GMA_IC_TEXT_TBL_PKG.INSERT_ROW (  X_ROWID => l_row_id,
                                    X_TEXT_CODE => X_TEXT_CODE,
                                    X_LANG_CODE => X_LANG_CODE,
                                    X_PARAGRAPH_CODE => X_PARAGRAPH_CODE,
                                    X_SUB_PARACODE => X_SUB_PARACODE,
