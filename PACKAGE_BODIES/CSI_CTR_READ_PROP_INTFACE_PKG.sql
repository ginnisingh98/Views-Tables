--------------------------------------------------------
--  DDL for Package Body CSI_CTR_READ_PROP_INTFACE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSI_CTR_READ_PROP_INTFACE_PKG" as
/* $Header: csitcrpb.pls 120.1 2006/07/12 22:47:36 epajaril noship $*/

G_PKG_NAME CONSTANT VARCHAR2(30)  := 'CSI_CTR_READ_PROP_INTFACE_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'csitcrpb.pls';
...
PROCEDURE insert_row(
   ...
   CURSOR C1 IS
   SELECT CSI_CTR_READ_PTY_INTERFACE_S.nextval
   FROM   dual;
BEGIN
   IF (px_COUNTER_READ_PROP_INT_ID IS NULL) OR (px_COUNTER_READ_PROP_INT_ID = FND_API.G_MISS_NUM) then
      OPEN C1;
      FETCH C1 INTO px_COUNTER_READ_PROP_INT_ID;
      CLOSE C1;
   END IF;

   INSERT INTO CSI_CTR_READ_PROP_INTERFACE(
        COUNTER_READ_PROP_INT_ID
    ...
    VALUES
        (px_COUNTER_READ_PROP_INT_ID
        ,decode(p_COUNTER_INTERFACE_ID, FND_API.G_MISS_NUM, NULL,p_COUNTER_INTERFACE_ID)
	...
	,decode(p_COUNTER_PROP_VALUE_ID, FND_API.G_MISS_NUM, NULL, p_COUNTER_PROP_VALUE_ID)
...
PROCEDURE Update_Row(
	...
	,p_ATTRIBUTE15                         VARCHAR2
        ...
        ,p_MIGRATED_FLAG                       VARCHAR2) IS
BEGIN
   UPDATE CSI_CTR_READ_PROP_INTERFACE
   SET    COUNTER_INTERFACE_ID = decode(p_COUNTER_INTERFACE_ID, NULL, COUNTER_INTERFACE_ID, FND_API.G_MISS_NUM, NULL, p_COUNTER_INTERFACE_ID)
          ,PARALLEL_WORKER_ID = decode(p_PARALLEL_WORKER_ID, NULL, PARALLEL_WORKER_ID, FND_API.G_MISS_NUM, NULL, p_PARALLEL_WORKER_ID)
	  ...
	  ,COUNTER_PROP_VALUE_ID = decode(p_COUNTER_PROP_VALUE_ID, NULL, COUNTER_PROP_VALUE_ID, FND_API.G_MISS_NUM, NULL, p_COUNTER_PROP_VALUE_ID)
   ...
   WHERE  COUNTER_READ_PROP_INT_ID = p_COUNTER_READ_PROP_INT_ID;
END update_row;
...
BEGIN
   ...
   IF (
       ...
       AND (    ( Recinfo.COUNTER_PROP_VALUE_ID  = p_COUNTER_PROP_VALUE_ID)
            OR (    ( Recinfo.COUNTER_PROP_VALUE_ID IS NULL )
                AND (  p_COUNTER_PROP_VALUE_ID IS NULL )))
       AND (    ( Recinfo.COUNTER_VALUE_ID = p_COUNTER_VALUE_ID)
            OR (    ( Recinfo.COUNTER_VALUE_ID IS NULL )
                AND (  p_COUNTER_VALUE_ID IS NULL )))
       AND (    ( Recinfo.COUNTER_PROPERTY_ID = p_COUNTER_PROPERTY_ID)
            OR (    ( Recinfo.COUNTER_PROPERTY_ID IS NULL )
                AND (  p_COUNTER_PROPERTY_ID IS NULL )))
       AND (    ( Recinfo.PROPERTY_VALUE = p_PROPERTY_VALUE)
            OR (    ( Recinfo.PROPERTY_VALUE IS NULL )
                AND (  p_PROPERTY_VALUE IS NULL )))
...
PROCEDURE delete_row(p_COUNTER_READ_PROP_INT_ID NUMBER) IS
BEGIN
