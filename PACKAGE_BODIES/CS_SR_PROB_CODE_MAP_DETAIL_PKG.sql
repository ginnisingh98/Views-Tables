--------------------------------------------------------
--  DDL for Package Body CS_SR_PROB_CODE_MAP_DETAIL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CS_SR_PROB_CODE_MAP_DETAIL_PKG" as
/* $Header: cspcmpdb.pls 115.0 2003/09/18 01:05:10 rhungund noship $ */

procedure INSERT_ROW (
  PX_PROBLEM_MAP_DETAIL_ID  IN OUT NOCOPY  NUMBER,
...
is
   cursor c1 is
   select cs_sr_prob_code_map_detail_s.nextval
   from dual;

BEGIN
   if ( PX_PROBLEM_MAP_DETAIL_ID IS NULL ) OR ( PX_PROBLEM_MAP_DETAIL_ID = FND_API.G_MISS_NUM) THEN
      open c1;
      fetch c1 into PX_PROBLEM_MAP_DETAIL_ID;
      close c1;
   end if;

  insert into CS_SR_PROB_CODE_MAPPING_DETAIL (
    ...
    PX_PROBLEM_MAP_DETAIL_ID,
    P_PROBLEM_MAP_ID,
    P_INCIDENT_TYPE_ID,
    P_INVENTORY_ITEM_ID,
    P_ORGANIZATION_ID,
    P_CATEGORY_ID,
    P_MAP_START_DATE_ACTIVE,
    P_MAP_END_DATE_ACTIVE,
    P_PROBLEM_CODE,
    P_START_DATE_ACTIVE,
    P_END_DATE_ACTIVE,
...
procedure LOCK_ROW (
  P_PROBLEM_MAP_DETAIL_ID in NUMBER,
  P_PROBLEM_MAP_ID in NUMBER,
  P_INCIDENT_TYPE_ID in NUMBER,
  P_INVENTORY_ITEM_ID in NUMBER,
  P_ORGANIZATION_ID in NUMBER,
  ...
  cursor c1 is select
    ...
    where PROBLEM_MAP_DETAIL_ID = P_PROBLEM_MAP_DETAIL_ID
    for update of PROBLEM_MAP_DETAIL_ID nowait;
begin
  for tlinfo in c1 loop
      if (    (tlinfo.PROBLEM_MAP_DETAIL_ID = P_PROBLEM_MAP_DETAIL_ID)
          AND (tlinfo.PROBLEM_MAP_ID = P_PROBLEM_MAP_ID)
          ...
          AND ((tlinfo.MAP_START_DATE_ACTIVE = P_MAP_START_DATE_ACTIVE)
               OR ((tlinfo.MAP_START_DATE_ACTIVE is null) AND (P_MAP_START_DATE_ACTIVE is null)))
          AND ((tlinfo.MAP_END_DATE_ACTIVE = P_MAP_END_DATE_ACTIVE)
               OR ((tlinfo.MAP_END_DATE_ACTIVE is null) AND (P_MAP_END_DATE_ACTIVE is null)))
...
begin
  update CS_SR_PROB_CODE_MAPPING_DETAIL set
    PROBLEM_MAP_ID = P_PROBLEM_MAP_ID,
    INCIDENT_TYPE_ID = P_INCIDENT_TYPE_ID,
    INVENTORY_ITEM_ID = P_INVENTORY_ITEM_ID,
    ORGANIZATION_ID = P_ORGANIZATION_ID,
    CATEGORY_ID = P_CATEGORY_ID,
    MAP_START_DATE_ACTIVE = P_MAP_START_DATE_ACTIVE,
    MAP_END_DATE_ACTIVE = P_MAP_END_DATE_ACTIVE,
    ...
    PROBLEM_MAP_DETAIL_ID = P_PROBLEM_MAP_DETAIL_ID,
...
begin
  delete from CS_SR_PROB_CODE_MAPPING_DETAIL
  where PROBLEM_MAP_DETAIL_ID = P_PROBLEM_MAP_DETAIL_ID;
...
end DELETE_ROW;   -- Enter further code below as specified in the Package spec.

end CS_SR_PROB_CODE_MAP_DETAIL_PKG;
