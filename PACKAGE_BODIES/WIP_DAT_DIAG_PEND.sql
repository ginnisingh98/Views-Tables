--------------------------------------------------------
--  DDL for Package Body WIP_DAT_DIAG_PEND
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WIP_DAT_DIAG_PEND" as
/* $Header: WIPDC05B.pls 120.1 2008/05/01 04:00:20 shjindal noship $ */
PROCEDURE init is
BEGIN
null;
END init;
...
if l_org_id is NULL then
 ...
 reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
  ...
  WIP_DIAG_DATA_COLL.pending_txns(l_org_id );

reportStr := '<BR>The output generated gives you the details regarding Pending Transactions';
...
EXCEPTION
  when others then
    ...
    fixInfo := 'Unexpected Exception in WIPDC05B.pls';
    isFatal := 'FALSE';
    report := JTF_DIAGNOSTIC_ADAPTUTIL.constructReport(statusStr,errStr,fixInfo,isFatal);
    reportClob := JTF_DIAGNOSTIC_ADAPTUTIL.getReportClob;
...
BEGIN
descStr := 'This data collector retrieves all relevant details of pending WIP transactions.<BR>'||
