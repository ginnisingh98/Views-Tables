--------------------------------------------------------
--  DDL for Package Body AME_RLU_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_RLU_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_rlu_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_RULE_USAGES_PK') Then
...
Begin
  ...
  Else
    If (p_rule_id =
        ame_rlu_shd.g_old_rec.rule_id and
      p_item_id =
        ame_rlu_shd.g_old_rec.item_id and
        p_object_version_number =
        ame_rlu_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_rlu_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_rlu_shd.g_old_rec.end_date;
  ...
  --
--
-- We need to trap the parent object:  ame_rules
--
  begin
    select null
      into l_dummy
...
Begin
  ...
  l_rec.approver_category                := p_approver_category;
  l_rec.object_version_number            := p_object_version_number;
  l_rec.old_start_date                   := p_old_start_date;
  --
