--------------------------------------------------------
--  DDL for Package Body JTF_RS_GROUPS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_RS_GROUPS_PKG" as
/* $Header: jtfrstgb.pls 120.0.12010000.2 2009/05/11 07:36:47 rgokavar ship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from JTF_RS_GROUPS_B
    where GROUP_ID = X_GROUP_ID
    ;
begin
  insert into JTF_RS_GROUPS_B (
    ATTRIBUTE1,
    ATTRIBUTE2,
    ATTRIBUTE3,
    ATTRIBUTE4,
...
begin
  update JTF_RS_GROUPS_B set
    ...
    ACCOUNTING_CODE = X_ACCOUNTING_CODE,
...
is
begin
  delete from JTF_RS_GROUPS_TL T
  where not exists
    (select NULL
  ...
  update JTF_RS_GROUPS_TL T set (
      GROUP_NAME,
      GROUP_DESC
    ) = (select
      B.GROUP_NAME,
      B.GROUP_DESC
    from JTF_RS_GROUPS_TL B
    where B.GROUP_ID = T.GROUP_ID
  ...
  where (
      ...
      SUBT.GROUP_ID,
      SUBT.LANGUAGE
    from JTF_RS_GROUPS_TL SUBB, JTF_RS_GROUPS_TL SUBT
  ...
  insert into JTF_RS_GROUPS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from JTF_RS_GROUPS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
