--------------------------------------------------------
--  DDL for Package Body HR_CONTINGENT_WORKER_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_CONTINGENT_WORKER_BE1" as 
...
p_exp_check_send_to_address    varchar2,
...
p_name_combination_warning     boolean) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  hr_contingent_worker_be1.create_cwk_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.contingent_worker.create_cwk';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<exp_check_send_to_address>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_exp_check_send_to_address);
    l_text:=l_text||'</exp_check_send_to_address>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
