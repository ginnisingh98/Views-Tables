--------------------------------------------------------
--  DDL for Package Body RG_DSS_VAR_DIMENSIONS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."RG_DSS_VAR_DIMENSIONS_PKG" as
/* $Header: rgidvdmb.pls 120.2 2002/11/14 02:59:25 djogg ship $ */
...
BEGIN
  ...
  -- name already exists for a different variable: ERROR
  FND_MESSAGE.set_name('RG', 'RG_FORMS_OBJECT_EXISTS_FOR');
  FND_MESSAGE.set_token('OBJECT1', 'RG_DSS_DIMENSION', TRUE);
  FND_MESSAGE.set_token('OBJECT2', 'RG_DSS_VARIABLE', TRUE);
  APP_EXCEPTION.raise_exception;
...
BEGIN
  IF (X_Master_Variable_Id IS NULL) THEN
    X_Master_Variable_Id := RG_DSS_VARIABLES_PKG.get_new_id;
  END IF;
  X_Variable_Id := X_Master_Variable_Id;
  ...
  IF (NumRecs >= 10) THEN
    ...
    FND_MESSAGE.set_name('RG', 'RG_DSS_MAX_DIMENSION_REACHED');
    FND_MESSAGE.set_token('NUM_DIM','10');
    APP_EXCEPTION.raise_exception;
  END IF;

  check_unique_sequence(X_Rowid, X_Variable_Id, X_Sequence);
  check_unique_dimension(X_Rowid, X_Variable_Id, X_Dimension_Id);

  INSERT INTO rg_dss_var_dimensions(
          variable_id,
          sequence,
          dimension_id,
          last_update_date,
...
BEGIN
  ...
  /* Ensure that at least one dimension exists for the variable.
     Allow summary dimension to be deleted                      */
  RG_DSS_VARIABLES_PKG.check_for_details(X_Variable_Id,'D');
END Delete_Row;
