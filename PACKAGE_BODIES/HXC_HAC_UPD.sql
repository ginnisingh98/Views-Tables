--------------------------------------------------------
--  DDL for Package Body HXC_HAC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_HAC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_hac_upd.';  -- Global package name
...
Begin
  ...
  --
  -- Update the hxc_approval_comps Row
  --
  update hxc_approval_comps
    set
     approval_comp_id                = p_rec.approval_comp_id
    ,approval_style_id               = p_rec.approval_style_id
    ,time_recipient_id               = p_rec.time_recipient_id
    ,approval_mechanism              = p_rec.approval_mechanism
    ,approval_mechanism_id           = p_rec.approval_mechanism_id
    ,wf_item_type                    = p_rec.wf_item_type
    ,wf_name                         = p_rec.wf_name
    ,start_date                      = p_rec.start_date
    ,end_date                        = p_rec.end_date
    ,object_version_number           = p_rec.object_version_number
    ,approval_order                  = p_rec.approval_order
    ,time_category_id                = p_rec.time_category_id
    ,parent_comp_id                  = p_rec.parent_comp_id
    ,parent_comp_ovn                 = p_rec.parent_comp_ovn
    ,run_recipient_extensions        = p_rec.run_recipient_extensions
        ,last_updated_by		     = fnd_global.user_id
    ,last_update_date		     = sysdate
    ,last_update_login	             = fnd_global.login_id

    where approval_comp_id = p_rec.approval_comp_id;

IF p_rec.approval_mechanism = 'ENTRY_LEVEL_APPROVAL' then
  update hxc_approval_comps
     set parent_comp_ovn = p_rec.object_version_number
     where parent_comp_id = p_rec.approval_comp_id and
           parent_comp_ovn = p_rec.object_version_number - 1 ;
ELSE

   delete hxc_approval_comps
   where parent_comp_id = p_rec.approval_comp_id and
         parent_comp_ovn = p_rec.object_version_number - 1 ;
...
Begin
  ...
  begin
    ...
    set
    approval_comp_ovn=p_rec.object_version_number where
    approval_comp_id=p_rec.approval_comp_id and
    approval_comp_ovn=hxc_hac_shd.g_old_rec.object_version_number;


    hxc_hac_rku.after_update
      ...
      => hxc_hac_shd.g_old_rec.approval_style_id
      ,p_time_recipient_id_o
      => hxc_hac_shd.g_old_rec.time_recipient_id
      ,p_approval_mechanism_o
      => hxc_hac_shd.g_old_rec.approval_mechanism
      ,p_approval_mechanism_id_o
      => hxc_hac_shd.g_old_rec.approval_mechanism_id
      ,p_wf_item_type_o
      => hxc_hac_shd.g_old_rec.wf_item_type
      ,p_wf_name_o
      => hxc_hac_shd.g_old_rec.wf_name
      ,p_start_date_o
      => hxc_hac_shd.g_old_rec.start_date
      ,p_end_date_o
      => hxc_hac_shd.g_old_rec.end_date
      ,p_object_version_number_o
      => hxc_hac_shd.g_old_rec.object_version_number
      ,p_approval_order_o
      => hxc_hac_shd.g_old_rec.approval_order
      ,p_time_category_id_o
	  => hxc_hac_shd.g_old_rec.time_category_id
	  ,p_parent_comp_id_o
	  => hxc_hac_shd.g_old_rec.parent_comp_id
	  ,p_parent_comp_ovn_o
      => hxc_hac_shd.g_old_rec.parent_comp_ovn
       ,p_run_recipient_extensions_o =>hxc_hac_shd.g_old_rec.run_recipient_extensions
...
Begin
  ...
  If (p_rec.time_recipient_id = hr_api.g_number) then
    ...
    hxc_hac_shd.g_old_rec.time_recipient_id;
  End If;
  If (p_rec.approval_mechanism = hr_api.g_varchar2) then
    p_rec.approval_mechanism :=
    hxc_hac_shd.g_old_rec.approval_mechanism;
  End If;
  If (p_rec.approval_mechanism_id = hr_api.g_number) then
    p_rec.approval_mechanism_id :=
    hxc_hac_shd.g_old_rec.approval_mechanism_id;
  End If;
  If (p_rec.wf_item_type = hr_api.g_varchar2) then
    p_rec.wf_item_type :=
    hxc_hac_shd.g_old_rec.wf_item_type;
  End If;
  If (p_rec.wf_name = hr_api.g_varchar2) then
    p_rec.wf_name :=
    hxc_hac_shd.g_old_rec.wf_name;
  End If;
  ...
  If (p_rec.end_date = hr_api.g_date) then
    ...
    hxc_hac_shd.g_old_rec.end_date;
  End If;
  If (p_rec.approval_order = hr_api.g_number) then
    p_rec.approval_order :=
    hxc_hac_shd.g_old_rec.approval_order;
  End If;
  If (p_rec.time_category_id = hr_api.g_number) then
    ...
    hxc_hac_shd.g_old_rec.time_category_id;
  End If;
  If (p_rec.parent_comp_id = hr_api.g_number) then
    p_rec.parent_comp_id :=
    hxc_hac_shd.g_old_rec.parent_comp_id;
  End If;
  If (p_rec.parent_comp_ovn = hr_api.g_number) then
    p_rec.parent_comp_ovn :=
    hxc_hac_shd.g_old_rec.parent_comp_ovn;
  End If;
  If (p_rec.run_recipient_extensions = hr_api.g_varchar2) then
      p_rec.run_recipient_extensions :=
      hxc_hac_shd.g_old_rec.run_recipient_extensions;
  End If;
  --
End convert_defs;
--
...
Begin
  ...
  --
  hxc_hac_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hxc_hac_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hxc_hac_upd.post_update
     (p_effective_date
     ,p_rec
     );
End upd;
