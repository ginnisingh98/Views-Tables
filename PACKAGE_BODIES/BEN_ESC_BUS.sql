--------------------------------------------------------
--  DDL for Package Body BEN_ESC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ESC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_esc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_stdnt_stat_cvg_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_stdnt_stat_cvg_id PK of record being inserted or updated.
...
--
Procedure chk_elig_stdnt_stat_cvg_id(p_elig_stdnt_stat_cvg_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_stdnt_stat_cvg_id';
...
Begin
  ...
  --
  l_api_updating := ben_esc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_stdnt_stat_cvg_id                => p_elig_stdnt_stat_cvg_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_stdnt_stat_cvg_id,hr_api.g_number)
     <>  ben_esc_shd.g_old_rec.elig_stdnt_stat_cvg_id) then
    --
    -- raise error as PK has changed
    --
    ben_esc_shd.constraint_error('BEN_ELG_STDNT_CVG_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_stdnt_stat_cvg_id is not null then
      --
  ...
  --
End chk_elig_stdnt_stat_cvg_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_stdnt_stat_cd >------|
-- ----------------------------------------------------------------------------
...
-- In Parameters
--   elig_stdnt_stat_cvg_id PK of record being inserted or updated.
--   stdnt_stat_cd Value of lookup code.
...
--   Internal table handler use only.
--
Procedure chk_stdnt_stat_cd(p_elig_stdnt_stat_cvg_id                in number,
                            p_stdnt_stat_cd               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_stdnt_stat_cd';
  l_api_updating boolean;
  --
Begin
  --
  ...
  if (l_api_updating
      and p_stdnt_stat_cd
      <> nvl(ben_esc_shd.g_old_rec.stdnt_stat_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_cvg_thru_rl(p_elig_stdnt_stat_cvg_id                in number,
...
--   Internal table handler use only.
--
Procedure chk_cvg_thru_cd(p_elig_stdnt_stat_cvg_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_cvg_thru_cd
      <> nvl(ben_esc_shd.g_old_rec.cvg_thru_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_cvg_strt_rl(p_elig_stdnt_stat_cvg_id                in number,
...
--   Internal table handler use only.
--
Procedure chk_cvg_strt_cd(p_elig_stdnt_stat_cvg_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_cvg_strt_cd
      <> nvl(ben_esc_shd.g_old_rec.cvg_strt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_cvg_strt_dt_dependency(p_elig_stdnt_stat_cvg_id 	in number,
                            	p_cvg_strt_cd           	in varchar2,
 				p_cvg_strt_rl 			in number,
                            	p_effective_date          	in date,
                           	p_object_version_number    	in number) is
...
Begin
  ...
  if (l_api_updating
      	and (nvl(p_cvg_strt_cd, hr_api.g_varchar2) <>
		      	nvl(ben_esc_shd.g_old_rec.cvg_strt_cd,hr_api.g_varchar2) or
 	     nvl(p_cvg_strt_rl, hr_api.g_number) <>
			nvl(ben_esc_shd.g_old_rec.cvg_strt_rl,hr_api.g_number))
      	or not l_api_updating) then
    	--
...
--   Internal table handler use only.
--
Procedure chk_cvg_thru_dt_dependency(p_elig_stdnt_stat_cvg_id 	in number,
                            	   p_cvg_thru_cd           	in varchar2,
				   p_cvg_thru_rl                in number,
                            	   p_effective_date          	in date,
                           	   p_object_version_number    	in number) is
...
Begin
  ...
  if (l_api_updating
      	and (nvl(p_cvg_thru_cd, hr_api.g_varchar2) <>
		      	nvl(ben_esc_shd.g_old_rec.cvg_thru_cd,hr_api.g_varchar2) or
 	     nvl(p_cvg_thru_rl, hr_api.g_number) <>
			nvl(ben_esc_shd.g_old_rec.cvg_thru_rl,hr_api.g_number))
      	or not l_api_updating) then
...
--   p_elig_stdnt_stat_cvg_id - primary key of the table
--   p_stdnt_stat_cd - duplicate value to be checked
...
--   Internal table handler use only.
--
Procedure chk_duplicate_rows(p_elig_stdnt_stat_cvg_id in number,
                             p_stdnt_stat_cd in varchar2,
  ...
  dummy varchar2(1);
  cursor c1 is select null from ben_elig_stdnt_stat_cvg_f
     where (elig_stdnt_stat_cvg_id <> p_elig_stdnt_stat_cvg_id or p_elig_stdnt_stat_cvg_id is null) and
           dpnt_cvg_eligy_prfl_id = p_dpnt_cvg_eligy_prfl_id and
           stdnt_stat_cd = p_stdnt_stat_cd and
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    hr_api.mandatory_arg_error
      (p_api_name       => l_proc,
       p_argument       => 'elig_stdnt_stat_cvg_id',
       p_argument_value => p_elig_stdnt_stat_cvg_id);
    --
    --
    --
  End If;
...
function return_legislation_code
  (p_elig_stdnt_stat_cvg_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_stdnt_stat_cvg_f b
    where b.elig_stdnt_stat_cvg_id      = p_elig_stdnt_stat_cvg_id
