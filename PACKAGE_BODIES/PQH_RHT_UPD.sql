--------------------------------------------------------
--  DDL for Package Body PQH_RHT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_RHT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_rht_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy pqh_rht_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_routing_history Row
  --
  ...
  forwarded_by_assignment_id        = p_rec.forwarded_by_assignment_id,
  forwarded_by_member_id            = p_rec.forwarded_by_member_id,
  forwarded_by_position_id          = p_rec.forwarded_by_position_id,
  forwarded_by_user_id              = p_rec.forwarded_by_user_id,
  forwarded_by_role_id              = p_rec.forwarded_by_role_id,
  forwarded_to_assignment_id        = p_rec.forwarded_to_assignment_id,
  forwarded_to_member_id            = p_rec.forwarded_to_member_id,
  forwarded_to_position_id          = p_rec.forwarded_to_position_id,
  forwarded_to_user_id              = p_rec.forwarded_to_user_id,
  forwarded_to_role_id              = p_rec.forwarded_to_role_id,
  notification_date                 = p_rec.notification_date,
  ...
  user_action_cd                    = p_rec.user_action_cd,
  from_range_name                   = p_rec.from_range_name,
  to_range_name                     = p_rec.to_range_name,
  list_range_name                   = p_rec.list_range_name,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in pqh_rht_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy pqh_rht_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.approval_cd = hr_api.g_varchar2) then
    p_rec.approval_cd :=
    pqh_rht_shd.g_old_rec.approval_cd;
  End If;
  If (p_rec.comments = hr_api.g_varchar2) then
    p_rec.comments :=
    pqh_rht_shd.g_old_rec.comments;
  End If;
  If (p_rec.forwarded_by_assignment_id = hr_api.g_number) then
    p_rec.forwarded_by_assignment_id :=
    pqh_rht_shd.g_old_rec.forwarded_by_assignment_id;
  End If;
  If (p_rec.forwarded_by_member_id = hr_api.g_number) then
    p_rec.forwarded_by_member_id :=
    pqh_rht_shd.g_old_rec.forwarded_by_member_id;
  End If;
  If (p_rec.forwarded_by_position_id = hr_api.g_number) then
    p_rec.forwarded_by_position_id :=
    pqh_rht_shd.g_old_rec.forwarded_by_position_id;
  End If;
  If (p_rec.forwarded_by_user_id = hr_api.g_number) then
    p_rec.forwarded_by_user_id :=
    pqh_rht_shd.g_old_rec.forwarded_by_user_id;
  End If;
  If (p_rec.forwarded_by_role_id = hr_api.g_number) then
    p_rec.forwarded_by_role_id :=
    pqh_rht_shd.g_old_rec.forwarded_by_role_id;
  End If;
  If (p_rec.forwarded_to_assignment_id = hr_api.g_number) then
    p_rec.forwarded_to_assignment_id :=
    pqh_rht_shd.g_old_rec.forwarded_to_assignment_id;
  End If;
  If (p_rec.forwarded_to_member_id = hr_api.g_number) then
    p_rec.forwarded_to_member_id :=
    pqh_rht_shd.g_old_rec.forwarded_to_member_id;
  End If;
  If (p_rec.forwarded_to_position_id = hr_api.g_number) then
    p_rec.forwarded_to_position_id :=
    pqh_rht_shd.g_old_rec.forwarded_to_position_id;
  End If;
  If (p_rec.forwarded_to_user_id = hr_api.g_number) then
    p_rec.forwarded_to_user_id :=
    pqh_rht_shd.g_old_rec.forwarded_to_user_id;
  End If;
  If (p_rec.forwarded_to_role_id = hr_api.g_number) then
    p_rec.forwarded_to_role_id :=
    pqh_rht_shd.g_old_rec.forwarded_to_role_id;
  End If;
  If (p_rec.notification_date = hr_api.g_date) then
    p_rec.notification_date :=
    pqh_rht_shd.g_old_rec.notification_date;
  End If;
  If (p_rec.pos_structure_version_id = hr_api.g_number) then
    p_rec.pos_structure_version_id :=
    pqh_rht_shd.g_old_rec.pos_structure_version_id;
  End If;
  If (p_rec.routing_category_id = hr_api.g_number) then
    p_rec.routing_category_id :=
    pqh_rht_shd.g_old_rec.routing_category_id;
  End If;
  If (p_rec.transaction_category_id = hr_api.g_number) then
    p_rec.transaction_category_id :=
    pqh_rht_shd.g_old_rec.transaction_category_id;
  End If;
  If (p_rec.transaction_id = hr_api.g_number) then
    p_rec.transaction_id :=
    pqh_rht_shd.g_old_rec.transaction_id;
  End If;
  If (p_rec.user_action_cd = hr_api.g_varchar2) then
    p_rec.user_action_cd :=
    pqh_rht_shd.g_old_rec.user_action_cd;
  End If;
  If (p_rec.from_range_name = hr_api.g_varchar2) then
    p_rec.from_range_name :=
    pqh_rht_shd.g_old_rec.from_range_name;
  End If;
  If (p_rec.to_range_name = hr_api.g_varchar2) then
    p_rec.to_range_name :=
    pqh_rht_shd.g_old_rec.to_range_name;
  End If;
  If (p_rec.list_range_name = hr_api.g_varchar2) then
    p_rec.list_range_name :=
    pqh_rht_shd.g_old_rec.list_range_name;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  pqh_rht_bus.update_validate(p_rec
  ,p_effective_date);
  --
