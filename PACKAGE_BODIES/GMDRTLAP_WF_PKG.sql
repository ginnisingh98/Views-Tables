--------------------------------------------------------
--  DDL for Package Body GMDRTLAP_WF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMDRTLAP_WF_PKG" AS
/* $Header: GMDRTLAB.pls 120.0 2005/05/25 19:49:39 appldev noship $ */
   PROCEDURE wf_init (
   ...
   IS
      ...
      l_itemtype                WF_ITEMS.ITEM_TYPE%TYPE :=  'GMDRTLAP';
      ...
      l_WorkflowProcess   VARCHAR2(30) := 'GMDRTLAP_PROCESS';
      l_count             NUMBER;
      BEGIN
      ...
      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
        					  aname => 'GMDRTLAP_OWNER_ID',
         					  avalue => l_performer_name);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRTLAP_ROUTING_DESC',
         					  avalue => l_routing_desc);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRTLAP_ROUTING_CLASS',
         					  avalue => l_routing_class);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRTLAP_ROUTING_QTY',
         					  avalue => l_routing_qty);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRTLAP_ROUTING_UOM',
         					  avalue => l_item_um);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRTLAP_EFF_START_DATE',
         					  avalue => l_effective_start_date);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRTLAP_EFF_END_DATE',
  ...
  EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDRTLAP_wf_pkg','wf_init',l_itemtype,l_itemkey,'Initial' );
      raise;
...
/* ######################################################################## */
   ...
   IS
      ...
      l_Routing_id         gmd_routings_b.Routing_id%TYPE:=to_number(wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_ROUTING_ID'));
      p_data_string       VARCHAR2(2000);
      p_wf_data_string    VARCHAR2(2000);
      p_lab_wf_item_type  VARCHAR2(8)  := 'GMDRTLAP';  -- Recipe Lab use Approval Workflow Inernal Name
      P_lab_Process_name  VARCHAR2(32) := 'GMDRTLAP_PROCESS'; -- Recipe Lab use Approval Workflow Process Inernal Name
      P_lab_activity_name VARCHAR2(80) := 'GMDRTLAP_NOTI_REQUEST';
     ...
     IF (p_funcmode = 'RUN') THEN
         ...
         THEN
	      ...
	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDRTLAP_ADDL_TEXT',
                                      avalue => l_data_string);

	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDRTLAP_APPROVER',
                                      avalue => p_role);

            p_resultout:='COMPLETE:Y';
        ELSE
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_TARGET_STATUS');
...
EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDRTLAP_wf_pkg','is_approval_req',p_itemtype,p_itemkey,p_role);
      raise;
END is_approval_req;
...
PROCEDURE REMINDAR_CHECK (
      ...
      l_mesg_cnt      number:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDRTLAP_MESG_CNT');
      l_approver      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_APPROVER');
BEGIN
...
BEGIN
     IF (p_funcmode = 'RUN') THEN
          l_start_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_START_STATUS');
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_TARGET_STATUS');
...
PROCEDURE MOREINFO_RESPONSE  (
      ...
      p_funcmode      IN VARCHAR2,
      p_resultout     OUT NOCOPY VARCHAR2) IS
      l_requester     VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_REQUSTER');
BEGIN
...
PROCEDURE APPEND_COMMENTS (
      ...
      l_comment       VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_COMMENT');
      l_mesg_comment  VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_DISP_COMMENT');
      l_performer      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRTLAP_CURR_PERFORMER');
BEGIN
...
--             l_html_mesg := replace(l_mesg_comment,wf_core.newline,'<BR>'||wf_core.newline);
	     ...
	     WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDRTLAP_DISP_COMMENT',
                                   avalue => l_mesg_comment);
--	     WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
--                                 itemkey => p_itemkey,
--         			           aname => 'GMDRTLAP_HTML_DISP_COMMENT',
--                                 avalue => l_html_mesg);
	     WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDRTLAP_COMMENT',
                                   avalue => l_comment);
       END IF;
END;
