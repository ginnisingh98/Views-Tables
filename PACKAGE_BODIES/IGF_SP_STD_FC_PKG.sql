--------------------------------------------------------
--  DDL for Package Body IGF_SP_STD_FC_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_SP_STD_FC_PKG" AS
/* $Header: IGFPI05B.pls 115.2 2003/03/19 08:50:13 smadathi noship $ */

  l_rowid VARCHAR2(25);
  old_references igf_sp_std_fc_all%ROWTYPE;
  new_references igf_sp_std_fc_all%ROWTYPE;
  ...
  */
  BEGIN
    ...
    IF (((old_references.spnsr_stdnt_id = new_references.spnsr_stdnt_id)) OR
        ((new_references.spnsr_stdnt_id IS NULL))) THEN
      NULL;
    ELSIF NOT igf_sp_stdnt_rel_pkg.get_pk_for_validation (
                new_references.spnsr_stdnt_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
    ...
    IF (((old_references.fee_class = new_references.fee_class)) OR
        ((new_references.fee_class IS NULL))) THEN
      NULL;
  ...
  */
  BEGIN
    ...
    igf_sp_std_prg_pkg.get_fk_igf_sp_std_fc (
      old_references.fee_cls_id
    );
  ...
  */
  ...
  PROCEDURE get_fk_igf_sp_stdnt_rel (
    x_spnsr_stdnt_id                    IN     NUMBER
  ) AS
  /*
  ||  Created By :
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SP_STDFC_SPSTD_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
...
/*
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SP_STDFC_LKUP_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igf_sp_std_fc_s.NEXTVAL
    INTO      x_fee_cls_id
    FROM      dual;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.spnsr_stdnt_id = x_spnsr_stdnt_id)
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igf_sp_std_fc_all
      SET
        spnsr_stdnt_id                    = new_references.spnsr_stdnt_id,
