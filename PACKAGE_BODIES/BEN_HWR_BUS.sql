--------------------------------------------------------
--  DDL for Package Body BEN_HWR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_HWR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_hwr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_hrs_wkd_in_perd_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   hrs_wkd_in_perd_rt_id PK of record being inserted or updated.
...
--
Procedure chk_hrs_wkd_in_perd_rt_id(p_hrs_wkd_in_perd_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_hrs_wkd_in_perd_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_hwr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_hrs_wkd_in_perd_rt_id                => p_hrs_wkd_in_perd_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_hrs_wkd_in_perd_rt_id,hr_api.g_number)
     <>  ben_hwr_shd.g_old_rec.hrs_wkd_in_perd_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_hwr_shd.constraint_error('BEN_HRS_WORKED_IN_PERD_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_hrs_wkd_in_perd_rt_id is not null then
      --
...
--
-- In Parameters
--   p_hrs_wkd_in_perd_rt_id PK
--   p_hrs_wkd_in_perd_fctr_id ID of FK column
...
--   Internal table handler use only.
--
Procedure chk_hrs_wkd_in_perd_fctr_id (p_hrs_wkd_in_perd_rt_id          in number,
  ...
  cursor c1 is
    select null
    from   ben_hrs_wkd_in_perd_fctr a
    where  a.hrs_wkd_in_perd_fctr_id = p_hrs_wkd_in_perd_fctr_id;
  --
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  ...
  if (l_api_updating
     ...
     <> nvl(ben_hwr_shd.g_old_rec.hrs_wkd_in_perd_fctr_id,hr_api.g_number)
     or not l_api_updating) and
     p_hrs_wkd_in_perd_fctr_id is not null then
    --
    -- check if hrs_wkd_in_perd_fctr_id value exists in ben_hrs_wkd_in_perd_fctr table
    --
    open c1;
      --
      ...
      if c1%notfound then
        ...
        close c1;
        --
        -- raise error as FK does not relate to PK in ben_hrs_wkd_in_perd_fctr
        -- table.
        --
        ben_hwr_shd.constraint_error('BEN_HRS_WORKED_IN_PERD_RT_FK1');
        --
      end if;
      --
    close c1;
...
--
-- In Parameters
--   p_hrs_wkd_in_perd_rt_id PK of record being inserted or updated.
--   p_hrs_wkd_in_perd_fctr_id Value of FK
...
and hrs_wkd_in_perd_rt_id <> nvl(p_hrs_wkd_in_perd_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_hrs_wkd_in_perd_fctr_id <> nvl(ben_hwr_shd.g_old_rec.hrs_wkd_in_perd_fctr_id,hr_api.g_number)
	...
	if c_dup%found then
		close c_dup;
		--
		-- raise error as this Hours Worked in Period criteria already exists for this profile
		--
		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
		fnd_message.set_token('VAR1','Hours Worked in Period criteria',TRUE);
...
is
   ...
   cursor c1 is select null
		 ...
		 -- CHANGED for Bug 5078478 .. commented the previous query and added new one
                 --  and p_hrs_wkd_in_perd_rt_id <> nvl(p_hrs_wkd_in_perd_rt_id,-1)
		   and hrs_wkd_in_perd_rt_id <> nvl(p_hrs_wkd_in_perd_rt_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_hrs_wkd_in_perd_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_hwr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
--   BEN_HRS_WKD_IN_PERD_FCTR table.
--
-- Pre Conditions
--   None.
--
-- In Parameters
--   hrs_wkd_in_perd_fctr_id FK of pointing to BEN_HRS_WKD_IN_PERD_FCTR tables.
...
Begin
  ...
  if p_hrs_wkd_in_perd_fctr_id is not null then
     ...
     fetch c1 into l_dummy;
     if c1%notfound then
        hr_utility.set_message(801,'Hrs_Wkd_NotExist');
        hr_utility.raise_error;
     end if;
     close c1;
  end if;
...
function return_legislation_code
  (p_hrs_wkd_in_perd_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_hrs_wkd_in_perd_rt_f b
    where b.hrs_wkd_in_perd_rt_id      = p_hrs_wkd_in_perd_rt_id
