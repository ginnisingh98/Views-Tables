--------------------------------------------------------
--  DDL for Package Body GMD_ACTIONS_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMD_ACTIONS_PVT" as
/* $Header: GMDVACPB.pls 115.1 2002/12/03 17:02:49 cnagarba noship $ */
procedure INSERT_ROW (
  X_ROWID in out nocopy VARCHAR2,
  X_ACTION_CODE in VARCHAR2,
  X_ACTION_INTERVAL in NUMBER,
  ...
  cursor C is select ROWID from GMD_ACTIONS_B
...
begin
  ...
  close c;
  if (    (recinfo.ACTION_INTERVAL = X_ACTION_INTERVAL)
  ...
  for tlinfo in c1 loop
    if (tlinfo.BASELANG = 'Y') then
      if (    (tlinfo.ACTION_DESC = X_ACTION_DESC)
      ) then
        null;
      else
        fnd_message.set_name('FND', 'FORM_RECORD_CHANGED');
...
begin
  update GMD_ACTIONS_B set
    ACTION_INTERVAL = X_ACTION_INTERVAL,
...
is
begin
  delete from GMD_ACTIONS_TL T
  where not exists
    (select NULL
    from GMD_ACTIONS_B B
    where B.ACTION_CODE = T.ACTION_CODE
    );

  update GMD_ACTIONS_TL T set (
      ACTION_DESC
    ) = (select
      B.ACTION_DESC
    from GMD_ACTIONS_TL B
    where B.ACTION_CODE = T.ACTION_CODE
  ...
  where (
      ...
      SUBT.ACTION_CODE,
      SUBT.LANGUAGE
    from GMD_ACTIONS_TL SUBB, GMD_ACTIONS_TL SUBT
    where SUBB.ACTION_CODE = SUBT.ACTION_CODE
    and SUBB.LANGUAGE = SUBT.SOURCE_LANG
    and (SUBB.ACTION_DESC <> SUBT.ACTION_DESC
  ));
  ...
  insert into GMD_ACTIONS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GMD_ACTIONS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
