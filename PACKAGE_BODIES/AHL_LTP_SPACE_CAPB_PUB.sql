--------------------------------------------------------
--  DDL for Package Body AHL_LTP_SPACE_CAPB_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_LTP_SPACE_CAPB_PUB" AS
/* $Header: AHLPSPCB.pls 115.5 2002/12/04 19:11:00 ssurapan noship $ */

G_PKG_NAME  VARCHAR2(30)  := 'AHL_LTP_SPACE_CAPB_PUB';
...
BEGIN
  ...
  SAVEPOINT process_space_capbl;
   ...
   -- Debug info.
   IF G_DEBUG='Y' THEN
       AHL_DEBUG_PUB.debug( 'enter ahl_ltp_space_capb_pub.Process Space capbl','+SPCBL+');
    END IF;
   ...
   IF p_x_space_rec.operation_flag = 'C' THEN
         AHL_LTP_SPACE_CAPB_PVT.CREATE_SPACE
           (
    ...
    ELSIF p_x_space_capbl_tbl.COUNT > 0 THEN
      ...
      LOOP
        IF p_x_space_capbl_tbl(i).operation_flag = 'C' THEN
         AHL_LTP_SPACE_CAPB_PVT.CREATE_SPACE_CAPBLTS
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
  ...
  --Standard check to count messages
   ...
   IF G_DEBUG='Y' THEN
    ...
    END IF;
         ...
         AHL_LTP_SPACE_CAPB_PVT.UPDATE_SPACE
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
    ...
    ELSIF p_x_space_capbl_tbl.COUNT > 0 THEN
      ...
      LOOP
        IF p_x_space_capbl_tbl(i).operation_flag = 'U' THEN
         AHL_LTP_SPACE_CAPB_PVT.UPDATE_SPACE_CAPBLTS
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
  ...
  --Standard check to count messages
   ...
   ELSIF p_x_space_rec.operation_flag = 'D' THEN
         AHL_LTP_SPACE_CAPB_PVT.DELETE_SPACE
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
    ...
    ELSIF p_x_space_capbl_tbl.COUNT > 0 THEN
      ...
      LOOP
        IF p_x_space_capbl_tbl(i).operation_flag = 'D' THEN
         AHL_LTP_SPACE_CAPB_PVT.DELETE_SPACE_CAPBLTS
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
 ...
 WHEN FND_API.G_EXC_UNEXPECTED_ERROR THEN
   ...
   IF G_DEBUG='Y' THEN
            ...
            AHL_DEBUG_PUB.debug( 'ahl_ltp_space_capb_pub.Process Space Capbl','+SPCBL+');
        END IF;
        -- Check if API is called in debug mode. If yes, disable debug.
        AHL_DEBUG_PUB.disable_debug;
...
WHEN OTHERS THEN
    ...
    THEN
    fnd_msg_pub.add_exc_msg(p_pkg_name        =>  'AHL_LTP_SPACE_CAPB_PUB',
   ...
   IF G_DEBUG='Y' THEN
            ...
            AHL_DEBUG_PUB.debug( 'ahl_ltp_space_unavl_pub.Process Space Capbl','+SPCBL+');
        END IF;
        -- Check if API is called in debug mode. If yes, disable debug.
        AHL_DEBUG_PUB.disable_debug;
