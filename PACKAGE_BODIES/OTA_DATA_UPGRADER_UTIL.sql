--------------------------------------------------------
--  DDL for Package Body OTA_DATA_UPGRADER_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_DATA_UPGRADER_UTIL" AS
/* $Header: otdatupg.pkb 120.1 2005/08/01 00:57:07 jbharath noship $ */
...
begin
  ...
  loop
	...
	l_plsql := 'begin '||p_upg_proc||'(:CTRL, :START, :END, :ROWCOUNT,:UPGRADEID); end;';
	execute immediate l_plsql
		using p_process_ctrl,
			l_start_rowid,
			l_end_rowid,
		OUT l_rows_processed,
		to_number(p_upgrade_id) ;
...
begin
  ...
  if l_table_rowcount > 2 * p_total_num_workers then
    if p_use_rowid = 'Y' then
	/*
	** Process upgrade using rowid....
	*/
       upgradeUsingRowid
 	(p_this_worker_num
	,p_total_num_workers
  ...
  elsif l_min_id is not null and l_max_id is not null then
    /*
