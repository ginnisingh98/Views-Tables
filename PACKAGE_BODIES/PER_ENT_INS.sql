--------------------------------------------------------
--  DDL for Package Body PER_ENT_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ENT_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_ent_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_calendar_entry_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  per_ent_ins.g_calendar_entry_id_i := p_calendar_entry_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: per_calendar_entries
...
--
  Cursor C_Sel1 is select per_calendar_entries_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_calendar_entries
     where calendar_entry_id =
             per_ent_ins.g_calendar_entry_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (per_ent_ins.g_calendar_entry_id_i is not null) Then
    --
    ...
    If C_Sel2%found Then
       ...
       fnd_message.set_token('TABLE_NAME','per_calendar_entries');
    ...
    p_rec.calendar_entry_id :=
      per_ent_ins.g_calendar_entry_id_i;
    per_ent_ins.g_calendar_entry_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.calendar_entry_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  per_ent_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  per_ent_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  per_ent_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  -- Having converted the arguments into the per_ent_rec
  ...
  --
  p_calendar_entry_id := l_rec.calendar_entry_id;
