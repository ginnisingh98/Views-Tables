--------------------------------------------------------
--  DDL for Package Body PER_CEO_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CEO_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_ceo_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  -- Update the per_comp_element_outcomes Row
  --
  update per_comp_element_outcomes
    set
     comp_element_outcome_id         = p_rec.comp_element_outcome_id
...
Begin
  ...
  --
  If (p_rec.competence_element_id = hr_api.g_number) then
    p_rec.competence_element_id :=
    per_ceo_shd.g_old_rec.competence_element_id;
  End If;
  If (p_rec.outcome_id = hr_api.g_number) then
    p_rec.outcome_id :=
    per_ceo_shd.g_old_rec.outcome_id;
  End If;
...
Begin
  ...
  --
  per_ceo_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_ceo_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_ceo_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
