--------------------------------------------------------
--  DDL for Package Body IGS_ST_GOVT_SEMESTER_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_ST_GOVT_SEMESTER_PKG" AS
/* $Header: IGSVI06B.pls 115.5 2002/11/29 04:31:59 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_ST_GOVT_SEMESTER%RowType;
  new_references IGS_ST_GOVT_SEMESTER%RowType;
  ...
  PROCEDURE BeforeRowInsertUpdateDelete1(
	...
	v_submission_yr		IGS_ST_GOVT_SEMESTER.submission_yr%TYPE;
	v_submission_number	IGS_ST_GOVT_SEMESTER.submission_number%TYPE;
  BEGIN
...
||  Changed the reference of "IGS_ST_VAL_GSEM.STAP_VAL_GSC_SDT_UPD" to program unit "IGS_ST_VAL_GSC.STAP_VAL_GSC_SDT_UPD". -- kdande
*/
...
PROCEDURE Check_Constraints (
 ...
 ELSIF upper(Column_name) = 'GOVT_SEMESTER' then
     new_references.govt_semester := IGS_GE_NUMBER.to_num(column_value);
END IF;

IF upper(column_name) = 'SUBMISSION_YR' OR
     column_name is null Then
     IF new_references.submission_yr  < 0000 OR
          new_references.submission_yr > 9999 Then
...
IF upper(column_name) = 'GOVT_SEMESTER' OR
     column_name is null Then
     IF new_references.govt_semester  < 1 OR
          new_references.govt_semester > 5 Then
       Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
      IGS_GE_MSG_STACK.ADD;
       App_Exception.Raise_Exception;
     END IF;
END IF;
 END Check_Constraints;
  ...
  BEGIN
    ...
    IGS_ST_GVTSEMLOAD_CA_PKG.GET_FK_IGS_ST_GOVT_SEMESTER (
      old_references.submission_yr,
      old_references.submission_number,
      old_references.govt_semester
      );

    IGS_ST_GVTSEMLOAD_OV_PKG.GET_FK_IGS_ST_GOVT_SEMESTER (
      old_references.submission_yr,
      old_references.submission_number,
      old_references.govt_semester
      );

    IGS_ST_GVT_STDNT_LBL_PKG.GET_FK_IGS_ST_GOVT_SEMESTER (
      old_references.submission_yr,
      old_references.submission_number,
      old_references.govt_semester
      );

    IGS_ST_GVT_STDNTLOAD_PKG.GET_FK_IGS_ST_GOVT_SEMESTER (
      old_references.submission_yr,
      old_references.submission_number,
      old_references.govt_semester
      );
...
AS
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_ST_GSEM_GSC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_ST_GOVT_SEMESTER
      where SUBMISSION_YR = X_SUBMISSION_YR
      and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
      and GOVT_SEMESTER = X_GOVT_SEMESTER;
...
begin
  ...
  insert into IGS_ST_GOVT_SEMESTER (
    SUBMISSION_YR,
    SUBMISSION_NUMBER,
    GOVT_SEMESTER,
    CREATION_DATE,
...
Before_DML (
  ...
  delete from IGS_ST_GOVT_SEMESTER
  where ROWID = X_ROWID;
