--------------------------------------------------------
--  DDL for Package Body BEN_RGR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_RGR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_rgr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
	  ...
	  (p_base_table_name	=> 'ben_popl_rptg_grp_f',
	   ...
	   p_base_key_value	=> p_rec.popl_rptg_grp_id);
    --
    ben_rgr_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_popl_rptg_grp_f Row
    --
    ...
    --
    ben_rgr_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_rgr_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.popl_rptg_grp_id,
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'ben_popl_rptg_grp_f'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_rgr_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.business_group_id = hr_api.g_number) then
    ...
    ben_rgr_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.rptg_grp_id = hr_api.g_number) then
    p_rec.rptg_grp_id :=
    ben_rgr_shd.g_old_rec.rptg_grp_id;
  End If;
  If (p_rec.pgm_id = hr_api.g_number) then
    p_rec.pgm_id :=
    ben_rgr_shd.g_old_rec.pgm_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_rgr_shd.g_old_rec.pl_id;
  End If;
  --iRec
  If (p_rec.ordr_num = hr_api.g_number) then
    p_rec.ordr_num :=
    ben_rgr_shd.g_old_rec.ordr_num;
  End If;
  --iRec
  If (p_rec.rgr_attribute_category = hr_api.g_varchar2) then
    p_rec.rgr_attribute_category :=
    ben_rgr_shd.g_old_rec.rgr_attribute_category;
  End If;
  If (p_rec.rgr_attribute1 = hr_api.g_varchar2) then
    p_rec.rgr_attribute1 :=
    ben_rgr_shd.g_old_rec.rgr_attribute1;
  End If;
  If (p_rec.rgr_attribute2 = hr_api.g_varchar2) then
    p_rec.rgr_attribute2 :=
    ben_rgr_shd.g_old_rec.rgr_attribute2;
  End If;
  If (p_rec.rgr_attribute3 = hr_api.g_varchar2) then
    p_rec.rgr_attribute3 :=
    ben_rgr_shd.g_old_rec.rgr_attribute3;
  End If;
  If (p_rec.rgr_attribute4 = hr_api.g_varchar2) then
    p_rec.rgr_attribute4 :=
    ben_rgr_shd.g_old_rec.rgr_attribute4;
  End If;
  If (p_rec.rgr_attribute5 = hr_api.g_varchar2) then
    p_rec.rgr_attribute5 :=
    ben_rgr_shd.g_old_rec.rgr_attribute5;
  End If;
  If (p_rec.rgr_attribute6 = hr_api.g_varchar2) then
    p_rec.rgr_attribute6 :=
    ben_rgr_shd.g_old_rec.rgr_attribute6;
  End If;
  If (p_rec.rgr_attribute7 = hr_api.g_varchar2) then
    p_rec.rgr_attribute7 :=
    ben_rgr_shd.g_old_rec.rgr_attribute7;
  End If;
  If (p_rec.rgr_attribute8 = hr_api.g_varchar2) then
    p_rec.rgr_attribute8 :=
    ben_rgr_shd.g_old_rec.rgr_attribute8;
  End If;
  If (p_rec.rgr_attribute9 = hr_api.g_varchar2) then
    p_rec.rgr_attribute9 :=
    ben_rgr_shd.g_old_rec.rgr_attribute9;
  End If;
  If (p_rec.rgr_attribute10 = hr_api.g_varchar2) then
    p_rec.rgr_attribute10 :=
    ben_rgr_shd.g_old_rec.rgr_attribute10;
  End If;
  If (p_rec.rgr_attribute11 = hr_api.g_varchar2) then
    p_rec.rgr_attribute11 :=
    ben_rgr_shd.g_old_rec.rgr_attribute11;
  End If;
  If (p_rec.rgr_attribute12 = hr_api.g_varchar2) then
    p_rec.rgr_attribute12 :=
    ben_rgr_shd.g_old_rec.rgr_attribute12;
  End If;
  If (p_rec.rgr_attribute13 = hr_api.g_varchar2) then
    p_rec.rgr_attribute13 :=
    ben_rgr_shd.g_old_rec.rgr_attribute13;
  End If;
  If (p_rec.rgr_attribute14 = hr_api.g_varchar2) then
    p_rec.rgr_attribute14 :=
    ben_rgr_shd.g_old_rec.rgr_attribute14;
  End If;
  If (p_rec.rgr_attribute15 = hr_api.g_varchar2) then
    p_rec.rgr_attribute15 :=
    ben_rgr_shd.g_old_rec.rgr_attribute15;
  End If;
  If (p_rec.rgr_attribute16 = hr_api.g_varchar2) then
    p_rec.rgr_attribute16 :=
    ben_rgr_shd.g_old_rec.rgr_attribute16;
  End If;
  If (p_rec.rgr_attribute17 = hr_api.g_varchar2) then
    p_rec.rgr_attribute17 :=
    ben_rgr_shd.g_old_rec.rgr_attribute17;
  End If;
  If (p_rec.rgr_attribute18 = hr_api.g_varchar2) then
    p_rec.rgr_attribute18 :=
    ben_rgr_shd.g_old_rec.rgr_attribute18;
  End If;
  If (p_rec.rgr_attribute19 = hr_api.g_varchar2) then
    p_rec.rgr_attribute19 :=
    ben_rgr_shd.g_old_rec.rgr_attribute19;
  End If;
  If (p_rec.rgr_attribute20 = hr_api.g_varchar2) then
    p_rec.rgr_attribute20 :=
    ben_rgr_shd.g_old_rec.rgr_attribute20;
  End If;
  If (p_rec.rgr_attribute21 = hr_api.g_varchar2) then
    p_rec.rgr_attribute21 :=
    ben_rgr_shd.g_old_rec.rgr_attribute21;
  End If;
  If (p_rec.rgr_attribute22 = hr_api.g_varchar2) then
    p_rec.rgr_attribute22 :=
    ben_rgr_shd.g_old_rec.rgr_attribute22;
  End If;
  If (p_rec.rgr_attribute23 = hr_api.g_varchar2) then
    p_rec.rgr_attribute23 :=
    ben_rgr_shd.g_old_rec.rgr_attribute23;
  End If;
  If (p_rec.rgr_attribute24 = hr_api.g_varchar2) then
    p_rec.rgr_attribute24 :=
    ben_rgr_shd.g_old_rec.rgr_attribute24;
  End If;
  If (p_rec.rgr_attribute25 = hr_api.g_varchar2) then
    p_rec.rgr_attribute25 :=
    ben_rgr_shd.g_old_rec.rgr_attribute25;
  End If;
  If (p_rec.rgr_attribute26 = hr_api.g_varchar2) then
    p_rec.rgr_attribute26 :=
    ben_rgr_shd.g_old_rec.rgr_attribute26;
  End If;
  If (p_rec.rgr_attribute27 = hr_api.g_varchar2) then
    p_rec.rgr_attribute27 :=
    ben_rgr_shd.g_old_rec.rgr_attribute27;
  End If;
  If (p_rec.rgr_attribute28 = hr_api.g_varchar2) then
    p_rec.rgr_attribute28 :=
    ben_rgr_shd.g_old_rec.rgr_attribute28;
  End If;
  If (p_rec.rgr_attribute29 = hr_api.g_varchar2) then
    p_rec.rgr_attribute29 :=
    ben_rgr_shd.g_old_rec.rgr_attribute29;
  End If;
  If (p_rec.rgr_attribute30 = hr_api.g_varchar2) then
    p_rec.rgr_attribute30 :=
    ben_rgr_shd.g_old_rec.rgr_attribute30;
  End If;
