--------------------------------------------------------
--  DDL for Package Body FA_FASRSVED_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FA_FASRSVED_XMLP_PKG" AS
/* $Header: FASRSVEDB.pls 120.0.12010000.1 2008/07/28 13:17:30 appldev ship $ */
...
IF (P_REPORT_TYPE = 'REVAL RESERVE') THEN
      RP_Report_Name := ':Revaluation Reserve Detail Report:';
ELSE  RP_Report_Name := ':Reserve Detail Report:';
END IF;
...
END IF;
  ...
  RP_PERIOD_FROM := P_PERIOD1;     return(l_period_PC);
END;
RETURN NULL; end;
...
END IF;
  ...
  RP_PERIOD_TO := P_PERIOD2;     return(l_period_PC);
END;
RETURN NULL; end;
...
--function out_of_balanceformula(BEGIN in number, ADDITION in number, DEPRECIATION in number, RECLASS in number, RETIREMENT in number, ADJUST in number, TRANSFER in number, END in number) return varchar2 is
function out_of_balanceformula(CHANGED_BEGIN in number, ADDITION in number, DEPRECIATION in number, RECLASS in number, RETIREMENT in number, ADJUST in number, TRANSFER in number, CHANGED_END in number) return varchar2 is
begin
...
--MOCK_TOTAL := NVL(BEGIN,0) + NVL(ADDITION,0) + NVL(DEPRECIATION,0)
MOCK_TOTAL := NVL(CHANGED_BEGIN,0) + NVL(ADDITION,0) + NVL(DEPRECIATION,0)
	+ NVL(RECLASS,0) - NVL(RETIREMENT,0) + NVL(ADJUST,0)
	+ NVL(TRANSFER,0);

--IF (MOCK_TOTAL = NVL(END,0))
IF (MOCK_TOTAL = NVL(CHANGED_END,0))
THEN RETURN (' ');
ELSE RETURN('*');
END IF;
END;
...
function ctr_out_of_balanceformula(CTR_BEGIN in number, CTR_ADD in number, CTR_DEPRN in number, CTR_RECLASS in number, CTR_RETIRE in number, CTR_ADJUST in number, CTR_TRANS in number, CTR_END in number) return varchar2 is
begin
...
MOCK_TOTAL := NVL(CTR_BEGIN,0) + NVL(CTR_ADD,0) + NVL(CTR_DEPRN,0)
	+ NVL(CTR_RECLASS,0) - NVL(CTR_RETIRE,0) + NVL(CTR_ADJUST,0)
	+ NVL(CTR_TRANS,0);

IF (MOCK_TOTAL = NVL(CTR_END,0))
THEN RETURN (' ');
ELSE RETURN('*');
END IF;
END;
...
function acct_out_of_balanceformula(ACCT_BEGIN in number, ACCT_ADD in number, ACCT_DEPRN in number, ACCT_RECLASS in number, ACCT_RETIRE in number, ACCT_ADJUST in number, ACCT_TRANS in number, ACCT_END in number) return varchar2 is
begin
...
MOCK_TOTAL := NVL(ACCT_BEGIN,0) + NVL(ACCT_ADD,0) + NVL(ACCT_DEPRN,0)
...
function bal_out_of_balanceformula(BAL_BEGIN in number, BAL_ADD in number, BAL_DEPRN in number, BAL_RECLASS in number, BAL_RETIRE in number, BAL_ADJUST in number, BAL_TRANS in number, BAL_END in number) return varchar2 is
begin
...
MOCK_TOTAL := NVL(BAL_BEGIN,0) + NVL(BAL_ADD,0) + NVL(BAL_DEPRN,0)
...
function rp_out_of_balanceformula(RP_BEGIN in number, RP_ADD in number, RP_DEPRN in number, RP_RECLASS in number, RP_RETIRE in number, RP_ADJUST in number, RP_TRANS in number, RP_END in number) return varchar2 is
begin
...
MOCK_TOTAL := NVL(RP_BEGIN,0) + NVL(RP_ADD,0) + NVL(RP_DEPRN,0)
...
function adjustformula(TAX in number, REVALUATION in number) return number is
begin

BEGIN

RETURN(nvl(TAX, 0)+ nvl(REVALUATION, 0));

END;
RETURN NULL; end;

function reval_tax_flagformula(REVALUATION in number, TAX in number) return varchar2 is
begin

BEGIN

IF ((REVALUATION <> 0) AND (TAX = 0))
	THEN 	IF (P_REPORT_TYPE = 'RESERVE')
		THEN RETURN('R');
		ELSE RETURN(' ');
		END IF;
ELSE IF ((REVALUATION <> 0) AND (TAX <> 0))
	THEN RETURN('B');
ELSE IF ((REVALUATION = 0) AND (TAX <> 0))
	THEN RETURN('T');
ELSE RETURN(' ');
...
--Functions to refer Oracle report placeholders--
 ...
 Function RP_BAL_LPROMPT_p return varchar2 is
	Begin
	 ...
	 END;
 Function RP_CTR_LPROMPT_p return varchar2 is
	Begin
	 return RP_CTR_LPROMPT;
	 END;
 Function RP_PERIOD_FROM_p return varchar2 is
	Begin
	 return RP_PERIOD_FROM;
	 END;
 Function RP_PERIOD_TO_p return varchar2 is
	Begin
	 return RP_PERIOD_TO;
	 END;
