--------------------------------------------------------
--  DDL for Package Body FII_AP_SCH_PAYMTS_F_C
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FII_AP_SCH_PAYMTS_F_C" AS
/* $Header: FIIAP11B.pls 120.10 2004/11/19 06:09:54 sgautam ship $ */
 ...
 g_acct_or_inv_date   Number; -- Added for Currency Conversion Enhancement , 4-APR-03
...
BEGIN
  ...
  l_mau := nvl( edw_currency.get_mau, 0.01 );
   ...
   fii_flex_mapping.init_cache('FII_AP_SCH_PAYMTS_F');
...
---------------------------------------------------
 ...
 BEGIN
   ...
   -- will be stored in the column Primary_Key5 . Modified for Currency Conversion Enhancement, 4-APR-03
   -----------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------
 ...
 l_fact_name   Varchar2(30) :='FII_AP_SCH_PAYMTS_F'  ;
 l_date1                Date:=Null;
 l_date2                Date:=Null;
 ...
 l_count Number:=0; --bug#3947925
   ...
   l_to_currency     VARCHAR2(15); -- Added for Currency Conversion Enhancement , 4-APR-03
   ...
   -- is less than the sysdate i.e. in past.  Added for Currency Conversion Enhancement , 4-APR-03
   ----------------------------------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------------------------------------
   -- This cursor is for getting records where the CREATION_DATE (i.e. GL_DATE or INVOICE_DATE )
   -- is greater than the sysdate i.e. in future.  Added for Currency Conversion Enhancement , 3-APR-03
   ----------------------------------------------------------------------------------------------------
...
Begin
  ...
  END IF;
  FII_AP_SCH_PAYMTS_F_C.g_push_date_range1 := nvl(l_from_date,
  		EDW_COLLECTION_UTIL.G_local_last_push_start_date - EDW_COLLECTION_UTIL.g_offset);
  FII_AP_SCH_PAYMTS_F_C.g_push_date_range2 := nvl(l_to_date,EDW_COLLECTION_UTIL.G_local_curr_push_start_date);
...
l_date2 := g_push_date_range2;
   ...
   if (l_count=0) then
    ...
    TRUNCATE_TABLE('FII_AP_TMP_SCHP_PK');--bug#3947925
   ...
   ----------------------------------------------------------------------------------------------------------
   -- See whether to use accounting date or invoice date . Added for Currency Conversion Enhancement 3-APR-03
   ----------------------------------------------------------------------------------------------------------
   ...
   IF (NOT LOCAL_SAME_AS_REMOTE) THEN
         TRUNCATE_TABLE('FII_AP_SCH_PAYMTS_FSTG');
   ELSE
         DELETE_STG;
   END IF;
   ...
   --  --------------------------------------------------------
   --  2. Identify Changed AP Scheduled Payments record
   --  --------------------------------------------------------
    edw_log.put_line(' ');
    edw_log.put_line('Identifying changed AP Scheduled Payments record');
   ...
   --  --------------------------------------------------------------------
   TRUNCATE_TABLE('fii_ap_tmp_schp_pk');
...
--  DELETE_STG;
   ...
   IF (NOT LOCAL_SAME_AS_REMOTE) THEN
           ...
           edw_log.put_line(' ');
           edw_log.put_line(to_char(g_row_count));
