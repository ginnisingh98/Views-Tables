--------------------------------------------------------
--  DDL for Package Body AME_RUL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_RUL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_rul_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    ,rule_key                             = p_rec.rule_key
    ,item_class_id                        = p_rec.item_class_id
    ,object_version_number                = p_rec.object_version_number
    where   rule_id  =   p_rec.rule_id
...
Begin
  ...
  begin
    ...
    ame_rul_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_rule_type_o
      => ame_rul_shd.g_old_rec.rule_type
      ,p_action_id_o
      => ame_rul_shd.g_old_rec.action_id
      ,p_start_date_o
      => ame_rul_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_rul_shd.g_old_rec.end_date
      ,p_description_o
      => ame_rul_shd.g_old_rec.description
      ,p_security_group_id_o
      => ame_rul_shd.g_old_rec.security_group_id
      ,p_rule_key_o
      => ame_rul_shd.g_old_rec.rule_key
      ,p_item_class_id_o
      => ame_rul_shd.g_old_rec.item_class_id
      ,p_object_version_number_o
      => ame_rul_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  --
  If (p_rec.rule_type = hr_api.g_number) then
    p_rec.rule_type :=
    ame_rul_shd.g_old_rec.rule_type;
  End If;
  If (p_rec.action_id = hr_api.g_number) then
    p_rec.action_id :=
    ame_rul_shd.g_old_rec.action_id;
  End If;
  ...
  If (p_rec.security_group_id = hr_api.g_number) then
    ...
    ame_rul_shd.g_old_rec.security_group_id;
  End If;
  If (p_rec.rule_key = hr_api.g_varchar2) then
    p_rec.rule_key :=
    ame_rul_shd.g_old_rec.rule_key;
  End If;
...
Begin
  ...
  --
  ame_rul_upd.convert_defs(p_rec);
  --
...
Procedure upd
  ...
  ,p_rule_type                    in     number    default hr_api.g_number
  ,p_description                  in     varchar2  default hr_api.g_varchar2
  ,p_rule_key                     in     varchar2  default hr_api.g_varchar2
  ,p_action_id                    in     number    default hr_api.g_number
