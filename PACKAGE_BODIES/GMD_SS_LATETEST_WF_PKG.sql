--------------------------------------------------------
--  DDL for Package Body GMD_SS_LATETEST_WF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMD_SS_LATETEST_WF_PKG" AS
/* $Header: GMDQSLTB.pls 120.6 2006/12/05 16:08:07 rlnagara noship $ */
...
PROCEDURE VERIFY_EVENT(
 ...
 l_transaction_type varchar2(100):='GMDQM_STABILITY_LATETEST';
 l_user varchar2(32);
 Approver ame_util.approverRecord;

   l_organization varchar2(3);  --INVCONV
   l_form varchar2(240);
   ...
   l_item_revision varchar2(3);   --INVCONV
   ...
   l_storage_subinventory varchar2(10); --INVCONV
   l_storage_locator varchar2(240);  --INVCONV
...
select  distinct k.organization_code, b.meaning, e.ss_no, h.variant_no,f.description,
   h.samples_per_time_point , h.storage_subinventory, j.concatenated_segments storage_locator,
   h.resources, i.spec_name, i.spec_vers, c.scheduled_date,
   d.concatenated_segments item_no, d.description item_desc1, e. revision, c.sampling_event_id,
   e.owner , c.name
...
Cursor C2 (se_id NUMBER) is
 ...
 BEGIN
     ...
     IF (l_debug = 'Y') THEN
 	      gmd_debug.log_initialize('TimepointLate');
     END IF;
    ...
    IF P_FUNCMODE='RUN' THEN
     ...
     /* Get application_id from FND_APPLICATION */
      ...
      wf_log_pkg.string(6, 'Dummy','Entered Stability Study Timepoint testing late '||l_event_key);
...
--             	l_form := 'GMDQSMPL_EDIT_F:SAMPLING_EVENT_ID="'||l_sample_event_id||'"';
		...
		IF (l_debug = 'Y') THEN
	        ...
	        END IF;
                 ...
                 l_workflow_process:='GMDQSTSL_SUB_PROCESS';
  ...
  EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMD_SS_LATETEST_WF_PKG','VERIFY_EVENT',p_itemtype,p_itemkey,l_log );
      raise;
...
PROCEDURE CHECK_NEXT_APPROVER(
 ...
 BEGIN
  ...
  EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMD_SS_LATETEST_WF_PKG','CHECK_NEXT_APPROVER',p_itemtype,p_itemkey,'Initial' );
      raise;
