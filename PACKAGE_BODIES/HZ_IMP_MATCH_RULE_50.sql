--------------------------------------------------------
--  DDL for Package Body HZ_IMP_MATCH_RULE_50
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_IMP_MATCH_RULE_50" AS
    g_match_rule_id NUMBER := 50;
...
----------- SETTING GLOBAL CONDITION RECORD AT THE PARTY_SITES LEVEL ---------
  ...
  select a.URL, a.party_orig_system, a.party_orig_system_reference, a.cp_orig_system, a.cp_orig_system_reference, a.site_orig_system, a.site_orig_system_reference, b.party_site_id, b.contact_point_id, b.party_id, b.party_action_flag, a.rowid, a.contact_point_type, decode(a.raw_phone_number, null, a.PHONE_COUNTRY_CODE||a.PHONE_AREA_CODE ||a.phone_number, a.raw_phone_number) as raw_phone_number 
   ...
   LOOP 
      FETCH l_cp_cur BULK COLLECT INTO 
       H_49E, H_P_PARTY_OS, H_P_PARTY_OSR, H_P_CP_OS, H_P_CP_OSR, H_P_PS_OS, H_P_PS_OSR, H_P_PARTY_SITE_ID, H_P_CONTACT_POINT_ID, H_P_PARTY_ID, H_P_N_PARTY, H_P_ROW_ID, H_P_CP_TYPE, H_P_CP_R_PH_NO 
       LIMIT g_limit; 
     ...
     FOR I in H_P_PARTY_OSR.FIRST..H_P_PARTY_OSR.LAST LOOP   
        ...
        H_TX8(I) := HZ_TRANS_PKG.CLEANSED_URL(H_49E(I), NULL, 'URL', 'CONTACT_POINTS', 'SEARCH' );
     ...
     BEGIN      
       FORALL I in H_P_PARTY_OSR.FIRST..H_P_PARTY_OSR.LAST  
         INSERT INTO HZ_SRCH_CPTS  ( 
          TX7, TX8, PARTY_SITE_ID, PARTY_ID, PARTY_OS, PARTY_OSR, PARTY_SITE_OS, PARTY_SITE_OSR, CONTACT_POINT_ID, CONTACT_PT_OS, CONTACT_PT_OSR, NEW_PARTY_FLAG, BATCH_ID, INT_ROW_ID, CONTACT_POINT_TYPE , TX10, TX158
        ) VALUES ( 
          H_TX7(I), H_TX8(I), H_P_PARTY_SITE_ID(I), H_P_PARTY_ID(I), H_P_PARTY_OS(I), H_P_PARTY_OSR(I), H_P_PS_OS(I), H_P_PS_OSR(I) , H_P_CONTACT_POINT_ID(I), H_P_CP_OS(I), H_P_CP_OSR(I), H_P_N_PARTY(I), P_BATCH_ID, H_P_ROW_ID(I), H_P_CP_TYPE(I) , H_CP_CUST_TX10(I), H_CP_CUST_TX158(I)
            );  
      EXCEPTION  
        WHEN OTHERS THEN  
          ROLLBACK to POP_CP;  
  ...
  END pop_cp; 
   PROCEDURE get_contact_cur( 
      ...
      where match_rule_id = 50
      union 
      select attribute_id 
      from hz_match_rule_secondary b 
      where match_rule_id = 50 ) and rownum = 1;   
...
----------- SETTING GLOBAL CONDITION RECORD AT THE PARTY_SITES LEVEL ---------
  ...
  select a.URL, a.party_orig_system, a.party_orig_system_reference, a.cp_orig_system, a.cp_orig_system_reference, a.site_orig_system, a.site_orig_system_reference, a.rowid, a.contact_point_type,decode(a.raw_phone_number, null, a.PHONE_COUNTRY_CODE||a.PHONE_AREA_CODE ||a.phone_number,a.raw_phone_number) as raw_phone_number  , a.party_id 
   ...
   LOOP 
       FETCH l_cp_cur BULK COLLECT INTO 
       H_49E, H_P_PARTY_OS, H_P_PARTY_OSR, H_P_CP_OS, H_P_CP_OSR, H_P_PS_OS, H_P_PS_OSR, H_P_ROW_ID, H_P_CP_TYPE, H_P_CP_R_PH_NO  , H_P_PARTY_ID 
       LIMIT g_limit; 
     IF l_cp_cur%NOTFOUND THEN 
       l_last_fetch:=TRUE; 
     END IF; 
     ...
     BEGIN      
       FORALL I in H_P_PARTY_OSR.FIRST..H_P_PARTY_OSR.LAST 
         INSERT INTO HZ_SRCH_CPTS ( 
          TX7, TX8, PARTY_OS, PARTY_OSR, PARTY_SITE_OS, PARTY_SITE_OSR, CONTACT_PT_OS, CONTACT_PT_OSR, BATCH_ID, INT_ROW_ID, CONTACT_POINT_TYPE, TX10, TX158 , PARTY_ID 
         ) VALUES ( 
          H_TX7(I), H_TX8(I), H_P_PARTY_OS(I), H_P_PARTY_OSR(I), H_P_PS_OS(I), H_P_PS_OSR(I), H_P_CP_OS(I), H_P_CP_OSR(I), P_BATCH_ID, H_P_ROW_ID(I), H_P_CP_TYPE(I), H_CP_CUST_TX10(I), H_CP_CUST_TX158(I) ,H_P_PARTY_ID(I) 
             ); 
       EXCEPTION  
         WHEN OTHERS THEN 
...
select /*+ ORDERED */ s1.party_id f, s2.party_id t,
-------PARTY_SITES ENTITY: SCORING SECTION ---------
...
0
 ...
 score 
from hz_dup_worker_chunk_gt p, HZ_STAGED_PARTY_SITES s1, HZ_STAGED_PARTY_SITES s2
where p.party_id = s1.party_id and s1.party_id<>s2.party_id 
...
and (
-------PARTY_SITES ENTITY: ACQUISITION ON NON-FILTER ATTRIBUTES---------
-- do an or between all the transformations of an attribute -- 
(
(s1.TX26 is not null and s2.TX26 like s1.TX26 || decode(sign(lengthb(s1.TX26)-3),1,'%',''))
)
)
-------PARTY_SITES ENTITY: ACQUISITION ON FILTER ATTRIBUTES---------
and
-- do an or between all the transformations of an attribute -- 
(
...
FND_FILE.put_line(FND_FILE.log,'Beginning update of Parties on the basis of CONTACT_POINTS');
...
score
from hz_dup_worker_chunk_gt p, hz_dup_results h1, HZ_STAGED_CONTACT_POINTS s1, HZ_STAGED_CONTACT_POINTS s2
...
-- do an or between all the transformations of an attribute -- 
(
(s1.TX8 is not null and s2.TX8 like s1.TX8 || decode(sign(lengthb(s1.TX8)-3),1,'%',''))
)
)
) group by f,t ;
...
FND_FILE.put_line(FND_FILE.log,'Ending update of Parties on the basis of CONTACT_POINTS');
FND_CONCURRENT.AF_Commit;
...
WHEN OTHERS THEN
         ...
         ELSE
             ...
             FND_MESSAGE.SET_TOKEN('PROC','HZ_IMP_MATCH_RULE_50.tca_join_entities');
...
score
from hz_imp_dup_parties h1, HZ_SRCH_CPTS s1, HZ_STAGED_CONTACT_POINTS s2
...
WHEN OTHERS THEN
         FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
         FND_MESSAGE.SET_TOKEN('PROC','HZ_IMP_MATCH_RULE_50.interface_tca_join_entities');
         FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
         FND_MSG_PUB.ADD;
         RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
...
score
from hz_int_dup_results h1, HZ_SRCH_CPTS s1, HZ_SRCH_CPTS s2
where
...
WHEN OTHERS THEN
         FND_MESSAGE.SET_NAME('AR', 'HZ_DQM_API_ERROR');
         FND_MESSAGE.SET_TOKEN('PROC','HZ_IMP_MATCH_RULE_50.interface_join_entities');
         FND_MESSAGE.SET_TOKEN('ERROR' ,SQLERRM );
         FND_MSG_PUB.ADD;
         RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
