--------------------------------------------------------
--  DDL for Package Body BEN_PND_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PND_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pnd_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_pl_dpnt_cvg_ctfn_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   pl_dpnt_cvg_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_pl_dpnt_cvg_ctfn_id(p_pl_dpnt_cvg_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pl_dpnt_cvg_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_pnd_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pl_dpnt_cvg_ctfn_id                => p_pl_dpnt_cvg_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pl_dpnt_cvg_ctfn_id,hr_api.g_number)
     <>  ben_pnd_shd.g_old_rec.pl_dpnt_cvg_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_pnd_shd.constraint_error('BEN_PL_DPNT_CVG_CTFN_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pl_dpnt_cvg_ctfn_id is not null then
      --
  ...
  --
End chk_pl_dpnt_cvg_ctfn_id;
--
-- ----------------------------------------------------------------------------
-- |---------------< chk_dpnt_cvg_ctfn_typ_unique >------------------------|
-- ----------------------------------------------------------------------------
--
-- Description
--   ensure that no two dependent certification types have the same value
--
-- Pre Conditions
--   None.
--
-- In Parameters
--     p_dpnt_cvg_ctfn_typ_cd  is dependent certification type code
--     p_rlshp_typ_cd          is relationship type code
--     p_pl_dpnt_cvg_ctfn_id   is primary key for ben_pl_dpnt_cvg_ctfn_f
--     p_pl_id
--     p_business_group_id
...
--
-- ----------------------------------------------------------------------------
Procedure chk_dpnt_cvg_ctfn_typ_unique
...
is
l_proc      varchar2(72) := g_package||'chk_dpnt_cvg_ctfn_typ_unique';
l_dummy    char(1);
cursor c1 is select null
               from ben_pl_dpnt_cvg_ctfn_f
              Where pl_dpnt_cvg_ctfn_id <> nvl(p_pl_dpnt_cvg_ctfn_id,-1)
                and nvl(dpnt_cvg_ctfn_typ_cd,0) = nvl(p_dpnt_cvg_ctfn_typ_cd,0)
...
End chk_dpnt_cvg_ctfn_typ_unique;
--
-- ----------------------------------------------------------------------------
-- |------< chk_rlshp_typ_cd >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_rlshp_typ_cd(p_pl_dpnt_cvg_ctfn_id                in number,
                            p_rlshp_typ_cd               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      and p_rlshp_typ_cd
      <> nvl(ben_pnd_shd.g_old_rec.rlshp_typ_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_rqd_flag(p_pl_dpnt_cvg_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_rqd_flag
      <> nvl(ben_pnd_shd.g_old_rec.rqd_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_dpnt_cvg_ctfn_typ_cd(p_pl_dpnt_cvg_ctfn_id                in number,
                            p_dpnt_cvg_ctfn_typ_cd               in varchar2,
                            p_effective_date              in date,
                            p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_pnd_shd.g_old_rec.dpnt_cvg_ctfn_typ_cd,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_lack_ctfn_sspnd_enrt_flag(p_pl_dpnt_cvg_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_pnd_shd.g_old_rec.lack_ctfn_sspnd_enrt_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_pl_dpnt_cvg_ctfn_id       in number,
  ...
  if (l_api_updating
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        fnd_message.set_token('TYPE_ID',-26);
        fnd_message.raise_error;
        --
      end if;
      --
...
--   Internal table handler use only.
--
Procedure chk_pfd_flag(p_pl_dpnt_cvg_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_pfd_flag
      <> nvl(ben_pnd_shd.g_old_rec.pfd_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
function return_legislation_code
  (p_pl_dpnt_cvg_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pl_dpnt_cvg_ctfn_f b
    where b.pl_dpnt_cvg_ctfn_id      = p_pl_dpnt_cvg_ctfn_id
