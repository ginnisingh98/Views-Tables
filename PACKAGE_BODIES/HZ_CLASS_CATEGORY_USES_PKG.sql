--------------------------------------------------------
--  DDL for Package Body HZ_CLASS_CATEGORY_USES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_CLASS_CATEGORY_USES_PKG" AS
/*$Header: ARHCAUTB.pls 115.4 2002/11/21 21:07:48 sponnamb ship $ */
...
BEGIN
    ...
    INSERT INTO HZ_CLASS_CATEGORY_USES (
        CLASS_CATEGORY,
        OWNER_TABLE,
        COLUMN_NAME,
    ...
    VALUES (
        ...
        DECODE( X_OWNER_TABLE, FND_API.G_MISS_CHAR, NULL, X_OWNER_TABLE ),
        DECODE( X_COLUMN_NAME, FND_API.G_MISS_CHAR, NULL, X_COLUMN_NAME ),
        DECODE( X_ADDITIONAL_WHERE_CLAUSE, FND_API.G_MISS_CHAR, NULL, X_ADDITIONAL_WHERE_CLAUSE ),
...
BEGIN
    ...
    UPDATE HZ_CLASS_CATEGORY_USES SET
        COLUMN_NAME = DECODE( X_COLUMN_NAME, NULL, COLUMN_NAME, FND_API.G_MISS_CHAR, NULL, X_COLUMN_NAME ),
        ADDITIONAL_WHERE_CLAUSE = DECODE( X_ADDITIONAL_WHERE_CLAUSE, NULL, ADDITIONAL_WHERE_CLAUSE, FND_API.G_MISS_CHAR, NULL, X_ADDITIONAL_WHERE_CLAUSE ),
...
PROCEDURE Lock_Row (
    ...
    CURSOR C IS
        SELECT * FROM HZ_CLASS_CATEGORY_USES
        WHERE  ROWID = x_Rowid
        FOR UPDATE NOWAIT;
    Recinfo C%ROWTYPE;
...
BEGIN
    ...
    AND ( ( Recinfo.OWNER_TABLE = X_OWNER_TABLE )
        OR ( ( Recinfo.OWNER_TABLE IS NULL )
            AND (  X_OWNER_TABLE IS NULL ) ) )
    AND ( ( Recinfo.COLUMN_NAME = X_COLUMN_NAME )
        OR ( ( Recinfo.COLUMN_NAME IS NULL )
            AND (  X_COLUMN_NAME IS NULL ) ) )
    AND ( ( Recinfo.ADDITIONAL_WHERE_CLAUSE = X_ADDITIONAL_WHERE_CLAUSE )
        OR ( ( Recinfo.ADDITIONAL_WHERE_CLAUSE IS NULL )
            AND (  X_ADDITIONAL_WHERE_CLAUSE IS NULL ) ) )
...
BEGIN
    ...
    SELECT
        ...
        NVL( OWNER_TABLE, FND_API.G_MISS_CHAR ),
        NVL( COLUMN_NAME, FND_API.G_MISS_CHAR ),
        NVL( ADDITIONAL_WHERE_CLAUSE, FND_API.G_MISS_CHAR ),
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        ...
        FND_MESSAGE.SET_TOKEN( 'RECORD', 'class_category_use_rec');
...
BEGIN
    ...
    DELETE FROM HZ_CLASS_CATEGORY_USES
    WHERE CLASS_CATEGORY = X_CLASS_CATEGORY
    AND   OWNER_TABLE = X_OWNER_TABLE;
