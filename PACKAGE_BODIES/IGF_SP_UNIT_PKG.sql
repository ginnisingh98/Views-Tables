--------------------------------------------------------
--  DDL for Package Body IGF_SP_UNIT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_SP_UNIT_PKG" AS
/* $Header: IGFPI03B.pls 115.2 2003/03/19 08:49:57 smadathi noship $ */

  l_rowid VARCHAR2(25);
  old_references igf_sp_unit_all%ROWTYPE;
  new_references igf_sp_unit_all%ROWTYPE;
  ...
  */
  BEGIN
    ...
    IF (((old_references.fee_cls_prg_id = new_references.fee_cls_prg_id)) OR
      ...
      NULL;
    ELSIF NOT igf_sp_prg_pkg.get_pk_for_validation (
                new_references.fee_cls_prg_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SP_SFCLU_SFCLP_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
...
/*
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SP_SFCLU_UV_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
