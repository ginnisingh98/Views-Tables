--------------------------------------------------------
--  DDL for Package Body PER_RTX_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_RTX_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_rtx_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the per_rating_levels_tl Row
  --
  update per_rating_levels_tl
    set
...
Begin
  ...
  --
  per_rtx_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_rtx_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_rtx_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
  ...
  cursor csr_upd_langs is
    select rtx.language
     ...
     where rtx.rating_level_id = p_rating_level_id
       and p_language_code in (rtx.language
                              ,rtx.source_lang);
  --
