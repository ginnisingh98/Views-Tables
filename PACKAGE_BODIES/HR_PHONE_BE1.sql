--------------------------------------------------------
--  DDL for Package Body HR_PHONE_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_PHONE_BE1" as 
...
p_validity                     varchar2) is
  ...
  --
  l_proc varchar2(72):='  hr_phone_be1.create_phone_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.phone.create_phone';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<parent_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_parent_id);
    l_text:=l_text||'</parent_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<parent_table>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_parent_table);
    l_text:=l_text||'</parent_table>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
