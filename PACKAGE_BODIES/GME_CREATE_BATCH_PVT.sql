--------------------------------------------------------
--  DDL for Package Body GME_CREATE_BATCH_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_CREATE_BATCH_PVT" AS
/* $Header: GMEVCRBB.pls 120.33.12010000.9 2009/04/13 15:48:47 gmurator ship $ */
   g_debug               VARCHAR2 (5)
   ...
   BEGIN
      ...
      IF l_formula_master.scale_type = 1 THEN
       ...
       /* Create batch does not need any values from here so dummy variable */
            ...
            -- Scaling worked OK, so we now need to calculate the total scaled
            -- formula output in the Routing UOM so that we can look up a few
            -- things in the process loss table.
            IF gme_common_pvt.routings.COUNT <> 0 THEN
...
--                       pdate                  => l_material_details (l_row_count).material_requirement_date,
               ...
               IF l_material_details (l_row_count).inventory_item_id <> l_formula_tbl(1).inventory_item_id THEN
                 ...
                 INTO  l_new_item_rec.inventory_item_id, l_new_item_rec.concatenated_segments,
                       ...
                       l_new_item_rec.primary_uom_code;
      ...
      IF l_prim_prod_found AND (p_creation_mode = 'PRODUCT') THEN
         ...
         IF (g_debug = gme_debug.g_log_statement) THEN
            ...
            gme_debug.put_line ('requested qty in dtl uom rounded=' || round(l_temp_qty));
         END IF;
            ...
            IF (ROUND (p_batch_size, 5) <> ROUND (l_temp_qty, 5) ) THEN
            ...
            IF (g_debug = gme_debug.g_log_statement) THEN
               gme_debug.put_line
                  ('Batch failed to be created for qty requested with creation by product qty');
            END IF;
...
-- from x_exception_material_tbl
            ...
            gme_phantom_pvt.create_phantom
                   ...
                   ,p_batch_header_rec            => l_gme_batch_header --would be nul in this case
