--------------------------------------------------------
--  DDL for Package Body BEN_PZR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PZR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pzr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_pstl_zip_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   pstl_zip_rt_id PK of record being inserted or updated.
...
--
Procedure chk_pstl_zip_rt_id(p_pstl_zip_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pstl_zip_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_pzr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pstl_zip_rt_id                => p_pstl_zip_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pstl_zip_rt_id,hr_api.g_number)
     <>  ben_pzr_shd.g_old_rec.pstl_zip_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_pzr_shd.constraint_error('BEN_PSTL_ZIP_RATE_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pstl_zip_rt_id is not null then
      --
...
Procedure chk_pstl_zip_rng_id(
  ...
  cursor c3 is
         ...
         where pstl_zip_rng_id = p_pstl_zip_rng_id
           ...
           and pstl_zip_rt_id <> nvl(p_pstl_zip_rt_id,hr_api.g_number)
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_pzr_shd.g_old_rec.pstl_zip_rng_id,hr_api.g_number)
    ...
    --
     ...
     fnd_message.set_token('VAR1','Postal Zip criteria');
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and pstl_zip_rt_id <> nvl(p_pstl_zip_rt_id,-1)
                   and p_effective_date between effective_start_date
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_pstl_zip_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_pzr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
function return_legislation_code
  (p_pstl_zip_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pstl_zip_rt_f b
    where b.pstl_zip_rt_id      = p_pstl_zip_rt_id
