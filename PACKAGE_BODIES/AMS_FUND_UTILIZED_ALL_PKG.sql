--------------------------------------------------------
--  DDL for Package Body AMS_FUND_UTILIZED_ALL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMS_FUND_UTILIZED_ALL_PKG" as
/* $Header: amslfutb.pls 115.2 2002/07/15 22:12:04 mpande noship $ */
...
begin
  ...
  if c_utilization_exists%notfound then
    ...
    l_version := 1;
    AMS_FUND_UTILIZED_ALL_PKG.INSERT_ROW(
        X_ROWID                     => l_row_id,
    ...
    close c_version;
    AMS_FUND_UTILIZED_ALL_PKG.UPDATE_ROW(
        P_UTILIZATION_ID            => l_utilization_id,
        P_LAST_UPDATE_DATE          => SYSDATE,
        P_LAST_UPDATED_BY           => l_user_id,
        P_LAST_UPDATE_LOGIN         => 0,
