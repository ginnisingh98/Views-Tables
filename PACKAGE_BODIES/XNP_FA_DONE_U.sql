--------------------------------------------------------
--  DDL for Package Body XNP_FA_DONE_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_FA_DONE_U" AS
/*$Header: XNPFAEDB.pls 120.2 2006/02/13 07:48:52 dputhiye ship $*/
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'FA_DONE' ) ;
    IF ( XNP$SDP_RESULT_CODE IS NULL) THEN
      x_error_message :='Missing Mandatory Attribute - SDP_RESULT_CODE' ;
      RAISE e_MISSING_MANDATORY_DATA ;
    END IF ;
    XNP_XML_UTILS.write_element ( 'SDP_RESULT_CODE', XNP$SDP_RESULT_CODE ) ;
  XNP_XML_UTILS.end_segment ( 'FA_DONE' ) ;
/* get the message body */
  XNP_XML_UTILS.get_document ( l_xml_body ) ;
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('FA_DONE',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'FA_DONE' ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'FA_DONE.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'FA_DONE', l_consumer_list );
  END IF;
