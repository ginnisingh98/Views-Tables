--------------------------------------------------------
--  DDL for Package Body WSH_CARRIER_CONTACT_INFO_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WSH_CARRIER_CONTACT_INFO_PKG" as
/* $Header: WSHCITHB.pls 120.2 2006/06/09 09:13:32 jnpinto noship $ */

/*============================================================================
Procedure: CREATE_CONTACTINFO
Purpose  :  Creates Contact points for the person working as employee in Carrier.
=============================================================================*/

--
G_PKG_NAME CONSTANT VARCHAR2(50) := 'WSH_CARRIER_CONTACT_INFO_PKG';
--
...
BEGIN
  ...
  IF l_debug_on THEN
      ...
      WSH_DEBUG_SV.log(l_module_name,'P_PRIMARY',P_PRIMARY);
      WSH_DEBUG_SV.log(l_module_name,'P_CONTACT_TYPE',P_CONTACT_TYPE);
      -- Corrected Bug 5298638 - parameter value of contact type to be shown in log file.
      WSH_DEBUG_SV.log(l_module_name,'P_STATUS',P_STATUS);
      -- Added Bug 5298638 - parameter value of status to be shown in log file.
  END IF;
  --
  ...
  --  Put information into email_rec_type, phone_rec_type and contact_points.
  ...
  l_phone_rec_type.phone_line_type    := P_CONTACT_TYPE;
...
PROCEDURE UPDATE_CONTACTINFO
  (
        ...
        P_PRIMARY                   IN  VARCHAR2,
        P_STATUS                    IN  VARCHAR2,
	P_CONTACT_TYPE              IN  VARCHAR2,--Added parameter for contact type - bug 5298638
        X_RETURN_STATUS             OUT NOCOPY  VARCHAR2,
        X_EXCEPTION_MSG             OUT NOCOPY  VARCHAR2,
        X_POSITION                  OUT NOCOPY  NUMBER,
        X_PROCEDURE                 OUT NOCOPY  VARCHAR2,
...
--Initialize the status to SUCCESS.
  ...
  IF l_debug_on THEN
      ...
      WSH_DEBUG_SV.log(l_module_name,'P_PRIMARY',P_PRIMARY);
      WSH_DEBUG_SV.log(l_module_name,'P_CONTACT_TYPE',P_CONTACT_TYPE);
      --Corrected Bug 5298638 - Parameter value of the contact type to be shown in log file.
      WSH_DEBUG_SV.log(l_module_name,'P_STATUS',P_STATUS);
      --Added Bug 5298638 - Parameter value of the status to be shown in log file.
  END IF;
  --
...
--Put Information into email_rec, phone_rec.
  ...
  l_phone_rec_type.phone_line_type             := P_CONTACT_TYPE;
  --Corrected Bug 5298638 - Instead of defaulting 'GEN' contact_type entered will be passed.
