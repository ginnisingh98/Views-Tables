--------------------------------------------------------
--  DDL for Package Body ISC_DBI_REV_PL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_REV_PL_PKG" AS
/* $Header: ISCRGBDB.pls 120.0 2005/05/25 17:19:11 appldev noship $ */
...
BEGIN
  ...
  ELSE -- when the LOV parameter is a SRep (no need to go through the SG hierarchy MV
      l_drill_bklg := '''pFunctionName=ISC_DBI_BKLG_DETAIL&VIEW_BY_NAME=VIEW_BY_ID&pParamIds=Y''';
      l_sg_where := '
