--------------------------------------------------------
--  DDL for Package Body BEN_PAP_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PAP_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pap_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_prtt_anthr_pl_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   prtt_anthr_pl_rt_id PK of record being inserted or updated.
...
--
Procedure chk_prtt_anthr_pl_rt_id(p_prtt_anthr_pl_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_prtt_anthr_pl_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_pap_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_prtt_anthr_pl_rt_id                => p_prtt_anthr_pl_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_prtt_anthr_pl_rt_id,hr_api.g_number)
     <>  ben_pap_shd.g_old_rec.prtt_anthr_pl_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_pap_shd.constraint_error('BEN_PRTT_ANTHR_PL_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_prtt_anthr_pl_rt_id is not null then
      --
...
--
-- In Parameters
--   p_prtt_anthr_pl_rt_id   PK of record being inserted or updated.
--   p_pl_id           	     Value of FK
...
and prtt_anthr_pl_rt_id <> nvl(p_prtt_anthr_pl_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_pl_id <> nvl(ben_pap_shd.g_old_rec.pl_id,hr_api.g_number)
	...
	if c_dup%found then
		close c_dup;
		--
		-- raise error as this Eligible for Another Plan criteria already exists for this profile
		--
		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
		fnd_message.set_token('VAR1','Eligible for Another Plan criteria',TRUE);
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_prtt_anthr_pl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_pap_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and prtt_anthr_pl_rt_id   <> nvl(p_prtt_anthr_pl_rt_id  ,-1)
...
function return_legislation_code
  (p_prtt_anthr_pl_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_prtt_anthr_pl_rt_f b
    where b.prtt_anthr_pl_rt_id      = p_prtt_anthr_pl_rt_id
