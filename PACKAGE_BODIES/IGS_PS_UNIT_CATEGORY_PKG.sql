--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_CATEGORY_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_CATEGORY_PKG" as
/* $Header: IGSPI75B.pls 115.6 2002/11/29 02:37:17 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_PS_UNIT_CATEGORY_ALL%RowType;
  new_references IGS_PS_UNIT_CATEGORY_ALL%RowType;
  ...
  PROCEDURE BeforeRowInsertUpdateDelete1(
	...
	v_unit_cd		IGS_PS_UNIT_CATEGORY_ALL.unit_cd%TYPE;
	v_version_number	IGS_PS_UNIT_CATEGORY_ALL.version_number%TYPE;
  ...
  BEGIN
	...
	IF  IGS_PS_VAL_UNIT.crsp_val_iud_uv_dtl (
			...
			v_message_name) = FALSE THEN
				...
				App_Exception.Raise_Exception;
	END IF;
	-- Validate IGS_PS_UNIT cat.  IGS_PS_UNIT cat is not updateable.
	IF p_inserting THEN
...
BEGIN
	...
	IF Column_Name IS NULL Then
		NULL;
	ELSIF Upper(Column_Name)='UNIT_CAT' Then
		New_References.Unit_Cat := Column_Value;
	ELSIF Upper(Column_Name)='UNIT_CD' Then
		New_References.Unit_Cd := Column_Value;
	END IF;

	IF Upper(Column_Name)='UNIT_CAT' OR Column_Name IS NULL Then
		IF New_References.Unit_Cat <> UPPER(New_References.Unit_Cat) Then
...
END Check_Constraints;
  ...
  BEGIN
    ...
    IF (((old_references.unit_cat = new_references.unit_cat)) OR
      ...
      NULL;
    ELSE
      IF NOT IGS_PS_UNIT_CAT_PKG.Get_PK_For_Validation (
        new_references.unit_cat) THEN
				  Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
				  IGS_GE_MSG_STACK.ADD;
			        App_Exception.Raise_Exception;
	END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UC_UCA_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UC_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  x_ORG_ID in NUMBER
    ...
    cursor C is select ROWID from IGS_PS_UNIT_CATEGORY_ALL
      where UNIT_CD = X_UNIT_CD
      and VERSION_NUMBER = X_VERSION_NUMBER
      and UNIT_CAT = X_UNIT_CAT;
...
begin
  ...
  insert into IGS_PS_UNIT_CATEGORY_ALL (
    UNIT_CD,
    VERSION_NUMBER,
    UNIT_CAT,
    CREATION_DATE,
...
begin
  ...
  delete from IGS_PS_UNIT_CATEGORY_ALL
  where ROWID = X_ROWID;
