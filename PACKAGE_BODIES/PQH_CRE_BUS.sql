--------------------------------------------------------
--  DDL for Package Body PQH_CRE_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_CRE_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_cre_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_criteria_rate_element_id    number         default null;
--
--  ---------------------------------------------------------------------------
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'CRITERIA_RATE_ELEMENT_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(pqh_cre_bus.g_criteria_rate_element_id, hr_api.g_number)
       = p_criteria_rate_element_id) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := pqh_cre_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pqh_cre_bus.g_criteria_rate_element_id    := p_criteria_rate_element_id;
    pqh_cre_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pqh_cre_shd.api_updating
  ...
  if nvl(p_rec.business_group_id, hr_api.g_number) <>
	     nvl(pqh_cre_shd.g_old_rec.business_group_id
	        ,hr_api.g_number
	        ) then
	    hr_api.argument_changed_error
	      (p_api_name   => l_proc
	      ,p_argument   => 'BUSINESS_GROUP_ID'
	      ,p_base_table => pqh_cre_shd.g_tab_nam
	      );
  end if;
  --
...
--
Procedure chk_criteria_rate_defn_id (p_criteria_rate_element_id          in number,
    ...
    open c1;
     ...
     fetch c1 into l_dummy;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        pqh_cre_shd.constraint_error('PQH_CRITERIA_RATE_ELEMENTS_FK1');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_business_group_id (p_criteria_rate_element_id          in number,
                            p_business_group_id          in number,
                            p_object_version_number in number) is
  --
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(pqh_cre_shd.g_old_rec.business_group_id,hr_api.g_number)
     or not l_api_updating) then
    --
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        pqh_cre_shd.constraint_error('PQH_CRITERIA_RATE_ELEMENTS_FK3');
        --
      end if;
      --
    close c1;
...
-- In Parameters
--   p_criteria_rate_element_id PK
--   p_element_type_id ID of FK column
--   p_object_version_number object version number
--
-- Post Success
--   Processing continues
...
--   Internal table handler use only.
--
Procedure chk_element_type_id (p_criteria_rate_element_id          in number,
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(pqh_cre_shd.g_old_rec.element_type_id,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if element_type_id value exists in
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        pqh_cre_shd.constraint_error('PQH_CRITERIA_RATE_ELEMENTS_FK4');
        --
      end if;
      --
    close c1;
...
-- In Parameters
--   p_criteria_rate_element_id PK
--   p_input_value_id ID of FK column
--   p_object_version_number object version number
--
-- Post Success
--   Processing continues
...
--   Internal table handler use only.
--
Procedure chk_input_value_id (p_criteria_rate_element_id          in number,
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(pqh_cre_shd.g_old_rec.input_value_id,hr_api.g_number)
     or not l_api_updating) then
    --
    -- check if input_value_id value exists in
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        close c1;
        --
        -- raise error as FK does not relate to PK in PAY_INPUT_VALUES_F
        -- table.
        --
        pqh_cre_shd.constraint_error('PQH_CRITERIA_RATE_ELEMENTS_FK5');
        --
      end if;
      --
    close c1;
...
Begin
  ...
  IF hr_startup_data_api_support.g_startup_mode
     ...
     hr_api.validate_bus_grp_id
       ...
       ,p_associated_column1 => pqh_cre_shd.g_tab_nam
                                || '.BUSINESS_GROUP_ID');
     --
...
Begin
  ...
  --
    --
  chk_startup_action(false
                    ,pqh_cre_shd.g_old_rec.business_group_id
                    ,pqh_cre_shd.g_old_rec.legislation_code
                    );
  IF hr_startup_data_api_support.g_startup_mode
                     NOT IN ('GENERIC','STARTUP') THEN
     --
