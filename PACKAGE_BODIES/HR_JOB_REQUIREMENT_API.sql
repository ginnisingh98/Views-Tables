--------------------------------------------------------
--  DDL for Package Body HR_JOB_REQUIREMENT_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_JOB_REQUIREMENT_API" as
/* $Header: pejbrapi.pkb 115.8 2002/12/11 11:21:19 pkakar ship $ */
--
-- Package Variables
--
g_package  varchar2(33) := '  hr_job_requirement_api.';
--
-- ----------------------------------------------------------------------------
...
procedure create_job_requirement
  ...
  --
  l_proc varchar2(72)       := g_package||'create_job_requirement';
  --
  -- bug 2298832 initialize l_analysis_criteria_id and segment variables with
  -- values where these are passed into program.
  --
  l_analysis_criteria_id     number := p_analysis_criteria_id;
  l_temp_analysis number := p_analysis_criteria_id;
  ...
  --
  -- bug 2298832 new variable to indicate whether key flex id parameter
  ...
  cursor csr_job_bg is
   ...
   --
   -- bug 2298832 get segment values where analysis criteria id is known
   --
...
--
  ...
  -- Issue a savepoint.
  --
  savepoint create_job_requirement;
  --
  -- Get business_group_id using position or job.
  --
  ...
  --
  -- 2298832 get segment values if p_analysis_criteria_id entered with a value
  --
  ...
  begin
    --
    -- Start of API User Hook for the before hook of create_job_requirement
    -- Specifically placed after the business_group_id is retrieved from
    -- the job.
    --
    hr_job_requirement_bk1.create_job_requirement_b
  ...
  exception
    when hr_api.cannot_find_prog_unit then
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'CREATE_JOB_REQUIREMENT'
        ,p_hook_type   => 'BP'
        );
    --
    -- End of API User Hook for the before hook of create_job_requirement
    --
  end;
  --
  --  bug 2298832: Only call this if p_analysis_criteria_id has no value
  --
  if l_null_ind = 0
  then
     --
  ...
  begin
    --
    -- Start of API User Hook for the after hook of create_job_requirement
    --
    hr_job_requirement_bk1.create_job_requirement_a
      (p_id_flex_num                   => p_id_flex_num
      ,p_job_id                        => p_job_id
      ,p_business_group_id             => l_business_group_id
      ,p_comments                      => p_comments
  ...
  exception
    ...
    --
    -- End of API User Hook for the after hook of create_job_requirement
    --
  end;
  --
...
exception
  when hr_api.validate_enabled then
    ...
    --
    ROLLBACK TO create_job_requirement;
    --
  ...
  when others then
    ...
    --
     p_job_requirement_id             := null;
    p_object_version_number          := null;
     p_analysis_criteria_id          := l_temp_analysis;
    raise;
    --
    -- End of fix.
    --
--
end create_job_requirement;
--
