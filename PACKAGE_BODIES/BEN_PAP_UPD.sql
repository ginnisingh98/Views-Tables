--------------------------------------------------------
--  DDL for Package Body BEN_PAP_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PAP_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pap_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_prtt_anthr_pl_rt_f Row
    --
    update  ben_prtt_anthr_pl_rt_f
    set
        prtt_anthr_pl_rt_id      = p_rec.prtt_anthr_pl_rt_id,
    excld_flag                      = p_rec.excld_flag,
    ordr_num                        = p_rec.ordr_num,
    vrbl_rt_prfl_id                   = p_rec.vrbl_rt_prfl_id,
    pl_id                           = p_rec.pl_id,
    business_group_id               = p_rec.business_group_id,
    pap_attribute_category          = p_rec.pap_attribute_category,
    pap_attribute1                  = p_rec.pap_attribute1,
    pap_attribute2                  = p_rec.pap_attribute2,
    pap_attribute3                  = p_rec.pap_attribute3,
    pap_attribute4                  = p_rec.pap_attribute4,
    pap_attribute5                  = p_rec.pap_attribute5,
    pap_attribute6                  = p_rec.pap_attribute6,
    pap_attribute7                  = p_rec.pap_attribute7,
    pap_attribute8                  = p_rec.pap_attribute8,
    pap_attribute9                  = p_rec.pap_attribute9,
    pap_attribute10                 = p_rec.pap_attribute10,
    pap_attribute11                 = p_rec.pap_attribute11,
    pap_attribute12                 = p_rec.pap_attribute12,
    pap_attribute13                 = p_rec.pap_attribute13,
    pap_attribute14                 = p_rec.pap_attribute14,
    pap_attribute15                 = p_rec.pap_attribute15,
    pap_attribute16                 = p_rec.pap_attribute16,
    pap_attribute17                 = p_rec.pap_attribute17,
    pap_attribute18                 = p_rec.pap_attribute18,
    pap_attribute19                 = p_rec.pap_attribute19,
    pap_attribute20                 = p_rec.pap_attribute20,
    pap_attribute21                 = p_rec.pap_attribute21,
    pap_attribute22                 = p_rec.pap_attribute22,
    pap_attribute23                 = p_rec.pap_attribute23,
    pap_attribute24                 = p_rec.pap_attribute24,
    pap_attribute25                 = p_rec.pap_attribute25,
    pap_attribute26                 = p_rec.pap_attribute26,
    pap_attribute27                 = p_rec.pap_attribute27,
    pap_attribute28                 = p_rec.pap_attribute28,
    pap_attribute29                 = p_rec.pap_attribute29,
    pap_attribute30                 = p_rec.pap_attribute30,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pap_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.excld_flag = hr_api.g_varchar2) then
    p_rec.excld_flag :=
    ben_pap_shd.g_old_rec.excld_flag;
  End If;
  If (p_rec.ordr_num = hr_api.g_number) then
    p_rec.ordr_num :=
    ben_pap_shd.g_old_rec.ordr_num;
  End If;
  If (p_rec.vrbl_rt_prfl_id = hr_api.g_number) then
    p_rec.vrbl_rt_prfl_id :=
    ben_pap_shd.g_old_rec.vrbl_rt_prfl_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_pap_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_pap_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pap_attribute_category = hr_api.g_varchar2) then
    p_rec.pap_attribute_category :=
    ben_pap_shd.g_old_rec.pap_attribute_category;
  End If;
  If (p_rec.pap_attribute1 = hr_api.g_varchar2) then
    p_rec.pap_attribute1 :=
    ben_pap_shd.g_old_rec.pap_attribute1;
  End If;
  If (p_rec.pap_attribute2 = hr_api.g_varchar2) then
    p_rec.pap_attribute2 :=
    ben_pap_shd.g_old_rec.pap_attribute2;
  End If;
  If (p_rec.pap_attribute3 = hr_api.g_varchar2) then
    p_rec.pap_attribute3 :=
    ben_pap_shd.g_old_rec.pap_attribute3;
  End If;
  If (p_rec.pap_attribute4 = hr_api.g_varchar2) then
    p_rec.pap_attribute4 :=
    ben_pap_shd.g_old_rec.pap_attribute4;
  End If;
  If (p_rec.pap_attribute5 = hr_api.g_varchar2) then
    p_rec.pap_attribute5 :=
    ben_pap_shd.g_old_rec.pap_attribute5;
  End If;
  If (p_rec.pap_attribute6 = hr_api.g_varchar2) then
    p_rec.pap_attribute6 :=
    ben_pap_shd.g_old_rec.pap_attribute6;
  End If;
  If (p_rec.pap_attribute7 = hr_api.g_varchar2) then
    p_rec.pap_attribute7 :=
    ben_pap_shd.g_old_rec.pap_attribute7;
  End If;
  If (p_rec.pap_attribute8 = hr_api.g_varchar2) then
    p_rec.pap_attribute8 :=
    ben_pap_shd.g_old_rec.pap_attribute8;
  End If;
  If (p_rec.pap_attribute9 = hr_api.g_varchar2) then
    p_rec.pap_attribute9 :=
    ben_pap_shd.g_old_rec.pap_attribute9;
  End If;
  If (p_rec.pap_attribute10 = hr_api.g_varchar2) then
    p_rec.pap_attribute10 :=
    ben_pap_shd.g_old_rec.pap_attribute10;
  End If;
  If (p_rec.pap_attribute11 = hr_api.g_varchar2) then
    p_rec.pap_attribute11 :=
    ben_pap_shd.g_old_rec.pap_attribute11;
  End If;
  If (p_rec.pap_attribute12 = hr_api.g_varchar2) then
    p_rec.pap_attribute12 :=
    ben_pap_shd.g_old_rec.pap_attribute12;
  End If;
  If (p_rec.pap_attribute13 = hr_api.g_varchar2) then
    p_rec.pap_attribute13 :=
    ben_pap_shd.g_old_rec.pap_attribute13;
  End If;
  If (p_rec.pap_attribute14 = hr_api.g_varchar2) then
    p_rec.pap_attribute14 :=
    ben_pap_shd.g_old_rec.pap_attribute14;
  End If;
  If (p_rec.pap_attribute15 = hr_api.g_varchar2) then
    p_rec.pap_attribute15 :=
    ben_pap_shd.g_old_rec.pap_attribute15;
  End If;
  If (p_rec.pap_attribute16 = hr_api.g_varchar2) then
    p_rec.pap_attribute16 :=
    ben_pap_shd.g_old_rec.pap_attribute16;
  End If;
  If (p_rec.pap_attribute17 = hr_api.g_varchar2) then
    p_rec.pap_attribute17 :=
    ben_pap_shd.g_old_rec.pap_attribute17;
  End If;
  If (p_rec.pap_attribute18 = hr_api.g_varchar2) then
    p_rec.pap_attribute18 :=
    ben_pap_shd.g_old_rec.pap_attribute18;
  End If;
  If (p_rec.pap_attribute19 = hr_api.g_varchar2) then
    p_rec.pap_attribute19 :=
    ben_pap_shd.g_old_rec.pap_attribute19;
  End If;
  If (p_rec.pap_attribute20 = hr_api.g_varchar2) then
    p_rec.pap_attribute20 :=
    ben_pap_shd.g_old_rec.pap_attribute20;
  End If;
  If (p_rec.pap_attribute21 = hr_api.g_varchar2) then
    p_rec.pap_attribute21 :=
    ben_pap_shd.g_old_rec.pap_attribute21;
  End If;
  If (p_rec.pap_attribute22 = hr_api.g_varchar2) then
    p_rec.pap_attribute22 :=
    ben_pap_shd.g_old_rec.pap_attribute22;
  End If;
  If (p_rec.pap_attribute23 = hr_api.g_varchar2) then
    p_rec.pap_attribute23 :=
    ben_pap_shd.g_old_rec.pap_attribute23;
  End If;
  If (p_rec.pap_attribute24 = hr_api.g_varchar2) then
    p_rec.pap_attribute24 :=
    ben_pap_shd.g_old_rec.pap_attribute24;
  End If;
  If (p_rec.pap_attribute25 = hr_api.g_varchar2) then
    p_rec.pap_attribute25 :=
    ben_pap_shd.g_old_rec.pap_attribute25;
  End If;
  If (p_rec.pap_attribute26 = hr_api.g_varchar2) then
    p_rec.pap_attribute26 :=
    ben_pap_shd.g_old_rec.pap_attribute26;
  End If;
  If (p_rec.pap_attribute27 = hr_api.g_varchar2) then
    p_rec.pap_attribute27 :=
    ben_pap_shd.g_old_rec.pap_attribute27;
  End If;
  If (p_rec.pap_attribute28 = hr_api.g_varchar2) then
    p_rec.pap_attribute28 :=
    ben_pap_shd.g_old_rec.pap_attribute28;
  End If;
  If (p_rec.pap_attribute29 = hr_api.g_varchar2) then
    p_rec.pap_attribute29 :=
    ben_pap_shd.g_old_rec.pap_attribute29;
  End If;
  If (p_rec.pap_attribute30 = hr_api.g_varchar2) then
    p_rec.pap_attribute30 :=
    ben_pap_shd.g_old_rec.pap_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_pap_shd.convert_args
  (
  p_prtt_anthr_pl_rt_id,
  null,
  null,
  p_excld_flag,
  p_ordr_num,
