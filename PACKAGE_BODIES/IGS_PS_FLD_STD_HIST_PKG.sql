--------------------------------------------------------
--  DDL for Package Body IGS_PS_FLD_STD_HIST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_FLD_STD_HIST_PKG" AS
  /* $Header: IGSPI14B.pls 115.7 2003/06/05 13:10:07 sarakshi ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_FLD_STD_HIST_ALL%RowType;
  new_references IGS_PS_FLD_STD_HIST_ALL%RowType;
 ...
 BEGIN
    ...
    IF upper(column_name) = 'MAJOR_FIELD_IND' OR
    column_name is null Then
	   IF ( new_references.major_field_ind NOT IN ( 'Y' , 'N' ) ) Then
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CFOSH_CV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID in NUMBER
    ...
    cursor C is select ROWID from IGS_PS_FLD_STD_HIST_ALL
      where COURSE_CD = X_COURSE_CD
      and FIELD_OF_STUDY = X_FIELD_OF_STUDY
      and HIST_START_DT = X_HIST_START_DT
      and VERSION_NUMBER = X_VERSION_NUMBER;
...
Before_DML (
  ...
  insert into IGS_PS_FLD_STD_HIST_ALL (
    COURSE_CD,
    VERSION_NUMBER,
    FIELD_OF_STUDY,
    HIST_START_DT,
...
begin
  ...
  if ( (tlinfo.HIST_END_DT = X_HIST_END_DT)
      ...
      AND ((tlinfo.MAJOR_FIELD_IND = X_MAJOR_FIELD_IND)
           OR ((tlinfo.MAJOR_FIELD_IND is null)
               AND (X_MAJOR_FIELD_IND is null)))
  ) then
    null;
  else
...
Before_DML (
  ...
  update IGS_PS_FLD_STD_HIST_ALL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_FLD_STD_HIST_ALL
     where COURSE_CD = X_COURSE_CD
     and FIELD_OF_STUDY = X_FIELD_OF_STUDY
     and HIST_START_DT = X_HIST_START_DT
     and VERSION_NUMBER = X_VERSION_NUMBER
...
Before_DML (
  ...
  delete from IGS_PS_FLD_STD_HIST_ALL
  where ROWID = X_ROWID;
