--------------------------------------------------------
--  DDL for Package Body HXC_HRT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_HRT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_hrt_bus.';  -- Global package name
...
g_retrieval_process_id        number         default null;
--
--  ---------------------------------------------------------------------------
...
Procedure set_security_group_id
  ...
  -- hxc_retrieval_processes and PER_BUSINESS_GROUPS
...
Begin
  ...
  --
  if ( nvl(hxc_hrt_bus.g_retrieval_process_id, hr_api.g_number)
       = p_retrieval_process_id) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := hxc_hrt_bus.g_legislation_code;
    if g_debug then
    ...
    --
    close csr_leg_code;
    hxc_hrt_bus.g_retrieval_process_id:= p_retrieval_process_id;
    hxc_hrt_bus.g_legislation_code  := l_legislation_code;
  end if;
  if g_debug then
...
Begin
  ...
  --
  IF NOT hxc_hrt_shd.api_updating
      (p_retrieval_process_id                 => p_rec.retrieval_process_id
...
--
-- Description:
-- This procedure ensures that a valid and a unique Retrieval Process Name
-- has been entered
--
-- Pre Conditions:
--   None
...
Procedure chk_name
  (
   p_name in hxc_retrieval_processes.name%TYPE,
   p_object_version_number in hxc_retrieval_processes.object_version_number%TYPE
  ) IS
--
  l_proc  varchar2(72);
--
-- cursor to check that a duplicate retrieval process name is not entered
--
...
THEN
--
      hr_utility.set_message(809, 'HXC_0086_HRT_RPROC_NAME_MAND');
...
--
-- Raise an error if the retrieval process name is not unique
--
  OPEN  csr_chk_name;
...
THEN
--
      hr_utility.set_message(809, 'HXC_0087_HRT_DUP_RPROCESS_NAME');
      hr_utility.raise_error;
--
END IF;
--
