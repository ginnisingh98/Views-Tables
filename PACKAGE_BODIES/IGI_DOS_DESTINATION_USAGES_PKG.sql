--------------------------------------------------------
--  DDL for Package Body IGI_DOS_DESTINATION_USAGES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_DOS_DESTINATION_USAGES_PKG" AS
/* $Header: igidosnb.pls 120.5.12000000.2 2007/06/14 04:34:16 pshivara ship $ */
...
l_unexp_level   number := FND_LOG.LEVEL_UNEXPECTED ;
  ...
  l_rowid VARCHAR2(25);
  old_references igi_dos_destination_usages%ROWTYPE;
  new_references igi_dos_destination_usages%ROWTYPE;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.set_column_values.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
  ...
  */
  BEGIN
    ...
    IF (((old_references.destination_id = new_references.destination_id)) OR
        ((new_references.destination_id IS NULL))) THEN
      NULL;
    ELSIF NOT igi_dos_destinations_pkg.get_pk_for_validation (
                new_references.destination_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.check_parent_existance.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.get_fk_igi_dos_destinations.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
      RETURN;
  ...
  END get_fk_igi_dos_destinations;
...
-- bug 3199481, start block
        IF (l_unexp_level >= l_debug_level) THEN
           FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.before_dml.Msg1',FALSE);
        END IF;
-- bug 3199481, end block
        app_exception.raise_exception;
      END IF;  */
...
-- bug 3199481, start block
        IF (l_unexp_level >= l_debug_level) THEN
           FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.before_dml.Msg2',FALSE);
        END IF;
-- bug 3199481, end block
        app_exception.raise_exception;
      END IF; */
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.lock_row.Msg1',FALSE);
      END IF;
-- bug 3199481, end block
      CLOSE c1;
      app_exception.raise_exception;
    ...
    IF (
        ((tlinfo.destination_id = x_destination_id) OR ((tlinfo.destination_id IS NULL) AND (X_destination_id IS NULL)))
...
-- bug 3199481, start block
      IF (l_unexp_level >= l_debug_level) THEN
         FND_LOG.MESSAGE(l_unexp_level, 'igi.plsql.igi_dos_destination_usages_pkg.lock_row.Msg2',FALSE);
      END IF;
-- bug 3199481, end block
      app_exception.raise_exception;
    END IF;
