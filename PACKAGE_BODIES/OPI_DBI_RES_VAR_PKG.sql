--------------------------------------------------------
--  DDL for Package Body OPI_DBI_RES_VAR_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OPI_DBI_RES_VAR_PKG" AS
/*$Header: OPIDRRSVRB.pls 120.0 2005/05/24 17:20:13 appldev noship $ */
...
BEGIN
    ...
    -- Outer select clause
    --
    -- For bug 3055129 changed the calculation for
    -- OPI_MEASURE6 - Change in Variance Amount
    --
    l_sel_clause :=
    'SELECT
      ...
      IF ((p_view_by_dim = 'RESOURCE+ENI_RESOURCE') AND
         ...
         (p_period_type = 'FII_TIME_WEEK' OR p_period_type = 'FII_TIME_ENT_PERIOD')) THEN
              l_sel_clause := l_sel_clause || ' ''pFunctionName=OPI_DBI_RES_VAR_JOB_DTL_REP&VIEW_BY_NAME=VIEW_BY_ID&VIEW_BY=RESOURCE+ENI_RESOURCE&pParamIds=Y'' OPI_ATTRIBUTE5 ,';
          ELSE
              l_sel_clause := l_sel_clause || 'NULL OPI_ATTRIBUTE5 ,';

      END IF;

      l_sel_clause := l_sel_clause ||
           opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_qty') || ' OPI_MEASURE1,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val') || ' OPI_MEASURE2,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_qty') || ' OPI_MEASURE3,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_val') || ' OPI_MEASURE4,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val') || ' -
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_val') || ' OPI_MEASURE5,
      ' || opi_dbi_rpt_util_pkg.change_str (
            opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val') || ' - ' ||
            opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_val'),
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_actual_val') || ' - ' ||
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_std_usage_val'),
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_actual_val') || ' - ' ||
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_std_usage_val'),
            'OPI_MEASURE6') || ',
      ' || opi_dbi_rpt_util_pkg.change_str ('oset.c_actual_val',
                                            'oset.c_std_usage_val',
                                            'oset.c_std_usage_val',
                                            'OPI_MEASURE7') || ',
      ' || opi_dbi_rpt_util_pkg.change_str ('oset.c_actual_val',
                            'oset.c_std_usage_val',
                            'oset.c_std_usage_val',
                            '') || ' - '
        || opi_dbi_rpt_util_pkg.change_str ('oset.p_actual_val',
                                'oset.p_std_usage_val',
                                'oset.p_std_usage_val',
                                '')
                                            || ' OPI_MEASURE8,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val_total')
                                    || ' OPI_MEASURE9,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_val_total')
                                    || ' OPI_MEASURE10,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val_total') || ' - '
        || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_val_total')
                                    || ' OPI_MEASURE11,
      ' || opi_dbi_rpt_util_pkg.change_str (
            opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val_total') ||' - ' ||
            opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_val_total'),
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_actual_val_total') ||' - ' ||
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_std_usage_val_total'),
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_actual_val_total') ||' - ' ||
            opi_dbi_rpt_util_pkg.nvl_str ('oset.p_std_usage_val_total'),
            'OPI_MEASURE12') || ',
      ' || opi_dbi_rpt_util_pkg.change_str ('oset.c_actual_val_total',
                            'oset.c_std_usage_val_total',
                            'oset.c_std_usage_val_total',
                            'OPI_MEASURE13') || ',
      ' || opi_dbi_rpt_util_pkg.change_str ('oset.c_actual_val_total',
                                'oset.c_std_usage_val_total',
                            'oset.c_std_usage_val_total',
                            '') ||
      ' - ' || opi_dbi_rpt_util_pkg.change_str ('oset.p_actual_val_total',
                                'oset.p_std_usage_val_total',
                                'oset.p_std_usage_val_total',
                                '')          || ' OPI_MEASURE14,
      ' || opi_dbi_rpt_util_pkg.nvl_str('oset.p_actual_val') || ' - '
    || opi_dbi_rpt_util_pkg.nvl_str('oset.p_std_usage_val')    || ' OPI_MEASURE15,
      ' || opi_dbi_rpt_util_pkg.change_str('oset.p_actual_val',
                           'oset.p_std_usage_val',
                       'oset.p_std_usage_val',
                       'OPI_MEASURE16') || ' ,
      ...
      ' || opi_dbi_rpt_util_pkg.change_str ('oset.p_actual_val_total',
                            ...
                            'oset.p_std_usage_val_total',
                            'OPI_MEASURE22') || ',
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_qty_total') || ' OPI_MEASURE17,
      ' || opi_dbi_rpt_util_pkg.nvl_str ('oset.c_std_usage_qty_total') || ' OPI_MEASURE18 ';
...
*/
                  p_qty1 := opi_dbi_rpt_util_pkg.nvl_str ('oset.c_actual_val');
...
BEGIN
    ...
    'SELECT
        ...
        ' || opi_dbi_rpt_util_pkg.nvl_str ('iset.c_actual_val')
                                           || ' OPI_MEASURE2,
        ' || opi_dbi_rpt_util_pkg.nvl_str ('iset.c_std_usage_val')
                                           || ' OPI_MEASURE4,
        ' || opi_dbi_rpt_util_pkg.nvl_str ('iset.c_actual_val') || ' -
        ' || opi_dbi_rpt_util_pkg.nvl_str ('iset.c_std_usage_val')
                           || 'OPI_MEASURE5,
        ' ||  opi_dbi_rpt_util_pkg.change_str (
                opi_dbi_rpt_util_pkg.nvl_str ('iset.c_actual_val') || ' - ' ||
                opi_dbi_rpt_util_pkg.nvl_str ('iset.c_std_usage_val'),
                opi_dbi_rpt_util_pkg.nvl_str ('iset.p_actual_val') || ' - ' ||
                opi_dbi_rpt_util_pkg.nvl_str ('iset.p_std_usage_val'),
                opi_dbi_rpt_util_pkg.nvl_str ('iset.p_actual_val') || ' - ' ||
                opi_dbi_rpt_util_pkg.nvl_str ('iset.p_std_usage_val'),
                'OPI_MEASURE6') || ',
        ' || opi_dbi_rpt_util_pkg.change_str ('iset.c_actual_val',
                                              'iset.c_std_usage_val',
                                              'iset.c_std_usage_val',
                                              'OPI_MEASURE7') || ',
        ' || opi_dbi_rpt_util_pkg.change_str ('iset.c_actual_val',
                              'iset.c_std_usage_val',
                              'iset.c_std_usage_val',
                              '') || ' - '
      || opi_dbi_rpt_util_pkg.change_str ('iset.p_actual_val',
                              'iset.p_std_usage_val',
                                  'iset.p_std_usage_val',
                                  '')
                               || 'OPI_MEASURE8,
    ' || opi_dbi_rpt_util_pkg.nvl_str('iset.p_actual_val') || ' - '
      || opi_dbi_rpt_util_pkg.nvl_str('iset.p_std_usage_val')    || ' OPI_MEASURE15,
    ' || opi_dbi_rpt_util_pkg.change_str('iset.p_actual_val',
                             'iset.p_std_usage_val',
                                 'iset.p_std_usage_val',
                         'OPI_MEASURE16');
