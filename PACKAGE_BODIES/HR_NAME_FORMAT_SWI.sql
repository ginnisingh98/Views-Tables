--------------------------------------------------------
--  DDL for Package Body HR_NAME_FORMAT_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_NAME_FORMAT_SWI" As
/* $Header: hrnmfswi.pkb 120.0 2005/05/31 01:35 appldev noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'hr_name_format_swi.';
--
-- ----------------------------------------------------------------------------
-- |--------------------------< create_name_format >--------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_name_format_swi;
  --
  ...
  --
  hr_nmf_ins.set_base_key_value
    (p_name_format_id => p_name_format_id
    );
  --
  -- Call API
  --
  hr_name_format_api.create_name_format
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_name_format_swi;
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint update_name_format_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  hr_name_format_api.update_name_format
    (p_validate                     => l_validate
    ,p_effective_date               => p_effective_date
    ,p_name_format_id               => p_name_format_id
    ,p_format_mask                  => p_format_mask
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_name_format_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_name_format_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  hr_name_format_api.delete_name_format
    (p_validate                     => l_validate
    ,p_name_format_id               => p_name_format_id
    ,p_object_version_number        => p_object_version_number
    );
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_name_format_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_object_version_number        := l_object_version_number;
