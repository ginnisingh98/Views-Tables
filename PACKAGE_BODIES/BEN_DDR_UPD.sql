--------------------------------------------------------
--  DDL for Package Body BEN_DDR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_DDR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ddr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    --
    -- Update the ben_dsgn_rqmt_f Row
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_ddr_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.dsgn_rqmt_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_ddr_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.mn_dpnts_rqd_num = hr_api.g_number) then
    p_rec.mn_dpnts_rqd_num :=
    ben_ddr_shd.g_old_rec.mn_dpnts_rqd_num;
  End If;
  If (p_rec.mx_dpnts_alwd_num = hr_api.g_number) then
    p_rec.mx_dpnts_alwd_num :=
    ben_ddr_shd.g_old_rec.mx_dpnts_alwd_num;
  End If;
  If (p_rec.no_mn_num_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_mn_num_dfnd_flag :=
    ben_ddr_shd.g_old_rec.no_mn_num_dfnd_flag;
  End If;
  If (p_rec.no_mx_num_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_mx_num_dfnd_flag :=
    ben_ddr_shd.g_old_rec.no_mx_num_dfnd_flag;
  End If;
  If (p_rec.cvr_all_elig_flag = hr_api.g_varchar2) then
    p_rec.cvr_all_elig_flag :=
    ben_ddr_shd.g_old_rec.cvr_all_elig_flag;
  End If;
  If (p_rec.oipl_id = hr_api.g_number) then
    p_rec.oipl_id :=
    ben_ddr_shd.g_old_rec.oipl_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_ddr_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.opt_id = hr_api.g_number) then
    p_rec.opt_id :=
    ben_ddr_shd.g_old_rec.opt_id;
  End If;
  If (p_rec.grp_rlshp_cd = hr_api.g_varchar2) then
    p_rec.grp_rlshp_cd :=
    ben_ddr_shd.g_old_rec.grp_rlshp_cd;
  End If;
  If (p_rec.dsgn_typ_cd = hr_api.g_varchar2) then
    p_rec.dsgn_typ_cd :=
    ben_ddr_shd.g_old_rec.dsgn_typ_cd;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_ddr_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.ddr_attribute_category = hr_api.g_varchar2) then
    p_rec.ddr_attribute_category :=
    ben_ddr_shd.g_old_rec.ddr_attribute_category;
  End If;
  If (p_rec.ddr_attribute1 = hr_api.g_varchar2) then
    p_rec.ddr_attribute1 :=
    ben_ddr_shd.g_old_rec.ddr_attribute1;
  End If;
  If (p_rec.ddr_attribute2 = hr_api.g_varchar2) then
    p_rec.ddr_attribute2 :=
    ben_ddr_shd.g_old_rec.ddr_attribute2;
  End If;
  If (p_rec.ddr_attribute3 = hr_api.g_varchar2) then
    p_rec.ddr_attribute3 :=
    ben_ddr_shd.g_old_rec.ddr_attribute3;
  End If;
  If (p_rec.ddr_attribute4 = hr_api.g_varchar2) then
    p_rec.ddr_attribute4 :=
    ben_ddr_shd.g_old_rec.ddr_attribute4;
  End If;
  If (p_rec.ddr_attribute5 = hr_api.g_varchar2) then
    p_rec.ddr_attribute5 :=
    ben_ddr_shd.g_old_rec.ddr_attribute5;
  End If;
  If (p_rec.ddr_attribute6 = hr_api.g_varchar2) then
    p_rec.ddr_attribute6 :=
    ben_ddr_shd.g_old_rec.ddr_attribute6;
  End If;
  If (p_rec.ddr_attribute7 = hr_api.g_varchar2) then
    p_rec.ddr_attribute7 :=
    ben_ddr_shd.g_old_rec.ddr_attribute7;
  End If;
  If (p_rec.ddr_attribute8 = hr_api.g_varchar2) then
    p_rec.ddr_attribute8 :=
    ben_ddr_shd.g_old_rec.ddr_attribute8;
  End If;
  If (p_rec.ddr_attribute9 = hr_api.g_varchar2) then
    p_rec.ddr_attribute9 :=
    ben_ddr_shd.g_old_rec.ddr_attribute9;
  End If;
  If (p_rec.ddr_attribute10 = hr_api.g_varchar2) then
    p_rec.ddr_attribute10 :=
    ben_ddr_shd.g_old_rec.ddr_attribute10;
  End If;
  If (p_rec.ddr_attribute11 = hr_api.g_varchar2) then
    p_rec.ddr_attribute11 :=
    ben_ddr_shd.g_old_rec.ddr_attribute11;
  End If;
  If (p_rec.ddr_attribute12 = hr_api.g_varchar2) then
    p_rec.ddr_attribute12 :=
    ben_ddr_shd.g_old_rec.ddr_attribute12;
  End If;
  If (p_rec.ddr_attribute13 = hr_api.g_varchar2) then
    p_rec.ddr_attribute13 :=
    ben_ddr_shd.g_old_rec.ddr_attribute13;
  End If;
  If (p_rec.ddr_attribute14 = hr_api.g_varchar2) then
    p_rec.ddr_attribute14 :=
    ben_ddr_shd.g_old_rec.ddr_attribute14;
  End If;
  If (p_rec.ddr_attribute15 = hr_api.g_varchar2) then
    p_rec.ddr_attribute15 :=
    ben_ddr_shd.g_old_rec.ddr_attribute15;
  End If;
  If (p_rec.ddr_attribute16 = hr_api.g_varchar2) then
    p_rec.ddr_attribute16 :=
    ben_ddr_shd.g_old_rec.ddr_attribute16;
  End If;
  If (p_rec.ddr_attribute17 = hr_api.g_varchar2) then
    p_rec.ddr_attribute17 :=
    ben_ddr_shd.g_old_rec.ddr_attribute17;
  End If;
  If (p_rec.ddr_attribute18 = hr_api.g_varchar2) then
    p_rec.ddr_attribute18 :=
    ben_ddr_shd.g_old_rec.ddr_attribute18;
  End If;
  If (p_rec.ddr_attribute19 = hr_api.g_varchar2) then
    p_rec.ddr_attribute19 :=
    ben_ddr_shd.g_old_rec.ddr_attribute19;
  End If;
  If (p_rec.ddr_attribute20 = hr_api.g_varchar2) then
    p_rec.ddr_attribute20 :=
    ben_ddr_shd.g_old_rec.ddr_attribute20;
  End If;
  If (p_rec.ddr_attribute21 = hr_api.g_varchar2) then
    p_rec.ddr_attribute21 :=
    ben_ddr_shd.g_old_rec.ddr_attribute21;
  End If;
  If (p_rec.ddr_attribute22 = hr_api.g_varchar2) then
    p_rec.ddr_attribute22 :=
    ben_ddr_shd.g_old_rec.ddr_attribute22;
  End If;
  If (p_rec.ddr_attribute23 = hr_api.g_varchar2) then
    p_rec.ddr_attribute23 :=
    ben_ddr_shd.g_old_rec.ddr_attribute23;
  End If;
  If (p_rec.ddr_attribute24 = hr_api.g_varchar2) then
    p_rec.ddr_attribute24 :=
    ben_ddr_shd.g_old_rec.ddr_attribute24;
  End If;
  If (p_rec.ddr_attribute25 = hr_api.g_varchar2) then
    p_rec.ddr_attribute25 :=
    ben_ddr_shd.g_old_rec.ddr_attribute25;
  End If;
  If (p_rec.ddr_attribute26 = hr_api.g_varchar2) then
    p_rec.ddr_attribute26 :=
    ben_ddr_shd.g_old_rec.ddr_attribute26;
  End If;
  If (p_rec.ddr_attribute27 = hr_api.g_varchar2) then
    p_rec.ddr_attribute27 :=
    ben_ddr_shd.g_old_rec.ddr_attribute27;
  End If;
  If (p_rec.ddr_attribute28 = hr_api.g_varchar2) then
    p_rec.ddr_attribute28 :=
    ben_ddr_shd.g_old_rec.ddr_attribute28;
  End If;
  If (p_rec.ddr_attribute29 = hr_api.g_varchar2) then
    p_rec.ddr_attribute29 :=
    ben_ddr_shd.g_old_rec.ddr_attribute29;
  End If;
  If (p_rec.ddr_attribute30 = hr_api.g_varchar2) then
    p_rec.ddr_attribute30 :=
    ben_ddr_shd.g_old_rec.ddr_attribute30;
  End If;
