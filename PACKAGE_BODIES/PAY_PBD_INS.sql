--------------------------------------------------------
--  DDL for Package Body PAY_PBD_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PBD_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_pbd_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_bal_attribute_default_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_bal_attribute_default_id  in  number) is
--
...
Begin
  ...
  --
  pay_pbd_ins.g_bal_attribute_default_id_i := p_bal_attribute_default_id;
  --
...
Procedure insert_dml
  (p_rec in out nocopy pay_pbd_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Insert the row into: pay_bal_attribute_defaults
  --
  insert into pay_bal_attribute_defaults
...
--
  Cursor C_Sel1 is select pay_bal_attribute_defaults_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from pay_bal_attribute_defaults
     where bal_attribute_default_id =
             pay_pbd_ins.g_bal_attribute_default_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (pay_pbd_ins.g_bal_attribute_default_id_i is not null) Then
    --
    ...
    p_rec.bal_attribute_default_id :=
      pay_pbd_ins.g_bal_attribute_default_id_i;
    pay_pbd_ins.g_bal_attribute_default_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.bal_attribute_default_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  pay_pbd_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  pay_pbd_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  pay_pbd_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  p_bal_attribute_default_id := l_rec.bal_attribute_default_id;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End ins;
--
