--------------------------------------------------------
--  DDL for Package Body BOM_CSTRACTC_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BOM_CSTRACTC_XMLP_PKG" AS
/* $Header: CSTRACTCB.pls 120.0 2007/12/24 09:50:19 dwkrishn noship $ */
  ...
  BEGIN
    ...
    IF P_PERIOD_ID_FROM = P_PERIOD_ID_TO THEN
      P_PERIOD_WHERE := 'and wepb.acct_period_id = :P_PERIOD_ID_FROM';
    ELSE
      P_PERIOD_WHERE := 'and wepb.acct_period_id between :P_PERIOD_ID_FROM and :P_PERIOD_ID_TO';
    END IF;
    ...
    ELSE
      ...
      END IF;
    END IF;
    IF P_MAINT_UNIT_FROM IS NOT NULL THEN
      IF P_MAINT_UNIT_TO IS NOT NULL THEN
        IF P_MAINT_UNIT_FROM = P_MAINT_UNIT_TO THEN
          P_MAINT_UNIT_AI_WHERE := 'and cii.serial_number = :P_MAINT_UNIT_FROM';
          P_MAINT_UNIT_RI_WHERE := 'and wdj.rebuild_serial_number = :P_MAINT_UNIT_FROM';
        ELSE
          P_MAINT_UNIT_AI_WHERE := 'and cii.serial_number between :P_MAINT_UNIT_FROM and :P_MAINT_UNIT_TO';
          P_MAINT_UNIT_RI_WHERE := 'and wdj.rebuild_serial_number between :P_MAINT_UNIT_FROM and :P_MAINT_UNIT_TO';
        END IF;
      ELSE
        P_MAINT_UNIT_AI_WHERE := 'and cii.serial_number >= :P_MAINT_UNIT_FROM';
        P_MAINT_UNIT_RI_WHERE := 'and wdj.rebuild_serial_number >= :P_MAINT_UNIT_FROM';
    ...
    ELSE
      IF P_MAINT_UNIT_TO IS NOT NULL THEN
        P_MAINT_UNIT_AI_WHERE := 'and cii.serial_number <= :P_MAINT_UNIT_TO';
        P_MAINT_UNIT_RI_WHERE := 'and wdj.rebuild_serial_number <= :P_MAINT_UNIT_TO';
      ELSE
