--------------------------------------------------------
--  DDL for Package Body IGF_SP_STD_UNIT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_SP_STD_UNIT_PKG" AS
/* $Header: IGFPI07B.pls 115.2 2003/03/19 08:50:47 smadathi noship $ */

  l_rowid VARCHAR2(25);
  old_references igf_sp_std_unit_all%ROWTYPE;
  new_references igf_sp_std_unit_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.fee_cls_unit_id                   := x_fee_cls_unit_id;
  ...
  */
  BEGIN
    ...
    IF (((old_references.fee_cls_prg_id = new_references.fee_cls_prg_id)) OR
        ((new_references.fee_cls_prg_id IS NULL))) THEN
      NULL;
    ELSIF NOT igf_sp_std_prg_pkg.get_pk_for_validation (
                new_references.fee_cls_prg_id
              ) THEN
      fnd_message.set_name ('FND', 'FORM_RECORD_DELETED');
      igs_ge_msg_stack.add;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igf_sp_std_unit_all
      WHERE    fee_cls_unit_id = x_fee_cls_unit_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      ...
      AND      fee_cls_prg_id = x_fee_cls_prg_id
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SP_STDU_SDFCP_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
...
/*
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGF', 'IGF_SP_STDU_UV_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.fee_cls_unit_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igf_sp_std_unit_s.NEXTVAL
    INTO      x_fee_cls_unit_id
    FROM      dual;
    ...
    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_fee_cls_unit_id                   => x_fee_cls_unit_id,
    ...
    INSERT INTO igf_sp_std_unit_all (
      fee_cls_unit_id,
      fee_cls_prg_id,
      unit_cd,
      version_number,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.fee_cls_prg_id = x_fee_cls_prg_id)
        AND (tlinfo.unit_cd = x_unit_cd)
        AND (tlinfo.version_number = x_version_number)
        AND (tlinfo.max_amount = x_max_amount)
       ) THEN
      NULL;
    ELSE
  ...
  */
  ...
  BEGIN
    ...
    UPDATE igf_sp_std_unit_all
      SET
        fee_cls_prg_id                    = new_references.fee_cls_prg_id,
  ...
  */
  BEGIN
    ...
    DELETE FROM igf_sp_std_unit_all
    WHERE rowid = x_rowid;
