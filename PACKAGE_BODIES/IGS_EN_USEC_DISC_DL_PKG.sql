--------------------------------------------------------
--  DDL for Package Body IGS_EN_USEC_DISC_DL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_EN_USEC_DISC_DL_PKG" AS
/* $Header: IGSEI49B.pls 115.7 2003/03/21 08:10:24 sarakshi ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_en_usec_disc_dl%ROWTYPE;
  new_references igs_en_usec_disc_dl%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.usec_disc_dl_id                   := x_usec_disc_dl_id;
    ...
    new_references.usec_disc_dl_date                 := x_usec_disc_dl_date;
    new_references.usec_disc_total_days              := x_usec_disc_total_days;
    new_references.usec_disc_offset_days             := x_usec_disc_offset_days;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_en_usec_disc_dl
      WHERE    usec_disc_dl_id = x_usec_disc_dl_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_en_usec_disc_dl
      WHERE    ((administrative_unit_status = x_administrative_unit_status) OR (administrative_unit_status IS NULL AND x_administrative_unit_status IS NULL))
      AND      uoo_id = x_uoo_id
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_EN_UDD_NDDS_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_EN_UDD_UOO_UFK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  BEGIN
    ...
    set_column_values (
      ...
      x_uoo_id,
      x_usec_disc_dl_date,
      x_usec_disc_total_days,
      x_usec_disc_offset_days,
      x_creation_date,
      x_created_by,
      x_last_update_date,
      x_last_updated_by,
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.usec_disc_dl_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_en_usec_disc_dl_s.NEXTVAL
    INTO      x_usec_disc_dl_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_usec_disc_dl_id                   => x_usec_disc_dl_id,
      ...
      x_uoo_id                            => x_uoo_id,
      x_usec_disc_dl_date                 => x_usec_disc_dl_date,
      x_usec_disc_total_days              => x_usec_disc_total_days,
      x_usec_disc_offset_days             => x_usec_disc_offset_days,
    ...
    INSERT INTO igs_en_usec_disc_dl (
      ...
      new_references.uoo_id,
      new_references.usec_disc_dl_date,
      new_references.usec_disc_total_days,
      new_references.usec_disc_offset_days,
      x_last_update_date,
      x_last_updated_by,
      x_last_update_date,
      x_last_updated_by,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ((tlinfo.non_std_disc_dl_stp_id = x_non_std_disc_dl_stp_id) OR ((tlinfo.non_std_disc_dl_stp_id IS NULL) AND (X_non_std_disc_dl_stp_id IS NULL)))
        AND ((tlinfo.administrative_unit_status = x_administrative_unit_status) OR ((tlinfo.administrative_unit_status IS NULL) AND (X_administrative_unit_status IS NULL)))
        ...
        AND (tlinfo.usec_disc_dl_date = x_usec_disc_dl_date)
        AND ((tlinfo.usec_disc_total_days = x_usec_disc_total_days) OR ((tlinfo.usec_disc_total_days IS NULL) AND (X_usec_disc_total_days IS NULL)))
        AND ((tlinfo.usec_disc_offset_days = x_usec_disc_offset_days) OR ((tlinfo.usec_disc_offset_days IS NULL) AND (X_usec_disc_offset_days IS NULL)))
       ) THEN
      NULL;
    ELSE
  ...
  BEGIN
    ...
    UPDATE igs_en_usec_disc_dl
      SET
        ...
        usec_disc_dl_date                 = new_references.usec_disc_dl_date,
        usec_disc_total_days              = new_references.usec_disc_total_days,
        usec_disc_offset_days             = new_references.usec_disc_offset_days,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_en_usec_disc_dl
    WHERE rowid = x_rowid;
