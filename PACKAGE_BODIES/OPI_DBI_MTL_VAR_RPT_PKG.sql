--------------------------------------------------------
--  DDL for Package Body OPI_DBI_MTL_VAR_RPT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OPI_DBI_MTL_VAR_RPT_PKG" 
/*$Header: OPIDMUVRPTB.pls 120.0 2005/05/24 19:20:46 appldev noship $ */
as

  Function get_description(p_dim_name varchar2) return varchar2;
  ...
  PROCEDURE mtl_status_sql (p_param IN BIS_PMV_PAGE_PARAMETER_TBL,
  ...
  l_description := get_description(p_view_by_dim);
  ...
  'FII_TIME_ENT_PERIOD') AND (UPPER(p_org)<>'ALL')) THEN
      l_sel_clause := l_sel_clause || ' ''pFunctionName=OPI_DBI_MTL_USAGE_JOB_DTL_REP&VIEW_BY_NAME=VIEW_BY_ID&VIEW_BY=ITEM+ENI_ITEM_ORG&pParamIds=Y'' OPI_ATTRIBUTE3 ,';
   ELSE
      l_sel_clause := l_sel_clause || 'NULL OPI_ATTRIBUTE3 ,';
   END IF;
   ...
   'oset.C_STD_VALUE            OPI_MEASURE1,
    ...
    oset.P_ACT_VALUE - oset.P_STD_VALUE     OPI_MEASURE19,
    oset.C_ACT_VALUE - oset.C_STD_VALUE     OPI_MEASURE5,
    CASE WHEN oset.P_ACT_VALUE - oset.P_STD_VALUE = 0 THEN to_number(NULL)
             ELSE (((oset.C_ACT_VALUE - oset.C_STD_VALUE) - (oset.P_ACT_VALUE - oset.P_STD_VALUE))/ABS(oset.P_ACT_VALUE - oset.P_STD_VALUE))*100 END  OPI_MEASURE6,
        decode(oset.P_STD_VALUE, 0, to_number(null), ((oset.P_ACT_VALUE - oset.P_STD_VALUE)/oset.P_STD_VALUE)*100) OPI_MEASURE20,
        decode(oset.C_STD_VALUE, 0, to_number(null), ((oset.C_ACT_VALUE - oset.C_STD_VALUE)/oset.C_STD_VALUE)*100) OPI_MEASURE7,
    CASE WHEN oset.C_STD_VALUE = 0 THEN to_number(NULL)
         WHEN oset.P_STD_VALUE = 0 THEN to_number(NULL)
         ELSE ((oset.C_ACT_VALUE - oset.C_STD_VALUE)/oset.C_STD_VALUE)*100 - ((oset.P_ACT_VALUE - oset.P_STD_VALUE)/oset.P_STD_VALUE)*100 END OPI_MEASURE8,
    oset.C_STD_VALUE_TOTAL  OPI_MEASURE9,
    oset.C_ACT_VALUE_TOTAL  OPI_MEASURE10,
    oset.C_ACT_VALUE_TOTAL - oset.C_STD_VALUE_TOTAL OPI_MEASURE11,
    CASE WHEN oset.P_ACT_VALUE_TOTAL - oset.P_STD_VALUE_TOTAL = 0 THEN to_number(NULL)
             ELSE (((oset.C_ACT_VALUE_TOTAL - oset.C_STD_VALUE_TOTAL) - (oset.P_ACT_VALUE_TOTAL - oset.P_STD_VALUE_TOTAL))/ABS(oset.P_ACT_VALUE_TOTAL - oset.P_STD_VALUE_TOTAL))*100 END  OPI_MEASURE12,
    CASE WHEN oset.C_STD_VALUE_TOTAL = 0 THEN to_number(NULL)
         ELSE (( oset.C_ACT_VALUE_TOTAL - oset.C_STD_VALUE_TOTAL )/ oset.C_STD_VALUE_TOTAL)*100  END    OPI_MEASURE13,
    CASE WHEN oset.C_STD_VALUE_TOTAL = 0 THEN to_number(NULL)
         WHEN oset.P_STD_VALUE_TOTAL = 0 THEN to_number(NULL)
         ELSE (( oset.C_ACT_VALUE_TOTAL - oset.C_STD_VALUE_TOTAL)/ oset.C_STD_VALUE_TOTAL)*100 -
                (( oset.P_ACT_VALUE_TOTAL - oset.P_STD_VALUE_TOTAL )/ oset.P_STD_VALUE_TOTAL )*100  END OPI_MEASURE14,
        decode(oset.C_STD_VALUE,0, to_number(null), ((oset.C_ACT_VALUE - oset.C_STD_VALUE)/oset.C_STD_VALUE)*100) OPI_MEASURE15,
        decode(oset.P_STD_VALUE,0, to_number(null), ((oset.P_ACT_VALUE - oset.P_STD_VALUE)/oset.P_STD_VALUE)*100) OPI_MEASURE16,
    CASE WHEN oset.C_STD_VALUE_TOTAL = 0 THEN to_number(NULL)
         ELSE (( oset.C_ACT_VALUE_TOTAL - oset.C_STD_VALUE_TOTAL )/ oset.C_STD_VALUE_TOTAL )*100  END  OPI_MEASURE17,
    CASE WHEN oset.P_STD_VALUE_TOTAL = 0 THEN to_number(NULL)
         ELSE (( oset.P_ACT_VALUE_TOTAL - oset.P_STD_VALUE_TOTAL )/ oset.P_STD_VALUE_TOTAL )*100  END  OPI_MEASURE18 ';
  ...
  begin
   ...
   l_description :=  (CASE p_dim_name
                    WHEN 'ITEM+ENI_ITEM_ORG' THEN 'v.description'
                    ELSE 'null'
                      END);
  ...
  PROCEDURE mtl_trend_sql(p_param in BIS_PMV_PAGE_PARAMETER_TBL,
                      x_custom_sql  OUT NOCOPY VARCHAR2,
                      x_custom_output OUT NOCOPY BIS_QUERY_ATTRIBUTES_TBL)
  IS
    l_query VARCHAR2(15000);
