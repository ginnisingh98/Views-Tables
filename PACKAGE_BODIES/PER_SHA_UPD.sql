--------------------------------------------------------
--  DDL for Package Body PER_SHA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_SHA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_sha_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy per_sha_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the per_std_holiday_absences Row
  --
  update per_std_holiday_absences
  set
  std_holiday_absences_id           = p_rec.std_holiday_absences_id,
  date_not_taken                    = p_rec.date_not_taken,
  person_id                         = p_rec.person_id,
  standard_holiday_id               = p_rec.standard_holiday_id,
  actual_date_taken                 = p_rec.actual_date_taken,
  reason                            = p_rec.reason,
  expired                           = p_rec.expired,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in per_sha_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update(p_rec in per_sha_shd.g_rec_type) is
--
...
Begin
  ...
  begin
    per_sha_rku.after_update
    (p_std_holiday_absences_id  => p_rec.std_holiday_absences_id
    ,p_date_not_taken           => p_rec.date_not_taken
    ,p_person_id                => p_rec.person_id
    ,p_standard_holiday_id      => p_rec.standard_holiday_id
    ,p_actual_date_taken        => p_rec.actual_date_taken
    ,p_reason                   => p_rec.reason
    ,p_expired                  => p_rec.expired
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy per_sha_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.date_not_taken = hr_api.g_date) then
    p_rec.date_not_taken :=
    per_sha_shd.g_old_rec.date_not_taken;
  End If;
  If (p_rec.person_id = hr_api.g_number) then
    p_rec.person_id :=
    per_sha_shd.g_old_rec.person_id;
  End If;
  If (p_rec.standard_holiday_id = hr_api.g_number) then
    p_rec.standard_holiday_id :=
    per_sha_shd.g_old_rec.standard_holiday_id;
  End If;
  If (p_rec.actual_date_taken = hr_api.g_date) then
    p_rec.actual_date_taken :=
    per_sha_shd.g_old_rec.actual_date_taken;
  End If;
  If (p_rec.reason = hr_api.g_varchar2) then
    p_rec.reason :=
    per_sha_shd.g_old_rec.reason;
  End If;
  If (p_rec.expired = hr_api.g_varchar2) then
    p_rec.expired :=
    per_sha_shd.g_old_rec.expired;
  End If;
  If (p_rec.attribute_category = hr_api.g_varchar2) then
    p_rec.attribute_category :=
    per_sha_shd.g_old_rec.attribute_category;
  End If;
  If (p_rec.attribute1 = hr_api.g_varchar2) then
    p_rec.attribute1 :=
    per_sha_shd.g_old_rec.attribute1;
  End If;
  If (p_rec.attribute2 = hr_api.g_varchar2) then
    p_rec.attribute2 :=
    per_sha_shd.g_old_rec.attribute2;
  End If;
  If (p_rec.attribute3 = hr_api.g_varchar2) then
    p_rec.attribute3 :=
    per_sha_shd.g_old_rec.attribute3;
  End If;
  If (p_rec.attribute4 = hr_api.g_varchar2) then
    p_rec.attribute4 :=
    per_sha_shd.g_old_rec.attribute4;
  End If;
  If (p_rec.attribute5 = hr_api.g_varchar2) then
    p_rec.attribute5 :=
    per_sha_shd.g_old_rec.attribute5;
  End If;
  If (p_rec.attribute6 = hr_api.g_varchar2) then
    p_rec.attribute6 :=
    per_sha_shd.g_old_rec.attribute6;
  End If;
  If (p_rec.attribute7 = hr_api.g_varchar2) then
    p_rec.attribute7 :=
    per_sha_shd.g_old_rec.attribute7;
  End If;
  If (p_rec.attribute8 = hr_api.g_varchar2) then
    p_rec.attribute8 :=
    per_sha_shd.g_old_rec.attribute8;
  End If;
  If (p_rec.attribute9 = hr_api.g_varchar2) then
    p_rec.attribute9 :=
    per_sha_shd.g_old_rec.attribute9;
  End If;
  If (p_rec.attribute10 = hr_api.g_varchar2) then
    p_rec.attribute10 :=
    per_sha_shd.g_old_rec.attribute10;
  End If;
  If (p_rec.attribute11 = hr_api.g_varchar2) then
    p_rec.attribute11 :=
    per_sha_shd.g_old_rec.attribute11;
  End If;
  If (p_rec.attribute12 = hr_api.g_varchar2) then
    p_rec.attribute12 :=
    per_sha_shd.g_old_rec.attribute12;
  End If;
  If (p_rec.attribute13 = hr_api.g_varchar2) then
    p_rec.attribute13 :=
    per_sha_shd.g_old_rec.attribute13;
  End If;
  If (p_rec.attribute14 = hr_api.g_varchar2) then
    p_rec.attribute14 :=
    per_sha_shd.g_old_rec.attribute14;
  End If;
  If (p_rec.attribute15 = hr_api.g_varchar2) then
    p_rec.attribute15 :=
    per_sha_shd.g_old_rec.attribute15;
  End If;
  If (p_rec.attribute16 = hr_api.g_varchar2) then
    p_rec.attribute16 :=
    per_sha_shd.g_old_rec.attribute16;
  End If;
  If (p_rec.attribute17 = hr_api.g_varchar2) then
    p_rec.attribute17 :=
    per_sha_shd.g_old_rec.attribute17;
  End If;
  If (p_rec.attribute18 = hr_api.g_varchar2) then
    p_rec.attribute18 :=
    per_sha_shd.g_old_rec.attribute18;
  End If;
  If (p_rec.attribute19 = hr_api.g_varchar2) then
    p_rec.attribute19 :=
    per_sha_shd.g_old_rec.attribute19;
  End If;
  If (p_rec.attribute20 = hr_api.g_varchar2) then
    p_rec.attribute20 :=
    per_sha_shd.g_old_rec.attribute20;
  End If;
...
Procedure upd
  (
  p_rec        in out nocopy per_sha_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  per_sha_bus.update_validate(p_rec);
  --
