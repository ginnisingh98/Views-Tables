--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_ACCTS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_ACCTS_PKG" AS
/* $Header: IGSPI2ZB.pls 115.4 2002/11/29 02:21:00 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_unit_accts_all%ROWTYPE;
  new_references igs_ps_unit_accts_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.unit_account_id                   := x_unit_account_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_unit_accts_all
      WHERE    unit_account_id = x_unit_account_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_FI_UAA_UV_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.unit_account_id
           )
         ) THEN
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_unit_accts_s.NEXTVAL
    INTO      x_unit_account_id
    FROM      dual;
    ...
    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_unit_account_id                   => x_unit_account_id,
      x_unit_cd                           => x_unit_cd,
    ...
    INSERT INTO igs_ps_unit_accts_all (
      unit_account_id,
      unit_cd,
      version_number,
      segment,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_unit_accts_all
    WHERE rowid = x_rowid;
