--------------------------------------------------------
--  DDL for Package Body BEN_ERP_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ERP_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_erp_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_enrt_perd_for_pl_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   enrt_perd_for_pl_id PK of record being inserted or updated.
...
--
Procedure chk_enrt_perd_for_pl_id(p_enrt_perd_for_pl_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_enrt_perd_for_pl_id';
...
Begin
  ...
  --
  l_api_updating := ben_erp_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_enrt_perd_for_pl_id                => p_enrt_perd_for_pl_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_enrt_perd_for_pl_id,hr_api.g_number)
     <>  ben_erp_shd.g_old_rec.enrt_perd_for_pl_id) then
    --
    -- raise error as PK has changed
    --
    ben_erp_shd.constraint_error('BEN_ENRT_PERD_FOR_PL_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_enrt_perd_for_pl_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_rt_end_dt_rl(p_enrt_perd_for_pl_id         in number,
...
--   Internal table handler use only.
--
Procedure chk_rt_end_dt_cd(p_enrt_perd_for_pl_id         in number,
...
Begin
  ...
  if (l_api_updating
      and p_rt_end_dt_cd
      <> nvl(ben_erp_shd.g_old_rec.rt_end_dt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_rt_strt_dt_rl(p_enrt_perd_for_pl_id      in number,
                            p_business_group_id        in number,
...
--   Internal table handler use only.
--
Procedure chk_rt_strt_dt_cd(p_enrt_perd_for_pl_id         in number,
...
Begin
  ...
  if (l_api_updating
      and p_rt_strt_dt_cd
      <> nvl(ben_erp_shd.g_old_rec.rt_strt_dt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_enrt_cvg_end_dt_rl(p_enrt_perd_for_pl_id       in number,
                                 p_business_group_id         in number,
...
--   Internal table handler use only.
--
Procedure chk_enrt_cvg_end_dt_cd(p_enrt_perd_for_pl_id      in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_erp_shd.g_old_rec.enrt_cvg_end_dt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
--   Internal table handler use only.
--
Procedure chk_enrt_cvg_strt_dt_rl(p_enrt_perd_for_pl_id      in number,
                                  p_business_group_id        in number,
...
--   Internal table handler use only.
--
Procedure chk_enrt_cvg_strt_dt_cd(p_enrt_perd_for_pl_id       in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_erp_shd.g_old_rec.enrt_cvg_strt_dt_cd,hr_api.g_varchar2)
      or not l_api_updating)
...
Begin
  ...
  If (dt_api.validate_dt_upd_mode(p_datetrack_mode => p_datetrack_mode)) then
    ...
    --
    If ((nvl(p_lee_rsn_id, hr_api.g_number) <> hr_api.g_number) and
...
function return_legislation_code
  (p_enrt_perd_for_pl_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_enrt_perd_for_pl_f b
    where b.enrt_perd_for_pl_id      = p_enrt_perd_for_pl_id
