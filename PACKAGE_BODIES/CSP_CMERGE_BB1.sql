--------------------------------------------------------
--  DDL for Package Body CSP_CMERGE_BB1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSP_CMERGE_BB1" AS
/* $Header: cscm101b.pls 115.2 99/07/16 08:47:22 porting ship  $ */
...
/* This procedure handles the merge process for the CS_ACCESS_CONTROL_TEMPLATES.
...
/* Put the header in the report to identify the block to be run */
        ...
        arp_message.set_line('CP_CMERGE_BB1.MERGE()+');

        IF ( process_mode = 'LOCK' ) Then
	        arp_message.set_name('AR', 'AR_LOCKING_TABLE');
     	  	arp_message.set_token('TABLE_NAME', 'CS_ACCESS_CONTROL_TEMPLATES',FALSE );
        	message_text := 'The locking is done in block CSP_CMERGE_BB1';
        	arp_message.set_line(message_text);
        ELSE
        	arp_message.set_name('AR', 'AR_UPDATING_TABLE');
        	arp_message.set_token('TABLE_NAME', 'CS_ACCESS_CONTROL_TEMPLATES',FALSE );
        	message_text := 'The merge is done in block CSP_CMERGE_BB1';
        	arp_message.set_line(message_text);
        END IF;
...
/* That the merge of CS_ACCESS_CONTROL_TEMPLATES is complete, use a cursor to
   check to make sure all data has been updated. If not report it to the
   log file.  */

        CS_CHECK_MERGE_DATA ( req_id, set_number, process_mode );

/* Report that the process for CS_ACCESS_CONTROL_TEMPLATES is complete */

        IF ( process_mode = 'LOCK' ) Then
        	message_text := '** LOCKING completed for table CS_ACCESS_CONTROL_TEMPLATES **';
        	arp_message.set_line(message_text);
        ELSE
        	message_text := '** MERGE completed for table CS_ACCESS_CONTROL_TEMPLATES **';
        	arp_message.set_line(message_text);
        END IF;
  ...
  BEGIN
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
              ...
              message_text := 'cp_install_site_id NOT found -- proceeding *** ';
              arp_message.set_line(message_text);
              arp_message.set_name( 'CS', 'CS_ROWS_UPDATED');
              number_of_rows := sql%rowcount;
              arp_message.set_token( 'NUM_ROWS',to_char( number_of_rows) );
  ...
  BEGIN
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
              ...
              message_text := 'Request id NOT found -- proceeding *** ';
              arp_message.set_line(message_text);
              arp_message.set_name( 'CS', 'CS_ROWS_UPDATED');
              number_of_rows := sql%rowcount;
              arp_message.set_token( 'NUM_ROWS',to_char( number_of_rows) );
...
/* templaray storage location for identifing the record in error */
        ...
        access_control_template_id NUMBER;
    ...
    BEGIN
        IF ( process_mode <> 'LOCK' ) Then
          ...
          LOOP
              FETCH CS_CHECK
               INTO
   	    	       	access_control_template_id;

              EXIT WHEN  CS_CHECK%NOTFOUND;
              message_text :=
                  'WARNING, Following access control template id has address(s) not merged ';
              arp_message.set_line(message_text);
              message_text := access_control_template_id;
              arp_message.set_line(message_text);
