--------------------------------------------------------
--  DDL for Package Body PAY_PTT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PTT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_ptt_bus.';  -- Global package name
--
...
Begin
  ...
  --
  --
  if (( nvl(pay_ptt_bus.g_user_table_id, hr_api.g_number)
       = p_user_table_id)
  and ( nvl(pay_ptt_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pay_ptt_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_ptt_bus.g_user_table_id               := p_user_table_id;
    pay_ptt_bus.g_language                    := p_language;
    pay_ptt_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_ptt_shd.api_updating
