--------------------------------------------------------
--  DDL for Package Body AME_ACT_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ACT_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_act_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_ACTIONS_PK') Then
...
Begin
  ...
  Else
    If (p_action_id =
        ame_act_shd.g_old_rec.action_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_act_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ame_act_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_act_shd.g_old_rec.end_date;
...
Begin
  ...
  If l_child_count >0 then
    fnd_message.set_name('PER','AME_400642_ATU_EXST_CNTDEL_ACT');
    --fnd_message.raise_error;
