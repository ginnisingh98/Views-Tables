--------------------------------------------------------
--  DDL for Package Body IGS_UC_REF_TARIFF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_UC_REF_TARIFF_PKG" AS
/* $Header: IGSXI41B.pls 115.3 2003/02/28 07:51:34 bayadav noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_uc_ref_tariff%ROWTYPE;
  new_references igs_uc_ref_tariff%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.exam_grade                        := x_exam_grade;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_uc_ref_tariff
      WHERE    exam_level = x_exam_level
      AND      exam_grade = x_exam_grade ;
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.tariff_score = x_tariff_score)
