--------------------------------------------------------
--  DDL for Package Body HRI_OPL_BDGTS_HDCNT_ORGMGR
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HRI_OPL_BDGTS_HDCNT_ORGMGR" AS
/* $Header: hriobhom.pkb 120.1 2005/06/29 07:02:12 ddutta noship $ */
--
...
BEGIN
  ...
  --
  INSERT INTO HRI_MDP_BDGTS_HDCNT_ORGMGR_CT (
     orgmgr_id
    ,budget_id
    ,budget_aggregate
   ...
   SELECT  orgmgr.sup_person_id                                       ORGMGR_ID
          ...
          ,grade_id                                                   GRADE_ID
          ,budget_value                                               HEADCOUNT_VALUE
          ,budget_value * DECODE(orgmgr.sub_relative_level, 0, 1, 0)  DR_HEADCOUNT_VALUE
          ,SYSDATE
          ,l_user_id
          ,l_user_id
          ,l_user_id
    ...
    WHERE bdgts.organization_id = orgmgr.sub_organization_id
      AND bdgts.budget_measurement_type = 'HEAD';
  --
  dbg(SQL%ROWCOUNT||' records inserted into HRI_MDP_BDGTS_HDCNT_ORGMGR_CT');
  dbg('Exiting process');
  COMMIT;
  --
EXCEPTION
...
--
  ...
  --
  run_sql_stmt_noerr('ALTER TRIGGER HRI_MDP_BDGTS_HDCNT_ORGMGR_CT_WHO DISABLE');
  --
  -- ---------------------------------------------------------------------------
  ...
  IF (fnd_installation.get_app_info('HRI',l_dummy1, l_dummy2, l_schema)) THEN
    ...
    IF (g_full_refresh = 'Y') THEN
      ...
      hri_utl_ddl.log_and_drop_indexes(
                        ...
                        p_table_name    => 'HRI_MDP_BDGTS_HDCNT_ORGMGR_CT',
                        p_table_owner   => l_schema);
      --
      -- Truncate the table
      --
      EXECUTE IMMEDIATE 'TRUNCATE TABLE ' || l_schema || '.HRI_MDP_BDGTS_HDCNT_ORGMGR_CT';
    --
    END IF;
    --
  END IF;
...
BEGIN
  ...
  --
  -- hri_bpl_conc_log.record_process_start('HRI_OPL_BDGTS_HDCNT_ORGMGR');
  --
  ...
  IF (g_full_refresh = 'Y') THEN
    ...
    IF (fnd_installation.get_app_info('HRI',l_dummy1, l_dummy2, l_schema)) THEN
      ...
      IF fnd_global.conc_request_id is null THEN
        ...
        fnd_stats.gather_table_stats(l_schema,'HRI_MDP_BDGTS_HDCNT_ORGMGR_CT');
        --
      END IF;
      --
    END IF;
  ...
  run_sql_stmt_noerr('ALTER TRIGGER HRI_MDP_BDGTS_HDCNT_ORGMGR_CT_WHO ENABLE');
  --
