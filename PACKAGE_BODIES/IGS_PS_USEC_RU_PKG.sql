--------------------------------------------------------
--  DDL for Package Body IGS_PS_USEC_RU_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_USEC_RU_PKG" AS
/* $Header: IGSPI2VB.pls 115.4 2002/11/29 02:20:10 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_usec_ru%ROWTYPE;
  new_references igs_ps_usec_ru%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.usecru_id                         := x_usecru_id;
  ...
  ||  Purpose : Handles the Unique Constraint logic defined for the columns.Created for bug#2563596
  ||  Known limitations, enhancements or remarks :
  ||  Change History :
  ||  Who             When            What
  ||  (reverse chronological order - newest change first)
  ...
  */
  ...
  ||  Created By : jbegum
  ||  Created On : 11-SEP-2002
  ||  Purpose : Validates the Unique Keys of the table.Created as part of bug#2563596
  ||  Known limitations, enhancements or remarks :
  ||  Change History :
  ||  Who             When            What
  ||  (reverse chronological order - newest change first)
  */
  ...
  ||  Known limitations, enhancements or remarks : proc name get_ufk_igs_ps_unit_ofr_opt_all
  ||                     changed to igs_ufk_igs_ps_unit_ofr_opt as the name can max be 30 Chars
  ||  Change History :
  ||  Who             When            What
  ||  (reverse chronological order - newest change first)
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_PURU_UOO_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_PURU_SRC_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PS_PURU_RUL_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_usec_ru_s.NEXTVAL
    INTO      x_usecru_id
    FROM      dual;
