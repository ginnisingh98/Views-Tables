--------------------------------------------------------
--  DDL for Package Body IGF_AW_FUND_TYPE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGF_AW_FUND_TYPE_PKG" AS
/* $Header: IGFWI03B.pls 115.9 2002/11/28 14:36:57 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igf_aw_fund_type_all%ROWTYPE;
  new_references igf_aw_fund_type_all%ROWTYPE;
  ...
  */
  BEGIN
    ...
    igf_aw_fund_cat_pkg.get_ufk_igf_aw_fund_type (
      old_references.fund_type
    );
  ...
  */
  BEGIN
    ...
    IF (((old_references.fund_type = new_references.fund_type)) OR
        ((old_references.fund_type IS NULL))) THEN
  ...
  */
  ...
  ||  Known limitations, enhancements or remarks :
  ||  Change History : 2166845
  ||  Who             When            What
  ||  vvutukur        13-May-2002     removed upper check on fund_type in cursor cur_rowid.bug#2344826.
  ||  adhawan         23-feb-02'      made the UNIQUE key incasesensitive by adding the UPPER clause
  ||  (reverse chronological order - newest change first)
  */

    l_org_id                     igf_aw_fund_type_all.org_id%TYPE  DEFAULT igf_aw_gen.get_org_id;
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igf_aw_fund_type
  ...
  */
  ...
  BEGIN
    ...
    SELECT igf_aw_fund_type_all_s.nextval INTO x_ft_id FROM dual;
    ...
    INSERT INTO igf_aw_fund_type_all (
      fund_type,
      description,
      active,
      ft_id,
  ...
  */
  BEGIN
    ...
    DELETE FROM igf_aw_fund_type_all
    WHERE rowid = x_rowid;
