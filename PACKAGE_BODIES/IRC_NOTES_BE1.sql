--------------------------------------------------------
--  DDL for Package Body IRC_NOTES_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_NOTES_BE1" as 
...
p_object_version_number        number) is
  ...
  --
  l_proc varchar2(72):='  irc_notes_be1.create_note_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.irc.api.notes.create_note';
  ...
  if (l_message='MESSAGE') then
    ...
    --
    l_text:='<note_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_note_id);
    l_text:=l_text||'</note_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
