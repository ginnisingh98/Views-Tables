--------------------------------------------------------
--  DDL for Package Body PER_SCN_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_SCN_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_scn_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    ...
    per_scn_shd.g_old_rec.name;
  End If;
  /* If (p_rec.template_file = hr_api.g_varchar2) then
    p_rec.template_file :=
    per_scn_shd.g_old_rec.template_file;
  End If; */
  --
End convert_defs;
--
...
Begin
  ...
  --
  per_scn_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_scn_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_scn_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
