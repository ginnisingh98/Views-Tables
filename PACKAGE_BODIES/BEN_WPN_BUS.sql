--------------------------------------------------------
--  DDL for Package Body BEN_WPN_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_WPN_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_wpn_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_wv_prtn_rsn_pl_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   wv_prtn_rsn_pl_id PK of record being inserted or updated.
...
--
Procedure chk_wv_prtn_rsn_pl_id(p_wv_prtn_rsn_pl_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_wv_prtn_rsn_pl_id';
...
Begin
  ...
  --
  l_api_updating := ben_wpn_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_wv_prtn_rsn_pl_id                => p_wv_prtn_rsn_pl_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_wv_prtn_rsn_pl_id,hr_api.g_number)
     <>  ben_wpn_shd.g_old_rec.wv_prtn_rsn_pl_id) then
    --
    -- raise error as PK has changed
    --
    ben_wpn_shd.constraint_error('BEN_WV_PRTN_RSN_PL_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_wv_prtn_rsn_pl_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_wv_prtn_rsn_cd(p_wv_prtn_rsn_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_wpn_shd.g_old_rec.wv_prtn_rsn_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_dflt_flag(p_wv_prtn_rsn_pl_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_dflt_flag
      <> nvl(ben_wpn_shd.g_old_rec.dflt_flag,hr_api.g_varchar2)
...
function return_legislation_code
  (p_wv_prtn_rsn_pl_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_wv_prtn_rsn_pl_f b
    where b.wv_prtn_rsn_pl_id      = p_wv_prtn_rsn_pl_id
