--------------------------------------------------------
--  DDL for Package Body GMD_INVENTORY_SPEC_VRS_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMD_INVENTORY_SPEC_VRS_PVT" AS
/* $Header: GMDVIVRB.pls 120.1.12010000.2 2009/03/18 21:29:13 plowe ship $ */
...
BEGIN
  IF p_spec_vr_id IS NOT NULL THEN
    ...
    ;
  ELSE
    GMD_API_PUB.log_message('GMD_NO_KEYS','TABLE_NAME', 'GMD_INVENTORY_SPEC_VRS');
    RETURN FALSE;
  END IF;
  ...
  IF (SQL%FOUND) THEN
    RETURN TRUE;
  ELSE
    GMD_API_PUB.log_message('GMD_NO_DATA_FOUND','TABLE_NAME', 'GMD_INVENTORY_SPEC_VRS');
    RETURN FALSE;
  END IF;
...
EXCEPTION
  WHEN locked_by_other_user THEN
    GMD_API_PUB.log_message('GMD_RECORD_LOCKED',
                            'TABLE_NAME', 'GMD_INVENTORY_SPEC_VRS',
...
BEGIN
  ...
  IF (p_inventory_spec_vrs.spec_vr_id IS NOT NULL) THEN
    SELECT *
...
EXCEPTION
 ...
 WHEN OTHERS
   THEN
     fnd_msg_pub.add_exc_msg ('GMD_INVENTORY_SPEC_VRS_PVT', 'FETCH_ROW');
     RETURN FALSE;
