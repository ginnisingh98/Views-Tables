--------------------------------------------------------
--  DDL for Package Body BEN_EET_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EET_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_eet_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_enrld_anthr_ptip_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_enrld_anthr_ptip_id PK of record being inserted or updated.
...
--
Procedure chk_elig_enrld_anthr_ptip_id(p_elig_enrld_anthr_ptip_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_enrld_anthr_ptip_id';
...
Begin
  ...
  --
  l_api_updating := ben_eet_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_enrld_anthr_ptip_id                => p_elig_enrld_anthr_ptip_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_enrld_anthr_ptip_id,hr_api.g_number)
     <>  ben_eet_shd.g_old_rec.elig_enrld_anthr_ptip_id) then
    --
    -- raise error as PK has changed
    --
    ben_eet_shd.constraint_error('BEN_ELIG_ENRLD_ANTHR_PTIP_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_enrld_anthr_ptip_id is not null then
      --
  ...
  --
End chk_elig_enrld_anthr_ptip_id;
--
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_only_pls_subj_cobra_flag(p_elig_enrld_anthr_ptip_id  in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_eet_shd.g_old_rec.only_pls_subj_cobra_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_enrl_det_dt_cd(p_elig_enrld_anthr_ptip_id    in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_eet_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_enrld_anthr_ptip_id    in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_eet_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
--
-- In Parameters
--   p_elig_enrld_anthr_ptip_id    elig_enrld_anthr_ptip_id
...
is
  ...
  cursor c1 is
    ...
    where  elig_enrld_anthr_ptip_id <> nvl(p_elig_enrld_anthr_ptip_id,-1)
...
function return_legislation_code
  (p_elig_enrld_anthr_ptip_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_enrld_anthr_ptip_f b
    where b.elig_enrld_anthr_ptip_id      = p_elig_enrld_anthr_ptip_id
