--------------------------------------------------------
--  DDL for Package Body HR_PERSON_ADDRESS_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_PERSON_ADDRESS_BE1" as 
...
p_party_id                     number) is
  ...
  --
  l_proc varchar2(72):='  hr_person_address_be1.create_person_address_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.person_address.create_person_address';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<pradd_ovlapval_override>';
if(P_PRADD_OVLAPVAL_OVERRIDE) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</pradd_ovlapval_override>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
...
l_text:=l_text||'FALSE';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<primary_flag>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_primary_flag);
    l_text:=l_text||'</primary_flag>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<style>';
