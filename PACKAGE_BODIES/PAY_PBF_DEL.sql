--------------------------------------------------------
--  DDL for Package Body PAY_PBF_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PBF_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_pbf_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_delete_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_delete_next_change) then
    ...
    where       balance_feed_id = p_rec.balance_feed_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_zap) then
    ...
    --
    pay_pbf_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
  ...
  hr_utility.set_location('Entering:'||l_proc, 5);
   begin
    ...
    pay_pbf_rkd.after_delete
      ...
      => pay_pbf_shd.g_old_rec.effective_end_date
      ,p_business_group_id_o
      => pay_pbf_shd.g_old_rec.business_group_id
      ,p_legislation_code_o
      => pay_pbf_shd.g_old_rec.legislation_code
      ,p_balance_type_id_o
      => pay_pbf_shd.g_old_rec.balance_type_id
      ,p_input_value_id_o
      => pay_pbf_shd.g_old_rec.input_value_id
      ,p_scale_o
      => pay_pbf_shd.g_old_rec.scale
      ,p_legislation_subgroup_o
      => pay_pbf_shd.g_old_rec.legislation_subgroup
      ,p_object_version_number_o
      => pay_pbf_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
--
  ...
  cursor csr_sel is
  select balance_type_id, input_value_id
...
Begin
  ...
  open csr_sel;
  fetch csr_sel into l_balance_type_id, l_input_value_id;
  close csr_sel;

  l_rec.balance_feed_id          := p_balance_feed_id;
  l_rec.balance_type_id		 := l_balance_type_id;
  l_rec.input_value_id		 := l_input_value_id;
