--------------------------------------------------------
--  DDL for Package Body IGS_AD_PER_STM_TYP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_PER_STM_TYP_PKG" AS
/* $Header: IGSAIG1B.pls 115.7 2003/10/30 13:17:51 akadam noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ad_per_stm_typ%ROWTYPE;
  new_references igs_ad_per_stm_typ%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    new_references.persl_stat_type_desc              := x_persl_stat_type_desc;
  ...
  */
  BEGIN
    ...
    igs_ad_appl_perstat_pkg.get_fk_igs_ad_per_stm_typ (
      old_references.persl_stat_type
    );

    igs_ad_aptyp_pestat_pkg.get_fk_igs_ad_per_stm_typ (
      old_references.persl_stat_type
    );
  ...
  */
  ...
  BEGIN
    ...
    IF (
        (tlinfo.persl_stat_type_desc = x_persl_stat_type_desc)
        AND ((tlinfo.step_catalog_cd = x_step_catalog_cd) OR ((tlinfo.step_catalog_cd IS NULL) AND (X_step_catalog_cd IS NULL)))
