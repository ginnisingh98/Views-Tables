--------------------------------------------------------
--  DDL for Package Body AME_STV_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_STV_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_stv_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_STRING_VALUES_PK') Then
...
Begin
  ...
  Else
    If (p_condition_id =
        ame_stv_shd.g_old_rec.condition_id and
        p_object_version_number =
        ame_stv_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_stv_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_stv_shd.g_old_rec.end_date;
