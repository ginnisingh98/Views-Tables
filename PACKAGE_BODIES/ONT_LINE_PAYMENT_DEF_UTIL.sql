--------------------------------------------------------
--  DDL for Package Body ONT_LINE_PAYMENT_DEF_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ONT_LINE_PAYMENT_DEF_UTIL" AS
...
--  FILENAME
--  
--      ONT_LINE_PAYMENT_Def_Util
--  
--  DESCRIPTION
--  
--      Body of package ONT_LINE_PAYMENT_Def_Util
--  
--  NOTES
--  
--  HISTORY
...
G_PKG_NAME                    CONSTANT VARCHAR2(30) := 'ONT_LINE_PAYMENT_Def_Util';
...
END  Get_Attr_Val_Date;
  ...
  PROCEDURE Clear_LINE_PAYMENT_Cache
  IS  
  BEGIN  
  g_cached_record.PAYMENT_NUMBER := null;
  g_cached_record.LINE_ID := null;
  g_cached_record.HEADER_ID := null;
   END Clear_LINE_PAYMENT_Cache;
 
 
FUNCTION Sync_LINE_PAYMENT_Cache
...
IS
CURSOR cache IS 
  SELECT * FROM   OE_AK_LINE_PAYMENTS_V
...
BEGIN
 ...
 ELSIF (p_foreign_database_object_name = 'OE_AK_ORDER_LINES_V') THEN
    IF NVL(p_record.LINE_ID,FND_API.G_MISS_NUM) = FND_API.G_MISS_NUM THEN 
      RETURN NULL;
    END IF;
    IF  ONT_LINE_Def_Util.Sync_LINE_Cache
      (p_LINE_ID => p_record.LINE_ID) = 1  then 
    RETURN ONT_LINE_Def_Util.Get_Attr_Val_Varchar2
      (p_foreign_attr_code,ONT_LINE_Def_Util.g_cached_record); 
  END IF;
END IF;
...
BEGIN
 ...
 ELSIF (p_foreign_database_object_name = 'OE_AK_ORDER_LINES_V') THEN
    ...
    IF  ONT_LINE_Def_Util.Sync_LINE_Cache
      (p_LINE_ID => p_record.LINE_ID) = 1  then 
    RETURN ONT_LINE_Def_Util.Get_Attr_Val_Date(p_foreign_attr_code,ONT_LINE_Def_Util.g_cached_record); 
  END IF;
END IF;
...
(   p_condition_id                  IN  NUMBER
,   p_line_payment_rec              IN  OE_AK_LINE_PAYMENTS_V%ROWTYPE 
) RETURN BOOLEAN
IS  
  CURSOR CONDNS IS  
...
FOR J in l_start_index ..g_conditions_tbl_cache.COUNT -1 LOOP  
  ...
  g_conditions_tbl_cache(j).value_op,Get_Attr_Val_Varchar2(g_conditions_tbl_cache(j).attribute_code,p_line_payment_rec ));
    ...
    ELSE
      IF (g_attr_condns_cache(l_condn_index).condition_id = 0 OR
         Validate_Defaulting_Condition
	       (g_attr_condns_cache(l_condn_index).condition_id,p_line_payment_rec)= TRUE) THEN 
	     l_index := l_index + 1;
  ...
  ELSE
    FOR c_rec IN ATTRC LOOP  
	  ...
	  IF (c_rec.condition_id = 0 OR
	         Validate_Defaulting_Condition
		  (c_rec.condition_id,p_line_payment_rec)= TRUE) THEN 
	     l_index := l_index + 1;
	     l_valid_condn_index_tbl(l_index) := l_condn_index;
      END IF;
      l_condn_index := l_condn_index + 1;
