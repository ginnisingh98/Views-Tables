--------------------------------------------------------
--  DDL for Package Body OTA_LP_CATEGORY_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LP_CATEGORY_SWI" As
/* $Header: otlciswi.pkb 115.1 2003/12/30 18:49 dhmulia noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'ota_lp_category_swi.';
--
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_lp_cat_inclusion_swi;
  --
  ...
  --
  ota_lci_ins.set_base_key_value
  ...
  -- Call API
  --
  ota_lp_category_api.create_lp_cat_inclusion
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_lp_cat_inclusion_swi;
    --
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_lp_cat_inclusion_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  ota_lp_category_api.delete_lp_cat_inclusion
    (p_learning_path_id             => p_learning_path_id
    ,p_category_usage_id            => p_category_usage_id
    ,p_object_version_number        => p_object_version_number
    ,p_validate                     => l_validate
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_lp_cat_inclusion_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_return_status := hr_multi_message.get_return_status_disable;
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint update_lp_cat_inclusion_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  ota_lp_category_api.update_lp_cat_inclusion
    (p_validate                     => l_validate
    ,p_effective_date               => p_effective_date
    ,p_learning_path_id             => p_learning_path_id
    ,p_object_version_number        => p_object_version_number
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_lp_cat_inclusion_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
