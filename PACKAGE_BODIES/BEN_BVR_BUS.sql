--------------------------------------------------------
--  DDL for Package Body BEN_BVR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BVR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_bvr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_bnft_vrbl_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   bnft_vrbl_rt_id PK of record being inserted or updated.
...
--
Procedure chk_bnft_vrbl_rt_id(p_bnft_vrbl_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_bnft_vrbl_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_bvr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_bnft_vrbl_rt_id                => p_bnft_vrbl_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_bnft_vrbl_rt_id,hr_api.g_number)
     <>  ben_bvr_shd.g_old_rec.bnft_vrbl_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_bvr_shd.constraint_error('BEN_BNFT_VRBL_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_bnft_vrbl_rt_id is not null then
      --
...
--     p_bnft_vrbl_rt_id         Primary Key of BEN_BNFT_VRBL_RT_F
...
cursor c1 is select null
             ...
             Where  bnft_vrbl_rt_id <> nvl(p_bnft_vrbl_rt_id,-1)
...
--
-- Description
--   This procedure is used to check that the profile is mutually exclusive
--   for the cvg_amt_calc_mthd_id.  A rule cannot exist with this cvg_amt_calc_
--   mthd_id.
--
-- Pre Conditions
--   None.
...
--   Internal table handler use only.
--
Procedure chk_brr_bvr_mutexcl(p_bnft_vrbl_rt_id            in number,
                               p_cvg_amt_calc_mthd_id      in number,
                               p_effective_date            in date,
                               p_business_group_id         in number,
                               p_object_version_number     in number) is
  ...
  if (l_api_updating or not l_api_updating)
    ...
    open c1;
      ...
      if c1%found then
        ...
        close c1;
        --
        -- raise an error as this cvg_amt_calc_mthd_id has
        -- been assigned to rule(s).
        --
        fnd_message.set_name('BEN','BEN_91965_BRR_BVR_EXCLU');
        fnd_message.raise_error;
        --
      end if;
      --
...
function return_legislation_code
  (p_bnft_vrbl_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_bnft_vrbl_rt_f b
    where b.bnft_vrbl_rt_id      = p_bnft_vrbl_rt_id
