--------------------------------------------------------
--  DDL for Package Body HR_COMPETENCE_ELEMENT_BE3
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_COMPETENCE_ELEMENT_BE3" as 
...
p_ins_comp_id                  number) is
  ...
  --
  l_proc varchar2(72):='  hr_competence_element_be3.update_personal_comp_element_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.competence_element.update_personal_comp_element';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<ins_ovn>';
    l_text:=l_text||fnd_number.number_to_canonical(p_ins_ovn);
    l_text:=l_text||'</ins_ovn>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<ins_comp_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_ins_comp_id);
    l_text:=l_text||'</ins_comp_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</competence_element>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
