--------------------------------------------------------
--  DDL for Package Body SSP_SSPRPSAP_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."SSP_SSPRPSAP_XMLP_PKG" AS
/* $Header: SSPRPSAPB.pls 120.1 2007/12/24 14:06:48 amakrish noship $ */
...
function c_sapformula(M_due_date in date, M_matching_date in date, M_PERSON_ID in number) return varchar2 is
begin
...
Due_date := M_due_date;
Ewc := M_due_date;
Qw  := ssp_sap_pkg.MATCHING_WEEK_OF_ADOPTION(M_matching_date);
