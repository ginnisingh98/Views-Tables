--------------------------------------------------------
--  DDL for Package Body OZF_CLAIMS_INT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OZF_CLAIMS_INT_PKG" as
/* $Header: ozftcinb.pls 115.4 2004/07/05 08:50:26 upoluri ship $ */
-- ===============================================================
-- Start of Comments
-- Package name
--          OZF_CLAIMS_INT_PKG
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'OZF_CLAIMS_INT_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'ozftcinb.pls';
...
PROCEDURE Insert_Row(
          px_interface_claim_id   IN OUT NOCOPY NUMBER,
...
BEGIN
   ...
   INSERT INTO OZF_CLAIMS_INT_ALL(
           interface_claim_id,
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
        ...
        WHERE INTERFACE_CLAIM_ID =  p_INTERFACE_CLAIM_ID
        FOR UPDATE of INTERFACE_CLAIM_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN c;
    ...
    CLOSE C;
    IF (
           (      Recinfo.interface_claim_id = p_interface_claim_id)
