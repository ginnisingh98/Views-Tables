--------------------------------------------------------
--  DDL for Package Body HRDPP_UPDATE_PTNL_LER_FOR_PER
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HRDPP_UPDATE_PTNL_LER_FOR_PER" as
/*
...
,dd(P_MNL_DT,I_MNL_DT)
,dd(P_DTCTD_DT,I_DTCTD_DT)
,dd(P_PROCD_DT,I_PROCD_DT)
,dd(P_UNPROCD_DT,I_UNPROCD_DT)
,dd(P_VOIDD_DT,I_VOIDD_DT)
,dd(P_MNLO_DT,I_MNLO_DT)
...
decode(l.pval002,cn,vn,vn,vh,
 hr_pump_get.gl(l.pval002,'BEN_PTNL_LER_FOR_PER_STAT',d(l.pval012),vn)) p2,
l.pval002 d2,
decode(l.pval003,cn,vn,vn,vh,
 hr_pump_get.gl(l.pval003,'BEN_PTNL_LER_FOR_PER_SRC',d(l.pval012),vn)) p3,
l.pval003 d3,
...
l.pval011 d11,
decode(l.pval012,cn,dn,d(l.pval012)) p12,
...
(P_PTNL_LER_FOR_PER_USER_KEY => c.p13);
end if;
--
if c.d14=cn then
L_CSD_BY_PTNL_LER_FOR_PER_ID:=nn;
elsif c.d14 is null then 
L_CSD_BY_PTNL_LER_FOR_PER_ID:=nh;
else
L_CSD_BY_PTNL_LER_FOR_PER_ID := 
hr_pump_get.get_csd_by_ptnl_ler_for_per_id
(P_CSD_BY_PPL_USER_KEY => c.p14);
end if;
--
if c.d15=cn then
L_TRGR_TABLE_PK_ID:=nn;
elsif c.d15 is null then 
L_TRGR_TABLE_PK_ID:=nh;
else
L_TRGR_TABLE_PK_ID := 
hr_pump_get.get_trgr_table_pk_id
(P_TRGR_TABLE_PK_USER_KEY => c.p15);
end if;
--
if c.d16=cn then
L_ENRT_PERD_ID:=nn;
elsif c.d16 is null then 
L_ENRT_PERD_ID:=nh;
else
L_ENRT_PERD_ID := 
hr_pump_get.get_enrt_perd_id
(P_ENRT_PERD_USER_KEY => c.p16);
end if;
--
if c.d17=cn or
c.p12 is null then
L_LER_ID:=nn;
elsif c.d17 is null then 
L_LER_ID:=nh;
else
L_LER_ID := 
hr_pump_get.get_ler_id
(P_BUSINESS_GROUP_ID => P_BUSINESS_GROUP_ID
,P_LIFE_EVENT_REASON => c.p17
,P_EFFECTIVE_DATE => c.p12);
end if;
--
if c.d18=cn then
L_PERSON_ID:=nn;
elsif c.d18 is null then 
L_PERSON_ID:=nh;
else
L_PERSON_ID := 
hr_pump_get.get_person_id
(P_PERSON_USER_KEY => c.p18);
end if;
--
L_REQUEST_ID := 
...
hr_pump_get.GET_PTNL_LER_FOR_PER_OVN
(P_PTNL_LER_FOR_PER_USER_KEY => c.p13);
end if;
--
...
,P_LF_EVT_OCRD_DT => c.p1
,P_TRGR_TABLE_PK_ID => L_TRGR_TABLE_PK_ID
,P_PTNL_LER_FOR_PER_STAT_CD => c.p2
,P_PTNL_LER_FOR_PER_SRC_CD => c.p3
,P_MNL_DT => c.p4
