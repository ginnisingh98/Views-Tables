--------------------------------------------------------
--  DDL for Package Body XNP_XDP_FULFILL_START_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_XDP_FULFILL_START_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'XDP_FULFILL_START' ;
  ...
  XNP_XML_UTILS.write_leaf_element ( 'LINE_ID', XNP$LINE_ID );
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'XDP_FULFILL_START' ) ;
    XNP_XML_UTILS.write_leaf_element ( 'LINE_ID', XNP$LINE_ID ) ;
    XNP_XML_UTILS.end_segment ( 'XDP_FULFILL_START' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'XDP_FULFILL_START.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'XDP_FULFILL_START', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_EVT_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$LINE_ID=>XNP$LINE_ID,
    x_msg_header=>l_msg_header,
