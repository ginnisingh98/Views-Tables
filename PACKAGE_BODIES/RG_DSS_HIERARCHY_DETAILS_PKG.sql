--------------------------------------------------------
--  DDL for Package Body RG_DSS_HIERARCHY_DETAILS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."RG_DSS_HIERARCHY_DETAILS_PKG" as
/* $Header: rgidhrdb.pls 120.2 2002/11/14 02:58:38 djogg ship $ */
...
BEGIN
  ...
  FND_MESSAGE.set_token('OBJECT2', 'RG_DSS_HIERARCHY', TRUE);
...
--  IF (X_Master_Hierarchy_Id IS NULL) THEN
--    X_Master_Hierarchy_Id := RG_DSS_HIERARCHIES_PKG.get_new_id;
--  END IF;
--  X_Hierarchy_Id := X_Master_Hierarchy_Id;

  IF (X_Hierarchy_Id IS NULL) THEN
	X_HIERARCHY_ID := RG_DSS_HIERARCHIES_PKG.get_new_id;
  END IF;

  IF (RG_DSS_HIERARCHIES_PKG.used_in_frozen_system(X_Hierarchy_Id)) THEN
    -- can't modify a frozen system
    FND_MESSAGE.set_name('RG', 'RG_DSS_FROZEN_SYSTEM');
    FND_MESSAGE.set_token('OBJECT', 'RG_DSS_HIERARCHY', TRUE);
    APP_EXCEPTION.raise_exception;
  END IF;

  check_unique_sequence(X_Rowid, X_Hierarchy_Id, X_Sequence);
  check_unique_segment(X_Rowid, X_Hierarchy_Id, X_Application_Column_Name);
  INSERT INTO rg_dss_hierarchy_details(
          hierarchy_id,
          sequence,
...
BEGIN
  ...
  IF (
      ...
      AND (   (Recinfo.root_node = X_Root_Node)
           OR (    (Recinfo.root_node IS NULL)
               AND (X_Root_Node IS NULL)))
...
BEGIN
  ...
  UPDATE rg_dss_hierarchy_details
  SET
...
BEGIN
  ...
  DELETE FROM rg_dss_hierarchy_details
  WHERE  rowid = X_Rowid;
