--------------------------------------------------------
--  DDL for Package Body PER_ABV_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ABV_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_abv_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    if l_value <> p_rec.value then

       p_rec.value := l_value;

    end if;

    -- Bug fix 2172818 ends

    --
    -- Update the per_assignment_budget_values_f Row
    --
    update  per_assignment_budget_values_f
    set
     assignment_budget_value_id           = p_rec.assignment_budget_value_id
    ...
    where   assignment_budget_value_id = p_rec.assignment_budget_value_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    per_abv_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_base_key_value	        => p_rec.assignment_budget_value_id
...
Begin
  ...
  begin
    ...
    per_abv_rku.after_update
      ...
      => per_abv_shd.g_old_rec.effective_end_date
      ,p_object_version_number
      => p_rec.object_version_number
      ,p_business_group_id_o
      => per_abv_shd.g_old_rec.business_group_id
      ,p_assignment_id_o
      => per_abv_shd.g_old_rec.assignment_id
      ,p_unit_o
      => per_abv_shd.g_old_rec.unit
      ,p_value_o
      => per_abv_shd.g_old_rec.value
      ,p_request_id_o
      => per_abv_shd.g_old_rec.request_id
      ,p_program_application_id_o
      => per_abv_shd.g_old_rec.program_application_id
      ,p_program_id_o
      => per_abv_shd.g_old_rec.program_id
      ,p_program_update_date_o
      => per_abv_shd.g_old_rec.program_update_date
      ,p_object_version_number_o
      => per_abv_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.unit = hr_api.g_varchar2) then
    ...
    per_abv_shd.g_old_rec.unit;
  End If;
  If (p_rec.value = hr_api.g_number) then
    p_rec.value :=
    per_abv_shd.g_old_rec.value;
  End If;
...
Begin
  ...
  --
  per_abv_upd.convert_defs(p_rec);
  --
  per_abv_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
--
  ...
  l_proc	  varchar2(72) := g_package||'upd';
  l_assignment_id     number := hr_api.g_number;
