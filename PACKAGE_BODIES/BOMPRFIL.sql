--------------------------------------------------------
--  DDL for Package Body BOMPRFIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BOMPRFIL" as
/* $Header: BOMRFILB.pls 115.1 99/07/16 05:16:02 porting ship $ */
...
IS
        ...
        SELECT  opt.profile_option_id,
                level_id,
                level_value,
                substrb(profile_option_value,1,10)
...
/* Set appropriate defaults */
  ...
  OPEN get_profile_cursor(appl_short_name,profile_name,user_id,resp_appl_id,
                        resp_id);
...
EXCEPTION
        WHEN    no_profile_found THEN
                return_code := -9999;
                return_message := 'BOM_NO_PROFILE_VALUE';
        WHEN    OTHERS THEN
                return_code := SQLCODE;
                return_message := 'BOMPRFIL.bom_pr_get_profile ' || SUBSTRB(SQLERRM,1,100);
END bom_pr_get_profile;
