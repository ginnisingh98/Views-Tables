--------------------------------------------------------
--  DDL for Package Body IGS_AD_DOC_STAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_DOC_STAT_PKG" as
/* $Header: IGSAI23B.pls 115.7 2003/10/30 13:11:20 akadam ship $*/
 l_rowid VARCHAR2(25);
  old_references IGS_AD_DOC_STAT%RowType;
  new_references IGS_AD_DOC_STAT%RowType;
...
PROCEDURE Set_Column_Values (
    ...
    x_s_adm_doc_status IN VARCHAR2 DEFAULT NULL,
  ...
  BEGIN
   ...
   new_references.adm_doc_status := x_adm_doc_status;
    ...
    new_references.s_adm_doc_status := x_s_adm_doc_status;
...
PROCEDURE BeforeRowInsertUpdate1(
  ...
  BEGIN
	-- Validate Admission documentation status closed ind.
	IF p_inserting OR ((old_references.s_adm_doc_status <>
				 new_references.s_adm_doc_status) OR
			(old_references.closed_ind = 'Y' AND new_references.closed_ind = 'N')) THEN
		IF IGS_AD_VAL_ADS.admp_val_sads_clsd(
					new_references.s_adm_doc_status,
...
ELSIF upper(Column_name) = 'S_ADM_DOC_STATUS' THEN
  new_references.S_ADM_DOC_STATUS:= COLUMN_VALUE ;
...
IF upper(Column_name) = 'S_ADM_DOC_STATUS' OR COLUMN_NAME IS NULL THEN
  IF new_references.S_ADM_DOC_STATUS<> upper(new_references.S_ADM_DOC_STATUS) then
    Fnd_Message.Set_Name('IGS','IGS_GE_INVALID_VALUE');
    IGS_GE_MSG_STACK.ADD;
    App_Exception.Raise_Exception ;
  END IF;
...
PROCEDURE Check_Parent_Existance AS
  BEGIN
    ...
    IF (((old_references.s_adm_doc_status = new_references.s_adm_doc_status)) OR
        ((new_references.s_adm_doc_status IS NULL))) THEN
      NULL;
    ELSE
  ...
  BEGIN
    ...
    IGS_AD_PS_APPL_INST_PKG.GET_FK_IGS_AD_DOC_STAT (
 ...
 FUNCTION Get_PK_For_Validation (
   ...
   AS
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     IGS_AD_DOC_STAT
      WHERE    adm_doc_status = x_adm_doc_status  AND
 ...
 END IF;
  ...
  PROCEDURE GET_FK_IGS_LOOKUPS_VIEW(
    x_s_adm_doc_status IN VARCHAR2
    ) AS

    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     IGS_AD_DOC_STAT
      WHERE    s_adm_doc_status = x_s_adm_doc_status ;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
       Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ADS_SADS_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
   ...
   NULL;
   --Call to afterowinsetupdaterow2 is removed as this function is obsoleted.
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_DOC_STAT
...
begin
  ...
  Before_DML (
    ...
    x_s_adm_doc_status=>X_S_ADM_DOC_STATUS ,
...
begin
  ...
  if ( (tlinfo.DESCRIPTION = X_DESCRIPTION)
      AND (tlinfo.S_ADM_DOC_STATUS = X_S_ADM_DOC_STATUS)
...
begin
  ...
  update IGS_AD_DOC_STAT set
    DESCRIPTION = NEW_REFERENCES.DESCRIPTION,
    S_ADM_DOC_STATUS = NEW_REFERENCES.S_ADM_DOC_STATUS,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_DOC_STAT
     where ADM_DOC_STATUS = X_ADM_DOC_STATUS
  ;
