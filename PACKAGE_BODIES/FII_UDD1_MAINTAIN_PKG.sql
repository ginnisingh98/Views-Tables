--------------------------------------------------------
--  DDL for Package Body FII_UDD1_MAINTAIN_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FII_UDD1_MAINTAIN_PKG" AS
/* $Header: FIIU1CMB.pls 120.1 2005/10/30 05:05:38 appldev noship $ */


        G_MASTER_VALUE_SET_ID  NUMBER(15)      := NULL;
        G_TOP_NODE_ID          NUMBER(15)      := NULL;
        G_TOP_NODE_VALUE       VARCHAR2(240)   := NULL;
        G_UNASSIGNED_UD1_ID    NUMBER(15);
...
--   Initialize (get the master value set and the top node)
   ...
   BEGIN
     ...
     ----------------------------------------------------------------
     FII_UTIL.initialize('FII_UDD1_MAINTAIN_PKG.log',
                         'FII_UDD1_MAINTAIN_PKG.out',l_dir,'FII_UDD1_MAINTAIN_PKG');
     ...
     l_check := FII_EXCEPTION_CHECK_PKG.detect_unmapped_local_vs('FII_USER_DEFINED_1');
     ...
     IF (FIIDIM_Debug) THEN
       FII_UTIL.Write_Log ('Initialize: Now start processing '|| 'User Defined dimension1');
     End If;


     -- --------------------------------------------------------
     -- Find the unassigned User Defined Dimension1 ID
     -- --------------------------------------------------------

     g_phase := 'Find the shipped FII value set id and the unassigned value id';

      FII_GL_EXTRACTION_UTIL.get_unassigned_id(G_UNASSIGNED_UD1_ID, G_FII_INT_VALUE_SET_ID, l_ret_code);
      IF(l_ret_code = -1) THEN
        RAISE UDIM1_fatal_err;
      END IF;

     -- --------------------------------------------------------
     -- Get the master value set and top node for User Defined Dimension1
     -- --------------------------------------------------------

     g_phase := 'Get the master value set and top node for User Defined Dimension1';
     Begin
        ...
        --If the User Defined Dimension1 dimension is not enabled, raise an exception.
        --Note that we will insert 'UNASSIGNED' to the dimension
        IF NVL(G_DBI_ENABLED_FLAG, 'N') <> 'Y' then
          RAISE UDIM1_NOT_ENABLED;
        END IF;
      ...
      Exception
        ...
        When TOO_MANY_ROWS Then
          FII_UTIL.Write_Log ('More than one master value set found for USER DEFINED Dimension1');
          RAISE UDIM1_fatal_err;
        When UDIM1_NOT_ENABLED then
           raise;
        When UDIM1_fatal_err then
           raise;
        When OTHERS Then
          FII_UTIL.Write_Log ('Unexpected error when getting master value set for USER DEFINED Dimension1');
	  FII_UTIL.Write_Log ('Error Message: '|| substr(sqlerrm,1,180));
          RAISE UDIM1_fatal_err;
     End;

     IF (FIIDIM_Debug) THEN
       FII_UTIL.Write_Log ('USER DEFINED DIMENSION1 Master Value Set ID: '|| G_MASTER_VALUE_SET_ID);
       FII_UTIL.Write_Log ('USER DEFINED DIMENSION1 Master Value Set: '||
                         Get_Value_Set_Name (G_MASTER_VALUE_SET_ID));
       FII_UTIL.Write_Log ('       and USER DEFINED Dimension1 Top Node: '|| G_TOP_NODE_VALUE);
     END IF;
	...
	RAISE UDIM1_fatal_err;
     ...
     --If the User Defined dimension1 is not enabled, raise an exception
   ...
   Exception
     ...
     When UDIM1_fatal_err then
       FII_UTIL.Write_Log ('FII_UDD1_MAINTAIN_PKG.Initialize : '|| 'User defined error');
       FND_CONCURRENT.Af_Rollback;
       FII_MESSAGE.Func_Fail(func_name => 'FII_UDD1_MAINTAIN_PKG.Initialize');
       raise;
...
--   Populate the table FII_DIM_NORM_HIER_GT
   ...
   BEGIN
     ...
     IF (FIIDIM_Debug) THEN
      FII_MESSAGE.Func_Ent ('FII_UDD1_MAINTAIN_PKG.Get_NORM_HIERARCHY_TMP');
     END IF;
    ...
    IF (FIIDIM_Debug) THEN
     FII_MESSAGE.Func_Succ ('FII_UDD1_MAINTAIN_PKG.Get_NORM_HIERARCHY_TMP');
    END IF;

   Exception

     When UDIM1_fatal_err then
       FII_UTIL.Write_Log ('FII_UDD1_MAINTAIN_PKG.Get_NORM_HIERARCHY_TMP: '||


                         'User defined error');
       FND_CONCURRENT.Af_Rollback;
       FII_MESSAGE.Func_Fail(func_name => 'FII_UDD1_MAINTAIN_PKG.Get_NORM_HIERARCHY_TMP');
       raise;
...
-- to different parents within FII_UDD1_HIER_GT (the TMP hierarchy table)
   ...
   BEGIN
     ...
     IF (FIIDIM_Debug) THEN
      FII_MESSAGE.Func_Ent ('FII_UDD1_MAINTAIN_PKG.Detect_Diamond_Shape');
     END IF;
    ...
    IF (FIIDIM_Debug) THEN
     FII_MESSAGE.Func_Succ ('FII_UDD1_MAINTAIN_PKG.Detect_Diamond_Shape');
    END IF;
    IF l_count > 0 THEN
      RAISE UDIM1_MULT_PAR_err;
    END IF;

   Exception

     When UDIM1_MULT_PAR_err then
       FII_UTIL.Write_Log ('FII_UDD1_MAINTAIN_PKG.Detect_Diamond_Shape: '||
...
-- This procedure will populate the TMP hierarchy table
    ...
    PROCEDURE  Flatten_UD1_Dim_Hier (p_vset_id NUMBER, p_root_node VARCHAR2)  IS
      CURSOR  MAIN_CSR is
    ...
    BEGIN
      ...
      IF (FIIDIM_Debug) THEN
       FII_MESSAGE.Func_Ent(func_name => 'FII_UDD1_MAINTAIN_PKG.'||
                             'Flatten_UD1_Dim_Hier');
      END IF;
      g_phase := 'Truncate table FII_UDD1_HIER_GT';
      FII_UTIL.truncate_table ('FII_UDD1_HIER_GT', 'FII', g_retcode);

      -----------------------------------------------------------------

      UDIM1_parent_node    := p_root_node;
      UDIM1_parent_vset_id := p_vset_id;
      ...
      UDIM1_parent_flex_id := p_parent_id;
	...
	FII_UTIL.Write_Log('Inserted ' || SQL%ROWCOUNT || ' rows into FII_UDD1_HIER_GT');
      ...
      -- We use (just created) TMP table FII_UDD1_HIER_GT for this purpose
	...
	----------------------------------------------------------------------
     ...
     IF (FIIDIM_Debug) THEN
      FII_MESSAGE.Func_Succ(func_name => 'FII_UDD1_MAINTAIN_PKG.'||
                             'Flatten_UD1_Dim_Hier');
     END IF;
    ...
    EXCEPTION
      ...
      WHEN  NO_DATA_FOUND THEN
        FII_UTIL.Write_Log ('Flatten_UD1_Dim_Hier: No Data Found');
        FII_MESSAGE.Func_Fail(func_name => 'FII_UDD1_MAINTAIN_PKG.'||
                             'Flatten_UD1_Dim_Hier');
        raise;

       WhEN UDIM1_MULT_PAR_err THEN
         FII_UTIL.Write_Log ('Flatten_UD1_Dim_Hier: Diamond Shape Detected');
         FII_MESSAGE.Func_Fail (func_name =>
		'FII_UDD1_MAINTAIN_PKG.Flatten_UD1_Dim_Hier');
         raise;

      WHEN OTHERS THEN
        FII_UTIL.Write_Log ('Flatten_UD1_Dim_Hier: '|| substr(sqlerrm,1,180));
        FII_MESSAGE.Func_Fail(func_name => 'FII_UDD1_MAINTAIN_PKG.'||
                             'Flatten_UD1_Dim_Hier');
        FII_UTIL.Write_Log ( 'G_PHASE: ' || G_PHASE);
        raise;

    END Flatten_UD1_Dim_Hier;


    -- **************************************************************************
-- Populate FII_UDD1_MAPPING_GT Table for FII_UDD1_MAPPINGS

   PROCEDURE Get_UD1_Mapping_GT  IS
   ...
   END IF;
     ...
     --FII_UDD1_HIERARCHIES. Note we should exclude all self leaf
	...
	--------------------------------------------------------
      --Call FND_STATS to collect statistics after populating the table
      g_phase := 'gather_table_stats for FII_UDD1_MAPPINGS_GT';

       FND_STATS.gather_table_stats
  	       (ownname	=> g_schema_name,
	        tabname	=> 'FII_UDD1_MAPPING_GT');

     IF (FIIDIM_Debug) THEN
	      FII_MESSAGE.Func_Succ(func_name => 'FII_UDD1_MAINTAIN_PKG.Get_UD1_Mapping_GT');
     END IF;

    EXCEPTION

      WHEN OTHERS THEN
        FII_UTIL.Write_Log ('Get_UD1_Mapping_GT -> phase: '|| g_phase);
        FII_UTIL.Write_Log ('Get_UD1_Mapping_GT: '|| substr(sqlerrm,1,180));
        FII_MESSAGE.Func_Fail(func_name => 'FII_UDD1_MAINTAIN_PKG.'||
                             'Get_UD1_Mapping_GT');
        raise;

    END Get_UD1_Mapping_GT;

-- **************************************************************************
-- Populate the pruned User Defined Dimension1 hierarchy FII_UDD1_HIERARCHIES by deleting from
-- FII_UDD1_HIER_GT (full version) the LVS records

   PROCEDURE Get_Pruned_UD1_GT  IS
   ...
   Begin
    ...
    --Delete from FII_UDD1_HIER_GT for child value set not equal to
    ...
    --for the latest FII_UDD1_HIER_GT
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        FII_UTIL.Write_Log ('Get_Pruned_UD1_GT -> phase: '|| g_phase);
        FII_UTIL.Write_Log ('Get_Pruned_UD1_GT: '|| substr(sqlerrm,1,180));
        FII_MESSAGE.Func_Fail(func_name => 'FII_UDD1_MAINTAIN_PKG.'||
                             'Get_Pruned_UD1_GT');
        raise;

    END Get_Pruned_UD1_GT;
...
--
   ...
   Begin
     ...
     FII_UTIL.truncate_table ('FII_FULL_UDD1_HIERS',  'FII', g_retcode);
     FII_UTIL.truncate_table ('FII_UDD1_HIERARCHIES', 'FII', g_retcode);
     FII_UTIL.truncate_table ('FII_UDD1_MAPPINGS', 'FII', g_retcode);
...
-- **************************************************************************
-- This is the main procedure of User Defined Dimension1 dimension program (initial populate).
  ...
  BEGIN
      ...
      IF (FIIDIM_Debug) THEN
       FII_MESSAGE.Func_Ent(func_name => 'FII_UDD1_MAINTAIN_PKG.Init_Load');
      END IF;
    ...
    --Call the Flatten User Defined Dimension1 dimension hierarchy routine to insert all mappings.

      Flatten_UD1_Dim_Hier (G_MASTER_VALUE_SET_ID, G_TOP_NODE_VALUE);
    ...
    --for the full dimension table since it will be used later
     FND_STATS.gather_table_stats
       (ownname	=> g_schema_name,
        tabname	=> 'FII_FULL_UDD1_HIERS');

    --==============================================================--

    --Delete/Update FII_UDD1_HIER_GT for pruned hierarchy table
    g_phase := 'Delete/Update FII_UDD1_HIER_GT for pruned hierarchy table';

     Get_Pruned_UD1_GT;

    --Copy FII_UDD1_HIER_GT to the final (pruned) dimension table
    ...
    --================================================================--
      ...
      --Populate FII_UDD1_MAPPINGS table
      g_phase := 'Populate FII_UDD1_MAPPINGS_GT table';

      Get_UD1_Mapping_GT;

      --Copy FII_UDD1_MAPPING_GT to FII_UDD1_MAPPINGS
      g_phase := 'Copy TMP UD1 Mapping table to the UD1 Mapping Table';
    ...
    --Call FND_STATS to collect statistics after re-populating the table.
     ...
     g_phase := 'gather_table_stats FII_UDD1_MAPPINGS';

     FND_STATS.gather_table_stats
        (ownname	=> g_schema_name,
         tabname	=> 'FII_UDD1_MAPPINGS');

     g_phase := 'gather_table_stats MLOG$_FII_UDD1_MAPPINGS';

     FND_STATS.gather_table_stats
        (ownname	=> g_schema_name,
         tabname	=> 'MLOG$_FII_UDD1_MAPPINGS');
    ...
    --=====================================================================
     ...
     IF (FIIDIM_Debug) THEN
      FII_MESSAGE.Func_Succ(func_name => 'FII_UDD1_MAINTAIN_PKG.Init_Load');
     END IF;
  ...
  EXCEPTION
    ...
    WHEN UDIM1_fatal_err THEN
      ...
      FII_UTIL.Write_Log ('FII_UDD1_MAINTAIN_PKG.Init_Load: '||
                        'User defined error');

      FND_CONCURRENT.Af_Rollback;
      FII_MESSAGE.Func_Fail(func_name	=> 'FII_UDD1_MAINTAIN_PKG.Init_Load');
      retcode := sqlcode;
    ...
    WHEN UDIM1_NOT_ENABLED THEN
      FII_UTIL.Write_Log ('>>> UDD1 Dimension Not Enabled...');
    ...
    WHEN OTHERS THEN
      ...
      FII_UTIL.Write_Log (
          'Other error in FII_UDD1_MAINTAIN_PKG.Init_Load: ' || substr(sqlerrm,1,180));
...
-- This is the main procedure of User Defined Dimension1 dimension program (incremental update).
   ...
   BEGIN
    ...
    g_phase := 'Copy TMP hierarchy table to the final pruned dimension table';
     ...
     -- All data is now in the temporary table FII_UDD1_HIER_GT,
     -- we need to maintain the permanent table FII_UDD1_HIERARCHIES
	...
	(PARENT_USER_DIM1_ID, CHILD_USER_DIM1_ID) IN
     ...
     g_phase := 'gather_table_stats for FII_UDD1_MAPPINGS';
     ...
     -- g_phase := 'gather_table_stats for MLOG$_FII_UDD1_MAPPINGS';
   ...
   EXCEPTION
    ...
    WHEN UDIM1_NOT_ENABLED THEN
      FII_UTIL.Write_Log ('>>> User Defined Dimension1 Not Enabled...');
