--------------------------------------------------------
--  DDL for Package Body PAY_KR_YEA20090101_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KR_YEA20090101_PKG" as
/* $Header: pykryea8.pkb 120.0.12010000.13 2010/04/05 10:13:23 vaisriva noship $ */
-- Constant
	-- Bug 3966549
	...
	if (p_yea_info.nationality = 'F' and p_yea_info.fixed_tax_rate = 'Y') then
		...
		*/
		...
		if p_yea_info.nationality = 'F' then
		        -- Bug 6666723
			--
			-- Start of Bug 9539550: Updated the Foreign Worker Income Exem calculation to use
			--			 only the current taxable earnings
			p_yea_info.cur_fw_income_exem := greatest(trunc(nvl(p_yea_info.cur_taxable,0) * l_fw_income_exem_rate),0);
			p_yea_info.foreign_worker_income_exem := nvl(p_yea_info.foreign_worker_income_exem,0)
			 					 + nvl(p_yea_info.cur_fw_income_exem,0);  -- Bug 8644512
			--
			if p_yea_info.foreign_worker_income_exem > 0 then
				...
				--
				-- Bug 4119539
				l_fw_inc_exem_res 		:= nvl(p_yea_info.cur_fw_income_exem,0) ;
				p_yea_info.non_taxable		:= nvl(p_yea_info.non_taxable,0) + nvl(p_yea_info.cur_fw_income_exem,0);
				p_yea_info.cur_total_ntax_earn	:= nvl(p_yea_info.cur_total_ntax_earn,0)
								 + nvl(p_yea_info.cur_fw_income_exem,0) ;
			-- End of Bug 9539550
				--
				/*
				-- Bug 7615517
			...
			if l_dummy > 0
			   ...
			   or p_yea_info.lt_housing_loan_intr_repay_1 > 0
				...
				for i in 1..p_yea_info.housing_saving_tbl.count loop
				       ...
				       /* Bug 9079478 */
				       if p_yea_info.housing_saving_type_tbl(i) in ('HST1','HST5') then
