--------------------------------------------------------
--  DDL for Package Body IGS_PE_UNT_SET_EXCL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PE_UNT_SET_EXCL_PKG" AS
  /* $Header: IGSNI35B.pls 115.4 2002/11/29 01:22:46 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PE_UNT_SET_EXCL%RowType;
  new_references IGS_PE_UNT_SET_EXCL%RowType;
  ...
  BEGIN
    ...
    new_references.puse_start_dt := x_puse_start_dt;
...
v_message_name  varchar2(30);
  BEGIN
	...
	IF (new_references.puse_start_dt IS NOT NULL) AND
		(p_inserting OR (p_updating AND
		(NVL(old_references.puse_start_dt, IGS_GE_DATE.IGSDATE('1900/01/01'))
		<> new_references.puse_start_dt)))
		THEN
	...
	IF p_inserting THEN
		...
		END IF;
	END IF;
	-- Validate if IGS_PE_PERSON is enrolled in the IGS_PS_UNIT set which is to be excluded.
	-- If so, raise an error as this is not allowable.
	IF p_inserting THEN
		IF IGS_EN_VAL_PUSE.enrp_val_puse_us (
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_puse_ar_iu
  -- AFTER INSERT OR UPDATE
  -- ON IGS_PE_UNT_SET_EXCL
  -- FOR EACH ROW
  ...
  BEGIN
	-- Validate for open ended IGS_PE_PERSON IGS_PS_UNIT set exclusion records.
	IF new_references.expiry_dt IS NULL THEN
  		-- Validate for open ended IGS_PE_UNT_SET_EXCL records.
  		IF new_references.expiry_dt IS NULL THEN
  			IF IGS_EN_VAL_PUSE.enrp_val_puse_open (
  					new_references.person_id,
 ...
 END IF;
  ...
  FUNCTION Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      ...
      AND      puse_start_dt = x_puse_start_dt
      FOR UPDATE NOWAIT;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PUSE_PEE_FK');
       IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PUSE_US_FK');
      IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
  ...
  BEGIN
    ...
    new_references.unit_set_cd ,
    new_references.us_version_number ,
    new_references.puse_start_dt ) THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PE_UNT_SET_EXCL
      where PERSON_ID = X_PERSON_ID
      and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
      and PEN_START_DT = X_PEN_START_DT
      and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
  ...
  Before_DML(
   ...
   x_puse_start_dt=>X_PUSE_START_DT,
  ...
  insert into IGS_PE_UNT_SET_EXCL (
    PERSON_ID,
    ENCUMBRANCE_TYPE,
    PEN_START_DT,
    S_ENCMB_EFFECT_TYPE,
...
begin
  ...
  update IGS_PE_UNT_SET_EXCL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PE_UNT_SET_EXCL
     where PERSON_ID = X_PERSON_ID
     and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
     and PEN_START_DT = X_PEN_START_DT
     and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
 ...
 Before_DML(
  ...
  delete from IGS_PE_UNT_SET_EXCL
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
