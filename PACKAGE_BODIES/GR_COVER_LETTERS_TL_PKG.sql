--------------------------------------------------------
--  DDL for Package Body GR_COVER_LETTERS_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_COVER_LETTERS_TL_PKG" AS
/*$Header: GRHICLTB.pls 115.8 2002/10/24 22:13:12 mgrosser ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_cover_letters_tl
   		  	     (cover_letter_id,
				  description,
				  text,
				  language,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_cover_letters_tl
	  SET	 cover_letter_id	 	 	 	 = p_cover_letter_id,
...
L_MSG_DATA		  		VARCHAR2(2000);
L_MSG_TOKEN		  		VARCHAR2(100);
L_BASE_DESC             GR_COVER_LETTERS_TL.description%TYPE;
L_BASE_TEXT				GR_COVER_LETTERS_TL.text%TYPE;
...
**   Cursors
*/
CURSOR c_get_cover_letter
 IS
   SELECT	clt.description,
			clt.text,
            clt.created_by,
			clt.creation_date,
			clt.last_updated_by,
			clt.last_update_date,
			clt.last_update_login
   FROM	    gr_cover_letters_tl clt
   WHERE	clt.cover_letter_id = p_cover_letter_id
   AND		clt.language = l_language;
CoverLetter				c_get_cover_letter%ROWTYPE;
...
/*	Initialization Routine */
   ...
   x_oracle_error := 0;
   x_msg_data := NULL;
   l_msg_token := p_cover_letter_id || ' ' || p_language;

/* Remove translations with no base row */

  delete from GR_COVER_LETTERS_TL T
  where not exists
    (select NULL
    from GR_COVER_LETTERS_B B
    where B.COVER_LETTER_ID =  T.COVER_LETTER_ID
    );

/* Redefault translations from the source language  */

   update gr_cover_letters_tl t set (
    description,
    text ) =
    ( select
      B.DESCRIPTION,
      B.TEXT
      from GR_COVER_LETTERS_TL B
      where B.COVER_LETTER_ID = T.COVER_LETTER_ID
   ...
   where (
      ...
      T.LANGUAGE
         ...
         SUBT.COVER_LETTER_ID,
         SUBT.LANGUAGE
         from GR_COVER_LETTERS_TL SUBB, GR_COVER_LETTERS_TL SUBT
         where SUBB.COVER_LETTER_ID = SUBT.COVER_LETTER_ID
...
*/
   ...
   OPEN c_get_cover_letter;
   FETCH c_get_cover_letter INTO CoverLetter;
   IF c_get_cover_letter%NOTFOUND THEN
      CLOSE c_get_cover_letter;
      RAISE Language_Missing_Error;
   ELSE
	  l_base_desc := CoverLetter.description;
	  l_base_text := CoverLetter.text;
	  l_created_by := CoverLetter.created_by;
	  l_creation_date := CoverLetter.creation_date;
	  l_last_updated_by := CoverLetter.last_updated_by;
	  l_last_update_date := CoverLetter.last_update_date;
	  l_last_update_login := CoverLetter.last_update_login;
      CLOSE c_get_cover_letter;
   END IF;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
	  ...
	  x_return_status := 'U';
	  x_oracle_error := SQLCODE;
	  l_msg_token := SUBSTR(SQLERRM, 1, 200);
	  FND_MESSAGE.SET_NAME('GR',
	                       'GR_UNEXPECTED_ERROR');
	  FND_MESSAGE.SET_TOKEN('TEXT',
	                        l_msg_token||sqlerrm,
...
CURSOR c_lock_letters_tl
 IS
   SELECT	*
   FROM		gr_cover_letters_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockLetterRcd	  c_lock_letters_tl%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_letters_tl;
   FETCH c_lock_letters_tl INTO LockLetterRcd;
   IF c_lock_letters_tl%NOTFOUND THEN
	  CLOSE c_lock_letters_tl;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_cover_letters_tl
   WHERE  	   rowid = p_rowid;
...
/*   Initialization Routine */
   ...
   DELETE FROM gr_cover_letters_tl
   WHERE 	   cover_letter_id = p_cover_letter_id;
...
/*		  p_cover_letter_id is the code to check.
...
CURSOR c_get_letters_tl_rowid
 IS
   SELECT clt.rowid
   FROM	  gr_cover_letters_tl clt
   WHERE  clt.cover_letter_id = p_cover_letter_id
   AND	  clt.language = p_language;
LetterTLRecord			   c_get_letters_tl_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_cover_letter_id || ' ' || p_language;

   x_key_exists := 'F';
   OPEN c_get_letters_tl_rowid;
   FETCH c_get_letters_tl_rowid INTO LetterTLRecord;
   IF c_get_letters_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := LetterTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_letters_tl_rowid;
...
BEGIN
	UPDATE GR_COVER_LETTERS_TL SET
		DESCRIPTION = X_DESCRIPTION,
		SOURCE_LANG = USERENV('LANG'),
		LAST_UPDATE_DATE = sysdate,
		LAST_UPDATED_BY = 0,
		LAST_UPDATE_LOGIN = 0
	WHERE (COVER_LETTER_ID = X_COVER_LETTER_ID)
...
BEGIN
	...
	IF Cur_rowid%FOUND THEN
		GR_COVER_LETTERS_TL_PKG.UPDATE_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_ROWID => l_row_id
			,P_COVER_LETTER_ID => X_COVER_LETTER_ID
	...
	ELSE
		GR_COVER_LETTERS_TL_PKG.INSERT_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_COVER_LETTER_ID => X_COVER_LETTER_ID
			,P_DESCRIPTION => X_DESCRIPTION
...
begin
  ...
  insert into GR_COVER_LETTERS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GR_COVER_LETTERS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from GR_COVER_LETTERS_TL T
    where T.COVER_LETTER_ID = B.COVER_LETTER_ID
