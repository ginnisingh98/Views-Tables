--------------------------------------------------------
--  DDL for Package Body AHL_LTP_SPACE_UNAVL_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AHL_LTP_SPACE_UNAVL_PUB" AS
/* $Header: AHLPSUAB.pls 115.4 2002/12/04 19:34:31 ssurapan noship $ */

G_PKG_NAME  VARCHAR2(30)  := 'AHL_LTP_SPACE_UNAVL_PUB';
G_DEBUG     VARCHAR2(1)   := FND_PROFILE.VALUE('AHL_API_FILE_DEBUG_ON');
--
-----------------------------------------------------------
-- PACKAGE
--    AHL_LTP_SPACE_UNAVL_PUB
--
-- PURPOSE
...
BEGIN
  ...
  SAVEPOINT process_space_restriction;
   ...
   -- Debug info.
   IF G_DEBUG='Y' THEN
       AHL_DEBUG_PUB.debug( 'enter ahl_ltp_space_unavl_pub.Process Space Restriction','+SUAVL+');
    END IF;
   ...
   THEN
     ...
     LOOP
       IF p_x_space_restriction_tbl(i).operation_flag = 'C' THEN
         AHL_LTP_SPACE_UNAVL_PVT.CREATE_SPACE_RESTRICTION
           (
         ...
         AHL_LTP_SPACE_UNAVL_PVT.UPDATE_SPACE_RESTRICTION
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
       ...
       ELSIF p_x_space_restriction_tbl(i).operation_flag = 'D' THEN
         AHL_LTP_SPACE_UNAVL_PVT.DELETE_SPACE_RESTRICTION
           (
            p_api_version             => l_api_version,
            p_init_msg_list           => p_init_msg_list,
            p_commit                  => p_commit,
 ...
 WHEN FND_API.G_EXC_UNEXPECTED_ERROR THEN
   ...
   IF G_DEBUG='Y' THEN
            ...
            AHL_DEBUG_PUB.debug( 'ahl_ltp_space_unavl_pub.Process Space Restriction','+SUAVL+');
        END IF;
        -- Check if API is called in debug mode. If yes, disable debug.
        AHL_DEBUG_PUB.disable_debug;
...
WHEN OTHERS THEN
    ...
    THEN
    fnd_msg_pub.add_exc_msg(p_pkg_name        =>  'AHL_LTP_SPACE_UNAVL_PUB',
