--------------------------------------------------------
--  DDL for Package Body HR_TMT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_TMT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_tmt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_tmt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_form_templates_tl Row
  --
  update hr_form_templates_tl
    set
     form_template_id                = p_rec.form_template_id
    ,language                        = p_rec.language
    ,source_lang                     = p_rec.source_lang
    ,user_template_name              = p_rec.user_template_name
...
Begin
  ...
  begin
    ...
    hr_tmt_rku.after_update
      ...
      ,p_source_lang
      => p_rec.source_lang
      ,p_user_template_name
      => p_rec.user_template_name
      ,p_description
      => p_rec.description
      ,p_source_lang_o
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    ...
    hr_tmt_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.user_template_name = hr_api.g_varchar2) then
    p_rec.user_template_name :=
    hr_tmt_shd.g_old_rec.user_template_name;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  hr_tmt_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hr_tmt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_tmt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_tmt_upd.post_update
     (p_rec
     );
End upd;
--
  ...
  cursor csr_upd_langs is
    select tmt.language
     ...
     where tmt.form_template_id = p_form_template_id
       and p_language_code in (tmt.language
                              ,tmt.source_lang);
  --
