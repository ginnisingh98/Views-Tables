--------------------------------------------------------
--  DDL for Package Body ARP_CPROF1_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ARP_CPROF1_PKG" as
/* $Header: AROCPR1B.pls 120.3.12010000.2 2009/09/17 21:48:59 rravikir ship $ */
--
--
--
 ...
 dummy_message   varchar2(2000);/*Bug 2806125, increased the maximum width to 2000*/
 --
 begin
 --
...
BEGIN
    if (p_site_use_id is null) then
        update hz_customer_profiles
           set credit_hold = p_credit_hold, LAST_UPDATE_DATE = sysdate
         where CUST_ACCOUNT_ID= p_customer_id
           and site_use_id is null;
    else
        update hz_customer_profiles
