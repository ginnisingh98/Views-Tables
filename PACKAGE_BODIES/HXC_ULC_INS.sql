--------------------------------------------------------
--  DDL for Package Body HXC_ULC_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_ULC_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_ulc_ins.';  -- Global package name
...
Begin
  ...
  --
  --
  -- Insert the row into: hxc_layout_components
  --
  insert into hxc_layout_components
      (layout_component_id
      ,layout_id
      ,parent_component_id
      ,component_name
...
--
  l_proc  varchar2(72) ;
--
  Cursor C_Sel1 is select hxc_layout_components_s.nextval from sys.dual;
--
Begin
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.layout_component_id;
  Close C_Sel1;
  --
  if g_debug then
  	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_ulc_bus.insert_validate
     (p_rec
     );
  --
  -- Call the supporting pre-insert operation
  --
  hxc_ulc_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_ulc_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_ulc_ins.post_insert
     (p_rec
     );
  --
  if g_debug then
...
Begin
  ...
  --
  p_layout_component_id := l_rec.layout_component_id;
