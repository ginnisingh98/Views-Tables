--------------------------------------------------------
--  DDL for Package Body ISC_DBI_SHIP_LATE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ISC_DBI_SHIP_LATE_PKG" AS
/* $Header: ISCRG67B.pls 120.1 2006/06/26 06:20:08 abhdixi noship $ */
...
BEGIN
  ...
  l_formula_sql :='
		...
		null					ISC_MEASURE_3, -- obsoleted 5.0 item
		a.days_late/ decode(a.late_line_cnt,
  ...
  l_outer_sql:= 'ISC_MEASURE_10,ISC_MEASURE_1,ISC_MEASURE_2,ISC_MEASURE_3,ISC_MEASURE_4,ISC_MEASURE_5,
		ISC_MEASURE_6,ISC_MEASURE_7,ISC_MEASURE_8,ISC_MEASURE_9,
		CURRENCY';


  l_inner_sql:='sum(shipped_line_cnt)		SHIPPED_LINE_CNT,
		sum(late_line_cnt)		LATE_LINE_CNT,
		sum(days_late)			DAYS_LATE,
		sum(book_to_ship_days_late)	BOOK_TO_SHIP_DAYS,
		sum(shipped_late_qty)		LATE_QTY
