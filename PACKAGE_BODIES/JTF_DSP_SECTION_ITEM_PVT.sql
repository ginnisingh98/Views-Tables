--------------------------------------------------------
--  DDL for Package Body JTF_DSP_SECTION_ITEM_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_DSP_SECTION_ITEM_PVT" AS
/* $Header: JTFVCSIB.pls 115.14 2004/07/09 18:51:46 applrt ship $ */
...
G_PKG_NAME  CONSTANT VARCHAR2(30):= 'JTF_DSP_SECTION_ITEM_PVT';
G_FILE_NAME CONSTANT VARCHAR2(12):= 'JTFVCSIB.pls';
...
BEGIN
  ...
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_INVALID_SCT_ID');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_INVALID_INV_ITEM_ID');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_INVALID_INV_ORG_ID');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
  ...
  IF (c1%FOUND) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SCT_HAS_CHILD_SCT');
    FND_MESSAGE.Set_Token('SECTION_ID', p_section_id);
    FND_MSG_PUB.Add;
    CLOSE c1;
    RAISE FND_API.G_EXC_ERROR;
...
BEGIN
  ...
  IF (p_section_item_id IS NULL) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_INVALID_PRIMARY_KEY');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
...
BEGIN
  ...
  ELSIF (l_return_status = FND_API.G_RET_STS_ERROR) THEN
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_SI_INVALID_CREATE');
    FND_MSG_PUB.Add;
...
BEGIN
  ...
  THEN
    ...
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
         FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_NO_SI_ID');
         FND_MESSAGE.Set_Token('SECTION_ID', p_section_id);
         FND_MESSAGE.Set_Token('INVENTORY_ITEM_ID', p_inventory_item_id);
         FND_MESSAGE.Set_Token('ORGANIZATION_ID', p_organization_id);
         FND_MSG_PUB.Add;
  ...
  ELSE
    ...
    -- section_id, inventory_item_id and organization_id is specified
    FND_MESSAGE.Set_Name('JTF', 'JTF_DSP_NO_SI_IDS_SPEC');
    FND_MSG_PUB.Add;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
...
BEGIN
  ...
  -- delete from jtf_dsp_msite_sct_items table
