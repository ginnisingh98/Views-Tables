--------------------------------------------------------
--  DDL for Package Body GR_PROPERTIES_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_PROPERTIES_TL_PKG" AS
/*$Header: GRHIPRTB.pls 120.1 2006/06/16 21:40:21 pbamb noship $*/
...
CURSOR c_get_descs
 IS
   SELECT	prt.description,
            prt.created_by,
			prt.creation_date,
			prt.last_updated_by,
			prt.last_update_date,
			prt.last_update_login
   FROM	    gr_properties_tl prt
   WHERE	prt.property_id = p_property_id
   AND		prt.language = l_language;
...
/* Redefault translations from the source language  */
   ...
   update gr_properties_tl t set (
    description ) =
    ( select
      B.DESCRIPTION
      from GR_PROPERTIES_TL B
      where B.PROPERTY_ID = T.PROPERTY_ID
   ...
   where (
      ...
      T.LANGUAGE
         ...
         SUBT.PROPERTY_ID,
         SUBT.LANGUAGE
         from GR_PROPERTIES_TL SUBB, GR_PROPERTIES_TL SUBT
...
*/
   ...
   ELSE
      l_base_desc := PropertyDesc.description;
...
CURSOR c_lock_properties_tl
 IS
   SELECT	*
   FROM		gr_properties_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockPropertyRcd	  c_lock_properties_tl%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_properties_tl;
   FETCH c_lock_properties_tl INTO LockPropertyRcd;
   IF c_lock_properties_tl%NOTFOUND THEN
	  CLOSE c_lock_properties_tl;
...
/*		  p_property_id is the safety phrase code to check.
...
CURSOR c_get_properties_tl_rowid
 IS
   SELECT prt.rowid
   FROM	  gr_properties_tl prt
   WHERE  prt.property_id = p_property_id
   AND	  prt.language = p_language;
PropertyTLRecord			   c_get_properties_tl_rowid%ROWTYPE;
...
BEGIN
   ...
   x_key_exists := 'F';
   OPEN c_get_properties_tl_rowid;
   FETCH c_get_properties_tl_rowid INTO PropertyTLRecord;
   IF c_get_properties_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := PropertyTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_properties_tl_rowid;
...
BEGIN
   UPDATE GR_PROPERTIES_TL SET
     ...
     LAST_UPDATE_LOGIN          = 0,
     SOURCE_LANG                = userenv('LANG')
     WHERE PROPERTY_ID       = X_PROPERTY_ID AND
...
BEGIN
	...
	IF Cur_rowid%FOUND THEN
		GR_PROPERTIES_TL_PKG.UPDATE_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_ROWID => l_row_id
	...
	ELSE
           insert into GR_PROPERTIES_TL (
	        ...
	        0,
              ...
              and not exists
                (select NULL
                from GR_PROPERTIES_TL T
                where T.PROPERTY_ID = X_PROPERTY_ID
...
begin
  ...
  insert into GR_PROPERTIES_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GR_PROPERTIES_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
