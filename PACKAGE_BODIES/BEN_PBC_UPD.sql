--------------------------------------------------------
--  DDL for Package Body BEN_PBC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PBC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pbc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_pbc_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.bnf_ctfn_typ_cd = hr_api.g_varchar2) then
    ...
    ben_pbc_shd.g_old_rec.bnf_ctfn_typ_cd;
  End If;
  If (p_rec.bnf_ctfn_recd_dt = hr_api.g_date) then
    p_rec.bnf_ctfn_recd_dt :=
    ben_pbc_shd.g_old_rec.bnf_ctfn_recd_dt;
  End If;
  If (p_rec.bnf_ctfn_rqd_flag = hr_api.g_varchar2) then
    p_rec.bnf_ctfn_rqd_flag :=
    ben_pbc_shd.g_old_rec.bnf_ctfn_rqd_flag;
  End If;
  If (p_rec.pl_bnf_id = hr_api.g_number) then
    p_rec.pl_bnf_id :=
    ben_pbc_shd.g_old_rec.pl_bnf_id;
  End If;
  If (p_rec.prtt_enrt_actn_id = hr_api.g_number) then
    p_rec.prtt_enrt_actn_id :=
    ben_pbc_shd.g_old_rec.prtt_enrt_actn_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_pbc_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.pbc_attribute_category = hr_api.g_varchar2) then
    p_rec.pbc_attribute_category :=
    ben_pbc_shd.g_old_rec.pbc_attribute_category;
  End If;
  If (p_rec.pbc_attribute1 = hr_api.g_varchar2) then
    p_rec.pbc_attribute1 :=
    ben_pbc_shd.g_old_rec.pbc_attribute1;
  End If;
  If (p_rec.pbc_attribute2 = hr_api.g_varchar2) then
    p_rec.pbc_attribute2 :=
    ben_pbc_shd.g_old_rec.pbc_attribute2;
  End If;
  If (p_rec.pbc_attribute3 = hr_api.g_varchar2) then
    p_rec.pbc_attribute3 :=
    ben_pbc_shd.g_old_rec.pbc_attribute3;
  End If;
  If (p_rec.pbc_attribute4 = hr_api.g_varchar2) then
    p_rec.pbc_attribute4 :=
    ben_pbc_shd.g_old_rec.pbc_attribute4;
  End If;
  If (p_rec.pbc_attribute5 = hr_api.g_varchar2) then
    p_rec.pbc_attribute5 :=
    ben_pbc_shd.g_old_rec.pbc_attribute5;
  End If;
  If (p_rec.pbc_attribute6 = hr_api.g_varchar2) then
    p_rec.pbc_attribute6 :=
    ben_pbc_shd.g_old_rec.pbc_attribute6;
  End If;
  If (p_rec.pbc_attribute7 = hr_api.g_varchar2) then
    p_rec.pbc_attribute7 :=
    ben_pbc_shd.g_old_rec.pbc_attribute7;
  End If;
  If (p_rec.pbc_attribute8 = hr_api.g_varchar2) then
    p_rec.pbc_attribute8 :=
    ben_pbc_shd.g_old_rec.pbc_attribute8;
  End If;
  If (p_rec.pbc_attribute9 = hr_api.g_varchar2) then
    p_rec.pbc_attribute9 :=
    ben_pbc_shd.g_old_rec.pbc_attribute9;
  End If;
  If (p_rec.pbc_attribute10 = hr_api.g_varchar2) then
    p_rec.pbc_attribute10 :=
    ben_pbc_shd.g_old_rec.pbc_attribute10;
  End If;
  If (p_rec.pbc_attribute11 = hr_api.g_varchar2) then
    p_rec.pbc_attribute11 :=
    ben_pbc_shd.g_old_rec.pbc_attribute11;
  End If;
  If (p_rec.pbc_attribute12 = hr_api.g_varchar2) then
    p_rec.pbc_attribute12 :=
    ben_pbc_shd.g_old_rec.pbc_attribute12;
  End If;
  If (p_rec.pbc_attribute13 = hr_api.g_varchar2) then
    p_rec.pbc_attribute13 :=
    ben_pbc_shd.g_old_rec.pbc_attribute13;
  End If;
  If (p_rec.pbc_attribute14 = hr_api.g_varchar2) then
    p_rec.pbc_attribute14 :=
    ben_pbc_shd.g_old_rec.pbc_attribute14;
  End If;
  If (p_rec.pbc_attribute15 = hr_api.g_varchar2) then
    p_rec.pbc_attribute15 :=
    ben_pbc_shd.g_old_rec.pbc_attribute15;
  End If;
  If (p_rec.pbc_attribute16 = hr_api.g_varchar2) then
    p_rec.pbc_attribute16 :=
    ben_pbc_shd.g_old_rec.pbc_attribute16;
  End If;
  If (p_rec.pbc_attribute17 = hr_api.g_varchar2) then
    p_rec.pbc_attribute17 :=
    ben_pbc_shd.g_old_rec.pbc_attribute17;
  End If;
  If (p_rec.pbc_attribute18 = hr_api.g_varchar2) then
    p_rec.pbc_attribute18 :=
    ben_pbc_shd.g_old_rec.pbc_attribute18;
  End If;
  If (p_rec.pbc_attribute19 = hr_api.g_varchar2) then
    p_rec.pbc_attribute19 :=
    ben_pbc_shd.g_old_rec.pbc_attribute19;
  End If;
  If (p_rec.pbc_attribute20 = hr_api.g_varchar2) then
    p_rec.pbc_attribute20 :=
    ben_pbc_shd.g_old_rec.pbc_attribute20;
  End If;
  If (p_rec.pbc_attribute21 = hr_api.g_varchar2) then
    p_rec.pbc_attribute21 :=
    ben_pbc_shd.g_old_rec.pbc_attribute21;
  End If;
  If (p_rec.pbc_attribute22 = hr_api.g_varchar2) then
    p_rec.pbc_attribute22 :=
    ben_pbc_shd.g_old_rec.pbc_attribute22;
  End If;
  If (p_rec.pbc_attribute23 = hr_api.g_varchar2) then
    p_rec.pbc_attribute23 :=
    ben_pbc_shd.g_old_rec.pbc_attribute23;
  End If;
  If (p_rec.pbc_attribute24 = hr_api.g_varchar2) then
    p_rec.pbc_attribute24 :=
    ben_pbc_shd.g_old_rec.pbc_attribute24;
  End If;
  If (p_rec.pbc_attribute25 = hr_api.g_varchar2) then
    p_rec.pbc_attribute25 :=
    ben_pbc_shd.g_old_rec.pbc_attribute25;
  End If;
  If (p_rec.pbc_attribute26 = hr_api.g_varchar2) then
    p_rec.pbc_attribute26 :=
    ben_pbc_shd.g_old_rec.pbc_attribute26;
  End If;
  If (p_rec.pbc_attribute27 = hr_api.g_varchar2) then
    p_rec.pbc_attribute27 :=
    ben_pbc_shd.g_old_rec.pbc_attribute27;
  End If;
  If (p_rec.pbc_attribute28 = hr_api.g_varchar2) then
    p_rec.pbc_attribute28 :=
    ben_pbc_shd.g_old_rec.pbc_attribute28;
  End If;
  If (p_rec.pbc_attribute29 = hr_api.g_varchar2) then
    p_rec.pbc_attribute29 :=
    ben_pbc_shd.g_old_rec.pbc_attribute29;
  End If;
  If (p_rec.pbc_attribute30 = hr_api.g_varchar2) then
    p_rec.pbc_attribute30 :=
    ben_pbc_shd.g_old_rec.pbc_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_pbc_shd.convert_args
  (
  p_pl_bnf_ctfn_prvdd_id,
  null,
  null,
  p_bnf_ctfn_typ_cd,
  p_bnf_ctfn_recd_dt,
