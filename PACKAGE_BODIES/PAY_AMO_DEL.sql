--------------------------------------------------------
--  DDL for Package Body PAY_AMO_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_AMO_DEL" as
...
Begin
  ...
  --
  -- Delete the pay_au_modules row.
  --
  delete from pay_au_modules
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_delete(p_rec in pay_amo_shd.g_rec_type) is
--
  l_proc  varchar2(72);
--
Begin
...
-- {End Of Comments}
-- -----------------------------------------------------------------------------
Procedure post_delete(p_rec in pay_amo_shd.g_rec_type) is
--
  l_proc  varchar2(72);
--
Begin
...
Begin
  ...
  --
  pay_amo_bus.delete_validate(p_rec);
  --
  ...
  --
  pay_amo_del.pre_delete(p_rec);
  --
  -- Delete the row.
  --
  pay_amo_del.delete_dml(p_rec);
  --
  -- Call the supporting post-delete operation
  --
  pay_amo_del.post_delete(p_rec);
  --
  -- Call to raise any errors on multi-message list
  hr_multi_message.end_validation_set;
  --
...
Begin
  ...
  -- business process
  --
  pay_amo_del.del(l_rec);
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End del;
--
begin
  g_package := '  pay_amo_del.';  -- Global package name
end pay_amo_del;
