--------------------------------------------------------
--  DDL for Package Body GR_LABEL_CLASSES_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_LABEL_CLASSES_B_PKG" AS
/*$Header: GRHILCBB.pls 115.2 2002/10/28 16:02:41 methomas ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_label_classes_b
   		  	     (label_class_code,
				  form_block,
				  rollup_type,
				  rollup_property,
...
/*	  Now call the check foreign key procedure */
   ...
   ELSE
	  ...
	  SET	 label_class_code               = p_label_class_code,
			 form_block	        = p_form_block,
			 rollup_type		= p_rollup_type,
		         rollup_property	= p_rollup_property,
			 rollup_label		= p_rollup_label,
...
CURSOR c_lock_label_class
 IS
   SELECT	*
   FROM		gr_label_classes_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockLabelClassRcd	  c_lock_label_class%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_label_class;
   FETCH c_lock_label_class INTO LockLabelClassRcd;
   IF c_lock_label_class%NOTFOUND THEN
	  CLOSE c_lock_label_class;
...
/*		Rollup property id */
   IF p_rollup_property IS NOT NULL THEN
     l_key_exists := 'T';
...
/*		Rollup Label */
   ...
   IF p_rollup_label IS NOT NULL THEN
      l_key_exists := 'T';
...
CURSOR c_get_label_codes
 IS
   ...
   WHERE	lab.label_class_code = p_label_class_code;
...
/*
** 	Now read the cursors to make sure the label class code isn't used.
**
**  Label codes
*/
   l_record_count := 0;
   l_code_block := 'c_get_label_codes';
   OPEN c_get_label_codes;
   FETCH c_get_label_codes INTO l_record_count;
...
CURSOR c_get_label_class_rowid
 IS
   SELECT lcb.rowid
   FROM	  gr_label_classes_b lcb
   WHERE  lcb.label_class_code = p_label_class_code;
LabelClassRecord			   c_get_label_class_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   l_msg_data := p_label_class_code;
   OPEN c_get_label_class_rowid;
   FETCH c_get_label_class_rowid INTO LabelClassRecord;
   IF c_get_label_class_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := LabelClassRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_label_class_rowid;
