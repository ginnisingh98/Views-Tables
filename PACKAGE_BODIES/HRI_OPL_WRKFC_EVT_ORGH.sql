--------------------------------------------------------
--  DDL for Package Body HRI_OPL_WRKFC_EVT_ORGH
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HRI_OPL_WRKFC_EVT_ORGH" AS
/* $Header: hriowevtorg.pkb 120.5.12000000.2 2007/04/12 13:23:46 smohapat noship $ */
  ...
  g_tab_sup_organztn_fk             g_number_tab_type;
    ...
    l_dbi_collection_start_date :=
           hri_oltp_conc_param.get_date_parameter_value
            (p_parameter_name     => 'FULL_REFRESH_FROM_DATE',
             p_process_table_name => 'HRI_MDS_WRKFC_ORGH_C01_CT');
...
BEGIN
  ...
  g_tab_sup_organztn_fk(g_row_count) := p_sup_organztn_fk;
...
-- ----------------------------------------------------------------------------
PROCEDURE process_snapshot(p_organization_fk  IN NUMBER,
  ...
  l_month_id      NUMBER;
  l_sup_organztn_fk          g_number_tab_type;
...
BEGIN
  ...
  IF l_sup_organztn_fk.EXISTS(1) THEN
    FOR i IN 1..l_sup_organztn_fk.LAST LOOP
      insert_row
...
BEGIN
  ...
  FOR org_snap IN work_csr LOOP

    process_snapshot
     (p_organization_fk => org_snap.organztn_fk,
      p_snapshot_date   => org_snap.snapshot_date);
...
BEGIN
  ...
  -- Disable WHO trigger
  run_sql_stmt_noerr('ALTER TRIGGER HRI_MDS_WRKFC_ORGH_C01_CT_WHO DISABLE');
  ...
  IF (g_full_refresh = 'Y' OR
    ...
    l_sql_stmt := 'TRUNCATE TABLE ' || l_schema || '.HRI_MDS_WRKFC_ORGH_C01_CT';
    ...
    ELSE
      ...
      hri_utl_ddl.log_and_drop_indexes
       (p_application_short_name => 'HRI',
        p_table_name             => 'HRI_MDS_WRKFC_ORGH_C01_CT',
        p_table_owner            => l_schema,
        p_index_excptn_lst       => 'HRI_MDS_WRKFC_ORGH_C01_CT_N1');
...
BEGIN
  ...
  IF (p_mthd_action_id > -1) THEN
    ...
    -- Log process end
    hri_bpl_conc_log.record_process_start('HRI_MDS_WRKFC_ORGH_C01_CT');
  ...
  -- Enable WHO trigger
  run_sql_stmt_noerr('ALTER TRIGGER HRI_MDS_WRKFC_ORGH_C01_CT_WHO ENABLE');
  ...
  -- Empty out workforce organization summary event queue
  l_sql_stmt := 'TRUNCATE TABLE ' || l_schema || '.HRI_EQ_WRKFC_EVT_ORGH';
  EXECUTE IMMEDIATE(l_sql_stmt);
...
BEGIN
  ...
  IF g_full_refresh = 'Y' THEN
    FOR chunk_rec IN chunk_csr LOOP
      g_row_count := 0;
      process_range_full(chunk_rec.start_object_id, chunk_rec.end_object_id, chunk_rec.object_lvl);
      IF g_row_count > 0 THEN
        bulk_insert_rows;
      END IF;
    END LOOP;
  ELSE
    FOR chunk_rec IN chunk_csr_incr LOOP
      g_row_count := 0;
      process_range_incr(chunk_rec.start_object_id, chunk_rec.end_object_id, chunk_rec.object_lvl);
      IF g_row_count > 0 THEN
        bulk_insert_rows;
      END IF;
    END LOOP;
