--------------------------------------------------------
--  DDL for Package Body PQP_PVD_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQP_PVD_UPD" as
/* $Header: pqpvdrhi.pkb 115.6 2003/02/17 22:14:43 tmehra noship $ */
--
-- ----------------------------------------------------------------------------
-- |                     Private Global Definitions                           |
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqp_pvd_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
-- {Start Of Comments}
--
-- Description:
--   This procedure controls the actual dml update logic. The processing of
--   this procedure is:
--   1) Increment the object_version_number by 1 if the object_version_number
--      is defined as an attribute for this entity.
--   2) To set and unset the g_api_dml status as required (as we are about to
--      perform dml).
--   3) To update the specified row in the schema using the primary key in
--      the predicates.
--   4) To trap any constraint violations that may have occurred.
--   5) To raise any other errors.
--
-- Prerequisites:
--   This is an internal private procedure which must be called from the upd
--   procedure.
--
-- In Parameters:
--   A Pl/Sql record structre.
--
-- Post Success:
--   The specified row will be updated in the schema.
--
-- Post Failure:
--   On the update dml failure it is important to note that we always reset the
--   g_api_dml status to false.
--   If a check, unique or parent integrity constraint violation is raised the
--   constraint_error procedure will be called.
--   If any other error is reported, the error will be raised after the
--   g_api_dml status is reset.
--
-- Developer Implementation Notes:
--   The update 'set' attribute list should be modified if any of your
--   attributes are not updateable.
--
-- Access Status:
--   Internal Row Handler Use Only.
--
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml
  (p_rec in out nocopy pqp_pvd_shd.g_rec_type
  ) is
--
  l_proc  varchar2(72) := g_package||'update_dml';
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  -- Increment the object version
  p_rec.object_version_number := p_rec.object_version_number + 1;
  --
  pqp_pvd_shd.g_api_dml := true;  -- Set the api dml status
  --
  -- Update the pqp_vehicle_details Row
  --
  update pqp_vehicle_details
    set
     vehicle_details_id              = p_rec.vehicle_details_id
    ,vehicle_type                    = p_rec.vehicle_type
    ,business_group_id               = p_rec.business_group_id
    ,registration_number             = p_rec.registration_number
    ,make                            = p_rec.make
    ,model                           = p_rec.model
    ,date_first_registered           = p_rec.date_first_registered
    ,engine_capacity_in_cc           = p_rec.engine_capacity_in_cc
    ,fuel_type                       = p_rec.fuel_type
    ,fuel_card                       = p_rec.fuel_card
    ,currency_code                   = p_rec.currency_code
    ,list_price                      = p_rec.list_price
    ,accessory_value_at_startdate    = p_rec.accessory_value_at_startdate
    ,accessory_value_added_later     = p_rec.accessory_value_added_later
--    ,capital_contributions           = p_rec.capital_contributions
--    ,private_use_contributions       = p_rec.private_use_contributions
    ,market_value_classic_car        = p_rec.market_value_classic_car
    ,co2_emissions                   = p_rec.co2_emissions
    ,vehicle_provider                = p_rec.vehicle_provider
    ,object_version_number           = p_rec.object_version_number
    ,vehicle_identification_number   = p_rec.vehicle_identification_number
    ,vehicle_ownership               = p_rec.vehicle_ownership
    ,vhd_attribute_category          = p_rec.vhd_attribute_category
    ,vhd_attribute1                  = p_rec.vhd_attribute1
    ,vhd_attribute2                  = p_rec.vhd_attribute2
    ,vhd_attribute3                  = p_rec.vhd_attribute3
    ,vhd_attribute4                  = p_rec.vhd_attribute4
    ,vhd_attribute5                  = p_rec.vhd_attribute5
    ,vhd_attribute6                  = p_rec.vhd_attribute6
    ,vhd_attribute7                  = p_rec.vhd_attribute7
    ,vhd_attribute8                  = p_rec.vhd_attribute8
    ,vhd_attribute9                  = p_rec.vhd_attribute9
    ,vhd_attribute10                 = p_rec.vhd_attribute10
    ,vhd_attribute11                 = p_rec.vhd_attribute11
    ,vhd_attribute12                 = p_rec.vhd_attribute12
    ,vhd_attribute13                 = p_rec.vhd_attribute13
    ,vhd_attribute14                 = p_rec.vhd_attribute14
    ,vhd_attribute15                 = p_rec.vhd_attribute15
    ,vhd_attribute16                 = p_rec.vhd_attribute16
    ,vhd_attribute17                 = p_rec.vhd_attribute17
    ,vhd_attribute18                 = p_rec.vhd_attribute18
    ,vhd_attribute19                 = p_rec.vhd_attribute19
    ,vhd_attribute20                 = p_rec.vhd_attribute20
    ,vhd_information_category        = p_rec.vhd_information_category
    ,vhd_information1                = p_rec.vhd_information1
    ,vhd_information2                = p_rec.vhd_information2
    ,vhd_information3                = p_rec.vhd_information3
    ,vhd_information4                = p_rec.vhd_information4
    ,vhd_information5                = p_rec.vhd_information5
    ,vhd_information6                = p_rec.vhd_information6
    ,vhd_information7                = p_rec.vhd_information7
    ,vhd_information8                = p_rec.vhd_information8
    ,vhd_information9                = p_rec.vhd_information9
    ,vhd_information10               = p_rec.vhd_information10
    ,vhd_information11               = p_rec.vhd_information11
    ,vhd_information12               = p_rec.vhd_information12
    ,vhd_information13               = p_rec.vhd_information13
    ,vhd_information14               = p_rec.vhd_information14
    ,vhd_information15               = p_rec.vhd_information15
    ,vhd_information16               = p_rec.vhd_information16
    ,vhd_information17               = p_rec.vhd_information17
    ,vhd_information18               = p_rec.vhd_information18
    ,vhd_information19               = p_rec.vhd_information19
    ,vhd_information20               = p_rec.vhd_information20
    where vehicle_details_id = p_rec.vehicle_details_id;
  --
  pqp_pvd_shd.g_api_dml := false;   -- Unset the api dml status
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
Exception
  When hr_api.check_integrity_violated Then
    -- A check constraint has been violated
    pqp_pvd_shd.g_api_dml := false;   -- Unset the api dml status
    pqp_pvd_shd.constraint_error
      (p_constraint_name => hr_api.strip_constraint_name(SQLERRM));
  When hr_api.parent_integrity_violated Then
    -- Parent integrity has been violated
    pqp_pvd_shd.g_api_dml := false;   -- Unset the api dml status
    pqp_pvd_shd.constraint_error
      (p_constraint_name => hr_api.strip_constraint_name(SQLERRM));
  When hr_api.unique_integrity_violated Then
    -- Unique integrity has been violated
    pqp_pvd_shd.g_api_dml := false;   -- Unset the api dml status
    pqp_pvd_shd.constraint_error
      (p_constraint_name => hr_api.strip_constraint_name(SQLERRM));
  When Others Then
    pqp_pvd_shd.g_api_dml := false;   -- Unset the api dml status
    Raise;
End update_dml;
--
-- ----------------------------------------------------------------------------
-- |------------------------------< pre_update >------------------------------|
-- ----------------------------------------------------------------------------
-- {Start Of Comments}
--
-- Description:
--   This private procedure contains any processing which is required before
--   the update dml.
--
-- Prerequisites:
--   This is an internal procedure which is called from the upd procedure.
--
-- In Parameters:
--   A Pl/Sql record structure.
--
-- Post Success:
--   Processing continues.
--
-- Post Failure:
--   If an error has occurred, an error message and exception wil be raised
--   but not handled.
--
-- Developer Implementation Notes:
--   Any pre-processing required before the update dml is issued should be
--   coded within this procedure. It is important to note that any 3rd party
--   maintenance should be reviewed before placing in this procedure.
--
-- Access Status:
--   Internal Row Handler Use Only.
--
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update
  (p_rec in pqp_pvd_shd.g_rec_type
  ) is
--
  l_proc  varchar2(72) := g_package||'pre_update';
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_update;
--
-- ----------------------------------------------------------------------------
-- |-----------------------------< post_update >------------------------------|
-- ----------------------------------------------------------------------------
-- {Start Of Comments}
--
-- Description:
--   This private procedure contains any processing which is required after the
--   update dml.
--
-- Prerequisites:
--   This is an internal procedure which is called from the upd procedure.
--
-- In Parameters:
--   A Pl/Sql record structure.
--
-- Post Success:
--   Processing continues.
--
-- Post Failure:
--   If an error has occurred, an error message and exception will be raised
--   but not handled.
--
-- Developer Implementation Notes:
--   Any post-processing required after the update dml is issued should be
--   coded within this procedure. It is important to note that any 3rd party
--   maintenance should be reviewed before placing in this procedure.
--
-- Access Status:
--   Internal Row Handler Use Only.
--
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure post_update
  (p_effective_date               in date
  ,p_rec                          in pqp_pvd_shd.g_rec_type
  ) is
--
  l_proc  varchar2(72) := g_package||'post_update';
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  begin
    --
    pqp_pvd_rku.after_update
      (p_effective_date              => p_effective_date
      ,p_vehicle_details_id
      => p_rec.vehicle_details_id
      ,p_vehicle_type
      => p_rec.vehicle_type
      ,p_business_group_id
      => p_rec.business_group_id
      ,p_registration_number
      => p_rec.registration_number
      ,p_make
      => p_rec.make
      ,p_model
      => p_rec.model
      ,p_date_first_registered
      => p_rec.date_first_registered
      ,p_engine_capacity_in_cc
      => p_rec.engine_capacity_in_cc
      ,p_fuel_type
      => p_rec.fuel_type
      ,p_fuel_card
      => p_rec.fuel_card
      ,p_currency_code
      => p_rec.currency_code
      ,p_list_price
      => p_rec.list_price
      ,p_accessory_value_at_startdate
      => p_rec.accessory_value_at_startdate
      ,p_accessory_value_added_later
      => p_rec.accessory_value_added_later
--      ,p_capital_contributions
--      => p_rec.capital_contributions
--      ,p_private_use_contributions
--      => p_rec.private_use_contributions
      ,p_market_value_classic_car
      => p_rec.market_value_classic_car
      ,p_co2_emissions
      => p_rec.co2_emissions
      ,p_vehicle_provider
      => p_rec.vehicle_provider
      ,p_object_version_number
      => p_rec.object_version_number
      ,p_vehicle_identification_numbe
      => p_rec.vehicle_identification_number
      ,p_vehicle_ownership
      => p_rec.vehicle_ownership
      ,p_vhd_attribute_category
      => p_rec.vhd_attribute_category
      ,p_vhd_attribute1
      => p_rec.vhd_attribute1
      ,p_vhd_attribute2
      => p_rec.vhd_attribute2
      ,p_vhd_attribute3
      => p_rec.vhd_attribute3
      ,p_vhd_attribute4
      => p_rec.vhd_attribute4
      ,p_vhd_attribute5
      => p_rec.vhd_attribute5
      ,p_vhd_attribute6
      => p_rec.vhd_attribute6
      ,p_vhd_attribute7
      => p_rec.vhd_attribute7
      ,p_vhd_attribute8
      => p_rec.vhd_attribute8
      ,p_vhd_attribute9
      => p_rec.vhd_attribute9
      ,p_vhd_attribute10
      => p_rec.vhd_attribute10
      ,p_vhd_attribute11
      => p_rec.vhd_attribute11
      ,p_vhd_attribute12
      => p_rec.vhd_attribute12
      ,p_vhd_attribute13
      => p_rec.vhd_attribute13
      ,p_vhd_attribute14
      => p_rec.vhd_attribute14
      ,p_vhd_attribute15
      => p_rec.vhd_attribute15
      ,p_vhd_attribute16
      => p_rec.vhd_attribute16
      ,p_vhd_attribute17
      => p_rec.vhd_attribute17
      ,p_vhd_attribute18
      => p_rec.vhd_attribute18
      ,p_vhd_attribute19
      => p_rec.vhd_attribute19
      ,p_vhd_attribute20
      => p_rec.vhd_attribute20
      ,p_vhd_information_category
      => p_rec.vhd_information_category
      ,p_vhd_information1
      => p_rec.vhd_information1
      ,p_vhd_information2
      => p_rec.vhd_information2
      ,p_vhd_information3
      => p_rec.vhd_information3
      ,p_vhd_information4
      => p_rec.vhd_information4
      ,p_vhd_information5
      => p_rec.vhd_information5
      ,p_vhd_information6
      => p_rec.vhd_information6
      ,p_vhd_information7
      => p_rec.vhd_information7
      ,p_vhd_information8
      => p_rec.vhd_information8
      ,p_vhd_information9
      => p_rec.vhd_information9
      ,p_vhd_information10
      => p_rec.vhd_information10
      ,p_vhd_information11
      => p_rec.vhd_information11
      ,p_vhd_information12
      => p_rec.vhd_information12
      ,p_vhd_information13
      => p_rec.vhd_information13
      ,p_vhd_information14
      => p_rec.vhd_information14
      ,p_vhd_information15
      => p_rec.vhd_information15
      ,p_vhd_information16
      => p_rec.vhd_information16
      ,p_vhd_information17
      => p_rec.vhd_information17
      ,p_vhd_information18
      => p_rec.vhd_information18
      ,p_vhd_information19
      => p_rec.vhd_information19
      ,p_vhd_information20
      => p_rec.vhd_information20
      ,p_vehicle_type_o
      => pqp_pvd_shd.g_old_rec.vehicle_type
      ,p_business_group_id_o
      => pqp_pvd_shd.g_old_rec.business_group_id
      ,p_registration_number_o
      => pqp_pvd_shd.g_old_rec.registration_number
      ,p_make_o
      => pqp_pvd_shd.g_old_rec.make
      ,p_model_o
      => pqp_pvd_shd.g_old_rec.model
      ,p_date_first_registered_o
      => pqp_pvd_shd.g_old_rec.date_first_registered
      ,p_engine_capacity_in_cc_o
      => pqp_pvd_shd.g_old_rec.engine_capacity_in_cc
      ,p_fuel_type_o
      => pqp_pvd_shd.g_old_rec.fuel_type
      ,p_fuel_card_o
      => pqp_pvd_shd.g_old_rec.fuel_card
      ,p_currency_code_o
      => pqp_pvd_shd.g_old_rec.currency_code
      ,p_list_price_o
      => pqp_pvd_shd.g_old_rec.list_price
      ,p_accessory_value_at_startda_o
      => pqp_pvd_shd.g_old_rec.accessory_value_at_startdate
      ,p_accessory_value_added_late_o
      => pqp_pvd_shd.g_old_rec.accessory_value_added_later
--      ,p_capital_contributions_o
--      => pqp_pvd_shd.g_old_rec.capital_contributions
--      ,p_private_use_contributions_o
--      => pqp_pvd_shd.g_old_rec.private_use_contributions
      ,p_market_value_classic_car_o
      => pqp_pvd_shd.g_old_rec.market_value_classic_car
      ,p_co2_emissions_o
      => pqp_pvd_shd.g_old_rec.co2_emissions
      ,p_vehicle_provider_o
      => pqp_pvd_shd.g_old_rec.vehicle_provider
      ,p_object_version_number_o
      => pqp_pvd_shd.g_old_rec.object_version_number
      ,p_vehicle_identification_num_o
      => pqp_pvd_shd.g_old_rec.vehicle_identification_number
      ,p_vehicle_ownership_o
      => pqp_pvd_shd.g_old_rec.vehicle_ownership
      ,p_vhd_attribute_category_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute_category
      ,p_vhd_attribute1_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute1
      ,p_vhd_attribute2_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute2
      ,p_vhd_attribute3_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute3
      ,p_vhd_attribute4_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute4
      ,p_vhd_attribute5_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute5
      ,p_vhd_attribute6_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute6
      ,p_vhd_attribute7_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute7
      ,p_vhd_attribute8_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute8
      ,p_vhd_attribute9_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute9
      ,p_vhd_attribute10_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute10
      ,p_vhd_attribute11_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute11
      ,p_vhd_attribute12_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute12
      ,p_vhd_attribute13_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute13
      ,p_vhd_attribute14_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute14
      ,p_vhd_attribute15_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute15
      ,p_vhd_attribute16_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute16
      ,p_vhd_attribute17_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute17
      ,p_vhd_attribute18_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute18
      ,p_vhd_attribute19_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute19
      ,p_vhd_attribute20_o
      => pqp_pvd_shd.g_old_rec.vhd_attribute20
      ,p_vhd_information_category_o
      => pqp_pvd_shd.g_old_rec.vhd_information_category
      ,p_vhd_information1_o
      => pqp_pvd_shd.g_old_rec.vhd_information1
      ,p_vhd_information2_o
      => pqp_pvd_shd.g_old_rec.vhd_information2
      ,p_vhd_information3_o
      => pqp_pvd_shd.g_old_rec.vhd_information3
      ,p_vhd_information4_o
      => pqp_pvd_shd.g_old_rec.vhd_information4
      ,p_vhd_information5_o
      => pqp_pvd_shd.g_old_rec.vhd_information5
      ,p_vhd_information6_o
      => pqp_pvd_shd.g_old_rec.vhd_information6
      ,p_vhd_information7_o
      => pqp_pvd_shd.g_old_rec.vhd_information7
      ,p_vhd_information8_o
      => pqp_pvd_shd.g_old_rec.vhd_information8
      ,p_vhd_information9_o
      => pqp_pvd_shd.g_old_rec.vhd_information9
      ,p_vhd_information10_o
      => pqp_pvd_shd.g_old_rec.vhd_information10
      ,p_vhd_information11_o
      => pqp_pvd_shd.g_old_rec.vhd_information11
      ,p_vhd_information12_o
      => pqp_pvd_shd.g_old_rec.vhd_information12
      ,p_vhd_information13_o
      => pqp_pvd_shd.g_old_rec.vhd_information13
      ,p_vhd_information14_o
      => pqp_pvd_shd.g_old_rec.vhd_information14
      ,p_vhd_information15_o
      => pqp_pvd_shd.g_old_rec.vhd_information15
      ,p_vhd_information16_o
      => pqp_pvd_shd.g_old_rec.vhd_information16
      ,p_vhd_information17_o
      => pqp_pvd_shd.g_old_rec.vhd_information17
      ,p_vhd_information18_o
      => pqp_pvd_shd.g_old_rec.vhd_information18
      ,p_vhd_information19_o
      => pqp_pvd_shd.g_old_rec.vhd_information19
      ,p_vhd_information20_o
      => pqp_pvd_shd.g_old_rec.vhd_information20
      );
    --
  exception
    --
    when hr_api.cannot_find_prog_unit then
      --
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'PQP_VEHICLE_DETAILS'
        ,p_hook_type   => 'AU');
      --
  end;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End post_update;
--
-- ----------------------------------------------------------------------------
-- |-----------------------------< convert_defs >-----------------------------|
-- ----------------------------------------------------------------------------
-- {Start Of Comments}
--
-- Description:
--   The Convert_Defs procedure has one very important function:
--   It must return the record structure for the row with all system defaulted
--   values converted into its corresponding parameter value for update. When
--   we attempt to update a row through the Upd process , certain
--   parameters can be defaulted which enables flexibility in the calling of
--   the upd process (e.g. only attributes which need to be updated need to be
--   specified). For the upd process to determine which attributes
--   have NOT been specified we need to check if the parameter has a reserved
--   system default value. Therefore, for all parameters which have a
--   corresponding reserved system default mechanism specified we need to
--   check if a system default is being used. If a system default is being
--   used then we convert the defaulted value into its corresponding attribute
--   value held in the g_old_rec data structure.
--
-- Prerequisites:
--   This private function can only be called from the upd process.
--
-- In Parameters:
--   A Pl/Sql record structure.
--
-- Post Success:
--   The record structure will be returned with all system defaulted parameter
--   values converted into its current row attribute value.
--
-- Post Failure:
--   No direct error handling is required within this function. Any possible
--   errors within this procedure will be a PL/SQL value error due to
--   conversion of datatypes or data lengths.
--
-- Developer Implementation Notes:
--   None.
--
-- Access Status:
--   Internal Row Handler Use Only.
--
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs
  (p_rec in out nocopy pqp_pvd_shd.g_rec_type
  ) is
--
Begin
  --
  -- We must now examine each argument value in the
  -- p_rec plsql record structure
  -- to see if a system default is being used. If a system default
  -- is being used then we must set to the 'current' argument value.
  --
  If (p_rec.vehicle_type = hr_api.g_varchar2) then
    p_rec.vehicle_type :=
    pqp_pvd_shd.g_old_rec.vehicle_type;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    pqp_pvd_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.registration_number = hr_api.g_varchar2) then
    p_rec.registration_number :=
    pqp_pvd_shd.g_old_rec.registration_number;
  End If;
  If (p_rec.make = hr_api.g_varchar2) then
    p_rec.make :=
    pqp_pvd_shd.g_old_rec.make;
  End If;
  If (p_rec.model = hr_api.g_varchar2) then
    p_rec.model :=
    pqp_pvd_shd.g_old_rec.model;
  End If;
  If (p_rec.date_first_registered = hr_api.g_date) then
    p_rec.date_first_registered :=
    pqp_pvd_shd.g_old_rec.date_first_registered;
  End If;
  If (p_rec.engine_capacity_in_cc = hr_api.g_number) then
    p_rec.engine_capacity_in_cc :=
    pqp_pvd_shd.g_old_rec.engine_capacity_in_cc;
  End If;
  If (p_rec.fuel_type = hr_api.g_varchar2) then
    p_rec.fuel_type :=
    pqp_pvd_shd.g_old_rec.fuel_type;
  End If;
  If (p_rec.fuel_card = hr_api.g_varchar2) then
    p_rec.fuel_card :=
    pqp_pvd_shd.g_old_rec.fuel_card;
  End If;
  If (p_rec.currency_code = hr_api.g_varchar2) then
    p_rec.currency_code :=
    pqp_pvd_shd.g_old_rec.currency_code;
  End If;
  If (p_rec.list_price = hr_api.g_number) then
    p_rec.list_price :=
    pqp_pvd_shd.g_old_rec.list_price;
  End If;
  If (p_rec.accessory_value_at_startdate = hr_api.g_number) then
    p_rec.accessory_value_at_startdate :=
    pqp_pvd_shd.g_old_rec.accessory_value_at_startdate;
  End If;
  If (p_rec.accessory_value_added_later = hr_api.g_number) then
    p_rec.accessory_value_added_later :=
    pqp_pvd_shd.g_old_rec.accessory_value_added_later;
  End If;
--  If (p_rec.capital_contributions = hr_api.g_number) then
--    p_rec.capital_contributions :=
--    pqp_pvd_shd.g_old_rec.capital_contributions;
--  End If;
--  If (p_rec.private_use_contributions = hr_api.g_number) then
--    p_rec.private_use_contributions :=
--    pqp_pvd_shd.g_old_rec.private_use_contributions;
--  End If;
  If (p_rec.market_value_classic_car = hr_api.g_number) then
    p_rec.market_value_classic_car :=
    pqp_pvd_shd.g_old_rec.market_value_classic_car;
  End If;
  If (p_rec.co2_emissions = hr_api.g_number) then
    p_rec.co2_emissions :=
    pqp_pvd_shd.g_old_rec.co2_emissions;
  End If;
  If (p_rec.vehicle_provider = hr_api.g_varchar2) then
    p_rec.vehicle_provider :=
    pqp_pvd_shd.g_old_rec.vehicle_provider;
  End If;
  If (p_rec.vehicle_identification_number = hr_api.g_varchar2) then
    p_rec.vehicle_identification_number :=
    pqp_pvd_shd.g_old_rec.vehicle_identification_number;
  End If;
  If (p_rec.vehicle_ownership = hr_api.g_varchar2) then
    p_rec.vehicle_ownership :=
    pqp_pvd_shd.g_old_rec.vehicle_ownership;
  End If;
  If (p_rec.vhd_attribute_category = hr_api.g_varchar2) then
    p_rec.vhd_attribute_category :=
    pqp_pvd_shd.g_old_rec.vhd_attribute_category;
  End If;
  If (p_rec.vhd_attribute1 = hr_api.g_varchar2) then
    p_rec.vhd_attribute1 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute1;
  End If;
  If (p_rec.vhd_attribute2 = hr_api.g_varchar2) then
    p_rec.vhd_attribute2 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute2;
  End If;
  If (p_rec.vhd_attribute3 = hr_api.g_varchar2) then
    p_rec.vhd_attribute3 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute3;
  End If;
  If (p_rec.vhd_attribute4 = hr_api.g_varchar2) then
    p_rec.vhd_attribute4 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute4;
  End If;
  If (p_rec.vhd_attribute5 = hr_api.g_varchar2) then
    p_rec.vhd_attribute5 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute5;
  End If;
  If (p_rec.vhd_attribute6 = hr_api.g_varchar2) then
    p_rec.vhd_attribute6 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute6;
  End If;
  If (p_rec.vhd_attribute7 = hr_api.g_varchar2) then
    p_rec.vhd_attribute7 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute7;
  End If;
  If (p_rec.vhd_attribute8 = hr_api.g_varchar2) then
    p_rec.vhd_attribute8 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute8;
  End If;
  If (p_rec.vhd_attribute9 = hr_api.g_varchar2) then
    p_rec.vhd_attribute9 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute9;
  End If;
  If (p_rec.vhd_attribute10 = hr_api.g_varchar2) then
    p_rec.vhd_attribute10 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute10;
  End If;
  If (p_rec.vhd_attribute11 = hr_api.g_varchar2) then
    p_rec.vhd_attribute11 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute11;
  End If;
  If (p_rec.vhd_attribute12 = hr_api.g_varchar2) then
    p_rec.vhd_attribute12 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute12;
  End If;
  If (p_rec.vhd_attribute13 = hr_api.g_varchar2) then
    p_rec.vhd_attribute13 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute13;
  End If;
  If (p_rec.vhd_attribute14 = hr_api.g_varchar2) then
    p_rec.vhd_attribute14 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute14;
  End If;
  If (p_rec.vhd_attribute15 = hr_api.g_varchar2) then
    p_rec.vhd_attribute15 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute15;
  End If;
  If (p_rec.vhd_attribute16 = hr_api.g_varchar2) then
    p_rec.vhd_attribute16 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute16;
  End If;
  If (p_rec.vhd_attribute17 = hr_api.g_varchar2) then
    p_rec.vhd_attribute17 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute17;
  End If;
  If (p_rec.vhd_attribute18 = hr_api.g_varchar2) then
    p_rec.vhd_attribute18 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute18;
  End If;
  If (p_rec.vhd_attribute19 = hr_api.g_varchar2) then
    p_rec.vhd_attribute19 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute19;
  End If;
  If (p_rec.vhd_attribute20 = hr_api.g_varchar2) then
    p_rec.vhd_attribute20 :=
    pqp_pvd_shd.g_old_rec.vhd_attribute20;
  End If;
  If (p_rec.vhd_information_category = hr_api.g_varchar2) then
    p_rec.vhd_information_category :=
    pqp_pvd_shd.g_old_rec.vhd_information_category;
  End If;
  If (p_rec.vhd_information1 = hr_api.g_varchar2) then
    p_rec.vhd_information1 :=
    pqp_pvd_shd.g_old_rec.vhd_information1;
  End If;
  If (p_rec.vhd_information2 = hr_api.g_varchar2) then
    p_rec.vhd_information2 :=
    pqp_pvd_shd.g_old_rec.vhd_information2;
  End If;
  If (p_rec.vhd_information3 = hr_api.g_varchar2) then
    p_rec.vhd_information3 :=
    pqp_pvd_shd.g_old_rec.vhd_information3;
  End If;
  If (p_rec.vhd_information4 = hr_api.g_varchar2) then
    p_rec.vhd_information4 :=
    pqp_pvd_shd.g_old_rec.vhd_information4;
  End If;
  If (p_rec.vhd_information5 = hr_api.g_varchar2) then
    p_rec.vhd_information5 :=
    pqp_pvd_shd.g_old_rec.vhd_information5;
  End If;
  If (p_rec.vhd_information6 = hr_api.g_varchar2) then
    p_rec.vhd_information6 :=
    pqp_pvd_shd.g_old_rec.vhd_information6;
  End If;
  If (p_rec.vhd_information7 = hr_api.g_varchar2) then
    p_rec.vhd_information7 :=
    pqp_pvd_shd.g_old_rec.vhd_information7;
  End If;
  If (p_rec.vhd_information8 = hr_api.g_varchar2) then
    p_rec.vhd_information8 :=
    pqp_pvd_shd.g_old_rec.vhd_information8;
  End If;
  If (p_rec.vhd_information9 = hr_api.g_varchar2) then
    p_rec.vhd_information9 :=
    pqp_pvd_shd.g_old_rec.vhd_information9;
  End If;
  If (p_rec.vhd_information10 = hr_api.g_varchar2) then
    p_rec.vhd_information10 :=
    pqp_pvd_shd.g_old_rec.vhd_information10;
  End If;
  If (p_rec.vhd_information11 = hr_api.g_varchar2) then
    p_rec.vhd_information11 :=
    pqp_pvd_shd.g_old_rec.vhd_information11;
  End If;
  If (p_rec.vhd_information12 = hr_api.g_varchar2) then
    p_rec.vhd_information12 :=
    pqp_pvd_shd.g_old_rec.vhd_information12;
  End If;
  If (p_rec.vhd_information13 = hr_api.g_varchar2) then
    p_rec.vhd_information13 :=
    pqp_pvd_shd.g_old_rec.vhd_information13;
  End If;
  If (p_rec.vhd_information14 = hr_api.g_varchar2) then
    p_rec.vhd_information14 :=
    pqp_pvd_shd.g_old_rec.vhd_information14;
  End If;
  If (p_rec.vhd_information15 = hr_api.g_varchar2) then
    p_rec.vhd_information15 :=
    pqp_pvd_shd.g_old_rec.vhd_information15;
  End If;
  If (p_rec.vhd_information16 = hr_api.g_varchar2) then
    p_rec.vhd_information16 :=
    pqp_pvd_shd.g_old_rec.vhd_information16;
  End If;
  If (p_rec.vhd_information17 = hr_api.g_varchar2) then
    p_rec.vhd_information17 :=
    pqp_pvd_shd.g_old_rec.vhd_information17;
  End If;
  If (p_rec.vhd_information18 = hr_api.g_varchar2) then
    p_rec.vhd_information18 :=
    pqp_pvd_shd.g_old_rec.vhd_information18;
  End If;
  If (p_rec.vhd_information19 = hr_api.g_varchar2) then
    p_rec.vhd_information19 :=
    pqp_pvd_shd.g_old_rec.vhd_information19;
  End If;
  If (p_rec.vhd_information20 = hr_api.g_varchar2) then
    p_rec.vhd_information20 :=
    pqp_pvd_shd.g_old_rec.vhd_information20;
  End If;
  --
End convert_defs;
--
-- ----------------------------------------------------------------------------
-- |---------------------------------< upd >----------------------------------|
-- ----------------------------------------------------------------------------
Procedure upd
  (p_effective_date               in date
  ,p_rec                          in out nocopy pqp_pvd_shd.g_rec_type
  ) is
--
  l_proc  varchar2(72) := g_package||'upd';
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  -- We must lock the row which we need to update.
  --
  pqp_pvd_shd.lck
    (p_rec.vehicle_details_id
    ,p_rec.object_version_number
    );
  --
  -- 1. During an update system defaults are used to determine if
  --    arguments have been defaulted or not. We must therefore
  --    derive the full record structure values to be updated.
  --
  -- 2. Call the supporting update validate operations.
  --
  convert_defs(p_rec);
  pqp_pvd_bus.update_validate
     (p_effective_date
     ,p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  pqp_pvd_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pqp_pvd_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pqp_pvd_upd.post_update
     (p_effective_date
     ,p_rec
     );
End upd;
--
-- ----------------------------------------------------------------------------
-- |---------------------------------< upd >----------------------------------|
-- ----------------------------------------------------------------------------
Procedure upd
  (p_effective_date               in     date
  ,p_vehicle_details_id           in     number
  ,p_object_version_number        in out nocopy number
  ,p_vehicle_type                 in     varchar2  default hr_api.g_varchar2
  ,p_registration_number          in     varchar2  default hr_api.g_varchar2
  ,p_make                         in     varchar2  default hr_api.g_varchar2
  ,p_model                        in     varchar2  default hr_api.g_varchar2
  ,p_date_first_registered        in     date      default hr_api.g_date
  ,p_engine_capacity_in_cc        in     number    default hr_api.g_number
  ,p_fuel_type                    in     varchar2  default hr_api.g_varchar2
  ,p_fuel_card                    in     varchar2  default hr_api.g_varchar2
  ,p_currency_code                in     varchar2  default hr_api.g_varchar2
  ,p_list_price                   in     number    default hr_api.g_number
  ,p_business_group_id            in     number    default hr_api.g_number
  ,p_accessory_value_at_startdate in     number    default hr_api.g_number
  ,p_accessory_value_added_later  in     number    default hr_api.g_number
--  ,p_capital_contributions        in     number    default hr_api.g_number
--  ,p_private_use_contributions    in     number    default hr_api.g_number
  ,p_market_value_classic_car     in     number    default hr_api.g_number
  ,p_co2_emissions                in     number    default hr_api.g_number
  ,p_vehicle_provider             in     varchar2  default hr_api.g_varchar2
  ,p_vehicle_identification_numbe in     varchar2  default hr_api.g_varchar2
  ,p_vehicle_ownership            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute_category       in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute1               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute2               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute3               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute4               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute5               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute6               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute7               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute8               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute9               in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute10              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute11              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute12              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute13              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute14              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute15              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute16              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute17              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute18              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute19              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_attribute20              in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information_category     in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information1             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information2             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information3             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information4             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information5             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information6             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information7             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information8             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information9             in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information10            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information11            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information12            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information13            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information14            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information15            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information16            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information17            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information18            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information19            in     varchar2  default hr_api.g_varchar2
  ,p_vhd_information20            in     varchar2  default hr_api.g_varchar2
  ) is
--
  l_rec	  pqp_pvd_shd.g_rec_type;
  l_proc  varchar2(72) := g_package||'upd';
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  -- Call conversion function to turn arguments into the
  -- l_rec structure.
  --
  l_rec :=
  pqp_pvd_shd.convert_args
  (p_vehicle_details_id
  ,p_vehicle_type
  ,p_business_group_id
  ,p_registration_number
  ,p_make
  ,p_model
  ,p_date_first_registered
  ,p_engine_capacity_in_cc
  ,p_fuel_type
  ,p_fuel_card
  ,p_currency_code
  ,p_list_price
  ,p_accessory_value_at_startdate
  ,p_accessory_value_added_later
--  ,p_capital_contributions
--  ,p_private_use_contributions
  ,p_market_value_classic_car
  ,p_co2_emissions
  ,p_vehicle_provider
  ,p_object_version_number
  ,p_vehicle_identification_numbe
  ,p_vehicle_ownership
  ,p_vhd_attribute_category
  ,p_vhd_attribute1
  ,p_vhd_attribute2
  ,p_vhd_attribute3
  ,p_vhd_attribute4
  ,p_vhd_attribute5
  ,p_vhd_attribute6
  ,p_vhd_attribute7
  ,p_vhd_attribute8
  ,p_vhd_attribute9
  ,p_vhd_attribute10
  ,p_vhd_attribute11
  ,p_vhd_attribute12
  ,p_vhd_attribute13
  ,p_vhd_attribute14
  ,p_vhd_attribute15
  ,p_vhd_attribute16
  ,p_vhd_attribute17
  ,p_vhd_attribute18
  ,p_vhd_attribute19
  ,p_vhd_attribute20
  ,p_vhd_information_category
  ,p_vhd_information1
  ,p_vhd_information2
  ,p_vhd_information3
  ,p_vhd_information4
  ,p_vhd_information5
  ,p_vhd_information6
  ,p_vhd_information7
  ,p_vhd_information8
  ,p_vhd_information9
  ,p_vhd_information10
  ,p_vhd_information11
  ,p_vhd_information12
  ,p_vhd_information13
  ,p_vhd_information14
  ,p_vhd_information15
  ,p_vhd_information16
  ,p_vhd_information17
  ,p_vhd_information18
  ,p_vhd_information19
  ,p_vhd_information20
  );
  --
  -- Having converted the arguments into the
  -- plsql record structure we call the corresponding record
  -- business process.
  --
  pqp_pvd_upd.upd
     (p_effective_date
     ,l_rec
     );
  p_object_version_number := l_rec.object_version_number;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End upd;
--
end pqp_pvd_upd;

/
