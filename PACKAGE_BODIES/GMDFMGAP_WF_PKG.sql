--------------------------------------------------------
--  DDL for Package Body GMDFMGAP_WF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMDFMGAP_WF_PKG" AS
/* $Header: GMDFMGAB.pls 120.1.12010000.3 2009/04/08 07:54:32 kannavar ship $ */
   PROCEDURE wf_init (
   ...
   IS
      ...
      l_itemtype                WF_ITEMS.ITEM_TYPE%TYPE :=  'GMDFMGAP';
      ...
      l_WorkflowProcess   VARCHAR2(30) := 'GMDFMGAP_PROCESS';
      l_count             NUMBER;
      BEGIN
      ...
      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
        					  aname => 'GMDFMGAP_OWNER_ID',
         					  avalue => l_performer_name);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDFMGAP_FORMULA_DESC',
         					  avalue => l_formula_desc);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDFMGAP_FORMULA_CLASS',
         					  avalue => l_formula_class);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDFMGAP_SCALE_TYPE',
         					  avalue => l_scale_type);
  ...
  EXCEPTION
      ...
      FND_MESSAGE.SET_TOKEN('PKG_NAME', 'GMDFMGAP_WF_PKG');
      FND_MESSAGE.SET_TOKEN('PROCEDURE_NAME', 'WF_INIT');
      APP_EXCEPTION.RAISE_EXCEPTION;
      WF_CORE.CONTEXT ('GMDFMGAP_wf_pkg','wf_init',l_itemtype,l_itemkey,'Initial' );
      raise;
...
/* ######################################################################## */
   ...
   IS
      ...
      l_formula_id         FM_FORM_MST_B.formula_id%TYPE:=to_number(wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_FORMULA_ID'));
      p_data_string       VARCHAR2(2000);
      p_lab_wf_item_type  VARCHAR2(8)  := 'GMDFMGAP';  -- Recipe Lab use Approval Workflow Inernal Name
      P_lab_Process_name  VARCHAR2(32) := 'GMDFMGAP_PROCESS'; -- Recipe Lab use Approval Workflow Process Inernal Name
      P_lab_activity_name VARCHAR2(80) := 'GMDFMGAP_NOTI_REQUEST';
    ...
    BEGIN
     ...
     /* Bug No.8355449 - Added the below line to enable m_fs_context */
     ...
     IF (p_funcmode = 'RUN') THEN
         ...
         THEN
	      ...
	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDFMGAP_ADDL_TEXT',
                                      avalue => l_data_string);

	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDFMGAP_APPROVER',
                                      avalue => p_role);

            p_resultout:='COMPLETE:Y';
      ELSE
        l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_TARGET_STATUS');
...
EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDFMGAP_wf_pkg','is_approval_req',p_itemtype,p_itemkey,p_role);
      raise;
END is_approval_req;
...
PROCEDURE REMINDAR_CHECK (
      ...
      l_mesg_cnt      number:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDFMGAP_MESG_CNT');
      l_approver      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_APPROVER');
BEGIN
...
/* added variables for recipe generation */
    ...
    CURSOR Cur_check_recipe (V_formula_id NUMBER) IS
     ...
     CURSOR c_get_orgn (V_formula_id NUMBER) IS
      ...
      /* or optional setup */
      CURSOR cur_recipe_enable (V_orgn_id NUMBER) IS
...
BEGIN
     IF (p_funcmode = 'RUN') THEN
          l_start_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_START_STATUS');
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_TARGET_STATUS');
...
PROCEDURE MOREINFO_RESPONSE  (
      ...
      p_funcmode      IN VARCHAR2,
      p_resultout     OUT NOCOPY VARCHAR2) IS
      l_requester     VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_REQUSTER');
BEGIN
...
PROCEDURE APPEND_COMMENTS (
      ...
      l_comment       VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_COMMENT');
      l_mesg_comment  VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_DISP_COMMENT');
      l_performer      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDFMGAP_CURR_PERFORMER');
BEGIN
...
--           l_html_mesg := replace(l_mesg_comment,wf_core.newline,'<BR>'||wf_core.newline);
	   ...
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDFMGAP_DISP_COMMENT',
                                   avalue => l_mesg_comment);
--	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
--                                   itemkey => p_itemkey,
--         			           aname => 'GMDFMGAP_HTML_DISP_COMMENT',
--                                   avalue => l_html_mesg);
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDFMGAP_COMMENT',
                                   avalue => l_comment);
       END IF;
END;
