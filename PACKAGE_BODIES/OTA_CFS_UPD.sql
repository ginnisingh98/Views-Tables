--------------------------------------------------------
--  DDL for Package Body OTA_CFS_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_CFS_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_cfs_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the ota_conference_servers Row
  --
  update ota_conference_servers_b
    set
     conference_server_id            = p_rec.conference_server_id
    ,url                             = p_rec.url
    ,type                            = p_rec.type
    ,owc_site_id                     = p_rec.owc_site_id
    ,owc_auth_token                  = p_rec.owc_auth_token
...
Begin
  ...
  If (p_rec.type = hr_api.g_varchar2) then
    ...
    ota_cfs_shd.g_old_rec.type;
  End If;
  If (p_rec.owc_site_id = hr_api.g_varchar2) then
    p_rec.owc_site_id :=
    ota_cfs_shd.g_old_rec.owc_site_id;
  End If;
  If (p_rec.owc_auth_token = hr_api.g_varchar2) then
    p_rec.owc_auth_token :=
    ota_cfs_shd.g_old_rec.owc_auth_token;
  End If;
...
Begin
  ...
  --
  ota_cfs_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ota_cfs_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ota_cfs_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
