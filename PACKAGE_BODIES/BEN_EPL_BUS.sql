--------------------------------------------------------
--  DDL for Package Body BEN_EPL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EPL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_epl_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_elig_pstl_cd_r_rng_cvg_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   elig_pstl_cd_r_rng_cvg_id PK of record being inserted or updated.
...
--
Procedure chk_elig_pstl_cd_r_rng_cvg_id(p_elig_pstl_cd_r_rng_cvg_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_elig_pstl_cd_r_rng_cvg_id';
...
Begin
  ...
  --
  l_api_updating := ben_epl_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_elig_pstl_cd_r_rng_cvg_id                => p_elig_pstl_cd_r_rng_cvg_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_elig_pstl_cd_r_rng_cvg_id,hr_api.g_number)
     <>  ben_epl_shd.g_old_rec.elig_pstl_cd_r_rng_cvg_id) then
    --
    -- raise error as PK has changed
    --
    ben_epl_shd.constraint_error('BEN_ELIG_PSTL_CD_R_RNG_CVG_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_elig_pstl_cd_r_rng_cvg_id is not null then
      --
  ...
  --
End chk_elig_pstl_cd_r_rng_cvg_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_excld_flag >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_elig_pstl_cd_r_rng_cvg_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_epl_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--   p_elig_pstl_cd_r_rng_cvg_id - primary key of the table
--   p_pstl_zip_rng_id - duplicate value to be checked
...
--   Internal table handler use only.
--
Procedure chk_duplicate_rows(p_elig_pstl_cd_r_rng_cvg_id in number,
  ...
  dummy varchar2(1);
  cursor c1 is select null from ben_elig_pstl_cd_r_rng_cvg_f
     where (elig_pstl_cd_r_rng_cvg_id <> p_elig_pstl_cd_r_rng_cvg_id or p_elig_pstl_cd_r_rng_cvg_id is null) and
           dpnt_cvg_eligy_prfl_id = p_dpnt_cvg_eligy_prfl_id and
           pstl_zip_rng_id = p_pstl_zip_rng_id and
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and elig_pstl_cd_r_rng_cvg_id <> nvl(p_elig_pstl_cd_r_rng_cvg_id,-1)
...
function return_legislation_code
  (p_elig_pstl_cd_r_rng_cvg_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_elig_pstl_cd_r_rng_cvg_f b
    where b.elig_pstl_cd_r_rng_cvg_id      = p_elig_pstl_cd_r_rng_cvg_id
