--------------------------------------------------------
--  DDL for Package Body MSC_REPPERIODS_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MSC_REPPERIODS_PUB" AS
/* $Header: MSCPRPDB.pls 120.2 2007/10/16 17:06:24 navgoel ship $ */
...
BEGIN
    ...
    --  Lock table msc_repetitive_periods before insert/delete/update
    LOCK TABLE msc_repetitive_periods IN EXCLUSIVE MODE;
    ...
    msc_calendar.select_calendar_defaults(
        arg_org_id,
        arg_instance_id,
        var_calendar_code,
    ...
    DELETE FROM msc_repetitive_periods
    WHERE  organization_id = arg_org_id
      AND  sr_instance_id = arg_instance_id;
    ...
    IF param.period_type = WORKDATE_PERIODS THEN
        ...
        var_end_date1 := msc_calendar.date_offset(
            arg_org_id,
            arg_instance_id,
            BUCKET_TYPE,
        ...
        var_end_date2 := msc_calendar.date_offset(
            arg_org_id,
            arg_instance_id,
            BUCKET_TYPE,
            var_end_date1,
        ...
        var_last_date1 := msc_calendar.date_offset(
            arg_org_id,
            arg_instance_id,
            BUCKET_TYPE,
            var_max_date,
        ...
        var_last_date2 := msc_calendar.date_offset(
            arg_org_id,
            arg_instance_id,
            BUCKET_TYPE,
            var_max_date,
        ...
        var_last_date3 := msc_calendar.date_offset(
            arg_org_id,
            arg_instance_id,
            BUCKET_TYPE,
            var_max_date,
    ...
    WHILE TRUE LOOP
        ...
        -- period, insert into the table msc_repetitive_periods
        ...
        IF var_curr_workday <> var_prev_workday THEN
            INSERT INTO msc_repetitive_periods
            (
                period_start_date,
                organization_id,
                sr_instance_id,
        ...
        IF var_curr_bucket = 1 THEN     -- First bucket
            IF param.period_type = WORKDATE_PERIODS THEN
                var_curr_date := msc_calendar.date_offset(
                    arg_org_id,
                    arg_instance_id,
                    BUCKET_TYPE,
                    var_curr_date,
    ...
    UPDATE msc_repetitive_periods s
    ...
    WHERE   EXISTS
            (SELECT NULL
            FROM    msc_calendar_dates  d
            WHERE   NOT EXISTS
                    (SELECT NULL
                    FROM    msc_repetitive_periods
                    WHERE   organization_id = s.organization_id
                      AND   sr_instance_id = s.sr_instance_id
    ...
    DELETE FROM msc_repetitive_periods   s
    WHERE   organization_id = arg_org_id
      AND   sr_instance_id = arg_instance_id
      AND   EXISTS
            (SELECT NULL
