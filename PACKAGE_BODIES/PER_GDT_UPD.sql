--------------------------------------------------------
--  DDL for Package Body PER_GDT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_GDT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_gdt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out NOCOPY per_gdt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the per_grades_tl Row
  --
  update per_grades_tl
    set
...
Procedure pre_update
  (p_rec in per_gdt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  begin
    ...
    per_gdt_rku.after_update
      ...
      => p_rec.name
      ,p_source_lang_o
      => per_gdt_shd.g_old_rec.source_lang
      ,p_name_o
      => per_gdt_shd.g_old_rec.name
      );
    --
  exception
    --
...
Begin
  ...
  --
  per_gdt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_gdt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_gdt_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  upd_key_flex_meanings( p_grade_id );
  --
...
Procedure upd_key_flex_meanings
  ...
  select l.language_code
   ...
   where grd.grade_id            = p_grade_id
     and grd.grade_definition_id = gd.grade_definition_id
     and l.installed_flag IN ('B', 'I');

  --
  l_userenv_language_code   VARCHAR2(4) := userenv('LANG'); --Bug 2962837.
  --
  l_gdt_rec  per_gdt_shd.g_rec_type;
  l_name varchar2(240);  --Bug 3255685
  --
...
Begin
  ...
  For l_language IN c_languages Loop
     ...
     --
     l_gdt_rec := per_gdt_shd.g_old_rec;
     ...
     --
     l_name := fnd_flex_ext.get_segs( per_gdt_shd.g_app_code
                                     , per_gdt_shd.g_flex_code
                                     , l_language.id_flex_num
                                     , l_language.grade_definition_id);
     l_gdt_rec.name := nvl(l_name, l_gdt_rec.name); --Bug fix 3255685
     --
     --Update the record
     --
     update_dml(l_gdt_rec);
     --
  End Loop;
  --
  -- Set the session language
