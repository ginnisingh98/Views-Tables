--------------------------------------------------------
--  DDL for Package Body IGI_IGIIACST_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_IGIIACST_XMLP_PKG" AS
/* $Header: IGIIACSTB.pls 120.0.12010000.1 2008/07/29 08:58:30 appldev ship $ */
...
procedure FDRCSID(idstring in varchar2) is
  BEGIN
    IF (idstring is NULL) THEN
      NULL;
    END IF;
        --        FDRCSID('$Header: IGIIACSTB.pls 120.0.12010000.1 2008/07/29 08:58:30 appldev ship $');
  END;
...
PROCEDURE Insert_info (
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RP_REPORT_NAME := 'Inflation Accounting : Cost Summary Report' || '(' || CURRENCY_CODE || ')';
  ...
  FUNCTION ACCT_AC_APROMPT_P RETURN VARCHAR2 IS
  BEGIN
    RETURN ACCT_AC_APROMPT;
  END ACCT_AC_APROMPT_P;
  ...
  FUNCTION RP_AC_LPROMPT_P RETURN VARCHAR2 IS
  BEGIN
    RETURN RP_AC_LPROMPT;
  END RP_AC_LPROMPT_P;
