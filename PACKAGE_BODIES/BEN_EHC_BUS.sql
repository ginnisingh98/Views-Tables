--------------------------------------------------------
--  DDL for Package Body BEN_EHC_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EHC_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ehc_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ELIG_HLTH_CVG_PRTE_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   ELIG_HLTH_CVG_PRTE_id PK of record being inserted or updated.
...
--
Procedure chk_ELIG_HLTH_CVG_PRTE_id(p_ELIG_HLTH_CVG_PRTE_id in number,
                                    p_effective_date        in date,
                                    p_object_version_number in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ELIG_HLTH_CVG_PRTE_id';
...
Begin
  ...
  --
  l_api_updating := ben_ehc_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ELIG_HLTH_CVG_PRTE_id       => p_ELIG_HLTH_CVG_PRTE_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ELIG_HLTH_CVG_PRTE_id,hr_api.g_number)
     <>  ben_ehc_shd.g_old_rec.ELIG_HLTH_CVG_PRTE_id) then
    --
    -- raise error as PK has changed
    --
    ben_ehc_shd.constraint_error('BEN_ELIG_HLTH_CVG_PRTE_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ELIG_HLTH_CVG_PRTE_id is not null then
      --
...
--
-- In Parameters
--   p_ELIG_HLTH_CVG_PRTE_id PK
--   p_pl_typ_opt_typ_id of FK column
...
--   Internal table handler use only.
--
Procedure chk_pl_typ_opt_typ_id(p_ELIG_HLTH_CVG_PRTE_id in number,
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_ehc_shd.g_old_rec.pl_typ_opt_typ_id,hr_api.g_number)
     or not l_api_updating)
     and p_pl_typ_opt_typ_id is not null then
    --
    -- check if pl_typ_opt_typ_id value exists in
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- ben_pl_typ_opt_typ_f table.
        --
        ben_ehc_shd.constraint_error('BEN_ELIG_HLTH_CVG_PRTE_F_FK1');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_oipl_id(p_ELIG_HLTH_CVG_PRTE_id in number,
                    p_oipl_id                 in number,
                    p_object_version_number in number,
                    p_effective_date        in date,
                    p_business_group_id     in number) is
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_ehc_shd.g_old_rec.oipl_id,hr_api.g_number)
     or not l_api_updating)
     and p_oipl_id is not null then
    --
    -- check if oipl_id value exists in
    -- ben_oipl_f table
    --
    open c1;
      --
      ...
      if c1%notfound then
        ...
        -- ben_oipl_f table.
        --
        ben_ehc_shd.constraint_error('BEN_ELIG_HLTH_CVG_PRTE_F_FK2');
        --
      end if;
      --
    close c1;
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_ELIG_HLTH_CVG_PRTE_id       in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_ehc_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   p_ELIG_HLTH_CVG_PRTE_id    ELIG_HLTH_CVG_PRTE_id
...
function return_legislation_code
  (p_ELIG_HLTH_CVG_PRTE_id in number) return varchar2 is
  --
  -- Declare cursor
  --
