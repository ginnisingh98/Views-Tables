--------------------------------------------------------
--  DDL for Package Body BEN_PDP_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PDP_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pdp_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_delete_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode <> 'ZAP') then
    --
    p_rec.effective_start_date := ben_pdp_shd.g_old_rec.effective_start_date;
    --
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  -- Bug 3756863 : Moved here from DELETE_VALIDATE
  ben_pdp_bus.crt_ordr_warning
     (p_prtt_enrt_rslt_id     => ben_pdp_shd.g_old_rec.prtt_enrt_rslt_id,
      p_effective_date        => p_effective_date,
      p_business_group_id     => ben_pdp_shd.g_old_rec.business_group_id);
  --


  if p_datetrack_mode <> hr_api.g_future_change then
     hr_utility.set_location('update the elig dpnt - cvrd_dpnt_id ', 5);
     update ben_elig_dpnt
        set elig_cvrd_dpnt_id = null
      where elig_cvrd_dpnt_id = p_rec.elig_cvrd_dpnt_id
        and dpnt_person_id = ben_pdp_shd.g_old_rec.dpnt_person_id
        and per_in_ler_id = ben_pdp_shd.g_old_rec.per_in_ler_id;
     hr_utility.set_location('Rows updated :'||sql%rowcount , 5);
  end if;
