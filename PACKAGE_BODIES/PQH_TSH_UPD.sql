--------------------------------------------------------
--  DDL for Package Body PQH_TSH_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_TSH_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_tsh_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the pqh_ss_trans_state_history Row
  --
...
Begin
  ...
  If (p_rec.creator_person_id = hr_api.g_number) then
    ...
    pqh_tsh_shd.g_old_rec.creator_person_id;
  End If;
  If (p_rec.creator_role = hr_api.g_varchar2) then
    p_rec.creator_role :=
    pqh_tsh_shd.g_old_rec.creator_role;
  End If;
  If (p_rec.status = hr_api.g_varchar2) then
    p_rec.status :=
    pqh_tsh_shd.g_old_rec.status;
  End If;
  If (p_rec.transaction_state = hr_api.g_varchar2) then
    p_rec.transaction_state :=
    pqh_tsh_shd.g_old_rec.transaction_state;
  End If;
  If (p_rec.effective_date = hr_api.g_date) then
    p_rec.effective_date :=
    pqh_tsh_shd.g_old_rec.effective_date;
  End If;
  If (p_rec.effective_date_option = hr_api.g_varchar2) then
    p_rec.effective_date_option :=
    pqh_tsh_shd.g_old_rec.effective_date_option;
  End If;
  ...
  If (p_rec.parent_transaction_id = hr_api.g_number) then
    ...
    pqh_tsh_shd.g_old_rec.parent_transaction_id;
  End If;
  If (p_rec.relaunch_function = hr_api.g_varchar2) then
    p_rec.relaunch_function :=
    pqh_tsh_shd.g_old_rec.relaunch_function;
  End If;
  If (p_rec.transaction_document IS NULL) then
    p_rec.transaction_document := pqh_tsh_shd.g_old_rec.transaction_document;
  End If;
  --
End convert_defs;
--
