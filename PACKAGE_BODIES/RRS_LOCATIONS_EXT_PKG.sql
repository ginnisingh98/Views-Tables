--------------------------------------------------------
--  DDL for Package Body RRS_LOCATIONS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."RRS_LOCATIONS_EXT_PKG" as
/* $Header: RRSLEXTB.pls 120.0.12010000.2 2009/01/05 18:39:30 sunarang ship $ */
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from RRS_LOCATIONS_EXT_B
...
insert into RRS_LOCATIONS_EXT_B (
    EXTENSION_ID,
    LOCATION_ID,
    COUNTRY,
    ATTR_GROUP_ID,
   ...
   insert into RRS_LOCATIONS_EXT_TL (
    EXTENSION_ID,
    LOCATION_ID,
    COUNTRY,
    ATTR_GROUP_ID,
...
begin
  ...
  close c;
  if (    ((recinfo.C_EXT_ATTR13 = X_C_EXT_ATTR13)
...
begin
  update RRS_LOCATIONS_EXT_B set
  ...
  update RRS_LOCATIONS_EXT_TL set
...
begin
  delete from RRS_LOCATIONS_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from RRS_LOCATIONS_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  delete from RRS_LOCATIONS_EXT_TL T
  where not exists
    (select NULL
    from RRS_LOCATIONS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update RRS_LOCATIONS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from RRS_LOCATIONS_EXT_TL SUBB, RRS_LOCATIONS_EXT_TL SUBT
  ...
  insert into RRS_LOCATIONS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from RRS_LOCATIONS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
