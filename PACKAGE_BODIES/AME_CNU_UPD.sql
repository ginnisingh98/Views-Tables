--------------------------------------------------------
--  DDL for Package Body AME_CNU_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_CNU_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_cnu_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
 ...
 ,p_rule_id =>  p_rec.rule_id
    ...
    --
    --
    -- Update the ame_condition_usages Row
    --
...
Begin
  ...
  begin
    ...
    ame_cnu_rku.after_update
      ...
      ,p_end_date_o
      => ame_cnu_shd.g_old_rec.end_date
      ,p_security_group_id_o
      => ame_cnu_shd.g_old_rec.security_group_id
      ,p_object_version_number_o
      => ame_cnu_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.start_date = hr_api.g_date) then
     p_rec.start_date := ame_cnu_shd.g_old_rec.start_date;
  End If;
  If (p_rec.end_date = hr_api.g_date) then
    p_rec.end_date := ame_cnu_shd.g_old_rec.end_date;
  End If;
  --
End convert_defs;
--
...
Begin
  ...
  --
  ame_cnu_upd.convert_defs(p_rec);
  --
  --
