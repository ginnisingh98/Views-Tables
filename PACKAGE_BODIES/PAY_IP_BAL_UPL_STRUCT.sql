--------------------------------------------------------
--  DDL for Package Body PAY_IP_BAL_UPL_STRUCT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_IP_BAL_UPL_STRUCT" AS
/* $Header: pyipbups.pkb 115.3 2002/12/04 12:35:59 atrivedi noship $ */
...
-- retcode = 0 means no error and retcode = 2 means an error occurred.
      ...
      is
         ...
         and    BF.balance_type_id + 0 = p_balance_type_id
         and    nvl(BF.business_group_id, p_business_group) = p_business_group;

   BEGIN
      hr_utility.set_location('paybalup.create_bal_upl_struct', 10);

      select pbg.business_group_id, bbh.business_group_name ,
             pbg.currency_code, pbg.legislation_code
      into l_bg_id, l_bg_name,
           l_currency_code, l_leg_code
