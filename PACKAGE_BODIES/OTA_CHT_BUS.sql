--------------------------------------------------------
--  DDL for Package Body OTA_CHT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_CHT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_cht_bus.';  -- Global package name
--
...
Begin
  ...
  --
  --
  if (( nvl(ota_cht_bus.g_chat_id, hr_api.g_number)
       = p_chat_id)
  and ( nvl(ota_cht_bus.g_language, hr_api.g_varchar2)
       = p_language)) then
    --
    ...
    -- variable.
    --
    l_legislation_code := ota_cht_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    ota_cht_bus.g_chat_id                     := p_chat_id;
    ota_cht_bus.g_language                    := p_language;
    ota_cht_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT ota_cht_shd.api_updating
...
Begin
  ...
  ota_cht_bus.set_security_group_id(p_chat_id => p_chat_id);
  --
