--------------------------------------------------------
--  DDL for Package Body PER_JBT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_JBT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_jbt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out NOCOPY per_jbt_shd.g_rec_type
  ) is
--
...
Procedure pre_update
  (p_rec in per_jbt_shd.g_rec_type
  ) is
--
...
Begin
  ...
  begin
    ...
    per_jbt_rku.after_update
      ...
      => p_rec.name
      ,p_source_lang_o
      => per_jbt_shd.g_old_rec.source_lang
      ,p_name_o
      => per_jbt_shd.g_old_rec.name
      );
    --
  exception
    --
...
Begin
  ...
  --
  per_jbt_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_jbt_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_jbt_upd.post_update
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  upd_key_flex_meanings( p_job_id );
  --
...
Begin
  ...
  For l_language IN c_languages Loop
     ...
     --
     l_jbt_rec.name := fnd_flex_ext.get_segs( per_jbt_shd.g_app_code
                                            , per_jbt_shd.g_flex_code
                                            , l_language.id_flex_num
                                            , l_language.job_definition_id);
     --
     --Update the record
     --
     update_dml(l_jbt_rec);
     --
  End Loop;
  --
  -- Set the session language
