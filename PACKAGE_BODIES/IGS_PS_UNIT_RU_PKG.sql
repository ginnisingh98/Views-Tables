--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_RU_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_RU_PKG" as
/* $Header: IGSPI90B.pls 115.7 2002/11/29 02:41:32 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_UNIT_RU%RowType;
  new_references IGS_PS_UNIT_RU%RowType;
...
IF (cur_rowid%FOUND) THEN
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UR_SRC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UR_UN_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UR_RUL_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_UNIT_RU
      where UNIT_CD = X_UNIT_CD
      and S_RULE_CALL_CD = X_S_RULE_CALL_CD;
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_UNIT_RU
     where UNIT_CD = X_UNIT_CD
     and S_RULE_CALL_CD = X_S_RULE_CALL_CD
  ;
begin
