--------------------------------------------------------
--  DDL for Package Body PQH_TRL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_TRL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  pqh_trl_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy pqh_trl_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the pqh_table_route_tl Row
  --
...
Begin
  ...
  --
  convert_defs(p_rec);
  pqh_trl_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  pqh_trl_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pqh_trl_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pqh_trl_upd.post_update
     (p_rec
     );
End upd;
--
  ...
  cursor csr_upd_langs is
    select trl.language
     ...
     where trl.table_route_id = p_table_route_id
       and p_language_code in (trl.language
                              ,trl.source_lang);
  --
