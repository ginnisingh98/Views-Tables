--------------------------------------------------------
--  DDL for Package Body IGS_PS_GRP_FEE_TRG_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_GRP_FEE_TRG_PKG" AS
  /* $Header: IGSPI16B.pls 115.7 2003/02/20 10:17:59 shtatiko ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_GRP_FEE_TRG%RowType;
  new_references IGS_PS_GRP_FEE_TRG%RowType;
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_cgft_br_iud
  -- BEFORE INSERT OR DELETE OR UPDATE
  -- ON IGS_PS_GRP_FEE_TRG
  -- FOR EACH ROW
  ...
  BEGIN
	IF p_inserting THEN
		...
		IF IGS_FI_VAL_CGFT.finp_val_cgft_ins (
	...
	IF p_inserting OR p_updating THEN
		-- Validate IGS_PS_COURSE Group not closed.
		IF IGS_EN_VAL_PCGE.enrp_val_crs_gp_clsd (
  ...
  BEGIN
	-- Validate for open IGS_PS_COURSE Group Fee Trig records.
	IF p_inserting OR
	    (new_references.logical_delete_dt IS NULL) THEN
		 -- Save the rowid of the current row.
		-- Validate for open ended IGS_PS_GRP_FEE_TRG records.
  		IF new_references.logical_delete_dt IS NULL THEN
  			IF IGS_FI_VAL_CGFT.finp_val_cgft_open(new_references.fee_cat,
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PS_GRP_FEE_TRG
      where FEE_CAT = X_FEE_CAT
...
Before_DML (
  ...
  insert into IGS_PS_GRP_FEE_TRG (
    FEE_CAT,
    FEE_CAL_TYPE,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_GRP_FEE_TRG
     where FEE_CAT = X_FEE_CAT
...
Before_DML (
  ...
  delete from IGS_PS_GRP_FEE_TRG
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
