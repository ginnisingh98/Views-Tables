--------------------------------------------------------
--  DDL for Package Body IGI_IGIIAOPD_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_IGIIAOPD_XMLP_PKG" AS
/* $Header: IGIIAOPDB.pls 120.0.12010000.1 2008/07/29 08:58:33 appldev ship $ */
...
procedure FDRCSID(idstring in varchar2) is
  BEGIN
    IF (idstring is NULL) THEN
      NULL;
    END IF;
        --        FDRCSID('$Header: IGIIAOPDB.pls 120.0.12010000.1 2008/07/29 08:58:33 appldev ship $');
  END;
...
PROCEDURE Insert_info (
  ...
  FUNCTION BLOG_TRANSFERFORMULA(BALANCE_TYPE IN VARCHAR2) RETURN NUMBER IS
    L_NUM NUMBER := 0;
  BEGIN
    IF (BALANCE_TYPE = 'Backlog') THEN
      L_NUM := 0;
  ...
  FUNCTION COST_MOVEMENTFORMULA(BALANCE_TYPE IN VARCHAR2
                               ...
                               ,RECLASS IN NUMBER
                               ,ASSET_COST_TRANSFER IN NUMBER) RETURN NUMBER IS
    L_NUM NUMBER := 0;
  BEGIN
    IF (BALANCE_TYPE = 'Cost') THEN
      L_NUM := NVL(ADDITION
                  ...
                  ,0) + NVL(REVALUATION
                  ,0) + NVL(RECLASS
                  ,0) + NVL(ASSET_COST_TRANSFER
                  ,0);
    END IF;
    RETURN (L_NUM);
  END COST_MOVEMENTFORMULA;
