--------------------------------------------------------
--  DDL for Package Body JTF_CAL_WF_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_CAL_WF_PVT" AS
/* $Header: jtfvcwb.pls 120.2 2005/11/30 04:25:51 sankgupt ship $ */

g_pkg_name     CONSTANT     VARCHAR2(30) := 'JTF_CAL_WF_PVT';
...
IS
  ...
  l_item_type                 VARCHAR2(8) := 'JTFCALWF';
...
BEGIN
  ...
  *****************************************************************************/
  ...
  SELECT to_char(jtf_cal_itemkey_s.NEXTVAL) INTO l_item_key
    FROM DUAL;
...
BEGIN
  ...
  *****************************************************************************/
  IF (l_Response = 'JTF_CAL_ADMIN_ACCESS')
  THEN
    fnd_grants_pkg.grant_function( p_api_version      => 1.0
                               , p_menu_name          => 'JTF_CAL_READ_ACCESS'
                               , p_instance_type      => 'INSTANCE'
                               , p_object_name        => 'JTF_TASK_RESOURCE'
                               , p_instance_pk1_value => to_char(l_GroupID)
                               , p_instance_pk2_value => 'RS_GROUP'
  ...
  THEN
    ...
    *****************************************************************************/
    wf_engine.SetItemAttrText( itemtype => itemtype
                             , itemkey  => itemkey
                             , aname    => 'ERROR_MESSAGE'
                             , avalue   => 'JTF_CAL_WF_PVT.ProcessSubscription(): ' || to_char(SQLCODE)||':'||SQLERRM
                             );
...
IS
  ...
  l_ItemType                 VARCHAR2(8)    := 'JTFCALWF';
...
BEGIN
  ...
  ** Get a WF itemkey
  *****************************************************************************/
  SELECT to_char(jtf_cal_itemkey_s.NEXTVAL) INTO l_ItemKey
  FROM DUAL;
...
BEGIN
  ...
  *****************************************************************************/
  fnd_grants_pkg.grant_function( p_api_version        => 1.0
                               , p_menu_name          => 'JTF_CAL_ADMIN_ACCESS'
                               , p_instance_type      => 'INSTANCE'
                               , p_object_name        => 'JTF_TASK_RESOURCE'
  ...
  THEN
    ...
    *****************************************************************************/
    wf_engine.SetItemAttrText( itemtype => itemtype
                             , itemkey  => itemkey
                             , aname    => 'ERROR_MESSAGE'
                             , avalue   => 'JTF_CAL_WF_PVT.ProcessRequest(): ' || to_char(SQLCODE)||':'||SQLERRM
                             );
...
IS
  ...
  *****************************************************************************/
  (b_TaskID NUMBER
  )IS SELECT to_char(jtf_cal_itemkey_s.NEXTVAL) ItemKey
  ...
  THEN
    ...
    *****************************************************************************/
    wf_engine.SetItemAttrText( itemtype => itemtype
                             , itemkey  => itemkey
                             , aname    => 'ERROR_MESSAGE'
                             , avalue   => 'JTF_CAL_WF_PVT.DetermineWFRole(): ' || to_char(SQLCODE)||':'||SQLERRM
                             );
...
IS
  ...
  *****************************************************************************/
  (b_TaskID NUMBER
      ...
      ,      to_char(jtf_cal_itemkey_s.NEXTVAL) ItemKey
      ,      jta.resource_id                    INVITEE
      ,      jta.resource_type_code             INVITEE_CODE
      ,      jtl.task_name                      TASK_NAME
      ,      jtl.description                    TASK_DESCRIPTION
  ...
  THEN
    ...
    *****************************************************************************/
    wf_engine.SetItemAttrText( itemtype => itemtype
                             , itemkey  => itemkey
                             , aname    => 'ERROR_MESSAGE'
                             , avalue   => 'JTF_CAL_WF_PVT.SendReminders(): ' || to_char(SQLCODE)||':'||SQLERRM
                             );
  ...
  THEN
    ...
    *****************************************************************************/
    ...
    wf_engine.SetItemAttrText( itemtype => itemtype
                             , itemkey  => itemkey
                             , aname    => 'ERROR_MESSAGE'
                             , avalue   => 'JTF_CAL_WF_PVT.GetRepeatingRule(): ' || to_char(SQLCODE)||':'||SQLERRM
                             );
...
EXCEPTION
  ...
  THEN
    ...
    wf_engine.SetItemAttrText( itemtype => itemtype
                             , itemkey  => itemkey
                             , aname    => 'ERROR_MESSAGE'
                             , avalue   => 'JTF_CAL_WF_PVT.GetInvitationStatus(): ' || to_char(SQLCODE)||':'||SQLERRM
                             );
    resultout := 'COMPLETE:ERROR';
