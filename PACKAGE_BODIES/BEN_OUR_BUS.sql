--------------------------------------------------------
--  DDL for Package Body BEN_OUR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_OUR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_our_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_org_unit_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   org_unit_rt_id PK of record being inserted or updated.
...
--
Procedure chk_org_unit_rt_id(p_org_unit_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_org_unit_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_our_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_org_unit_rt_id                => p_org_unit_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_org_unit_rt_id,hr_api.g_number)
     <>  ben_our_shd.g_old_rec.org_unit_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_our_shd.constraint_error('BEN_ORG_UNIT_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_org_unit_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_org_unit_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_our_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   organization_id FK of pointing to HR_ALL_ORGANIZATION_UNITS tables.
...
--   Internal table handler use only.
--
Procedure chk_organization_id(p_organization_id      in number,
...
Begin
  ...
  if p_organization_id is not null then
     ...
     if c1%notfound then
        hr_utility.set_message(801,'Org_Id_not_exist');
        hr_utility.raise_error;
     end if;
     close c1;
  end if;
...
--
-- In Parameters
--   p_org_unit_rt_id        PK of record being inserted or updated.
--   p_organization_id       Value of FK
...
and org_unit_rt_id <> nvl(p_org_unit_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_organization_id <> nvl(ben_our_shd.g_old_rec.organization_id,hr_api.g_number)
      or not l_api_updating) then
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Organization criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Organization criteria',TRUE);
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and org_unit_rt_id   <> nvl(p_org_unit_rt_id,-1)
                   -- and organization_id   <> nvl(p_organization_id,-1)
...
function return_legislation_code
  (p_org_unit_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_org_unit_rt_f b
    where b.org_unit_rt_id      = p_org_unit_rt_id
