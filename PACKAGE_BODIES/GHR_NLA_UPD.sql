--------------------------------------------------------
--  DDL for Package Body GHR_NLA_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GHR_NLA_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ghr_nla_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out NOCOPY ghr_nla_shd.g_rec_type) is
--
  l_proc  varchar2(72);
--
...
Begin
  ...
  --
  --
  -- Update the ghr_noac_las Row
  --
  update ghr_noac_las
  set
  noac_la_id                        = p_rec.noac_la_id,
  nature_of_action_id               = p_rec.nature_of_action_id,
  lac_lookup_code                   = p_rec.lac_lookup_code,
  enabled_flag                      = p_rec.enabled_flag,
  date_from                         = p_rec.date_from,
  date_to                           = p_rec.date_to,
  object_version_number             = p_rec.object_version_number,
  valid_first_lac_flag              = p_rec.valid_first_lac_flag,
  valid_second_lac_flag             = p_rec.valid_second_lac_flag
  where noac_la_id = p_rec.noac_la_id;
  --
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ghr_nla_shd.g_rec_type) is
--
  l_proc  varchar2(72) ;
--
Begin
...
--
 ...
 ,p_nature_of_action_id_o         =>ghr_nla_shd.g_old_rec.nature_of_action_id
 ,p_lac_lookup_code_o             =>ghr_nla_shd.g_old_rec.lac_lookup_code
 ,p_enabled_flag_o                =>ghr_nla_shd.g_old_rec.enabled_flag
 ,p_date_from_o                   =>ghr_nla_shd.g_old_rec.date_from
 ,p_date_to_o                     =>ghr_nla_shd.g_old_rec.date_to
 ,p_object_version_number_o       =>ghr_nla_shd.g_old_rec.object_version_number
 ,p_valid_first_lac_flag_o        =>ghr_nla_shd.g_old_rec.valid_first_lac_flag
 ,p_valid_second_lac_flag_o       =>ghr_nla_shd.g_old_rec.valid_second_lac_flag
      );
    --
  exception
    --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out NOCOPY ghr_nla_shd.g_rec_type) is
--
  l_proc  varchar2(72);
--
  l_rec ghr_nla_shd.g_rec_type;
Begin
  ...
  If (p_rec.nature_of_action_id = hr_api.g_number) then
    ...
    ghr_nla_shd.g_old_rec.nature_of_action_id;
  End If;
  If (p_rec.lac_lookup_code = hr_api.g_varchar2) then
    p_rec.lac_lookup_code :=
    ghr_nla_shd.g_old_rec.lac_lookup_code;
  End If;
  If (p_rec.enabled_flag = hr_api.g_varchar2) then
    p_rec.enabled_flag :=
    ghr_nla_shd.g_old_rec.enabled_flag;
  End If;
  If (p_rec.date_from = hr_api.g_date) then
    p_rec.date_from :=
    ghr_nla_shd.g_old_rec.date_from;
  End If;
  If (p_rec.date_to = hr_api.g_date) then
    p_rec.date_to :=
    ghr_nla_shd.g_old_rec.date_to;
  End If;
  If (p_rec.valid_first_lac_flag = hr_api.g_varchar2) then
    p_rec.valid_first_lac_flag :=
    ghr_nla_shd.g_old_rec.valid_first_lac_flag;
  End If;
  If (p_rec.valid_second_lac_flag = hr_api.g_varchar2) then
    p_rec.valid_second_lac_flag :=
    ghr_nla_shd.g_old_rec.valid_second_lac_flag;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  ghr_nla_bus.update_validate(p_rec
  ,p_effective_date);
  --
