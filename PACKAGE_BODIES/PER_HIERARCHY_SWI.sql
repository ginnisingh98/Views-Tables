--------------------------------------------------------
--  DDL for Package Body PER_HIERARCHY_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_HIERARCHY_SWI" As
/* $Header: pepghswi.pkb 115.0 2003/09/25 04:00 krmahade noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'per_hierarchy_swi.';
--
-- ----------------------------------------------------------------------------
-- |---------------------------< create_hierarchy >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_hierarchy_swi;
  --
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_hierarchy_swi;
    --
...
PROCEDURE delete_hierarchy
  ...
  --
  -- Other variables
  l_proc    varchar2(72) := g_package ||'delete_hierarchy';
Begin
  hr_utility.set_location(' Entering:' || l_proc,10);
  --
  -- Issue a savepoint
  --
  savepoint delete_hierarchy_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_hierarchy_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_object_version_number        := l_object_version_number;
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint update_hierarchy_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  per_hierarchy_api.update_hierarchy
    (p_validate                     => l_validate
    ,p_hierarchy_id                 => p_hierarchy_id
    ,p_name                         => p_name
    ,p_request_id                   => p_request_id
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_hierarchy_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_object_version_number        := l_object_version_number;
