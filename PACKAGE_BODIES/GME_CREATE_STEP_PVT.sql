--------------------------------------------------------
--  DDL for Package Body GME_CREATE_STEP_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_CREATE_STEP_PVT" AS
   /*  $Header: GMEVCRSB.pls 120.16.12010000.4 2010/03/04 17:01:33 gmurator ship $ */
...
-- if scale_type = 0, then let's leave the plan_rsrc_qty to that passed in by GMD... scale_type = 0 is fixed
   ...
   BEGIN
      ...
      ELSE
         ...
         l_cont_ind := 1;
                         --Assigning hard coded value now have to use GMD api
      END IF;
      ...
      IF (p_step_id IS NOT NULL) THEN
         ...
         IF (p_plan_start_date IS NULL) AND (p_plan_cmplt_date IS NOT NULL) THEN
            ...
            IF l_use_workday_cal = fnd_api.g_true THEN
               IF (NVL (g_debug, -1) = gme_debug.g_log_statement) THEN
                  gme_debug.put_line (   'plan_cmplt date is not null'
                                      || TO_CHAR (p_plan_cmplt_date
                                                 ,'DD-MON-YYYY HH24:MI:SS') );
               END IF;
               ...
               gmp_calendar_api.get_contiguous_periods
                                ...
                                ,p_duration           => l_step_duration_tab
                                                                    (x_step_no)
                                ,p_output_tbl         => l_contig_period_tbl
                                ,x_return_status      => l_return_status);
      ...
      ELSE
         ...
         ELSIF     (p_gme_batch_header_rec.plan_start_date IS NULL)
            ...
            IF l_use_workday_cal = fnd_api.g_true THEN
               IF (NVL (g_debug, -1) = gme_debug.g_log_statement) THEN
                  gme_debug.put_line (   ' both dates null '
            ...
            FOR k IN x_rsrc_tab.FIRST .. x_rsrc_tab.LAST LOOP
               ...
               IF p_gme_batch_header_rec.update_inventory_ind = 'Y' THEN
                  FOR m IN 1 .. l_cal_count LOOP
                     ...
                     x_rsrc_tab (k).plan_cmplt_date :=
                                             l_contig_period_tbl (m).end_date;
...
BEGIN
  for curnode in (select d.dep_step_id, dep_type,standard_delay,s.batchstep_no from gme_batch_step_dependencies d, gme_batch_steps s
                  where s.batchstep_id = d.batchstep_id and d.batchstep_id = node)
  loop
...
END get_longest_in_branch;
      ...
      FOR step_activity IN cur_get_activity (v_step_id, v_batch_id) LOOP
         ...
         FOR step_resource IN
            ...
            IF step_resource.usage_um = l_hour_um THEN
               x_usage_hour := step_resource.plan_rsrc_usage;
            ELSE
               get_usage_in_hours (step_resource.plan_rsrc_usage
   ...
   BEGIN
      ...
      -- This is not the full fix which we may someday need.
      IF (l_txn_usage <= 0) THEN
         l_txn_usage :=
