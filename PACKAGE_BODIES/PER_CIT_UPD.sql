--------------------------------------------------------
--  DDL for Package Body PER_CIT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CIT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_cit_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the per_cagr_entitlement_items_tl Row
  --
  update per_cagr_entitlement_items_tl
    set
...
Begin
  ...
  --
  convert_defs(p_rec);
  per_cit_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  per_cit_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  per_cit_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  per_cit_upd.post_update
     (p_rec
     );
End upd;
--
  ...
  cursor csr_upd_langs is
    select cit.language
     ...
     where cit.cagr_entitlement_item_id = p_cagr_entitlement_item_id
       and p_language_code in (cit.language
                              ,cit.source_lang);
  --
