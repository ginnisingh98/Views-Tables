--------------------------------------------------------
--  DDL for Package Body IGS_EN_MRG_ID_ACT_CH_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_EN_MRG_ID_ACT_CH_PKG" AS
/* $Header: IGSEI27B.pls 115.3 2002/11/28 23:38:26 nsidana ship $ */


  l_rowid VARCHAR2(25);
  old_references IGS_EN_MRG_ID_ACT_CH%RowType;
  new_references IGS_EN_MRG_ID_ACT_CH%RowType;
...
IGS_GE_MSG_STACK.ADD;
    ...
    IF (((old_references.smir_id = new_references.smir_id)) OR
        ((new_references.smir_id IS NULL))) THEN
      NULL;
    ELSE
       IF NOT IGS_EN_MERGE_ID_ROWS_PKG.Get_PK_For_Validation (
...
IGS_GE_MSG_STACK.ADD;
 ...
 FUNCTION Get_PK_For_Validation (
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_EN_SMIAC_SMIA_FK');
IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_EN_SMIAC_SMIR_FK');
IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  END GET_FK_IGS_EN_MERGE_ID_ROWS;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_EN_MRG_ID_ACT_CH
      where SMIR_ID = X_SMIR_ID
      and TABLE_ALIAS = X_TABLE_ALIAS
      and ACTION_ID = X_ACTION_ID;
...
IGS_GE_MSG_STACK.ADD;
  ...
  insert into IGS_EN_MRG_ID_ACT_CH (
    SMIR_ID,
    TABLE_ALIAS,
    ACTION_ID,
    PERFORM_ACTION_IND,
...
IGS_GE_MSG_STACK.ADD;
  ...
  if ( (tlinfo.PERFORM_ACTION_IND = X_PERFORM_ACTION_IND)
  ) then
    null;
  else
...
IGS_GE_MSG_STACK.ADD;
  ...
  update IGS_EN_MRG_ID_ACT_CH set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_EN_MRG_ID_ACT_CH
     where SMIR_ID = X_SMIR_ID
     and TABLE_ALIAS = X_TABLE_ALIAS
     and ACTION_ID = X_ACTION_ID
  ;
...
begin
  ...
  delete from IGS_EN_MRG_ID_ACT_CH
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
