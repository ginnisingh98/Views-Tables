--------------------------------------------------------
--  DDL for Package Body HR_COMPETENCE_ELEMENT_BE5
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_COMPETENCE_ELEMENT_BE5" as 
...
p_competence_created           number) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  hr_competence_element_be5.maintain_student_comp_elemen_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.competence_element.maintain_student_comp_elemen';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<competence_created>';
    l_text:=l_text||fnd_number.number_to_canonical(p_competence_created);
    l_text:=l_text||'</competence_created>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</competence_element>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
