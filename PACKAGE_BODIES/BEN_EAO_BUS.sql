--------------------------------------------------------
--  DDL for Package Body BEN_EAO_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EAO_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_eao_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_enrld_anthr_oipl_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   enrld_anthr_oipl_rt_id PK of record being inserted or updated.
...
--
Procedure chk_enrld_anthr_oipl_rt_id(p_enrld_anthr_oipl_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_enrld_anthr_oipl_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_eao_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_enrld_anthr_oipl_rt_id                => p_enrld_anthr_oipl_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_enrld_anthr_oipl_rt_id,hr_api.g_number)
     <>  ben_eao_shd.g_old_rec.enrld_anthr_oipl_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_eao_shd.constraint_error('BEN_ENRLD_ANTHR_OIPL_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_enrld_anthr_oipl_rt_id is not null then
      --
  ...
  --
End chk_enrld_anthr_oipl_rt_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_oipl_id >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_oipl_id (p_enrld_anthr_oipl_rt_id in number,
                       p_oipl_id                  in number,
  ...
  cursor c3 is
     select null
       ...
       where  eao.oipl_id = p_oipl_id
       and eao.vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
       and eao.enrld_anthr_oipl_rt_id <> nvl(p_enrld_anthr_oipl_rt_id,hr_api.g_number)

       and eao.business_group_id + 0 = p_business_group_id
       and p_validation_start_date <= eao.effective_end_date
       and p_validation_end_date >= eao.effective_start_date;
--
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  ...
  if (l_api_updating
     ...
     <> nvl(ben_eao_shd.g_old_rec.oipl_id,hr_api.g_number)
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_eao_shd.constraint_error('BEN_ENRLD_ANTHR_OIPL_RT_DT1');
        --
      end if;
      --
    close c1;
  ...
  --
     ...
     fnd_message.set_token('VAR1','Enrolled in Another Option in Plan criteria');
...
--   Internal table handler use only.
--
Procedure chk_enrl_det_dt_cd(p_enrld_anthr_oipl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_eao_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_enrld_anthr_oipl_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_eao_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and enrld_anthr_oipl_rt_id   <> nvl(p_enrld_anthr_oipl_rt_id  ,-1)
		   -- and oipl_id   <> nvl(p_oipl_id  ,-1)
...
function return_legislation_code
  (p_enrld_anthr_oipl_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_enrld_anthr_oipl_rt_f b
    where b.enrld_anthr_oipl_rt_id      = p_enrld_anthr_oipl_rt_id
