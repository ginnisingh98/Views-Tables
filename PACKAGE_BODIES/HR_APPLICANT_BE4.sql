--------------------------------------------------------
--  DDL for Package Body HR_APPLICANT_BE4
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_APPLICANT_BE4" as 
...
p_appl_override_warning        boolean) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  hr_applicant_be4.convert_to_applicant_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.applicant.convert_to_applicant';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<appl_override_warning>';
if(P_APPL_OVERRIDE_WARNING) then
l_text:=l_text||'TRUE';
else
l_text:=l_text||'FALSE';
end if;
    l_text:=l_text||'</appl_override_warning>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='</applicant>';
    --
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
