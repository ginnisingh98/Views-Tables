--------------------------------------------------------
--  DDL for Package Body ICX_CATG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ICX_CATG" AS
/* $Header: ICXSTDCB.pls 115.2 99/07/17 03:26:58 porting ship $ */
...
BEGIN
  ...
  IF icx_sec.validateSession THEN
    ...
    l_region_code  := 'ICX_CATG_DETLS_LOAD_R';
   ...
   IF l_total_rows = 0 THEN
      ...
      icx_admin_sig.toolbar(language_code => v_language_code,
                            disp_find => 'icx_store_catg_templates.find_categories');
...
',cattributes => 'VALIGN="MIDDLE"');
   ...
   ELSE
      ...
      FND_MESSAGE.SET_NAME('ICX','ICX_CATG_XL_LINE1');
      htp.p(FND_MESSAGE.GET);
      FND_MESSAGE.SET_NAME('ICX','ICX_CATG_XL_LINE2');
      FND_MESSAGE.SET_TOKEN('ICX_DATE_FORMAT', l_date_format);
      htp.p(FND_MESSAGE.GET);
      FND_MESSAGE.SET_NAME('ICX','ICX_CATG_XL_LINE3');
      htp.p(FND_MESSAGE.GET);
      htp.p;
      ...
      FOR i IN ak_query_pkg.g_items_table.FIRST..ak_query_pkg.g_items_table.LAST LOOP
        IF (ak_query_pkg.g_items_table(i).secured_column = 'F'
           AND ak_query_pkg.g_items_table(i).node_display_flag = 'Y' )
             OR (ak_query_pkg.g_items_table(i).attribute_code = 'ICX_CATEGORY_ID') THEN
