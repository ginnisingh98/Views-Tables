--------------------------------------------------------
--  DDL for Package Body HR_FIT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_FIT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_fit_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Procedure update_dml
  (p_rec in out nocopy hr_fit_shd.g_rec_type
  ) is
--
...
Begin
  ...
  --
  --
  -- Update the hr_form_items_tl Row
  --
  update hr_form_items_tl
    set
     form_item_id                    = p_rec.form_item_id
    ,language                        = p_rec.language
    ,source_lang                     = p_rec.source_lang
    ,user_item_name                  = p_rec.user_item_name
...
Procedure pre_update
  (p_rec in hr_fit_shd.g_rec_type
  ) is
--
...
Begin
  ...
  begin
    ...
    hr_fit_rku.after_update
      ...
      => p_rec.source_lang
      ,p_user_item_name
      => p_rec.user_item_name
      ,p_description
      => p_rec.description
      ,p_source_lang_o
      => hr_fit_shd.g_old_rec.source_lang
      ,p_user_item_name_o
      => hr_fit_shd.g_old_rec.user_item_name
      ,p_description_o
      => hr_fit_shd.g_old_rec.description
      );
    --
  exception
    --
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'HR_FORM_ITEMS_TL'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    ...
    hr_fit_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.user_item_name = hr_api.g_varchar2) then
    p_rec.user_item_name :=
    hr_fit_shd.g_old_rec.user_item_name;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  hr_fit_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hr_fit_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_fit_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_fit_upd.post_update
     (p_rec
     );
End upd;
--
...
Procedure upd_tl
  ...
  cursor csr_upd_langs is
    select fit.language
     ...
     where fit.form_item_id = p_form_item_id
       and p_language_code in (fit.language
                              ,fit.source_lang);
  --
