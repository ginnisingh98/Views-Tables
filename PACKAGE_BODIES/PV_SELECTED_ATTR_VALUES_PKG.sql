--------------------------------------------------------
--  DDL for Package Body PV_SELECTED_ATTR_VALUES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PV_SELECTED_ATTR_VALUES_PKG" as
/* $Header: pvrtsavb.pls 115.3 2003/09/24 22:08:45 pklin ship $ */
-- Start of Comments
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'PV_SELECTED_ATTR_VALUES_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'pvrtsavb.pls';
...
PROCEDURE Insert_Row(
 ...
 IS
   CURSOR C2 IS SELECT PV_SELECTED_ATTR_VALUES_S.nextval FROM sys.dual;
BEGIN
   If (px_ATTR_VALUE_ID IS NULL) OR (px_ATTR_VALUE_ID = FND_API.G_MISS_NUM) then
       OPEN C2;
       FETCH C2 INTO px_ATTR_VALUE_ID;
       CLOSE C2;
   End If;
   INSERT INTO PV_SELECTED_ATTR_VALUES(
           ATTR_VALUE_ID
          ,LAST_UPDATE_DATE
          ,LAST_UPDATED_BY
          ,CREATION_DATE
	  ...
	  ,SCORE
          ...
          ,decode( p_SELECTION_CRITERIA_ID, FND_API.G_MISS_NUM, NULL, p_SELECTION_CRITERIA_ID)
...
PROCEDURE Update_Row(
 ...
 BEGIN
    ...
    SET
             ...
             ,SELECTION_CRITERIA_ID = decode( p_SELECTION_CRITERIA_ID, FND_API.G_MISS_NUM, SELECTION_CRITERIA_ID, p_SELECTION_CRITERIA_ID)
    ...
    where ATTR_VALUE_ID = p_ATTR_VALUE_ID;
...
PROCEDURE Delete_Row(
 ...
 IS
 BEGIN
   DELETE FROM PV_SELECTED_ATTR_VALUES
    WHERE ATTR_VALUE_ID = p_ATTR_VALUE_ID;
   If (SQL%NOTFOUND) then
       RAISE NO_DATA_FOUND;
   End If;
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
        ...
        WHERE ATTR_VALUE_ID =  p_ATTR_VALUE_ID
        FOR UPDATE of ATTR_VALUE_ID NOWAIT;
   Recinfo C%ROWTYPE;
 BEGIN
    OPEN C;
    ...
    CLOSE C;
    if (
           (      Recinfo.ATTR_VALUE_ID = p_ATTR_VALUE_ID)
       ...
       AND (    ( Recinfo.SELECTION_CRITERIA_ID = p_SELECTION_CRITERIA_ID)
            OR (    ( Recinfo.SELECTION_CRITERIA_ID IS NULL )
                AND (  p_SELECTION_CRITERIA_ID IS NULL )))
