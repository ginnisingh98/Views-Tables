--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_REF_HIST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_REF_HIST_PKG" as
/* $Header: IGSPI89B.pls 115.7 2002/11/29 02:41:17 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_UNIT_REF_HIST_ALL%RowType;
  new_references IGS_PS_UNIT_REF_HIST_ALL%RowType;
...
IF (cur_rowid%FOUND) THEN
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_URCH_RCT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
	Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_URCH_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID in NUMBER
    ...
    cursor C is select ROWID from IGS_PS_UNIT_REF_HIST_ALL
      where UNIT_CD = X_UNIT_CD
      and HIST_START_DT = X_HIST_START_DT
      and REFERENCE_CD = X_REFERENCE_CD
      and VERSION_NUMBER = X_VERSION_NUMBER
...
begin
  ...
  insert into IGS_PS_UNIT_REF_HIST_ALL (
    UNIT_CD,
    VERSION_NUMBER,
    REFERENCE_CD_TYPE,
    REFERENCE_CD,
...
begin
  ...
  update IGS_PS_UNIT_REF_HIST_ALL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_UNIT_REF_HIST_ALL
     where UNIT_CD = X_UNIT_CD
     and HIST_START_DT = X_HIST_START_DT
     and REFERENCE_CD = X_REFERENCE_CD
     and VERSION_NUMBER = X_VERSION_NUMBER
...
begin
  ...
  delete from IGS_PS_UNIT_REF_HIST_ALL
  where ROWID = X_ROWID
  ;
  if (sql%notfound) then
    raise no_data_found;
