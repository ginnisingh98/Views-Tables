--------------------------------------------------------
--  DDL for Package Body BEN_BDI_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BDI_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_bdi_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_bdi_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_batch_dpnt_info Row
  --
  update ben_batch_dpnt_info
  set
  batch_dpnt_id                     = p_rec.batch_dpnt_id,
  benefit_action_id                 = p_rec.benefit_action_id,
  person_id                         = p_rec.person_id,
  pgm_id                            = p_rec.pgm_id,
  pl_id                             = p_rec.pl_id,
  oipl_id                           = p_rec.oipl_id,
  contact_typ_cd                    = p_rec.contact_typ_cd,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_bdi_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_bdi_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.benefit_action_id = hr_api.g_number) then
    ...
    ben_bdi_shd.g_old_rec.benefit_action_id;
  End If;
  If (p_rec.person_id = hr_api.g_number) then
    p_rec.person_id :=
    ben_bdi_shd.g_old_rec.person_id;
  End If;
  If (p_rec.pgm_id = hr_api.g_number) then
    p_rec.pgm_id :=
    ben_bdi_shd.g_old_rec.pgm_id;
  End If;
  If (p_rec.pl_id = hr_api.g_number) then
    p_rec.pl_id :=
    ben_bdi_shd.g_old_rec.pl_id;
  End If;
  If (p_rec.oipl_id = hr_api.g_number) then
    p_rec.oipl_id :=
    ben_bdi_shd.g_old_rec.oipl_id;
  End If;
  If (p_rec.contact_typ_cd = hr_api.g_varchar2) then
    p_rec.contact_typ_cd :=
    ben_bdi_shd.g_old_rec.contact_typ_cd;
  End If;
  If (p_rec.dpnt_person_id = hr_api.g_number) then
    p_rec.dpnt_person_id :=
    ben_bdi_shd.g_old_rec.dpnt_person_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_bdi_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.enrt_cvg_strt_dt = hr_api.g_date) then
    p_rec.enrt_cvg_strt_dt :=
    ben_bdi_shd.g_old_rec.enrt_cvg_strt_dt;
  End If;
  If (p_rec.enrt_cvg_thru_dt = hr_api.g_date) then
    p_rec.enrt_cvg_thru_dt :=
    ben_bdi_shd.g_old_rec.enrt_cvg_thru_dt;
  End If;
  If (p_rec.actn_cd = hr_api.g_varchar2) then
    p_rec.actn_cd :=
    ben_bdi_shd.g_old_rec.actn_cd;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_bdi_bus.update_validate(p_rec,p_effective_date);
  --
