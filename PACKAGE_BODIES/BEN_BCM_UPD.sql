--------------------------------------------------------
--  DDL for Package Body BEN_BCM_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BCM_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ben_bcm_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the ben_cwb_matrix Row
  --
  update ben_cwb_matrix
    set
     cwb_matrix_id                   = p_rec.cwb_matrix_id
    ,name                            = p_rec.name
    ,date_saved                      = p_rec.date_saved
    ,plan_id                         = p_rec.plan_id
    ,matrix_typ_cd                   = p_rec.matrix_typ_cd
    ,person_id                       = p_rec.person_id
    ,row_crit_cd                     = p_rec.row_crit_cd
    ,col_crit_cd                     = p_rec.col_crit_cd
    ,alct_by_cd                      = p_rec.alct_by_cd
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    ...
    ben_bcm_shd.g_old_rec.name;
  End If;
  If (p_rec.date_saved = hr_api.g_date) then
    p_rec.date_saved :=
    ben_bcm_shd.g_old_rec.date_saved;
  End If;
  If (p_rec.plan_id = hr_api.g_number) then
    ...
    ben_bcm_shd.g_old_rec.plan_id;
  End If;
  If (p_rec.matrix_typ_cd = hr_api.g_varchar2) then
    p_rec.matrix_typ_cd :=
    ben_bcm_shd.g_old_rec.matrix_typ_cd;
  End If;
  If (p_rec.person_id = hr_api.g_number) then
    ...
    ben_bcm_shd.g_old_rec.person_id;
  End If;
  If (p_rec.row_crit_cd = hr_api.g_varchar2) then
    p_rec.row_crit_cd :=
    ben_bcm_shd.g_old_rec.row_crit_cd;
  End If;
  If (p_rec.col_crit_cd = hr_api.g_varchar2) then
    p_rec.col_crit_cd :=
    ben_bcm_shd.g_old_rec.col_crit_cd;
  End If;
  If (p_rec.alct_by_cd = hr_api.g_varchar2) then
    p_rec.alct_by_cd :=
    ben_bcm_shd.g_old_rec.alct_by_cd;
  End If;
...
Begin
  ...
  --
  ben_bcm_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  ben_bcm_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  ben_bcm_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Procedure upd
  ...
  ,p_date_saved                   in     date      default hr_api.g_date
