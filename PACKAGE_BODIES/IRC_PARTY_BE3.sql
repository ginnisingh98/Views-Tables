--------------------------------------------------------
--  DDL for Package Body IRC_PARTY_BE3
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IRC_PARTY_BE3" as 
...
p_asg_object_version_number    number) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  irc_party_be3.registered_user_application_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.irc.api.party.registered_user_application';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<application_received_date>';
    l_text:=l_text||fnd_date.date_to_canonical(p_application_received_date);
    l_text:=l_text||'</application_received_date>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<vacancy_id>';
