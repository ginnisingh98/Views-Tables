--------------------------------------------------------
--  DDL for Package Body HZ_FIN_NUMBERS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HZ_FIN_NUMBERS_EXT_PKG" as
/* $Header: ARHEXFNB.pls 120.4 2006/02/09 10:07:11 jhuang noship $ */
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from HZ_FIN_NUMBERS_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into HZ_FIN_NUMBERS_EXT_B (
    EXTENSION_ID,
    FINANCIAL_NUMBER_ID,
    ATTR_GROUP_ID,
    C_EXT_ATTR1,
  ...
  insert into HZ_FIN_NUMBERS_EXT_TL (
    TL_EXT_ATTR15,
    TL_EXT_ATTR16,
    TL_EXT_ATTR17,
    TL_EXT_ATTR18,
...
begin
  ...
  close c;
  if (    (recinfo.FINANCIAL_NUMBER_ID = X_FINANCIAL_NUMBER_ID)
...
begin
  update HZ_FIN_NUMBERS_EXT_B set
    FINANCIAL_NUMBER_ID = X_FINANCIAL_NUMBER_ID,
  ...
  update HZ_FIN_NUMBERS_EXT_TL set
...
begin
  delete from HZ_FIN_NUMBERS_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from HZ_FIN_NUMBERS_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  insert /*+ append parallel(tt) */ into HZ_FIN_NUMBERS_EXT_TL tt (
    TL_EXT_ATTR15,
    TL_EXT_ATTR16,
    TL_EXT_ATTR17,
    TL_EXT_ATTR18,
  ...
  (SELECT /*+ no_merge ordered parallel(b) */
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
   FROM HZ_FIN_NUMBERS_EXT_TL B ,
        FND_LANGUAGES L
   WHERE L.INSTALLED_FLAG IN ( 'I','B' )
   AND B.LANGUAGE = USERENV ( 'LANG' )
  ) v, HZ_FIN_NUMBERS_EXT_TL t
