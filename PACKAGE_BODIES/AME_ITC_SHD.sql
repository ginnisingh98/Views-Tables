--------------------------------------------------------
--  DDL for Package Body AME_ITC_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_ITC_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_itc_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_ITEM_CLASSES_PK') Then
...
Begin
  ...
  Else
    If (p_item_class_id =
        ame_itc_shd.g_old_rec.item_class_id and
        p_object_version_number =
        ame_itc_shd.g_old_rec.object_version_number
) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_itc_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ame_itc_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_itc_shd.g_old_rec.end_date;
