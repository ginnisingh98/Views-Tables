--------------------------------------------------------
--  DDL for Package Body BEN_PBM_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PBM_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pbm_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_pl_r_oipl_prem_by_mo_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   pl_r_oipl_prem_by_mo_id PK of record being inserted or updated.
...
--
Procedure chk_pl_r_oipl_prem_by_mo_id(p_pl_r_oipl_prem_by_mo_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pl_r_oipl_prem_by_mo_id';
...
Begin
  ...
  --
  l_api_updating := ben_pbm_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pl_r_oipl_prem_by_mo_id                => p_pl_r_oipl_prem_by_mo_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pl_r_oipl_prem_by_mo_id,hr_api.g_number)
     <>  ben_pbm_shd.g_old_rec.pl_r_oipl_prem_by_mo_id) then
    --
    -- raise error as PK has changed
    --
    ben_pbm_shd.constraint_error('BEN_PL_R_OIPL_PREM_BY_MO_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pl_r_oipl_prem_by_mo_id is not null then
      --
  ...
  --
End chk_pl_r_oipl_prem_by_mo_id;
--
-- ----------------------------------------------------------------------------
-- |------< chk_mnl_adj_flag >------|
-- ----------------------------------------------------------------------------
...
--   Internal table handler use only.
--
Procedure chk_mnl_adj_flag(p_pl_r_oipl_prem_by_mo_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_mnl_adj_flag
      <> nvl(ben_pbm_shd.g_old_rec.mnl_adj_flag,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_uom(p_pl_r_oipl_prem_by_mo_id                in number,
                  p_uom                         in varchar2,
                  p_effective_date              in date,
                  p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_pbm_shd.g_old_rec.uom,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
function return_legislation_code
  (p_pl_r_oipl_prem_by_mo_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pl_r_oipl_prem_by_mo_f b
    where b.pl_r_oipl_prem_by_mo_id      = p_pl_r_oipl_prem_by_mo_id
