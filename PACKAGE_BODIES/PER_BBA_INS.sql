--------------------------------------------------------
--  DDL for Package Body PER_BBA_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_BBA_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  per_bba_ins.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< insert_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure insert_dml(p_rec in out nocopy per_bba_shd.g_rec_type) is
--
...
Begin
  ...
  --
  --
  -- Insert the row into: per_bf_balance_amounts
  --
  insert into per_bf_balance_amounts
  (      balance_amount_id,
      balance_type_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_insert(p_rec  in out nocopy per_bba_shd.g_rec_type) is
--
  l_proc  varchar2(72) := g_package||'pre_insert';
--
Cursor C_Sel1 is select per_bf_balance_amounts_s.nextval from sys.dual;
--
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.balance_amount_id;
  Close C_Sel1;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
...
Begin
  ...
  --
  --
  per_bba_bus.insert_validate(p_effective_date,
                             p_rec
                             );
  --
...
Begin
  ...
  --
  -- Having converted the arguments into the per_bba_rec
  ...
  --
  p_balance_amount_id := l_rec.balance_amount_id;
