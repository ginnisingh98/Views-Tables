--------------------------------------------------------
--  DDL for Package Body PAY_PPE_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_PPE_BUS" as
/* $Header: pypperhi.pkb 115.2 2000/10/10 04:48:04 pkm ship        $ */
--
-- ----------------------------------------------------------------------------
-- |                     Private Global Definitions                           |
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_ppe_bus.';  -- Global package name
--
...
Begin
  ...
  --
  if ( nvl(pay_ppe_bus.g_process_event_id, hr_api.g_number)
    ...
    -- variable.
    --
    l_legislation_code := pay_ppe_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_ppe_bus.g_process_event_id  := p_process_event_id;
    pay_ppe_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_ppe_shd.api_updating
  ...
  if nvl(p_rec.assignment_id, hr_api.g_number) <>
     nvl(pay_ppe_shd.g_old_rec.assignment_id, hr_api.g_number)
  then
  ...
  if nvl(p_rec.effective_date, hr_api.g_date) <>
     nvl(pay_ppe_shd.g_old_rec.effective_date, hr_api.g_date)
  then
  ...
  if nvl(p_rec.change_type, hr_api.g_varchar2) <>
     nvl(pay_ppe_shd.g_old_rec.change_type, hr_api.g_varchar2)
  then
...
--
-- Description:
--   This procedure is used to ensure that the assignment_id exists in
--   PER_ALL_ASSIGNMENTS_F for the given effective date
--
-- Pre Conditions:
...
--
-- Post Success:
--   Processing continues if the assignment_id exists
--
-- Post Failure:
--   An application error is raised if any of the change_type is not
--   valid
...
Begin
  ...
  open csr_assignment_exists;
    fetch csr_assignment_exists into l_assignment_exists;
    --
    if csr_assignment_exists%notfound then
      --
      -- The assignment_id for this record is does not exist for the effective date
      --
      close csr_assignment_exists;
      fnd_message.set_name('PAY','HR_xxxx_INVALID_ASSIGNMENT_ID');
      fnd_message.raise_error;
    end if;
    hr_utility.set_location(l_proc,30);
    --
...
Begin
  ...
  --
  -- dont have PK at this point on insert
  -- pay_ppe_bus.set_security_group_id
  --  (p_process_event_id => p_rec.process_event_id
  --  );
  --
  --
