--------------------------------------------------------
--  DDL for Package Body IGS_AD_SBMPS_FN_ITTT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_SBMPS_FN_ITTT_PKG" as
/* $Header: IGSAI61B.pls 115.5 2003/10/30 13:21:12 rghosh ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_AD_SBMPS_FN_ITTT%RowType;
  new_references IGS_AD_SBMPS_FN_ITTT%RowType;
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_scfit_br_iu
  -- BEFORE INSERT OR UPDATE
  -- ON IGS_AD_SBMPS_FN_ITTT
  -- FOR EACH ROW
...
PROCEDURE Check_Parent_Existance as
  BEGIN
    ...
    IF (((old_references.submission_yr = new_references.submission_yr) AND
         ...
         (old_references.funding_source = new_references.funding_source) AND
        ...
        ((new_references.submission_yr IS NULL) OR
         ...
         (new_references.funding_source IS NULL) OR
         (new_references.sequence_number IS NULL))) THEN
      NULL;
    ELSE
      IF NOT IGS_AD_SBM_PS_FNTRGT_PKG.Get_PK_For_Validation (
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SCFIT_SCFT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  END GET_FK_IGS_AD_SBM_PS_FNTRGT;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SCFIT_ITT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_SBMPS_FN_ITTT
      where SUBMISSION_YR = X_SUBMISSION_YR
      and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
      and COURSE_CD = X_COURSE_CD
      and CRV_VERSION_NUMBER = X_CRV_VERSION_NUMBER
...
begin
  ...
  Before_DML (
   ...
   insert into IGS_AD_SBMPS_FN_ITTT (
    SUBMISSION_YR,
    SUBMISSION_NUMBER,
    COURSE_CD,
    CRV_VERSION_NUMBER,
...
begin
  ...
  update IGS_AD_SBMPS_FN_ITTT set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_SBMPS_FN_ITTT
     where SUBMISSION_YR = X_SUBMISSION_YR
     and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
     and COURSE_CD = X_COURSE_CD
     and CRV_VERSION_NUMBER = X_CRV_VERSION_NUMBER
...
begin
  ...
  delete from IGS_AD_SBMPS_FN_ITTT
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
