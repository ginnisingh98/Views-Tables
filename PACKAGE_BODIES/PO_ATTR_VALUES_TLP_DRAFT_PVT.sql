--------------------------------------------------------
--  DDL for Package Body PO_ATTR_VALUES_TLP_DRAFT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PO_ATTR_VALUES_TLP_DRAFT_PVT" AS
/* $Header: PO_ATTR_VALUES_TLP_DRAFT_PVT.plb 120.2 2005/07/26 16:22 bao noship $ */

d_pkg_name CONSTANT varchar2(50) :=
  PO_LOG.get_package_base('PO_ATTR_VALUES_TLP_DRAFT_PVT');
...
--  given the draft_id or draft_id + attribute_values_tlp_id
--    If only draft_id is provided, this program returns FND_API.G_TRUE for
--  any draft changes in this table for the draft
--    If the whole primary key is provided (draft_id + attribute_values_tlp id),
--  then it returns true if there is draft for this particular record in
--  the draft table
--Parameters:
--IN:
--p_draft_id_tbl
...
--  po attribute_values_tlp unique identifier
--IN OUT:
--OUT:
--Returns:
...
BEGIN
  ...
  FORALL i IN 1..p_draft_id_tbl.COUNT
    ...
    WHERE EXISTS (SELECT 1
                  ...
                  WHERE  PAVTD.draft_id = p_draft_id_tbl(i)
                  AND    PAVTD.attribute_values_tlp_id =
                           NVL(p_attribute_values_tlp_id_tbl(i),
...
--Function:
--    Checks whether there is any draft changes in the draft table
--  given the primary key or part of the primary key
--    If only draft_id is provided, this program returns FND_API.G_TRUE for
--  any draft changes in this table for the draft
--    If the whole primary key is provided (draft_id+attribute_values_tlp_id),
--  then it return true if there is draft for this particular record in
--  the draft table
--Parameters:
--IN:
--p_draft_id
...
--  attribute values tlp record unique identifier
--IN OUT:
--OUT:
--Returns:
--  FND_API.G_TRUE if there are draft changes
...
BEGIN
  ...
  IF (p_draft_info.attr_values_tlp_changed = FND_API.G_FALSE) THEN
    IF (PO_LOG.d_stmt) THEN
  ...
  d_position := 10;
  -- Merge Changes
  PO_ATTR_VALUES_TLP_DRAFT_PKG.merge_changes
...
END PO_ATTR_VALUES_TLP_DRAFT_PVT;
