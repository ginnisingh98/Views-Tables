--------------------------------------------------------
--  DDL for Package Body WIP_WPS_COMMON
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WIP_WPS_COMMON" AS
/* $Header: wipzcomb.pls 120.1 2005/08/26 12:10:42 seli noship $ */
...
BEGIN
  ...
  LOOP
     ...
     wip_wps_res_instance_avail.calc_ins_avail(p_organization_id,
						  p_dept_table(x),
						  p_resource_table(x),
						  p_simulation_set,
						  p_instance_table(x),
...
END get_DiscreteJob_Progress;
 ...
 l_old_completion_date DATE;
 l_old_duration NUMBER;

 l_new_start_date DATE;
 l_new_completion_date DATE;

 l_sched_seq_num NUMBER;
 ...
 BEGIN
     ...
     if( l_old_start_date = p_new_start_date and
          l_old_completion_date = p_new_completion_date ) then
          return;
     end if;
     ...
     if( l_sched_seq_num is null) then
       -- update op resource
     ...
     else  -- for simutaneous resources
       l_offset := p_new_start_date - l_old_start_date;
  --       yl_debug.dump('offset=' || l_offset || ' subgrpnum=' || l_sub_grp_num);
         -- update op resources
       update wip_operation_resources wor
     ...
     if( l_old_start_date > p_new_start_date or
         l_old_completion_date < p_new_completion_date ) then
     -- op need to be updated
       if( l_old_start_date < p_new_start_date ) then
         l_new_start_date := l_old_start_date;
       else
          l_new_start_date := p_new_start_date;
       end if;

       if( l_old_completion_date > p_new_completion_date ) then
         l_new_completion_date := l_old_completion_date;
       else
          l_new_completion_date := p_new_completion_date;
       end if;
...
--yl_debug.dump('Updating op resource...');
 ...
 Begin
  ...
  if (p_new_start_date is null or p_new_completion_date is null) then
    return;
  end if;

  if( p_entity_type = JOB_TYPE_DISCRETE ) then -- discrete
  ...
  elsif ( p_entity_type = JOB_TYPE_WSM ) then --osfm

    if(p_source = WSM_DATA_SOURCE_EXE ) then
      update_wip_op_resource(
        p_wip_entity_id,
        p_operation_seq_num,
        p_resource_seq_num,
 ...
 IS
   ...
   l_on_hand NUMBER := 0;
   sub_inv_option NUMBER;
 BEGIN

  sub_inv_option := FND_PROFILE.VALUE('WIP_REQUIREMENT_ATT_OPTION');

  if( sub_inv_option = 1 ) then -- net
    OPEN qoh_net_cur;
    FETCH qoh_net_cur INTO l_on_hand;
    CLOSE qoh_net_cur;
  elsif ( sub_inv_option = 2 ) then
    OPEN qoh_all_cur;
    FETCH qoh_all_cur INTO l_on_hand;
    CLOSE qoh_all_cur;
  end if;

  return l_on_hand;

 END get_component_on_hand;
