--------------------------------------------------------
--  DDL for Package Body FA_FASRCPVW_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FA_FASRCPVW_XMLP_PKG" AS
/* $Header: FASRCPVWB.pls 120.0.12010000.2 2009/07/22 12:19:05 gigupta ship $ */
...
into P_MASS_RECLASS_ID_1,P_REQUEST_ID_1
...
where request_id=P_CONC_REQUEST_ID;
  ...
  FUNCTION D_SAME_CATFORMULA(OLD_CATEGORY IN VARCHAR2) RETURN VARCHAR2 IS
  BEGIN
    IF (OLD_CATEGORY = C_NEW_CATEGORY) THEN
      RETURN ('*');
    END IF;
    RETURN (' ');
  END D_SAME_CATFORMULA;

  FUNCTION D_BASIC_RATEFORMULA(BASIC_RATE IN NUMBER) RETURN VARCHAR2 IS
  BEGIN
    IF (BASIC_RATE IS NOT NULL) THEN
      RETURN (SUBSTR(TO_CHAR(BASIC_RATE
                           ,'990D00')
                   ,2
                   ,6));
    END IF;
  ...
  BEGIN
    IF (ADJUSTED_RATE IS NOT NULL) THEN
      RETURN (SUBSTR(TO_CHAR(ADJUSTED_RATE
                           ,'990D00')
                   ,2
                   ,6));
    END IF;
  ...
  BEGIN
    IF (SALVAGE_VAL_PERCENTAGE IS NOT NULL) THEN
      RETURN (SUBSTR(TO_CHAR(SALVAGE_VAL_PERCENTAGE
                           ,'990D00')
                   ,2
                   ,6));
    END IF;
  ...
  BEGIN
    ...
    SET
      STATUS = 'PREVIEWED'
    WHERE MASS_RECLASS_ID = P_MASS_RECLASS_ID_1
      AND STATUS = 'PREVIEW';
    COMMIT;
    RETURN (C_COUNT);
  ...
  BEGIN
    ...
    FA_MASS_REC_UTILS_PKG.GET_SELECTION_CRITERIA(X_MASS_RECLASS_ID => P_MASS_RECLASS_ID_1
