--------------------------------------------------------
--  DDL for Package Body IGS_PE_PERS_UNT_EXCL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PE_PERS_UNT_EXCL_PKG" AS
  /* $Header: IGSNI33B.pls 120.1 2005/09/08 14:23:26 appldev noship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PE_PERS_UNT_EXCL%RowType;
  new_references IGS_PE_PERS_UNT_EXCL%RowType;
  ...
  BEGIN
    ...
    new_references.pue_start_dt := x_pue_start_dt;
...
v_message_name  varchar2(30);
  BEGIN
	...
	IF (new_references.pue_start_dt IS NOT NULL) AND
		(p_inserting OR (p_updating AND
		(NVL(old_references.pue_start_dt, IGS_GE_DATE.IGSDATE('1900/01/01'))
		<> new_references.pue_start_dt)))
		THEN
	...
	IF p_inserting THEN
		...
		END IF;
	END IF;
	-- Validate if IGS_PE_PERSON is enrolled in the IGS_PS_UNIT which is to be excluded.  If so,
	-- raise an error as this is not allowable.
	IF p_inserting THEN
		IF IGS_EN_VAL_PUE.enrp_val_pue_unit (
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_pue_ar_iu
  -- AFTER INSERT OR UPDATE
  -- ON IGS_PE_PERS_UNT_EXCL
  -- FOR EACH ROW
  ...
  BEGIN
	-- Validate for open ended IGS_PE_PERSON IGS_PS_UNIT exclusion records.
	IF new_references.expiry_dt IS NULL THEN
  		-- Validate for open ended IGS_PE_PERS_UNT_EXCL records.
  		IF new_references.expiry_dt IS NULL THEN
  			IF IGS_EN_VAL_PUE.enrp_val_pue_open (
  					new_references.person_id,
 ...
 END IF;
  ...
  FUNCTION Get_PK_For_Validation (
    ...
    CURSOR cur_rowid IS
      ...
      AND      pue_start_dt = x_pue_start_dt
      FOR UPDATE NOWAIT;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PUE_PEE_FK');
       IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
  ...
  BEGIN
    ...
    Fetch cur_rowid INTO lv_rowid;
    IF (cur_rowid%FOUND) THEN
      Fnd_Message.Set_Name ('IGS', 'IGS_PE_PUE_UN_FK');
       IGS_GE_MSG_STACK.ADD;
      Close cur_rowid;
      App_Exception.Raise_Exception;
      Return;
    ...
    ELSIF (p_action = 'VALIDATE_INSERT') THEN
      IF  Get_PK_For_Validation (
  		...
  		NEW_REFERENCES.s_encmb_effect_type ,
    		...
    		NEW_REFERENCES.pee_sequence_number,
    		NEW_REFERENCES.unit_cd ,
    		NEW_REFERENCES.pue_start_dt) THEN
         Fnd_Message.Set_Name ('IGS', 'IGS_GE_RECORD_ALREADY_EXISTS');
         IGS_GE_MSG_STACK.ADD;
          App_Exception.Raise_Exception;
      END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_PE_PERS_UNT_EXCL
      where PERSON_ID = X_PERSON_ID
      and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
      and PEN_START_DT = X_PEN_START_DT
      and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
 ...
 Before_DML(
  ...
  insert into IGS_PE_PERS_UNT_EXCL (
    PERSON_ID,
    ENCUMBRANCE_TYPE,
    PEN_START_DT,
    S_ENCMB_EFFECT_TYPE,
...
begin
 ...
 Before_DML(
  ...
  update IGS_PE_PERS_UNT_EXCL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PE_PERS_UNT_EXCL
     where PERSON_ID = X_PERSON_ID
     and ENCUMBRANCE_TYPE = X_ENCUMBRANCE_TYPE
     and PEN_START_DT = X_PEN_START_DT
     and S_ENCMB_EFFECT_TYPE = X_S_ENCMB_EFFECT_TYPE
...
begin
 Before_DML(
  ...
  delete from IGS_PE_PERS_UNT_EXCL
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
