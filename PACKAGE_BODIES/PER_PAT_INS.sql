--------------------------------------------------------
--  DDL for Package Body PER_PAT_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PAT_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_pat_ins.';  -- Global package name
g_debug   boolean      := hr_utility.debug_enabled;
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_allocated_task_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_allocated_task_id  in  number) is
--
...
Begin
  ...
  --
  per_pat_ins.g_allocated_task_id_i := p_allocated_task_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: per_allocated_tasks
  --
  insert into per_allocated_tasks
      (allocated_task_id
...
--
  Cursor C_Sel1 is select per_allocated_tasks_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_allocated_tasks
     where allocated_task_id =
             per_pat_ins.g_allocated_task_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (per_pat_ins.g_allocated_task_id_i is not null) Then
    --
    ...
    p_rec.allocated_task_id :=
      per_pat_ins.g_allocated_task_id_i;
    per_pat_ins.g_allocated_task_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.allocated_task_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  per_pat_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  per_pat_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  per_pat_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  p_allocated_task_id := l_rec.allocated_task_id;
