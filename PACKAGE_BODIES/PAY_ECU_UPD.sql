--------------------------------------------------------
--  DDL for Package Body PAY_ECU_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_ECU_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_ecu_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the pay_element_class_usages_f Row
    --
    update  pay_element_class_usages_f
    set
    ...
    where   element_class_usage_id = p_rec.element_class_usage_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    pay_ecu_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_base_key_value         => p_rec.element_class_usage_id
...
Begin
  ...
  begin
    ...
    pay_ecu_rku.after_update
      ...
      => pay_ecu_shd.g_old_rec.effective_end_date
      ,p_run_type_id_o
      => pay_ecu_shd.g_old_rec.run_type_id
      ,p_classification_id_o
      => pay_ecu_shd.g_old_rec.classification_id
...
Begin
  ...
  --
  pay_ecu_upd.convert_defs(p_rec);
  --
  pay_ecu_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
