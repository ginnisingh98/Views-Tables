--------------------------------------------------------
--  DDL for Package Body BEN_CRP_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_CRP_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_crp_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_crp_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_cbr_per_in_ler Row
  --
  update ben_cbr_per_in_ler
  set
  cbr_per_in_ler_id                 = p_rec.cbr_per_in_ler_id,
  init_evt_flag                     = p_rec.init_evt_flag,
  cnt_num                           = p_rec.cnt_num,
  per_in_ler_id                     = p_rec.per_in_ler_id,
  cbr_quald_bnf_id                  = p_rec.cbr_quald_bnf_id,
  prvs_elig_perd_end_dt             = p_rec.prvs_elig_perd_end_dt,
  business_group_id                 = p_rec.business_group_id,
  crp_attribute_category            = p_rec.crp_attribute_category,
  crp_attribute1                    = p_rec.crp_attribute1,
  crp_attribute2                    = p_rec.crp_attribute2,
  crp_attribute3                    = p_rec.crp_attribute3,
  crp_attribute4                    = p_rec.crp_attribute4,
  crp_attribute5                    = p_rec.crp_attribute5,
  crp_attribute6                    = p_rec.crp_attribute6,
  crp_attribute7                    = p_rec.crp_attribute7,
  crp_attribute8                    = p_rec.crp_attribute8,
  crp_attribute9                    = p_rec.crp_attribute9,
  crp_attribute10                   = p_rec.crp_attribute10,
  crp_attribute11                   = p_rec.crp_attribute11,
  crp_attribute12                   = p_rec.crp_attribute12,
  crp_attribute13                   = p_rec.crp_attribute13,
  crp_attribute14                   = p_rec.crp_attribute14,
  crp_attribute15                   = p_rec.crp_attribute15,
  crp_attribute16                   = p_rec.crp_attribute16,
  crp_attribute17                   = p_rec.crp_attribute17,
  crp_attribute18                   = p_rec.crp_attribute18,
  crp_attribute19                   = p_rec.crp_attribute19,
  crp_attribute20                   = p_rec.crp_attribute20,
  crp_attribute21                   = p_rec.crp_attribute21,
  crp_attribute22                   = p_rec.crp_attribute22,
  crp_attribute23                   = p_rec.crp_attribute23,
  crp_attribute24                   = p_rec.crp_attribute24,
  crp_attribute25                   = p_rec.crp_attribute25,
  crp_attribute26                   = p_rec.crp_attribute26,
  crp_attribute27                   = p_rec.crp_attribute27,
  crp_attribute28                   = p_rec.crp_attribute28,
  crp_attribute29                   = p_rec.crp_attribute29,
  crp_attribute30                   = p_rec.crp_attribute30,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_crp_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_crp_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.init_evt_flag = hr_api.g_varchar2) then
    p_rec.init_evt_flag :=
    ben_crp_shd.g_old_rec.init_evt_flag;
  End If;
  If (p_rec.cnt_num = hr_api.g_number) then
    p_rec.cnt_num :=
    ben_crp_shd.g_old_rec.cnt_num;
  End If;
  If (p_rec.per_in_ler_id = hr_api.g_number) then
    p_rec.per_in_ler_id :=
    ben_crp_shd.g_old_rec.per_in_ler_id;
  End If;
  If (p_rec.cbr_quald_bnf_id = hr_api.g_number) then
    p_rec.cbr_quald_bnf_id :=
    ben_crp_shd.g_old_rec.cbr_quald_bnf_id;
  End If;
  If (p_rec.prvs_elig_perd_end_dt = hr_api.g_date) then
    p_rec.prvs_elig_perd_end_dt :=
    ben_crp_shd.g_old_rec.prvs_elig_perd_end_dt;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_crp_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.crp_attribute_category = hr_api.g_varchar2) then
    p_rec.crp_attribute_category :=
    ben_crp_shd.g_old_rec.crp_attribute_category;
  End If;
  If (p_rec.crp_attribute1 = hr_api.g_varchar2) then
    p_rec.crp_attribute1 :=
    ben_crp_shd.g_old_rec.crp_attribute1;
  End If;
  If (p_rec.crp_attribute2 = hr_api.g_varchar2) then
    p_rec.crp_attribute2 :=
    ben_crp_shd.g_old_rec.crp_attribute2;
  End If;
  If (p_rec.crp_attribute3 = hr_api.g_varchar2) then
    p_rec.crp_attribute3 :=
    ben_crp_shd.g_old_rec.crp_attribute3;
  End If;
  If (p_rec.crp_attribute4 = hr_api.g_varchar2) then
    p_rec.crp_attribute4 :=
    ben_crp_shd.g_old_rec.crp_attribute4;
  End If;
  If (p_rec.crp_attribute5 = hr_api.g_varchar2) then
    p_rec.crp_attribute5 :=
    ben_crp_shd.g_old_rec.crp_attribute5;
  End If;
  If (p_rec.crp_attribute6 = hr_api.g_varchar2) then
    p_rec.crp_attribute6 :=
    ben_crp_shd.g_old_rec.crp_attribute6;
  End If;
  If (p_rec.crp_attribute7 = hr_api.g_varchar2) then
    p_rec.crp_attribute7 :=
    ben_crp_shd.g_old_rec.crp_attribute7;
  End If;
  If (p_rec.crp_attribute8 = hr_api.g_varchar2) then
    p_rec.crp_attribute8 :=
    ben_crp_shd.g_old_rec.crp_attribute8;
  End If;
  If (p_rec.crp_attribute9 = hr_api.g_varchar2) then
    p_rec.crp_attribute9 :=
    ben_crp_shd.g_old_rec.crp_attribute9;
  End If;
  If (p_rec.crp_attribute10 = hr_api.g_varchar2) then
    p_rec.crp_attribute10 :=
    ben_crp_shd.g_old_rec.crp_attribute10;
  End If;
  If (p_rec.crp_attribute11 = hr_api.g_varchar2) then
    p_rec.crp_attribute11 :=
    ben_crp_shd.g_old_rec.crp_attribute11;
  End If;
  If (p_rec.crp_attribute12 = hr_api.g_varchar2) then
    p_rec.crp_attribute12 :=
    ben_crp_shd.g_old_rec.crp_attribute12;
  End If;
  If (p_rec.crp_attribute13 = hr_api.g_varchar2) then
    p_rec.crp_attribute13 :=
    ben_crp_shd.g_old_rec.crp_attribute13;
  End If;
  If (p_rec.crp_attribute14 = hr_api.g_varchar2) then
    p_rec.crp_attribute14 :=
    ben_crp_shd.g_old_rec.crp_attribute14;
  End If;
  If (p_rec.crp_attribute15 = hr_api.g_varchar2) then
    p_rec.crp_attribute15 :=
    ben_crp_shd.g_old_rec.crp_attribute15;
  End If;
  If (p_rec.crp_attribute16 = hr_api.g_varchar2) then
    p_rec.crp_attribute16 :=
    ben_crp_shd.g_old_rec.crp_attribute16;
  End If;
  If (p_rec.crp_attribute17 = hr_api.g_varchar2) then
    p_rec.crp_attribute17 :=
    ben_crp_shd.g_old_rec.crp_attribute17;
  End If;
  If (p_rec.crp_attribute18 = hr_api.g_varchar2) then
    p_rec.crp_attribute18 :=
    ben_crp_shd.g_old_rec.crp_attribute18;
  End If;
  If (p_rec.crp_attribute19 = hr_api.g_varchar2) then
    p_rec.crp_attribute19 :=
    ben_crp_shd.g_old_rec.crp_attribute19;
  End If;
  If (p_rec.crp_attribute20 = hr_api.g_varchar2) then
    p_rec.crp_attribute20 :=
    ben_crp_shd.g_old_rec.crp_attribute20;
  End If;
  If (p_rec.crp_attribute21 = hr_api.g_varchar2) then
    p_rec.crp_attribute21 :=
    ben_crp_shd.g_old_rec.crp_attribute21;
  End If;
  If (p_rec.crp_attribute22 = hr_api.g_varchar2) then
    p_rec.crp_attribute22 :=
    ben_crp_shd.g_old_rec.crp_attribute22;
  End If;
  If (p_rec.crp_attribute23 = hr_api.g_varchar2) then
    p_rec.crp_attribute23 :=
    ben_crp_shd.g_old_rec.crp_attribute23;
  End If;
  If (p_rec.crp_attribute24 = hr_api.g_varchar2) then
    p_rec.crp_attribute24 :=
    ben_crp_shd.g_old_rec.crp_attribute24;
  End If;
  If (p_rec.crp_attribute25 = hr_api.g_varchar2) then
    p_rec.crp_attribute25 :=
    ben_crp_shd.g_old_rec.crp_attribute25;
  End If;
  If (p_rec.crp_attribute26 = hr_api.g_varchar2) then
    p_rec.crp_attribute26 :=
    ben_crp_shd.g_old_rec.crp_attribute26;
  End If;
  If (p_rec.crp_attribute27 = hr_api.g_varchar2) then
    p_rec.crp_attribute27 :=
    ben_crp_shd.g_old_rec.crp_attribute27;
  End If;
  If (p_rec.crp_attribute28 = hr_api.g_varchar2) then
    p_rec.crp_attribute28 :=
    ben_crp_shd.g_old_rec.crp_attribute28;
  End If;
  If (p_rec.crp_attribute29 = hr_api.g_varchar2) then
    p_rec.crp_attribute29 :=
    ben_crp_shd.g_old_rec.crp_attribute29;
  End If;
  If (p_rec.crp_attribute30 = hr_api.g_varchar2) then
    p_rec.crp_attribute30 :=
    ben_crp_shd.g_old_rec.crp_attribute30;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_crp_bus.update_validate(p_rec
  ,p_effective_date);
  --
