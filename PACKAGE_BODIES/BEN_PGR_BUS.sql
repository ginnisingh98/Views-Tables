--------------------------------------------------------
--  DDL for Package Body BEN_PGR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PGR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pgr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ppl_grp_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   ppl_grp_rt_id PK of record being inserted or updated.
...
--
Procedure chk_ppl_grp_rt_id(p_ppl_grp_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ppl_grp_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_pgr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ppl_grp_rt_id                => p_ppl_grp_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ppl_grp_rt_id,hr_api.g_number)
     <>  ben_pgr_shd.g_old_rec.ppl_grp_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_pgr_shd.constraint_error('BEN_PPL_GRP_RT_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ppl_grp_rt_id is not null then
      --
...
--
-- In Parameters
--   p_ppl_grp_rt_id         PK of record being inserted or updated.
--   p_people_group_id       Value of FK
...
and ppl_grp_rt_id <> nvl(p_ppl_grp_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_people_group_id <> nvl(ben_pgr_shd.g_old_rec.people_group_id,hr_api.g_number)
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this People Group criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','People Group criteria',TRUE);
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and ppl_grp_rt_id <> nvl(p_ppl_grp_rt_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_ppl_grp_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_pgr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
function return_legislation_code
  (p_ppl_grp_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_ppl_grp_rt_f b
    where b.ppl_grp_rt_id      = p_ppl_grp_rt_id
