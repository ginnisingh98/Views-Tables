--------------------------------------------------------
--  DDL for Package Body IGS_PR_OU_AWD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PR_OU_AWD_PKG" AS
/* $Header: IGSQI40B.pls 115.6 2003/02/25 09:38:23 anilk noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_pr_ou_awd%ROWTYPE;
  new_references igs_pr_ou_awd%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_PR_POA_PRO_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
