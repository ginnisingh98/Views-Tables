--------------------------------------------------------
--  DDL for Package Body BEN_LGL_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LGL_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lgl_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_lgl_enty_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   lgl_enty_rt_id PK of record being inserted or updated.
...
--
Procedure chk_lgl_enty_rt_id(p_lgl_enty_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_lgl_enty_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_lgl_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_lgl_enty_rt_id                => p_lgl_enty_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_lgl_enty_rt_id,hr_api.g_number)
     <>  ben_lgl_shd.g_old_rec.lgl_enty_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_lgl_shd.constraint_error('BEN_LGL_ENTY_RTS_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_lgl_enty_rt_id is not null then
      --
...
--
-- In Parameters
--   p_lgl_enty_rt_id        PK of record being inserted or updated.
...
and lgl_enty_rt_id <> nvl(p_lgl_enty_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_organization_id <> nvl(ben_lgl_shd.g_old_rec.organization_id,hr_api.g_number)
	...
	if c_dup%found then
    		close c_dup;
   		--
   		-- raise error as this Legal Entity criteria already exists for this profile
   		--
   		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
   		fnd_message.set_token('VAR1','Legal Entity criteria',TRUE);
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and lgl_enty_rt_id <> nvl(p_lgl_enty_rt_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_lgl_enty_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_lgl_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
...
function return_legislation_code
  (p_lgl_enty_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_lgl_enty_rt_f b
    where b.lgl_enty_rt_id      = p_lgl_enty_rt_id
