--------------------------------------------------------
--  DDL for Package Body PQP_GB_SCOTLAND_LGPS_PENSIONPY
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQP_GB_SCOTLAND_LGPS_PENSIONPY" AS
/* $Header: pqgbsclgps.pkb 120.3.12010000.5 2009/03/14 11:25:55 dwkrishn noship $ */
--
g_package                 VARCHAR2(31) := 'PQP_GB_SCOTLAND_LGPS_PENSIONPY.';
...
--      AND paaf.assignment_id = nvl(p_assignment_id, paaf.assignment_id)
      ...
      AND paaf.EFFECTIVE_END_DATE >= v_eff_start_date -- Bug 8315098
...
--7369484  Changed to Scottish configuration value
...
--One row with data PQP_UK_RATE_TYPE and PQP_LGPS_SCOTLAND_PENSION_PAY
--and second row with PQP_UK_ELEMENT_ATTRIBUTION and Pay Value.
-- 7369484 Start Changes made in cursor  to fetch  both rate types
      ...
      LOOP
	    ...
	    --6666135 Begin
		  ...
		  CLOSE c2_PQP_UK_RATE_TYPE;
            ...
            END IF;
		-- 7369484 Begin
		IF c2_PQP_UK_RATE_TYPE_data.EEI_INFORMATION1 = 'PQP_LGPS_PENSION_PAY'
	        THEN
	        b_eng_rate := TRUE;
		  IF b_scot_rate = TRUE
		    THEN
			...
			RAISE skip_assignment;
		  END IF;
            ELSIF c2_PQP_UK_RATE_TYPE_data.EEI_INFORMATION1 = 'PQP_LGPS_SCOTLAND_PENSION_PAY'
            THEN
	     	-- 7369484 End
	       ...
	       IF b_eng_rate = TRUE
		    ...
		    RAISE skip_assignment;
	       END IF;
	       END IF; -- 7369484  if rate type is English rate type
...
--        l_warning_msg := 'PQP_LGPS_MISSING_RATE_INFO: Historic Rate - Rate Type "LGPS Pensionable Pay" not set for elements against Assignment '||c_all_assignments.assignment_number;
          ...
          THEN
	   l_warning_msg := 'PQP_LGPS_MISSING_RATE_INFO: Neither Historic Rate - Rate Type "LGPS Scotland Pensionable Pay" nor User defined formula set for elements against Assignment '||c_all_assignments.assignment_number;
--6813970 end
           fnd_file.put_line (fnd_file.LOG, l_warning_msg);
...
--6813970 end
	Begin
	       l_lgps_pensionable_pay:=pqp_rates_history_calc.get_historic_rate(p_assignment_id        => c_all_assignments.assignment_id
                                                                       ,p_rate_name            => 'LGPS Scotland Pensionable Pay'
                                                                       ,p_effective_date       => v_assignment_eff_date  --v_eff_start_date
                                                                       ,p_time_dimension       => 'A'
...
--6813970 end
      ...
      END IF;   --check for presence of formula in configuration value screen
          ...
          THEN
 	     l_lgps_pensionable_pay := floor(l_lgps_pensionable_pay);
