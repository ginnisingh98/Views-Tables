--------------------------------------------------------
--  DDL for Package Body OTA_FORUM_THREAD_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_FORUM_THREAD_SWI" As
/* $Header: otftsswi.pkb 120.2 2005/09/20 14:36 asud noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'ota_forum_thread_swi.';
--
-- ----------------------------------------------------------------------------
-- |--------------------------< create_forum_thread >-------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint create_forum_thread_swi;
  --
  ...
  -- Register Surrogate ID or user key values
    --
    ota_fts_ins.set_base_key_value
    (
  ...
  -- Call API
  --
  ota_forum_thread_api.create_forum_thread
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_forum_thread_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    l_forum_thread_id             := null;
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint update_forum_thread_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  ota_forum_thread_api.update_forum_thread
    (p_validate                     => l_validate
    ,p_effective_date               => p_effective_date
    ,p_forum_id                     => p_forum_id
    ,p_business_group_id            => p_business_group_id
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_forum_thread_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_object_version_number        := null;
...
Begin
  ...
  -- Issue a savepoint
  --
  savepoint delete_forum_thread_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  ota_forum_thread_api.delete_forum_thread
    (p_validate                     => l_validate
    ,p_forum_thread_id              => p_forum_thread_id
    ,p_object_version_number        => p_object_version_number
    );
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_forum_thread_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_return_status := hr_multi_message.get_return_status_disable;
