--------------------------------------------------------
--  DDL for Package Body HR_DELIVERY_METHODS_BE2
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_DELIVERY_METHODS_BE2" as 
...
p_attribute20                  varchar2) is
  ...
  --
  l_proc varchar2(72):='  hr_delivery_methods_be2.update_delivery_method_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.delivery_methods.update_delivery_method';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<delivery_method_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_delivery_method_id);
    l_text:=l_text||'</delivery_method_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<date_start>';
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<comm_dlvry_method>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_comm_dlvry_method);
    l_text:=l_text||'</comm_dlvry_method>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<preferred_flag>';
    l_text:=l_text||irc_utilities_pkg.removeTags(p_preferred_flag);
    l_text:=l_text||'</preferred_flag>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
