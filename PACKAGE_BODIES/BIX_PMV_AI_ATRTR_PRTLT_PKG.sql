--------------------------------------------------------
--  DDL for Package Body BIX_PMV_AI_ATRTR_PRTLT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_PMV_AI_ATRTR_PRTLT_PKG" AS
/*$Header: bixiatrp.plb 120.0 2005/05/25 17:18:47 appldev noship $ */
...
BEGIN
   ...
   poa_dbi_util_pkg.add_column(bix_col_tab,'call_calls_abandoned',    'aban' ,'N',2, 'XTD');
   poa_dbi_util_pkg.add_column(bix_col_tab,'call_calls_transferred',  'tran' ,'N',2, 'XTD');
  ...
  poa_dbi_util_pkg.add_column(p_cols,'0','aban','N',2,'XTD');
  poa_dbi_util_pkg.add_column(p_cols,'0','tran','N',2,'XTD');
  poa_dbi_util_pkg.add_column(p_cols,'0','hand','N',2,'XTD');
  ...
  l_union_all_text := 'UNION ALL '||bix_pmv_dbi_utl_pkg.get_continued_measures
   ...
   BEGIN
      ...
      l_sqltext := 'SELECT name VIEWBY,
                          -- SUM(p_aban)*100/NULLIF(SUM(p_offrd),0)        BIX_PMV_AI_PREVABANRATE,
                           SUM(curr_aban)*100/NULLIF(SUM(curr_offrd),0)  BIX_PMV_AI_ABANRATE    ,
                          -- SUM(p_tran)*100/NULLIF(SUM(p_hand) ,0)        BIX_PMV_AI_PREVTRANRATE,
                           SUM(curr_tran)*100/NULLIF(SUM(curr_hand) ,0) BIX_PMV_AI_TRANRATE
