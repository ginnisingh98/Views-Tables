--------------------------------------------------------
--  DDL for Package Body CS_SR_RES_CODE_MAP_DETAIL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CS_SR_RES_CODE_MAP_DETAIL_PKG" as
/* $Header: csrcmpdb.pls 120.0 2006/03/01 17:41:34 aseethep noship $ */
...
is
   cursor c1 is
   select cs_sr_res_code_mapping_dtl_s.nextval
   from dual;

BEGIN
   if ( PX_RESOLUTION_MAP_DETAIL_ID IS NULL ) OR ( PX_RESOLUTION_MAP_DETAIL_ID = FND_API.G_MISS_NUM) THEN
      open c1;
      fetch c1 into PX_RESOLUTION_MAP_DETAIL_ID;
      close c1;
   end if;

  insert into CS_SR_RES_CODE_MAPPING_DETAIL (
...
procedure LOCK_ROW (
  ...
  cursor c1 is select
    ...
    where RESOLUTION_MAP_DETAIL_ID = P_RESOLUTION_MAP_DETAIL_ID
    for update of RESOLUTION_MAP_DETAIL_ID nowait;
begin
  for tlinfo in c1 loop
      if (    (tlinfo.RESOLUTION_MAP_DETAIL_ID = P_RESOLUTION_MAP_DETAIL_ID)
          AND (tlinfo.RESOLUTION_MAP_ID = P_RESOLUTION_MAP_ID)
          ...
          AND (tlinfo.RESOLUTION_CODE = P_RESOLUTION_CODE)
...
begin
  update CS_SR_RES_CODE_MAPPING_DETAIL set
    RESOLUTION_MAP_ID = P_RESOLUTION_MAP_ID,
...
begin
  delete from CS_SR_RES_CODE_MAPPING_DETAIL
  where RESOLUTION_MAP_DETAIL_ID = P_RESOLUTION_MAP_DETAIL_ID;
