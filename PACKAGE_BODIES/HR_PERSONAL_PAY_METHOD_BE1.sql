--------------------------------------------------------
--  DDL for Package Body HR_PERSONAL_PAY_METHOD_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_PERSONAL_PAY_METHOD_BE1" as 
...
p_ppm_information_category     varchar2) is
  ...
  --
  l_proc varchar2(72):='  hr_personal_pay_method_be1.create_personal_pay_method_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.personal_pay_method.create_personal_pay_method';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<org_payment_method_id>';
    l_text:=l_text||fnd_number.number_to_canonical(p_org_payment_method_id);
    l_text:=l_text||'</org_payment_method_id>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<amount>';
