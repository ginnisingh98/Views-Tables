--------------------------------------------------------
--  DDL for Package Body BEN_LSR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LSR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lsr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_los_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   los_rt_id PK of record being inserted or updated.
...
--
Procedure chk_los_rt_id(p_los_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_los_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_lsr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_los_rt_id                => p_los_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_los_rt_id,hr_api.g_number)
     <>  ben_lsr_shd.g_old_rec.los_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_lsr_shd.constraint_error('BEN_LOS_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_los_rt_id is not null then
      --
...
is
   ...
   cursor c1 is select null
                 ...
                 where vrbl_rt_prfl_id = p_vrbl_rt_prfl_id
                   and los_rt_id <> nvl(p_los_rt_id,-1)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_los_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_lsr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   los_fctr_id FK of pointing to BEN_LOS_FCTR tables.
...
Begin
  ...
  if p_los_fctr_id is not null then
     ...
     fetch c1 into l_dummy;
     if c1%notfound then
        hr_utility.set_message(801,'LOS_Fctr_NotExist');
        hr_utility.raise_error;
     end if;
     close c1;
  end if;
...
--
-- In Parameters
--   p_los_rt_id            PK of record being inserted or updated.
--   p_los_fctr_id           Value of FK
...
and los_rt_id <> nvl(p_los_rt_id,hr_api.g_number)
and business_group_id + 0 = p_business_group_id
...
--
  ...
  --
  if (l_api_updating
      and p_los_fctr_id <> nvl(ben_lsr_shd.g_old_rec.los_fctr_id,hr_api.g_number)
      or not l_api_updating) then
	...
	if c_dup%found then
		close c_dup;
		--
		-- raise error as this Length of Service criteria already exists for this profile
		--
		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
		fnd_message.set_token('VAR1','Length of Service criteria',TRUE);
...
function return_legislation_code
  (p_los_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
