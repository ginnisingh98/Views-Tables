--------------------------------------------------------
--  DDL for Package Body BEN_WLR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_WLR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_wlr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_wk_loc_rt_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   wk_loc_rt_id PK of record being inserted or updated.
...
--
Procedure chk_wk_loc_rt_id(p_wk_loc_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_wk_loc_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_wlr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_wk_loc_rt_id                => p_wk_loc_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_wk_loc_rt_id,hr_api.g_number)
     <>  ben_wlr_shd.g_old_rec.wk_loc_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_wlr_shd.constraint_error('BEN_WORK_LOCATION_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_wk_loc_rt_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_wk_loc_rt_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_wlr_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
...
--
-- In Parameters
--   loaction_id FK pointing to HR_LOCATIONS table.
--   effective_date or current transaction
--
-- Post Success
--   Processing continues
--
...
--   Internal table handler use only.
--
Procedure chk_location_id(p_location_id      in number
  ...
  cursor c1 is select null
               ...
               and    p_effective_date <= nvl(inactive_date, p_effective_date);
  --
Begin
  --
  hr_utility.set_location('Entering:'||l_proc, 5);
  ...
  if p_location_id is not null then
     ...
     if c1%notfound then
        hr_utility.set_message(801,'Location_NotFound');
        hr_utility.raise_error;
     end if;
     close c1;
  end if;
...
--
-- In Parameters
--   p_wk_loc_rt_id          PK of record being inserted or updated.
--   p_location_id           Value of FK
...
and wk_loc_rt_id <> nvl(p_wk_loc_rt_id,hr_api.g_number)
...
--
  ...
  --
  if (l_api_updating
      and p_location_id <> nvl(ben_wlr_shd.g_old_rec.location_id,hr_api.g_number)
      or not l_api_updating) then
	...
	if c_dup%found then
		close c_dup;
		--
		-- raise error as this Work Location criteria already exists for this profile
		--
		fnd_message.set_name('BEN', 'BEN_92992_DUPS_ROW');
		fnd_message.set_token('VAR1','Work Location criteria',TRUE);
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   -- changed against bug: 5113011
                   and wk_loc_rt_id   <> nvl(p_wk_loc_rt_id  ,-1)
                   -- and location_id   <> nvl(p_location_id  ,-1)
...
function return_legislation_code
  (p_wk_loc_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_wk_loc_rt_f b
    where b.wk_loc_rt_id      = p_wk_loc_rt_id
