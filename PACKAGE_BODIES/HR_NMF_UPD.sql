--------------------------------------------------------
--  DDL for Package Body HR_NMF_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_NMF_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hr_nmf_upd.';  -- Global package name
...
Begin
  ...
  --
  -- Update the hr_name_formats Row
  --
  update hr_name_formats
    set
     name_format_id                  = p_rec.name_format_id
...
Begin
  ...
  --
  If (p_rec.format_name = hr_api.g_varchar2) then
    p_rec.format_name :=
    hr_nmf_shd.g_old_rec.format_name;
  End If;
  If (p_rec.legislation_code = hr_api.g_varchar2) then
    ...
    hr_nmf_shd.g_old_rec.legislation_code;
  End If;
  If (p_rec.user_format_choice = hr_api.g_varchar2) then
...
Begin
  ...
  --
  hr_nmf_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hr_nmf_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hr_nmf_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  (p_name_format_id
  ,hr_api.g_varchar2     --p_format_name
  ,hr_api.g_varchar2     --p_legislation_code
  ,hr_api.g_varchar2     --p_user_format_choice
  ,p_format_mask
  ,p_object_version_number
  );
  --
