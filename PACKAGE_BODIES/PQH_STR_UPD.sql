--------------------------------------------------------
--  DDL for Package Body PQH_STR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_STR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_str_upd.';  -- Global package name
...
if g_debug then
  ...
  --
  --
  -- Update the pqh_fr_stat_situation_rules Row
  --
  update pqh_fr_stat_situation_rules
    set
     stat_situation_rule_id          = p_rec.stat_situation_rule_id
    ,statutory_situation_id          = p_rec.statutory_situation_id
    ,processing_sequence             = p_rec.processing_sequence
    ...
    ,exclude_flag                    = p_rec.exclude_flag
...
Begin
  ...
  If (p_rec.statutory_situation_id = hr_api.g_number) then
    ...
    pqh_str_shd.g_old_rec.statutory_situation_id;
  End If;
  If (p_rec.processing_sequence = hr_api.g_number) then
    p_rec.processing_sequence :=
    pqh_str_shd.g_old_rec.processing_sequence;
  End If;
  ...
  If (p_rec.required_flag = hr_api.g_varchar2) then
    ...
    pqh_str_shd.g_old_rec.required_flag;
  End If;
  If (p_rec.exclude_flag = hr_api.g_varchar2) then
    p_rec.exclude_flag :=
    pqh_str_shd.g_old_rec.exclude_flag;
  End If;
  --
...
Begin
  ...
  --
  pqh_str_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  pqh_str_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  pqh_str_upd.post_update
     (p_effective_date
     ,p_rec
     );
  --
