--------------------------------------------------------
--  DDL for Package Body BEN_AUTOMATIC_ENROLLMENTS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_AUTOMATIC_ENROLLMENTS" as
/* $Header: benauten.pkb 120.10.12010000.2 2009/02/10 18:28:00 krupani ship $ */
  --
  g_package varchar2(30) := 'ben_automatic_enrollments.';
  --
  ...
  l_opt_id           number ;
  --
  --- To validate the plan type , type id is compared # 2508745
  cursor c_pl_typ is
    select pl_typ_id
     from  ben_pl_f pl
  ...
  --- To validate the option , opt_id is compared # 2508745
   cursor c_opt is
    select opt_id
    ...
    --- if the person enroled in option  he must conitnue with the same option and plan type
    --- regardless of program and plan. if the person enrolled in plan  he must conitnue with
    --- same plan type  to reinstate the    dependent # 2508745

    --- Find then ler_id which last updated the dpnt row
    ---- this is not taking the first piece of dentrolled row but the second piece
    ...
    --- when the previois result_id available
            ...
            if l_tot_rqmt_allow  = 'Y' then
               ...
               for l_prev_pen_dpnts in c_prev_pen_dpnts(l_rslt_id,l_pil_id) loop
                   ...
                   if c_epe_dpnt%notfound then
                      ...
                      --
                      ben_sspndd_enrollment.g_cfw_dpnt_flag := 'Y' ;
                      hr_utility.set_location(' g_cfw_dpnt_flag '||ben_sspndd_enrollment.g_cfw_dpnt_flag,122);
                      --
                      ben_ELIG_DPNT_api.process_dependent(
                       ...
                       --
                       ben_sspndd_enrollment.g_cfw_dpnt_flag := 'N' ;
                       hr_utility.set_location(' g_cfw_dpnt_flag '||ben_sspndd_enrollment.g_cfw_dpnt_flag,122);
                       --
                   end if;
                   close c_epe_dpnt;
  ...
  procedure process_person(p_person_id         in number,
    ...
    --
    cursor c_choice_info(v_per_in_ler_id number) is
    ...
    cursor c_bnft(v_elig_per_elctbl_chc_id number) is
    ...
    l_per_in_ler_id number := benutils.get_per_in_ler_id
    ...
    --
    -- Bug 5529696
    -- Following cursor is meant to query PGM_IDs and PNIP_ID's into which benrtchg made enrollments (due to change in BNFT_AMT)
    -- and hence did not get caught by c_choice_info.
    --
   cursor c_pgm_pnip_id(p_per_in_ler_id number) is
    --
    ...
    --
    cursor c_lf_evt_ocrd_dt(p_per_in_ler_id number) is
       ...
       and    pil.per_in_ler_stat_cd not in ('BCKDT','VOIDD');
    --
    ll_lf_evt_ocrd_dt ben_per_in_ler.lf_evt_ocrd_dt%type;
  ...
  begin
    ...
    -- a call to get the benmngle effective run date.
    ben_env_object.get(p_rec => l_rec);
    hr_utility.set_location('Effective date: '||l_rec.effective_date,10);

    --
    -- Check if we need to close the enrollment, we only do this if all
    -- electable choices for a per_in_ler are autoutomatic.
    --

      open c_elctbl_epe;
      --
      fetch c_elctbl_epe into l_dummy;
      if c_elctbl_epe%found then
        --
        hr_utility.set_location('Per in Ler will not be closed : '||l_proc,10);
        l_cls_enrt_flag := false;
        --
      else
        --
        hr_utility.set_location('Per in Ler will be closed : '||l_proc,10);
    ...
    close c_elctbl_epe;
    --
    open c_choice_info(l_per_in_ler_id);
      --
      loop
        --
        ...
        --
        --bug#4478186 - moved the code from down below
        if l_choice_info.pgm_id is not null then
          ...
          /* bug 7623319
          if l_pgm_enrt_dt.enrt_perd_strt_dt < l_rec.effective_date then
             l_effective_dt := l_pgm_enrt_dt.enrt_perd_strt_dt;
          else
             l_effective_dt := l_rec.effective_date;
          end if;
	  */
	  -- bug 7623319: Assign automatic on enrollment period start date
	  l_effective_dt := l_pgm_enrt_dt.enrt_perd_strt_dt;
          close c_pgm_enrt_dt;
	  ...
	  /* bug 7623319
          if l_pl_enrt_dt.enrt_perd_strt_dt < l_rec.effective_date then
	  ...
	  */
        ...
        -- Get participant enrollment result information
         ...
         hr_utility.set_location('l_effective_dt'||l_effective_dt,11);
        --
        if l_choice_info.crntly_enrd_flag = 'Y' then
          ...
          --
	  --Bug 5589466 :  Reset l_prtt_result if the current election is a new one
	  l_prtt_result := null;
	  --End 5589466
        ...
        open c_bnft(l_choice_info.elig_per_elctbl_chc_id);
        loop
        ...
        --
        /*  moved above to pass the l_effective_dt to cursor to get current
         *  enrollment
        if l_choice_info.pgm_id is not null then
          open c_pgm_enrt_dt(l_choice_info.elig_per_elctbl_chc_id,
                             l_choice_info.pgm_id);
...
--hr_utility.set_location('ann_rt_val='||l_rt_table(1).ann_rt_val,13);
        ben_election_information.election_information
          (p_elig_per_elctbl_chc_id => l_choice_info.elig_per_elctbl_chc_id,
           p_prtt_enrt_rslt_id      => l_choice_info.prtt_enrt_rslt_id,
           p_effective_date         => l_effective_dt,
           p_enrt_mthd_cd           => 'A',
           p_business_group_id      => p_business_group_id,
           p_enrt_bnft_id           => l_bnft.enrt_bnft_id,
           p_bnft_val               => l_bnft.val,
	   ...
	   p_pen_attribute29   =>l_prtt_result.pen_attribute29,
 	   p_pen_attribute30   =>l_prtt_result.pen_attribute30,
           -- 3517682 end
           p_object_version_number  => l_prtt_result.object_version_number,
        ...
        if l_prev_pgm_id = -99999 then
          ...
          if l_prev_pgm_id is not null then
             ...
             ben_proc_common_enrt_rslt.process_post_results
              (p_person_id          => p_person_id,
               p_enrt_mthd_cd       => 'A',
              --bug#1420160 p_effective_date     => l_rec.effective_date,
               p_effective_date     => l_effective_dt,
               p_business_group_id  => p_business_group_id,
               p_per_in_ler_id      => l_per_in_ler_id);
            ben_proc_common_enrt_rslt.process_post_enrollment
...
--    close c_choice_info;
  ...
  elsif (l_cls_enrt_flag and l_prev_pgm_id = -99999) then
         --
         hr_utility.set_location ('inside -99999', 213);
         --
         open c_lf_evt_ocrd_dt (l_per_in_ler_id);
         --
         fetch c_lf_evt_ocrd_dt
          into ll_lf_evt_ocrd_dt;
         --
         close c_lf_evt_ocrd_dt;
         --
         -- only for programs we need to call multi_rows_edit so for plan not in programs it is not called.
	 -- Also when election_information is called from benrtchg then only the following case will arise and since
 	 -- in benrtchg, p_effective_date is used as least(p_effective_date,p_lf_evt_ocrd_dt) here also p_effective_date is used
	 -- in the similar way.

         for l_pgm_pnip_id in c_pgm_pnip_id (l_per_in_ler_id)
          loop
            --
	      l_cursor := TRUE;
            if l_pgm_pnip_id.pgm_id is not null then	     --PGM
               --
               l_prev_pgm_id := l_pgm_pnip_id.pgm_id;
               --
               ben_prtt_enrt_result_api.multi_rows_edit
                               (p_person_id              => p_person_id,
                                p_effective_date         => least(p_effective_date,ll_lf_evt_ocrd_dt),
                                p_business_group_id      => p_business_group_id,
                                p_pgm_id                 => l_pgm_pnip_id.pgm_id
                               );
               --
               ben_proc_common_enrt_rslt.process_post_results
                                (p_person_id              => p_person_id,
            ...
            --
            elsif l_pgm_pnip_id.pl_id is not null then         -- PNIP
               --
               l_prev_pl_id := l_pgm_pnip_id.pl_id;
               --
               ben_proc_common_enrt_rslt.process_post_results
                               (p_person_id              => p_person_id,
                                p_enrt_mthd_cd           => 'A',
      ...
      if l_cursor then
	 --
         if l_prev_pgm_id <> -99999 and l_prev_pl_id = -99999 then        --only plan in pgm exist
            --
            ben_proc_common_enrt_rslt.process_post_enrollment
                                (p_per_in_ler_id          => l_per_in_ler_id,
                                 p_pgm_id                 => l_prev_pgm_id,
         ...
         --
         elsif l_prev_pgm_id = -99999 and l_prev_pl_id <> -99999 then          --only plan not in pgm exist
            --
            ben_proc_common_enrt_rslt.process_post_enrollment
                                (p_per_in_ler_id          => l_per_in_ler_id,
                                 p_pgm_id                 => null,
         ...
         --
            ...
            --
            ben_proc_common_enrt_rslt.process_post_enrollment                    --both plan in pgm and not in pgm exist
                                (p_per_in_ler_id          => l_per_in_ler_id,
                                 p_pgm_id                 => l_prev_pgm_id,
                                 p_pl_id                  => null,
                                 p_enrt_mthd_cd           => 'A',
  ...
  begin
    ...
    --
    -- Code only runs in Life Event or Scheduled Enrollment mode
    -- ABSENCES : Automatic enrollments should happen for absence mode.
    --
    ...
    --
    -- First derive person population we are going to process
    --
    open c_person_population;
      --
      loop
        --
        fetch c_person_population into l_person_id;
        exit when c_person_population%notfound;
        --
        process_person(p_person_id         => l_person_id,
                       p_business_group_id => p_business_group_id,
                       p_ler_id            => p_ler_id,
                       p_effective_date    => p_effective_date);
        --
