--------------------------------------------------------
--  DDL for Package Body FARX_C_MCR
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FARX_C_MCR" AS
/* $Header: FARXCMCRB.pls 120.0.12010000.2 2009/07/19 11:52:28 glchen ship $ */
...
BEGIN
    ...
    SELECT count(1) INTO h_count
     ...
     WHERE mass_change_id = to_number(argument1) AND rownum < 2;
    ...
    /* Call the inner procedure to review reclass. */
    FARX_MCR.Review_Change(
     X_Mass_Change_Id     => to_number(argument1),
     X_RX_Flag            => 'YES',
     retcode              => h_retcode,
     errbuf               => errbuf);
