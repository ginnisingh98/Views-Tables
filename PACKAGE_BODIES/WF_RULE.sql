--------------------------------------------------------
--  DDL for Package Body WF_RULE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."WF_RULE" as
/* $Header: wfruleb.pls 120.5.12010000.4 2014/05/21 22:28:29 alsosa ship $ */
--------------------------------------------------------------------------
/*
...
begin
  ...
  while(pos is not null) loop
    if (l_parameter_list(pos).getName() NOT IN
        ...
        'CONTEXT','SEND_COMMENT','PRIORITY','DUE_DATE','#CURRENT_PHASE')) then
...
return varchar2 is
 ...
 l_result_str varchar2(10);

begin
 -- Bug 10277369: Store the event parameters into a temo variable and set it back to event
 -- after executing the default rule function
 l_event_paramlist := p_event.getParameterList();
 l_result := setParametersIntoParameterList(p_subscription_guid,p_event);
 l_result_str := default_rule(p_subscription_guid, p_event);
 p_event.setParameterList(l_event_paramlist);
