--------------------------------------------------------
--  DDL for Package Body XNP_CSIWIPBC_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIWIPBC_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIWIPBC' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIWIPBC' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIWIPBC' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIWIPBC.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIWIPBC', l_consumer_list );
  END IF;
...
NULL ;
DECLARE
   l_txn_type varchar2(8) := 'CSIWIPBC';
   l_txn_type_id number   := 75;
   l_txn_id number;
 ...
 BEGIN
   ...
   IF l_txn_id is null THEN
     l_error_rec.error_text := 'xnp_xml_utils.decode failed. could not derive mtl_txn_id.';
     RAISE dpl_error;
   END IF;
   l_error_rec.source_id := l_txn_id;
   debug('  mtl_transaction_id : '||l_txn_id);
 ...
 EXCEPTION
   WHEN dpl_error THEN
     rollback to dpl_sp;
     init_debug_file;
     debug('  Transaction '||l_txn_id||' failed. '||l_error_rec.error_text);
