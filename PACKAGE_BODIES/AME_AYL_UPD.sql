--------------------------------------------------------
--  DDL for Package Body AME_AYL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_AYL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_ayl_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  --
  --
  -- Update the ame_action_types_tl Row
  --
...
Begin
  ...
  If (p_rec.source_lang = hr_api.g_varchar2) then
    ...
    ame_ayl_shd.g_old_rec.source_lang;
  End If;
  If (p_rec.user_action_type_name = hr_api.g_varchar2) then
    p_rec.user_action_type_name :=
    ame_ayl_shd.g_old_rec.user_action_type_name;
  End If;
...
Procedure upd
  ...
  ,p_user_action_type_name        in     varchar2  default hr_api.g_varchar2
...
Begin
  ...
  for l_lang in csr_upd_langs loop
    ame_ayl_upd.upd
      ...
      ,p_user_action_type_name       => p_user_action_type_name
