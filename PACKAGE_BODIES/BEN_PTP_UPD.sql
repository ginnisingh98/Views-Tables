--------------------------------------------------------
--  DDL for Package Body BEN_PTP_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PTP_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ptp_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_ptp_shd.upd_effective_end_date
     (p_effective_date	       => p_effective_date,
      p_base_key_value	       => p_rec.pl_typ_id,
...
--
  ...
  begin
    --Bug 3412944
    if ben_ptp_shd.g_old_rec.opt_typ_cd <> p_rec.opt_typ_cd then
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_ptp_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    ben_ptp_shd.g_old_rec.name;
  End If;
  If (p_rec.mx_enrl_alwd_num = hr_api.g_number) then
    p_rec.mx_enrl_alwd_num :=
    ben_ptp_shd.g_old_rec.mx_enrl_alwd_num;
  End If;
  If (p_rec.mn_enrl_rqd_num = hr_api.g_number) then
    p_rec.mn_enrl_rqd_num :=
    ben_ptp_shd.g_old_rec.mn_enrl_rqd_num;
  End If;
  If (p_rec.pl_typ_stat_cd = hr_api.g_varchar2) then
    p_rec.pl_typ_stat_cd :=
    ben_ptp_shd.g_old_rec.pl_typ_stat_cd;
  End If;
  If (p_rec.opt_typ_cd = hr_api.g_varchar2) then
    p_rec.opt_typ_cd :=
    ben_ptp_shd.g_old_rec.opt_typ_cd;
  End If;
  If (p_rec.opt_dsply_fmt_cd = hr_api.g_varchar2) then
    p_rec.opt_dsply_fmt_cd :=
    ben_ptp_shd.g_old_rec.opt_dsply_fmt_cd;
  End If;
  If (p_rec.comp_typ_cd = hr_api.g_varchar2) then
    p_rec.comp_typ_cd :=
    ben_ptp_shd.g_old_rec.comp_typ_cd;
  End If;
  If (p_rec.ivr_ident = hr_api.g_varchar2) then
    p_rec.ivr_ident :=
    ben_ptp_shd.g_old_rec.ivr_ident;
  End If;
  If (p_rec.no_mx_enrl_num_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_mx_enrl_num_dfnd_flag :=
    ben_ptp_shd.g_old_rec.no_mx_enrl_num_dfnd_flag;
  End If;
  If (p_rec.no_mn_enrl_num_dfnd_flag = hr_api.g_varchar2) then
    p_rec.no_mn_enrl_num_dfnd_flag :=
    ben_ptp_shd.g_old_rec.no_mn_enrl_num_dfnd_flag;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_ptp_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.ptp_attribute_category = hr_api.g_varchar2) then
    p_rec.ptp_attribute_category :=
    ben_ptp_shd.g_old_rec.ptp_attribute_category;
  End If;
  If (p_rec.ptp_attribute1 = hr_api.g_varchar2) then
    p_rec.ptp_attribute1 :=
    ben_ptp_shd.g_old_rec.ptp_attribute1;
  End If;
  If (p_rec.ptp_attribute2 = hr_api.g_varchar2) then
    p_rec.ptp_attribute2 :=
    ben_ptp_shd.g_old_rec.ptp_attribute2;
  End If;
  If (p_rec.ptp_attribute3 = hr_api.g_varchar2) then
    p_rec.ptp_attribute3 :=
    ben_ptp_shd.g_old_rec.ptp_attribute3;
  End If;
  If (p_rec.ptp_attribute4 = hr_api.g_varchar2) then
    p_rec.ptp_attribute4 :=
    ben_ptp_shd.g_old_rec.ptp_attribute4;
  End If;
  If (p_rec.ptp_attribute5 = hr_api.g_varchar2) then
    p_rec.ptp_attribute5 :=
    ben_ptp_shd.g_old_rec.ptp_attribute5;
  End If;
  If (p_rec.ptp_attribute6 = hr_api.g_varchar2) then
    p_rec.ptp_attribute6 :=
    ben_ptp_shd.g_old_rec.ptp_attribute6;
  End If;
  If (p_rec.ptp_attribute7 = hr_api.g_varchar2) then
    p_rec.ptp_attribute7 :=
    ben_ptp_shd.g_old_rec.ptp_attribute7;
  End If;
  If (p_rec.ptp_attribute8 = hr_api.g_varchar2) then
    p_rec.ptp_attribute8 :=
    ben_ptp_shd.g_old_rec.ptp_attribute8;
  End If;
  If (p_rec.ptp_attribute9 = hr_api.g_varchar2) then
    p_rec.ptp_attribute9 :=
    ben_ptp_shd.g_old_rec.ptp_attribute9;
  End If;
  If (p_rec.ptp_attribute10 = hr_api.g_varchar2) then
    p_rec.ptp_attribute10 :=
    ben_ptp_shd.g_old_rec.ptp_attribute10;
  End If;
  If (p_rec.ptp_attribute11 = hr_api.g_varchar2) then
    p_rec.ptp_attribute11 :=
    ben_ptp_shd.g_old_rec.ptp_attribute11;
  End If;
  If (p_rec.ptp_attribute12 = hr_api.g_varchar2) then
    p_rec.ptp_attribute12 :=
    ben_ptp_shd.g_old_rec.ptp_attribute12;
  End If;
  If (p_rec.ptp_attribute13 = hr_api.g_varchar2) then
    p_rec.ptp_attribute13 :=
    ben_ptp_shd.g_old_rec.ptp_attribute13;
  End If;
  If (p_rec.ptp_attribute14 = hr_api.g_varchar2) then
    p_rec.ptp_attribute14 :=
    ben_ptp_shd.g_old_rec.ptp_attribute14;
  End If;
  If (p_rec.ptp_attribute15 = hr_api.g_varchar2) then
    p_rec.ptp_attribute15 :=
    ben_ptp_shd.g_old_rec.ptp_attribute15;
  End If;
  If (p_rec.ptp_attribute16 = hr_api.g_varchar2) then
    p_rec.ptp_attribute16 :=
    ben_ptp_shd.g_old_rec.ptp_attribute16;
  End If;
  If (p_rec.ptp_attribute17 = hr_api.g_varchar2) then
    p_rec.ptp_attribute17 :=
    ben_ptp_shd.g_old_rec.ptp_attribute17;
  End If;
  If (p_rec.ptp_attribute18 = hr_api.g_varchar2) then
    p_rec.ptp_attribute18 :=
    ben_ptp_shd.g_old_rec.ptp_attribute18;
  End If;
  If (p_rec.ptp_attribute19 = hr_api.g_varchar2) then
    p_rec.ptp_attribute19 :=
    ben_ptp_shd.g_old_rec.ptp_attribute19;
  End If;
  If (p_rec.ptp_attribute20 = hr_api.g_varchar2) then
    p_rec.ptp_attribute20 :=
    ben_ptp_shd.g_old_rec.ptp_attribute20;
  End If;
  If (p_rec.ptp_attribute21 = hr_api.g_varchar2) then
    p_rec.ptp_attribute21 :=
    ben_ptp_shd.g_old_rec.ptp_attribute21;
  End If;
  If (p_rec.ptp_attribute22 = hr_api.g_varchar2) then
    p_rec.ptp_attribute22 :=
    ben_ptp_shd.g_old_rec.ptp_attribute22;
  End If;
  If (p_rec.ptp_attribute23 = hr_api.g_varchar2) then
    p_rec.ptp_attribute23 :=
    ben_ptp_shd.g_old_rec.ptp_attribute23;
  End If;
  If (p_rec.ptp_attribute24 = hr_api.g_varchar2) then
    p_rec.ptp_attribute24 :=
    ben_ptp_shd.g_old_rec.ptp_attribute24;
  End If;
  If (p_rec.ptp_attribute25 = hr_api.g_varchar2) then
    p_rec.ptp_attribute25 :=
    ben_ptp_shd.g_old_rec.ptp_attribute25;
  End If;
  If (p_rec.ptp_attribute26 = hr_api.g_varchar2) then
    p_rec.ptp_attribute26 :=
    ben_ptp_shd.g_old_rec.ptp_attribute26;
  End If;
  If (p_rec.ptp_attribute27 = hr_api.g_varchar2) then
    p_rec.ptp_attribute27 :=
    ben_ptp_shd.g_old_rec.ptp_attribute27;
  End If;
  If (p_rec.ptp_attribute28 = hr_api.g_varchar2) then
    p_rec.ptp_attribute28 :=
    ben_ptp_shd.g_old_rec.ptp_attribute28;
  End If;
  If (p_rec.ptp_attribute29 = hr_api.g_varchar2) then
    p_rec.ptp_attribute29 :=
    ben_ptp_shd.g_old_rec.ptp_attribute29;
  End If;
  If (p_rec.ptp_attribute30 = hr_api.g_varchar2) then
    p_rec.ptp_attribute30 :=
    ben_ptp_shd.g_old_rec.ptp_attribute30;
  End If;
  --FHR
  If (p_rec.short_name = hr_api.g_varchar2) then
    p_rec.short_name :=
    ben_ptp_shd.g_old_rec.short_name;
  End If;
  If (p_rec.short_code = hr_api.g_varchar2) then
    p_rec.short_code :=
    ben_ptp_shd.g_old_rec.short_code;
  End If;

  If (p_rec.legislation_code = hr_api.g_varchar2) then
    p_rec.legislation_code :=
          ben_ptp_shd.g_old_rec.legislation_code;
  End If;

  If (p_rec.legislation_subgroup = hr_api.g_varchar2) then
    p_rec.legislation_subgroup :=
          ben_ptp_shd.g_old_rec.legislation_subgroup;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
--
...
Procedure upd
  ...
  p_pl_typ_stat_cd               in varchar2         default hr_api.g_varchar2,
