--------------------------------------------------------
--  DDL for Package Body CN_COLLECT_AIA_OM
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CN_COLLECT_AIA_OM" AS
-- $Header: CNCOAIOMB.pls 120.0.12010000.2 2009/06/23 10:21:12 rajukum noship $
...
BEGIN
  ...
  STATEMENT := 'begin cn_collect_aia_om'||cached_org_append||
    '.collect(:err,:ret,:start,:end,:orgid); end;';

  dbms_sql.parse(c, STATEMENT, dbms_sql.native);

  dbms_sql.bind_variable(c,'err', dummy_vch, 1000);
