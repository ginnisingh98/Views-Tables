--------------------------------------------------------
--  DDL for Package Body IGS_AS_GPC_ACA_STNDG_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AS_GPC_ACA_STNDG_PKG" AS
/* $Header: IGSDI52B.pls 115.5 2002/11/28 23:23:44 nsidana ship $ */

  l_rowid VARCHAR2(25);
  old_references igs_as_gpc_aca_stndg%ROWTYPE;
  new_references igs_as_gpc_aca_stndg%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.gpc_aca_stndg_id                  := x_gpc_aca_stndg_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_as_gpc_aca_stndg
      WHERE    gpc_aca_stndg_id = x_gpc_aca_stndg_id
      FOR UPDATE NOWAIT;
  ...
  */
    CURSOR cur_rowid IS
      ...
      AND      progression_status = x_progression_status
...
IF NOT IGS_LOOKUPS_VIEW_PKG.Get_PK_For_Validation ('AS_GRADING_PERIOD',
...
BEGIN
 ...
 IF cur_rowid%FOUND THEN
   ...
   Fnd_Message.Set_Name ('IGS','IGS_AS_GSD_LVAL_FK');
   IGS_GE_MSG_STACK.ADD ;
...
END GET_FK_IGS_LOOKUPS_VIEW  ;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.gpc_aca_stndg_id
           )
         ) THEN
        fnd_message.set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
        igs_ge_msg_stack.add;
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_as_gpc_aca_stndg_s.NEXTVAL
    INTO      x_gpc_aca_stndg_id
    FROM      dual;

    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_gpc_aca_stndg_id                  => x_gpc_aca_stndg_id,
    ...
    INSERT INTO igs_as_gpc_aca_stndg (
      gpc_aca_stndg_id,
      grading_period_cd,
      progression_status,
      creation_date,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_as_gpc_aca_stndg
    WHERE rowid = x_rowid;
