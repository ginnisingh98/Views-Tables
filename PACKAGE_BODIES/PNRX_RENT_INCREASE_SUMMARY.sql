--------------------------------------------------------
--  DDL for Package Body PNRX_RENT_INCREASE_SUMMARY
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PNRX_RENT_INCREASE_SUMMARY" AS
/* $Header: PNRXRSRB.pls 120.0 2007/10/03 14:27:01 rthumma noship $ */
   ...
   IS
      ...
      l_currency_code             pn_index_leases.currency_code%TYPE;  -- slk
      ...
      term_rec                    pn_rent_increase_summary_itf%ROWTYPE;
      term_rec_null               pn_rent_increase_summary_itf%ROWTYPE;
...
--lease_number_where_clause := 'AND l.lease_num = '||'''NE213142''';
	...
	lease_date_where_clause :=    ' AND ilp.ASSESSMENT_DATE  BETWEEN '
      ...
      pnp_debug_pkg.put_log_msg (
         'lease_date_where_clause = '|| lease_date_where_clause
      );
      ...
      THEN
         location_code_field := ',ten.location_id ';
      ELSE
         location_code_from :=
                              ', PN_TENANCIES_ALL ten , PN_LOCATIONS_ALL loc';
         location_code_where :=
...
-- slk end
   ...
   EXCEPTION
      ...
      THEN
         ...
         RAISE;
         COMMIT;
   END rent_increase_summary;
END pnrx_rent_increase_summary;
