--------------------------------------------------------
--  DDL for Package Body PAY_KW_SOE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KW_SOE" AS
/* $Header: pykwsoer.pkb 120.0.12000000.3 2007/02/08 05:12:17 spendhar noship $ */
   ...
   if l_save_asg_run_bal = 'Y' then
	...
	/* Following OR condition added for SI element check to display the balances correctly */
   	...
   	If l_local_nat <> l_nat OR nvl(l_rr_processed,'*') <> 'P' then
   		/*Logic for non-Kuwaiti national */
   	        l_si_needed_chr := 'N';
	        l_st_needed_chr := 'N';
	        l_sn_needed_chr := 'N';
	        l_st2_needed_chr := 'N';
...
FUNCTION kw_loan_type (p_assignment_action_id NUMBER, p_run_result_id NUMBER , p_effective_date date)
  ...
  CURSOR csr_loan_type IS
	  SELECT hr_general.decode_lookup('KW_LOAN_TYPE',result_value)
	  FROM   pay_run_results         rr
...
,       nvl(ettl.reporting_name,ettl.element_name) || pay_kw_soe.kw_loan_type(rr.assignment_action_id ,rr.run_result_id,:effective_date  ) COL02
...
group by nvl(ettl.reporting_name,ettl.element_name) || pay_kw_soe.kw_loan_type(rr.assignment_action_id ,rr.run_result_id,:effective_date  )
, ettl.reporting_name
,nvl(ettl.reporting_name,et.element_type_id)
order by nvl(ettl.reporting_name,ettl.element_name) || pay_kw_soe.kw_loan_type(rr.assignment_action_id ,rr.run_result_id,:effective_date  )';
--
return l_sql;
--
end getElements;
