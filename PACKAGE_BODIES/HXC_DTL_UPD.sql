--------------------------------------------------------
--  DDL for Package Body HXC_DTL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_DTL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_dtl_upd.';  -- Global package name
...
Begin
  ...
  --
  --
  -- Update the hxc_alias_definitions_tl Row
  --
  update hxc_alias_definitions_tl
    set
...
Begin
  ...
  --
  convert_defs(p_rec);
  hxc_dtl_bus.update_validate
     (p_rec
     );
  --
  -- Call the supporting pre-update operation
  --
  hxc_dtl_upd.pre_update(p_rec);
  --
  -- Update the row.
  --
  hxc_dtl_upd.update_dml(p_rec);
  --
  -- Call the supporting post-update operation
  --
  hxc_dtl_upd.post_update
     (p_rec
     );
End upd;
--
  ...
  cursor csr_upd_langs is
    select dtl.language
     ...
     where dtl.alias_definition_id = p_alias_definition_id
       and p_language_code in (dtl.language
                              ,dtl.source_lang);
  --
  l_proc  varchar2(72);
  --
