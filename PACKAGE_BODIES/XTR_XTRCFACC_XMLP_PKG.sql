--------------------------------------------------------
--  DDL for Package Body XTR_XTRCFACC_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XTR_XTRCFACC_XMLP_PKG" AS
/* $Header: XTRCFACCB.pls 120.1 2007/12/28 12:43:35 npannamp noship $ */
  FUNCTION ROLLING_BAL1FORMULA(ACCOUNT_BAL_p in number) RETURN VARCHAR2 IS
  BEGIN

      SELECT (OPEN_BAL + ACCOUNT_BAL_p)/AMT_UNIT2 INTO ROLLING_BAL FROM DUAL;
  ...
  FUNCTION OPEN_BAL1FORMULA(ACCT_NOS_P in varchar2,ccy_p in varchar2,COMPANY_p in varchar2)RETURN VARCHAR2 IS
  BEGIN

SELECT nvl(round(OPENING_BALANCE,0),0)/amt_unit2 INTO OPEN_BAL FROM XTR_BANK_ACCOUNTS_V where PARTY_CODE = COMPANY_p and CURRENCY = CCY_p and ACCOUNT_NUMBER = ACCT_NOS_P;
  ...
  BEGIN
    ...
    FOR c IN GET_LANGUAGE_DESC LOOP
      ...
      ELSIF C.ITEM_NAME = 'Z1INCLUDE_CURRENCY_OPTION' THEN
        Z1INCLUDE_CURRENCY_OPTION := C.LANG_NAME;
      ELSIF C.ITEM_NAME = 'Z1PARAMETERS' THEN
        Z1PARAMETERS := C.LANG_NAME;
      ELSIF C.ITEM_NAME = 'Z1SETTLE_FROM' THEN
        Z1SETTLE_FROM := C.LANG_NAME;
      ELSIF C.ITEM_NAME = 'Z1SUMMARISE' THEN
        Z1SUMMARISE := C.LANG_NAME;
      ...
      ELSIF C.ITEM_NAME = 'Z2CCY' THEN
        Z2CCY := C.LANG_NAME;
      ELSIF C.ITEM_NAME = 'Z2CFLOWS' THEN
        Z2CFLOWS := C.LANG_NAME;
      ...
      ELSIF C.ITEM_NAME = 'Z2NOS_OF' THEN
        Z2NOS_OF := C.LANG_NAME;
  ...
  BEGIN
    ...
    ACCOUNT_NUM2 := P_BANK_ACCOUNT_NUMBER;
    ...
    IF P_EXPOSURE_TYPE = 'FIRM' THEN
      P_1 := 'and dda_deal_subtype = ''FIRM''';
    ELSIF P_EXPOSURE_TYPE = 'INDICATIVE' THEN
      P_1 := 'and dda_deal_subtype = ''INDIC''';
    ELSIF P_EXPOSURE_TYPE = 'NONE' THEN
      P_1 := 'and dda_deal_subtype not in (''FIRM'',''INDIC'')';
