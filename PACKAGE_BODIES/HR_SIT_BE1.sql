--------------------------------------------------------
--  DDL for Package Body HR_SIT_BE1
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_SIT_BE1" as 
...
p_pea_object_version_number    number) is
  l_event_key number;
  l_event_data clob;
  ...
  --
  l_proc varchar2(72):='  hr_sit_be1.create_sit_a';
begin
  hr_utility.set_location('Entering: '||l_proc,10);
  -- check the status of the business event
  l_event_name:='oracle.apps.per.api.sit.create_sit';
  ...
  if (l_message='MESSAGE') then
    ...
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
    l_text:='<id_flex_num>';
    l_text:=l_text||fnd_number.number_to_canonical(p_id_flex_num);
    l_text:=l_text||'</id_flex_num>';
    dbms_lob.writeAppend(l_event_data,length(l_text),l_text);
