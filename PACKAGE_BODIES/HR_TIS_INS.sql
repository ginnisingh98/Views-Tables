--------------------------------------------------------
--  DDL for Package Body HR_TIS_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_TIS_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hr_tis_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_topic_integrations_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_topic_integrations_id  in  number) is
--
...
Begin
  ...
  --
  hr_tis_ins.g_topic_integrations_id_i := p_topic_integrations_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: hr_ki_topic_integrations
  --
...
--
  Cursor C_Sel1 is select hr_ki_topic_integrations_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from hr_ki_topic_integrations
     where topic_integrations_id =
             hr_tis_ins.g_topic_integrations_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (hr_tis_ins.g_topic_integrations_id_i is not null) Then
    --
    ...
    p_rec.topic_integrations_id :=
      hr_tis_ins.g_topic_integrations_id_i;
    hr_tis_ins.g_topic_integrations_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.topic_integrations_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hr_tis_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hr_tis_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hr_tis_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  -- Having converted the arguments into the hr_tis_rec
  ...
  --
  p_topic_integrations_id := l_rec.topic_integrations_id;
