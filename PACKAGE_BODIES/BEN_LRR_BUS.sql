--------------------------------------------------------
--  DDL for Package Body BEN_LRR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LRR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lrr_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_lee_rsn_rl_id >------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   lee_rsn_rl_id PK of record being inserted or updated.
...
--
Procedure chk_lee_rsn_rl_id(p_lee_rsn_rl_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_lee_rsn_rl_id';
...
Begin
  ...
  --
  l_api_updating := ben_lrr_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_lee_rsn_rl_id                => p_lee_rsn_rl_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_lee_rsn_rl_id,hr_api.g_number)
     <>  ben_lrr_shd.g_old_rec.lee_rsn_rl_id) then
    --
    -- raise error as PK has changed
    --
    ben_lrr_shd.constraint_error('BEN_LER_RL_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_lee_rsn_rl_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_ordr_to_aply_num(p_lee_rsn_rl_id in number,
  ...
  cursor chk_unique is
     select null
        ...
        where ordr_to_aply_num = p_ordr_to_aply_num
          and lee_rsn_rl_id <> nvl(p_lee_rsn_rl_id, hr_api.g_number)
          and lee_rsn_id = p_lee_rsn_id  --parent
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_lrr_shd.g_old_rec.ordr_to_aply_num,hr_api.g_number)
...
--   Internal table handler use only.
--
Procedure chk_formula_id(p_lee_rsn_rl_id                 in number,
                            p_lee_rsn_id                 in number,
...
Begin
  ...
  if (l_api_updating
      and p_formula_id
      <> nvl(ben_lrr_shd.g_old_rec.formula_id,hr_api.g_number)
...
function return_legislation_code
  (p_lee_rsn_rl_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_lee_rsn_rl_f b
    where b.lee_rsn_rl_id      = p_lee_rsn_rl_id
