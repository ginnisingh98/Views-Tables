--------------------------------------------------------
--  DDL for Package Body ASP_SVCCONTRACT_ALERT_AGENT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."ASP_SVCCONTRACT_ALERT_AGENT" as
/* $Header: aspascab.pls 120.4 2005/09/13 17:20 axavier noship $ */
---------------------------------------------------------------------------
-- Package Name:   ASP_SVCCONTRACT_ALERT_AGENT
---------------------------------------------------------------------------
-- Description:
...
/*-------------------------------------------------------------------------*
 |                             Private Constants
 *-------------------------------------------------------------------------*/
G_PKG_NAME  CONSTANT VARCHAR2(30):= 'ASP_SVCCONTRACT_ALERT_AGENT';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'aspascab.pls';
...
IS
  ...
  CURSOR get_customer_id(c_contract_header_id in number) is
      ...
      AND   CPL.object1_id1 = TO_CHAR(HZP.party_id)
      and rownum < 2;
...
BEGIN
  ...
  If(funcmode = 'RUN') Then
    ...
    l_contract_header_id := wf_engine.GetItemAttrText( itemtype =>itemtype,
      ...
      BEGIN
        OPEN get_customer_id(l_contract_header_id);
...
End ASP_SVCCONTRACT_ALERT_AGENT;
