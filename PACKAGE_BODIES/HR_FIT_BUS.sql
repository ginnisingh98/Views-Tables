--------------------------------------------------------
--  DDL for Package Body HR_FIT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_FIT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hr_fit_bus.';  -- Global package name
--
...
begin
  ...
  --
  hr_fim_bus.set_security_group_id
...
Begin
  ...
  hr_utility.set_location('Entering:'|| l_proc, 10);
  --
  l_legislation_code := hr_fim_bus.return_legislation_code
    (p_form_item_id                 => p_form_item_id
    );
  --
...
Begin
  ...
  --
  IF NOT hr_fit_shd.api_updating
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 10);
  --
  l_api_updating := hr_fit_shd.api_updating
    (p_form_item_id                 => p_form_item_id
  ...
  --
  if (  (   l_api_updating
        and nvl(hr_fit_shd.g_old_rec.source_lang,hr_api.g_varchar2) <>
...
-- |--------------------------< chk_user_item_name >--------------------------|
-- ----------------------------------------------------------------------------
Procedure chk_user_item_name
  (p_form_item_id                 in number
  ,p_language                     in varchar2
  ,p_user_item_name               in varchar2
  ) is
  --
  l_proc                         varchar2(72) := g_package || 'chk_user_item_name';
  l_api_updating                 boolean;
  --
Begin
  hr_utility.set_location('Entering:'||l_proc, 10);
  ...
  hr_api.mandatory_arg_error
    (p_api_name                     => l_proc
    ,p_argument                     => 'user_item_name'
    ,p_argument_value               => p_user_item_name
    );
  --
  hr_utility.set_location('Leaving:'||l_proc, 100);
End chk_user_item_name;
--
-- ----------------------------------------------------------------------------
-- |---------------------------< chk_description >----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  chk_user_item_name
    (p_form_item_id                 => p_rec.form_item_id
    ,p_language                     => p_rec.language
    ,p_user_item_name               => p_rec.user_item_name
    );
  --
  chk_description
    (p_form_item_id                 => p_rec.form_item_id
