--------------------------------------------------------
--  DDL for Package Body OKL_DEAL_CREAT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_DEAL_CREAT_PVT" AS
/* $Header: OKLRDCRB.pls 120.5.12010000.9 2010/03/24 06:24:14 bkatraga ship $ */

    G_MODULE VARCHAR2(255) := 'okl.authoring.okl_deal_creat_pvt';
    G_DEBUG_ENABLED CONSTANT VARCHAR2(40) := OKL_DEBUG_PUB.CHECK_LOG_ENABLED;
...
-- End of comments
  ...
  BEGIN
	...
	OPEN l_source_crs;
        ...
        -- copy from template
        IF (upper(p_source_code) = 'TEMPLATE') THEN
        ...
        */
        ...
        ELSIF (upper(p_source_code) = 'COPY' ) THEN
        ...
        ELSIF (upper(p_source_code) = 'QUOTE') THEN
...
-- End of comments
  ...
  BEGIN
    ...
    IF(upper(p_source_code) <> 'NEW' AND p_source_contract_number IS NULL) THEN
	 x_return_status := OKC_API.g_ret_sts_error;
    ...
    IF(upper(p_source_code) <> 'NEW' AND p_source_contract_number IS NOT NULL) THEN
    ...
    IF (upper(p_source_code) = 'NEW') THEN
    ...
    --Added by dpsingh for LE Uptake
        ...
        -- copy from template
    ELSIF (upper(p_source_code) = 'TEMPLATE') THEN
...
-- update contract header for template_yn
   IF ( p_template_yn = 'Y' AND ( upper(p_source_code) = 'QUOTE' OR upper(p_source_code) = 'COPY' OR upper(p_source_code) = 'RE-LEASE'
      OR upper(p_source_code) = 'TEMPLATE' )) THEN
...
-- End of comments
  PROCEDURE create_deal(
	...
	SELECT
	 LSQ.ID QUOTE_ID
--	 ,LSQ.REFERENCE_NUMBER QUOTE_NUMBER
--	,LSQT.SHORT_DESCRIPTION QUOTE_DESCRIPTION
	FROM
	...
	SELECT
	 LAP.ID LEASEAPP_ID
	--,LAP.REFERENCE_NUMBER LEASEAPP_NUMBER
	--,LAPT.SHORT_DESCRIPTION LEASEAPP_DESCRIPTION
	FROM
    ...
    lp_khrv_rec OKL_CONTRACT_PUB.khrv_rec_type;
    lx_khrv_rec OKL_CONTRACT_PUB.khrv_rec_type;
    p_kpl_rec   OKL_DEAL_CREAT_PVT.party_rec_type;
    x_kpl_rec   OKL_DEAL_CREAT_PVT.party_rec_type;
    lp_org_id NUMBER;
    ...
    CURSOR c_rule_group(p_chr_id NUMBER, p_rgd_code VARCHAR2) IS
    SELECT id,rgd_code
    FROM okc_rule_groups_b
    WHERE dnz_chr_id = p_chr_id
    AND   rgd_code = p_rgd_code;

    CURSOR c_orig_system_source(p_chr_id NUMBER) IS
    SELECT orig_system_source_code
    FROM   okc_k_headers_all_b
    WHERE  id = p_chr_id;

    --Added by bkatraga for bug 9343568
    CURSOR c_check_rule(p_chr_id NUMBER, p_rgd_code VARCHAR2, p_rule_cat VARCHAR2) IS
    SELECT count(1)
     ...
     WHERE rgp.chr_id = p_chr_id
       ...
       AND rul.RULE_INFORMATION_CATEGORY = p_rule_cat;

    l_rule_count  NUMBER;
    --end bkatraga

    l_tax_schedule_yn VARCHAR2(1);
    lp_lahdtx_rgpv_rec OKL_OKC_MIGRATION_PVT.rgpv_rec_type;
    lx_lahdtx_rgpv_rec OKL_OKC_MIGRATION_PVT.rgpv_rec_type;
    lp_lahdtx_rulv_rec Okl_Rule_Pub.rulv_rec_type;
    lx_lahdtx_rulv_rec Okl_Rule_Pub.rulv_rec_type;
    l_rgp_id                  NUMBER;
  ...
  lp_org_id := p_org_id;
   ...
   IF(upper(p_source_code) <> 'NEW' AND p_source_contract_number IS NOT NULL) THEN
    ...
    IF(p_customer_name IS NULL AND p_program_name IS NULL) THEN
      ...
      IF (upper(p_source_code) = 'TEMPLATE') THEN
       ...
       CLOSE l_src_chr_id_crs;
      ELSIF (upper(p_source_code) = 'QUOTE') THEN
       OPEN l_quote_crs(lp_org_id, p_organization_id, p_source_contract_number);
       FETCH l_quote_crs INTO p_source_chr_id;
       CLOSE l_quote_crs;
      ELSIF (upper(p_source_code) = 'LEASEAPP') THEN
       OPEN l_leaseapp_crs(lp_org_id, p_organization_id, p_source_contract_number);
       FETCH l_leaseapp_crs INTO p_source_chr_id;
       CLOSE l_leaseapp_crs;
      ELSIF (upper(p_source_code) = 'PROGAGRMNTTEMP') THEN
       OPEN  l_progAgrmntTemp1_crs;
       FETCH l_progAgrmntTemp1_crs INTO p_source_chr_id;
       CLOSE l_progAgrmntTemp1_crs;
      ELSIF (upper(p_source_code) = 'LEASEAPPTEMP') THEN
    ...
    IF (p_scs_code = 'LEASE') THEN
       p_kpl_rec.object1_id1       := lp_org_id;
       p_kpl_rec.object1_id2       := '#';
       p_kpl_rec.jtot_object1_code := 'OKX_OPERUNIT';
       p_kpl_rec.rle_code          := 'LESSOR';
       p_kpl_rec.dnz_chr_id        := l_chr_id;
       p_kpl_rec.chr_id            := l_chr_id;

       OKL_DEAL_CREAT_PVT.create_party(
         p_api_version     => l_api_version,
         p_init_msg_list   => p_init_msg_list,
         x_return_status   => x_return_status,
         x_msg_count       => x_msg_count,
   ...
   -- update contract header for template_yn
   IF ( p_template_type IS NOT NULL AND ( upper(p_source_code) = 'QUOTE' OR upper(p_source_code) = 'COPY'
        OR upper(p_source_code) = 'RE-LEASE' OR upper(p_source_code) = 'TEMPLATE' OR upper(p_source_code) = 'PROGAGRMNTTEMP')) THEN
   ...
   ELSIF ( p_template_type IS NULL AND ( upper(p_source_code) = 'QUOTE' OR upper(p_source_code) = 'COPY'
           OR upper(p_source_code) = 'RE-LEASE' OR upper(p_source_code) = 'TEMPLATE' OR upper(p_source_code) = 'PROGAGRMNTTEMP'
           OR upper(p_source_code) = 'LEASEAPPTEMP')) THEN
   ...
   OPEN c_orig_system_source(x_chr_id);
   FETCH c_orig_system_source INTO l_orig_system_source_code;
   CLOSE c_orig_system_source;

   IF NVL(l_orig_system_source_code,'XXXX') <> 'OKL_IMPORT' THEN
     OPEN c_tax_schedule_yn;
     FETCH c_tax_schedule_yn INTO l_tax_schedule_yn;
     CLOSE c_tax_schedule_yn;

     OPEN c_rule_group(x_chr_id,'LAHDTX');
     FETCH c_rule_group INTO l_rgp_id,l_rgd_code;
     CLOSE c_rule_group;

     IF (NVL(l_rgd_code,'XXXXXX') <> 'LAHDTX') THEN
       -- Create rule group LAHDTX
       lp_lahdtx_rgpv_rec.id := NULL;
       lp_lahdtx_rgpv_rec.rgd_code := 'LAHDTX';
       lp_lahdtx_rgpv_rec.dnz_chr_id := x_chr_id;
       lp_lahdtx_rgpv_rec.chr_id := x_chr_id;
       lp_lahdtx_rgpv_rec.rgp_type := 'KRG';
       ...
       OKL_RULE_PUB.create_rule_group(
           ...
           x_msg_count      => x_msg_count,
           x_msg_data       => x_msg_data,
           p_rgpv_rec       => lp_lahdtx_rgpv_rec,
           x_rgpv_rec       => lx_lahdtx_rgpv_rec);
       ...
       l_rgp_id := lx_lahdtx_rgpv_rec.id;
     END IF;

     --Added by bkatraga for bug 9343568
     OPEN c_check_rule(x_chr_id,'LAHDTX','LASTPR');
     FETCH c_check_rule INTO l_rule_count;
     CLOSE c_check_rule;

     IF(l_rule_count = 0) THEN
     --end bkatraga
      -- Create rule LASTPR
      lp_lahdtx_rulv_rec.id := NULL;
      lp_lahdtx_rulv_rec.rgp_id := l_rgp_id;
      lp_lahdtx_rulv_rec.rule_information_category := 'LASTPR';
      lp_lahdtx_rulv_rec.dnz_chr_id := x_chr_id;
      lp_lahdtx_rulv_rec.rule_information5 := l_tax_schedule_yn;
      lp_lahdtx_rulv_rec.WARN_YN := 'N';
      lp_lahdtx_rulv_rec.STD_TEMPLATE_YN := 'N';
      ...
      OKL_RULE_PUB.create_rule(
          ...
          x_msg_count      => x_msg_count,
          x_msg_data       => x_msg_data,
          p_rulv_rec       => lp_lahdtx_rulv_rec,
          x_rulv_rec       => lx_lahdtx_rulv_rec);
      ...
      ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
         RAISE OKC_API.G_EXCEPTION_ERROR;
      END IF;
     END IF; --Added by bkatraga for bug 9343568
   END IF;
...
-- End of comments
  PROCEDURE update_release_contract(
    ...
    p_bill_to_site_use_id          IN  NUMBER,  -- Bug 6493413
    p_durv_rec                     IN  deal_rec_type  --Added by bkatraga for bug 9392566
    ) AS
  ...
  If(l_rbr_code is not null and l_rbr_code = 'CUSTOMER_CHANGE') Then
    ...
    -- gboomina Bug 6493413 - Start
    -- Bill To Address validation
    IF ( p_bill_to_site_use_id IS NULL) THEN
    	 x_return_status := OKC_API.g_ret_sts_error;
      l_ak_prompt := GET_AK_PROMPT('OKL_LA_LAVENB', 'OKL_LA_BILLTO');
      OKC_API.SET_MESSAGE( p_app_name => g_app_name
				                     , p_msg_name => 'OKL_REQUIRED_VALUE'
                     				, p_token1 => 'COL_NAME'
                      			, p_token1_value => l_ak_prompt
...
End If;
  ...
  If(l_rbr_code is not null and (l_rbr_code = 'CUSTOMER_CHANGE'  or  l_rbr_code = 'PRODUCT_CHANGE')) Then
    ...
    If(l_rbr_code is not null and l_rbr_code = 'CUSTOMER_CHANGE') Then
     ...
     lp_chrv_rec.cust_acct_id := l_cust_acct_id;
    -- gboomina Bug 6493413 - Start
    -- populating Bill to Site Use Id as Bill To Address is updatable during
    -- Release Customer Change
     lp_chrv_rec.bill_to_site_use_id := p_bill_to_site_use_id;
    -- gboomina Bug 6493413 - End
    ...
    --Added by bkatraga for bug 9392566
    lp_khrv_rec.attribute_category := p_durv_rec.attribute_category;
    lp_khrv_rec.attribute1 := p_durv_rec.attribute1;
    lp_khrv_rec.attribute2 := p_durv_rec.attribute2;
    lp_khrv_rec.attribute3 := p_durv_rec.attribute3;
    lp_khrv_rec.attribute4 := p_durv_rec.attribute4;
    lp_khrv_rec.attribute5 := p_durv_rec.attribute5;
    lp_khrv_rec.attribute6 := p_durv_rec.attribute6;
    lp_khrv_rec.attribute7 := p_durv_rec.attribute7;
    lp_khrv_rec.attribute8 := p_durv_rec.attribute8;
    lp_khrv_rec.attribute9 := p_durv_rec.attribute9;
    lp_khrv_rec.attribute10 := p_durv_rec.attribute10;
    lp_khrv_rec.attribute11 := p_durv_rec.attribute11;
    lp_khrv_rec.attribute12 := p_durv_rec.attribute12;
    lp_khrv_rec.attribute13 := p_durv_rec.attribute13;
    lp_khrv_rec.attribute14 := p_durv_rec.attribute14;
    lp_khrv_rec.attribute15 := p_durv_rec.attribute15;
    --end bkatraga
  ...
  PROCEDURE update_deal(
	...
	*/
    ...
    CURSOR c_lessor(p_chr_id NUMBER) IS
    SELECT id
    FROM okc_k_party_roles_b
    WHERE dnz_chr_id = p_chr_id
    AND rle_code ='LESSOR';
    ...
    CURSOR c_orig_pdt_csr(p_chr_id IN NUMBER) IS
    SELECT pdt_id
    FROM okl_k_headers
    WHERE id = p_chr_id;

    l_orig_pdt_rec c_orig_pdt_csr%ROWTYPE;
    ...
    -- ER# 9115610 - Start
	CURSOR c_chk_pdt_effective(cp_pdt_id OKL_PRODUCTS.ID%TYPE, cp_effective_date okl_k_headers_full_v.start_date%type) IS
	SELECT 'Y' FROM
	   OKL_PRODUCTS
	  WHERE ID = cp_pdt_id
	    AND cp_effective_date BETWEEN FROM_DATE AND NVL(TO_DATE,cp_effective_date);

	l_chk_pdt VARCHAR(1) DEFAULT NULL;
    -- ER# 9115610 - End
  ...
  BEGIN
    ...
    x_return_status := OKC_API.START_ACTIVITY(p_api_name      => l_api_name,
                                              p_pkg_name      => g_pkg_name,
                                              p_init_msg_list => p_init_msg_list,
                                              l_api_version   => l_api_version,
                                              p_api_version   => p_api_version,
    ...
    -- sjalasut, initialize chr_id so that the Entity Object in the java layer can populate
    -- the if back in the VO
    x_durv_rec.chr_id := p_durv_rec.chr_id;

    -- ER# 9115610 - Start
	OPEN c_chk_pdt_effective(p_durv_rec.khr_pdt_id, p_durv_rec.chr_start_date);
	  FETCH c_chk_pdt_effective INTO l_chk_pdt;
	CLOSE c_chk_pdt_effective;

	IF l_chk_pdt IS NULL THEN
       OKL_API.SET_MESSAGE(p_app_name => g_app_name, p_msg_name => 'OKL_INACTIVE_PDT');
	   RAISE OKC_API.G_EXCEPTION_ERROR;
	END IF;
    -- ER# 9115610 - End
    ...
    If(l_orig_sys_src_code is not null and l_orig_sys_src_code = 'OKL_RELEASE') Then
      ...
      update_release_contract(p_api_version    	=> p_api_version,
        p_init_msg_list  	=> p_init_msg_list,
        x_return_status  	=> x_return_status,
        x_msg_count      	=> x_msg_count,
        x_msg_data       	=> x_msg_data,
        ...
        p_bill_to_site_use_id   => p_durv_rec.labill_labacc_billto, -- Bug 6493413
        p_durv_rec              => p_durv_rec  --Added by bkatraga for bug 9392566
        );
      IF (x_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
        RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
      ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
    ...
    Else
      ...
      x_durv_rec.chr_TEMPLATE_YN      :=  p_durv_rec.chr_template_yn;
     ...
     -- Added for Bill-To site
     lp_chrv_rec.bill_to_site_use_id := p_durv_rec.labill_labacc_billto;

     -- sjalasut, added code below to populate the attribute columns for lp_khrv_rec
     -- changes made as part of Authoring OA Migration. START changes
     lp_khrv_rec.attribute_category := p_durv_rec.attribute_category;
     lp_khrv_rec.attribute1 := p_durv_rec.attribute1;
     lp_khrv_rec.attribute2 := p_durv_rec.attribute2;
     lp_khrv_rec.attribute3 := p_durv_rec.attribute3;
     ...
     lp_khrv_rec.attribute14 := p_durv_rec.attribute14;
     lp_khrv_rec.attribute15 := p_durv_rec.attribute15;
     -- sjalasut, added code above to populate the attribute columns for lp_khrv_rec
     -- changes made as part of Authoring OA Migration. END  changes
   ...
   */
   ...
   --Bug# 7440232 start
   OPEN c_orig_pdt_csr(p_chr_id => lp_chrv_rec.id);
   FETCH c_orig_pdt_csr INTO l_orig_pdt_rec;
   CLOSE c_orig_pdt_csr;

   IF (NVL(l_orig_pdt_rec.pdt_id, OKL_API.G_MISS_NUM) <>
       NVL(lp_khrv_rec.pdt_id, OKL_API.G_MISS_NUM)) THEN

     l_count := 0;
     FOR l_rates_rec IN c_rates_csr(lp_chrv_rec.id, 'ACTUAL') LOOP
        l_count := l_count + 1;
        l_krpdel_tbl(l_count).khr_id := lp_chrv_rec.id;
...
-- contact
   IF (p_durv_rec.contact_id IS NULL AND p_durv_rec.contact_name IS NOT NULL ) THEN
    ...
    lp_ctcv_rec.id := NULL;
    OPEN c_lessor(p_durv_rec.chr_id);
    FETCH c_lessor INTO lp_ctcv_rec.cpl_id;
    CLOSE c_lessor;
...
Begin
    ...
    OKL_COPY_CONTRACT_PUB.copy_lease_contract_new(
      ...
      p_contract_number          => p_contract_number,
      p_contract_number_modifier => NULL,
      p_renew_ref_yn             => OKL_API.G_FALSE,
      p_trans_type               => 'CFA',
      x_chr_id                   => x_chr_id);
...
Begin
    ...
    lp_cplv_rec.cognomen:= p_kpl_rec.cognomen;
    lp_cplv_rec.alias:= p_kpl_rec.alias;
...
Begin
  ...
  l_return_status        VARCHAR2(1) default OKL_API.G_RET_STS_SUCCESS;
  l_api_name             CONSTANT VARCHAR2(30) := 'load_deal';
  l_api_version          CONSTANT NUMBER := 1.0;

  CURSOR c_hdr_full_uv(p_chr_id OKC_K_HEADERS_ALL_B.ID%TYPE) IS
  SELECT *
  FROM okl_k_hdrs_full_uv
  WHERE chr_id = p_chr_id;

  l_hdr_uv_rec c_hdr_full_uv%ROWTYPE;

  CURSOR c_hdr_full_v(p_chr_id OKC_K_HEADERS_ALL_B.ID%TYPE) IS
  SELECT *
  FROM okl_k_headers_full_v
  WHERE id = p_chr_id;

  l_hdr_rec c_hdr_full_v%ROWTYPE;

  CURSOR c_okc_hdr(p_chr_id OKC_K_HEADERS_ALL_B.ID%TYPE) IS
  SELECT contract_number
  FROM okc_k_headers_all_b
  WHERE id = p_chr_id;

  CURSOR c_bill_to_site(p_site_use_id NUMBER) IS
  SELECT description
  FROM okl_la_bill_to_uv
  WHERE id1 = p_site_use_id;

  CURSOR c_gvr_data(p_scs_code   VARCHAR2,
                    p_chr_id     NUMBER,
                    p_dnz_chr_id NUMBER) IS
  SELECT mla.contract_number,gvr.chr_id_referred,gvr.id
  FROM  okl_k_headers_full_v mla,
        okc_governances_v gvr
  WHERE mla.id = gvr.chr_id_referred
  AND   mla.scs_code = p_scs_code
  AND   gvr.dnz_chr_id = p_dnz_chr_id
  AND   gvr.chr_id = p_chr_id;

  CURSOR c_ledger(p_chr_id NUMBER, p_organization_id NUMBER) IS
--  start:|   21-Nov-2008 cklee Bug 7569200 -- Fixed display LEDGER NAME issue         |
--  SELECT bks.ledger_id,bks.short_name,ou.name
  SELECT bks.ledger_id,bks.name,ou.name
--  end:|   21-Nov-2008 cklee Bug 7569200 -- Fixed display LEDGER NAME issue         |

  FROM gl_ledgers_public_v bks,
       hr_operating_units ou,
       okc_k_headers_b chr
  WHERE bks.ledger_id = ou.set_of_books_id
  AND ou.organization_id = chr.authoring_org_id
  AND chr.authoring_org_id = p_organization_id
  AND chr.id = p_chr_id;

--start|   27-Jun-2008 cklee Bug 6957971                                            |
/*cklee 06/27/08
  CURSOR c_legal_address(p_party_id NUMBER) IS
  SELECT cust_site.description
  FROM okx_cust_site_uses_v cust_site
  WHERE cust_site.party_id = p_party_id
  AND cust_site.site_use_code = 'LEGAL';
*/
-- Each account may have one primary legal address (purpose = Legal)
-- So when there have multiple accounts for the customer, we need
-- to add additional cust_account_id as a key to retrieve specific
-- legal address for the account
  CURSOR c_legal_address(p_party_id NUMBER, cp_cust_acct_id NUMBER) IS
  SELECT cust_site.description
  FROM okx_cust_site_uses_v cust_site
  WHERE cust_site.party_id = p_party_id
  AND cust_site.cust_account_id = cp_cust_acct_id
  AND cust_site.site_use_code = 'LEGAL';
--end|   27-Jun-2008 cklee Bug 6957971                                            |

  CURSOR c_le_name(cp_legal_entity_id NUMBER) IS
  SELECT name
  FROM xle_entity_profiles
  WHERE legal_entity_id = cp_legal_entity_id;

  CURSOR c_quote_name(p_chr_orig_system_id1 NUMBER) IS
  SELECT lsq.reference_number
  FROM okl_lease_quotes_b lsq
  WHERE LSQ.parent_object_id = p_chr_orig_system_id1
  AND lsq.status = 'CT-ACCEPTED';

  CURSOR c_program_template_name(p_orig_system_id1 NUMBER, p_khr_khr_id NUMBER) IS
  SELECT chr.contract_number
  FROM okl_k_headers_full_v chr
  WHERE chr.id = p_orig_system_id1
  AND chr.template_type_code = 'PROGRAM'
  AND NVL(chr.template_yn,'N') = 'Y'
  AND EXISTS(SELECT 1
             FROM okl_vp_associations vpaso
	     WHERE vpaso.chr_id =  p_khr_khr_id
	     AND vpaso.assoc_object_id = p_orig_system_id1
	     AND vpaso.assoc_object_type_code = 'LC_TEMPLATE');

  CURSOR c_orig_lease_application(p_orig_syststem_id1 NUMBER) IS
  SELECT LAP.reference_number
  FROM OKL_LEASE_APPLICATIONS_B LAP,
       OKL_LEASE_APPLICATIONS_TL LAPT
  WHERE LAP.id = LAPT.id
  AND LAPT.LANGUAGE = USERENV('LANG')
  AND lap.id = p_orig_syststem_id1;

  CURSOR c_cust_name(cp_party_id NUMBER) IS
  SELECT hp.party_name
  FROM hz_parties hp
  WHERE hp.party_id = cp_party_id;

  CURSOR c_cust_acct_num_csr(cp_cust_acct_id NUMBER)IS
  ...
  OPEN c_hdr_full_uv(p_chr_id);
  FETCH c_hdr_full_uv INTO l_hdr_uv_rec;

  OPEN c_hdr_full_v(p_chr_id);
  FETCH c_hdr_full_v INTO l_hdr_rec;

  X_DEAL_VALUES_REC.ACCEPTANCE_METHOD_MEANING     :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_ACCEPTANCE_METHOD',L_HDR_UV_REC.KHR_AMD_CODE);
  X_DEAL_VALUES_REC.ASSIGNABLE_MEANING            :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',l_HDR_UV_REC.KHR_ASSIGNABLE_YN);
  X_DEAL_VALUES_REC.BILL_TO_SITE_USE_ID           :=  L_HDR_REC.BILL_TO_SITE_USE_ID;

  OPEN c_bill_to_site(l_HDR_REC.BILL_TO_SITE_USE_ID);
  FETCH c_bill_to_site INTO X_DEAL_VALUES_REC.BILL_TO_ADDRESS_DESC;
  CLOSE c_bill_to_site;

  X_DEAL_VALUES_REC.BOOK_CLASS_MEANING           :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_BOOK_CLASS',L_HDR_UV_REC.KHR_DEAL_TYPE);
  X_DEAL_VALUES_REC.CAP_INTERIM_INTERST_MEANING  :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',l_HDR_UV_REC.R_LACPLN_RULE_INFORMATION1);
  X_DEAL_VALUES_REC.CAP_INTERIM_INT_RGD_CODE     :=  'LACPLN';
  X_DEAL_VALUES_REC.CAP_INTERIM_INT_RGP_ID       :=  L_HDR_UV_REC.RG_LACPLN_ID;
  X_DEAL_VALUES_REC.CAP_INTERIM_INT_RUL_ID       :=  L_HDR_UV_REC.R_LACPLN_ID;
  X_DEAL_VALUES_REC.CAP_INTERIM_INT_RUL_INF1     :=  L_HDR_UV_REC.R_LACPLN_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.CAP_INTERIM_INT_RUL_INF_CAT  :=  'LACPLN';
  X_DEAL_VALUES_REC.CONSUMER_CREDIT_ACT_MEANING  :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',L_HDR_UV_REC.KHR_CREDIT_ACT_YN);
  X_DEAL_VALUES_REC.CONVERTED_ACCT_MEANING       :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',L_HDR_UV_REC.KHR_CONVERTED_ACCOUNT_YN);

  OPEN c_gvr_data('CREDITLINE_CONTRACT',L_HDR_UV_REC.CHR_ID,L_HDR_UV_REC.CHR_ID);
  FETCH c_gvr_data INTO X_DEAL_VALUES_REC.CREDIT_LINE_CONTRACT_NUMBER,X_DEAL_VALUES_REC.CREDIT_LINE_CHR_ID,X_DEAL_VALUES_REC.CREDIT_GVR_ID;
  CLOSE c_gvr_data;

  X_DEAL_VALUES_REC.CURRENCY_CONV_TYPE_MEANING    :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_CONVERSION_TYPE_CODE',L_HDR_UV_REC.KHR_CURRENCY_CONVERSION_TYPE);

  -- splitting the customer name and customer account number cursors to support
  -- ui query to show customer name without customer account.
  OPEN c_cust_name(L_HDR_UV_REC.CUST_OBJECT1_ID1);
  FETCH c_cust_name INTO X_DEAL_VALUES_REC.CUSTOMER_NAME;
  CLOSE c_cust_name;

  OPEN c_cust_acct_num_csr(L_HDR_UV_REC.CHR_CUST_ACCT_ID);
  FETCH c_cust_acct_num_csr INTO X_DEAL_VALUES_REC.CUSTOMER_ACCOUNT;
  CLOSE c_cust_acct_num_csr;

  X_DEAL_VALUES_REC.CUSTOMER_CPL_ID              :=  L_HDR_UV_REC.CUST_ID;
  IF(L_HDR_UV_REC.CUST_JTOT_OBJECT1_CODE IS NULL)THEN
    X_DEAL_VALUES_REC.CUSTOMER_JTOT_OBJECT1_CODE   := 'OKX_PARTY';
  ELSE
    X_DEAL_VALUES_REC.CUSTOMER_JTOT_OBJECT1_CODE   :=  L_HDR_UV_REC.CUST_JTOT_OBJECT1_CODE;
  END IF;

  X_DEAL_VALUES_REC.CUSTOMER_OBJECT1_ID1         :=  L_HDR_UV_REC.CUST_OBJECT1_ID1;
  X_DEAL_VALUES_REC.CUSTOMER_OBJECT1_ID2         :=  L_HDR_UV_REC.CUST_OBJECT1_ID2;
  X_DEAL_VALUES_REC.CUST_ACCT_ID                 :=  L_HDR_UV_REC.CHR_CUST_ACCT_ID;
  X_DEAL_VALUES_REC.CUST_PO_NUMBER               :=  L_HDR_UV_REC.CHR_CUST_PO_NUMBER;
  X_DEAL_VALUES_REC.DEAL_TYPE                    :=  L_HDR_UV_REC.KHR_DEAL_TYPE;

  X_DEAL_VALUES_REC.DESCRIPTION                  :=  L_HDR_UV_REC.CHR_DESCRIPTION;
  X_DEAL_VALUES_REC.ELIG_FOR_PREFUNDING_MEANING  :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',L_HDR_UV_REC.KHR_PREFUNDING_ELIGIBLE_YN);
  X_DEAL_VALUES_REC.ID                           :=  L_HDR_UV_REC.CHR_ID;
  X_DEAL_VALUES_REC.INTEREST_CALC_MEANING         :=  L_HDR_UV_REC.INTEREST_CALCULATION_MEANING;

  OPEN c_ledger(l_hdr_rec.id,l_hdr_rec.authoring_org_id);
  FETCH c_ledger INTO X_DEAL_VALUES_REC.LEDGER_ID, X_DEAL_VALUES_REC.LEDGER_NAME, X_DEAL_VALUES_REC.OPERATING_UNIT_NAME;
  CLOSE c_ledger;

  IF (l_HDR_UV_REC.CHR_ORIG_SYSTEM_SOURCE_CODE = 'OKL_IMPORT') THEN
      X_DEAL_VALUES_REC.LEGACY_NUMBER             :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_REFERENCE1;
  END IF;

--start|   27-Jun-2008 cklee Bug 6957971                                            |
  OPEN c_legal_address(l_HDR_UV_REC.cust_object1_id1,
                       L_HDR_UV_REC.CHR_CUST_ACCT_ID); -- cklee 06/27/08
  FETCH c_legal_address INTO X_DEAL_VALUES_REC.LEGAL_ADDRESS;
  CLOSE c_legal_address;
--end|   27-Jun-2008 cklee Bug 6957971                                            |


  X_DEAL_VALUES_REC.LEGAL_ADDRESS_ID              :=  NULL;

  OPEN c_le_name(l_hdr_rec.legal_entity_id);
  FETCH c_le_name INTO X_DEAL_VALUES_REC.LEGAL_ENTITY_NAME;
  CLOSE c_le_name;

  X_DEAL_VALUES_REC.LESSOR_INSURED_MEANING        :=  NULL;
  X_DEAL_VALUES_REC.LESSOR_PAYEE_MEANING          :=  NULL;
  X_DEAL_VALUES_REC.LESSOR_SERV_ORG_CODE          :=  NULL;

  OPEN c_gvr_data('MASTER_LEASE',l_HDR_UV_REC.CHR_ID,l_HDR_UV_REC.CHR_ID);
  FETCH c_gvr_data INTO X_DEAL_VALUES_REC.MLA_CONTRACT_NUMBER,X_DEAL_VALUES_REC.MLA_CHR_ID,X_DEAL_VALUES_REC.MLA_GVR_ID;
  CLOSE c_gvr_data;

  X_DEAL_VALUES_REC.NNTF_RGD_CODE               :=  'LANNTF';
  X_DEAL_VALUES_REC.NNTF_RGP_ID                 :=  L_HDR_UV_REC.RG_LANNTF_ID;
  X_DEAL_VALUES_REC.NNTF_RUL_ID                 :=  L_HDR_UV_REC.R_LANNTF_ID;
  X_DEAL_VALUES_REC.NNTF_RUL_INF1               :=  L_HDR_UV_REC.R_LANNTF_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.NNTF_RUL_INF_CAT            :=  'LANNTF';
  X_DEAL_VALUES_REC.NON_NOTIFICATION_MEANING    :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',L_HDR_UV_REC.R_LANNTF_RULE_INFORMATION1);

  FOR c_prompt_rec in c_prompt_name
  LOOP
    IF (c_prompt_rec.rule_code = 'LACPLN') THEN
        X_DEAL_VALUES_REC.COL_LACPLN_FORM_LEFT_PROMPT :=  C_PROMPT_REC.FORM_LEFT_PROMPT;
    ELSIF (c_prompt_rec.rule_code = 'LANNTF') THEN
        X_DEAL_VALUES_REC.COL_LANNTF_FORM_LEFT_PROMPT :=  C_PROMPT_REC.FORM_LEFT_PROMPT;
    ELSIF (c_prompt_rec.rule_code = 'LAREBL') THEN
        X_DEAL_VALUES_REC.COL_LAREBL_FORM_LEFT_PROMPT :=  C_PROMPT_REC.FORM_LEFT_PROMPT;
    ELSIF (c_prompt_rec.rule_code = 'LARLES') THEN
        X_DEAL_VALUES_REC.COL_LARLES_FORM_LEFT_PROMPT :=  C_PROMPT_REC.FORM_LEFT_PROMPT;
    ELSIF (c_prompt_rec.rule_code = 'LATOWN') THEN
        X_DEAL_VALUES_REC.COL_LATOWN_FORM_LEFT_PROMPT :=  C_PROMPT_REC.FORM_LEFT_PROMPT;
    ELSIF (c_prompt_rec.rule_code = 'LAPACT') THEN
        X_DEAL_VALUES_REC.COL_LAPACT_FORM_LEFT_PROMPT :=  C_PROMPT_REC.FORM_LEFT_PROMPT;
    END IF;
  END LOOP;

  IF (l_HDR_UV_REC.CHR_ORIG_SYSTEM_SOURCE_CODE = 'OKL_QUOTE') THEN
     X_DEAL_VALUES_REC.ORIGINATION_QUOTE_ID       :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1;
     OPEN c_quote_name(l_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1);
     FETCH c_quote_name INTO X_DEAL_VALUES_REC.ORIGINATION_QUOTE_NAME;
     CLOSE c_quote_name;
  END IF;

  X_DEAL_VALUES_REC.ORIG_SYSTEM_ID1               :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1;
  X_DEAL_VALUES_REC.ORIG_SYSTEM_REFERENCE1        :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_REFERENCE1;
  X_DEAL_VALUES_REC.ORIG_SYSTEM_SOURCE_CODE       :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_SOURCE_CODE;

  IF (l_HDR_UV_REC.CHR_ORIG_SYSTEM_SOURCE_CODE = 'OKL_LEASE_APP') THEN
      OPEN c_orig_lease_application(L_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1);
      FETCH c_orig_lease_application INTO X_DEAL_VALUES_REC.ORIGINATION_LEASE_APPLICATION;
      X_DEAL_VALUES_REC.LEASE_APPLICATION_ID   := L_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1;
      X_DEAL_VALUES_REC.LEASE_APPLICATION_NAME := X_DEAL_VALUES_REC.ORIGINATION_LEASE_APPLICATION;
      CLOSE c_orig_lease_application;
  END IF;

  X_DEAL_VALUES_REC.PRIVATE_ACT_BOND_MEANING    :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',L_HDR_UV_REC.R_LAPACT_RULE_INFORMATION1);
  X_DEAL_VALUES_REC.PRODUCT_DESCRIPTION         :=  L_HDR_UV_REC.PRODUCT_DESCRIPTION;
  X_DEAL_VALUES_REC.PRODUCT_NAME                :=  L_HDR_UV_REC.PRODUCT_NAME;

  IF ((L_HDR_UV_REC.CHR_ORIG_SYSTEM_SOURCE_CODE = 'OKC_HDR') AND (L_HDR_UV_REC.KHR_KHR_ID IS NOT NULL))THEN
     X_DEAL_VALUES_REC.PROGRAM_TEMPLATE_CHR_ID    :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1;
     OPEN c_program_template_name(l_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1,l_HDR_UV_REC.KHR_KHR_ID);
     FETCH c_program_template_name INTO X_DEAL_VALUES_REC.PROGRAM_TEMPLATE_NAME;
     CLOSE c_program_template_name;
  END IF;

  X_DEAL_VALUES_REC.PRV_ACT_BOND_RGD_CODE         :=  'LAPACT';
  X_DEAL_VALUES_REC.PRV_ACT_BOND_RGP_ID           :=  L_HDR_UV_REC.RG_LAPACT_ID;
  X_DEAL_VALUES_REC.PRV_ACT_BOND_RUL_ID           :=  L_HDR_UV_REC.R_LAPACT_ID;
  X_DEAL_VALUES_REC.PRV_ACT_BOND_RUL_INF1         :=  L_HDR_UV_REC.R_LAPACT_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.PRV_ACT_BOND_RUL_INF_CAT      :=  'LAPACT';

  X_DEAL_VALUES_REC.REBOOK_LIMIT_DATE             :=  TO_CHAR(TO_DATE(L_HDR_UV_REC.R_LAREBL_RULE_INFORMATION1,'YYYY/MM/DD'));
  X_DEAL_VALUES_REC.REBOOK_LIMIT_DATE_RGD_CODE    :=  'LAREBL';
  X_DEAL_VALUES_REC.REBOOK_LIMIT_DATE_RGP_ID      :=  L_HDR_UV_REC.RG_LAREBL_ID;
  X_DEAL_VALUES_REC.REBOOK_LIMIT_DATE_RUL_ID      :=  L_HDR_UV_REC.R_LAREBL_ID;
  X_DEAL_VALUES_REC.REBOOK_LIMIT_RUL_INF1         :=  L_HDR_UV_REC.R_LAREBL_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.REBOOK_LIMIT_RUL_INF_CAT      :=  'LAREBL';

  IF (L_HDR_UV_REC.CHR_ORIG_SYSTEM_SOURCE_CODE = 'OKL_RELEASE') THEN
     X_DEAL_VALUES_REC.REPLACES_CHR_ID            :=  L_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1;
     OPEN c_okc_hdr(l_HDR_UV_REC.CHR_ORIG_SYSTEM_ID1);
     FETCH c_okc_hdr INTO X_DEAL_VALUES_REC.REPLACES_CONTRACT_NUMBER;
     CLOSE c_okc_hdr;
  END IF;

  X_DEAL_VALUES_REC.REP_CONTACT_ID                :=  L_HDR_UV_REC.CONTACT_ID;
  X_DEAL_VALUES_REC.REP_CONTACT_JTOT_OBJECT1_CODE :=  L_HDR_UV_REC.CONTACT_JTOT_OBJECT1_CODE;
  X_DEAL_VALUES_REC.REP_CONTACT_OBJECT1_ID1       :=  L_HDR_UV_REC.CONTACT_OBJECT1_ID1;

  IF (L_HDR_UV_REC.CONTACT_OBJECT1_ID2 <> '#') THEN
      X_DEAL_VALUES_REC.REP_CONTACT_OBJECT1_ID2   :=  L_HDR_UV_REC.CONTACT_OBJECT1_ID2;
  END IF;

  X_DEAL_VALUES_REC.REVENUE_RECOGNITION_MEANING   :=  L_HDR_UV_REC.REVENUE_RECOGNITION_MEANING;
  X_DEAL_VALUES_REC.REVOLVING_CREDIT_YN           :=  L_HDR_UV_REC.KHR_REVOLVING_CREDIT_YN;

  X_DEAL_VALUES_REC.RLES_RGD_CODE                 :=  'LARLES';
  X_DEAL_VALUES_REC.RLES_RGP_ID                   :=  L_HDR_UV_REC.RG_LARLES_ID;
  X_DEAL_VALUES_REC.RLES_RUL_ID                   :=  L_HDR_UV_REC.R_LARLES_ID;
  X_DEAL_VALUES_REC.RLES_RUL_INF1                 :=  L_HDR_UV_REC.R_LARLES_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.RLES_RUL_INF_CAT              :=  'LARLES';
  X_DEAL_VALUES_REC.RELEASED_ASSET_MEANING        :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_YES_NO',L_HDR_UV_REC.R_LARLES_RULE_INFORMATION1);

  OPEN c_salesrep(L_HDR_UV_REC.contact_object1_id1);
  FETCH c_salesrep INTO X_DEAL_VALUES_REC.SALES_REPRESENTATIVE_NAME;
  CLOSE c_salesrep;

  X_DEAL_VALUES_REC.SCS_CODE_MEANING            :=  NULL;
  X_DEAL_VALUES_REC.SPLIT_FROM_CHR_ID           :=  NULL;
  X_DEAL_VALUES_REC.SPLIT_FROM_CONTRACT_NUMBER  :=  NULL;
  X_DEAL_VALUES_REC.STS_CODE_MEANING            :=  L_HDR_UV_REC.CHR_STS_MEANING;

  X_DEAL_VALUES_REC.TAX_OWNER_CODE            :=  L_HDR_UV_REC.R_LATOWN_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.TAX_OWNER_MEANING         :=  OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_TAX_OWNER',l_HDR_UV_REC.R_LATOWN_RULE_INFORMATION1);
  X_DEAL_VALUES_REC.TAX_OWNER_RGD_CODE        :=  'LATOWN';
  X_DEAL_VALUES_REC.TAX_OWNER_RGP_ID          :=  L_HDR_UV_REC.RG_LATOWN_ID;
  X_DEAL_VALUES_REC.TAX_OWNER_RUL_ID          :=  L_HDR_UV_REC.R_LATOWN_ID;
  X_DEAL_VALUES_REC.TAX_OWNER_RUL_INF1        :=  L_HDR_UV_REC.R_LATOWN_RULE_INFORMATION1;
  X_DEAL_VALUES_REC.TAX_OWNER_RUL_INF_CAT     :=  'LATOWN';

  X_DEAL_VALUES_REC.UPG_ORIG_SYSTEM_REF       :=  NULL;
  X_DEAL_VALUES_REC.UPG_ORIG_SYSTEM_REF_ID    :=  NULL;
  X_DEAL_VALUES_REC.VPA_CONTRACT_NUMBER       :=  L_HDR_UV_REC.PROGRAM_CONTRACT_NUMBER;
  X_DEAL_VALUES_REC.VPA_KHR_ID                :=  L_HDR_UV_REC.KHR_KHR_ID;
  X_DEAL_VALUES_REC.VERS_VERSION              :=  L_HDR_UV_REC.VERS_VERSION;

  -- sjalasut, added product subclass code which will serve as a criteria to the Product List of values page
  X_DEAL_VALUES_REC.PRODUCT_SUBCLASS_CODE     := 'LEASE';

  --Bug# 8370699
  X_DEAL_VALUES_REC.LAST_ACTIVATION_DATE      := OKL_LLA_UTIL_PVT.GET_LAST_ACTIVATION_DATE(p_chr_id => p_chr_id);
  ...
  PROCEDURE load_booking_summary(
      ...
      x_msg_data                   OUT NOCOPY VARCHAR2,
      p_chr_id                     IN  NUMBER,
      x_booking_summary_rec        OUT NOCOPY booking_summary_rec) IS

  l_return_status        VARCHAR2(1) default OKL_API.G_RET_STS_SUCCESS;
  l_api_name             CONSTANT VARCHAR2(30) := 'load_booking_summary';
  l_api_version          CONSTANT NUMBER := 1.0;

  CURSOR c_khr_currency(p_chr_id IN NUMBER) IS
  SELECT CURRENCY_CODE
  FROM   OKC_K_HEADERS_B
  WHERE  ID = p_chr_id;

  CURSOR c_residual_amt(p_chr_id NUMBER) IS
  SELECT SUM(kle.residual_value) total_residual_amount
  ...
  CURSOR c_eot(p_chr_id NUMBER) IS
  SELECT rul.rule_information1 eot_option_code,
         rul.rule_information2 eot_option_amount
  FROM   okc_rule_groups_b rgp,
         okc_rules_b rul
  WHERE  rgp.dnz_chr_id = p_chr_id
  AND    rgp.rgd_code = 'AMTFOC'
  AND    rgp.dnz_chr_id = rgp.dnz_chr_id
  AND    rul.rgp_id = rgp.id
  AND    rul.rule_information_category = 'AMBPOC';

  CURSOR c_upfront_tax(p_chr_id NUMBER) IS
  ...
  CURSOR c_rvi_premium(p_chr_id NUMBER) IS
  SELECT kle.amount rvi_premium
  ...
  l_eot_option_code       okc_rules_b.rule_information1%TYPE;
  l_currency_code         okc_k_headers_b.currency_code%TYPE;
  l_total_financed_amount NUMBER;
  l_total_funded          NUMBER;
  l_total_subsidies       NUMBER;
  l_eot_amount            NUMBER;
  l_upfront_sales_tax     NUMBER;
  l_rvi_premium           NUMBER;
  l_residual_amount       NUMBER;

  BEGIN
    IF( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
          FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'OKL_DEAL_CREAT_PVT.load_booking_summary.', 'Begin(+)');
     END IF;
    ...
    IF( FND_LOG.LEVEL_STATEMENT >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
            FND_LOG.STRING(FND_LOG.LEVEL_STATEMENT, 'OKL_DEAL_CREAT_PVT.load_booking_summary.',
                   'p_chr_id :'||p_chr_id);
    END IF;

    x_booking_summary_rec.dnz_chr_id := p_chr_id;

    OPEN c_khr_currency(p_chr_id => p_chr_id);
    FETCH c_khr_currency INTO l_currency_code;
    CLOSE c_khr_currency;
    ...
    OKL_EXECUTE_FORMULA_PUB.execute(
                                   ...
                                   p_line_id              => NULL,
                                   x_value                => l_total_financed_amount);
    ...
    -- Format the financed amt
    x_booking_summary_rec.total_financed_amount := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_total_financed_amount,
                                                            l_currency_code);
    ...
    OKL_EXECUTE_FORMULA_PUB.execute(
                                   ...
                                   x_msg_count            => x_msg_count,
                                   x_msg_data             => x_msg_data,
                                   p_formula_name         => 'CONTRACT_TOT_FNDED',
                                   p_contract_id          => p_chr_id,
                                   p_line_id              => NULL,
                                   x_value                => l_total_funded);
    ...
    -- Format the funded amt
    x_booking_summary_rec.total_funded := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_total_funded,
                                                            l_currency_code);

    OPEN c_residual_amt(p_chr_id => p_chr_id);
    FETCH c_residual_amt INTO l_residual_amount;
    CLOSE c_residual_amt;
    -- Format the residual amt
    x_booking_summary_rec.total_residual_amount := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_residual_amount,
                                                            l_currency_code);

    OKL_SUBSIDY_PROCESS_PVT.get_contract_subsidy_amount(
                                   p_api_version       => p_api_version,
                                   p_init_msg_list     => p_init_msg_list,
                                   x_return_status     => x_return_status,
                                   x_msg_count         => x_msg_count,
                                   x_msg_data          => x_msg_data,
                                   p_chr_id            => p_chr_id,
                                   p_accounting_method => NULL,
                                   x_subsidy_amount    => l_total_subsidies);
    ...
    -- Format the subsidy amt
    x_booking_summary_rec.total_subsidies := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_total_subsidies,
                                                            l_currency_code);

    OPEN c_eot(p_chr_id => p_chr_id);
    FETCH c_eot INTO l_eot_option_code,l_eot_amount;
    CLOSE c_eot;
    -- Format the end of term amt
    x_booking_summary_rec.eot_amount := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_eot_amount,
                                                            l_currency_code);

    x_booking_summary_rec.eot_option := OKL_LLA_UTIL_PVT.GET_LOOKUP_MEANING('OKL_QUOTE_RULE_OPTION_TYPE',l_eot_option_code);

    OPEN c_upfront_tax(p_chr_id => p_chr_id);
    FETCH c_upfront_tax INTO l_upfront_sales_tax;
    CLOSE c_upfront_tax;
    -- Format the upfront tax amt
    x_booking_summary_rec.total_upfront_sales_tax := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_upfront_sales_tax,
                                                            l_currency_code);

    OPEN c_rvi_premium(p_chr_id => p_chr_id);
    FETCH c_rvi_premium INTO l_rvi_premium;
    CLOSE c_rvi_premium;
    -- Format the rvi premium amt
    x_booking_summary_rec.rvi_premium := OKL_ACCOUNTING_UTIL.format_amount(
                                                            l_rvi_premium,
                                                            l_currency_code);
    ...
    IF( FND_LOG.LEVEL_PROCEDURE >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
          FND_LOG.STRING(FND_LOG.LEVEL_PROCEDURE, 'OKL_DEAL_CREAT_PVT.load_booking_summary.', 'End(-)');
     END IF;

    EXCEPTION
    WHEN OKL_API.G_EXCEPTION_ERROR THEN
         IF c_khr_currency%ISOPEN THEN
            CLOSE c_khr_currency;
         END IF;
         IF c_residual_amt%ISOPEN THEN
            CLOSE c_residual_amt;
         END IF;
         IF c_eot%ISOPEN THEN
            CLOSE c_eot;
         END IF;
         IF c_upfront_tax%ISOPEN THEN
            CLOSE c_upfront_tax;
         END IF;
         IF c_rvi_premium%ISOPEN THEN
            CLOSE c_rvi_premium;
         END IF;
         x_return_status := OKL_API.HANDLE_EXCEPTIONS(
                                    l_api_name,
                                    ...
                                    '_PVT');
         IF( FND_LOG.LEVEL_STATEMENT >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
              FND_LOG.STRING(FND_LOG.LEVEL_STATEMENT, 'OKL_DEAL_CREAT_PVT.load_booking_summary.', 'EXP - ERROR');
         END IF;
    WHEN OKL_API.G_EXCEPTION_UNEXPECTED_ERROR THEN
         IF c_khr_currency%ISOPEN THEN
            CLOSE c_khr_currency;
         ...
         x_return_status :=OKL_API.HANDLE_EXCEPTIONS(
                                   ...
                                   '_PVT');
         IF( FND_LOG.LEVEL_STATEMENT >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
              FND_LOG.STRING(FND_LOG.LEVEL_STATEMENT, 'OKL_DEAL_CREAT_PVT.load_booking_summary.', 'EXP - UNEXCP ERROR');
         END IF;
    WHEN OTHERS THEN
         IF c_khr_currency%ISOPEN THEN
            CLOSE c_khr_currency;
         ...
         x_return_status :=OKL_API.HANDLE_EXCEPTIONS(
                                   ...
                                   '_PVT');
         IF( FND_LOG.LEVEL_STATEMENT >= FND_LOG.G_CURRENT_RUNTIME_LEVEL) THEN
              FND_LOG.STRING(FND_LOG.LEVEL_STATEMENT, 'OKL_DEAL_CREAT_PVT.load_booking_summary.', 'EXP - OTHERS');
         END IF;
       END load_booking_summary;
