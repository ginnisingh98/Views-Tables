--------------------------------------------------------
--  DDL for Package Body BEN_XCV_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_XCV_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_xcv_ins.';  -- Global package name
...
Begin
  ...
  --
  -- Insert the row into: ben_ext_crit_val
  --
...
Begin
  ...
  --
  open c1;
  fetch c1 into p_rec.ext_crit_val_id;
  close c1;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End pre_insert;
...
--
  ...
  --
  create_app_ownerships('EXT_CRIT_VAL_ID', p_rec.ext_crit_val_id);
  --
...
Begin
  ...
  --
  p_ext_crit_val_id := l_rec.ext_crit_val_id;
