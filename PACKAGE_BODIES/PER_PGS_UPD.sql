--------------------------------------------------------
--  DDL for Package Body PER_PGS_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PGS_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_pgs_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the per_grade_spines_f Row
    --
    ...
    ,ceiling_step_id                      = p_rec.ceiling_step_id
    ,starting_step                        = p_rec.starting_step
    ,request_id                           = p_rec.request_id
    ,program_application_id               = p_rec.program_application_id
    ,program_id                           = p_rec.program_id
    ,program_update_date                  = p_rec.program_update_date
    ,object_version_number                = p_rec.object_version_number
    where   grade_spine_id = p_rec.grade_spine_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    per_pgs_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_base_key_value         => p_rec.grade_spine_id
...
Begin
  ...
  begin
    ...
    per_pgs_rku.after_update
      ...
      => per_pgs_shd.g_old_rec.effective_end_date
      ,p_business_group_id_o
      => per_pgs_shd.g_old_rec.business_group_id
      ,p_parent_spine_id_o
      => per_pgs_shd.g_old_rec.parent_spine_id
      ,p_grade_id_o
      => per_pgs_shd.g_old_rec.grade_id
      ,p_ceiling_step_id_o
      => per_pgs_shd.g_old_rec.ceiling_step_id
      ,p_request_id_o
      => per_pgs_shd.g_old_rec.request_id
      ,p_program_application_id_o
      => per_pgs_shd.g_old_rec.program_application_id
      ,p_program_id_o
      => per_pgs_shd.g_old_rec.program_id
      ,p_program_update_date_o
      => per_pgs_shd.g_old_rec.program_update_date
      ,p_object_version_number_o
      => per_pgs_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.grade_id = hr_api.g_number) then
    ...
    per_pgs_shd.g_old_rec.grade_id;
  End If;
  If (p_rec.ceiling_step_id = hr_api.g_number) then
    p_rec.ceiling_step_id :=
    per_pgs_shd.g_old_rec.ceiling_step_id;
  End If;
  If (p_rec.starting_step = hr_api.g_number) then
    p_rec.starting_step :=
    per_pgs_shd.g_old_rec.starting_step;
  End If;
...
Begin
  ...
  --
  per_pgs_upd.convert_defs(p_rec);
  --
  per_pgs_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
