--------------------------------------------------------
--  DDL for Package Body BEN_ECC_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_ECC_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ecc_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_ecc_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_elctbl_chc_ctfn Row
  --
  update ben_elctbl_chc_ctfn
  set
  elctbl_chc_ctfn_id                = p_rec.elctbl_chc_ctfn_id,
  ...
  ecc_attribute_category            = p_rec.ecc_attribute_category,
  ecc_attribute1                    = p_rec.ecc_attribute1,
  ecc_attribute2                    = p_rec.ecc_attribute2,
  ecc_attribute3                    = p_rec.ecc_attribute3,
  ecc_attribute4                    = p_rec.ecc_attribute4,
  ecc_attribute5                    = p_rec.ecc_attribute5,
  ecc_attribute6                    = p_rec.ecc_attribute6,
  ecc_attribute7                    = p_rec.ecc_attribute7,
  ecc_attribute8                    = p_rec.ecc_attribute8,
  ecc_attribute9                    = p_rec.ecc_attribute9,
  ecc_attribute10                   = p_rec.ecc_attribute10,
  ecc_attribute11                   = p_rec.ecc_attribute11,
  ecc_attribute12                   = p_rec.ecc_attribute12,
  ecc_attribute13                   = p_rec.ecc_attribute13,
  ecc_attribute14                   = p_rec.ecc_attribute14,
  ecc_attribute15                   = p_rec.ecc_attribute15,
  ecc_attribute16                   = p_rec.ecc_attribute16,
  ecc_attribute17                   = p_rec.ecc_attribute17,
  ecc_attribute18                   = p_rec.ecc_attribute18,
  ecc_attribute19                   = p_rec.ecc_attribute19,
  ecc_attribute20                   = p_rec.ecc_attribute20,
  ecc_attribute21                   = p_rec.ecc_attribute21,
  ecc_attribute22                   = p_rec.ecc_attribute22,
  ecc_attribute23                   = p_rec.ecc_attribute23,
  ecc_attribute24                   = p_rec.ecc_attribute24,
  ecc_attribute25                   = p_rec.ecc_attribute25,
  ecc_attribute26                   = p_rec.ecc_attribute26,
  ecc_attribute27                   = p_rec.ecc_attribute27,
  ecc_attribute28                   = p_rec.ecc_attribute28,
  ecc_attribute29                   = p_rec.ecc_attribute29,
  ecc_attribute30                   = p_rec.ecc_attribute30,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_ecc_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_ecc_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.enrt_ctfn_typ_cd = hr_api.g_varchar2) then
    ...
    ben_ecc_shd.g_old_rec.enrt_ctfn_typ_cd;
  End If;
  If (p_rec.rqd_flag = hr_api.g_varchar2) then
    p_rec.rqd_flag :=
    ben_ecc_shd.g_old_rec.rqd_flag;
  End If;
  If (p_rec.elig_per_elctbl_chc_id = hr_api.g_number) then
    p_rec.elig_per_elctbl_chc_id :=
    ben_ecc_shd.g_old_rec.elig_per_elctbl_chc_id;
  End If;
  If (p_rec.enrt_bnft_id = hr_api.g_number) then
    p_rec.enrt_bnft_id :=
    ben_ecc_shd.g_old_rec.enrt_bnft_id;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_ecc_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.ecc_attribute_category = hr_api.g_varchar2) then
    p_rec.ecc_attribute_category :=
    ben_ecc_shd.g_old_rec.ecc_attribute_category;
  End If;
  If (p_rec.ecc_attribute1 = hr_api.g_varchar2) then
    p_rec.ecc_attribute1 :=
    ben_ecc_shd.g_old_rec.ecc_attribute1;
  End If;
  If (p_rec.ecc_attribute2 = hr_api.g_varchar2) then
    p_rec.ecc_attribute2 :=
    ben_ecc_shd.g_old_rec.ecc_attribute2;
  End If;
  If (p_rec.ecc_attribute3 = hr_api.g_varchar2) then
    p_rec.ecc_attribute3 :=
    ben_ecc_shd.g_old_rec.ecc_attribute3;
  End If;
  If (p_rec.ecc_attribute4 = hr_api.g_varchar2) then
    p_rec.ecc_attribute4 :=
    ben_ecc_shd.g_old_rec.ecc_attribute4;
  End If;
  If (p_rec.ecc_attribute5 = hr_api.g_varchar2) then
    p_rec.ecc_attribute5 :=
    ben_ecc_shd.g_old_rec.ecc_attribute5;
  End If;
  If (p_rec.ecc_attribute6 = hr_api.g_varchar2) then
    p_rec.ecc_attribute6 :=
    ben_ecc_shd.g_old_rec.ecc_attribute6;
  End If;
  If (p_rec.ecc_attribute7 = hr_api.g_varchar2) then
    p_rec.ecc_attribute7 :=
    ben_ecc_shd.g_old_rec.ecc_attribute7;
  End If;
  If (p_rec.ecc_attribute8 = hr_api.g_varchar2) then
    p_rec.ecc_attribute8 :=
    ben_ecc_shd.g_old_rec.ecc_attribute8;
  End If;
  If (p_rec.ecc_attribute9 = hr_api.g_varchar2) then
    p_rec.ecc_attribute9 :=
    ben_ecc_shd.g_old_rec.ecc_attribute9;
  End If;
  If (p_rec.ecc_attribute10 = hr_api.g_varchar2) then
    p_rec.ecc_attribute10 :=
    ben_ecc_shd.g_old_rec.ecc_attribute10;
  End If;
  If (p_rec.ecc_attribute11 = hr_api.g_varchar2) then
    p_rec.ecc_attribute11 :=
    ben_ecc_shd.g_old_rec.ecc_attribute11;
  End If;
  If (p_rec.ecc_attribute12 = hr_api.g_varchar2) then
    p_rec.ecc_attribute12 :=
    ben_ecc_shd.g_old_rec.ecc_attribute12;
  End If;
  If (p_rec.ecc_attribute13 = hr_api.g_varchar2) then
    p_rec.ecc_attribute13 :=
    ben_ecc_shd.g_old_rec.ecc_attribute13;
  End If;
  If (p_rec.ecc_attribute14 = hr_api.g_varchar2) then
    p_rec.ecc_attribute14 :=
    ben_ecc_shd.g_old_rec.ecc_attribute14;
  End If;
  If (p_rec.ecc_attribute15 = hr_api.g_varchar2) then
    p_rec.ecc_attribute15 :=
    ben_ecc_shd.g_old_rec.ecc_attribute15;
  End If;
  If (p_rec.ecc_attribute16 = hr_api.g_varchar2) then
    p_rec.ecc_attribute16 :=
    ben_ecc_shd.g_old_rec.ecc_attribute16;
  End If;
  If (p_rec.ecc_attribute17 = hr_api.g_varchar2) then
    p_rec.ecc_attribute17 :=
    ben_ecc_shd.g_old_rec.ecc_attribute17;
  End If;
  If (p_rec.ecc_attribute18 = hr_api.g_varchar2) then
    p_rec.ecc_attribute18 :=
    ben_ecc_shd.g_old_rec.ecc_attribute18;
  End If;
  If (p_rec.ecc_attribute19 = hr_api.g_varchar2) then
    p_rec.ecc_attribute19 :=
    ben_ecc_shd.g_old_rec.ecc_attribute19;
  End If;
  If (p_rec.ecc_attribute20 = hr_api.g_varchar2) then
    p_rec.ecc_attribute20 :=
    ben_ecc_shd.g_old_rec.ecc_attribute20;
  End If;
  If (p_rec.ecc_attribute21 = hr_api.g_varchar2) then
    p_rec.ecc_attribute21 :=
    ben_ecc_shd.g_old_rec.ecc_attribute21;
  End If;
  If (p_rec.ecc_attribute22 = hr_api.g_varchar2) then
    p_rec.ecc_attribute22 :=
    ben_ecc_shd.g_old_rec.ecc_attribute22;
  End If;
  If (p_rec.ecc_attribute23 = hr_api.g_varchar2) then
    p_rec.ecc_attribute23 :=
    ben_ecc_shd.g_old_rec.ecc_attribute23;
  End If;
  If (p_rec.ecc_attribute24 = hr_api.g_varchar2) then
    p_rec.ecc_attribute24 :=
    ben_ecc_shd.g_old_rec.ecc_attribute24;
  End If;
  If (p_rec.ecc_attribute25 = hr_api.g_varchar2) then
    p_rec.ecc_attribute25 :=
    ben_ecc_shd.g_old_rec.ecc_attribute25;
  End If;
  If (p_rec.ecc_attribute26 = hr_api.g_varchar2) then
    p_rec.ecc_attribute26 :=
    ben_ecc_shd.g_old_rec.ecc_attribute26;
  End If;
  If (p_rec.ecc_attribute27 = hr_api.g_varchar2) then
    p_rec.ecc_attribute27 :=
    ben_ecc_shd.g_old_rec.ecc_attribute27;
  End If;
  If (p_rec.ecc_attribute28 = hr_api.g_varchar2) then
    p_rec.ecc_attribute28 :=
    ben_ecc_shd.g_old_rec.ecc_attribute28;
  End If;
  If (p_rec.ecc_attribute29 = hr_api.g_varchar2) then
    p_rec.ecc_attribute29 :=
    ben_ecc_shd.g_old_rec.ecc_attribute29;
  End If;
  If (p_rec.ecc_attribute30 = hr_api.g_varchar2) then
    p_rec.ecc_attribute30 :=
    ben_ecc_shd.g_old_rec.ecc_attribute30;
  End If;
  If (p_rec.susp_if_ctfn_not_prvd_flag = hr_api.g_varchar2) then
    p_rec.susp_if_ctfn_not_prvd_flag :=
    ben_ecc_shd.g_old_rec.susp_if_ctfn_not_prvd_flag;
  End If;
  If (p_rec.ctfn_determine_cd = hr_api.g_varchar2) then
    p_rec.ctfn_determine_cd :=
    ben_ecc_shd.g_old_rec.ctfn_determine_cd;
  End If;
  If (p_rec.request_id = hr_api.g_number) then
    p_rec.request_id :=
    ben_ecc_shd.g_old_rec.request_id;
  End If;
  If (p_rec.program_application_id = hr_api.g_number) then
    p_rec.program_application_id :=
    ben_ecc_shd.g_old_rec.program_application_id;
  End If;
  If (p_rec.program_id = hr_api.g_number) then
    p_rec.program_id :=
    ben_ecc_shd.g_old_rec.program_id;
  End If;
  If (p_rec.program_update_date = hr_api.g_date) then
    p_rec.program_update_date :=
    ben_ecc_shd.g_old_rec.program_update_date;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_ecc_bus.update_validate(p_rec
  ,p_effective_date);
  --
