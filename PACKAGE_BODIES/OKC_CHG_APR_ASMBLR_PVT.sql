--------------------------------------------------------
--  DDL for Package Body OKC_CHG_APR_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_CHG_APR_ASMBLR_PVT" AS
/* $Header: OKCRCAAB.pls 120.0 2005/05/25 22:36:23 appldev noship $ */
...
PROCEDURE acn_assemble(
  ...
  p_estimated_amount		IN NUMBER  )  IS
...
BEGIN
 ...
 l_corrid_rec.corrid := 'CH_APP' ;

 -- check if action is enabled
 IF OKC_K_SIGN_ASMBLR_PVT.isActionEnabled(l_corrid_rec.corrid) = 'Y' THEN

    l_msg_tbl := okc_aq_pvt.msg_tab_typ();
  FOR corr_rec IN cur_corr_csr LOOP

 IF corr_rec.element_name = 'CHAPP_DATE' THEN
  l_msg_tbl.extend;
  l_msg_tbl(i).element_name     := corr_rec.element_name;
  IF corr_rec.format_mask IS NOT NULL THEN
       l_msg_tbl(i).element_value := to_char(p_chapp_date,
		      corr_rec.format_mask);
  ELSE
       l_msg_tbl(i).element_value := to_char(p_chapp_date,'DD-MON-YY');
  END IF;
  --l_msg_tbl(i).element_value    := P_chapp_date;
 ELSIF corr_rec.element_name = 'CHANGE_ID' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
  l_msg_tbl(i).element_value   := P_change_id;
 ELSIF corr_rec.element_name = 'K_CLASS' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
