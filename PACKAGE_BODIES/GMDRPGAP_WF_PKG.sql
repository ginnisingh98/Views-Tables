--------------------------------------------------------
--  DDL for Package Body GMDRPGAP_WF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMDRPGAP_WF_PKG" AS
/* $Header: GMDRPGAB.pls 120.0 2005/05/25 19:40:48 appldev noship $ */
   PROCEDURE wf_init (
   ...
   IS
      ...
      l_itemtype                WF_ITEMS.ITEM_TYPE%TYPE :=  'GMDRPGAP';
      ...
      /* Krishna Prasad , Modified added orgn_id for WF Convergence */
      ...
      l_WorkflowProcess   VARCHAR2(30) := 'GMDRPGAP_PROCESS';
      l_count             NUMBER;
      BEGIN
      ...
      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
        					  aname => 'GMDRPGAP_OWNER_ID',
         					  avalue => l_performer_name);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRPGAP_RECIPE_DESC',
         					  avalue => l_recipe_description);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRPGAP_CREATION_ORG_CODE',
         					  avalue => l_creation_orgn_code);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRPGAP_ROUTING_NO',
         					  avalue => l_routing_no);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRPGAP_ROUTING_VERS',
         					  avalue => l_routing_vers);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRPGAP_FORMULA_NO',
         					  avalue => l_formula_no);

      WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRPGAP_FORMULA_VERS',
  ...
  EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDRPGAP_wf_pkg','wf_init',l_itemtype,l_itemkey,'Initial' );
      raise;
...
/* ######################################################################## */
   ...
   IS
      ...
      l_recipe_id         GMD_RECIPES_B.recipe_id%TYPE:=to_number(wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_RECIPE_ID'));
      p_data_string       VARCHAR2(2000);
      p_wf_data_string    VARCHAR2(2000);
      p_lab_wf_item_type  VARCHAR2(8)  := 'GMDRPGAP';  -- Recipe Lab use Approval Workflow Inernal Name
      P_lab_Process_name  VARCHAR2(32) := 'GMDRPGAP_PROCESS'; -- Recipe Lab use Approval Workflow Process Inernal Name
      P_lab_activity_name VARCHAR2(80) := 'GMDRPGAP_NOTI_REQUEST';
     ...
     IF (p_funcmode = 'RUN') THEN
         ...
         THEN
	      ...
	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDRPGAP_ADDL_TEXT',
                                      avalue => l_data_string);

	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDRPGAP_APPROVER',
                                      avalue => p_role);

            p_resultout:='COMPLETE:Y';
         ELSE
           l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_TARGET_STATUS');
...
EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDRPGAP_wf_pkg','is_approval_req',p_itemtype,p_itemkey,p_role);
      raise;
END is_approval_req;
...
PROCEDURE REMINDAR_CHECK (
      ...
      l_mesg_cnt      number:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDRPGAP_MESG_CNT');
      l_approver      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_APPROVER');
BEGIN
...
PROCEDURE REQ_APPROVED (
  ...
  l_target_status     GMD_STATUS_NEXT.TARGET_STATUS%TYPE;
  l_recipe_id         GMD_RECIPES_B.recipe_id%TYPE:=to_number(wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_RECIPE_ID'));
 /* added variables for recipe generation B3604554*/
...
BEGIN
	...
	END IF; /* IF cur_recipe_enable%FOUND */
...
BEGIN
     IF (p_funcmode = 'RUN') THEN
          l_start_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_START_STATUS');
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_TARGET_STATUS');
...
PROCEDURE MOREINFO_RESPONSE  (
      ...
      p_funcmode      IN VARCHAR2,
      p_resultout     OUT NOCOPY VARCHAR2) IS
      l_requester     VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_REQUSTER');
BEGIN
...
PROCEDURE APPEND_COMMENTS (
      ...
      l_comment       VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_COMMENT');
      l_mesg_comment  VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_DISP_COMMENT');
      l_performer      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRPGAP_CURR_PERFORMER');
BEGIN
...
--           l_html_mesg := replace(l_mesg_comment,wf_core.newline,'<BR>'||wf_core.newline);
	   ...
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDRPGAP_DISP_COMMENT',
                                   avalue => l_mesg_comment);
--	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
--                                   itemkey => p_itemkey,
--         			           aname => 'GMDRPGAP_HTML_DISP_COMMENT',
--                                   avalue => l_html_mesg);
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDRPGAP_COMMENT',
                                   avalue => l_comment);
       END IF;
END;
