--------------------------------------------------------
--  DDL for Package Body PAY_KR_YEA20070101_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_KR_YEA20070101_PKG" as
/* $Header: pykryea6.pkb 120.7.12010000.6 2008/08/06 07:45:43 ubhat ship $ */
-- Constant
	-- Bug 3966549
	...
	if (p_yea_info.nationality = 'F' and p_yea_info.fixed_tax_rate = 'Y') then
		...
		if p_yea_info.non_resident_flag = 'N' then
			...
			p_yea_info.total_credit_card_exp :=p_yea_info.emp_cre_card_direct_exp2004 + p_yea_info.dpnt_cre_card_direct_exp2004
			                                   + p_yea_info.giro_tuition_paid_exp2004 + p_yea_info.cash_receipt_exp2005 - l_double_exem_amt ;                -- Bug 5682131 l_double_exem_amt added for Bug 6630135
			...
			----------------------------------------------------------------
			if (p_effective_date >= fnd_date.canonical_to_date('2007/09/01') and p_yea_info.small_bus_install > 0) then
