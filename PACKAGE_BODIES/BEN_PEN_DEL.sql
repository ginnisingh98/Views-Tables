--------------------------------------------------------
--  DDL for Package Body BEN_PEN_DEL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PEN_DEL" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_pen_del.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_delete_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode <> 'ZAP') then
    --
    p_rec.effective_start_date := ben_pen_shd.g_old_rec.effective_start_date;
    --
...
Begin
  ...
  delete_dml
    (p_rec			 => p_rec,
     ...
     p_validation_end_date	 => l_validation_end_date);
  --
  -- Check multi_rows_edit validation
  --
  if (ben_prtt_enrt_result_api.g_multi_rows_validate) then
        ben_pen_bus.multi_rows_edit
          (p_person_id         => ben_pen_shd.g_old_rec.person_id,
           p_effective_date    => p_effective_date,
           p_business_group_id => ben_pen_shd.g_old_rec.business_group_id,
           p_pgm_id 	       => ben_pen_shd.g_old_rec.pgm_id
          );
  end if;
...
--
  ...
  if HRI_BPL_BEN_UTIL.enable_ben_col_evt_que then
      hr_utility.set_location('Logging PEN delete event'||l_proc, 5);
      hri_opl_ben_elig_enrl_eq.delete_event (
                 p_rec              => ben_pen_shd.g_old_rec ,
                 p_effective_date   => p_effective_date,
                 p_datetrack_mode   => p_datetrack_mode );
  end if;
--
