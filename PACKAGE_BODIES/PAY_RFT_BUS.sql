--------------------------------------------------------
--  DDL for Package Body PAY_RFT_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_RFT_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_rft_bus.';  -- Global package name
--
--  ---------------------------------------------------------------------------
...
--
-- Description:
--   This procedure checks the validity of the display_name
...
--
-- Post Failure:
--   Failure might occur if the display_name is null.
...
Begin
  ...
  --
        hr_api.mandatory_arg_error
                   ( p_api_name       =>  l_proc
                    ,p_argument       =>  'DISPLAY_NAME'
                    ,p_argument_value =>  p_display_name
                   );
  --
  hr_utility.set_location(' Leaving:'|| l_proc, 20);
  --
...
--   report_format_mapping_id and language.
...
Begin
  ...
  --
        ...
        if csr_unique_key%found then
             ...
             fnd_message.set_token( 'COL2' , 'LANGUAGE');
             fnd_message.set_token( 'COL1_VAL', p_report_format_mapping_id);
             fnd_message.set_token( 'COL2_VAL', p_language);
             fnd_message.raise_error ;
...
Begin
  ...
  --
  IF NOT pay_rft_shd.api_updating
