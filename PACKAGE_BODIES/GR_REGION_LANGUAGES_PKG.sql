--------------------------------------------------------
--  DDL for Package Body GR_REGION_LANGUAGES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_REGION_LANGUAGES_PKG" AS
/*$Header: GRHIRGLB.pls 115.1 2002/10/25 17:26:46 gkelly ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_region_languages
   		  	     (region_code,
   		  	      language,
				  created_by,
				  creation_date,
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_region_languages
   WHERE  	   rowid = p_rowid;
...
**		  p_language is the language code to check.
...
CURSOR c_get_region_rowid
 IS
   SELECT rc.rowid
   FROM	  gr_region_languages rc
   WHERE  rc.region_code = p_region_code
   AND	  rc.language = p_language;
