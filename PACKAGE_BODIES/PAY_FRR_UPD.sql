--------------------------------------------------------
--  DDL for Package Body PAY_FRR_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_FRR_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_frr_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the pay_formula_result_rules_f Row
    --
...
Begin
  ...
  If (p_rec.element_type_id = hr_api.g_number) then
    ...
    pay_frr_shd.g_old_rec.element_type_id;
  End If;
  If (p_rec.status_processing_rule_id = hr_api.g_number) then
    p_rec.status_processing_rule_id :=
    pay_frr_shd.g_old_rec.status_processing_rule_id;
  End If;
...
Begin
  ...
  --
  pay_frr_upd.convert_defs(p_rec);
  --
  pay_frr_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
Procedure upd
  ...
  ,p_status_processing_rule_id    in     number    default hr_api.g_number
