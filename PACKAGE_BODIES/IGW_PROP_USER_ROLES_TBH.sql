--------------------------------------------------------
--  DDL for Package Body IGW_PROP_USER_ROLES_TBH
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGW_PROP_USER_ROLES_TBH" as
 /* $Header: igwtpurb.pls 115.4 2002/11/15 00:47:12 ashkumar ship $*/
...
cursor c is select ROWID from IGW_PROP_USER_ROLES
...
insert into igw_prop_user_roles (
  ...
  EXCEPTION
      when others then
      fnd_msg_pub.add_exc_msg(p_pkg_name 		=> 	'IGW_PROP_USER_ROLES_TBH',
...
x_return_status := fnd_api.g_ret_sts_success;
     ...
     if(p_mode = 'I') then
      ...
      update IGW_PROP_USER_ROLES set
...
END UPDATE_ROW;
----------------------------------------------------------------------------------------------------
-------- Since the primary key is updateable on the front end rowid is REQUIRED for doing delete ------
