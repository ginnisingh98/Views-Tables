--------------------------------------------------------
--  DDL for Package Body AME_GPI_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_GPI_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_gpi_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    --
    -- Update the ame_approval_group_items Row
    --
    update  ame_approval_group_items
    set
     approval_group_item_id               = p_rec.approval_group_item_id
    ,approval_group_id                    = p_rec.approval_group_id
    ,parameter_name                       = p_rec.parameter_name
    ,parameter                            = p_rec.parameter
    ,order_number                         = p_rec.order_number
    ,security_group_id                    = p_rec.security_group_id
    ,object_version_number                = p_rec.object_version_number
    where   approval_group_item_id  =   p_rec.approval_group_item_id
...
Begin
  ...
  begin
    ...
    ame_gpi_rku.after_update
      ...
      => p_rec.object_version_number
      ,p_approval_group_id_o
      => ame_gpi_shd.g_old_rec.approval_group_id
      ,p_parameter_name_o
      => ame_gpi_shd.g_old_rec.parameter_name
      ,p_parameter_o
      => ame_gpi_shd.g_old_rec.parameter
      ,p_order_number_o
      => ame_gpi_shd.g_old_rec.order_number
      ,p_start_date_o
      => ame_gpi_shd.g_old_rec.start_date
      ,p_end_date_o
      => ame_gpi_shd.g_old_rec.end_date
      ,p_security_group_id_o
      => ame_gpi_shd.g_old_rec.security_group_id
      ,p_object_version_number_o
      => ame_gpi_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  --
  If (p_rec.approval_group_id = hr_api.g_number) then
    p_rec.approval_group_id :=
    ame_gpi_shd.g_old_rec.approval_group_id;
  End If;
  If (p_rec.parameter_name = hr_api.g_varchar2) then
    ...
    ame_gpi_shd.g_old_rec.parameter_name;
  End If;
  If (p_rec.parameter = hr_api.g_varchar2) then
    p_rec.parameter :=
    ame_gpi_shd.g_old_rec.parameter;
  End If;
  If (p_rec.order_number = hr_api.g_number) then
    p_rec.order_number :=
    ame_gpi_shd.g_old_rec.order_number;
  End If;
...
Begin
  ...
  --
  ame_gpi_upd.convert_defs(p_rec);
  --
  ame_gpi_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
