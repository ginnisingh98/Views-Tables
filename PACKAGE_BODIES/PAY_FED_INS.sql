--------------------------------------------------------
--  DDL for Package Body PAY_FED_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_FED_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_fed_ins.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_insert_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
--
  Cursor C_Sel1 Is
    select t.created_by,
           t.creation_date
    from   pay_us_emp_fed_tax_rules_f t
    where  t.emp_fed_tax_rule_id       = p_rec.emp_fed_tax_rule_id
    and    t.effective_start_date =
             pay_fed_shd.g_old_rec.effective_start_date
    and    t.effective_end_date   = (p_validation_start_date - 1);
--
  l_proc            varchar2(72) := g_package||'dt_insert_dml';
  l_created_by          pay_us_emp_fed_tax_rules_f.created_by%TYPE;
  l_creation_date       pay_us_emp_fed_tax_rules_f.creation_date%TYPE;
  l_last_update_date    pay_us_emp_fed_tax_rules_f.last_update_date%TYPE;
  l_last_updated_by     pay_us_emp_fed_tax_rules_f.last_updated_by%TYPE;
  l_last_update_login   pay_us_emp_fed_tax_rules_f.last_update_login%TYPE;
--
Begin
  ...
  --
  -- Insert the row into: pay_us_emp_fed_tax_rules_f
  --
  ...
  (   p_rec.emp_fed_tax_rule_id,
      ...
      nvl(p_rec.fit_exempt, 'N'),
      nvl(p_rec.futa_tax_exempt, 'N'),
      nvl(p_rec.medicare_tax_exempt, 'N'),
      nvl(p_rec.ss_tax_exempt, 'N'),
      nvl(p_rec.wage_exempt, 'N'),
      nvl(p_rec.statutory_employee, 'N'),
      p_rec.w2_filed_year,
...
--
  Cursor C_Sel1 is select pay_us_emp_fed_tax_rules_s.nextval from sys.dual;
--
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.emp_fed_tax_rule_id;
  Close C_Sel1;
  --
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  p_emp_fed_tax_rule_id         := l_rec.emp_fed_tax_rule_id;
