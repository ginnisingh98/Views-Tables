--------------------------------------------------------
--  DDL for Package Body BEN_PRB_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PRB_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_prb_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< chk_pl_regy_bod_id >------------------------|
-- ----------------------------------------------------------------------------
...
--
-- In Parameters
--   pl_regy_bod_id PK of record being inserted or updated.
...
--
Procedure chk_pl_regy_bod_id(p_pl_regy_bod_id              in number,
                             p_effective_date              in date,
                             p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_pl_regy_bod_id';
...
Begin
  ...
  --
  l_api_updating := ben_prb_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_pl_regy_bod_id              => p_pl_regy_bod_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_pl_regy_bod_id,hr_api.g_number)
     <>  ben_prb_shd.g_old_rec.pl_regy_bod_id) then
    --
    -- raise error as PK has changed
    --
    ben_prb_shd.constraint_error('BEN_PL_REGY_BOD_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_pl_regy_bod_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_rptg_grp_id (p_pl_regy_bod_id        in number,
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_prb_shd.g_old_rec.rptg_grp_id,hr_api.g_number)
     or not l_api_updating) and
     p_rptg_grp_id is not null then
    --
    ...
    open c1;
      ...
      if c1%notfound then
        ...
        -- table.
        --
        ben_prb_shd.constraint_error('BEN_PL_REGY_BOD_FK1');
        --
      end if;
      --
    close c1;
...
-- In Parameters
--   p_pl_regy_bod_id PK
--   p_organization_id ID FK column
--   p_effective_date session date
--   p_object_version_number object version number
--
-- Post Success
...
--   Internal table handler use only.
--
Procedure chk_organization_id (p_pl_regy_bod_id        in number,
...
Begin
  ...
  if (l_api_updating
     ...
     <> nvl(ben_prb_shd.g_old_rec.organization_id,hr_api.g_number)
     or not l_api_updating) and
...
-- In Parameters
--   pl_regy_bod_id PK of record being inserted or updated.
--   quald_flag Value of lookup code.
...
--   Internal table handler use only.
--
Procedure chk_quald_flag(p_pl_regy_bod_id           in number,
                         p_quald_flag               in varchar2,
                         p_effective_date           in date,
                         p_object_version_number    in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_quald_flag';
  l_api_updating boolean;
  --
Begin
  --
  ...
  if (l_api_updating
      and p_quald_flag
      <> nvl(ben_prb_shd.g_old_rec.quald_flag,hr_api.g_varchar2)
      or not l_api_updating)
      and p_quald_flag is not null then
    --
    -- check if value of lookup falls within lookup type.
    --
    if hr_api.not_exists_in_hr_lookups
          (p_lookup_type    => 'YES_NO',
           p_lookup_code    => p_quald_flag,
           p_effective_date => p_effective_date) then
      --
      -- raise error as does not exist as lookup
      --
      fnd_message.set_name('BEN','BEN_91095_INVLD_QUALIFIED_FLAG');
      fnd_message.raise_error;
      --
    end if;
    --
...
--   Internal table handler use only.
--
function prp_rows_exists (p_pl_regy_bod_id in number ) return Boolean  is
  --
  l_proc         varchar2(72) := g_package||'prp_rows_exists';
  l_dummy        varchar2(1);
  --
  cursor c1 is
    select null
    from   ben_pl_regy_prp_f a
    where  a.pl_regy_bod_id  = p_pl_regy_bod_id;
  --
Begin
  --
  hr_utility.set_location('Entering:'||l_proc,5);
  --
  -- check if child rows exists in ben_pl_regy_prps.
  --
  open c1;
    --
    fetch c1 into l_dummy;
    if c1%found then
      ...
      --
      return true;
      -- ben_prb_shd.constraint_error('BEN_PL_REGY_PRP_DT1');
      --
    else
      --
      close c1;
...
Begin
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    --
    --If (prp_rows_exists(p_pl_regy_bod_id => p_pl_regy_bod_id)) Then
    --   l_table_name := 'ben_pl_regy_prp';
    --   Raise l_rows_exist;
    --End If;
    --
    --
...
function return_legislation_code
  (p_pl_regy_bod_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_pl_regy_bod_f b
    where b.pl_regy_bod_id      = p_pl_regy_bod_id
