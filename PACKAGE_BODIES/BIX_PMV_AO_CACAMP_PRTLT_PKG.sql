--------------------------------------------------------
--  DDL for Package Body BIX_PMV_AO_CACAMP_PRTLT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIX_PMV_AO_CACAMP_PRTLT_PKG" AS
/*$Header: bixocamp.plb 115.8 2004/04/22 12:47:19 pubalasu noship $ */
...
IF l_source_code_id IS NOT NULL THEN
   l_source_code_where_clause := ' AND mv.campaign_id IN (
   select source_code_for_id from ams_source_codes where arc_source_code_for=''CAMP''
   and source_code_id in (:l_source_code_id) and active_flag=''Y'' ) ';
END IF;
...
END IF;
   ...
   l_sqltext := '
            SELECT decode(campmast.value,null,''</a><nobr>''|| :l_unknown ||''</nobr><a href=#>'',campmast.value)  VIEWBY,
                 nvl(max(campmast.id),-1) VIEWBYID,
                 ...
                 round(sum(nvl(sum(ABAND),0)) over()*100/
                ...
                3600*nvl(sum(contacts)/decode(MIN(login),0,null,MIN(login)),0)
                BIX_PMV_AO_CONTPERHR,
                 3600*sum(sum(contacts)) over()/decode(MIN(login),0,null,MIN(login))
                BIX_PMV_TOTAL9,
             ...
             FROM
             (
             /* Add another Inline view */
             SELECT camp ,sourcecode sourcecode,OFFRD, HAND,ABAND,USABAND,TALK,SR,LEAD,OPP,CONTACTS,PR, SUM(LOGIN) over() LOGIN
             FROM
                 ( SELECT campaign_id camp,
                   min(source_code_id) sourcecode,
...
l_sqltext := l_sqltext || l_call_where_clause||
               ...
               ,(
			   select source_code_for_id,camp.id id,camp.value value from bim_dimv_campaigns camp,ams_source_codes scodes
			   where scodes.source_code_id=camp.id and
			   arc_source_code_for=''CAMP'' and active_flag=''Y'' ) campmast
			   where summ.camp=campmast.source_code_for_id(+)
               and camp<>-999
               GROUP BY campmast.value
...
l_custom_rec.attribute_value := 'CAMPAIGN+CAMPAIGN';
...
NULL;
END GET_SQL;
END BIX_PMV_AO_CACAMP_PRTLT_PKG;
