--------------------------------------------------------
--  DDL for Package Body BEN_JRT_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_JRT_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ben_jrt_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    p_rec.object_version_number :=
      dt_api.get_object_version_number
        ...
        ,p_base_key_value  => p_rec.job_rt_id
        );
    --
    --
    --
    -- Update the ben_job_rt_f Row
    --
    update  ben_job_rt_f
    set
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    ben_jrt_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_base_key_value         => p_rec.job_rt_id
...
Begin
  ...
  If (p_rec.ordr_num = hr_api.g_number) then
    ...
    ben_jrt_shd.g_old_rec.ordr_num;
  End If;
  If (p_rec.jrt_attribute_category = hr_api.g_varchar2) then
    p_rec.jrt_attribute_category :=
    ben_jrt_shd.g_old_rec.jrt_attribute_category;
  End If;
  If (p_rec.jrt_attribute1 = hr_api.g_varchar2) then
    p_rec.jrt_attribute1 :=
    ben_jrt_shd.g_old_rec.jrt_attribute1;
  End If;
  If (p_rec.jrt_attribute2 = hr_api.g_varchar2) then
    p_rec.jrt_attribute2 :=
    ben_jrt_shd.g_old_rec.jrt_attribute2;
  End If;
  If (p_rec.jrt_attribute3 = hr_api.g_varchar2) then
    p_rec.jrt_attribute3 :=
    ben_jrt_shd.g_old_rec.jrt_attribute3;
  End If;
  If (p_rec.jrt_attribute4 = hr_api.g_varchar2) then
    p_rec.jrt_attribute4 :=
    ben_jrt_shd.g_old_rec.jrt_attribute4;
  End If;
  If (p_rec.jrt_attribute5 = hr_api.g_varchar2) then
    p_rec.jrt_attribute5 :=
    ben_jrt_shd.g_old_rec.jrt_attribute5;
  End If;
  If (p_rec.jrt_attribute6 = hr_api.g_varchar2) then
    p_rec.jrt_attribute6 :=
    ben_jrt_shd.g_old_rec.jrt_attribute6;
  End If;
  If (p_rec.jrt_attribute7 = hr_api.g_varchar2) then
    p_rec.jrt_attribute7 :=
    ben_jrt_shd.g_old_rec.jrt_attribute7;
  End If;
  If (p_rec.jrt_attribute8 = hr_api.g_varchar2) then
    p_rec.jrt_attribute8 :=
    ben_jrt_shd.g_old_rec.jrt_attribute8;
  End If;
  If (p_rec.jrt_attribute9 = hr_api.g_varchar2) then
    p_rec.jrt_attribute9 :=
    ben_jrt_shd.g_old_rec.jrt_attribute9;
  End If;
  If (p_rec.jrt_attribute10 = hr_api.g_varchar2) then
    p_rec.jrt_attribute10 :=
    ben_jrt_shd.g_old_rec.jrt_attribute10;
  End If;
  If (p_rec.jrt_attribute11 = hr_api.g_varchar2) then
    p_rec.jrt_attribute11 :=
    ben_jrt_shd.g_old_rec.jrt_attribute11;
  End If;
  If (p_rec.jrt_attribute12 = hr_api.g_varchar2) then
    p_rec.jrt_attribute12 :=
    ben_jrt_shd.g_old_rec.jrt_attribute12;
  End If;
  If (p_rec.jrt_attribute13 = hr_api.g_varchar2) then
    p_rec.jrt_attribute13 :=
    ben_jrt_shd.g_old_rec.jrt_attribute13;
  End If;
  If (p_rec.jrt_attribute14 = hr_api.g_varchar2) then
    p_rec.jrt_attribute14 :=
    ben_jrt_shd.g_old_rec.jrt_attribute14;
  End If;
  If (p_rec.jrt_attribute15 = hr_api.g_varchar2) then
    p_rec.jrt_attribute15 :=
    ben_jrt_shd.g_old_rec.jrt_attribute15;
  End If;
  If (p_rec.jrt_attribute16 = hr_api.g_varchar2) then
    p_rec.jrt_attribute16 :=
    ben_jrt_shd.g_old_rec.jrt_attribute16;
  End If;
  If (p_rec.jrt_attribute17 = hr_api.g_varchar2) then
    p_rec.jrt_attribute17 :=
    ben_jrt_shd.g_old_rec.jrt_attribute17;
  End If;
  If (p_rec.jrt_attribute18 = hr_api.g_varchar2) then
    p_rec.jrt_attribute18 :=
    ben_jrt_shd.g_old_rec.jrt_attribute18;
  End If;
  If (p_rec.jrt_attribute19 = hr_api.g_varchar2) then
    p_rec.jrt_attribute19 :=
    ben_jrt_shd.g_old_rec.jrt_attribute19;
  End If;
  If (p_rec.jrt_attribute20 = hr_api.g_varchar2) then
    p_rec.jrt_attribute20 :=
    ben_jrt_shd.g_old_rec.jrt_attribute20;
  End If;
  If (p_rec.jrt_attribute21 = hr_api.g_varchar2) then
    p_rec.jrt_attribute21 :=
    ben_jrt_shd.g_old_rec.jrt_attribute21;
  End If;
  If (p_rec.jrt_attribute22 = hr_api.g_varchar2) then
    p_rec.jrt_attribute22 :=
    ben_jrt_shd.g_old_rec.jrt_attribute22;
  End If;
  If (p_rec.jrt_attribute23 = hr_api.g_varchar2) then
    p_rec.jrt_attribute23 :=
    ben_jrt_shd.g_old_rec.jrt_attribute23;
  End If;
  If (p_rec.jrt_attribute24 = hr_api.g_varchar2) then
    p_rec.jrt_attribute24 :=
    ben_jrt_shd.g_old_rec.jrt_attribute24;
  End If;
  If (p_rec.jrt_attribute25 = hr_api.g_varchar2) then
    p_rec.jrt_attribute25 :=
    ben_jrt_shd.g_old_rec.jrt_attribute25;
  End If;
  If (p_rec.jrt_attribute26 = hr_api.g_varchar2) then
    p_rec.jrt_attribute26 :=
    ben_jrt_shd.g_old_rec.jrt_attribute26;
  End If;
  If (p_rec.jrt_attribute27 = hr_api.g_varchar2) then
    p_rec.jrt_attribute27 :=
    ben_jrt_shd.g_old_rec.jrt_attribute27;
  End If;
  If (p_rec.jrt_attribute28 = hr_api.g_varchar2) then
    p_rec.jrt_attribute28 :=
    ben_jrt_shd.g_old_rec.jrt_attribute28;
  End If;
  If (p_rec.jrt_attribute29 = hr_api.g_varchar2) then
    p_rec.jrt_attribute29 :=
    ben_jrt_shd.g_old_rec.jrt_attribute29;
  End If;
  If (p_rec.jrt_attribute30 = hr_api.g_varchar2) then
    p_rec.jrt_attribute30 :=
    ben_jrt_shd.g_old_rec.jrt_attribute30;
  End If;
  --
...
Begin
  ...
  --
  ben_jrt_upd.convert_defs(p_rec);
  --
