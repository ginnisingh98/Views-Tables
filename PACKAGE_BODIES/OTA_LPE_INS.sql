--------------------------------------------------------
--  DDL for Package Body OTA_LPE_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LPE_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_lpe_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_lp_enrollment_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_lp_enrollment_id  in  number) is
--
...
Begin
  ...
  --
  ota_lpe_ins.g_lp_enrollment_id_i := p_lp_enrollment_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: ota_lp_enrollments
  --
  insert into ota_lp_enrollments
      (lp_enrollment_id
      ,learning_path_id
      ,person_id
      ,contact_id
...
--
  Cursor C_Sel1 is select ota_lp_enrollments_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from ota_lp_enrollments
     where lp_enrollment_id =
             ota_lpe_ins.g_lp_enrollment_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (ota_lpe_ins.g_lp_enrollment_id_i is not null) Then
    --
    ...
    p_rec.lp_enrollment_id :=
      ota_lpe_ins.g_lp_enrollment_id_i;
    ota_lpe_ins.g_lp_enrollment_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.lp_enrollment_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  ota_lpe_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  ota_lpe_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  ota_lpe_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  p_lp_enrollment_id := l_rec.lp_enrollment_id;
