--------------------------------------------------------
--  DDL for Package Body JTF_DELIVERABLE_GRP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JTF_DELIVERABLE_GRP" AS
/* $Header: JTFGDLVB.pls 115.17 2004/07/09 18:49:38 applrt ship $ */
g_amv_api_version CONSTANT NUMBER := 1.0;

TYPE DlvCurTyp IS REF CURSOR;

g_view_name CONSTANT VARCHAR2(48) := 'jtf_dsp_amv_items_v';
...
PROCEDURE list_deliverable (
	...
	x_file_id_tbl          OUT  NUMBER_TABLE ) IS
...
PROCEDURE list_deliverable (
	...
	sql_cond VARCHAR2(480) := 'FROM ' || g_view_name
...
BEGIN
	...
	IF p_category_id IS NOT NULL
		...
		FND_MESSAGE.set_name('JTF', 'JTF_DSP_AVAIL_INVLD');
...
PROCEDURE save_deliverable (
	...
	x_msg_count			OUT	NUMBER,
	x_msg_data			OUT	VARCHAR2,
	p_deliverable_rec		IN OUT DELIVERABLE_REC_TYPE ) IS
...
l_appl_id := 671;
			...
			*/
			...
			IF NOT JTF_DSPMGRVALIDATION_GRP.check_deliverable_accessname(
...
PROCEDURE save_deliverable (
	...
	x_msg_count              OUT  NUMBER,
	x_msg_data               OUT  VARCHAR2,
	p_deliverable_tbl        IN OUT DELIVERABLE_TBL_TYPE ) IS
...
PROCEDURE save_deliverable (
	...
	x_msg_count              OUT  NUMBER,
	x_msg_data               OUT  VARCHAR2,
	p_dlv_ath_rec            IN OUT DLV_ATH_REC_TYPE ) IS
	...
	l_attachment_rec JTF_Attachment_GRP.ATTACHMENT_REC_TYPE;
	l_language_code_tbl JTF_PhysicalMap_GRP.LANGUAGE_CODE_TBL_TYPE
		:= JTF_PhysicalMap_GRP.LANGUAGE_CODE_TBL_TYPE(NULL);
...
BEGIN
  ...
  BEGIN
	...
	ELSE
		...
		ELSE
			...
			IF l_attachment_id IS NOT NULL THEN
				...
				-- validate it!
				--modified by G. Zhang 05/23/01 10:57AM
				--IF NOT JTF_DSPMGRVALIDATION_GRP.check_attachment_deliverable(
				--	l_attachment_id, p_dlv_ath_rec.deliverable_id) THEN
				--	-- invalid attachment for the given deliverable
				--	RAISE FND_API.g_exc_error;
				--END  IF;

				IF NOT JTF_DSPMGRVALIDATION_GRP.check_default_attachment(
	...
	IF l_create_attachment = FND_API.g_true THEN
		...
		JTF_Attachment_GRP.save_attachment(
	...
	IF l_create_mapping = FND_API.g_true THEN
		...
		JTF_PhysicalMap_GRP.save_physicalmap(
			p_api_version            => p_api_version,
			x_return_status          => l_return_status,
			x_msg_count              => x_msg_count,
			x_msg_data               => x_msg_data,
...
PROCEDURE save_deliverable (
	...
	x_msg_count              OUT  NUMBER,
	x_msg_data               OUT  VARCHAR2,
	p_dlv_ath_tbl        	IN OUT DLV_ATH_TBL_TYPE ) IS
