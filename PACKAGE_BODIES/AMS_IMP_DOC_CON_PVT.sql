--------------------------------------------------------
--  DDL for Package Body AMS_IMP_DOC_CON_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AMS_IMP_DOC_CON_PVT" as
 /* $Header: amsvidcb.pls 115.4 2002/11/14 22:03:20 jieli noship $ */
 -- ===============================================================
 -- Start of Comments
 -- Package name
 --          AMS_Imp_Doc_Con_PVT
 ...
 G_PKG_NAME CONSTANT VARCHAR2(30):= 'AMS_Imp_Doc_Con_PVT';
 G_FILE_NAME CONSTANT VARCHAR2(12) := 'amsvidcb.pls';
 ...
 --       p_imp_doc_rec            IN   imp_doccon_rec_type  Required
 --
 --   OUT
...
PROCEDURE Create_Imp_Doc_Content(
     ...
     p_imp_doc_rec              IN   imp_doccon_rec_type  := g_miss_imp_doc_rec,
     x_imp_doccontent_id              OUT NOCOPY  NUMBER
      )

  IS
 L_API_NAME                  CONSTANT VARCHAR2(30) := 'Create_Imp_Doc_Content';
 L_API_VERSION_NUMBER        CONSTANT NUMBER   := 1.0;
    ...
    CURSOR c_id IS
       SELECT ams_imp_doc_content_s.NEXTVAL
       FROM dual;

    CURSOR c_id_exists (l_id IN NUMBER) IS
       SELECT 1
       FROM ams_imp_doc_content
       WHERE imp_doc_content_id = l_id;
 ...
 BEGIN
    ...
    IF p_imp_doc_rec.imp_doc_content_id IS NULL OR p_imp_doc_rec.imp_doc_content_id = FND_API.g_miss_num THEN
       LOOP
          l_dummy := NULL;
          OPEN c_id;
       ...
       END LOOP;
    ELSE
          l_imp_document_id := p_imp_doc_rec.imp_doc_content_id;
    END IF;
       ...
       IF ( p_validation_level IS NULL OR
       ...
       -- Invoke table handler(AMS_IMP_DOC_CON_PKG.Insert_Row)
       AMS_IMP_DOC_CON_PKG.Insert_Row(
           px_imp_doc_content_id  => l_imp_document_id,
           ...
           p_file_id => p_imp_doc_rec.file_id,
           p_file_name => p_imp_doc_rec.file_name
 );

           x_imp_doccontent_id := l_imp_document_id;
       IF x_return_status <> FND_API.G_RET_STS_SUCCESS THEN
           RAISE FND_API.G_EXC_ERROR;
       END IF;
 --
 ...
 PROCEDURE Update_Imp_Doc_Con(
     ...
     p_imp_doc_rec               IN    imp_doccon_rec_type,
 ...
 CURSOR c_get_imp_doc(imp_doc_content_id NUMBER) IS
     SELECT *
     FROM  ams_imp_doc_content
     WHERE  imp_doc_content_id = p_imp_doc_rec.imp_doc_content_id;
     -- Hint: Developer need to provide Where clause


 L_API_NAME                  CONSTANT VARCHAR2(30) := 'Update_Imp_Doc_Con';
 L_API_VERSION_NUMBER        CONSTANT NUMBER   := 1.0;
 -- Local Variables
 l_object_version_number     NUMBER;
 l_imp_document_id    NUMBER;
 l_ref_imp_doc_rec  c_get_Imp_Doc%ROWTYPE ;
 l_tar_imp_doc_rec  imp_doccon_rec_type := P_imp_doc_rec;
 l_rowid  ROWID;
  ...
  BEGIN
       ...
       OPEN c_get_Imp_Doc( l_tar_imp_doc_rec.imp_doc_content_id);
  ...
  p_token_value  => 'Imp_Doc') ;
       ...
       -- Invoke table handler(AMS_IMP_DOC_CON_PKG.Update_Row)
       AMS_IMP_DOC_CON_PKG.Update_Row(
           p_imp_doc_content_id  => p_imp_doc_rec.imp_doc_content_id,
           p_last_updated_by  => FND_GLOBAL.USER_ID,
           px_object_version_number  => l_object_version_number,
           p_last_update_login  => FND_GLOBAL.conc_login_id,
           p_last_update_date  => SYSDATE,
 ...
 PROCEDURE Delete_Imp_Doc_Con(
     ...
     x_msg_count                  OUT NOCOPY  NUMBER,
     x_msg_data                   OUT NOCOPY  VARCHAR2,
     p_imp_doccontent_id          IN  NUMBER,
     p_object_version_number      IN   NUMBER
     )

  IS
 L_API_NAME                  CONSTANT VARCHAR2(30) := 'Delete_Imp_Doc_Con';
 L_API_VERSION_NUMBER        CONSTANT NUMBER   := 1.0;
 l_object_version_number     NUMBER;
  ...
  BEGIN
       ...
       -- Invoke table handler(AMS_IMP_DOC_CON_PKG.Delete_Row)
       AMS_IMP_DOC_CON_PKG.Delete_Row(
           p_imp_doc_content_id  => p_imp_doccontent_id,
 ...
 PROCEDURE Lock_Imp_Doc_Con(
  ...
  IS
 L_API_NAME                  CONSTANT VARCHAR2(30) := 'Lock_Imp_Doc_Con';
 L_API_VERSION_NUMBER        CONSTANT NUMBER   := 1.0;
 L_FULL_NAME                 CONSTANT VARCHAR2(60) := g_pkg_name ||'.'|| l_api_name;
 l_imp_document_id                  NUMBER;

 CURSOR c_imp_doc IS
    SELECT imp_doc_content_id
    FROM ams_imp_doc_content
    WHERE imp_doc_content_id = p_imp_doccontent_id
 ...
 BEGIN
       ...
       IF p_validation_mode = JTF_PLSQL_API.g_create THEN
          ...
          'ams_imp_doc_content',
          'imp_doc_content_id = ''' || p_imp_doc_rec.imp_doc_content_id ||''''
          );
       ELSE
          l_valid_flag := AMS_Utility_PVT.check_uniqueness(
          'ams_imp_doc_content',
          'imp_doc_content_id = ''' || p_imp_doc_rec.imp_doc_content_id ||
          ''' AND imp_doc_content_id <> ' || p_imp_doc_rec.imp_doc_content_id
          );
       END IF;
 ...
 BEGIN
    ...
    IF p_validation_mode = JTF_PLSQL_API.g_create THEN
       ...
       IF p_imp_doc_rec.imp_doc_content_id = FND_API.G_MISS_NUM OR p_imp_doc_rec.imp_doc_content_id IS NULL THEN
                AMS_Utility_PVT.Error_Message(p_message_name => 'AMS_API_MISSING_FIELD');
                FND_MESSAGE.set_token('MISS_FIELD', 'imp_doc_content_id' );
                x_return_status := FND_API.g_ret_sts_error;
       END IF;
    ...
    ELSE
       ...
       IF p_imp_doc_rec.imp_doc_content_id = FND_API.G_MISS_NUM THEN
                AMS_Utility_PVT.Error_Message(p_message_name => 'AMS_API_MISSING_FIELD');
                FND_MESSAGE.set_token('MISS_FIELD', 'imp_doc_content_id' );
                x_return_status := FND_API.g_ret_sts_error;
          RETURN;
 ...
 PROCEDURE Complete_Imp_Doc_Rec (
    p_imp_doc_rec IN imp_doccon_rec_type,
    x_complete_rec OUT NOCOPY imp_doccon_rec_type)
 IS
 ...
 BEGIN
    ...
    -- imp_doc_content_id
    IF p_imp_doc_rec.imp_doc_content_id IS NULL THEN
       x_complete_rec.imp_doc_content_id := l_imp_doc_rec.imp_doc_content_id;
    END IF;
    ...
    -- file_id
    IF p_imp_doc_rec.file_id IS NULL THEN
       x_complete_rec.file_id := l_imp_doc_rec.file_id;
    END IF;

    -- file_name
    IF p_imp_doc_rec.file_name IS NULL THEN
       x_complete_rec.file_name := l_imp_doc_rec.file_name;
    END IF;
 ...
 PROCEDURE Default_Item_Attribute ( p_imp_doc_rec IN imp_doccon_rec_type ,
                                 x_imp_doc_rec OUT NOCOPY imp_doccon_rec_type )
 IS
    l_imp_doc_rec imp_doccon_rec_type := p_imp_doc_rec;
 BEGIN
 ...
 L_API_VERSION_NUMBER        CONSTANT NUMBER   := 1.0;
 l_object_version_number     NUMBER;
 l_imp_doc_rec  imp_doccon_rec_type;
