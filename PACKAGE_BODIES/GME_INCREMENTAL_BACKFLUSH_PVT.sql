--------------------------------------------------------
--  DDL for Package Body GME_INCREMENTAL_BACKFLUSH_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_INCREMENTAL_BACKFLUSH_PVT" AS
/* $Header: GMEVIBFB.pls 120.13.12010000.11 2010/04/09 17:58:45 gmurator ship $
...
========================================================================================================*/
  ...
  BEGIN
    ...
    IF nvl(l_batch_header_rec.parentline_id,0) = 0 THEN
       ...
       */
      l_hold_new_actual := l_in_material_detail_rec.actual_qty +
                      ((l_in_material_detail_rec.wip_plan_qty * l_incr_factor) / 100);
	...
	IF ( NVL(G_DEBUG,-1) <= GME_DEBUG.G_LOG_STATEMENT ) THEN
	  ...
	  -- add the incr qty to old actual to come up with new actual
          -- as p_incr_factor now depicts the percent increase from previous actual.
          l_new_actual := l_material_detail_rec.actual_qty +
...
-- calculation so that the logic would become applicable for the phantom item also*/
  ...
  BEGIN
    ...
    IF ( NVL(G_DEBUG,-1) = GME_DEBUG.G_LOG_STATEMENT ) THEN
      gme_debug.put_line (g_pkg_name||'.'||l_api_name||' gme_transactions_pvt.get_mat_trans returned '||l_mmt_tbl.count||' trxns with return_status='||x_return_status);
    END IF;
        ...
        IF l_mmt_rec.transaction_type_id = l_trxn_type THEN
          ...
          WHILE j <= l_mmln_tbl.COUNT AND l_qty > 0 LOOP
            ...
            IF l_skip_lot = 0 THEN
               ...
               IF ( NVL(G_DEBUG,-1) = GME_DEBUG.G_LOG_STATEMENT ) THEN
                 gme_debug.put_line (g_pkg_name||'.'||l_api_name||' in lot loop; l_mmln_tbl(j).transaction_quantity='||l_mmln_tbl(j).transaction_quantity);
  ...
  EXCEPTION
    WHEN error_get_trans OR error_get_lot OR error_trans OR inv_negative THEN
      NULL;
    WHEN OTHERS THEN
      fnd_msg_pub.add_exc_msg (g_pkg_name, l_api_name);
      IF g_debug <= gme_debug.g_log_procedure THEN
