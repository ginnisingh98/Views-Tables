--------------------------------------------------------
--  DDL for Package Body AP_RECT_MATCH_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_RECT_MATCH_PKG" AS
/*$Header: aprcvmtb.pls 120.61.12010000.9 2010/02/08 09:50:51 baole ship $*/
...
PROCEDURE Get_Total_Proration_Quantity(
	        ...
	        X_Matching_Case		OUT NOCOPY VARCHAR2,
		X_Calling_Sequence      IN	   VARCHAR2);
...
G_PKG_NAME          CONSTANT VARCHAR2(30) := 'AP_RECT_MATCH_PKG';
...
G_MODULE_NAME           CONSTANT VARCHAR2(100) := 'AP.PLSQL.AP_RECT_MATCH_PKG.';
...
G_allow_account_override	ap_system_parameters.allow_flex_override_flag%TYPE;
G_encumbrance_flag		financials_system_parameters.purch_encumbrance_flag%TYPE;
G_match_option			po_line_locations.match_option%TYPE;
G_approval_workflow_flag	ap_system_parameters.approval_workflow_flag%TYPE;
--G_Ussgl_Transaction_Code      ap_invoices.ussgl_transaction_code%TYPE; - Bug 4277744
G_login_id			number;
G_User_id			number;
...
--Contract Payments
G_Shipment_Type			po_line_locations_all.shipment_type%TYPE;
...
BEGIN
   ...
   l_api_name := 'Base_Credit_RCV_Match';

   IF (G_LEVEL_PROCEDURE >= G_CURRENT_RUNTIME_LEVEL) THEN
      FND_LOG.STRING(G_LEVEL_PROCEDURE,G_MODULE_NAME||l_api_name,'AP_RECT_MATCH_PKG.Base_Credit_Rcv_Match(+)');
   END IF;

   -- Update the calling sequence (for error message).
   current_calling_sequence := 'AP_MATCHING_PKG.base_credit_rcv_match<-'||X_calling_sequence;
...
PROCEDURE Get_Info(X_Invoice_ID  	 IN NUMBER,
 ...
 current_calling_sequence 	VARCHAR2(2000);
 l_accounting_date	ap_invoices_all.gl_date%TYPE;
 l_debug_info		VARCHAR2(2000);
 l_api_name		VARCHAR2(32);
BEGIN
   ...
   l_api_name := 'Get_Info';
   IF (G_LEVEL_PROCEDURE >= G_CURRENT_RUNTIME_LEVEL) THEN
      FND_LOG.STRING(G_LEVEL_PROCEDURE,G_MODULE_NAME||l_api_name,'AP_RECT_MATCH_PKG.Get_Info(+)');
   END IF;
   ...
   IF (g_invoice_currency_code <> g_base_currency_code) THEN
   ...
   g_period_name := AP_UTILITIES_PKG.get_current_gl_date(g_accounting_date,
                                                         g_org_id); /* bug 5174518 */


   IF (g_period_name IS NULL) THEN

      --Get gl_period and Date from a future period for the accounting date
      -- 8714506 Used l_accounting_date instead of g_accounting_date. Using
      -- same variable for in/out parmeters causing to make in parameters
      -- as Null
      ap_utilities_pkg.get_only_open_gl_date(p_date => g_accounting_date,
  				             p_period_name => g_period_name,
  				             p_gl_date => l_accounting_date,
                                             p_org_id  => g_org_id); /*bug 5174518 */

      g_accounting_date :=   l_accounting_date;   /* Bug#8714506 */
...
Procedure Get_Dist_Proration_Info(X_Invoice_Id		 IN NUMBER,
				  X_Invoice_Line_Number  IN NUMBER,
				  X_Rcv_Transaction_Id   IN NUMBER,
			          X_Po_Line_Location_Id  IN NUMBER,
				  X_Match_Mode		 IN VARCHAR2,
  ...
  --Modified below cursor for bug #7015553 to receive projec_id and task_id
 --values from receivables tables if these are different in PO and receipt.

  CURSOR po_distributions_cursor IS
  ...
  l_base_match_amt     NUMBER;
  l_dist_match_amount  NUMBER;
  l_invoiced_qty       NUMBER;
  l_billed_txn_qty     NUMBER;
  l_tot_shipment_qty_billed  NUMBER;
  l_total_quantity     NUMBER;
  l_matching_case      VARCHAR2(2);
  l_amount	       NUMBER;
  ...
  l_award_set_id	  po_distributions.award_id%TYPE;
...
BEGIN
 ...
 IF (G_LEVEL_PROCEDURE >= G_CURRENT_RUNTIME_LEVEL) THEN
      FND_LOG.STRING(G_LEVEL_PROCEDURE,G_MODULE_NAME||l_api_name,'AP_RECT_MATCH_PKG.Get_Dist_Proration_Info(+)');
 END IF;
 ...
 IF(X_Match_Mode IN ('STD-PS','CR-PS')) THEN
   ...
   Get_Total_Proration_Quantity
	                  ( X_Rcv_Transaction_Id   => x_rcv_transaction_id,
	                    X_PO_Line_Location_Id  => x_po_line_location_id,
	                    X_Match_Quantity  => x_match_quantity,
			    X_Total_Quantity  => l_total_quantity,
			    X_Matching_Case   => l_matching_case,
			    X_Calling_Sequence => current_calling_sequence);

   OPEN PO_Distributions_Cursor;
   ...
   LOOP
      ...
      l_debug_info := 'Get distribution level quantities from RCV API ';
      IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
         FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,l_debug_info);
      END IF;
      ...
      RCV_INVOICE_MATCHING_SV.get_delivered_quantity(
       ...
       -- calculate actual qty delivered
       l_delivered_txn_qty := nvl(l_delivered_txn_qty,0)
                                + nvl(l_corrected_txn_qty,0)
                                 - nvl(l_returned_txn_qty,0);

       -- calculate actual qty ordered
       --bug5093924
        --We will subtract the cancelled quantity only if quantity ordered is not
        --same as cancelled quantity.Else it wont create any invoice distributions.
        --This is to bring the same behaviour as in PO matching.
         if(l_ordered_txn_qty <> l_cancelled_txn_qty or l_matching_case<>'O') then
            l_ordered_txn_qty :=  nvl(l_ordered_txn_qty,0)
                                - nvl(l_cancelled_txn_qty,0);
         end if;


      --Commented below code for bug #8858939

      /*  -- calculate actual qty ordered
       l_ordered_txn_qty :=  nvl(l_ordered_txn_qty,0)
                                - nvl(l_cancelled_txn_qty,0);	*/

       l_debug_info := 'Get Distribution level qty billed';
       IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
          FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,l_debug_info);
       END IF;
       ...
       l_debug_info := 'Populate x_dist_tab with po /invoice info';
       ...
       IF l_matching_case = 'DB' THEN

	  IF( x_match_mode = 'STD-PS') THEN

              l_amount := ap_utilities_pkg.ap_round_currency(
	        		(X_match_amount * (l_delivered_txn_qty - l_billed_txn_qty)/
                                l_total_quantity),g_invoice_currency_code);

              l_quantity_invoiced :=  ROUND(X_match_quantity *
                              	    ((l_delivered_txn_qty - l_billed_txn_qty)/
                                       l_total_quantity),15);
	  ...
	  --bugfix:5639042
	     ...
	     l_amount := ap_utilities_pkg.ap_round_currency(
	                         (X_match_amount * l_billed_txn_qty)/l_total_quantity,
			          g_invoice_currency_code);

             l_quantity_invoiced := ROUND((X_match_quantity * l_billed_txn_qty)
                                          /l_total_quantity,15) ;


	  END IF;

       ELSIF l_matching_case = 'OB' THEN

          IF(x_match_mode = 'STD-PS') THEN
              l_amount := ap_utilities_pkg.ap_round_currency(
	      	  	    (X_match_amount * (l_ordered_txn_qty - l_billed_txn_qty)/
                                l_total_quantity),g_invoice_currency_code);

              l_quantity_invoiced := ROUND(x_match_quantity *
              				((l_ordered_txn_qty - l_billed_txn_qty)/
	                                l_total_quantity),15);

          ELSE /*for -ve invoices */
              l_amount := ap_utilities_pkg.ap_round_currency(
	      		   (X_match_amount * l_billed_txn_qty)/l_total_quantity,
			    g_invoice_currency_code);
       ...
       ELSE
          ...
          l_amount := ap_utilities_pkg.ap_round_currency(
	                x_match_amount * l_ordered_txn_qty/l_total_quantity,
			g_invoice_currency_code);
          l_quantity_invoiced := ROUND(x_match_quantity * l_ordered_txn_qty/l_total_quantity,15);

       END IF;

       x_dist_tab(l_po_distribution_id).quantity_invoiced := l_quantity_invoiced;

       x_dist_tab(l_po_distribution_id).amount := l_amount;
       x_dist_tab(l_po_distribution_id).unit_price := x_unit_price;
       x_dist_tab(l_po_distribution_id).po_ccid := l_po_ccid;
 ...
 ELSE /* for distribution level match */
   ...
   FOR i IN nvl(x_dist_tab.FIRST,0)..nvl(x_dist_tab.LAST,0) LOOP
	 ...
	 End If;
         ...
         --invoices only, for non-foriegn currency invoices g_line_base_amount
         --will be NULL.

         IF (g_line_base_amount IS NOT NULL) THEN
...
*/
     IF (x_dist_tab(i).project_id IS NOT NULL) THEN
         ...
         WHEN 'PO Expenditure Item Date/Transaction Date' THEN
            x_dist_tab(i).expenditure_item_date := nvl(x_dist_tab(i).expenditure_item_date,
	    							g_invoice_date);
     ...
     IF (g_invoice_type_lookup_code <> 'PREPAYMENT') THEN
        ...
        IF (nvl(x_dist_tab(i).accrue_on_receipt_flag,'N') = 'N'
           ...
           AND g_allow_account_override = 'Y'
  ...
  END IF; /*IF (x_dist_tab.exists(i)) */

 END LOOP;

 --Base Amount Rounding
 --Need to perform base_amount rounding for only foreign currency invoices only
 IF (g_line_base_amount <> l_sum_dist_base_amount AND g_exchange_rate IS NOT NULL and
     l_rounding_index is not null) THEN
...
/**----------------------------------------------------------------------
GET_TOTAL_PRORATION_QUANTITY :: returns the Total Quantity and Receipt Match Case
given the rcv_transaction_id. These values are calculated as follows:

O -- qty ORDERED on PO shipment
C -- qty CANCELLED  on PO shipment
D -- qty DELIVERED on rcv_transaction
B -- Total qty BILLED on the rcv_transaction
a -- qty being Invoiced right now (X_quantity)

If a < = D - B        then  Case = 'DB' , Total_Quantity  = (D - B)
If a < = (O - C) - B  then  Case = 'OB' , Total_quantity = (O - C) - B
Else                        Case = 'O'  , Total_Quantity = O
-----------------------------------------------------------------------**/
PROCEDURE Get_Total_Proration_Quantity
			  ( X_Rcv_Transaction_id	IN	NUMBER,
	                    X_PO_Line_Location_Id 	IN 	NUMBER,
...
BEGIN
    ...
    IF (G_LEVEL_PROCEDURE >= G_CURRENT_RUNTIME_LEVEL) THEN
      FND_LOG.STRING(G_LEVEL_PROCEDURE,G_MODULE_NAME||l_api_name,'AP_RECT_MATCH_PKG.Get_Total_Proration_Quantity(+)');
    END IF;

    current_calling_sequence := 'Get_Total_Proration_Quantity<-'||current_calling_sequence;
    l_debug_info := 'Get total quantity for proration';
    IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
      FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,l_debug_info);
    END IF;

    -- Call RCV API to get receipt quantities in the receipt UOM
    -- Actually calling the AP wrapper around the RCV API to get the
    -- quantities in the receipt UOM only

    l_debug_info := ' Getting receipt quantities';
    IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
      FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,l_debug_info);
    END IF;
    ...
    -- Calculate the Total quantity and Receipt matching case
    If (X_match_quantity <= (l_delivered_qty - l_billed_qty)) Then

        X_total_quantity := l_delivered_qty - l_billed_qty;
        X_matching_case := 'DB';
    Elsif (X_match_quantity <= (l_ordered_qty - l_cancelled_qty) - l_billed_qty)
    Then
        X_total_quantity := (l_ordered_qty - l_cancelled_qty)- l_billed_qty;
        X_matching_case := 'OB';
    Else
        X_total_quantity := l_ordered_qty;
        X_matching_case := 'O';
    End if;

    --For the case of RTS debit memos:
    --Quantity_Billed should be selected from rcv_transactions
    --only when the invoice is matched to a receipt and from
    --po_line_locations when the invoice is matched to a PO.

    IF X_Match_Quantity < 0 THEN
        --For credit/debit memos if match_option is receipt
	--then the total_quantity is the quantity_billed on
	--the rcv_transaction else quantity_billed on the po_shipment.

      IF g_match_option = 'R' THEN

	--Bugfix: 5546861 backed out the fix 5513689, and made sure
	--both the bugs are fixed. Due to the fix done by 5513689,
	--we were creating distributions for debit memo with +ve quantities, since
	--in the proration formula (match quantity and total quantity being negative) (-a/-b)
	--will give +ve value.

	x_total_quantity :=  l_billed_qty;
      ELSE
...
EXCEPTION
 WHEN OTHERS THEN
   IF (SQLCODE <> -20001) THEN
     ...
     FND_MESSAGE.SET_TOKEN('ERROR',SQLERRM);
     FND_MESSAGE.SET_TOKEN('CALLING_SEQUENCE',current_calling_sequence);
     FND_MESSAGE.SET_TOKEN('PARAMETERS','Match Option = '||g_match_option
			  ||', Shipment_id = '||to_char(X_PO_Line_Location_id)
			  ||', Rcv Transaction Id = '||to_char(x_rcv_transaction_id)
			  ||', Match Quantity = '||to_char(x_match_quantity));
     FND_MESSAGE.SET_TOKEN('DEBUG_INFO',l_debug_info);
   END IF;
   APP_EXCEPTION.RAISE_EXCEPTION;
   ...
   IF (X_LINE_TYPE_LOOKUP_CODE = 'ITEM') THEN
    ...
    SELECT    X_INVOICE_ID,		--invoice_id
 	   ...
 	   -- RCV.USSGL_TRANSACTION_CODE,--ussgl_transaction_code - Bug 4277744
	      ...
	      RCV.RCV_SHIPMENT_LINE_ID,   --rcv_shipment_line_id
    ...
    SELECT    X_INVOICE_ID, 		--invoice_id
     ...
     --bug 5102208
	      SUBSTRB(X_CHARGE_LINE_DESCRIPTION || AIL.description, 1, 240),--de              --scription
...
l_rounded_index                 NUMBER;
l_rounded_distribution_id ap_invoice_distributions.invoice_distribution_id%TYPE;
	 ...
	 GMS_AP_API.CREATE_AWARD_DISTRIBUTIONS
         ...
         AP_DBI_PKG.Maintain_DBI_Summary
              (p_table_name => 'AP_INVOICE_DISTRIBUTIONS',
               ...
               p_key_value2 => x_dist_tab(i).invoice_distribution_id,
                p_calling_sequence => current_calling_sequence);
...
BEGIN
   ...
   --bugfix:5509730 added the IF condition and the ELSE part
   IF (g_invoice_type_lookup_code <> 'PREPAYMENT') THEN
    ...
    IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
        FND_LOG.STRING(G_LEVEL_STATEMENT,G_MODULE_NAME||l_api_name,l_debug_info);
    END IF;
    FOR i in x_dist_tab.first..x_dist_tab.last LOOP
...
EXCEPTION
 WHEN OTHERS THEN
   IF (SQLCODE <> -20001) THEN
	...
	'PO Distribution Id = '||TO_CHAR(X_Dist_tab(i).po_distribution_id)
...
BEGIN
    ...
    --For the case of automatic debit memo creation from RTS transaction,
    --we do match the debit memo to the receipt regardless of the match
    --option on the po shipment. So when updating the quantities need to
    --make sure we do not update the quantities/amounts on the receipt
    --if the match option is 'P'. So, Need the below IF condition so that
    --quantity_billed on the parent 'RECEIVE' transaction for this RTS transaction,
...
/*========================================================================================
**************PRICE AND QUANTITY CORRECTION OF INVOICE MATCHED TO RECEIPT*****************
=========================================================================================*/
...
BEGIN
   ...
   l_api_name := 'Price_Quantity_Correct_Inv_Rcv';

   current_calling_sequence := 'Price_Quantity_Correct_Inv_RCV<-'||x_calling_sequence;
...
EXCEPTION
    WHEN others then
		...
		||', rcv_transaction_id = '||to_char(x_rcv_transaction_id));
            fnd_message.set_token('DEBUG_INFO',l_debug_info);
        End if;
...
END Price_Quantity_Correct_Inv_RCV;
...
l_po_dist_id 		 po_distributions.po_distribution_id%TYPE;
l_amount 		 ap_invoice_distributions.amount%TYPE;
l_corrected_quantity 	 ap_invoice_distributions.corrected_quantity%TYPE;
l_base_amount		 ap_invoice_distributions.base_amount%TYPE;
l_total_quantity_billed  po_distributions.quantity_billed%TYPE;
l_dist_ccid		 ap_invoice_distributions.dist_code_combination_id%TYPE;
l_invoice_distribution_id ap_invoice_distributions.invoice_distribution_id%TYPE;
l_max_dist_amount	  ap_invoice_distributions.amount%TYPE := 0;
l_sum_prorated_amount     ap_invoice_distributions.amount%TYPE := 0;
...
BEGIN
 ...
 /* Bug 5585744, Changed the index for x_corr_dist_tab from l_po_dist_id to
 ...
 IF (x_match_mode IN ('STD-PS','CR-PS'))THEN
   ...
   END IF ; /* x_match_type = 'PRICE_CORRECTION' */


 --For the case when user distributes the correction, we still
 --need to populate the PL/SQL table base_amount, invoice_distribution_id...
 ELSE  /* match_mode in 'STD-PD','CR-PD' ...*/
 ...
 --Perform Base Amount rounding for foreign currency invoices only
...
PROCEDURE Insert_Corr_Invoice_Dists(x_invoice_id          IN NUMBER,
 ...
 l_rounded_dist_id	     ap_invoice_distributions.invoice_distribution_id%type := NULL;
 l_sum_amount_prorated	     ap_invoice_distributions.amount%TYPE := 0;
 l_max_dist_amount	     ap_invoice_distributions.amount%TYPE := 0;
