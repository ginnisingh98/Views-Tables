--------------------------------------------------------
--  DDL for Package Body OKC_TAV_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_TAV_PVT" AS
/* $Header: OKCSTAVB.pls 120.0 2005/05/25 19:10:58 appldev noship $ */
...
/*  FUNCTION get_rec (
  ...
  END get_rec;
  ---------------------------------------------------------------------------
  -- FUNCTION get_rec for: OKC_TIME_TPA_VALUE_V
  ---------------------------------------------------------------------------
*/
  FUNCTION get_rec (
    ...
    CURSOR okc_tavv_pk_csr (p_id                 IN NUMBER) IS
    SELECT
            ID,
            OBJECT_VERSION_NUMBER,
--Bug 3122962            SFWT_FLAG,
            ...
            LAST_UPDATE_DATE,
            LAST_UPDATE_LOGIN
      FROM Okc_Time_Tpa_Value_V
     WHERE okc_time_tpa_value_v.id = p_id;
    l_okc_tavv_pk                  okc_tavv_pk_csr%ROWTYPE;
    l_tavv_rec                     tavv_rec_type;
  BEGIN
    x_no_data_found := TRUE;
    -- Get current database values
    OPEN okc_tavv_pk_csr (p_tavv_rec.id);
    FETCH okc_tavv_pk_csr INTO
              l_tavv_rec.ID,
              l_tavv_rec.OBJECT_VERSION_NUMBER,
--Bug 3122962              l_tavv_rec.SFWT_FLAG,
              l_tavv_rec.SPN_ID,
              l_tavv_rec.TVE_ID_GENERATED_BY,
              l_tavv_rec.TVE_ID_LIMITED,
              l_tavv_rec.DESCRIPTION,
              l_tavv_rec.SHORT_DESCRIPTION,
              l_tavv_rec.COMMENTS,
              l_tavv_rec.DATETIME,
              l_tavv_rec.DNZ_CHR_ID,
              l_tavv_rec.TZE_ID,
              l_tavv_rec.ATTRIBUTE_CATEGORY,
              l_tavv_rec.ATTRIBUTE1,
              l_tavv_rec.ATTRIBUTE2,
              l_tavv_rec.ATTRIBUTE3,
              l_tavv_rec.ATTRIBUTE4,
              l_tavv_rec.ATTRIBUTE5,
              l_tavv_rec.ATTRIBUTE6,
              l_tavv_rec.ATTRIBUTE7,
              l_tavv_rec.ATTRIBUTE8,
              l_tavv_rec.ATTRIBUTE9,
              l_tavv_rec.ATTRIBUTE10,
              l_tavv_rec.ATTRIBUTE11,
              l_tavv_rec.ATTRIBUTE12,
              l_tavv_rec.ATTRIBUTE13,
              l_tavv_rec.ATTRIBUTE14,
              l_tavv_rec.ATTRIBUTE15,
              l_tavv_rec.CREATED_BY,
              l_tavv_rec.CREATION_DATE,
              l_tavv_rec.LAST_UPDATED_BY,
              l_tavv_rec.LAST_UPDATE_DATE,
              l_tavv_rec.LAST_UPDATE_LOGIN;
    x_no_data_found := okc_tavv_pk_csr%NOTFOUND;
    CLOSE okc_tavv_pk_csr;
    RETURN(l_tavv_rec);
  END get_rec;
  ...
  FUNCTION get_rec (
    ...
    l_row_notfound                 BOOLEAN := TRUE;
  BEGIN
    RETURN(get_rec(p_tavv_rec, l_row_notfound));
  END get_rec;

  ----------------------------------------------------------
  -- FUNCTION null_out_defaults for: OKC_TIME_TPA_VALUE_V --
  ----------------------------------------------------------
  ...
  BEGIN
    IF (l_tavv_rec.object_version_number = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.object_version_number := NULL;
    END IF;
--Bug 3122962
/*    IF (l_tavv_rec.sfwt_flag = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.sfwt_flag := NULL;
    END IF;
*/
    IF (l_tavv_rec.spn_id = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.spn_id := NULL;
    END IF;
    IF (l_tavv_rec.tve_id_generated_by = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.tve_id_generated_by := NULL;
    END IF;
    IF (l_tavv_rec.tve_id_limited = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.tve_id_limited := NULL;
    END IF;
    IF (l_tavv_rec.dnz_chr_id = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.dnz_chr_id := NULL;
    END IF;
    IF (l_tavv_rec.tze_id = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.tze_id := NULL;
    END IF;
    IF (l_tavv_rec.description = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.description := NULL;
    END IF;
    IF (l_tavv_rec.short_description = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.short_description := NULL;
    END IF;
    IF (l_tavv_rec.comments = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.comments := NULL;
    END IF;
    IF (l_tavv_rec.datetime = OKC_API.G_MISS_DATE) THEN
      l_tavv_rec.datetime := NULL;
    END IF;
    IF (l_tavv_rec.attribute_category = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute_category := NULL;
    END IF;
    IF (l_tavv_rec.attribute1 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute1 := NULL;
    END IF;
    IF (l_tavv_rec.attribute2 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute2 := NULL;
    END IF;
    IF (l_tavv_rec.attribute3 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute3 := NULL;
    END IF;
    IF (l_tavv_rec.attribute4 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute4 := NULL;
    END IF;
    IF (l_tavv_rec.attribute5 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute5 := NULL;
    END IF;
    IF (l_tavv_rec.attribute6 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute6 := NULL;
    END IF;
    IF (l_tavv_rec.attribute7 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute7 := NULL;
    END IF;
    IF (l_tavv_rec.attribute8 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute8 := NULL;
    END IF;
    IF (l_tavv_rec.attribute9 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute9 := NULL;
    END IF;
    IF (l_tavv_rec.attribute10 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute10 := NULL;
    END IF;
    IF (l_tavv_rec.attribute11 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute11 := NULL;
    END IF;
    IF (l_tavv_rec.attribute12 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute12 := NULL;
    END IF;
    IF (l_tavv_rec.attribute13 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute13 := NULL;
    END IF;
    IF (l_tavv_rec.attribute14 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute14 := NULL;
    END IF;
    IF (l_tavv_rec.attribute15 = OKC_API.G_MISS_CHAR) THEN
      l_tavv_rec.attribute15 := NULL;
    END IF;
    IF (l_tavv_rec.created_by = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.created_by := NULL;
    END IF;
    IF (l_tavv_rec.creation_date = OKC_API.G_MISS_DATE) THEN
      l_tavv_rec.creation_date := NULL;
    END IF;
    IF (l_tavv_rec.last_updated_by = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.last_updated_by := NULL;
    END IF;
    IF (l_tavv_rec.last_update_date = OKC_API.G_MISS_DATE) THEN
      l_tavv_rec.last_update_date := NULL;
    END IF;
    IF (l_tavv_rec.last_update_login = OKC_API.G_MISS_NUM) THEN
      l_tavv_rec.last_update_login := NULL;
    END IF;
    RETURN(l_tavv_rec);
...
*/
  ...
  ---------------------------------------------------------------------------
  --------------------------------------------------
  -- Validate_Attributes for:OKC_TIME_TPA_VALUE_V --
  --------------------------------------------------
  PROCEDURE Validate_Spn_Id (
    ...
    BEGIN
      ...
      IF (p_tavv_rec.SPN_ID IS NOT NULL AND
          p_tavv_rec.SPN_ID <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_spnv_pk_csr(p_tavv_rec.SPN_ID);
  ...
  PROCEDURE Validate_Generated_By (
      x_return_status                OUT NOCOPY VARCHAR2,
      p_tavv_rec                     IN tavv_rec_type) IS
      item_not_found_error          EXCEPTION;
      l_row_notfound                 BOOLEAN := TRUE;
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_tavv_rec.TVE_ID_GENERATED_BY IS NOT NULL AND
          p_tavv_rec.TVE_ID_GENERATED_BY <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_tvev_pk_csr(p_tavv_rec.TVE_ID_GENERATED_BY);
        FETCH okc_tvev_pk_csr INTO l_okc_tvev_pk;
        l_row_notfound := okc_tvev_pk_csr%NOTFOUND;
        CLOSE okc_tvev_pk_csr;
        IF (l_row_notfound) THEN
          OKC_API.set_message(G_APP_NAME, G_INVALID_VALUE,G_COL_NAME_TOKEN,'TVE_ID_GENERATED_BY');
          RAISE item_not_found_error;
        END IF;
      END IF;
    EXCEPTION
      ...
      WHEN OTHERS THEN
        OKC_API.SET_MESSAGE(p_app_name     => g_app_name,
                            ...
                            p_token1_value => sqlcode,
                            p_token2       => g_col_name_token,
                            p_token2_value => 'TVE_ID_GENERATED_BY',
                            p_token3       => g_sqlerrm_token,
                            p_token3_value => sqlerrm);
      -- notify caller of an UNEXPECTED error
        x_return_status := OKC_API.G_RET_STS_UNEXP_ERROR;
    END Validate_Generated_By ;
  ...
  PROCEDURE Validate_Tve_Id_Limited (
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_tavv_rec.TVE_ID_LIMITED IS NOT NULL AND
          p_tavv_rec.TVE_ID_LIMITED <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_tvev_pk_csr(p_tavv_rec.TVE_ID_LIMITED);
        FETCH okc_tvev_pk_csr INTO l_okc_tvev_pk;
        l_row_notfound := okc_tvev_pk_csr%NOTFOUND;
        CLOSE okc_tvev_pk_csr;
        IF (l_row_notfound) THEN
  ...
  PROCEDURE Validate_DNZ_Chr_ID (
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_tavv_rec.dnz_chr_id IS NOT NULL AND
          p_tavv_rec.dnz_chr_id <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okc_chrv_pk_csr(p_tavv_rec.dnz_chr_id);
  ...
  BEGIN
    x_return_status := OKC_API.G_RET_STS_SUCCESS;
    IF (p_tavv_rec.datetime = OKC_API.G_MISS_DATE OR
        p_tavv_rec.datetime IS NULL) THEN
  ...
  PROCEDURE Validate_Time_Zone_Id (
    ...
    BEGIN
      x_return_status                := OKC_API.G_RET_STS_SUCCESS;
      IF (p_tavv_rec.TZE_ID IS NOT NULL AND
          p_tavv_rec.TZE_ID <> OKC_API.G_MISS_NUM)
      THEN
        OPEN okx_timezones_v_pk_csr(p_tavv_rec.TZE_ID);
...
/*  PROCEDURE Validate_SFWT_Flag (
    ...
    p_tavv_rec                     IN tavv_rec_type) IS
  BEGIN
    IF upper(p_tavv_rec.sfwt_flag) = 'Y' OR
       upper(p_tavv_rec.sfwt_flag) = 'N'
    THEN
       IF p_tavv_rec.sfwt_flag = 'Y' OR
          p_tavv_rec.sfwt_flag = 'N'
       THEN
         x_return_status := OKC_API.G_RET_STS_SUCCESS;
       ELSE
...
*/
  ...
  BEGIN
    IF p_tavv_rec.id = OKC_API.G_MISS_NUM OR
       p_tavv_rec.id IS NULL
    THEN
      OKC_API.set_message(G_APP_NAME, G_REQUIRED_VALUE,G_COL_NAME_TOKEN,'id');
      x_return_status := OKC_API.G_RET_STS_ERROR;
    ELSIF p_tavv_rec.object_version_number = OKC_API.G_MISS_NUM OR
          p_tavv_rec.object_version_number IS NULL
    THEN
      OKC_API.set_message(G_APP_NAME, G_REQUIRED_VALUE,G_COL_NAME_TOKEN,'object_version_number');
      x_return_status := OKC_API.G_RET_STS_ERROR;
    END IF;
    IF (p_tavv_rec.spn_id is NOT NULL AND
        p_tavv_rec.spn_id <> OKC_API.G_MISS_NUM) THEN
    ...
    END IF;
    IF (p_tavv_rec.tve_id_generated_by is NOT NULL AND
        p_tavv_rec.tve_id_generated_by <> OKC_API.G_MISS_NUM) THEN
      Validate_Generated_By (l_return_status,
                             p_tavv_rec);
      IF (l_return_status <> OKC_API.G_RET_STS_SUCCESS) THEN
        IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN  -- need to leave
          x_return_status := l_return_status;
    ...
    END IF;
    IF (p_tavv_rec.tve_id_limited is NOT NULL AND
        p_tavv_rec.tve_id_limited <> OKC_API.G_MISS_NUM) THEN
      ...
      END IF;
    END IF;
    IF p_tavv_rec.dnz_chr_id <> 0 Then
...
*/
  ...
  ---------------------------------------------------------------------------
  ----------------------------------------------
  -- Validate_Record for:OKC_TIME_TPA_VALUE_V --
  ----------------------------------------------
...
*/
  ...
  ---------------------------------------------------------------------------
  -------------------------------------------
  -- validate_row for:OKC_TIME_TPA_VALUE_V --
  -------------------------------------------
  PROCEDURE validate_row(
...
--Bug 3122962    l_okc_timevalues_tl_rec        okc_timevalues_tl_rec_type;
  BEGIN
    ...
    l_return_status := Validate_Attributes(l_tavv_rec);
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := Validate_Record(l_tavv_rec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
  ...
  END validate_row;
  ------------------------------------------
  -- PL/SQL TBL validate_row for:TAVV_TBL --
  ------------------------------------------
  PROCEDURE validate_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
*/
  -----------------------------------------
  -- insert_row for:OKC_TIME_TPA_VALUE_V --
  -----------------------------------------
  PROCEDURE insert_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
--Bug 3122962    lx_okc_timevalues_tl_rec       okc_timevalues_tl_rec_type;
    ...
    FUNCTION fill_who_columns (
      ...
      l_tavv_rec	tavv_rec_type := p_tavv_rec;
    BEGIN
      l_tavv_rec.CREATION_DATE := SYSDATE;
      l_tavv_rec.CREATED_BY := FND_GLOBAL.USER_ID;
      l_tavv_rec.LAST_UPDATE_DATE := l_tavv_rec.CREATION_DATE;
      l_tavv_rec.LAST_UPDATED_BY := FND_GLOBAL.USER_ID;
      l_tavv_rec.LAST_UPDATE_LOGIN := FND_GLOBAL.LOGIN_ID;
      RETURN(l_tavv_rec);
    END fill_who_columns;
    ---------------------------------------------
    -- Set_Attributes for:OKC_TIME_TPA_VALUE_V --
    ---------------------------------------------
    FUNCTION Set_Attributes (
      p_tavv_rec IN  tavv_rec_type,
      ...
      l_return_status                VARCHAR2(1) := OKC_API.G_RET_STS_SUCCESS;
    BEGIN
      x_tavv_rec := p_tavv_rec;
      x_tavv_rec.OBJECT_VERSION_NUMBER := 1;
--Bug 3122962      x_tavv_rec.SFWT_FLAG := 'N';
      RETURN(l_return_status);
    END Set_Attributes;
  BEGIN
    l_return_status := OKC_API.START_ACTIVITY(l_api_name,
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_tavv_rec := null_out_defaults(p_tavv_rec);
    -- Set primary key value
    l_tavv_rec.ID := get_seq_id;
    --- Setting item attributes
    l_return_status := Set_Attributes(
      l_tavv_rec,                        -- IN
      l_def_tavv_rec);                   -- OUT
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_def_tavv_rec := fill_who_columns(l_def_tavv_rec);
    --- Validate all non-missing attributes (Item Level Validation)
    l_return_status := Validate_Attributes(l_def_tavv_rec);
    --- If any errors happen abort API
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := Validate_Record(l_def_tavv_rec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    ...
    --------------------------------------
    migrate(l_def_tavv_rec, l_tve_rec);
--Bug 3122962    migrate(l_def_tavv_rec, l_okc_timevalues_tl_rec);
    --------------------------------------------
    ...
    ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    migrate(lx_tve_rec, l_def_tavv_rec);
/*    insert_row(
      p_init_msg_list,
      x_return_status,
      x_msg_count,
    ...
    ELSIF (x_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    migrate(lx_okc_timevalues_tl_rec, l_def_tavv_rec);
*/
    -- Set OUT values
    x_tavv_rec := l_def_tavv_rec;
    OKC_API.END_ACTIVITY(x_msg_count, x_msg_data);
  EXCEPTION
    WHEN OKC_API.G_EXCEPTION_ERROR THEN
      x_return_status := OKC_API.HANDLE_EXCEPTIONS
  ...
  END insert_row;
  ----------------------------------------
  -- PL/SQL TBL insert_row for:TAVV_TBL --
  ----------------------------------------
  PROCEDURE insert_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
*/
  ---------------------------------------
  -- lock_row for:OKC_TIME_TPA_VALUE_V --
  ---------------------------------------
  PROCEDURE lock_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
--Bug 3122962    l_okc_timevalues_tl_rec        okc_timevalues_tl_rec_type;
  BEGIN
    ...
    -- Move VIEW record to "Child" records
    --------------------------------------
    migrate(p_tavv_rec, l_tve_rec);
--Bug 3122962    migrate(p_tavv_rec, l_okc_timevalues_tl_rec);
    --------------------------------------------
...
*/
  ...
  END lock_row;
  --------------------------------------
  -- PL/SQL TBL lock_row for:TAVV_TBL --
  --------------------------------------
  PROCEDURE lock_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
*/
  -----------------------------------------
  -- update_row for:OKC_TIME_TPA_VALUE_V --
  -----------------------------------------
  PROCEDURE update_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
--Bug 3122962    lx_okc_timevalues_tl_rec       okc_timevalues_tl_rec_type;
    ...
    FUNCTION fill_who_columns (
      ...
      l_tavv_rec	tavv_rec_type := p_tavv_rec;
    BEGIN
      l_tavv_rec.LAST_UPDATE_DATE := SYSDATE;
      l_tavv_rec.LAST_UPDATED_BY := FND_GLOBAL.USER_ID;
      l_tavv_rec.LAST_UPDATE_LOGIN := FND_GLOBAL.LOGIN_ID;
      RETURN(l_tavv_rec);
    END fill_who_columns;
    ...
    BEGIN
      x_tavv_rec := p_tavv_rec;
      -- Get current database values
      l_tavv_rec := get_rec(p_tavv_rec, l_row_notfound);
      IF (l_row_notfound) THEN
        l_return_status := OKC_API.G_RET_STS_UNEXP_ERROR;
      END IF;
      IF (x_tavv_rec.id = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.id := l_tavv_rec.id;
      END IF;
      IF (x_tavv_rec.object_version_number = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.object_version_number := l_tavv_rec.object_version_number;
      END IF;
--Bug 3122962
/*      IF (x_tavv_rec.sfwt_flag = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.sfwt_flag := l_tavv_rec.sfwt_flag;
      END IF;
*/
      IF (x_tavv_rec.spn_id = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.spn_id := l_tavv_rec.spn_id;
      END IF;
      IF (x_tavv_rec.tve_id_generated_by = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.tve_id_generated_by := l_tavv_rec.tve_id_generated_by;
      END IF;
      IF (x_tavv_rec.dnz_chr_id = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.dnz_chr_id := l_tavv_rec.dnz_chr_id;
      END IF;
      IF (x_tavv_rec.tze_id = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.tze_id := l_tavv_rec.tze_id;
      END IF;
      IF (x_tavv_rec.tve_id_limited = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.tve_id_limited := l_tavv_rec.tve_id_limited;
      END IF;
      IF (x_tavv_rec.description = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.description := l_tavv_rec.description;
      END IF;
      IF (x_tavv_rec.short_description = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.short_description := l_tavv_rec.short_description;
      END IF;
      IF (x_tavv_rec.comments = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.comments := l_tavv_rec.comments;
      END IF;
      IF (x_tavv_rec.datetime = OKC_API.G_MISS_DATE)
      THEN
        x_tavv_rec.datetime := l_tavv_rec.datetime;
      END IF;
      IF (x_tavv_rec.attribute_category = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute_category := l_tavv_rec.attribute_category;
      END IF;
      IF (x_tavv_rec.attribute1 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute1 := l_tavv_rec.attribute1;
      END IF;
      IF (x_tavv_rec.attribute2 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute2 := l_tavv_rec.attribute2;
      END IF;
      IF (x_tavv_rec.attribute3 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute3 := l_tavv_rec.attribute3;
      END IF;
      IF (x_tavv_rec.attribute4 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute4 := l_tavv_rec.attribute4;
      END IF;
      IF (x_tavv_rec.attribute5 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute5 := l_tavv_rec.attribute5;
      END IF;
      IF (x_tavv_rec.attribute6 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute6 := l_tavv_rec.attribute6;
      END IF;
      IF (x_tavv_rec.attribute7 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute7 := l_tavv_rec.attribute7;
      END IF;
      IF (x_tavv_rec.attribute8 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute8 := l_tavv_rec.attribute8;
      END IF;
      IF (x_tavv_rec.attribute9 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute9 := l_tavv_rec.attribute9;
      END IF;
      IF (x_tavv_rec.attribute10 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute10 := l_tavv_rec.attribute10;
      END IF;
      IF (x_tavv_rec.attribute11 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute11 := l_tavv_rec.attribute11;
      END IF;
      IF (x_tavv_rec.attribute12 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute12 := l_tavv_rec.attribute12;
      END IF;
      IF (x_tavv_rec.attribute13 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute13 := l_tavv_rec.attribute13;
      END IF;
      IF (x_tavv_rec.attribute14 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute14 := l_tavv_rec.attribute14;
      END IF;
      IF (x_tavv_rec.attribute15 = OKC_API.G_MISS_CHAR)
      THEN
        x_tavv_rec.attribute15 := l_tavv_rec.attribute15;
      END IF;
      IF (x_tavv_rec.created_by = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.created_by := l_tavv_rec.created_by;
      END IF;
      IF (x_tavv_rec.creation_date = OKC_API.G_MISS_DATE)
      THEN
        x_tavv_rec.creation_date := l_tavv_rec.creation_date;
      END IF;
      IF (x_tavv_rec.last_updated_by = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.last_updated_by := l_tavv_rec.last_updated_by;
      END IF;
      IF (x_tavv_rec.last_update_date = OKC_API.G_MISS_DATE)
      THEN
        x_tavv_rec.last_update_date := l_tavv_rec.last_update_date;
      END IF;
      IF (x_tavv_rec.last_update_login = OKC_API.G_MISS_NUM)
      THEN
        x_tavv_rec.last_update_login := l_tavv_rec.last_update_login;
      END IF;
      RETURN(l_return_status);
    END populate_new_record;
    ---------------------------------------------
...
--      x_tavv_rec.SFWT_FLAG := upper(p_tavv_rec.SFWT_FLAG);
      x_tavv_rec.OBJECT_VERSION_NUMBER := NVL(x_tavv_rec.OBJECT_VERSION_NUMBER, 0) + 1;
      RETURN(l_return_status);
    END Set_Attributes;
  BEGIN
    l_return_status := OKC_API.START_ACTIVITY(l_api_name,
    ...
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
    END IF;
    l_return_status := populate_new_record(l_tavv_rec, l_def_tavv_rec);
    IF (l_return_status = OKC_API.G_RET_STS_UNEXP_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_UNEXPECTED_ERROR;
    ELSIF (l_return_status = OKC_API.G_RET_STS_ERROR) THEN
      RAISE OKC_API.G_EXCEPTION_ERROR;
...
*/
  ...
  END update_row;
  ----------------------------------------
  -- PL/SQL TBL update_row for:TAVV_TBL --
  ----------------------------------------
  PROCEDURE update_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
*/
  -----------------------------------------
  -- delete_row for:OKC_TIME_TPA_VALUE_V --
  -----------------------------------------
  PROCEDURE delete_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
...
--Bug 3122962    l_okc_timevalues_tl_rec        okc_timevalues_tl_rec_type;
  ...
  BEGIN
    ...
    -- Move VIEW record to "Child" records
    --------------------------------------
--Bug 3122962    migrate(l_tavv_rec, l_okc_timevalues_tl_rec);
    migrate(l_tavv_rec, l_tve_rec);
    --------------------------------------------
...
*/
  ...
  END delete_row;
  ----------------------------------------
  -- PL/SQL TBL delete_row for:TAVV_TBL --
  ----------------------------------------
  PROCEDURE delete_row(
    p_api_version                  IN NUMBER,
    p_init_msg_list                IN VARCHAR2,
