--------------------------------------------------------
--  DDL for Package Body XNP_CSIWIPAC_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_CSIWIPAC_U" AS 
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'CSIWIPAC' ;
...
/* construct the message body */
    XNP_XML_UTILS.begin_segment ( 'CSIWIPAC' ) ;
    ...
    END IF ;
    XNP_XML_UTILS.write_leaf_element ( 'ORGANIZATION_ID', XNP$ORGANIZATION_ID ) ;
    XNP_XML_UTILS.end_segment ( 'CSIWIPAC' ) ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'CSIWIPAC.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'CSIWIPAC', l_consumer_list );
  END IF;
...
NULL ;
DECLARE
      l_txn_type    varchar2(30) := 'CSIWIPAC';
      l_txn_type_id number       := 73;
      l_txn_id      number;
