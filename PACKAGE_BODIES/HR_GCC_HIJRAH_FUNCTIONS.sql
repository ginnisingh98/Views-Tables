--------------------------------------------------------
--  DDL for Package Body HR_GCC_HIJRAH_FUNCTIONS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_GCC_HIJRAH_FUNCTIONS" as
/* $Header: pegcchjf.pkb 120.0 2005/05/31 09:16:13 appldev noship $ */
  ...
  begin
    if p_input_date is not null then
      hr_gcc_hijrah_functions.validate_date(p_input_date,l_input_date);
  ...
  begin
    hr_gcc_hijrah_functions.validate_date(p_input_date,l_input_date);
    l_greg_date := to_date(hr_gcc_hijrah_functions.hijrah_to_gregorian(l_input_date),'YYYY/MM/DD');
    l_new_greg_date := l_greg_date + p_num;
    l_new_hij_date := hr_gcc_hijrah_functions.gregorian_to_hijrah(l_new_greg_date);
  ...
  begin
    hr_gcc_hijrah_functions.validate_date(p_high_date,l_high_date);
    hr_gcc_hijrah_functions.validate_date(p_low_date,l_low_date);
    l_high_greg_date := to_date(hr_gcc_hijrah_functions.hijrah_to_gregorian(l_high_date),'YYYY/MM/DD');
    l_low_greg_date := to_date(hr_gcc_hijrah_functions.hijrah_to_gregorian(l_low_date),'YYYY/MM/DD');
  ...
  begin
    ...
    into   l_day
    from   dual;
    return(hr_general.decode_lookup('GCC_HIJRAH_DAYS',l_day));
  end get_day;
  ...
  begin
    ...
    return(hr_general.decode_lookup('GCC_HIJRAH_MONTHS',l_month));
  end get_month;
...
begin
    l_length := length(p_input_date);
        ...
        if(l_error = 1) then
            l_field := hr_general.decode_lookup('KW_FORM_LABELS','HIJRAH_DATE');
            l_range := hr_general.decode_lookup('KW_FORM_LABELS','INVALID_FORMAT');
            hr_utility.set_message(800, 'HR_375412_KW_INVALID_DATE');
     ...
     if(l_year<=0 or l_year >4089) then  --check if year/month/day is negative
        l_flag := 1;
        l_field := hr_general.decode_lookup('KW_FORM_LABELS','HIJRAH_YEAR');
        l_range := hr_general.decode_lookup('KW_FORM_LABELS','YEAR_RANGE');
        hr_utility.set_message(800, 'HR_375412_KW_INVALID_DATE');
        hr_utility.set_message_token('FIELD',l_field);
        hr_utility.set_message_token('RANGE',l_range);
        hr_utility.raise_error;
     ...
     if(l_month <= 0 or l_month> 12)then
        l_flag := 2;
        l_field := hr_general.decode_lookup('KW_FORM_LABELS','HIJRAH_MONTH');
        l_range := hr_general.decode_lookup('KW_FORM_LABELS','MONTH_RANGE');
        hr_utility.set_message(800, 'HR_375412_KW_INVALID_DATE');
        hr_utility.set_message_token('FIELD',l_field);
        hr_utility.set_message_token('RANGE',l_range);
        hr_utility.raise_error;
     ...
     if(l_day <= 0 or l_day >30)then
        l_flag := 3;
        l_field := hr_general.decode_lookup('KW_FORM_LABELS','HIJRAH_DATE');
        l_range := hr_general.decode_lookup('KW_FORM_LABELS','DATE_RANGE');
        hr_utility.set_message(800, 'HR_375412_KW_INVALID_DATE');
        hr_utility.set_message_token('FIELD',l_field);
        hr_utility.set_message_token('RANGE',l_range);
        hr_utility.raise_error;
