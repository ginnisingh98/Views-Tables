--------------------------------------------------------
--  DDL for Package Body AME_RUL_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_RUL_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_rul_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_RULES_PK') Then
...
Begin
  ...
  Else
    If (p_rule_id =
        ame_rul_shd.g_old_rec.rule_id and
        p_object_version_number =
        ame_rul_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_rul_shd.g_old_rec;
...
--
  ...
  -- end date to the specified new effective end date. This query is modified to
  -- account for rules with future rule dates
  --
  update  ame_rules t
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_rul_shd.g_old_rec.end_date;
