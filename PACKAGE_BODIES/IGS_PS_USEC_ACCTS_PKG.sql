--------------------------------------------------------
--  DDL for Package Body IGS_PS_USEC_ACCTS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_USEC_ACCTS_PKG" AS
/* $Header: IGSPI3AB.pls 115.4 2002/11/29 02:24:20 nsidana noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_ps_usec_accts_all%ROWTYPE;
  new_references igs_ps_usec_accts_all%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.usec_account_id                   := x_usec_account_id;
  ...
  */
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_usec_accts_all
      WHERE    usec_account_id = x_usec_account_id
      FOR UPDATE NOWAIT;
  ...
  */
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      CLOSE cur_rowid;
      fnd_message.set_name ('IGS', 'IGS_FI_USEA_UOO_FK');
      igs_ge_msg_stack.add;
      app_exception.raise_exception;
      RETURN;
    END IF;
  ...
  BEGIN
    ...
    IF (p_action = 'INSERT') THEN
      ...
      IF ( get_pk_for_validation(
             new_references.usec_account_id
           )
         ) THEN
  ...
  */
  ...
  BEGIN
    ...
    SELECT    igs_ps_usec_accts_s.NEXTVAL
    INTO      x_usec_account_id
    FROM      dual;
    ...
    before_dml(
      p_action                            => 'INSERT',
      x_rowid                             => x_rowid,
      x_usec_account_id                   => x_usec_account_id,
      x_uoo_id                            => x_uoo_id,
      x_segment                           => x_segment,
    ...
    INSERT INTO igs_ps_usec_accts_all (
      usec_account_id,
      uoo_id,
      segment,
      segment_num,
  ...
  */
  BEGIN
    ...
    DELETE FROM igs_ps_usec_accts_all
    WHERE rowid = x_rowid;
