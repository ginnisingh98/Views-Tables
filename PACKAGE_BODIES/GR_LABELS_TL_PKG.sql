--------------------------------------------------------
--  DDL for Package Body GR_LABELS_TL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_LABELS_TL_PKG" AS
/*$Header: GRHILATB.pls 120.1 2006/06/16 21:39:22 pbamb noship $*/
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ROLLBACK TO SAVEPOINT Insert_Row;
	  l_msg_token := p_label_code || ' ' || p_language;
	  x_return_status := 'E';
...
PROCEDURE Update_Row
				  ...
				  p_label_code IN VARCHAR2,
				  p_language IN VARCHAR2,
				  p_label_description IN VARCHAR2,
				  p_source_lang IN VARCHAR2,
				  p_created_by IN NUMBER,
				  p_creation_date IN DATE,
				  p_last_updated_by IN NUMBER,
...
/*	  Now call the check foreign key procedure */
   ...
   ELSE
	  ...
	  SET	 label_code		 	 	 	 	 = p_label_code,
	  		 language						 = p_language,
			 label_description				 = p_label_description,
...
CURSOR c_get_descs
 IS
   SELECT	lat.label_description,
            lat.created_by,
			lat.creation_date,
			lat.last_updated_by,
			lat.last_update_date,
			lat.last_update_login
   FROM	    gr_labels_tl lat
   WHERE	lat.label_code = p_label_code
   AND		lat.language = l_language;
LabelDesc				c_get_descs%ROWTYPE;
...
/* Redefault translations from the source language  */
   ...
   where (
      ...
      T.LANGUAGE
         ...
         SUBT.LABEL_CODE,
         SUBT.LANGUAGE
         from GR_LABELS_TL SUBB, GR_LABELS_TL SUBT
         where SUBB.LABEL_CODE = SUBT.LABEL_CODE
         and SUBB.LANGUAGE = SUBT.SOURCE_LANG
         and (SUBB.LABEL_DESCRIPTION <> SUBT.LABEL_DESCRIPTION
          or (SUBB.LABEL_DESCRIPTION is null and SUBT.LABEL_DESCRIPTION is not null)
          or (SUBB.LABEL_DESCRIPTION is not null and SUBT.LABEL_DESCRIPTION is null)
  ));
...
*/
   ...
   OPEN c_get_descs;
   FETCH c_get_descs INTO LabelDesc;
   IF c_get_descs%NOTFOUND THEN
      CLOSE c_get_descs;
      RAISE Language_Missing_Error;
   ELSE
      l_base_desc := LabelDesc.label_description;
	  l_created_by := LabelDesc.created_by;
	  l_creation_date := LabelDesc.creation_date;
	  l_last_updated_by := LabelDesc.last_updated_by;
	  l_last_update_date := LabelDesc.last_update_date;
	  l_last_update_login := LabelDesc.last_update_login;
      CLOSE c_get_descs;
   END IF;
...
CURSOR c_lock_labels_tl
 IS
   SELECT	*
   FROM		gr_labels_tl
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockLabelRcd	  c_lock_labels_tl%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_labels_tl;
   FETCH c_lock_labels_tl INTO LockLabelRcd;
   IF c_lock_labels_tl%NOTFOUND THEN
	  CLOSE c_lock_labels_tl;
...
CURSOR c_get_labels_tl_rowid
 IS
   SELECT lat.rowid
   FROM	  gr_labels_tl lat
   WHERE  lat.label_code = p_label_code
   AND	  lat.language = p_language;
LabelTLRecord			   c_get_labels_tl_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_label_code || ' ' || p_language;

   x_key_exists := 'F';
   OPEN c_get_labels_tl_rowid;
   FETCH c_get_labels_tl_rowid INTO LabelTLRecord;
   IF c_get_labels_tl_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := LabelTLRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_labels_tl_rowid;
...
BEGIN
	UPDATE GR_LABELS_TL SET
		LABEL_DESCRIPTION = X_LABEL_DESCRIPTION,
		SOURCE_LANG = USERENV('LANG'),
		LAST_UPDATE_DATE = sysdate,
		LAST_UPDATED_BY = 0,
		LAST_UPDATE_LOGIN = 0
	WHERE (LABEL_CODE = X_LABEL_CODE)
...
BEGIN
	...
	IF Cur_rowid%FOUND THEN
		GR_LABELS_TL_PKG.UPDATE_ROW(
			P_COMMIT => 'T'
			,P_CALLED_BY_FORM => 'F'
			,P_ROWID => l_row_id
			,P_LABEL_CODE => X_LABEL_CODE
			,P_LANGUAGE => X_LANGUAGE
			,P_LABEL_DESCRIPTION => X_LABEL_DESCRIPTION
           ...
           insert into GR_LABELS_TL (
	        ...
	        0,
              ...
              and not exists
                (select NULL
                from GR_LABELS_TL T
                where T.LABEL_CODE = X_LABEL_CODE
...
begin
  ...
  insert into GR_LABELS_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from GR_LABELS_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
