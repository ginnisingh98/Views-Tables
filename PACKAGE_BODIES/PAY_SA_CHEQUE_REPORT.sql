--------------------------------------------------------
--  DDL for Package Body PAY_SA_CHEQUE_REPORT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_SA_CHEQUE_REPORT" AS
/* $Header: pysachqr.pkb 120.7.12000000.1 2007/02/22 08:43:46 vbattu noship $ */
...
------------------------------------------------------------------------------------------
  PROCEDURE CHEQUE_LISTING
   ...
   AS
	...
	/* Cursor for fetching assignment action id and assignment id for current payroll action id */

/******** Commented for Order by clause *********/
/*

	CURSOR csr_get_assact_det (l_pact_id number)  IS
...
/*** ORDER BY PAYDATE ***/
  ...
  BEGIN
    ...
    l_str_rep_label := '<REP_LABEL>' || get_lookup_meaning('SA_FORM_LABELS','SA_CHQR') || '</REP_LABEL>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_rep_label),l_str_rep_label);

    l_str_py_label := '<PY_LABEL>' || get_lookup_meaning('SA_FORM_LABELS','PY_LABEL') || '</PY_LABEL>';
    ...
    l_str_n := '<N>' || get_lookup_meaning('SA_FORM_LABELS','S_NO') || '</N>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_n),l_str_n);

    l_str_name_l := '<NAME_L>' || get_lookup_meaning('SA_FORM_LABELS','NAME') || '</NAME_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_name_l),l_str_name_l);

    l_str_eno_l := '<ENO_L>' || get_lookup_meaning('SA_FORM_LABELS','EMP_NO') || '</ENO_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_eno_l),l_str_eno_l);

    l_str_org_l := '<ORG_L>' || get_lookup_meaning('SA_FORM_LABELS','ORG_L') || '</ORG_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_org_l),l_str_org_l);

    l_str_job_l := '<JOB_L>' || get_lookup_meaning('SA_FORM_LABELS','JOB_L') || '</JOB_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_job_l),l_str_job_l);

    l_str_amt_l := '<AMT_L>' || get_lookup_meaning('SA_FORM_LABELS','AMOUNT') || '</AMT_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_amt_l),l_str_amt_l);

    l_str_chq_l := '<CHQ_L>' || get_lookup_meaning('SA_FORM_LABELS','CHQ_L') || '</CHQ_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_chq_l),l_str_chq_l);

    l_str_pd_l := '<PD_L>' || get_lookup_meaning('SA_FORM_LABELS','PD_L') || '</PD_L>';
...
------------------------------------------------------------------
  ...
  BEGIN
    IF (p_report='CHQR') THEN
      ...
      Into p_pdf_blob
      From fnd_lobs
      Where file_id = (select max(file_id) from fnd_lobs where file_name like '%PAY_CHQ_ar_SA.rtf');
    END IF;
