--------------------------------------------------------
--  DDL for Package Body GR_ITEM_PROPERTIES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_ITEM_PROPERTIES_PKG" AS
/*$Header: GRHIIPB.pls 115.7 2002/10/24 21:27:57 gkelly ship $*/
...
/* 	   Now check the primary key doesn't already exist */
   ...
   END IF;
/*B1319565 Added for Technical Parameters */
   INSERT INTO gr_item_properties
   		  	     (item_code,
				  sequence_number,
				  property_id,
   		  	      label_code,
...
EXCEPTION
   ...
   WHEN Item_Exists_Error THEN
      ROLLBACK TO SAVEPOINT Insert_Row;
	  l_msg_token := p_item_code || ' ' || p_label_code || ' ' || p_property_id;
	  x_return_status := 'E';
...
/*	  Now call the check foreign key procedure */
   ...
   /*B1319565 Added for Technical Parameters */
	  ...
	  SET	 item_code						 = p_item_code,
			 ...
			 print_tech_parm                               = p_print_tech_parm
	  WHERE  rowid = p_rowid;
...
/*B1319565 Added for Technical Parameters */
   ...
   DELETE FROM gr_item_properties
   WHERE  	   rowid = p_rowid;
...
*/
   ...
   ELSIF p_delete_option = 'B' THEN
      IF p_item_code IS NULL OR
		 ...
		 l_msg_token := 'Item or Label Code';
		 RAISE Null_Delete_Option_Error;
	  ELSE
...
BEGIN
   ...
   l_msg_data := p_item_code || ' ' || p_label_code || ' ' || p_property_id;
