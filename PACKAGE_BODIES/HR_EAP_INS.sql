--------------------------------------------------------
--  DDL for Package Body HR_EAP_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_EAP_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hr_eap_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_ext_application_id_i  number   default null;
...
l_ki_app_id       number(15);
l_proc     varchar2(72) := g_package || 'register';

BEGIN


--
-- If same external_application_name already exists in hr_ki_ext_applications
-- table then raise error.
--

  hr_eap_bus.CHK_EXTERNAL_APPLICATION_NAME
  (
...
-- insert record into hr_ki_ext_applications
  ...
  hr_eap_ins.ins
    (
     p_ext_application_id          => l_ki_app_id
    ,p_external_application_name   => p_app_code
    ,p_external_application_id     => l_app_id
    );

   p_ki_app_id := l_ki_app_id;
...
procedure set_base_key_value
  (p_ext_application_id  in  number) is
--
...
Begin
  ...
  --
  hr_eap_ins.g_ext_application_id_i := p_ext_application_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: hr_ki_ext_applications
  --
...
--
  Cursor C_Sel1 is select hr_ki_ext_applications_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from hr_ki_ext_applications
     where ext_application_id =
             hr_eap_ins.g_ext_application_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (hr_eap_ins.g_ext_application_id_i is not null) Then
    --
    ...
    p_rec.ext_application_id :=
      hr_eap_ins.g_ext_application_id_i;
    hr_eap_ins.g_ext_application_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.ext_application_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hr_eap_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hr_eap_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hr_eap_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Begin
  ...
  --
  p_ext_application_id := l_rec.ext_application_id;
  --
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
End ins;
