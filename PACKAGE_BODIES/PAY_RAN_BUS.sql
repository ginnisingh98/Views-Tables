--------------------------------------------------------
--  DDL for Package Body PAY_RAN_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_RAN_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_ran_bus.';  -- Global package name
--
-- The following two global variables are only to be
-- used by the return_legislation_code function.
--
g_legislation_code            varchar2(150)  default null;
g_range_id                    number         default null;
--
--  ---------------------------------------------------------------------------
...
Procedure set_security_group_id
  ...
  -- pay_ranges_f and PER_BUSINESS_GROUPS_PERF
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
       (p_associated_column1
        => nvl(p_associated_column1,'RANGE_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  if ( nvl(pay_ran_bus.g_range_id, hr_api.g_number)
       = p_range_id) then
    --
    ...
    -- variable.
    --
    l_legislation_code := pay_ran_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pay_ran_bus.g_range_id                    := p_range_id;
    pay_ran_bus.g_legislation_code  := l_legislation_code;
  end if;
...
Begin
  ...
  --
  IF NOT pay_ran_shd.api_updating
