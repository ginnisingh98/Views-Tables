--------------------------------------------------------
--  DDL for Package Body OE_UPDATE_LINE_ACK_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OE_UPDATE_LINE_ACK_UTIL" AS
/* $Header: OEXULAUB.pls 120.0 2005/05/31 22:38:21 appldev noship $ */
...
,p_return_status OUT NOCOPY VARCHAR2
  ...
  l_orig_sys_shipment_ref	VARCHAR2(50)	:= p_orig_sys_shipment_ref;
  ...
  l_api_name                    CONSTANT VARCHAR2(30) := 'Update_Line_Ack';
...
*/
    ...
    SELECT order_source_id
     ...
     WHERE order_source_id			= l_order_source_id
       ...
       AND change_sequence 			= l_change_sequence
       AND orig_sys_line_ref 			= l_orig_sys_line_ref
       AND orig_sys_shipment_ref 		= l_orig_sys_shipment_ref
...
*/
    ...
    END IF;
				...
				IF l_debug_level  > 0 THEN
				    oe_debug_pub.add(  'ACKNOWLEDGMENT CODE AND DATE UPDATED FOR LINE REF: '|| L_ORIG_SYS_LINE_REF ) ;
				END IF;
...
/* -----------------------------------------------------------
   End of Update Line Ack
   -----------------------------------------------------------
*/
      IF l_debug_level  > 0 THEN
          oe_debug_pub.add(  'END OF UPDATE LINE ACK' ) ;
      END IF;
