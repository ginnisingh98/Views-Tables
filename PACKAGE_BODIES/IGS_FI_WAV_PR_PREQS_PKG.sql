--------------------------------------------------------
--  DDL for Package Body IGS_FI_WAV_PR_PREQS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_FI_WAV_PR_PREQS_PKG" AS
/* $Header: IGSSIF7B.pls 120.0 2005/09/09 20:00:40 appldev noship $ */

  l_rowid VARCHAR2(25);
  old_references igs_fi_wav_pr_preqs%ROWTYPE;
  new_references igs_fi_wav_pr_preqs%ROWTYPE;
  ...
  */
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.waiver_relation_id                := x_waiver_relation_id;
    ...
    new_references.sup_waiver_name                   := x_sup_waiver_name;
    new_references.sub_waiver_name                   := x_sub_waiver_name;
  ...
  */
  BEGIN
    ...
    IF (((old_references.fee_cal_type = new_references.fee_cal_type) AND
         ...
         (old_references.sub_waiver_name = new_references.sub_waiver_name)) OR
        ((new_references.fee_cal_type IS NULL) OR
         (new_references.fee_ci_sequence_number IS NULL) OR
         (new_references.sub_waiver_name IS NULL))) THEN
      NULL;
    ...
    IF (((old_references.fee_cal_type = new_references.fee_cal_type) AND
         (old_references.fee_ci_sequence_number = new_references.fee_ci_sequence_number) AND
         (old_references.sup_waiver_name = new_references.sup_waiver_name)) OR
        ((new_references.fee_cal_type IS NULL) OR
         (new_references.fee_ci_sequence_number IS NULL) OR
         (new_references.sup_waiver_name IS NULL))) THEN
      NULL;
    ELSIF NOT igs_fi_waiver_pgms_pkg.get_pk_for_validation (
                new_references.fee_cal_type,
                new_references.fee_ci_sequence_number,
  ...
  */
  ...
  BEGIN
    ...
    IF (
        ...
        AND (tlinfo.sup_waiver_name = x_sup_waiver_name)
        AND (tlinfo.sub_waiver_name = x_sub_waiver_name)
       ) THEN
      NULL;
    ELSE
  ...
  */
    ...
    CURSOR cur_wav_pr_preqs(cp_rowid         varchar2) IS
      SELECT object_version_number
      FROM   igs_fi_wav_pr_preqs
      WHERE  rowid = cp_rowid
      FOR UPDATE NOWAIT;

    l_n_object_version_number           igs_fi_wav_pr_preqs.object_version_number%TYPE;

  BEGIN

    OPEN cur_wav_pr_preqs(x_rowid);
    FETCH cur_wav_pr_preqs INTO l_n_object_version_number;
    CLOSE cur_wav_pr_preqs;
