--------------------------------------------------------
--  DDL for Package Body GR_RISK_COMBINATIONS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_RISK_COMBINATIONS_PKG" AS
/*$Header: GRHIRCB.pls 115.4 2002/10/28 16:15:39 gkelly ship $*/
...
/*	  Now call the check foreign key procedure */
   ...
   Check_Foreign_Keys
			     (p_combination_sequence,
			      p_risk_phrase_code,
				  p_combination_group_number,
				  p_risk_phrase_code_combo,
				  p_display_order,
				  l_return_status,
				  l_oracle_error,
				  l_msg_data);
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_risk_combinations
   		  	     (combination_sequence_number,
   		  	      risk_phrase_code,
				  combination_group_number,
				  risk_phrase_code_combo,
				  display_order,
				  created_by,
				  creation_date,
				  last_updated_by,
				  ...
				  last_update_login)
          VALUES
		         (gr_risk_combinations_s.nextval,
		          p_risk_phrase_code,
				  p_combination_group_number,
				  p_risk_phrase_code_combo,
				  p_display_order,
...
/*	 Get the assigned sequence number */
   ...
   SELECT gr_risk_combinations_s.currval
   INTO   l_current_seq
   FROM	  dual;
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_risk_combinations
	  SET	 combination_sequence_number	 = p_combination_sequence,
	         risk_phrase_code 	 	 	 = p_risk_phrase_code,
	  		 combination_group_number		 = p_combination_group_number,
			 risk_phrase_code_combo		 = p_risk_phrase_code_combo,
...
CURSOR c_lock_combinations
 IS
   SELECT	*
   FROM		gr_risk_combinations
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockComboRcd	  c_lock_combinations%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_combinations;
   FETCH c_lock_combinations INTO LockComboRcd;
   IF c_lock_combinations%NOTFOUND THEN
	  CLOSE c_lock_combinations;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_risk_combinations
   WHERE  	   rowid = p_rowid;
...
/*   Initialization Routine */
   ...
   x_oracle_error := 0;
   x_msg_data := NULL;
   l_msg_token := p_risk_phrase_code;
...
/*   Check the main risk phrase code first. */

   l_risk_phrase := p_risk_phrase_code;
   GR_risk_PHRASES_B_PKG.Check_Primary_Key
					(l_risk_phrase,
					 'F',
					 l_rowid,
					 l_key_exists);
...
/*	 Now check the combination phrase	 */

   l_risk_phrase := p_risk_phrase_code_combo;
   GR_risk_PHRASES_B_PKG.Check_Primary_Key
					(l_risk_phrase,
					 'F',
					 l_rowid,
...
/*		  p_combination_sequence is the risk phrase combination to check.
...
CURSOR c_get_combo_rowid
 IS
   SELECT rc.rowid
   FROM	  gr_risk_combinations rc
   WHERE  rc.combination_sequence_number = p_combination_sequence;
ComboRecord			   c_get_combo_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   OPEN c_get_combo_rowid;
   FETCH c_get_combo_rowid INTO ComboRecord;
   IF c_get_combo_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := ComboRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_combo_rowid;
