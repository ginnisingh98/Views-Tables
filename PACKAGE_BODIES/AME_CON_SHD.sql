--------------------------------------------------------
--  DDL for Package Body AME_CON_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_CON_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_con_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_CONDITIONS_PK') Then
...
Begin
  ...
  Else
    If (p_condition_id =
        ame_con_shd.g_old_rec.condition_id and
        p_object_version_number =
        ame_con_shd.g_old_rec.object_version_number) Then
      --
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_con_shd.g_old_rec;
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_con_shd.g_old_rec.end_date;
...
Begin
  ...
  l_rec.condition_type                   := p_condition_type;
  l_rec.attribute_id                     := p_attribute_id;
  l_rec.parameter_one                    := p_parameter_one;
  l_rec.parameter_two                    := p_parameter_two;
  l_rec.parameter_three                  := p_parameter_three;
  l_rec.include_lower_limit              := p_include_lower_limit;
  l_rec.include_upper_limit              := p_include_upper_limit;
  l_rec.start_date                       := p_start_date;
  l_rec.end_date                         := p_end_date;
  l_rec.security_group_id                := p_security_group_id;
  l_rec.condition_key                    := p_condition_key;
