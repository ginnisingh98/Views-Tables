--------------------------------------------------------
--  DDL for Package Body CSM_DEBRIEF_EXPENSES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSM_DEBRIEF_EXPENSES_PKG" AS
/* $Header: csmudbeb.pls 120.4 2008/03/24 08:57:46 ptomar ship $ */
...
/*** Globals ***/
g_object_name  CONSTANT VARCHAR2(30) := 'CSM_DEBRIEF_EXPENSES_PKG';  -- package name
g_pub_name     CONSTANT VARCHAR2(30) := 'CSF_M_DEBRIEF_EXPENSES';  -- publication item name
g_debug_level           NUMBER; -- debug level

CURSOR c_debrief_expenses( b_user_name VARCHAR2, b_tranid NUMBER) is
  SELECT *
...
-- of a line.
  OPEN c_cdl
    (b_debrief_line_id => p_record.debrief_line_id
    );
  FETCH c_cdl
  INTO r_cdl;
  IF c_cdl%found
...
EXCEPTION WHEN OTHERS THEN
  /*** defer record when any process exception occurs ***/
    CSM_UTIL_PKG.LOG
    ( 'Exception occurred in CSM_DEBRIEF_EXPENSES_PKG.APPLY_RECORD:' || ' ' || sqlerrm
      || ' for PK ' || p_record.debrief_line_id ,'CSM_DEBRIEF_EXPENSES_PKG.APPLY_RECORD',FND_LOG.LEVEL_EXCEPTION );
...
BEGIN
  ...
  /*** loop through debrief expenses records in inqueue ***/
  FOR r_debrief_expenses IN c_debrief_expenses( p_user_name, p_tranid) LOOP
...
EXCEPTION WHEN OTHERS THEN
  /*** catch and log exceptions ***/
    ...
    'CSM_DEBRIEF_EXPENSES_PKG.APPLY_CLIENT_CHANGES',FND_LOG.LEVEL_EXCEPTION );
  x_return_status := FND_API.G_RET_STS_ERROR;
END APPLY_CLIENT_CHANGES;

END CSM_DEBRIEF_EXPENSES_PKG;
