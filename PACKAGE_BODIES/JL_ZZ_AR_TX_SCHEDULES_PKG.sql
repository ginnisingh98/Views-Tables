--------------------------------------------------------
--  DDL for Package Body JL_ZZ_AR_TX_SCHEDULES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."JL_ZZ_AR_TX_SCHEDULES_PKG" AS
/* $Header: jlzztscb.pls 120.5 2005/03/31 22:34:13 nipatel ship $ */
  ...
  PROCEDURE Insert_Row
    ...
    CURSOR C IS
      ...
      AND tax_categ_slab_id = X_tax_categ_slab_id;
  ...
  BEGIN
    ...
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_SCHEDULES_PKG.INSERT_ROW<-' ||
                                X_calling_sequence;

    debug_info := 'Insert into JL_ZZ_AR_TX_SCHEDULES  ';
    INSERT INTO jl_zz_ar_tx_schedules (tax_categ_slab_id,
                                       tax_category_id,
                                       end_date_active,
                                       min_taxable_basis,
                                       last_update_date,
    ...
    WHEN OTHERS THEN
      IF (SQLCODE <> -20001) THEN
        ...
        FND_MESSAGE.SET_TOKEN('PARAMETERS',
                              ...
                              ' tax_categ_slab_id = ' || X_tax_categ_slab_id ||
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_SCHEDULES_PKG.LOCK_ROW<-' ||
                                X_calling_sequence;
    debug_info := 'Open cursor C';
    OPEN C;
    debug_info := 'Fetch cursor C';
    ...
    IF ((Recinfo.tax_categ_slab_id = X_tax_categ_slab_id) AND
        ...
        ((Recinfo.max_taxable_basis = X_max_taxable_basis) OR
         ((Recinfo.max_taxable_basis IS NULL) AND
          (X_max_taxable_basis IS NULL))) AND
  ...
  BEGIN
    --Update  the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_SCHEDULES_PKG.UPDATE_ROW<-' ||
                                X_calling_sequence;
    debug_info := 'Update JL_ZZ_AR_TX_SCHEDULES';

    UPDATE jl_zz_ar_tx_schedules
    SET tax_categ_slab_id  = X_tax_categ_slab_id,
        tax_category_id    = X_tax_category_id,
        end_date_active    = X_end_date_active,
        min_taxable_basis  = X_min_taxable_basis,
        last_update_date   = X_last_update_date,
        last_updated_by    = X_last_updated_by,
        max_taxable_basis  = X_max_taxable_basis,
        tax_code           = X_tax_code,
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence :='JL_ZZ_AR_TX_SCHEDULE_PKG.CHECK_UNIQUE<-' ||
                               X_calling_sequence;
    SELECT  COUNT(1)
    INTO l_dummy
    FROM jl_zz_ar_tx_schedules
    ...
    IF (l_dummy >=1) THEN
      ...
      FND_MESSAGE.SET_TOKEN('ERROR',SQLERRM);
      FND_MESSAGE.SET_TOKEN('CALLING_SEQUENCE',current_calling_sequence);
      FND_MESSAGE.SET_TOKEN('PARAMETERS',' tax_category_id = ' || X_tax_category_id ||
                            ' tax_categ_slab_id = ' || X_tax_categ_slab_id ||
                            ' end_date_active = ' || to_char(X_end_date_active,'YYYY/MM/DD'));
      FND_MESSAGE.SET_TOKEN('DEBUG_INFO',debug_info);
      APP_EXCEPTION.RAISE_EXCEPTION;
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence :=' JL_ZZ_AR_TX_SCHEDULES_PKG.<-CHECK_OVERLAPPED_DATES' ||
                               X_calling_sequence;
    SELECT COUNT(1)
    INTO l_dummy
    FROM jl_zz_ar_tx_schedules a
    WHERE tax_category_id = X_tax_category_id
    AND tax_categ_slab_id = X_tax_categ_slab_id
    ...
    IF (l_dummy >=1) THEN
      ...
      FND_MESSAGE.SET_TOKEN('PARAMETERS',
                            ...
                            ' tax_categ_slab_id = ' || to_char(X_tax_categ_slab_id) ||
                            ' end_date_active = ' || to_char(X_end_date_active,'YYYY/MM/DD') ||
  ...
  BEGIN
    --  Update the calling sequence
    --
    current_calling_sequence := 'JL_ZZ_AR_TX_SCHEDULES_PKG.CHECK_GAPS<-' ||
                                X_calling_sequence;
    --
    ...
    INTO l_dummy
    FROM jl_zz_ar_tx_schedules a
    WHERE a.tax_category_id = X_tax_category_id
    AND a.tax_categ_slab_id = X_tax_categ_slab_id
