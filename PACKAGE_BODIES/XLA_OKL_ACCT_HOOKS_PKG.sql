--------------------------------------------------------
--  DDL for Package Body XLA_OKL_ACCT_HOOKS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XLA_OKL_ACCT_HOOKS_PKG" AS
-- $Header: xlaapokl.pkb 120.1 2007/05/17 07:27:58 svellani noship $
...
|                                                                            |
| DESCRIPTION                                                                |
|     Call accounting program integration APIs for Lease Management          |
|                                                                            |
| HISTORY                                                                    |
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'xla.plsql.xla_okl_acct_hooks_pkg';
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_okl_acct_hooks_pkg.trace');
END trace;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      xla_exceptions_pkg.raise_message
         (p_location   => 'xla_okl_acct_hooks_pkg.print_logfile');
END print_logfile;
...
BEGIN
   ...
   ----------------------------------------------------------------------------
   -- Calling different events depending on the event name
   ----------------------------------------------------------------------------
   ...
   WHEN  p_event_name = 'preaccounting'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call preaccounting API for
                            Lease Management'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      okl_sla_processing_pvt.preaccounting
   ...
   WHEN p_event_name = 'extract'        THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call extract API for
                            Lease Management'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      okl_sla_processing_pvt.extract
            ( p_application_id     => p_application_id
             ,p_accounting_mode    => p_accounting_mode);
   ...
   WHEN p_event_name = 'postprocessing'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call postprocessing API for
                            Lease Management'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

      okl_sla_processing_pvt.postprocessing
            ( p_application_id     => p_application_id
             ,p_accounting_mode    => p_accounting_mode);
   ...
   WHEN p_event_name = 'postaccounting'  THEN
      ...
      IF (C_LEVEL_EVENT >= g_log_level) THEN
         trace
            (p_msg      => 'Ready to call postaccounting API for
                            Lease Management'
            ,p_level    => C_LEVEL_EVENT
            ,p_module   => l_log_module);
      END IF;

     okl_sla_processing_pvt.postaccounting
            ( p_application_id     => p_application_id
             ,p_ledger_id          => p_ledger_id
             ,p_process_category   => p_process_category
             ,p_end_date           => p_end_date
...
WHEN xla_exceptions_pkg.application_exception THEN
   ...
   xla_exceptions_pkg.raise_message
      ...
      ,p_token_1        => 'LOCATION'
      ,p_value_1        => 'xla_okl_acct_hooks_pkg.main'
      ,p_token_2        => 'ERROR'
