--------------------------------------------------------
--  DDL for Package Body HXC_ERR_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_ERR_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  hxc_err_ins.';  -- Global package name
g_debug    boolean	:= hr_utility.debug_enabled;
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_error_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_error_id  in  number) is
--
  l_proc       varchar2(72);
--
Begin
  ...
  --
  hxc_err_ins.g_error_id_i := p_error_id;
  --
  if g_debug then
...
Begin
  ...
  --
  --
  -- Insert the row into: hxc_errors
  --
...
--
  Cursor C_Sel1 is select hxc_errors_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from hxc_errors
     where error_id =
             hxc_err_ins.g_error_id_i;
--
  l_proc   varchar2(72);
  l_exists varchar2(1);
--
Begin
  ...
  --
  If (hxc_err_ins.g_error_id_i is not null) Then
    --
  ...
  Else
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.error_id;
    Close C_Sel1;
  End If;
  --
  if g_debug then
...
Begin
  ...
  --
  hxc_err_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_err_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_err_ins.post_insert
     (p_rec
     );
  --
  -- Call to raise any errors on multi-message list
...
Procedure ins
  ...
  ,p_message_field                  in     varchar2 default null
  ,p_message_tokens                 in     varchar2 default null
  ,p_application_short_name         in     varchar2 default null
  ,p_error_id                       in     out nocopy number
