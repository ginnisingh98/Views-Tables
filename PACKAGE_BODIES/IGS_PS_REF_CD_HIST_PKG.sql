--------------------------------------------------------
--  DDL for Package Body IGS_PS_REF_CD_HIST_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_REF_CD_HIST_PKG" AS
/* $Header: IGSPI30B.pls 115.6 2003/05/09 06:26:09 sarakshi ship $ */

  l_rowid VARCHAR2(25);
  old_references IGS_PS_REF_CD_HIST_ALL%RowType;
  new_references IGS_PS_REF_CD_HIST_ALL%RowType;
  ...
  BEGIN
	...
	IF upper(column_name)= 'REFERENCE_CD' OR
		column_name is null THEN
		IF new_references.reference_cd <> UPPER(new_references.reference_cd)
		THEN
            	Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
            	IGS_GE_MSG_STACK.ADD;
            	App_Exception.Raise_Exception;
	...
	IF upper(column_name)= 'REFERENCE_CD_TYPE' OR
		column_name is null THEN
		IF new_references.reference_cd_type <> UPPER(new_references.reference_cd_type)
		THEN
            	Fnd_Message.Set_Name ('IGS', 'IGS_GE_INVALID_VALUE');
            	IGS_GE_MSG_STACK.ADD;
            	App_Exception.Raise_Exception;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CRCH_CV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_CRCH_RCT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_ORG_ID IN NUMBER
    ...
    cursor C is select ROWID from IGS_PS_REF_CD_HIST_ALL
      where COURSE_CD = X_COURSE_CD
      and REFERENCE_CD_TYPE = X_REFERENCE_CD_TYPE
      and REFERENCE_CD = X_REFERENCE_CD
      and HIST_START_DT = X_HIST_START_DT
...
Before_DML (
  ...
  insert into IGS_PS_REF_CD_HIST_ALL (
    COURSE_CD,
    VERSION_NUMBER,
    REFERENCE_CD_TYPE,
    REFERENCE_CD,
...
Before_DML (
  ...
  update IGS_PS_REF_CD_HIST_ALL set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_PS_REF_CD_HIST_ALL
     where COURSE_CD = X_COURSE_CD
     and REFERENCE_CD_TYPE = X_REFERENCE_CD_TYPE
     and REFERENCE_CD = X_REFERENCE_CD
     and HIST_START_DT = X_HIST_START_DT
...
Before_DML (
  ...
  delete from IGS_PS_REF_CD_HIST_ALL
    where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
