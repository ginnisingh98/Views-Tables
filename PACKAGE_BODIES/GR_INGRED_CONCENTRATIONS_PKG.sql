--------------------------------------------------------
--  DDL for Package Body GR_INGRED_CONCENTRATIONS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_INGRED_CONCENTRATIONS_PKG" AS
/*$Header: GRHINCB.pls 120.0 2005/07/08 11:17:56 methomas noship $*/
...
/*     Initialization Routine */
   ...
   x_oracle_error := 0;
   x_msg_data := NULL;
   l_msg_token := p_product_item_id || ' ' || p_ingredient_item_id;
...
/* 	   Now check the primary key doesn't already exist */
   ...
   INSERT INTO gr_ingredient_concentrations
   		  	     (organization_id,
		 	      product_item_id,
			      ingredient_item_id,
...
/*	  Now call the check foreign key procedure */
   ...
   IF l_return_status <> 'S' THEN
      RAISE Foreign_Key_Error;
   ELSE
      UPDATE gr_ingredient_concentrations
	  SET	 organization_id                 = p_organization_id,
		 product_item_id                 = p_product_item_id,
...
CURSOR c_lock_concs_tl
 IS
   SELECT	*
   FROM		gr_ingredient_concentrations
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockConcRcd	  c_lock_concs_tl%ROWTYPE;
BEGIN
...
/*	   Now lock the record */
   ...
   OPEN c_lock_concs_tl;
   FETCH c_lock_concs_tl INTO LockConcRcd;
   IF c_lock_concs_tl%NOTFOUND THEN
	  CLOSE c_lock_concs_tl;
...
/*  Now call the check integrity procedure */
   ...
   DELETE FROM gr_ingredient_concentrations
   WHERE  	   rowid = p_rowid;
...
/*		  p_product_item_id is the item id to check.
**	      p_ingredient_item_id is the second part of the key
...
CURSOR c_get_concs_rowid
 IS
   SELECT ic.rowid
   FROM	  gr_ingredient_concentrations ic
   WHERE  ic.organization_id     = p_organization_id
   AND    ic.product_item_id    = p_product_item_id
   AND	  ic.ingredient_item_id = p_ingredient_item_id;
ConcRecord			   c_get_concs_rowid%ROWTYPE;

BEGIN

   l_msg_data := p_product_item_id || ' ' || p_ingredient_item_id;

   x_key_exists := 'F';
   OPEN c_get_concs_rowid;
   FETCH c_get_concs_rowid INTO ConcRecord;
   IF c_get_concs_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := ConcRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_concs_rowid;
