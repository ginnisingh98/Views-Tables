--------------------------------------------------------
--  DDL for Package Body MTH_EQUIPMENTS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MTH_EQUIPMENTS_EXT_PKG" as
/* $Header: mthuetlb.pls 120.0.12010000.3 2009/08/21 09:50:08 sdonthu ship $ */
/*
...
procedure INSERT_ROW (
  ...
  X_SEGMENT_FK_KEY in NUMBER,
  X_SHIFT_WORKDAY_FK_KEY in NUMBER,
  ...
  cursor C is select ROWID from MTH_EQUIPMENTS_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into MTH_EQUIPMENTS_EXT_B (
    EXTENSION_ID,
    ATTR_GROUP_ID,
    EQUIPMENT_PK_KEY,
    C_EXT_ATTR1,
  ...
  insert into MTH_EQUIPMENTS_EXT_TL (
    EXTENSION_ID,
    ATTR_GROUP_ID,
    EQUIPMENT_PK_KEY,
    TL_EXT_ATTR1,
...
begin
  ...
  if (    ((recinfo.ATTR_GROUP_ID = X_ATTR_GROUP_ID)
      ...
      AND ((recinfo.EQUIPMENT_PK_KEY = X_EQUIPMENT_PK_KEY)
           OR ((recinfo.EQUIPMENT_PK_KEY is null) AND (X_EQUIPMENT_PK_KEY is null)))
      ...
      AND ((recinfo.SEGMENT_FK_KEY = X_SEGMENT_FK_KEY)
           OR ((recinfo.SEGMENT_FK_KEY is null) AND (X_SEGMENT_FK_KEY is null)))
      AND ((recinfo.SHIFT_WORKDAY_FK_KEY = X_SHIFT_WORKDAY_FK_KEY)
           OR ((recinfo.SHIFT_WORKDAY_FK_KEY is null) AND (X_SHIFT_WORKDAY_FK_KEY is null)))
...
begin
  update MTH_EQUIPMENTS_EXT_B set
    ATTR_GROUP_ID = X_ATTR_GROUP_ID,
    EQUIPMENT_PK_KEY = X_EQUIPMENT_PK_KEY,
    ...
    SEGMENT_FK_KEY = X_SEGMENT_FK_KEY,
    SHIFT_WORKDAY_FK_KEY = X_SHIFT_WORKDAY_FK_KEY,
  ...
  update MTH_EQUIPMENTS_EXT_TL set
...
begin
  delete from MTH_EQUIPMENTS_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from MTH_EQUIPMENTS_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  delete from MTH_EQUIPMENTS_EXT_TL T
  where not exists
    (select NULL
    from MTH_EQUIPMENTS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update MTH_EQUIPMENTS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from MTH_EQUIPMENTS_EXT_TL SUBB, MTH_EQUIPMENTS_EXT_TL SUBT
  ...
  insert into MTH_EQUIPMENTS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from MTH_EQUIPMENTS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
