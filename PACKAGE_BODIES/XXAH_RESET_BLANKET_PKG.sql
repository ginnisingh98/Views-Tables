--------------------------------------------------------
--  DDL for Package Body XXAH_RESET_BLANKET_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XXAH_RESET_BLANKET_PKG" 
AS
  ...
  PROCEDURE write_log
    ...
    PRAGMA AUTONOMOUS_TRANSACTION;
  BEGIN
    write_log( p_message, 'XXAH_RESET_BLANKET_PKG' );
  ...
  /* Procedure xxah_reset_po is based on Oracle Datafix from NOTEID: ID 390023.1*/
	/*=======================================================================+
 	 |  Copyright (c) 2009 Oracle Corporation Redwood Shores, California, USA|
 	 |                            All rights reserved.                       |
 	 +=======================================================================*/
   /* $Header: poxrespo.sql 120.0.12000000.4 2010/06/09 00:20:47 vrecharl noship $ */
  ...
  */
    PROCEDURE datafix_reset_po(
        p_po_number VARCHAR2
      , p_org_id NUMBER
      , p_delete_action_history VARCHAR2  -- 'Y/N'
    )
    IS
    --DECLARE
    ...
    select p_delete_action_history
  ...
  IS
    -- c_reset_po: Selects which POs need to be reset
    CURSOR c_reset_po
    IS
      SELECT pha.segment1 PO_NUMBER
        , pha.org_id ORG_ID
      FROM po_headers_all pha
      WHERE pha.authorization_status IN ('IN PROCESS', 'PRE-APPROVED')
      AND pha.type_lookup_code = 'BLANKET'
      AND pha.segment1 = p_blanket_nr
    ;

    TYPE t_reset_po IS TABLE OF c_reset_po%ROWTYPE;
    v_reset_po t_reset_po;
  BEGIN
    write_log( 'Concurrent program to run datafix from note ID: 390023.1');
    write_log( 'Blanket Purchase Agreement: ' || p_blanket_nr || ' will be reset' );
  -- FETCH into bulk all PO's that need to be reset
    OPEN c_reset_po;
    FETCH c_reset_po BULK COLLECT INTO v_reset_po;
    CLOSE c_reset_po;

    -- Process the fetched bulk
    FOR indx IN 1 .. v_reset_po.COUNT
    LOOP
      -- Call to datafix from oracle
      datafix_reset_po( v_reset_po(indx).po_number, v_reset_po(indx).org_id, 'Y' );
      write_log( 'Reset completed of Blanket Purchase Agreement: ' || v_reset_po(indx).po_number );
    END LOOP;
