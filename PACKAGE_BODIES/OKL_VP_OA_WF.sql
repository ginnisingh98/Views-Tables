--------------------------------------------------------
--  DDL for Package Body OKL_VP_OA_WF
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_VP_OA_WF" AS
/* $Header: OKLROAWB.pls 120.2 2006/09/29 12:13:15 ssdeshpa noship $ */

  G_NO_MATCHING_RECORD CONSTANT fnd_new_messages.message_name%TYPE DEFAULT 'OKL_LLA_NO_MATCHING_RECORD';
  G_COL_NAME_TOKEN CONSTANT VARCHAR2(200)  := OKL_API.G_COL_NAME_TOKEN;

  G_OPERATING_AGRMNT_EVENT CONSTANT wf_events.name%TYPE DEFAULT 'oracle.apps.okl.la.vp.approve_operating_agreement';

  G_AGRMNT_APPROVAL_WF  CONSTANT VARCHAR2(2)  DEFAULT 'WF';
  G_AGRMNT_APPROVAL_AME CONSTANT VARCHAR2(3)  DEFAULT 'AME';

  G_ITEM_TYPE_WF CONSTANT VARCHAR2(10) DEFAULT 'OKLOAAPP';
  G_ITEM_TYPE_AME CONSTANT VARCHAR2(10) DEFAULT 'OKLAMAPP';

  G_TRANS_APP_NAME_OA CONSTANT ame_calling_apps.application_name%TYPE DEFAULT 'OKL LA Operating Agreement Approval';
  ...
  PROCEDURE raise_oa_event_approval(p_api_version   IN NUMBER
    ...
    CURSOR c_get_oa_num_csr(cp_chr_id okc_k_headers_b.id%TYPE)IS
    SELECT id
          ,contract_number
      FROM okc_k_headers_b
     WHERE id = cp_chr_id;
    cv_get_oa_num c_get_oa_num_csr%ROWTYPE;
    ...
    l_module CONSTANT fnd_log_messages.module%TYPE := 'okl.plsql.OKL_VP_OA_WF.RAISE_OA_EVENT_APPROVAL';
  ...
  BEGIN
    ...
    IF(l_debug_enabled='Y') THEN
      okl_debug_pub.log_debug(FND_LOG.LEVEL_PROCEDURE,l_module,'begin debug OKLRXXXB.pls call raise_oa_event_approval');
    END IF;
    ...
    ELSIF(NVL(l_approval_process,'NONE') = G_AGRMNT_APPROVAL_WF)THEN
      ...
      IF(NVL(l_debug_enabled,'N')='Y') THEN
        okl_debug_pub.log_debug(FND_LOG.LEVEL_STATEMENT,
                                l_module,
                                'l_approval_process '||l_approval_process||' raising operating agreement approval event'
                                );
      END IF; -- end of NVL(l_debug_enabled,'N')='Y'
    ...
    IF(l_approval_process IN (G_AGRMNT_APPROVAL_WF, G_AGRMNT_APPROVAL_AME))THEN
      OPEN c_get_oa_num_csr(p_chr_id); FETCH c_get_oa_num_csr INTO cv_get_oa_num;
      CLOSE c_get_oa_num_csr;

      -- get the agreement information to put as event parameters and raise the event
      wf_event.AddParameterToList(G_WF_ITM_AGREEMENT_ID, cv_get_oa_num.id, l_parameter_list);
      wf_event.AddParameterToList(G_WF_ITM_AGREEMENT_NUMBER, cv_get_oa_num.contract_number, l_parameter_list);
      ...
      wf_event.AddParameterToList(G_WF_ITM_TRANSACTION_ID,cv_get_oa_num.id,l_parameter_list);
      --added by akrangan
	wf_event.AddParameterToList('ORG_ID',mo_global.get_current_org_id ,l_parameter_list);

      lv_wf_item_key := get_item_key_wf;
      -- Raise Event
      wf_event.RAISE(p_event_name => G_OPERATING_AGRMNT_EVENT,
    ...
    IF(l_debug_enabled='Y') THEN
      okl_debug_pub.log_debug(FND_LOG.LEVEL_PROCEDURE,l_module,'end debug OKLRXXXB.pls call raise_oa_event_approval');
    END IF;
  ...
  BEGIN
    IF(funcmode = 'RUN')THEN
      l_approval_option := fnd_profile.value('OKL_VP_OA_APPROVAL_PROCESS');
  ...
  PROCEDURE get_agrmnt_approver(itemtype  IN VARCHAR2
    ...
    l_module CONSTANT fnd_log_messages.module%TYPE := 'okl.plsql.OKL_VP_OA_WF.GET_AGRMNT_APPROVER';
    l_debug_enabled VARCHAR2(10);
    ...
    IF(funcmode = 'RUN')THEN
      ...
      IF l_user_id IS NOT NULL THEN
       FOR l_fnd_users_rec IN l_fnd_users_csr(l_user_id) LOOP
         ...
         -- Message: "Operating Agreement <AGR_NUMBER> requires approval"
         set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVAL_SUB', G_WF_ITM_MESSAGE_SUBJECT);

         wf_engine.SetItemAttrText (itemtype => itemtype,
                                    itemkey  => itemkey,
                                    aname    => G_WF_ITM_MESSAGE_BODY,
                                    avalue   => 'plsql:okl_vp_oa_wf.get_msg_doc/'||itemkey
                                   );
  ...
  PROCEDURE update_agrmnt_status(itemtype   IN VARCHAR2
    ...
    l_module CONSTANT fnd_log_messages.module%TYPE := 'okl.plsql.OKL_VP_OA_WF.UPDATE_AGRMNT_STATUS';
    l_debug_enabled VARCHAR2(10);
  ...
  PROCEDURE process_oa_for_ame(itemtype   IN VARCHAR2
                              ,itemkey   IN VARCHAR2
                              ,actid     IN NUMBER
                              ,funcmode  IN VARCHAR2
                              ,resultout OUT NOCOPY VARCHAR2) IS
    l_api_name CONSTANT VARCHAR2(200) DEFAULT 'process_oa_for_ame';
    l_module CONSTANT fnd_log_messages.module%TYPE := 'okl.plsql.OKL_VP_OA_WF.PROCESS_OA_FOR_AME';
    l_debug_enabled VARCHAR2(10);
    lv_agreement_id okc_k_headers_b.id%TYPE;
  BEGIN
    IF(funcmode = 'RUN')THEN
      ...
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVAL_SUB', G_WF_ITM_APP_REQUEST_SUB);
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVAL_REM', G_WF_ITM_APP_REMINDER_SUB);
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVAL_REM', G_WF_ITM_APP_REMINDER_HEAD);
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVED_SUB', G_WF_ITM_APP_APPROVED_SUB);
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVED_SUB', G_WF_ITM_APP_APPROVED_HEAD);
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_REJECT_SUB', G_WF_ITM_APP_REJECTED_SUB);
      set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_REJECT_SUB', G_WF_ITM_APP_REJECTED_HEAD);
  ...
  BEGIN
    ...
    IF(funcmode = 'RUN')THEN
      ...
      IF(G_WF_ITM_APPROVED_YN_YES = lv_approval_status)THEN
        set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_APPROVED_SUB', G_WF_ITM_MESSAGE_SUBJECT);
      ELSIF(G_WF_ITM_APPROVED_YN_NO = lv_approval_status)THEN
        set_attrib_message(itemtype, itemkey, 'OKL_VN_OA_REQ_REJECT_SUB', G_WF_ITM_MESSAGE_SUBJECT);
      END IF;
      wf_engine.SetItemAttrText (itemtype => itemtype,
                                 itemkey  => itemkey,
                                 aname    => G_WF_ITM_MESSAGE_BODY,
