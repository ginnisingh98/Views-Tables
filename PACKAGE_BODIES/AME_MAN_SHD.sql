--------------------------------------------------------
--  DDL for Package Body AME_MAN_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_MAN_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_man_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_MANDATORY_ATTRIBUTES_PK') Then
...
Begin
  ...
  Else
    If (p_attribute_id =
        ame_man_shd.g_old_rec.attribute_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_man_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ame_man_shd.g_old_rec.object_version_number) Then
...
Begin
 ...
 ,p_action_type_id =>  p_action_type_id
  ...
  --
  update ame_mandatory_attributes t
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_man_shd.g_old_rec.end_date;
