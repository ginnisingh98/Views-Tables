--------------------------------------------------------
--  DDL for Package Body PER_ALLOCATED_TASK_SWI
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_ALLOCATED_TASK_SWI" As
/* $Header: pepatswi.pkb 120.0 2005/09/28 07:49 lsilveir noship $ */
--
-- Package variables
--
g_package  varchar2(33) := 'per_allocated_task_swi.';
--
-- ----------------------------------------------------------------------------
-- |---------------------------< create_alloc_task >--------------------------|
-- ----------------------------------------------------------------------------
PROCEDURE create_alloc_task
  ...
  l_proc    varchar2(72) := g_package ||'create_alloc_task';
Begin
  hr_utility.set_location(' Entering:' || l_proc,10);
  --
  -- Issue a savepoint
  --
  savepoint create_alloc_task_swi;
  --
  ...
  --
  per_pat_ins.set_base_key_value
...
exception
  when hr_multi_message.error_message_exist then
    ...
    --
    rollback to create_alloc_task_swi;
    --
...
PROCEDURE delete_alloc_task
  ...
  --
  -- Other variables
  l_proc    varchar2(72) := g_package ||'delete_alloc_task';
Begin
  hr_utility.set_location(' Entering:' || l_proc,10);
  --
  -- Issue a savepoint
  --
  savepoint delete_alloc_task_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
  ...
  -- Call API
  --
  per_allocated_task_api.delete_alloc_task
    (p_validate                     => l_validate
    ,p_allocated_task_id            => p_allocated_task_id
    ,p_object_version_number        => p_object_version_number
    );
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to delete_alloc_task_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
    p_return_status := hr_multi_message.get_return_status_disable;
...
PROCEDURE update_alloc_task
  ...
  --
  -- Other variables
  l_proc    varchar2(72) := g_package ||'update_alloc_task';
Begin
  hr_utility.set_location(' Entering:' || l_proc,10);
  --
  -- Issue a savepoint
  --
  savepoint update_alloc_task_swi;
  --
  -- Initialise Multiple Message Detection
  --
  hr_multi_message.enable_message_list;
...
exception
  when hr_multi_message.error_message_exist then
    ...
    -- at least one message exists in the list.
    --
    rollback to update_alloc_task_swi;
    --
    -- Reset IN OUT parameters and set OUT parameters
    --
