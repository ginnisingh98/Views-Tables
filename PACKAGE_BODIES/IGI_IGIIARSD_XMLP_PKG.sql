--------------------------------------------------------
--  DDL for Package Body IGI_IGIIARSD_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGI_IGIIARSD_XMLP_PKG" AS
/* $Header: IGIIARSDB.pls 120.0.12010000.1 2008/07/29 08:58:50 appldev ship $ */
...
procedure FDRCSID(idstring in varchar2) is
  BEGIN
    IF (idstring is NULL) THEN
      NULL;
    END IF;
        --        FDRCSID('$Header: IGIIARSDB.pls 120.0.12010000.1 2008/07/29 08:58:50 appldev ship $');
  END;
...
PROCEDURE Insert_info (
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RP_REPORT_NAME := 'Inflation Accounting : Cost Detail Report' || '(' || CURRENCY_CODE || ')';
  ...
  FUNCTION FA_BEGINNINGFORMULA(BALANCE_TYPE IN VARCHAR2
    ...
    L_BEGIN NUMBER := 0;
  BEGIN
    IF (BALANCE_TYPE = 'Historic') THEN
      L_BEGIN := BEGINNING;
    END IF;
    RETURN (L_BEGIN);
  END FA_BEGINNINGFORMULA;

  FUNCTION IAC_BEGINNINGFORMULA(BALANCE_TYPE IN VARCHAR2
                               ,BEGINNING IN NUMBER) RETURN NUMBER IS
    L_BEGIN NUMBER := 0;
  BEGIN
    IF (BALANCE_TYPE = 'IAC') THEN
      L_BEGIN := BEGINNING;
    END IF;
    RETURN (L_BEGIN);
  END IAC_BEGINNINGFORMULA;
  ...
  BEGIN
    ...
    END IF;
    RETURN (L_NUM);
  END IAC_ENDINGFORMULA;

  FUNCTION FA_DEPRECIATIONFORMULA(BALANCE_TYPE IN VARCHAR2
  ...
  BEGIN
    ...
    END IF;
    RETURN (L_NUM);
  END FA_DEPRECIATIONFORMULA;

  FUNCTION IAC_DEPRECIATIONFORMULA(BALANCE_TYPE IN VARCHAR2
                                  ,DEPRECIATION IN NUMBER) RETURN NUMBER IS
    L_BEGIN NUMBER := 0;
  BEGIN
    IF (BALANCE_TYPE = 'IAC') THEN
      L_BEGIN := DEPRECIATION;
    END IF;
    RETURN (L_BEGIN);
  END IAC_DEPRECIATIONFORMULA;
  ...
  FUNCTION ACCT_ACT_APROMPT_P RETURN VARCHAR2 IS
  BEGIN
    RETURN ACCT_ACT_APROMPT;
  END ACCT_ACT_APROMPT_P;
  ...
  END RP_CTR_LPROMPT_P;

END IGI_IGIIARSD_XMLP_PKG;
