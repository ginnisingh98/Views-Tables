--------------------------------------------------------
--  DDL for Package Body CSD_SC_DOMAINS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSD_SC_DOMAINS_PKG" as
/* $Header: csdtscdb.pls 115.3 2003/11/05 00:18:08 gilam noship $ */
-- Start of Comments
...
G_PKG_NAME CONSTANT VARCHAR2(30):= 'CSD_SC_DOMAINS_PKG';
G_FILE_NAME CONSTANT VARCHAR2(12) := 'csdtscdb.pls';
...
PROCEDURE Insert_Row(
 ...
 IS
   CURSOR C2 IS SELECT CSD_SC_DOMAINS_S1.nextval FROM sys.dual;
BEGIN
   If (px_SC_DOMAIN_ID IS NULL) OR (px_SC_DOMAIN_ID = FND_API.G_MISS_NUM) then
       OPEN C2;
       FETCH C2 INTO px_SC_DOMAIN_ID;
       CLOSE C2;
   End If;
   INSERT INTO CSD_SC_DOMAINS(
           SC_DOMAIN_ID
...
BEGIN
    ...
    where SC_DOMAIN_ID = p_SC_DOMAIN_ID;
...
PROCEDURE Lock_Row(
 ...
 IS
   CURSOR C IS
       ...
       FROM CSD_SC_DOMAINS
       WHERE SC_DOMAIN_ID =  p_SC_DOMAIN_ID
       FOR UPDATE of SC_DOMAIN_ID NOWAIT;
   Recinfo C%ROWTYPE;
BEGIN
    OPEN C;
    ...
    CLOSE C;
    if (
           (      Recinfo.SC_DOMAIN_ID = p_SC_DOMAIN_ID)
