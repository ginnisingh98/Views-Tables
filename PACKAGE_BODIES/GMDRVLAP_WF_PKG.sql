--------------------------------------------------------
--  DDL for Package Body GMDRVLAP_WF_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMDRVLAP_WF_PKG" AS
/* $Header: GMDRVLAB.pls 120.1 2006/06/06 06:24:21 kmotupal noship $ */
   PROCEDURE wf_init (
   ...
   IS
      ...
      l_WorkflowProcess   VARCHAR2(30) := 'GMDRVLAP_PROCESS';
      l_count             NUMBER;
      BEGIN
      	...
      	WF_ENGINE.SETITEMATTRNUMBER(itemtype => l_itemtype,itemkey => l_itemkey,
         				  	       aname => 'GMDRVLAP_RECIPE_ID',
         	                               avalue => p_recipe_id);
      	WF_ENGINE.SETITEMATTRNUMBER(itemtype => l_itemtype,itemkey => l_itemkey,
         				  	       aname => 'GMDRVLAP_RECIPE_VALIDITYRULEID',
         	                               avalue => p_recipe_validity_rule_id);

      	WF_ENGINE.SETITEMATTRNUMBER(itemtype => l_itemtype,itemkey => l_itemkey,
         				  	       aname => 'GMDRVLAP_START_STATUS',
         	                               avalue => p_start_status);

      	WF_ENGINE.SETITEMATTRNUMBER(itemtype => l_itemtype,itemkey => l_itemkey,
         				  	       aname => 'GMDRVLAP_TARGET_STATUS',
      	...
      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_RECIPE_NO',
         					  avalue => l_recipe_no);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_RECIPE_VERS',
      	...
      	WF_ENGINE.SETITEMATTRNUMBER(itemtype => l_itemtype,itemkey => l_itemkey,
         				  	       aname => 'GMDRVLAP_TIMEOUT',
         	                               avalue => l_wf_timeout);
      	WF_ENGINE.SETITEMATTRNUMBER(itemtype => l_itemtype,itemkey => l_itemkey,
         				  	       aname => 'GMDRVLAP_MESG_CNT',
         	                               avalue => 1);
	 ...
	 END IF;
      	...
      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_ORGN_CODE',
         					  avalue => l_orgn_code);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_RECIPE_USE',
         					  avalue => l_recipe_use);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_ITEM_NO',
         					  avalue => l_item_no);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_PREFERENCE',
         					  avalue => l_preference);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_STD_QTY',
         					  avalue => l_std_qty);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_MIN_QTY',
         					  avalue => l_min_qty);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_MAX_QTY',
         					  avalue => l_max_qty);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_EFF_START_DATE',
         					  avalue => l_effective_start_date);

      	WF_ENGINE.SETITEMATTRTEXT(itemtype => l_itemtype,itemkey => l_itemkey,
         					  aname => 'GMDRVLAP_EFF_END_DATE',
  ...
  EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDRVLAP_wf_pkg','wf_init',l_itemtype,l_itemkey,'Initial' );
      raise;
...
/* ######################################################################## */
   ...
   IS
      ...
      p_lab_wf_item_type  VARCHAR2(8)  := 'GMDRVLAP';  -- Recipe Lab use Approval Workflow Inernal Name
      P_lab_Process_name  VARCHAR2(32) := 'GMDRVLAP_PROCESS'; -- Recipe Lab use Approval Workflow Process Inernal Name
      P_lab_activity_name VARCHAR2(80) := 'GMDRVLAP_NOTI_REQUEST';
      P_table_name        VARCHAR2(32) := 'GMD_RECIPE_VALIDITY_RULES'; -- Key Table
      P_where_clause      VARCHAR2(100); -- Where clause to be appended
    ...
    BEGIN
      l_recipe_validity_rule_id:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDRVLAP_RECIPE_VALIDITYRULEID');
      -- Bug# 5030408 Kapil M : Removed the table name
      P_where_clause := ' RECIPE_VALIDITY_RULE_ID='||l_recipe_validity_rule_id;
     IF (p_funcmode = 'RUN') THEN
         ...
         THEN
	      ...
	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDRVLAP_ADDL_TEXT',
                                      avalue => l_data_string);

	      WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
      			              itemkey => p_itemkey,
         					  aname => 'GMDRVLAP_APPROVER',
                                      avalue => p_role);

            p_resultout:='COMPLETE:Y';
        ELSE
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_TARGET_STATUS');
...
EXCEPTION
      WHEN OTHERS THEN
      WF_CORE.CONTEXT ('GMDRVLAP_wf_pkg','is_approval_req',p_itemtype,p_itemkey,p_role);
      raise;
END is_approval_req;
...
PROCEDURE REMINDAR_CHECK (
      ...
      l_mesg_cnt      number:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDRVLAP_MESG_CNT');
      l_approver      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_APPROVER');
BEGIN
...
BEGIN
     l_recipe_validity_rule_id:=wf_engine.getitemattrnumber(p_itemtype, p_itemkey,'GMDRVLAP_RECIPE_VALIDITYRULEID');
     IF (p_funcmode = 'RUN') THEN
          l_start_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_START_STATUS');
          l_target_status := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_TARGET_STATUS');
...
PROCEDURE MOREINFO_RESPONSE  (
      ...
      p_funcmode      IN VARCHAR2,
      p_resultout     OUT NOCOPY VARCHAR2) IS
      l_requester     VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_REQUSTER');
BEGIN
...
PROCEDURE APPEND_COMMENTS (
      ...
      l_comment       VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_COMMENT');
      l_mesg_comment  VARCHAR2(4000):=wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_DISP_COMMENT');
      l_performer      VARCHAR2(80) := wf_engine.getitemattrtext(p_itemtype, p_itemkey,'GMDRVLAP_CURR_PERFORMER');
BEGIN
...
--           l_html_mesg := replace(l_mesg_comment,wf_core.newline,'<BR>'||wf_core.newline);
	   ...
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDRVLAP_DISP_COMMENT',
                                   avalue => l_mesg_comment);
--	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
--                                   itemkey => p_itemkey,
--         			           aname => 'GMDRVLAP_HTML_DISP_COMMENT',
--                                   avalue => l_html_mesg);
	   WF_ENGINE.SETITEMATTRTEXT(itemtype => p_itemtype,
                                   itemkey => p_itemkey,
         			           aname => 'GMDRVLAP_COMMENT',
                                   avalue => l_comment);
       END IF;
END;
