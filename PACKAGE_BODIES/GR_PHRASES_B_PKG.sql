--------------------------------------------------------
--  DDL for Package Body GR_PHRASES_B_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GR_PHRASES_B_PKG" AS
/*$Header: GRHIPHRB.pls 115.6 2002/10/28 19:49:24 methomas ship $*/
...
CURSOR c_lock_phrase
 IS
   SELECT	last_update_date
   FROM		gr_phrases_b
   WHERE	rowid = p_rowid
   FOR UPDATE NOWAIT;
LockPhraseRcd	  c_lock_phrase%ROWTYPE;
...
/*	   Now lock the record */
   ...
   OPEN c_lock_phrase;
   FETCH c_lock_phrase INTO LockPhraseRcd;
   IF c_lock_phrase%NOTFOUND THEN
	  CLOSE c_lock_phrase;
...
CURSOR c_get_audit
 IS
   SELECT COUNT(*)
   FROM	  gr_audit au
   WHERE  au.label_phrase_code = p_phrase_code;
...
CURSOR c_get_doc_structure
 IS
   ...
   WHERE  ds.phrase_code = p_phrase_code;
...
CURSOR c_get_item_properties
 IS
   ...
   WHERE  ip.alpha_value = p_phrase_code;

/*   Phrase Conflicts */

CURSOR c_get_phrase_conflicts
 IS
   SELECT COUNT(*)
   FROM	  gr_phrase_conflicts pc
   WHERE  pc.phrase_code = p_phrase_code
   OR	  pc.phrase_code_also_involved = p_phrase_code
   OR	  pc.phrase_code_solution = p_phrase_code;

/*   Item Names */

CURSOR c_get_phrase_groups
 IS
   SELECT COUNT(*)
   FROM	  gr_phrase_group_assoc pga
   WHERE  pga.phrase_code = p_phrase_code;
/*
** Item Risk Phrases
*/
CURSOR c_get_item_risks
 IS
   ...
   WHERE  irp.phrase_code = p_phrase_code;
/*
** Item Safety Phrases
*/
CURSOR c_get_item_safety
 IS
   ...
   WHERE  isp.phrase_code = p_phrase_code;
...
/*  Phrase Conflicts */
   ...
   l_record_count := 0;
   l_code_block := 'c_get_phrase_conflicts';
   OPEN c_get_phrase_conflicts;
   FETCH c_get_phrase_conflicts INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
	  l_msg_data := l_msg_data || 'gr_phrase_conflicts, ';
   END IF;
   CLOSE c_get_phrase_conflicts;

/*  Phrase Group Associations */

   l_record_count := 0;
   l_code_block := 'c_get_phrase_groups';
   OPEN c_get_phrase_groups;
   FETCH c_get_phrase_groups INTO l_record_count;
   IF l_record_count <> 0 THEN
      l_return_status := 'E';
	  l_msg_data := l_msg_data || 'gr_phrase_groups, ';
   END IF;
   CLOSE c_get_phrase_groups;
/*
** Item Risk Phrases
*/
   l_record_count := 0;
...
CURSOR c_get_phrase_rowid
 IS
   SELECT phr.rowid
   FROM	  gr_phrases_b phr
   WHERE  phr.phrase_code = p_phrase_code;
PhraseRecord			   c_get_phrase_rowid%ROWTYPE;

BEGIN

   x_key_exists := 'F';
   l_msg_data := p_phrase_code;
   OPEN c_get_phrase_rowid;
   FETCH c_get_phrase_rowid INTO PhraseRecord;
   IF c_get_phrase_rowid%FOUND THEN
      x_key_exists := 'T';
	  x_rowid := PhraseRecord.rowid;
   ELSE
      x_key_exists := 'F';
   END IF;
   CLOSE c_get_phrase_rowid;
