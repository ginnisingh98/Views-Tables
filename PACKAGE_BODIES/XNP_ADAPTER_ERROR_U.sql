--------------------------------------------------------
--  DDL for Package Body XNP_ADAPTER_ERROR_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_ADAPTER_ERROR_U" AS
/*$Header: XNPADERB.pls 120.2 2006/02/13 07:37:49 dputhiye ship $*/
...
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('ADAPTER_ERROR',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'ADAPTER_ERROR' ;
  ...
  XNP_XML_UTILS.write_element ( 'FE_NAME', XNP$FE_NAME );
  XNP_XML_UTILS.write_element ( 'CHANNEL_NAME', XNP$CHANNEL_NAME );
  XNP_XML_UTILS.write_element ( 'STATUS_CODE', XNP$STATUS_CODE );
  XNP_XML_UTILS.write_element ( 'DESCRIPTION', XNP$DESCRIPTION );
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'ADAPTER_ERROR.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'ADAPTER_ERROR', l_consumer_list );
  END IF;
  l_recipient_list := p_recipient_list ;
l_queue_name := 'XNP_IN_EVT_Q';
/* create the XML message */
  CREATE_MSG (
    XNP$FE_NAME=>XNP$FE_NAME,
    XNP$CHANNEL_NAME=>XNP$CHANNEL_NAME,
    XNP$STATUS_CODE=>XNP$STATUS_CODE,
    XNP$DESCRIPTION=>XNP$DESCRIPTION,
    x_msg_header=>l_msg_header,
...
END ;
PROCEDURE SEND  (   XNP$FE_NAME VARCHAR2,
  XNP$CHANNEL_NAME VARCHAR2,
  XNP$STATUS_CODE VARCHAR2,
  XNP$DESCRIPTION VARCHAR2,
  x_message_id OUT NOCOPY  NUMBER,
...
BEGIN
	xnp_adapter.notify_fmc(p_msg_header,
		p_msg_text,
		l_error_code,
		l_error_message
	) ;
