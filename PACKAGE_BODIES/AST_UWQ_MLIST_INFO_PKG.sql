--------------------------------------------------------
--  DDL for Package Body AST_UWQ_MLIST_INFO_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AST_UWQ_MLIST_INFO_PKG" AS
/* $Header: astuminb.pls 120.3 2006/01/12 02:08:12 rkumares ship $ */
...
BEGIN
	...
	l_object_code := 'PARTY%';
    if  p_action_key in ('astuminb_cust_notes', 'astuminb_prof_cust_notes', 'astuminb_cust_notes_man', 'astuminb_prof_cust_notes_man') then  -- begin of main "if"

       if l_customer_id is not null then
	     l_object_id := l_customer_id;
    ...
    elsif p_action_key in ('astuminb_cont_notes', 'astuminb_prof_cont_notes', 'astuminb_cont_notes_man', 'astuminb_prof_cont_notes_man') then  -- begin of main "if"

     if l_source_type in ('PERSON_LIST', 'CONSUMER') then
	   return;
     end if;
         ...
         for c1_rec in c_party_type(l_party_id)
          LOOP
          l_party_type := c1_rec.party_type;
          END LOOP;
         end if;
	...
	if p_action_key in ('astuminb_cust_notes','astuminb_cont_notes','astuminb_cust_notes_man','astuminb_cont_notes_man') then
	    l_cur_ind := 'DESC'  ;

	elsif p_action_key in ('astuminb_prof_cust_notes','astuminb_prof_cont_notes','astuminb_prof_cust_notes_man','astuminb_prof_cont_notes_man') then
...
end ast_uwq_mlist_notes;
END ast_uwq_mlist_info_pkg;
