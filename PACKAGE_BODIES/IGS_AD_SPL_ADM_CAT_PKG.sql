--------------------------------------------------------
--  DDL for Package Body IGS_AD_SPL_ADM_CAT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_SPL_ADM_CAT_PKG" AS
/* $Header: IGSAIA3B.pls 120.2 2005/10/03 08:22:24 appldev ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_ad_spl_adm_cat%RowType;
  new_references igs_ad_spl_adm_cat%RowType;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.spl_adm_cat_id := x_spl_adm_cat_id;
    ...
    new_references.spl_adm_cat := x_spl_adm_cat;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (((old_references.spl_adm_cat = new_references.spl_adm_cat)) OR
        ((new_references.spl_adm_cat IS NULL))) THEN
      NULL;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ad_spl_adm_cat
      WHERE    spl_adm_cat_id = x_spl_adm_cat_id
      FOR UPDATE NOWAIT;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      ...
      AND      spl_adm_cat = x_spl_adm_cat 	and      ((l_rowid is null) or (rowid <> l_rowid))
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ASAC_ACAI_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ASAC_ACDC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
    ...
    IF (p_action = 'INSERT') THEN
      ...
      Null;
	     IF Get_Pk_For_Validation(
    		new_references.spl_adm_cat_id)  THEN
	       Fnd_Message.Set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
      IGS_GE_MSG_STACK.ADD;
	       App_Exception.Raise_Exception;
	     END IF;
  ...
  ***************************************************************/
    ...
    cursor C is select ROWID from IGS_AD_SPL_ADM_CAT
             where                 SPL_ADM_CAT_ID= X_SPL_ADM_CAT_ID;
 ...
 begin
  ...
  END IF;
 insert into IGS_AD_SPL_ADM_CAT (
		 SPL_ADM_CAT_ID
		,PERSON_ID
		...
		,X_LAST_UPDATED_BY
		,X_LAST_UPDATE_LOGIN
)RETURNING SPL_ADM_CAT_ID INTO X_SPL_ADM_CAT_ID;
 IF (x_mode = 'S') THEN
...
if ( (tlinfo.PERSON_ID = X_PERSON_ID)
  ...
  AND (tlinfo.SPL_ADM_CAT = X_SPL_ADM_CAT)
  ) then
    null;
  else
 ...
 update IGS_AD_SPL_ADM_CAT set
      ...
      SPL_ADM_CAT =  NEW_REFERENCES.SPL_ADM_CAT,
...
EXCEPTION
  ...
  ***************************************************************/
    ...
    cursor c1 is select ROWID from IGS_AD_SPL_ADM_CAT
             where     SPL_ADM_CAT_ID= X_SPL_ADM_CAT_ID;
begin
	open c1;
...
x_rowid => X_ROWID
  ...
  IF (x_mode = 'S') THEN
    igs_sc_gen_001.set_ctx('R');
  END IF;
 delete from IGS_AD_SPL_ADM_CAT
 where ROWID = X_ROWID;
  if (sql%notfound) then
     fnd_message.set_name ('IGS', 'IGS_SC_POLICY_UPD_DEL_EXCEP');
     igs_ge_msg_stack.add;
