--------------------------------------------------------
--  DDL for Package Body XNP_ACK_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_ACK_U" AS
/* $Header: XNPACKSB.pls 120.2 2006/02/13 05:58:07 dputhiye ship $*/
...
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('ACK',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  XNP_XML_UTILS.write_element ( 'CODE', XNP$CODE );
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'ACK.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'ACK', l_consumer_list );
  END IF;
