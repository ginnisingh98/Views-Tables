--------------------------------------------------------
--  DDL for Package Body HXC_HAD_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_HAD_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_had_ins.';  -- Global package name
...
Begin
  ...
  --
  --
  -- Insert the row into: hxc_alias_definitions
  --
  insert into hxc_alias_definitions
      (alias_definition_id
      ,description
...
--
  l_proc  varchar2(72);
--
  Cursor C_Sel1 is select hxc_alias_definitions_s.nextval from sys.dual;
--
Begin
  if g_debug then
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.alias_definition_id;
  Close C_Sel1;
  --
  if g_debug then
	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_had_bus.insert_validate
     (p_rec
     );
  --
  -- Call the supporting pre-insert operation
  --
  hxc_had_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_had_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_had_ins.post_insert
     (p_rec
     );
  --
  if g_debug then
...
Begin
  ...
  --
  -- Having converted the arguments into the hxc_had_rec
  ...
  --
  p_alias_definition_id := l_rec.alias_definition_id;
