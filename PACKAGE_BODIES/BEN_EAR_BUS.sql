--------------------------------------------------------
--  DDL for Package Body BEN_EAR_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_EAR_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_ear_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_enrld_anthr_plip_rt_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   enrld_anthr_plip_rt_id PK of record being inserted or updated.
...
--
Procedure chk_enrld_anthr_plip_rt_id(p_enrld_anthr_plip_rt_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_enrld_anthr_plip_rt_id';
...
Begin
  ...
  --
  l_api_updating := ben_ear_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_enrld_anthr_plip_rt_id                => p_enrld_anthr_plip_rt_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_enrld_anthr_plip_rt_id,hr_api.g_number)
     <>  ben_ear_shd.g_old_rec.enrld_anthr_plip_rt_id) then
    --
    -- raise error as PK has changed
    --
    ben_ear_shd.constraint_error('BEN_ENRLD_ANTHR_PLIP_RT_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_enrld_anthr_plip_rt_id is not null then
      --
  ...
  --
End chk_enrld_anthr_plip_rt_id;
...
and enrld_anthr_plip_rt_id <> nvl(p_enrld_anthr_plip_rt_id,hr_api.g_number)
...
--
  ...
  if (l_api_updating
      and
      (p_plip_id <> nvl(ben_ear_shd.g_old_rec.plip_id,hr_api.g_number) or
       p_enrl_det_dt_cd <>  nvl(ben_ear_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2)
      )
	...
	if c_dup%found then
   		...
   		fnd_message.set_token('VAR1','Enrolled in Another Plan in Program criteria',TRUE);
...
--   Internal table handler use only.
--
Procedure chk_enrl_det_dt_cd(p_enrld_anthr_plip_rt_id    in number,
                             p_enrl_det_dt_cd              in varchar2,
                             p_effective_date              in date,
                             p_object_version_number       in number) is
  --
...
Begin
  ...
  if (l_api_updating
      ...
      <> nvl(ben_ear_shd.g_old_rec.enrl_det_dt_cd,hr_api.g_varchar2)
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_enrld_anthr_plip_rt_id    in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_ear_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
      or not l_api_updating) then
    --
    -- check if value of lookup falls within lookup type.
    --
...
l_proc   varchar2(72) := g_package||' chk_duplicate_ordr_num ';
   ...
   cursor c1 is select null
		   ...
		   and enrld_anthr_plip_rt_id   <> nvl(p_enrld_anthr_plip_rt_id  ,-1)
...
function return_legislation_code
  (p_enrld_anthr_plip_rt_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_enrld_anthr_plip_rt_f b
    where b.enrld_anthr_plip_rt_id      = p_enrld_anthr_plip_rt_id
