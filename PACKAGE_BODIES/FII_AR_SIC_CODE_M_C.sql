--------------------------------------------------------
--  DDL for Package Body FII_AR_SIC_CODE_M_C
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."FII_AR_SIC_CODE_M_C" AS
/* $Header: FIIAR02B.pls 120.1 2004/03/01 05:42:11 sgautam ship $ */
 ...
 l_dimension_name   Varchar2(30) :='EDW_SIC_CODE_M'  ;
...
Begin
  ...
  END IF;
  FII_AR_SIC_CODE_M_C.g_push_date_range1 := nvl(l_from_date,
  		EDW_COLLECTION_UTIL.G_local_last_push_start_date - EDW_COLLECTION_UTIL.g_offset);
  FII_AR_SIC_CODE_M_C.g_push_date_range2 := nvl(l_to_date,EDW_COLLECTION_UTIL.G_local_curr_push_start_date);
  if g_debug_flag = 'Y' then
   edw_log.put_line( 'The collection range is from '||
        to_char(FII_AR_SIC_CODE_M_C.g_push_date_range1,'MM/DD/YYYY HH24:MI:SS')||' to '||
        to_char(FII_AR_SIC_CODE_M_C.g_push_date_range2,'MM/DD/YYYY HH24:MI:SS'));
   edw_log.put_line(' ');
  end if;
...
-- -----------------------------------------------------------------------------
   ...
   l_temp_date := sysdate;
        ...
        Push_EDW_SICM_SIC_LSTG(FII_AR_SIC_CODE_M_C.g_push_date_range1, FII_AR_SIC_CODE_M_C.g_push_date_range2);
...
commit;
 ...
 Exception When others then
   ...
   FII_AR_SIC_CODE_M_C.g_exception_msg  := l_exception_msg;
   rollback;
   EDW_COLLECTION_UTIL.wrapup(FALSE, 0, FII_AR_SIC_CODE_M_C.g_exception_msg,g_push_date_range1, g_push_date_range2);

commit;
End;


Procedure Push_EDW_SICM_SIC_LSTG(p_from_date IN date, p_to_date IN DATE) IS
    l_date1 DATE;
    l_date2 DATE;
    l_rows_inserted NUMBER :=0;
BEGIN
   if g_debug_flag = 'Y' then
   	edw_log.put_line('Starting Push_EDW_SICM_SIC_LSTG');
   end if;
l_date1 := p_from_date;
l_date2 := p_to_date;
   Insert Into
...
USER_ATTRIBUTE5,
NULL, --DESCRIPTION,
NULL, --UPDATE_FACT_FLAG,
    NULL, -- OPERATION_CODE
    'READY'
   from FII_AR_SICM_SIC_LCV
...
commit;
   ...
   if g_debug_flag = 'Y' then
   	edw_log.put_line('Completed Push_EDW_SICM_SIC_LSTG');
   end if;
 Exception When others then
   raise;
commit;
