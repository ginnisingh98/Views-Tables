--------------------------------------------------------
--  DDL for Package Body HR_TEMPLATE_ITEM_TAB_PAGES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HR_TEMPLATE_ITEM_TAB_PAGES_PKG" as
/* $Header: hrtiplct.pkb 115.3 2002/12/11 10:01:23 raranjan noship $ */
...
procedure INSERT_ROW (
  ...
  X_TEMPLATE_ITEM_TAB_PAGE_ID in NUMBER,
  ...
  cursor C is select ROWID from HR_TEMPLATE_ITEM_TAB_PAGES
    where TEMPLATE_ITEM_TAB_PAGE_ID = X_TEMPLATE_ITEM_TAB_PAGE_ID
    ;
begin
...
procedure LOCK_ROW (
  ...
  cursor c1 is select
    ...
    where TEMPLATE_ITEM_TAB_PAGE_ID = X_TEMPLATE_ITEM_TAB_PAGE_ID
    for update of TEMPLATE_ITEM_TAB_PAGE_ID nowait;
begin
...
begin
  update HR_TEMPLATE_ITEM_TAB_PAGES set
...
procedure LOAD_ROW (
  ...
  X_TEMPLATE_ITEM_TAB_PAGE_ID NUMBER;
begin
 ...
 begin
   ...
   into x_template_item_tab_page_id
   from hr_template_item_tab_pages
   where template_tab_page_id = x_template_tab_page_id
   and template_item_id = x_template_item_id;
 exception
   when no_data_found then
     select hr_template_item_tab_pages_s.nextval
     into x_template_item_tab_page_id
     from dual;
 end;
...
end HR_TEMPLATE_ITEM_TAB_PAGES_PKG;
