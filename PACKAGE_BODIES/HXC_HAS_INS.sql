--------------------------------------------------------
--  DDL for Package Body HXC_HAS_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."HXC_HAS_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  hxc_has_ins.';  -- Global package name
...
Begin
  ...
  --
  -- Insert the row into: hxc_approval_styles
  --
  insert into hxc_approval_styles
      (approval_style_id
...
--
  l_proc  varchar2(72);
--
  Cursor C_Sel1 is select hxc_approval_styles_s.nextval from sys.dual;
--
Begin
  if g_debug then
  ...
  --
  Open C_Sel1;
  Fetch C_Sel1 Into p_rec.approval_style_id;
  Close C_Sel1;
  --
  if g_debug then
	hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  hxc_has_bus.insert_validate
     (p_rec
     );
  --
  -- Call the supporting pre-insert operation
  --
  hxc_has_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  hxc_has_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  hxc_has_ins.post_insert
     (p_rec
     );
  --
  if g_debug then
...
Begin
  ...
  --
  p_approval_style_id := l_rec.approval_style_id;
