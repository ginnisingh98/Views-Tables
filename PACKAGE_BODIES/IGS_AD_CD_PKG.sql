--------------------------------------------------------
--  DDL for Package Body IGS_AD_CD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_CD_PKG" AS
/* $Header: IGSAI08B.pls 115.7 2003/10/30 13:10:14 akadam ship $ */
...
/*****  Bug No :   1956374
  ...
  l_rowid VARCHAR2(25);
  old_references IGS_AD_CD%RowType;
  new_references IGS_AD_CD%RowType;
  ...
  PROCEDURE BeforeRowInsertUpdate1(
    ...
    BEGIN
	-- Validate TAC Admission Code.
	IF p_inserting OR ((old_references.tac_admission_cd <> new_references.tac_admission_cd) OR
			(old_references.closed_ind = 'Y' AND new_references.closed_ind = 'N')) THEN
		IF IGS_AD_VAL_ACO.admp_val_tac_closed(
		...
		END IF;
	END IF;
	-- Validate Basis for Admission type.
	IF p_inserting OR (old_references.basis_for_admission_type <>
			new_references.basis_for_admission_type) OR
			(old_references.closed_ind = 'Y' AND new_references.closed_ind = 'N') THEN
  ...
  BEGIN
    ...
    ELSE
      ...
      END IF;
    END IF;
    IF (((old_references.tac_admission_cd = new_references.tac_admission_cd)) OR
        ((new_references.tac_admission_cd IS NULL))) THEN
      NULL;
    ELSE
      IF NOT IGS_AD_TAC_AD_CD_PKG.Get_PK_For_Validation (
         new_references.tac_admission_cd ,
         'N') THEN
     		Fnd_Message.Set_Name ('FND', 'FORM_RECORD_DELETED');
     		IGS_GE_MSG_STACK.ADD;
  ...
  BEGIN
    ...
    IGS_AD_PS_APPL_PKG.GET_FK_IGS_AD_CD (
  ...
  RETURN BOOLEAN  AS
	CURSOR cur_rowid IS
      	SELECT   rowid
      	FROM     IGS_AD_CD
      	WHERE    admission_cd = x_admission_cd AND
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ACO_BFA_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_ACO_TAC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
	...
	IF (p_action = 'INSERT') THEN
      	...
      	IF  Get_PK_For_Validation (
          		new_references.admission_cd  ) THEN
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_CD
...
begin
  ...
  if ( (tlinfo.DESCRIPTION = X_DESCRIPTION)
      AND (tlinfo.TAC_ADMISSION_CD = X_TAC_ADMISSION_CD)
...
begin
  ...
  update IGS_AD_CD set
    DESCRIPTION = NEW_REFERENCES.DESCRIPTION,
    TAC_ADMISSION_CD = NEW_REFERENCES.TAC_ADMISSION_CD,
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_CD
     where ADMISSION_CD = X_ADMISSION_CD
  ;
begin
  open c1;
