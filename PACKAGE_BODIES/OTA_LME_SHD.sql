--------------------------------------------------------
--  DDL for Package Body OTA_LME_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_LME_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ota_lme_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'OTA_LP_MEMBER_ENROLLMENTS_PK') Then
...
Begin
  --
  If (p_lp_member_enrollment_id is null and
    ...
    l_fct_ret := false;
  Else
    If (p_lp_member_enrollment_id
        = ota_lme_shd.g_old_rec.lp_member_enrollment_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ota_lme_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ota_lme_shd.g_old_rec.object_version_number) Then
...
Begin
  ...
  hr_api.mandatory_arg_error
    (p_api_name           => l_proc
    ,p_argument           => 'LP_MEMBER_ENROLLMENT_ID'
    ,p_argument_value     => p_lp_member_enrollment_id
    );
