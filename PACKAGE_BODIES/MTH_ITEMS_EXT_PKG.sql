--------------------------------------------------------
--  DDL for Package Body MTH_ITEMS_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."MTH_ITEMS_EXT_PKG" as
/* $Header: mthuitlb.pls 120.0.12010000.3 2009/08/21 09:50:52 sdonthu ship $ */
/*
...
procedure INSERT_ROW (
  ...
  cursor C is select ROWID from MTH_ITEMS_EXT_B
    where EXTENSION_ID = X_EXTENSION_ID
    ;
begin
  insert into MTH_ITEMS_EXT_B (
    EXTENSION_ID,
    ATTR_GROUP_ID,
    ITEM_PK_KEY,
    C_EXT_ATTR1,
...
begin
  ...
  if (    ((recinfo.ATTR_GROUP_ID = X_ATTR_GROUP_ID)
      ...
      AND ((recinfo.ITEM_PK_KEY = X_ITEM_PK_KEY)
           OR ((recinfo.ITEM_PK_KEY is null) AND (X_ITEM_PK_KEY is null)))
...
begin
  update MTH_ITEMS_EXT_B set
    ATTR_GROUP_ID = X_ATTR_GROUP_ID,
    ITEM_PK_KEY = X_ITEM_PK_KEY,
...
is
begin
  delete from MTH_ITEMS_EXT_TL T
  where not exists
    (select NULL
    from MTH_ITEMS_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update MTH_ITEMS_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from MTH_ITEMS_EXT_TL SUBB, MTH_ITEMS_EXT_TL SUBT
  ...
  insert into MTH_ITEMS_EXT_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from MTH_ITEMS_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
