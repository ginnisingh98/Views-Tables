--------------------------------------------------------
--  DDL for Package Body XNP_ERROR_RESP_U
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XNP_ERROR_RESP_U" AS
/*$Header: XNPERREB.pls 120.2 2006/02/13 07:46:10 dputhiye ship $*/
...
IF (l_xml_body IS NULL) THEN
  XNP_XML_UTILS.write_element('ERROR_RESP',l_xml_body );
  XNP_XML_UTILS.get_document( l_xml_body ) ;
END IF;
...
/* append header parameters to make header */
  ...
  l_msg_header.message_code := 'ERROR_RESP' ;
...
/* handle exceptions */
  ...
  WHEN OTHERS THEN
    ...
    x_error_message := 'ERROR_RESP.create_msg()::' || SQLERRM ;
END ;
...
/* check if the consumer list is NULL */
  ...
  IF (l_consumer_list IS NULL) THEN
    XNP_MESSAGE.GET_SUBSCRIBER_LIST( 'ERROR_RESP', l_consumer_list );
  END IF;
