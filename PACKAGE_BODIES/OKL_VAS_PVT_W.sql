--------------------------------------------------------
--  DDL for Package Body OKL_VAS_PVT_W
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKL_VAS_PVT_W" as
  /* $Header: OKLIVASB.pls 120.0 2005/08/04 23:15:13 manumanu noship $ */
  ...
  if a0 is not null and a0.count > 0 then
      if a0.count > 0 then
        ...
        while true loop
          ...
          t(ddindx).assoc_object_type_code := a7(indx);
          t(ddindx).assoc_object_id := a8(indx);
          t(ddindx).assoc_object_version := a9(indx);
          ...
          t(ddindx).creation_date := a31(indx);
          t(ddindx).last_updated_by := a32(indx);
          t(ddindx).last_update_date := a33(indx);
          t(ddindx).last_update_login := a34(indx);
          ddindx := ddindx+1;
          if a0.last =indx
            then exit;
          end if;
    ...
    a34 := JTF_NUMBER_TABLE();
      ...
      if t.count > 0 then
        ...
        while true loop
          ...
          a7(indx) := t(ddindx).assoc_object_type_code;
          a8(indx) := t(ddindx).assoc_object_id;
          a9(indx) := t(ddindx).assoc_object_version;
          ...
          a32(indx) := t(ddindx).last_updated_by;
          a33(indx) := t(ddindx).last_update_date;
          a34(indx) := t(ddindx).last_update_login;
          indx := indx+1;
          if t.last =ddindx
            then exit;
          end if;
  ...
  as
    ddp_vasv_rec okl_vas_pvt.vasv_rec_type;
    ddx_vasv_rec okl_vas_pvt.vasv_rec_type;
    ddindx binary_integer; indx binary_integer;
  begin
  ...
  as
    ddp_vasv_tbl okl_vas_pvt.vasv_tbl_type;
    ddx_vasv_tbl okl_vas_pvt.vasv_tbl_type;
