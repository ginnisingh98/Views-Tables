--------------------------------------------------------
--  DDL for Package Body PO_GA_ORG_ASSIGN_DRAFT_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PO_GA_ORG_ASSIGN_DRAFT_PVT" AS
/* $Header: PO_GA_ORG_ASSIGN_DRAFT_PVT.plb 120.2 2005/07/26 16:21 bao noship $ */

d_pkg_name CONSTANT varchar2(50) :=
  PO_LOG.get_package_base('PO_GA_ORG_ASSIGN_DRAFT_PVT');
...
--  given the draft_id or draft_id + org_assignment_id
--    If only draft_id is provided, this program returns FND_API.G_TRUE for
--  any draft changes in this table for the draft
--    If the whole primary key is provided (draft_id + org assignment id), then
...
--  po ga org assignment unique identifier
--IN OUT:
--OUT:
--Returns:
...
BEGIN
  ...
  FORALL i IN 1..p_draft_id_tbl.COUNT
    ...
    WHERE EXISTS (SELECT 1
                  ...
                  WHERE  PGOAD.draft_id = p_draft_id_tbl(i)
                  AND    PGOAD.org_assignment_id =
                           NVL(p_org_assignment_id_tbl(i),
...
--  None
--Function:
--  Process org assignment draft records and merge them to transaction table. It
...
BEGIN
  ...
  IF (p_draft_info.ga_org_assign_changed = FND_API.G_FALSE) THEN
    IF (PO_LOG.d_stmt) THEN
  ...
  d_position := 10;
  -- Merge Changes
  PO_GA_ORG_ASSIGN_DRAFT_PKG.merge_changes
