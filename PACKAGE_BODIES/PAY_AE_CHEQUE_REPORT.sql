--------------------------------------------------------
--  DDL for Package Body PAY_AE_CHEQUE_REPORT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_AE_CHEQUE_REPORT" AS
/* $Header: pyaechqr.pkb 120.0.12010000.3 2010/03/25 11:51:03 bkeshary ship $ */
...
/*** ORDER BY PAYDATE ***/
  ...
  BEGIN
    ...
    l_str_rep_label := '<REP_LABEL>' || get_lookup_meaning('AE_FORM_LABELS','AE_CHQR') || '</REP_LABEL>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_rep_label),l_str_rep_label);

    l_str_py_label := '<PY_LABEL>' || get_lookup_meaning('AE_FORM_LABELS','PY_LABEL') || '</PY_LABEL>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_py_label),l_str_py_label);

    l_str_py_name := '<PY_NAME>' ||'<![CDATA['|| l_py_name ||']]>'|| '</PY_NAME>'; /* Added for Bug No : 9454120 */
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_py_name),l_str_py_name);

    l_str_n := '<N>' || get_lookup_meaning('AE_FORM_LABELS','S_NO') || '</N>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_n),l_str_n);

    l_str_name_l := '<NAME_L>' || get_lookup_meaning('AE_FORM_LABELS','NAME') || '</NAME_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_name_l),l_str_name_l);

    l_str_eno_l := '<ENO_L>' || get_lookup_meaning('AE_FORM_LABELS','EMP_NO') || '</ENO_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_eno_l),l_str_eno_l);

    l_str_org_l := '<ORG_L>' || get_lookup_meaning('AE_FORM_LABELS','ORG_L') || '</ORG_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_org_l),l_str_org_l);

    l_str_job_l := '<JOB_L>' || get_lookup_meaning('AE_FORM_LABELS','JOB_L') || '</JOB_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_job_l),l_str_job_l);

    l_str_amt_l := '<AMT_L>' || get_lookup_meaning('AE_FORM_LABELS','AMOUNT') || '</AMT_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_amt_l),l_str_amt_l);

    l_str_chq_l := '<CHQ_L>' || get_lookup_meaning('AE_FORM_LABELS','CHQ_L') || '</CHQ_L>';
    dbms_lob.writeAppend( l_xfdf_string, length(l_str_chq_l),l_str_chq_l);

    l_str_pd_l := '<PD_L>' || get_lookup_meaning('AE_FORM_LABELS','PD_L') || '</PD_L>';
 	...
 	If i > 0 then
 		...
 		WHILE j <= i LOOP
			...
			Exit when csr_chq_det%NOTFOUND;
				...
				l_str_er_name3 := '<EEORG>'||'<![CDATA['||substr(l_organization,1,40)||']]>'||'</EEORG>'; /* Added for Bug No : 9454120 */
...
-------------------------------------------------------------------------------------------
  ...
  BEGIN
	...
	/* Added CDATA to handle special characters Bug No:9454120 */
	  l_str9 := '<![CDATA['||l_str9||']]>';
...
------------------------------------------------------------------
  ...
  BEGIN
    IF (p_report='CHQR') THEN
      ...
      Into p_pdf_blob
      From fnd_lobs
      Where file_id = (select max(file_id) from fnd_lobs where file_name like '%PAY_CHQ_ar_AE.rtf');
    END IF;
