--------------------------------------------------------
--  DDL for Package Body GMA_AUDIT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GMA_AUDIT_PKG" AS
/* $Header: GMAAUDTB.pls 115.2 2001/12/20 01:49:29 pkm ship       $ */
  PROCEDURE RETRIEVE_RESULT_TABLES(AUDIT_WHERE_CLAUSE   VARCHAR2,
                                  TABLE_WHERE_CLAUSE   VARCHAR2,
                                  QUERY_TABLES IN OUT AUDIT_REQUIRED_TABLES_TYPE)   AS
  ...
  BEGIN
      audit_sql_stmt := ' select distinct FND_TABLES.TABLE_NAME   TABLE_NAME ,
                ...
                from  FND_AUDIT_GROUPS ,
                      ...
                      FND_AUDIT_TABLES    ,
                      FND_TABLES
                WHERE GMA_AUDIT_TMPLT_DTL.AUDIT_GROUP_ID  = FND_AUDIT_GROUPS.AUDIT_GROUP_ID
                  AND GMA_AUDIT_TMPLT_DTL.APPLICATION_ID  = FND_AUDIT_GROUPS.APPLICATION_ID
      ...
      LOOP
        ...
        BEGIN
          ...
          FROM tab
          where tname like substrb(AUDIT_TABLE_REC.TABLE_NAME,1,26)||'_AC1';
          ...
          ELSE
             table_sql_stmt := '  SELECT 1 DATA_EXISTS_FLAG  '||
                             '   FROM '||substrb(AUDIT_TABLE_REC.TABLE_NAME,1,26)||'_A  ' ||
...
--      missing_base_table_flag := 1;
     ...
     ******/
     ...
     data_select_stmt :=   ' SELECT '||l_where_clause || '  AUDIT_KEY,AUDIT_TIMESTAMP,AUDIT_TRANSACTION_TYPE,AUDIT_USER_NAME,'
                         || P_SELECT_CLAUSE
                         || ' FROM '||substrb(P_TABLE_NAME,1,26)||'_AC1'
     ...
     LOOP
       ...
       IF PREV_AUDIT_KEY IS NULL  OR
          ...
          v_select_stmt1 := v_select_stmt || ' AND '||pk_column_list ||' = '||''''||AUDIT_DATA_REC.AUDIT_KEY||'''';
          OPEN DISP_COL_VALUE for v_select_stmt1;
       ...
       END IF;
       INSERT INTO GMA_AUDIT_REP_DTL(REP_ID
                                 ,TABLE_NAME
                                 ,AUDIT_KEY
                                 ,AUDIT_TIMESTAMP
