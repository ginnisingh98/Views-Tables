--------------------------------------------------------
--  DDL for Package Body PQH_TSH_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PQH_TSH_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pqh_tsh_bus.';  -- Global package name
--
...
g_approval_history_id                 number         default null;
--
--  ---------------------------------------------------------------------------
  ...
  if csr_sec_grp%notfound then
     ...
     hr_multi_message.add
      ...
      ,p_associated_column2
        => nvl(p_associated_column2,'APPROVAL_HISTORY_ID')
       );
     --
  else
    close csr_sec_grp;
...
Begin
  ...
  --
  /*if (( nvl(pqh_tsh_bus.g_transaction_history_id, hr_api.g_number)
       = p_transaction_history_id)
  and ( nvl(pqh_tsh_bus.g_approval_history_id, hr_api.g_number)
       = p_approval_history_id)) then
    --
    -- The legislation code has already been found with a previous
    -- call to this function. Just return the value in the global
    -- variable.
    --
    l_legislation_code := pqh_tsh_bus.g_legislation_code;
    ...
    --
    close csr_leg_code;
    pqh_tsh_bus.g_transaction_history_id      := p_transaction_history_id;
    pqh_tsh_bus.g_approval_history_id                 := p_approval_history_id;
    pqh_tsh_bus.g_legislation_code  := l_legislation_code;
  end if;*/
...
Begin
  ...
  --
  IF NOT pqh_tsh_shd.api_updating
      (p_transaction_history_id            => p_rec.transaction_history_id
      ,p_approval_history_id               => p_rec.approval_history_id
      ) THEN
