--------------------------------------------------------
--  DDL for Package Body IGS_PS_UNIT_GRD_SCHM_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_PS_UNIT_GRD_SCHM_PKG" AS
/* $Header: IGSPI99B.pls 115.12 2002/11/29 02:44:02 nsidana ship $ */
  l_rowid VARCHAR2(25);
  old_references igs_ps_unit_grd_schm%RowType;
  new_references igs_ps_unit_grd_schm%RowType;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    -- Populate New Values.
    new_references.unit_grading_schema_id := x_unit_grading_schema_id;
  ...
  Change History
  Who             When            What
  shtatiko        18-NOV-2002     Added check for default_flag as part
  ...
  ***************************************************************/
  ...
  AS
  BEGIN
    IGS_EN_SU_ATTEMPT_PKG.GET_UFK_IGS_PS_UNIT_GRD_SCHM (old_references.unit_version_number,
                                                       old_references.grading_schema_code,
                                                       old_references.grd_schm_version_number,
					               old_references.unit_code
                                                       ) ;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_unit_grd_schm
      WHERE    unit_grading_schema_id = x_unit_grading_schema_id
      FOR UPDATE NOWAIT;
  ...
  ***************************************************************/
    ...
    CURSOR cur_rowid IS
      SELECT   rowid
      FROM     igs_ps_unit_grd_schm
      WHERE    unit_version_number = x_unit_version_number
      AND      grading_schema_code = x_grading_schema_code
      AND      grd_schm_version_number = x_grd_schm_version_number
      AND      unit_code = x_unit_code 	and      ((l_rowid is null) or (rowid <> l_rowid))
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UGS_UV_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  ***************************************************************/
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_PS_UGS_GS_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
    ...
    IF (p_action = 'INSERT') THEN
      ...
      Null;
	     IF Get_Pk_For_Validation(
    		new_references.unit_grading_schema_id)  THEN
	       Fnd_Message.Set_name('IGS','IGS_GE_RECORD_ALREADY_EXISTS');
      IGS_GE_MSG_STACK.ADD;
	       App_Exception.Raise_Exception;
	     END IF;
...
l_rowid:=NULL;
  ...
  ***************************************************************/
    ...
    cursor C is select ROWID from IGS_PS_UNIT_GRD_SCHM
             where                 UNIT_GRADING_SCHEMA_ID= X_UNIT_GRADING_SCHEMA_ID
;
 ...
 begin
   ...
   select igs_ps_unit_grd_schm_s.nextval
   into x_unit_grading_schema_id
   from dual;
   Before_DML(
 		p_action=>'INSERT',
 		x_rowid=>X_ROWID,
 	       x_unit_grading_schema_id=>X_UNIT_GRADING_SCHEMA_ID,
 	       ...
 	       x_default_flag=>NVL(X_DEFAULT_FLAG,'N' ),
     ...
     insert into IGS_PS_UNIT_GRD_SCHM (
		...
		,LAST_UPDATE_LOGIN
	        ...
	        NEW_REFERENCES.UNIT_GRADING_SCHEMA_ID
	        ,NEW_REFERENCES.UNIT_CODE
	        ,NEW_REFERENCES.UNIT_VERSION_NUMBER
	        ,NEW_REFERENCES.GRADING_SCHEMA_CODE
	        ,NEW_REFERENCES.GRD_SCHM_VERSION_NUMBER
...
if ( (  tlinfo.UNIT_CODE = X_UNIT_CODE)
 ...
 begin
   ...
   Before_DML(
 		...
 		x_rowid=>X_ROWID,
	       ...
	       x_last_updated_by=>X_LAST_UPDATED_BY,
	       x_last_update_login=>X_LAST_UPDATE_LOGIN);
   update IGS_PS_UNIT_GRD_SCHM set
  ...
  ***************************************************************/
    ...
    cursor c1 is select ROWID from IGS_PS_UNIT_GRD_SCHM
             where     UNIT_GRADING_SCHEMA_ID= X_UNIT_GRADING_SCHEMA_ID
;
begin
	open c1;
...
x_rowid => X_ROWID
 ...
 delete from IGS_PS_UNIT_GRD_SCHM
 where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
