--------------------------------------------------------
--  DDL for Package Body IBY_CC_VALIDATE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IBY_CC_VALIDATE" AS
  ...
  BEGIN
	...
	IF v_spread >= 0 THEN
		...
		GetCCType(p_api_version, p_init_msg_list, p_cc_id,
			...
			v_CardType );
		x_cc_valid:=(v_CardType<>c_InvalidCC);
