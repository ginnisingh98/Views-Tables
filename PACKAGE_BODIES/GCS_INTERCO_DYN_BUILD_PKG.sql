--------------------------------------------------------
--  DDL for Package Body GCS_INTERCO_DYN_BUILD_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GCS_INTERCO_DYN_BUILD_PKG" AS
/* $Header: gcsicdbb.pls 120.10 2007/09/21 21:23:01 spala noship $ */
...
--
  -- The API name
  g_api		VARCHAR2(100) := 'gcs.plsql.GCS_INTERCO_DYN_BUILD_PKG';
  ...
  -- Example
  --   GCS_INTERCO_DYN_BUILD_PKG.Create_Package
  -- Notes
  --
  ...
  PROCEDURE Interco_Create_Package(
   ...
   -- control each line to < 80 chars and put in <= 50 lines each time
    ...
    l_api_name VARCHAR2(50) := 'Interco_Create_Package';
  ...
  BEGIN
    ...
    --                  l_api_name || to_char(sysdate, ' DD-MON-YYYY HH:MI:SS'));

    body_block :=
'CREATE OR REPLACE PACKAGE BODY GCS_INTERCO_DYNAMIC_PKG AS
/* $Header $ */
...
';
    ...
    line_num := GCS_DYNAMIC_UTIL_PKG.Build_Comma_List(
		'      giet.', GCS_UTILITY_PKG.g_nl, ' NULL,', line_num);
...
';
    ...
    END LOOP;
			   ...
			   --******--
    line_num := GCS_DYNAMIC_UTIL_PKG.Build_Join_List(
	'    AND   fb.', '  =  giet. ', GCS_UTILITY_PKG.g_nl, line_num);
                            --******--
...
';
    ...
    END LOOP;
			   ...
			   --******--
       line_num := GCS_DYNAMIC_UTIL_PKG.Build_Comma_List(
		'      giet.', GCS_UTILITY_PKG.g_nl, '', line_num);
...
line_num := GCS_DYNAMIC_UTIL_PKG.Build_interco_Comma_List(
                      '   giet.',
		      'DECODE(giet.elim_lob_id, NULL,giet.',
                       'DECODE(p_lob_dim_col_name, ',
                        ' giet.elim_lob_id, giet.',
                       ')),',GCS_UTILITY_PKG.g_nl, ' NULL,', line_num);
...
';
 ...
 ';
    ...
    END LOOP;
--****--------
-- Intracompany Statements follows -----------------------------
...
body_block := '  DECODE(GREATEST(SUM(NVL(ytd_credit_balance_e,0)),
                          SUM(NVL(ytd_debit_balance_e,0))),
                           SUM(NVL(ytd_debit_balance_e,0)), 0,
                             ABS(SUM(NVL(ytd_debit_balance_e,0))-
                                   SUM(NVL(ytd_credit_balance_e,0)))),
...
';
    ...
    WHILE curr_pos <= body_len LOOP
      ...
      line_num := line_num + 1;
    END LOOP;
body_block := ' FROM  GCS_ENTRY_LINES_GT gel,
                GCS_INTERCO_HDR_GT gihg1
                WHERE   gihg1.entry_id = gel.entry_id
...
--*********** very last piece of this function ***********---
...
';
    ...
    END LOOP;
       ...
       line_num := GCS_DYNAMIC_UTIL_PKG.Build_Comma_List(
		'gel.', GCS_UTILITY_PKG.g_nl, '  NULL, ', line_num);
...
body_block := '
  ...
  ';
    ...
    line_num := GCS_DYNAMIC_UTIL_PKG.Build_Join_List(
		'  AND      giet1.', '  =  gel. ', GCS_UTILITY_PKG.g_nl, line_num);
