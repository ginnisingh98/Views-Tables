--------------------------------------------------------
--  DDL for Package Body OTA_MANDATORY_ENROLL_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_MANDATORY_ENROLL_UTIL" as
/* $Header: otmandatoryenr.pkb 120.4 2008/01/25 11:48:49 shwnayak noship $ */


g_package  varchar2(33) := 'ota_mandatory_enroll_util.';  -- Global package name
...
BEGIN
 ...
 FORALL i IN 1 .. l_rec.event_id.COUNT
  ...
  process_mandatory_enr_requests(l_conc_request_id);
...
PROCEDURE process_mandatory_enr_requests(p_conc_request_id IN NUMBER) IS
 ...
 usergroup_whereclause varchar2(4000);
  ...
  l_event_start_date := trunc(ota_learner_access_util.get_event_start_date(request.event_id,sysdate));
    IF request.usergroup_id IS NULL THEN
       FOR learner IN csr_get_learners (request.person_id, request.organization_id, request.job_id, request.position_id, request.org_structure_version_id, request.event_id, request.enr_prereq_type,l_event_start_date) LOOP
          create_request_member_record(learner.person_id,request.mandatory_enr_request_id,request.event_id,request.enr_prereq_type,learner.completed_crs_prereq,learner.completed_comp_prereq,l_numberof_records_processed);
    ...
    OPEN csr_get_lrnr_in_ug FOR sql_stmnt USING request.event_id,request.event_id,l_event_start_date,l_event_start_date,request.event_id;
       LOOP
          ...
          EXIT WHEN csr_get_lrnr_in_ug%NOTFOUND;
            create_request_member_record(lrnr_rec.person_id,request.mandatory_enr_request_id,request.event_id,request.enr_prereq_type,lrnr_rec.completed_crs_prereq,lrnr_rec.completed_comp_prereq,l_numberof_records_processed);
       END LOOP;
