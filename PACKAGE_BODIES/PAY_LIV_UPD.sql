--------------------------------------------------------
--  DDL for Package Body PAY_LIV_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_LIV_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  pay_liv_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  ...
  If (p_datetrack_mode = hr_api.g_correction) then
    ...
    --
    -- Update the pay_link_input_values_f Row
    --
    update  pay_link_input_values_f
    set
     link_input_value_id                  = p_rec.link_input_value_id
    ,element_link_id                      = p_rec.element_link_id
    ,input_value_id                       = p_rec.input_value_id
    ,costed_flag                          = p_rec.costed_flag
    ...
    where   link_input_value_id = p_rec.link_input_value_id
...
Begin
  ...
  If (p_datetrack_mode <> hr_api.g_correction) then
    ...
    --
    pay_liv_shd.upd_effective_end_date
      (p_effective_date         => p_effective_date
      ,p_base_key_value         => p_rec.link_input_value_id
...
Begin
  ...
  begin
    ...
    pay_liv_rku.after_update
      ...
      => pay_liv_shd.g_old_rec.effective_end_date
      ,p_element_link_id_o
      => pay_liv_shd.g_old_rec.element_link_id
      ,p_input_value_id_o
      => pay_liv_shd.g_old_rec.input_value_id
      ,p_costed_flag_o
      => pay_liv_shd.g_old_rec.costed_flag
      ,p_default_value_o
      => pay_liv_shd.g_old_rec.default_value
      ,p_max_value_o
      => pay_liv_shd.g_old_rec.max_value
      ,p_min_value_o
      => pay_liv_shd.g_old_rec.min_value
      ,p_warning_or_error_o
      => pay_liv_shd.g_old_rec.warning_or_error
      ,p_object_version_number_o
      => pay_liv_shd.g_old_rec.object_version_number
      );
    --
  exception
    --
...
Begin
  ...
  If (p_rec.input_value_id = hr_api.g_number) then
    ...
    pay_liv_shd.g_old_rec.input_value_id;
  End If;
  If (p_rec.costed_flag = hr_api.g_varchar2) then
    p_rec.costed_flag :=
    pay_liv_shd.g_old_rec.costed_flag;
  End If;
...
Begin
  ...
  --
  pay_liv_upd.convert_defs(p_rec);
  --
  pay_liv_bus.update_validate
    (p_rec                              => p_rec
    ,p_effective_date                   => p_effective_date
    ,p_datetrack_mode                   => p_datetrack_mode
    ,p_validation_start_date            => l_validation_start_date
...
Begin
  ...
  pay_liv_upd.upd
    ...
    ,p_datetrack_mode
    ,l_rec
    ,p_default_range_warning
    ,p_default_formula_warning
