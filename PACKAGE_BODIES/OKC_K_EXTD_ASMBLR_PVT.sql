--------------------------------------------------------
--  DDL for Package Body OKC_K_EXTD_ASMBLR_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OKC_K_EXTD_ASMBLR_PVT" AS
/* $Header: OKCRKEAB.pls 120.0 2005/05/25 23:11:00 appldev noship $ */
...
l_msg_tbl := okc_aq_pvt.msg_tab_typ();
 ...
 ELSIF corr_rec.element_name = 'K_END_DATE' THEN
   l_msg_tbl.extend;
  l_msg_tbl(i).element_name    := corr_rec.element_name;
  --l_msg_tbl(i).element_value   := P_k_end_date;
  IF corr_rec.format_mask IS NOT NULL THEN
       l_msg_tbl(i).element_value := to_char(p_k_end_date,
		      corr_rec.format_mask);
  ELSE
       l_msg_tbl(i).element_value := to_char(p_k_end_date,'DD-MON-YY');
  END IF;
