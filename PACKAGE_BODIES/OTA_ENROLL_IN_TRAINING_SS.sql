--------------------------------------------------------
--  DDL for Package Body OTA_ENROLL_IN_TRAINING_SS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."OTA_ENROLL_IN_TRAINING_SS" as
/* $Header: otssetrn.pkb 120.3 2008/04/25 13:25:28 smahanka noship $ */

g_package  varchar2(33)	:= ' ota_enroll_in_training_ss.';  -- Global package name
...
--      l_notification_text     := '  The appropriate cost center has been charged.';
   ...
   /*   wf_engine.setItemAttrText (itemtype => p_item_type
      ...
      l_object_version_number := OTA_ENROLL_IN_TRAINING_SS.Get_Booking_OVN (p_booking_id => l_booking_id);
...
BEGIN
  ...
  IF ( csr_booking_status_id%NOTFOUND ) THEN
     ...
     --
     hr_utility.set_message (810,'OTA_13667_WEB_STATUS_NOT_SEEDE');--Changed for 6819558.
...
l_person_details
	OTA_ENROLL_IN_TRAINING_SS.csr_person_to_enroll_details%ROWTYPE;
l_cancel_boolean  boolean;
l_dummy number;
Begin
p_double_book := 'N';
l_person_details := OTA_ENROLL_IN_TRAINING_SS.Get_Person_To_Enroll_Details(p_person_id => p_person_id);
...
--  l_return_value := null;
  ...
  END IF;
  OPEN c_delegate_active;
  FETCH c_delegate_active INTO l_delegate_active;

  IF (c_delegate_active%FOUND) THEN
