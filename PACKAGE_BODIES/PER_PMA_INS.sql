--------------------------------------------------------
--  DDL for Package Body PER_PMA_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_PMA_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_pma_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_appraisal_period_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_appraisal_period_id  in  number) is
--
...
Begin
  ...
  --
  per_pma_ins.g_appraisal_period_id_i := p_appraisal_period_id;
  --
...
Begin
  ...
  --
  --
  -- Insert the row into: per_appraisal_periods
  --
  insert into per_appraisal_periods
      (appraisal_period_id
...
--
  Cursor C_Sel1 is select per_appraisal_periods_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_appraisal_periods
     where appraisal_period_id =
             per_pma_ins.g_appraisal_period_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (per_pma_ins.g_appraisal_period_id_i is not null) Then
    --
    ...
    p_rec.appraisal_period_id :=
      per_pma_ins.g_appraisal_period_id_i;
    per_pma_ins.g_appraisal_period_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.appraisal_period_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  per_pma_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  per_pma_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  per_pma_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  -- Having converted the arguments into the per_pma_rec
  ...
  --
  p_appraisal_period_id := l_rec.appraisal_period_id;
