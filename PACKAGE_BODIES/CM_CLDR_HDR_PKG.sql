--------------------------------------------------------
--  DDL for Package Body CM_CLDR_HDR_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CM_CLDR_HDR_PKG" AS
/* $Header: gmfcldrb.pls 120.2 2005/08/16 13:38:05 sschinch noship $ */
PROCEDURE INSERT_ROW (
  ...
  cursor C is select ROWID from CM_CLDR_HDR_B
    where CALENDAR_CODE = X_CALENDAR_CODE
    ;
begin
  insert into CM_CLDR_HDR_B (
    CALENDAR_CODE,
    START_DATE,
    IN_USE,
    TEXT_CODE,
...
procedure LOCK_ROW (
  ...
  cursor c is select
    ...
    from CM_CLDR_HDR_B
    where CALENDAR_CODE = X_CALENDAR_CODE
    for update of CALENDAR_CODE nowait;
  recinfo c%rowtype;
...
*
*
*  delete from CM_CLDR_HDR_B
*  where CALENDAR_CODE = X_CALENDAR_CODE;
*/

  update CM_CLDR_HDR_B
     set DELETE_MARK = 1
   where CALENDAR_CODE = X_CALENDAR_CODE;
...
is
begin
  delete from CM_CLDR_HDR_TL T
  where not exists
    (select NULL
    from CM_CLDR_HDR_B B
    where B.CALENDAR_CODE = T.CALENDAR_CODE
    );

  update CM_CLDR_HDR_TL T set (
      CALENDAR_DESC
    ) = (select
      B.CALENDAR_DESC
    from CM_CLDR_HDR_TL B
    where B.CALENDAR_CODE = T.CALENDAR_CODE
  ...
  where (
      ...
      SUBT.CALENDAR_CODE,
      SUBT.LANGUAGE
    from CM_CLDR_HDR_TL SUBB, CM_CLDR_HDR_TL SUBT
    where SUBB.CALENDAR_CODE = SUBT.CALENDAR_CODE
  ...
  insert into CM_CLDR_HDR_TL (
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from CM_CLDR_HDR_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
  and B.LANGUAGE = userenv('LANG')
  and not exists
    (select NULL
    from CM_CLDR_HDR_TL T
    where T.CALENDAR_CODE = B.CALENDAR_CODE
    and T.LANGUAGE = L.LANGUAGE_CODE);
end ADD_LANGUAGE;
