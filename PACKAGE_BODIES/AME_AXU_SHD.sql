--------------------------------------------------------
--  DDL for Package Body AME_AXU_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AME_AXU_SHD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ame_axu_shd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
Begin
  --
  If (p_constraint_name = 'AME_ACTION_TYPE_USAGES_PK') Then
...
Begin
  ...
  Else
    If (p_action_type_id =
        ame_axu_shd.g_old_rec.action_type_id and
    ...
    Else
      ...
      --
      Open C_Sel1;
      Fetch C_Sel1 Into ame_axu_shd.g_old_rec;
      ...
      If (p_object_version_number
          <> ame_axu_shd.g_old_rec.object_version_number) Then
...
Begin
 ...
 ,p_rule_type =>  p_rule_type
  ...
  --
  update ame_action_type_usages t
...
Begin
  ...
  if (p_datetrack_mode = hr_api.g_update) then
    ...
    p_validation_end_date   := ame_axu_shd.g_old_rec.end_date;
