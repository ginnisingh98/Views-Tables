--------------------------------------------------------
--  DDL for Package Body BEN_BAI_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_BAI_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_bai_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------------------------------< update_dml >------------------------------|
-- ----------------------------------------------------------------------------
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure update_dml(p_rec in out nocopy ben_bai_shd.g_rec_type) is
--
...
Begin
  ...
  --
  -- Update the ben_batch_actn_item_info Row
  --
  update ben_batch_actn_item_info
  set
  batch_actn_item_id                = p_rec.batch_actn_item_id,
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure pre_update(p_rec in ben_bai_shd.g_rec_type) is
--
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_bai_shd.g_rec_type) is
--
...
Begin
  ...
  If (p_rec.benefit_action_id = hr_api.g_number) then
    ...
    ben_bai_shd.g_old_rec.benefit_action_id;
  End If;
  If (p_rec.person_id = hr_api.g_number) then
    p_rec.person_id :=
    ben_bai_shd.g_old_rec.person_id;
  End If;
  If (p_rec.actn_typ_id = hr_api.g_number) then
    p_rec.actn_typ_id :=
    ben_bai_shd.g_old_rec.actn_typ_id;
  End If;
  If (p_rec.cmpltd_dt = hr_api.g_date) then
    p_rec.cmpltd_dt :=
    ben_bai_shd.g_old_rec.cmpltd_dt;
  End If;
  If (p_rec.due_dt = hr_api.g_date) then
    p_rec.due_dt :=
    ben_bai_shd.g_old_rec.due_dt;
  End If;
  If (p_rec.rqd_flag = hr_api.g_varchar2) then
    p_rec.rqd_flag :=
    ben_bai_shd.g_old_rec.rqd_flag;
  End If;
  If (p_rec.actn_cd = hr_api.g_varchar2) then
    p_rec.actn_cd :=
    ben_bai_shd.g_old_rec.actn_cd;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_bai_shd.g_old_rec.business_group_id;
  End If;
...
Begin
  ...
  --
  convert_defs(p_rec);
  ben_bai_bus.update_validate(p_rec
  ,p_effective_date);
  --
