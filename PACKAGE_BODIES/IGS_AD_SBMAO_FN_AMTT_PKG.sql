--------------------------------------------------------
--  DDL for Package Body IGS_AD_SBMAO_FN_AMTT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."IGS_AD_SBMAO_FN_AMTT_PKG" as
/* $Header: IGSAI65B.pls 115.7 2003/10/30 13:21:47 rghosh ship $ */
  l_rowid VARCHAR2(25);
  old_references IGS_AD_SBMAO_FN_AMTT%RowType;
  new_references IGS_AD_SBMAO_FN_AMTT%RowType;
  ...
  -- Trigger description :-
  -- "OSS_TST".trg_safat_br_iu
  -- BEFORE INSERT OR UPDATE
  -- ON IGS_AD_SBMAO_FN_AMTT
  -- FOR EACH ROW
  ...
  BEGIN
	...
	IF p_inserting OR
		...
		END IF;
	END IF;
	-- Validate Attendance Mode closed ind.
	IF p_inserting OR
	    (old_references.attendance_mode <> new_references.attendance_mode) THEN
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFAT_AM_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFAT_FS_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFAT_GSC_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFAT_ITT_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
  ...
  BEGIN
    ...
    IF (cur_rowid%FOUND) THEN
      Close cur_rowid;
      Fnd_Message.Set_Name ('IGS', 'IGS_AD_SAFAT_OU_FK');
      IGS_GE_MSG_STACK.ADD;
      App_Exception.Raise_Exception;
      Return;
    END IF;
...
procedure INSERT_ROW (
  ...
  X_MODE in VARCHAR2 default 'R'
    ...
    cursor C is select ROWID from IGS_AD_SBMAO_FN_AMTT
      where SUBMISSION_YR = X_SUBMISSION_YR
      and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
      and ORG_UNIT_CD = X_ORG_UNIT_CD
      and FUNDING_SOURCE = X_FUNDING_SOURCE
...
begin
  ...
  insert into IGS_AD_SBMAO_FN_AMTT (
    SUBMISSION_YR,
    SUBMISSION_NUMBER,
    ORG_UNIT_CD,
    OU_START_DT,
...
IGS_GE_MSG_STACK.ADD;
  ...
  if ( (TRUNC(tlinfo.OU_START_DT) = TRUNC(X_OU_START_DT))
...
begin
  ...
  update IGS_AD_SBMAO_FN_AMTT set
...
procedure ADD_ROW (
  ...
  cursor c1 is select rowid from IGS_AD_SBMAO_FN_AMTT
     where SUBMISSION_YR = X_SUBMISSION_YR
     and SUBMISSION_NUMBER = X_SUBMISSION_NUMBER
     and ORG_UNIT_CD = X_ORG_UNIT_CD
     and FUNDING_SOURCE = X_FUNDING_SOURCE
...
begin
  ...
  delete from IGS_AD_SBMAO_FN_AMTT
  where ROWID = X_ROWID;
  if (sql%notfound) then
    raise no_data_found;
  end if;
