--------------------------------------------------------
--  DDL for Package Body PER_KR_GRADES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_KR_GRADES_PKG" AS
/* $Header: pekrsg01.pkb 115.1 2002/12/03 09:34:22 viagarwa noship $ */
-------------------------------------------------------------------------------------
...
,p_creation_date         DATE
  ...
  --
  CURSOR c_s1 IS
    SELECT per_kr_grades_s.NEXTVAL
    FROM   dual;
  --
BEGIN
  --
  OPEN c_s1;
  FETCH c_s1 INTO p_grade_id;
...
BEGIN
  ...
  CLOSE c1;
  --
  IF (     (recinfo.grade_id          = p_grade_id)
     AND   (recinfo.business_group_id = p_business_group_id)
     AND   (recinfo.grade_name        = p_grade_name)
