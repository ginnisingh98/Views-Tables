--------------------------------------------------------
--  DDL for Package Body RRS_HIERARCHIES_EXT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."RRS_HIERARCHIES_EXT_PKG" as
/* $Header: RRSHEXTB.pls 120.0.12010000.1 2009/08/27 18:38:27 sunarang noship $ */
procedure INSERT_ROW (
  ...
  X_SITE_GROUP_VERSION_ID in NUMBER,
  X_HIERARCHY_PURPOSE_CODE in VARCHAR2,
...
l_extension_id NUMBER;
  cursor C is select ROWID from RRS_HIERARCHIES_EXT_B
...
select nvl(X_EXTENSION_ID ,EGO_EXTFWK_S.nextval)
  ...
  insert into RRS_HIERARCHIES_EXT_B (
    EXTENSION_ID,
    SITE_GROUP_VERSION_ID,
    HIERARCHY_PURPOSE_CODE,
    ATTR_GROUP_ID,
    C_EXT_ATTR1,
    C_EXT_ATTR2,
    C_EXT_ATTR3,
    ...
    l_extension_id,
    X_SITE_GROUP_VERSION_ID,
    X_HIERARCHY_PURPOSE_CODE,
    X_ATTR_GROUP_ID,
    X_C_EXT_ATTR1,
    X_C_EXT_ATTR2,
    X_C_EXT_ATTR3,
  ...
  insert into RRS_HIERARCHIES_EXT_TL (
    EXTENSION_ID,
    SITE_GROUP_VERSION_ID ,
    HIERARCHY_PURPOSE_CODE,
    ATTR_GROUP_ID,
...
begin
  ...
  if (    ((recinfo.N_EXT_ATTR17 = X_N_EXT_ATTR17)
      ...
      AND (recinfo.SITE_GROUP_VERSION_ID = X_SITE_GROUP_VERSION_ID)
      AND (recinfo.HIERARCHY_PURPOSE_CODE = X_HIERARCHY_PURPOSE_CODE)
...
begin
  update RRS_HIERARCHIES_EXT_B set
    ...
    SITE_GROUP_VERSION_ID = X_SITE_GROUP_VERSION_ID,
    HIERARCHY_PURPOSE_CODE = X_HIERARCHY_PURPOSE_CODE,
  ...
  update RRS_HIERARCHIES_EXT_TL set
...
begin
  delete from RRS_HIERARCHIES_EXT_TL
  where EXTENSION_ID = X_EXTENSION_ID;
  ...
  delete from RRS_HIERARCHIES_EXT_B
  where EXTENSION_ID = X_EXTENSION_ID;
...
is
begin
  delete from RRS_HIERARCHIES_EXT_TL T
  where not exists
    (select NULL
    from RRS_HIERARCHIES_EXT_B B
    where B.EXTENSION_ID = T.EXTENSION_ID
    );

  update RRS_HIERARCHIES_EXT_TL T set (
      TL_EXT_ATTR1,
      TL_EXT_ATTR2,
      TL_EXT_ATTR3,
      TL_EXT_ATTR4,
  ...
  where (
      ...
      SUBT.EXTENSION_ID,
      SUBT.LANGUAGE
    from RRS_HIERARCHIES_EXT_TL SUBB, RRS_HIERARCHIES_EXT_TL SUBT
  ...
  insert into RRS_HIERARCHIES_EXT_TL (
    ...
    B.EXTENSION_ID,
    B.SITE_GROUP_VERSION_ID,
    B.HIERARCHY_PURPOSE_CODE,
    B.ATTR_GROUP_ID,
    B.LAST_UPDATE_DATE,
    B.LAST_UPDATED_BY,
    B.LAST_UPDATE_LOGIN,
    ...
    L.LANGUAGE_CODE,
    B.SOURCE_LANG
  from RRS_HIERARCHIES_EXT_TL B, FND_LANGUAGES L
  where L.INSTALLED_FLAG in ('I', 'B')
