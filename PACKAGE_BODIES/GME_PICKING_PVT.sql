--------------------------------------------------------
--  DDL for Package Body GME_PICKING_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_PICKING_PVT" AS
/* $Header: GMEVPCKB.pls 120.7.12010000.3 2009/05/18 06:35:00 apmishra ship $ */
...
/*************************************************************************************************/
   ...
   BEGIN
      ...
      FOR i IN 1 .. l_count LOOP
         l_mtl_dtl_tbl (i).inventory_item_id :=
                                          p_mtl_req_tbl (i).inventory_item_id;
         l_mtl_dtl_tbl (i).organization_id :=
                                            p_mtl_req_tbl (i).organization_id;
         l_mtl_dtl_tbl (i).plan_qty := p_mtl_req_tbl (i).open_qty;
         l_mtl_dtl_tbl (i).dtl_um := p_mtl_req_tbl (i).dtl_um;
         l_mtl_dtl_tbl (i).revision := p_mtl_req_tbl (i).revision;
         l_mtl_dtl_tbl (i).batch_id := p_mtl_req_tbl (i).batch_id;
         l_mtl_dtl_tbl (i).material_detail_id :=
                                         p_mtl_req_tbl (i).material_detail_id;
         l_mtl_dtl_tbl (i).material_requirement_date :=
                                               p_mtl_req_tbl (i).mtl_req_date;
         l_mtl_dtl_tbl (i).subinventory := p_mtl_req_tbl (i).subinventory;
         l_mtl_dtl_tbl (i).locator_id := p_mtl_req_tbl (i).locator_id;
         l_mtl_dtl_tbl (i).line_type := gme_common_pvt.g_line_type_ing;
      END LOOP;

      /* Try to allocate any other move order lines that exist and are not allocated
         These could be move orders created during batch create */
      FOR i IN 1 .. l_count LOOP
   ...
   BEGIN
      ...
      FOR i IN 1 .. l_resv_tbl.COUNT LOOP
         IF (l_resv_tbl (i).subinventory_code <>
                                            p_mo_line_rec.to_subinventory_code) THEN
            l_count := l_count + 1;
