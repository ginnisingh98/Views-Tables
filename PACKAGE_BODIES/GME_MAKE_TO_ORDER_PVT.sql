--------------------------------------------------------
--  DDL for Package Body GME_MAKE_TO_ORDER_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_MAKE_TO_ORDER_PVT" AS
/*  $Header: GMECRBTB.pls 120.5.12010000.3 2010/04/01 20:10:53 gmurator ship $
 +=========================================================================+
...
BEGIN
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.log_initialize ('OPM_MaketoOrder');
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' ENTER API  ');
--   gme_debug.log_initialize ('OPM_MaketoOrder');
   END IF;
   ...
   THEN
      IF (g_debug <= gme_debug.g_log_statement) THEN
         gme_debug.put_line(g_pkg_name||'.'||l_api_name||' compatability call failure ');
      END IF;
      RAISE FND_API.G_EXC_UNEXPECTED_ERROR;
   END IF;

   FND_FILE.Put_Line(FND_FILE.LOG, ' compatible OK          ');
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' begin processing ');
   END IF;

   IF( NVL(p_so_line_id,0) = 0 ) THEN
      IF (g_debug <= gme_debug.g_log_statement) THEN
         gme_debug.put_line(g_pkg_name||'.'||l_api_name||' sales order line id parameter is missing ');
      END IF;
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Check for existing reservations against GME PROD supply ');
   END IF;
   ...
   IF (cur_prod_reservations%NOTFOUND) or (l_count=0) THEN
     IF g_debug <= gme_debug.g_log_procedure THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name || ' No existing reservations against PRODUCTION supply');
     END IF;
     CLOSE cur_prod_reservations;
   ELSE
     IF g_debug <= gme_debug.g_log_procedure THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name || 'Reservations already against PRODUCTION supply so terminate here');
     END IF;
     CLOSE cur_prod_reservations;
     RETURN;
   END IF;
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||'Retrieve the sales order line for line_id '||p_so_line_id);
   END IF;
   ...
   IF(get_so_line_rec%NOTFOUND) THEN
      ...
      IF (g_debug <= gme_debug.g_log_statement) THEN
        gme_debug.put_line(g_pkg_name||'.'||l_api_name||'RETURNING - sales order not found for for line_id '||p_so_line_id);
      END IF;
      RAISE FND_API.G_EXC_ERROR;
   END IF;
   CLOSE get_so_line_rec;
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Retrieve Item for organization_id  '||l_so_line_rec.ship_from_org_id);
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Retrieve Item for inventory_item_id '||l_so_line_rec.inventory_item_id);
   END IF;
   ...
   IF(cur_item%NOTFOUND) THEN
      CLOSE cur_item;
      IF (g_debug <= gme_debug.g_log_statement) THEN
        gme_debug.put_line(g_pkg_name||'.'||l_api_name||'Failure to retrieve inventory item ');
      END IF;
      RAISE FND_API.G_EXC_ERROR;
   END IF;
   CLOSE cur_item;
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Determine whether make to order rule exists ');
   END IF;
   ...
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Return status from Retrieve_Rule is '||l_return_status);
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Rule ID to work with is '||l_mto_assignments_rec.rule_id);
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Rule being honored is '||l_mto_rules_rec.rule_name);
   END IF;

   IF( l_return_status <> FND_API.G_RET_STS_SUCCESS ) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Failure to Get_Rule so terminate processing for line '||p_so_line_id);
      FND_MESSAGE.Set_Name('GMI','Missing');
   ...
   IF (NVL(l_mto_rules_rec.rule_id,0) = 0) THEN
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||'No MAKE to ORDER rule exists to cover order line '||p_so_line_id);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||'No Further Processing Required for order line '||p_so_line_id);
     RETURN;
   END IF;

   /* ============== Start Processing Make to Order Rule ===============*/
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Start processing rule definition for '||l_mto_rules_rec.rule_name);
   END IF;

   /* ================== Assess the rule for checking available inventory =================*/
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Check Availability set to  '||l_mto_rules_rec.check_availability);
   END IF;
   IF NVL(l_mto_rules_rec.check_availability,'N') = 'N' THEN
     IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' No need for Availabilty check create for full order qty');
     END IF;
     ...
     IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Batch quantity needed is '||l_batch_qty);
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Secondary qty  needed is '||l_batch_qty2);
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Retrieve any existing inv reservations ');
     END IF;
     ...
     l_count := 1;
     IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Loop thru existing reservations for'||l_rsv_tbl.COUNT);
     END IF;
     WHILE l_count <= l_rsv_tbl.COUNT LOOP
       l_rsv_row_rec := l_rsv_tbl(l_count);
       /* Relieve any reservations against INV */
       IF (g_debug <= gme_debug.g_log_statement) THEN
         gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Relieve Reservation here for ID '||l_rsv_row_rec.reservation_id);
       END IF;
  ...
  IF NVL(l_mto_rules_rec.check_availability,'N') = 'Y' THEN
     IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' NEED to check Inventory Availabilty ahead of creating batch');
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' start point is the order qty ' ||l_so_line_rec.ordered_quantity);
     END IF;
...
inv_convert.inv_um_convert(l_so_line_rec.inventory_item_id,
   ...
   exception when no_data_found then
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Deduct qty already reserved, this is '||l_total_primary_rsv_qty);
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Now batch qty required stands at '||l_batch_qty);
    END IF;
    IF (l_batch_qty <=  0) THEN
       IF (g_debug <= gme_debug.g_log_statement) THEN
         gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Full Order Qty already reserved to INV');
         gme_debug.put_line(g_pkg_name||'.'||l_api_name||' No need for batch creation Return here');
       END IF;
       RETURN;
    END IF;
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Need to assess ATR level so invoke create tree');
    END IF;
    ...
    inv_quantity_tree_pvt.create_tree
	 ...
	 , p_demand_source_type_id      => inv_reservation_global.g_source_type_oe
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' After create tree status returned is '||l_return_status);
    END IF;
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Query tree for ATR next ');
    END IF;
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' After Query tree return status is '||l_return_status);
    END IF;
    IF l_return_status = fnd_api.g_ret_sts_error THEN
       RAISE fnd_api.g_exc_error;
    END IF;
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
       ...
       gme_debug.put_line('l_rqoh ' || l_rqoh);
       gme_debug.put_line('l_qr ' || l_qr);
       gme_debug.put_line('l_qs ' || l_qs);
       gme_debug.put_line('l_satr ' || l_satr);
       gme_debug.put_line('l_satt ' || l_satt);
       gme_debug.put_line('l_sqoh ' || l_sqoh);
       gme_debug.put_line('l_srqoh ' || l_srqoh);
       gme_debug.put_line('l_sqr ' || l_sqr);
       gme_debug.put_line('l_sqs ' || l_sqs);
    END IF;
    /* Deduct any ATR from the manufacture qty */
    /* Convert atr quantity into Orderline UOM  */
    IF l_so_line_rec.order_quantity_uom <> l_primary_uom_code THEN
      l_atr_qty_in_so_um := inv_convert.inv_um_convert(l_so_line_rec.inventory_item_id,
	                              null,
                                    l_so_line_rec.ship_from_org_id,
	                              5,
	                              l_atr,
	                              ...
	                              null,
                                    null);
      l_atr := l_atr_qty_in_so_um;
    END IF;
    IF l_atr >= l_batch_qty THEN
      /* Stock is available to reserve to meet the sales quantity so no further
 * action required */
      gme_debug.put_line('Stock is available to meet the demand so batch creation not necessary ');
    END IF;
  ...
  IF (g_debug <= gme_debug.g_log_statement) THEN
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' End of availability checking and batch qty now '||l_batch_qty);
  END IF;
  /* Now set up data and invoke create_batch
  ==========================================*/
  IF (l_batch_qty <=  0) THEN
    IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Entire requested qty aviaable in inventory so Return now');
    END IF;
    RETURN;
  END IF;
  ...
  gme_debug.put_line('populate the batch record in readiness for create_batch API');
  ...
  IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line ('before apps init FND_GLOBAL.USER_ID '||FND_GLOBAL.USER_ID);
     gme_debug.put_line ('before apps init FND_GLOBAL.RESP_ID '||FND_GLOBAL.RESP_ID);
     gme_debug.put_line ('before apps init FND_GLOBAL.RESP_APPL_ID '||FND_GLOBAL.RESP_APPL_ID);
  END IF;
  ...
  IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line ('after apps init FND_GLOBAL.USER_ID '||FND_GLOBAL.USER_ID);
     gme_debug.put_line ('after apps init FND_GLOBAL.RESP_ID '||FND_GLOBAL.RESP_ID);
     gme_debug.put_line ('after apps init FND_GLOBAL.RESP_APPL_ID '||FND_GLOBAL.RESP_APPL_ID);
  END IF;

  IF l_mto_rules_rec.batch_creation_user is not NULL THEN
    IF (l_mto_rules_rec.batch_creation_user <> l_user_id) THEN
      IF (g_debug <= gme_debug.g_log_statement) THEN
        gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Batch creation user is '||l_mto_rules_rec.batch_creation_user);
      END IF;
      -- Bug 9355688 - Initialize with proper values not hard coded -1.
  ...
  gme_debug.put_line('Calling Create Batch API');
  FND_FILE.Put_Line(FND_FILE.LOG, 'call Create Batch API  ');
  FND_FILE.Put_Line(FND_FILE.LOG, 'Batch qty needed is '||l_batch_qty);
  IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Invoke Create_Batch API here');
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Batch qty required is '||l_batch_qty);
     gme_debug.put_line ('before create batch FND_GLOBAL.USER_ID '||FND_GLOBAL.USER_ID);
     gme_debug.put_line ('before create batch FND_GLOBAL.RESP_ID '||FND_GLOBAL.RESP_ID);
     gme_debug.put_line ('before create batch FND_GLOBAL.RESP_APPL_ID '||FND_GLOBAL.RESP_APPL_ID);
  END IF;
  ...
  IF (g_debug <= gme_debug.g_log_statement) THEN
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Return Status from Create_Batch API is '||l_return_status);
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Message       from Create_Batch API is '||l_msg_data     );
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Batch No      from Create_Batch API is '||
                       l_out_batch_header_rec.batch_no);
  END IF;
 ...
 * in the batch */
  ...
  ===================================================================================*/
  IF (g_debug <= gme_debug.g_log_statement) THEN
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Retrieve product line - must reserve to this supply');
  END IF;
  ...
  IF(get_batch_line%NOTFOUND) THEN
     ...
     IF (g_debug <= gme_debug.g_log_statement) THEN
       gme_debug.put_line('get_batch_line%NOTFOUND, returning from create_batch_for_order_line');
     END IF;
     RAISE fnd_api.g_exc_error;
  END IF;
  CLOSE get_batch_line;
  ...
  ===================================== */
  IF (g_debug <= gme_debug.g_log_statement) THEN
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' analyse copy attachments '||l_mto_rules_rec.copy_attachments);
  END IF;
  IF (NVL(l_mto_rules_rec.copy_attachments,'N') = 'Y') THEN
    IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Invoke copy attachments ');
    END IF;
    ...
    IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Return Status from copy attachments '||l_return_status);
    END IF;
  END IF;
  ...
  =======================================================================================================*/
  IF (g_debug <= gme_debug.g_log_statement) THEN
    gme_debug.put_line(g_pkg_name||'.'||l_api_name||' analyse auto lot generation'||l_mto_rules_rec.auto_lot_generation);
  END IF;
  ...
  IF l_item_rec.lot_control_code > 1 AND (l_mto_rules_rec.auto_lot_generation = 'Y') THEN
    ...
    IF (l_return_status <> FND_API.G_RET_STS_SUCCESS) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' FAILURE to generate pending lot ');
    END IF;
  END IF;  -- End of pending lot generation

  gme_debug.put_line('Reserve the batch Quantity to this order line');
  ...
  IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Invoke INV_RESERVATION_PUB.Create_Reservation ');
  END IF;
  ...
  ===============================================================*/
  IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Notify_CSR for batch_id'||l_rsv_rec.supply_source_header_id);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Notify_CSR for batch_line_id'||l_rsv_rec.supply_source_line_id);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Notify_CSR for sales_line_id'||l_rsv_rec.demand_source_line_id);
  END IF;
--FND_GLOBAL.apps_initialize(l_user_id,-1,-1);
  ...
  IF g_debug <= gme_debug.g_log_procedure THEN
              gme_debug.put_line  (  g_pkg_name || '.'
                                 || l_api_name
                                 || ' after calling notify_CSR  for create reservation status is '
                                 || l_return_status );
  END IF;

  IF l_return_status IN  (fnd_api.g_ret_sts_error, fnd_api.g_ret_sts_unexp_error) THEN
    IF g_debug <= gme_debug.g_log_procedure THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Exit after Notifications Failure   ');
    END IF;
    RAISE FND_API.G_EXC_ERROR;
  END IF;
...
EXCEPTION
    ...
    gme_debug.put_line('sqlcode : ' ||to_char(sqlcode));
    gme_debug.put_line('sqlerr : '|| SUBSTRB(SQLERRM, 1, 150));
...
BEGIN
     gme_debug.put_line('Entering copy_attachemnts routine..');
     ...
     gme_debug.put_line('Exiting copy_attachments procedure without any errors');
EXCEPTION
 WHEN OTHERS THEN
    GMI_RESERVATION_UTIL.Println('In others exception in copy_attachments');
    x_return_status := FND_API.G_RET_STS_ERROR;
...
BEGIN
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' ENTER FUNCTION '||l_api_name||' for line '||p_line_id);
      gme_debug.put_line ( '*** function call to check if order line qualifies for OPM make to order ***' );
   END IF;
   ...
   IF(get_order_line_rec%NOTFOUND) THEN
      ...
      IF (g_debug <= gme_debug.g_log_statement) THEN
        gme_debug.put_line('get_order_line_rec%NOTFOUND, RETURN now');
      END IF;
      Return FALSE;
   END IF;
   CLOSE get_order_line_rec;
   ...
   /* ============== Establish Item Characteristics ====================*/
   IF (g_debug <= gme_debug.g_log_statement) THEN
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Retrieve Item for organization_id  '||l_order_line_rec.ship_from_org_id);
      gme_debug.put_line(g_pkg_name||'.'||l_api_name||' Retrieve Item for inventory_item_id '||l_order_line_rec.inventory_item_id);
   END IF;
   ...
   IF(cur_item%NOTFOUND) THEN
      CLOSE cur_item;
      IF (g_debug <= gme_debug.g_log_statement) THEN
         gme_debug.put_line('Failure to retrieve item; terminate processing');
      END IF;
      Return FALSE;
   END IF;
   CLOSE cur_item;
...
END line_qualifies_for_MTO;
 ...
 BEGIN
  IF (g_debug <= gme_debug.g_log_statement) THEN
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' ENTER API  ');
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' organization_id  '||p_mto_assignments_rec.organization_id);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' inventory_item_id '||p_mto_assignments_rec.inventory_item_id);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' item_type         '||p_mto_assignments_rec.item_type);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' customer_id       '||p_mto_assignments_rec.customer_id);
     gme_debug.put_line(g_pkg_name||'.'||l_api_name||' site_use_id       '||p_mto_assignments_rec.site_use_id);
  END IF;
  x_return_status := FND_API.G_RET_STS_SUCCESS;
  ...
  IF(get_assignment%NOTFOUND) THEN
    ...
    gme_debug.put_line('No rule assigment to match input criteria; returning from Retrieve Rule');
    RETURN;
  END IF;
  Close get_assignment;
  ...
  Fetch get_rule into l_mto_rules_rec;
   IF(get_rule%NOTFOUND) THEN
      CLOSE get_rule;
      gme_debug.put_line('Error encountered on rule retrieval for Rule ID '||l_mto_assignments_rec.rule_id);
      RAISE FND_API.G_EXC_ERROR;
   END IF;
  Close get_rule;
