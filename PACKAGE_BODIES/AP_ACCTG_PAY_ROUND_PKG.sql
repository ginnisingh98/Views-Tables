--------------------------------------------------------
--  DDL for Package Body AP_ACCTG_PAY_ROUND_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."AP_ACCTG_PAY_ROUND_PKG" AS
/* $Header: apacrndb.pls 120.22.12010000.24 2010/04/16 10:27:01 subehera ship $ */
...
BEGIN
  ...
  IF (p_inv_rec.invoice_currency_code <> ap_accounting_pay_pkg.g_base_currency_code) OR
     ...
     /* -- Bug 6600341
      -- Final Payment Rounding should not be done if there are XLA Events
      -- still pending processing
          ...
          AND    NVL(accrual_posted_flag, 'N') = 'N';*/ --bug 7614480, commented the above snippet
   ...
   3. if p_xla_event_rec.event_id IS NOT NULL then get the maximum accounting_event_id
      from amoung the related payment, payment adjusted and the prepay adjustment events for rounding
    */
  SELECT DECODE(COUNT(*), 0, 'Y', 'N')
  INTO exec_final_payment
  FROM DUAL
  WHERE EXISTS(
          ...
          UNION
          -- prepay application
          SELECT 'unreversed, unvalidated prepayment application not selected for validation'
            FROM ap_invoice_distributions_all aid
...
--bug 7614480   end
          ...
          IF (G_LEVEL_PROCEDURE >= G_CURRENT_RUNTIME_LEVEL ) THEN
              l_log_msg := 'Fetched Acctg event id: '|| l_acctg_event_id
              || ' , current XLA Event ID is: ' || p_xla_event_rec.event_id
              || ' , Invoice ID: ' || p_inv_rec.invoice_id;
  ...
  SELECT SUM(amount), count(1)
   ...
   WHERE aid.accounting_event_id = p_acct_event_id
     ...
     AND evnt.event_type_code IN ('INVOICE ADJUSTED',
				  ...
				  'DEBIT MEMO ADJUSTED') -- added for bug#9545528
  ...
  LOOP
   ...
   IF b_generate_pay_dist then
      ...
      AND     NOT EXISTS(SELECT 'reversed'
			   FROM Ap_Invoice_Distributions_All AID
                          WHERE AID.accounting_event_id = NVL(APH.related_prepay_app_event_id,
      ...
      IF (  l_inv_dist_rec.line_type_lookup_code in ('ITEM', 'NONREC_TAX','ACCRUAL', 'TRV') --bug9398335
...
PROCEDURE Total_Pay
  ...
  l_err_clr_rate_total_amt       NUMBER := 0;
  l_chrg_clr_rate_total_amt      NUMBER := 0;
  ...
  l_err_clr_rate_sum_amt         NUMBER := 0;
  l_chrg_clr_rate_sum_amt        NUMBER := 0;
  ...
  l_err_clr_rate_diff_amt        NUMBER := 0;
  l_chrg_clr_rate_diff_amt       NUMBER := 0;
  ...
  l_pre_err_clr_rounding_amt  NUMBER :=0;
  l_pre_chrg_clr_rounding_amt NUMBER :=0;
...
--Bug 7270829
 ...
 If there is a difference between the total and sum amounts then we will insert
  ...
  l_err_clr_rate_diff_amt := l_err_clr_rate_total_amt - l_err_clr_rate_sum_amt;
  l_chrg_clr_rate_diff_amt := l_chrg_clr_rate_total_amt - l_chrg_clr_rate_sum_amt;
...
until the complete rounding amount is consumed*/
  ...
  l_err_clr_rounding_amt  := NVL(l_err_clr_rate_diff_amt, 0);
  l_chrg_clr_rounding_amt := NVL(l_chrg_clr_rate_diff_amt, 0);
  ...
  WHILE (l_clr_rate_rounding_amt <> 0 or l_mat_rate_rounding_amt <> 0 or
        ...
        l_err_clr_rounding_amt <> 0 or l_chrg_clr_rounding_amt <> 0 or
   ...
   IF NVL(l_inv_dist_rounding_amt, 0) <> 0 THEN --8449083
    ...
    IF NVL(l_disc_pay_rounding_amt, 0) <> 0 THEN --8449083
      ...
      IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
        ...
        FND_LOG.STRING(G_LEVEL_STATEMENT,
                       ...
                       l_log_msg);
      END IF;
      IF (sign(l_disc_pay_rounding_amt + l_inv_paid_base_amt) <>
         sign(l_inv_dist_amt) and
         (l_disc_pay_rounding_amt + l_inv_paid_base_amt) <> 0) then
        -- bug 8403738
        l_pre_disc_pay_rounding_amt := l_disc_pay_rounding_amt;
        l_disc_pay_rounding_amt := l_inv_paid_base_amt +
    ...
    IF NVL(l_disc_clr_rounding_amt, 0) <> 0 THEN --8449083
      ...
      IF (G_LEVEL_STATEMENT >= G_CURRENT_RUNTIME_LEVEL) THEN
        ...
        FND_LOG.STRING(G_LEVEL_STATEMENT,
                       ...
                       l_log_msg);
      END IF;
      IF (sign(l_disc_clr_rounding_amt + l_inv_clr_base_amt) <>
         sign(l_inv_dist_amt) and
         (l_disc_clr_rounding_amt + l_inv_clr_base_amt) <> 0) then
        -- bug 8403738
        l_pre_disc_clr_rounding_amt := l_disc_clr_rounding_amt;
        l_disc_clr_rounding_amt := l_inv_clr_base_amt +
    ...
    IF ( l_pre_inv_dist_rounding_amt <= l_inv_dist_rounding_amt OR
         ...
         l_pre_err_clr_rounding_amt <= l_err_clr_rounding_amt OR
         ...
         sign(l_pre_disc_clr_rounding_amt) <> sign(l_disc_clr_rounding_amt) OR
         sign(l_pre_err_clr_rounding_amt) <> sign(l_err_clr_rounding_amt) OR
         sign(l_pre_chrg_clr_rounding_amt) <> sign(l_chrg_clr_rounding_amt) ) THEN
...
BEGIN
  ...
  Begin
     ...
     SELECT nvl(ac.base_amount,   0),
      ...
      GROUP BY ac.check_id, ac.amount, ac.base_amount , ac.future_pay_due_date, aps.recon_accounting_flag
     ...
     IF ( l_sum_cash_amt <> l_ac_base_amount AND l_sum_cash_amt <> 0) Then
	   ...
	   INTO l_pay_hist_rec.Payment_History_ID,
		...
		l_pay_hist_rec.Bank_To_Base_XRate
	     ...
	     IF ( l_transaction_type = 'CLEARING' AND 1=2) Then
	     ...
	     ELSIF ( l_transaction_type = 'MATURITY' AND 1=2) Then
