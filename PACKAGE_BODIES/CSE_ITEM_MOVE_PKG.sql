--------------------------------------------------------
--  DDL for Package Body CSE_ITEM_MOVE_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."CSE_ITEM_MOVE_PKG" AS
-- $Header: CSEITMVB.pls 120.0.12010000.2 2008/09/20 00:21:30 fli ship $
...
PROCEDURE Decode_Message(
   ...
   l_Api_Name            CONSTANT  VARCHAR2(30) := 'CSE_ITEM_MOVE_PKG';
   l_Item_Id                NUMBER;
   l_Revision               VARCHAR2(3);
   l_Lot_Number             VARCHAR2(30);
   l_Serial_Number          VARCHAR2(30);
   l_Quantity               NUMBER;
   l_From_Netwk_Locn_id     NUMBER;
   l_To_Netwk_Locn_id       NUMBER;
...
BEGIN
   ...
   Xnp_Xml_Utils.Decode(P_Msg_Text,'FROM_NETWORK_LOC_ID',l_From_Netwk_Locn_Id);
   Xnp_Xml_Utils.Decode(P_Msg_Text,'TO_NETWORK_LOC_ID',l_To_Netwk_Locn_Id);
   Xnp_Xml_Utils.Decode(P_Msg_Text,'FROM_PARTY_SITE_ID',l_From_Party_Site_Id);
   Xnp_Xml_Utils.Decode(P_Msg_Text,'TO_PARTY_SITE_ID', l_To_Party_Site_Id);
   ...
   X_Item_Move_Attr_Rec.Item_Id                    := l_Item_Id;
   X_Item_Move_Attr_Rec.Revision                   := l_Revision;
   X_Item_Move_Attr_Rec.Lot_Number                 := l_Lot_Number;
   X_Item_Move_Attr_Rec.Serial_Number              := l_Serial_Number;
   X_Item_Move_Attr_Rec.Quantity                   := l_Quantity;
   X_Item_Move_Attr_Rec.From_Network_Location_Id   := l_From_Netwk_Locn_Id;
   X_Item_Move_Attr_Rec.To_Network_Location_Id     := l_To_Netwk_Locn_Id;
   X_Item_Move_Attr_Rec.From_party_site_Id         := l_From_Party_Site_Id;
   X_Item_Move_Attr_Rec.To_party_site_Id           := l_To_Party_Site_Id;
   X_Item_Move_Attr_Rec.Work_Order_Number          := l_Work_Order_Number;
   X_Item_Move_Attr_Rec.Transaction_Date           := TO_DATE(l_Transaction_Date_Str, 'YYYY/MM/DD HH24:MI:SS');
   X_Item_Move_Attr_Rec.Transacted_By              := l_Transacted_By;
   X_Item_Move_Attr_Rec.Message_id                 := P_Msg_Header.Message_Id;
...
PROCEDURE Update_Ib_Repository(
   ...
   l_Resolve_Id_Column             VARCHAR2(1)   DEFAULT    FND_API.G_FALSE;
   ...
   l_From_Hz_Location_Id           NUMBER;
   l_To_Hz_Location_Id             NUMBER;
   l_from_Location_Type_Code       VARCHAR2(30) ;
   l_to_Location_Type_Code         VARCHAR2(30) ;
...
END IF;
 ...
 IF (l_debug = 'Y') THEN
    CSE_DEBUG_PUB.ADD('****  ITEM MOVED TRANSACTION  ******');
    CSE_DEBUG_PUB.ADD('-----------------------------------------------');
 END IF;
   ...
   l_Transaction_Type_Id   := CSE_UTIL_PKG.Get_Txn_Type_ID
                              ('ITEM_MOVE',
                               l_App_Short_Name);
-- populate party_site_id and network_location_id
   If (P_Item_Move_Attr_Rec.From_Network_Location_id is not null) then
      l_From_hz_location_Id   :=P_Item_Move_Attr_Rec.From_Network_Location_id ;
      l_from_location_type_code:='HZ_LOCATIONS';
   elsif (P_Item_Move_Attr_Rec.From_party_site_id is not null) then
      l_from_hz_location_Id    :=P_Item_Move_Attr_Rec.from_party_site_id ;
      l_from_location_type_code:='HZ_PARTY_SITES';
   end if;

   If (P_Item_Move_Attr_Rec.To_Network_Location_id is not null) then
      l_To_hz_location_Id   :=P_Item_Move_Attr_Rec.To_Network_Location_id ;
      l_to_location_type_code  :='HZ_LOCATIONS';
   elsif (P_Item_Move_Attr_Rec.To_party_site_id is not null) then
      l_To_hz_location_Id    :=P_Item_Move_Attr_Rec.To_party_site_id ;
      l_to_location_type_code  :='HZ_PARTY_SITES';
   end if;

  IF (l_debug = 'Y') THEN
     CSE_DEBUG_PUB.ADD('  from location ' ||l_from_location_type_code||'-'||l_From_hz_location_Id);
      CSE_DEBUG_PUB.ADD('  to location ' ||l_to_location_type_code||'-'||l_to_hz_location_Id);
  END IF;
   ...
   l_Instance_Query_Rec.Inventory_Item_Id := P_Item_Move_Attr_Rec.Item_Id;
   l_Instance_Query_Rec.Inventory_Revision:= P_Item_Move_Attr_Rec.Revision;
   l_Instance_Query_Rec.Lot_Number        := P_Item_Move_Attr_Rec.Lot_Number;
   l_Instance_Query_Rec.Serial_Number     := P_Item_Move_Attr_Rec.Serial_Number;
   l_Instance_Query_Rec.Location_Id       := l_From_Hz_Location_Id;
   l_Instance_Query_Rec.Location_Type_Code:= l_from_Location_Type_Code;
   l_Instance_Query_Rec.Instance_Usage_Code:=CSE_DATASTRUCTURES_PUB.G_OUT_OF_SERVICE;
   l_Active_Instance_Only := FND_API.G_TRUE; --Added for bug 7423694, base bug 7233717
  ...
  IF(l_Instance_Header_Tbl_Out.COUNT=0) THEN
      ...
      fnd_message.set_token('ITEM',P_Item_Move_Attr_Rec.Item_Id);
   ...
   IF(P_Item_Move_Attr_Rec.Quantity >
   ...
   --Step 4,If the Item is serialized,
    ...
    IF(P_Item_Move_Attr_Rec.Serial_Number IS NOT NULL) THEN
       ...
       l_Instance_Rec.Location_Id       :=l_To_Hz_Location_Id;
       l_Instance_Rec.Location_Type_Code:=l_to_Location_Type_Code;
       ...
       l_Txn_Rec.Source_Transaction_Date:=P_Item_Move_Attr_Rec.Transaction_Date;
       l_Txn_Rec.Transaction_Type_Id    := l_transaction_type_id;
       l_Txn_Rec.Source_Group_Ref := P_Item_Move_Attr_Rec.Work_Order_Number;
       l_Txn_Rec.Transaction_Quantity   := P_Item_Move_Attr_Rec.Quantity;
       l_Txn_Rec.Transacted_By          := P_Item_Move_Attr_Rec.Transacted_By;
       l_Txn_Rec.Transaction_Status_Code:= CSE_DATASTRUCTURES_PUB.G_PENDING;
       l_Txn_Rec.Message_Id             := P_Item_Move_Attr_Rec.Message_Id;
...
-- Step 5  ,if the item is non serialized
 ...
 IF(P_Item_Move_Attr_Rec.Serial_Number IS  NULL) THEN
...
--Step 5.1 ,Check for the existence of the destination record
   ...
   l_Instance_Query_Rec.Lot_Number        := P_Item_Move_Attr_Rec.Lot_Number;
   l_Instance_Query_Rec.Serial_Number     := P_Item_Move_Attr_Rec.Serial_Number;
   l_Instance_Query_Rec.Location_Id       := l_To_Hz_Location_Id;
   l_Instance_Query_Rec.Location_Type_Code:= l_to_Location_Type_Code;
   l_Instance_Query_Rec.Instance_Usage_Code:=CSE_DATASTRUCTURES_PUB.G_OUT_OF_SERVICE;
   l_Active_Instance_Only                 := FND_API.G_FALSE;
...
-- If destination item instance is found
  ...
  l_D_object_version_number:=l_Dest_Instance_Rec.object_version_number;
   ...
   END IF;
    ...
    l_Instance_Rec.Quantity  := (P_Item_Move_Attr_Rec.Quantity +
...
END IF;
       ...
       l_Instance_Rec.Inventory_Item_id := P_Item_Move_Attr_Rec.Item_id;
       l_Instance_Rec.Inventory_Revision:= P_Item_Move_Attr_Rec.Revision;
       ...
       l_Instance_Rec.Serial_Number := P_Item_Move_Attr_Rec.Serial_Number;
       ...
       l_Instance_Rec.Lot_Number := P_Item_Move_Attr_Rec.Lot_Number;
       l_Instance_Rec.Quantity := P_Item_Move_Attr_Rec.Quantity;
...
PROCEDURE Update_eib_instances(
   ...
   X_Error_Message          OUT NOCOPY VARCHAR2) IS
BEGIN
 IF NOT P_Item_Move_Attr_tbl.COUNT = 0 THEN
FOR i IN  P_Item_Move_Attr_tbl.FIRST ..  P_Item_Move_Attr_tbl.LAST LOOP
 IF P_Item_Move_Attr_tbl.EXISTS(i) THEN
    update_ib_repository( p_item_move_attr_tbl(i),
                          x_return_status,
                          x_error_message);
 END IF;
END LOOP;
