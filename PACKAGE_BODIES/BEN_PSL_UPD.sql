--------------------------------------------------------
--  DDL for Package Body BEN_PSL_UPD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_PSL_UPD" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  ben_psl_upd.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |----------------------------< dt_update_dml >-----------------------------|
-- ----------------------------------------------------------------------------
  ...
  If (p_datetrack_mode = 'CORRECTION') then
    ...
    p_rec.object_version_number :=
      ...
      (p_base_table_name    => 'ben_per_info_chg_cs_ler_f',
       ...
       p_base_key_value => p_rec.per_info_chg_cs_ler_id);
    --
    ben_psl_shd.g_api_dml := true;  -- Set the api dml status
    --
    -- Update the ben_per_info_chg_cs_ler_f Row
    --
    ...
    --
    ben_psl_shd.g_api_dml := false;   -- Unset the api dml status
    --
...
Begin
  ...
  If (p_datetrack_mode <> 'CORRECTION') then
    ...
    --
    ben_psl_shd.upd_effective_end_date
     (p_effective_date         => p_effective_date,
      p_base_key_value         => p_rec.per_info_chg_cs_ler_id,
  ...
  exception
    ...
    when hr_api.cannot_find_prog_unit then
      ...
      hr_api.cannot_find_prog_unit_error
        (p_module_name => 'ben_per_info_chg_cs_ler_f'
        ,p_hook_type   => 'AU');
      --
  end;
  --
...
-- {End Of Comments}
-- ----------------------------------------------------------------------------
Procedure convert_defs(p_rec in out nocopy ben_psl_shd.g_rec_type) is
--
...
Begin
  ...
  --
  If (p_rec.per_info_chg_cs_ler_rl = hr_api.g_number) then
    p_rec.per_info_chg_cs_ler_rl :=
    ben_psl_shd.g_old_rec.per_info_chg_cs_ler_rl;
  End If;
  If (p_rec.name = hr_api.g_varchar2) then
    p_rec.name :=
    ben_psl_shd.g_old_rec.name;
  End If;
  If (p_rec.old_val = hr_api.g_varchar2) then
    p_rec.old_val :=
    ben_psl_shd.g_old_rec.old_val;
  End If;
  If (p_rec.new_val = hr_api.g_varchar2) then
    p_rec.new_val :=
    ben_psl_shd.g_old_rec.new_val;
  End If;
  If (p_rec.whatif_lbl_txt = hr_api.g_varchar2) then
      p_rec.whatif_lbl_txt :=
      ben_psl_shd.g_old_rec.whatif_lbl_txt;
  End If;
  If (p_rec.rule_overrides_flag = hr_api.g_varchar2) then
    p_rec.rule_overrides_flag :=
    ben_psl_shd.g_old_rec.rule_overrides_flag;
  End If;
  If (p_rec.source_column = hr_api.g_varchar2) then
    p_rec.source_column :=
    ben_psl_shd.g_old_rec.source_column;
  End If;
  If (p_rec.source_table = hr_api.g_varchar2) then
    p_rec.source_table :=
    ben_psl_shd.g_old_rec.source_table;
  End If;
  If (p_rec.business_group_id = hr_api.g_number) then
    p_rec.business_group_id :=
    ben_psl_shd.g_old_rec.business_group_id;
  End If;
  If (p_rec.psl_attribute_category = hr_api.g_varchar2) then
    p_rec.psl_attribute_category :=
    ben_psl_shd.g_old_rec.psl_attribute_category;
  End If;
  If (p_rec.psl_attribute1 = hr_api.g_varchar2) then
    p_rec.psl_attribute1 :=
    ben_psl_shd.g_old_rec.psl_attribute1;
  End If;
  If (p_rec.psl_attribute2 = hr_api.g_varchar2) then
    p_rec.psl_attribute2 :=
    ben_psl_shd.g_old_rec.psl_attribute2;
  End If;
  If (p_rec.psl_attribute3 = hr_api.g_varchar2) then
    p_rec.psl_attribute3 :=
    ben_psl_shd.g_old_rec.psl_attribute3;
  End If;
  If (p_rec.psl_attribute4 = hr_api.g_varchar2) then
    p_rec.psl_attribute4 :=
    ben_psl_shd.g_old_rec.psl_attribute4;
  End If;
  If (p_rec.psl_attribute5 = hr_api.g_varchar2) then
    p_rec.psl_attribute5 :=
    ben_psl_shd.g_old_rec.psl_attribute5;
  End If;
  If (p_rec.psl_attribute6 = hr_api.g_varchar2) then
    p_rec.psl_attribute6 :=
    ben_psl_shd.g_old_rec.psl_attribute6;
  End If;
  If (p_rec.psl_attribute7 = hr_api.g_varchar2) then
    p_rec.psl_attribute7 :=
    ben_psl_shd.g_old_rec.psl_attribute7;
  End If;
  If (p_rec.psl_attribute8 = hr_api.g_varchar2) then
    p_rec.psl_attribute8 :=
    ben_psl_shd.g_old_rec.psl_attribute8;
  End If;
  If (p_rec.psl_attribute9 = hr_api.g_varchar2) then
    p_rec.psl_attribute9 :=
    ben_psl_shd.g_old_rec.psl_attribute9;
  End If;
  If (p_rec.psl_attribute10 = hr_api.g_varchar2) then
    p_rec.psl_attribute10 :=
    ben_psl_shd.g_old_rec.psl_attribute10;
  End If;
  If (p_rec.psl_attribute11 = hr_api.g_varchar2) then
    p_rec.psl_attribute11 :=
    ben_psl_shd.g_old_rec.psl_attribute11;
  End If;
  If (p_rec.psl_attribute12 = hr_api.g_varchar2) then
    p_rec.psl_attribute12 :=
    ben_psl_shd.g_old_rec.psl_attribute12;
  End If;
  If (p_rec.psl_attribute13 = hr_api.g_varchar2) then
    p_rec.psl_attribute13 :=
    ben_psl_shd.g_old_rec.psl_attribute13;
  End If;
  If (p_rec.psl_attribute14 = hr_api.g_varchar2) then
    p_rec.psl_attribute14 :=
    ben_psl_shd.g_old_rec.psl_attribute14;
  End If;
  If (p_rec.psl_attribute15 = hr_api.g_varchar2) then
    p_rec.psl_attribute15 :=
    ben_psl_shd.g_old_rec.psl_attribute15;
  End If;
  If (p_rec.psl_attribute16 = hr_api.g_varchar2) then
    p_rec.psl_attribute16 :=
    ben_psl_shd.g_old_rec.psl_attribute16;
  End If;
  If (p_rec.psl_attribute17 = hr_api.g_varchar2) then
    p_rec.psl_attribute17 :=
    ben_psl_shd.g_old_rec.psl_attribute17;
  End If;
  If (p_rec.psl_attribute18 = hr_api.g_varchar2) then
    p_rec.psl_attribute18 :=
    ben_psl_shd.g_old_rec.psl_attribute18;
  End If;
  If (p_rec.psl_attribute19 = hr_api.g_varchar2) then
    p_rec.psl_attribute19 :=
    ben_psl_shd.g_old_rec.psl_attribute19;
  End If;
  If (p_rec.psl_attribute20 = hr_api.g_varchar2) then
    p_rec.psl_attribute20 :=
    ben_psl_shd.g_old_rec.psl_attribute20;
  End If;
  If (p_rec.psl_attribute21 = hr_api.g_varchar2) then
    p_rec.psl_attribute21 :=
    ben_psl_shd.g_old_rec.psl_attribute21;
  End If;
  If (p_rec.psl_attribute22 = hr_api.g_varchar2) then
    p_rec.psl_attribute22 :=
    ben_psl_shd.g_old_rec.psl_attribute22;
  End If;
  If (p_rec.psl_attribute23 = hr_api.g_varchar2) then
    p_rec.psl_attribute23 :=
    ben_psl_shd.g_old_rec.psl_attribute23;
  End If;
  If (p_rec.psl_attribute24 = hr_api.g_varchar2) then
    p_rec.psl_attribute24 :=
    ben_psl_shd.g_old_rec.psl_attribute24;
  End If;
  If (p_rec.psl_attribute25 = hr_api.g_varchar2) then
    p_rec.psl_attribute25 :=
    ben_psl_shd.g_old_rec.psl_attribute25;
  End If;
  If (p_rec.psl_attribute26 = hr_api.g_varchar2) then
    p_rec.psl_attribute26 :=
    ben_psl_shd.g_old_rec.psl_attribute26;
  End If;
  If (p_rec.psl_attribute27 = hr_api.g_varchar2) then
    p_rec.psl_attribute27 :=
    ben_psl_shd.g_old_rec.psl_attribute27;
  End If;
  If (p_rec.psl_attribute28 = hr_api.g_varchar2) then
    p_rec.psl_attribute28 :=
    ben_psl_shd.g_old_rec.psl_attribute28;
  End If;
  If (p_rec.psl_attribute29 = hr_api.g_varchar2) then
    p_rec.psl_attribute29 :=
    ben_psl_shd.g_old_rec.psl_attribute29;
  End If;
  If (p_rec.psl_attribute30 = hr_api.g_varchar2) then
    p_rec.psl_attribute30 :=
    ben_psl_shd.g_old_rec.psl_attribute30;
  End If;
...
Begin
  ...
  --
  l_rec :=
  ben_psl_shd.convert_args
  (
  p_per_info_chg_cs_ler_id,
  p_name,
  null,
  null,
  p_per_info_chg_cs_ler_rl,
  p_old_val,
  p_new_val,
  p_whatif_lbl_txt,
