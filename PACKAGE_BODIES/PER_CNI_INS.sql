--------------------------------------------------------
--  DDL for Package Body PER_CNI_INS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PER_CNI_INS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33) := '  per_cni_ins.';  -- Global package name
--
-- The following global variables are only to be used by
-- the set_base_key_value and pre_insert procedures.
--
g_config_information_id_i  number   default null;
--
-- ----------------------------------------------------------------------------
-- |------------------------< set_base_key_value >----------------------------|
-- ----------------------------------------------------------------------------
procedure set_base_key_value
  (p_config_information_id  in  number) is
--
...
Begin
  ...
  --
  per_cni_ins.g_config_information_id_i := p_config_information_id;
  --
...
Begin
  ...
  --
  -- Insert the row into: per_ri_config_information
  --
  insert into per_ri_config_information
      (configuration_code
...
--
  Cursor C_Sel1 is select per_ri_config_information_s.nextval from sys.dual;
--
  Cursor C_Sel2 is
    Select null
      from per_ri_config_information
     where config_information_id =
             per_cni_ins.g_config_information_id_i;
--
...
Begin
  hr_utility.set_location('Entering:'||l_proc, 5);
  --
  If (per_cni_ins.g_config_information_id_i is not null) Then
    --
    ...
    p_rec.config_information_id :=
      per_cni_ins.g_config_information_id_i;
    per_cni_ins.g_config_information_id_i := null;
  Else
    --
    ...
    --
    Open C_Sel1;
    Fetch C_Sel1 Into p_rec.config_information_id;
    Close C_Sel1;
  End If;
  --
  hr_utility.set_location(' Leaving:'||l_proc, 10);
...
Begin
  ...
  --
  per_cni_ins.pre_insert(p_rec);
  --
  -- Insert the row
  --
  per_cni_ins.insert_dml(p_rec);
  --
  -- Call the supporting post-insert operation
  --
  per_cni_ins.post_insert
     (p_effective_date
     ,p_rec
     );
  --
...
Begin
  ...
  --
  p_config_information_id := l_rec.config_information_id;
