--------------------------------------------------------
--  DDL for Package Body GME_COMMON_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."GME_COMMON_PVT" AS
/* $Header: GMEVCMNB.pls 120.36.12010000.6 2010/03/11 17:40:18 gmurator ship $ */
...
/************************************************************************************************
 ...
 Swapna K Bug#7385805 13-OCT-2008
  ...
  TRX_SOURCE_LINE_ID NOT IN (l_mtl_dtl_rec.material_detail_id)) which fetches the total qty allocated to find out org level shortages.
...
***********************************************************************************************/
   ...
   PROCEDURE set_move_to_temp_off
   IS
   BEGIN
      gme_common_pvt.g_move_to_temp := fnd_api.g_false;
   END set_move_to_temp_off;
...
/* If we have found atleast one resource falling below capacity then return true */
   ...
   BEGIN
      IF (g_debug IS NOT NULL AND p_tree_mode <> g_tree_reservation_mode) THEN
      ...
      LOOP
         ...
         IF (l_open_qty_prim > 0) THEN
           ...
           /*************************************************************************
            ...
            This issuse occurs only at org lvel. So we need to get sum of move order
            allcoation at org level where no reservation existis.
            *************************************************************************/

           IF p_invoke_mode = 'O' then
             BEGIN
...
--      x_open_qty := NVL(p_mtl_dtl_rec.wip_plan_qty, p_mtl_dtl_rec.plan_qty)
  --                   - (p_mtl_dtl_rec.actual_qty + NVL(l_reserved_qty,0) - NVL(p_mtl_dtl_rec.backordered_qty,0) + NVL(l_mo_line_qty,0));
...
--===================================================================== */
   ...
   --
   --  REQUIREMENTS
   --       Pass in the batchstep_id of the batch to fetched and vaildated.
   --
   --  SYNOPSIS:
   --
   --===================================================================== */
