--------------------------------------------------------
--  DDL for Package Body BEN_LRE_BUS
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BEN_LRE_BUS" as
...
-- ----------------------------------------------------------------------------
--
g_package  varchar2(33)	:= '  ben_lre_bus.';  -- Global package name
--
-- ----------------------------------------------------------------------------
-- |------< chk_ler_rqrs_enrt_ctfn_id >------|
-- ----------------------------------------------------------------------------
--
-- Description
...
--
-- In Parameters
--   ler_rqrs_enrt_ctfn_id PK of record being inserted or updated.
...
--
Procedure chk_ler_rqrs_enrt_ctfn_id(p_ler_rqrs_enrt_ctfn_id                in number,
                           p_effective_date              in date,
                           p_object_version_number       in number) is
  --
  l_proc         varchar2(72) := g_package||'chk_ler_rqrs_enrt_ctfn_id';
...
Begin
  ...
  --
  l_api_updating := ben_lre_shd.api_updating
    (p_effective_date              => p_effective_date,
     p_ler_rqrs_enrt_ctfn_id                => p_ler_rqrs_enrt_ctfn_id,
     p_object_version_number       => p_object_version_number);
  --
  if (l_api_updating
     and nvl(p_ler_rqrs_enrt_ctfn_id,hr_api.g_number)
     <>  ben_lre_shd.g_old_rec.ler_rqrs_enrt_ctfn_id) then
    --
    -- raise error as PK has changed
    --
    ben_lre_shd.constraint_error('BEN_LER_RQRS_ENRT_CTFN_F_PK');
    --
  elsif not l_api_updating then
    --
    -- check if PK is null
    --
    if p_ler_rqrs_enrt_ctfn_id is not null then
      --
...
--   Internal table handler use only.
--
Procedure chk_ctfn_rqd_when_rl(p_ler_rqrs_enrt_ctfn_id     in number,
...
--   Internal table handler use only.
--
Procedure chk_excld_flag(p_ler_rqrs_enrt_ctfn_id                in number,
...
Begin
  ...
  if (l_api_updating
      and p_excld_flag
      <> nvl(ben_lre_shd.g_old_rec.excld_flag,hr_api.g_varchar2)
  ...
  If (p_datetrack_mode = 'DELETE' or
    ...
    If (dt_api.rows_exist
          (p_base_table_name => 'ben_ler_enrt_ctfn_f',
           ...
           p_from_date       => p_validation_start_date,
           p_to_date         => p_validation_end_date)) Then
      l_table_name := 'ben_ler_enrt_ctfn_f';
      Raise l_rows_exist;
    End If;
    --
  End If;
...
function return_legislation_code
  (p_ler_rqrs_enrt_ctfn_id in number) return varchar2 is
  --
  -- Declare cursor
  --
  cursor csr_leg_code is
    ...
    from   per_business_groups a,
           ben_ler_rqrs_enrt_ctfn_f b
    where b.ler_rqrs_enrt_ctfn_id      = p_ler_rqrs_enrt_ctfn_id
