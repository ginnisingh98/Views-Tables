--------------------------------------------------------
--  DDL for Package Body BIM_CMPACQ_IND_TEMP_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."BIM_CMPACQ_IND_TEMP_PVT" AS
/* $Header: bimvcqib.pls 115.3 2000/07/27 17:30:21 pkm ship      $ */
...
BEGIN
      ...
      IF p_drill_down = 'Y'
      THEN
          where_clause := where_clause || ' and cmp.parent_campaign_id = :p_campaign_id ' ;
      ELSIF p_campaign_id IS NULL
      THEN
          where_clause := where_clause || ' and nvl(cmp.parent_campaign_id,-999) = nvl(:p_campaign_id, -999) ' ;
      ELSE
          where_clause := where_clause || ' and cmp.campaign_id = :p_campaign_id ' ;
      END IF;
   ...
   EXECUTE IMMEDIATE ' INSERT INTO bim_camp_acqu_summ_temp
       ...
       select cmp.campaign_name,' ||
         ...
         from bim_cmpgn_perf_summ cperf,
              bim_campaigns_denorm dnm,
              bim_dimv_campaigns cmp,
              bim_dimv_media med ' || from_clause ||
      ' where cperf.campaign_id = dnm.campaign_id
       ...
       group by cmp.campaign_name, '  || v_view_by
    USING
     p_campaign_id,
     p_start_date,
     p_end_date,
   ...
   --  dbms_output.put_line ( 'Number of rows inserted in temp table is : ' || v_num_rows_inserted );
    ...
    EXECUTE IMMEDIATE 'UPDATE bim_camp_acqu_summ_temp tmp
      ...
      ' where cperf.campaign_id = dnm.campaign_id
         and dnm.parent_campaign_id = cmp.campaign_id
         and cperf.media_id = med.media_id' ||  where_clause  ||
       ' and tmp.subject_name = cmp.campaign_name
         and cperf.period_start_date  >=  :p_start_date
         and cperf.period_end_date <= :p_end_date
  ...
  -- changed on 7/21 to fix bug: 1361701
  -- measure4 => New Customer Revenue
  -- measure3 => Total Revenue
  -- measure2 => % of sales from new customer
...
BEGIN
      ...
      ' where cperf.campaign_id = dnm.campaign_id
       ...
       ' and cperf.period_start_date >= per.start_date
         ...
         and cperf.secondary_interest_code_id = nvl(:p_secondary_interest_code_id, secondary_interest_code_id)
         and cperf.bill_to_geography_code = nvl(:p_geography_code, cperf.bill_to_geography_code )
       group by cmp.campaign_name, per.period_name, per.start_date'
     USING
      p_campaign_id,
      p_period_type,
      p_start_date,
