--------------------------------------------------------
--  DDL for Package Body PAY_GB_P11D_EDI_2008
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."PAY_GB_P11D_EDI_2008" AS
/* $Header: pygbp11d08.pkb 120.3.12010000.20 2010/02/22 11:56:05 krreddy ship $ */
         ...
         for asset in get_data loop
             if asset.cash_equivalent >= 1 then
                ...
                if asset.asset_type = 'OTHER' then
                   assets(l_count).other_description := substr(nvl(asset.asset_description,' '),1,70);  -- Added nvl for the bug fix# 9354567
                else
  ...
  is
         ...
         l_row_num                     number;
         l_valid_benefit_end_date_flag varchar2(35);
         l_pos                         number;
	 ...
	 edi_att3            varchar2(6);
         ...
         -- edi_dtm2   varchar2(6); -- commented EOY 2008
         edi_dtm3   varchar2(6); -- EOY 2008
         edi_dtmg   varchar2(4);
	 ...
	 edi_cat             := rpad('F',18);
         ...
         fetch get_data into l_row_num,
	                     l_benefit_start_date,
                             ...
                             l_valid_benefit_end_date_flag, -- EOY 2008
         ...
         -- EOY 2008
         l_pos := instr(l_valid_benefit_end_date_flag, ':');
         l_valid_benefit_end_date_flag := substr(l_valid_benefit_end_date_flag,l_pos+1,1);
         ...
         /* modified below if for bug 8277887 */
            if (to_date(substr(l_date_first_registered,1,10),'YYYY/MM/DD') <
               ...
               (to_date(substr(l_date_first_registered,1,10),'YYYY/MM/DD') >=
                to_date('1998/01/01','YYYY/MM/DD') and  to_number(nvl(l_mileage_band,0)) = 0 )) then
		...
		/* added fuel type E85 and decoded it to G for bug 7594748 */
           ...
           /* modified below if for bug 8277887 */
            if (to_date(substr(l_date_first_registered,1,10),'YYYY/MM/DD') >=
                to_date('1998/01/01','YYYY/MM/DD') and to_number(nvl(l_mileage_band,0)) = 0 ) or
                (l_fuel_type = 'E') then
            ...
            if (to_date(substr(l_date_first_registered,1,10),'YYYY/MM/DD') >=
             ...
             p_edi_rec20 := edi_dtm3 || edi_dtm375 ||
                            rpad(to_char(to_date(substr(l_date_first_registered,1,10),'YYYY/MM/DD'),'YYYYMMDD'),36) ||
                            edi_dtm102 || fnd_global.local_chr(10); -- EOY 2008

             if to_number(edi_date_from) >= to_number(edi_tax_year_start) then -- EOY 2008
                p_edi_rec21 := edi_dtm3 || edi_dtm488 || rpad(edi_date_from,36) || edi_dtm102 || fnd_global.local_chr(10); -- EOY 2008
             end if;

             if to_number(edi_date_to) <= to_number(edi_tax_year_end) then     -- EOY 2008
                -- EOY 2008
                hr_utility.trace('edi_date_to:'||edi_date_to);
                hr_utility.trace('l_valid_benefit_end_date_flag:'||l_valid_benefit_end_date_flag);
                IF substr(edi_date_to,5,4) = '0405' THEN
                     IF l_valid_benefit_end_date_flag = 'Y' THEN
                        p_edi_rec22 := edi_dtm3 || edi_dtm489 || rpad(edi_date_to,36) || edi_dtm102 || fnd_global.local_chr(10); -- EOY 2008
                        hr_utility.trace('printing edi_date_to');
                     ELSE
                        hr_utility.trace('NOT printing edi_date_to');
                    END IF ;
                ELSE
                    p_edi_rec22 := edi_dtm3 || edi_dtm489 || rpad(edi_date_to,36) || edi_dtm102 || fnd_global.local_chr(10); -- EOY 2008
                    hr_utility.trace('Unconditional printing edi_date_to');
                END IF ;
             end if;
             ...
             if l_free_fuel_reinstated <> 'N' and to_number(l_cash_equivalent_for_fuel) > 0
                ...
                p_edi_rec23 := edi_dtm3 || edi_dtmg || rpad(l_withdrawn_date, 36) || edi_dtm102 || fnd_global.local_chr(10); -- EOY 2008
             end if;
        end if;
  end get_car_or_fuel;
  /******************* Car and Car Fuel (Multi Occurance) ***********************/
  ...
  is
	     ...
	     edi_cat             varchar2(18);
    	 edi_tax_qualifier12 varchar2(3);
    	 edi_tax_qualifier139 varchar2(3);
  ...
  begin
	     ...
	     edi_cat              := rpad('G',18);
         edi_tax_qualifier12  := rpad('12',3);
         edi_tax_qualifier139 := rpad('139',3); -- EOY 2008
         ...
         -- EOY 2008
         open get_data2;
         fetch get_data2 into l_vans_fuel;
         close get_data2;
 --commented the below logic for bug 7231650
       --  if l_cash_equivalent >= 1 and l_vans_fuel >=1 then
       --bug 7231650 added the below logic
       if l_cash_equivalent >= 1 or l_vans_fuel >=1 then
            p_edi_rec1  := edi_ftx1a || edi_cat || rpad(' ', 71) || rpad(' ', 71) ||
                           rpad(' ', 70) || fnd_global.local_chr(10);
            p_edi_rec2  := edi_tax1 || edi_tax_qualifier12 || fnd_global.local_chr(10);
            p_edi_rec3  := edi_moa1 || pay_gb_p11d_magtape.format_edi_currency(l_cash_equivalent) ||
            ...
            -- EOY 2008
          --commenting the below added if as van fuel have to be printed even it is null or zero.
	    --bug 7231650 enclsoed the two edi records in IF condition
	    --if  l_vans_fuel >=1 then
            p_edi_rec4  := edi_tax1 || edi_tax_qualifier139 || fnd_global.local_chr(10);
            p_edi_rec5  := edi_moa1 || pay_gb_p11d_magtape.format_edi_currency(nvl(l_vans_fuel,0)) ||
                           edi_currency || fnd_global.local_chr(10);
	    --end if;
       end if;
  end get_vans;
  ...
  begin
       ...
       if to_number(p_value1) > 5000 then
          ...
          edi_qty    := rpad('QTY0',6);
          edi_qtyg   := rpad('G',4);
          -- edi_dtm2   := rpad('DTM2',6); commented EOY 2008
          edi_dtm3   := rpad('DTM3',6);
	  ...
	  --bug 7042975  changed p_tax_year to to_char(to_number(p_tax_year)-1)
             edi_tax_year_start := to_char(to_number(p_tax_year)-1) || '0406';
  ...
  is
	 ...
	 l_further_description                varchar2(255); --Added for the bug# 8708099
         edi_cat             varchar2(18);
         edi_ftx1a           varchar2(18);
         edi_tax1            varchar2(6);
         edi_moa1            varchar2(6);
	 ...
	 edi_tax_qualifier90 varchar2(3);
		...
		to_number(pai.action_information6)  cost_to_you,
                ...
                pai.action_information9  description  --Added for the bug# 8708099
         from   per_all_assignments_f   paf,
       	    	pay_assignment_actions  paa,
       		    pay_action_information  pai,
       		    pay_action_information  pai_person
  ...
  begin
        ...
        l_other_description := ' ';
        l_further_description  := ' ';  --Added for the bug# 8708099
...
--Modifications for the bug# 8708099 starts
              if (rtrim(l_other_description) = 'OTHER') then
                        l_further_description := rpad(substr(expense.description,1,30),30,' ');
              end if;
--Modifications for the bug# 8708099 ends

	end loop;

--Modifications for the bug# 8708099 starts
if (rtrim(l_other_description) <> 'OTHER') then
	l_further_description := rpad (' ',30,' ');
end if;
--Modifications for the bug# 8708099 ends
...
/*
            p_edi_rec1  := edi_ftx1a || edi_cat || rpad(rpad(l_other_description,30),71) || rpad(' ',71) ||
                           rpad(' ',70) || fnd_global.local_chr(10);
*/
            p_edi_rec1  := edi_ftx1a || edi_cat || rpad(rpad(l_other_description,30),71) || rpad(rpad(l_further_description,30),71) ||
                           rpad(' ',70) || fnd_global.local_chr(10);
--Modifications for the bug# 8708099 ends
  ...
  begin
       ...
       edi_recipient_id      := rpad('HMRC',35); -- EOY 2008
       edi_test_indicator    := ' ';
       edi_urgent_marker     := ' ';
  ...
  begin
       ...
       */
       ...
       /*BUG 7354812 */
       if p_submitter_ref IS NULL then
          p_error_msg2  := ':The P11D Submitter Number is missing';
          p_error_count := p_error_count + 1;

	  --Bug 8663332: Added put_line to write error msg in
          --log file even if logging is not set
          fnd_file.put_line(fnd_file.log,'ERROR_MSG: The P11D Submitter Number is missing');
     ...
     /* bug 7594748 start */
         p_edi_rec2 := edi_nad1b   || rpad(upper(substr(p_party,1,35)), 36)  ||
  ...
  is
       edi_att2                varchar2(6);
       edi_dtm2                varchar2(6); -- EOY 2008
       edi_att_qualifier3      varchar2(4);
       edi_att_qualifier11     varchar2(4);
       edi_att_qualifier19     varchar2(4);
       edi_att_qualifier9      varchar2(4); -- EOY 2008
       edi_dtm_qualifer329     varchar2(4); -- EOY 2008
       edi_dtm_qualifier102    varchar2(4); -- EOY 2008
       ...
       l_date_of_birth         varchar2(25); -- EOY 2008
       l_gender                varchar2(1);  -- EOY 2008
  ...
  begin
       ...
       edi_dtm2                := rpad('DTM2',6);  -- EOY 2008
       edi_att_qualifier9      := rpad('9',4);     -- EOY 2008
       edi_dtm_qualifer329     := rpad('329',4);   -- EOY 2008
       edi_dtm_qualifier102    := rpad('102',3);   -- EOY 2008
       ...
       */
       ...
       if l_ni_number <> 'NONE' then  -- EOY 2008
       ...
       /*EOY 2008 starts*/
       if l_gender <> ' ' then
          p_edi_rec6 :=  edi_att2 || edi_att_qualifier9 || rpad(l_gender,35)|| fnd_global.local_chr(10) ;
       end if ;

       if l_date_of_birth <> ' ' then
           l_date_of_birth := to_char(to_date(l_date_of_birth,'YYYY/MM/DD HH24:MI:SS'), 'YYYYMMDD') ;
           p_edi_rec7 :=  edi_dtm2 || edi_dtm_qualifer329 || rpad(l_date_of_birth,36) || edi_dtm_qualifier102 || fnd_global.local_chr(10);
       end if ;
       /*EOY 2008 ends */
  ...
  begin
     hr_utility.trace('\nxxxx get_benefit called - ' ||p_benefit_type) ;
       ...
       if p_benefit_type = 'VANS 2007' then   -- EOY 2008
          hr_utility.trace('\nxxxx Vans called - ' ||p_benefit_type) ;
          hr_utility.trace('Vans');
          -- EOY 2008. Added 2 extra parameters p_edi_rec4 and p_edi_rec5
  ...
  is
       ...
       /* modified cursor csr_benefit for bug 7231911 */
    /*   cursor csr_benefit is
       ...
       select /*+ ORDERED use_nl(paa,pai,pai_a,pai_person)
             ...
             'VANS 2005', pai.action_information15,
             'VANS 2007', pai.action_information14,  -- EOY 2008
--Modified the below condition for the bug fix 9374377.
--We are checking if the Cash Equavalant value is > 1, then
--check if the Maximum Amount Outstanding is > 5000.
--If it is then return 1 else return 0.
--             'INT FREE AND LOW INT LOANS', pai.action_information11,
             'INT FREE AND LOW INT LOANS',
			  decode(sign(pai.action_information11 - 1), 0, 0, -1, 0, 1,
			  decode(sign(pai.action_information7 - 5000),0,0,-1,0,1,1)),
--
             'PVT MED TREATMENT OR INSURANCE', pai.action_information7,
             'RELOCATION EXPENSES', pai.action_information5,
             'SERVICES SUPPLIED', pai.action_information7,
  ...
  begin
  /* modified the fetching logic of csr_benefit for bug 7231911 */
    /*   open csr_benefit;
       fetch csr_benefit into l_total;
       close csr_benefit; */
       for v_csr_benefit in csr_benefit
      loop
            if v_csr_benefit.total >= 1 then
             return 1;
            end if;
      end loop;
