--------------------------------------------------------
--  DDL for Package HR_RU_ASSIGNMENT_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."HR_RU_ASSIGNMENT_API" AUTHID CURRENT_USER AS
/* $Header: peasgrui.pkh 120.2 2005/10/27 03:08:46 vikgupta noship $ */
/*#
 * This package contains assignment APIs for Russia
 * @rep:scope public
 * @rep:product PER
 * @rep:displayname Assignment for Russia
*/
-- ----------------------------------------------------------------------------
...
-- {Start Of Comments}
/*#
 * This API creates a secondary assignment for an employee for Russia.
 *
 * <p><b>Licensing</b><br>
 ...
 * @param p_assignment_status_type_id Identifies the assignment status of
 ...
 * @param p_special_ceiling_step_id Highest allowed step for the grade
 * scale associated with the grade of the secondary assignment.
 ...
 * generated.
 * @param p_start_reason Reason for the assignment status change. If there
 * is no change reason the parameter can be null. Valid values are defined in
 * the EMP_ASSIGN_REASON lookup type.
 * @param p_comments Comment text.
 * @param p_date_probation_end End date of probation period
 * @param p_default_code_comb_id Identifier for the General Ledger
 * Accounting Flexfield combination that applies to this assignment.
 * @param p_employment_category Employment category. Valid values are
 * defined in the EMP_CAT lookup type.
 ...
 * @param p_perf_review_period_frequency Units of performance review
 * period. Valid values are defined in the FREQUENCY lookup type.
 * @param p_probation_period Length of probation period
 * @param p_probation_unit Units of probation period. Valid values are
 * defined in the QUALIFYING_UNITS lookup type.
 * @param p_sal_review_period Length of salary review period
 * @param p_sal_review_period_frequency Units of salary review period.
 * Valid values are defined in the FREQUENCY lookup type.
 * @param p_set_of_books_id Identifies General Ledger set of books.
 * @param p_source_type Recruitment activity which this assignment is
 * sourced from. Valid values are defined in the REC_TYPE lookup type.
 ...
 * @param p_labour_union_member_flag Value 'Y' indicates employee is a
 * labour union member. Other values indicate not a member.
 * @param p_hourly_salaried_code Identifies if the assignment is paid
 * hourly or is salaried. Valid values defined in the HOURLY_SALARIED_CODE
 * lookup type.
 ...
 * @param p_employer Legal Employer of the assignment
 * @param p_sec_emp Identifies if the assignment is primary employment for
 * the employee. Valid values are defined in the 'YES_NO' lookup type.
 * @param p_contract_number Contract Number of the assignment.
 * @param p_issue_date Contract Issue Date of the assignment.
 * @param p_cont_end_date Contract End Date of the assignment.
 * @param p_limit_reason Limited Term Contract Reason for the assignment.
 * Valid values are defined in the 'RU_LIMITED_CONTRACT_REASON' lookup type.
 * @param p_end_reason Contract Termination Reason for the assignment.
 * Valid values are defined in the 'LEAV_REAS' lookup type.
 * @param p_territory_cond Territory conditions of the assignment. Valid
 * values are defined in the 'RU_TERRITORY_CONDITIONS' lookup type.
 * @param p_spl_work_cond Special working conditions of the assignment.
 * Valid values are defined in the 'RU_SPECIAL_WORK_CONDITIONS' lookup type.
 * @param p_csr Calculated Record of Service of the assignment. Valid
 * values are defined in the 'RU_CALC_RECORD_SERVICE' lookup type.
 * @param p_csr_add_info Calculated Record of Service additional
 * Information of the assignment
 * @param p_lsr Long Service Record of the assignment. Valid values are
 * defined in the 'RU_LONG_SERVICE' lookup type.
 * @param p_lsr_add_info Long Service Record additional information of the
 * assignment.
 * @param p_unint_service_rec Uninterrupted Service Record of the
 * assignment. Valid values are defined in the 'YES_NO' lookup type.
 * @param p_total_service_rec Total Service Record of the assignment.Valid
 * values are defined in the 'YES_NO' lookup type.
 * @param p_pension_years Additional Pension Years.
 * @param p_pension_months Additional Pension Months.
 * @param p_pension_days Additional Pension Days.
 ...
 * @param p_establishment_id This identifies the establishment Legal Entity
 * for this assignment.
 * @param p_collective_agreement_id Collective Agreement that applies to
 * this assignment.
 ...
 * @param p_notice_period_uom Units for notice period. Valid values are
 * defined in the QUALIFYING_UNITS lookup type.
 * @param p_employee_category Employee Category. Valid values are defined
 * in the EMPLOYEE_CATG lookup type.
 * @param p_work_at_home Indicate whether this assignment is to work at
 * home. Valid values are defined in the YES_NO lookup type.
 ...
 * @param p_cagr_grade_def_id If a value is passed in for this parameter,
 * it identifies an existing CAGR Key Flexfield combination to associate with
 * the assignment, and segment values are ignored. If a value is not passed in,
 ...
 * segments
 * @param p_assignment_id If p_validate is false, then this uniquely
 * identifies the created assignment. If p_validate is true, then set to null.
 * @param p_soft_coding_keyflex_id If a value is passed in for this
 * parameter, it identifies an existing Soft Coded Key Flexfield combination to
 * associate with the assignment, and segment values are ignored. If a value is
 * not passed in, then the individual Soft Coded Key Flexfield segments
 * supplied will be used to choose an existing combination or create a new
 * combination. When the API completes, if p_validate is false, then this
 * uniquely identifies the associated combination of the Soft Coded Key
 * flexfield for this assignment. If p_validate is true, then set to null.
 ...
 * set to null.
 * @param p_assignment_sequence If p_validate is false, then an
 * automatically incremented number is associated with this assignment,
 * depending on the number of assignment which already exist. If p_validate is
 * true then set to null.
 * @param p_comment_id If p_validate is false and comment text was
 * provided, then will be set to the identifier of the created assignment
 * comment record. If p_validate is true or no comment text was provided, then
 * will be null.
 * @param p_concatenated_segments If p_validate is false, set to Soft Coded
 ...
 * @param p_other_manager_warning If set to true, then a manager existed in
 * the organization prior to calling this API and the manager flag has been set
 * to 'Y' for yes.
 * @param p_hourly_salaried_warning Set to true if values entered for
 * Salary Basis and Hourly Salaried Code are invalid as of p_effective_date.
 * @param p_gsp_post_process_warning Set to the name of a warning message
 * from the Message Dictionary if any Grade Ladder related errors have been
 * encountered while running this API.
 * @rep:displayname Create Secondary Employee Assignment for Russia
...
-- {Start Of Comments}
/*#
 * This API updates employee assignment details for Russia
 *
 * <p><b>Licensing</b><br>
 * This API is licensed for use with Human Resources.
 *
 ...
 * @param p_effective_date Determines when the DateTrack operation comes
 * into force.
 * @param p_datetrack_update_mode Indicates which DateTrack mode to use
 * when updating the record. You must set to either UPDATE, CORRECTION,
 ...
 * @param p_change_reason Reason for the assignment status change. If there
 * is no change reason the parameter can be null. Valid values are defined in
 * the EMP_ASSIGN_REASON lookup type.
 * @param p_assignment_status_type_id Assignment status. The system status
 * must be the same as before the update, otherwise one of the status change
 * APIs should be used.
 * @param p_comments Comment text.
 * @param p_date_probation_end End date of probation period
 * @param p_default_code_comb_id Identifier for the General Ledger
 ...
 * @param p_pension_months Additional Pension Months.
 * @param p_pension_days Additional Pension Days.
 * @param p_concat_segments Concatenated segments for Soft Coded Key
 * Flexfield. Concatenated segments can be supplied instead of individual
 * segments.
 * @param p_contract_id Contract associated with this assignment
 * @param p_establishment_id This identifies the establishment Legal Entity
 ...
 * @param p_work_at_home Indicate whether this assignment is to work at
 * home. Valid values are defined in the YES_NO lookup type.
 * @param p_job_post_source_name Name of the source of the job posting that
 ...
 * @param p_concatenated_segments If p_validate is false, set to Soft Coded
 * Key Flexfield concatenated segments, if p_validate is true, set to null.
 * @param p_no_managers_warning Set to true if as a result of the update
 * there is no manager in the organization. Otherwise set to false.
 * @param p_other_manager_warning If set to true, then a manager existed in
 * the organization prior to calling this API and the manager flag has been set
 * to 'Y' for yes.
 * @param p_hourly_salaried_warning Set to true if values entered for
 * Salary Basis and Hourly Salaried Code are invalid date as of
 * p_effective_date.
 * @param p_gsp_post_process_warning Set to the name of a warning message
 * from the Message Dictionary if any Grade Ladder related errors have been
 * encountered while running this API.
 * @rep:displayname Update Employee Assignment for Russia
 * @rep:category BUSINESS_ENTITY PER_EMPLOYEE_ASG
 * @rep:lifecycle active
 * @rep:category MISC_EXTENSIONS HR_DATAPUMP
 * @rep:category MISC_EXTENSIONS HR_USER_HOOKS
