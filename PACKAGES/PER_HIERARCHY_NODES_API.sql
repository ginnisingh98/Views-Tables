--------------------------------------------------------
--  DDL for Package PER_HIERARCHY_NODES_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PER_HIERARCHY_NODES_API" AUTHID CURRENT_USER as
 ...
 * within a generic hierarchy structure.
 * @rep:scope public
 * @rep:product per
 * @rep:displayname Generic Hierarchy Node
*/
 G_MODE   VARCHAR2(10);
--
-- ----------------------------------------------------------------------------
...
-- {Start Of Comments}
/*#
 * This API creates a generic hierarchy node.
 *
 * A generic hierarchy data structure consists of individual data values called
 * nodes which are arranged in parent-child relationships to create a hierarchy
 * or tree of data. The top node is known as the root node, under which there
 * can be many child and grandchild nodes. The value for each node is drawn
 * from the value set that has been identified as providing data for that
 * particular level of the structure. Each node, with the exception of the root
 * node, will be linked to a parent node. In this way a hierarchy of data
 ...
 * The generic hierarchy structure version under which the node will be created
 * must exist.
 *
 * <p><b>Post Success</b><br>
 * The API creates the hierarchy node.
 *
 * <p><b>Post Failure</b><br>
 * The API does not create the hierarchy node and an error is raised.
 * @param p_validate If true, then validation alone will be performed and the
 * database will remain unchanged. If false and all validation checks pass,
 * then the database will be modified.
 * @param p_hierarchy_node_id If p_validate is false then uniquely identifies
 * the generic hierarchy node created. If p_validate is true then set to null.
 * @param p_business_group_id The business group under which this generic
 * hierarchy node is created.
 * @param p_entity_id Provides the value of the node. This is the value set
 * value drawn from the value set held by the child node type specified in the
 * node_type parameter.
 * @param p_hierarchy_version_id Uniquely identifies the generic hierarchy
 * structure version that the node is part of.
 * @param p_node_type The child node type which is providing the value set
 * source of values for this node.
 * @param p_seq The sequence of the node relative to the other nodes that may
 * exist at this level (under this parent).
 * @param p_parent_hierarchy_node_id Uniquely identifies the parent generic
 * hierarchy node under which this node exists.
 ...
 * version number of the created generic hierarchy node. If p_validate is true,
 ...
 * @rep:displayname Create Generic Hierarchy Node
...
-- {Start Of Comments}
/*#
 * This API updates a generic hierarchy node.
 *
 * A generic hierarchy data structure consists of individual data values called
 * nodes which are arranged in parent-child relationships to create a hierarchy
 * or tree of data. The top node is known as the root node, under which there
 * can be many child and grandchild nodes, etc. The value for each node is
 * drawn from the value set that has been identified as providing data for that
 * particular level of the structure. Each node, with the exception of the root
 * node, will be linked to a parent node. In this way a hierarchy of data
 * values is constructed.
 *
 ...
 *
 * <p><b>Prerequisites</b><br>
 * The node type to be updated must exist.
 *
 * <p><b>Post Success</b><br>
 * The API updates the node type.
 *
 * <p><b>Post Failure</b><br>
 * The API does not update the node type and an error is raised.
 * @param p_validate If true, then validation alone will be performed and the
 * database will remain unchanged. If false and all validation checks pass,
 * then the database will be modified.
 * @param p_hierarchy_node_id Uniquely identifies the node type to be updated.
 * @param p_entity_id Provides the value of the node. This is the value set
 * value drawn from the value set held by the child node type specified in the
 * node_type parameter.
 * @param p_node_type The child node type which is providing the value set
 ...
 * generic hierarchy node to be updated. When the API completes if p_validate
 * is false, will be set to the new version number of the updated generic
 * hierarchy node. If p_validate is true will be set to the same value which
 * was passed in.
 * @param p_attribute_category This context value determines which flexfield
 * structure to use with the descriptive flexfield segments.
 * @param p_attribute1 Descriptive flexfield segment.
 ...
 * applicable during the start to end active date range. This date does not
 * determine when the changes take effect
 * @rep:displayname Update Generic Hierarchy Node
 * @rep:category BUSINESS_ENTITY PER_GENERIC_HIERARCHY
 * @rep:category MISC_EXTENSIONS HR_USER_HOOKS
 * @rep:scope public
 * @rep:lifecycle active
...
-- {Start Of Comments}
/*#
 * This API deletes a generic hierarchy node.
 *
 * A generic hierarchy data structure consists of individual data values called
 * nodes which are arranged in parent-child relationships to create a hierarchy
 * or tree of data. The top node is known as the root node, under which there
 ...
 *
 * <p><b>Prerequisites</b><br>
 * The generic hierarchy node must exist and must not have any child generic
 * hierarchy nodes referencing it.
 *
 * <p><b>Post Success</b><br>
 * The hierarchy node is deleted.
 *
 * <p><b>Post Failure</b><br>
 * The API does not delete the hierarchy node and an error is raised.
 * @param p_validate If true, then validation alone will be performed and the
 * database will remain unchanged. If false and all validation checks pass,
 * then the database will be modified.
 * @param p_hierarchy_node_id Uniquely identifies the generic hierarchy node to
 * be deleted.
 * @param p_object_version_number Current version number of the generic
 * hierarchy node to be deleted.
 * @rep:displayname Delete Generic Hierarchy Node
 * @rep:category BUSINESS_ENTITY PER_GENERIC_HIERARCHY
 * @rep:category MISC_EXTENSIONS HR_USER_HOOKS
 * @rep:scope public
 * @rep:lifecycle active
...
--   p_hierarchy_node_id                 Yes  number   PK of record
