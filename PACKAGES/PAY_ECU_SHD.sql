--------------------------------------------------------
--  DDL for Package PAY_ECU_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PAY_ECU_SHD" AUTHID CURRENT_USER as
...
Type g_rec_type Is Record
  (element_class_usage_id          number(9)
...
-- Global table name
g_tab_nam  constant varchar2(30) := 'PAY_ELEMENT_CLASS_USAGES_F';
--
-- ----------------------------------------------------------------------------
-- |---------------------------< constraint_error >---------------------------|
-- ----------------------------------------------------------------------------
...
--
-- Description:
--   This procedure is used to find out the uniqueness of the record.
--   It checks for the uniqueness of the following within table.
--   RUN TYPE ID
--   CLASSIFICATION ID
...
--
-- Post Success:
-- If it passes then there is no record exists with the given values.
--
-- Post Failure:
--   A failure can only occur when a records already exists with the
--   give values. In this case it raises an error.
--
-- Developer Implementation Notes:
--   None.
--
...
--           p_base_key_value = :element_class_usage_id).
--
-- Post Success:
