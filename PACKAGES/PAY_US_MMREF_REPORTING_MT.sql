--------------------------------------------------------
--  DDL for Package PAY_US_MMREF_REPORTING_MT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PAY_US_MMREF_REPORTING_MT" AUTHID CURRENT_USER AS
 /* $Header: payusmmrfrecmt.pkh 120.0.12000000.1 2007/02/23 10:16:39 sackumar noship $ */
 /*===========================================================================+
 ...
 +============================================================================+
  ...
  ----------- -------- ------- -------- -------------------------------------
  01-aug-2006 ppanda   115.0            Enhanced Multi Threading architecture for Mag Media
                                        report. This is the initial version created.
  =============================================================================*/
  ...
  CURSOR mmrf_employee IS
     SELECT 'ASSIGNMENT_ACTION_ID=C', AI.locked_action_id, -- YREND assignment action
      ...
      WHERE PA.payroll_action_id = pay_magtape_generic.get_parameter_value
        ...
        AND SS.effective_start_date =  (select max(paf2.effective_start_date)
  ...
  */
  ...
  CURSOR OH_LOCAL_MMRF_EMPLOYEE IS
     ...
     SELECT 'TRANSFER_ACT_ID=P',
      ...
      WHERE PA.payroll_action_id    = pay_magtape_generic.get_parameter_value
                                                                   ('TRANSFER_PAYROLL_ACTION_ID')
        AND aa.assignment_action_id   = pay_magtape_generic.get_parameter_value('TRANSFER_ACT_ID')
        AND AA.payroll_action_id       = PA.payroll_action_id
        AND AI.locking_action_id        = AA.assignment_action_id
        AND SS.assignment_id             = AA.assignment_id
        AND PE.person_id                   = SS.person_id
