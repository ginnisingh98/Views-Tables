--------------------------------------------------------
--  DDL for Package PA_ONLINE_EXP
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PA_ONLINE_EXP" AUTHID CURRENT_USER AS
/* $Header: PAXONTES.pls 120.1 2005/08/09 04:31:38 avajain noship $ */
...
PRAGMA RESTRICT_REFERENCES ( get_day, WNDS, WNPS );

PROCEDURE GET_NUM_DAYOFWEEK (X_name_dayof_week IN VARCHAR2,
			     X_num_dayof_week OUT NOCOPY VARCHAR2);

PROCEDURE FIND_DELETE_EXP_ITEMS(x_denorm_id IN NUMBER, X_COLUMN IN NUMBER,X_status OUT NOCOPY NUMBER);

FUNCTION GET_LAST_RTE_COMMENT(X_exp_id IN NUMBER) RETURN VARCHAR2 ;
PRAGMA RESTRICT_REFERENCES ( GET_LAST_RTE_COMMENT, WNDS, WNPS ) ;

FUNCTION GET_EXP_TOTAL_HOURS( X_exp_id IN NUMBER,
			      X_exp_class_code IN VARCHAR2) RETURN NUMBER ;
PRAGMA RESTRICT_REFERENCES ( GET_EXP_TOTAL_HOURS, WNDS, WNPS ) ;

FUNCTION GET_EI_TOTAL_HOURS( X_exp_id IN NUMBER,
		             X_exp_class_code IN VARCHAR2) RETURN NUMBER ;
PRAGMA RESTRICT_REFERENCES ( GET_EI_TOTAL_HOURS, WNDS, WNPS ) ;

FUNCTION GET_DENORM_TOTAL_HOURS (x_exp_id IN NUMBER,
                                 x_exp_class_code IN VARCHAR2) RETURN NUMBER ;
PRAGMA RESTRICT_REFERENCES ( GET_DENORM_TOTAL_HOURS, WNDS, WNPS ) ;

FUNCTION GET_EXP_TOTAL_COST( X_exp_id IN NUMBER,
                             X_exp_class_code IN VARCHAR2) RETURN NUMBER ;
PRAGMA RESTRICT_REFERENCES ( GET_EXP_TOTAL_COST, WNDS, WNPS ) ;

FUNCTION GET_EI_TOTAL_COST( X_exp_id IN NUMBER,
                            X_exp_class_code IN VARCHAR2) RETURN NUMBER ;
PRAGMA RESTRICT_REFERENCES ( GET_EI_TOTAL_COST, WNDS, WNPS ) ;

FUNCTION GET_DENORM_TOTAL_COST ( X_exp_id IN NUMBER,
                                 X_exp_class_code IN VARCHAR2) RETURN NUMBER ;
PRAGMA RESTRICT_REFERENCES ( GET_DENORM_TOTAL_COST, WNDS, WNPS ) ;
...
--PRAGMA RESTRICT_REFERENCES ( GET_EXP_TRANS, WNDS, WNPS ) ;
...
-- there is no need to explicitly define purity level, so removing the
-- purity level all togeather.
--PRAGMA RESTRICT_REFERENCES ( GET_EXP_SRC, WNDS, WNPS ) ;
...
-- there is no need to explicitly define purity level, so removing the
-- purity level all togeather.
--PRAGMA RESTRICT_REFERENCES ( GET_EXP_SRC_CODE, WNDS, WNPS ) ;
