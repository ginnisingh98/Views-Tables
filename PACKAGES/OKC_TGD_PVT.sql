--------------------------------------------------------
--  DDL for Package OKC_TGD_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."OKC_TGD_PVT" AUTHID CURRENT_USER AS
/* $Header: OKCSTGDS.pls 120.0 2005/05/25 22:58:04 appldev noship $ */
  ---------------------------------------------------------------------------
...
/*
  ...
  TYPE okc_timevalues_tl_tbl_type IS TABLE OF okc_timevalues_tl_rec_type
        INDEX BY BINARY_INTEGER;
*/
  TYPE tgdv_rec_type IS RECORD (
    id                             NUMBER := OKC_API.G_MISS_NUM,
    object_version_number          NUMBER := OKC_API.G_MISS_NUM,
--Bug 3122962
    sfwt_flag                      OKC_TIME_TPG_DELIMITED_V.SFWT_FLAG%TYPE := 'N',
    npt_id                         NUMBER := OKC_API.G_MISS_NUM,
    dnz_chr_id                 NUMBER := OKC_API.G_MISS_NUM,
    tze_id                 NUMBER := OKC_API.G_MISS_NUM,
    tve_id_limited                 NUMBER := OKC_API.G_MISS_NUM,
    ...
    last_update_login              NUMBER := OKC_API.G_MISS_NUM);
  g_miss_tgdv_rec                         tgdv_rec_type;
  TYPE tgdv_tbl_type IS TABLE OF tgdv_rec_type
        INDEX BY BINARY_INTEGER;
  ---------------------------------------------------------------------------
  ...
  -- GLOBAL VARIABLES
  ---------------------------------------------------------------------------
  G_PKG_NAME			CONSTANT VARCHAR2(200) := 'OKC_TGD_PVT';
  ...
  PROCEDURE insert_row(
    ...
    x_tgdv_tbl                     OUT NOCOPY tgdv_tbl_type);
