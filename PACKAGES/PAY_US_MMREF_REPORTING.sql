--------------------------------------------------------
--  DDL for Package PAY_US_MMREF_REPORTING
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PAY_US_MMREF_REPORTING" AUTHID CURRENT_USER AS
 /* $Header: pyusmmye.pkh 120.5.12010000.2 2009/11/12 11:58:41 svannian ship $ */
 /*===========================================================================+
 ...
 +============================================================================+
  ...
  18-NOV-2005 saurgupt 115.21 4644692   Added ORDERED hint in cursor mmrf_employee.
  04-AUG-2006  ppanda  115.22           Added following new function to support multithread architecture
                                             in Federal W-2 Magnetic Media
                                             - get_report_category_mt
  28-AUG-2006  ppanda  115.31           Three New formula function added to the package
                                        for   Federal W-2 Magnetic Media MultiThread process
  13-DEC-2006   ppanda  115.36          Function modified b  adding additional parameters
  				        assignment_action_id, p_tax_jd_code,
                                        p_tax_unit_info1 and , p_tax_unit_info2
                                        This is to fix Bug # 5709609

  10-JAN-2007   ppanda  115.37          Function set_application_error modified by adding a
                                        new parameter assignment_Action_id
  =============================================================================*/
  ...
  */
        ...
        AND AA.tax_unit_id
                   = substr(ppa.legislative_parameters,
                            instr(ppa.legislative_parameters,
                                  'TRANSFER_GRE=') + length('TRANSFER_GRE='))
        AND hou.organization_id  = AA.tax_unit_id
        and nvl(pay_us_archive_util.get_archive_value(ppa.payroll_action_id,
                                                            'A_LC_FEDERAL_TAX_RULES_ORG_GOVERNMENT_EMPLOYER',
                                                             AA.tax_unit_id),'N') = 'Y'
     order by 8;          /* Bug # 8851771 */
  ...
  CURSOR mmrf_employee IS
     SELECT /*+ ORDERED */ 'ASSIGNMENT_ACTION_ID=C', AI.locked_action_id, -- YREND assignment action
  ...
  */
  ...
  FUNCTION get_tax_unit_info  (tax_unit_id	IN NUMBER		-- Context
