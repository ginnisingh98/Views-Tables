--------------------------------------------------------
--  DDL for Package OE_BULK_MSG_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."OE_BULK_MSG_PUB" AUTHID CURRENT_USER AS
/* $Header: OEBUMSGS.pls 120.0 2005/06/01 03:09:44 appldev noship $ */
...
TYPE T_TBL_V1   IS TABLE OF VARCHAR2(1);
TYPE T_TBL_V30  IS TABLE OF VARCHAR2(30);
TYPE T_TBL_V50  IS TABLE OF VARCHAR2(50);
TYPE T_TBL_V2000  IS TABLE OF VARCHAR2(2000);
TYPE T_TBL_NUM  IS TABLE OF NUMBER;

-- API message record type
   TYPE G_MSG_REC_TYPE IS RECORD
   ( MESSAGE          		  T_TBL_V2000
    ,ENTITY_CODE       		  T_TBL_V30
    ,ENTITY_ID         		  T_TBL_NUM
    ,HEADER_ID         		  T_TBL_NUM
    ,LINE_ID           		  T_TBL_NUM
    ,ORDER_SOURCE_ID          T_TBL_NUM
    ,ORIG_SYS_DOCUMENT_REF	  T_TBL_V50
    ,ORIG_SYS_DOCUMENT_LINE_REF   T_TBL_V50
    ,SOURCE_DOCUMENT_TYPE_ID      T_TBL_NUM
    ,SOURCE_DOCUMENT_ID		  T_TBL_NUM
    ,SOURCE_DOCUMENT_LINE_ID  T_TBL_NUM
    ,ATTRIBUTE_CODE       	  T_TBL_V30
    ,CONSTRAINT_ID		      T_TBL_NUM
    ,PROCESS_ACTIVITY		  T_TBL_NUM
    ,NOTIFICATION_FLAG        T_TBL_V1
    ,MESSAGE_TEXT             T_TBL_V2000
    ,TYPE                 	  T_TBL_V30
    ,ENTITY_REF       		  T_TBL_V50
    ,ORIG_SYS_SHIPMENT_REF    T_TBL_V50
    ,CHANGE_SEQUENCE   	  	  T_TBL_V50
   );
...
(   p_encoded		    IN	VARCHAR2    := FND_API.G_TRUE	    ,
    ...
    p_data		    OUT NOCOPY /* file.sql.39 change */ VARCHAR2
);
...
(   p_msg_index     IN  NUMBER      := G_NEXT           ,
    p_encoded       IN  VARCHAR2    := FND_API.G_TRUE   ,
    p_data          OUT NOCOPY /* file.sql.39 change */ VARCHAR2 ,
    p_msg_index_out OUT NOCOPY /* file.sql.39 change */ NUMBER
);
...
-- message stack and inserts them into the OE_PROCESSING_MSGS table.
