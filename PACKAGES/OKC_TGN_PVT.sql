--------------------------------------------------------
--  DDL for Package OKC_TGN_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."OKC_TGN_PVT" AUTHID CURRENT_USER AS
/* $Header: OKCSTGNS.pls 120.0 2005/05/25 19:09:33 appldev noship $ */
  ---------------------------------------------------------------------------
...
/*
  ...
  TYPE okc_timevalues_tl_tbl_type IS TABLE OF okc_timevalues_tl_rec_type
        INDEX BY BINARY_INTEGER;
*/
  TYPE tgnv_rec_type IS RECORD (
    id                             NUMBER := OKC_API.G_MISS_NUM,
    object_version_number          NUMBER := OKC_API.G_MISS_NUM,
--Bug 3122962
    sfwt_flag                      OKC_TIME_TPG_NAMED_V.SFWT_FLAG%TYPE := 'N',
    cnh_id                         NUMBER := OKC_API.G_MISS_NUM,
    tve_id_limited                 NUMBER := OKC_API.G_MISS_NUM,
    dnz_chr_id                 NUMBER := OKC_API.G_MISS_NUM,
    tze_id                 NUMBER := OKC_API.G_MISS_NUM,
    description                    OKC_TIME_TPG_NAMED_V.DESCRIPTION%TYPE := OKC_API.G_MISS_CHAR,
    short_description              OKC_TIME_TPG_NAMED_V.SHORT_DESCRIPTION%TYPE := OKC_API.G_MISS_CHAR,
    comments                       OKC_TIME_TPG_NAMED_V.COMMENTS%TYPE := OKC_API.G_MISS_CHAR,
    attribute_category             OKC_TIME_TPG_NAMED_V.ATTRIBUTE_CATEGORY%TYPE := OKC_API.G_MISS_CHAR,
    attribute1                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE1%TYPE := OKC_API.G_MISS_CHAR,
    attribute2                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE2%TYPE := OKC_API.G_MISS_CHAR,
    attribute3                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE3%TYPE := OKC_API.G_MISS_CHAR,
    attribute4                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE4%TYPE := OKC_API.G_MISS_CHAR,
    attribute5                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE5%TYPE := OKC_API.G_MISS_CHAR,
    attribute6                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE6%TYPE := OKC_API.G_MISS_CHAR,
    attribute7                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE7%TYPE := OKC_API.G_MISS_CHAR,
    attribute8                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE8%TYPE := OKC_API.G_MISS_CHAR,
    attribute9                     OKC_TIME_TPG_NAMED_V.ATTRIBUTE9%TYPE := OKC_API.G_MISS_CHAR,
    attribute10                    OKC_TIME_TPG_NAMED_V.ATTRIBUTE10%TYPE := OKC_API.G_MISS_CHAR,
    attribute11                    OKC_TIME_TPG_NAMED_V.ATTRIBUTE11%TYPE := OKC_API.G_MISS_CHAR,
    attribute12                    OKC_TIME_TPG_NAMED_V.ATTRIBUTE12%TYPE := OKC_API.G_MISS_CHAR,
    attribute13                    OKC_TIME_TPG_NAMED_V.ATTRIBUTE13%TYPE := OKC_API.G_MISS_CHAR,
    attribute14                    OKC_TIME_TPG_NAMED_V.ATTRIBUTE14%TYPE := OKC_API.G_MISS_CHAR,
    attribute15                    OKC_TIME_TPG_NAMED_V.ATTRIBUTE15%TYPE := OKC_API.G_MISS_CHAR,
    created_by                     NUMBER := OKC_API.G_MISS_NUM,
    creation_date                  OKC_TIME_TPG_NAMED_V.CREATION_DATE%TYPE := OKC_API.G_MISS_DATE,
    last_updated_by                NUMBER := OKC_API.G_MISS_NUM,
    last_update_date               OKC_TIME_TPG_NAMED_V.LAST_UPDATE_DATE%TYPE := OKC_API.G_MISS_DATE,
    last_update_login              NUMBER := OKC_API.G_MISS_NUM);
  g_miss_tgnv_rec                         tgnv_rec_type;
  TYPE tgnv_tbl_type IS TABLE OF tgnv_rec_type
        INDEX BY BINARY_INTEGER;
  ---------------------------------------------------------------------------
  ...
  -- GLOBAL VARIABLES
  ---------------------------------------------------------------------------
  G_PKG_NAME			CONSTANT VARCHAR2(200) := 'OKC_TGN_PVT';
