--------------------------------------------------------
--  DDL for Package HR_AE_PERSON_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."HR_AE_PERSON_API" AUTHID CURRENT_USER as
/* $Header: peperaei.pkh 120.5.12010000.2 2008/08/06 09:25:13 ubhat ship $ */
/*#
 * This package contains Update person APIs.
 * @rep:scope public
 * @rep:product PER
 * @rep:displayname Update person for UAE
*/
--
-- ----------------------------------------------------------------------------
-- |-------------------< update_ae_person >------------------------------------|
 ...
 *
 * Note: The business group must have the AE legislation code.
 *
 * <p><b>Licensing</b><br>
 ...
 * Record will be updated for the Person.
 *
 ...
 * then the person will be updated.
 * @param p_effective_date The effective date of the change.
 * @param p_datetrack_update_mode Update mode.
 * @param p_person_id ID of the person.
 * @param p_person_type_id Person type ID. Person type changes are separately
 * maintained in per_person_type_usages_f table. So composite person type is not
 * allowed as input unless it is same as the type the person is holding on
 * p_effective_date.
 ...
 * date.
 * @param p_uses_tobacco_flag Uses tobacco flag.
 ...
 * @param p_place_of_birth Dependent on voluntary service Town or city of birth.
 ...
 * @param p_object_version_number If p_validate is false, set to the new version number of the
 * updated person record. If p_validate is true set to the same value you passed in.
 * @param p_employee_number If p_validate is false, set to the value of the employee number
 * after the person record has been updated. If p_validate is true, set to  the same value
 * you passed in. This parameter depends on the employee number generation method of the business group.
 * @param p_effective_start_date If p_validate is false, set to the effective start date of the
 * person. If p_validate is true, set to null.
 * @param p_effective_end_date If p_validate is false, set to the effective end date of the person.
 * If p_validate is true, set to null.
 * @param p_full_name If p_validate is false, set to the complete full name of the person.
 * If p_validate is true, set to null.
 * @param p_comment_id If p_validate is false and any comment text exists, set to the id
 * of the corresponding person comment row. If p_validate is true, or no comment text
 * exists this will be null.
 * @param p_name_combination_warning Set to true if the new combination (if changed) of last name,
 * first name and date of birth already existed prior to the update. Else, set to false.
 * @param p_assign_payroll_warning Set to true if the date of birth has been updated to a null value,
 * and this person is an employee, otherwise set to false.
 * @param p_orig_hire_warning Set to true if the original date of hire is not null and the
...
procedure update_ae_person
  ...
  ,p_mother_name                  in      varchar2 default hr_api.g_varchar2
  ...
  ,p_alt_mother_name           	  in      varchar2 default hr_api.g_varchar2
  ...
  ,p_alt_place_of_birth           in      varchar2 default hr_api.g_varchar2
  ,p_date_of_change               in      varchar2 default hr_api.g_varchar2
  ,p_reason_for_change            in      varchar2 default hr_api.g_varchar2
