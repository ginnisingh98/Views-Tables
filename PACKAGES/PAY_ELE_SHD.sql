--------------------------------------------------------
--  DDL for Package PAY_ELE_SHD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PAY_ELE_SHD" AUTHID CURRENT_USER as
...
Type g_rec_type Is Record
  ...
  ,subpriority                     number(9)         -- Increased length
  ,personal_payment_method_id      number(9)
  ,date_earned                     date
  ,object_version_number           number(9)
  ,source_id                       number(15)
  ,balance_adj_cost_flag           varchar2(9)       -- Increased length
  ,element_type_id                 number(9)
  ,all_entry_values_null           varchar2(30)
  );


-- Used when mimicing an element entry value hook package
-- eg get cache of old values in to a local table structure
Type g_val_rec_type Is Record
...
Type g_old_val_tab_type is Table of g_val_rec_type index by binary_integer;


  -- Cont Calc Changes
  cursor csr_get_eevals(cp_element_entry_id number,
                        cp_eff_date date) is
    select
   ...
   order by element_entry_value_id;
...
--           p_base_key_value = :element_entry_id).
--
-- Post Success:
--   Processing continues.
--
