--------------------------------------------------------
--  DDL for Package PAY_IE_NES_REPORT_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PAY_IE_NES_REPORT_PKG" AUTHID CURRENT_USER as
/* $Header: pyienes.pkh 120.0.12010000.1 2009/07/29 10:42:42 knadhan noship $ */
level_cnt number;
...
g_send_emp varchar2(50);
g_comments varchar2(300);

g_qtr_start_date date;
g_qtr_end_date date;
g_reference_start_date date;
g_reference_end_date date;
...
CURSOR  c_body IS
SELECT 'TRANSFER_ACT_ID=P', ptoa.assignment_action_id
FROM    pay_assignment_actions ptoa
WHERE   ptoa.payroll_action_id = to_number(pay_magtape_generic.get_parameter_value
	     ('TRANSFER_PAYROLL_ACTION_ID'))
ORDER BY ptoa.assignment_action_id;
