--------------------------------------------------------
--  DDL for Package FA_FASRSVED_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."FA_FASRSVED_XMLP_PKG" AUTHID CURRENT_USER AS
/* $Header: FASRSVEDS.pls 120.0.12010000.1 2008/07/28 13:17:31 appldev ship $ */
	P_BOOK	varchar2(15);
	...
	RP_CTR_LPROMPT	varchar2(50);
	RP_PERIOD_FROM	varchar2(20);
	RP_PERIOD_TO	varchar2(20);
	...
	--function out_of_balanceformula(BEGIN in number, ADDITION in number, DEPRECIATION in number, RECLASS in number, RETIREMENT in number, ADJUST in number, TRANSFER in number, END in number) return varchar2  ;
	function out_of_balanceformula(CHANGED_BEGIN in number, ADDITION in number, DEPRECIATION in number, RECLASS in number, RETIREMENT in number, ADJUST in number, TRANSFER in number, CHANGED_END in number) return varchar2  ;
	function ctr_out_of_balanceformula(CTR_BEGIN in number, CTR_ADD in number, CTR_DEPRN in number, CTR_RECLASS in number, CTR_RETIRE in number, CTR_ADJUST in number, CTR_TRANS in number, CTR_END in number) return varchar2  ;
	...
	Function RP_PERIOD_FROM_p return varchar2;
	Function RP_PERIOD_TO_p return varchar2;
