--------------------------------------------------------
--  DDL for Package WMS_DEVICE_INTEGRATION_PUB
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."WMS_DEVICE_INTEGRATION_PUB" AUTHID CURRENT_USER AS
/* $Header: WMSDEVIS.pls 120.2 2005/08/10 19:43:01 htnguyen noship $ */
/*#
  * User defined device integration procedures allows user to process the WMS
  * request to warehouse automation devices.
  * @rep:scope public
  * @rep:product WMS
  * @rep:lifecycle active
  * @rep:displayname User defined Device Interfacing API for WMS
  * @rep:category BUSINESS_ENTITY WMS_DEVICE_INTEGRATION
  */
--
-- Name
...
------------------------------------------------------------------------
  /*#
  * This procedure SYNC_DEVICE_REQUEST is meant to initiate a request to a WMS Device.
  * It is expected that 3rd party device vendors would extend this
  * package to implement the actual logic required to interface with
  * the device.
  *    The primary source of input to this package would be the Temporary
  *    table WMS_DEVICE_REQUEST. Depending on the level of details desired,
  *    the records in this table will be joined with other tables.
  *
  *         MTL_SYSTEM_ITEMS           -  Item
  *         MTL_ITEM_LOCATIONS         -  Location
  *         MTL_PARAMETERS             -  Organization
  ...
  * program could then wait on this Pipe and pass the data to
  * the actual device.
  * @ param x_status_code Status of request. ( S = Success, E = Error)
  * @ paraminfo {@rep:required}
  * @ param x_status_msg Optional status message related to this specific request
  * @ paraminfo {@rep:required}
  * @ param x_device_status Optional status message related to the device
  * @ paraminfo {@rep:required}
  * @ param p_request_id Request Id generated by WMS
  * @ paraminfo {@rep:required}
  * @ param p_device_id Device ID of the device this request is initiated for
  * @ paraminfo {@rep:required}
  * @ param p_resubmit_flag N - Invoked in conjunction with processing transaction,Y - Invoked to resubmit a request to device
  * @ paraminfo {@rep:required}
  * @rep:scope public
  * @rep:lifecycle active
  * @rep:displayname Process WMS request to hardware devices
  */
  PROCEDURE SYNC_DEVICE_REQUEST(
      p_request_id            IN  NUMBER,
      p_device_id             IN  NUMBER,
...
--
  /*#
  * This procedure UPDATE_REQUEST is used to update device requests records
  * after device has processed records
  * @ param p_request_id Request id of the record
  * @ paraminfo {@rep:required}
  * @ param p_device_id Device ID associated with the request
  * @ paraminfo {@rep:optional}
  * @ param p_status_code Status after device had processed the record
  * @ paraminfo {@rep:required}
  * @ param p_status_msg Any message after device has processed the record
  * @ paraminfo {@rep:optional}
  * @rep:scope public
  * @rep:lifecycle active
  * @rep:displayname Update device history records
  */
...
--
   /*#
  * This procedure SYNC_DEVICE is used to invoke or terminate a WMS
  * Device requests.
  * It is expected that 3rd party device vendors would extend this
  * package to implement the actual logic required to interface with
  * the device.
  * @ param p_organization_id  organization id for the request
  * @ paraminfo {@rep:required}
  * @ param p_device_id Device ID associated with the request
  * @ paraminfo {@rep:optional}
  * @ param p_employee_id user identifier who is working (invokes or terminates) on device
  * @ paraminfo {@rep:required}
  * @ param p_sign_on_flag 'Y' when signing on,'N' when signing off
  * @ paraminfo {@rep:required}
  * @ param x_status_code Status of request. ( S = Success, E = Error)
  * @ paraminfo {@rep:required}
  * @ param x_device_status Optional status message related to the device
  ...
  * @rep:scope public
  * @rep:lifecycle active
  * @rep:displayname API to invoke or terminate a WMS Device requests
  */
  PROCEDURE SYNC_DEVICE(
