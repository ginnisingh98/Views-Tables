--------------------------------------------------------
--  DDL for Package FA_FASCOSTD_XMLP_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."FA_FASCOSTD_XMLP_PKG" AUTHID CURRENT_USER AS
/* $Header: FASCOSTDS.pls 120.0.12010000.1 2008/07/28 13:16:35 appldev ship $ */
	P_BOOK	varchar2(15);
	...
	function out_of_balanceformula(BEGIN_P in number, ADDITION in number, REVALUATION in number, RECLASS in number, RETIREMENT in number,
	ADJUSTMENT in number, TRANSFER in number, CAPITALIZATION in number, END_P in number) return varchar2  ;
	...
	CTR_ADJUST in number, CTR_TRANS in number, CTR_CAPITAL in number, CTR_END in number) return varchar2  ;
	...
	ACCT_ADJUST in number, ACCT_TRANS in number, ACCT_CAPITAL in number, ACCT_END in number) return varchar2  ;
	...
	BAL_ADJUST in number, BAL_TRANS in number, BAL_CAPITAL in number, BAL_END in number) return varchar2  ;
	...
	RP_ADJUST in number, RP_TRANS in number, RP_CAPITAL in number, RP_END in number) return varchar2  ;
	...
	Function P_PERIOD_FROM_p return varchar2;
	Function P_PERIOD_TO_p return varchar2;
...
procedure Insert_Info(Book		in	varchar2,
