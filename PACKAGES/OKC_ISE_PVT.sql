--------------------------------------------------------
--  DDL for Package OKC_ISE_PVT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."OKC_ISE_PVT" AUTHID CURRENT_USER AS
/* $Header: OKCSISES.pls 120.0 2005/05/25 18:51:44 appldev noship $ */
  ---------------------------------------------------------------------------
...
/*  TYPE okc_timevalues_tl_rec_type IS RECORD (
  ...
  TYPE okc_timevalues_tl_tbl_type IS TABLE OF okc_timevalues_tl_rec_type
        INDEX BY BINARY_INTEGER;
*/
  TYPE isev_rec_type IS RECORD (
    id                             NUMBER := OKC_API.G_MISS_NUM,
    object_version_number          NUMBER := OKC_API.G_MISS_NUM,
--Bug 3122962 swft_flag
    ...
    last_update_login              NUMBER := OKC_API.G_MISS_NUM);
  g_miss_isev_rec                         isev_rec_type;
  TYPE isev_tbl_type IS TABLE OF isev_rec_type
        INDEX BY BINARY_INTEGER;
  ---------------------------------------------------------------------------
  ...
  -- GLOBAL VARIABLES
  ---------------------------------------------------------------------------
  G_PKG_NAME			CONSTANT VARCHAR2(200) := 'OKC_ISE_PVT';
  ...
  PROCEDURE insert_row(
    ...
    x_isev_tbl                     OUT NOCOPY isev_tbl_type);
