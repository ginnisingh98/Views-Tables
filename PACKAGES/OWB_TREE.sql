--------------------------------------------------------
--  DDL for Package OWB_TREE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."OWB_TREE" AUTHID CURRENT_USER AS
/* $Header: MRPOSTRS.pls 120.0 2005/05/25 03:50:29 appldev noship $ */
...
LOOKUPS_COUNT                   CONSTANT NUMBER := 27;
...
TYPE char80_var_arr IS varray(27) OF VARCHAR2(80);

lookups char80_var_arr;

TYPE nodeRec IS RECORD (
			tree_type             number_arr := number_arr(),
			parent_node_type      number_arr := number_arr(),
			state                 number_arr := number_arr(),
			depth                 number_arr := number_arr(),
			label                 char240_arr := char240_arr(),
			icon                  char80_arr := char80_arr(),
			data                  char80_arr := char80_arr()
			);
