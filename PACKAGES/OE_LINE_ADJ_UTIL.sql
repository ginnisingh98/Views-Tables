--------------------------------------------------------
--  DDL for Package OE_LINE_ADJ_UTIL
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."OE_LINE_ADJ_UTIL" AUTHID CURRENT_USER AS
/* $Header: OEXULADS.pls 120.1.12010000.2 2009/06/26 10:00:47 nitagarw ship $ */
...
-- eBTax Changes
G_TAX_RATE                      CONSTANT NUMBER := 110;

--bug 2965218
G_SEND_ALL_LINES_FOR_DSP        VARCHAR2(1);
G_GROUP_PRICING_DSP             VARCHAR2(1);
G_CODE_RELEASE_LEVEL            VARCHAR2(10);
G_OPM_ITEM_CATCHWEIGHT_USED     VARCHAR2(1);



FUNCTION G_MISS_OE_AK_LINE_ADJ_REC
RETURN OE_AK_LINE_PRCADJS_V%ROWTYPE;
...
Procedure Delete_Prg_Lines(p_line_id In Number, p_operation In Varchar2);

Procedure Check_Canceled_PRG(p_old_line_rec in Oe_Order_Pub.line_rec_type,
                             p_new_line_rec in Oe_Order_Pub.line_rec_type);
...
--G_CHANGED_LINE_TBL OE_ORDER_ADJ_PVT.Index_TBl_Type;
--bug 3020702
TYPE G_CHANGED_LINE_REC1 IS RECORD
(line_id number, header_id number);
TYPE G_CHANGED_LINE_TBL1 IS TABLE OF G_CHANGED_LINE_REC1
INDEX by BINARY_INTEGER;
G_CHANGED_LINE_TBL G_CHANGED_LINE_TBL1;
--bug 3020702


G_PRG_TBL OE_GLOBALS.Number_Tbl_Type;
...
Procedure Set_PRG_Cache(p_header_id IN NUMBER);
Function Is_PRG_LINE(p_line_id IN NUMBER) RETURN BOOLEAN;
Procedure Reset_PRG_Cache;
Function Is_Pricing_Related_Change(p_new_line_rec In Oe_Order_Pub.Line_Rec_Type,
                                   p_old_line_rec   In Oe_Order_Pub.Line_Rec_Type) Return Boolean;
