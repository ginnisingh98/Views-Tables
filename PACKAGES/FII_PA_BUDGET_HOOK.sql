--------------------------------------------------------
--  DDL for Package FII_PA_BUDGET_HOOK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."FII_PA_BUDGET_HOOK" AUTHID CURRENT_USER as
/* $Header: FIIPA14S.pls 120.0 2002/08/24 05:00:22 appldev noship $ */

  --
  -- Package FII_PA_BUDGET_HOOK implements collection hooks for
  -- EDW Project Budget. Collection hooks like FII_PA_COST_HOOK are
  -- used by EDW collection engine to pass control to
  -- product specific code at different stages of collection process.
  --
  -- When FII_PA_BUDGET_HOOK finishes execution, control is
  ...
  -- Function PRE_FACT_COLL is Project Budget fact pre-collection hook.
  -- The function implements "denormalized" Task Owning organization
  -- foreign key in facts (PROJECT_ORG_FK_KEY).
  --
  -- The function does the following:
  --
  -- 1. For all new rows in the FII_PA_BUDGET_FSTG staging table it
  ...
  --
  -- 2. Update PROJECT_ORG_FK_KEY for all records in the FII_PA_BUDGET_F
