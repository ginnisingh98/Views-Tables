--------------------------------------------------------
--  DDL for Package PAY_NO_EMP_CONT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "APPS"."PAY_NO_EMP_CONT" AUTHID CURRENT_USER AS
/* $Header: pynoempcont.pkh 120.5 2006/05/26 07:03:48 pgopal noship $ */
...
--------------------------------Cursor Definitions ----------------------------------------------------------
	...
	/* cursor csr_payroll_action_id (p_payroll_id  pay_all_payrolls_f.PAYROLL_ID%type ,p_date_earned  DATE ) is
	...
	and  ppa.effective_date between trunc(Add_months(p_date_earned,MOD(TO_NUMBER(TO_CHAR(p_date_earned,'MM')),2)-1),'MM')
        ...
        --Bug Fix 4465274, modified the cursor
        /*
	cursor csr_payroll_action_id (p_date_earned  DATE ) is
	select ppa.PAYROLL_ACTION_ID
	from pay_payroll_actions	ppa
	    ,pay_run_types_f		prt
	where prt.LEGISLATION_CODE = 'NO'
	and   prt.RUN_TYPE_NAME <> 'Employer Contributions'
	and   prt.RUN_TYPE_ID	=  ppa.RUN_TYPE_ID
	and   p_date_earned between prt.EFFECTIVE_START_DATE and prt.EFFECTIVE_END_DATE
	and  ppa.effective_date between trunc(Add_months(p_date_earned,MOD(TO_NUMBER(TO_CHAR(p_date_earned,'MM')),2)-1),'MM')
	...
	*/
        ...
        --Bug Fix 5042861, modified the cursor for performance fix to remove TABLE ACCESS FULL (SQL ID:16539845)
	...
	and   pet.element_name = 'Tax Deduction Base'
	...
	*/
	...
	/* Bug Fix: 5042861 - Modified cursor csr_lu_mu for performance fix to remove MERGE JOIN CARTESIAN*/
	cursor csr_lu_mu (l_assignment_id   per_all_assignments_f.ASSIGNMENT_ID%type
	                 ,l_assignment_action_id   pay_assignment_actions.ASSIGNMENT_ACTION_ID   %type
			 ,p_date_earned  DATE
...
FUNCTION populate_tables
  ...
  ,g_mu_tab  		IN OUT 	NOCOPY PAY_NO_EMP_CONT.g_mu_tabtype ) RETURN NUMBER ;
...
FUNCTION lookup_cell
  ...
  ,l_zone      IN  NUMBER ) RETURN NUMBER ;
...
FUNCTION ec_main_calculation
  ...
  ,l_curr_under_62			IN VARCHAR2 ) RETURN NUMBER ;
...
FUNCTION display_table_calc
  (g_tab_calc  IN  PAY_NO_EMP_CONT.g_tab_calc_tabtype ) RETURN NUMBER ;
